...
class ImgDiagDumper {
 ...
 private:
  ...
  void DiffObjectContents(mirror::Object* obj,
    ...
    if (obj->IsClass()) {
      os << tabs << "Class " << Class::PrettyClass(obj->AsClass()) << " " << obj << "\n";
    } else {
      os << tabs << "Instance of " << Class::PrettyClass(klass) << " " << obj << "\n";
    }
  ...
  bool DumpImageDiffMap(pid_t image_diff_pid,
    ...
    for (const auto& vk_pair : clean_object_class_values) {
      os << "    " << Class::PrettyClass(vk_pair.second) << " (" << vk_pair.first << ")\n";
    }
