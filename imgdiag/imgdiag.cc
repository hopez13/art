...
class ImgDiagDumper {
 public:
  ...
  bool Init() {
    ...
    auto open_proc_maps = [&os](pid_t pid, /*out*/ std::vector<android::procinfo::MapInfo>* proc_maps) {
      // Open /proc/<pid>/maps to view memory maps.
      if (android::procinfo::ReadProcessMaps(pid, proc_maps)) {
        os << "Could not read maps for " << pid;
        return false;
      }
      return true;
    };
 ...
 private:
  ...
  bool DumpImageDiffMap(const ImageHeader& image_header, const std::string& image_location)
    ...
    auto find_boot_map = [&os, &image_location_base_name](const std::vector<android::procinfo::MapInfo>& maps, const char* tag)
        -> std::optional<android::procinfo::MapInfo> {
      // Find the memory map for the current boot image component.
      for (const android::procinfo::MapInfo& mapinfo : maps) {
        // The map name ends with ']' if it's an anonymous memmap. We need to special case that
        // to find the boot image map in some cases.
        if (EndsWith(mapinfo.name, image_location_base_name) ||
            EndsWith(mapinfo.name, image_location_base_name + "]")) {
          if ((mapinfo.flags & PROT_WRITE) != 0) {
            return mapinfo;
          }
    ...
    std::optional<android::procinfo::MapInfo> maybe_boot_map = find_boot_map(image_proc_maps_, "image");
    if (!maybe_boot_map) {
      return false;
    }
