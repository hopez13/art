...
class ImgDiagDumper {
 public:
  ...
  bool Init() {
    ...
    std::unique_ptr<std::vector<uint8_t>> tmp_remote_contents(
        new std::vector<uint8_t>(boot_map_size_));
    ...
    std::unique_ptr<std::vector<uint8_t>> tmp_zygote_contents;
    if (zygote_diff_pid_ != -1) {
      ...
      tmp_zygote_contents.reset(new std::vector<uint8_t>(boot_map_size_));
      if (!zygote_map_file->PreadFully(&(*tmp_zygote_contents)[0], boot_map_size_,
                                       boot_map_.start)) {
    ...
    auto tmp_page_map_file =
        std::unique_ptr<File>(OS::OpenFileForReading(page_map_file_name.c_str()));
    if (tmp_page_map_file == nullptr) {
    ...
    page_map_file_ = std::move(tmp_page_map_file);
    clean_pagemap_file_ = std::move(tmp_clean_pagemap_file);
    kpageflags_file_ = std::move(tmp_kpageflags_file);
    kpagecount_file_ = std::move(tmp_kpagecount_file);
 ...
 private:
  ...
  bool ComputeDirtyBytes(const uint8_t* image_begin,
                         size_t* dirty_pages,
                         size_t* different_pages,
                         size_t* different_bytes,
                         size_t* different_int32s,
    ...
    for (uintptr_t begin = boot_map_.start; begin != boot_map_.end; ++begin) {
      ...
      if (previous_page_idx != page_idx) {
        ...
        int dirtiness = (IsPageDirty(page_map_file_.get(),       // Image-diff-pid procmap
                                     clean_pagemap_file_.get(),  // Self procmap
                                     kpageflags_file_.get(),
                                     kpagecount_file_.get(),
  ...
  void ComputeObjectDirty(const uint8_t* current,
                          ...
                          size_t obj_size,
                          std::set<size_t>* dirty_page_set_local,
                          size_t* different_objects,
                          size_t* dirty_object_bytes,
                          size_t* false_dirty_object_bytes,
                          ClassData* obj_class_data,
                          std::set<const uint8_t*>* image_dirty_objects,
                          std::set<const uint8_t*>* zygote_dirty_objects,
                          std::map<off_t /* field offset */, int /* count */>* field_dirty_count,
                          std::vector<const uint8_t*>* dirty_objects,
                          std::vector<const uint8_t*>* false_dirty_objects) {
    ...
    if (different_image_object) {
      ...
      if (different_zygote_object) {
        ...
        zygote_dirty_objects->insert(current);
      } else {
        // Just different from image.
        image_dirty_objects->insert(current);
      }
    ...
    if (different_image_object) {
      if (dirty_objects != nullptr) {
        ...
        dirty_objects->push_back(current);
      }
      /*
      ...
      */
    } else if (ItemIsOnDirtyPage(current, obj_size, dirty_page_set_local)) {
      ...
      false_dirty_objects->push_back(current);
      obj_class_data->false_dirty_objects.push_back(current);
      false_dirty_object_bytes += obj_size;
  ...
  bool DumpImageDiffMap()
    ...
    while (reinterpret_cast<uintptr_t>(current) < reinterpret_cast<uintptr_t>(end_image_ptr)) {
      ...
      std::map<off_t /* field offset */, int /* count */>* field_dirty_count = nullptr;
      if (klass->IsClassClass()) {
    ...
    if (!zygote_dirty_objects.empty()) {
      ...
      for (const uint8_t* obj_bytes : zygote_dirty_objects) {
    ...
    for (const uint8_t* obj_bytes : image_dirty_objects) {
      auto obj = const_cast<mirror::Object*>(reinterpret_cast<const mirror::Object*>(obj_bytes));
      ptrdiff_t offset = obj_bytes - begin_image_ptr;
