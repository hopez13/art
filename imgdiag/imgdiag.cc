 ...
 private:
  ...
  static bool GetPageFrameNumbers(File* page_map_file,
    ...
    if (!page_map_file->PreadFully(page_frame_numbers.data(),
      ...
      *error_msg = StringPrintf("Failed to read the virtual page index entries from %s, errno: %s",
  ...
  // Note: On failure, `page_flags[.]` shall be clobbered.
  static bool GetPageFlagsOrCounts(File* kpage_file,
    ...
    while (i != size) {
      ...
      if (!kpage_file->PreadFully(page_flags_or_counts.data() + start,
        ...
        *error_msg = StringPrintf("Failed to read the page flags or counts from %s, errno: %s",
