...
ParentMap CalculateParentMap(const std::vector<const ImageHeader*>& image_headers)
  ...
  auto process_object_fields = [&parent_map, &next](mirror::Object* parent_obj) REQUIRES_SHARED(
                                   Locks::mutator_lock_) {
  ...
  auto process_array_elements = [&parent_map, &next](mirror::Object* parent_obj) REQUIRES_SHARED(
                                    Locks::mutator_lock_) {
    CHECK(parent_obj->IsObjectArray());
