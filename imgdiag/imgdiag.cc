...
enum RemoteProcesses {
  kRemoteProcessesImageOnly,
  kRemoteProcessesZygoteOnly,
  kRemoteProcessesImageAndZygote,
};

struct MappingData {
  // The count of pages that are
...
static bool ObjectsDiffer(T* obj1, T* obj2) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  return memcmp(obj1, obj2, obj_size) != 0;
}
...
struct RegionCommon {
 public:
  RegionCommon(std::ostream* os,
    ...
    different_objects_(0),
    dirty_object_bytes_(0),
    false_dirty_object_bytes_(0) { }

  void DumpSamplesAndOffsetCount() {
    os_ << "      sample object addresses: ";
    for (size_t i = 0; i < dirty_objects_.size() && i < kMaxAddressPrint; ++i) {
      auto art_method = dirty_objects_[i];
      os_ << reinterpret_cast<void*>(art_method) << ", ";
    }
    os_ << "\n";
    os_ << "      dirty byte +offset:count list = ";
    auto field_dirty_count_sorted =
        SortByValueDesc<off_t, int, size_t>(field_dirty_count_);
    for (auto pair : field_dirty_count_sorted) {
  ...
  size_t GetDifferentObjectCount() const { return different_objects_; }
  size_t GetDirtyObjectBytes() const { return dirty_object_bytes_; }
  size_t GetFalseDirtyObjectCount() const { return false_dirty_objects_.size(); }
  size_t GetFalseDirtyObjectBytes() const { return false_dirty_object_bytes_; }
  size_t GetZygoteDirtyObjectCount() const { return zygote_dirty_objects_.size(); }

 protected:
  bool ObjectIsOnDirtyPage(T* object, const std::set<size_t>& dirty_pages)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  void AddZygoteDirtyObject(T* object) REQUIRES_SHARED(Locks::mutator_lock_) {
    zygote_dirty_objects_.insert(object);
  }

  void AddImageDirtyObject(T* object) REQUIRES_SHARED(Locks::mutator_lock_) {
    image_dirty_objects_.insert(object);
  }

  void AddFalseDirtyObject(T* object) REQUIRES_SHARED(Locks::mutator_lock_) {
    false_dirty_objects_.push_back(object);
    false_dirty_object_bytes_ += ObjectSize(object);
  }
  ...
  size_t different_objects_;

  // Local objects that are dirty (differ in at least one byte).
  size_t dirty_object_bytes_;
  std::vector<T*> dirty_objects_;

  // Local objects that are clean, but located on dirty pages.
  size_t false_dirty_object_bytes_;
  std::vector<T*> false_dirty_objects_;
  ...
  std::set<T*> image_dirty_objects_;
  ...
  std::set<T*> zygote_dirty_objects_;
...
class RegionSpecializedBase<mirror::Object> : public RegionCommon<mirror::Object> {
 public:
  ...
  void CheckObjectSanity(const uint8_t* current)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  const uint8_t* AdvanceToNextObject(const uint8_t* current)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    mirror::Object* object = reinterpret_cast<mirror::Object*>(const_cast<uint8_t*>(current));
    return current + RoundUp(ObjectSize(object), kObjectAlignment);
  }

  void VisitObject(mirror::Object* object)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  void AddCleanObject(mirror::Object* object)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    class_data_[object->GetClass()].AddCleanObject();
  }

  void AddFalseDirtyObject(mirror::Object* object)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    RegionCommon<mirror::Object>::AddFalseDirtyObject(object);
    class_data_[object->GetClass()].AddFalseDirtyObject(object);
  }
  ...
  void AddDirtyObject(mirror::Object* object, mirror::Object* object_remote)
    ...
    if (klass->IsClassClass()) {
      ...
      dirty_objects_.push_back(object);
    }
    class_data_[klass].AddDirtyObject(object, object_remote);
  }

  void DiffObjectContents(mirror::Object* obj, uint8_t* remote_bytes)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    const char* tabs = "    ";
    // Attempt to find fields for all dirty bytes.
    mirror::Class* klass = obj->GetClass();
    if (obj->IsClass()) {
      os_ << tabs << "Class " << mirror::Class::PrettyClass(obj->AsClass()) << " " << obj << "\n";
    } else {
      os_ << tabs << "Instance of " << mirror::Class::PrettyClass(klass) << " " << obj << "\n";
    }
  ...
  void DumpDirtyObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    auto dirty_object_class_values =
        SortByValueDesc<mirror::Class*, int, ClassData>(
            class_data_,
    ...
    for (const auto& vk_pair : dirty_object_class_values) {
      ...
      ClassData& obj_class_data = class_data_[klass];
      int object_sizes = obj_class_data.dirty_object_size_in_bytes;
      float avg_dirty_bytes_per_class =
          obj_class_data.dirty_object_byte_count * 1.0f / object_sizes;
      float avg_object_size = object_sizes * 1.0f / dirty_object_count;
      const std::string& descriptor = obj_class_data.descriptor;
      ...
      if (strcmp(descriptor.c_str(), "Ljava/lang/Class;") == 0) {
        ...
        os_ << "      field contents:\n";
        for (mirror::Object* obj : obj_class_data.dirty_objects) {
          // remote class object
          ...
          os_ << "        " << reinterpret_cast<const void*>(obj) << " ";
  ...
  void DumpFalseDirtyObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    for (const auto& vk_pair : false_dirty_object_class_values) {
      ...
      mirror::Class* klass = vk_pair.second;
      ClassData& obj_class_data = class_data_[klass];
      int object_sizes = obj_class_data.false_dirty_byte_count;
  ...
  void DumpCleanObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    // vector of pairs (int count, Class*)
 ...
 private:
  ...
  struct ClassData {
   ...
   private:
    ...
    static size_t CountDirtyBytes(mirror::Object* obj1, mirror::Object* obj2)
        REQUIRES_SHARED(Locks::mutator_lock_) {
...
class RegionSpecializedBase<ArtMethod> : RegionCommon<ArtMethod> {
 public:
  ...
  void CheckObjectSanity(const uint8_t* current ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  }

  const uint8_t* AdvanceToNextObject(
      const uint8_t* current ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    return nullptr;
  }

  void VisitObject(ArtMethod* object ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  }

  void AddFalseDirtyObject(ArtMethod* object)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    RegionCommon<ArtMethod>::AddFalseDirtyObject(object);
  }

  void AddCleanObject(ArtMethod* object ATTRIBUTE_UNUSED) {
  }

  void AddDirtyObject(ArtMethod* object, ArtMethod* object_remote)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    size_t obj_size = ObjectSize(object);
    ++different_objects_;
    dirty_object_bytes_ += obj_size;
    ...
    const uint8_t* current = reinterpret_cast<const uint8_t*>(object);
    const uint8_t* current_remote = reinterpret_cast<const uint8_t*>(object_remote);
    // ArtMethods always log their dirty count and objects.
    for (size_t i = 0; i < obj_size; ++i) {
      if (current[i] != current_remote[i]) {
        field_dirty_count_[i]++;
      }
  ...
  void DiffObjectContents(ArtMethod* obj ATTRIBUTE_UNUSED, uint8_t* remote_bytes ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  }
  ...
  void DumpDirtyObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    DumpSamplesAndOffsetCount();
    os << "      field contents:\n";
    const auto& dirty_objects_list = region_data->dirty_objects;
    for (mirror::Object* object : dirty_objects_list) {
      // remote method
      auto art_method = reinterpret_cast<ArtMethod*>(object);
      // remote class
      ...
      mirror::Class* declaring_class =
        RemoteContentsPointerToLocal(remote_declaring_class,
                                     *RegionCommon<ArtMethod>::remote_contents_,
                                     RegionCommon<ArtMethod::image_header);
      DumpOneArtMethod(art_method, declaring_class, remote_declaring_class, pointer_size);
    }
    */
  }
  ...
  void DumpFalseDirtyObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    /*
    auto& art_method_false_dirty_objects = obj_class_data.false_dirty_objects;
    os << "      field contents:\n";
    for (mirror::Object* obj : art_method_false_dirty_objects) {
      ...
      DumpOneArtMethod(art_method, declaring_class, nullptr, pointer_size);
    }
    */
  }
...
template <typename T>
class ObjectRegionData : public RegionSpecializedBase<T> {
 public:
  ObjectRegionData(std::ostream* os,
                   std::vector<uint8_t>* remote_contents,
                   std::vector<uint8_t>* zygote_contents,
                   const backtrace_map_t& boot_map,
                   const ImageHeader& image_header) :
  ...
  void ProcessRegion(const MappingData& mapping_data,
    ...
    while (reinterpret_cast<uintptr_t>(current) < reinterpret_cast<uintptr_t>(end_image_ptr)) {
      ComputeObjectDirty(current, begin_image_ptr, mapping_data.dirty_page_set);

      current = RegionSpecializedBase<T>::AdvanceToNextObject(current);
    }
    ...
    float true_dirtied_percent =
        RegionCommon<T>::GetDirtyObjectBytes() * 1.0f / (mapping_data.dirty_pages * kPageSize);

    // Object specific statistics.
    os_ << RegionCommon<T>::GetDifferentObjectCount() << " different objects, \n  "
        << RegionCommon<T>::GetDirtyObjectBytes() << " different object [bytes], \n  "
        << RegionCommon<T>::GetFalseDirtyObjectCount() << " false dirty objects,\n  "
        << RegionCommon<T>::GetFalseDirtyObjectBytes() << " false dirty object [bytes], \n  "
        << true_dirtied_percent << " different objects-vs-total in a dirty page;\n  "
        << "";

    if (RegionCommon<T>::GetZygoteDirtyObjectCount() != 0) {
      ...
      CHECK_EQ(remotes, kRemoteProcessesImageAndZygote);
      os_ << "\n" << "  Zygote dirty objects (probably shared dirty): ";
      DiffDirtyObjects(kProcessTypeZygote, begin_image_ptr, RegionCommon<T>::zygote_contents_);
    }
    os_ << "\n";
    switch (remotes) {
      case kRemoteProcessesZygoteOnly:
        os_ << "  Zygote shared dirty objects: ";
        break;
     case kRemoteProcessesImageAndZygote:
        os_ << "  Application dirty objects (private dirty): ";
        break;
     case kRemoteProcessesImageOnly:
        os_ << "  Application dirty objects (unknown whether private or shared dirty): ";
        break;
    }
    DiffDirtyObjects(kProcessTypeRemote, begin_image_ptr, RegionCommon<T>::remote_contents_);
    RegionSpecializedBase<T>::DumpDirtyObjects();
    RegionSpecializedBase<T>::DumpFalseDirtyObjects();
    RegionSpecializedBase<T>::DumpCleanObjects();
  }

 private:
  std::ostream& os_;

  void DiffDirtyObjects(ProcessType process_type,
                        const uint8_t* begin_image_ptr,
                        std::vector<uint8_t>* contents)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    os_ << RegionCommon<T>::dirty_objects_.size() << "\n";
    const std::set<T*>& objects =
        (process_type == kProcessTypeZygote) ?
            RegionCommon<T>::zygote_dirty_objects_:
            RegionCommon<T>::image_dirty_objects_;
    for (T* obj : objects) {
      ...
      RegionSpecializedBase<T>::DiffObjectContents(obj, remote_bytes);
    }
  }
  ...
  void ComputeObjectDirty(const uint8_t* current,
    ...
    T* object_zygote = reinterpret_cast<T*>(const_cast<uint8_t*>(current_zygote));
    // Visit and classify objects at the current location.
    mirror::Object* object = reinterpret_cast<mirror::Object*>(const_cast<uint8_t*>(current));
    RegionSpecializedBase<T>::VisitObject(object);
    ...
    if (different_image_object) {
      ...
      if (object_zygote != nullptr) {
        different_zygote_object = ObjectsDiffer(object, object_zygote);
      }
      if (different_zygote_object) {
        // Different from zygote.
        RegionCommon<T>::AddZygoteDirtyObject(object);
        RegionSpecializedBase<T>::AddDirtyObject(object, object_remote);
      } else {
        // Just different from image.
        RegionCommon<T>::AddImageDirtyObject(object);
        RegionSpecializedBase<T>::AddDirtyObject(object, object_remote);
      }
    } else {
      RegionSpecializedBase<T>::AddCleanObject(object);
    }
    if (!different_image_object && RegionCommon<T>::ObjectIsOnDirtyPage(object, dirty_pages)) {
      ...
      RegionSpecializedBase<T>::AddFalseDirtyObject(object);
    }
  }

  DISALLOW_COPY_AND_ASSIGN(ObjectRegionData);
};
...
class ImgDiagDumper {
 ...
 private:
  ...
  bool DumpImageDiffMap()
    ...
    ObjectRegionData<mirror::Object> object_region_data(os_,
                                                        &remote_contents_,
                                                        &zygote_contents_,
                                                        boot_map_,
                                                        image_header_);
    ...
    RemoteProcesses remotes;
    if (zygote_pid_only_) {
      remotes = kRemoteProcessesZygoteOnly;
    } else if (zygote_diff_pid_ > 0) {
      remotes = kRemoteProcessesImageAndZygote;
    } else {
      remotes = kRemoteProcessesImageOnly;
    }
