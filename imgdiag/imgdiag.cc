...
class ReferenceFieldVisitor {
 public:
  ...
  void operator()(ObjPtr<mirror::Object> obj, MemberOffset offset, bool is_static) const
    ...
    // Skip fields that point to null.
    ...
    // Skip references without an ArtField (ObjectArray elements).
    if (field == nullptr) {
      return;
    }
...
std::string GetPathFromClass(mirror::Object* obj,
                             const std::unordered_map<mirror::Object*, ParentInfo>& obj_parents)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  auto parent_info_it = obj_parents.find(obj);
  std::string path;
  while (parent_info_it != obj_parents.end() && parent_info_it->second.parent != nullptr) {
    const ParentInfo& parent_info = parent_info_it->second;
    path = ART_FORMAT(".{}{}", parent_info.path, path);
    parent_info_it = obj_parents.find(parent_info.parent);
  }

  if (parent_info_it == obj_parents.end()) {
...
std::unordered_map<mirror::Object*, ParentInfo> CalculateParentMap(const ImageHeader& image_header,
                                                                   uint8_t* base,
                                                                   PointerSize pointer_size)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  std::unordered_map<mirror::Object*, ParentInfo> parent_map;
  std::vector<mirror::Object*> next;

  // Collect all Class objects.
  ImgObjectVisitor collect_classes_visitor([&](mirror::Object* entry)
                                               REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  image_header.VisitObjects(&collect_classes_visitor, base, pointer_size);
  ...
  while (!next.empty()) {
    ...
    // There is no ArtField for array elements, handle them separately.
    if (parent_obj->IsObjectArray()) {
      ObjPtr<mirror::ObjectArray<mirror::Object>> array =
          parent_obj->AsObjectArray<mirror::Object>();
...
size_t CountUnreachableObjects(const std::unordered_map<mirror::Object*, ParentInfo>& parent_map,
  ...
  ImgObjectVisitor print_non_reachable_visitor(
      [&](mirror::Object* entry) REQUIRES_SHARED(Locks::mutator_lock_) {
        if (parent_map.count(entry) == 0 && !entry->IsString()) {
          non_reachable += 1;
        }
      });
  image_header.VisitObjects(&print_non_reachable_visitor, base, pointer_size);
...
class RegionSpecializedBase<mirror::Object> : public RegionCommon<mirror::Object> {
 public:
  ...
  void VisitEntries(VisitorClass* visitor,
    ...
    parent_map_ =
        CalculateParentMap(RegionCommon<mirror::Object>::image_header_, base, pointer_size);
    size_t unreachable_objects = CountUnreachableObjects(
        parent_map_, RegionCommon<mirror::Object>::image_header_, base, pointer_size);
    os_ << "Number of non-string objects not reached from classes:" << unreachable_objects << "\n";
  }
 ...
 private:
  ...
  std::unordered_map<mirror::Object*, ParentInfo> parent_map_;
