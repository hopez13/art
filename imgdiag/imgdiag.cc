...
class RegionSpecializedBase<ArtMethod> : RegionCommon<ArtMethod> {
 public:
  ...
  void VisitObject(ArtMethod* method ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  }

  void AddFalseDirtyObject(ArtMethod* method)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    RegionCommon<ArtMethod>::AddFalseDirtyObject(method);
  }

  void AddCleanObject(ArtMethod* method ATTRIBUTE_UNUSED) {
  }

  void AddDirtyObject(ArtMethod* method, ArtMethod* method_remote)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    size_t obj_size = ObjectSize(method);
    ++different_objects_;
    dirty_object_bytes_ += obj_size;
    // Increment counts for the fields that are dirty
    ...
    dirty_objects_.push_back(method);
  }

  void DiffObjectContents(ArtMethod* method ATTRIBUTE_UNUSED,
  ...
  void DumpDirtyObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    DumpSamplesAndOffsetCount();
    os_ << "      field contents:\n";
    const auto& dirty_objects_list = dirty_objects_;
    for (ArtMethod* method : dirty_objects_list) {
      // remote method
  ...
  void DumpFalseDirtyObjects() REQUIRES_SHARED(Locks::mutator_lock_) {
    os_ << "      field contents:\n";
    for (ArtMethod* method : false_dirty_objects_) {
      // local class
