...
class ImgDiagDumper {
  ...
  bool Init() {
    ...
    for (gc::space::ImageSpace* space : Runtime::Current()->GetHeap()->GetBootImageSpaces()) {
      ...
      if (!GetPageFrameNumbers(clean_pagemap_file.get(),
                               ...
                               &error_msg)) {
        os << "Failed to get page frame number image space " << space->GetImageLocation()
           << ", error: " << error_msg;
        return false;
      }

      size_t num_dirty_pages = 0u;
      size_t first_dirty_page = page_frame_numbers.size();
      for (size_t i = 0u, size = page_frame_numbers.size(); i != size; ++i) {
        ...
        if (!kpageflags_file->PreadFully(&kpage_flags_entry,
                                         kPageFlagsEntrySize,
                                         page_frame_numbers[i] * kPageFlagsEntrySize)) {
          os << "Failed to get page flags for PFN " << page_frame_numbers[i]
             << ", page #" << i << " in " << space->GetImageLocation()
             << ", error: " << strerror(errno);
          return false;
        }
        if (UNLIKELY((kpage_flags_entry & kPageFlagsDirtyMask) != 0u)) {
          ++num_dirty_pages;
          if (first_dirty_page == page_frame_numbers.size()) {
            first_dirty_page = i;
          }
        }
      }
      if (num_dirty_pages != 0u) {
        os << "Found " << num_dirty_pages << " dirty pages for " << space->GetImageLocation()
           << ", first dirty page: " << first_dirty_page;
 ...
 private:
  ...
  bool DumpImageDiffMap(const ImageHeader& image_header, const std::string& image_location)
    ...
    auto find_boot_map = [&os, &image_location_base_name](BacktraceMap& maps,
                                                          const char* tag,
                                                          /*out*/ backtrace_map_t* boot_map) {
    ...
    if (!find_boot_map(*image_proc_maps_, "image", &boot_map)) {
      return false;
    }
    ...
    if (zygote_diff_pid_ != -1) {
      backtrace_map_t zygote_boot_map{};
      if (!find_boot_map(*zygote_proc_maps_, "zygote", &zygote_boot_map)) {
        return false;
      }
