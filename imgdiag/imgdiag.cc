 ...
 public:
  RegionSpecializedBase(std::ostream* os,
      ...
      : RegionCommon<mirror::Object>(os, remote_contents, zygote_contents, boot_map, image_header),
        os_(*os),
        dirty_objects_os_(dirty_objects_os) { }
  ...
  void DiffEntryContents(mirror::Object* entry,
    ...
    if (!dirty_static_fields.empty()) {
      if (output_dirty_objects && dirty_objects_os_ != nullptr) {
        *dirty_objects_os_ << mirror::Class::PrettyDescriptor(entry->AsClass()) << "\n";
      }
...
class ImgDiagDumper {
 ...
 private:
  ...
  std::ostream* dirty_objects_os_;  // Ostream dump for dirty objects.
...
struct ImgDiagArgs : public CmdlineArgs {
 protected:
  ...
  virtual ParseStatus ParseCustom(const StringPiece& option,
    ...
    } else if (option.starts_with("--dirty-objects-dump=")) {
      std::string dump_filename = option.substr(strlen("--dirty-objects-dump=")).ToString();
      const char* filename = dump_filename.c_str();
      dirty_objects_out_.reset(new std::ofstream(filename));
      if (!dirty_objects_out_->good()) {
        *error_msg = "Failed to open output file";
        return kParseError;
      }
      dirty_objects_os_ = dirty_objects_out_.get();
    } else {
  ...
  virtual std::string GetUsage() const {
    ...
    usage +=  // Optional.
        ...
        "  --dirty-objects-dump=<file>: provide the file to dump dirty objects of interest.\n"
        "      Example: --dirty-objects-dump=/tmp/dirty-image-classes\n"
        "\n";
 ...
 public:
  ...
  std::ostream* dirty_objects_os_ = nullptr;
  std::unique_ptr<std::ofstream> dirty_objects_out_;
};
...
struct ImgDiagMain : public CmdlineMain<ImgDiagArgs> {
  virtual bool ExecuteWithRuntime(Runtime* runtime) {
    ...
    return DumpImage(runtime,
                     ...
                     args_->dirty_objects_os_) == EXIT_SUCCESS;
  }
};
