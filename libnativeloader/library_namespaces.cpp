...
std::unordered_set<std::string> GetApexNamespacesFromDexPath(JNIEnv* env, jstring java_dex_path) {
  ...
  for (auto i = iter; i != end; ++i) {
    apex_namespaces.emplace((*i).str(1));
  }
...
Result<NativeLoaderNamespace*> LibraryNamespaces::Create(JNIEnv* env, uint32_t target_sdk_version,
  ...
  for (const auto& ns_name : GetApexNamespacesFromDexPath(env, dex_path)) {
    ...
    const std::string& public_libraries = get_public_libraries_by_name(ns_name);
    if (apex_ns && !public_libraries.empty()) {
      linked = app_ns->Link(*apex_ns, public_libraries);
      if (!linked) {
        return linked.error();
      }
    }
