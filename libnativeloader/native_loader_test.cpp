...
jni com_android_bar libbar.so:libbar2.so
  ...
  Result<std::map<std::string, std::string>> jni_libs = ParseApexLibrariesConfig(file_content, "jni");
  ...
  Result<std::map<std::string, std::string>> public_libs = ParseApexLibrariesConfig(file_content, "public");
...
TEST(NativeLoaderApexLibrariesConfigParser, RejectMalformed) {
  ASSERT_FALSE(ParseApexLibrariesConfig("com_android_foo").ok());
}
