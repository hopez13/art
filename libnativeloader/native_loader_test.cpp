...
TEST_P(NativeLoaderTest_Create, UnbundledProductApp) {
  ...
  if (is_product_vndk_version_defined()) {
    ...
    // at the end. Skip the EXPECT_CALL in SetExpectations and create a more
    // lenient variant of it here.
    ...
    EXPECT_CALL(
        *mock,
        mock_link_namespaces(
            Eq(IsBridged()), _, NsEq("system"), StartsWith(expected_shared_libs_to_platform_ns)))
        .WillOnce(Return(true));
  }
  SetExpectations();
  RunTest();
