...
Result<NativeLoaderNamespace*> LockedCreateClassLoaderNamespace(JNIEnv* env,
...
#if defined(ART_TARGET_ANDROID)
  std::lock_guard<std::mutex> guard(g_namespaces_mutex);
  Result<NativeLoaderNamespace*> ns = LockedCreateClassLoaderNamespace(env,
                                                                       target_sdk_version,
                                                                       class_loader,
                                                                       is_shared,
                                                                       dex_path,
...
void* OpenNativeLibrary(JNIEnv* env, int32_t target_sdk_version, const char* path,
  ...
  if ((ns = g_namespaces->FindNamespaceByClassLoader(env, class_loader)) == nullptr) {
    ...
    Result<NativeLoaderNamespace*> isolated_ns =
        LockedCreateClassLoaderNamespace(env,
                                         target_sdk_version,
                                         class_loader,
                                         /*is_shared=*/false,
                                         /*dex_path=*/nullptr,
