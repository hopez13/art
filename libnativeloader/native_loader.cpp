...
Result<void> CreateNativeloadDefaultNamespaceLibsLink(NativeLoaderNamespace& ns)
  ...
  if (links != nullptr && *links != 0) {
    ...
    Result<void> linked = ns.Link(nullptr, links);
    if (!linked.ok()) {
      return linked.error();
    }
  }
...
Result<NativeLoaderNamespace*> GetNativeloaderExtraLibsNamespace() REQUIRES(g_namespaces_mutex) {
  if (g_nativeloader_extra_libs_namespace) {
  ...
  Result<void> linked =
      CreateNativeloadDefaultNamespaceLibsLink(*g_nativeloader_extra_libs_namespace);
  if (!linked.ok()) {
    return linked.error();
  }
  return g_nativeloader_extra_libs_namespace;
...
Result<NativeLoaderNamespace*> UnlockedCreateClassLoaderNamespace(JNIEnv* env,
  ...
  if (Debuggable()) {
    Result<void> linked = CreateNativeloadDefaultNamespaceLibsLink(*ns.value());
    if (!linked.ok()) {
      return linked.error();
    }
  }
...
#if defined(ART_TARGET_ANDROID)
  std::lock_guard<std::mutex> guard(g_namespaces_mutex);
  Result<NativeLoaderNamespace*> ns = UnlockedCreateClassLoaderNamespace(env,
                                                                         target_sdk_version,
                                                                         class_loader,
                                                                         is_shared,
                                                                         dex_path,
...
void* OpenNativeLibrary(JNIEnv* env, int32_t target_sdk_version, const char* path,
  ...
  if ((ns = g_namespaces->FindNamespaceByClassLoader(env, class_loader)) == nullptr) {
    ...
    Result<NativeLoaderNamespace*> isolated_ns =
        UnlockedCreateClassLoaderNamespace(env,
                                           target_sdk_version,
                                           class_loader,
                                           false /* is_shared */,
                                           nullptr,
