...
public class LibnativeloaderTest extends BaseHostJUnit4Test {
    ...
    public void testLoadLibrariesFromSystemApp() throws Exception {
        try (DeviceContext ctx = new DeviceContext(getDevice())) {
            ctx.instrumentApp("android.test.app.system");
        }
    }
    ...
    public void testLoadLibrariesFromVendorApp() throws Exception {
        try (DeviceContext ctx = new DeviceContext(getDevice())) {
            ctx.instrumentApp("android.test.app.vendor");
        }
    }
    ...
    private static class DeviceContext implements AutoCloseable {
        ...
        void softReboot() throws DeviceNotAvailableException {
            ...
            assertThat(assertCommandSucceeds("getprop dev.bootcomplete")).isEqualTo("1");
        }
        ...
        void instrumentApp(String appName) throws DeviceNotAvailableException {
            String res = assertCommandSucceeds("am instrument -w " + appName);
            // It would be nice to directly propagate both successes and failures from the tests in
            // the instrumented app. Short of that, just look for the "OK" status on the last line.
            assertThat(res).containsMatch("\\nOK[^\\n]*\\Z");
        }
    }
