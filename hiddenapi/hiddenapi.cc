...
class DexMember {
 public:
  ...
  void SetHidden(bool bit1, bool bit2) {
    const uint32_t old_flags = it_.GetRawMemberAccessFlagsWithHiddenBits();
    // Check that the item is not using dex-level hidden bits. If it is,
    // GetRawMemberAccessFlags() will remove them.
    CHECK_EQ(old_flags, it_.GetRawMemberAccessFlags());
 ...
 private:
  ...
  static const uint8_t* ReverseSearchUnsignedLeb128(const uint8_t* end_ptr, uint32_t value) {
    ...
    CHECK_EQ(DecodeUnsignedLeb128(&tmp_ptr), value);
