...
public class Main {
    ...
    private static int $noinline$BitCountBoolean(boolean x) { return Integer.bitCount(x ? 1 : 0); }

    /// CHECK-START: int Main.$noinline$BitCountByte(byte) builder (after)
    /// CHECK-DAG:     <<Result:i\d+>>  InvokeStaticOrDirect intrinsic:IntegerBitCount
    /// CHECK-DAG:                      Return [<<Result>>]
    private static int $noinline$BitCountByte(byte x) { return Integer.bitCount(x); }

    /// CHECK-START: int Main.$noinline$BitCountShort(short) builder (after)
    /// CHECK-DAG:     <<Result:i\d+>>  InvokeStaticOrDirect intrinsic:IntegerBitCount
    /// CHECK-DAG:                      Return [<<Result>>]
    private static int $noinline$BitCountShort(short x) { return Integer.bitCount(x); }

    /// CHECK-START: int Main.$noinline$BitCountChar(char) builder (after)
    /// CHECK-DAG:     <<Result:i\d+>>  InvokeStaticOrDirect intrinsic:IntegerBitCount
    /// CHECK-DAG:                      Return [<<Result>>]
    private static int $noinline$BitCountChar(char x) { return Integer.bitCount(x); }

    /// CHECK-START: int Main.$noinline$BitCountInt(int) builder (after)
    /// CHECK-DAG:     <<Result:i\d+>>  InvokeStaticOrDirect intrinsic:IntegerBitCount
    /// CHECK-DAG:                      Return [<<Result>>]
    private static int $noinline$BitCountInt(int x) { return Integer.bitCount(x); }
    ...
    /// CHECK-DAG:                      Return [<<Result>>]
    private static int $noinline$BitCountLong(long x) { return Long.bitCount(x); }
