...
public class Main {
  ...
  private static boolean CRC32ByteArrayThrowsIOBException(byte[] bytes, int off,
      int len) {
    try {
      crc32.reset();
      crc32.update(bytes, off, len);
  ...
  private static boolean CRC32ByteArrayThrowsNPException() {
    crc32.reset();
    try {
  ...
  private static void TestCRC32UpdateBytes() {
    ...
    assertEqual(true,
        CRC32ByteArrayThrowsIOBException(new byte[] {0, 0, 0, 0}, 2, 3));
    assertEqual(true,
        CRC32ByteArrayThrowsIOBException(new byte[] {0, 0, 0, 0}, 3, 2));
