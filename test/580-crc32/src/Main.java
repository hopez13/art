...
public class Main {
    ...
    private static void TestCRC32Update() {
        ...
        assertEqual(0xFF41D912L, CRC32BytesUsingUpdateInt(0x0100, 0x010000, 0x01000000));
        assertEqual(0xFF41D912L, CRC32BytesUsingUpdateInt(0xff00, 0xffff00, 0xffffff00));
        assertEqual(0xFF41D912L, CRC32BytesUsingUpdateInt(0x1200, 0x123400, 0x12345600));

        assertEqual(0x909FB2F2L, CRC32BytesUsingUpdateInt(1, 1, 1));
        assertEqual(0x909FB2F2L, CRC32BytesUsingUpdateInt(0x0101, 0x010001, 0x01000001));
        assertEqual(0x909FB2F2L, CRC32BytesUsingUpdateInt(0xff01, 0xffff01, 0xffffff01));
        assertEqual(0x909FB2F2L, CRC32BytesUsingUpdateInt(0x1201, 0x123401, 0x12345601));

        assertEqual(0xE33A9F71L, CRC32BytesUsingUpdateInt(0x0f, 0x0f, 0x0f));
        assertEqual(0xE33A9F71L, CRC32BytesUsingUpdateInt(0x010f, 0x01000f, 0x0100000f));
        assertEqual(0xE33A9F71L, CRC32BytesUsingUpdateInt(0xff0f, 0xffff0f, 0xffffff0f));
        assertEqual(0xE33A9F71L, CRC32BytesUsingUpdateInt(0x120f, 0x12340f, 0x1234560f));

        assertEqual(0xFFFFFF00L, CRC32BytesUsingUpdateInt(0x0ff, 0x0ff, 0x0ff));
        assertEqual(0xFFFFFF00L, CRC32BytesUsingUpdateInt(0x01ff, 0x0100ff, 0x010000ff));
        assertEqual(0xFFFFFF00L, CRC32BytesUsingUpdateInt(0x00ffff, 0x00ffffff, 0xffffffff));
        assertEqual(0xFFFFFF00L, CRC32BytesUsingUpdateInt(0x12ff, 0x1234ff, 0x123456ff));
    ...
    private static void TestCRC32UpdateBytes() {
        ...
        assertEqual(CRC32BytesUsingUpdateInt(0, -1, Byte.MIN_VALUE, Byte.MAX_VALUE),
        ...
        assertEqual(
                CRC32BytesUsingUpdateInt(bytes, 0, 8 * 1024), CRC32ByteArray(bytes, 0, 8 * 1024));
    ...
    private static void TestCRC32UpdateByteBuffer() {
        ...
        assertEqual(CRC32BytesUsingUpdateInt(0, 0, 0), CRC32ByteBuffer(new byte[] {0, 0, 0}, 0, 3));
        assertEqual(CRC32BytesUsingUpdateInt(1, 1, 1), CRC32ByteBuffer(new byte[] {1, 1, 1}, 0, 3));
        assertEqual(CRC32BytesUsingUpdateInt(0x0f, 0x0f, 0x0f),
                CRC32ByteBuffer(new byte[] {0x0f, 0x0f, 0x0f}, 0, 3));
        assertEqual(CRC32BytesUsingUpdateInt(0xff, 0xff, 0xff),
                CRC32ByteBuffer(new byte[] {-1, -1, -1}, 0, 3));
        assertEqual(CRC32BytesUsingUpdateInt(1, 2), CRC32ByteBuffer(new byte[] {1, 2}, 0, 2));
        assertEqual(CRC32BytesUsingUpdateInt(0, -1, Byte.MIN_VALUE, Byte.MAX_VALUE),
        ...
        assertEqual(
                CRC32BytesUsingUpdateInt(bytes, 0, 8 * 1024), CRC32ByteBuffer(bytes, 0, 8 * 1024));

        int off = rnd.nextInt(bytes.length / 2);
        for (int len = 0; len <= 16; ++len) {
            assertEqual(
                    CRC32BytesUsingUpdateInt(bytes, off, len), CRC32ByteBuffer(bytes, off, len));
        }

        // Check there are no issues with unaligned accesses.
        for (int o = 1; o < 8; ++o) {
            for (int l = 0; l <= 16; ++l) {
                assertEqual(CRC32BytesUsingUpdateInt(bytes, o, l), CRC32ByteBuffer(bytes, o, l));
            }
        }

        int len = bytes.length / 2;
        assertEqual(
                CRC32BytesUsingUpdateInt(bytes, 0, len - 1), CRC32ByteBuffer(bytes, 0, len - 1));
        assertEqual(CRC32BytesUsingUpdateInt(bytes, 0, len), CRC32ByteBuffer(bytes, 0, len));
        assertEqual(
                CRC32BytesUsingUpdateInt(bytes, 0, len + 1), CRC32ByteBuffer(bytes, 0, len + 1));

        len = rnd.nextInt(bytes.length + 1);
        off = rnd.nextInt(bytes.length - len);
        assertEqual(CRC32BytesUsingUpdateInt(bytes, off, len), CRC32ByteBuffer(bytes, off, len));
    }
    ...
    private static void TestCRC32UpdateDirectByteBuffer() {
        ...
        assertEqual(CRC32BytesUsingUpdateInt(0xff, 0xff, 0xff),
                CRC32DirectByteBuffer(new byte[] {-1, -1, -1}, 0, 3));
        assertEqual(CRC32BytesUsingUpdateInt(1, 2), CRC32DirectByteBuffer(new byte[] {1, 2}, 0, 2));
        assertEqual(CRC32BytesUsingUpdateInt(0, -1, Byte.MIN_VALUE, Byte.MAX_VALUE),
        ...
        for (int o = 1; o < 8; ++o) {
            for (int l = 0; l <= 16; ++l) {
                assertEqual(
                        CRC32BytesUsingUpdateInt(bytes, o, l), CRC32DirectByteBuffer(bytes, o, l));
            }
        }
        ...
        assertEqual(CRC32BytesUsingUpdateInt(bytes, 0, len), CRC32DirectByteBuffer(bytes, 0, len));
        assertEqual(CRC32BytesUsingUpdateInt(bytes, 0, len + 1),
                CRC32DirectByteBuffer(bytes, 0, len + 1));
        ...
        off = rnd.nextInt(bytes.length - len);
        assertEqual(
                CRC32BytesUsingUpdateInt(bytes, off, len), CRC32DirectByteBuffer(bytes, off, len));
    }
