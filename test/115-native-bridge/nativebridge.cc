...
// Status transition utility of testSignalHandlerNotReturn.
//
// The transition is fixed actually...
class SignalHandlerTestStatus {
  ...
  void Set(TestStatus state) {
    switch (state) {
      case TestStatus::kNone:
        Transfer(TestStatus::kHandleSecond, state);
        return;

      case TestStatus::kRaiseFirst:
        Transfer(TestStatus::kNone, state);
        return;

      case TestStatus::kHandleFirst:
        Transfer(TestStatus::kRaiseFirst, state);
        return;

      case TestStatus::kRaiseSecond:
        Transfer(TestStatus::kHandleFirst, state);
        return;

      case TestStatus::kHandleSecond:
        Transfer(TestStatus::kRaiseSecond, state);
        return;
 ...
 private:
  ...
  // double confirm whether the status transition is correct.
  void Transfer(TestStatus expected_last, TestStatus tobe) {
    if (state_ == expected_last) {
      state_ = tobe;
    } else {
      printf("ERROR: unexpected signal handling status!\n");
    }
  }
};
