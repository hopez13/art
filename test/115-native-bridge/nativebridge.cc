...
enum class TestStatus {
  kNone,      // default status except testSignalHandlerNotReturn.
  kRaiseFirst,
  kHandleFirst,
  kRaiseSecond,
  kHandleSecond,
...
class SignalHandlerTestStatus {
 public:
  ...
  bool Expect(TestStatus state) {
    if (state_ != state) {
      printf("ERROR: unexpected signal handling state!\n");
    }
    return (state_ == state);
  }

  // double confirm is the status is correct.
  void Set(TestStatus state) {
    switch (state) {
      case TestStatus::kNone:
        if (Expect(TestStatus::kHandleSecond)) {
          state_ = state;
        }
        return;

      case TestStatus::kRaiseFirst:
        if (Expect(TestStatus::kNone)) {
          state_ = state;
        }
        return;

      case TestStatus::kHandleFirst:
        if (Expect(TestStatus::kRaiseFirst)) {
          state_ = state;
        }
        return;

      case TestStatus::kRaiseSecond:
        if (Expect(TestStatus::kHandleFirst)) {
          state_ = state;
        }
        return;

      case TestStatus::kHandleSecond:
        if (Expect(TestStatus::kRaiseSecond)) {
          state_ = state;
        }
        return;
    }
...
//
// 1. Raise signal.
// 2. Raise another signal in NativeBridge's signal handler.
// 3. Expect that NativeBridge's signal handler invokes again.
//
