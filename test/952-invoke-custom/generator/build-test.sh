...
if [ "x$ANDROID_BUILD_TOP" = "x" ]; do
  echo Build environment is not set-up.
  exit -1
fi

# This only works internally for now (sorry folks!)
jack_source_url=https://x20web.corp.google.com/teams/android-runtime/jack/jack-test-annotations-lib.jack
jack_annotations_lib=$HOME/Downloads/jack-test-annotations-lib.jack

curl -o $jack_annotations_lib $jack_source_url
