...
public class ChildClass {
    ...
    enum DexDomain { CorePlatform, Platform, Application }
    ...
    public static void runTest(String libFileName, int parentDomainOrdinal, int childDomainOrdinal,
            boolean everythingSdked) throws Exception {
        ...
        // hardening is disabled (i.e. target SDK is Q or earlier). The only configuration where
        // this workaround used to work is for ChildClass in the Application domain and ParentClass
        // in the Platform domain, so only test that configuration with hidden api check hardening
        // disabled.
        ...
        for (Hiddenness hiddenness : Hiddenness.values()) {
            ...
            } else if (parentDomain == DexDomain.CorePlatform
                    && childDomain == DexDomain.Platform) {
            ...
            for (boolean isStatic : booleanValues) {
                ...
                for (Visibility visibility : Visibility.values()) {
                    ...
                    checkMethod(ParentInterface.class, name, /*isStatic*/ false, visibility,
                            expected, invokesMemberCallback, testHiddenApiCheckHardeningDisabled);
                }
    ...
    private static void checkMemberCallback(Class<?> klass, String name, boolean isPublic,
            boolean isField, boolean expectedCallback) {
        try {
            ...
            boolean actualCallback =
                    consumer.recordedValue != null && consumer.recordedValue.contains(name);
    ...
    private static void checkField(Class<?> klass, String name, boolean isStatic,
        ...
        if (JLI.canDiscoverWithLookupFindStaticSetter(publicLookup, klass, name, int.class)
                != canDiscover) {
            throwDiscoveryException(klass, name, true,
                    "MethodHandles.publicLookup().findStaticSetter()", canDiscover);
        }
    ...
    private static void checkMethod(Class<?> klass, String name, boolean isStatic,
        ...
        // With hidden api check hardening enabled, only white and light greylisted methods should
        // be discoverable.
    ...
    private static void checkConstructor(Class<?> klass, Visibility visibility,
            Hiddenness hiddenness, Behaviour behaviour, boolean testHiddenApiCheckHardeningDisabled)
            throws Exception {
        boolean isPublic = (visibility == Visibility.Public);
        ...
        Class<?> args[] =
                new Class[] {visibility.mAssociatedType.mClass, hiddenness.mAssociatedType.mClass};
        Object initargs[] = new Object[] {
                visibility.mAssociatedType.mDefaultValue, hiddenness.mAssociatedType.mDefaultValue};
        ...
        if (Reflection.canDiscoverWithGetDeclaredConstructor(klass, args) != canDiscover) {
            throwDiscoveryException(
                    klass, fullName, false, "getDeclaredConstructor()", canDiscover);
        }

        if (Reflection.canDiscoverWithGetDeclaredConstructors(klass, args) != canDiscover) {
            throwDiscoveryException(
                    klass, fullName, false, "getDeclaredConstructors()", canDiscover);
        }
        ...
        if (JLI.canDiscoverWithLookupFindConstructor(publicLookup, klass, methodType)
            ...
            throwDiscoveryException(klass, fullName, false,
                    "MethodHandles.publicLookup().findConstructor", canDiscover);
        }
    ...
    private static void throwDiscoveryException(
            Class<?> klass, String name, boolean isField, String fn, boolean canAccess) {
    ...
    private static void throwAccessException(
            Class<?> klass, String name, boolean isField, String fn) {
        throw new RuntimeException("Expected to be able to access "
                + (isField ? "field " : "method ") + klass.getName() + "." + name + " using " + fn
                + ". " + configMessage);
    }
