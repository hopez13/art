...
public class ChildClass {
  ...
  private static void checkOverriding(String suffix,
                                      ...
                                      Behaviour behaviour) throws Exception {
    if (isStatic) {
      // Does not make sense to override a static method.
      return;
    }

    switch (visibility) {
      case Private:
        // Does not make sense to override a private method.
        return;
      case Public:
      case Protected:
        break;
      case Package:
        ...
        if (!isParentInBoot || !isChildInBoot) {
          behaviour =  ? Behaviour.Granted : Behaviour.Denied;
        }
        break;
    }
