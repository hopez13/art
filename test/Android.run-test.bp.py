  ...
  with open("Android.run-test.bp", mode="wt") as f:
    ...
    for mode in ["host", "target", "jvm"]:
      ...
      for shard in ["{:02}".format(i) for i in range(100)]:
        ...
        # sources.
        extra_srcs = set()
        for metadata_file in glob.glob(f"?{shard}-*/test-metadata.json") + glob.glob(f"??{shard}-*/test-metadata.json"):
            with open(metadata_file, "r") as metadata_f:
                try:
                    metadata = json.load(metadata_f)
                except json.JSONDecodeError as e:
                    sys.exit(f"error: {metadata_file}: {e}")
            srcs = metadata.get("srcs_for_test_data_shard", [])
            if not isinstance(srcs, list):
                sys.exit(f"error: {metadata_file}: srcs_for_test_data_shard must be a list of strings")
            for s in srcs:
