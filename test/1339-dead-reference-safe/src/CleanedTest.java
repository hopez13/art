...
@DeadReferenceSafe
public final class CleanedTest {
  ...
  private static void $noinline$loop() {
    ...
    for (int i = 0; i < INNER_ITERS; ++i) {
      ...
      // x is dead here.
      if (i == 5) {
        // Since there is an @Cleaned reference to x.n, x should be kept live
        // until it is reassigned. Thus the last instance should not be finalized.
        Main.$noinline$gcAndCheck(nFinalized, 5, "Cleaned",
            "@Cleaned failed to keep object live.");
      }
    }
  }
  ...
  private static void reset(int expected_count) {
    ...
    if (nFinalized.get() != expected_count) {
      System.out.println("CleanedTest: Wrong number of finalized objects:"
                         + nFinalized.get());
    }
    nFinalized.set(0);
  }
  ...
  public static void runTest() {
    try {
      Main.ensureCompiled(CleanedTest.class, "$noinline$loop");
    ...
    if (count != INNER_ITERS) {
      System.out.println("CleanedTest: Final count wrong: " + count);
    }
    reset(INNER_ITERS);
  }
}
