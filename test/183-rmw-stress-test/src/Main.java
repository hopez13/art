...
class Main {
    ...
    // This test uses `AtomicReference` which is implemented using a `VarHandle` instead of
    // using a `VarHandle` directly. This is because the normal `VarHandle` checks are done
    // without read barrier which makes them likely to fail and take the slow-path to the
    // runtime. The `AtomicReference` uses a boot-image `VarHandle` which is optimized to
    // avoid those checks, making it more likely to hit bugs in the raw RMW operation.
            ...
            if (i == 0) {
                ...
                threads[i] = new Thread() {
                    public void run() {
                        ...
                        // the value and cause the thread 0 to fail.
                        assertTrue(value != null);
                        while (!stopFlag.stop) {
