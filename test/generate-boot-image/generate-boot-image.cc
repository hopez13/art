...
static std::vector<std::string> GetDexFiles() {
  const char* bootclasspath_env = getenv("BOOTCLASSPATH");
  CHECK(bootclasspath_env != nullptr);
...
static int GenerateBootImage(const std::string& dir, const std::string& compiler_filter) {
