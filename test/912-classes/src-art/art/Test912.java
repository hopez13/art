...
public class Test912 {
    ...
    private static void testClass(Class<?> base) throws Exception {
        ...
        if (mod != base.getModifiers()) {
            throw new RuntimeException(
                    "Unexpected modifiers: " + base.getModifiers() + " vs " + mod);
        }
    ...
    private static void testClassLoader(Class<?> c) {
        ...
        if (cl == null) {
            ...
            if (!(cl instanceof ClassLoader)) {
                throw new RuntimeException(
                        "Unexpected \"classloader\": " + cl + " (" + cl.getClass() + ")");
            }
            if (cl != c.getClassLoader()) {
                throw new RuntimeException(
                        "Unexpected classloader: " + c.getClassLoader() + " vs " + cl);
            }
        }
    }
    ...
    private static void testClassVersion() throws Exception {
        ...
        Class target = ((ClassLoader) ctor.newInstance(
                                ByteBuffer.wrap(DEX_BYTES), Test912.class.getClassLoader()))
                               .loadClass("Transform");
    ...
    private static void testClassEvents() throws Exception {
        ...
        final String PRELOAD_FOR_JIT[] = {
                "java.nio.charset.CoderMalfunctionError", "java.util.NoSuchElementException",
    ...
    private static void TestRecursiveClassPrepareEvents() {
        ...
        runRecursiveClassPrepareEvents(() -> {
            ...
            try {
                ...
                Constructor<?> ctor =
                        class_loader_class.getConstructor(ByteBuffer.class, ClassLoader.class);
                Class<?> target = ((ClassLoader) ctor.newInstance(ByteBuffer.wrap(DEX_BYTES),
                                           Test912.class.getClassLoader()))
                                          .loadClass("Transform");
                target.newInstance();
            } catch (Exception e) {
            }
    ...
    private static class TestForNonInit {
        public static double doubleValue =
                Math.random(); // So it can't be compile-time initialized.
    }
