...
public class Main {
   ...
   * known bug, between 5 and 10 retries were sufficient. Use the RETRIES to configure how many
  ...
  private static void testConcurrentProcessing(RunnerFactory factory, String testName) throws Throwable {
  ...
  private static abstract class TaskRunner extends Thread {
       ...
       *
       * @return true if the task has been retrieved and is not available to any other threads.
       * Otherwise false. If false is returned, then either the task was no longer present on the
       * queue due to another thread grabbing it, or, in case of spurious failure, the task is still
       * available and no other thread managed to grab it.
       */
  ...
  private static void check(int actual, int expected, String msg, int iteration) {
    if (actual != expected) {
      System.err.println("[iteration " + iteration + "] " + msg + " : " + actual + " != " + expected);
      System.exit(1);
    }
  }
}
