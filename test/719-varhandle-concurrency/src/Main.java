...
public class Main {
    ...
    static { QA = MethodHandles.arrayElementVarHandle(TestTask[].class); }
     ...
     *
     * The actual task-grabbing mechanism is implemented in subclasses through grabTask(). This
     * allows testing various mechanisms, like compareAndSet() and compareAndExchange().
     */
    private static abstract class TaskRunner extends Thread {
        protected final TestTask[] tasks;

        TaskRunner(TestTask[] tasks) { this.tasks = tasks; }
    ...
    private static class TaskRunnerWithCompareAndExchange extends TaskRunner {
        TaskRunnerWithCompareAndExchange(TestTask[] tasks) { super(tasks); }
    ...
    private static class TaskRunnerWithCompareAndSet extends TaskRunner {
        TaskRunnerWithCompareAndSet(TestTask[] tasks) { super(tasks); }
    ...
    private static class TaskRunnerWithWeakCompareAndSet extends TaskRunner {
        TaskRunnerWithWeakCompareAndSet(TestTask[] tasks) { super(tasks); }
    ...
    private static class TestTask {
        ...
        public void exec() { action.accept(ord); }
    }
