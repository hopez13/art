...
gtests:
* run-tests  
  Java tests run through the runtime.
* gtests  
  C++ tests exercising various aspects of ART.

## ART run-tests

ART run-tests are written in Java and/or
[Smali](https://github.com/JesusFreke/smali) (compiled/assembled as Dex code)
and sometimes native code (written as C/C++ testing libraries). They are
...
scripts located in the `test/etc` directory (resp. `default-build`,
`default-run`, and `default-check`). These scripts rely on environment variables
set by `test/run-test`.
...
(implemented in `test/etc/default-build`) is to compare its standard output with
...
ART gtests are located in various directories of the ART project (usually in the
same directory as the exercised code) and are implemented as one C++ (`.cc`)
file or more. The construction logic of these tests is implemented in ART's
build system (`Android.bp` and `Android*.mk` files). On host, these gtests can
be run by executing `m test-art-host-gtests`. On device, the best supported
approach is to run these test in a chroot environment (see `README.chroot.md` in
this directory).
