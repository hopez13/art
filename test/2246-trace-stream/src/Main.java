...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        testTracing(
                /* streaming=*/true, /* flags= */ 0, stream_parser, BaseTraceParser.STREAMING_DUAL_CLOCK_VERSION);

        System.out.println("***** streaming test - wall clock *******");
        StreamTraceParser stream_parser_wall_clock = new StreamTraceParser();
        testTracing(
                /* streaming=*/true, /* flags= */ WALL_CLOCK_FLAG, stream_parser, BaseTraceParser.STREAMING_WALL_CLOCK_VERSION);

        System.out.println("***** non streaming test - dual clock *******");
        NonStreamTraceParser non_stream_parser = new NonStreamTraceParser();
        testTracing(/* streaming=*/false, /* flags= */ 0, non_stream_parser, BaseTraceParser.DUAL_CLOCK_VERSION);
        ...
        testTracing(/* streaming=*/false, /* flags= */ WALL_CLOCK_FLAG, non_stream_parser_wall_clock, BaseTraceParser.WALL_CLOCK_VERSION);

    }

    public static void testTracing(boolean streaming, int flags, BaseTraceParser parser, int expected_version)
            throws Exception {
        Main m = new Main();
        Thread t = new Thread(() -> {
            try {
