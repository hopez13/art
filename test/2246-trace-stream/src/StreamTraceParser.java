...
public class StreamTraceParser {
  public static final int kMagicNumber = 0x574f4c53;
  public static final int kDualClockVersion = 3;
  public static final int kTraceVersionDualClock = 0xF3;
  ...
  public void validateTraceHeader(int expected_version) throws Exception{
    ...
    if (magic_number != kMagicNumber) {
      throw new Exception("Magic number doesn't match. Expected " + Integer.toHexString(kMagicNumber) + " Got " + Integer.toHexString(magic_number));
    }
    // Read 2-byte version
    int version = readNumber(2);
    if (version != expected_version) {
      throw new Exception("Unexpected version. Expected " + expected_version + " Got " + version);
    }
    ...
    if (version >= kDualClockVersion) {
      // Read 2-byte record size.
      // TODO(mythria): Check why this is needed. We can derive record_size from version. Not sure
      // why this is needed.
