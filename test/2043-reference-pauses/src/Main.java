 ...
 * finalizers. Also makes ure that finalizer-reachable data is not collected.
...
public class Main {
    ...
    // A finalizable object that referes to lots of otherwise unreachable stuff.
    ...
    long timeReachable() {
        ...
        for (int i = 0; i < NBATCHES; ++i) {
            for (int j = i * NREFS / NBATCHES; j < (i + 1) * NREFS / NBATCHES; ++j) {
                Integer newObj = new Integer(j);
                strongRefs[j] = newObj;
                weakRefs.set(j, new WeakReference(newObj));
            }
            for (int j = (i + 1) * NREFS / NBATCHES - 1; j >= 0; --j) {
                WeakReference<Integer> wr = weakRefs.get(j);
                long startNanos = System.nanoTime();
                Integer referent = wr.get();
                long totalNanos = System.nanoTime() - startNanos;
    ...
    static Runnable allocFinalizable = new Runnable() {
        public void run() {
            // Allocate and drop a lot of finalizable objects that take a long time
            // to mark. Designed to be hard to optimize away. Each of these object will
            // build a new one in its finalizer before really going away.
            ResurrectingObject a[] = new ResurrectingObject[2];
