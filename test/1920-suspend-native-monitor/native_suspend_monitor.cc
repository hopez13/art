...
namespace art {
namespace Test1920NativeSuspendMonitor {
...
extern "C" JNIEXPORT void JNICALL Java_art_Test1920_nativeSpin(JNIEnv* env, jclass) {
  jrawMonitorID mon;
  if (JvmtiErrorToException(
      env, jvmti_env, jvmti_env->CreateRawMonitor("Test1920 monitor", &mon))) {
    return;
  }
  while (!done.load()) {
...
extern "C" JNIEXPORT void JNICALL Java_art_Test1920_resume(JNIEnv*, jclass) {
  should_pause.store(false);
  while (paused.load()) { }
}

extern "C" JNIEXPORT void JNICALL Java_art_Test1920_reset(JNIEnv*, jclass) {
  started.store(false);
}
extern "C" JNIEXPORT void JNICALL Java_art_Test1920_nativeFinish(JNIEnv*, jclass) {
  done.store(true);
}
