...
public class Test1930 {
    ...
    private static void testLockedTwiceNJ() {
        ...
        executeLockedNative(
                () -> { executeLockedNative(() -> { printMonitorUsage(lk); }, lk); }, lk);
    }
    ...
    private static void testLockWait() throws Exception {
        ...
        synchronized (lk) {
            ...
            sem.acquire();
            // Wait for the other thread to go to sleep trying to get the mutex. This might take a
            // (short) time since we try spinning first for better performance.
...
public static void printPreLock(Object lock) {
    System.out.println(String.format("Pre-lock[%s]: %s", Thread.currentThread().getName(),
            Monitors.getObjectMonitorUsage(lock)));
}
...
public static void printMonitorUsage(Object m) {
    System.out.println(String.format(
            "Thread[%s]: %s", Thread.currentThread().getName(), Monitors.getObjectMonitorUsage(m)));
}
}
