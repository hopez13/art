...
class Main {
    ...
    public static void testLoadingSecondaryAppImageInLoadedClassLoader() throws Exception {
        ...
        assertFalse(
                "Secondary app image isn't already loaded", checkAppImageLoaded(SECONDARY_NAME));

        PathClassLoader pcl = new PathClassLoader(DEX_FILE, LIBRARY_SEARCH_PATH, null);
        pcl.addDexPath(SECONDARY_DEX_FILE);

        assertTrue(
                "Ensure app image is loaded if it should be", checkAppImageLoaded(SECONDARY_NAME));

        Class<?> secondaryCls = pcl.loadClass("Secondary");
        assertTrue(
                "Ensure Secondary class is in the app image", checkAppImageContains(secondaryCls));
    ...
    private static void assertFalse(String message, boolean flag) { assertTrue(message, !flag); }
}
