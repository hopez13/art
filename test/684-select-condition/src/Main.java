...
public class Main {
  ...
  static float $noinline$test() {
    ...
    int one = $noinline$getField();  // r0(field) = InvokeStaticOrDirect[]
    float a = (float)one;        // s0(a_1) = TypeConversion[r0(one)]
    a = a < 2.0f ? a : 2.0f;     // s16(a_2) = Select[s0(a_1), C(2.0f), GreaterThanOrEqual[s0(a_1), s16(2.0f)]]
    // The following call is added to clobber caller-saves, forcing the output of the Select to be allocated to s16.
    $noinline$nothing();
    return a;
  }
