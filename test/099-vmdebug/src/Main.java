...
public class Main {
    ...
    private static void checkBiggerThan0(int i) throws Exception {
        if (i < 0) {
    ...
    private static void testGetAllocCount() throws Exception {
        // Invoke at least one GC and wait for 20 seconds or so to get allocated objects.
        for (int i = 0; i < 20; ++i) {
            Runtime.getRuntime().gc();
            Thread.sleep(1000L);
        }
        ...
        checkBiggerThan0(alloc_objects);
        checkBiggerThan0(alloc_bytes);
        checkBiggerThan0(freed_objects);
        checkBiggerThan0(freed_bytes);
        checkBiggerThan0(gc_invoc);
        checkBiggerThan0(class_init_count);
        checkBiggerThan0(class_init_time);
        checkBiggerThan0(ext_alloc_objects);
        checkBiggerThan0(ext_alloc_bytes);
        checkBiggerThan0(ext_freed_objects);
        checkBiggerThan0(ext_freed_bytes);
        ...
        checkBiggerThan0(VMDebug.getAllocCount(KIND_ALLOCATED_OBJECTS));
        checkBiggerThan0(VMDebug.getAllocCount(KIND_GC_INVOCATIONS));
    }
