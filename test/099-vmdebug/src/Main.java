...
public class Main {
    ...
    private static void testGetVmFeatureList() throws Exception {
        ...
        for (int i = 0; i <  feature_list.length; i++) {
            System.out.println(feature_list[i]);
        }
    }

    private static void testDebuggerDetails() throws Exception {
        boolean debugger_connected = VMDebug.isDebuggerConnected();
        System.out.println(debugger_connected ? "Debugger Connected" : "Debugger Not Connected");
        boolean debugging_enabled = VMDebug.isDebuggingEnabled();
        System.out.println(debugging_enabled ? "Debugging Enabled" : "Debugging Not Enabled");
