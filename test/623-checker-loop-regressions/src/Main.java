...
public class Main {
    ...
    static void largeBody(int[] x) {
        for (int i = 0; i < 100; i++) {
            x[i] = x[i] * 1 + x[i] * 2 + x[i] * 3 + x[i] * 4 + x[i] * 5 + x[i] * 6 + x[i] * 7
                    ...
                    + x[i] * 14 + x[i] * 15 + x[i] * 1 + x[i] * 2 + x[i] * 3 + x[i] * 4 + x[i] * 5
                    + x[i] * 6 + x[i] * 7 + x[i] * 8 + x[i] * 9 + x[i] * 10 + x[i] * 11 + x[i] * 12
    ...
    /// CHECK-DAG: VecLoad
    /// CHECK-DAG: ArrayGet
    private static final float $noinline$ensureSlowPathFPSpillFill(
            float[] a, float[] b, float[] c, float[] d, int[] e) {
