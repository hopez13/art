...
public class Main {
    ...
    /// CHECK:                      ArraySet
    /// CHECK-NOT:                  ArraySet
    private static final void $noinline$unrollingInTheNest_TryCatchNotBlocking(
            int[] a, int[] b, int x) {
        for (int k = 0; k < 16; k++) {
            for (int j = 0; j < 16; j++) {
                for (int i = 0; i < 128; i++) {
    ...
    // Each one of the three `for` loops has an `if`. Plus an `if` inside the outer `for`. The try
    // catch has the 5th `if`.
    /// CHECK-START: void Main.$noinline$unrollingTwoLoopsInTheNest(int[], int[], int) loop_optimization (before)
    /// CHECK:                       If
    /// CHECK:                       If
    /// CHECK:                       If
    ...
    /// CHECK:                      ArraySet
    /// CHECK-NOT:                  ArraySet
    private static final void $noinline$unrollingTwoLoopsInTheNest_OneBlocking(
            int[] a, int[] b, int x) {
        for (int k = 0; k < 128; k++) {
            if (x > 100) {
                for (int j = 0; j < 128; j++) {
    ...
    /// CHECK:                      ArraySet
    /// CHECK-NOT:                  ArraySet
    private static final void $noinline$unrollingTwoLoopsInTheNest_OtherBlocking(
            int[] a, int[] b, int x) {
        for (int k = 0; k < 128; k++) {
            if (x > 100) {
                for (int j = 0; j < 128; j++) {
    ...
    // Each one of the three `for` loops has an `if`. Plus an `if` inside the outer `for`. The try
    // catchs have the 5th and 6th `if`.
    /// CHECK-START: void Main.$noinline$unrollingTwoLoopsInTheNest_BothBlocking(int[], int[], int) loop_optimization (before)
    /// CHECK:                       If
    /// CHECK:                       If
    /// CHECK:                       If
    ...
    /// CHECK:                      ArraySet
    /// CHECK-NOT:                  ArraySet
    private static final void $noinline$unrollingTwoLoopsInTheNest_BothBlocking(
            int[] a, int[] b, int x) {
        for (int k = 0; k < 128; k++) {
            if (x > 100) {
                for (int j = 0; j < 128; j++) {
    ...
    public static void main(String[] args) {
        // Use existing tests to show that the difference between having a try catch inside or
        // outside the loop.
    ...
    private static void $noinline$unreachable() { throw new Error("Unreachable"); }
