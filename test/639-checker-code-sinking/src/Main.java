...
public class Main {
    ...
    static void $noinline$changeIntField() { mainField.intField = 42; }
    ...
    static void $noinline$changeStaticObjectField() { obj = new Main(); }
    ...
    public String $opt$noinline$toString() { return "" + intField; }
    ...
    private static int testSinkToSecondCatch() {
        ...
        try {
            // We need a different boolean to the one above, so that the compiler cannot optimize
            // this return away.
    ...
    // Tests that we don't sink the Object creation into a catch handler surrounded by try/catch,
    // even when that inner catch is not at the boundary of the outer try catch.
    ...
    // We currently have a heuristic that disallows inlining methods if their basic blocks end with
    // a throw. We could add code so that `requireNonNull`'s block doesn't end with a throw but that
    // would mean that the string builder optimization wouldn't fire as it requires all uses to be
    // in the same block. If `requireNonNull` is inlined at some point, we need to re-mark it as
    // $inline$ so that the test is operational again.
