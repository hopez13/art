...
public class Main {
    public static void main(String[] args) throws ClassNotFoundException, NoSuchMethodException {
        ...
        String[] classes = new String[] {
                "VirtualMethods", "DirectMethods", "SyntheticMethods", "NonSyntheticMethods"};
    ...
    private static void checkMethodList(Class<?> klass, int expectedLength) {
        ...
        for (Method method : methods) {
            if (!METHOD_NAME.equals(method.getName())) {
                throw new RuntimeException(className + ": expected declared method name: \""
                        + METHOD_NAME + "\", got: \"" + method.getName() + "\"");
            }
        ...
        if (methods.length >= 1 && !hasNumberReturnType) {
            throw new RuntimeException(
                    className + ": expected a method with return type \"Number\"");
        }
        if (methods.length >= 2 && !hasDoubleReturnType) {
            throw new RuntimeException(
                    className + ": expected a method with return type \"Double\"");
        }
    }
    ...
    private static void checkMethod(Class<?> klass) throws NoSuchMethodException {
        ...
        if (!METHOD_NAME.equals(method.getName())) {
            ...
            throw new RuntimeException(className
                    + ": expected method return type: \"Number\", got \""
                    + method.getReturnType().toString() + "\"");
        }
    }

    private static final String DEX_EXTRA =
            new File(System.getenv("DEX_LOCATION"), "690-hiddenapi-same-name-methods-ex.jar")
                    .getAbsolutePath();
