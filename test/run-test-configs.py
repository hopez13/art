...
class SkippedTest(collections.namedtuple("SkippedTest", ['skip_group',
                                                         ...
                                                         'test_to_skip'])):
  def get_cmd(self):
    return " ".join(["art/test/run-test"] + self.args + [self.test_to_skip.name])
...
class ExecutedTest(collections.namedtuple("ExecutedTest", ['test_group',
                                                           ...
                                                           'success'])):
  def get_cmd(self):
    return " ".join(["art/test/run-test"] + self.args + [self.name])

def make_test_groups(configs, skip_groups, tests):
  skips = []
  groups = []
  all_tests = frozenset(tests)
  for config in configs:
    config_tests = set(all_tests)
    for skip_group in skip_groups:
      if skip_group.is_relevant(config):
        for to_skip in skip_group.tests:
          # Only note the skip if we were thinking of doing the test. Do allow for a skip to be
          # noted multiple times however if there were multiple skip-criteria it hit.
          if to_skip.name in all_tests:
...
def run_one_test(test):
  # Start the test. Put output into a pipe so we can display it later back when we are
  # single-threaded again. Otherwise we could interleave.
...
def run_test_groups(args, reporter, test_groups):
  results = []
  with concurrent.futures.ProcessPoolExecutor(args.jobs) as executor:
    for result in executor.map(run_one_test, get_all_tests(test_groups)):
      reporter(result)
...
def report_outcome(success, skip, fail):
  ...
  for test in success:
    print("succeeded: {:20}\t$ {}".format(test.name, test.get_cmd()))
  for test in skip:
    print("skipped: {:20} (Reason: {})\t$ {}".format(test.test_to_skip.name, test.test_to_skip.reason, test.get_cmd()))
  for test in fail:
    print("failed: {:20}\t$ {}".format(test.name, test.get_cmd()))
  return 0

def loud_reporter(test):
  if test.success:
    print("succeeded: {:20}\t$ {}".format(test.name, test.get_cmd()))
  else:
    print("Failed: {:20}\t$ {}\n{}".format(test.name, test.get_cmd(), test.output))

def run_tests(args):
  configs = make_config_list(args)
  test_groups, skipped_tests = make_test_groups(configs, make_skip_list(args.skips), args.tests)
  successes, failures = run_test_groups(args, loud_reporter, test_groups)
...
def main():
  ...
  # The fact that these have string "True" & "False" is intentional as it means we don't need to
  # special case them when we are looking through the skips xml file.
  ...
  parser.add_argument("--heap_poisoning",
                      required=False,
                      action="store_const",
                      default="False",
                      const="True",
