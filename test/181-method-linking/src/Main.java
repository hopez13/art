...
public class Main {
    public static void main(String args[]) {
        ...
        d.callAFoo();  // pkg1.C.foo (not overridden by pkg2.D.foo)
                       // However, RI ends up in pkg2.D.foo() even though it should not
                       // override package-private pkg1.A.foo()/pkg1.C.foo().
        ...
        d2.callBFoo();  // pkg2.D2.foo (overrides package-private pkg2.B.foo in the same package)
                        // However, ART ends up in pkg2.B.foo() even though it should be overridden
                        // by pkg2.D2.foo() - ART never overrides 2 vtable entries with one method
                        // and the public pkg1.C2.foo() is found first.
