...
public class Main {
    ...
    boolean tryToRun(boolean lastChance) {
        ...
        final long referenceTime =
                Math.min(referenceTime1, Math.min(referenceTime2, referenceTime3));
        ...
        if (actualTime > 500_000_000) {
            ...
            System.out.println("Notifications ran too quickly; no blocking GC? msec = "
                    + (actualTime / 1_000_000)
                    + " reference(msec) = " + (referenceTime / 1_000_000));
        }
        ...
        if (deallocated > allocated
                || deallocated < allocated - 5 /* slop for register references */) {
