...
static void methodExitCB(jvmtiEnv* jvmti,
  ...
  if (method == data->exit_method || method == data->enter_method || data->in_callback) {
    ...
    // char *fname, *fsig, *fgen;
    // jvmti->GetMethodName(method, &fname, &fsig, &fgen);
    // LOG(INFO) << "=> (exit ) cb for " << fname << fsig;
    // jvmti->Deallocate(reinterpret_cast<unsigned char*>(fname));
    // jvmti->Deallocate(reinterpret_cast<unsigned char*>(fsig));
    // jvmti->Deallocate(reinterpret_cast<unsigned char*>(fgen));
    return;
  }
...
static void methodEntryCB(jvmtiEnv* jvmti,
  ...
  if (method == data->exit_method || method == data->enter_method || data->in_callback) {
    ...
    // char *fname, *fsig, *fgen;
    // jvmti->GetMethodName(method, &fname, &fsig, &fgen);
    // LOG(INFO) << "<= (enter) cb for " << fname << fsig;
    // jvmti->Deallocate(reinterpret_cast<unsigned char*>(fname));
    // jvmti->Deallocate(reinterpret_cast<unsigned char*>(fsig));
    // jvmti->Deallocate(reinterpret_cast<unsigned char*>(fgen));
    return;
...
extern "C" JNIEXPORT void JNICALL Java_art_Trace_disableMethodTracing(
    JNIEnv* env, jclass klass ATTRIBUTE_UNUSED, jthread thr_UNUSED ATTRIBUTE_UNUSED) {
  // TODO Figure out why the hell this is needed.
  jthread thr;
  jvmti_env->GetCurrentThread(&thr);
