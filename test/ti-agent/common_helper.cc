...
class JvmtiExtensionFunctionCleaner {
 public:
  JvmtiExtensionFunctionCleaner(JNIEnv* env, jvmtiEnv* jvmtienv)
      : env_(env), jvmtienv_(jvmtienv), funcs_(nullptr), n_funcs_(0)  {}
  ~JvmtiExtensionFunctionCleaner() {
    for (jint i = 0; i < n_funcs_; i++) {
      jvmtiExtensionFunctionInfo* finfo = &funcs_[i];
      DeallocateParams(finfo);
      DoDeallocate(finfo->id);
      DoDeallocate(finfo->short_description);
      DoDeallocate(finfo->params);
      DoDeallocate(finfo->errors);
    }
    n_funcs_ = 0;
    DoDeallocate(funcs_);
    funcs_ = nullptr;
  ...
  bool Init() {
    return !JvmtiErrorToException(env_, jvmtienv_,
                                  jvmtienv_->GetExtensionFunctions(&n_funcs_, &funcs_));
  }
  ...
  const jvmtiExtensionFunctionInfo* GetFunction(jint i) const {
    CHECK_LT(i, n_funcs_);
    return &funcs_[i];
  }

 private:
  void DeallocateParams(jvmtiExtensionFunctionInfo* finfo) {
    for (jint i = 0; i < finfo->param_count; i++) {
      jvmtiParamInfo* param = &finfo->params[i];
      DoDeallocate(param->name);
    }
  }

  template<typename T> void DoDeallocate(T* ptr) {
    jvmtienv_->Deallocate(reinterpret_cast<unsigned char*>(ptr));
  }

  JNIEnv* env_;
  jvmtiEnv* jvmtienv_;
  jvmtiExtensionFunctionInfo* funcs_;
  jint n_funcs_;
};
...
extern "C" JNIEXPORT void JNICALL Java_art_Trace_watchAllFieldAccesses(JNIEnv* env) {
  ...
  JvmtiExtensionFunctionCleaner funcs(env, jvmti_env);
  if (!funcs.Init()) {
    return;
  }
  for (jint i = 0; i < funcs.NumFuncs(); i++) {
    const jvmtiExtensionFunctionInfo* func = funcs.GetFunction(i);
    if (strcmp("com.android.art.field.set_global_field_access_watch", func->id) == 0) {
      jvmtiError err = func->func(jvmti_env);
      // Ignore duplicate
  ...
  // We don't have the nice ART extension. Do everything manually.
...
extern "C" JNIEXPORT void JNICALL Java_art_Trace_watchAllFieldModifications(JNIEnv* env) {
  ...
  for (jint i = 0; i < funcs.NumFuncs(); i++) {
    const jvmtiExtensionFunctionInfo* func = funcs.GetFunction(i);
    if (strcmp("com.android.art.field.set_global_field_modification_watch", func->id) == 0) {
      jvmtiError err = func->func(jvmti_env);
      // Ignore duplicate
      if (err != JVMTI_ERROR_DUPLICATE) {
        JvmtiErrorToException(env, jvmti_env, err);
...
extern "C" JNIEXPORT void JNICALL Java_art_Trace_enableTracing(
  ...
  if ((field_modify != nullptr || field_access != nullptr) && IsJVM()) {
    // We might need the classPrepareCB to watch new fields.
    // TODO Might want to make a new jvmtiEnv to get these events...
    if (JvmtiErrorToException(env,
                              jvmti_env,
                              jvmti_env->SetEventNotificationMode(JVMTI_ENABLE,
