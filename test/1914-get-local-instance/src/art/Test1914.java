...
public class Test1914 {
    ...
    public static interface SafepointFunction {
        public void invoke(Thread thread, Method target, int depth) throws Exception;
    }
    ...
    public static class TestCase {
        ...
        public TestCase(Method target) { this(null, target); }
            ...
            for (StackTrace.StackFrameData frame : StackTrace.GetStackTrace(thr)) {
                if (frame.method.equals(target)
                        || (frame.method.getName().equals(target.getName())
                                && Arrays.deepEquals(frame.method.getParameterTypes(),
                                        target.getParameterTypes())
                                && ((Method) frame.method)
                                           .getReturnType()
                                           .equals(target.getReturnType()))) {
                    return frame;
                }
            }
    ...
    public static Object getProxyObject(final Class... k) {
        return Proxy.newProxyInstance(Test1914.class.getClassLoader(), k, (p, m, a) -> {
