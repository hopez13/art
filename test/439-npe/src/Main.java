    ...
    static void $opt$noinline$testRegisterRetrieval() {
        ...
        try {
            for (i = 0; i < array.length; ++i) {
                ...
                // none of them anonymous. Hopefully, all available physical registers will be
                // allocated to these variables, so that when `m.intField` throws NPE during the
                // third iteration, we will fully test retrieval of values from all physical
                // registers.
                addInt = m.intField;
                addLong = m.longField;
