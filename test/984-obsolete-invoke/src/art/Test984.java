...
public class Test984 {
    static class Transform {
        // This method must be 'static' so that when we try to invoke it through a j.l.r.Method we
        // will simply use the jmethodID directly and not do any lookup in any receiver object.
    ...
    // from a stack with an obsolete method in it. There should be no other ways to obtain an
    // obsolete jmethodID in ART without unsafe casts.
    public static Method obsolete_method = null;

    public static void doTest() {
        // Capture the obsolete method.
        //
        // NB The obsolete method must be direct so that we will not look in the receiver type to
        ...
        try {
            ...
            if (e instanceof InternalError || e.getCause() instanceof InternalError) {
                ...
                System.out.println(
                        "Unexpected error type for calling obsolete method! Expected either "
    ...
    // Gets the first obsolete method on the current threads stack (NB only looks through the first
    // 30 stack frames).
