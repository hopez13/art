...
def setup_test_env():
  ...
  if not _user_input_variants['address_sizes']:
    ...
    _user_input_variants['address_sizes_target']['jvm'].add(
        env.ART_PHONY_TEST_HOST_SUFFIX)
...
def run_tests(tests):
  ...
  target_address_combinations += len(uncombinated_input_variants)
  ...
  config = itertools.product(tests, target_input_variants, _user_input_variants['run'],
  ...
  uncombinated_config = itertools.product(tests, uncombinated_input_variants)

  # Get up to "count" values from a tuple even if the tuple has less than count elements.
  def safe_tuple_get(tpl, count, defvalue):
    ctr = 0
    while ctr < count:
      if ctr < len(tpl):
        yield tpl[ctr]
  ...
  def start_combination(config_tuple, address_size):
      ...
      jni, image, pictest, debuggable, jvmti, cdex_level = safe_tuple_get(config_tuple, 14, "")
