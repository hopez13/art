...
# directory.
RUN_TEST_SET = set()
# The semaphore object is used by the testrunner to limits the number of
...
def run_tests(tests):
  ...
  workers reaches the maximum allowed concurrency, the method wait for a
  existing thread worker to release the semaphore object. Worker releases
  ...
  for test, target, run, prebuild, compiler, relocate, trace, gc, \
    ...
    for address_size in ADDRESS_SIZES_TARGET[target]:
      if stop_testrunner:
        ...
        # stops creating any any threads and wait for all the exising threads
        # to end.
...
def run_test(command, test, test_variant, test_name):
  ...
  # However, if not, the information is printed on the same line overriding the
