...
building 'test-art-host-run-test-dependencies' for host tests,
 'test-art-target-run-test-dependencies' for target tests,
 and 'test-art-run-test-dependencies' for building dependencies for both.
...
# DISABLED_TEST_CONTAINER holds information about the disabled tests. Its a map
...
# The Dict contains list of all possible variant for a given type. For eg,
...
# The default value of the number of jobs is set to number of active
# processor + 2. ninja uses the same default when not provided with
# any jobs flag.
n_thread = os.sysconf('SC_NPROCESSORS_ONLN') + 2
...
def get_disabled_test_info():
  ...
  :returns: The method returns a dict of tests mapped to the variants list
...
def parse_test_name(test_name):
  ...
  It supports two type of test_name:
  1) Like 001-HelloWorld. In this case, it will just verify if the test actually
  exists and if it does, it return the testname.
  ...
  correctly. If yes, it will initiale the various VARIANT_TYPES to use the
  variant required to run the test. In the end it will again return the test_name
...
def parse_option():
  ...
  parser.add_option('--no-relocate', action='store_true', dest='no_relocate')
  ...
  parser.add_option('--npicimage', action='store_true', dest='npicimage')
  parser.add_option('--multipicimage', action='store_true', dest='multipicimage')
  parser.add_option('--multinpicimage', action='store_true', dest='multinpicimage')
