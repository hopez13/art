...
def run_tests(tests):
  ...
  total_test_count *= len(_user_input_variants['run'])
  total_test_count *= len(_user_input_variants['prebuild'])
  total_test_count *= len(_user_input_variants['relocate'])
  total_test_count *= len(_user_input_variants['trace'])
  total_test_count *= len(_user_input_variants['gc'])
  total_test_count *= len(_user_input_variants['jni'])
  total_test_count *= len(_user_input_variants['image'])
  total_test_count *= len(_user_input_variants['pictest'])
  total_test_count *= len(_user_input_variants['debuggable'])
  total_test_count *= len(_user_input_variants['compiler'])
  total_test_count *= len(_user_input_variants['jvmti'])
  ...
  for test, target, run, prebuild, compiler, relocate, trace, gc, \
    ...
    for address_size in _user_input_variants['address_sizes_target'][target]:
      ...
      if compiler == 'optimizing':
        ...
        options_test += ' --speed-with-profile'
...
def parse_option():
  ...
  for variant_type in VARIANT_TYPE_DICT:
    for variant in VARIANT_TYPE_DICT[variant_type]:
      if options.get(variant.replace('-', '_')):
