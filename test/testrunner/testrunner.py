    ...
    if not test_skipped:
      if all(test_passed):
        ...
        flaked_tests.append((test_name,
                             str(command) + "\n" +
                             "\n".join("Flake {}/{}\n{}".format(cnt, len(script_output), out) \
                                       for cnt, out in zip(itertools.count(1),
                                                           script_output[:-1]))))
        ...
        failed_tests.append((test_name, str(command) + "\n" +
                             "\n".join("Failure {}/{}\n{}".format(cnt, len(script_output), out) \
                                       for cnt, out in zip(itertools.count(1), script_output))))
        if not env.ART_TEST_KEEP_GOING:
          stop_testrunner = True
...
def get_disabled_test_info():
  ...
  for failure in known_failures_info:
    ...
    if fail_type not in ("failure", "flake", 'unsupported'):
      raise ValueError("type '%s' is not 'failure', 'unsupported' or 'flake'" % fail_type)
    ...
    to_add = flaky_test_info if fail_type == 'flake' else disabled_test_info
...
def parse_option():
  ...
  parser.add_argument("--retry-flakes", "-r", dest="flake_retry", type=int, default=None,
                      help="Number of times to retry tests marked as flaky. Default=1. Must not" +
                           " be set if --gdb is passed")
  ...
  if options['gdb']:
    ...
    if options['flake_retry'] is not None:
      raise ValueError("Cannot have non-default flake-retry with gdb.")
    else:
      # Don't want to retry at all if we are running with gdb
      flake_retry = 0
  flake_retry = 1 if options['flake_retry'] is None else options['flake_retry']
