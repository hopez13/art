...
def run_tests(tests=None):
  ...
  total_test_count *= len(TARGET_TYPES)
  ...
  total_test_count *= len(ADDRESS_SIZES)
...
def run_test(command, test, test_variant, test_name):
  ...
  print_mutex.acquire()
  sys.stdout.write(prefix + out + suffix)
  sys.stdout.flush()
  print_mutex.release()
  semaphore.release()
...
def print_analysis():
  if not verbose:
    print ' ' * last_print_length + '\r',
  if len(skipped_tests) > 0:
    print COLOR_SKIP + 'SKIPPED TEST' + COLOR_NORMAL
    for test in skipped_tests:
      print test

  if len(failed_tests) > 0:
    print
    print COLOR_ERROR + 'FAILED TEST' + COLOR_NORMAL
