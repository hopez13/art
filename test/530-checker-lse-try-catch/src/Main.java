...
public class Main {
    public static void main(String[] args) {
        ...
        assertEquals(
                1, $noinline$testRedundantStore(new Point(), boolean_throw, boolean_other_throw));
    ...
    // Check that we don't eliminate the first store to `main.sumForKeepStoreInsideTryCatch` since
    // it is observable.
    ...
    private static int $noinline$returnValue(int value) { return value; }
    ...
    private static int $noinline$testDontKeepStoreInsideCatch(int[] array) {
        ...
        try {
            ...
            // These sets can be eliminated even though we have invokes since this catch is not part
            // of an outer try.
    ...
    // This store potentially can be eliminated too, but our phi creation logic doesn't realize it
    // can create a Phi for `main.sumForKeepStoreInsideTryCatch` and skip a store+load.
    ...
    // Check that the stores to array[0] are not eliminated since we can throw in between the
    // stores.
