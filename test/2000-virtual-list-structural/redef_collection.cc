...
namespace art {
namespace Test2000RedefineCollection {
...
static void JNICALL RedefineObjectHook(jvmtiEnv *jvmti_env,
  ...
  if (strcmp(name, "java/util/AbstractCollection") != 0) {
    return;
  }

  dex::Reader reader(class_data, class_data_len);
  dex::u4 class_index = reader.FindClassIndex("Ljava/util/AbstractCollection;");
  if (class_index == dex::kNoIndex) {
    env->ThrowNew(env->FindClass("java/lang/RuntimeException"),
                  "Failed to find AbstractCollection in dex file!");
    return;
  }

  reader.CreateClassIr(class_index);
  auto dex_ir = reader.GetIr();
  auto count_field = std::unique_ptr(dex_ir->Alloc<dex::FieldId
