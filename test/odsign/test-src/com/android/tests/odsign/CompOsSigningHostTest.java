...
public class CompOsSigningHostTest extends ActivationTest {
    ...
    public static void beforeClassWithDevice(TestInformation testInfo) throws Exception {
        ...
        assertCommandSucceeds(device, "cmd jobscheduler run android 5132251");
        // It takes time. Just don't spam.
        TimeUnit.SECONDS.sleep(SECONDS_BEFORE_PROGRESS_CHECK);
        // The job runs asynchronously. To wait until it completes, wait until the final siganture
        // file appears.
        waitForFileExist(device, PENDING_ARTIFACTS_DIR + "/compos.info.signature",
                ODREFRESH_MAX_SECONDS - SECONDS_BEFORE_PROGRESS_CHECK);
    ...
    private static void waitForFileExist(ITestDevice device, String path, int timeout)
            throws Exception {
        for (int i = 0; i < timeout; i++) {
            if (device.doesFileExist(path)) {
                CLog.d("File found at iteration " + i);
                return;
            }
