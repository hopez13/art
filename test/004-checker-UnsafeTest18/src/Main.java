...
public class Main {
    ...
    private static void load() { unsafe.loadFence(); }
    ...
    private static void store() { unsafe.storeFence(); }
    ...
    private static void full() { unsafe.fullFence(); }
    ...
    public static void main(String[] args) {
        ...
        fork(new Runnable() {
            public void run() {
                for (int i = 0; i < 10; i++) set32(m, intOffset, i);
            }
        });
        join();
        ...
        fork(new Runnable() {
            public void run() {
                for (int i = 0; i < 10; i++) set64(m, longOffset, (long) (100 + i));
            }
        });
        join();
        ...
        fork(new Runnable() {
            public void run() {
                for (int i = 0; i < 10; i++) setObj(m, objOffset, sThreads[i]);
            }
        });
        join();
        ...
        fork(new Runnable() {
            public void run() {
                for (int i = 0; i < 10; i++) add32(m, intOffset, i + 1);
            }
        });
        join();
        ...
        fork(new Runnable() {
            public void run() {
                for (int i = 0; i < 10; i++) add64(m, longOffset, (long) (i + 1));
            }
        });
        join();
