...
public class TestFpAbs {
    ...
    private static float $opt$noinline$absSP(float f) { return Math.abs(f); }

    /// CHECK-START: double TestFpAbs.$opt$noinline$absDP(double) builder (after)
    /// CHECK-DAG: <<Result:d\d+>> Abs
    /// CHECK-DAG:                 Return [<<Result>>]
    private static double $opt$noinline$absDP(double d) { return Math.abs(d); }
    ...
    public static void main() {
        ...
        // A few NaN numbers.
        int[] spnans = {0x7f800001, // signaling
        ...
        for (int i = 0; i < spnans.length; i++) {
            ...
            expectEqualsNaN32(
                    spnans[i] & Integer.MAX_VALUE, Float.floatToRawIntBits($opt$noinline$absSP(f)));
        }
        ...
        for (double d = -1.5d; d <= -1.49999999999d;
        ...
        // A few NaN numbers.
        long[] dpnans = {0x7ff0000000000001L, 0x7ff4000000000000L, 0x7ff8000000000000L,
                0x7fffffffffffffffL, 0xfff0000000000001L, 0xfff4000000000000L, 0xfff8000000000000L,
                0xffffffffffffffffL};
        for (int i = 0; i < dpnans.length; i++) {
            double d = Double.longBitsToDouble(dpnans[i]);
            expectEqualsNaN64(
                    dpnans[i] & Long.MAX_VALUE, Double.doubleToRawLongBits($opt$noinline$absDP(d)));
        }
    ...
    private static void expectEquals32(int expected, int result) {
        if (expected != result) {
            throw new Error("Expected: 0x" + Integer.toHexString(expected) + ", found: 0x"
    ...
    private static void expectEquals64(long expected, long result) {
        if (expected != result) {
            throw new Error("Expected: 0x" + Long.toHexString(expected) + ", found: 0x"
