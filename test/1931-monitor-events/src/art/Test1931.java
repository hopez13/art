...
public class Test1931 {
    ...
    public static void run() throws Exception {
        ...
        // TODO It would be good (but annoying) to do this with jasmin/smali in order to test if
        // it's different without the reflection.
    ...
    public static void testIllegalWait(final Monitors.NamedLock lk) throws Exception {
        Monitors.LockController controller1 =
                new Monitors.LockController(lk, /*timed_wait time*/ -100);
        controller1.DoLock();
        controller1.waitForLockToBeHeld();
        try {
    ...
    public static void testWait(Monitors.NamedLock lk) throws Exception {
        ...
        try (AutoCloseable suppress = SuppressContention(controller2)) {
            // If controller1 has a spurious wakeup we could see contention here. Suppress it so it
