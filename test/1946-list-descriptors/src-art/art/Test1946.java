...
public class Test1946 {
    ...
    private static <T> void checkSubset(T[] full, T[] sub, final String msg) {
        Supplier<String> msgGen = ()
                -> Arrays.toString(full) + " does not contain all of " + Arrays.toString(sub) + ": "
                + msg;
        check(new HashSet<T>(Arrays.asList(full)).containsAll(Arrays.asList(sub)), msgGen);
    }
    ...
    public static void run() throws Exception {
        ...
        checkSubset(getClassloaderDescriptors(Test1946.class.getClassLoader()),
                new String[] {"Lart/Test1946;", "Lart/Test1946$TMP1;", "Lart/Test1946$TMP2;",
                        "Lart/Test1946$TMP3;"},
                "Missing entries for current class classloader.");
        // Check that the result is exactly what we expect and includes classes that fail
        // verification.
        checkEq(getClassloaderDescriptors(
        ...
        checkEq(getClassloaderDescriptors(
                        makeClassLoaderFrom(TEST_CLASSES, Object.class.getClassLoader())),
                new String[] {"LA;", "LB;", "LC;", "LD;", "LE;"},
                "Unexpected classes in custom classloader");
        checkEq(getClassloaderDescriptors(
                        makeClassLoaderFrom(TEST_CLASSES, Test1946.class.getClassLoader())),
                new String[] {"LA;", "LB;", "LC;", "LD;", "LE;"},
                "Unexpected classes in custom classloader");
        // Check we only get 1 copy of each descriptor.
        checkEq(getClassloaderDescriptors(
                        makeClassLoaderFrom(Arrays.asList(TEST_CLASSES, TEST_CLASSES),
    ...
    private static ClassLoader makeClassLoaderFrom(byte[] data, ClassLoader parent)
            throws Exception {
