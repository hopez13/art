...
public class Main {
    ...
    private static int closedLinearStepOverflow() {
        int closed = 0;
        // Note that this isn't a "one-off" error. We are using MIN and MAX to make sure we
        // overflow.
    ...
    /// CHECK-DAG: <<Int:i\d+>>  IntConstant 10   loop:none
    /// CHECK-DAG:               Return [<<Int>>] loop:none
    private static int closedByParametersWithInline() { return $inline$closedByParameters(0, 10); }
