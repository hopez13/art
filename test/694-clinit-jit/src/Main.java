        ...
        static {
            ...
            new Thread(new Runnable() {
                public void run() {
                    ...
                    // Give some time for the JIT compiler to compile
                    // $noinline$runInternalHotMethod. Only compiled code invoke the entrypoint of
                    // an ArtMethod.
                    try {
                        Thread.sleep(1000);
                    } catch (Exception e) {
                ...
                public int $noinline$runInternalHotMethod(boolean invokeStaticMethod) {
                    if (invokeStaticMethod) {
                        // The bug used to be here: the compiled code of
                        // $noinline$runInternalHotMethod would invoke the entrypoint of
                        // $noinline$runHotMethod, which was incorrectly updated to the JIT
                        // entrypoint and therefore not hitting the resolution trampoline which
                        // would have waited for the class to be initialized.
