  ...
  public static class InnerInitialized {
    ...
    static {
      ...
      new Thread(new Runnable() {
        public void run() {
          for (int i = 0; i < 100000; i++) {
            runInternalHotMethod(false);
          }
          // Give some time for the JIT compiler to compile runInternalHotMethod.
          // Only compiled code invoke the entrypoint of an ArtMethod.
          try {
            Thread.sleep(1000);
          } catch (Exception e) {
          }
          int value = runInternalHotMethod(true);
          if (value != 42) {
            throw new Error("Expected 42, got " + value);
          }
        }

        public int runInternalHotMethod(boolean invokeStaticMethod) {
          if (invokeStaticMethod) {
            // The bug used to be here: the compiled code of runInternalHotMethod would invoke
            // the entrypoint of $noinline$runHotMethod, which was incorrectly updated
            // to the JIT entrypoint and therefore not hitting the resolution
