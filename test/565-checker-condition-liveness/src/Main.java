...
public class Main {
  ...
  /// CHECK-DAG:  <<IntArg:i\d+>>   ParameterValue        env_uses:[23,23]
  /// CHECK-DAG:  <<RefArg:l\d+>>   ParameterValue        env_uses:[11,23,23]
  /// CHECK-DAG:  <<Array:l\d+>>    ParameterValue        env_uses:[11,23,23]
  /// CHECK-DAG:  <<Const1:i\d+>>   IntConstant 1         env_uses:[23,23]
  /// CHECK-DAG:                    SuspendCheck          env:[[_,<<IntArg>>,<<RefArg>>,<<Array>>]]           liveness:10
  /// CHECK-DAG:                    NullCheck             env:[[<<Const1>>,<<IntArg>>,<<RefArg>>,<<Array>>]]  liveness:20
  /// CHECK-DAG:                    X86BoundsCheckMemory  env:[[<<Const1>>,<<IntArg>>,<<RefArg>>,<<Array>>]]  liveness:22
  /// CHECK-DAG:                    TryBoundary

  /// CHECK-START-{X86,X86_64}-DEBUGGABLE: void Main.testThrowIntoCatchBlock(int, java.lang.Object, int[]) liveness (after)
  /// CHECK-DAG:  <<IntArg:i\d+>>   ParameterValue        env_uses:[11,23,23]
  /// CHECK-DAG:  <<RefArg:l\d+>>   ParameterValue        env_uses:[11,23,23]
  /// CHECK-DAG:  <<Array:l\d+>>    ParameterValue        env_uses:[11,23,23]
  /// CHECK-DAG:  <<Const1:i\d+>>   IntConstant 1         env_uses:[23,23]
  /// CHECK-DAG:                    SuspendCheck          env:[[_,<<IntArg>>,<<RefArg>>,<<Array>>]]           liveness:10
  ...
  /// CHECK-DAG:  <<IntArg:i\d+>>   ParameterValue        env_uses:[]
  /// CHECK-DAG:  <<RefArg:l\d+>>   ParameterValue        env_uses:[11,19,19]
  /// CHECK-DAG:  <<Array:l\d+>>    ParameterValue        env_uses:[11,19,19]
  /// CHECK-DAG:  <<Const1:i\d+>>   IntConstant 1         env_uses:[]
  /// CHECK-DAG:                    SuspendCheck          env:[[_,<<IntArg>>,<<RefArg>>,<<Array>>]]           liveness:10
  /// CHECK-DAG:                    NullCheck             env:[[<<Const1>>,<<IntArg>>,<<RefArg>>,<<Array>>]]  liveness:16
  /// CHECK-DAG:                    X86BoundsCheckMemory  env:[[<<Const1>>,<<IntArg>>,<<RefArg>>,<<Array>>]]  liveness:18

  /// CHECK-START-{X86,X86_64}-DEBUGGABLE: void Main.testBoundsCheck(int, java.lang.Object, int[]) liveness (after)
  /// CHECK-DAG:  <<IntArg:i\d+>>   ParameterValue        env_uses:[11,19,19]
  /// CHECK-DAG:  <<RefArg:l\d+>>   ParameterValue        env_uses:[11,19,19]
  /// CHECK-DAG:  <<Array:l\d+>>    ParameterValue        env_uses:[11,19,19]
  /// CHECK-DAG:  <<Const1:i\d+>>   IntConstant 1         env_uses:[19,19]
  /// CHECK-DAG:                    SuspendCheck          env:[[_,<<IntArg>>,<<RefArg>>,<<Array>>]]           liveness:10
  /// CHECK-DAG:                    NullCheck             env:[[<<Const1>>,<<IntArg>>,<<RefArg>>,<<Array>>]]  liveness:16
  /// CHECK-DAG:                    X86BoundsCheckMemory  env:[[<<Const1>>,<<IntArg>>,<<RefArg>>,<<Array>>]]  liveness:18
