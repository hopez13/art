...
Java_art_Test1974_GetWeakGlobalJniRef(JNIEnv* env, jclass klass [[maybe_unused]], jobject r) {
...
Java_art_Test1974_GetObjectsWithTag(JNIEnv* env, jclass klass [[maybe_unused]], jlong tag) {
  jsize cnt = 0;
  jobject* res = nullptr;
...
Java_art_Test1974_StartAssignObsoleteIncrementedId(JNIEnv* env, jclass k [[maybe_unused]]) {
  ...
  ObsoleteEvent oe = [](jvmtiEnv* env [[maybe_unused]], jlong* obsolete, jlong* non_obsolete) {
...
extern "C" JNIEXPORT void JNICALL
Java_art_Test1974_EndAssignObsoleteIncrementedId(JNIEnv* env, jclass k [[maybe_unused]]) {
  jint id = FindExtensionEvent(env, "com.android.art.heap.obsolete_object_created");
  if (env->ExceptionCheck()) {
    LOG(INFO) << "Could not find extension event!";
    return;
...
Java_art_Test1974_CollectFreedTags(JNIEnv* env, jclass k [[maybe_unused]]) {
  if (JvmtiErrorToException(
          env,
          jvmti_env,
