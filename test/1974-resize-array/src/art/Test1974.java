...
public class Test1974 {
    ...
    public static void runInstance() {
        ...
        ResizeArray(
                () -> theInstanceClass.instance_field, theInstanceClass.instance_field.length + 5);
        ...
        System.out.println("Same value? "
                + (theInstanceClass.instance_field == theOtherInstanceClass.instance_field));
    }
    ...
    public static void runInstanceSelfRef() {
        ...
        System.out.println("Same inner-structure? "
    ...
    public static void runMultiThreadLocal() throws Exception {
        ...
        final Supplier<Object[]> getArr = new Supplier<Object[]>() {
        ...
        final Consumer<String> print1 = (String s) -> {
            msg1.add(s);
        };
        final Consumer<String> print2 = (String s) -> {
            msg2.add(s);
        };
        Function<Consumer<String>, Runnable> r = (final Consumer<String> c) -> () -> {
    ...
    public static void runWithJvmtiTagsObsolete() throws Exception {
        ...
        if (obsolete_tagged_obj.length >= 1) {
            DbgPrintln(
                    "Found objects with obsolete tag: " + Arrays.deepToString(obsolete_tagged_obj));
            boolean bad = false;
            if (obsolete_tagged_obj.length != 1) {
                System.out.println("Found obsolete tag'd objects: "
                        + Arrays.deepHashCode(obsolete_tagged_obj) + " but only expected one!");
                bad = true;
            }
            if (!Arrays.deepEquals(Arrays.copyOf(arr, ((Object[]) obsolete_tagged_obj[0]).length),
                        (Object[]) obsolete_tagged_obj[0])) {
                System.out.println(
                        "Obsolete array was unexpectedly different than non-obsolete one!");
                bad = true;
            }
