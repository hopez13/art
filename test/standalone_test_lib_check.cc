...
Result<std::vector<std::string>> GetDynamicLibDeps(const std::string& filename) {
  if (elf_version(EV_CURRENT) == EV_NONE) {
    return Errorf("libelf initialization failed: %s", elf_errmsg(-1));
  }

  android::base::unique_fd fd(open(filename.c_str(), O_RDONLY));
  if (fd.get() == -1) {
    return ErrnoErrorf("Error opening %s", filename);
  }

  Elf* elf = elf_begin(fd.get(), ELF_C_READ, /*ref=*/nullptr);
  if (elf == nullptr) {
    return Errorf("Error creating ELF object for %s: %s", filename, elf_errmsg(-1));
  }
  ...
  for (Elf_Scn* dyn_scn = nullptr; (dyn_scn = elf_nextscn(elf, dyn_scn)) != nullptr;) {
    ...
    if (gelf_getshdr(dyn_scn, &scn_hdr) != &scn_hdr) {
      return Errorf("Failed to retrieve ELF section header in %s: %s", filename, elf_errmsg(-1));
    }
    ...
    if (scn_hdr.sh_type == SHT_DYNAMIC) {
      ...
      for (int i = 0; i < scn_hdr.sh_size / scn_hdr.sh_entsize; i++) {
        ...
        if (gelf_getdyn(data, i, &dyn_entry) != &dyn_entry) {
          return Errorf("Failed to get entry %d in ELF dynamic section of %s: %s",
                        i,
                        filename,
                        elf_errmsg(-1));
        }
        ...
        if (dyn_entry.d_tag == DT_NEEDED) {
          ...
          if (lib_name == nullptr) {
            return Errorf("Failed to get string from entry %d in ELF dynamic section of %s: %s",
                          i,
                          filename,
                          elf_errmsg(-1));
          }
