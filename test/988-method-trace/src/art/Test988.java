...
public class Test988 {
    ...
    static {
        ...
        NON_DETERMINISTIC_OUTPUT_METHODS.add((Executable ex) -> {
        ...
        try {
            NON_DETERMINISTIC_TYPE_NAMES.add(Proxy.getProxyClass(
                    Test988.class.getClassLoader(), new Class[] {Runnable.class}));
        } catch (Exception e) {
        }
    }
    ...
    private static String arrayToString(Object val) {
        ...
        if ((new Object[0]).getClass().isAssignableFrom(klass)) {
            return Arrays.toString(Arrays.stream((Object[]) val)
    ...
    static final class MethodReturn implements Printable {
        ...
        public void Print() {
            ...
            System.out.println(whitespace(cnt) + "<= " + methodToString(m) + " -> <" + klass_print
                    + ": " + print + ">");
        }
    }
    ...
    static void addToResults(Printable obj) {
        ...
        // will reserve more space. Without this we may get into strange problems where
        // ArrayList::add cecks there is enough space (which involves a couple of method calls)
        // which then use up the space and by the time we actually add this record there is no
        // capacity left.
        results_index++;
    ...
    static final class IterOp implements IntUnaryOperator {
        public int applyAsInt(int x) { return iter_fibonacci(x); }
    }
    ...
    static final class RecurOp implements IntUnaryOperator {
        public int applyAsInt(int x) { return fibonacci(x); }
    }
    ...
    static final class NativeOp implements IntUnaryOperator {
        public int applyAsInt(int x) { return nativeFibonacci(x); }
    }
    ...
    public static void run() throws Exception {
        ...
        Runnable runnable = (Runnable) Proxy.newProxyInstance(Test988.class.getClassLoader(),
                new Class[] {Runnable.class}, new TestRunnableInvokeHandler());
        Trace.enableMethodTracing(Test988.class,
    ...
    static class IntrinsicsTest {
        ...
        static void doTest() {
            ...
            // since in non-tracing operation they are effectively inlined by the optimizing
            // compiler.
