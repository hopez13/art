...
public class Main {
  ...
  public static int $noinline$boxUnboxByteAsUint8(byte value) {
    ...
    // so that after inlining we can merge the `TypeConversion` in `InstanceFieldGet` with
    // a modified type. The Phi shall be eliminated by `dead_code_elimination$after_inlining`.
    // The null check that shall be eliminated in `instruction_simplifier$after_gvn (after)`
    ...
    // shall remain during all simplifier passes all the way to the disassembly.
