...
extern "C" JNIEXPORT jint JNICALL Java_Main_setPolicy(JNIEnv* env, jclass, jint pol) {
  ...
  for (jint i = 0; i < n_ext; i++) {
    ...
    if (kSetPolicyName == std::string_view(cur_info->id)) {
      set_policy = reinterpret_cast<SetPolicy>(cur_info->func);
    }
    if (kGetPolicyName == std::string_view(cur_info->id)) {
      get_policy = reinterpret_cast<GetPolicy>(cur_info->func);
    }
  ...
  // Cleanup the array.
  Dealloc(infos);
  if (set_policy == nullptr || get_policy == nullptr) {
