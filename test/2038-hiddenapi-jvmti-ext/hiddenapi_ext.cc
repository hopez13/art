...
extern "C" JNIEXPORT jint JNICALL Java_Main_disablePolicy(JNIEnv* env, jclass, jint pol) {
  jint res;
  GetPolicy get_policy = reinterpret_cast<GetPolicy>(GetExtension(env, kGetPolicyName));
  if (get_policy == nullptr) {
    return -1;
  }
  DisablePolicy disable_policy = reinterpret_cast<DisablePolicy>(GetExtension(env, kSetPolicyName));
