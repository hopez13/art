...
public class UnifiedReal {
  ...
  // sin(asin(...) compisition. It's not clear this matters in practice.
  ...
  public boolean definitelyIrrational() {
    ...
    if (crProperty.kind == IS_SQRT) {
      // IS_SQRT arguments are never a perfect square.
      return true;
    } else if (crProperty.kind == IS_IRRATIONAL) {
      return true;
    } else if (crProperty.kind == IS_LOG) {
      // If this is rational, then n ln(arg) = m ln(10), or arg^n = 10^m, n and m integers.
      return !commonPower(crProperty.arg, BoundedRational.TEN);
    } else if (crProperty.kind == IS_SIN_PI || crProperty.kind == IS_TAN_PI
        || crProperty.kind == IS_ASIN || crProperty.kind == IS_ATAN) {
      // for SIN_PI and TAN_PI, always irrational for the arguments we allow.
      // For ASIN, if this were rational, we would have rational1 = sin(rational2),
      // but that occurs only at zero. For ATAN, Laczkovich's proof of Lambert's
      // Theorem supports the same argument.
      return true;
    } else {
      return definitelyTranscendental();
    }
  }
  ...
  @SuppressWarnings("ReferenceEquality")
  public boolean sameCrFactor(UnifiedReal u) {
  ...
  public boolean isComparable(UnifiedReal u) {
    ...
    return (sameCrFactor(u)
            && (isNonzero(crProperty) || crFactor.signum(DEFAULT_INITIAL_TOLERANCE) != 0
                || crFactor.signum(ZERO_COMPARISON_TOLERANCE) != 0))
        ...
        || (sameMonotonicCrKind(u) && ratFactor.equals(u.ratFactor))
        || crValue().compareTo(u.crValue(), DEFAULT_RELATIVE_TOLERANCE, DEFAULT_COMPARISON_TOLERANCE) != 0;
  }
  ...
  public int compareTo(UnifiedReal u) {
    ...
    if (sameMonotonicCrKind(u) && ratFactor.equals(u.ratFactor)) {
      ...
      return ratFactor.signum() * ratFactor.compareTo(u.ratFactor);
    }
  ...
  private static UnifiedReal logRep(byte kind, @NonNull BoundedRational arg) {
    ...
    if (intArg != null) {
      UnifiedReal smallPowerLn = lgSmallPower(kind, intArg);
  ...
  // Dont track ln() or log() arguments whose representation is larger than this.
  ...
  private static UnifiedReal multiplySqrts(BoundedRational x, BoundedRational y) {
    if (x.equals(y)) {
      ...
      BoundedRational product = BoundedRational.multiply(x, y);
      if (product.signum() == 0) {
        return ZERO;
      }
      if (product != null) {
        BoundedRational[] decomposedProduct = product.extractSquare();
  ...
  /**
   * Return the square root. This may fail to return a known rational value, even when the result is
   * rational.
   */
  public UnifiedReal sqrt() {
    ...
    if (isOne(crProperty)) {
      BoundedRational[] decomposedProduct = ratFactor.extractSquare();
      if (decomposedProduct[1].bitLength() < 200) {
        // The limit here must be < BoundedRational.EXTRACT_SQUARE_MAX_LEN, so that we can't
        // generate IS_SQRT results for rational values.
      ...
      } // else probably not worth tracking.
    }
