...
public class Reflection {
  ...
  public static boolean canInvokeConstructor(Class<?> klass, Class<?> args[]) {
    try {
      Constructor c = klass.getDeclaredConstructor(args);
      c.setAccessible(true);
      Object initargs[] = new Object[args.length];
      for (int i = 0; i < args.length; ++i) {
        if (args[i] == Boolean.TYPE) {
          initargs[i] = Boolean.valueOf(false);
        } else if (args[i] == Character.TYPE) {
          initargs[i] = Character.valueOf('A');
        } else if (args[i] == Integer.TYPE) {
          initargs[i] = Integer.valueOf(0);
        } else if (args[i] == Long.TYPE) {
          initargs[i] = Long.valueOf(0);
        } else if (args[i] == Float.TYPE) {
          initargs[i] = Float.valueOf(0);
        } else if (args[i] == Double.TYPE) {
          initargs[i] = Double.valueOf(0);
        } else {
          throw new IllegalStateException("Unexpected type " + args[i].toString());
        }
      }
