...
public class Test1913 {
  ...
  // This doesn't work consistently on the RI for some reason.
  public static final boolean RUN_SAME_THREAD_TESTS = false;
  ...
  public static void ObjectMethod(Runnable safepoint) {
    Object TARGET = new TestClass1("ObjectMethod");
    safepoint.run();
    reportValue(TARGET);
  }
  ...
  public static class TestCase {
    ...
    public void exec(final SafepointFunction safepoint) throws Exception {
      if (RUN_SAME_THREAD_TESTS) {
        execLocal(safepoint);
      }
      execConcurrent(safepoint);
    }

    public void execLocal(final SafepointFunction safepoint) throws Exception {
      System.out.println("Running " + target + " with " + safepoint);
      Runnable run = () -> {
            Thread thr = Thread.currentThread();
            StackTrace.StackFrameData frame = findStackFrame(thr);
            Locals.VariableDescription desc = findTargetVar(frame.current_location);
            try {
              safepoint.invoke(thr, target, desc, frame.depth);
            } catch (Exception e) {
              throw new Error("Safepoint function " + safepoint + " threw exception", e);
            }
          };
      target.invoke(null, run);
    }

    public void execConcurrent(SafepointFunction safepoint) throws Exception {
