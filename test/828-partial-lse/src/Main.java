...
class Main {
    public static void $noinline$testMain(Main m) { expectEquals(0, m.myField); }
    ...
    public static void $noinline$doTest(boolean testValue) {
        Main m = new Main();
        // LSE will find that this store can be removed, as both branches override the value with a
        // new one.
        m.myField = 42;
        if (testValue) {
            // LSE will remove this store as well, as it's the value after the store of 42 is
            // removed.
            m.myField = 0;
