...
public class Main {
  ...
  /// CHECK-START: void Main.$opt$noinline$testFillArrayData() builder (after)
  ...
  /// CHECK-START: void Main.$opt$noinline$testFillArrayData() load_store_elimination (after)
  /// CHECK-DAG: <<ConstPos:i\d+>>  IntConstant 40960
  /// CHECK-DAG:                    InvokeStaticOrDirect [<<ConstPos>>,<<ConstPos>>]
  //
  /// CHECK-NOT:                    IntConstant -24576
  public static void $opt$noinline$testFillArrayData() {
    // Use an array initializer to hint the use of filled-new-array.
    //
    //  - During instruction building of fill-array-data the compiler only knows the array element data size,
    //    not whether it is signed or unsigned. Thus for the char case ArraySets with type kInt16 are created;
    //    'IntConstant -24575' is used for initialization instead of 'IntConstant 40960'.
    //  - LoadStoreElimination pass replaces uses of a[0] with negative IntConstant which is incorrect.
  ...
  public static void main(String[] args) {
    ...
    $opt$noinline$testFillArrayData();
  }
