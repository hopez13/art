...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    // Single character.
    assertStringEquals("a", $noinline$stringGetCharsAndBack("a"));
    // Strings < 8 characters.
    assertStringEquals("foobar", $noinline$stringGetCharsAndBack("foobar"));
    // Strings > 8 characters of various lengths.
    assertStringEquals(smallString, $noinline$stringGetCharsAndBack(smallString));
    assertStringEquals(mediumString, $noinline$stringGetCharsAndBack(mediumString));
    assertStringEquals(largeString, $noinline$stringGetCharsAndBack(largeString));

    // Get only a substring:
    // Substring < 8 characters.
    assertStringEquals(smallString.substring(5, 10),
        $noinline$stringGetCharsRange(smallString, 5, 10, 0));
    // Substring > 8 characters.
    assertStringEquals(smallString.substring(7, 28),
        $noinline$stringGetCharsRange(smallString, 7, 28, 0));

    // Get full string with offset in the char array.
    assertStringEquals(smallString, $noinline$stringGetCharsAndBackOffset(smallString, 17));

    // Get a substring with an offset in the char array.
    // Substring < 8 characters.
    assertStringEquals(smallString.substring(5, 10),
        $noinline$stringGetCharsRange(smallString, 5, 10, 17));
    // Substring > 8 characters.
    assertStringEquals(smallString.substring(7, 28),
        $noinline$stringGetCharsRange(smallString, 7, 28, 17));
  ...
  public static String $noinline$stringGetCharsAndBack(String src) {
    char[] dst = new char[src.length()];
    src.getChars(0, src.length(), dst, 0);
    return new String(dst);
  }

  public static String $noinline$stringGetCharsAndBackOffset(String src, int offset) {
    char[] dst = new char[src.length() + offset];
    src.getChars(0, src.length(), dst, offset);
    return new String(dst, offset, src.length());
  }

  public static String $noinline$stringGetCharsRange(String src, int srcBegin,
      int srcEnd, int offset) {
