...
public class Monitors {
    public native static void setupMonitorEvents(Class<?> method_klass,
            Method monitor_contended_enter_event, Method monitor_contended_entered_event,
            Method monitor_wait_event, Method monitor_waited_event, Class<?> lock_klass,
            Thread thr);
    ...
    public static class NamedLock {
        ...
        public String toString() { return String.format("NamedLock[%s]", name); }
    ...
    public static final class MonitorUsage {
        ...
        public MonitorUsage(Object monitor, Thread owner, int entryCount, Thread[] waiters,
        ...
        public String toString() {
            return String.format(
                    ...
                    monitor, (owner != null) ? owner.getName() : "<NULL>", entryCount,
                    toNameList(waiters), toNameList(notifyWaiters));
        }
    }
    ...
    public static class LockController {
        ...
        public LockController(NamedLock lock) { this(lock, 10 * 1000); }
        ...
        public boolean IsWorkerThread(Thread thd) { return Objects.equals(runner, thd); }
                    ...
                    synchronized (lock) {
                        ...
                        try {
                            ...
                            throw new TestException(
                                    "Got an error while performing action " + cur_action, e);
                        }
                    }
                } finally {
                    held = false;
        ...
        public void waitForLockToBeHeld() throws Exception {
            while (true) {
                if (IsLocked()
                        && Objects.equals(runner, Monitors.getObjectMonitorUsage(lock).owner)) {
                    return;
                }
            }
        }
        ...
        public synchronized void waitForNotifySleep() throws Exception {
            ...
            do {
                checkException();
                    ...
                    || !Arrays.asList(Monitors.getObjectMonitorUsage(lock).notifyWaiters)
                                .contains(runner));
        }
        ...
        public synchronized void waitForContendedSleep() throws Exception {
            ...
            do {
                checkException();
                    ...
                    || !Arrays.asList(Monitors.getObjectMonitorUsage(lock).waiters)
                                .contains(runner));
        }
        ...
        public synchronized void DoUnlock() throws Exception {
            Error throwing = null;
            if (!IsLocked()) {
                // We might just be racing some exception that was thrown by the worker thread.
                // Cache the exception, we will throw one from the worker before this one.
            ...
            // Make sure to throw any exception that occurred since it might not have unlocked due
            // to our request.
            checkException();
            DoCleanup();
            if (throwing != null) {
