...
public class Locals {
    ...
    public static class VariableDescription {
        ...
        public String toString() {
            return String.format("VariableDescription { "
                            + "Sig: '%s', Name: '%s', Gen_sig: '%s', slot: %d, start: %d, len: %d"
                            + "}",
                    this.signature, this.name, this.generic_signature, this.slot,
        ...
        public boolean equals(Object other) {
            if (!(other instanceof VariableDescription)) {
                ...
                return Objects.equals(v.signature, signature) && Objects.equals(v.name, name)
                        && Objects.equals(v.generic_signature, generic_signature) && v.slot == slot
                        && v.start_location == start_location && v.length == length;
            }
        }
        public int hashCode() {
    ...
    public static VariableDescription GetVariableAtLocation(
        ...
        for (VariableDescription var : vars) {
            if (var.start_location <= loc && var.length + var.start_location > loc
                    && var.name.equals(name) && var.signature.equals(sig)) {
                return var;
            }
        }
        throw new Error("Unable to find variable " + name + " (sig: " + sig + ") in " + e
                + " at loc " + loc);
    }
