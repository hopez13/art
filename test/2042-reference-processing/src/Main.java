 ...
 *
 * We maintain a list of nominally LIVE_OBJS finalizable objects.
 ...
 */
public class Main {
    static final int LIVE_OBJS = 150;
    ...
    // Highest number of object that we've allocated so far.
    int nextAllocated = 0;
    int maxDropped = 0;
    int liveObjects = 0;
    ...
    class FinalizableObject {
        ...
        protected void finalize() {
            ...
            if (!finalized.offer(n) && REPORT_DROPS) {
                System.out.println("Dropped finalization of of " + n);
            }
        }
    }
    ...
    FinalizableObject listHead;  // List is in decending order.
    ...
    void testLoop() {
        ...
        addObjects(LIVE_OBJS - DROP_OBJS);
        final int ITERS = (TOTAL_OBJS - (LIVE_OBJS - DROP_OBJS)) / DROP_OBJS;
        for (int i = 0; i < ITERS; ++i) {
            addObjects(DROP_OBJS);
            if (liveObjects != LIVE_OBJS) {
                System.out.println("Unexpected live object count");
            }
            dropObjects(DROP_OBJS);
            emptyAndCheckQueues();
        }
        dropObjects(LIVE_OBJS - DROP_OBJS);
