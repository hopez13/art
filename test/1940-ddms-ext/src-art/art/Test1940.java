...
public class Test1940 {
    ...
    private static boolean chunkEq(Chunk c1, Chunk c2) {
        ...
        return a.type() == b.type() && a.offset() == b.offset() && a.length() == b.length()
    ...
    private static String printChunk(Chunk k) {
        ...
        return String.format(
                "Chunk(Type: 0x%X, Len: %d, data: %s)", c.type(), c.length(), Arrays.toString(out));
    }
    ...
    private static final class MyDdmHandler extends ChunkHandler {
        ...
        public Chunk handleChunk(Chunk req) {
            ...
            if (req.type == MY_DDMS_TYPE) {
                ...
                return new Chunk(
                        MY_DDMS_RESPONSE_TYPE, new byte[] {0}, /*offset*/ 12, /*length*/ 55);
            } else {
    ...
    private static class ChunkWrapper {
        ...
        ChunkWrapper(Chunk c) { this.c = c; }

        int type() { return c.type; }
    ...
    public static void HandlePublish(int type, byte[] data) throws Exception {
        if (PRINT_ALL_CHUNKS) {
            System.out.println("Unknown Chunk published: "
                    + printChunk(new Chunk(type, data, 0, data.length)));
        }
    ...
    public static void run() throws Exception {
        DdmHandler BaseHandler = (type, data) -> {
            System.out.println(
                    "Chunk published: " + printChunk(new Chunk(type, data, 0, data.length)));
        };
        CURRENT_HANDLER = BaseHandler;
        initializeTest();
        Method publish = Test1940.class.getDeclaredMethod(
                "HandlePublish", Integer.TYPE, new byte[0].getClass());
        ...
        data = new byte[] {0x1};
        System.out.println("Sending data " + Arrays.toString(data) + " to chunk handler "
                + MY_EMPTY_DDMS_TYPE);
        ...
        AwaitChunkHandler wait_thd = new AwaitChunkHandler(
                (x) -> types_seen[0] && types_seen[1] && types_seen[2], (type, cdata) -> {
                    switch (type) {
                        case TYPE_THCR:
                            types_seen[0] = true;
                            break;
        ...
        // method Tracing
        AwaitChunkHandler mpse =
                new AwaitChunkHandler((x) -> x.type == TYPE_MPSE, (type, cdata) -> {
