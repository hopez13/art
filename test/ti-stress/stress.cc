...
class ScopedFieldInfo {
 public:
  ScopedFieldInfo(jvmtiEnv* jvmtienv, JNIEnv* env, jclass field_klass, jfieldID field)
      : jvmtienv_(jvmtienv),
        env_(env),
...
void JNICALL FieldAccessHook(jvmtiEnv* jvmtienv,
  ...
  ScopedMethodInfo method_info(jvmtienv, env, m);
  ScopedFieldInfo field_info(jvmtienv, env, field_klass, field);
...
#undef SEND_VALUE
  ...
  LOG(INFO) << "MODIFY field \"" << field_info << "\" on object of "
            ...
            << oss.str() << ". Thread is \"" << info.GetName() << "\".";
  env->DeleteLocalRef(oklass);
}
