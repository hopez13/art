...
public class Main {
    ...
    public static void reportValue(Object val) { System.out.println("\tValue is '" + val + "'"); }
    ...
    public static class IntRunner implements Runnable {
        ...
        public void run() {
            ...
            // Wait up to 300 seconds for OSR to kick in if we expect it. If we don't give up after
            // only 3 seconds.
            Instant osrDeadline = Instant.now().plusSeconds(expectOsr ? 600 : 3);
            do {
                // Don't actually do anything here.
                inBusyLoop = true;
            } while (normalJit && !Main.isInOsrCode("run")
                    && osrDeadline.compareTo(Instant.now()) > 0);
            // We shouldn't be doing OSR since we are using JVMTI and the set prevents OSR.
            // Set local will also push us to interpreter but the get local may remain in compiled
            // code.
            if (normalJit) {
                boolean inOsr = Main.isInOsrCode("run");
                if (expectOsr && !inOsr) {
                    throw new Error("Expected to be in OSR but was not. interpreter: "
                            + Main.isInterpreted());
                } else if (!expectOsr && inOsr) {
                    throw new Error("Expected not to be in OSR but was. interpreter: "
                            + Main.isInterpreted());
                }
            }
            reportValue(TARGET);
    ...
    public static void runGet() throws Exception {
        ...
        try {
            ...
            if (depth != 0) {
                throw new Error("Expected depth 0 but got " + depth);
            }
