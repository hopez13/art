...
public class Test905 {
    ...
    public static class GcMarker {
        ...
        public void add(Object referent) { mList.add(new PhantomReference(referent, mQueue)); }
        public void waitForGc() { waitForGc(mList.size()); }
    ...
    private static void stress(int allocations) {
        ...
        // Allocate objects, Save every other one. We want to be sure that it's only the deleted
        // objects that get their tags cleared and non-deleted objects correctly keep track of their
        // tags.
        ...
        for (int i = 0; i < saved.length; i++) {
            ...
            if (Arrays.binarySearch(freedTags1, expectedTag1) >= 0) {
                System.out.println(
                        "Saved object was marked as deleted in env 1. Object was " + expectedTag1);
            }
