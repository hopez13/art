...
public class Test905 {
  ...
  private static void stress() {
    ...
    // Allocate objects, Save every other one.
    ...
    for (int i = 0; i < saved.length; i++) {
      ...
      if (Main.getTag(saved[i]) != expectedTag1) {
        System.out.println("Saved object has unexpected tag in env 1. Expected " + expectedTag1 + " got " + Main.getTag(saved[i]));
      }
      if (getTag2(saved[i]) != 1 + expectedTag1) {
        System.out.println("Saved object has unexpected tag in env 2. Expected " + (expectedTag1 + 1) + " got " + getTag2(saved[i]));
      }
      if (Arrays.binarySearch(freedTags1, expectedTag1) >= 0) {
        System.out.println("Saved object was marked as deleted in env 1. Object was " + expectedTag1);
      }
      if (Arrays.binarySearch(freedTags2, expectedTag1 + 1) >= 0) {
        System.out.println("Saved object was marked as deleted in env 2. Object was " + (expectedTag1 + 1));
      }
    }
  }
