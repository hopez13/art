...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            Class<?> otherMain = Class.forName("Main");
            Method m = otherMain.getDeclaredMethod("testClassLoading");
            m.invoke(null);
        } catch (Throwable t) {
    ...
    public static void testClassLoading() {
        ...
        for (String signature : bad_class_names) {
            try {
                getFieldId(TestClass.class, "bogus", signature);
                System.out.println("FAIL!");
            } catch (NoSuchFieldError nsfe) {
                if (!(nsfe.getCause() instanceof NoClassDefFoundError)) {
                  throw nsfe;
                }
                NoClassDefFoundError ncdfe = (NoClassDefFoundError) nsfe.getCause();
