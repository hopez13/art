...
public class TestFloatDouble {
  ...
  /// CHECK-START: float other.TestFloatDouble.testDotProdSimpleFloat(float[], float[]) loop_optimization (before)
  /// CHECK-DAG: <<Const0:f\d+>>  FloatConstant 0                                           loop:none
  /// CHECK-DAG: <<Const1:i\d+>>  IntConstant 1                                           loop:none
  /// CHECK-DAG: <<Const2:i\d+>>  IntConstant 0                                           loop:none
  /// CHECK-DAG: <<Phi1:f\d+>>    Phi [<<Const0>>,{{f\d+}}]                               loop:<<Loop:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Phi2:i\d+>>    Phi [<<Const2>>,{{i\d+}}]                               loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Get1:f\d+>>    ArrayGet [{{l\d+}},<<Phi2>>]                            loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Get2:f\d+>>    ArrayGet [{{l\d+}},<<Phi2>>]                            loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Mul:f\d+>>     Mul [<<Get1>>,<<Get2>>]                                 loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  Add [<<Phi1>>,<<Mul>>]                                  loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  Add [<<Phi2>>,<<Const1>>]                               loop:<<Loop>>      outer_loop:none

  /// CHECK-START-{X86_64}: float other.TestFloatDouble.testDotProdSimpleFloat(float[], float[]) loop_optimization (after)
  /// CHECK-DAG: <<Const0:f\d+>>  FloatConstant 0                                         loop:none
  /// CHECK-DAG: <<IConst0:i\d+>> IntConstant 0                                           loop:none
  /// CHECK-DAG: <<Const1:i\d+>>  IntConstant 1                                           loop:none
  /// CHECK-DAG: <<Const4:i\d+>>  IntConstant 4                                           loop:none
  /// CHECK-DAG: <<Set:d\d+>>     VecSetScalars [<<Const0>>]                              loop:none
  /// CHECK-DAG: <<Phi1:i\d+>>    Phi [<<IConst0>>,{{i\d+}}]                               loop:<<Loop:B\d+>> outer_loop:none
  ...
  /// CHECK-DAG:                  VecDotProd [<<Phi2>>,<<Load1>>,<<Load2>>] type:Float32  loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  Add [<<Phi1>>,<<Const4>>]                               loop:<<Loop>>      outer_loop:none
  //
  ...
  /// CHECK-START: double other.TestFloatDouble.testDotProdSimpleDouble(double[], double[]) loop_optimization (before)
  /// CHECK-DAG: <<Const0:d\d+>>  DoubleConstant 0                                           loop:none
  /// CHECK-DAG: <<Const1:i\d+>>  IntConstant 1                                           loop:none
  /// CHECK-DAG: <<Const2:i\d+>>  IntConstant 0                                           loop:none
  /// CHECK-DAG: <<Phi1:d\d+>>    Phi [<<Const0>>,{{d\d+}}]                               loop:<<Loop:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Phi2:i\d+>>    Phi [<<Const2>>,{{i\d+}}]                               loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Get1:d\d+>>    ArrayGet [{{l\d+}},<<Phi2>>]                            loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Get2:d\d+>>    ArrayGet [{{l\d+}},<<Phi2>>]                            loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Mul:d\d+>>     Mul [<<Get1>>,<<Get2>>]                                 loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  Add [<<Phi1>>,<<Mul>>]                                  loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  Add [<<Phi2>>,<<Const1>>]                               loop:<<Loop>>      outer_loop:none
  ...
  /// CHECK-DAG: <<IConst0:i\d+>> IntConstant 0                                           loop:none
  /// CHECK-DAG: <<Const1:i\d+>>  IntConstant 1                                           loop:none
  /// CHECK-DAG: <<Const4:i\d+>>  IntConstant 2                                          loop:none
  /// CHECK-DAG: <<Set:d\d+>>     VecSetScalars [<<Const0>>]                              loop:none
  /// CHECK-DAG: <<Phi1:i\d+>>    Phi [<<IConst0>>,{{i\d+}}]                               loop:<<Loop:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Phi2:d\d+>>    Phi [<<Set>>,{{d\d+}}]                                  loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Load1:d\d+>>   VecLoad [{{l\d+}},<<Phi1>>]                             loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Load2:d\d+>>   VecLoad [{{l\d+}},<<Phi1>>]                             loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  VecDotProd [<<Phi2>>,<<Load1>>,<<Load2>>] type:Float64  loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                  Add [<<Phi1>>,<<Const4>>]                               loop:<<Loop>>      outer_loop:none
  //
  /// CHECK-DAG: <<Reduce:d\d+>>  VecReduce [<<Phi2>>]                                    loop:none
  /// CHECK-DAG:                  VecExtractScalar [<<Reduce>>]                           loop:none
  ...
  public static void run() {
    ...
    for (int i = 0; i != 1024; ++i) a[i] = MAX_F;
