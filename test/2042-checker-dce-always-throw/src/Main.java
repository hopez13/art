...
public class Main {
  ...
  // testDoNotSimplify wants to test that we have the necessary conditions for us to simplify the
  // always throws (e.g. InvokeStaticOrDirect followed by a Goto) but we are blocking this due to
  // being in a try. Changing the Goto here for the exit would be wrong since we want to flow to the
  // catch rather than the Exit. The preconditions are tricky to do with just one function (since we
  // will have an invoke followed by a TryBoundary rather than a Goto) but we can do so with the
  // help of the inliner.
  ...
  // it. Like in testDoNotSimplifyInTry, we need the help of the inliner to have an invoke followed by a Goto.
