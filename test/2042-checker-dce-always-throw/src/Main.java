...
public class Main {
    ...
    private static void alwaysThrows() throws Error { throw new Error(""); }
    ...
    // Tests that we have the necessary conditions for us to simplify the always throwing
    // instruction (e.g. InvokeStaticOrDirect followed by a Goto) but we are blocking this due to
    // being in a try. Changing the Goto here for the exit would be wrong since we want to flow to
    // the catch rather than the Exit. The preconditions are tricky to do with just one function
    // (since we will have an invoke followed by a TryBoundary rather than a Goto) but we can do so
    // with the help of the inliner.
    ...
    // We are able to simplify the `alwaysThrows` even though we are inside of the catch { ... }
    // since the if makes it so that we are not the first block of the catch and therefore not in
    // the "catch_block".
    ...
    // Similar to testSimplifyInCatch, but now the throw is in an outer try and we shouldn't
    // simplify it. Like in testDoNotSimplifyInTry, we need the help of the inliner to have an
    // invoke followed by a Goto.
    private static int $noinline$testDoNotSimplifyInCatchInOuterTry(int num, int other_num) {
        try {
            try {
                ...
                if (num == 0) {
                    // We use `other_num` here because otherwise we propagate the knowledge that
                    // `num` equals zero.
    ...
    // Check that when we perform SimplifyAlwaysThrows, that the phi for `phi_value` exists, and
    // that we correctly update it after running DCE.
    ...
    private static int $noinline$testUpdatePhisCorrectly(int num, int other_num) {
        ...
        if (num == 0) {
            ...
            // just one. We use `other_num` here because otherwise we propagate the knowledge that
            // `num` equals zero.
            while (other_num == 0) {
    ...
    private static int $noinline$foo(int num) { return num; }
    ...
    /// CHECK-START: int Main.$noinline$testDeleteAllUsesBeforeDeletingInstruction(int, int) dead_code_elimination$after_inlining (after)
    /// CHECK-NOT:   Phi
    private static int $noinline$testDeleteAllUsesBeforeDeletingInstruction(
            int num, int other_num) {
        int phi_value = 0;
        if (num == 0) {
            alwaysThrows();

            // This while loop is here so that we have a Goto after `alwaysThrows` and therefore
            // perform the `SimplifyAlwaysThrows` optimization. We use `other_num` here because
            // otherwise we propagate the knowledge that `num` equals zero.
            while (other_num == 0) {
                // Assign to phi_value so that the loop is not empty.
                phi_value = 2;
            }
