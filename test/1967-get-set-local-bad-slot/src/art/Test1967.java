...
public class Test1967 {
    ...
    public static interface SafepointFunction {
        public void invoke(Thread thread, Method target, Locals.VariableDescription TARGET_desc,
    ...
    public static SafepointFunction BadSet(final String type, final SetterFunction get,
            final Object v, final ToIntFunction<Integer> transform) {
        return new SafepointFunction() {
            public void invoke(
                    Thread t, Method method, Locals.VariableDescription desc, int depth) {
                int real_slot = transform.applyAsInt(desc.slot);
                try {
                    get.SetVar(t, depth, real_slot, v);
                    System.out.println(this + " on " + method + " set value: " + v);
                } catch (Exception e) {
                    System.out.println(this + " on " + method + " failed to set value " + v
                            + " due to " + e.getMessage());
                }
            }
    ...
    public static class TestCase {
        ...
        public TestCase(Method target) { this.target = target; }
        ...
        public static class ThreadPauser implements Runnable {
            ...
            public void waitForOtherThreadToPause() throws Exception { sem_wakeup_main.acquire(); }

            public void wakeupOtherThread() throws Exception { sem_wait.release(); }
        }
    ...
    public static void run() throws Exception {
        ...
        final SafepointFunction[] SAFEPOINTS = new SafepointFunction[] {
                ...
                BadSet("Int_at_too_high", Locals::SetLocalVariableInt, Integer.MAX_VALUE,
                        (i) -> i + 100),
                BadSet("Long_at_too_high", Locals::SetLocalVariableLong, Long.MAX_VALUE,
                        (i) -> i + 100),
                BadSet("Object_at_too_high", Locals::SetLocalVariableObject, "NEW_FOR_SET",
                        (i) -> i + 100),
                ...
                BadSet("Int_at_negative", Locals::SetLocalVariableInt, Integer.MAX_VALUE,
                        (i) -> - 1),
                BadSet("Long_at_negative", Locals::SetLocalVariableLong, Long.MAX_VALUE,
                        (i) -> - 1),
                BadSet("Object_at_negative", Locals::SetLocalVariableObject, "NEW_FOR_SET",
                        (i) -> - 1),
        };
