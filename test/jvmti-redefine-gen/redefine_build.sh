...
mkdir -p src/test_${NUMBER}_transforms/
...
find ./redefine_targets -type f | while read jva; do
  res=src/test_${NUMBER}_transforms/$(basename $jva)
  $ANDROID_BUILD_TOP/art/test/jvmti-redefine-gen/redefine-gen.py -o $res test_${NUMBER}_transforms.$(basename ${jva} | cut -d'.' -f1) /dev/null /dev/null
...
find ./redefine_targets -type f | while read jva; do
  ...
  res=src/test_${NUMBER}_transforms/$(basename $jva)
  $ANDROID_BUILD_TOP/art/test/jvmti-redefine-gen/redefine-gen.py -o $res test_${NUMBER}_transforms.$(basename ${jva} | cut -d'.' -f1) $class_file $dex_file
  echo BUILD FINAL $res
done

# Clear out the old jar and dex files
for d in *.{dex,jar}; do
  # We don't care if we fail due to missing files (eg due to --jvm).
  (mv $d PRE_REDEFINE_$d && echo mv $d PRE_REDEFINE_$d || true)
done
