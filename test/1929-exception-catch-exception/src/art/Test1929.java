...
public class Test1929 {
    ...
    static {
        try {
            TEST_METHODS = Arrays.asList(Test1929.class.getDeclaredMethod("doThrow"),
    ...
    private static void PrintStack() {
        ...
        for (StackTrace.StackFrameData e : StackTrace.GetStackTrace(Thread.currentThread())) {
            if (Objects.equals(
                        e.method.getDeclaringClass().getPackage(), Test1929.class.getPackage())) {
                System.out.println("\t\t" + e.method
                        + " @ line = " + Breakpoint.locationToLine(e.method, e.current_location));
            }
        }
    }
    ...
    public static void ExceptionCatchEvent(
        ...
        System.out.println(thr.getName() + ": " + method
                + " @ line = " + Breakpoint.locationToLine(method, location) + " caught "
    ...
    public static class ThrowCatchBase implements ExceptionHandler {
        public void exceptionOccurred(Executable m, long loc, Throwable exception) {
            ...
            try {
                throw new BaseTestException("ThrowBaseHandler during throw from " + m
                                + " @ line = " + Breakpoint.locationToLine(m, loc),
                        exception);
            } catch (BaseTestException t) {
                System.out.println(
                        "Caught " + t.getClass().getName() + ": \"" + t.getMessage() + "\"");
                if (PRINT_FULL_EXCEPTION) {
    ...
    public static class ThrowTestExceptionNoRethrowHandler implements ExceptionHandler {
        public void exceptionOccurred(Executable m, long loc, Throwable exception) {
            if (exception instanceof TestExceptionNoRethrow) {
                System.out.println(
                        "\tInstance of TestExceptionNoRethrow was thrown. Not throwing again.");
            } else {
                System.out.println("\tThrowing TestExceptionNoRethrow!");
                throw new TestExceptionNoRethrow(
                        "ThrowTestExceptionNoRethrowHandler during throw from " + m
                                + " @ line = " + Breakpoint.locationToLine(m, loc),
                        exception);
            }
        }
    }
    public static void doThrow() { throw new TestException("doThrow"); }
    ...
    public static void throwCatchBaseTestExceptionTwice() {
        // The implementation of this has to change depending upon the runtime slightly due to
        // compiler optimizations present in DX/D8/Jack.
        try {
    ...
    public static void run() throws Exception {
        // Set up breakpoints
        Exceptions.setupExceptionTracing(Test1929.class, TestException.class, null,
                Test1929.class.getDeclaredMethod("ExceptionCatchEvent", Thread.class,
            ...
            for (Runnable test : tests) {
                try {
                    ...
                    System.out.printf("Test \"%s\": Caught error %s:\"%s\" with handler \"%s\"\n",
                            test.getClass().getName(), e.getClass().getName(), e.getMessage(),
