...
$(warning $(call name-to-var,$(1))_CORE_IMAGE_$(3)_pic_$(4))
  ifeq ($(2),no-image)
...
define define-test-art-run-test-group
  ifeq ($(2),$(filter $(2),$(ADDRESS_SIZES)))
    build_target := test-art-$(1)-run-test$(2)
  else
    build_target := test-art-$(1)-run-test$-$(2)
  endif

  ps_data := $$(shell ps aux | grep $$(build_target) )
  regex := -j\s*[0-9]*
  jobs_flag := $$(shell [[ "$$(ps_data)" =~ $$(regex) ]] && echo $$$$BASH_MATCH || echo -j1)
  .PHONY: $$(build_target)
  $$(build_target) : test-art-$(1)-run-test-dependencies
			./art/test/testrunner/testrunner.py --$(1) --(2) --verbose $$(jobs_flag)
  build_target :=
  ps_data :=
  jobs_flag :=
  regex :=
...
define define-test-art-host-or-target-run-test-group
  ...
  .PHONY: $$(build_target)
  $$(build_target) : test-art-$(1)-run-test-dependencies
			./art/test/testrunner/testrunner.py --$(1) --verbose $$(jobs_flag)
  build_target :=
  ps_data :=
  jobs_flag :=
  regex :=
