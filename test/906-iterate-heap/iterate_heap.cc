...
extern "C" JNIEXPORT void JNICALL Java_Main_iterateThroughHeapCount(JNIEnv* env ATTRIBUTE_UNUSED,
  ...
  std::thread test(Run, heap_filter, klass_filter, &config);
  test.join();
...
//   std::thread test = std::thread([heap_filter, klass_filter, &config] { 
//     Run(heap_filter, klass_filter, &config);
//   });
//   test.join();
...
//   std::thread test = std::thread([heap_filter, klass_filter] { 
