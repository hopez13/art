...
public class Test906 {
    ...
    // Number of times we will try to count the heap in various ways. If we are unlucky and end up
    // in the middle of a GC we could incorrectly fail. This is expected to be incredibly rare so 10
    ...
    private static void testHeapCount() throws Exception {
        ...
        for (int i = 0; i < ITERATE_RETRIES; i++) {
            try {
                ...
                int tagged =
                        iterateThroughHeapCount(HEAP_FILTER_OUT_UNTAGGED, null, Integer.MAX_VALUE);
                int untagged =
                        iterateThroughHeapCount(HEAP_FILTER_OUT_TAGGED, null, Integer.MAX_VALUE);
                int taggedClass = iterateThroughHeapCount(
                        HEAP_FILTER_OUT_CLASS_UNTAGGED, null, Integer.MAX_VALUE);
                int untaggedClass = iterateThroughHeapCount(
                        HEAP_FILTER_OUT_CLASS_TAGGED, null, Integer.MAX_VALUE);
                int filteredClass = iterateThroughHeapCount(0, Foobar.class, Integer.MAX_VALUE);

                if (all != tagged + untagged) {
                    throw new IllegalStateException(
                            "Instances: " + all + " != " + tagged + " + " + untagged);
                }
                if (all != taggedClass + untaggedClass) {
                    throw new IllegalStateException(
                            "By class: " + all + " != " + taggedClass + " + " + untaggedClass);
                }
                if (filteredClass != foobars.length) {
                    throw new IllegalStateException("Missed objects of foobar type. "
                            + filteredClass + " != " + foobars.length);
                }
                if (tagged != 6) {
    ...
    public static void doTest() throws Exception {
        ...
        int n = iterateThroughHeapData(
                HEAP_FILTER_OUT_UNTAGGED, null, classTags, sizes, tags, lengths);
    ...
    static class HeapElem implements Comparable<HeapElem> {
        ...
        public String toString() {
            return "{tag=" + tag + ", class-tag=" + classTag
                    + ", size=" + (tag >= 100 ? "<class>" : size) // Class size is dependent on
                                                                  // 32-bit vs 64-bit, so strip it.
                    + ", length=" + length + "}";
        }
    }

    private static ArrayList<HeapElem> sort(
            int n, long classTags[], long sizes[], long tags[], int lengths[]) {
    ...
    private static void setTag(Object o, long tag) { Main.setTag(o, tag); }
    private static long getTag(Object o) { return Main.getTag(o); }
    ...
    private static native int iterateThroughHeapCount(
            int heapFilter, Class<?> klassFilter, int stopAfter);
    private static native int iterateThroughHeapData(int heapFilter, Class<?> klassFilter,
            long classTags[], long sizes[], long tags[], int lengths[]);
    private static native int iterateThroughHeapAdd(int heapFilter, Class<?> klassFilter);
