...
public class Main {
  ...
  private static void doFollowReferencesTestNonRoot(ArrayList<Object> tmpStorage) {
    A a = createTree();
    tmpStorage.add(a);
    doFollowReferencesTestImpl(null, Integer.MAX_VALUE, -1);
    doFollowReferencesTestImpl(a, Integer.MAX_VALUE, -1);
    tmpStorage.clear();
  }

  private static void doFollowReferencesTestImpl(A root, int stopAfter, int followSet) {
    followReferences(0, null, root == null ? null : root.foo, stopAfter, followSet);
    // TODO: Test filters.
  }
  ...
  private static native int followReferences(int heapFilter, Class<?> klassFilter,
      Object initialObject, int stopAfter, int followSet);
}
