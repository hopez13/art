...
public class Test1949 {
    ...
    //
    // In the transformed version of the dex file there is a new method. The new list of methodIDs
    // is:
    ...
    // alphabet (package xyz, method foo). If it is further forward than the other method-ids then
    // the JIT will read an incorrect (but valid) method-id from the old-dex file. This is why the
    // error wasn't caught in our other tests (package art is always at the front).
    //
    // The final method that causes the OOB read needs to be a native method because that is the
    // only method-type the jit uses dex-file information to keep track of.
    ...
    private static ClassLoader CreateClassLoader(byte[] clz, byte[] dex) throws Exception {
        if (isDalvik) {
            ...
            Constructor<?> ctor =
                    class_loader_class.getConstructor(ByteBuffer.class, ClassLoader.class);
            /* on Dalvik, this is a DexFile; otherwise, it's null */
            return (ClassLoader) ctor.newInstance(
                    ByteBuffer.wrap(dex), Test1949.class.getClassLoader());
        } else {
