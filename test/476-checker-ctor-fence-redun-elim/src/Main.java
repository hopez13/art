...
public class Main {
  public static void main(String[] args) throws Exception {
    ...
    // by actually running the code and printing out the resulting side effects.
    // All prints are defined to be deterministic.
    ...
    Class<?>[] testClasses = new Class<?>[] {
      ...
      TestNonEscaping.class,
      TestDontOptimizeAcrossEscape.class,
      TestDontOptimizeAcrossEscape.class
    };
    ...
    // Call exercise() for each class.
    // Then print all of the objects.

    // Note: MakeBoundType is special and we have to call the constructor directly
    // to prevent inlining it.
    Object inst = exerciseNewInstance(TestDontOptimizeAcrossEscape.MakeBoundType.class, 123);
    System.out.println(valueToString(inst));

    Object inst2 = exerciseNewInstance(TestDontOptimizeAcrossEscape.MakeBoundTypeSub.class, 123);
    System.out.println(valueToString(inst2));
  }

  // For every public static method
  // *) Invoke it
  // *) Print all static fields values
  private static void exerciseTestClass(Class<?> klass) throws Exception {
    for (Method method : klass.getDeclaredMethods()) {
      if (java.lang.reflect.Modifier.isStatic(method.getModifiers())
        ...
        System.out.println("Static field values after test:");

        // Print all static fields
        for (Field field : fields) {
          if (Modifier.isStatic(field.getModifiers())) {
            try {
              System.out.print(field.getName());
              System.out.print(": ");
              System.out.println(valueToString(field.get(null)));
