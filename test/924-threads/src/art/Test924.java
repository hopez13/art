...
public class Test924 {
    ...
    private static void doStateTests(Function<Runnable, Thread> mkThread) throws Exception {
        ...
        // Waiting.
        cdl1.await();
        // This is super inconsistent so just wait for the desired state for up to 5 minutes then
        ...
        synchronized (cdl3_2) {
            ...
            // Since internal thread suspension (For GC or other cases) can happen at any time and
            // changes the thread state we just have it print the majority thread state across 11
            // calls over 55 milliseconds.
    ...
    private static void doAllThreadsTests() {
        ...
        if (!threadList.containsAll(expectedList)) {
            throw new RuntimeException(
                    "Expected " + expectedList + " as subset, got " + threadList);
        }
    ...
    // Call getThreadState 'votes' times waiting 'wait' millis between calls and print the most
    // common result.
    ...
    private static void printThreadState(Thread t) { printThreadState(getThreadState(t)); }
    ...
    private static void printThreadInfo(Thread t) {
        ...
        System.out.println(
                threadInfo[4] == null ? "null" : threadInfo[4].getClass()); // Context CL.
    }

    public static final class NativeWaiter {
        public long struct;
        public NativeWaiter() { struct = nativeWaiterStructAlloc(); }
