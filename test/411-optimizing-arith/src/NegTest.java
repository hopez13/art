...
public class NegTest {
    ...
    private static void negLong() {
        ...
        assertEquals(
                9223372036854775807L, $opt$noinline$NegLong(-9223372036854775807L)); // -(2^63 - 1)
        assertEquals(
                -9223372036854775807L, $opt$noinline$NegLong(9223372036854775807L)); // 2^63 - 1
        // See remark regarding the negation of the maximum negative
        // (long) value in negInt().
        assertEquals(
                -9223372036854775808L, $opt$noinline$NegLong(-9223372036854775808L)); // -(2^63)
    }
    ...
    private static int $opt$noinline$NegInt(int a) { return -a; }

    private static long $opt$noinline$NegLong(long a) { return -a; }

    private static float $opt$noinline$NegFloat(float a) { return -a; }

    private static double $opt$noinline$NegDouble(double a) { return -a; }
}
