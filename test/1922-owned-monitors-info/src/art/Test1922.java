...
public class Test1922 {
    // Set to true to run with all combinations of locks. This isn't really needed for the test to
    // be useful and fully representative.
    ...
    public static class Target {
        ...
        public synchronized void lockSync(Runnable safepoint) { safepoint.run(); }
        ...
        public void callSafepoint(Runnable safepoint) { safepoint.run(); }
    }
    ...
    public static class PrintOwnedMonitorsStackDepthRunnable implements Runnable {
        ...
        public PrintOwnedMonitorsStackDepthRunnable(Thread target) { this.target = target; }
        public void run() {
    ...
    public static class PrintOwnedMonitorsRunnable implements Runnable {
        public final Thread target;
        public PrintOwnedMonitorsRunnable(Thread target) { this.target = target; }
        public void run() {
            System.out.println(
                    "Owned monitors: " + Arrays.toString(sortByString(getOwnedMonitors(target))));
        }
    }
    ...
    public static void run() throws Exception {
        ...
        runTestsCurrentThread(
                "owned-monitor", new PrintOwnedMonitorsRunnable(Thread.currentThread()));

        System.out.println("owner-monitors, no suspend, Other thread");
        runTestsOtherThread(
                "owned-monitor", false, (t) -> { new PrintOwnedMonitorsRunnable(t).run(); });

        System.out.println("owner-monitors, suspend, Other thread");
        runTestsOtherThread(
                "owned-monitor", true, (t) -> { new PrintOwnedMonitorsRunnable(t).run(); });
    ...
    public static void runTestsOtherThread(String name, boolean suspend, Consumer<Thread> printer) {
        ...
        List<Function<Runnable, Runnable>> MkSafepoints =
                Arrays.asList((r)
                                      -> { return new CallLockOther(t, l1, r); },
                        (r)
                                -> { return new CallLockExtra(t, l2, r); },
                        (r)
                                -> { return new CallLockNative(t, l3, r); },
                        (r)
                                -> { return new CallLockThisNative(t, r); },
                        (r)
                                -> { return new CallLockClass(t, r); },
                        (r)
                                -> { return new CallLockSync(t, r); },
    ...
    public static class CallSafepoint implements Runnable {
        ...
        public void run() { target.callSafepoint(safepoint); }
    }
    public static class CallLockOther implements Runnable {
        ...
        public void run() { target.lockOther(l, safepoint); }
    }
    public static class CallLockExtra implements Runnable {
        ...
        public void run() { target.lockExtra(l, safepoint); }
    }
    public static class CallLockThisNative implements Runnable {
        ...
        public void run() { target.lockThisNative(safepoint); }
    }
    public static class CallLockNative implements Runnable {
        ...
        public void run() { target.lockNative(l, safepoint); }
    }
    public static class CallLockClass implements Runnable {
        ...
        public void run() { target.lockClass(safepoint); }
    }
    public static class CallLockSync implements Runnable {
        ...
        public void run() { target.lockSync(safepoint); }
    }
}
