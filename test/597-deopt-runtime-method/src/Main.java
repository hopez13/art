...
public class Main implements Runnable {
    ...
    public void run() {
        if (threadIndex == 0) {
            try {
                // Sleep 5 seconds so that the other thread is in busy loop.
                // Can't use synchronization since we want the other code to
                // be a pure busy loop to catch it in a suspend-test.
                Thread.sleep(5000);
            } catch (Exception e) {}
            deoptimizeAll();
            sExitFlag = true;
