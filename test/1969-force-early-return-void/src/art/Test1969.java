...
public class Test1969 {
    ...
    public void runTestOn(Supplier<TestConfig> config) throws Exception {
        ...
        try {
            ...
            normal_thread.join();
            System.out.println(
                    "NORMAL RUN: result for " + normal_run + " on " + normal_thread.getName());
        } catch (Exception e) {
    ...
    public static class ClassLoadObject implements Runnable {
        ...
        private static class TC0 {
            ...
            static { foo = 100 + curClass; }
        }

        private static class TC1 {
            public static int foo;
            static { foo = 200 + curClass; }
        }

        private static class TC2 {
            public static int foo;
            static { foo = 300 + curClass; }
        }

        private static class TC3 {
            public static int foo;
            static { foo = 400 + curClass; }
        }

        private static class TC4 {
            public static int foo;
            static { foo = 500 + curClass; }
        }

        private static class TC5 {
            public static int foo;
            static { foo = 600 + curClass; }
        }

        private static class TC6 {
            public static int foo;
            static { foo = 700 + curClass; }
        }

        private static class TC7 {
            public static int foo;
            static { foo = 800 + curClass; }
        }

        private static class TC8 {
            public static int foo;
            static { foo = 900 + curClass; }
        }

        private static class TC9 {
            public static int foo;
            static { foo = 1000 + curClass; }
        }
    ...
    public static class ObjectInitTestObject implements Runnable {
        ...
        public void run() { new ObjectInitTarget(() -> cnt++); }

        public String toString() { return "ObjectInitTestObject { cnt: " + cnt + " }"; }
    }
    ...
    public static final class StaticMethodObject implements Runnable {
        ...
        public void run() { calledFunction(() -> cnt++); }

        public final String toString() { return "StaticMethodObject { cnt: " + cnt + " }"; }
    }

    // Only used by CTS to run without class-load tests.
    public static void run() throws Exception { new Test1969(false).runTests(); }
    ...
    public void runTests() throws Exception {
        ...
        runTestOn(SynchronizedTestObject::new,
                (thr)
        ...
        final Method staticCalledMethod =
                StaticMethodObject.class.getDeclaredMethod("calledFunction", Runnable.class);
        ...
        final Executable initCalledMethod =
                ObjectInitTestObject.ObjectInitTarget.class.getConstructor(Runnable.class);
        ...
        runTestOn(ObjectInitTestObject::new,
                (thr)
                        -> setupSuspendBreakpointFor(initCalledMethod, initFunctionLoc, thr),
                SuspendEvents::clearSuspendMethodEvent);

        if (canRunClassLoadTests && CanRunClassLoadingTests()) {
            System.out.println("Test stopped during class-load.");
            runTestOn(ClassLoadObject::new,
                    (thr)
