...
public class Main {
    public static void main(String[] args) {
        ...
        Unmodifiable.doTest(new Transform[] {
                new Transform(),
        });
    ...
    private static Redefinition.CommonClassDefinition mapCCD(CommonClassDefinition d) {
        return new Redefinition.CommonClassDefinition(
                d.target, d.class_file_bytes, d.dex_file_bytes);
    }

    private static Redefinition.CommonClassDefinition[] toCCDA(CommonClassDefinition[] ds) {
        return Arrays.stream(ds)
                .map(Main::mapCCD)
                .toArray(Redefinition.CommonClassDefinition[] ::new);
    }

    public static void doCommonClassRedefinition(Class<?> target, byte[] classfile, byte[] dexfile)
            throws Exception {
    ...
    public static void doCommonMultiClassRedefinition(
            Class<?>[] targets, byte[][] classfiles, byte[][] dexfiles) throws Exception {
    ...
    public static void addCommonTransformationResult(
