...
def default_run(ctx, args, **kwargs):
  ...
  def run(cmdline: str,
    ...
    # As a work-around, we warp the executed command so that it always returns 0 exit code
    # and we also make it print the actual exit code as the last line of its stdout.
    if parse_exit_code_from_stdout:
      assert proc.returncode == 0, "ADB crashed (out:'{proc.stdout}' err:'{proc.stderr}')"
