...
def default_run(ctx, args, **kwargs):
  ...
  for name, new_value in kwargs.items():
    ...
    if isinstance(old_value, list):
      setattr(args, name, old_value + new_value)
    else:
  ...
  def run(cmdline: str,
    ...
    proc = subprocess.run([cmdline],
                          ...
                          encoding="utf8",
                          capture_output=True)
    if (check and proc.returncode != 0) or VERBOSE:
...
# Add the libdir to the argv passed to the main function.
  ...
  if USE_JVM:
    ...
    if CREATE_RUNNER:
      ...
      proc = run(cmdline, env, check=False)
      assert args.stdout_file and args.stderr_file
      with open(args.stdout_file, "a") as f:
        f.write(proc.stdout)
        if proc.returncode != 0:
          f.write("exit status: {}\n".format(proc.returncode))
      with open(args.stderr_file, "a") as f:
        f.write(proc.stderr)
  ...
  def get_prebuilt_lldb_path():
    ...
    if isdir(f"{ANDROID_BUILD_TOP}/{CLANG_BASE}/{PREBUILT_NAME}"):
      assert isdir(CLANG_PREBUILT_HOST_PATH), (
        "The prebuilt clang directory exists, but the specific clang"
...
# Remove whitespace.
  ...
  if not HOST:
    ...
    def run_cmd(cmdline: str, env={}, check: bool = True) -> subprocess.CompletedProcess:
      ...
      return adb.shell(f"{chroot_prefix} sh {DEX_LOCATION}/cmdline.sh", check=check)
