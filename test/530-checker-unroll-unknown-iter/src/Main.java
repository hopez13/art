...
public class Main {
  ...
  /// CHECK-DAG: <<PhiRed:i\d+>>  Phi                                       loop:<<LoopA:B\d+>> outer_loop:none
  /// CHECK-DAG: <<PhiInd:i\d+>>  Phi                                       loop:<<LoopA>>      outer_loop:none
  ...
  /// CHECK-DAG: <<AddA:i\d+>>    Add [<<PhiRed>>,<<GetA>>]                 loop:<<LoopA>>      outer_loop:none
  /// CHECK-DAG: <<AddB:i\d+>>    Add [<<PhiInd>>,<<ConstN1>>]              loop:<<LoopA>>      outer_loop:none
  //
  ...
  /// CHECK-DAG: <<Phi1:i\d+>>    Phi [<<Const0>>,<<AddA>>]                 loop:<<LoopA:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Phi2:i\d+>>    Phi [<<AddL>>,<<AddB>>]                   loop:<<LoopA>>      outer_loop:none
  /// CHECK-DAG: <<Phi3:i\d+>>    Phi [<<Const0>>,<<AddC>>]                 loop:<<LoopA>>      outer_loop:none
  ...
  /// CHECK-DAG: <<AddB:i\d+>>    Add [<<Phi2>>,<<ConstN1>>]                loop:<<LoopA>>      outer_loop:none
  ...
  /// CHECK-DAG: <<Phi4:i\d+>>    Phi [<<Phi1>>,<<AddF>>]                   loop:<<LoopB:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Phi5:i\d+>>    Phi [<<Phi2>>,<<AddG>>]                   loop:<<LoopB>>      outer_loop:none
  /// CHECK-DAG: <<Phi6:i\d+>>    Phi [<<Const0>>,<<AddH>>]                 loop:<<LoopB>>      outer_loop:none
  ...
  /// CHECK-DAG: <<AddE:i\d+>>    Add [<<Phi5>>,<<ConstN1>>]                loop:<<LoopB>>      outer_loop:none
  ...
  // This loop will be dynamically unrolled.
  private static void testArrayFill(int[] array, int[] carray) {
    int n = array.length;
    if (carray == null) {
      for (int i = 0; i < n; i++) {
        array[i] = (n % 2) + i;
  ...
  public void run(int N) {
    int[] iarray = new int[N+3];
    testArrayFill(iarray, null);
    expectEquals(5356, testArraySumDown(iarray));
    expectEquals(-2009024460, testHashCode(iarray));

    int rr[] = new int[N+3];
    testArrayFill(rr, iarray);
    expectEquals(5253, testArraySumByTwo(rr));
    expectEquals(32492, testCheckSum(rr));
