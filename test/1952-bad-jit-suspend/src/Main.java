...
public class Main {
  ...
  private volatile static boolean should_stop = false;
  ...
  public static void main(String[] args) throws Exception {
    ...
    Thread wait_thread = new Thread(() -> {
      try {
        // Wait 10 seconds for a deadlock to occur. This is usually long enough.
        Thread.sleep(10*1000);
      } catch (Exception e) {}
      should_stop = true;
    }, "Wait thread");
    Thread redefine_thread = new Thread(() -> {
      while (!should_stop) {
        doRetransform(TargetClass.class);
      }
    }, "Redefine Thread");
    wait_thread.start();
    redefine_thread.start();
    while (!should_stop) {
      maybeClearJit();
      doNothing();
    }
  ...
  public static native void maybeClearJit();
  public static native void doRetransform(Class<?> k);
}
