...
extern "C" JNIEXPORT void JNICALL Java_Main_doRetransform(JNIEnv* env, jclass, jclass target) {
  jclass main = static_cast<jclass>(env->NewGlobalRef(env->FindClass("Main")));
  ...
  jvmti_env->RetransformClasses(1, &target);
}
...
class NoOps : public slicer::Transformation {
 public:
  ...
  virtual bool Apply(lir::CodeIr* code_ir) {
    if (strcmp("foo", code_ir->ir_method->decl->name->c_str()) != 0) {
      return true;
    }
...
static JNICALL void LoadHookCb(jvmtiEnv *jvmti,
  ...
  if (class_being_redefined == nullptr || strcmp(name, "Main$TargetClass") != 0) {
    return;
  }
...
extern "C" JNIEXPORT void Java_Main_maybeClearJit(JNIEnv* env, jclass main) {
  if (clear_jit.load()) {
    Runtime* runtime = Runtime::Current();
    ArtMethod* doNothingMethod =
        jni::DecodeArtMethod(env->GetStaticMethodID(main, "doNothing", "()V"));
