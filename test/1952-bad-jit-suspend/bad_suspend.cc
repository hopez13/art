...
// Marker to request that the main thread clear the jit and barrier to wait for this to occur.
...
static JNICALL void AgentThreadRedefine(jvmtiEnv* jvmti,
  ...
  while (should_continue.load()) {
    if (runtime->GetJit()->GetCodeCache()->WillExecuteJitCode(doNothingMethod)) {
...
JNICALL void ClassPrepareJit(jvmtiEnv* jvmti,
  ...
  // b/70838465: Currently we prevent this SuspendThread call from working in an attempt to solve
  // this issue but it seems this is insufficient. Put this suspend here so if we do decide to just
  // revert go/aog/716625 this test will work more reliably.
