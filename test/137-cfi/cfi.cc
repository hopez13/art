...
extern "C" JNIEXPORT jboolean JNICALL Java_Main_wait(JNIEnv*, jobject, jint, jboolean, jdouble) {
  // Loop forever.
  printf("Going to wait forever\n");
  // In order to avoid problems whereby it's possible to be at a
  // point in a libc function that doesn't unwind, always use a
  // busy loop instead of sleeps.
...
extern "C" JNIEXPORT jboolean JNICALL Java_Main_unwindOtherProcess(
  ...
  if (result) {
    ...
    // "mini-debug-info" does not include parameters to save space.
    std::vector<std::string> seq = {
        // "Java_Main_wait",                         // The wait function being executed in the
                                                     // other runtime.
                                                     // Note: For some reason, the name isn't
                                                     // resolved, so don't look for it right now.
        "Main.wait",                                 // The corresponding Java native method frame.
        "java.util.Arrays.binarySearch0",            // Framework method.
        "Base.runBase",                              // Method in other dex file.
        "Main.main"                                  // The Java entry method.
    };
