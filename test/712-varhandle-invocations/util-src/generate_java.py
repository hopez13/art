...
#    if var_handle_kind.is_view() and var_type.width == 8:
#        expansions['expected_exception'] = "else if (!VarHandleUnitTestHelpers.is64Bit()) {\n" \
...
#                                           "}"
    if var_handle_kind.may_throw_read_only and not accessor.is_read_only():
        ...
        expansions['expected_exception'] = "else { expectExceptionOfType(ReadOnlyBufferException.class); }"
    else:
...
class ${test_class} extends VarHandleUnitTest {
    ...
    public void doTest() throws Exception {
        VarHandleUnitTestHelpers.is64Bit();
