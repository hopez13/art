...
public class TestGenerator {
    public static void main(String[] args) throws IOException {
        ...
        Path initTestInvoke =
                base.resolve(TestGenerator.class.getPackage().getName().replace('.', '/'))
        ...
        Path initTest1948 =
                base.resolve("art").resolve(art.Test1948.class.getSimpleName() + ".class");
        byte[] finalClassBytes = finalClass.toByteArray();
        byte[] finalDexBytes = getFinalDexBytes(d8Bin, finalClassBytes);
        generateTestCode(new FileInputStream(initTest1948.toFile()).readAllBytes(), finalClassBytes,
                finalDexBytes, new FileOutputStream(initTest1948.toFile()));
    }

    // Modify the Test1948 class bytecode so it has the transformed version of TestInvoker as a
    // string constant.
    private static void generateTestCode(byte[] initClass, byte[] transClass, byte[] transDex,
    ...
    private static void generateStringAccessorMethod(ClassVisitor cv, String name, String ret) {
        MethodVisitor mv =
                cv.visitMethod(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC | Opcodes.ACC_SYNTHETIC,
    ...
    // need the dex-file bytes to be used by the test class to redefine TestInvoker. We use d8
    // because it doesn't require setting up a directory structures or matching file names like dx
    // does.
    ...
    private static byte[] getFinalDexBytes(String d8Bin, byte[] classBytes) throws IOException {
        ...
        Process d8Proc = new ProcessBuilder(d8Bin,
                ...
                "--output", tempDir.toAbsolutePath().toString(), "--min-api",
                "28", // Allow the new invoke ops.
        ...
        try {
            if (res == 0) {
                byte[] out =
                        new FileInputStream(tempDir.resolve("classes.dex").toFile()).readAllBytes();
    ...
    private static void generateInvoker(byte[] inputClass, String toCall, OutputStream output)
            throws IOException {
    ...
    private static void generateRunTest(ClassVisitor cv, String toCall) {
        MethodVisitor mv = cv.visitMethod(
                Opcodes.ACC_PUBLIC, "runTest", "(Ljava/lang/Runnable;)V", null, null);
        MethodType mt = MethodType.methodType(Void.TYPE);
        Handle mh = new Handle(Opcodes.H_INVOKESTATIC, Type.getInternalName(Responses.class),
                toCall, mt.toMethodDescriptorString(), false);
        ...
        mv.visitLdcInsn(mh);
        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, Type.getInternalName(MethodHandle.class),
                "invokeExact", "()V", false);
