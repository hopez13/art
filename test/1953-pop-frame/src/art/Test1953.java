...
public class Test1953 {
    ...
    public static TestSuspender makeSuspend(
            final ThreadRunnable setup, final ThreadRunnable clean) {
    ...
    public void runTestOn(TestRunnable testObj, ThreadRunnable su, ThreadRunnable cl)
            throws Exception {
    ...
    public void runTestOn(TestRunnable testObj, TestSuspender su) throws Exception {
        System.out.println("Single call with PopFrame on " + testObj
                + " base-call-count: " + testObj.getBaseCallCount());
    ...
    public static abstract class AbstractTestObject implements TestRunnable {
        ...
        public AbstractTestObject() { callerCnt = 0; }

        public int getBaseCallCount() { return callerCnt; }
    ...
    public static class RedefineTestObject extends AbstractTestObject implements Runnable {
        public static enum RedefineState {
            ORIGINAL,
            REDEFINED,
        }
        ;
    ...
    public static class ClassLoadObject implements TestRunnable {
        ...
        private static class TC3 {
            public static int foo;
            static { foo = 4; }
        }
        private static class TC4 {
            public static int foo;
            static { foo = 5; }
        }
        private static class TC5 {
            public static int foo;
            static { foo = 6; }
        }
        private static class TC6 {
            public static int foo;
            static { foo = 7; }
        }
        private static class TC7 {
            public static int foo;
            static { foo = 8; }
        }
        private static class TC8 {
            public static int foo;
            static { foo = 9; }
        }
        private static class TC9 {
            public static int foo;
            static { foo = 10; }
        }
        ...
        public int getBaseCallCount() { return baseCallCnt; }
    ...
    public static class StandardTestObject extends AbstractTestObject implements Runnable {
        ...
        public StandardTestObject() { this(true); }
        ...
        public String toString() { return "StandardTestObject { cnt: " + cnt + " }"; }
    }

    public static class SynchronizedFunctionTestObject
            extends AbstractTestObject implements Runnable {
        public int cnt;
        ...
        public String toString() { return "SynchronizedFunctionTestObject { cnt: " + cnt + " }"; }
    }
    public static class SynchronizedTestObject extends AbstractTestObject implements Runnable {
        public int cnt;
        public final Object lock;

        public SynchronizedTestObject() { this(new Object()); }
        ...
        public String toString() { return "SynchronizedTestObject { cnt: " + cnt + " }"; }
    }
    ...
    public static class ExceptionCatchTestObject extends AbstractTestObject implements Runnable {
        ...
        public void doThrow() { throw new TestError(); }

        public String toString() { return "ExceptionCatchTestObject { cnt: " + cnt + " }"; }
    }
    ...
    public static class ExceptionThrowFarTestObject implements TestRunnable {
        ...
        public void callingFunction() { calledFunction(); }
        public void calledFunction() {
            cnt++;
            if (catchInCalled) {
                try {
        ...
        public String toString() { return "ExceptionThrowFarTestObject { cnt: " + cnt + " }"; }
    }
    ...
    public static class ExceptionThrowTestObject implements TestRunnable {
        ...
        public String toString() { return "ExceptionThrowTestObject { cnt: " + cnt + " }"; }
    }
    ...
    public static class NativeCalledObject extends AbstractTestObject {
        ...
        public String toString() { return "NativeCalledObject { cnt: " + cnt + " }"; }
    }
    ...
    public static class NativeCallerObject implements TestRunnable {
        ...
        public int getBaseCallCount() { return baseCnt; }
        ...
        public String toString() { return "NativeCallerObject { cnt: " + cnt + " }"; }
    }
    public static class SuspendSuddenlyObject extends AbstractTestObject {
        ...
        public String toString() { return "SuspendSuddenlyObject { cnt: " + cnt + " }"; }
    }
    ...
    public void runTests() throws Exception {
        ...
        runTestOn(new StandardTestObject(),
                (thr)
                        -> setupSuspendBreakpointFor(calledFunction, loc, thr),
        ...
        // Add a breakpoint on the second line after the start of the function
        // Annoyingly r8 generally has the first instruction (a monitor enter) not be marked as
        // being on any line but javac has it marked as being on the first line of the function.
        // Just use the second entry on the line-number table to get the breakpoint. This should be
        // good for both.
        ...
        runTestOn(new SynchronizedFunctionTestObject(),
                (thr)
                        -> setupSuspendBreakpointFor(
                                syncFunctionCalledFunction, syncFunctionLoc, thr),
                SuspendEvents::clearSuspendBreakpointFor);
        ...
        runTestOn(new SynchronizedTestObject(lock),
                (thr)
                        -> setupSuspendBreakpointFor(syncCalledFunction, syncLoc, thr),
                SuspendEvents::clearSuspendBreakpointFor);
        synchronized (lock) {}
        ...
        runTestOn(new StandardTestObject(),
                (thr)
                        -> setupSuspendSingleStepAt(calledFunction, loc, thr),
        ...
        runTestOn(new FieldBasedTestObject(),
                (thr)
                        -> setupFieldSuspendFor(
                                FieldBasedTestObject.class, target_field, true, thr),
        ...
        runTestOn(new FieldBasedTestObject(),
                (thr)
                        -> setupFieldSuspendFor(
                                FieldBasedTestObject.class, target_field, false, thr),
                SuspendEvents::clearFieldSuspendFor);

        System.out.println("Test stopped during Method Exit of doNothing");
        runTestOn(new StandardTestObject(false),
                (thr)
                        -> setupSuspendMethodEvent(doNothingMethod, /*enter*/ false, thr),
        ...
        runTestOn(new StandardTestObject(false),
                (thr)
                        -> setupSuspendMethodEvent(doNothingMethod, /*enter*/ true, thr),
                SuspendEvents::clearSuspendMethodEvent);

        System.out.println("Test stopped during Method Exit of calledFunction");
        runTestOn(new StandardTestObject(false),
                (thr)
                        -> setupSuspendMethodEvent(calledFunction, /*enter*/ false, thr),
                SuspendEvents::clearSuspendMethodEvent);

        System.out.println("Test stopped during Method Enter of calledFunction");
        runTestOn(new StandardTestObject(false),
                (thr)
                        -> setupSuspendMethodEvent(calledFunction, /*enter*/ true, thr),
                SuspendEvents::clearSuspendMethodEvent);
        ...
        runTestOn(new ExceptionOnceObject(/*throwInSub*/ false),
                (thr)
                        -> setupSuspendMethodEvent(exceptionOnceCalledMethod, /*enter*/ false, thr),
                SuspendEvents::clearSuspendMethodEvent);
        ...
        if (IS_ART && canRunClassLoadTests && CanRunClassLoadingTests()) {
            ...
            runTestOn(new StandardTestObject(false),
                    (thr)
                            -> setupSuspendPopFrameEvent(1, doNothingMethod, thr),
            ...
            runTestOn(new StandardTestObject(false),
                    (thr)
                            -> setupSuspendPopFrameEvent(0, doNothingMethod, thr),
                    SuspendEvents::clearSuspendPopFrameEvent);
            ...
            runTestOn(new ExceptionThrowTestObject(false),
                    (thr)
                            -> setupSuspendPopFrameEvent(0, exceptionThrowCalledMethod, thr),
                    SuspendEvents::clearSuspendPopFrameEvent);
            ...
            runTestOn(new ExceptionCatchTestObject(),
                    (thr)
                            -> setupSuspendPopFrameEvent(0, exceptionCatchThrowMethod, thr),
                    SuspendEvents::clearSuspendPopFrameEvent);
        }
        ...
        runTestOn(new ExceptionThrowTestObject(true),
                (thr)
                        -> setupSuspendExceptionEvent(
                                exceptionThrowCalledMethod, /*catch*/ true, thr),
        ...
        runTestOn(new ExceptionCatchTestObject(),
                (thr)
                        -> setupSuspendExceptionEvent(
                                exceptionCatchCalledMethod, /*catch*/ true, thr),
                SuspendEvents::clearSuspendExceptionEvent);
        ...
        runTestOn(new ExceptionThrowTestObject(false),
                (thr)
                        -> setupSuspendExceptionEvent(
                                exceptionThrowCalledMethod, /*catch*/ false, thr),
                SuspendEvents::clearSuspendExceptionEvent);
        ...
        runTestOn(new ExceptionThrowFarTestObject(false),
                (thr)
                        -> setupSuspendExceptionEvent(
                                exceptionThrowFarCalledMethod, /*catch*/ false, thr),
                SuspendEvents::clearSuspendExceptionEvent);
        ...
        if (canRunClassLoadTests && CanRunClassLoadingTests()) {
            // This test doesn't work on RI since the RI disallows use of PopFrame during a
            // ClassLoad event. See b/116003018 for more information.
            System.out.println("Test stopped during a ClassLoad event.");
            runTestOn(new ClassLoadObject(),
                    (thr)
                            -> setupSuspendClassEvent(
                                    EVENT_TYPE_CLASS_LOAD, ClassLoadObject.CLASS_NAMES, thr),
                    SuspendEvents::clearSuspendClassEvent);

            // The RI handles a PopFrame during a ClassPrepare event incorrectly. See b/116003018
            // for more information.
            System.out.println("Test stopped during a ClassPrepare event.");
            runTestOn(new ClassLoadObject(),
                    (thr)
                            -> setupSuspendClassEvent(
                                    EVENT_TYPE_CLASS_PREPARE, ClassLoadObject.CLASS_NAMES, thr),
                    SuspendEvents::clearSuspendClassEvent);
        }
        ...
        runTestOn(new RedefineTestObject(),
                (thr)
                        -> setupSuspendBreakpointFor(redefineCalledFunction, redefLoc, thr),
                (thr) -> {
                    clearSuspendBreakpointFor(thr);
                    Redefinition.doCommonClassRedefinition(RedefineTestObject.class,
                            RedefineTestObject.CLASS_BYTES, RedefineTestObject.DEX_BYTES);
                });

        System.out.println("Test stopped during a native method fails");
        runTestOn(new NativeCalledObject(), SuspendEvents::setupWaitForNativeCall,
    ...
    // constant propagation and eliminating the failing paths before the verifier is able to load
    // the class.
    ...
    public boolean CanRunClassLoadingTests() {
        ...
        if (!ranClassLoadTest) {
            ...
            // This will never be executed. If classes are not preverified the verifier will load
            // RCLT1 when the enclosing method is run. This behavior makes the class-load/prepare
            // test cases impossible to successfully run (they will deadlock).
            new RCLT1().foo();
