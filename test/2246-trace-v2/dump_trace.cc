...
bool ProcessTraceEntries(std::unique_ptr<File>& file,
                         std::map<int, int>& curr_depth_map,
  ...
  const uint8_t* curr_buff_ptr = buffer;
  int32_t method_id = method_value >> kTraceActionBits;
  uint8_t event_type = method_value & 0x3;
  int curr_depth = 0;
  if (curr_depth_map.find(thread_id) != curr_depth_map.end()) {
    curr_depth = curr_depth_map[thread_id];
  }
  ...
  if (method_map.find(method_id) == method_map.end()) {
    printf("No entry for init method %d\n", method_id);
  }
  if (print_thread_events) {
    print_trace_entry(thread_name, method_map[method_id], &curr_depth, event_type);
  }
  for (int i = 0; i < num_records; i++) {
    uint32_t diff = 0;
    bool success = DecodeUnsignedLeb128Checked(&curr_buff_ptr, buffer + total_size - 1, &diff);
    if (!success) {
      printf("Reading past the buffer???\n");
    }
    ...
    // Read timestamps
    DecodeUnsignedLeb128(&curr_buff_ptr);
    if (is_dual_clock) {
      DecodeUnsignedLeb128(&curr_buff_ptr);
    }
  }
  curr_depth_map[thread_id] = curr_depth;
  return true;
}
...
extern "C" JNIEXPORT void JNICALL Java_Main_dumpTrace(JNIEnv* env,
  ...
  while (has_entries) {
    ...
    switch (entry_header) {
      ...
      case kTraceEntries:
        ProcessTraceEntries(
            file, curr_depth_map, thread_map, method_map, is_dual_clock, thread_name);
        break;
      case kSummary:
        has_entries = false;
        break;
