...
public class Main {
     ...
     * }
     */
    public static final byte[] REDEFINED_DEX_FILE = Base64.getDecoder().decode(
    ...
    private static final class BaseResetIter implements ResetIterator<Object[]> {
        ...
        public boolean hasNext() { return have_next; }

        public void reset() { have_next = true; }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        long off_secret_array =
                genericFieldOffset(Transform.class.getDeclaredField("SECRET_ARRAY"));
        long off_secret_number =
                genericFieldOffset(Transform.class.getDeclaredField("SECRET_NUMBER"));

        System.out.println("Reading normally.");
        System.out.println("\tOriginal secret number is: " + Transform.SECRET_NUMBER);
        System.out.println(
                "\tOriginal secret array is: " + Arrays.toString((byte[]) Transform.SECRET_ARRAY));
        ...
        System.out.println("\tPost-redefinition secret array is: "
                + Arrays.toString((byte[]) Transform.SECRET_ARRAY));
        ...
        // Try calling all the public, non-static methods on the obsolete class. Make sure we cannot
        // get j.l.r.{Method,Field} objects or instances.
        TConsumer<Class> cc = (Class c) -> {
            for (Method m : Class.class.getDeclaredMethods()) {
                if (Modifier.isPublic(m.getModifiers()) && !Modifier.isStatic(m.getModifiers())) {
                    ...
                    for (Object[] arr : iter) {
                        try {
                            System.out.println(m + " on " + safePrint(c) + " with " + deepPrint(arr)
                                    + " = " + safePrint(m.invoke(c, arr)));
                        } catch (Throwable e) {
                            System.out.println(m + " with " + deepPrint(arr) + " throws "
                                    + safePrint(e) + ": " + safePrint(e.getCause()));
                        }
                    }
                }
            }
    ...
    public static Iterable<Object[]> CollectParameterValues(Method m, Class<?> obsolete_class)
            throws Exception {
        Class<?>[] types = m.getParameterTypes();
        final Object[][] params = new Object[types.length][];
        for (int i = 0; i < types.length; i++) {
            if (types[i].equals(Class.class)) {
                params[i] = new Object[] {null, Object.class, obsolete_class, Transform.class,
                        Long.TYPE, Class.class};
                        ...
                        .map((x) -> {
                            if (x == null) {
                                ...
                                if (((Object[]) x).length == 0) {
                                    return "new " + x.getClass().getComponentType().getName()
                                            + "[0]";
                                } else {
    ...
    private static class ComboIter implements ResetIterator<Object[]> {
        ...
        public boolean hasNext() { return next.hasNext() || my_vals.hasNext(); }
