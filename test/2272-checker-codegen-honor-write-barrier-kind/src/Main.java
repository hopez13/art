...
public class Main {
    ...
    // When the bug was present, $noinline$testArraySetsHonorWriteBarrier would never set the card as
    // dirty (which is incorrect). arr[1]'s' write barrier depends on arr[0]'s write barrier. The
    // disappeared BoundType in prepare_for_register_allocation shouldn't skip marking the card dirty.
    ...
    // When the bug was present, $noinline$testArraySetsDontSkipWriteBarrier would never set the card
    // as dirty (which is incorrect). arr[1]'s write barrier depends on arr[0]'s write barrier. The
    // code path should mark the card dirty without a null check on `o` in `arr[0] = o`.
