...
public class Main {
  ...
  /// CHECK-DAG: <<Cons:i\d+>>   IntConstant {{2|4|8}}                     loop:none
  /// CHECK-DAG: <<Set:d\d+>>    VecSetScalars [{{i\d+}}{{(,j\d+)?}}]      loop:none
  /// CHECK-DAG: <<Phi:d\d+>>    Phi [<<Set>>,{{d\d+}}]                    loop:<<Loop1:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Load:d\d+>>   VecLoad [{{l\d+}},<<I:i\d+>>{{(,j\d+)?}}] loop:<<Loop1>>      outer_loop:none
  /// CHECK-DAG:                 VecAdd [<<Phi>>,<<Load>>{{(,j\d+)?}}]     loop:<<Loop1>>      outer_loop:none
  ...
  /// CHECK-DAG: <<Cons2:i\d+>>  IntConstant {{2|4}}                       loop:none
  /// CHECK-DAG: <<Set:d\d+>>    VecSetScalars [{{j\d+}}{{(,j\d+)?}}]      loop:none
  /// CHECK-DAG: <<Phi:d\d+>>    Phi [<<Set>>,{{d\d+}}]                    loop:<<Loop:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Load:d\d+>>   VecLoad [{{l\d+}},<<I:i\d+>>{{(,j\d+)?}}] loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                 VecAdd [<<Phi>>,<<Load>>{{(,j\d+)?}}]     loop:<<Loop>>      outer_loop:none
  ...
  /// CHECK-DAG: <<Phi:d\d+>>    Phi [<<Set>>,{{d\d+}}]                    loop:<<Loop:B\d+>> outer_loop:none
  /// CHECK-DAG: <<Load:d\d+>>   VecLoad [{{l\d+}},<<I:i\d+>>{{(,j\d+)?}}] loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                 VecSub [<<Phi>>,<<Load>>{{(,j\d+)?}}]     loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG:                 Add [<<I>>,<<Cons>>]                      loop:<<Loop>>      outer_loop:none
  /// CHECK-DAG: <<Red:d\d+>>    VecReduce [<<Phi>>{{(,j\d+)?}}]           loop:none
  /// CHECK-DAG: <<Extr:i\d+>>   VecExtractScalar [<<Red>>{{(,j\d+)?}}]    loop:none
