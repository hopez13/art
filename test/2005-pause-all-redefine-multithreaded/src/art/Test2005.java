...
public class Test2005 {
        ...
        public void Check() throws Exception {
            for (String s : results) {
                if (!s.equals("Hello from " + getName())
                        && !s.equals("Hello, " + DEFAULT_VAL + ", " + DEFAULT_VAL + ", "
                                + DEFAULT_VAL + " from " + getName())
                        && !s.equals(
    ...
    public static void doRedefinition() throws Exception {
        ...
        // If some of the other threads finished between calling mytg.activeCount and enumerate we
        // will have nulls. These nulls are interpreted as currentThread by SuspendThreadList so we
        ...
        UpdateFieldValuesAndResumeThreads(thread_set.toArray(new Thread[0]), Transform.class,
                field_set.toArray(new Field[0]), DEFAULT_VAL);
    }
