...
class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // The order here matters: currently app images are never unloaded, so we need to first
    // test failing to load an app image then successfully
    testLoadingInvalidSecondaryAppImage();
  ...
  public static void testLoadingInvalidSecondaryAppImage() throws Exception {
    secondaryAppImageTestHelper(
        new PathClassLoader(DEX_FILE, LIBRARY_SEARCH_PATH, Main.class.getClassLoader()),
        false);
  }

  public static void testLoadingSecondaryAppImage() throws Exception {
    secondaryAppImageTestHelper(Main.class.getClassLoader(), true);
  }

  private static void secondaryAppImageTestHelper(
      ClassLoader parent, boolean inAppImage) throws Exception {
    ...
    assertEquals("Ensure app image is loaded if it should be",
        checkAppImageLoaded("596-app-images-ex"), inAppImage);

    Class<?> secondaryCls = pcl.loadClass("Secondary");
    assertEquals("Ensure Secondary class is in the app image if the CLC is correct",
        checkAppImageContains(secondaryCls), inAppImage);
    assertEquals("Ensure Secondary class is preinitialized if the CLC is correct",
        checkInitialized(secondaryCls), inAppImage);
