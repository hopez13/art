...
public class Test1934 {
    ...
    private static void ensureClassesLoaded() {
        // Depending on timing this class might (or might not) be loaded during testing of Stop. If
        // it is and the StopThread occurs inside of it we will get a ExceptionInInitializerError
        // which is not what we are looking for. In order to avoid this ever happening simply
        // initialize the class that can cause it early.
        ensureInitialized(java.util.concurrent.locks.LockSupport.class);
    }

    public static Thread createThread(Runnable r, String name) {
        return new Thread(
                Thread.currentThread().getThreadGroup(), r, name, /* 10 mb */ 10 * 1000000);
    }

    public static void testStopBeforeStart() throws Exception {
        final Throwable[] out_err = new Throwable[] {
                null,
        };
    ...
    public static void testStopInNative() throws Exception {
        ...
        Thread target = createThread(() -> {
            ...
            // We need to make sure we do something that can get the exception to be actually
            // noticed.
            doNothing();
        ...
        nativeDoInterleaved(
                native_monitor_id, () -> { Threads.stopThread(target, new Error("AWESOME")); });
        target.join();
