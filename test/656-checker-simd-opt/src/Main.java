...
public class Main {
  ...
  public static final int kArraySize = 130;
  ...
  /// CHECK-DAG: <<CP1:i\d+>>   IntConstant 1                        loop:none
  /// CHECK-DAG: <<CP2:i\d+>>   IntConstant 2                        loop:none
  /// CHECK-DAG: <<Arr0:l\d+>>  ParameterValue                       loop:none
  /// CHECK-DAG: <<Arr1:l\d+>>  ParameterValue                       loop:none
  ...
  //
  // Array size is chosen to be such a constant, that the loop trip count is a multiple of
  // vector length and unroll factor; hence clean up is needed exclusively for the array
  // references test.
  private static void $noinline$stencilConstSize(int[] a, int[] b) {
    for (int i = 1; i < kArraySize - 1; i++) {
      a[i] = b[i - 1] + b[i] + b[i + 1];
    }
  }
