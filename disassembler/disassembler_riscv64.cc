...
static uint32_t GetRs1Short16(uint16_t insn16) { return BitFieldExtract(insn16, 7, 3) + 8u; }

static uint32_t GetRs2Short16(uint16_t insn16) { return BitFieldExtract(insn16, 2, 3) + 8u; }

static uint32_t GetRs116(uint16_t insn16) { return BitFieldExtract(insn16, 7, 5); }

static uint32_t GetRs216(uint16_t insn16) { return BitFieldExtract(insn16, 2, 5); }

template <typename T>
static T GetImm16(uint16_t insn16) {
  static_assert(std::is_integral_v<T>, "Should be integer");
  T v = BitFieldInsert(BitFieldExtract(insn16, 2, 5), BitFieldExtract(insn16, 12, 1), 5, 1);
  ...
  switch (insn16 & 3u) {
      ...
      switch (funct3) {
        case 0b000u:
          if (insn16 != 0) {
            ...
            uint32_t decoded =
                BitFieldExtract(nzuimm, 0, 1) << 3 | BitFieldExtract(nzuimm, 1, 1) << 3 |
                BitFieldExtract(nzuimm, 3, 5) << 6 | BitFieldExtract(nzuimm, 7, 2) << 4;
    ...
    case 0b01u:
      switch (funct3) {
        ...
        case 0b101u: {
          int32_t disp = BitFieldExtract(insn16, 1, 3) << 1  | BitFieldExtract(insn16, 11, 1) << 4 |
                         BitFieldExtract(insn16, 0, 1) << 5  | BitFieldExtract(insn16, 7,  1) << 6 |
                         BitFieldExtract(insn16, 6, 1) << 7  | BitFieldExtract(insn16, 9,  3) << 8 |
        ...
        case 0b110u:
        case 0b111u: {
          int32_t disp = BitFieldExtract(insn16, 1, 2) << 1 | BitFieldExtract(insn16, 10, 2) << 3 |
                         BitFieldExtract(insn16, 0, 1) << 5 | BitFieldExtract(insn16, 5,  2) << 6 |
    ...
    case 0b10u:
      switch (funct3) {
        ...
        case 0b001u: {
          offset = BitFieldExtract(insn16, 5, 2) << 3 | BitFieldExtract(insn16, 12, 1) << 5 |
                   BitFieldExtract(insn16, 2, 3) << 6;
        ...
        case 0b010u: {
          uint32_t rd = GetRs116(insn16);
          if (rd != 0) {
            offset = BitFieldExtract(insn16, 5, 3) << 2 | BitFieldExtract(insn16, 12, 1) << 5 |
                     BitFieldExtract(insn16, 2, 2) << 6;
