...
static void DumpReg(std::ostream& os, uint32_t reg, bool is_o32_abi) {
  if (is_o32_abi) {
    os << gO32AbiRegNames[reg];
  } else {
    os << gN64AbiRegNames[reg];
  }
}
      ...
      for (const char* args_fmt = gMipsInstructions[i].args_fmt; *args_fmt; ++args_fmt) {
        switch (*args_fmt) {
          ...
          case 'c':  // Floating-point condition code flag in bc1f/bc1t and movf/movt.
            args << "cc" << (rt >> 2);
            break;
          case 'D': DumpReg(args, rd, is_o32_abi_); break;
          ...
          case 'p':  // 19-bit offset in addiupc.
            ...
            break;
          case 'S': DumpReg(args, rs, is_o32_abi_); break;
          case 's': args << 'f' << rs; break;
          case 'T': DumpReg(args, rt, is_o32_abi_); break;
          ...
          case 'w':  // MSA +x(rs).
            {
              ...
              args << StringPrintf("%+d(", s10 << df);
              DumpReg(args, rd, is_o32_abi_);
              args << ')';
              break;
            }
