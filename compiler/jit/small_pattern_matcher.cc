...
template <int offset, typename T, typename RT>
static RT ReturnFieldAt([[maybe_unused]] ArtMethod* method, mirror::Object* obj)
...
template <int offset, typename T, typename RT>
static RT ReturnStaticFieldAt(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
...
#define SWITCH_CASE(offset, object_case, primitive_case, type, return_type) \
  case offset: return is_object \
      ? reinterpret_cast<void*>(&object_case<offset>) \
      : reinterpret_cast<void*>(&primitive_case<offset, type, return_type>);

#define DO_SWITCH_OFFSET(offset, O, P, T, RT) \
  switch (offset) { \
    SWITCH_CASE(0, O, P, T, RT) \
    SWITCH_CASE(4, O, P, T, RT) \
    SWITCH_CASE(8, O, P, T, RT) \
    SWITCH_CASE(12, O, P, T, RT) \
    SWITCH_CASE(16, O, P, T, RT) \
    SWITCH_CASE(20, O, P, T, RT) \
    SWITCH_CASE(24, O, P, T, RT) \
    SWITCH_CASE(28, O, P, T, RT) \
    SWITCH_CASE(32, O, P, T, RT) \
    SWITCH_CASE(36, O, P, T, RT) \
    SWITCH_CASE(40, O, P, T, RT) \
    SWITCH_CASE(44, O, P, T, RT) \
    SWITCH_CASE(48, O, P, T, RT) \
    SWITCH_CASE(52, O, P, T, RT) \
    SWITCH_CASE(56, O, P, T, RT) \
    SWITCH_CASE(60, O, P, T, RT) \
    SWITCH_CASE(64, O, P, T, RT) \
...
#define DO_SWITCH(offset, O, P, K) \
  switch (K) { \
    case Primitive::kPrimBoolean: \
      DO_SWITCH_OFFSET(offset, O, P, uint8_t, int32_t); \
    case Primitive::kPrimInt: \
      DO_SWITCH_OFFSET(offset, O, P, int32_t, int32_t); \
    case Primitive::kPrimLong: \
      DO_SWITCH_OFFSET(offset, O, P, int64_t, int64_t); \
    case Primitive::kPrimNot: \
      DO_SWITCH_OFFSET(offset, O, P, int32_t, int32_t); \
    case Primitive::kPrimFloat: \
      if (kRuntimeISA == InstructionSet::kArm64) { \
        DO_SWITCH_OFFSET(offset, O, P, float, float); \
      } else { \
        return nullptr; \
      } \
    case Primitive::kPrimDouble: \
      if (kRuntimeISA == InstructionSet::kArm64) { \
        DO_SWITCH_OFFSET(offset, O, P, double, double); \
      } else { \
        return nullptr; \
      } \
    default: \
