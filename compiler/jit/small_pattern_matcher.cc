...
static T ReturnFieldAt([[maybe_unused]] ArtMethod* method, mirror::Object* obj)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  return obj->GetFieldPrimitive<T, /* kIsVolatile= */ false>(MemberOffset(offset));
}

template <int offset>
static mirror::Object* ReturnFieldObjectAt([[maybe_unused]] ArtMethod* method, mirror::Object* obj)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  return obj->GetFieldObject<mirror::Object>(MemberOffset(offset));
}

template <int offset, typename T>
static T ReturnStaticFieldAt(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
...
template <int offset>
static mirror::Object* ReturnStaticFieldObjectAt([[maybe_unused]] ArtMethod* method)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ObjPtr<mirror::Class> cls = method->GetDeclaringClass();
  MemberOffset first_field_offset = cls->GetFirstReferenceStaticFieldOffset(kRuntimePointerSize);
...
static void SetFieldAt([[maybe_unused]] ArtMethod* method, mirror::Object* obj, T value)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  obj->SetFieldPrimitive<T, /* kIsVolatile= */ false>(MemberOffset(offset), value);
}
template <int offset>
static void SetFieldObjectAt([[maybe_unused]] ArtMethod* method,
                             ...
                             mirror::Object* value)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  obj->SetFieldObject</* kTransactionActive */ false>(MemberOffset(offset), value);
}
template <int offset, typename T>
...
#define SWITCH_CASE(offset, object_case, primitive_case, type) \
  case offset: return is_object ? reinterpret_cast<void*>(&object_case<offset>) \
                                : reinterpret_cast<void*>(&primitive_case<offset, type>);

#define DO_SWITCH_OFFSET(O, P, T) \
  switch (offset) { \
    SWITCH_CASE(4, O, P, T) SWITCH_CASE(8, O, P, T) SWITCH_CASE(12, O, P, T) \
    SWITCH_CASE(16, O, P, T) SWITCH_CASE(20, O, P, T) SWITCH_CASE(24, O, P, T) \
    SWITCH_CASE(28, O, P, T) SWITCH_CASE(32, O, P, T) SWITCH_CASE(36, O, P, T) \
    SWITCH_CASE(40, O, P, T) SWITCH_CASE(44, O, P, T) SWITCH_CASE(48, O, P, T) \
    SWITCH_CASE(52, O, P, T) SWITCH_CASE(56, O, P, T) SWITCH_CASE(60, O, P, T) \
    SWITCH_CASE(64, O, P, T) \
    default: return nullptr; \
  }

#define DO_SWITCH(O, P, K) \
  switch (K) { \
    case Primitive::kPrimBoolean: \
      DO_SWITCH_OFFSET(O, P, uint8_t); \
    case Primitive::kPrimFloat: \
      DO_SWITCH_OFFSET(O, P, float); \
    case Primitive::kPrimDouble: \
      DO_SWITCH_OFFSET(O, P, double); \
    case Primitive::kPrimInt: \
      DO_SWITCH_OFFSET(O, P, int32_t); \
    case Primitive::kPrimLong: \
      DO_SWITCH_OFFSET(O, P, int64_t); \
...
const void* SmallPatternMatcher::TryMatch(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  if (insns_size == 4u) {
    if (!is_recognizable_constructor) {
      // We don't recognize any other method whose size is 4.
      return nullptr;
    }
    for (DexInstructionPcPair pair : accessor) {
      ...
      switch (instruction.Opcode()) {
        case Instruction::INVOKE_DIRECT: {
          if (is_object_init_invoke(instruction)) {
  ...
  if (insns_size == 1u) {
    ...
    if (instruction.Opcode() == Instruction::RETURN_VOID) {
      return reinterpret_cast<void*>(&EmptyMethod);
    } else if (instruction.Opcode() == Instruction::RETURN_OBJECT) {
  ...
  //   return-{object} vX
  // Or:
  //   iput-{object,wide,boolean} v1, vx0, field
  //   return-void
  // Or:
  ...
  //   iput-{object,wide,boolean} v1, v0, field
  //   return-void
  if (accessor.InsnsSizeInCodeUnits() == 3u ||
      (accessor.InsnsSizeInCodeUnits() == 6u && is_recognizable_constructor)) {
    uint16_t number_of_vregs = accessor.RegistersSize();
    uint16_t number_of_parameters = accessor.InsSize();
    uint16_t obj_reg = number_of_vregs - number_of_parameters;
    ...
    for (DexInstructionPcPair pair : accessor) {
      ...
      switch (pair->Opcode()) {
        ...
        case Instruction::RETURN: {
          ...
          if (is_static) {
            DO_SWITCH(ReturnStaticFieldObjectAt, ReturnStaticFieldAt, field_type);
          } else {
            DO_SWITCH(ReturnFieldObjectAt, ReturnFieldAt, field_type);
          }
        }
        case Instruction::RETURN_VOID: {
          if (!is_put) {
          ...
          if (is_final) {
            ...
            DO_SWITCH(ConstructorSetFieldObjectAt, ConstructorSetFieldAt, field_type);
          } else {
            DO_SWITCH(SetFieldObjectAt, SetFieldAt, field_type);
          }
        }
        default:
          return nullptr;
