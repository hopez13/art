...
struct PerfJitCodeLoad : PerfJitBase {
  ...
  uint32_t thread_id_;     // Thread id where the jitted code is running.
  uint64_t vma_;           // Address of the code section. 0 if code_address_ uses absolute address.
...
// Logs debug line information (kDebugInfo), for source line/column mapping.
