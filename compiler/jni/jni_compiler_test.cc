...
enum class JniKind {
  kNormal,    // Regular kind of natives you get when they aren't annotated.
...
// Recursive case.
template <typename T, typename ... Args>
size_t count_nonnull_refs_helper(T arg,
                                 Args ... args) {
...
#define EXPECT_JNI_ENV_AND_OBJECT_FOR_CURRENT_JNI(env, thisObj) expectValidJniEnvAndObject(env, thisObj)
...
#if 0
template <typename T, T fn>
struct make_jni_test_decorator<T, fn> {
  static_assert(false, "Invalid function type T, must be function with parameters JNIEnv*, (jclass|jobject) type");
};
#endif
...
#define NORMAL_JNI_ONLY_NOWRAP(func) ({ ASSERT_TRUE(IsCurrentJniNormal()); reinterpret_cast<void*>(&(func)); })
// Same as above, but with nullptr. When we want to test the stub functionality.
#define NORMAL_JNI_ONLY_NULLPTR ({ ASSERT_TRUE(IsCurrentJniNormal()); nullptr; })
...
void JniCompilerTest::UpcallReturnTypeChecking_InstanceImpl() {
  ...
  check_jni_abort_catcher.Check(std::string() + "attempt to return an instance of java.lang.String from java.lang.Class MyClassNatives.instanceMethodThatShouldReturnClass" + CurrentJniStringSuffix() + "()");

  // Here, we just call the method incorrectly; we should catch that too.
  env_->CallObjectMethod(jobj_, jmethod_);
  check_jni_abort_catcher.Check(std::string() + "attempt to return an instance of java.lang.String from java.lang.Class MyClassNatives.instanceMethodThatShouldReturnClass" + CurrentJniStringSuffix() + "()");
  env_->CallStaticObjectMethod(jklass_, jmethod_);
  check_jni_abort_catcher.Check(std::string() + "calling non-static method java.lang.Class MyClassNatives.instanceMethodThatShouldReturnClass" + CurrentJniStringSuffix() + "() with CallStaticObjectMethodV");
}
...
void JniCompilerTest::UpcallReturnTypeChecking_StaticImpl() {
  ...
  // This native method is bad, and tries to return a jstring as a jclass.
  env_->CallStaticObjectMethod(jklass_, jmethod_);
  check_jni_abort_catcher.Check(std::string() + "attempt to return an instance of java.lang.String from java.lang.Class MyClassNatives.staticMethodThatShouldReturnClass" + CurrentJniStringSuffix() + "()");

  // Here, we just call the method incorrectly; we should catch that too.
  env_->CallStaticObjectMethod(jklass_, jmethod_);
  check_jni_abort_catcher.Check(std::string() + "attempt to return an instance of java.lang.String from java.lang.Class MyClassNatives.staticMethodThatShouldReturnClass" + CurrentJniStringSuffix() + "()");
  env_->CallObjectMethod(jobj_, jmethod_);
  check_jni_abort_catcher.Check(std::string() + "calling static method java.lang.Class MyClassNatives.staticMethodThatShouldReturnClass" + CurrentJniStringSuffix() + "() with CallObjectMethodV");
}
...
void JniCompilerTest::UpcallArgumentTypeChecking_InstanceImpl() {
  ...
  check_jni_abort_catcher.Check(std::string() + "bad arguments passed to void MyClassNatives.instanceMethodThatShouldTakeClass" + CurrentJniStringSuffix() + "(int, java.lang.Class)");
}
...
void JniCompilerTest::UpcallArgumentTypeChecking_StaticImpl() {
  ...
  check_jni_abort_catcher.Check(std::string() + "bad arguments passed to void MyClassNatives.staticMethodThatShouldTakeClass" + CurrentJniStringSuffix() + "(int, java.lang.Class)");
}
...
void JniCompilerTest::WithoutImplementationRefReturnImpl() {
  ...
  SetUpForTest(false, "withoutImplementationRefReturn", "()Ljava/lang/Object;", NORMAL_JNI_ONLY_NULLPTR);
