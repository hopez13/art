...
static CompiledMethod* ArtJniCompileMethodInternal(CompilerDriver* driver,
  ...
  bool is_critical_native =
      (static_cast<uint32_t>(optimization_flags) & Compiler::kCriticalNative) != 0;
...
#if 0
    if (kUseReadBarrier) {
      LOG(WARNING) << "JniCompile: unsupported @CriticalNative with read barrier config "
                   << art::PrettyMethod(method_idx, dex_file, /* with signature */ true);
      is_critical_native = false;
      // TODO: support @CriticalNative on read barriers
    }
#endif
  ...
  if (main_jni_conv->SizeOfReturnValue() != 0 && !reference_return) {
    if (LIKELY(!is_critical_native)) {
      ...
      if (!jni_return_reg.Equals(mr_return_reg)) {
        ...
        // XX: I don't think it's ever passed on stack is it? __Store wouldn't support this above.
