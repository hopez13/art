...
static JniCompiledMethod ArtJniCompileMethodInternal(const CompilerOptions& compiler_options,
  ...
  if (LIKELY(!is_critical_native && !is_fast_native)) {
    ...
    if (is_static) {
      ...
      // Add a no-op move for the `this` argument to avoid the next
      // argument being treated as non-null if it's a reference.
      // We have already spilled `this` as raw reference above. Since `this`
      ...
      DCHECK(mr_conv->IsCurrentParamInRegister());  // On all supported architectures.
      src_args.emplace_back(mr_conv->CurrentParamRegister(), kObjectReferenceSize);
      dest_args.emplace_back(mr_conv->CurrentParamRegister(), kObjectReferenceSize);
      refs.push_back(kInvalidReferenceOffset);
      mr_conv->Next();
      main_jni_conv->Next();
    }
