...
enum class JniOptimizationFlags {
  kNone                       = 0x0,
  kFastNative                 = 0x1,
  kCriticalNative             = 0x2,
};
...
static JniCompiledMethod ArtJniCompileMethodInternal(CompilerDriver* driver,
  ...
  const bool is_fast_native = (optimization_flags == JniOptimizationFlags::kFastNative);

  // i.e. if the method was annotated with @CriticalNative
  bool is_critical_native = (optimization_flags == JniOptimizationFlags::kCriticalNative);
...
JniCompiledMethod ArtQuickJniCompileMethod(CompilerDriver* compiler,
  ...
  JniOptimizationFlags optimization_flags = JniOptimizationFlags::kNone;
  if (UNLIKELY(method == nullptr)) {
    ...
    optimization_flags = JniOptimizationFlags::kFastNative;
  } else if (method->IsAnnotatedWithCriticalNative()) {
    DCHECK(!method->IsAnnotatedWithFastNative());  // Checked above.
    optimization_flags = JniOptimizationFlags::kCriticalNative;
  }
