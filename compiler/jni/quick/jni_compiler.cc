...
static JniCompiledMethod ArtJniCompileMethodInternal(const CompilerOptions& compiler_options,
  ...
  if (LIKELY(!is_critical_native)) {
    ...
    saved_cookie_reg = __ CoreRegisterWithSize(callee_save_scratch_regs[1], cookie_size);
    callee_save_temp = __ CoreRegisterWithSize(callee_save_scratch_regs[2], cookie_size);
...
static void PushLocalReferenceFrame(JNIMacroAssembler<kPointerSize>* jni_asm,
  ...
  __ Load(saved_cookie_reg, jni_env_reg, jni_env_cookie_offset, cookie_size);

  // Set the cookie in JNI environment to the current segment state.
  __ Load(temp_reg, jni_env_reg, jni_env_segment_state_offset, cookie_size);
  __ Store(jni_env_reg, jni_env_cookie_offset, temp_reg, cookie_size);
}
...
static void PopLocalReferenceFrame(JNIMacroAssembler<kPointerSize>* jni_asm,
  ...
  __ Load(temp_reg, jni_env_reg, jni_env_cookie_offset, cookie_size);
  __ Store(jni_env_reg, jni_env_segment_state_offset, temp_reg, cookie_size);

  // Restore the cookie in JNI environment to the saved value.
  __ Store(jni_env_reg, jni_env_cookie_offset, saved_cookie_reg, cookie_size);
}
