...
static JniCompiledMethod ArtJniCompileMethodInternal(const CompilerOptions& compiler_options,
  ...
  if (kUseReadBarrier && is_static && !is_critical_native) {
    ...
    if (main_jni_conv->IsCurrentParamOnStack()) {
      ...
      __ Store(out_off, method_register, static_cast<size_t>(kPointerSize));
    } else {
      ManagedRegister out_reg = main_jni_conv->CurrentParamRegister();
      if (!out_reg.Equals(method_register)) {
        __ Move(out_reg, method_register, static_cast<size_t>(kPointerSize));
      }
    }
    main_jni_conv->Next();
  ...
  if (LIKELY(!is_critical_native)) {
    ...
    if (is_synchronized) {
      ...
      if (is_static) {
        ...
        if (main_jni_conv->IsCurrentParamOnStack()) {
          FrameOffset out_off = main_jni_conv->CurrentParamStackOffset();
          __ CreateHandleScopeEntry(out_off, this_offset, /*null_allowed=*/ false);
        } else {
          ManagedRegister out_reg = main_jni_conv->CurrentParamRegister();
  ...
  if (UNLIKELY(is_critical_native)) {
    ...
    if (is_static) {
      ...
      if (main_jni_conv->IsCurrentParamOnStack()) {
        ...
        // TODO: Get hold of the register used to copy the method pointer,
...
static void CopyParameter(JNIMacroAssembler<kPointerSize>* jni_asm,
  ...
  if (ref_param) {
    ...
    handle_scope_offset = mr_conv->CurrentParamStackOffset();
    // Check handle scope offset is in the spill area in the caller's frame.
    CHECK_GT(handle_scope_offset.Uint32Value(), mr_conv->GetDisplacement().Uint32Value());
  }
