...
static JniCompiledMethod ArtJniCompileMethodInternal(const CompilerOptions& compiler_options,
  ...
  // 2.2. Spill all register arguments to preserve them across the `JniLockObject()`
  ...
  // 2.3 Lock the synchronization object (`this` or class) for synchronized methods.
  if (UNLIKELY(is_synchronized)) {
    // We are using a custom calling convention for locking where the assembly thunk
    // gets the object to lock in a register (even on x86). All JNI assemblers must
    // preserve the method register and `this` register when spilling arguments above.
  ...
  // 5. Transition out of runnable (if normal native) and unlock the object (if synchronized).
  ...
  // 5.5 Unlock the synchronization object for synchronized methods.
  if (UNLIKELY(is_synchronized)) {
    ManagedRegister to_lock = main_jni_conv->LockingArgumentRegister();
    mr_conv->ResetIterator(FrameOffset(current_frame_size));
    if (is_static) {
    ...
    // Preserve returned reference across the `JniUnlockObject()` call.
    ManagedRegister saved_reference_register = ManagedRegister::NoRegister();
    if (reference_return) {
      saved_reference_register = __ CoreRegisterWithSize(callee_save_temp, kObjectReferenceSize);
      __ Move(saved_reference_register, mr_conv->ReturnRegister(), kObjectReferenceSize);
    }
    __ CallFromThread(QUICK_ENTRYPOINT_OFFSET(kPointerSize, pJniUnlockObject));
    if (reference_return) {
      __ Move(mr_conv->ReturnRegister(), saved_reference_register, kObjectReferenceSize);
    }
  }
