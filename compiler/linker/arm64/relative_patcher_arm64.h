...
class Arm64RelativePatcher FINAL : public ArmBaseRelativePatcher {
 public:
  enum class BakerReadBarrierKind : uint8_t {
    kOffset,  // Field get or array get with constant offset (i.e. constant index).
    kIndex,   // Array get with index in register.
    kRoot,    // GC root load.
    kLast
  };

  static constexpr uint32_t kInvalidHolderReg = /* sp/zr is invalid */ 31u;

  static uint32_t EncodeBakerReadBarrierData(BakerReadBarrierKind kind,
                                             uint32_t base_reg,
                                             uint32_t holder_reg = kInvalidHolderReg) {
    CheckValidReg(base_reg);
    if (kind == BakerReadBarrierKind::kOffset) {
      CheckValidReg(holder_reg);
    } else {
      DCHECK_EQ(holder_reg, kInvalidHolderReg);
    }
    return BakerReadBarrierKindField::Encode(kind) |
           BakerReadBarrierBaseRegField::Encode(base_reg) |
           BakerReadBarrierHolderRegField::Encode(holder_reg);
  }
 ...
 private:
  ...
  using BakerReadBarrierBaseRegField =
      BitField<uint32_t, kBitsForBakerReadBarrierKind, kBitsForRegister>;
  using BakerReadBarrierHolderRegField =
