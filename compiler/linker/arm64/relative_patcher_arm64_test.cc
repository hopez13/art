...
class Arm64RelativePatcherTest : public RelativePatcherTest {
 ...
 protected:
  ...
  std::vector<uint8_t> CompileBakerOffsetThunk(uint32_t base_reg, uint32_t holder_reg) {
    ...
    auto* patcher = static_cast<Arm64RelativePatcher*>(patcher_.get());
...
TEST_F(Arm64RelativePatcherTestDefault, BakerOffsetThunkInTheMiddle) {
  ...
  AddCompiledMethod(MethodRef(2u), filler1_code, ArrayRef<const LinkerPatch>());

  // Enforce thunk reservation with a tiny method.
  AddCompiledMethod(MethodRef(3u), kNopCode, ArrayRef<const LinkerPatch>());

  // Allow reaching the thunk from the very beginning of a method 1MiB away. Backward branch
  // reaches the full 1MiB. Thinks to subtract:
  ...
  AddCompiledMethod(MethodRef(4u), filler2_code, ArrayRef<const LinkerPatch>());
  ...
  EXPECT_EQ(last_method_offset - first_method_offset, 2 * MB);
...
TEST_F(Arm64RelativePatcherTestDefault, BakerAndMethodCallInteraction) {
  ...
  // TODO: Write the test using Baker thunks and a method call thunk.
}
