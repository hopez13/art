...
ArmBaseRelativePatcher::ThunkKey Arm64RelativePatcher::GetBakerReadBarrierKey(
  ...
  switch (type) {
    ...
    case BakerReadBarrierKind::kIndex:
      params.index_params.base_reg = BakerReadBarrierFirstRegField::Decode(value);
...
std::vector<uint8_t> Arm64RelativePatcher::CompileThunk(const ThunkKey& key) {
  ...
  switch (key.GetType()) {
    ...
    case ThunkType::kBakerReadBarrierRoot: {
      ...
      auto root_reg = Register::GetXRegFromCode(key.GetRootParams().root_reg);
      UseScratchRegisterScope temps(assembler.GetVIXLAssembler());
      temps.Exclude(ip0, ip1);
      __ Add(ip1, ip1, Operand(0x300));
      __ Mov(ip0, root_reg);
      __ Br(ip1);
      break;
    }
