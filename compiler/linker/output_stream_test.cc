...
TEST_F(OutputStreamTest, BufferedFlush) {
  struct CheckingOutputStream : OutputStream {
    ...
    bool WriteFully(const void* buffer [[maybe_unused]],
                    size_t byte_count [[maybe_unused]]) override {
      LOG(FATAL) << "UNREACHABLE";
      UNREACHABLE();
    }

    off_t Seek(off_t offset [[maybe_unused]], Whence whence [[maybe_unused]]) override {
      LOG(FATAL) << "UNREACHABLE";
      UNREACHABLE();
    }
