...
class ArmBaseRelativePatcher : public RelativePatcher {
 ...
 protected:
  ...
  enum class ThunkType {
    ...
    kBakerReadBarrierOffset,  // Baker read barrier, load field or array element at known offset.
    kBakerReadBarrierIndex,   // Baker read barrier, array load with index in register.
  ...
  struct BakerReadBarrierIndexParams {
  ...
  union ThunkParams {
    ...
    BakerReadBarrierIndexParams index_params;
  ...
  class ThunkKey {
   public:
    ...
    BakerReadBarrierOffsetParams GetOffsetParams() const {
      DCHECK(type_ == ThunkType::kBakerReadBarrierOffset);
      return params_.offset_params;
    }

    BakerReadBarrierIndexParams GetIndexParams() const {
      DCHECK(type_ == ThunkType::kBakerReadBarrierIndex);
      return params_.index_params;
    }
