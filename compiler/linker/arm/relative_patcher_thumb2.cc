...
static void EmitNullCheckSlowPath(arm::ArmVIXLAssembler& assembler, vixl::aarch32::Label* label) {
  using namespace vixl::aarch32;  // NOLINT(build/namespaces)
  // Add null check slow path. The stack map is at the address pointed to by LR.
  DCHECK(label->IsReferenced());
  __ Bind(label);
  ...
  switch (kind) {
    case BakerReadBarrierKind::kField: {
      ...
      if (width == BakerReadBarrierWidth::kWide) {
        ...
        __ Ldrh(ip, ldr_address);             // Load the LDR immediate half-word with "Rt | imm12".
      ...
      if (holder_reg.Is(base_reg)) {
        EmitNullCheckSlowPath(assembler, &throw_npe);   // Add null check slow path if needed.
      }
      break;
    }
    case BakerReadBarrierKind::kArray: {
