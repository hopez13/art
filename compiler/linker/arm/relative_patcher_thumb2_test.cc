...
class Thumb2RelativePatcherTest : public RelativePatcherTest {
 ...
 protected:
  ...
  // LDR register, lsl #2. Bits 5-6 are the imm2, i.e. the lsl shift.
...
void Thumb2RelativePatcherTest::TestBakerField(uint32_t offset, uint32_t ref_reg) {
  ...
  for (uint32_t base_reg : valid_regs) {
    for (uint32_t holder_reg : valid_regs) {
      uint32_t ldr = kLdrWInsn | (offset >> 2) | (base_reg << 16) | (ref_reg << 12);
...
TEST_F(Thumb2RelativePatcherTest, BakerGcRoot) {
  ...
  for (uint32_t root_reg : valid_regs) {
    ++method_idx;
    uint32_t ldr = kLdrWInsn | (/* offset */ 8 >> 2) | (/* base_reg */ 0 << 16) | (root_reg << 12);
...
TEST_F(Thumb2RelativePatcherTest, BakerGcRootOffsetBits) {
  ...
  const uint32_t ldr = kLdrWInsn
      | (/* offset */ 8 >> 2) | (/* base_reg */ 0 << 16) | (/* root_reg */ 0 << 12);
