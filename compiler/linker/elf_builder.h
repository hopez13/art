...
//   .vdex                       - Reserved NOBITS space for mapping of the vdex file.
 ...
 public:
  ...
  class SymbolSection FINAL : public Section {
   public:
    ...
    void Sort() {
      // Sort all symbols by absolute address for sanity.
  ...
  ElfBuilder(InstructionSet isa, const InstructionSetFeatures* features, OutputStream* output)
      : isa_(isa),
        ...
        vdex_(this, ".vdex", SHT_NOBITS, SHF_ALLOC, nullptr, 0, kPageSize, 0),
  ...
  void PrepareDynamicSection(const std::string& elf_file_path,
    ...
    if (vdex_size != 0) {
      vdex_.AllocateVirtualMemory(vdex_size);
    }
    ...
    if (vdex_size != 0u) {
      Elf_Word oatvdex = dynstr_.Add("oatvdex");
      dynsym_.Add(oatvdex, &vdex_, vdex_.GetAddress(), vdex_size, STB_GLOBAL, STT_OBJECT);
      Elf_Word oatvdexlastword = dynstr_.Add("oatvdexlastword");
      Elf_Word oatvdexlastword_address = vdex_.GetAddress() + vdex_size - 4;
      dynsym_.Add(oatvdexlastword, &vdex_, oatvdexlastword_address, 4, STB_GLOBAL, STT_OBJECT);
    }
