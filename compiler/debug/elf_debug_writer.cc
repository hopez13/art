...
std::vector<uint8_t> MakeElfFileForJIT(
  ...
  if (kIsDebugBuild) {
    ...
    size_t num_cies = 0;
    size_t num_fdes = 0;
    ElfDebugReader<ElfTypes> reader(buffer);
    ...
    reader.VisitDebugFrame([&](const auto* cie ATTRIBUTE_UNUSED) {
      num_cies++;
    }, [&](const auto* fde, const auto* cie ATTRIBUTE_UNUSED) {
...
std::vector<uint8_t> PackElfFileForJIT(
  ...
  std::vector<uint8_t> inner_elf_file;
  {
    ...
    std::deque<ElfDebugReader<ElfTypes>> readers;
    ...
    for (ElfDebugReader<ElfTypes>& reader : readers) {
    ...
    *num_symbols = symbols.size();
    ...
    for (ElfDebugReader<ElfTypes>& reader : readers) {
      reader.VisitDebugFrame([&](const auto* cie) {
        if (!copied_cie) {
