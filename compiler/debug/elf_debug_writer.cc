...
std::vector<uint8_t> MakeElfFileForJIT(InstructionSet isa,
                                       const InstructionSetFeatures* features [[maybe_unused]],
  ...
  if (kIsDebugBuild) {
    ...
    reader.VisitDebugFrame([&](const Reader::CIE* cie [[maybe_unused]]) { num_cies++; },
                           [&](const Reader::FDE* fde, const Reader::CIE* cie [[maybe_unused]]) {
