...
template <typename ElfTypes>
void PrettyPrintElf(const uint8_t* elf_file, const char* title) {
  using Elf_Addr = typename ElfTypes::Addr;
  using Elf_Sym = typename ElfTypes::Sym;
  LOG(INFO) << title;
  ReadElfSymbols<ElfTypes>(
      ...
      [&](Elf_Sym sym, const char* name) {
        LOG(INFO) << "  Symbol " << std::hex << sym.st_value << " " << sym.st_size << " " << name;
      },
      [&](Elf_Addr addr, Elf_Addr size, ArrayRef<const uint8_t> opcodes) {
        uint32_t hash = FNVHash<ArrayRef<const uint8_t>>()(opcodes);
        LOG(INFO) << "  FDE " << std::hex << addr << " " << size << " opcodes_hash=" << hash;
      });
}
...
std::vector<uint8_t> PackElfFileForJIT(
  ...
  // Produce the main ELF file.
  std::vector<uint8_t> inner_elf_file;
  {
    inner_elf_file.reserve(KB);
    ...
    for (const uint8_t* added_elf_file : added_elf_files) {
      // PrettyPrintElf<ElfTypes>(added_elf_file, "Input ELF file:");
      ReadElfSymbols<ElfTypes>(
          added_elf_file,
          [&](Elf_Sym sym, const char* name) {
  ...
  // Compress the main ELF file and wrap it in another ELF file (to save space).
  ...
  // PrettyPrintElf<ElfTypes>(outer_elf_file.data(), "Packed ELF file:");
