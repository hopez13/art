...
static void WriteDebugSymbols(linker::ElfBuilder<ElfTypes>* builder,
  ...
  if (debug_info.dex_files != nullptr && builder->GetVdex()->Exists() && !mini_debug_info) {
    auto vdex = builder->GetVdex();
    uint64_t dex0_address = vdex->GetAddress() + debug_info.vdex_dex_files_offset;
    for (const DexFile* dex_file : *(debug_info.dex_files)) {
      ...
      symtab->Add(strtab->Write("$dexfile"), vdex, dex_address, 0, STB_LOCAL, STT_NOTYPE);
      for (uint32_t i = 0; i < dex_file->NumClassDefs(); ++i) {
        ...
        for (ClassDataItemIterator it(*dex_file, class_data); it.HasNext(); it.Next()) {
          ...
          uint64_t offset = reinterpret_cast<const uint8_t*>(code_item->insns_) - dex_file->Begin();
          uint64_t address = dex_address + offset;
          size_t size = code_item->ins_size_ * sizeof(code_item->insns_[0]);
          symtab->Add(name_offset, vdex, address, size, STB_GLOBAL, STT_FUNC);
        }
      }
    }
  }
