...
// Special symbol name which identifies the start of dex file.
constexpr const char* kDefFileSymbolName = "$dexfile";
...
static void WriteDebugSymbols(linker::ElfBuilder<ElfTypes>* builder,
  ...
  if (!debug_info.dex_files.empty() && builder->GetDex()->Exists()) {
    ...
    for (auto it : debug_info.dex_files) {
      ...
      symtab->Add(strtab->Write(kDefFileSymbolName), dex, dex_address, 0, STB_LOCAL, STT_NOTYPE);
      if (mini_debug_info) {
