...
TEST_F(DwarfTest, DebugFrame) {
  ...
  DW_CHECK_NEXT("DW_CFA_def_cfa_expression: <decoding error>");
  ...
  DW_CHECK_NEXT("DW_CFA_expression: reg6 <decoding error>");
  opcodes.ValExpression(reg, expr, arraysize(expr));
  DW_CHECK_NEXT("DW_CFA_val_expression: reg6 <decoding error>");
...
TEST_F(DwarfTest, DebugLineSpecialOpcodes) {
  // DW_CHECK_NEXT("warning: last sequence in debug line table at offset 0x00000000 is not terminated");
  const bool is64bit = false;
  const int code_factor_bits = 1;
