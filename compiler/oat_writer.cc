 ...
 public:
  ...
  bool VisitMethod(size_t class_def_method_index ATTRIBUTE_UNUSED,
    ...
    if (compiled_method != nullptr) {
        ...
        // Also process patches with .bss entries in this pass.
...
size_t OatWriter::InitMethodBssMappings(size_t offset) {
  ...
  for (size_t i = 0, size = dex_files_->size(); i != size; ++i) {
    ...
    if (it != bss_method_entry_references_.end()) {
      ...
      offset += MethodBssMapping::ComputeSize(number_of_method_indexes);
      ++number_of_dex_files;
    }
  }
...
size_t OatWriter::WriteMethodBssMappings(OutputStream* out,
  ...
  for (size_t i = 0, size = dex_files_->size(); i != size; ++i) {
    ...
    if (it != bss_method_entry_references_.end()) {
      ...
      size_t number_of_method_indexes = method_indexes.NumSetBits();
      DCHECK_NE(number_of_method_indexes, 0u);
      size_t mappings_size = MethodBssMapping::ComputeSize(number_of_method_indexes);
      std::unique_ptr<uint8_t[]> storage(new uint8_t[mappings_size]);
      ...
      for (uint32_t method_index : method_indexes.Indexes()) {
        init_it->method_index = method_index;
        init_it->bss_offset =
            bss_method_entries_.Get(MethodReference(dex_file, method_index)) - bss_start_;
        ++init_it;
      }
      DCHECK(init_it == mappings->end());

      DCHECK_EQ(relative_offset, oat_dex_file->method_bss_mapping_offset_) << i
          << " " << dex_file->GetLocation();
      DCHECK_OFFSET();
      if (!out->WriteFully(storage.get(), mappings_size)) {
        PLOG(ERROR) << "Failed to write method bss mapping to " << out->GetLocation();
        return 0u;
      }
