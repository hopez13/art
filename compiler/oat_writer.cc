 ...
 public:
  ...
  bool VisitMethod(size_t class_def_method_index ATTRIBUTE_UNUSED, const ClassDataItemIterator& it)
    ...
    if (compiled_method != nullptr &&
        ...
        !compiled_method->GetVmapTable().empty()) {
      ArrayRef<const uint8_t> map = compiled_method->GetVmapTable();
      // Deduplication is already done on a pointer basis by the compiler driver,
      // so we can simply compare the pointers to find out if things are duplicated.
      if (offset_map_->find(map.data()) == offset_map_->end()) {
        ...
        written_bytes_ += length + sizeof(length);
        offset_ += length + sizeof(length);
      }
    }
...
class OatWriter::WriteQuickeningIndicesMethodVisitor : public DexMethodVisitor {
 public:
  WriteQuickeningIndicesMethodVisitor(OatWriter* writer,
                                      OutputStream* out,
                                      uint32_t offset,
  ...
  bool VisitMethod(size_t class_def_method_index ATTRIBUTE_UNUSED,
    ...
    if (compiled_method != nullptr &&
        compiled_method->GetQuickCode().empty() &&
        !compiled_method->GetVmapTable().empty()) {
      uint32_t code_item_offset = it.GetMethodCodeItemOffset();
      ...
      written_bytes_ += sizeof(uint32_t) + sizeof(uint32_t);
      offset_ += sizeof(uint32_t) + sizeof(uint32_t);
    }
    return true;
  }
...
bool OatWriter::WriteQuickeningInfo(OutputStream* vdex_out) {
  ...
  if (compiler_driver_->GetCompilerOptions().IsAnyCompilationEnabled()) {
    ...
    WriteQuickeningIndicesMethodVisitor visitor2(this,
                                                 vdex_out,
                                                 start_offset,
                                                 visitor1.GetNumberOfWrittenBytes(),
                                                 offset_map,
                                                 &dex_files_indices);
    if (!VisitDexMethods(&visitor2)) {
      PLOG(ERROR) << "Failed to write the vdex quickening info. File: " << vdex_out->GetLocation();
      return false;
    }
    ...
    if (!vdex_out->WriteFully(
            dex_files_indices.data(), sizeof(uint32_t) * dex_files_indices.size())) {
      PLOG(ERROR) << "Failed to write the vdex quickening info. File: " << vdex_out->GetLocation();
      return false;
    }
