...
bool OatWriter::SeekToDexFile(OutputStream* vdex_out, File* vdex_file, OatDexFile* oat_dex_file) {
  ...
  off_t actual_offset = vdex_out->Seek(file_offset, kSeekSet);
...
bool OatWriter::OpenDexFiles(
  ...
  LOG(ERROR) << "file = " << vdex_file->GetPath();
  LOG(ERROR) << "map_offset = " << map_offset;
  LOG(ERROR) << "length = " << length;
  LOG(ERROR) << "oat offset = " << oat_data_offset_;
