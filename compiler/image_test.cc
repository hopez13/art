...
class ImageTest : public CommonCompilerTest {
 protected:
  ...
  mirror::Class* GetResolvedType(const char* descriptor,
                                 const DexFile& dex_file,
                                 ObjPtr<mirror::DexCache> dex_cache) REQUIRES_SHARED(Locks::mutator_lock_) {
    const DexFile::TypeId* type_id = dex_file.FindTypeId(descriptor);
    if (type_id == nullptr) {
      return nullptr;
    }
    mirror::Class* klass = dex_cache->GetResolvedType(
        dex_file.GetIndexForTypeId(*type_id));
    return klass;
  }

  ArtMethod* FindCopiedMethod(ArtMethod* origin, mirror::Class* klass) REQUIRES_SHARED(Locks::mutator_lock_) {
...
TEST_F(ImageTest, TestDefaultMethods) {
  ...
  PointerSize pointer_size = class_linker_->GetImagePointerSize();
  ASSERT_EQ(1UL, helper.extra_dex_files.size());
  const DexFile* dex_file = helper.extra_dex_files[0].get();
  Thread* self = Thread::Current();
  ScopedObjectAccess soa(self);
  ObjPtr<mirror::DexCache> dex_cache = class_linker_->FindDexCache(self, *dex_file);
  ...
  mirror::Class* iface_klass = GetResolvedType("LIface;", *dex_file, dex_cache);
  ...
  mirror::Class* impl_klass = GetResolvedType("LImpl;", *dex_file, dex_cache);
  ...
  mirror::Class* iterable_klass = GetResolvedType(
      "Ljava/lang/Iterable;", *dex_file, dex_cache);
  ...
  ASSERT_NE(nullptr, code);
  ASSERT_FALSE(class_linker_->IsQuickToInterpreterBridge(code));
  mirror::Class* iterablebase_klass = GetResolvedType(
      "LIterableBase;", *dex_file, dex_cache);
