...
static jobject CreateGlobalReference(Thread* self, ObjPtr<mirror::Object> obj)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ScopedObjectAccessUnchecked soa(self);
  ScopedLocalRef<jobject> local_class_loader(soa.Env(), soa.AddLocalReference<jobject>(obj));
  return soa.Env()->NewGlobalRef(local_class_loader.get());
}
...
void CommonCompilerTest::CompileMethod(ArtMethod* method) {
  ...
  {
    Thread* self = Thread::Current();
    jobject class_loader = CreateGlobalReference(self, method->GetClassLoader());
