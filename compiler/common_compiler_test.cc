...
class CommonCompilerTestImpl::OneCompiledMethodStorage final : public CompiledCodeStorage {
 public:
  ...
  CompiledMethod* CreateCompiledMethod(InstructionSet instruction_set,
                                       ...
                                       ArrayRef<const uint8_t> cfi [[maybe_unused]],
                                       ArrayRef<const linker::LinkerPatch> patches,
                                       bool is_intrinsic [[maybe_unused]]) override {
  ...
  ArrayRef<const uint8_t> GetThunkCode(const linker::LinkerPatch& patch [[maybe_unused]],
                                       /*out*/ std::string* debug_name [[maybe_unused]]) override {
    LOG(FATAL) << "Unsupported.";
    UNREACHABLE();
  }

  void SetThunkCode(const linker::LinkerPatch& patch [[maybe_unused]],
                    ArrayRef<const uint8_t> code [[maybe_unused]],
                    const std::string& debug_name [[maybe_unused]]) override {
    LOG(FATAL) << "Unsupported.";
    UNREACHABLE();
  }
