...
void CommonCompilerTestImpl::SetUp() {
  {
    ...
    GetRuntime()->SetInstructionSet(instruction_set_);
    for (uint32_t i = 0; i < static_cast<uint32_t>(CalleeSaveType::kLastCalleeSaveType); ++i) {
      CalleeSaveType type = CalleeSaveType(i);
      if (!GetRuntime()->HasCalleeSaveMethod(type)) {
        GetRuntime()->SetCalleeSaveMethod(GetRuntime()->CreateCalleeSaveMethod(), type);
      }
    }
  }
}
...
void CommonCompilerTestImpl::SetUpRuntimeOptions(RuntimeOptions* options ATTRIBUTE_UNUSED) {
...
void CommonCompilerTestImpl::CompileDirectMethod(Handle<mirror::ClassLoader> class_loader,
  ...
  ObjPtr<mirror::Class> klass =
      GetClassLinker()->FindClass(self, class_descriptor.c_str(), class_loader);
  CHECK(klass != nullptr) << "Class not found " << class_name;
  auto pointer_size = GetClassLinker()->GetImagePointerSize();
