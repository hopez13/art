  ...
  bool HasEachKindOfRecord() {
    ...
    for (auto& entry : verifier_deps_->dex_deps_) {
      ...
      has_unverified_classes |=
          std::count(entry.second->verified_classes_.begin(),
                     entry.second->verified_classes_.end(),
                     false) > 0;
      has_redefined_classes |=
          std::count(entry.second->redefined_classes_.begin(),
                     entry.second->redefined_classes_.end(),
                     true) > 0;
    }
