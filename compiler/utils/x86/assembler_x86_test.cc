...
TEST_F(AssemblerX86Test, RepeatAI) {
  EXPECT_NE(RepeatAI(/*f*/ nullptr, /*imm_bytes*/ 1U, "{mem} ${imm}")
            .find("15(%edi,%eax,1) $0\n15(%edi,%eax,1) $-1\n15(%edi,%eax,1) $18\n"
                  "16(%edi,%ebx,2) $0\n16(%edi,%ebx,2) $-1\n16(%edi,%ebx,2) $18\n"),
            std::string::npos);
}
...
TEST_F(AssemblerX86Test, RepeatRA) {
  EXPECT_NE(RepeatRA(/*f*/ nullptr, "%{reg} {mem}")
            .find("%eax 15(%edi,%eax,1)\n%eax 16(%edi,%ebx,2)\n%eax 17(%edi,%ecx,4)\n"
                  "%eax 18(%edi,%edx,8)\n%eax -1(%eax)\n%eax (%ebx)\n%eax 1(%esi)\n"
                  "%eax 987654321(%edi)\n%eax 15(%esp,%eax,1)\n%eax 16(%esp,%ebx,2)\n"),
            std::string::npos);
}
...
TEST_F(AssemblerX86Test, RepeatAR) {
  EXPECT_NE(RepeatAR(/*f*/ nullptr, "{mem} %{reg}")
            .find("15(%edi,%eax,1) %eax\n15(%edi,%eax,1) %ebx\n"
                  "15(%edi,%eax,1) %ecx\n15(%edi,%eax,1) %edx\n"
                  "15(%edi,%eax,1) %ebp\n15(%edi,%eax,1) %esp\n"
                  "15(%edi,%eax,1) %esi\n15(%edi,%eax,1) %edi\n"),
            std::string::npos);
...
TEST_F(AssemblerX86Test, RepeatFA) {
  EXPECT_NE(RepeatFA(/*f*/ nullptr, "%{reg} {mem}")
            .find("%XMM0 15(%edi,%eax,1)\n%XMM0 16(%edi,%ebx,2)\n"
                  "%XMM0 17(%edi,%ecx,4)\n%XMM0 18(%edi,%edx,8)\n"
                  "%XMM0 -1(%eax)\n%XMM0 (%ebx)\n%XMM0 1(%esi)\n"
                  "%XMM0 987654321(%edi)\n%XMM0 15(%esp,%eax,1)\n"),
            std::string::npos);
...
TEST_F(AssemblerX86Test, RepeatAF) {
  EXPECT_NE(RepeatAF(/*f*/ nullptr, "{mem} %{reg}")
            .find("15(%edi,%eax,1) %XMM0\n15(%edi,%eax,1) %XMM1\n15(%edi,%eax,1) %XMM2\n"
                  "15(%edi,%eax,1) %XMM3\n15(%edi,%eax,1) %XMM4\n15(%edi,%eax,1) %XMM5\n"
                  "15(%edi,%eax,1) %XMM6\n15(%edi,%eax,1) %XMM7\n"),
            std::string::npos);
}
