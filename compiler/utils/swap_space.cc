...
static void DumpFreeMap(const FreeBySizeSet& free_by_size) {
  ...
  for (const auto& entry : free_by_size) {
    if (last_size != entry.size_) {
      last_size = entry.size_;
      LOG(INFO) << "Size " << last_size;
    }
    LOG(INFO) << "  0x" << std::hex << entry.free_by_start_entry_->Start()
        << " size=" << std::dec << entry.free_by_start_entry_->size;
  }
}

void SwapSpace::RemoveChunk(FreeBySizeSet::const_iterator free_by_size_pos) {
  auto free_by_start_pos = free_by_size_pos->free_by_start_entry_;
...
static size_t CollectFree(const FreeByStartSet& free_by_start, const FreeBySizeSet& free_by_size) {
  ...
  size_t sum1 = 0;
  for (const auto& entry : free_by_size) {
    sum1 += entry.free_by_start_entry_->size;
  }
...
void* SwapSpace::Alloc(size_t size) {
  ...
  if (it != free_by_size_.end()) {
    auto entry = it->free_by_start_entry_;
    ...
    if (old_chunk.size == size) {
      ...
      it->free_by_start_entry_->ptr += size;
      it->free_by_start_entry_->size -= size;
