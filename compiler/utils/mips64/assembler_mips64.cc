...
void Mips64Assembler::SplatiB(VectorRegister wd, VectorRegister ws, int shamt4) {
  CHECK(IsUint<4>(shamt4)) << shamt4;
  EmitMsaELM(0x1, shamt4 | kMsaDfNByteMask, ws, wd, 0x19);
}

void Mips64Assembler::SplatiH(VectorRegister wd, VectorRegister ws, int shamt3) {
  CHECK(IsUint<3>(shamt3)) << shamt3;
  EmitMsaELM(0x1, shamt3 | kMsaDfNHalfwordMask, ws, wd, 0x19);
}

void Mips64Assembler::SplatiW(VectorRegister wd, VectorRegister ws, int shamt2) {
  CHECK(IsUint<2>(shamt2)) << shamt2;
  EmitMsaELM(0x1, shamt2 | kMsaDfNWordMask, ws, wd, 0x19);
}

void Mips64Assembler::SplatiD(VectorRegister wd, VectorRegister ws, int shamt1) {
  CHECK(IsUint<1>(shamt1)) << shamt1;
  EmitMsaELM(0x1, shamt1 | kMsaDfNDoublewordMask, ws, wd, 0x19);
}
