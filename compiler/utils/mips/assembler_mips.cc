...
void MipsAssembler::AdjustBaseOffsetAndElementSizeShift(Register& base,
  ...
  if (IsInt<16>(offset)) {
    ...
    // Let's treat `offset` as 32-bit to simplify handling of sign
    ...
    tmp += (tmp & (UINT32_C(1) << 15)) << 1;  // Account for sign extension in daddiu.
    int16_t mid = Low16Bits(tmp);
    int16_t hi = High16Bits(tmp);
    // Aui instruction was introduced by R6, so for pre-R6 we are using Lui + Addu instead.
    if (IsR6()) {
      Aui(AT, base, hi);
    } else {
      Lui(AT, hi);
