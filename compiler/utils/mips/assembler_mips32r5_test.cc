...
class AssemblerMIPS32r5Test : public AssemblerTest<mips::MipsAssembler,
 ...
 protected:
  ...
  std::string GetAssemblerCmdName() OVERRIDE {
    // We assemble and link for MIPS32R5. See GetAssemblerParameters() for details.
    return "gcc";
  }

  std::string GetAssemblerParameters() OVERRIDE {
    // We assemble and link for MIPS32R5. The reason is that object files produced for MIPS32R5
    // (and MIPS64R5) with the GNU assembler don't have correct final offsets in PC-relative
    ...
    return " -march=mips32r5 -mmsa -modd-spreg -Wa,--no-warn"
