...
void Arm64JNIMacroAssembler::TestBit(Register reg,
                                     size_t bit,
                                     JNIMacroLabel* label,
                                     JNIMacroUnaryCondition cond) {
  ...
  switch (cond) {
    case JNIMacroUnaryCondition::kZero:
      ___ Tbz(reg, bit, Arm64JNIMacroLabel::Cast(label)->AsArm64());
      break;
    case JNIMacroUnaryCondition::kNotZero:
      ___ Tbnz(reg, bit, Arm64JNIMacroLabel::Cast(label)->AsArm64());
      break;
    default:
      LOG(FATAL) << "Not implemented unary condition: " << static_cast<int>(cond);
      UNREACHABLE();
