...
// STP signed offset for X-register can encode any 4-byte aligned offset smaller than this cutoff.
...
// STP signed offset for D-register can encode any 4-byte aligned offset smaller than this cutoff.
...
void Arm64JNIMacroAssembler::MoveArguments(ArrayRef<ArgumentLocation> dests,
  ...
  // for that case by using LDP/STP, except for situations that arise for
  // normal native even with low number of arguments. The initial spill
  // for outgoing reference regs is done as raw 32-bit values, so we use
  // LDP for the non-reference spilling. We also optimize loading args to
  // registers with LDP, whether references or not, except for the initial
  // non-null reference which we do not need to load at all.
  ...
  while (dest_regs != 0u) {
    ...
    for (size_t i = 0; i != arg_count; ++i) {
      ...
      auto try_mark_processed = [&](ManagedRegister dest_reg) {
        ...
        dest_regs &= ~get_mask(dest.GetRegister());  // Destination register shall be filled.
        return true;
      };
      if (!try_mark_processed(dest.GetRegister())) {
        continue;
      }
      if (src.IsRegister()) {
        if (ref != kInvalidReferenceOffset) {
        ...
        src_regs &= ~get_mask(src.GetRegister());  // Allow clobbering source register.
                 ...
                 try_mark_processed(dests[i + 1u].GetRegister())) {
