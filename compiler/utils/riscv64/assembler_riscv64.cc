...
void Riscv64Assembler::AddConst32(XRegister rd, XRegister rs1, int32_t value) {
  ...
  constexpr int32_t kHighestValueForSimpleAdjustment = 0xffe;
  constexpr int32_t kPositiveValueSimpleAdjustment = 0x7ff;
  constexpr int32_t kLowestValueForSimpleAdjustment = -0x1000;
  constexpr int32_t kNegativeValueSimpleAdjustment = -0x800;

  if (value >= 0 && value <= kHighestValueForSimpleAdjustment) {
    Addiw(rd, rs1, kPositiveValueSimpleAdjustment);
    Addiw(rd, rd, value - kPositiveValueSimpleAdjustment);
  } else if (value < 0 && value >= kLowestValueForSimpleAdjustment) {
    Addiw(rd, rs1, kNegativeValueSimpleAdjustment);
    Addiw(rd, rd, value - kNegativeValueSimpleAdjustment);
  } else {
