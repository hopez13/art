  ...
  if (IsExtensionEnabled(Riscv64Extension::kZca)) {
    if (rd != Zero) {
      if (rs1 != Zero || rs2 != Zero) {
        ...
        // TODO: we use clang for testing assembler and unfortunattly it (clang 18.0.1) does not
        // support conversion from 'add rd, Zero, Zero' into 'c.li. rd, 0' so once clang supports it
        // the lines bellow should be uncommented
...
void Riscv64Assembler::Ret() { Jalr(Zero, RA); }
...
void Riscv64Assembler::FinalizeLabeledBranch(Riscv64Label* label) {
  const uint32_t alignment =
      IsCompressedInstructionsEnabled() ? sizeof(uint16_t) : sizeof(uint32_t);
...
void Riscv64Assembler::Bind(Riscv64Label* label) {
  ...
  while (label->IsLinked()) {
    ...
    uint32_t prev = IsExtensionEnabled(Riscv64Extension::kZca) ?
