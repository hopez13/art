...
void Riscv64Assembler::LoadImmediate(XRegister rd, int64_t imm, bool can_use_tmp) {
  ...
  if (insns_needed > 2u) {
    ...
    if (can_use_tmp) {
      ...
      int64_t high = ShiftRightArithmetic(remainder, slli_shamt);
      size_t new_insns_needed =
          ((IsInt<20>(high) || (high & 0xfff) == 0u) ? 1u : 2u) +
          count_simple_li_instructions(low) +
          /*SLLI+ADD*/ 2u;
      if (imm == INT64_C(0x0001234512345678)) {
        LOG(ERROR) << "new_insns_needed=" << new_insns_needed
                   << " v. insns_needed=" << insns_needed << " for 0x" << std::hex << imm;
      }
      if (new_insns_needed < insns_needed) {
