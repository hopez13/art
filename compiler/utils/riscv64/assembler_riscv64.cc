...
ALWAYS_INLINE static inline int64_t ShiftRightArithmetic(int64_t value, size_t shift) {
  DCHECK_GT(shift, 0u);
  DCHECK_LT(shift, BitSizeOf<int64_t>());
  // Shifting signed values right is implementation-defined behavior.
  // Use unsigned arithmetic to achieve the desired result.
  uint64_t uvalue = static_cast<uint64_t>(value);
  return static_cast<int64_t>((uvalue >> shift) - ((uvalue >> 63) << (64 - shift)));
}
...
void Riscv64Assembler::LoadImmediate(XRegister rd, int64_t imm, bool can_use_tmp) {
  ...
  auto emit_simple_li_helper = [&](int64_t value,
                                   auto&& addi,
                                   auto&& addiw,
                                   auto&& slli,
                                   auto&& lui) {
    ...
    if (IsInt<12>(value)) {
      ...
      addi(rd, Zero, ShiftRightArithmetic(value, CTZ(value)));
  ...
  auto emit_simple_li = [&](int64_t value) {
    emit_simple_li_helper(value,
                          [&](XRegister rd, XRegister rs, int32_t imm) { Addi(rd, rs, imm); },
                          [&](XRegister rd, XRegister rs, int32_t imm) { Addiw(rd, rs, imm); },
                          [&](XRegister rd, XRegister rs, int32_t imm) { Slli(rd, rs, imm); },
                          [&](XRegister rd, uint32_t imm20) { Lui(rd, imm20); });
  };
  auto count_simple_li_instructions = [&](int64_t value) {
    ...
    emit_simple_li_helper(value, count_rri, count_rri, count_rri, count_ru);
    return num_instructions;
  };

  // If LUI+ADDI is not enough, we can generate up to 3 SLLI+ADDI afterwards (up to 8 instructions
  // total). The ADDI from the first SLLI+ADDI pair can be a no-op.
  auto emit_with_slli_addi_helper = [&](int64_t value,
                                        auto&& addi,
                                        auto&& addiw,
                                        auto&& slli,
                                        auto&& lui) {
    ...
    while (!is_simple_li_value(value)) {
      ...
      value = ShiftRightArithmetic(remaining, shamt);
      ++num_sll_addi;
    }
    if (num_sll_addi != 0u && IsInt<20>(value) && !IsInt<12>(value)) {
      ...
      // Emit the signed 20-bit value with LUI reduce the SLLI shamt by 12 to compensate.
      sll_shamts[num_sll_addi - 1u] -= 12u;
      lui(rd, dchecked_integral_cast<uint32_t>(value & 0xfffff));
    } else {
      emit_simple_li_helper(value, addi, addiw, slli, lui);
    }
  ...
  auto emit_with_slli_addi = [&](int64_t value) {
    emit_with_slli_addi_helper(value,
                               [&](XRegister rd, XRegister rs, int32_t imm) { Addi(rd, rs, imm); },
                               [&](XRegister rd, XRegister rs, int32_t imm) { Addiw(rd, rs, imm); },
                               [&](XRegister rd, XRegister rs, int32_t imm) { Slli(rd, rs, imm); },
                               [&](XRegister rd, uint32_t imm20) { Lui(rd, imm20); });
  ...
  auto count_instructions_with_slli_addi = [&](int64_t value) {
    ...
    auto count_rri = [&](XRegister, XRegister, int32_t) { ++num_instructions; };
    auto count_ru = [&](XRegister, uint32_t) { ++num_instructions; };
    emit_with_slli_addi_helper(value, count_rri, count_rri, count_rri, count_ru);
    return num_instructions;
  };
  ...
  if (insns_needed > 2u) {
    ...
    if ((imm & 1) == 0 && (imm & 0xfff) != 0) {
      int64_t value = ShiftRightArithmetic(imm, CTZ(imm));
    ...
    if (imm > 0) {
      ...
      DCHECK_LE(shamt, 32u);  // Oterwise we would not get here as `insns_needed` would be <= 2.
      ...
      if (is_simple_li_value(value)){
        ...
        if (new_insns_needed < insns_needed) {
          ...
          emit_simple_li((value & 0xfff) == 0 ? value : value + shifted_out);
          Srli(rd, rd, shamt);
          return;
        }
      }
      ...
      if (IsInt(ctz + 20, value)) {
        ...
        if (new_insns_needed < insns_needed) {
          ...
          if (IsInt(ctz + 12, value)) {
            Addi(rd, Zero, ShiftRightArithmetic(value, ctz));
            Slli(rd, rd, ctz);
          } else {
    ...
    if (can_use_tmp) {
      ...
      int64_t high = imm - low;
      size_t new_insns_needed =
          count_simple_li_instructions(ShiftRightArithmetic(high, CTZ(high))) + /*SLLI*/ 1u +
          count_simple_li_instructions(low) + /* ADD */ 1u;
      if (new_insns_needed < insns_needed) {
        DCHECK_NE(low & 0xfffff000, 0);
        DCHECK_EQ(high & 0xffffffff, 0);
        LoadImmediate(rd, high, /*can_use_tmp=*/ false);  // Simple `li` + SLLI.
        LoadImmediate(TMP, low, /*can_use_tmp=*/ false);
        Add(rd, rd, TMP);
        return;
      }
    }
  ...
  emit_with_slli_addi(
      trailing_slli_shamt != 0u ? ShiftRightArithmetic(imm, trailing_slli_shamt) : imm);
