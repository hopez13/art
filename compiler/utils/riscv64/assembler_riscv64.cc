...
void Riscv64Assembler::EmitB(uint16_t imm, XRegister rs2, XRegister rs1, int funct3, int opcode) {
  // Discar bit[0], and get low12 bit.
  imm = Low12Bits(imm >> 1);
  uint32_t encoding = (static_cast<uint32_t>(imm) & 0x800) >> 11 << 31 |
                      (static_cast<uint32_t>(imm) & 0x03f0) >> 4 << 25 |
                      static_cast<uint32_t>(rs2) << 20 | static_cast<uint32_t>(rs1) << 15 |
                      static_cast<uint32_t>(funct3) << 12 |
                      (static_cast<uint32_t>(imm) & 0xf) << 8 |
                      (static_cast<uint32_t>(imm) & 0x0400) >> 10 << 7 | opcode;
  Emit(encoding);
}
...
// FP classification instructions (RV32F+RV32D): opcode = 0x53, funct3 = 0x1, funct7 = 0b111X00D
...
void Riscv64Assembler::Beqz(XRegister rs, int32_t imm12) {
  // TODO: CHECK_NE(rs, Zero);
  Beq(rs, Zero, imm12);
}

void Riscv64Assembler::Bnez(XRegister rs, int32_t imm12) {
  // TODO: CHECK_NE(rs, Zero);
  Bne(rs, Zero, imm12);
}

void Riscv64Assembler::Blez(XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Bge(Zero, rt, imm12);
}

void Riscv64Assembler::Bgez(XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Bge(rt, Zero, imm12);
}
void Riscv64Assembler::Bltz(XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Blt(rt, Zero, imm12);
}

void Riscv64Assembler::Bgtz(XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Blt(Zero, rt, imm12);
}

void Riscv64Assembler::Bgt(XRegister rs, XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Blt(rt, rs, imm12);
}

void Riscv64Assembler::Ble(XRegister rs, XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Bge(rt, rs, imm12);
}

void Riscv64Assembler::Bgtu(XRegister rs, XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Bltu(rt, rs, imm12);
}

void Riscv64Assembler::Bleu(XRegister rs, XRegister rt, int32_t imm12) {
  // TODO: CHECK_NE(rt, Zero);
  Bgeu(rt, rs, imm12);
}
...
void Riscv64Assembler::J(int32_t offset) {
  Jal(Zero, (offset & 0x1fffff)); }

void Riscv64Assembler::Call(int32_t offset) {
  if (IsInt<12>(offset)) {
    Jal(RA, offset);
  } else {
    Auipc(RA, High20Bits(offset));
    Jalr(RA, RA, Low12Bits(offset));
  }
}
