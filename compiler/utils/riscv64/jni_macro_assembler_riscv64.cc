...
void Riscv64JNIMacroAssembler::MoveArguments(ArrayRef<ArgumentLocation> dests,
  ...
  while (dest_regs != 0u) {
    ...
    for (size_t i = 0; i != arg_count; ++i) {
      ...
      } else if (src.GetType() == Primitive::kPrimFloat) {
        // For correct NaN-boxing, load to `FTMP` and transfer 64 bits.
        __ FLoadw(FTMP, SP, src.GetFrameOffset().Int32Value());
        DCHECK(dest.GetRegister().AsRiscv64().IsXRegister());
        __ FMvXD(dest.GetRegister().AsRiscv64().AsXRegister(), FTMP);
      } else {
