...
class AssemblerRISCV64Test : public AssemblerTest<riscv64::Riscv64Assembler,
 ...
 protected:
  ...
  void TestLoadConst64(const std::string& test_name,
    ...
    // For these values `LoadConst64()` emits the same code as `Li()`.
    int64_t test_values1[] = {
        ...
        INT64_C(-0x80000001), INT64_C(-0x80000800),  // LUI+ADDI
        ...
        INT64_C(0x0fffffffffff0000), INT64_C(0x0ffffffffffff000),  // LUI+SRLI
    ...
    int64_t test_values2[] = {
        // Li:        LUI+ADDIW+SLLI+ADDI+SLLI+ADDI+SLLI+ADDI
        // LoadConst: LUI+ADDIW+SLLI+LUI+ADDIW+ADD (using TMP)
        INT64_C(0x1234567887654321),
        INT64_C(-0x1234567887654321),

        // Li:        LUI+SLLI+ADDI+SLLI+ADDI+SLLI
        // LoadConst: LUI+SLLI+LUI+ADD (using TMP)
        INT64_C(0x1234500012345000),
    };
    for (int64_t value : test_values2) {
      emit_load_const(A0, value);
      if (can_use_tmp) {
        int64_t low = (value & 0xffffffff) - ((value & 0x80000000) << 1);
        int64_t high = value - low;
        std::string rd = GetRegisterName(A0);
        std::string tmp = GetRegisterName(TMP);
        expected += "li " + rd + ", " + std::to_string(high) + "\n"
                    "li " + tmp + ", " + std::to_string(low) + "\n"
                    "add " + rd + ", " + rd + ", " + tmp + "\n";
      } else {
        expected += "li a0, " + std::to_string(value) + "\n";
      }
