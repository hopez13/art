...
// The clang assembler we're currently using for testing does not auto-forward
// `beq zero, rs2, offset` to `c.beqz` but newer clang assembler versions do.
...
// The clang assembler we're currently using for testing does not auto-forward
// `bne zero, rs2, offset` to `c.bnez` but newer clang assembler versions do.
// TODO (riscv64): Enable this test when we're using a clang assembler that auto-forwards.
