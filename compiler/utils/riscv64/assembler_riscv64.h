...
class Riscv64Assembler final : public Assembler {
 ...
 private:
  ...
  // Rearrange given offset, scaled by 4, in the way {offset[5], offset[4:2] | offset[7:6]}
  static constexpr uint32_t ExtractOffset52_76(int32_t offset) {
    ...
    // The equivalent form is {offset[5:2], offset[7:6]}
  ...
  // Rearrange given offset, scaled by 8, in the way {offset[5], offset[4:3] | offset[8:6]}
  static constexpr uint32_t ExtractOffset53_86(int32_t offset) {
    ...
    // The equivalent form is {offset[5:3], offset[8:6]}
  ...
  // Rearrange given offset, scaled by 4, in the way {offset[5:3], offset[2] | offset[6]}
  static constexpr uint32_t ExtractOffset52_6(int32_t offset) {
    ...
    // The equivalent form is {offset[5:2], offset[6]}
    uint32_t imm_25 = BitFieldExtract(u_offset, 2, 4);
    uint32_t imm_66 = BitFieldExtract(u_offset, 6, 1);

    return BitFieldInsert(imm_66, imm_25, 1, 4);
  }
  ...
  static constexpr uint32_t ExtractOffset53_76(int32_t offset) {
    ...
    uint32_t imm_35 = BitFieldExtract(u_offset, 3, 3);
    uint32_t imm_67 = BitFieldExtract(u_offset, 6, 2);

    return BitFieldInsert(imm_67, imm_35, 2, 3);
  }

  static constexpr bool IsImmCLuiEncodable(uint32_t uimm) {
    // 16.5
    // C.LUI loads the non-zero 6-bit immediate field into bits 17–12 of the destination
    // register, clears the bottom 12 bits, and sign-extends bit 17 into all higher bits
    // of the destination
    // uimm == 0 is reserved.
