...
class Riscv64Assembler final : public Assembler {
 public:
  ...
  void Andi(XRegister rd, XRegister rs1, int64_t imm12);      // AND Immediate, define the imm12 as int64_t type to detect some over range scenario
  ...
  void Ori(XRegister rd, XRegister rs1, int64_t imm12);       // OR Immediate, define the imm12 as int64_t type to detect some over range scenario
  void Xor(XRegister rd, XRegister rs1, XRegister rs2);     // XOR
  void Xori(XRegister rd, XRegister rs1, int64_t imm12);      // Exclusive-OR Immediate, define the imm12 as int64_t type to detect some over range scenario
  ...
  void Slliw(XRegister rd, XRegister rs1, uint16_t shamt5);   // Shift Left Logical Word Immediate (RV64I-only)
  void Srlw(XRegister rd, XRegister rs1, XRegister rs2);    // Shift right Logical Word(RV64I-only)
  void Srliw(XRegister rd, XRegister rs1, uint16_t shamt5);   // Shift right Logical Word Immediate (RV64I-only)
  void Sraw(XRegister rd, XRegister rs1, XRegister rs2);    // Shift Right Arithmetic Word (RV64I-only)
  void Sraiw(XRegister rd, XRegister rs1, uint16_t shamt5);   // Shift Right Arithmetic Word Immediate (RV64I-only)
  ...
  void ScDAq(XRegister rd, XRegister rs2, XRegister rs1);    // Store-Conditional Doubleword with aq bit set
  void ScDRl(XRegister rd, XRegister rs2, XRegister rs1);    // Store-Conditional Doubleword with rl bit set
  void ScDAqrl(XRegister rd, XRegister rs2, XRegister rs1);  // Store-Conditional Doubleword with aq and rl bit set
  ...
  void Fence(uint16_t prec = riscv64::FenceParameters::fence_input | riscv64::FenceParameters::fence_output | riscv64::FenceParameters::fence_read | riscv64::FenceParameters::fence_write,
           uint16_t succ = riscv64::FenceParameters::fence_input | riscv64::FenceParameters::fence_output | riscv64::FenceParameters::fence_read | riscv64::FenceParameters::fence_write);  // Fence Memory and I/O
  ...
  void Bgeu(XRegister rs1, XRegister rs2, uint16_t imm12);    // Branch if Greater Than or Equal, Unsigned
  void Bgt(XRegister rs1, XRegister rs2, uint16_t imm12);     // Pseudo instruction Branch if Greater Than
  void Ble(XRegister rs1, XRegister rs2, uint16_t imm12);     // Pseudo instruction Branch if Less Than or Equal
  void Bgtu(XRegister rs1, XRegister rs2, uint16_t imm12);    // Pseudo instruction Branch if Greater Than, Unsigned
  void Bleu(XRegister rs1, XRegister rs2, uint16_t imm12);    // Pseudo instruction Branch if Less Than or Equal, Unsigned
  ...
  // Arithmetic
  void FaddS(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Add, Single-Precision
  void FsubS(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Subtract, Single-Precision
  void FmulS(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Multiply, Single-Precision
  void FmaddS(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);   // Floating-point Fused Multiply-Add, Single-Precision
  void FmsubS(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);   // Floating-point Fused Multiply- Subtarct, Single-Precision
  void FnmaddS(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);  // Floating-point Fused Negative Multiply-Add, Single-Precision
  void FnmsubS(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);  // Floating-point Fused Negative Multiply-Subtract, Single-Precision
  void FdivS(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Divide, Single-Precision
  void FsqrtS(FRegister frd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);                     // Floating-point Square Root, Single-Precision

  // Move
  void FmvS(FRegister frd, FRegister frs1);                    // Pseudo Instruction Floating-point Move Floating-point, Single-Precision
  void FmvWX(FRegister frd, XRegister rs1);                    // Floating-point Move Word from Integer, Single-Precision
  void FmvXW(XRegister rd, FRegister frs1);                    // Floating-point Move Word to Integer, Single-Precision

  void FclassS(XRegister rd, FRegister frs1);                  // Floating-point Classify, Single-Precision

  void FcvtLS(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Long from Single
  void FcvtLuS(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Unsigned Long from Single
  void FcvtWS(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Word from Single
  void FcvtWuS(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert toUnsigned Word from Single
  void FcvtSL(FRegister frd, XRegister rs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Single From Long
  void FcvtSLu(FRegister frd, XRegister rs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Single from Unsigned Long
  void FcvtSW(FRegister frd, XRegister rs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Single From Word
  void FcvtSWu(FRegister frd, XRegister rs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Single From Unsigned Word

  void FmaxS(FRegister frd, FRegister frs1, FRegister frs2);    // Floating-point Maximum, Single-Precision
  void FminS(FRegister frd, FRegister frs1, FRegister frs2);    // Floating-point MiNImum, Single-Precision
  ...
  void FsgnjS(FRegister frd, FRegister frs1, FRegister frs2);   // Floating-point Sign Inject, Single-Precision
  void FsgnjnS(FRegister frd, FRegister frs1, FRegister frs2);  // Floating-point Sign Inject-Negate, Single-Precision
  void FsgnjxS(FRegister frd, FRegister frs1, FRegister frs2);  // Floating-point Sign Inject-Xor, Single-Precision

  // Compare
  void FeqS(XRegister rd, FRegister frs1, FRegister frs2);      // Floating-point Equals, Single-Precision
  void FleS(XRegister rd, FRegister frs1, FRegister frs2);      // Floating-point Less Than or Equals, Single-Precision
  void FltS(XRegister rd, FRegister frs1, FRegister frs2);      // Floating-point Less Than, Single-Precision
  ...
  // Arithmetic
  void FaddD(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Add, Double-Precision
  void FsubD(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Subtract, Double-Precision
  void FmulD(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Multiply, Double-Precision
  void FmaddD(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);   // Floating-point Fused Multiply-Add, Double-Precision
  void FmsubD(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);   // Floating-point Fused Multiply- Subtract, Double-Precision
  void FnmaddD(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);  // Floating-point Fused Negative Multiply-Add, Double-Precision
  void FnmsubD(FRegister frd, FRegister frs1, FRegister frs2, FRegister frs3, RoundingMode rm = RoundingMode::DYN);  // Floating-point Fused Negative Multiply-Subtract, Double-Precision
  void FdivD(FRegister frd, FRegister frs1, FRegister frs2, RoundingMode rm = RoundingMode::DYN);    // Floating-point Divide, Double-Precision
  void FsqrtD(FRegister frd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Square Root, Double-Precision

  void FmvDX(FRegister frd, XRegister rs1);                    // Floating-point Move Word from Integer, Double-Precision
  void FmvXD(XRegister rd, FRegister frs1);                    // Floating-point Move Word to Integer, Double-Precision
  void FmvD(FRegister frd, FRegister frs1);                    // Pseudo Instruction Floating-point Move Double to Double

  void FclassD(XRegister rd, FRegister frs1);                  // Floating-point Classify, Double-Precision

  void FcvtLD(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Long from Double
  void FcvtLuD(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Unsigned Long from Double
  void FcvtWD(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Word from Double-Precision
  void FcvtWuD(XRegister rd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Unsigned Word from Double-Precision
  void FcvtDL(FRegister frd, XRegister rs1, RoundingMode rm = RoundingMode::DYN);   // Floating-point Convert to Double From Long
  void FcvtDLu(FRegister frd, XRegister rs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Double from Unsigned Long
  void FcvtDW(FRegister frd, XRegister rs1);                          // Floating-point Convert to Double From Word, widening convertion no RM field
  void FcvtDWu(FRegister frd, XRegister rs1);                         // Floating-point Convert to Double From Unsigned Word, widening convertion no RM field
  void FcvtDS(FRegister frd, FRegister frs1);                         // Floating-point Convert to Double from Single, widening convertion no RM field
  void FcvtSD(FRegister frd, FRegister frs1, RoundingMode rm = RoundingMode::DYN);  // Floating-point Convert to Single from Double

  void FmaxD(FRegister frd, FRegister frs1, FRegister frs2);    // Floating-point Maximum, Double-Precision
  void FminD(FRegister frd, FRegister frs1, FRegister frs2);    // Floating-point Minimum, Double-Precision
  ...
  void FsgnjD(FRegister frd, FRegister frs1, FRegister frs2);   // Floating-point Sign Inject, Double-Precision
  void FsgnjnD(FRegister frd, FRegister frs1, FRegister frs2);  // Floating-point Sign Inject-Negate, Double-Precision
  void FsgnjxD(FRegister frd, FRegister frs1, FRegister frs2);  // Floating-point Sign Inject-Xor, Double-Precision

  // Compare
  void FeqD(XRegister rd, FRegister frs1, FRegister frs2);      // Floating-point Equals, Double-Precision
  void FleD(XRegister rd, FRegister frs1, FRegister frs2);      // Floating-point Less Than or Equals, Double-Precision
  void FltD(XRegister rd, FRegister frs1, FRegister frs2);      // Floating-point Less Than, Double-Precision
 ...
 public:
    ...
    switch (type) {
      ...
      case kLoadDoubleword:
        if (!IsAligned<kRiscv64DoublewordSize>(offset)) {
          ...
          // Implicit null checks must be associated with the first 32-bit load/store in a pair and not the last
          null_checker();
          Slli(TMP2, TMP2, 32);
          Lwu(AT, base, offset);
          Add(AT, AT, TMP2);
  ...
  // ART_PORTING_TO_DO: Inc(Dec)reaseFrameSize belong to base class JNIMacroAssembler.
  // After JNIMacroAssembler is implemented, should add declaration 'override'.
  void IncreaseFrameSize(size_t adjust);
  void DecreaseFrameSize(size_t adjust);

  // TODO: will analyze it at stage 2
 ...
 private:
  ...
  void EmitR(int32_t funct7, Register rs2, Register rs1, int32_t funct3, Register rd, int32_t opcode);
  void EmitR4(Register rs3, int32_t funct2, Register rs2, Register rs1, int32_t funct3, Register rd, int32_t opcode);
  void EmitI(uint16_t imm12, XRegister rs1, int32_t funct3, Register rd, int32_t opcode);
  void EmitS(uint16_t imm7, Register rs2, XRegister rs1, int32_t funct3, uint16_t imm5, int32_t opcode);
