...
class Riscv64Assembler final : public Assembler {
 public:
  ...
  void CLwsp(XRegister rd, uint32_t offset);
  void CLdsp(XRegister rd, uint32_t offset);
  void CFLdsp(FRegister rd, uint32_t offset);
  void CSwsp(XRegister rs2, uint32_t offset);
  void CSdsp(XRegister rs2, uint32_t offset);
  void CFSdsp(FRegister rs2, uint32_t offset);

  void CLw(XRegister rd_s, XRegister rs1_s, uint32_t offset);
  void CLd(XRegister rd_s, XRegister rs1_s, uint32_t offset);
  void CFLd(FRegister rd_s, XRegister rs1_s, uint32_t offset);
  void CSw(XRegister rs2_s, XRegister rs1_s, uint32_t offset);
  void CSd(XRegister rs2_s, XRegister rs1_s, uint32_t offset);
  void CFSd(FRegister rs2_s, XRegister rs1_s, uint32_t offset);
 ...
 private:
  ...
  //   ---------------------------------
  //
  void EmitCR(uint16_t funct4, XRegister rd_rs1, XRegister rs2, uint16_t opcode) {
  ...
  //
  template <typename Reg>
  void EmitCI(uint16_t funct3, Reg rd_rs1, uint16_t imm6, uint16_t opcode) {
    DCHECK(IsUint<3>(funct3));
    DCHECK(IsUint<5>(static_cast<uint32_t>(rd_rs1)));
  ...
  //
  template <typename Reg>
  void EmitCM(uint16_t funct3, uint16_t imm5, XRegister rs1_s, Reg rd_rs2_s, uint16_t opcode) {
    DCHECK(IsUint<3>(funct3));
    DCHECK(IsUint<5>(imm5));
    DCHECK(IsShortReg(rs1_s));
    DCHECK(IsShortReg(rd_rs2_s));
    DCHECK(IsUint<2>(opcode));
  ...
  //   ---------------------------------
  //
  void EmitCA(uint16_t funct6, XRegister rd_s1_s, uint16_t funct2, XRegister rs2_s, uint16_t opcode) {
    DCHECK(IsUint<6>(funct6));
    DCHECK(IsShortReg(rd_s1_s));
    DCHECK(IsUint<2>(funct2));
    DCHECK(IsShortReg(rs2_s));
    DCHECK(IsUint<2>(opcode));

    uint32_t encoding = funct6 << 10 | EncodeShortReg(rd_s1_s) << 7 |
                        static_cast<uint32_t>(funct2) << 5 | EncodeShortReg(rs2_s) << 2 | opcode;
    Emit16(encoding);
  }
  ...
  //   ---------------------------------
  //
  void EmitCB(uint16_t funct3, int32_t offset8, XRegister rs1_s, uint16_t opcode) {
    DCHECK(IsUint<3>(funct3));
  ...
  //   ---------------------------------
  //
  void EmitCJ(uint16_t funct3, int32_t offset, uint16_t opcode) {
    DCHECK_ALIGNED(offset, 2);
    ...
    uint32_t encoding = funct3 << 13 | static_cast<uint32_t>(jumpt) << 2 | opcode;
    Emit16(encoding);
  }
