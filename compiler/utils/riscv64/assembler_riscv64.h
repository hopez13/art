...
// The `Riscv64Extension` enumeration is used for restricting the instructions that the
// assembler can use. Some restrictions are checked only in debug mode (for example load and
// store instructions check `kLoadStore`), other restrictions are checked at run time and
// affect the emitted code (for example, the `SextW()` pseudo-instruction selects between an
// implementation from "Zcb", "Zbb" and a two-instruction sequence from basic instruction set.
 ...
 private:
  static uint32_t ConvertExtensions(
    ...
    return
        ...
        (instruction_set_features->HasCompressed() ? kRiscv64CompressedExtensionsMask : 0u);
  }
...
using ScopedCExclusion = ScopedExtensionsExclusion<kRiscv64CompressedExtensionsMask>;
