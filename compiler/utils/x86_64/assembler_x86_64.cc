...
void X86_64Assembler::andn(CpuRegister dst, CpuRegister src1, CpuRegister src2) {
  ...
  uint8_t byte_one = EmitVexByte1(dst.NeedsRex(), /*x*/ false, src2.NeedsRex(), 2);
  uint8_t byte_two = EmitVexByte2(/*w*/ true, 128,
                                  X86_64ManagedRegister::FromCpuRegister(src1.AsRegister()), 0);
  EmitUint8(byte_zero);
  EmitUint8(byte_one);
  EmitUint8(byte_two);
  // Opcode field
...
void X86_64Assembler::blsi(CpuRegister dst, CpuRegister src) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  uint8_t byte_zero = EmitVexByteZero(/*is_two_byte*/ false);
  uint8_t byte_one = EmitVexByte1(/*r*/ false, /*x*/ false, src.NeedsRex(), 2);
  uint8_t byte_two = EmitVexByte2(/*w*/ true, 128,
                                  X86_64ManagedRegister::FromCpuRegister(dst.AsRegister()), 0);
  EmitUint8(byte_zero);
  EmitUint8(byte_one);
  EmitUint8(byte_two);
  EmitUint8(0xF3);
