...
class ArmVIXLAssemblerTest : public AssemblerTestBase {
 ...
 protected:
  ...
  void DumpAndCheck(std::vector<uint8_t>& code, const char* testname, const std::string& expected) {
    ...
    // objdump on buildbot seems to sometimes add annotation like "bne #226 <.text+0x1e8>".
    // It unclear why it does not reproduce locally. As work-around, remove the annotation.
    disassembly = Replace(disassembly, " <.text+", "", /*remove_rest_of_line=*/true);
