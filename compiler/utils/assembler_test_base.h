...
class AssemblerTestBase : public testing::Test {
 public:
  ...
  void Driver(const std::vector<uint8_t>& art_code,
    ...
    std::string ref_asm_file = scratch_dir_->GetPath() + test_name + ".ref.S";
    WriteFile(ref_asm_file, assembly_text.data(), assembly_text.size());

    // Assemble reference object file.
    std::string ref_obj_file = scratch_dir_->GetPath() + test_name + ".ref.o";
    ...
    std::string art_obj_file = scratch_dir_->GetPath() + test_name + ".art.o";
    if (Is64BitInstructionSet(isa)) {
      WriteElf<true>(art_obj_file, isa, art_code);
    } else {
      WriteElf<false>(art_obj_file, isa, art_code);
    }
    ...
    Replace(art_disassembly, art_obj_file, scratch_dir_->GetPath() + "<filename>");
    std::string ref_disassembly;
    ASSERT_TRUE(Disassemble(ref_obj_file, &ref_disassembly));
    Replace(ref_disassembly, ref_obj_file, scratch_dir_->GetPath() + "<filename>");
    ...
    if (art_code.size() > ref_code.size()) {
      ...
      EXPECT_TRUE(false) << "ART code is smaller then the reference code. Too good to be true?";
    } else {
