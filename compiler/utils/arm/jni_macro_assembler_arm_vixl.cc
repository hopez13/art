...
static inline bool NoSpillGap(const ArgumentLocation& src1, const ArgumentLocation& src2) {
  DCHECK(!src1.IsRegister());
  DCHECK(!src2.IsRegister());
  uint32_t src1_offset = src1.GetFrameOffset().Uint32Value();
  uint32_t src2_offset = src2.GetFrameOffset().Uint32Value();
  DCHECK_LT(src1_offset, src2_offset);
  return src1_offset + src1.GetSize() == src2_offset;
}
...
static inline size_t GetSpillChunkSize(ArrayRef<ArgumentLocation> dests,
  ...
  if (!first_src.IsRegister()) {
    ...
    // at a time when the source types are word-sized.
    if (have_extra_temp &&
        start + 1u != dests.size() &&
        !srcs[start + 1u].IsRegister() &&
        NoSpillGap(first_src, srcs[start + 1u]) &&
        dests[start].GetSize() == 4u &&
        dests[start + 1u].GetSize() == 4u &&
        NoSpillGap(dests[start], dests[start + 1u]) &&
        dests[start].GetFrameOffset().Uint32Value() < 1024u) {  // Max STRD immediate offset.
...
static inline bool UseStrdForChunk(ArrayRef<ArgumentLocation> dests,
                                   ArrayRef<ArgumentLocation> srcs,
                                   size_t start,
                                   size_t length) {
  DCHECK_GE(length, 2u);
  ...
  // Note: The destination may not be 8B aligned (but it is 4B aligned).
  return length == 2u &&
...
static inline bool UseVstrForChunk(ArrayRef<ArgumentLocation> dests,
                                   ArrayRef<ArgumentLocation> srcs,
                                   size_t start,
                                   size_t length) {
  DCHECK_GE(length, 2u);
  ...
  return length == 2u &&
         ...
         dests[start].GetFrameOffset().Uint32Value() < 1024u;  // Max VSTR offset.
}
...
void ArmVIXLJNIMacroAssembler::MoveArguments(ArrayRef<ArgumentLocation> dests,
  ...
  DCHECK(std::all_of(
      ...
      [&](const ArgumentLocation& loc) { return is_core_reg(loc.GetRegister().AsArm()); }));
  ...
  // Spill args first. Look for opportunities to spill multiple arguments at once.
  {
    ...
    vixl32::Register saved_base_reg;  // Base register initialized from post-increment spill.
    ...
    auto move_two_words = [&](FrameOffset dest_offset, FrameOffset src_offset) {
      ...
      DCHECK(!saved_base_reg.IsValid());
      DCHECK_LT(dest_offset.Uint32Value(), 1024u);  // Max STRD immediate offset.
    ...
    for (size_t i = num_reg_dests, arg_count = dests.size(); i != arg_count; ) {
      ...
      if (chunk_size == 1u) {
        ...
        } else if (dest.GetSize() == 8u && xtemp.IsValid() && frame_offset < 1024u) {
        ...
        move_two_words(dest.GetFrameOffset(), src.GetFrameOffset());
      } else if (UseStrdForChunk(dests, srcs, i, chunk_size)) {
        DCHECK(!saved_base_reg.IsValid());
        ___ Strd(AsVIXLRegister(srcs[i].GetRegister().AsArm()),
                 AsVIXLRegister(srcs[i + 1u].GetRegister().AsArm()),
                 MemOperand(sp, frame_offset));
      } else if (UseVstrForChunk(dests, srcs, i, chunk_size)) {
        DCHECK(!saved_base_reg.IsValid());
        size_t sreg = static_cast<size_t>(srcs[i].GetRegister().AsArm().AsSRegister());
        DCHECK_ALIGNED(sreg, 2u);
        ___ Vstr(vixl32::DRegister(sreg / 2u), MemOperand(sp, frame_offset));
      } else {
        bool write_back = false;
        if (frame_offset != 0u) {  // We use SP for offset 0.
          size_t next_chunk_size = GetSpillChunkSize(dests, srcs, i + chunk_size, xtemp.IsValid());
          write_back = (next_chunk_size >= 2u) &&
                       NoSpillGap(dests[i + chunk_size - 1u], dests[i + chunk_size]) &&
                       !UseStrdForChunk(dests, srcs, i + chunk_size, next_chunk_size) &&
                       !UseVstrForChunk(dests, srcs, i + chunk_size, next_chunk_size);
        }
        ...
        if (frame_offset == 0u) {
          ...
          DCHECK(!write_back);
          base_reg = sp;
        } else if (saved_base_reg.IsValid()) {
          base_reg = saved_base_reg;
        } else {
          base_reg = temps.Acquire();
        ...
        if (IsCoreRegisterOrPair(src_reg)) {
          ...
          ___ Stm(base_reg, write_back ? WRITE_BACK : NO_WRITE_BACK, RegisterList(core_reg_mask));
        } else {
          ...
          if (IsAligned<2u>(start_sreg) &&
            ...
            ___ Vstm(F64, base_reg, write_back ? WRITE_BACK : NO_WRITE_BACK, dreg_list);
          } else {
            uint32_t sreg_count = total_size / kSRegSizeInBytes;
            SRegisterList sreg_list(vixl32::SRegister(start_sreg), sreg_count);
            ___ Vstm(F32, base_reg, write_back ? WRITE_BACK : NO_WRITE_BACK, sreg_list);
          }
        }

        if (write_back) {
          DCHECK(!base_reg.Is(sp));
          saved_base_reg = base_reg;
        } else {
          saved_base_reg = vixl32::Register();
          if (!base_reg.Is(sp)) {
            temps.Release(base_reg);
          }
        }
      }
      i += chunk_size;
  ...
  while (src_regs != 0u) {
    ...
    for (size_t i = 0; i != num_reg_dests; ++i) {
      ...
      // There are also no partial overlaps of 8-byte arguments, otherwise we would have
      // to tweak this check; Move() can deal with partial overlap for historical reasons.
  ...
  // Now fill destination registers from FP registers of stack slots, looking for
    ...
    if (dests[i].GetSize() == 4u) {
      ...
      if (j != num_reg_dests && dests[j].GetSize() == 4u) {
        if (!srcs[i].IsRegister() &&
            !srcs[j].IsRegister() &&
            NoSpillGap(srcs[i], srcs[j]) &&
            srcs[i].GetFrameOffset().Uint32Value() < 1024u) {  // Max LDRD immediate offset.
