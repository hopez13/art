...
void DexToDexCompiler::MarkForCompilation(Thread* self,
  ...
  active_dex_file_ = method_ref.dex_file;
  DCHECK_EQ(method_ref.dex_file, active_dex_file_) << method_ref.PrettyMethod();
  MutexLock lock(self, lock_);
  // Detect the duplicate code items.
  ...
  should_quicken_.insert(method_ref);
}
...
bool DexToDexCompiler::ShouldCompileMethod(const MethodReference& ref) const {
  return should_quicken_.find(ref) != should_quicken_.end();
}
  ...
  if (UNLIKELY(shared_code_items_.find(code_item) != shared_code_items_.end())) {
    ...
    const bool already_quickened = existing != shared_code_item_quicken_info_.end();
    {
      CompilationState state(this,
                             unit,
                             compilation_level,
                             already_quickened ? &existing->second : nullptr);
      quicken_data = state.Compile();
    }

    // Already quickened, check that the data matches what was previously seen.
    if (already_quickened) {
      const std::vector<uint8_t>& existing_data(existing->second);
      if (existing_data != quicken_data) {
        ...
        optimizer::ArtDecompileDEX(dex_file,
                                   *code_item,
                                   ArrayRef<const uint8_t>(existing_data),
      ...
      bool inserted = shared_code_item_quicken_info_.emplace(code_item, quicken_data).second;
