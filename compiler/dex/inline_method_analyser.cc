...
class Matcher {
 public:
  ...
  static bool Match(const CodeItemData* code_item, MatchFn* const (&pattern)[size]);
 ...
 private:
  explicit Matcher(const CodeItemData* code_item)
      : code_item_(code_item),
        instruction_(code_item->Instructions().begin()) {}

  static bool DoMatch(const CodeItemData* code_item, MatchFn* const* pattern, size_t size);

  const CodeItemData* const code_item_ = nullptr;
...
template <size_t size>
bool Matcher::Match(const CodeItemData* code_item, MatchFn* const (&pattern)[size]) {
...
bool Matcher::DoMatch(const CodeItemData* code_item, MatchFn* const* pattern, size_t size) {
...
size_t CountForwardedConstructorArguments(const CodeItemData* code_item,
...
bool DoAnalyseConstructor(const CodeItemData* code_item,
                          ArtMethod* method,
                          /*inout*/ ConstructorIPutData (&iputs)[kMaxConstructorIPuts])
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  for (const DexInstructionPcPair& pair : code_item->Instructions()) {
    ...
    if (instruction.Opcode() == Instruction::RETURN_VOID) {
      ...
      if (target_method->GetDeclaringClass()->IsObjectClass()) {
        DCHECK_EQ(CodeItemData(target_method).Instructions().begin()->Opcode(),
                  Instruction::RETURN_VOID);
      } else {
        NullableCodeItemData target_code_item(target_method);
...
bool AnalyseConstructor(const CodeItemData* code_item,
                        ArtMethod* method,
                        InlineMethod* result)
    REQUIRES_SHARED(Locks::mutator_lock_) {
...
bool InlineMethodAnalyser::AnalyseMethodCode(ArtMethod* method, InlineMethod* result) {
  NullableCodeItemData code_item(method);
...
bool InlineMethodAnalyser::AnalyseMethodCode(const CodeItemData* code_item,
...
bool InlineMethodAnalyser::AnalyseReturnMethod(const CodeItemData* code_item,
                                               InlineMethod* result) {
...
bool InlineMethodAnalyser::AnalyseConstMethod(const CodeItemData* code_item,
                                              InlineMethod* result) {
...
bool InlineMethodAnalyser::AnalyseIGetMethod(const CodeItemData* code_item,
                                             const MethodReference& method_ref,
                                             bool is_static,
                                             ArtMethod* method,
                                             InlineMethod* result) {
...
bool InlineMethodAnalyser::AnalyseIPutMethod(const CodeItemData* code_item,
                                             const MethodReference& method_ref,
                                             bool is_static,
                                             ArtMethod* method,
                                             InlineMethod* result) {
