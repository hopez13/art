...
class DexToDexCompiler {
 public:
  ...
  bool ShouldCompileMethod(const MethodReference& ref) const;
 ...
 private:
  ...
  class CompilationState {
   public:
    ...
    // Returns the dequickend index if an instruction is quickened, otherwise return index.
  ...
  // State for adding methods (should this be in it's own class?).
  const DexFile* active_dex_file_;

  // Lock that guards duplicate code items and the bitmap.
  mutable Mutex lock_;
  // Record what code items are already seen.
  std::set<MethodReference> should_quicken_;
  std::unordered_set<const DexFile::CodeItem*> seen_code_items_ GUARDED_BY(lock_);
  // Guarded by lock_ during writing.
  ...
  std::unordered_map<const DexFile::CodeItem*,
                     std::vector<uint8_t>> shared_code_item_quicken_info_ GUARDED_BY(lock_);
};
