...
void ImageWriter::PruneAndPreloadDexCache(ObjPtr<mirror::DexCache> dex_cache,
  ...
  for (size_t i = 0, end = dex_file.NumFieldIds(); i < end; ++i) {
    ...
    // Check if the referencing class is in the image. Note that we want to check referencing
    // class rather than the declaring class to preserve the semantics, i.e. using a field id
    // results in resolving the referencing class and that can throw OOME.
    ...
    if (field == nullptr || i < stored_index) {
      if (last_class != nullptr) {
        ...
        if (field != nullptr) {
          // If the referencing class is in the image, the defining class must also be there.
