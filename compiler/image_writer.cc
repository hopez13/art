...
void ImageWriter::PrepareDexCacheArraySlots() {
  ...
  for (const ClassLinker::DexCacheData& data : class_linker->GetDexCachesData()) {
    ...
    if (compile_app_image_) {
      if (image_info.dex_cache_bitmap_ == nullptr) {
        size_t num_bits =
            RoundUp(image_info.bin_slot_sizes_[kBinDexCacheArray], kPageSize) /
            kPageSize;
        size_t num_bytes = num_bits / kBitsPerByte;
      ...
      for (size_t i = 0; i < dex_cache->NumResolvedTypes(); ++i) {
        if (!resolved_types[i].IsNull()) {
          ...
          SetBit(bitmap[page / kBitsPerByte], page % kBitsPerByte);
        }
      }
      ...
      if (dex_cache->NumStrings() > 0) {
        ...
        uint32_t last_str_page =
            (start + layout.StringOffset(dex_cache->NumStrings() - 1)) /
            kPageSize;
...
void ImageWriter::CopyAndFixupNativeData(size_t oat_index) {
  ...
  uint8_t* dex_cache_bitmap_addr =
      image_info.image_->Begin() + image_info.bin_slot_offsets_[kBinDexCacheBitmap];
  memcpy(dex_cache_bitmap_addr,
         image_info.dex_cache_bitmap_.get(),
         image_info.bin_slot_sizes_[kBinDexCacheBitmap]);
