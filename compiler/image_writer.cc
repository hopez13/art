...
class ImageWriter::ClassReferenceVisitor {
 public:
  ClassReferenceVisitor(ImageWriter* image_writer_,
                        bool* early_exit_,
  ...
  ALWAYS_INLINE void operator() (ObjPtr<mirror::Object> obj,
    ...
    if (ref == nullptr || visited->find(ref) != visited->end()) {
      return;
    }
    visited->emplace(ref);
    if (ref->IsClass()) {
      *result = *result ||
      ...
      if (ref == mirror::Method::StaticClass() || ref == mirror::Constructor::StaticClass()) {
    ...
    for (size_t i = 0u; i < num_static_fields; ++i) {
      ...
      if (ref != nullptr) {
        if (ref->IsClass()) {
          ...
          if (!result) {
            bool tmp = false;
            ClassReferenceVisitor visitor(this, &my_early_exit, visited, &tmp);
