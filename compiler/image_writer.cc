...
void ImageWriter::WriteFixups(size_t oat_index) {
  ...
  if (image_info.object_fixup_bytes_ > 0u) {
    ...
    for (auto& pair : image_info.objects_fixups_) {
      const ObjectFixups& fixups = pair.second;
  ...
  if (!IsImageBinSlotAssigned(obj)) {
    ...
    if (obj->IsString()) {
      ...
      for (LengthPrefixedArray<ArtField>* cur_fields : fields) {
        ...
        if (cur_fields != nullptr) {
          ...
          for (size_t i = 0, count = cur_fields->size(); i < count; ++i) {
            ...
            if (IsClassThatRequiresFastFixups(declaring_class)) {
              LOG(ERROR) << "COUNT FIELD";
              CountObjectFixup(declaring_class, oat_index);
            }
            native_object_relocations_.emplace(
                field,
      ...
      if (class_loader->GetClassTable() != nullptr) {
        std::vector<ObjPtr<mirror::Class>> to_prune;
        DCHECK(compile_app_image_);
        ...
        image_info.class_table_->Visit([&to_prune, this](ObjPtr<mirror::Class> klass)
            REQUIRES_SHARED(Locks::mutator_lock_) {
          if (IsClassThatRequiresFastFixups(klass)) {
            to_prune.push_back(klass);
          }
          return true;
        });
        for (ObjPtr<mirror::Class> klass : to_prune) {
          std::string temp;
...
void ImageWriter::AssignMethodOffset(ArtMethod* method,
  ...
  mirror::Class* klass = method->GetDeclaringClass();
  if (IsClassThatRequiresFastFixups(klass)) {
    LOG(ERROR) << "COUNT METHOD";
    CountObjectFixup(klass, oat_index);
  }
...
class ImageWriter::VisitReferencesVisitor {
 public:
  ...
  ALWAYS_INLINE void operator() (ObjPtr<mirror::Object> obj,
    ...
    if (image_writer_->IsClassThatRequiresFastFixups(ref)) {
      // We want to be able to quickly fixup references to boot classes.
      LOG(ERROR) << "COUNT REF " << obj->PrettyTypeOf() << " " << ref->PrettyTypeOf();
      image_writer_->CountObjectFixup(ref, oat_index_);
    }
...
void ImageWriter::FixupObject(Object* orig, Object* copy) {
  ...
  LOG(ERROR) << "Fixing up " << orig->PrettyTypeOf();
...
void ImageWriter::CountObjectFixup(ObjPtr<mirror::Object> ref, size_t oat_index) {
  // Increment count and insert if not already added.
  ++GetImageInfo(oat_index).objects_fixups_[ref.Ptr()].fixup_count_;
}
...
void ImageWriter::CopyReference(mirror::HeapReference<mirror::Object>* dest,
  ...
  if (src != nullptr) {
    ...
    auto it = image_info.objects_fixups_.find(src.Ptr());
    if (it != image_info.objects_fixups_.end()) {
      LOG(ERROR) << "ACTUAL COPYHEAL";
...
void ImageWriter::CopyReference(mirror::CompressedReference<mirror::Object>* dest,
  ...
  if (src != nullptr) {
    ...
    if (it != image_info.objects_fixups_.end()) {
      const uint32_t image_offset =
          reinterpret_cast<uint8_t*>(dest) - image_info.image_->Begin();
      LOG(ERROR) << "ACTUAL COPYREF " << image_offset;
