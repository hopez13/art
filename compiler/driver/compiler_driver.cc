...
void CompilerDriver::Verify(jobject jclass_loader,
  ...
  if (verifier_deps != nullptr) {
    ...
    ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
    if (verifier_deps->Verify(class_loader, soa.Self())) {
      // We successfully verified the dependencies, now set the class status
      // of previously verified classes.
      for (const DexFile* dex_file : dex_files) {
  ...
  // If there is no passed `verifier_deps` or, the passed `verifier_deps` did not verify,
  // create a new one for non boot image compilation.
  if (!GetCompilerOptions().IsBootImage()) {
