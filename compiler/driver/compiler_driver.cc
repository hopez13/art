...
class InitializeClassVisitor : public CompilationVisitor {
 public:
  ...
  void Visit(size_t class_def_index) OVERRIDE {
    ...
    if (klass != nullptr && !SkipClass(manager_->GetClassLoader(), dex_file, klass.Get())) {
      VisitorHelper(klass, class_loader);
    } else {
      // In case failing to find class or class def which throw exceptions
      soa.Self()->ClearException();
    }
  }

  // A helper function for initializing klass.
  void VisitorHelper(Handle<mirror::Class> klass, Handle<mirror::ClassLoader>& class_loader)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (klass->IsVerified()) {
      ...
      if (!klass->IsInitialized()) {
        ...
        // fields.
        bool is_parent_init = InitializeDependencies(klass, class_loader, soa.Self());
        if (is_parent_init) {
        ...
        if (!klass->IsInitialized() && is_parent_init) {
    ...
    // back up the status before doing initialization for static encoded fiels,
    // cause the static encoded branch want to remain the status to uninitialized.
 ...
 private:
  ...
  bool PreResolveTypes(Thread* self, const Handle<mirror::Class>& klass)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    auto pointer_size = manager_->GetClassLinker()->GetImagePointerSize();
  ...
  // Initialize the klass's dependencies before initialize itself recursively.
  // Checking for interfaces is also necessary since interfaces can contains
  ...
  bool InitializeDependencies(const Handle<mirror::Class>& klass,
    ...
    if (klass->HasSuperClass()) {
      ...
      if (!handle_scope_super->IsInitialized()) {
        this->VisitorHelper(handle_scope_super, class_loader);
        if (!handle_scope_super->IsInitialized()) {
          return false;
        }
      }
    }
    // Primitive class cannot have interfaces
    ...
    for (size_t i = 0; i < num_if; i++) {
      ...
      VisitorHelper(handle_interface, class_loader);
    ...
    PreResolveTypes(self, klass);

    return true;
  }

  // In this phase the classes containing class initializers are ignored. Make sure no
  // clinit appears in kalss's super class class chain and interfaces.
