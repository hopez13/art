...
class InitializeClassVisitor : public CompilationVisitor {
  ...
  void TryInitializeClass(Handle<mirror::Class> klass, Handle<mirror::ClassLoader>& class_loader)
    ...
    if (klass->IsVerified()) {
      ...
      if (!klass->IsInitialized()) {
        ...
        if (!klass->IsInitialized() && is_parent_init && manager_->GetCompiler()->IsImageClass(descriptor)) {
 ...
 private:
  ...
  REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (rtn_type == nullptr) {
        // LOG(ERROR) << "XC Param Type not found.";
        self->ClearException();
        return false;
      } else {
        // LOG(ERROR) << "XC Return type resolved: " << rtn_type->PrettyTypeOf();
      }
  ...
  REQUIRES_SHARED(Locks::mutator_lock_) {
      // LOG(ERROR) << "XC PreResolveTypes: " << klass->PrettyClass();
      ...
      if (klass->IsInterface()) {
        ...
        for (int i = super_klass->GetVTableLength() - 1; i >= 0; --i) {
          auto* m = klass->GetVTableEntry(i, pointer_size);
          auto* super_m = super_klass->GetVTableEntry(i, pointer_size);
          ...
          if (klass->GetClassLoader() != super_klass->GetClassLoader()) {
            ...
            for (uint32_t j = 0; j < num_methods; ++j) {
              auto* m = klass->GetIfTable()->GetMethodArray(i)->GetElementPtrSize<ArtMethod*>(
                  j, pointer_size);
              auto* super_m = super_klass->GetVirtualMethod(j, pointer_size);
              if (!ResolveTypesOfMethods(self, m) || !ResolveTypesOfMethods(self, super_m)) {
                return false;
              }
            }
