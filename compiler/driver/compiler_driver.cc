...
// Checks if any of the verified classes from the vdex are actually redefinitions
// of boot classpath classes.
bool CompilerDriver::VdexVerifiedIsBootRedefintion(
    const std::vector<const DexFile*>& dex_files,
    TimingLogger* timings) {
  ...
  TimingLogger::ScopedTiming t("Fast Verify: Collisions Check", timings);

  verifier::VerifierDeps* verifier_deps =
      Runtime::Current()->GetCompilerCallbacks()->GetVerifierDeps();
  DCHECK(verifier_deps != nullptr && !verifier_deps->OutputOnly());
  ...
  for (const DexFile* dex_file : dex_files) {
    ...
    for (ClassAccessor accessor : dex_file->GetClasses()) {
      if (unverified_classes.find(accessor.GetClassIdx()) == unverified_classes.end()) {
        // The class is verified in the vdex file.
        const char* descriptor = accessor.GetDescriptor();
...
bool CompilerDriver::FastVerify(jobject jclass_loader,
  ...
  // in the SDK.)
  if (VdexVerifiedIsBootRedefintion(dex_files, timings)) {
