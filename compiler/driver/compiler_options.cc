...
template struct CompilerOptionsMap<SimpleParseArgumentMap, SimpleParseArgumentMapKey>;

using Parser = CmdlineParser<SimpleParseArgumentMap, SimpleParseArgumentMapKey>;

static std::unique_ptr<Parser> CreateSimpleParser() {
  ...
  return std::unique_ptr<Parser>(new Parser(parser_builder->Build()));
}
