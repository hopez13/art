...
class OptimizingUnitTest : public CommonCompilerTest {
 protected:
  OptimizingUnitTest() : pool_and_allocator_() { }
  ...
  HGraph* CreateGraph() { return art::CreateGraph(&pool_and_allocator_); }

  HGraph* CreateCFG(const uint16_t* data, DataType::Type return_type = DataType::Type::kInt32) {
    return art::CreateCFG(&pool_and_allocator_, data, return_type);
  }
