  ...
  switch (type) {
    ...
    case Primitive::kPrimLong: {
      ...
      if (is_volatile && !codegen->GetInstructionSetFeatures().HasAtomicLdrdAndStrd()) {
        ...
        __ Ldrd(trg_lo, vixl32::Register(trg_lo.GetCode() + 1), MemOperand(base, offset));
      }
      if (is_volatile) {
        __ Dmb(vixl32::ISH);
      }
...
static void GenUnsafePut(LocationSummary* locations,
  ...
  if (type == Primitive::kPrimLong) {
    ...
    if (is_volatile && !codegen->GetInstructionSetFeatures().HasAtomicLdrdAndStrd()) {
      ...
      __ Strd(value_lo, vixl32::Register(value_lo.GetCode() + 1), MemOperand(base, offset));
    }
  } else {
...
static void CheckPosition(ArmVIXLAssembler* assembler,
  ...
  if (pos.IsConstant()) {
    ...
    __ Cbz(pos_reg, slow_path->GetEntryLabel());
  } else {
    // Check that pos >= 0.
    vixl32::Register pos_reg = RegisterFrom(pos);
    __ Cmp(pos_reg, 0);
