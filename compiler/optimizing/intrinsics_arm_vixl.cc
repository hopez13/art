...
// Compute base source address for the System.arraycopy intrinsic in `src_base`.
static void GenSystemArrayCopyBaseSourceAddress(ArmVIXLAssembler* assembler,
                                                Primitive::Type type,
                                                const vixl32::Register& src,
                                                const Location& src_pos,
                                                const vixl32::Register& src_base) {
  ...
  if (src_pos.IsConstant()) {
    ...
    __ Add(src_base, src, Operand(RegisterFrom(src_pos), vixl32::LSL, element_size_shift));
    __ Add(src_base, src_base, data_offset);
  }
}

// Compute base destination address for the System.arraycopy intrinsic in `dst_base`.
static void GenSystemArrayCopyBaseDestinationAddress(ArmVIXLAssembler* assembler,
                                                     Primitive::Type type,
                                                     const vixl32::Register& dst,
                                                     const Location& dst_pos,
                                                     const vixl32::Register& dst_base) {
  DCHECK_EQ(type, Primitive::kPrimNot);
  const int32_t element_size = Primitive::ComponentSize(type);
  const uint32_t element_size_shift = Primitive::ComponentSizeShift(type);
  const uint32_t data_offset = mirror::Array::DataOffset(element_size).Uint32Value();
  ...
  if (dst_pos.IsConstant()) {
    int32_t constant = Int32ConstantFrom(dst_pos);
    __ Add(dst_base, dst, element_size * constant + data_offset);
  } else {
    __ Add(dst_base, dst, Operand(RegisterFrom(dst_pos), vixl32::LSL, element_size_shift));
    __ Add(dst_base, dst_base, data_offset);
  }
}

// Compute end source address for the System.arraycopy intrinsic in `src_end`.
static void GenSystemArrayCopyEndSourceAddress(ArmVIXLAssembler* assembler,
                                               Primitive::Type type,
                                               const Location& copy_length,
                                               const vixl32::Register& src_base,
                                               const vixl32::Register& src_end) {
  DCHECK_EQ(type, Primitive::kPrimNot);
  const int32_t element_size = Primitive::ComponentSize(type);
  const uint32_t element_size_shift = Primitive::ComponentSizeShift(type);
  ...
  if (copy_length.IsConstant()) {
    ...
    __ Add(src_end, src_base, Operand(RegisterFrom(copy_length), vixl32::LSL, element_size_shift));
  }
}
...
class ReadBarrierSystemArrayCopySlowPathARMVIXL : public SlowPathCodeARMVIXL {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    GenSystemArrayCopyBaseDestinationAddress(assembler, type, dest, dest_pos, dst_curr_addr);
...
void IntrinsicCodeGeneratorARMVIXL::VisitSystemArrayCopy(HInvoke* invoke) {
  ...
  if (length.IsConstant() && Int32ConstantFrom(length) == 0) {
    ...
    if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
      ...
      GenSystemArrayCopyBaseSourceAddress(GetAssembler(), type, src, src_pos, temp1);
      // Compute the end source address in `temp3`.
      GenSystemArrayCopyEndSourceAddress(GetAssembler(), type, length, temp1, temp3);
      ...
      GenSystemArrayCopyBaseDestinationAddress(GetAssembler(), type, dest, dest_pos, temp2);
