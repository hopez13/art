...
void IntrinsicCodeGeneratorARMVIXL::VisitIntegerValueOf(HInvoke* invoke) {
  ...
  IntrinsicVisitor::ComputeIntegerValueOfInfo(&info);
  LocationSummary* locations = invoke->GetLocations();
  ArmVIXLAssembler* const assembler = GetAssembler();
  ...
  if (invoke->InputAt(0)->IsConstant()) {
    ...
    if (value >= info.low && value <= info.high) {
      ...
      // valueOf is a final field :-( Ideally, we'd merge this memory barrier with the allocation
      // one.
    ...
    __ Mov(temp, info.high - info.low + 1);
    __ Cmp(out, temp);
