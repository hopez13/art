...
class BakerReadBarrierCasSlowPathARMVIXL : public SlowPathCodeARMVIXL {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    // The old value we have read is a from-space reference to the same object as `expected`.
    ...
    UseScratchRegisterScope temps(assembler->GetVIXLAssembler());
    vixl32::Register adjusted_old_value = temps.Acquire();          // Saved `tmp` from main path.

    // Recalculate the `tmp_ptr` clobbered above and store the adjusted old value in `ip`.
...
static void GenCas(HInvoke* invoke, DataType::Type type, CodeGeneratorARMVIXL* codegen) {
  ...
  __ B(ne, failure, /* far_target */ false);
  if (type == DataType::Type::kReference) {
    assembler->MaybePoisonHeapReference(value);
  }
  __ Strex(tmp, value, MemOperand(tmp_ptr));
