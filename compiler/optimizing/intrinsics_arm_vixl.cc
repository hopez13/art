...
void IntrinsicCodeGeneratorARMVIXL::VisitStringEquals(HInvoke* invoke) {
  ...
  if (const_string != nullptr) {
    ...
    vixl32::Register reg = temp.IsValid() ? temp : out;  // Prefer low register (R0).
    __ Ldr(reg, MemOperand(arg, count_offset));
    __ Cmp(reg, Operand(mirror::String::GetFlaggedCount(const_string_length, is_compressed)));
    __ B(ne, &return_false, /* far_target */ false);
  } else {
    // Load `count` fields of this and argument strings.
