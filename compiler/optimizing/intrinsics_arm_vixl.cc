...
void IntrinsicLocationsBuilderARMVIXL::VisitReferenceRefersTo(HInvoke* invoke) {
  IntrinsicVisitor::CreateReferenceGetReferentLocations(invoke);
}
...
void IntrinsicCodeGeneratorARMVIXL::VisitReferenceRefersTo(HInvoke* invoke) {
  ...
  if (kEmitCompilerReadBarrier) {
    ...
    // Load the lockword and check if it a forwarding address.
    ...
    codegen_->GenerateMemoryBarrier(MemBarrierKind::kLoadAny);  // For correct memory visibility.
