...
static void GenerateCompareAndSet(CodeGeneratorARMVIXL* codegen,
  ...
  bool init_failure_for_cmp_early =
      init_failure_for_cmp &&
      !success.Is((type == DataType::Type::kInt64) ? LowRegisterFrom(old_value)
...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke,
  ...
  if (value_type == DataType::Type::kFloat64) {
    vixl32::Register expected_reg = RegisterFrom(locations->GetTemp(0));
    vixl32::Register expected_high = RegisterFrom(locations->GetTemp(1));
    ...
    expected = LocationFrom(expected_reg, expected_high);
    ...
    expected = LocationFrom(expected_reg);
