  ...
  *is_singleton_and_non_escaping = true;
#ifdef AART
  std::cout << "ESCAPE " << reference->GetId() << std::endl;
#endif
  ...
  for (const HUseListNode<HInstruction*>& use : reference->GetUses()) {
    ...
    if (no_escape != nullptr && (*no_escape)(reference, user)) {
      ...
      *is_singleton_and_non_escaping = false;
#ifdef AART
      std::cout << "  check user " << user->GetId() << " says no" << std::endl;
#endif
      return;
      ...
      *is_singleton_and_non_escaping = false;
#ifdef AART
      std::cout << "  user " << user->GetId() << " says no" << std::endl;
#endif
      return;
      ...
      *is_singleton_and_non_escaping = false;
#ifdef AART
      std::cout << "  unresolved user " << user->GetId() << " says no" << std::endl;
#endif
      return;
    } else if (user->IsReturn()) {
      *is_singleton_and_non_escaping = false;
#ifdef AART
      std::cout << "  return user " << user->GetId() << " escapes" << std::endl;
#endif
    }
  }
  ...
  for (const HUseListNode<HEnvironment*>& use : reference->GetEnvUses()) {
    ...
    if (user->GetHolder()->IsDeoptimize()) {
      *is_singleton_and_non_escaping = false;
#ifdef AART
      std::cout << "  deopt user " << user->GetHolder()->GetId() << " escapes" << std::endl;
#endif
      break;
    }
  }
