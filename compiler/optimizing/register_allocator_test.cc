...
TEST_F(RegisterAllocatorTest, FreeUntil) {
  ...
  LiveInterval* interval = LiveInterval::MakeFixedInterval(GetAllocator(), 0, DataType::Type::kInt32);
...
HGraph* RegisterAllocatorTest::BuildDiv(HInstruction** div) {
  ...
  *div =
      new (GetAllocator()) HDiv(DataType::Type::kInt32, first, second, 0);  // don't care about dex_pc.
...
TEST_F(RegisterAllocatorTest, SpillInactive) {
  ...
  locations = new (GetAllocator()) LocationSummary(second->GetDefinedBy(), LocationSummary::kNoCall);
  locations->SetOut(Location::RequiresRegister());
  ...
  locations = new (GetAllocator()) LocationSummary(fourth->GetDefinedBy(), LocationSummary::kNoCall);
  locations->SetOut(Location::RequiresRegister());
