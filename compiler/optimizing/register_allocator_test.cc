...
bool RegisterAllocatorTest::Check(const uint16_t* data, Strategy strategy) {
  ...
  RegisterAllocator* register_allocator =
      RegisterAllocator::Create(GetArena(), &codegen, liveness, strategy);
...
TEST_F(RegisterAllocatorTest, ValidateIntervals) {
  ...
  std::unique_ptr<const X86InstructionSetFeatures> features_x86(
      X86InstructionSetFeatures::FromCppDefines());
  x86::CodeGeneratorX86 codegen(graph, *features_x86.get(), CompilerOptions());
  ArenaVector<LiveInterval*> intervals(GetArena()->Adapter());

  // Test with two intervals of the same range.
  {
    static constexpr size_t ranges[][2] = {{0, 42}};
    intervals.push_back(BuildInterval(ranges, arraysize(ranges), GetArena(), 0));
    intervals.push_back(BuildInterval(ranges, arraysize(ranges), GetArena(), 1));
    ASSERT_TRUE(RegisterAllocator::ValidateIntervals(
        intervals, 0, 0, codegen, GetArena(), true, false));
  ...
  // Test with two non-intersecting intervals.
  {
    ...
    intervals.push_back(BuildInterval(ranges1, arraysize(ranges1), GetArena(), 0));
    static constexpr size_t ranges2[][2] = {{42, 43}};
    intervals.push_back(BuildInterval(ranges2, arraysize(ranges2), GetArena(), 1));
    ASSERT_TRUE(RegisterAllocator::ValidateIntervals(
        intervals, 0, 0, codegen, GetArena(), true, false));
...
TEST_F(RegisterAllocatorTest, FreeUntil) {
  ...
  SsaLivenessAnalysis liveness(graph, &codegen, GetScopedArena());
  liveness.Analyze();
  RegisterAllocatorLinearScan register_allocator(GetArena(), &codegen, liveness);
  ...
  LiveInterval* interval = LiveInterval::MakeFixedInterval(GetArena(), 0, DataType::Type::kInt32);
  interval->AddRange(40, 50);
  register_allocator.inactive_.push_back(interval);

  interval = LiveInterval::MakeFixedInterval(GetArena(), 0, DataType::Type::kInt32);
  ...
  register_allocator.registers_array_ = GetArena()->AllocArray<size_t>(1);
...
HGraph* RegisterAllocatorTest::BuildIfElseWithPhi(HPhi** phi,
  ...
  HInstruction* test = new (GetArena()) HInstanceFieldGet(parameter,
                                                          nullptr,
                                                          DataType::Type::kBool,
                                                          MemberOffset(22),
                                                          false,
  ...
  block->AddInstruction(new (GetArena()) HIf(test));
  ...
  *phi = new (GetArena()) HPhi(GetArena(), 0, 0, DataType::Type::kInt32);
  join->AddPhi(*phi);
  *input1 = new (GetArena()) HInstanceFieldGet(parameter,
                                               nullptr,
                                               ...
                                               graph->GetDexFile(),
                                               0);
  *input2 = new (GetArena()) HInstanceFieldGet(parameter,
                                               nullptr,
                                               DataType::Type::kInt32,
                                               MemberOffset(42),
                                               false,
...
HGraph* RegisterAllocatorTest::BuildFieldReturn(HInstruction** field, HInstruction** ret) {
  ...
  *field = new (GetArena()) HInstanceFieldGet(parameter,
                                              nullptr,
                                              DataType::Type::kInt32,
                                              MemberOffset(42),
                                              false,
  ...
  *ret = new (GetArena()) HReturn(*field);
...
HGraph* RegisterAllocatorTest::BuildTwoSubs(HInstruction** first_sub, HInstruction** second_sub) {
  ...
  *first_sub = new (GetArena()) HSub(DataType::Type::kInt32, parameter, constant1);
  block->AddInstruction(*first_sub);
  *second_sub = new (GetArena()) HSub(DataType::Type::kInt32, *first_sub, constant2);
...
HGraph* RegisterAllocatorTest::BuildDiv(HInstruction** div) {
  ...
  graph->AddBlock(entry);
  graph->SetEntryBlock(entry);
  HInstruction* first = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kInt32);
  HInstruction* second = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kInt32);
  ...
  *div =
      new (GetArena()) HDiv(DataType::Type::kInt32, first, second, 0);  // don't care about dex_pc.
...
TEST_F(RegisterAllocatorTest, SpillInactive) {
  ...
  graph->AddBlock(entry);
  graph->SetEntryBlock(entry);
  HInstruction* one = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kInt32);
  HInstruction* two = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kInt32);
  HInstruction* three = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kInt32);
  HInstruction* four = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kInt32);
  ...
  // intervals.
  HPhi* user = new (GetArena()) HPhi(GetArena(), 0, 1, DataType::Type::kInt32);
  user->AddInput(one);
  user->SetBlock(block);
  LocationSummary* locations = new (GetArena()) LocationSummary(user, LocationSummary::kNoCall);
  locations->SetInAt(0, Location::RequiresRegister());
  static constexpr size_t phi_ranges[][2] = {{20, 30}};
  BuildInterval(phi_ranges, arraysize(phi_ranges), GetArena(), -1, user);

  // Create an interval with lifetime holes.
  static constexpr size_t ranges1[][2] = {{0, 2}, {4, 6}, {8, 10}};
  LiveInterval* first = BuildInterval(ranges1, arraysize(ranges1), GetArena(), -1, one);
  first->uses_.push_front(*new(GetArena()) UsePosition(user, false, 8));
  first->uses_.push_front(*new(GetArena()) UsePosition(user, false, 7));
  first->uses_.push_front(*new(GetArena()) UsePosition(user, false, 6));

  locations = new (GetArena()) LocationSummary(first->GetDefinedBy(), LocationSummary::kNoCall);
  ...
  LiveInterval* second = BuildInterval(ranges2, arraysize(ranges2), GetArena(), -1, two);
  locations = new (GetArena()) LocationSummary(second->GetDefinedBy(), LocationSummary::kNoCall);
  locations->SetOut(Location::RequiresRegister());
  ...
  LiveInterval* third = BuildInterval(ranges3, arraysize(ranges3), GetArena(), -1, three);
  third->uses_.push_front(*new(GetArena()) UsePosition(user, false, 8));
  third->uses_.push_front(*new(GetArena()) UsePosition(user, false, 4));
  third->uses_.push_front(*new(GetArena()) UsePosition(user, false, 3));
  locations = new (GetArena()) LocationSummary(third->GetDefinedBy(), LocationSummary::kNoCall);
  locations->SetOut(Location::RequiresRegister());
  ...
  LiveInterval* fourth = BuildInterval(ranges4, arraysize(ranges4), GetArena(), -1, four);
  locations = new (GetArena()) LocationSummary(fourth->GetDefinedBy(), LocationSummary::kNoCall);
  locations->SetOut(Location::RequiresRegister());
