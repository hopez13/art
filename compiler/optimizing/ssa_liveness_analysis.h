...
class UsePosition : public ValueObject {
 public:
  ...
  UsePosition(const UsePosition& src) = default;
  UsePosition& operator=(const UsePosition& src) = default;
...
class EnvUsePosition : public ValueObject {
 public:
  ...
  EnvUsePosition(const EnvUsePosition& src) = default;
  EnvUsePosition& operator=(const EnvUsePosition& src) = default;
...
using EnvUsePositionList = ArenaForwardList<EnvUsePosition>;
...
class LiveInterval : public ArenaObject<kArenaAllocSsaLiveness> {
 public:
  ...
  void AddTempUse(HInstruction* instruction, size_t temp_index) {
    ...
    uses_.emplace_front(instruction, temp_index, position);
  ...
  void AddUse(HInstruction* instruction,
    ...
    if ((!uses_.empty()) &&
      ...
      uses_.emplace_after(insert_pos, instruction, input_index, position);
    ...
    if (is_environment) {
      ...
      env_uses_.emplace_front(environment, input_index, position);
    } else {
      DCHECK(uses_.empty() || position <= uses_.front().GetPosition());
      uses_.emplace_front(instruction, input_index, position);
    }
  ...
  void AddPhiUse(HInstruction* instruction, size_t input_index, HBasicBlock* block) {
    ...
    uses_.emplace_front(instruction, input_index, block->GetLifetimeEnd());
  }
  ...
  void AddHighInterval(bool is_temp = false) {
    ...
    for (const UsePosition& use : uses_) {
      pos = high_or_low_interval_->uses_.emplace_after(pos, use);
    }

    auto env_pos = high_or_low_interval_->env_uses_.before_begin();
    for (const EnvUsePosition& env_use : env_uses_) {
      env_pos = high_or_low_interval_->env_uses_.emplace_after(env_pos, env_use);
    }
  }
 ...
 private:
  LiveInterval(ArenaAllocator* allocator,
      ...
      : allocator_(allocator),
        ...
        uses_(allocator->Adapter(kArenaAllocSsaLiveness)),
        env_uses_(allocator->Adapter(kArenaAllocSsaLiveness)),
        type_(type),
  ...
  void AddBackEdgeUses(const HBasicBlock& block_at_use) {
    ...
    for (HLoopInformationOutwardIterator it(block_at_use);
      ...
      insert_pos = uses_.emplace_after(insert_pos, back_edge_use_position);
    }
  }
