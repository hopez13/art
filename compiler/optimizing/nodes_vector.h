...
// Reduces the given vector into the first element as add/min/max,
// viz. add-reduce[ x1, .. , xn ] = [ y, ---- ], where y = sum xi
 ...
 public:
  HVecSetScalars(ArenaAllocator* arena,
      ...
      : HVecOperation(arena,
                      ...
                      vector_length,
                      dex_pc) {
    for (size_t i = 0; i < num_of_scalars; i++) {
