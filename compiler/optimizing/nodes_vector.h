...
// and base is a power of two. For example, the value Alignment(0, 16) means memory is
// perfectly aligned at a 16-byte boundary, whereas the value Alignment(4, 16) means
// memory is always exactly 4 bytes above such a boundary.
class Alignment {
 public:
  Alignment(uint16_t offset, uint16_t base) : offset_(offset), base_(base) {
    DCHECK_LT(offset, base);
    DCHECK(IsPowerOfTwo(base));
  }

  // Returns true if memory is "at least" aligned at the given boundary.
  bool IsAlignedAt(uint16_t base) const {
    return offset_ == 0 && (base_ % base) == 0;
  }

  std::string ToString() const {
    return "ALIGN(" + std::to_string(offset_) + "," + std::to_string(base_) + ")";
  }
...
class HVecOperation : public HVariableInputSizeInstruction {
 public:
  HVecOperation(ArenaAllocator* arena,
    ...
    DCHECK_LT(1, vector_length);
  }

  uint16_t GetVectorLength() const { return vector_length_; }
 ...
 public:
  HVecMemoryOperation(ArenaAllocator* arena,
      ...
      : HVecOperation(arena, packed_type, side_effects, number_of_inputs, vector_length, dex_pc),
        alignment_(0, Primitive::ComponentSize(packed_type)) { }
