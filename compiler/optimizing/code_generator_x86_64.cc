...
class LoadMethodTypeSlowPathX86_64: public SlowPathCode {
  public:
    LoadMethodTypeSlowPathX86_64(HLoadMethodType* mt) : SlowPathCode(mt) {}
...
void LocationsBuilderX86_64::VisitLoadMethodType(HLoadMethodType* load) {
  LocationSummary* locations = new (GetGraph()->GetAllocator()) LocationSummary(load, LocationSummary::CallKind::kCallOnSlowPath);
  locations->SetOut(Location::RequiresRegister());
  if (codegen_->EmitNonBakerReadBarrier()) {
    // For non-Baker read barrier we have a temp-clobbering call.
  } else {
