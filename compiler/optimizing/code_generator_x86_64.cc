...
class LoadClassSlowPathX86_64 : public SlowPathCode {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (cls_ == at_ && cls_->GetLoadKind() == HLoadClass::LoadKind::kBssEntry) {
      DCHECK(out.IsValid());
      // Store the resolved Class to the BSS entry.
...
void LocationsBuilderX86_64::VisitLoadClass(HLoadClass* cls) {
  ...
  if (cls->NeedsAccessCheck() || load_kind == HLoadClass::LoadKind::kDexCacheViaMethod) {
    InvokeRuntimeCallingConvention calling_convention;
