  ...
  switch (invoke->GetCodePtrLocation()) {
    ...
    case HInvokeStaticOrDirect::CodePtrLocation::kCallCriticalNative: {
      ...
      size_t out_frame_size =
          RoundUp(calling_convention_visitor.GetOffset(), kNativeStackAlignment);
      if (kIsDebugBuild) {
        ScopedObjectAccess soa(Thread::Current());
        uint32_t shorty_len;
        const char* shorty = invoke->GetResolvedMethod()->GetShorty(&shorty_len);
      ...
      switch (invoke->GetType()) {
        ...
        case DataType::Type::kInt16:
          ...
          break;
        default:
          DCHECK(invoke->GetType() == DataType::Type::kInt32 ||
                 invoke->GetType() == DataType::Type::kInt64 ||
                 invoke->GetType() == DataType::Type::kFloat32 ||
                 invoke->GetType() == DataType::Type::kFloat64 ||
                 invoke->GetType() == DataType::Type::kVoid) << invoke->GetType();
          break;
      }
      if (out_frame_size != 0u) {
...
Location CriticalNativeCallingConventionVisitorX86_64::GetNextLocation(DataType::Type type) {
  ...
  if (location.IsInvalid()) {
    if (DataType::Is64BitType(type)) {
      location = Location::DoubleStackSlot(offset_);
    } else {
      location = Location::StackSlot(offset_);
    }
    offset_ += kFramePointerSize;

    if (for_register_allocation_) {
      // The argument can be in any location, we shall need to move it
      // to the right stack slot after adjusting the SP.
