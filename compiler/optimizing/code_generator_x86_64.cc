...
void InstructionCodeGeneratorX86_64::GenerateMethodEntryExitHook(HInstruction* instruction) {
  ...
  __ cmpb(Address(CpuRegister(TMP), 1), Immediate(0));
  ...
  __ gs()->movq(entry_addr,
                Address::Absolute(Thread::TraceBufferPtrOffset<kX86_64PointerSize>().SizeValue(),
                                  /* no_rip= */ true));
  __ leaq(CpuRegister(entry_addr), Address(CpuRegister(entry_addr), CpuRegister(index), TIMES_8, 0));
  ...
  __ movq(Address(entry_addr, 0), CpuRegister(method));
  // Record the method action
  uint32_t trace_action = instruction->IsMethodExitHook() ? 1 : 0;
  __ movq(Address(entry_addr, -8), Immediate(trace_action));
  // Get the timestamp. rdtsc returns timestamp in RAX + RDX even in 64-bit
  // architectures.
  __ rdtsc();
  __ movl(Address(entry_addr, -16), CpuRegister(RAX));
  __ movl(Address(entry_addr, -20), CpuRegister(RDX));
