...
void LocationsBuilderX86_64::VisitDeoptimizationFlag(HDeoptimizationFlag* flag) {
  LocationSummary* locations = new (GetGraph()->GetArena())
      LocationSummary(flag, LocationSummary::kNoCall);
  locations->SetOut(Location::RequiresRegister());
}

void InstructionCodeGeneratorX86_64::VisitDeoptimizationFlag(HDeoptimizationFlag* flag) {
  int offset = codegen_->GetFrameSize() - codegen_->FrameEntrySpillSize()
      - kShouldDeoptimizeFlagSize;
  __ movl(flag->GetLocations()->Out().AsRegister<CpuRegister>(), Address(CpuRegister(RSP), offset));
}
