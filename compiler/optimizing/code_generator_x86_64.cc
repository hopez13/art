...
// TODO(solanes): Turn into kIsDebugBuild after presubmit is green.
static constexpr bool kEnableRuntimeGCCardChecks = true;  // kIsDebugBuild;
...
void LocationsBuilderX86_64::VisitArraySet(HArraySet* instruction) {
  ...
  if (needs_write_barrier || checking_dirty_bit) {
    // Used by reference poisoning, emitting write barrier, or checking write barrier.
    locations->AddTemp(Location::RequiresRegister());
    if (instruction->GetWriteBarrierKind() != WriteBarrierKind::kDontEmit || checking_dirty_bit) {
      // Only used when emitting a write barrier, or when checking for the card table (i.e.
      // kEnableRuntimeGCCardChecks is true)
      locations->AddTemp(Location::RequiresRegister());
    }
  }
}
  ...
  switch (value_type) {
    ...
    case DataType::Type::kReference: {
      ...
      if (!value.IsRegister()) {
        ...
        if (needs_write_barrier && write_barrier_kind == WriteBarrierKind::kEmitBeingReliedOn) {
        ...
        // We are setting null so there's no need to add a `kEnableRuntimeGCCardChecks` check here.
      ...
      if (needs_write_barrier) {
        ...
        if (kEnableRuntimeGCCardChecks &&
          ...
          CpuRegister card = locations->GetTemp(1).AsRegister<CpuRegister>();
          codegen_->CheckGCCardIsSet(temp, card, array, value.AsRegister<CpuRegister>());
        }
      }
...
void CodeGeneratorX86_64::CheckGCCardIsSet(CpuRegister temp,
                                           CpuRegister card,
                                           CpuRegister object,
                                           CpuRegister value) {
  NearLabel done;
