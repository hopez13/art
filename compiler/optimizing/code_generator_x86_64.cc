...
void InstructionCodeGeneratorX86_64::VisitMethodEntryHook(HMethodEntryHook* instruction) {
  ...
  int offset = instrumentation::Instrumentation::NeedEntryExitHooksOffset().Int32Value();
  __ movq(CpuRegister(TMP), Immediate(address));
  __ cmpw(Address(CpuRegister(TMP), offset), Immediate(0));
  __ j(kEqual, &done);
  codegen_->InvokeRuntime(kQuickTraceEntryHook, instruction, 0);
  __ Bind(&done);
}
...
void LocationsBuilderX86_64::VisitMethodExitHookVoid(HMethodExitHookVoid* method_hook) {
  ...
  locations->SetTempAt(0, Location::RegisterLocation(R8));
}
...
void InstructionCodeGeneratorX86_64::GenerateExitHookCheck(HInstruction* instruction) {
  ...
  // Load DeoptimizeFlag onto R8. Add a DCHECK that this is R8
  CpuRegister should_deopt = instruction->GetLocations()->GetTemp(0).AsRegister<CpuRegister>();
  __ movl(should_deopt,
          Address(CpuRegister(RSP), codegen_->GetStackOffsetOfShouldDeoptimizeFlag()));
  __ cmpl(should_deopt, Immediate(0));
  __ j(kNotEqual, &call);
  ...
  __ Bind(&call);
  codegen_->InvokeRuntime(kQuickTraceExitHook, instruction, 0);
