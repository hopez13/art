...
void LocationsBuilderX86_64::VisitX86BitwiseAddRight(HX86BitwiseAddRight* instruction) {
...
void InstructionCodeGeneratorX86_64::VisitX86BitwiseAddRight(HX86BitwiseAddRight* instruction) {
  LocationSummary* locations = instruction->GetLocations();
...
void LocationsBuilderX86_64::VisitX86AndNeg(HX86AndNeg* instruction) {
  DCHECK(DataType::IsIntOrLongType(instruction->GetType())) << instruction->GetType();
  LocationSummary* locations = new (GetGraph()->GetAllocator()) LocationSummary(instruction);
  locations->SetInAt(0, Location::RequiresRegister());
  locations->SetOut(Location::RequiresRegister(), Location::kNoOutputOverlap);
...
void InstructionCodeGeneratorX86_64::VisitX86AndNeg(HX86AndNeg* instruction) {
  LocationSummary* locations = instruction->GetLocations();
  Location src = locations->InAt(0);
  Location dest = locations->Out();
