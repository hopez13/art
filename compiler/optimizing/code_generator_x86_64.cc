...
void CodeGeneratorX86_64::GenerateFrameEntry() {
  ...
  if (GetGraph()->GetArtMethod() != nullptr &&
    ...
    __ movl(CpuRegister(TMP),
            Address(CpuRegister(kMethodRegisterArgument), ArtMethod::DeclaringClassOffset().Int32Value()));
    __ cmpb(Address(CpuRegister(TMP),  status_byte_offset), Immediate(shifted_visibly_initialized_value));
    ...
    __ movl(CpuRegister(TMP), Address(CpuRegister(TMP), mirror::Class::ClinitThreadIdOffset().Int32Value()));
    __ gs()->cmpl(
        CpuRegister(TMP),
