...
void CodeGeneratorX86_64::GenerateVirtualCall(
  ...
  GenerateInlineCacheCheck(invoke->GetDexPc(), temp);
...
void CodeGeneratorX86_64::GenerateInlineCacheCheck(uint32_t dex_pc, CpuRegister klass) {
  ...
  if (GetCompilerOptions().IsBaseline() && !Runtime::Current()->IsAotCompiler()) {
    ...
    __ cmpl(Address(CpuRegister(TMP), 4), klass);
    __ j(kEqual, &done);
...
void InstructionCodeGeneratorX86_64::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  codegen_->GenerateInlineCacheCheck(invoke->GetDexPc(), temp);
