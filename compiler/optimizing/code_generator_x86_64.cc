...
void LocationsBuilderX86_64::VisitVecSet1(HVecSet1* instruction) {
...
void InstructionCodeGeneratorX86_64::VisitVecSet1(HVecSet1* instruction) {
  LocationSummary* locations = instruction->GetLocations();
  XmmRegister reg = locations->Out().AsFpuRegister<XmmRegister>();
  switch (instruction->GetPackedType()) {
    case Primitive::kPrimByte:
      DCHECK_EQ(16, instruction->GetVectorLength());
      ...
      break;
    case Primitive::kPrimShort:
      DCHECK_EQ(8, instruction->GetVectorLength());
      __ movd(reg, locations->InAt(0).AsRegister<CpuRegister>());
      __ punpcklwd(reg, reg);
      __ pshufd(reg, reg, Immediate(0));
      break;
    case Primitive::kPrimInt:
      DCHECK_EQ(4, instruction->GetVectorLength());
      __ movd(reg, locations->InAt(0).AsRegister<CpuRegister>());
      __ pshufd(reg, reg, Immediate(0));
      break;
    case Primitive::kPrimLong:
...
void InstructionCodeGeneratorX86_64::VisitVecAnd(HVecAnd* instruction) {
  ...
  switch (instruction->GetPackedType()) {
    ...
    case Primitive::kPrimInt:
    case Primitive::kPrimLong:
      DCHECK_LE(2, instruction->GetVectorLength());
      DCHECK_LE(instruction->GetVectorLength(), 16);
      __ pand(dst, src);
      break;
    case Primitive::kPrimFloat:
      DCHECK_EQ(4, instruction->GetVectorLength());
