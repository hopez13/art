...
class BoundsCheckSlowPathMemoryX86_64 : public SlowPathCode {
 public:
  explicit BoundsCheckSlowPathMemoryX86_64(HX86BoundsCheck* instruction)
    : SlowPathCode(instruction) {}
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    // Load the array length into our temporary.
    uint32_t len_offset = instruction_->AsX86BoundsCheck()->IsStringCharAt()
    ...
    if (instruction_->AsX86BoundsCheck()->IsStringCharAt()) {
    ...
    codegen->EmitParallelMoves(
        ...
        Location::RegisterLocation(calling_convention.GetRegisterAt(1)),
        DataType::Type::kInt32);
QuickEntrypointEnum entrypoint = instruction_->AsX86BoundsCheck()->IsStringCharAt()
        ? kQuickThrowStringBounds
        : kQuickThrowArrayBounds;
...
void LocationsBuilderX86_64::VisitX86BoundsCheck(HX86BoundsCheck* instruction) {
...
void InstructionCodeGeneratorX86_64::VisitX86BoundsCheck(HX86BoundsCheck* instruction) {
  LocationSummary* locations = instruction->GetLocations();
  Location index_loc = locations->InAt(0);
  CpuRegister array_base = locations->InAt(1).AsRegister<CpuRegister>();
  SlowPathCode* slow_path =
      new (GetGraph()->GetAllocator()) BoundsCheckSlowPathMemoryX86_64(instruction);
  ...
  // Handle the case where array is a string.
