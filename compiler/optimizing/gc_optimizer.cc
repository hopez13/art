...
class GcOptimizerVisitor : public HGraphVisitor {
 ...
 private:
  ...
  Primitive::Type ReadBarrierStateType() const;
...
void GcOptimizerVisitor::VisitInstanceFieldGet(HInstanceFieldGet* field_get) {
  ...
  if (field_get->GetType() == Primitive::kPrimNot
    ...
    Primitive::Type rb_state_type = ReadBarrierStateType();
    HLoadReadBarrierState* rb_state =
        new (allocator) HLoadReadBarrierState(rb_state_type, object, prev_field_get->GetDexPc());
    ...
    // See also GcOptimizerVisitor::ReadBarrierStateType.
    if (rb_state_type == Primitive::kPrimVoid) {
...
Primitive::Type GcOptimizerVisitor::ReadBarrierStateType() const {
