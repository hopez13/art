...
class CFREVisitor : public HGraphVisitor {
 public:
  ...
  void VisitNullCheck(HNullCheck* null_check) OVERRIDE {
    ...
    // we LOG(FATAL) instead of trying to handle it.
    VisitAlias(null_check, /* unexpected */ "NullCheck");
  }
 ...
 private:
  ...
  void VisitAlias(HInstruction* aliasing_inst, const char* unexpected = nullptr) {
    ...
    if (HasInterestingPublishTargetAsInput(aliasing_inst))  {
      if (unexpected != nullptr) {
        LOG(WARNING) << "Unexpected instruction: " << unexpected << "; should not be legal in graph";
  ...
  bool HasInterestingPublishTargetAsInput(HInstruction* invoke) {
