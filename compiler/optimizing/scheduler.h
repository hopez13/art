 ...
 public:
  SchedulingNode(HInstruction* instr, ArenaAllocator* arena, bool is_scheduling_barrier)
      : latency_(0),
        ...
        data_predecessors_(arena->Adapter(kArenaAllocScheduling)),
        other_predecessors_(arena->Adapter(kArenaAllocScheduling)),
...
class SchedulingGraph {
 public:
  explicit SchedulingGraph(const HScheduler* scheduler, ArenaAllocator* arena)
      : scheduler_(scheduler),
        ...
        nodes_map_(arena_->Adapter(kArenaAllocScheduling)) {}
 ...
 protected:
  ...
  bool HasSideEffectDependency(const HInstruction* node, const HInstruction* other);
...
class SchedulingNodeSelector : public ArenaObject<kArenaAllocScheduling> {
 public:
 ...
 * with a heuristics to favor curtain instruction patterns like materialized condition.
 */
