...
void RegisterAllocatorLinearScan::AllocateSpillSlotFor(LiveInterval* interval) {
  ...
  size_t number_slots_needed = parent->NumberSpillSlotsNeeded();
  size_t slot = 0;
  for (size_t e = spill_slots->size(); slot < e; ++slot) {
    bool found = true;
    for (size_t s = slot, u = std::min(slot + number_slots_needed, e); s < u; s++) {
  ...
  size_t upper = slot + number_slots_needed;
...
void RegisterAllocatorLinearScan::AllocateSpillSlotForCatchPhi(HPhi* phi) {
  ...
  if (phi->IsVRegEquivalentOf(previous_phi)) {
    ...
    catch_phi_spill_slots_ += interval->NumberSpillSlotsNeeded();
  }
}
