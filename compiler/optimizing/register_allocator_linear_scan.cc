...
static bool ShouldProcess(bool processing_core_registers, LiveInterval* interval) {
  ...
  bool is_core_register = (interval->GetType() != DataType::Type::kDouble)
      && (interval->GetType() != DataType::Type::kFloat);
...
void RegisterAllocatorLinearScan::ProcessInstruction(HInstruction* instruction) {
    ...
    if (temp.IsRegister() || temp.IsFpuRegister()) {
      ...
      switch (temp.GetPolicy()) {
        ...
        case Location::kRequiresFpuRegister: {
          LiveInterval* interval =
              LiveInterval::MakeTempInterval(allocator_, DataType::Type::kDouble);
          temp_intervals_.push_back(interval);
          interval->AddTempUse(instruction, i);
          if (codegen_->NeedsTwoRegisters(DataType::Type::kDouble)) {
  ...
  bool core_register = (instruction->GetType() != DataType::Type::kDouble)
      && (instruction->GetType() != DataType::Type::kFloat);
