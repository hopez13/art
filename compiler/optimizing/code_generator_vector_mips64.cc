...
void InstructionCodeGeneratorMIPS64::VisitVecReplicateScalar(HVecReplicateScalar* instruction) {
  ...
  switch (instruction->GetPackedType()) {
    ...
    case Primitive::kPrimFloat:
      DCHECK_EQ(4u, instruction->GetVectorLength());
      __ SplatiW(dst, VectorRegisterFrom(locations->InAt(0)), 0);
      break;
    case Primitive::kPrimDouble:
      DCHECK_EQ(2u, instruction->GetVectorLength());
      __ SplatiD(dst, VectorRegisterFrom(locations->InAt(0)), 0);
      break;
    default:
      LOG(FATAL) << "Unsupported SIMD type";
      UNREACHABLE();
...
void InstructionCodeGeneratorMIPS64::VisitVecAbs(HVecAbs* instruction) {
  ...
  switch (instruction->GetPackedType()) {
    case Primitive::kPrimByte:
      DCHECK_EQ(16u, instruction->GetVectorLength());
      __ FillB(dst, ZERO);
      __ Add_aB(dst, dst, src);
      break;
    case Primitive::kPrimChar:
    case Primitive::kPrimShort:
      DCHECK_EQ(8u, instruction->GetVectorLength());
      __ FillH(dst, ZERO);
      __ Add_aH(dst, dst, src);
      break;
    case Primitive::kPrimInt:
      DCHECK_EQ(4u, instruction->GetVectorLength());
      __ FillW(dst, ZERO);
      __ Add_aW(dst, dst, src);
      break;
    case Primitive::kPrimLong:
      DCHECK_EQ(2u, instruction->GetVectorLength());
      __ FillD(dst, ZERO);
      __ Add_aD(dst, dst, src);
      break;
    case Primitive::kPrimFloat:
      DCHECK_EQ(4u, instruction->GetVectorLength());
      __ LdiW(dst, -1);
...
int32_t InstructionCodeGeneratorMIPS64::VecAddress(LocationSummary* locations,
  ...
  int32_t offset = is_string_char_at
      ? mirror::String::ValueOffset().Int32Value()
      : mirror::Array::DataOffset(size).Int32Value();
  ...
  switch (instruction->GetPackedType()) {
    case Primitive::kPrimChar:
      ...
      if (mirror::kUseStringCompression && instruction->IsStringCharAt()) {
        ...
        VectorRegister vec_temp = VectorRegisterFrom(locations->GetTemp(0));
        __ Lw(TMP, locations->InAt(0).AsRegister<GpuRegister>(), count_offset);
        __ Dext(TMP, TMP, 0, 1);
        __ Bnezc(TMP, &uncompressed_load);
        // Zero extend 8 compressed bytes into 8 chars.
        Location index = locations->InAt(1);
        base = locations->InAt(0).AsRegister<GpuRegister>();
        offset = mirror::String::ValueOffset().Int32Value();
        if (index.IsConstant()) {
          offset += index.GetConstant()->AsIntConstant()->GetValue();
        } else {
          __ Daddu(TMP, base, index.AsRegister<GpuRegister>());
          base = TMP;
        }
        __ LoadFpuFromOffset(kLoadDoubleword,
                             locations->Out().AsFpuRegister<FpuRegister>(),
                             base,
                             offset);
        __ FillB(vec_temp, ZERO);
        __ IlvrB(reg, vec_temp, reg);
        __ Bc(&done);
        // Load 8 direct uncompressed chars.
        __ Bind(&uncompressed_load);
        offset = VecAddress(locations, size, /* is_string_char_at */ true, &base);
    ...
    case Primitive::kPrimShort:
      DCHECK_EQ(8u, instruction->GetVectorLength());
      offset = VecAddress(locations, size, instruction->IsStringCharAt(), &base);
      __ LdH(reg, base, offset);
      break;
    case Primitive::kPrimBoolean:
    case Primitive::kPrimByte:
      DCHECK_EQ(16u, instruction->GetVectorLength());
      offset = VecAddress(locations, size, /* is_string_char_at */ false, &base);
...
void InstructionCodeGeneratorMIPS64::VisitVecStore(HVecStore* instruction) {
  ...
  GpuRegister base;
  int32_t offset = VecAddress(locations, size, /* is_string_char_at */ false, &base);
  switch (instruction->GetPackedType()) {
    case Primitive::kPrimBoolean:
    case Primitive::kPrimByte:
      DCHECK_EQ(16u, instruction->GetVectorLength());
