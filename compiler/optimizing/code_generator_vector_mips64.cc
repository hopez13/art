...
void InstructionCodeGeneratorMIPS64::VisitVecMultiplyAccumulate(HVecMultiplyAccumulate* instr) {
  ...
  VectorRegister acc = VectorRegisterFrom(locations->InAt(HVecMultiplyAccumulate::kInputAccumulatorIndex));
  VectorRegister left = VectorRegisterFrom(locations->InAt(HVecMultiplyAccumulate::kInputMulLeftIndex));
  VectorRegister right = VectorRegisterFrom(locations->InAt(HVecMultiplyAccumulate::kInputMulRightIndex));
  switch (instr->GetPackedType()) {
    case Primitive::kPrimByte:
    ...
    case Primitive::kPrimFloat:
      DCHECK_EQ(4u, instr->GetVectorLength());
      if (instr->GetOpKind() == HInstruction::kAdd) {
        __ FmaddW(acc, left, right);
      } else {
        __ FmsubW(acc, left, right);
      }
      break;
    case Primitive::kPrimDouble:
      DCHECK_EQ(2u, instr->GetVectorLength());
      if (instr->GetOpKind() == HInstruction::kAdd) {
        __ FmaddD(acc, left, right);
      } else {
        __ FmsubD(acc, left, right);
      }
      break;
    default:
      LOG(FATAL) << "Unsupported SIMD type";
