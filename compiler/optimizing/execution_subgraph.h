...
class ExecutionSubgraph : public ArenaObject<kArenaAllocLSA> {
 public:
  ...
  ExecutionSubgraph(HGraph* graph, bool valid, ScopedArenaAllocator* allocator)
      : graph_(graph),
        ...
        allowed_successors_(valid ? graph_->GetBlocks().size() : 0,
 ...
 private:
  ...
  // The map from block_id -> allowed-successors. This is the canonical
  // representation of this subgraph. If a bit in the bitset is not set then the
  // corresponding outgoing edge of that block is not considered traversable.
