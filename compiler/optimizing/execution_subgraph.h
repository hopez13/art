...
// (1) Any block which inevitably leads to a removed block is also removed and
// (2) any block which is between 2 removed blocks is removed.
//
...
class ExecutionSubgraph : public ArenaObject<kArenaAllocLSA> {
 public:
  ...
  // Instantiate a subgraph. analysis_possiblecontrols whether or not to even
  ...
  static bool CanAnalyse(HGraph* graph) {
    ...
    // analyse the graph. We avoid this case to ensure prevent excessive memory
    // and time usage while allowing a simpler algorithm with a fixed-width
    // branching factor.
