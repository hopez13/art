...
vixl::aarch64::Literal<uint32_t>* CodeGeneratorPatchesARM64::DeduplicateUint32Literal(
    uint32_t value) {
...
vixl::aarch64::Literal<uint64_t>* CodeGeneratorPatchesARM64::DeduplicateUint64Literal(
    uint64_t value) {
...
void CodeGeneratorPatchesARM64::EmitJitRootPatches(uint8_t* code, const uint8_t* roots_data) {
  for (const auto& entry : jit_string_patches_) {
    const StringReference& string_reference = entry.first;
    vixl::aarch64::Literal<uint32_t>* table_entry_literal = entry.second;
    uint64_t index_in_table = code_generation_data_->GetJitStringRootIndex(string_reference);
    PatchJitRootUse(code, roots_data, table_entry_literal, index_in_table);
  }
  for (const auto& entry : jit_class_patches_) {
    const TypeReference& type_reference = entry.first;
    vixl::aarch64::Literal<uint32_t>* table_entry_literal = entry.second;
    uint64_t index_in_table = code_generation_data_->GetJitClassRootIndex(type_reference);
    PatchJitRootUse(code, roots_data, table_entry_literal, index_in_table);
  }
}

vixl::aarch64::Literal<uint32_t>* CodeGeneratorPatchesARM64::DeduplicateBootImageAddressLiteral(
    uint64_t address) {
  return DeduplicateUint32Literal(dchecked_integral_cast<uint32_t>(address));
}

vixl::aarch64::Literal<uint32_t>* CodeGeneratorPatchesARM64::DeduplicateJitStringLiteral(
    const DexFile& dex_file, dex::StringIndex string_index, Handle<mirror::String> handle) {
  code_generation_data_->ReserveJitStringRoot(StringReference(&dex_file, string_index), handle);
...
vixl::aarch64::Literal<uint32_t>* CodeGeneratorPatchesARM64::DeduplicateJitClassLiteral(
    const DexFile& dex_file, dex::TypeIndex type_index, Handle<mirror::Class> handle) {
  code_generation_data_->ReserveJitClassRoot(TypeReference(&dex_file, type_index), handle);
