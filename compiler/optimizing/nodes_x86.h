...
class HX86AndNeg final : public HUnaryOperation {
 public:
  HX86AndNeg(DataType::Type result_type, HInstruction* input, uint32_t dex_pc = kNoDexPc)
      : HUnaryOperation(kX86AndNeg, result_type, input, dex_pc) {
  ...
  DECLARE_INSTRUCTION(X86AndNeg);

 protected:
  DEFAULT_COPY_CONSTRUCTOR(X86AndNeg);
};

class HX86BitwiseAddRight final : public HUnaryOperation {
 public:
  HX86BitwiseAddRight(DataType::Type result_type, InstructionKind op,  HInstruction* input, uint32_t dex_pc = kNoDexPc)
      : HUnaryOperation(kX86BitwiseAddRight, result_type, input, dex_pc),
        op_kind_(op) {
    DCHECK_EQ(result_type, DataType::Kind(input->GetType()));
  ...
  DECLARE_INSTRUCTION(X86BitwiseAddRight);

 protected:
  const InstructionKind op_kind_;

  DEFAULT_COPY_CONSTRUCTOR(X86BitwiseAddRight);
};
