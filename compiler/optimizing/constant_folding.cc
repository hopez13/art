...
void HConstantFoldingVisitor::PropagateValue(HBasicBlock* starting_block,
  ...
  if (recording_stats) {
    ...
    // LOG(ERROR) << "Before ReplaceUsesDominatedBy for " << *variable << ". It has " << uses_before << " uses";
    // if (uses_before == 1) {
    //   MaybeRecordStat(stats_, MethodCompilationStat::kCalledPropagatedIfValueOnlyOneUse, uses_before - uses_after);
    // }
  }
  ...
  // MaybeRecordStat(stats_, MethodCompilationStat::kCalledPropagatedIfValue, uses_before - uses_after);
  ...
  if (recording_stats) {
    ...
    // LOG(ERROR) << "After ReplaceUsesDominatedBy for " << *variable << ". It has " << uses_after << " uses";
...
void HConstantFoldingVisitor::VisitIf(HIf* inst) {
  // This optimization can take a lot of compile time since we have a lot of If instructions in graphs.
