...
static_assert(kBakerReadBarrierBitsForRegister == 4, "TEST");  // FIXME: Remove.
...
static void CheckValidReg(uint32_t reg) {
  DCHECK(reg < ip.GetCode() && reg != kBakerCcEntrypointRegister.GetCode()) << reg;
}
  ...
  if (GetCompilerOptions().GenerateAnyDebugInfo()) {
    ...
    switch (kind) {
      ...
      case BakerReadBarrierKind::kGcRoot:
        ...
        oss << "_r" << BakerReadBarrierFirstRegField::Decode(encoded_data);
        DCHECK_EQ(kBakerReadBarrierInvalidEncodedReg, BakerReadBarrierSecondRegField::Decode(encoded_data));
        break;
    }
    *debug_name = oss.str();
  }
