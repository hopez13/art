...
static bool CanGenerateTest(HInstruction* condition) {
  if (!IsBooleanValueOrMaterializedCondition(condition)) {
    ...
    if (cond->GetLeft()->GetType() == Primitive::kPrimLong) {
      ...
      if (locations->InAt(1).IsConstant() && (c < kCondLT || c > kCondGE)) {
        ...
        if (!LowRegisterFrom(locations->InAt(0)).IsLow() ||
            value_low > std::numeric_limits<uint8_t>::max()) {
          return false;
        }
      }
    }
...
static bool CanGenerateConditionalMove(const Location& out, const Location& src) {
  ...
  if (src.IsConstant()) {
    ...
    if (out.IsRegister()) {
      if (!RegisterFrom(out).IsLow() || static_cast<uint32_t>(Int32ConstantFrom(src)) > max_imm) {
        return false;
      }
    } else {
      ...
      if (!HighRegisterFrom(out).IsLow() || Low32Bits(value) > max_imm ||
          High32Bits(value) > max_imm) {
        return false;
      }
    }
  }
...
void InstructionCodeGeneratorARMVIXL::VisitSelect(HSelect* select) {
  ...
  if (!Primitive::IsFloatingPointType(type) && CanGenerateTest(condition)) {
    bool invert = false;
