...
void CodeGeneratorARMVIXL::MaybeIncrementHotness(bool is_frame_entry) {
  ...
  if (GetGraph()->IsCompilingBaseline() && !Runtime::Current()->IsAotCompiler()) {
    ...
    {
      // Scope the use of ip, the 'Tst' instruction below needs it.
      UseScratchRegisterScope temps(GetVIXLAssembler());
      temps.Exclude(ip);
      if (!is_frame_entry) {
      ...
      __ Mov(ip, address);
      __ Ldrh(r4, MemOperand(ip, ProfilingInfo::BaselineHotnessCountOffset().Int32Value()));
      __ Add(r4, r4, 1);
      __ Strh(r4, MemOperand(ip, ProfilingInfo::BaselineHotnessCountOffset().Int32Value()));
    }
    __ Tst(r4, 0xffff);
    if (!is_frame_entry) {
      __ Pop(r4);
