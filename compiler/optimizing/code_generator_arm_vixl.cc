  ...
  switch (load_type) {
    ...
    case DataType::Type::kReference: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        {
          // Ensure that between load and MaybeRecordImplicitNullCheck there are
          // no pools emitted.
          EmissionCheckScope guard(GetVIXLAssembler(), kMaxMacroInstructionSizeInBytes);
          GetAssembler()->LoadFromOffset(kLoadWord, RegisterFrom(out), base,
                                         offset);
          codegen_->MaybeRecordImplicitNullCheck(instruction);
        }
        if (is_volatile) {
    ...
    case DataType::Type::kReference: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        if (index.IsConstant()) {
          ...
          {
            // Ensure that between load and MaybeRecordImplicitNullCheck there
    ...
    case DataType::Type::kReference: {
...

      {
        ...
        if (index.IsConstant()) {
          ...
          vixl32::Register temp = temps.Acquire();
          __ Add(temp, array, data_offset);
          codegen_->StoreToShiftedRegOffset(value_type, LocationFrom(source),
                                            temp, RegisterFrom(index));
        }
