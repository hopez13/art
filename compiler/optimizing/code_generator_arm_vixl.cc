...
void CodeGeneratorARMVIXL::GenerateFrameEntry() {
  ...
  if (!skip_overflow_check) {
    ...
    if (!blocked_core_registers_[R4]) {
      // TODO: Remove this check when R4 is made a callee-save
      // register in ART compiled code. Currently we need to
      // make sure r4 is not blocked, e.g. in special purpose
      // TestCodeGeneratorARMVIXL;
