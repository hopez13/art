  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kBitstringCheck: {
      ...
      GenerateReferenceLoadTwoRegisters(instruction,
                                        ...
                                        maybe_temp_loc,
                                        kWithoutReadBarrier);
      // TODO: Some code sequences generated below can be further improved. See CheckCast codegen.
      if (mask_bits == 16u) {
        // Load only the bitstring part of the status word.
        __ Ldrh(out, MemOperand(out, mirror::Class::StatusOffset().Int32Value()));
        // Check if the bitstring bits are equal to `path_to_root`.
        __ Sub(out, out, Operand(path_to_root));
      } else {
        // /* uint32_t */ temp = temp->status_
        __ Ldr(out, MemOperand(out, mirror::Class::StatusOffset().Int32Value()));
        if (GetAssembler()->ShifterOperandCanHold(SUB, path_to_root)) {
          // Compare the bitstring bits using SUB.
          __ Sub(out, out, path_to_root);
          // Shift out bits that do not contribute to the comparison.
          __ Lsl(out, out, Operand(dchecked_integral_cast<uint32_t>(32u - mask_bits)));
        } else if (IsUint<16>(path_to_root)) {
          // Extract the bitstring bits.
          __ Ubfx(out, out, 0, mask_bits);
          // Check if the bitstring bits are equal to `path_to_root`.
          __ Cmp(out, Operand(path_to_root));
        } else {
          // Shift out bits that do not contribute to the comparison.
          __ Lsl(out, out, Operand(dchecked_integral_cast<uint32_t>(32u - mask_bits)));
          // Compare the bitstring bits using SUB.
          __ Sub(out, out, Operand(path_to_root << (32u - mask_bits)));
        }
      }
  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kBitstringCheck: {
      ...
      if (mask_bits == 16u) {
        ...
        // Check if the bitstring bits are equal to `path_to_root`.
        __ Cmp(temp, Operand(path_to_root));
      } else {
        // /* uint32_t */ temp = temp->status_
        ...
        if (GetAssembler()->ShifterOperandCanHold(SUB, path_to_root)) {
          // Compare the bitstring bits using SUB.
          __ Sub(temp, temp, Operand(path_to_root));
          // Shift out bits that do not contribute to the comparison.
          __ Lsls(temp, temp, Operand(dchecked_integral_cast<uint32_t>(32u - mask_bits)));
        } else if (IsUint<16>(path_to_root)) {
          if (temp.IsLow()) {
            ...
            // Compare the bitstring bits using SUB.
            __ Sub(temp, temp, Operand(path_to_root & 0x00ffu));  // 16-bit SUB (immediate) T2
            __ Sub(temp, temp, Operand(path_to_root & 0xff00u));  // 32-bit SUB (immediate) T3
            // Shift out bits that do not contribute to the comparison.
            __ Lsls(temp, temp, Operand(dchecked_integral_cast<uint32_t>(32u - mask_bits)));
          } else {
            // Extract the bitstring bits.
          ...
          // Shift out bits that do not contribute to the comparison.
          __ Lsl(temp, temp, Operand(dchecked_integral_cast<uint32_t>(32u - mask_bits)));
          // Check if the bitstring bits are equal to `path_to_root`.
          __ Cmp(temp, Operand(path_to_root << mask_bits));
        }
      }
      __ B(ne, type_check_slow_path->GetEntryLabel());
      break;
