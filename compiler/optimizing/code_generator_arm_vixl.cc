...
void InstructionCodeGeneratorARMVIXL::GenerateSuspendCheck(HSuspendCheck* instruction,
  ...
  if (slow_path == nullptr) {
    slow_path = new (GetGraph()->GetAllocator()) SuspendCheckSlowPathARMVIXL(instruction, successor);
...
void CodeGeneratorARMVIXL::UpdateReferenceFieldWithBakerReadBarrier(HInstruction* instruction,
  ...
  SlowPathCodeARMVIXL* slow_path =
      new (GetGraph()->GetAllocator()) LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathARMVIXL(
          instruction,
          ref,
          obj,
          /* offset */ 0u,
