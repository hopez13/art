  ...
  switch (type) {
    ...
    case Primitive::kPrimInt: {
      ...
      if (index.IsConstant()) {
        ...
        vixl32::Register temp = temps.Acquire();
        const vixl32::Register released = temp;
  ...
  if (index.IsValid()) {
    ...
    if (index.IsConstant()) {
      ...
      vixl32::Register index_reg = index.IsRegisterPair()
          ...
          : RegisterFrom(index);
      UseScratchRegisterScope temps(GetVIXLAssembler());
      const vixl32::Register temporary = temps.Acquire();
      __ Add(temporary, obj, Operand(index_reg, ShiftType::LSL, scale_factor));
      GetAssembler()->LoadFromOffset(kLoadWord, ref_reg, temporary, offset);
    }
  } else {
    // /* HeapReference<Object> */ ref = *(obj + offset)
