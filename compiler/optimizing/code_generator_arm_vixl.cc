...
class RawReferenceLoadARMVIXL : public ArenaObject<kArenaAllocCodeGenerator> {
 public:
  RawReferenceLoadARMVIXL(HInstruction* instruction,
                          Location ref,
                          vixl32::Register obj,
                          uint32_t offset,
                          Location index,
  ...
  void Emit(CodeGenerator* codegen, bool needs_null_check) {
    CodeGeneratorARMVIXL* arm_codegen = down_cast<CodeGeneratorARMVIXL*>(codegen);
    MacroAssembler* masm = arm_codegen->GetVIXLAssembler();
    Primitive::Type type = Primitive::kPrimNot;
    vixl32::Register ref_reg = RegisterFrom(ref_, type);
    ...
    if (index_.IsValid()) {
      ...
      if (index_.IsConstant()) {
        size_t computed_offset =
            (Int32ConstantFrom(index_) << scale_factor_) + offset_;
        arm_codegen->GetAssembler()->LoadFromOffset(kLoadWord, ref_reg, obj_, computed_offset);
      } else {
        // Handle the special case of the
        // UnsafeGetObject/UnsafeGetObjectVolatile and UnsafeCASObject
        // intrinsics, which use a register pair as index ("long
        // offset"), of which only the low part contains data.
        vixl32::Register index_reg = index_.IsRegisterPair()
            ? LowRegisterFrom(index_)
            : RegisterFrom(index_);
        UseScratchRegisterScope temps(masm);
        vixl32::Register temp = temps.Acquire();
        __ Add(temp, obj_, Operand(index_reg, ShiftType::LSL, scale_factor_));
        arm_codegen->GetAssembler()->LoadFromOffset(kLoadWord, ref_reg, temp, offset_);
      }
    } else {
      // /* HeapReference<mirror::Object> */ ref = *(obj + offset)
      arm_codegen->GetAssembler()->LoadFromOffset(kLoadWord, ref_reg, obj_, offset_);
    }

    if (needs_null_check) {
      arm_codegen->MaybeRecordImplicitNullCheck(instruction_);
    }

    // Object* ref = ref_addr->AsMirrorPtr()
    arm_codegen->GetAssembler()->MaybeUnpoisonHeapReference(ref_reg);
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(RawReferenceLoadARMVIXL);
};
...
class LoadReferenceWithBakerReadBarrierSlowPathARMVIXL : public ReadBarrierMarkSlowPathBaseARMVIXL {
 public:
  LoadReferenceWithBakerReadBarrierSlowPathARMVIXL(HInstruction* instruction,
                                                   RawReferenceLoadARMVIXL* raw_ref_load,
                                                   Location ref,
                                                   vixl32::Register obj,
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    __ B(cc, &done);  // Carry flag is the last bit shifted out by LSRS.
    GenerateReadBarrierMarkRuntimeCall(codegen);
    __ Bind(&done);
 ...
 private:
  ...
  RawReferenceLoadARMVIXL* raw_ref_load_;
...
void CodeGeneratorARMVIXL::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  RawReferenceLoadARMVIXL* raw_ref_load = new (GetGraph()->GetArena()) RawReferenceLoadARMVIXL(
