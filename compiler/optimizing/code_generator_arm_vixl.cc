...
static constexpr size_t kArmInstrMaxSizeInBytes = 4;
...
void InstructionCodeGeneratorARMVIXL::GenerateCompareTestAndBranch(HCondition* condition,
  ...
  vixl32::Label fallthrough_target;
  vixl32::Label* true_target =
      true_target_in == nullptr ? &fallthrough_target : true_target_in;
  vixl32::Label* false_target =
      false_target_in == nullptr ? &fallthrough_target : false_target_in;
  ...
  switch (result_type) {
    case Primitive::kPrimByte:
      switch (input_type) {
        case Primitive::kPrimLong:
          // Type conversion from long to byte is a result of code transformations.
          __ Sbfx(OutputRegister(conversion),
                  in.AsRegisterPairLow<vixl32::Register>(),
                  0,
                  8);
          break;
        case Primitive::kPrimBoolean:
    ...
    case Primitive::kPrimInt:
      switch (input_type) {
        ...
        case Primitive::kPrimFloat: {
          ...
          __ Vcvt(F32, I32, temp, InputSRegisterAt(conversion, 0));
    ...
    case Primitive::kPrimChar:
      switch (input_type) {
        case Primitive::kPrimLong:
          // Type conversion from long to char is a result of code transformations.
          __ Ubfx(OutputRegister(conversion),
                  in.AsRegisterPairLow<vixl32::Register>(),
                  0,
                  16);
          break;
    ...
    case Primitive::kPrimFloat:
      switch (input_type) {
        ...
        case Primitive::kPrimChar: {
          ...
          __ Vcvt(I32, F32, OutputSRegister(conversion), OutputSRegister(conversion));
          break;
        }
...
void ParallelMoveResolverARMVIXL::EmitMove(size_t index) {
  ...
  if (source.IsRegister()) {
    if (destination.IsRegister()) {
      __ Mov(destination.AsRegister<vixl32::Register>(),
...
void ParallelMoveResolverARMVIXL::SpillScratch(ATTRIBUTE_UNUSED int reg) {
  TODO_VIXL32(FATAL);
}

void ParallelMoveResolverARMVIXL::RestoreScratch(ATTRIBUTE_UNUSED int reg) {
  TODO_VIXL32(FATAL);
}
