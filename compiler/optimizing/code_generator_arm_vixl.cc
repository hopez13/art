...
void InstructionCodeGeneratorARMVIXL::DivRemByPowerOfTwo(HBinaryOperation* instruction) {
  ...
  auto can_be_encoded_in_and = [this](uint32_t imm) {
    return (codegen_->GetInstructionSet() == InstructionSet::kThumb2) ?
        vixl::aarch32::ImmediateT32::IsImmediateT32(imm) :
        vixl::aarch32::ImmediateA32::IsImmediateA32(imm);
  };
  ...
  if (HasNonNegativeResultOrMinInt(instruction->GetLeft())) {
    ...
    if (instruction->IsDiv()) {
      ...
      if (can_be_encoded_in_and(abs_imm - 1)) {
  ...
  switch (div->GetResultType()) {
    case DataType::Type::kInt32: {
      if (div->InputAt(1)->IsConstant()) {
        ...
        } else if (IsPowerOfTwo(AbsOrMin(value)) && value != 2 && value != -2 &&
