...
using helpers::PairLocationFrom;
...
// TODO(VIXL): Remove with later pop when codegen complete.
...
void SlowPathCodeARMVIXL::SaveLiveRegisters(CodeGenerator* codegen,
...
void SlowPathCodeARMVIXL::RestoreLiveRegisters(CodeGenerator* codegen, LocationSummary* locations) {
  ...
  // TODO: check the coherency of stack_offset after this.
  CodeGeneratorARMVIXL* arm_codegen = down_cast<CodeGeneratorARMVIXL*>(codegen);
...
class LoadClassSlowPathARMVIXL : public SlowPathCodeARMVIXL {
 public:
  LoadClassSlowPathARMVIXL(HLoadClass* cls,
                           HInstruction* at,
                           uint32_t dex_pc,
                           bool do_clinit)
...
void CodeGeneratorARMVIXL::Move32(Location destination, Location source) {
  ...
  if (destination.IsRegister()) {
    ...
    if (source.IsRegister()) {
      ...
      GetAssembler()->StoreSToOffset(SRegisterFrom(source),
                                     sp,
                                     destination.GetStackIndex());
    } else {
...
void LocationsBuilderARMVIXL::VisitLoadString(HLoadString* load) {
  ...
  // See InstructionCodeGeneratorARMVIXL::VisitLoadString.<Paste>
...
void InstructionCodeGeneratorARMVIXL::VisitLoadString(HLoadString* load) {
  ...
  DCHECK(load->GetLoadKind() == HLoadString::LoadKind::kDexCacheViaMethod);
  InvokeRuntimeCallingConventionARMVIXL calling_convention;
    ...
    case Primitive::kPrimLong:
      switch (input_type) {
        ...
        case Primitive::kPrimFloat: {
          ...
          locations->SetOut(PairLocationFrom(r0, r1));
          break;
        }
        ...
        case Primitive::kPrimDouble: {
          ...
          InvokeRuntimeCallingConventionARMVIXL calling_convention;
          locations->SetInAt(0, PairLocationFrom(calling_convention.GetFpuRegisterAt(0),
    ...
    case Primitive::kPrimFloat:
      switch (input_type) {
        ...
        case Primitive::kPrimLong: {
          ...
          InvokeRuntimeCallingConventionARMVIXL calling_convention;
          locations->SetInAt(0, PairLocationFrom(calling_convention.GetRegisterAt(0),
...
void InstructionCodeGeneratorARMVIXL::HandleFieldGet(HInstruction* instruction,
  ...
  if (field_type == Primitive::kPrimNot || field_type == Primitive::kPrimDouble) {
    ...
    // TODO(VIXL): Didn't we do that just above? Why is it done again?
    codegen_->MaybeRecordImplicitNullCheck(instruction);
  }
...
void InstructionCodeGeneratorARMVIXL::GenerateGcRootFieldLoad(HInstruction* instruction,
                                                              Location root,
                                                              vixl32::Register obj,
                                                              uint32_t offset,
...
void CodeGeneratorARMVIXL::GenerateStaticOrDirectCall(
  ...
  GetAssembler()->LoadFromOffset(
      kLoadWord, lr, RegisterFrom(callee_method),
