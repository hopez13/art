...
Location CriticalNativeCallingConventionVisitorARMVIXL::GetNextLocation(DataType::Type type) {
  ...
  if (DataType::Is64BitType(type)) {
    ...
    offset_ = RoundUp(offset_, 2 * kFramePointerSize);
  ...
  switch (invoke->GetCodePtrLocation()) {
    ...
    case HInvokeStaticOrDirect::CodePtrLocation::kCallCriticalNative: {
      ...
      size_t out_frame_size =
          RoundUp(calling_convention_visitor.GetOffset(), kAapcsStackAlignment);
      if (kIsDebugBuild) {
        ScopedObjectAccess soa(Thread::Current());
        uint32_t shorty_len;
      ...
      switch (invoke->GetType()) {
        ...
        case DataType::Type::kFloat64:
          ...
          break;
        default:
          DCHECK(invoke->GetType() == DataType::Type::kBool ||
                 invoke->GetType() == DataType::Type::kInt8 ||
                 invoke->GetType() == DataType::Type::kUint16 ||
                 invoke->GetType() == DataType::Type::kInt16 ||
                 invoke->GetType() == DataType::Type::kInt32 ||
