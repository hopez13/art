...
void CodeGeneratorARMVIXL::MaybeIncrementHotness(bool is_frame_entry) {
  ...
  if (GetGraph()->IsCompilingBaseline() && !Runtime::Current()->IsAotCompiler()) {
    ...
    temps.Exclude(ip);
    __ Mov(lr, address);
    __ Ldrh(ip, MemOperand(lr, ProfilingInfo::BaselineHotnessCountOffset().Int32Value()));
    __ CompareAndBranchIfZero(ip, slow_path->GetEntryLabel());
    __ Add(ip, ip, -1);
    __ Strh(ip, MemOperand(lr, ProfilingInfo::BaselineHotnessCountOffset().Int32Value()));
    __ Bind(slow_path->GetExitLabel());
  }
}
