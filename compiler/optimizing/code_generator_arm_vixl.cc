...
static vixl32::Condition GenerateLongTestConstant(HCondition* condition,
  ...
  switch (cond) {
    ...
    case kCondLT: {
      ...
      __ Cmp(left_low, value_low);
      __ Sbcs(temps.Acquire(), left_high, value_high);
...
static vixl32::Condition GenerateLongTest(HCondition* condition,
  ...
  const vixl32::Register left_high = HighRegisterFrom(left);
  const vixl32::Register left_low = LowRegisterFrom(left);
  const vixl32::Register right_high = HighRegisterFrom(right);
  const vixl32::Register right_low = LowRegisterFrom(right);
  vixl32::Register first_high = left_high;
  vixl32::Register first_low = left_low;
  vixl32::Register second_high = right_high;
  vixl32::Register second_low = right_low;
  ...
  switch (cond) {
    ...
    case kCondAE: {
      ...
      __ it(eq);
      __ cmp(eq, first_low, second_low);
      ret = ARMUnsignedCondition(cond);
      break;
    }
    case kCondLE:
    ...
    case kCondLT: {
      ...
      __ Cmp(first_low, second_low);
      __ Sbcs(temps.Acquire(), first_high, second_high);
      ret = ARMCondition(cond);
      break;
    }
    default:
...
static bool CanGenerateConditionalMove(const Location& out, const Location& src) {
  ...
  if (out.IsRegister()) {
    ...
    return false;
  } else if (!HighRegisterFrom(out).IsLow()) {
    return false;
  }
  ...
  if (src.IsConstant()) {
    ...
    if (out.IsRegister()) {
      if (static_cast<uint32_t>(Int32ConstantFrom(src)) > max_imm) {
        return false;
      }
    } else {
...
static bool CanGenerateSelectITBlock(Primitive::Type type, HInstruction* condition) {
  ...
  if (!IsBooleanValueOrMaterializedCondition(condition)) {
    ...
    if (cond->GetLeft()->GetType() == Primitive::kPrimLong) {
      ...
      if ((c < kCondLT || c > kCondGE) && !LowRegisterFrom(locations->InAt(0)).IsLow()) {
        return false;
      }

      if (locations->InAt(1).IsConstant()) {
        const uint32_t value_low = Low32Bits(Int64ConstantFrom(locations->InAt(1)));

        // We also check that any constants fit in an 8-bit unsigned integer,
        // so that a 16-bit CMP encoding can be used.
        if ((c < kCondLT || c > kCondGE) && value_low > std::numeric_limits<uint8_t>::max()) {
          return false;
        }
      }
    }
...
void InstructionCodeGeneratorARMVIXL::VisitSelect(HSelect* select) {
  ...
  if (CanGenerateSelectITBlock(type, condition)) {
    bool invert = false;
  ...
  if (out.Equals(second)) {
    ...
    if (!out.Equals(first)) {
      codegen_->MoveLocation(out, first, select->GetType());
    }
  }

  GenerateTestAndBranch(select, 2, true_target, false_target, /* far_target */ false);
  codegen_->MoveLocation(out, src, select->GetType());
  __ Bind(&select_end);
}
