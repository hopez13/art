...
class ReadBarrierUpdateFieldsSlowPathARMVIXL : public SlowPathCodeARMVIXL {
 public:
  ReadBarrierUpdateFieldsSlowPathARMVIXL(HInstruction* instruction,
                                         Location obj,
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    LocationSummary* locations = instruction_->GetLocations();
    vixl32::Register reg_obj = RegisterFrom(obj_);
    DCHECK(locations->CanCall());
    DCHECK(!locations->GetLiveRegisters()->ContainsCoreRegister(reg_obj.GetCode()));
    ...
    InvokeRuntimeCallingConventionARMVIXL calling_convention;
    CodeGeneratorARMVIXL* arm_codegen = down_cast<CodeGeneratorARMVIXL*>(codegen);
    arm_codegen->Move32(LocationFrom(calling_convention.GetRegisterAt(0)), obj_);
    arm_codegen->MoveConstant(LocationFrom(calling_convention.GetRegisterAt(1)), field1_offset_);
    arm_codegen->MoveConstant(LocationFrom(calling_convention.GetRegisterAt(2)), field2_offset_);
    ...
    arm_codegen->Move32(obj_, LocationFrom(r0));

    RestoreLiveRegisters(codegen, locations);
    __ B(GetExitLabel());
  }

  const char* GetDescription() const OVERRIDE { return "ReadBarrierUpdateFieldsSlowPathARMVIXL"; }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ReadBarrierUpdateFieldsSlowPathARMVIXL);
};
    ...
    case Primitive::kPrimNot: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        if (generates_own_read_barrier) {
          ...
          // InstructionCodeGeneratorARMVIXL::VisitUpdateFields.
          GetAssembler()->LoadFromOffset(kLoadWord, RegisterFrom(out), base, offset);
          GetAssembler()->MaybeUnpoisonHeapReference(RegisterFrom(out));
        }
        if (is_volatile) {
...
void LocationsBuilderARMVIXL::VisitUpdateFields(HUpdateFields* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
...
void InstructionCodeGeneratorARMVIXL::VisitUpdateFields(HUpdateFields* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
  ...
  vixl32::Register obj = RegisterFrom(obj_loc);
  vixl32::Register temp = RegisterFrom(locations->GetTemp(0));

  uint32_t monitor_offset = mirror::Object::MonitorOffset().Int32Value();

  // /* int32_t */ monitor = obj->monitor_
  GetAssembler()->LoadFromOffset(kLoadWord, temp, obj, monitor_offset);
  codegen_->MaybeRecordImplicitNullCheck(instruction);
  ...
  // on `temp`.
  __ Add(obj, obj, Operand(temp, ShiftType::LSR, 32));

  SlowPathCodeARMVIXL* slow_path =
      new (GetGraph()->GetArena()) ReadBarrierUpdateFieldsSlowPathARMVIXL(
          instruction,
          obj_loc,
