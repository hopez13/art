...
// Check whether a loop has instructions which makes peeling/unrolling not beneficial.
...
static bool EnableLoopPeeling(InstructionSet isa) {
  switch (isa) {
...
static bool EnableLoopUnrolling(InstructionSet isa) {
  switch (isa) {
    case InstructionSet::kArm64: {
      return true;
    }
...
// Return whether a loop has loop invariant exits.
static bool HasLoopInvariantExits(HLoopInformation* loop_info, HGraph* graph) {
...
bool HLoopOptimization::TryUnrollingForBranchPenaltyReduction(LoopNode* loop_node) {
  ...
  if (!EnableLoopUnrolling(compiler_driver_->GetInstructionSet()) ||
...
bool HLoopOptimization::TryPeelingForLoopInvariantExitsElimination(LoopNode* loop_node) {
  ...
  if (!EnableLoopPeeling(compiler_driver_->GetInstructionSet()) ||
      !PeelUnrollHelper::IsLoopCopyable(loop_info) ||
      HasUnwantedInstructionForPeelingUnrolling(loop_info) ||
      IsLoopTooBigForPeelingUnrolling(loop_info) ||
      !HasLoopInvariantExits(loop_info, graph_)) {
    return false;
  }
...
bool HLoopOptimization::TraverseLoopsInnerToOuter(LoopNode* node) {
  ...
  for ( ; node != nullptr; node = node->next) {
    ...
    if (node->inner == nullptr) {
      ...
      // Don't run peeling/unrolling if compiler_driver_ is nullptr as InstructionSet is needed.
