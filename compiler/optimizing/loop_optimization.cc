...
static bool IsAddConstHelper(HInstruction* instruction,
  ...
  if (IsInt64AndGet(instruction, &value)) {
    *c += value;
    ...
    return IsAddConstHelper(instruction->InputAt(0), a, b, c) &&
           IsAddConstHelper(instruction->InputAt(1), a, b, c);
  } else if (*a == nullptr) {
    *a = instruction;
    return true;
  } else if (*b == nullptr) {
...
static bool IsAddConst(HInstruction* instruction,
  ...
  if (instruction->IsAdd()) {
    ...
    if (IsAddConstHelper(instruction->InputAt(0), a, b, c) &&
        IsAddConstHelper(instruction->InputAt(1), a, b, c) && *b != nullptr) {
      return true;
    }
    // Found a + b.
