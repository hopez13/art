...
void HLoopOptimization::SimplifyBlocks(LoopNode* node) {
  for (HBlocksInLoopIterator it(*node->loop_info); !it.Done(); it.Advance()) {
    ...
    // Remove instructions that are dead or try to apply strength reduction.
...
bool HLoopOptimization::IsPhiInduction(HPhi* phi) {
  ...
  if (set != nullptr) {
    for (HInstruction* i : *set) {
      // Check that, other than phi, all instruction are in the graph,
      // removable, and all uses are contained within the cycle.
      if (i != phi) {
