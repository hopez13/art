...
bool HLoopOptimization::TrySetPhiReduction(HPhi* phi) {
  DCHECK(iset_->empty());
  // Unclassified phi cycles are candidates for reductions.
  if (!induction_range_.IsClassified(phi)) {
    HInputsRef inputs = phi->GetInputs();
    if (inputs.size() == 2) {
      HInstruction* reduction = inputs[1];
      // Accept operations like x = x + .., provided that the phi
      // is used exactly once inside the loop, and by the reduction.
      if (HasReductionFormat(reduction, phi)) {
        HLoopInformation* loop_info = phi->GetBlock()->GetLoopInformation();
        int32_t use_count = 0;
        bool single_use_inside_loop =
            IsOnlyUsedAfterLoop(loop_info, reduction, /*collect_loop_uses*/ true, &use_count) &&
            iset_->size() == 1;
