...
static bool IsZeroExtensionAndGet(HInstruction* instruction,
  ...
  if (instruction->GetType() == type && (instruction->IsArrayGet() ||
                                         instruction->IsStaticFieldGet() ||
                                         instruction->IsInstanceFieldGet())) {
    if (type == DataType::Type::kUint8 || type == DataType::Type::kUint16) {
      *operand = instruction;
      return true;
    }
  }
...
bool HLoopOptimization::VectorizeUse(LoopNode* node,
  ...
  if (node->loop_info->IsDefinedOutOfTheLoop(instruction)) {
    ...
    // (1) matching vector type (exact match or signed/unsigned integral type of the same size) or,
    // (2) loop-invariant base,
    // (3) unit stride index,
    // (4) vectorizable right-hand-side value.
    HInstruction* base = instruction->InputAt(0);
    ...
    if (DataType::ToSignedType(type) == DataType::ToSignedType(instruction->GetType()) &&
        node->loop_info->IsDefinedOutOfTheLoop(base) &&
        induction_range_.IsUnitStride(instruction, index, graph_, &offset)) {
      if (!is_string_char_at) {
        // Except for String.charAt(), keep VecLoad packed types signed to avoid generating
        // two different VecLoad instructions for essentially the same load.
        type = DataType::ToSignedType(type);
      }
      if (generate_code) {
        GenerateVecSub(index, offset);
