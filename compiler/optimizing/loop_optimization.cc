...
static constexpr bool kForceLoopUnrolling = false;
...
void HLoopOptimization::CheckLoopNestLoopInfoRecursively(LoopNode* loop_node) {
  while (loop_node != nullptr) {
    DCHECK_EQ(loop_node->loop_info, loop_node->loop_info->GetHeader()->GetLoopInformation());
    if (loop_node->inner != nullptr) {
      CheckLoopNestLoopInfoRecursively(loop_node->inner);
    }
...
bool HLoopOptimization::TryUnrollingForBranchPenaltyReduction(LoopNode* loop_node) {
  ...
  for (HBlocksInLoopIterator block_it(*loop_info);
    ...
    for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
      ...
      // TODO: Unroll loops with intrinsified invokes and ClinitChecks.
  ...
  DCHECK_EQ(unrolling_factor, kArm64ScalarMaxUnrollFactor);
  ...
  PeelOrUnrollByTwo(loop_node, /* unrolling */ true, &bb_map, &hir_map);
...
bool HLoopOptimization::MaybeDoUnrolling(LoopNode* loop_node) {
  if (!kEnableLoopUnrolling || !PeelUnrollHelper::IsLoopCopyable(loop_node->loop_info)) {
    return false;
  }

  if (kForceLoopUnrolling) {
    PeelOrUnrollByTwo(loop_node, /* unrolling */ true);
    return true;
  }

  if (TryUnrollingForBranchPenaltyReduction(loop_node)) {
    return true;
  }
...
void HLoopOptimization::PeelOrUnrollByTwo(LoopNode* loop_node,
                                          bool do_unrolling,
                                          SuperblockCloner::HBasicBlockMap* bb_map,
                                          SuperblockCloner::HInstructionMap* hir_map) {
  ...
  if (kIsDebugBuild) {
    // Check that loop_info for the loop hierarchy was not changed.
    LoopNode* cur_loop_node = top_loop_;
    while (cur_loop_node != nullptr) {
      DCHECK_EQ(cur_loop_node->loop_info,
                cur_loop_node->loop_info->GetHeader()->GetLoopInformation());
      if (cur_loop_node->inner != nullptr) {
        CheckLoopNestLoopInfoRecursively(cur_loop_node->inner);
      }
...
void HLoopOptimization::PeelOrUnrollByTwo(LoopNode* loop_node,
  ...
  SuperblockCloner::HInstructionMap hir_map(
      std::less<HInstruction*>(), arena->Adapter(kArenaAllocSuperblockCloner));
  PeelOrUnrollByTwo(loop_node, do_unrolling, &bb_map, &hir_map);
}
  ...
  for ( ; node != nullptr; node = node->next) {
    ...
    if (node->inner == nullptr) {
      ...
      if (!inner_loop_optimized && node->inner == nullptr) {
        inner_loop_optimized |= MaybeDoUnrolling(node);
      }
