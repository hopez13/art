...
bool HLoopOptimization::ShouldVectorize(LoopNode* node, HBasicBlock* block, int64_t trip_count) {
  ...
  // (2) initialize static loop peeling votes, never to exceed (1).
  // (3) variable to record how many reference share same alignment.
    ...
    for (auto j = i; ++j != vector_refs_->end(); ) {
      if (i->type == j->type && (i->lhs || j->lhs)) {
        ...
        if (a == b) {
          ...
          // Count the number references that have the same alignment (since
          // base and offset are the same), where at least one is a write.
          num_same_alignment++;
        } else {
