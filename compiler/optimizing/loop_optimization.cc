...
bool HLoopOptimization::TryPeelingForLoopInvariantExitsElimination(LoopAnalysisInfo* analysis_info,
  ...
  if (do_opt) {
    ...
    // Staticly evaluate loop check after peeling for loop invariant condition.
...
bool HLoopOptimization::TryPeelingAndUnrolling(LoopNode* node) {
  ...
  if (!TryPeelingForLoopInvariantExitsElimination(&analysis_info, /*do_opt*/ false) &&
      !TryUnrollingForBranchPenaltyReduction(&analysis_info, /*do_opt*/ false)) {
    return false;
  }
