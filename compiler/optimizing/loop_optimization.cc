...
static constexpr uint32_t SIMD_NO_UNROLL_FACTOR = 1;
static constexpr uint32_t SIMD_MAXIMUM_UNROLL_FACTOR = 8;
static constexpr uint32_t SIMD_HEURISTIC_MAX_BODY_SIZE = 50;
...
// Return highest possible N - power of two - such that val > N.
template <typename T>
...
uint32_t HLoopOptimization::CalculateBeneficialUnrollingFactor(LoopNode* node,
    int64_t trip_count) {
  // TODO: Unroll loops with unknown trip count.
  if (trip_count < vector_length_) {
    return SIMD_NO_UNROLL_FACTOR;
  }

  HBasicBlock* vector_body = node->loop_info->GetHeader()->GetSuccessors()[1];
  uint32_t instruction_count = vector_body->GetInstructions().CountSize();

  // Find a beneficial unroll factor with the following restrictions:
  //  - At least one iteration of the transformed loop should be executed.
  //  - The loop body shouldn't be "too big" (heuristic).
  uint32_t uf1 = SIMD_HEURISTIC_MAX_BODY_SIZE / instruction_count;
  uint32_t uf2 = trip_count / vector_length_;
  uint32_t unroll_factor = TruncToPowerof2(std::min({uf1, uf2, SIMD_MAXIMUM_UNROLL_FACTOR}));
...
void HLoopOptimization::Vectorize(LoopNode* node,
  ...
  if (unroll_factor != SIMD_NO_UNROLL_FACTOR) {
