...
// unrolling, vectoriation). Returns true if anything changed.
    ...
    if (inputs.size() == 2) {
      ...
      if (HasReductionFormat(reduction, phi)) {
        ...
        if (single_use_inside_loop) {
          // Link reduction back, and start recording feed values.
...
// other phis for reductions, but no other side effects.
