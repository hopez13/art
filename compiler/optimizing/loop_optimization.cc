...
void HLoopOptimization::VectorizeTraditional(LoopNode* node,
  ...
  if (ptc != nullptr) {
    vector_mode_ = kSequential;
    HBasicBlock* preheader_for_peeling_loop = nullptr;
  ...
  for (HBlocksInLoopIterator block_it(*node->loop_info);
    ...
    for (HInstructionIterator it(cur_block->GetInstructions()); !it.Done(); it.Advance()) {
      ...
      if (i != vector_map_->end()) {
        ...
        // original scalar loop body, is located not in the newly created vector loop body but
        ...
        bool in_vector_loop_body =
            vector_header_->GetLoopInformation()->Contains(*instr->GetBlock());
        DCHECK_IMPLIES(!in_vector_loop_body,
                        vector_external_set_->find(instr) != vector_external_set_->end());

        if (in_vector_loop_body &&
