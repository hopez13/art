...
static bool IsEmptyBody(HBasicBlock* block, ArenaSet<HInstruction*>* iset) {
  ...
  return phi == nullptr && iset->find(i) != iset->end() && i->GetNext() != nullptr && i->GetNext()->IsGoto();
}
...
void HLoopOptimization::RemoveLoop(LoopNode* node) {
  DCHECK(node != nullptr);
  LoopNode* outer = node->outer;
  DCHECK(node->inner == nullptr);
  LoopNode* previous = node->previous;
  LoopNode* next = node->next;
  if (previous != nullptr) {
    // Within sequence.
    previous->next = next;
    if (next != nullptr) {
