...
bool HLoopOptimization::TrySetVectorType(Primitive::Type type, uint64_t* restrictions) {
  ...
  switch (compiler_driver_->GetInstructionSet()) {
    case kArm:
    case kThumb2:
      // ARM 32-bit platforms are not guaranteed to have a NEON and VFP extensions however there
      // is an implicit assumption in ART compiler that NEON feature is present.
      switch (type) {
