...
bool HLoopOptimization::TryDynamicUnrolling(LoopAnalysisInfo* analysis_info, bool generate_code) {
  ...
  uint32_t versioning_cnt = log2(unrolling_factor);
  uint32_t iter=0;
  ...
  // Modify original and unrolled loops induction variables & conditions
  DataType::Type data_type = stc->GetType();
  DCHECK(DataType::IsIntegralType(data_type));

  HInstruction* const_uf = graph_->GetConstant(data_type, unrolling_factor);
  ...
  HInstruction* induc_advance_orig = new (global_allocator_) HAdd(data_type, induc_orig, const_one);
  blk_back_edge_orig->InsertInstructionBefore(induc_advance_orig, blk_back_edge_orig->GetLastInstruction());
  induc_orig->AddInput(induc_advance_orig);

  HInstruction* inst_last_orig_header = orig_header->GetLastInstruction();
  HInstruction* inst_comp_orig = new (global_allocator_) HGreaterThanOrEqual(induc_orig, inst_tc_orig);
  ...
  HPhi* induc_other = new (global_allocator_) HPhi(global_allocator_, kNoRegNumber, 0, data_type);
  ...
  HInstruction* induc_advance_other = new (global_allocator_) HAdd(data_type, induc_other, const_one);
  blk_back_edge_other->InsertInstructionBefore(induc_advance_other, blk_back_edge_other->GetLastInstruction());
  induc_other->AddInput(induc_advance_other);

  HInstruction* inst_last_another_header = another_header->GetLastInstruction();
  HInstruction* inst_comp_other = new (global_allocator_) HGreaterThanOrEqual(induc_other, inst_tc_other);
