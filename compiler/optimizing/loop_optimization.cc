...
void HLoopOptimization::LocalRun() {
  ...
  if (top_loop_ != nullptr) {
    ...
    ArenaSafeMap<HInstruction*, HInstruction*> map(std::less<HInstruction*>(),
...
bool HLoopOptimization::CanVectorize(LoopNode* node, HBasicBlock* block, int64_t tc) {
  ...
  vector_length_ = 0;
  vector_refs_->clear();
  vector_runtime_testa_ =
  vector_runtime_testb_= nullptr;
      ...
      if (std::get<2>(*i) == std::get<2>(*j) && (std::get<3>(*i) || std::get<3>(*j))) {
        ...
        if (a == b) {
          ...
          if (x != y) {
            ...
            if (vector_runtime_testa_ != nullptr) {
              return false;
            }
            vector_runtime_testa_ = a;
            vector_runtime_testb_ = b;
          }
        }
      }
    }
...
void HLoopOptimization::Vectorize(LoopNode* node, HBasicBlock* block, HBasicBlock* exit, int64_t tc) {
  ...
  iset_->clear();
  CHECK(IsSimpleLoopHeader(header));  // fills iset_

  // Generate preheader:
  // stc = <trip-count>;
  // vtc = stc - stc % VL;
  HInstruction* stc = induction_range_.GenTrip(node->loop_info, graph_, preheader);
  HInstruction* vtc = stc;
  if (needs_cleanup) {
    DCHECK(IsPowerOfTwo(vector_length_));
    HInstruction* rem = Insert(preheader, new (global_allocator_)
                               HAnd(induc_type, stc, graph_->GetIntConstant(vector_length_ - 1)));
  ...
  // vtc = a != b ? vtc : 0;
  if (vector_runtime_testa_ != nullptr) {
    HInstruction* rt = Insert(preheader, new (global_allocator_)
                              HNotEqual(vector_runtime_testa_, vector_runtime_testb_));
    vtc = Insert(preheader, new (global_allocator_) HSelect(rt, vtc, graph_->GetIntConstant(0), kNoDexPc));
    needs_cleanup = true;
  }
  ...
  // Remove original loop, update loop hierarchy.
...
void HLoopOptimization::GenNewLoop(LoopNode* node,
  ...
  vector_induc_ = new (global_allocator_)
      HPhi(global_allocator_, kNoRegNumber, 0, HPhi::ToPhiType(induc_type));
  ...
  for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
    CHECK(VectorizeDef(node, it.Current(), true));  // generates code mapping
  }
  for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
...
bool HLoopOptimization::VectorizeDef(LoopNode* node, HInstruction* instruction, bool gen) {
  ...
  if (instruction->IsArraySet()) {
    ...
    if (AcceptVectorType(type, &restrictions) &&
  ...
  // Otherwise reject anything with any effects outside the immediate loop-body.
  // Note that actual uses are inspected during right-hand-side tree traversal.
  return instruction->IsGoto() || (!IsUsedOutsideLoop(node->loop_info, instruction) &&
...
bool HLoopOptimization::VectorizeUse(LoopNode* node,
  ...
  if (node->loop_info->IsDefinedOutOfTheLoop(instruction)) {
    ...
    if ((to == Primitive::kPrimByte || to == Primitive::kPrimShort) && from == Primitive::kPrimInt) {
      ...
      // (1) supported int,
      // (2) vectorizable operand.
      if (AcceptVectorType(from, &restrictions) &&
    ...
    if ((HasVectorRestrictions(restrictions, kNoMul) && instruction->IsMul()) ||
        (HasVectorRestrictions(restrictions, kNoDiv) && instruction->IsDiv()) ||
        (HasVectorRestrictions(restrictions, kNoHiBits) && (instruction->IsShr() ||
                                                            instruction->IsUShr()))) {
      return false;
    }
    ...
    if ((HasVectorRestrictions(restrictions, kNoShift)) ||
        (HasVectorRestrictions(restrictions, kNoShr) && instruction->IsShr()) ||
        (HasVectorRestrictions(restrictions, kNoHiBits) && (instruction->IsShr() ||
                                                            instruction->IsUShr()))) {
      return false;
    }
    // Accept shift operator for vectorizable/constant operands.
    // TODO: accept symbolic, albeit loop invariant shift factors.
    HInstruction* opa = instruction->InputAt(0);
    HInstruction* opb = instruction->InputAt(1);
    if (VectorizeUse(node, opa, gen, type, restrictions) && opb->IsIntConstant()) {
      if (gen) {
        GenVecOp(instruction, vector_map_->Get(opa), opb, type);
      }
      return true;
    }
    ...
    switch (invoke->GetIntrinsic()) {
      ...
      case Intrinsics::kMathAbsDouble: {
        ...
        if (VectorizeUse(node, opa, gen, type, restrictions) &&
            AcceptVectorType(int_type, &restrictions)) {  // needs integer VecSet1
          if (gen) {
...
bool HLoopOptimization::AcceptVectorType(Primitive::Type type, uint64_t* restrictions) {
        ...
        switch (type) {
          case Primitive::kPrimByte:
            *restrictions |= kNoMul | kNoDiv | kNoShift;
            return AcceptVectorLength(16);
          case Primitive::kPrimShort:
            *restrictions |= kNoDiv;
            return AcceptVectorLength(8);
          case Primitive::kPrimInt:
            *restrictions |= kNoDiv;
            return AcceptVectorLength(4);
...
bool HLoopOptimization::AcceptVectorLength(uint32_t length) {
  DCHECK_LE(2u, length);
  // First time set?
  if (vector_length_ == 0) {
...
void HLoopOptimization::GenVecInv(HInstruction* org, Primitive::Type type) {
  if (!simd_) {
    vector_map_->Put(org, org);  // self pass-through for scalar invariants
    return;
  }
  HInstruction* vector = new (global_allocator_) HVecSet1(global_allocator_,
                                                          org,
                                                          type,
                                                          vector_length_);
...
void HLoopOptimization::GenVecMem(HInstruction* org,
  ...
  if (simd_) {
    ...
    if (opb != nullptr) {
      vector = new (global_allocator_) HVecStore(global_allocator_, org->InputAt(0), opa, opb, type, vector_length_);
    } else  {
      vector = new (global_allocator_) HVecLoad(global_allocator_, org->InputAt(0), opa, type, vector_length_);
    }
  } else {
...
void HLoopOptimization::GenVecOp(HInstruction* org,
  ...
  if (!simd_) {
    ...
    if (type == Primitive::kPrimByte || type == Primitive::kPrimShort) {
  ...
  switch (org->GetKind()) {
    case HInstruction::kNeg:
      DCHECK(opb == nullptr);
      if (simd_) {
        vector = new (global_allocator_) HVecNeg(global_allocator_, opa, type, vector_length_);
      } else {
        vector = new (global_allocator_) HNeg(type, opa);
      }
      break;
    case HInstruction::kTypeConversion:
      DCHECK(opb == nullptr);
      if (simd_) {
        vector = new (global_allocator_) HVecCnv(global_allocator_, opa, type, vector_length_);
      } else {
        vector = new (global_allocator_) HTypeConversion(type, opa, kNoDexPc);
      }
      break;
    case HInstruction::kAdd:
      if (simd_) {
        vector = new (global_allocator_) HVecAdd(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HAdd(type, opa, opb);
      }
      break;
    case HInstruction::kSub:
      if (simd_) {
        vector = new (global_allocator_) HVecSub(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HSub(type, opa, opb);
      }
      break;
    case HInstruction::kMul:
      if (simd_) {
        vector = new (global_allocator_) HVecMul(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HMul(type, opa, opb);
      }
      break;
    case HInstruction::kDiv:
      if (simd_) {
        vector = new (global_allocator_) HVecDiv(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HDiv(type, opa, opb, kNoDexPc);
      }
      break;
    case HInstruction::kAnd:
      if (simd_) {
        vector = new (global_allocator_) HVecAnd(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HAnd(type, opa, opb);
      }
      break;
    case HInstruction::kOr:
      if (simd_) {
        vector = new (global_allocator_) HVecOr(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HOr(type, opa, opb);
      }
      break;
    case HInstruction::kXor:
      if (simd_) {
        vector = new (global_allocator_) HVecXor(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HXor(type, opa, opb);
      }
      break;
    case HInstruction::kShl:
      if (simd_) {
        vector = new (global_allocator_) HVecShl(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HShl(type, opa, opb);
      }
      break;
    case HInstruction::kShr:
      if (simd_) {
        vector = new (global_allocator_) HVecShr(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HShr(type, opa, opb);
      }
      break;
    case HInstruction::kUShr:
      if (simd_) {
        vector = new (global_allocator_) HVecUShr(global_allocator_, opa, opb, type, vector_length_);
      } else {
        vector = new (global_allocator_) HUShr(type, opa, opb);
      }
      break;
