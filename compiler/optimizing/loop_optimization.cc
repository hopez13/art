  ...
  if (node->loop_info->IsDefinedOutOfTheLoop(instruction)) {
    ...
    switch (invoke->GetIntrinsic()) {
      ...
      case Intrinsics::kMathAbsDouble: {
        ...
        if (VectorizeUse(node, opa, generate_code, type, restrictions) &&
            TrySetVectorType(int_type, &restrictions)) {  // needs integer VecReplicateScalar
          if (generate_code) {
            // Masking with maximum signed value clears sign bit.
            if (type == Primitive::kPrimFloat) {
              opb = graph_->GetIntConstant(std::numeric_limits<int32_t>::max());
            } else {
              opb = graph_->GetLongConstant(std::numeric_limits<int64_t>::max());
            }
            GenerateVecInv(opb, int_type);
            GenerateVecOp(instruction, vector_map_->Get(opa), vector_map_->Get(opb), type);
          }
          return true;
        }
