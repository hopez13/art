...
void HLoopOptimization::SetPeelingCandidate(const ArrayReference* candidate, int64_t trip_count) {
  // Current heuristic: use dynamic loop peeling for large known trip counts on X86.
  switch (compiler_driver_->GetInstructionSet()) {
    case kX86:
    case kX86_64:
      if (trip_count >= 2 * vector_length_ && !candidate) {
