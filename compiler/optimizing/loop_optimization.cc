...
void HLoopOptimization::ModifyLoopInductionForDynamicUnrolling(uint32_t unroll_factor,
  ...
  if (induction_range_.GetStride(orig_header_phi, &stride_val) == true) {
    ...
    HInstruction* inst_comp1 = new (global_allocator_) HGreaterThan(inst_init_val, cond_val);
    preheader->InsertInstructionBefore(inst_comp1, preheader->GetLastInstruction());

    HInstruction* inst_val1 = new (global_allocator_) HSub(orig_header_phi->GetType(),
                                                           inst_init_val, inst_mul);
    preheader->InsertInstructionBefore(inst_val1, preheader->GetLastInstruction());

    HInstruction* inst_val2 = new (global_allocator_) HAdd(orig_header_phi->GetType(),
                                                           inst_init_val, inst_mul);
    preheader->InsertInstructionBefore(inst_val2, preheader->GetLastInstruction());

    HInstruction* inst_sel = new (global_allocator_) HSelect(inst_comp1, inst_val1,
                                                             inst_val2, kNoDexPc);
    preheader->InsertInstructionBefore(inst_sel, preheader->GetLastInstruction());

    cond_orig_header->ReplaceInput(inst_sel, index_cond_val);
  } else {
