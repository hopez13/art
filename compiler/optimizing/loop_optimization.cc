...
HLoopOptimization::HLoopOptimization(HGraph* graph,
    ...
    : HOptimization(graph, kLoopOptimizationPassName),
      ...
      vector_peeling_base_(nullptr),
      vector_peeling_offset_(nullptr),
...
void HLoopOptimization::Vectorize(LoopNode* node,
  ...
  uint32_t vl = vector_length_ * unroll;
  ...
  bool needs_cleanup = trip_count == 0 || (trip_count % vl) != 0;
  ...
  // Setup dynamic loop peeling trip count, if needed.
  // ptc = <peeling-needed-for-candidate>
  HInstruction* ptc = nullptr;
  if (vector_peeling_base_ != nullptr) {
...
void HLoopOptimization::GenerateVecMem(HInstruction* org,
  ...
  if (vector_mode_ == kVector) {
    ...
    if (base == vector_peeling_base_ && offset == vector_peeling_offset_) {
