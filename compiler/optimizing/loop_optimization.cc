...
static bool IsSaturatedAdd(DataType::Type type, int64_t lo, int64_t hi, bool is_unsigned) {
  ...
  if (DataType::Size(type) == 1 &&
      ((lo == kNoLo && hi == std::numeric_limits<uint8_t>::max() && is_unsigned) ||
       (lo == std::numeric_limits<int8_t>::min()  &&
        hi == std::numeric_limits<int8_t>::max()  && !is_unsigned))) {
    return true;
  } else if (DataType::Size(type) == 2 &&
             ((lo == kNoLo && hi == std::numeric_limits<uint16_t>::max() && is_unsigned) ||
              (lo == std::numeric_limits<int16_t>::min()  &&
               hi == std::numeric_limits<int16_t>::max()  && !is_unsigned))) {
    return true;
  }
  // TODO: a few more forms....
  return false;
}
...
bool HLoopOptimization::VectorizeSaturationIdiom(LoopNode* node,
  ...
  // Saturation add or subtract on narrower operands?
  ...
  if (!IsNarrowerOperands(clippee->InputAt(0), clippee->InputAt(1), type, &r, &s, &is_unsigned)) {
    return false;
  } else if (clippee->IsAdd() && IsSaturatedAdd(type, lo, hi, is_unsigned)) {
    is_add = true;
  } else if (clippee->IsSub() && IsSaturatedSub(type, lo, hi, is_unsigned)) {
    is_add = false;
  } else {
    return false;
  }
