  ...
  if (VectorizeUse(node, acc, generate_code, sub_type, restrictions) &&
    ...
    if (generate_code) {
      if (vector_mode_ == kVector) {
        ...
        // By the vectorization algorithm "GenerateVecOp" must not be called more than once for
        // each original loop body instruction. As the SAD idiom processes both "current"
        // instruction ("instruction") and its ABS input in one go, we must check that for the
        // scalar case the ABS instruction has not yet been processed.
  ...
  if (VectorizeUse(node, acc, generate_code, op_type, restrictions) &&
    ...
    if (generate_code) {
      if (vector_mode_ == kVector) {
        ...
        // By the vectorization algorithm "GenerateVecOp" must not be called more than once for
        // each original loop body instruction. As the DotProd idiom processes both "current"
        // instruction ("instruction") and its MUL input in one go, we must check that for the
        // scalar case the MUL instruction has not yet been processed.
