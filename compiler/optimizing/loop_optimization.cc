  ...
  if (node->loop_info->IsDefinedOutOfTheLoop(instruction)) {
    ...
    switch (invoke->GetIntrinsic()) {
      ...
      case Intrinsics::kMathAbsDouble: {
        ...
        if (VectorizeUse(node, opa, gen, type, restrictions) &&
            AcceptVectorType(int_type, &restrictions)) {  // needs integer VecReplicateScalar
          if (gen) {
...
void HLoopOptimization::GenVecInv(HInstruction* org, Primitive::Type type) {
  ...
  HInstruction* vector = new (global_allocator_) HVecReplicateScalar(global_allocator_,
                                                                     org,
                                                                     type,
                                                                     vector_length_);
