...
bool HLoopOptimization::VectorizeUse(LoopNode* node,
  ...
  if (node->loop_info->IsDefinedOutOfTheLoop(instruction)) {
    ...
    if ((HasVectorRestrictions(restrictions, kNoShift)) ||
      ...
      return false;
    } else if (HasVectorRestrictions(restrictions, kNoHiBits) && (instruction->IsShr() ||
                                                                  instruction->IsUShr())) {
      return false;
    }
...
void HLoopOptimization::GenVecOp(HInstruction* org,
  ...
  switch (org->GetKind()) {
    ...
    case HInstruction::kNot:
      DCHECK(opb == nullptr);
      if (simd_) {
        vector = new (global_allocator_) HVecNot(global_allocator_, opa, type, vector_length_);
      } else {
        vector = new (global_allocator_) HNot(type, opa);
      }
      break;
    case HInstruction::kBooleanNot:
      ...
      if (simd_) {
        ...
        vector = new (global_allocator_) HBooleanNot(opa);
      }
      break;
