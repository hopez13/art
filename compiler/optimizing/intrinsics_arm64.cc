...
// Return the scratch register holding VarHandle.varType read without read barrier.
static Register GenerateVarHandleAccessModeAndVarTypeChecks(HInvoke* invoke,
                                                            CodeGeneratorARM64* codegen,
                                                            SlowPathCodeARM64* slow_path,
                                                            DataType::Type type) {
  ...
  __ Ldp(temp, temp2, MemOperand(varhandle, var_type_offset.Int32Value()));
  ...
  __ Ldrh(temp2, MemOperand(temp.X(), primitive_type_offset.Int32Value()));
...
static void GenerateSubTypeObjectCheckNoReadBarrier(CodeGeneratorARM64* codegen,
  ...
  __ Ldr(temp, MemOperand(object.X(), class_offset.Int32Value()));
  codegen->GetAssembler()->MaybeUnpoisonHeapReference(temp);
  vixl::aarch64::Label loop;
  __ Bind(&loop);
  __ Cmp(type, temp);  // For heap poisoning, compare poisoned references.
  __ B(&success, eq);
  __ Ldr(temp, MemOperand(temp.X(), super_class_offset.Int32Value()));
  codegen->GetAssembler()->MaybeUnpoisonHeapReference(temp);
...
static void GenerateVarHandleInstanceFieldCheck(HInvoke* invoke,
  ...
  MacroAssembler* masm = codegen->GetVIXLAssembler();
  Register varhandle = InputRegisterAt(invoke, 0).X();
  Register object = InputRegisterAt(invoke, 1).X();
  ...
  __ Ldp(temp, temp2, MemOperand(varhandle, coordinate_type0_offset.Int32Value()));
...
static void GenerateVarHandleFieldReference(HInvoke* invoke,
  ...
  __ Ldr(method.X(), MemOperand(varhandle, art_field_offset.Int32Value()));
...
void IntrinsicLocationsBuilderARM64::VisitVarHandleGet(HInvoke* invoke) {
  ...
  if (kEmitCompilerReadBarrier && !kUseBakerReadBarrier) {
    ...
    locations->AddTemp(Location::RegisterLocation(x20.GetCode()));
...
void IntrinsicCodeGeneratorARM64::VisitVarHandleSet(HInvoke* invoke) {
  ...
  GenerateVarHandleFieldCheck(invoke, codegen_, slow_path);
  Register var_type_scratch_no_rb =
      GenerateVarHandleAccessModeAndVarTypeChecks(invoke, codegen_, slow_path, value_type);
  if (value_type == DataType::Type::kReference && !value.IsZero()) {
    UseScratchRegisterScope temps(masm);
    DCHECK(temps.IsAvailable(var_type_scratch_no_rb));
    temps.Exclude(var_type_scratch_no_rb);
    // If the value type is a reference, check it against the varType.
    // False negatives due varType being an interface or array type or
    // due to the missing read barrier are handled by the slow path.
    GenerateSubTypeObjectCheckNoReadBarrier(codegen_, slow_path, value.W(), var_type_scratch_no_rb);
  }
