    ...
    if (!optimizations.GetDoesNotNeedTypeCheck()) {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        if (!optimizations.GetDestinationIsNonPrimitiveArray()) {
          /* TODO: There is some refactoring opportunity here: the
             following statement is executed in both the `then` and
             the `else` branches of this `if`.  See other places too
             (both here and in other back ends).  */
