...
void IntrinsicCodeGeneratorARM64::VisitStringCompareTo(HInvoke* invoke) {
  ...
  __ Bic(temp3, temp0, Operand(static_cast<int32_t>(0x80000000)));
  __ Bic(temp5, temp1, Operand(static_cast<int32_t>(0x80000000)));
  // Return zero if both strings are empty.
  __ Orr(out, temp3, temp5);
  __ Cbz(out, &end);
  // out = length diff.
  __ Subs(out, temp3, temp5);
  // temp2 = min(len(str), len(arg)).
  __ Csel(temp2, temp5, temp3, gt);
  // Shorter string is empty?
  __ Cbz(temp2, &end);

  // Check if both strings using same compression style to use this comparison loop.
  __ Eor(temp3.W(), temp0, Operand(temp1));
  __ Cmp(temp3.W(), Operand(0));
  __ B(lt, &different_compression);
