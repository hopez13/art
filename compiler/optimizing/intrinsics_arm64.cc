...
class BakerReadBarrierCasSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  // Note: The slow path's exit label goes back to the main path. The `exit_loop` label
  // goes to the result materialization based on the Z flag.
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    __ B(ne, GetExitLabel());  // If taken, Z=false indicates failure.

    // The `old_value` we have read did not match `expected` (which is always a to-space reference)
    // but after the read barrier in GenerateUnsafeCasOldValueMoveWithBakerReadBarrier() the marked
...
static void GenCas(HInvoke* invoke, DataType::Type type, CodeGeneratorARM64* codegen) {
  ...
  Register old_value = temps.AcquireSameSizeAs(value);             // Value in memory.
