...
void IntrinsicCodeGeneratorARM64::VisitCRC32UpdateBytes(HInvoke* invoke) {
  ...
  vixl::aarch64::Label process_aligned_data, align_data_loop;

  static constexpr uint32_t k8ByteAlignedMask = 0x07;
  ...
  __ Tst(ptr, k8ByteAlignedMask);
  __ B(&process_aligned_data, eq);

  // Make ptr 8-byte aligned by processing the data byte by byte.
  __ Bind(&align_data_loop);
  __ Subs(len, len, 1);
  __ B(&done, lo);
  __ Ldrb(array_elem, MemOperand(ptr, 1, PostIndex));
  __ Crc32b(out, out, array_elem);
  __ Tst(ptr, k8ByteAlignedMask);
  __ B(&align_data_loop, ne);

  __ Bind(&process_aligned_data);
  __ Subs(len, len, 8);
  ...
  // len can have the following values:
  //   -8 (first there bits 000): nothing available
  //   -7 (first there bits 001): a byte available
  //   -6 (first there bits 010): two bytes available
  //   -5 (first there bits 011): three bytes available
  //   -4 (first there bits 100): four bytes available
  //   -3 (first there bits 101): five bytes available
  //   -2 (first there bits 110): six bytes available
  //   -1 (first there bits 111): seven bytes available
