...
void IntrinsicCodeGeneratorARM64::VisitStringCompareTo(HInvoke* invoke) {
  ...
  if (mirror::kUseStringCompression) {
    __ Mov(temp3, Operand(temp0));
    __ Mov(temp5, Operand(temp1));
...
void IntrinsicCodeGeneratorARM64::VisitStringGetCharsNoCheck(HInvoke* invoke) {
  ...
  vixl::aarch64::Label string_uncompressed;
  if (mirror::kUseStringCompression) {
    // String's length.
    __ Ldr(tmp3, MemOperand(srcObj, count_offset));
    __ Cmp(tmp3, Operand(0));
    __ B(gt, &string_uncompressed);
    // __ Tbz(tmp3, kWRegSize - 1, &string_uncompressed);
