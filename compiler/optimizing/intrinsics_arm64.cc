...
void IntrinsicCodeGeneratorARM64::VisitSystemArrayCopyChar(HInvoke* invoke) {
  ...
  // can save some conditional branching (e.g. we don't branch back to the
  // initial condition at the end of every iteration only to potentially
  // immediately branch again).
  //
  ...
  constexpr int unroll_threshold = 2 * chars_per_block;
  ...
  auto emitUnrolledTailLoop = [&](const int tail_length) {
    ...
    if ((tail_length & 1) != 0) {
      // As the last copy, don't bother using post-index addressing.
      __ Ldrh(tmp, MemOperand(src_curr_addr));
      __ Strh(tmp, MemOperand(dst_curr_addr));
    }
  };
