...
class ReadBarrierSystemArrayCopySlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ReadBarrierSystemArrayCopySlowPathARM64(HInstruction* instruction, Register temp3, Location tmp)
      : SlowPathCodeARM64(instruction), temp3_(temp3), tmp_(tmp) {
  ...
  void EmitNativeCode(CodeGenerator* codegen_in) OVERRIDE {
    ...
    Register src_stop_addr = temp3_;
    DCHECK(src_stop_addr.IsX());
    ...
    if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
      ...
      SlowPathCodeARM64* read_barrier_slow_path =
          new (GetAllocator()) ReadBarrierSystemArrayCopySlowPathARM64(
              invoke, src_stop_addr, LocationFrom(tmp));
