...
void IntrinsicCodeGeneratorARM64::VisitReferenceGetReferent(HInvoke* invoke) {
  ...
  Register temp0 = WRegisterFrom(codegen_->GenerateCalleeMethodStaticOrDirectCall(
                                 invoke_direct, locations->GetTemp(0)));

  // Now get declaring class.
  __ Ldr(temp0, HeapOperand(temp0, ArtMethod::DeclaringClassOffset().Int32Value()));
  ...
  if (slow_path_flag_offset == disable_flag_offset + 1) {
    ...
    __ Ldr(temp0.X(), HeapOperand(temp0, disable_flag_offset));
  } else {
    UseScratchRegisterScope temps(masm);
