...
void IntrinsicCodeGeneratorARM64::VisitStringCompareTo(HInvoke* invoke) {
  ...
  if (mirror::kUseStringCompression) {
    ...
    __ Tbz(temp5, kWRegSize - 1, &continue_process);
  ...
  if (mirror::kUseStringCompression) {
    vixl::aarch64::Label uncompressed_string_move, continue_process_move;
    __ Tbz(temp5, kWRegSize - 1, &uncompressed_string_move);
    __ And(temp4, temp4, 0xff);
    __ Sub(out, temp4.W(), Operand(temp0.W(), UXTB));
    __ B(&continue_process_move);
    __ Bind(&uncompressed_string_move);
    __ And(temp4, temp4, 0xffff);
    __ Sub(out, temp4.W(), Operand(temp0.W(), UXTH));
    __ Bind(&continue_process_move);
  } else {
    __ And(temp4, temp4, 0xffff);
    __ Sub(out, temp4.W(), Operand(temp0.W(), UXTH));
  }
