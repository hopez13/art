...
void IntrinsicCodeGeneratorARM64::VisitCRC32UpdateBytes(HInvoke* invoke) {
  ...
  __ Sub(len, len, 8);
  // When len < 8 sub will make len negative.
  // Go to process data by 4 bytes, 2 bytes and a byte.
  __ Tbnz(len, 31, &process_4bytes);
  ...
  // if len is not negative, process the next 8 bytes.
  __ Tbz(len, 31, &loop);
