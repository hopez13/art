...
class ReadBarrierSystemArrayCopySlowPathARM64 : public SlowPathCodeARM64 {
 ...
 private:
  // The register location containing the address of the current source
  // value (read cursor) in the source array.
  Register src_curr_addr_;
  // The register location containing the address of the current
  // destination element (write cursor) in the destination array.
  Register dst_curr_addr_;
  // The register location containing the address of the past-the-end
  // (read stop) element in the source array.
...
void IntrinsicCodeGeneratorARM64::VisitSystemArrayCopy(HInvoke* invoke) {
                               ...
                               optimizations.GetCountIsDestinationLength());
  {
    ...
    if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
      ...
      // Current reference value, loaded from the source array and
      // stored into the destination array.
      Register ref = temps.AcquireW();

      // TODO: Also convert this intrinsic to the IsGcMarking strategy?

      // /* int32_t */ monitor = src->monitor_
      __ Ldr(ref, HeapOperand(src.W(), monitor_offset));
      ...
      // on `ref`.
      __ Add(src.X(), src.X(), Operand(ref.X(), LSR, 32));
      ...
      SlowPathCodeARM64* read_barrier_slow_path =
          new (GetAllocator()) ReadBarrierSystemArrayCopySlowPathARM64(
              invoke, src_curr_addr, dst_curr_addr, src_stop_addr, LocationFrom(ref), temp);
      codegen_->AddSlowPath(read_barrier_slow_path);

      // Given the numeric representation, it's enough to check the low bit of the rb_state.
      static_assert(ReadBarrier::WhiteState() == 0, "Expecting white to have value 0");
      static_assert(ReadBarrier::GrayState() == 1, "Expecting gray to have value 1");
      __ Tbnz(ref, LockWord::kReadBarrierStateShift, read_barrier_slow_path->GetEntryLabel());

      // Fast-path copy.
      // Iterate over the arrays and do a raw copy of the objects. We don't need to
      // poison/unpoison.
      __ Bind(&loop);
      __ Ldr(ref, MemOperand(src_curr_addr, element_size, PostIndex));
      __ Str(ref, MemOperand(dst_curr_addr, element_size, PostIndex));
      __ Cmp(src_curr_addr, src_stop_addr);
      __ B(&loop, ne);
