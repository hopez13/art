...
class HSelectGenerator : public HOptimization {
 ...
 private:
  ...
  // -100 : x);`), we generate a double diamond pattern but it is not a clear cut one due to the
  // merging of the blocks. `TryFixupDoubleDiamondPattern` recognizes that pattern and fixes up the
  // graph to have a clean double diamond that `TryGenerateSelectSimpleDiamondPattern` can use to
  // generate selects.
  //
  // In ASCII, it turns:
  //
