    ...
    if (invoke->InputAt(0)->IsConstant()) {
      ...
      if (index < static_cast<uint32_t>(info.length)) {
        info.value_boot_image_reference = IntrinsicObjects::EncodePatch(
            IntrinsicObjects::PatchType::kValueOfObject, index + array_offset);
      } else {
        // Not in the cache.
      ...
      info.array_data_boot_image_reference =
          IntrinsicObjects::EncodePatch(IntrinsicObjects::PatchType::kValueOfArray, array_offset);
    }
  } else {
    ...
    if (invoke->InputAt(0)->IsIntConstant()) {
      ...
      if (index < static_cast<uint32_t>(info.length)) {
        ObjPtr<mirror::Object> object =
            IntrinsicObjects::GetValueOfObject(boot_image_live_objects, array_offset, index);
      ...
      info.array_data_boot_image_reference =
          ...
          IntrinsicObjects::GetValueOfArrayDataOffset(
              boot_image_live_objects, array_offset).Uint32Value();
    }
  }
