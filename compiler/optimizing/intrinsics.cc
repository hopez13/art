...
void IntrinsicVisitor::ComputeIntegerValueOfLocations(HInvoke* invoke,
  ...
  ComputeIntegerValueOfInfo(&info);
  ...
  // Most common case is that the classes are in the boot image and initialized,
  // which is easy to generate code for. Bail if not.
  ScopedObjectAccess soa(Thread::Current());
  gc::Heap* heap = Runtime::Current()->GetHeap();
  if (!heap->ObjectIsInBootImageSpace(info.integer_cache) ||
      !info.integer_cache->IsInitialized() ||
      !heap->ObjectIsInBootImageSpace(info.integer) ||
      !info.integer->IsInitialized() ||
      !heap->ObjectIsInBootImageSpace(info.cache)) {
    return;
  }
...
void IntrinsicVisitor::ComputeIntegerValueOfInfo(/* out */ IntegerValueOfInfo* info) {
