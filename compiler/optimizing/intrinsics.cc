...
bool IntrinsicVisitor::CheckIntegerCacheFields(ObjPtr<mirror::ObjectArray<mirror::Object>> cache) {
  ObjPtr<mirror::Class> cache_class =
      WellKnownClasses::java_lang_Integer_IntegerCache_cache->GetDeclaringClass();
  ...
  ObjPtr<mirror::Class> integer_class =
      WellKnownClasses::java_lang_Integer_valueOf->GetDeclaringClass();
...
void IntrinsicVisitor::ComputeIntegerValueOfLocations(HInvoke* invoke,
  ...
  if (compiler_options.IsBootImage()) {
    ...
    if (kIsDebugBuild) {
      DCHECK_EQ(low, GetIntegerCacheField(cache_class, kLowFieldName));
      DCHECK_EQ(high, GetIntegerCacheField(cache_class, kHighFieldName));
      ObjPtr<mirror::ObjectArray<mirror::Object>> current_cache = GetIntegerCacheArray(cache_class);
