...
HBasicBlock* GraphTest::CreateIfBlock(HGraph* graph) {
  ...
  HInstruction* equal = new (GetArena()) HEqual(instr, instr);
  if_block->AddInstruction(equal);
  instr = new (GetArena()) HIf(equal);
...
HBasicBlock* GraphTest::CreateGotoBlock(HGraph* graph) {
  ...
  HInstruction* got = new (GetArena()) HGoto();
...
HBasicBlock* GraphTest::CreateReturnBlock(HGraph* graph) {
  HBasicBlock* block = new (GetArena()) HBasicBlock(graph);
  graph->AddBlock(block);
  HInstruction* return_instr = new (GetArena()) HReturnVoid();
...
HBasicBlock* GraphTest::CreateExitBlock(HGraph* graph) {
  HBasicBlock* block = new (GetArena()) HBasicBlock(graph);
  graph->AddBlock(block);
  HInstruction* exit_instr = new (GetArena()) HExit();
