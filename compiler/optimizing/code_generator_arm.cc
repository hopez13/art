...
void SlowPathCodeARM::SaveLiveRegisters(CodeGenerator* codegen, LocationSummary* locations) {
  ...
  for (size_t i = 0, e = codegen->GetNumberOfFloatingPointRegisters(); i < e; ++i) {
    if ((fp_spills & 1 << i) == 1u) {
      ...
      filling_the_list = true;
    } else {
      if (filling_the_list) {
        orig_offset = SaveContiguousSRegisterList(pos, i - 1, codegen, orig_offset);
      }
      filling_the_list = false;
    }
  }

  // If fp_spills contains the register with the highest possible num, we need to process the last
  // interval that includes the register.
  if (filling_the_list) {
    orig_offset = SaveContiguousSRegisterList(pos,
...
void SlowPathCodeARM::RestoreLiveRegisters(CodeGenerator* codegen, LocationSummary* locations) {
  ...
  for (size_t i = 0, e = codegen->GetNumberOfFloatingPointRegisters(); i < e; ++i) {
    if ((fp_spills & 1 << i) == 1u) {
      ...
      if (filling_the_list) {
        orig_offset = RestoreContiguousSRegisterList(pos, i - 1, codegen, orig_offset);
      }
      filling_the_list = false;
    }
  }
  ...
  // interval that includes the register.
  if (filling_the_list) {
    orig_offset = RestoreContiguousSRegisterList(pos,
                                                 codegen->GetNumberOfFloatingPointRegisters(),
                                                 codegen,
                                                 orig_offset);
  }
