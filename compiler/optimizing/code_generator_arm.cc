...
class ReadBarrierUpdateFieldsSlowPathARM : public SlowPathCodeARM {
 public:
  ReadBarrierUpdateFieldsSlowPathARM(HInstruction* instruction,
                                     Location obj,
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    arm_codegen->MoveConstant(Location::RegisterLocation(calling_convention.GetRegisterAt(1)),
                              field1_offset_);
    arm_codegen->MoveConstant(Location::RegisterLocation(calling_convention.GetRegisterAt(2)),
                              field2_offset_);
    arm_codegen->InvokeRuntime(kQuickReadBarrierUpdateFields,
                               instruction_,
                               instruction_->GetDexPc(),
                               this);
    CheckEntrypointTypes<
      kQuickReadBarrierUpdateFields, mirror::Object*, mirror::Object*, uint32_t, uint32_t>();
    arm_codegen->Move32(obj_, Location::RegisterLocation(R0));

    RestoreLiveRegisters(codegen, locations);
    __ b(GetExitLabel());
  }

  const char* GetDescription() const OVERRIDE { return "ReadBarrierUpdateFieldsSlowPathARM"; }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ReadBarrierUpdateFieldsSlowPathARM);
};
    ...
    case Primitive::kPrimNot: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        if (generates_own_read_barrier) {
          ...
          // InstructionCodeGeneratorARM::VisitUpdateFields.
          Register out_reg = out.AsRegister<Register>();
...
void LocationsBuilderARM::VisitUpdateFields(HUpdateFields* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
...
void InstructionCodeGeneratorARM::VisitUpdateFields(HUpdateFields* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
  ...
  __ LoadFromOffset(kLoadWord, temp, obj, monitor_offset);
  codegen_->MaybeRecordImplicitNullCheck(instruction);
  ...
  // on `temp_reg`.
  __ add(obj, obj, ShifterOperand(temp, LSR, 32));

  SlowPathCodeARM* slow_path = new (GetGraph()->GetArena()) ReadBarrierUpdateFieldsSlowPathARM(
      instruction,
      obj_loc,
  ...
  __ Lsrs(temp, temp, LockWord::kReadBarrierStateShift + 1);
