...
class ReadBarrierMarkSlowPathARM : public SlowPathCodeARM {
 public:
  ReadBarrierMarkSlowPathARM(HInstruction* instruction, Location ref, Location entrypoint)
...
void InstructionCodeGeneratorARM::VisitLoadString(HLoadString* load) {
  ...
  switch (load_kind) {
    ...
    case HLoadString::LoadKind::kJitTableAddress: {
      ...
      GenerateGcRootFieldLoad(load, out_loc, out, /* offset */ 0, kEmitCompilerReadBarrier);
      return;
    }
    default:
      break;
...
void InstructionCodeGeneratorARM::GenerateGcRootFieldLoad(HInstruction* instruction,
  ...
  if (requires_read_barrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      Location temp = Location::RegisterLocation(IP);
      SlowPathCodeARM* slow_path =
...
void CodeGeneratorARM::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  if (always_update_field) {
    ...
    slow_path = new (GetGraph()->GetArena()) ReadBarrierMarkSlowPathARM(
        instruction,
        ref,
        /*entrypoint*/ Location::NoLocation());
  }
  AddSlowPath(slow_path);
