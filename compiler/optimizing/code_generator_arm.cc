...
static bool LoadClassNeedsTemporary(HLoadClass::LoadKind load_kind) {
...
void LocationsBuilderARM::VisitLoadClass(HLoadClass* cls) {
  ...
  locations->SetOut(Location::RequiresRegister());
  if (LoadClassNeedsTemporary(load_kind)) {
    // We need a temporary register for the read barrier marking slow
...
void InstructionCodeGeneratorARM::VisitLoadClass(HLoadClass* cls) NO_THREAD_SAFETY_ANALYSIS {
  ...
  Location out_loc = locations->Out();
  Register out = out_loc.AsRegister<Register>();
  Location maybe_temp_loc =
      LoadClassNeedsTemporary(load_kind) ? locations->GetTemp(0) : Location::NoLocation();
  ...
  switch (load_kind) {
    ...
    case HLoadClass::LoadKind::kBssEntry: {
      ...
      GenerateGcRootFieldLoad(
          cls, out_loc, out, /* placeholder */ 0u, maybe_temp_loc, kCompilerReadBarrierOption);
    ...
    case HLoadClass::LoadKind::kJitTableAddress: {
      ...
      GenerateGcRootFieldLoad(
          cls, out_loc, out, /* offset */ 0u, maybe_temp_loc, kCompilerReadBarrierOption);
      break;
    }
    case HLoadClass::LoadKind::kDexCacheViaMethod:
    case HLoadClass::LoadKind::kInvalid:
...
static bool LoadStringNeedsTemporary(HLoadString::LoadKind load_kind) {
  return kEmitCompilerReadBarrier && kUseBakerReadBarrier &&
...
void LocationsBuilderARM::VisitLoadString(HLoadString* load) {
  ...
  if (LoadStringNeedsTemporary(load_kind)) {
    // We need a temporary register for the read barrier marking slow
    // path in InstructionCodeGeneratorARM::GenerateGcRootFieldLoad
    locations->AddTemp(Location::RequiresRegister());
  }
...
void InstructionCodeGeneratorARM::VisitLoadString(HLoadString* load) NO_THREAD_SAFETY_ANALYSIS {
  ...
  HLoadString::LoadKind load_kind = load->GetLoadKind();
  Location maybe_temp_loc =
      LoadStringNeedsTemporary(load_kind) ? locations->GetTemp(0) : Location::NoLocation();
...
void InstructionCodeGeneratorARM::GenerateGcRootFieldLoad(HInstruction* instruction,
  ...
  if (read_barrier_option == kWithReadBarrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      //
      //   GcRoot<mirror::Object> root = obj.field;
      ...
      //       if (mark_bit_state == 0) {
      //       root = temp2(root);  // root = ReadBarrier::Mark(root);  // Runtime entry point call.
      //     }
      //   }
...
void CodeGeneratorARM::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  // need to enter the slow path. Then, in the slow path, check the
  // mark bit of the loaded reference to decide whether we need to
  // mark it or not.
  //
