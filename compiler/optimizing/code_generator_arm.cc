...
class LoadStringSlowPathARM : public SlowPathCodeARM {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    // If the `temp` is R0, we preserve the address in `out` across the kSaveEverything call.
    InvokeRuntimeCallingConvention calling_convention;
    bool temp_moved_to_out =
        call_saves_everything_except_r0 && (temp == calling_convention.GetRegisterAt(0));
    if (temp_moved_to_out) {
      DCHECK_NE(out, calling_convention.GetRegisterAt(0));
      __ mov(out, ShifterOperand(temp));
    }
    ...
    if (call_saves_everything_except_r0) {
      // The string entry address was preserved in `temp` or `out` thanks to kSaveEverything.
      __ str(R0, Address(temp_moved_to_out ? out : temp));
    } else {
