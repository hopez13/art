    ...
    if (cond->GetLeft()->GetType() == Primitive::kPrimLong) {
      ...
      if (locations->InAt(1).IsConstant()) {
        ...
        if (c < kCondLT || c > kCondGE) {
          // Since IT blocks longer than a 16-bit instruction are deprecated by ARMv8, we check that
          // the least significant half of the first input to be compared is in a low register (the
          // other half is read outside an IT block), and the constant fits in an 8-bit unsigned
          // integer, so that a 16-bit CMP encoding can be used.
          if (!ArmAssembler::IsLowRegister(locations->InAt(0).AsRegisterPairLow<Register>()) ||
              Low32Bits(value) > std::numeric_limits<uint8_t>::max()) {
            return false;
          }
  ...
  if (src.IsConstant()) {
    ...
    if (out.IsRegister()) {
      if (!ArmAssembler::IsLowRegister(out.AsRegister<Register>()) ||
          static_cast<uint32_t>(CodeGenerator::GetInt32ValueOf(src.GetConstant())) > max_imm) {
        return false;
      }
    } else {
      ...
      if (!ArmAssembler::IsLowRegister(out.AsRegisterPairHigh<Register>()) ||
          Low32Bits(value) > max_imm || High32Bits(value) > max_imm) {
        return false;
      }
    }
  }
...
Label* CodeGeneratorARM::GetFinalLabel(HInstruction* instruction, Label* final_label) {
  DCHECK(!instruction->IsGoto() && !instruction->IsSuspendCheck());
...
void LocationsBuilderARM::VisitSelect(HSelect* select) {
  ...
  if (is_floating_point) {
    ...
    locations->SetInAt(0, Location::RequiresRegister());
    locations->SetInAt(1, ArmEncodableConstantOrRegister(select->GetTrueValue(), MOV));
  }
  ...
  if (IsBooleanValueOrMaterializedCondition(select->GetCondition())) {
    ...
    locations->SetInAt(0, ArmEncodableConstantOrRegister(select->GetFalseValue(), MOV));
