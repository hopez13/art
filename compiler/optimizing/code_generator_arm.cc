...
static void inline CheckLastTempIsBakerCcEntrypointRegister(HInstruction* instruction) {
...
static void inline EmitPlaceholderBne(CodeGeneratorARM* codegen, Label* bne_label) {
    ...
    case Primitive::kPrimNot: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        if (index.IsConstant()) {
          ...
          data_offset += helpers::Int64ConstantFrom(index) << Primitive::ComponentSizeShift(type);
          Location maybe_temp =
