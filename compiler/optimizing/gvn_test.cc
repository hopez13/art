...
TEST_F(GVNTest, LocalFieldElimination) {
  ...
  HInstruction* parameter = new (GetArena()) HParameterValue(graph->GetDexFile(),
                                                             dex::TypeIndex(0),
                                                             0,
  ...
  block->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                           nullptr,
                                                           DataType::Type::kReference,
                                                           MemberOffset(42),
                                                           false,
  ...
  // Kill the value.
  block->AddInstruction(new (GetArena()) HInstanceFieldSet(parameter,
                                                           parameter,
                                                           nullptr,
                                                           DataType::Type::kReference,
                                                           MemberOffset(42),
  ...
  block->AddInstruction(new (GetArena()) HExit());
...
TEST_F(GVNTest, GlobalFieldElimination) {
  ...
  block->AddInstruction(new (GetArena()) HIf(block->GetLastInstruction()));
  HBasicBlock* then = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* else_ = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* join = new (GetArena()) HBasicBlock(graph);
  graph->AddBlock(then);
  ...
  then->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                          nullptr,
                                                          DataType::Type::kBool,
                                                          MemberOffset(42),
                                                          false,
                                                          ...
                                                          graph->GetDexFile(),
                                                          0));
  then->AddInstruction(new (GetArena()) HGoto());
  else_->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                           nullptr,
                                                           DataType::Type::kBool,
                                                           MemberOffset(42),
                                                           false,
                                                           ...
                                                           graph->GetDexFile(),
                                                           0));
  else_->AddInstruction(new (GetArena()) HGoto());
  join->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                          nullptr,
                                                          DataType::Type::kBool,
                                                          MemberOffset(42),
                                                          false,
                                                          ...
                                                          graph->GetDexFile(),
                                                          0));
  join->AddInstruction(new (GetArena()) HExit());
...
TEST_F(GVNTest, LoopFieldElimination) {
  ...
  block->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                           ...
                                                           graph->GetDexFile(),
                                                           0));
  block->AddInstruction(new (GetArena()) HGoto());

  HBasicBlock* loop_header = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* loop_body = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* exit = new (GetArena()) HBasicBlock(graph);
  ...
  loop_header->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                                 nullptr,
                                                                 DataType::Type::kBool,
                                                                 MemberOffset(42),
                                                                 false,
  ...
  loop_header->AddInstruction(new (GetArena()) HIf(block->GetLastInstruction()));

  // Kill inside the loop body to prevent field gets inside the loop header
  // and the body to be GVN'ed.
  loop_body->AddInstruction(new (GetArena()) HInstanceFieldSet(parameter,
                                                               parameter,
                                                               nullptr,
                                                               DataType::Type::kBool,
                                                               MemberOffset(42),
  ...
  loop_body->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                               nullptr,
                                                               DataType::Type::kBool,
                                                               MemberOffset(42),
                                                               false,
  ...
  loop_body->AddInstruction(new (GetArena()) HGoto());

  exit->AddInstruction(new (GetArena()) HInstanceFieldGet(parameter,
                                                          nullptr,
                                                          DataType::Type::kBool,
                                                          MemberOffset(42),
                                                          false,
  ...
  exit->AddInstruction(new (GetArena()) HExit());
...
TEST_F(GVNTest, LoopSideEffects) {
  ...
  HBasicBlock* outer_loop_header = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* outer_loop_body = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* outer_loop_exit = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* inner_loop_header = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* inner_loop_body = new (GetArena()) HBasicBlock(graph);
  HBasicBlock* inner_loop_exit = new (GetArena()) HBasicBlock(graph);
  ...
  outer_loop_header->AddInstruction(new (GetArena()) HSuspendCheck());
  outer_loop_header->AddInstruction(new (GetArena()) HIf(parameter));
  outer_loop_body->AddInstruction(new (GetArena()) HGoto());
  inner_loop_header->AddInstruction(new (GetArena()) HSuspendCheck());
  inner_loop_header->AddInstruction(new (GetArena()) HIf(parameter));
  inner_loop_body->AddInstruction(new (GetArena()) HGoto());
  inner_loop_exit->AddInstruction(new (GetArena()) HGoto());
  outer_loop_exit->AddInstruction(new (GetArena()) HExit());
  ...
  // Check that the only side effect of loops is to potentially trigger GC.
  {
    ...
    entry->AddInstruction(new (GetArena()) HInstanceFieldSet(parameter,
                                                             parameter,
                                                             nullptr,
                                                             DataType::Type::kReference,
                                                             MemberOffset(42),
  ...
  // Check that the side effects of the outer loop does not affect the inner loop.
  {
    outer_loop_body->InsertInstructionBefore(
        new (GetArena()) HInstanceFieldSet(parameter,
                                           parameter,
                                           nullptr,
                                           DataType::Type::kReference,
                                           MemberOffset(42),
