...
class WBEVisitor final : public HGraphVisitor {
 public:
  ...
  // has to be a set since multiple predecessors might flow into the same block, and we want to
  // know all instructions.
  // TODO(solanes): How to make this a ScopedArenaHashsSet? We would need to pass the kArenaAllocWBE
  // in the constructor.
  ...
  void VisitBasicBlock(HBasicBlock* block) override {
    ...
    CHECK_LT(block->GetBlockId(), GetGraph()->GetBlocks().size());
    CHECK(write_barriers_per_block_[block->GetBlockId()].empty())
    ...
    // block.
    // LoopHeader blocks will be visited before their body, making the computation always empty.
  ...
  void VisitArraySet(HArraySet* instruction) override {
    if (instruction->GetSideEffects().Includes(SideEffects::CanTriggerGC())) {
      ClearCurrentValues(instruction->GetBlock());
    }
 ...
 private:
  void ClearCurrentValues(HBasicBlock* block) {
    write_barriers_per_block_[block->GetBlockId()].clear();
  }
