...
class SuperblockCloner : public ValueObject {
 public:
  ...
  // Set edge successor remapping info specified by corresponding edge sets.
  ...
  // Create a clone for each basic blocks in orig_bb_set adding corresponding entries into bb_map_
  // and hir_map_.
 ...
 private:
  // Fill the 'exits' vector with the subgraph exits. Return the number of exits.
  size_t FindSubgraphExits(ArenaVector<HBasicBlock*>* exits);

  // Find and record information about the area in the graph for which control-flow (back edges,
  // loops, dominators) needs to be adjusted.
  void FindAndSetLocalAreaForAdjustments();

  // Remap edges' successors according to the info specified in the edges sets.
  //
  ...
  // Adjust control-flow (back edges, loops, dominators) for the local area defined by
  // FindAndSetLocalAreaForAdjustments.
  void AdjustControlFlowInfo();

  // Resolve Data Flow - adjust phis' and instructions' inputs in order to have a valid graph in
  // the SSA form.
  ...
  // Adjust copy instruction's inputs: if the input of the original instruction is defined in the
  // orig_bb_set, replace it with a corresponding copy otherwise leave it the same as original.
  void ReplaceInputsWithCopies(HInstruction* copy_instr);

  // Recursively clone the environment for the copy instruction. If the input of the original
  // environment is defined in the orig_bb_set, replace it with a corresponding copy otherwise
  // leave it the same as original.
  ...
  // Remap incoming or original internal edge to its copy, adjust the phi inputs in orig_succ and
  // copy_succ.
  void RemapOrigInternalOrIncomingEdge(HBasicBlock* orig_block, HBasicBlock* orig_succ);

  // Add copy internal edge (from copy_block to copy_succ), update phis in the copy_succ.
  void AddCopyInternalEdge(HBasicBlock* orig_block, HBasicBlock* orig_succ);

  // Remap copy internal edge to its origin, adjust the phi inputs in orig_succ
  ...
  // Find back edges in the subgraph.
...
class PeelUnrollHelper : public ValueObject {
 public:
  ...
  // Return whether the loop can be peeled/unrolled (static function).
  static bool IsLoopCopyable(HLoopInformation* loop_info);

  // Return whether the loop can be peeled/unrolled.
  bool IsLoopCopyable() const { return cloner_.IsSubgraphCopyable(); }
 ...
 protected:
  // Apply loop peeling/unrolling for the loop specified by 'loop_info'.
  //
  // Depending on 'do_unroll' either unroll loop by 2 or peel one iteration from it.
...
class PeelUnrollSimpleHelper : public ValueObject {
 public:
  ...
  bool IsLoopCopyable() const { return helper_.IsLoopCopyable(); }
...
// Collect edge remapping info for loop peeling/unrolling for the loop specified by loop info.
