...
class SuperblockCloner : public ValueObject {
 public:
  ...
  void CleanUp(bool keep_both_loops);
  ...
  void AdjustControlFlowInfo(bool keep_both_loops);
 ...
 private:
  ...
  // "keep_both_loops" value true means keep both orig as well as copy loop
  void RecalculateBackEdgesInfo(ArenaBitVector* outer_loop_bb_set, bool keep_both_loops);
  GraphAnalysisResult AnalyzeLoopsLocally(ArenaBitVector* outer_loop_bb_set);
  void CleanUpControlFlow(bool keep_both_loops);
...
class PeelUnrollHelper : public ValueObject {
 public:
  ...
  HInstruction* AddLoopUnrollEpilogue(HGraph* graph, int unroll_factor);
