...
class LSEVisitor : public HGraphDelegateVisitor {
 ...
 private:
  ...
  void PopulateHeapFromPredecessors(HBasicBlock* block) {
    ...
    //
    // Pre-allocate a vector of size `predecessors.size()` so we don't have to
    // allocate this vector on every loop iteration.
    ScopedArenaVector<HInstruction*> pred_values(allocator_.Adapter(kArenaAllocLSE));
    pred_values.reserve(predecessors.size());
    for (size_t i = 0; i < heap_values.size(); i++) {
      pred_values.clear();
      MergePredecessorValues(block, i, pred_values);
    }
  }
