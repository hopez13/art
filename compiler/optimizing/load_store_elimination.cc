 ...
 *
 * Then we determine which stores to eliminate. We are keeping a record of the
 * stores and Phi placeholders that become observable in the previous processing
...
class LSEVisitor final : private HGraphDelegateVisitor {
 ...
 private:
  ...
  bool TryReplacingLoopPhiPlaceholderWithMutableIndex(
...
LSEVisitor::Value LSEVisitor::PrepareLoopValue(HBasicBlock* block, size_t idx) {
  ...
  // either with the pre-header value or with a new Phi.
  // For array locations, the index may be defined inside the loop but the only known
  // value should be only for replacement with the default value
  ...
  if (kIsDebugBuild) {
    ...
    if (index != nullptr && loop_info->Contains(*index->GetBlock())) {
      CHECK(pre_header_value.IsDefault() || pre_header_value.NeedsLoopPhi());
    }
  }
...
void LSEVisitor::UpdateValueRecordForStoreElimination(/*inout*/ValueRecord* value_record) {
  ...
  if (value_record->value.NeedsNonLoopPhi()) {
    ...
    value_record->value = Value::ForPhiPlaceholder(value_record->value.GetPhiPlaceholder(),
                                                   /*needs_loop_phi=*/ true);
  }
}

bool LSEVisitor::TryReplacingLoopPhiPlaceholderWithMutableIndex(
    const PhiPlaceholder* phi_placeholder,
    DataType::Type type,
    Phase phase,
    /*inout*/ ArenaBitVector* phi_placeholders_to_materialize) {
  ...
  while (!work_queue.empty()) {
    ...
    for (HBasicBlock* predecessor : current_block->GetPredecessors()) {
      ValueRecord* value_record = &heap_values_for_[predecessor->GetBlockId()][idx];
      if (phase == Phase::kStoreElimination) {
        UpdateValueRecordForStoreElimination(value_record);
        DCHECK(!value_record->value.NeedsNonLoopPhi());
      }
      Value value = ReplacementOrValue(value_record->value);
      if (value.NeedsPhi()) {
  ...
  Value replacement = Value::ForInstruction(GetDefaultValue(type));
  for (uint32_t phi_placeholder_index : visited.Indexes()) {
    DCHECK(phi_placeholder_replacements_[phi_placeholder_index].IsInvalid());
    phi_placeholder_replacements_[phi_placeholder_index] = replacement;
  }
...
const LSEVisitor::PhiPlaceholder* LSEVisitor::FindLoopPhisToMaterialize(
  ...
  while (!work_queue.empty()) {
    ...
    if (!phi_placeholders_to_materialize->IsBitSet(PhiPlaceholderIndex(current_phi_placeholder))) {
      // Replaced by TryReplacingLoopPhiPlaceholderWithMutableIndex().
    ...
    if (current_block->IsLoopHeader()) {
      // Check if the index is defined inside the loop.
      ...
      if (index != nullptr && current_block->GetLoopInformation()->Contains(*index->GetBlock())) {
        if (TryReplacingLoopPhiPlaceholderWithMutableIndex(
                current_phi_placeholder, type, phase, phi_placeholders_to_materialize)) {
          continue;
        } else {
    ...
    for (HBasicBlock* predecessor : current_block->GetPredecessors()) {
      ...
      if (value.IsUnknown()) {
        ...
        DCHECK(phase == Phase::kStoreElimination || current_block->IsLoopHeader());
        return current_phi_placeholder;  // Report the loop Phi placeholder.
      }
...
const LSEVisitor::PhiPlaceholder* LSEVisitor::TryToMaterializeLoopPhis(
  ...
  const PhiPlaceholder* loop_phi_with_unknown_input = FindLoopPhisToMaterialize(
      phi_placeholder, &phi_placeholders_to_materialize, type, Phase::kLoadElimination);
...
void LSEVisitor::FindOldValueForPhiPlaceholder(const PhiPlaceholder* phi_placeholder,
  ...
  phi_placeholders_to_materialize.ClearAllBits();
  const PhiPlaceholder* loop_phi_with_unknown_input = FindLoopPhisToMaterialize(
      phi_placeholder, &phi_placeholders_to_materialize, type, Phase::kStoreElimination);
  if (loop_phi_with_unknown_input != nullptr) {
