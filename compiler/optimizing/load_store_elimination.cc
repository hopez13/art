 ...
 *
 * The initial phase walks over blocks and their instructions. The initial set
 * of heap values for a basic block is
 ...
 *    zero from the array allocation throught the loop.
 ...
 * For each instructions in the block
 ...
 * on loop Phi placeholders to determine whether they can be eliminated. We look
 ...
 * so we are guaranteed progress when repeating this process until each load
 * is either marked for replacement or found to be unreplaceable. (At most
 * `loads_requiring_loop_phis_.size() + phi_placeholders_.size()` iterations.)
 *
