 ...
 *   locations since they have no alises. Singletons are helpful in narrowing
 *   down the life span of a heap location such that it does not need to
 *   participate in merging heap values throughout the method. Allocation of
 *   a singleton can be eliminated if that singleton is not used and does not
 *   persist at method return/deoptimization.
...
class LSEVisitor : public HGraphDelegateVisitor {
 ...
 private:
  static bool IsLoad(HInstruction* instruction) {
    ...
    // Unresolved load is treated as an invocation.
  ...
  static bool IsStore(HInstruction* instruction) {
    ...
    // Unresolved store is treated as an invocation.
  ...
  void MergePredecessorValues(HBasicBlock* block) {
    ...
    for (size_t i = 0; i < heap_values.size(); i++) {
      ...
      // the store as the heap value as long as possible. In case we can't
