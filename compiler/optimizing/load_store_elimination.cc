      ...
      for (HInstruction* ins : to_remove) {
        ...
        if (ins->IsInstanceFieldGet() || ins->IsInstanceFieldSet()) {
          ...
          if (UNLIKELY(ins->InputAt(0) == ins->InputAt(1))) {
            replacement = ins->IsEqual() ? GetGraph()->GetIntConstant(true)
                                         : GetGraph()->GetIntConstant(false);
          } else {
            replacement = ins->IsEqual() ? GetGraph()->GetIntConstant(false)
                                         : GetGraph()->GetIntConstant(true);
          }
