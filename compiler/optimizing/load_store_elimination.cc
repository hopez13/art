 ...
 private:
  ...
  void RemoveAllMonitorOperationsOf(HInstruction* object) {
    ...
    for (HMonitorOperation* monitor_op : to_delete) {
      ...
      // loop information in graphs with irreducible loops is unsupported.
...
void LSEVisitor::FinishFullLSE() {
  ...
  for (const LoadStoreRecord& record : loads_and_stores_) {
    ...
    // HBasicBlock* block = load->GetBlock();
    // int load_id = load->GetId();
    ...
    if ((load->IsInstanceFieldGet() && load->AsInstanceFieldGet()->IsVolatile()) ||
      ...
      // LOG(INFO) << "Eliminated load " << load_id << ": " << load->DebugName()
      //           << "Graph name: " << block->GetGraph()->GetArtMethod()->PrettyMethod();
    }
  }
  ...
  for (const LoadStoreRecord& record : loads_and_stores_) {
    if (IsStore(record.load_or_store) && !kept_stores_.IsBitSet(record.load_or_store->GetId())) {
      // HBasicBlock* block = record.load_or_store->GetBlock();
      // int store_id = record.load_or_store->GetId();
      ...
      if ((record.load_or_store->IsInstanceFieldSet() &&
        ...
        // ScopedObjectAccess soa(Thread::Current());
        // LOG(INFO) << "Eliminated store " << store_id << ": " << record.load_or_store->DebugName()
        //           << "Graph name: " << block->GetGraph()->GetArtMethod()->PrettyMethod();
