...
class LSEVisitor : public HGraphDelegateVisitor {
 public:
  ...
  // Scan the list of removed loads to see if we can reuse type_conversion.
  void TryToReuseTypeConversion(HInstruction* type_conversion,
  ...
  void RemoveInstructions() {
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      if (type_conversion != nullptr) {
        TryToReuseTypeConversion(type_conversion, substitute, i);
