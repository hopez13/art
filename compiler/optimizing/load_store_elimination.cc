...
class LSEVisitor final : private HGraphDelegateVisitor {
 ...
 private:
  ...
  void UpdateOldValueRecord(/*inout*/ValueRecord* old_value_record);
...
LSEVisitor::LSEVisitor(HGraph* graph,
  ...
  // Reserve space for all Phi placeholders, so that they keep their memory addresses.
...
bool LSEVisitor::MaterializeLoopPhis(const ArenaBitVector& phi_placeholders_to_materialize,
  ...
  while (remaining_phi_placeholders != 0u) {
    ...
    if (!MaterializeLoopPhis(current_subset, type, can_create_phis)) {
      DCHECK(!can_create_phis);
      ...
      // However, some Phi placeholders outside the current subset may still be replaceable.
      for (size_t phi_placeholder_index : current_subset) {
        DCHECK(phi_placeholder_replacements_[phi_placeholder_index].IsInvalid());
        phi_placeholder_replacements_[phi_placeholder_index] = Value::Unknown();
      }
      return false;
    }
...
const LSEVisitor::PhiPlaceholder* LSEVisitor::TryToMaterializeLoopPhis(
  ...
  bool success =
      MaterializeLoopPhis(phi_placeholders_to_materialize, type, /*can_create_phis=*/ true);
  DCHECK(success);
...
void LSEVisitor::UpdateOldValueRecord(/*inout*/ValueRecord* old_value_record) {
  while (old_value_record->stored_by.IsInstruction() &&
         !kept_stores_.IsBitSet(old_value_record->stored_by.GetInstruction()->GetId())) {
    auto it = store_records_.find(old_value_record->stored_by.GetInstruction());
    DCHECK(it != store_records_.end());
    *old_value_record = it->second.old_value_record;
  }
  if (old_value_record->stored_by.NeedsPhi() &&
      !phi_placeholders_to_search_for_kept_stores_.IsBitSet(
           PhiPlaceholderIndex(old_value_record->stored_by))) {
    ...
    old_value_record->value = old_value_record->stored_by;
  }
  old_value_record->value = ReplacementOrValue(old_value_record->value);
  if (old_value_record->value.NeedsNonLoopPhi()) {
    ...
    old_value_record->value = Value::ForPhiPlaceholder(old_value_record->value.GetPhiPlaceholder(),
                                                       /*needs_loop_phi=*/ true);
  }
}
...
void LSEVisitor::FindOldValueForPhiPlaceholder(const PhiPlaceholder* phi_placeholder,
  ...
  while (!work_queue.empty()) {
    const PhiPlaceholder* current_phi_placeholder = work_queue.back();
    work_queue.pop_back();
    DCHECK(phi_placeholder_replacements_[PhiPlaceholderIndex(current_phi_placeholder)].IsInvalid());
    HBasicBlock* current_block = blocks[current_phi_placeholder->GetBlockId()];
    DCHECK_GE(current_block->GetPredecessors().size(), 2u);
    size_t idx = current_phi_placeholder->GetHeapLocation();
    for (HBasicBlock* predecessor : current_block->GetPredecessors()) {
      ValueRecord* record = &heap_values_for_[predecessor->GetBlockId()][idx];
      UpdateOldValueRecord(record);
      if (record->value.IsUnknown()) {
        // Mark the current placeholder as well as the input Phi placeholder (if different)
        // as unreplaceable. Other Phi placeholders in the work queue may still be replaceable.
        phi_placeholder_replacements_[PhiPlaceholderIndex(phi_placeholder)] = Value::Unknown();
        phi_placeholder_replacements_[PhiPlaceholderIndex(current_phi_placeholder)] =
            Value::Unknown();
        return;
      }
      DCHECK(!record->value.NeedsNonLoopPhi());
      if (record->value.NeedsLoopPhi()) {
        // Visit the predecessor Phi placeholder if it's not visited yet.
        if (!visited.IsBitSet(PhiPlaceholderIndex(record->value))) {
          visited.SetBit(PhiPlaceholderIndex(record->value));
          work_queue.push_back(record->value.GetPhiPlaceholder());
        }
      }
    }
  }

  bool success = MaterializeLoopPhis(visited, type, /*can_create_phis=*/ false);
  if (!success) {
    // The current placeholder may or may not have been marked as unknown.
    DCHECK(phi_placeholder_replacements_[PhiPlaceholderIndex(phi_placeholder)].IsInvalid() ||
           phi_placeholder_replacements_[PhiPlaceholderIndex(phi_placeholder)].IsUnknown());
    phi_placeholder_replacements_[PhiPlaceholderIndex(phi_placeholder)] = Value::Unknown();
  }
}
...
void LSEVisitor::FindStoresWritingOldValues() {
  ...
  for (auto&& entry : store_records_) {
    ...
    UpdateOldValueRecord(&store_record.old_value_record);
