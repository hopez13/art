...
class LSEVisitor final : private HGraphDelegateVisitor {
 ...
 private:
  ...
  Value Replacement(Value value, bool for_partial = false) const {
    DCHECK(value.NeedsPhi() || (for_partial && value.IsMergedUnknown()))
        << value << " for_partial: " << std::boolalpha << for_partial;
  ...
  Value ReplacementOrValue(Value value, bool for_partial = false) const {
    ...
    if ((value.NeedsPhi() || (for_partial && value.IsMergedUnknown())) &&
      ...
      return Replacement(value, for_partial);
    } else {
  ...
  void MaterializeNonLoopPhis(PhiPlaceholder phi_placeholder,
                              DataType::Type type,
                              bool for_partial = false);
  ...
  bool MaterializeLoopPhis(const ScopedArenaVector<size_t>& phi_placeholder_indexes,
                           DataType::Type type,
                           Phase phase);
  bool MaterializeLoopPhis(ArrayRef<const size_t> phi_placeholder_indexes,
                           DataType::Type type,
                           Phase phase);
  bool MaterializeLoopPhis(const ArenaBitVector& phi_placeholders_to_materialize,
                           DataType::Type type,
                           Phase phase);
  bool MaterializeLoopPhis(PhiPlaceholder phi_placeholder, DataType::Type type, Phase phase);
...
LSEVisitor::Value LSEVisitor::PriorValueHolder::ToValue() const {
  if (IsDefault()) {
    ...
    return Value::ForPhiPlaceholder(GetPhiPlaceholder(), false);
  } else if (IsMergedUnknown()) {
...
void LSEVisitor::MaterializeNonLoopPhis(PhiPlaceholder phi_placeholder,
                                        DataType::Type type,
                                        bool for_partial) {
...
bool LSEVisitor::MaterializeLoopPhis(PhiPlaceholder phi_placeholder,
                                     DataType::Type type,
                                     Phase phase) {
  size_t placeholder_idx = PhiPlaceholderIndex(phi_placeholder);
  ArrayRef<const size_t> idx(&placeholder_idx, 1);
  return MaterializeLoopPhis(idx, type, phase);
}

bool LSEVisitor::MaterializeLoopPhis(const ScopedArenaVector<size_t>& phi_placeholder_indexes,
                                     DataType::Type type,
                                     Phase phase) {
  return MaterializeLoopPhis(ArrayRef<const size_t>(phi_placeholder_indexes), type, phase);
}
...
bool LSEVisitor::MaterializeLoopPhis(ArrayRef<const size_t> phi_placeholder_indexes,
  ...
  for (size_t phi_placeholder_index : phi_placeholder_indexes) {
    ...
    for (HBasicBlock* predecessor : block->GetPredecessors()) {
      Value value = ReplacementOrValue(heap_values_for_[predecessor->GetBlockId()][idx].value,
                                       /*for_partial=*/phase == Phase::kPartialElimination);
      if (value.NeedsNonLoopPhi() ||
          (phase == Phase::kPartialElimination && value.IsMergedUnknown())) {
        DCHECK(phase == Phase::kLoadElimination || phase == Phase::kPartialElimination);
        MaterializeNonLoopPhis(
            value.GetPhiPlaceholder(), type, /*for_partial=*/phase == Phase::kPartialElimination);
        value = Replacement(value, /*for_partial=*/phase == Phase::kPartialElimination);
      }
      if (!value.NeedsLoopPhi()) {
  ...
  for (auto& [new_inst, hl] : ids) {
      ...
      for (HBasicBlock* entry : excluded_cohort.EntryBlocksReversePostOrder()) {
        ...
        auto setup_materializations = [&, new_inst = new_inst, hl = hl](uint32_t pred_idx,
          ...
          for (const HeapLocation* loc : hl) {
            ...
            Value value = heap_values_for_[old_pred->GetBlockId()][loc_off].value;
            if (value.NeedsLoopPhi()) {
              auto repl =
              ...
              if (!repl.IsInvalid() && !repl.IsUnknown()) {
                ...
                MaterializeLoopPhis(
                    value.GetPhiPlaceholder(), hl.front()->GetType(), Phase::kPartialElimination);
                value =
                    phi_placeholder_replacements_[PhiPlaceholderIndex(value.GetPhiPlaceholder())];
              }
    ...
    for (auto& user : new_inst->GetUses()) {
      if (in_escape_cohort(user.GetUser()->GetBlock())) {
        ...
        DCHECK(user.GetUser()->IsFieldAccess() || user.GetUser()->IsEqual() ||
    ...
    auto get_value_at = [&](HInstruction* ins) -> HInstruction* {
      ...
      if (pred.IsInstruction()) {
        ...
        MaterializeNonLoopPhis(pred.GetPhiPlaceholder(),
                               ...
                               true);
        // MaterializeLoopPhis(pred.GetPhiPlaceholder(),
        //                     heap_location_collector_.GetHeapLocation(loc)->GetType(),
        //                     Phase::kPartialElimination);
        auto res = Replacement(pred, true).GetInstruction();
