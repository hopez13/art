    ...
    bool Equals(Value other) const {
      ...
      if (type_ != other.type_) {
        ...
        return IsDefault() || (IsInstruction() && GetInstruction() == other.GetInstruction()) ||
               ...
               (IsMergedUnknown() && phi_placeholder_ == other.phi_placeholder_);
      }
    }
...
LSEVisitor::Value LSEVisitor::MergePredecessorValues(HBasicBlock* block, size_t idx) {
  ...
  for (size_t i = 1u, size = predecessors.size(); i != size; ++i) {
    if (merged_value.IsMergedUnknown() && merged_value.GetMergeBlock(GetGraph()) == block) {
      break;
    }
    Value pred_value =
...
void LSEVisitor::SearchMergedUnknownsForKeptStores() {
  for (const Value& val : kept_merged_unknowns_) {
    ...
    std::for_each(val.GetMergeBlock(GetGraph())->GetPredecessors().cbegin(),
                  val.GetMergeBlock(GetGraph())->GetPredecessors().cend(),
                  keep);
  }
}
