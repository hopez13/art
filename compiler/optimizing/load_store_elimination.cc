...
class LSEVisitor final : private HGraphDelegateVisitor {
 ...
 private:
  ...
  bool TryReplacingLoopPhiPlaceholderWithoutPhi(
...
bool LSEVisitor::TryReplacingLoopPhiPlaceholderWithoutPhi(
    const PhiPlaceholder* phi_placeholder,
    bool use_default,
    DataType::Type type,
    /*inout*/ArenaBitVector* phi_placeholders_to_materialize) {
  ...
  DCHECK_EQ(replacement == nullptr, use_default);
  if (use_default) {
    replacement = GetDefaultValue(type);
  }
...
const LSEVisitor::PhiPlaceholder* LSEVisitor::FindLoopPhisToMaterialize(
  ...
  while (!work_queue.empty()) {
    ...
    if (!phi_placeholders_to_materialize->IsBitSet(PhiPlaceholderIndex(current_phi_placeholder))) {
      // Replaced by TryReplacingLoopPhiPlaceholderWithoutPhi().
    ...
    if (current_block->IsLoopHeader()) {
      ...
      bool index_defined_in_loop =
          (index != nullptr) && current_block->GetLoopInformation()->Contains(*index->GetBlock());
      if (index_defined_in_loop || !can_use_default_or_phi) {
        if ((!index_defined_in_loop || can_use_default_or_phi) &&
            TryReplacingLoopPhiPlaceholderWithoutPhi(current_phi_placeholder,
                                                     /*use_default=*/ index_defined_in_loop,
                                                     type,
