...
class LSEVisitor : public HGraphDelegateVisitor {
 public:
  LSEVisitor(HGraph* graph,
      ...
      : HGraphDelegateVisitor(graph, stats),
        ...
        pred_values_(allocator_.Adapter(kArenaAllocLSE)) {
  }
 ...
 private:
      ...
      *all_predecessors_have_same_value &=
      ...
      if (pred_store_value == kUnknownHeapValue) {
        ...
        *all_predecessors_store_same_value = false;
        ...
        pred_values_.push_back(pred_store_value);
        *all_predecessors_store_same_value &= (pred_store_value == pred_values_[0]);
      }
    }
  ...
  void MergePredecessorValues(HBasicBlock* block, int heap_index) {
    ...
    if (can_be_merged && pred_values_.size() > 1) {
      ...
      merged_type = GetCommonType(pred_values_,
                                  &types_are_compatible,
                                  &all_default_values);
      ...
      if (!types_are_compatible) {
        ...
        // constant.
        for (size_t j = 0, n = pred_values_.size(); j < n; j++) {
          if (pred_values_[j] == kDefaultHeapValue) {
            pred_values_[j] = GetDefaultValue(merged_type);
          }
        }
      } else {
        ...
        DCHECK(pred_values_[0] == kDefaultHeapValue);
      }
    }
  ...
  void PopulateHeapFromPredecessors(HBasicBlock* block) {
    ...
    for (size_t i = 0, n = heap_values.size(); i < n; i++) {
  ...
  // order we visit blocks (reverse post order), so that the inputs of all Phis
  // are visited first.
  ...
  // Vector is for memory reuse in MergePredecessorValues().
  ScopedArenaVector<HInstruction*> pred_values_;
