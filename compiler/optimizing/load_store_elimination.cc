...
void LSEVisitor::SearchPhiPlaceholdersForKeptStores() {
  ...
  while (!work_queue.empty()) {
    ...
    bool is_kept_merged_unknown = kept_merged_unknowns_.IsBitSet(cur_phi_idx);
    work_queue.pop_back();
    size_t idx = phi_placeholder->GetHeapLocation();
    HBasicBlock* block = blocks[phi_placeholder->GetBlockId()];
    for (HBasicBlock* predecessor : block->GetPredecessors()) {
      ...
      for (size_t i = start; i != end; ++i) {
        ...
        if (!stored_by.IsUnknown() && (i == idx || may_alias(i) || is_kept_merged_unknown)) {
          if (stored_by.NeedsPhi()) {
            size_t phi_placeholder_index = PhiPlaceholderIndex(stored_by);
            if (is_kept_merged_unknown) {
              // Propogate merged-unknown keep since otherwise this might look
            ...
            if (!is_kept_merged_unknown && IsPartialNoEscape(predecessor, idx)) {
              if (not_kept_stores) {
