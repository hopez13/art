...
void LSEVisitor::FindStoresWritingOldValues() {
  ...
  for (const LoadStoreRecord& record : loads_and_stores_) {
    ...
    if (!is_store || !kept_stores_.IsBitSet(record.load_or_store->GetId())) {
      continue;  // Ignore all loads and stores that are not kept.
    }
    auto it = store_records_.find(record.load_or_store);
    DCHECK(it != store_records_.end());
    ValueRecord old_record = it->second.old_record;
    if (old_record.value.IsUnknown()) {
      continue;  // We cannot eliminate this store because we do not know the old value.
    }
    // TODO: Search for Phi placeholder replacements if they are unknown.
    Value old_value = ReplacementOrValue(old_record.value);
    HInstruction* stored_value = FindSubstitute(it->second.stored_value);
    if (old_value.Equals(stored_value)) {
      eliminated_stores.SetBit(record.load_or_store->GetId());
    }
  }
