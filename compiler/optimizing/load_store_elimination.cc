 ...
 private:
  ...
  void VisitDeoptimize(HDeoptimize* instruction) override {
    ...
    for (size_t i = 0u, size = heap_values.size(); i != size; ++i) {
      ...
      if (info->IsSingleton()) {
        ...
        // Override as true to keep extra stores as Deoptimize itself can throw.
        if ((!info->IsSingletonAndRemovable() &&
             !(info->IsPartialSingleton() && IsPartialNoEscape(block, i)))) {
          observable = true;
        }
      }
      if (observable) {
  ...
  void HandleExit(HBasicBlock* block, bool must_keep_stores = false) {
    ...
    for (size_t i = 0u, size = heap_values.size(); i != size; ++i) {
      ...
      if (must_keep_stores || (!ref_info->IsSingletonAndRemovable() &&
  ...
  void HandleThrowingInstruction(HInstruction* instruction) {
    ...
    // TODO(solanes): Move this `GetTryCatchInformation` to the try catch CL.
    HandleExit(instruction->GetBlock(),
  ...
  void VisitNewInstance(HNewInstance* new_instance) override {
    ...
    for (size_t i = 0u, size = heap_values.size(); i != size; ++i) {
      ...
      } else if (!info->IsSingletonAndRemovable() &&
                 !(info->IsPartialSingleton() && IsPartialNoEscape(block, i))) {
