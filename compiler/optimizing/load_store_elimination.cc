...
class LSEVisitor final : private HGraphDelegateVisitor {
 ...
 private:
  ...
  bool EscapingObject(ReferenceInfo* info, HBasicBlock* block, size_t index) {
  ...
  void VisitDeoptimize(HDeoptimize* instruction) override {
    ...
    for (size_t i = 0u, size = heap_values.size(); i != size; ++i) {
      ...
      if (info->IsSingleton()) {
        ...
        if (!finalizable_object && !EscapingObject(info, block, i)) {
  ...
  void HandleExit(HBasicBlock* block) {
    ...
    for (size_t i = 0u, size = heap_values.size(); i != size; ++i) {
      ...
      if (EscapingObject(ref_info, block, i)) {
        KeepStores(heap_values[i].stored_by);
        heap_values[i].stored_by = Value::PureUnknown();
      }
    }
  ...
  void VisitNewInstance(HNewInstance* new_instance) override {
    ...
    for (size_t i = 0u, size = heap_values.size(); i != size; ++i) {
      ...
      } else if (EscapingObject(info, block, i)) {
