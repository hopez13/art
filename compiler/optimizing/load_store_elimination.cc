  ...
  for (auto& [new_inst, hl] : ids) {
    ...
    for (const auto& excluded_cohort : nesg->GetExcludedCohorts()) {
      ...
      for (HBasicBlock* rpo_blk :
        ...
        if (rpo_blk->GetPredecessors().size() == 1u) {
          // Single predecessor. Just use the old value.
          cur_val = materializations.Get(rpo_blk->GetSinglePredecessor());
        } else {
          HPhi* phi_val = new (GetGraph()->GetAllocator()) HPhi(GetGraph()->GetAllocator(),
                                                                kNoRegNumber,
                                                                rpo_blk->GetPredecessors().size(),
                                                                new_inst->GetType());
          for (auto [pred, off] : ZipCount(MakeIterationRange(rpo_blk->GetPredecessors()))) {
            phi_val->SetRawInputAt(off, pred != rpo_blk ? materializations.Get(pred) : phi_val);
          }
          rpo_blk->AddPhi(phi_val);
          cur_val = phi_val;
        }
        LOG(ERROR) << "In blk " << rpo_blk->GetBlockId() << " materialization is " << *cur_val;
        materializations.Put(rpo_blk, cur_val);
      }
      ...
      for (HBasicBlock* entry : excluded_cohort.EntryBlocks()) {
        HInstruction* merged_inst = materializations.Get(entry);
