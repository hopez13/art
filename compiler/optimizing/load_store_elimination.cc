...
namespace art {
  ...
  #define LOG_PLSE(LVL) if (current_phase_ == Phase::kPartialElimination) LOG(LVL)
 ...
 private:
  ...
  class Value {
   public:
    ...
    constexpr Value() : type_(Type::kInvalid), value_(std::nullopt) {}
...
void LSEVisitor::MaterializeNonLoopPhis(PhiPlaceholder phi_placeholder, DataType::Type type) {
  ...
  while (!work_queue.empty()) {
    ...
    if (phi_inputs.size() == current_block->GetPredecessors().size()) {
      ...
      LOG_PLSE(WARNING) << "Created " << type << " phi " << *phi_placeholder_replacements_[PhiPlaceholderIndex(current_phi_placeholder)].GetInstruction();
...
bool LSEVisitor::MaterializeLoopPhis(ArrayRef<const size_t> phi_placeholder_indexes,
  ...
  for (size_t phi_placeholder_index : phi_placeholder_indexes) {
    ...
    DCHECK(DataType::IsTypeConversionImplicit(type, phi->GetType())) << "phi type: " << phi->GetType() << " vs " << type;
    for (size_t i = 0, size = block->GetPredecessors().size(); i != size; ++i) {
      ...
      DCHECK(DataType::IsTypeConversionImplicit(input->GetType(), phi->GetType())) << value << " phi: " << phi->GetType() << " input: " << input->GetType() << " request: " << type << " init value: " << heap_values_for_[predecessor->GetBlockId()][idx].value;
    }
  }
  ...
  for (size_t phi_placeholder_index : phi_placeholder_indexes) {
    ...
    // LOG_PLSE(WARNING) << "Created " << type << " phi " << *phi_placeholder_replacements_[phi_placeholder_index].GetInstruction();
  }
  if (type == DataType::Type::kReference) {
...
void __attribute__((optnone)) LSEVisitor::MovePartialEscapes() {
  ...
  struct HeapReferenceData {
   public:
    ...
    HeapReferenceData(const LSEVisitor* vis, ScopedArenaAllocator* alloc)
        : heap_locs_(alloc,
                     vis->heap_location_collector_.GetNumberOfHeapLocations(),
                     false,
                     kArenaAllocLSE),
          materializations_(
              ...
              2 * vis->GetGraph()->GetBlocks().size(), nullptr, alloc->Adapter(kArenaAllocLSE)),
          collector_(vis->heap_location_collector_) {}
    ...
    void AddMaterialization(HBasicBlock* blk, HInstruction* ins) {
      materializations_.resize(
          std::min(blk->GetBlockId() + 1, static_cast<uint32_t>(materializations_.size())),
          nullptr);
    ...
    HInstruction* GetMaterialization(HBasicBlock* blk) const {
      if (materializations_.size() <= blk->GetBlockId() || materializations_[blk->GetBlockId()] == nullptr) {
  ...
  std::vector<HInstruction*> new_ref_phis;
  ...
  for (auto& [new_inst, hl] : ids) {
    ...
    LOG(INFO) << "Working on " << *new_inst << " in " << GetGraph()->PrettyMethod();
    // All should have same ref-info.
    auto nesg = hl.front()->GetReferenceInfo()->GetNoEscapeSubgraph();
    if (!nesg->IsValid() || !nesg->ContainsBlock(new_inst->GetBlock())) {
    ...
    for (const auto& excluded_cohort : nesg->GetExcludedCohorts()) {
      ...
      for (HBasicBlock* entry : excluded_cohort.EntryBlocksReversePostOrder()) {
        ...
        auto setup_materializations = [&, new_inst = new_inst, hl = hl](uint32_t pred_idx,
                                                                        HBasicBlock* old_pred) {
          DCHECK_EQ(entry->GetPredecessors()[pred_idx], old_pred);
          if (UNLIKELY(!new_inst->GetBlock()->Dominates(entry))) {
            LOG(ERROR) << "Initial materialization in non-dominating block " << entry->GetBlockId()
                       << " is null!";
            pred_vals.push_back(GetGraph()->GetNullConstant());
            return;
          }
          HBasicBlock* bb = materialization_blocks.GetOrCreate({entry, pred_idx}, [&]() {
            HBasicBlock* out = new (GetGraph()->GetAllocator()) HBasicBlock(GetGraph());
            GetGraph()->AddBlock(out);
            LOG(ERROR) << "creating materialization block " << out->GetBlockId() << " on edge "
                       << old_pred->GetBlockId() << "->" << entry->GetBlockId();
            out->AddInstruction(new (GetGraph()->GetAllocator()) HGoto());
            return out;
          });
          ...
          LOG(ERROR) << "In blk " << bb->GetBlockId() << " initial materialization is "
                     << *repl_create;
          materializations.Put(bb, repl_create);
          ...
          for (const HeapLocation* loc : hl) {
            ...
            DCHECK(value.IsDefault() || value.IsInstruction()) << GetGraph()->PrettyMethod() << ": " << value;
            ...
            if (!value.IsDefault()) {
              ...
              LOG(WARNING) << "Adding " << *set_value << " for materialization setup!";
            }
          }
        };
        for (const auto& [pred, pred_idx] :
             ZipCount(MakeIterationRange(entry->GetPredecessors()))) {
          DCHECK(!is_materialization_block(pred));
        ...
        if (LIKELY(pred_vals.size() == 1u)) {
          LOG(ERROR) << "In blk " << entry->GetBlockId() << " materialization is "
                     << *pred_vals.front();
          CHECK(materializations.find(entry) == materializations.end())
              << "entry: " << entry->GetBlockId() << " -> "
              << materializations.find(entry)->first->GetBlockId() << ", "
              << *materializations.find(entry)->second;
    ...
    for (HBasicBlock* blk : GetGraph()->GetReversePostOrder()) {
      ...
      if (materializations.find(blk) != materializations.end()) {
        ...
        LOG(ERROR) << "In blk " << blk->GetBlockId() << " materialization is "
      ...
      if (blk->IsExitBlock()) {
        ...
        // correctly.)
        cur_val = materializations.Get(blk->GetLoopInformation()->GetPreHeader());
      } else {
        ...
        if (found_differences) {
          blk->AddPhi(phi_val);
          new_ref_phis.push_back(phi_val);
          cur_val = phi_val;
        } else {
          cur_val = *init;
        }
    ...
    // Any other materialization blocks should also get their values so we've
    // got a full map (since previous go arounds might move new-insts with the
    // to-be-moved object in the environment).
    std::for_each(GetGraph()->GetBlocks().cbegin() + kFirstMaterializationBlock,
                  GetGraph()->GetBlocks().cend(),
                  [&](HBasicBlock* blk) {
                    if (blk == nullptr || materializations.find(blk) != materializations.end()) {
                      return;
                    } else {
                      // TODO Do this better.
                      // We must be a materialization block used in a prior partial LSE but not in
                      // this one. Just fill in with the prior blocks value (since all
                      // materialization blocks have only one pred+succ)
                      auto [next, pred_num] =
                          std::find_if(materialization_blocks.begin(),
                                       materialization_blocks.end(),
                                       [&](const auto& entry) { return blk == entry.second; })
                              ->first;
                      materializations.Put(blk,
                                           materializations.Get(next->GetPredecessors()[pred_num]));
                    }
                  });
    ...
    auto is_post_escape = [&](HBasicBlock* blk) {
      return std::any_of(
          nesg->GetExcludedCohorts().cbegin(),
          nesg->GetExcludedCohorts().cend(),
    ...
    // Do we needto add a constructor-fence.
    ...
    for (auto& user : new_inst->GetUses()) {
      if (in_escape_cohort(user.GetUser()->GetBlock())) {
      ...
      } else if (is_post_escape(user.GetUser()->GetBlock())) {
        ...
        LOG(ERROR) << "User " << *user.GetUser() << " being moved to materialization!";
    ...
    for (HInstruction* ins : to_remove) {
      ...
      if (ins->IsInstanceFieldGet() || ins->IsInstanceFieldSet()) {
        // TODO Replace any uses? IDK I think the uses should be gone already?
    ...
    if (!constructor_fences.empty()) {
      ...
      for (const ExecutionSubgraph::ExcludedCohort& ec : nesg->GetExcludedCohorts()) {
        for (HBasicBlock* blk : ec.EntryBlocks()) {
          for (HBasicBlock* materializer :
               Filter(MakeIterationRange(blk->GetPredecessors()), is_materialization_block)) {
            HInstruction* new_cf = new (GetGraph()->GetAllocator()) HConstructorFence(
                materializations.Get(materializer), pc, GetGraph()->GetAllocator());
    ...
    auto get_value_at = [&](HInstruction* ins) -> HInstruction* {
      ...
      LOG(WARNING) << "using " << pred << " as default value for " << *ins;
      if (pred.IsInstruction()) {
        return pred.GetInstruction();
      } else if (pred.IsMergedUnknown() || pred.NeedsPhi()) {
        // TODO This is slightly wrong. Need to expand MLP to do the recursion needed. Looks like we
        // sort of have FindLoopPhisToMaterialize bs.
        // MaterializeNonLoopPhis(pred.GetPhiPlaceholder(),
      ...
      LOG(FATAL) << "Got final value for " << *ins << " as " << pred;
      return nullptr;
    };
    for (auto& [ins, idx] : to_predicate) {
      ...
      if (ins->IsInstanceFieldGet()) {
        ...
        if (ins->InputAt(0) == ins->InputAt(1)) {
          ...
          HInstruction* is_null = new (GetGraph()->GetAllocator())
              HNotEqual(materializations.Get(ins->GetBlock()), GetGraph()->GetNullConstant());
          ...
          ins->ReplaceInput(materializations.Get(ins->GetBlock()), this_is_first ? 0 : 1);
    ...
    for (auto& [user, idx] : env_uses) {
      ...
      HInstruction* merged_inst = materializations.Get(user->GetHolder()->GetBlock());
      LOG(INFO) << "Replacing env use of " << *user->GetHolder() << "@" << idx << " with "
                << *merged_inst;
  ...
  for (const auto& [source, materialization] : materialization_blocks) {
    auto entry = source.first;
    auto pred_idx = source.second;
    auto pred = source.first->GetPredecessors()[pred_idx];
    materialization->InsertBetween(pred, entry);
    LOG(ERROR) << "Adding materialization block " << materialization->GetBlockId() << " on edge "
               << pred->GetBlockId() << "->" << entry->GetBlockId();
  }
  if (made_changes) {
    ReferenceTypePropagation rtp_fixup(GetGraph(),
                                       ...
                                       Handle<mirror::DexCache>(),
                                       /* is_first_run= */ false);
    rtp_fixup.Visit(ArrayRef<HInstruction* const>(new_ref_phis));
