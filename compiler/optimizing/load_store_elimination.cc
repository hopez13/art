...
#define LSE_VLOG if (LoadStoreElimination::kVerboseLoggingMode) VLOG(compiler)
  ...
  for (HeapReferenceData& ref_data : helper.GetHeapRefs()) {
    ...
    for (const auto& excluded_cohort : ref_data.GetNoEscapeSubgraph()->GetExcludedCohorts()) {
      ...
      for (HBasicBlock* entry : excluded_cohort.EntryBlocksReversePostOrder()) {
        ...
        auto setup_materializations = [&](uint32_t pred_idx, HBasicBlock* old_pred) {
          ...
          LSE_VLOG << "In blk " << bb->GetBlockId() << " initial materialization is "
                   << *repl_create;
        ...
        if (LIKELY(pred_vals.size() == 1u)) {
          LSE_VLOG << "In blk " << entry->GetBlockId() << " materialization is "
                   << *pred_vals.front();
          ...
          entry->AddPhi(phi);
          LSE_VLOG << "In blk " << entry->GetBlockId() << " materialization is " << *phi;
    ...
    for (HBasicBlock* blk : GetGraph()->GetReversePostOrder()) {
      ...
      if (ref_data.HasMaterialization(blk)) {
        ...
        LSE_VLOG << "In blk " << blk->GetBlockId() << " materialization is "
      ...
      LSE_VLOG << "In blk " << blk->GetBlockId() << " materialization is " << *cur_val;
    ...
    for (auto& user : new_inst->GetUses()) {
      if (ref_data.InEscapeCohort(user.GetUser()->GetBlock())) {
        ...
        LSE_VLOG << "User " << *user.GetUser() << " being moved to materialization!";
        constructor_fences.push_back({user.GetUser()->AsConstructorFence(), user.GetIndex()});
      } else {
        LSE_VLOG << "User " << *user.GetUser() << " not contained in cohort!";
    ...
    auto get_value_at = [&](HInstruction* ins) -> HInstruction* {
      ...
      LSE_VLOG << "using " << pred << " as default value for " << *ins;
      if (pred.IsInstruction()) {
        ...
        LSE_VLOG << pred << " materialized to " << *res;
        return res;
      }
      LSE_VLOG << "Got final value for " << *ins << " as " << pred;
      return nullptr;
    };
