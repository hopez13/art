  ...
  while (!work_queue.empty()) {
    ...
    for (HBasicBlock* predecessor : block->GetPredecessors()) {
      ...
      for (size_t i = start; i != end; ++i) {
        ...
        if (!stored_by.IsUnknown() &&
            (i == idx || may_alias(i) || (is_partial_kept_merged_unknown && i == idx))) {
          if (stored_by.NeedsPhi()) {
            ...
            if (is_partial_kept_merged_unknown) {
              ...
              // Ensure that we will actually look at the phi being kept.
              DCHECK(!phi_placeholders_to_search_for_kept_stores_.IsBitSet(phi_placeholder_index) ||
                     std::find(work_queue.cbegin(), work_queue.cend(), phi_placeholder_index) !=
                         work_queue.cend())
                  << "to_search? " << std::boolalpha
                  << phi_placeholders_to_search_for_kept_stores_.IsBitSet(phi_placeholder_index)
                  << " queued? " << std::boolalpha
                  << (std::find(work_queue.cbegin(), work_queue.cend(), phi_placeholder_index) !=
                      work_queue.cend());
            }
