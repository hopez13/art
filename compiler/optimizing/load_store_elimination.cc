...
void LSEVisitor::SearchPhiPlaceholdersForKeptStores() {
  ...
  while (!work_queue.empty()) {
    ...
    if (unknown_array_stores) {
      // Since we are an array, actually ended up here, are in a loop, and don't
      // have a known set this must be from a previous iteration (otherwise we'd
      // either not reach this because stores were kept from escapes or just
      // known from earlier). We need to keep all stores that could affect this
      // read.
      //
      ...
      //     branch).
      // (3) If we are not in a loop then the only way we could end up with
      //     an unknown value is if the array escaped so that will handle those
      //     writes.
      //
