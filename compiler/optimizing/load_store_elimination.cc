...
LSEVisitor::LSEVisitor(HGraph* graph,
    ...
    : HGraphDelegateVisitor(graph, stats),
      ...
      verbose_((false) && graph->PrettyMethod() ==
               "int Main.testLoop14(TestClass2, int)") {
  // Clear bit vectors.
  ...
  for (size_t i = 1u, size = predecessors.size(); i != size; ++i) {
    ...
    if (!merged_value.IsUnknown()) {
      if (pred_value.IsUnknown()) {
        ...
        merged_value =
            ReplacementOrValue(Value::ForPhiPlaceholder(phi_placeholder, needs_loop_phi));
      }
    }
  }
  return merged_value;
...
void LSEVisitor::MergePredecessorRecords(HBasicBlock* block) {
  ...
  if (verbose_) {
    LOG(ERROR) << "VMARKO: MergePredecessorRecords #" << block->GetBlockId()
        << " <-" << [&]() {
          std::ostringstream oss;
          for (HBasicBlock* p : block->GetPredecessors()) {
            oss << " " << p->GetBlockId();
          ...
          for (size_t i = 0; i != num; ++i) {
            oss << " loc[" << i << "]: ";
            for (HBasicBlock* p : block->GetPredecessors()) {
              ScopedArenaVector<ValueRecord>& hvs = heap_values_for_[p->GetBlockId()];
...
void LSEVisitor::VisitGetLocation(HInstruction* instruction, size_t idx) {
  ...
  if (verbose_) {
    LOG(ERROR) << "VMARKO: VisitGetLocation "
        ...
        << " " << idx << " " << record.value.IsDefault()
        << "/" << record.value.IsUnknown() << "/"
        << record.value.NeedsPhi();
  }
    ...
    for (; loads_and_stores_index != loads_and_stores_size; ++loads_and_stores_index) {
      ...
      if (verbose_) {
        LOG(ERROR) << "VMARKO: Process " << load_or_store->DebugName()
            << "@" << load_or_store->GetDexPc() << "/" << load_or_store->GetId() << " idx: " << idx
            << "pred values:" << [&]() {
              std::ostringstream oss;
              for (HBasicBlock* p : block->GetPredecessors()) {
                const ValueRecord& d = heap_values_for_[p->GetBlockId()][idx];
      ...
      if (stored_value == nullptr) {
          ...
          if (local_heap_values[idx].IsUnknown()) {
            ...
            if (verbose_) {
              LOG(ERROR) << "VMARKO: Adding substitute for " << load_or_store->DebugName() << "@"
                  << load_or_store->GetDexPc() << " -> " << heap_value->DebugName() << "@"
                  << heap_value->GetDexPc() << "/" << heap_value->GetId();
            }
        ...
        if (replacement.NeedsLoopPhi()) {
          ...
          // Use the load fetched a known value, use it, otherwise use the load.
...
void LSEVisitor::ProcessLoadsRequiringLoopPhis() {
  for (auto& entry : loads_requiring_loop_phi_) {
    ...
    while (record.value.NeedsLoopPhi() &&
      ...
      if (loop_phi_with_unknown_input != nullptr) {
        DCHECK(phi_placeholder_replacements_[PhiPlaceholderIndex(record.value)].IsInvalid());
        ProcessLoopPhiWithUnknownInput(loop_phi_with_unknown_input);
      } else {
        DCHECK(phi_placeholder_replacements_[PhiPlaceholderIndex(record.value)].IsValid());
      }
    }
    ...
    if (record.value.NeedsLoopPhi()) {
      ...
      if (verbose_) {
        LOG(ERROR) << "VMARKO: [x] Adding substitute for " << load->DebugName() << "@"
            << load->GetDexPc() << " -> " << heap_value->DebugName() << "@"
            << heap_value->GetDexPc() << "/" << heap_value->GetId();
      }
