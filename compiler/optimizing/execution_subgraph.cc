...
void ExecutionSubgraph::Prune() {
  ...
  // the edges that were both (1) explored and (2) reached the exit node.
...
void ExecutionSubgraph::RemoveConcavity() {
  ...
  // It's unlikely there will be many unreachable blocks (much smaller than
  // total number of blocks) and iterating through Indexes is slow. Just turn it
  // into a vector so we won't need to do the slow iteration repeatedly.
  ScopedArenaVector<uint32_t> initial(
      idxs.begin(), idxs.end(), allocator_->Adapter(kArenaAllocLSA));
  // Exclude blocks that are between 2 different exclusions. This ensures we
  // won't need to do any sort of complicated predicated-allocate.
  ...
  ArenaBitVector after_skipped_blocks(
      allocator_, graph_->GetBlocks().size(), false, ArenaAllocKind::kArenaAllocLSA);
  for (const HBasicBlock* blk : graph_->GetBlocks()) {
    if (blk == nullptr || unreachable_blocks_.IsBitSet(blk->GetBlockId())) {
      continue;
    }
    for (auto skipped1 : initial) {
      if (UNLIKELY(graph_->PathBetween(skipped1, blk->GetBlockId()))) {
        after_skipped_blocks.SetBit(blk->GetBlockId());
        break;
      }
    }
  }
  for (uint32_t blk_id : after_skipped_blocks.Indexes()) {
    for (uint32_t skipped2 : initial) {
      if (UNLIKELY(graph_->PathBetween(blk_id, skipped2))) {
        RemoveBlock(graph_->GetBlocks()[blk_id]);
        break;
      }
    }
  }
