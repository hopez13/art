...
void ExecutionSubgraph::Prune() {
  ...
  // This is basically a DFS of the graph with some edges skipped.
  {
    ...
    ScopedArenaVector<uint32_t> current_path(temporaries.Adapter(kArenaAllocLSE));
...
void ExecutionSubgraph::RemoveConcavity() {
  ...
  DCHECK(!needs_prune_);
  ArenaBitVector bv(allocator_, graph_->GetBlocks().size(), false, kArenaAllocLSA);
  auto idxs = unreachable_blocks_vec_.Indexes();
  bv.Copy(&unreachable_blocks_vec_);
  // This is likely to be pretty small in general (much smaller than total
  // number of blocks) and iterating through Indexes is slow. Just turn it into
  // a vector so we won't need to do the slow iteration repeatedly.
  ...
  for (const HBasicBlock* blk : graph_->GetBlocks()) {
    if (blk == nullptr || bv.IsBitSet(blk->GetBlockId())) {
      continue;
    }
    for (uint32_t skipped1 : initial) {
