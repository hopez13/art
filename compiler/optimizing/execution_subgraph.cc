...
class SubgraphPruneHelper {
 public:
  using MaybeResult = std::optional<std::bitset<ExecutionSubgraph::kMaxFilterableSuccessors>>;
  SubgraphPruneHelper(ExecutionSubgraph* source,
                      ScopedArenaVector<MaybeResult>& results,
                      ArenaBitVector& visiting)
      : subgraph_(source), results_(results), visiting_(visiting) {}

  bool ReachesEnd(const HBasicBlock* blk) {
    MaybeResult& maybe_result = results_[blk->GetBlockId()];
    if (visiting_.IsBitSet(blk->GetBlockId())) {
      // We are in a loop so the block is live.
      return true;
    } else if (maybe_result) {
      CHECK(maybe_result->any() || subgraph_->unreachable_blocks_vec_.IsBitSet(blk->GetBlockId()));
      return maybe_result->any();
    }
    visiting_.SetBit(blk->GetBlockId());
    // what we currently allow.
    std::bitset<ExecutionSubgraph::kMaxFilterableSuccessors> succ_bitmap =
        subgraph_->GetAllowedSuccessors(blk);
    // The new allowed successors. We use visiting to break loops so we don't
    // need to figure out how many bits to turn on.
    maybe_result = std::bitset<ExecutionSubgraph::kMaxFilterableSuccessors>();
    std::bitset<ExecutionSubgraph::kMaxFilterableSuccessors>& result = maybe_result.value();
    for (auto [succ, i] :
         ZipCount(MakeIterationRange(blk->GetSuccessors().begin(), blk->GetSuccessors().end()))) {
      if (succ_bitmap.test(i) && ReachesEnd(succ)) {
        result.set(i);
      }
    }
    visiting_.ClearBit(blk->GetBlockId());
    bool res = result.any();
    if (!res) {
      // If this is a sink block it will be removed from the successors of all
      // its predecessors and made unreachable.
      CHECK(blk != nullptr);
      subgraph_->unreachable_blocks_vec_.SetBit(blk->GetBlockId());
    }
    return res;
  }
 ...
 private:
  ...
  ScopedArenaVector<MaybeResult>& results_;
  ArenaBitVector& visiting_;

  DISALLOW_COPY_AND_ASSIGN(SubgraphPruneHelper);
};
...
void ExecutionSubgraph::RemoveConcavity() {
  ...
  // predicated-allocate.
  // TODO It might be worth it to remove this at some point.
  for (const HBasicBlock* blk : graph_->GetBlocks()) {
