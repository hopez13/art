...
static void BoundTypeForClassCheck(HInstruction* check) {
  ...
  ArtField* field = handle_cache_.GetObjectClassHandle()->GetInstanceField(0);
  ...
  if (receiver_type.IsExact()) {
    // We statically know the type of the receiver, so we can replace the compare. We do it
    // here instead of instruction simplifier to avoid duplicating the pattern recognizer.
    int value = 0;
    if (receiver_type.IsEqual(class_rti)) {
      value = compare->IsEqual() ? 1 : 0;
    } else {
      value = compare->IsNotEqual() ? 1 : 0;
    }
    compare->ReplaceWith(check->GetBlock()->GetGraph()->GetIntConstant(value));
    return;
  }
