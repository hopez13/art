    ...
    if (dominates) {
      if (bound_type == nullptr) {
        ...
        if (ShouldCreateBoundType(
          ...
          bound_type->SetReferenceTypeInfo(bound_type->GetUpperBound());
        } else {
...
static void BoundTypeForClassCheck(HInstruction* check) {
...

  {
    ...
    if (receiver_type.IsExact()) {
      // We statically know the type of the receiver, so we can replace the compare.
      int value = 0;
