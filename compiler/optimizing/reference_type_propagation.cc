...
class ReferenceTypePropagation::RTPVisitor : public HGraphDelegateVisitor {
 ...
 private:
  mirror::Class* LookupResolvedType(Thread* self, const DexFile& dex_file, dex::TypeIndex type_idx)
      REQUIRES_SHARED(Locks::mutator_lock_);
...
void ReferenceTypePropagation::RTPVisitor::UpdateReferenceTypeInfo(HInstruction* instr,
  ...
  ScopedObjectAccess soa(Thread::Current());
  mirror::Class* klass = LookupResolvedType(soa.Self(), dex_file, type_idx);
...
mirror::Class* ReferenceTypePropagation::RTPVisitor::LookupResolvedType(Thread* self,
                                                                        const DexFile& dex_file,
                                                                        dex::TypeIndex type_idx) {
  mirror::DexCache* dex_cache = FindDexCacheWithHint(self, dex_file, hint_dex_cache_);
  ObjPtr<mirror::Class> klass = dex_cache->GetResolvedType(type_idx);
  if (klass == nullptr) {
    klass = Runtime::Current()->GetClassLinker()->LookupResolvedType(
        dex_file, type_idx, dex_cache, class_loader_.Get());
  }
  return klass.Ptr();
}
