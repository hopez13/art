...
// Note: The `mask` is used for small types and it must contain the bits to keep
// for `GetAndUpdateOp::kSet` and the bits to replace for `GetAndUpdateOp::kAdd`.
...
static void GenerateByteSwapAndExtract(Riscv64Assembler* assembler,
  ...
  bool is_small = (DataType::Size(type) < 4u);
  DataType::Type swap_type = is_small ? DataType::Type::kInt32 : type;
...
static void GenerateVarHandleGetAndUpdate(HInvoke* invoke,
  ...
  if (is_small) {
    ...
    switch (get_and_update_op) {
      ...
      case GetAndUpdateOp::kSet:
        ...
        __ Li(mask, (1 << (data_size * kBitsPerByte)) - 1);
        __ Sllw(mask, mask, shift);
        // The argument does not need to me masked for `GetAndUpdateOp::kAdd`,
  ...
  if (use_cas) {
    // Allocate scratch registers for temps that can be clobbered on retry.
    DCHECK_EQ(available_scratch_registers, 2u);
