...
static void EmitLoadReserved(Riscv64Assembler* assembler,
  ...
  switch (type) {
    ...
    case DataType::Type::kReference:
      __ LrW(old_value, ptr, aqrl);
      // TODO(riscv64): Update assembler to use a single-instruction `ZextW()`
      // macro, either the ADD.UW, or the C.ZEXT.W (16-bit encoding).
      __ AddUw(old_value, old_value, Zero);
      break;
    case DataType::Type::kInt64:
...
static void GenerateCompareAndSet(Riscv64Assembler* assembler,
  ...
  if (mask != kNoXRegister) {
    ...
    __ And(store_result, old_value, mask);
    __ Bne(store_result, expected, cmp_failure);
    to_store = store_result;
...
static void CreateVarHandleCompareAndSetOrExchangeLocations(HInvoke* invoke,
  ...
  if (value_type == DataType::Type::kReference && codegen->EmitReadBarrier()) {
    ...
    bool small = (data_size < 2u);
    bool byte_swap = (value_type != DataType::Type::kReference && data_size != 1u);
    bool fp = DataType::IsFloatingPointType(value_type);
    size_t temps_needed =
        // The offset temp is used for the `tmp_ptr`.
        1u +
        // For small values, we need a temp for the `mask` and maybe also for the `shift`.
        (small ? (return_success ? 1u : 2u) : 0u) +
    ...
    size_t scratch_registers_abailable = 2u;
    DCHECK_EQ(scratch_registers_abailable,
    ...
    if (temps_needed > old_temp_count + scratch_registers_abailable) {
      locations->AddRegisterTemps(temps_needed - (old_temp_count + scratch_registers_abailable));
    }
  }
}
...
static XRegister PrepareXRegister(CodeGeneratorRISCV64* codegen,
  ...
  if (shift != kNoXRegister) {
    Riscv64Assembler* assembler = codegen->GetAssembler();
    __ Slliw(result.AsRegister<XRegister>(), loc.AsRegister<XRegister>(), shift);
...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke,
  ...
  // Scratch may be needed for `new_value` and `expected`.
  ScratchRegisterScope srs(assembler);
  ...
  if (value_type == DataType::Type::kReference && codegen->EmitReadBarrier()) {
    ...
    DCHECK_EQ(expected_index, 1u + GetExpectedVarHandleCoordinatesCount(invoke));
    next_temp = expected_index == 1u ? 2u : 1u;  // Cannot use the class register for static field.
  ...
  auto get_temp = [&]() {
    if (available_scratch_registers != 0u) {
      ...
      CHECK_LT(next_temp, locations->GetTempCount());
  ...
  size_t data_size = DataType::Size(value_type);
  if (data_size < 4u) {
    DCHECK_EQ(tmp_ptr, locations->GetTemp(0u).AsRegister<XRegister>());
    ...
    __ Slli(shift, shift, data_size * kBitsPerByte);
    __ Li(mask, (1 << (data_size * kBitsPerByte)) - 1);
    __ Sll(mask, mask, shift);
  }
  ...
  if (value_type == DataType::Type::kReference && codegen->EmitReadBarrier()) {
    // The `old_value_temp` is used first for the unmarked `old_value` and then for the unmarked
  ...
  } else if (value_type == DataType::Type::kUint8) {
    // Note: If we didn't clobber the masked value in `GenerateCompareAndSet()`, we could
    // shift that masked value and we would not need to zero-extend the old value here.
    __ ZextB(old_value, old_value);
  } else if (value_type == DataType::Type::kUint16) {
    // Note: If we didn't clobber the masked value in `GenerateCompareAndSet()`, we could
    // shift that masked value and we would not need to zero-extend the old value here.
