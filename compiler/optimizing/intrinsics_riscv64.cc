...
void IntrinsicCodeGeneratorRISCV64::VisitStringGetCharsNoCheck(HInvoke* invoke) {
  ...
  // For RISCV64 there are 2 scratch core registers,
  // yet it is onerously to use the both ones here,
  // because the assertion will work in Riscv64Assembler::AdjustBaseAndOffset,
  // inasmuch as a scratch register must be available there for adjustment
  ...
  //    (add destination_begin_offset * 2 that equals to destination_begin_offset << 1);
  __ Slli(tmp, destination_begin_offset, 1u);
  // 3. add the value of the scaled offset (destination_begin_offset) to the destination register.
  __ Add(destination_ptr, destination_ptr, tmp);
  ...
  __ Slli(tmp, source_begin_index, 1u);
  __ Add(source_ptr, source_ptr, tmp);
  ...
  // ! Unaligned addresses are acceptable here and it is not required to embed additional
  ...
  if (mirror::kUseStringCompression) {
    ...
    const size_t compressed_char_size = DataType::Size(DataType::Type::kInt8);
    DCHECK_EQ(compressed_char_size, 1u);
