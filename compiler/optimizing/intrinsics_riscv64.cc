 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen_in) override {
    ...
    int32_t entry_point_offset =
        Thread::ReadBarrierMarkEntryPointsOffset<kRiscv64PointerSize>(tmp_reg);
...
static void GenSystemArrayCopyAddresses(Riscv64Assembler* assembler,
  ...
  if (dst_pos.IsConstant()) {
    ...
    // __ Li(dst_base, dst, element_size * constant + data_offset);
...
void IntrinsicCodeGeneratorRISCV64::VisitSystemArrayCopy(HInvoke* invoke) {
  ...
  if (!length.IsConstant() && !optimizations.GetCountIsSourceLength() &&
