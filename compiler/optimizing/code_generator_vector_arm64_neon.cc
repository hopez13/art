...
void LocationsBuilderARM64::NeonVisitVecCondition(HVecCondition* instruction) {
  LOG(FATAL) << "No SIMD for " << instruction->GetId();
  CHECK(false);
}

void InstructionCodeGeneratorARM64::NeonVisitVecCondition(HVecCondition* instruction) {
  LOG(FATAL) << "No SIMD for " << instruction->GetId();
  CHECK(false);
}
...
Location InstructionCodeGeneratorARM64::NeonAllocateSIMDScratchLocation(
  ...
  DCHECK_EQ(codegen_->GetTraditionalSIMDRegisterWidth(), kQRegSizeInBytes);
...
void SaveRestoreLiveRegistersHelperNeonImpl(CodeGeneratorARM64* codegen,
  ...
  DCHECK_LE(codegen->GetTraditionalSIMDRegisterWidth(), kQRegSizeInBytes);
