...
bool UnsignedUseAnalyzer::IsNonNegativeUse(HInstruction* target_user, HInstruction* value) {
  ...
  for (const HUseListNode<HInstruction*>& use : value->GetUses()) {
    ...
    // JFYI: you might ask why not to use HTypeConversion to bind such information. A short answer
    // is that it is been tried. Such a solution increases the complexity of optimizations.
    // HTypeConversioncan can create a dependency which does not exist in the input program,
    // for example: between two uses, 1st - cmp, 2nd - target_user.
    if (user->IsCondition()) {
    ...
    // TODO The value is non-negative if it is initialize by a positive number and every its
    //      modification keeps the value non-negative, for example the division operation.
  }
