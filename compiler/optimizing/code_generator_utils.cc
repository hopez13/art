...
bool UnsignedUseAnalyzer::IsNonNegativeUse(HInstruction* target_user, HInstruction* value) {
  ...
  for (const HUseListNode<HInstruction*>& use : value->GetUses()) {
    ...
    // JFYI: We're not using HTypeConversation to bind the new information because it would
    // increase the complexity of optimziations: HTypeConversion can create a dependency
