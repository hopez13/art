...
// Remove duplicate live interval pointers, in a manner that maintains determinism.
// (Sorting and using std::unique, for example, would *not* maintain determinism.)
static void RemoveDuplicateIntervals(ArenaVector<LiveInterval*>* intervals,
                                     ArenaAllocator* allocator) {
  ArenaSet<LiveInterval*> seen(allocator->Adapter(kArenaAllocRegisterAllocator));
  auto it = std::remove_if(intervals->begin(), intervals->end(),
      [&seen] (LiveInterval* p) {
        bool already_seen = seen.find(p) != seen.end();
        if (already_seen) {
          return true;
        } else {
          seen.insert(p);
...
void RegisterAllocatorGraphColor::ColorSpillSlots(ArenaVector<LiveInterval*>* intervals,
  ...
  RemoveDuplicateIntervals(intervals, allocator_);
