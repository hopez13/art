...
void RegisterAllocatorGraphColor::AllocateSpillSlotForCatchPhi(HInstruction* instruction) {
  if (instruction->IsPhi() && instruction->AsPhi()->IsCatchPhi()) {
    ...
    if (phi->IsVRegEquivalentOf(previous_phi)) {
      ...
      catch_phi_spill_slot_counter_ += interval->NumberSpillSlotsNeeded();
    }
  }
}
...
void RegisterAllocatorGraphColor::ColorSpillSlots(ArenaVector<LiveInterval*>* intervals,
  ...
  for (auto it = interval_endpoints.begin(), end = interval_endpoints.end(); it != end; ++it) {
    ...
    size_t number_slots_needed = parent_interval->NumberSpillSlotsNeeded();
    ...
    if (is_interval_beginning) {
      ...
      for (; true; ++slot) {
        bool found = true;
        for (size_t s = slot, u = slot + number_slots_needed; s < u; s++) {
          if (taken.IsBitSet(s)) {
            found = false;
            break;  // failure
          }
      ...
      *num_stack_slots_used = std::max(*num_stack_slots_used, slot + number_slots_needed);
      if (number_slots_needed > 1 && *num_stack_slots_used % 2 != 0) {
