...
class InterferenceNode : public DeletableArenaObject<kArenaAllocRegisterAllocator> {
 public:
...
class ColoringIteration {
 public:
  ...
  void BuildInterferenceGraph(
      ...
      const ScopedArenaVector<std::unique_ptr<InterferenceNode>>& physical_nodes);
 ...
 private:
  ...
  ScopedArenaHashMap<LiveInterval*, std::unique_ptr<InterferenceNode>> interval_node_map_;
...
RegisterAllocatorGraphColor::RegisterAllocatorGraphColor(ScopedArenaAllocator* allocator,
  ...
  size_t number_of_core_registers = codegen_->GetNumberOfCoreRegisters();
  physical_core_nodes_.reserve(number_of_core_registers);
  for (size_t i = 0; i != number_of_core_registers; ++i) {
    LiveInterval* interval =
        LiveInterval::MakeFixedInterval(allocator_, i, DataType::Type::kInt32);
    physical_core_nodes_.emplace_back(new (allocator_) InterferenceNode(interval, liveness));
    physical_core_nodes_.back()->stage = NodeStage::kPrecolored;
  ...
  size_t number_of_fp_registers = codegen_->GetNumberOfFloatingPointRegisters();
  physical_fp_nodes_.reserve(number_of_fp_registers);
  for (size_t i = 0; i != number_of_fp_registers; ++i) {
    LiveInterval* interval =
        LiveInterval::MakeFixedInterval(allocator_, i, DataType::Type::kFloat32);
    physical_fp_nodes_.emplace_back(new (allocator_) InterferenceNode(interval, liveness));
    physical_fp_nodes_.back()->stage = NodeStage::kPrecolored;
...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  ...
  for (bool processing_core_regs : {true, false}) {
    ...
    while (true) {
      ...
      ScopedArenaAllocator coloring_attempt_allocator(codegen_->GetGraph()->GetArenaStack());
      ...
      ScopedArenaVector<std::unique_ptr<InterferenceNode>>& physical_nodes = processing_core_regs
      ...
      for (const std::unique_ptr<InterferenceNode>& node : physical_core_nodes_) {
        node->ClearCoalesceOpportunities();
      }
      for (const std::unique_ptr<InterferenceNode>& node : physical_fp_nodes_) {
        node->ClearCoalesceOpportunities();
      }
...
bool RegisterAllocatorGraphColor::Validate(bool log_fatal_on_failure) {
  for (bool processing_core_regs : {true, false}) {
    ScopedArenaAllocator allocator(codegen_->GetGraph()->GetArenaStack());
    ...
    ScopedArenaVector<std::unique_ptr<InterferenceNode>>& physical_nodes = processing_core_regs
        ? physical_core_nodes_
        : physical_fp_nodes_;
    for (const std::unique_ptr<InterferenceNode>& fixed : physical_nodes) {
...
void ColoringIteration::AddPotentialInterference(InterferenceNode* from,
  ...
  if (from->IsPrecolored()) {
    ...
    InterferenceNode* physical_node = physical_nodes[to->GetInterval()->GetRegister()].get();
    ...
    if (to->IsPair()) {
      InterferenceNode* high_node =
          physical_nodes[to->GetInterval()->GetHighInterval()->GetRegister()].get();
...
void ColoringIteration::BuildInterferenceGraph(
  ...
  for (LiveInterval* parent : intervals) {
    for (LiveInterval* sibling = parent; sibling != nullptr; sibling = sibling->GetNextSibling()) {
      ...
      if (range != nullptr) {
        std::unique_ptr<InterferenceNode> node_ptr(
            new (allocator_) InterferenceNode(sibling, register_allocator_->liveness_));
        InterferenceNode* node = node_ptr.get();
        interval_node_map_.Insert(std::make_pair(sibling, std::move(node_ptr)));
        ...
        if (sibling->HasRegister()) {
          ...
          InterferenceNode* physical_node = physical_nodes[sibling->GetRegister()].get();
  ...
  ScopedArenaVector<InterferenceNode*> live(
      allocator_->Adapter(kArenaAllocRegisterAllocator));
...
void ColoringIteration::CreateCoalesceOpportunity(InterferenceNode* a,
  ...
  CoalesceOpportunity* opportunity =
      new (allocator_) CoalesceOpportunity(
          a, b, kind, position, register_allocator_->liveness_);
  ...
  for (InterferenceNode* node : prunable_nodes_) {
    ...
    if (next_sibling != nullptr && interval->GetEnd() == next_sibling->GetStart()) {
      ...
      if (it != interval_node_map_.end()) {
        InterferenceNode* sibling_node = it->second.get();
    ...
    if (parent->HasRegister()
      ...
      if (it != interval_node_map_.end()) {
        InterferenceNode* parent_node = it->second.get();
        CreateCoalesceOpportunity(node,
                                  parent_node,
    ...
    if (interval->IsSplit() && liveness.IsAtBlockBoundary(interval->GetStart() / 2)) {
      ...
      for (HBasicBlock* predecessor : block->GetPredecessors()) {
        ...
        if (existing != nullptr) {
          ...
          if (it != interval_node_map_.end()) {
            InterferenceNode* existing_node = it->second.get();
            CreateCoalesceOpportunity(node,
                                      existing_node,
    ...
    if (defined_by != nullptr && defined_by->IsPhi()) {
      ...
      for (size_t i = 0, e = inputs.size(); i < e; ++i) {
        ...
        if (it != interval_node_map_.end()) {
          InterferenceNode* input_node = it->second.get();
    ...
    for (const UsePosition& use : matching_use_range) {
      ...
      if (input.IsRegister() || input.IsFpuRegister()) {
        ...
        InterferenceNode* fixed_node = input.IsRegister()
            ? register_allocator_->physical_core_nodes_[input.reg()].get()
            : register_allocator_->physical_fp_nodes_[input.reg()].get();
        CreateCoalesceOpportunity(node,
                                  fixed_node,
