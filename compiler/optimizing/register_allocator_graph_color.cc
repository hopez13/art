...
void RegisterAllocatorGraphColor::ColorSpillSlots(const ArenaVector<LiveInterval*>& intervals,
  ...
  for (LiveInterval* parent_interval : intervals) {
    // TODO: Could consider lifetime holes, but that makes this algorithm more complex.
    DCHECK(parent_interval->IsParent());
    DCHECK(!parent_interval->HasSpillSlot());
    size_t start = parent_interval->GetStart();
    // TODO: Only need to go to the last sibling that needs a stack slot.
  ...
  // (Duplicates can appear when multiple siblings of the same interval request a stack slot.)
  // TODO: This is not quite deterministic. Might need to remove duplicates ahead of time,
  //       and then explicitly ignore the interval pointers in the sort.
  std::sort(interval_endpoints.begin(), interval_endpoints.end());
  auto unique_it = std::unique(interval_endpoints.begin(), interval_endpoints.end());
  interval_endpoints.erase(unique_it, interval_endpoints.end());
  ...
  for (auto it = interval_endpoints.begin(), end = interval_endpoints.end(); it != end; ++it) {
    ...
    size_t position;  // TODO: Verify on non-debug build, since this may not be used.
