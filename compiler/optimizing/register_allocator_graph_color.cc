...
static constexpr size_t kDominatesExitBlockWeightMultiplier = 3;

enum class CoalesceKind {
  kAdjacentSibling,       // Prevents moves at interval split points.
  kFixedOutputSibling,    // Prevents moves, but adds interferences with a fixed node.
  kFixedInput,            // Prevents moves, but adds interferences with a fixed node.
...
static size_t ComputeCoalescePriority(CoalesceKind kind,
  ...
  switch (kind) {
    case CoalesceKind::kAdjacentSibling:
      return CostForMoveAt(position, liveness);
    case CoalesceKind::kFixedOutputSibling:
      // This will reduce a move, but can make the interference graph harder to color,
      // so we reduce priority a bit.
      return CostForMoveAt(position, liveness) / 2;
    case CoalesceKind::kFixedInput:
...
struct CoalesceOpportunity : public ArenaObject<kArenaAllocRegisterAllocator> {
  CoalesceOpportunity(InterferenceNode* _a,
                      InterferenceNode* _b,
                      CoalesceKind kind,
                      size_t position,
                      const SsaLivenessAnalysis& liveness)
...
class InterferenceNode : public ArenaObject<kArenaAllocRegisterAllocator> {
 public:
  ...
  void AddInterference(InterferenceNode* other, bool guaranteed_not_interfering_yet) {
    ...
    DCHECK(stage != NodeStage::kPruned && other->stage != NodeStage::kPruned);
  ...
  void RemoveInterference(InterferenceNode* other) {
    ...
    DCHECK(other->stage == NodeStage::kPruned) << "Should only remove interferences when pruning";
    auto it = std::find(adjacent_nodes_.begin(), adjacent_nodes_.end(), other);
    if (it != adjacent_nodes_.end()) {
  ...
  //       (Stale = pointers to deallocated memory.) Perhaps there is a better way to represent
  //       fixed nodes.
  ...
  void SetAlias(InterferenceNode* rep) {
    DCHECK(rep->stage != NodeStage::kPruned);
...
RegisterAllocatorGraphColor::RegisterAllocatorGraphColor(ArenaAllocator* allocator,
      ...
      : RegisterAllocator(allocator, codegen, liveness),
        ...
        spill_worklist_(GreaterNodePriority, allocator->Adapter(kArenaAllocRegisterAllocator)),
  ...
  for (bool processing_core_regs : {true, false}) {
    ...
    while (true) {
      ...
      freeze_worklist_ = ArenaDeque<InterferenceNode*>(
          coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator));
      spill_worklist_ = ArenaPriorityQueue<InterferenceNode*, decltype(&GreaterNodePriority)>(
...
void RegisterAllocatorGraphColor::AddPotentialInterference(InterferenceNode* from,
  ...
  if (from->IsPrecolored()) {
    ...
    from->AddInterference(physical_node, guaranteed_not_interfering_yet);
  ...
  if (both_directions) {
    AddPotentialInterference(to, from, guaranteed_not_interfering_yet, false);
  }
}
...
void RegisterAllocatorGraphColor::BuildInterferenceGraph(
  ...
  ArenaVector<std::tuple<size_t, bool, InterferenceNode*>> range_endpoints(
      coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator));
  range_endpoints.reserve(4 * prunable_nodes_.size());  // TODO: Good estimate?
...
// TODO: Currently, we ignore intervals not in the interval node map (corresponding
//       usually to the parent intervals of constants and stack arguments). Verify that
//       those intervals need not be involved in coalescing.
void RegisterAllocatorGraphColor::FindCoalesceOpportunities() {
  ...
  for (InterferenceNode* node : prunable_nodes_) {
    ...
    // TODO: The following blocks are partially copied from liveness analysis.
    //       Can we share this code?

    // Try to prevent moves across blocks.
    // TODO: This does not lead to many succeeding coalesce attempts. Could remove.
      ...
      for (HBasicBlock* predecessor : block->GetPredecessors()) {
        ...
        if (existing != nullptr) {
          ...
          if (it != interval_node_map_.end()) {
            ...
            // TODO: The move position actually depends on the CFG structure;
            //       see the resolution code.
            CreateCoalesceOpportunity(node,
                                      existing_node,
    ...
    if (defined_by != nullptr) {
      ...
      if (out.IsUnallocated() && out.GetPolicy() == Location::kSameAsFirstInput) {
        ...
        // TODO: I believe we could consider lifetime holes here (and in linear scan).
    ...
    for (; use != nullptr && use->GetPosition() < interval->GetEnd(); use = use->GetNext()) {
      ...
      if (user == nullptr) {
        // TODO: It's not clear why this check is necessary, but sometimes user is null.
        continue;
      }
      LocationSummary* locations = user->GetLocations();
      if (use->GetInputIndex() >= locations->GetInputCount()) {
        // TODO: It's not clear why this check is necessary.
        continue;
      }
      Location input = locations->InAt(use->GetInputIndex());
      // TODO: Handle pair intervals too?
      if (input.IsRegister()) {
        InterferenceNode* fixed_node = physical_core_nodes_[input.reg()];
        CreateCoalesceOpportunity(node,
                                  fixed_node,
                                  CoalesceKind::kFixedInput,
                                  user->GetLifetimePosition());
      } else if (input.IsFpuRegister()) {
        InterferenceNode* fixed_node = physical_fp_nodes_[input.reg()];
        CreateCoalesceOpportunity(node,
                                  fixed_node,
                                  CoalesceKind::kFixedInput,
                                  user->GetLifetimePosition());
...
bool RegisterAllocatorGraphColor::GreaterNodePriority(const InterferenceNode* lhs,
                                                      const InterferenceNode* rhs) {
  // (1) Choose the node that requires a color.
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  while (true) {
    if (!simplify_worklist_.empty()) {
      ...
      DCHECK(node->stage == NodeStage::kSimplifyWorklist)
          << "Cannot transition away from simplify worklist";
      DCHECK_LT(node->GetOutDegree(), num_regs)
          << "Nodes in simplify worklist should be low degree";
      DCHECK(!node->IsMoveRelated())
          << "Nodes in the simplify worklist should not be move related";
      ...
      if (node->stage == NodeStage::kFreezeWorklist) {
        DCHECK_LT(node->GetOutDegree(), num_regs)
            << "Nodes in the freeze worklist should be low degree";
        DCHECK(node->IsMoveRelated())
            << "Nodes in the freeze worklist should be move related";
        FreezeMoves(node);
        PruneNode(node, num_regs);
      }
      ...
      if (node->stage == NodeStage::kSpillWorklist) {
        DCHECK_GE(node->GetOutDegree(), num_regs)
            << "Nodes in the spill worklist should be high degree";
        FreezeMoves(node);
        PruneNode(node, num_regs);
      }
    } else {
...
void RegisterAllocatorGraphColor::PruneNode(InterferenceNode* node,
                                            size_t num_regs) {
  DCHECK(node->stage != NodeStage::kPruned);
  ...
  for (InterferenceNode* adj : node->GetAdjacentNodes()) {
    ...
    DCHECK(adj->stage != NodeStage::kPruned) << "Should be no interferences with pruned nodes";

    if (adj->IsPrecolored()) {
      // No effect on pre-colored nodes; they're never pruned.
    } else {
      // Remove the interference.
      bool was_high_degree = adj->GetOutDegree() >= num_regs;
...
void RegisterAllocatorGraphColor::FreezeMoves(InterferenceNode* node) {
  for (CoalesceOpportunity* opportunity : node->GetCoalesceOpportunities()) {
    ...
    InterferenceNode* other = opportunity->a->GetAlias() == node
        ? opportunity->b->GetAlias()
        : opportunity->a->GetAlias();
    if (other != node && other->stage == NodeStage::kFreezeWorklist && !other->IsMoveRelated()) {
      other->stage = NodeStage::kSimplifyWorklist;
...
void RegisterAllocatorGraphColor::CheckTransitionFromFreezeWorklist(InterferenceNode* node,
                                                                    size_t num_regs) {
  if (node->GetOutDegree() < num_regs && !node->IsMoveRelated()) {
    DCHECK(node->stage == NodeStage::kFreezeWorklist);
    node->stage = NodeStage::kSimplifyWorklist;
    simplify_worklist_.push_back(node);
  }
}
...
bool RegisterAllocatorGraphColor::PrecoloredHeuristic(InterferenceNode* from,
  ...
  if (from->IsPair() || into->IsPair()) {
    ...
    //       to fix this situation, but that may have issues as well.
    return false;
  }
  ...
  for (InterferenceNode* adj : from->GetAdjacentNodes()) {
    if (adj->GetOutDegree() < num_regs || adj->IsPrecolored() || adj->ContainsInterference(into)) {
      // Ok.
    } else {
      return false;
    }
...
void RegisterAllocatorGraphColor::Combine(InterferenceNode* from,
  ...
  // Add interferences.
  for (InterferenceNode* adj : from->GetAdjacentNodes()) {
    bool was_low_degree = adj->GetOutDegree() < num_regs;
    AddPotentialInterference(adj, into, /*guaranteed_not_interfering_yet*/ false);
    if (was_low_degree && adj->GetOutDegree() >= num_regs) {
...
void RegisterAllocatorGraphColor::Coalesce(CoalesceOpportunity* opportunity,
                                           size_t num_regs) {
  InterferenceNode* from = opportunity->a->GetAlias();
  InterferenceNode* into = opportunity->b->GetAlias();
  DCHECK(from->stage != NodeStage::kPruned && into->stage != NodeStage::kPruned);
...
// TODO: Could use this to help in coalesce heuristics too.
  ...
  while (!pruned_nodes_.empty()) {
    ...
    if (alias != node) {
      ...
      for (CoalesceOpportunity* opportunity : node->GetCoalesceOpportunities()) {
        ...
        LiveInterval* other_interval = opportunity->a->GetAlias() == node
            ? opportunity->b->GetAlias()->GetInterval()
            : opportunity->a->GetAlias()->GetInterval();
