...
class ArchDefaultLoopHelper : public ArchNoOptsLoopHelper {
 public:
  ...
  // Maximum number of basic blocks for partial loop unrolling with unknown iterations.
  static constexpr uint32_t kUnknownIterMaxBodySizeBlocks = 2;
  ...
  // Identify whether partial loop unrolling for uncounted loop will be feasible and beneficial.
  virtual uint32_t IsUnrollingUnknownIterBeneficial(const LoopAnalysisInfo* analysis_info) const {
    ...
    if (num_blocks != kUnknownIterMaxBodySizeBlocks)
      return 0;
    ...
    for (HInstructionIterator it(header->GetPhis()); !it.Done(); it.Advance()) {
      ...
      // No need to check if PHI has less inputs.
      ...
      for (size_t i = 0; i < phi_inputs.size(); ++i) {
        ...
        // This PHI has other PHI as I/P which is defined in loop header.
        // Due to this, resolving references in partial unrolled loop will be difficult.
        if (phi_input->IsPhi() && (phi_input->GetBlock() == header)) {
          return 0;
        }

        // Checking whether back edge index input contains current PHI as input or not.
...
class X86_64LoopHelper : public ArchDefaultLoopHelper {
  ...
  // According to Intel® 64 and 32 Architectures Optimization Reference Manual, avoid
  // excessive loop unrolling to ensure LSD (loop stream decoder) is operating efficiently.
