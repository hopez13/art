...
class HeapLocationCollector : public HGraphVisitor {
 public:
  ...
  size_t GetArrayHeapLocation(HInstruction* array,
    ...
    DCHECK(vector_length >= HeapLocation::kScalar);
 ...
 private:
  ...
  bool ComputeMayAlias(size_t index1, size_t index2) const {
    ...
    if (loc1->IsArray() && loc2->IsArray()) {
      ...
      size_t vlength1 = loc1->GetVectorLength();
      size_t vlength2 = loc2->GetVectorLength();
      if (!CanArrayElementsAlias(idx1, vlength1, idx2, vlength2)) {
        return false;
      }
