...
class ExecutionSubgraph : public ArenaObject<kArenaAllocLSA> {
 public:
  ...
  struct ExcludedCohort {
   public:
    ...
    bool SucceedsBlock(const HBasicBlock* blk) const {
      ...
      return std::any_of(GetEntryBlocks().cbegin(), GetEntryBlocks().cend(), [&](auto entry) -> bool {
  ...
  ArraySlice<const ExcludedCohort> GetExcludedCohorts() const {
    DCHECK(!valid_ || !needs_prune_);
    if (!valid_ || unreachable_blocks_.empty()) {
      return ArraySlice<const ExcludedCohort>();
    } else {
      return ArraySlice<const ExcludedCohort>(excluded_list_->data(), excluded_list_->size());
    }
  }
 ...
 private:
  ...
  void RemoveConcavity() {
    ...
    DCHECK(!needs_prune_);
    std::unordered_set<const HBasicBlock*> initial_unreachable(GetUnreachableBlocks());
  ...
  void RecalculateExcludedCohort() {
    DCHECK(!needs_prune_);
    excluded_list_.emplace();
    std::vector<ExcludedCohort>& res = excluded_list_.value();
  ...
  mutable std::optional<std::vector<ExcludedCohort>> excluded_list_;
  bool valid_;
  bool needs_prune_;
};
...
class ReferenceInfo : public ArenaObject<kArenaAllocLSA> {
 public:
  ReferenceInfo(HInstruction* reference, size_t pos, bool for_elimination)
  ...
  void PrunePartialEscapeWrites() {
    ...
    for (const HUseListNode<HInstruction*>& use : reference_->GetUses()) {
      ...
      if (additional_exclusions.find(user->GetBlock()) == additional_exclusions.end() &&
          subgraph_.GetUnreachableBlocks().find(user->GetBlock()) == subgraph_.GetUnreachableBlocks().end() &&
          ...
          //  user->IsArraySet() ||
          //  (user->IsArrayGet() && !user->InputAt(1)->IsConstant())) &&
...
class HeapLocationCollector : public HGraphVisitor {
 public:
  ...
  bool InstructionEligableForLSERemoval(HInstruction* inst) {
  ...
  void DumpReferenceStats(OptimizingCompilerStats* stats) {
    ...
    for (auto hl : heap_locations_) {
      ...
      if (ri->IsSingletonAndRemovable()) {
        if (InstructionEligableForLSERemoval(instruction)) {
      ...
      if (ri->IsPartialSingleton() && instruction->IsNewInstance() &&
          ...
          InstructionEligableForLSERemoval(instruction)) {
 ...
 private:
  ...
  ReferenceInfo* GetOrCreateReferenceInfo(HInstruction* instruction) {
    ...
    if (ref_info == nullptr) {
      ...
      ref_info = new (allocator_) ReferenceInfo(instruction, pos, for_elimination_);
...
class LoadStoreAnalysis {
 public:
  explicit LoadStoreAnalysis(HGraph* graph, OptimizingCompilerStats* stats, ScopedArenaAllocator* local_allocator, bool for_elimination = true)
    : graph_(graph),
      stats_(stats),
