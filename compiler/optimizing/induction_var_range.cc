...
bool InductionVarRange::IsUnitStride(HInstruction* instruction, /*out*/ HInstruction** offset) const {
  HLoopInformation* loop = nullptr;
  HInductionVarAnalysis::InductionInfo* info = nullptr;
  HInductionVarAnalysis::InductionInfo* trip = nullptr;
...
HInstruction* InductionVarRange::GenerateTripCount(HLoopInformation* loop, HGraph* graph, HBasicBlock* block) {
  HInductionVarAnalysis::InductionInfo *trip =
      induction_analysis_->LookupInfo(loop, GetLoopControl(loop));
  if (trip != nullptr && !IsUnsafeTripCount(trip)) {
    HInstruction* taken_test = nullptr;
    ...
    if (needs_taken_test) {
      success = GenerateCode(trip->op_b, nullptr, graph, block, &taken_test, false, false);
    }
    success = success && GenerateCode(trip->op_a, nullptr, graph, block, &trip_expr, false, false);
    if (success && needs_taken_test) {
  ...
  if (info != nullptr) {
    ...
    switch (info->induction_class) {
      case HInductionVarAnalysis::kInvariant:
        ...
        // sub expressions, viz. no induction, there is no need to keep the
        // adjust the is_min parameter).
        switch (info->operation) {
          case HInductionVarAnalysis::kAdd:
