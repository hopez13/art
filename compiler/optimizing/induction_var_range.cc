                ...
                switch (info->operation) {
                  case HInductionVarAnalysis::kAdd:
                    return TryGenerateAddWithoutOverflow(
                        context, loop, info, graph, block, opa, opb, result);
                  case HInductionVarAnalysis::kSub:
                    return TryGenerateSubWithoutOverflow(
                        context, loop, info, graph, block, opa, result);
                  default:
...
void InductionVarRange::MaybeInsertInstruction(HGraph* graph,
                                               HBasicBlock* block,
                                               /*in*/ HInstruction* instruction,
                                               /*out*/ HInstruction** result) {
  DCHECK(graph != nullptr);
  if (instruction->GetId() != -1) {
    // `instruction` is already in the graph.
    *result = instruction;
  } else {
    *result = Insert(block, instruction);
  }
}
...
bool InductionVarRange::TryGenerateAddWithoutOverflow(const HBasicBlock* context,
  ...
  if (a_is_const && b_is_const) {
    ...
    if (add_val.is_known) {
      ...
      if (graph != nullptr) {
        HInstruction* instruction = graph->GetConstant(info->type, add_val.b_constant);
        MaybeInsertInstruction(graph, block, instruction, result);
      }
      return true;
    }
  }
  ...
  if (a_is_const && val_a == 0) {
    if (graph != nullptr) {
      MaybeInsertInstruction(graph, block, opb, result);
    }
    return true;
  }
  ...
  if (b_is_const && val_b == 0) {
    if (graph != nullptr) {
      MaybeInsertInstruction(graph, block, opa, result);
    }
    return true;
  }
...
bool InductionVarRange::TryGenerateSubWithoutOverflow(const HBasicBlock* context,
  ...
  if (IsConstant(context, loop, info->op_a, kExact, &val_a)) {
    ...
    if (sub_val.is_known) {
      ...
      // Known value not overflowing.
      if (graph != nullptr) {
        HInstruction* instruction = graph->GetConstant(info->type, sub_val.b_constant);
        MaybeInsertInstruction(graph, block, instruction, result);
      }
      return true;
    }
