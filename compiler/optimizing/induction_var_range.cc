...
bool InductionVarRange::GenerateLastValuePeriodic(HInductionVarAnalysis::InductionInfo* info,
  ...
  if (IsConstant(info->op_a, kExact, &x) &&
      IsConstant(info->op_b, kExact, &y) &&
    ...
    if (graph != nullptr) {
      ...
      HInstruction* is_even = Insert(block, new (graph->GetArena()) HEqual(
          Insert(block, new (graph->GetArena())
                 HAnd(info->type, trip_expr, graph->GetIntConstant(1))),
    ...
    if (*needs_taken_test) {
      ...
      if (!GenerateCode(
        ...
        return false;
         ...
         HInstruction* x_instr = graph->GetIntConstant(x);
        *result = Insert(block, new (graph->GetArena())
                         HSelect(taken_test, *result, x_instr, kNoDexPc));
      }
