...
HInstruction* InductionVarRange::GenTrip(HLoopInformation* loop, HGraph* graph, HBasicBlock* block) {
  HInductionVarAnalysis::InductionInfo *trip =
      induction_analysis_->LookupInfo(loop, GetLoopControl(loop));
  if (trip != nullptr && !IsUnsafeTripCount(trip)) {
    HInstruction* taken_test = nullptr;
    ...
    if (needs_taken_test) {
      CHECK(GenerateCode(trip->op_b, nullptr, graph, block, &taken_test, false, false));
    }
    CHECK(GenerateCode(trip->op_a, nullptr, graph, block, &trip_expr, false, false));
    if (needs_taken_test) {
