...
/** Generates an integral constant, if not already in the graph. */
static HInstruction* GetInt(HGraph* graph, Primitive::Type type, int64_t value) {
  if (type == Primitive::kPrimLong) {
    return graph->GetLongConstant(value);
  } else {
    DCHECK_EQ(type, Primitive::kPrimInt);
    return graph->GetIntConstant(value);
  }
}
  ...
  if (IsConstant(info->op_a->op_a, kExact, &a) &&
    ...
    if (GenerateCode(info->op_b, nullptr, graph, block, graph ? &c : nullptr, false, false)) {
      if (graph != nullptr) {
        ...
        *result = Insert(block, new (graph->GetArena()) HAdd(type, GetInt(graph, type, sum), c));
      }
      return true;
    }
  }
    ...
    if (GenerateCode(info->op_a, nullptr, graph, block, &opa, false, false) &&
      ...
      if (graph != nullptr) {
        ...
        if (fpow == 0) {
          ...
          *result = GetInt(graph, type, 0);
        } else {
          // Last value: a * f ^ m + b or a * f ^ -m + b.
          HInstruction* e = nullptr;
          if (info->operation == HInductionVarAnalysis::kMul) {
            e = new (graph->GetArena()) HMul(type, opa, GetInt(graph, type, fpow));
          } else {
            e = new (graph->GetArena()) HDiv(type, opa, GetInt(graph, type, fpow), kNoDexPc);
          }
...
bool InductionVarRange::GenerateLastValuePeriodic(HInductionVarAnalysis::InductionInfo* info,
  ...
  if (period == 2 &&
    ...
    if (graph != nullptr) {
      ...
      HInstruction* msk =
          Insert(block, new (graph->GetArena()) HAnd(type, t, GetInt(graph, type, 1)));
      HInstruction* is_even =
          Insert(block, new (graph->GetArena()) HEqual(msk, GetInt(graph, type, 0), kNoDexPc));
    ...
    switch (info->induction_class) {
            ...
            if (is_min) {
              ...
              if (GenerateCode(info->op_a, trip, graph, block, &opb, in_body, is_min)) {
                if (graph != nullptr) {
                  *result = Insert(block,
                                   new (graph->GetArena())
                                       HSub(type, opb, GetInt(graph, type, 1)));
                }
                return true;
              }
            }
            ...
            if (GenerateCode(trip,       trip, graph, block, &opa, in_body, is_min_a) &&
              ...
              if (graph != nullptr) {
                ...
                if (stride_value == 1) {
                  ...
                  HInstruction* mul =
                      new (graph->GetArena()) HMul(type, GetInt(graph, type, stride_value), opa);
      ...
      case HInductionVarAnalysis::kPeriodic: {
        ...
        if (IsConstantValue(extreme)) {
          if (graph != nullptr) {
            *result = GetInt(graph, type, extreme.b_constant);
          }
          return true;
        }
        break;
