...
InductionVarRange::Value InductionVarRange::GetGeometric(HInductionVarAnalysis::InductionInfo* info,
  ...
  int64_t a = 0;
  int64_t f = 0;
  if (IsConstant(info->op_a, kExact, &a) && IsIntAndGet(info->fetch, &f) && f >= 1) {
...
bool InductionVarRange::GenerateRangeOrLastValue(HInstruction* context,
  ...
  *needs_taken_test = IsBodyTripCount(trip);
  ...
  if (is_last_value) {
    ...
    } else if (info->induction_class == HInductionVarAnalysis::kGeometric) {
      ...
      if (graph != nullptr) {
        ...
        if (fpowt == 0) {
          ...
          *result = graph->GetIntConstant(0);
          ...
          HInstruction* mul = Insert(block, new (graph->GetArena())
                                     HMul(info->type, opa, graph->GetIntConstant(fpowt)));
          *result = Insert(block, new (graph->GetArena()) HAdd(info->type, mul, opb));
          ...
          HInstruction* div =
              Insert(block, new (graph->GetArena())
                     HDiv(info->type, opa, graph->GetIntConstant(fpowt), kNoDexPc));
