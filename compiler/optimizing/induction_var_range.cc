        ...
        switch (info->operation) {
            ...
            if (GenerateCode(context,
              ...
              switch (info->operation) {
                case HInductionVarAnalysis::kAdd: {
                  ...
                  Value val_a = GetVal(context, loop, info->op_a, trip, is_min);
                  const bool a_is_const = IsConstantValue(val_a);
                  Value val_b = GetVal(context, loop, info->op_b, trip, is_min);
                  const bool b_is_const = IsConstantValue(val_b);
                  if (a_is_const && b_is_const) {
                    // Calculate `a + b` and use that. Note that even when the values are known,
                    // their addition can still overflow.
                    Value sub_val = AddValue(val_a, val_b);
                    if (sub_val.is_known) {
                  ...
                  if (a_is_const && val_a.b_constant == 0) {
                    if (graph != nullptr) {
                      operation = opb;
                    }
                    break;
                  ...
                  if (b_is_const && val_b.b_constant == 0) {
                    if (graph != nullptr) {
                      operation = opa;
                    }
                    break;
                ...
                case HInductionVarAnalysis::kSub: {
                  ...
                  if (a_is_const) {
                    // Calculate `a - b` and use that. Note that even when the values are known,
                    // their substraction can still overflow.
                    Value sub_val = SubValue(val_a, val_b);
                    if (sub_val.is_known) {
                      DCHECK(IsConstantValue(sub_val));
                      // Known value not overflowing.
                      if (graph != nullptr) {
                  ...
                  // Couldn't safely calculate the substraction.
                  return false;
                }
                case HInductionVarAnalysis::kMul: {
                  if (graph != nullptr) {
          ...
          case HInductionVarAnalysis::kTripCountInBodyUnsafe:
            if (is_min) {
              ...
              return true;
            } else if (IsContextInBody(context, loop) || context == loop->GetHeader()) {
              if (GenerateCode(context,
                               loop,
                               info->op_a,
                               trip,
                ...
                if (graph != nullptr) {
                  ...
                  if (IsContextInBody(context, loop)) {
                    *result =
                    ...
                    // Generate the full trip count. This will be combined with a is_taken test so
                    // we don't want to substract one.
