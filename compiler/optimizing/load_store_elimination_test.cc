...
class LoadStoreEliminationTestBase : public SuperTest, public OptimizingUnitTestHelper {
 public:
  ...
  HNewInstance* MakeNewInstance(HInstruction* cls,
    ...
    return new (GetAllocator()) HNewInstance(cls,
                                             0,
                                             ti ? *ti : load->GetTypeIndex(),
                                             graph_->GetDexFile(),
                                             false,
                                             QuickEntrypointEnum::kQuickAllocObjectInitialized);
  }

  HInstanceFieldSet* MakeIFieldSet(HInstruction* inst, HInstruction* data, MemberOffset off) {
    return new (GetAllocator()) HInstanceFieldSet(
        inst, data, nullptr, data->GetType(), off, false, 0, 0, graph_->GetDexFile(), 0);
  }

  HInstanceFieldGet* MakeIFieldGet(HInstruction* inst, DataType::Type type, MemberOffset off) {
    return new (GetAllocator()) HInstanceFieldGet(
        inst, nullptr, type, off, false, 0, 0, graph_->GetDexFile(), 0);
  }
  ...
  HParameterValue* MakeParam(DataType::Type type, std::optional<dex::TypeIndex> ti = std::nullopt) {
    return new (GetAllocator()) HParameterValue(
...
#undef GET_BLOCK
  ...
  HPhi* escape_end_phi =
      new (GetAllocator()) HPhi(GetAllocator(), kNoRegNumber, 2, DataType::Type::kReference);
  HInstruction* read_escape_end = MakeIFieldGet(escape_end_phi, DataType::Type::kInt32, MemberOffset(10));
...
#undef GET_BLOCK
  ...
  entry->AddInstruction(param1);
  entry->AddInstruction(param2);
  entry->AddInstruction(param3);
  entry->AddInstruction(param_obj1);
  entry->AddInstruction(param_obj2);
  ...
  HInstruction* read1 = MakeIFieldGet(new_inst1, DataType::Type::kReference, MemberOffset(10));
  read1->SetReferenceTypeInfo(
      ReferenceTypeInfo::CreateUnchecked(graph_->GetHandleCache()->GetObjectClassHandle(), true));
  HInstruction* read2 = MakeIFieldGet(new_inst2, DataType::Type::kReference, MemberOffset(10));
...
#undef GET_BLOCK
  ...
  EXPECT_INS_EQ(pred_get1->GetDefaultValue(), merge_value_return1) << " pred-get is: " << *pred_get1;
  ...
  EXPECT_INS_EQ(pred_get2->GetDefaultValue(), merge_value_return2) << " pred-get is: " << *pred_get2;
...
#undef GET_BLOCK
  ...
  entry->AddInstruction(param_1);
  entry->AddInstruction(param_2);
  ...
  HInstruction* obj1_mat = FindSingleInstruction<HNewInstance>(graph_, escape_1->GetSinglePredecessor());
  HInstruction* obj1_set = FindSingleInstruction<HInstanceFieldSet>(graph_, escape_1->GetSinglePredecessor());
  HInstruction* obj2_mat = FindSingleInstruction<HNewInstance>(graph_, escape_2->GetSinglePredecessor());
  HInstruction* obj2_set = FindSingleInstruction<HInstanceFieldSet>(graph_, escape_2->GetSinglePredecessor());
...
#undef GET_BLOCK
  ...
  ASSERT_NE(pred_get, nullptr);
  EXPECT_INS_EQ(pred_get->GetTarget(), merge_alloc);
  EXPECT_INS_EQ(pred_get->GetDefaultValue(), merge_value_return) << "get is " << pred_get->DumpWithArgs();
  EXPECT_INS_EQ(merge_value_return->InputAt(0), merge_value_merge)
      << " phi is: " << *merge_value_return;
  EXPECT_INS_EQ(merge_value_return->InputAt(1), c3) << " phi is: " << merge_value_return->DumpWithArgs();
  EXPECT_INS_EQ(merge_value_merge->InputAt(0), graph_->GetIntConstant(0))
      << " phi is: " << *merge_value_merge;
  EXPECT_INS_EQ(merge_value_merge->InputAt(1), c2) << " phi is: " << merge_value_merge->DumpWithArgs();
}
// // ENTRY
// obj = new Obj();
// obj.field = 3;
