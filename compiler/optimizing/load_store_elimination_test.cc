...
class LoadStoreEliminationTest : public CommonCompilerTest {
 public:
  LoadStoreEliminationTest() : pool_(), allocator_(&pool_) {}
  ...
  void CreateTestControlFlowGraph() {
    ...
    pre_header_ = new (&allocator_) HBasicBlock(graph_);
    ...
    // array, i, j, i+1, i+4.
    array_ = new (&allocator_) HParameterValue(graph_->GetDexFile(),
                                               dex::TypeIndex(0),
                                               0,
                                               DataType::Type::kReference);
    i_ = new (&allocator_) HParameterValue(graph_->GetDexFile(),
                                           dex::TypeIndex(1),
                                           1,
                                           DataType::Type::kInt32);
    j_ = new (&allocator_) HParameterValue(graph_->GetDexFile(),
                                           dex::TypeIndex(1),
                                           2,
                                           DataType::Type::kInt32);
    i_add1_ = new (&allocator_) HAdd(DataType::Type::kInt32, i_, c1);
    i_add4_ = new (&allocator_) HAdd(DataType::Type::kInt32, i_, c4);
    ...
    entry_->AddInstruction(new (&allocator_) HGoto());

    // pre_header block
    pre_header_->AddInstruction(new (&allocator_) HGoto());

    // loop header block has following instructions:
    // phi = 0;
    // if (phi >= 128);
    phi_ = new (&allocator_) HPhi(&allocator_, 0, 0, DataType::Type::kInt32);
    cmp_ = new (&allocator_) HGreaterThanOrEqual(phi_, c128);
    if_ = new (&allocator_) HIf(cmp_);
    ...
    // phi++;
    HInstruction* inc_phi = new (&allocator_) HAdd(DataType::Type::kInt32, phi_, c1);
  ...
  HInstruction* AddVecLoad(HBasicBlock* block, HInstruction* array, HInstruction* index) {
    ...
    HInstruction* vload = new (&allocator_) HVecLoad(&allocator_,
                                                     array,
                                                     index,
                                                     DataType::Type::kInt32,
                                                     4,
  ...
  HInstruction* AddVecStore(HBasicBlock* block,
    ...
    if (vdata == nullptr) {
      HInstruction* c1 = graph_->GetIntConstant(1);
      vdata = new (&allocator_) HVecReplicateScalar(&allocator_, c1, DataType::Type::kInt32, 4);
      block->InsertInstructionBefore(vdata, block->GetLastInstruction());
    }
    HInstruction* vstore = new (&allocator_) HVecStore(&allocator_,
                                                       array,
                                                       index,
                                                       vdata,
                                                       DataType::Type::kInt32,
  ...
  HInstruction* AddArrayGet(HBasicBlock* block, HInstruction* array, HInstruction* index) {
    ...
    DCHECK(array != nullptr);
    DCHECK(index != nullptr);
    HInstruction* get = new (&allocator_) HArrayGet(array, index, DataType::Type::kInt32, 0);
  ...
  HInstruction* AddArraySet(HBasicBlock* block,
    ...
    HInstruction* store = new (&allocator_) HArraySet(array,
                                                      index,
                                                      data,
                                                      DataType::Type::kInt32,
                                                      0);
...
TEST_F(LoadStoreEliminationTest, Loop2) {
  ...
  HInstruction* array_b = new (&allocator_) HNewArray(c0, c128, 0);
