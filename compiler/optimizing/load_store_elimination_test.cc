...
#undef GET_BLOCK
  ...
  EXPECT_EQ(pred_get->InputAt(0), merge_alloc);
  EXPECT_EQ(pred_get->InputAt(1), merge_value) << *pred_get << " vs " << *merge_value;
  EXPECT_EQ(merge_value->InputAt(1), c2) << *merge_value << " vs " << *c2;
}
