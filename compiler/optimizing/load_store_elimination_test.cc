...
class LoadStoreEliminationTestBase : public SuperTest, public OptimizingUnitTestHelper {
 public:
  ...
  void PerformLSE(OptimizingCompilerStats* stats = nullptr) {
    graph_->ForceWIPLSE();
    graph_->BuildDominatorTree();
    LoadStoreElimination lse(graph_, /*stats=*/stats);
    lse.Run();
    std::ostringstream oss;
    EXPECT_TRUE(CheckGraphSkipRefTypeInfoChecks(oss)) << oss.str();
  }

  void PerformLSEWithPartial(OptimizingCompilerStats* stats = nullptr) {
    graph_->ForceWIPLSE();
    PerformLSE(stats);
  }
...
enum class EscapeOrder {
  kSameAsAlloc, kReverseOfAlloc
};
std::ostream& operator<<(std::ostream& os, const EscapeOrder& ord) {
  switch (ord) {
    case EscapeOrder::kSameAsAlloc:
      return os << "SameAsAlloc";
    case EscapeOrder::kReverseOfAlloc:
      return os << "ReverseOfAlloc";
  }
}

class EscapeOrderDependentTestGroup
    : public LoadStoreEliminationTestBase<CommonCompilerTestWithParam<EscapeOrder>> { };
...
TEST_F(LoadStoreEliminationTest, DISABLED_PartialLoadPreserved) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
TEST_F(LoadStoreEliminationTest, DISABLED_PartialLoadPreserved2) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
// TODO TESTS: graph_undominated_entrypoint.cfg
// TODO TESTS: graph_lse_generates_phi.cfg
...
TEST_F(LoadStoreEliminationTest, PartialPhiPropogation) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
#undef GET_BLOCK
  ...
  EXPECT_INS_EQ(instance_phi->InputAt(0), FindSingleInstruction<HNewInstance>(graph_, escape_route_crit_break->GetSinglePredecessor()));
  EXPECT_INS_EQ(instance_phi->InputAt(1), graph_->GetNullConstant());
  // Check materialize block
  EXPECT_INS_EQ(FindSingleInstruction<HInstanceFieldSet>(graph_, escape_route_crit_break->GetSinglePredecessor())->InputAt(1), c12);
...
// EXIT
TEST_P(EscapeOrderDependentTestGroup, PredicatedEnvUse) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
#undef GET_BLOCK
  ...
  if (order == EscapeOrder::kReverseOfAlloc) {
  ...
  EXPECT_INS_EQ(moved_new_inst2->GetEnvironment()->GetInstructionAt(0),
                order == EscapeOrder::kSameAsAlloc ? moved_new_inst1 : static_cast<HInstruction*>(graph_->GetNullConstant()));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    EscapeOrderDependentTestGroup,
    testing::Values(EscapeOrder::kSameAsAlloc, EscapeOrder::kReverseOfAlloc));
...
// return obj.field;
// EXIT
TEST_F(LoadStoreEliminationTest, DISABLED_PartialLoadPreserved3) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
// ELIMINATE
// return obj.field
TEST_F(LoadStoreEliminationTest, DISABLED_PartialLoadPreserved5) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
#undef GET_BLOCK
  ...
  // PerformLSE expects this to be empty.
  graph_->ClearDominanceInformation();
  blks.Dump(LOG_STREAM(WARNING) << "Pre LSE");
  PerformLSEWithPartial();
  blks.Dump(LOG_STREAM(WARNING) << "Post LSE");
...
#undef GET_BLOCK
  ...
  // PerformLSE expects this to be empty.
  graph_->ClearDominanceInformation();
  blks.Dump(LOG_STREAM(WARNING) << "pre-partial!" << std::endl);
  PerformLSEWithPartial();
  blks.Dump(LOG_STREAM(WARNING) << "post-partial!" << std::endl);
