...
#undef GET_BLOCK
  ...
  HInstruction* suspend = new (GetAllocator()) HSuspendCheck();
  HInstruction* start_goto = new (GetAllocator()) HGoto();
  start->AddInstruction(param1);
  start->AddInstruction(param2);
  ...
  HPredicatedInstanceFieldGet* pred_get = FindSingleInstruction<HPredicatedInstanceFieldGet>(graph_, breturn);
  ASSERT_NE(pred_get, nullptr);
