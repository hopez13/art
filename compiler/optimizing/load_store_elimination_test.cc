...
std::tuple<Target*...> FindSingleInstructions(
  ...
  PatternMatchGraphVisitor vis(graph, [&](Target* t) {
    EXPECT_EQ(std::get<Target*>(res), nullptr) << *std::get<Target*>(res) << " already found!";
...
Target* FindSingleInstruction(
  ...
  Target* res = nullptr;
  PatternMatchGraphVisitor vis(graph, [&](Target* t) {
    EXPECT_EQ(res, nullptr) << *res << " already found!";
    res = t;
  });
  if (std::holds_alternative<std::initializer_list<HBasicBlock*>>(blks)) {
    for (HBasicBlock* blk : std::get<std::initializer_list<HBasicBlock*>>(blks)) {
...
#undef GET_BLOCK
  ...
  HPhi* merge_phi = FindSingleInstruction<HPhi>(graph_);
...
#undef GET_BLOCK
  ...
  HPhi* merge_alloc;
  std::tie(pred_get, merge_alloc) =
      FindSingleInstructions<HPredicatedInstanceFieldGet, HPhi>(graph_, breturn);
  ASSERT_NE(merge_alloc, nullptr);
  EXPECT_EQ(merge_alloc->InputAt(0), new_inst);
  EXPECT_EQ(merge_alloc->InputAt(1), null_const);
  ASSERT_NE(pred_get, nullptr);
  EXPECT_EQ(pred_get->InputAt(0), merge_alloc);
  EXPECT_EQ(pred_get->InputAt(1), c2);
}
