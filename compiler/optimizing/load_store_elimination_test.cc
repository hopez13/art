...
TEST_F(LoadStoreEliminationTest, MovePredicatedAlloc) {
  ...
  AdjacencyListGraph blks(SetupFromAdjacencyList(
      "entry",
      "exit",
      {{"entry", "left"}, {"entry", "right"}, {"right", "breturn"}, {"left", "breturn"}, {"breturn", "exit"}}));
#define GET_BLOCK(name) HBasicBlock* name = blks.Get(#name)
  GET_BLOCK(entry);
  GET_BLOCK(exit);
  GET_BLOCK(breturn);
...
#undef GET_BLOCK
  ...
  HPhi* merge_alloc1 = FindOrNull(merges.begin(), merges.end(), [&](HPhi* p) {
    return p->GetType() == DataType::Type::kReference && p->InputAt(0)->IsNewInstance() &&
...
// // allow us to entierly remove the allocation in this test.
// return obj.foo;
