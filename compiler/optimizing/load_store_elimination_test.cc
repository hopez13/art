...
class LoadStoreEliminationTestBase : public SuperTest, public OptimizingUnitTestHelper {
 public:
  void SetUp() override {
    ...
    // Make sure we do the test in Runnable mode.
    soa.emplace(Thread::Current());
  }

  void TearDown() override {
    // Go back to suspended.
    soa.reset();
    SuperTest::TearDown();
  ...
  HLoadClass* MakeClassLoad(std::optional<dex::TypeIndex> ti = std::nullopt) {
    return new (GetAllocator()) HLoadClass(graph_->GetCurrentMethod(),
                                           ...
                                           graph_->GetDexFile(),
                                           /* klass= */ ScopedNullHandle<mirror::Class>(),
  ...
  // SOA we used to transition into and out of runnable.
  std::optional<ScopedObjectAccess> soa = std::nullopt;
...
#undef CHECK_INST
    static_assert(!std::is_invocable<Func, HInstruction*>::value,
