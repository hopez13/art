...
TEST_F(LoadStoreEliminationTest, PartialLoadElimination) {
  ...
  for (HBasicBlock* blk : graph_->GetBlocks()) {
    if (!blks.HasBlock(blk)) {
      continue;
    }
    LOG(INFO) << "Block id: " << blk->GetBlockId() << " is " << blks.GetName(blk);
  }
  ...
  // PerformLSE expects this to be empty so whatevs.
...
// EXIT
// return PHI(foo_l, foo_r)
TEST_F(LoadStoreEliminationTest, PartialStoreEliminationMovement) {
  InitGraph();
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
                                                 "exit_REAL",
                                                 { { "entry", "left" },
  ...
  ASSERT_FALSE(IsRemoved(read_left));
  ASSERT_FALSE(IsRemoved(phi_final));
  ASSERT_TRUE(phi_final->GetInputs()[1] == c1);
  ASSERT_TRUE(phi_final->GetInputs()[0] == read_left);
  ASSERT_TRUE(write_entry->GetBlock() == left);
}
