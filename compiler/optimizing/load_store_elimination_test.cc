...
// NOTE: By default in the case of predicated SIMD all vector loads and stores are
// predicated by the same HVecPredWhile instruction.
class LoadStoreEliminationSIMDTest : public LoadStoreEliminationTest,
 ...
 public:
  ...
  std::tuple<HBasicBlock*, HBasicBlock*, HBasicBlock*, HBasicBlock*> CreateDiamondShapedCFG() {
    auto [upper, left, right, down] = LoadStoreEliminationTest::CreateDiamondShapedCFG();
    predicate_ = IsPredicatedSIMD() ? AddPredicate(upper) : nullptr;
    SetHasSIMD();
    return std::make_tuple(upper, left, right, down);
  }
 ...
 private:
  HInstruction* predicate_ = nullptr;
};
