...
// return out.foo
TEST_F(LoadStoreEliminationTest, PartialMarkerPhi) {
  ScopedObjectAccess soa(Thread::Current());
  VariableSizedHandleScope vshs(soa.Self());
  CreateGraph(&vshs);
  AdjacencyListGraph blks(SetupFromAdjacencyList("entry",
...
#undef GET_BLOCK
  EnsurePredecessorOrder(breturn, {left_merge_crit_break, right});
  ...
  EXPECT_INS_EQ(FindSingleInstruction<HPredicatedInstanceFieldGet>(graph_), nullptr)
      << " Should not be able to do anything with this!";
  EXPECT_TRUE(false) << "Not done yet";
