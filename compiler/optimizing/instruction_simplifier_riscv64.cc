...
class InstructionSimplifierRiscv64Visitor final : public HGraphVisitor {
 ...
 private:
  ...
  void VisitAdd(HAdd* add) override {
    ...
    if (auto left = add->GetLeft();
        left->IsShl() && TryReplaceShiftAddWithOneInstruction(left->AsShl(), add)) {
      return;
    } else if (auto right = add->GetRight();
               right->IsShl() && TryReplaceShiftAddWithOneInstruction(right->AsShl(), add)) {
      return;
    }
  }
