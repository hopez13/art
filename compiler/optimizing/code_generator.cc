...
void CodeGenerator::ClearSpillSlotsFromLoopPhisInStackMap(HSuspendCheck* suspend_check,
                                                          HParallelMove* parallel_move) const {
  ...
  DCHECK(block->GetFirstInstruction() == parallel_move);

  for (size_t i = 0, num_moves = parallel_move->NumMoves(); i != num_moves; ++i) {
    Location dest = parallel_move->MoveOperandsAt(i)->GetDestination();
    // All parallel moves in loop headers are spills.
    DCHECK(dest.IsStackSlot() || dest.IsDoubleStackSlot() || dest.IsSIMDStackSlot());
