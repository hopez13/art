...
void CodeGenerator::CreateStringBuilderAppendLocations(HStringBuilderAppend* instruction,
  ...
  for (size_t i = 0, num_args = instruction->GetNumberOfArguments(); i != num_args; ++i) {
    StringAppend::Argument arg_type =
        static_cast<StringAppend::Argument>(f & StringAppend::kArgMask);
    switch (arg_type) {
      case StringAppend::Argument::kStringBuilder:
      case StringAppend::Argument::kString:
      case StringAppend::Argument::kCharArray:
        static_assert(sizeof(StackReference<mirror::Object>) == sizeof(uint32_t), "Size check.");
        FALLTHROUGH_INTENDED;
      case StringAppend::Argument::kBoolean:
      case StringAppend::Argument::kChar:
      case StringAppend::Argument::kInt:
      case StringAppend::Argument::kFloat:
        locations->SetInAt(i, Location::StackSlot(stack_offset));
        break;
      case StringAppend::Argument::kLong:
      case StringAppend::Argument::kDouble:
      ...
      default:
        LOG(FATAL) << "Unexpected arg format: 0x" << std::hex
            << (f & StringAppend::kArgMask) << " full format: 0x" << format;
        UNREACHABLE();
    }
    f >>= StringAppend::kBitsPerArg;
    stack_offset += sizeof(uint32_t);
  }
  DCHECK_EQ(f, 0u);
