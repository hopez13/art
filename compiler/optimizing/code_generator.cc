...
void CodeGenerator::EmitLiterals(uint8_t* code,
  ...
  size_t index = 0;
  for (auto& entry : jit_string_entries_) {
    ...
    mirror::String* string = class_linker->ResolveString(
        entry_dex_file, entry.first.string_index, h_dex_cache);
    DCHECK(string != nullptr) << "JIT literals require strings to have been loaded";
    literals->Set(index, string);
    entry.second = index;
    ++index;
  }
  EmitLiteralPatches(code, literal_data);
}
