...
class CodeGenerator::CompilationData : public DeletableArenaObject<kArenaAllocCodeGenerator> {
 public:
  static std::unique_ptr<CompilationData> Create(ArenaStack* arena_stack,
    ...
    void* memory = allocator.Alloc<CompilationData>(kArenaAllocCodeGenerator);
    return std::unique_ptr<CompilationData>(
        ::new (memory) CompilationData(std::move(allocator), instruction_set));
  }
 ...
 private:
  CompilationData(ScopedArenaAllocator&& allocator, InstructionSet instruction_set)
...
ScopedArenaAllocator* CodeGenerator::GetScopedAllocator() {
  ...
  return compilation_data_->GetScopedAllocator();
}
