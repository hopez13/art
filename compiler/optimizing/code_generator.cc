...
class CodeGenerator::CompilationData : public DeletableArenaObject<kArenaAllocCodeGenerator> {
 public:
  static std::unique_ptr<CompilationData> Create(ArenaStack* arena_stack,
                                                 InstructionSet instruction_set) {
    std::unique_ptr<ScopedArenaAllocator> allocator(ScopedArenaAllocator::Create(arena_stack));
    ScopedArenaAllocator* alloc = allocator.get();
    return std::unique_ptr<CompilationData>(
        new (alloc) CompilationData(std::move(allocator), instruction_set));
  }
 ...
 private:
  CompilationData(std::unique_ptr<ScopedArenaAllocator> allocator, InstructionSet instruction_set)
      : allocator_(std::move(allocator)),
        stack_map_stream_(allocator_.get(), instruction_set),
...
void CodeGenerator::CompilationData::EmitJitRoots(
...
StackMapStream* CodeGenerator::GetStackMapStream() {
  DCHECK(compilation_data_ != nullptr);
  return compilation_data_->GetStackMapStream();
}

void CodeGenerator::ReserveJitStringRoot(StringReference string_reference,
                                         Handle<mirror::String> string) {
  DCHECK(compilation_data_ != nullptr);
  compilation_data_->ReserveJitStringRoot(string_reference, string);
}

uint64_t CodeGenerator::GetJitStringRootIndex(StringReference string_reference) {
  DCHECK(compilation_data_ != nullptr);
  return compilation_data_->GetJitStringRootIndex(string_reference);
}

void CodeGenerator::ReserveJitClassRoot(TypeReference type_reference, Handle<mirror::Class> klass) {
  DCHECK(compilation_data_ != nullptr);
  compilation_data_->ReserveJitClassRoot(type_reference, klass);
}

uint64_t CodeGenerator::GetJitClassRootIndex(TypeReference type_reference) {
  DCHECK(compilation_data_ != nullptr);
  return compilation_data_->GetJitClassRootIndex(type_reference);
}

void CodeGenerator::EmitJitRootPatches(uint8_t* code ATTRIBUTE_UNUSED,
                                       const uint8_t* roots_data ATTRIBUTE_UNUSED) {
  DCHECK(compilation_data_ != nullptr);
  DCHECK_EQ(compilation_data_->GetNumberOfJitStringRoots(), 0u);
  DCHECK_EQ(compilation_data_->GetNumberOfJitClassRoots(), 0u);
}
...
void CodeGenerator::GenerateSlowPaths() {
  DCHECK(compilation_data_ != nullptr);
  size_t code_start = 0;
  for (const std::unique_ptr<SlowPathCode>& slow_path_ptr : compilation_data_->GetSlowPaths()) {
...
void CodeGenerator::InitializeCompilationData() {
  DCHECK(compilation_data_ == nullptr);
  compilation_data_ = CompilationData::Create(graph_->GetArenaStack(), GetInstructionSet());
}
...
size_t CodeGenerator::GetNumberOfJitRoots() const {
  DCHECK(compilation_data_ != nullptr);
  return compilation_data_->GetNumberOfJitRoots();
}
...
void CodeGenerator::AddSlowPath(SlowPathCode* slow_path) {
  DCHECK(compilation_data_ != nullptr);
  compilation_data_->AddSlowPath(slow_path);
}
...
void CodeGenerator::EmitJitRoots(uint8_t* code,
  ...
  compilation_data_->EmitJitRoots(roots);
