...
static void GenerateVarHandleSet(HInvoke* invoke, CodeGeneratorX86* codegen) {
  ...
  switch (invoke->GetIntrinsic()) {
    ...
    case Intrinsics::kVarHandleSetOpaque:
      // According to c++ memory model, setOpaque would need a std::atomic_thread_fence(std::
      // memory_order_release) before the operation. But since the operation's memory model is
      // std::memory_order_relaxed, we agreed not to generate any fence for now.
      break;
