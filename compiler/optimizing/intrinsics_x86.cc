...
void IntrinsicLocationsBuilderX86::VisitVarHandleCompareAndSet(HInvoke* invoke) {
  ...
  if (new_value_type == DataType::Type::kInt64 ||
      new_value_type == DataType::Type::kFloat64) {
    // We avoid the case of a non-constant Int64/Float64 value because we would need to place it in
    // a register pair. If the slow path is taken, the ParallelMove might fail to move the pair
    // according to the X86DexCallingConvention in case of an overlap (e.g., move the int64 value
    // from <EAX, EBX> to <EBX, ECX>).
    return;
  }

  if (new_value_type == DataType::Type::kBool ||
      new_value_type == DataType::Type::kInt8 ||
      new_value_type == DataType::Type::kUint8 ||
      new_value_type == DataType::Type::kInt16 ||
      new_value_type == DataType::Type::kUint16) {
    // There is no cmpxchg() for bytes and words implemented yet.
    return;
  }
  ...
  locations->SetOut(Location::RegisterLocation(EBX));
}
...
void IntrinsicCodeGeneratorX86::VisitVarHandleCompareAndSet(HInvoke* invoke) {
  ...
  // Check the varType.primitiveType against the type of the value we're trying to set.
  GenerateVarTypePrimitiveTypeCheck(vh_object, temp, new_value_type, slow_path, assembler);
  ...
  CodeGeneratorX86* codegen_x86 = down_cast<CodeGeneratorX86*>(codegen_);
  if (expected_value_loc.IsConstant()) {
    __ movl(eax, Immediate(CodeGenerator::GetInt32ValueOf(expected_value_loc.GetConstant())));
  } else {
  ...
  switch (new_value_type) {
    ...
    case DataType::Type::kInt16:
    case DataType::Type::kUint16:
      // TODO: add LockCmpxchg for words and byte registers
      UNREACHABLE();
    case DataType::Type::kInt32:
    ...
    case DataType::Type::kReference: {
      ...
      bool expected_needs_write_barrier =
          CodeGenerator::StoreNeedsWriteBarrier(expected_value_type,
                                                invoke->InputAt(expected_value_index));
      uint32_t expected_coordinates_count = GetExpectedVarHandleCoordinatesCount(invoke);
      ...
      if (kPoisonHeapReferences && expected_needs_write_barrier) {
