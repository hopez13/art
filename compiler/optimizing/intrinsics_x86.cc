...
static void GenSystemArrayCopyBaseSourceAddress(X86Assembler* assembler,
                                                Primitive::Type type,
                                                const Register& src,
...
static void GenSystemArrayCopyBaseDestinationAddress(X86Assembler* assembler,
                                                     Primitive::Type type,
                                                     const Register& dst,
  ...
  if (dst_pos.IsConstant()) {
    ...
    __ leal(dst_base, Address(dst, dst_pos.AsRegister<Register>(), scale_factor, data_offset));
  }
}

// Compute end source address for the System.arraycopy intrinsic in `src_end`.
static void GenSystemArrayCopyEndSourceAddress(X86Assembler* assembler,
                                               Primitive::Type type,
  ...
  if (copy_length.IsConstant()) {
    ...
    __ leal(src_end, Address(src_base, copy_length.AsRegister<Register>(), scale_factor, 0));
  }
}
