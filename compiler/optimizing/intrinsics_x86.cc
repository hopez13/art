...
void IntrinsicCodeGeneratorX86::VisitVarHandleGet(HInvoke* invoke) {
  ...
  Location out = locations->Out();
  // Use 'out' as a temporary register if it's not FpuRegister
  Register temp = out.IsRegister() ? out.AsRegister<Register>() :
  ...
  // Use the input register for offset, as we don't need the VarHandle instance anymore
  Register offset = varhandle_object;
  ...
  codegen_x86->MoveFromAddress(out, Address(temp, offset, TIMES_1, 0), type);
