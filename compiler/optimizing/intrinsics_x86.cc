...
static void CreateVarHandleSetLocations(HInvoke* invoke, bool is_volatile) {
  // The only read barrier implementation supporting the
  // VarHandleGet intrinsic is the Baker-style read barriers.
  if (kEmitCompilerReadBarrier && !kUseBakerReadBarrier) {
    return;
  ...
  if (value_type == DataType::Type::kInt64 && (!value->IsConstant() || is_volatile)) {
    ...
    // int64 value from <EAX, EBX> to <EBX, ECX>).
    return;
  }
...
static void GenerateVarHandleSet(HInvoke* invoke, bool is_volatile, CodeGeneratorX86* codegen) {
  // The only read barrier implementation supporting the
  // VarHandleGet intrinsic is the Baker-style read barriers.
  DCHECK(!kEmitCompilerReadBarrier || kUseBakerReadBarrier);
...
void IntrinsicLocationsBuilderX86::VisitVarHandleSet(HInvoke* invoke) {
  CreateVarHandleSetLocations(invoke, /* is_volatile= */ false);
}

void IntrinsicCodeGeneratorX86::VisitVarHandleSet(HInvoke* invoke) {
  GenerateVarHandleSet(invoke, /* is_volatile= */ false, codegen_);
}

void IntrinsicLocationsBuilderX86::VisitVarHandleSetVolatile(HInvoke* invoke) {
  CreateVarHandleSetLocations(invoke, /* is_volatile= */ true);
}

void IntrinsicCodeGeneratorX86::VisitVarHandleSetVolatile(HInvoke* invoke) {
  GenerateVarHandleSet(invoke, /* is_volatile= */ true, codegen_);
}
