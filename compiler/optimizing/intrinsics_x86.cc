...
void IntrinsicCodeGeneratorX86::VisitVarHandleGet(HInvoke* invoke) {
  ...
  if (invoke->GetNumberOfArguments() == 1) {
    ...
    // Load the object's class and check it against coordType0. If it's not an exact match, check
    // if it's an inherited type.
    ...
    __ MaybeUnpoisonHeapReference(temp);
    // Bail to runtime if the object is a primitive.
    __ cmpw(Address(temp, primitive_type_offset), Immediate(Primitive::kPrimNot));
    __ j(kNotEqual, slow_path->GetEntryLabel());
