...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke, CodeGeneratorX86* codegen) {
  ...
  if (type == DataType::Type::kReference) {
    ...
    GenPrimitiveLockedCmpxchg(type, codegen, expected_value, new_value, reference, offset, temp);
    Location out = locations->Out();
    ...
    GenPrimitiveCAS(
        type, codegen, expected_value, new_value, reference, offset, locations->Out(), temp);
  }
