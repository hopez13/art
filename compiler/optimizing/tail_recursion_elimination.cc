...
bool TailRecursionElimination::TransformMethodGraph(HBasicBlock* exit_blk, HBasicBlock* first_block, TREContext& trec) {
  ...
  HIntConstant* inst_accumulator_val = nullptr;
  if (trec.ret_op_->IsBinaryOperation() && (trec.ret_op_->IsMul() || trec.ret_op_->IsDiv())) {
    inst_accumulator_val = graph_->GetIntConstant(1);
  } else {
    inst_accumulator_val = graph_->GetIntConstant(0);
  }
