        ...
        for (const HUseListNode<HInstruction*>& use1 : inst->GetUses()) {
          ...
          if (inst1->IsIf()) {
            ...
            for (int iter_blk = 0; iter_blk < 2; iter_blk++) {
              ...
              while (cur_blk != exit_blk) {
                ...
                if ((last_inst != nullptr) && (last_inst->IsReturn())) {
                  if (iter_blk == 0) {
                    trec.list_true_return_.push_back(last_inst);
                  } else {
                    trec.list_false_return_.push_back(last_inst);
                  }
                }
            ...
            if ((if_true_invoke == true) && (if_false_invoke == true)) {
              ...
              if ((exit_blk->GetPredecessors().size() == 1) && (num_blk[0] > 2)) {
                return false;
              } else if ((exit_blk->GetPredecessors().size() > 1) && (num_blk[0] > 1)) {
                return false;
              } else {
                if_break_true = true;
                inst_if = inst2;
              ...
              if ((exit_blk->GetPredecessors().size() == 1) && (num_blk[1] > 2)) {
                return false;
              } else if ((exit_blk->GetPredecessors().size() > 1) && (num_blk[1] > 1)) {
                return false;
              } else {
                if_break_true = false;
                inst_if = inst2;
...
bool TailRecursionElimination::TransformMethodGraph(HBasicBlock* exit_blk, HBasicBlock* first_block, TREContext& trec) {
  ...
  // don't handle if any of paths of recursion breaking IF condition doesn't contain return
  if ((trec.list_true_return_.size() == 0) || (trec.list_false_return_.size() == 0)) {
    return false;
  } else if ((trec.list_true_return_.size() == 1) && (trec.list_false_return_.size() == 1)) {
    HInstruction* true_return = trec.list_true_return_.at(0);
    HInstruction* false_return = trec.list_false_return_.at(0);

    if (true_return == false_return) {  // means single return statement
      ret_loop_back_edge = true_return;
      last_accu_op = true_return->InputAt(0);
    ...
    } else {  // means both true/false paths contains return separate return
      if (trec.if_true_exit_ == true) {
        ret_loop_back_edge = false_return;
        if_exit_val = true_return->InputAt(0);
        last_accu_op = false_return->InputAt(0);
      } else {
        ret_loop_back_edge = true_return;
        if_exit_val = false_return->InputAt(0);
        last_accu_op = true_return->InputAt(0);
      }
    }
  } else {  // means invoke path contains more than one return
    std::vector<HInstruction*> list_return;
    std::vector<HInstruction*>::iterator iter;
    if (trec.if_true_exit_ == true) {
      list_return = trec.list_false_return_;

      HInstruction* true_return = trec.list_true_return_.at(0);
      if_exit_val = true_return->InputAt(0);
    } else {
      list_return = trec.list_true_return_;

      HInstruction* false_return = trec.list_false_return_.at(0);
      if_exit_val = false_return->InputAt(0);
    }

    // means that non-invoke path of recursion breaking IF condition contains multiple returns
    if (list_return.size() == 1) {
      return false;
    }

    // assuming, exit block can be the only successor of block having last statement as return
    // add new block which will contain PHI for multiple return and new return statement
    HBasicBlock* new_pre_exit = new (allocator) HBasicBlock(graph_);
    DCHECK(new_pre_exit);
    graph_->AddBlock(new_pre_exit);

    // new PHI statment
    HPhi* phi_return = new (allocator) HPhi(allocator, kNoRegNumber, 0, HPhi::ToPhiType(ret_type));
    DCHECK(phi_return);
    new_pre_exit->AddPhi(phi_return);

    // new return statment
    HInstruction* inst_ret = new (allocator) HReturn(phi_return);
    DCHECK(inst_ret);
    new_pre_exit->AddInstruction(inst_ret);

    new_pre_exit->AddSuccessor(exit_blk);

    // add inputs of "phi_return" and updating successor of block having old return statement
    for (iter = list_return.begin(); iter != list_return.end(); /*iter++ */) {
      HInstruction* inst_return = *iter;
      DCHECK(inst_return);
      iter++;
      phi_return->AddInput(inst_return->InputAt(0));

      HBasicBlock* inst_blk = inst_return->GetBlock();
      HBasicBlock* cur_succ = inst_blk->GetSingleSuccessor();
      inst_blk->ReplaceSuccessor(cur_succ, new_pre_exit);

      inst_blk->RemoveInstruction(inst_return);
      inst_blk->AddInstruction(new (allocator) HGoto(kNoDexPc));
    }

    ret_loop_back_edge = inst_ret;
    last_accu_op = phi_return;
  }
  CHECK(last_accu_op);
