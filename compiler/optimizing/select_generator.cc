...
HBasicBlock* HSelectGenerator::TryFixupDoubleDiamondPattern(HBasicBlock* block) {
  ...
  // The innner if branch has to be block with just a compartison and an if.
  ...
  // First merge merges the outer if with one of the inner if branches. The block must be a Phi and a Goto.
  ...
  // Second merge is first_merge and the remainder branch merging. It must be phi + goto, or phi + return
  // Depending on the first merge, we define the second merge.
  HBasicBlock* merges_into_second_merge = BlocksMergeTogether(single_goto, inner_if_true_block) ?
