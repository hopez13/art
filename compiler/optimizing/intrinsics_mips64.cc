...
void IntrinsicLocationsBuilderMIPS64::VisitSystemArrayCopyChar(HInvoke* invoke) {
  ...
  locations->AddTemp(Location::RegisterLocation(out_location.AsRegister<GpuRegister>()));
}
  ...
  if (pos.IsConstant()) {
      ...
      if (!length_is_input_length) {
        ...
        if (length.IsConstant()) {
          if (IsInt<16>(length.GetConstant()->AsIntConstant()->GetValue())) {
            ...
            __ Bltc(AT, TMP, slow_path->GetEntryLabel());
          }
        } else {
          __ Bltc(AT, length.AsRegister<GpuRegister>(), slow_path->GetEntryLabel());
        }
      }
    } else {
      // Check that (length(input) - pos) >= zero.
      __ LoadFromOffset(kLoadWord, AT, input, length_offset);
      __ Daddiu64(TMP, AT, -pos_const, TMP);
      __ Bltzc(TMP, slow_path->GetEntryLabel());

      // Check that (length(input) - pos) >= length.
      if (length.IsConstant()) {
        if (IsInt<16>(length.GetConstant()->AsIntConstant()->GetValue())) {
          __ Slti(AT, TMP, length.GetConstant()->AsIntConstant()->GetValue());
          __ Bnezc(AT, slow_path->GetEntryLabel());
        } else {
          __ LoadConst32(AT, length.GetConstant()->AsIntConstant()->GetValue());
          __ Bltc(TMP, AT, slow_path->GetEntryLabel());
        }
      } else {
        __ Bltc(TMP, length.AsRegister<GpuRegister>(), slow_path->GetEntryLabel());
      }
    }
    ...
    if (length.IsConstant()) {
      ...
      if (IsInt<16>(length_constant)) {
        __ Slti(AT, TMP, length_constant);
        __ Bnezc(AT, slow_path->GetEntryLabel());
      } else {
        __ LoadConst32(AT, length_constant);
        __ Bltc(TMP, AT, slow_path->GetEntryLabel());
      }
    } else {
      __ Bltc(TMP, length.AsRegister<GpuRegister>(), slow_path->GetEntryLabel());
...
void IntrinsicCodeGeneratorMIPS64::VisitSystemArrayCopyChar(HInvoke* invoke) {
  ...
  if (src_pos.IsConstant()) {
    ...
    __ Daddiu64(src_base, src, data_offset + char_size * src_pos_const, AT);
  } else {
    __ Daddiu64(TMP, src, data_offset, AT);
    __ Dlsa(src_base, src_pos.AsRegister<GpuRegister>(), TMP, char_shift);
  }
  if (dest_pos.IsConstant()) {
    int32_t dest_pos_const = dest_pos.GetConstant()->AsIntConstant()->GetValue();

    __ Daddiu64(dest_base, dest, data_offset + char_size * dest_pos_const, AT);
  } else {
    __ Daddiu64(dest_base, dest, data_offset, AT);
