...
class CodeGeneratorARM64 : public CodeGenerator {
 public:
  ...
  // If `value` is non-null, crash if the card table is not valid. This check is only emitted for
  // the CC GC. We assert `(!clean || !self->is_gc_marking)`, since the card table should not be set
  // to clean when the CC GC is marking for eliminated write barriers.
  void CheckGCCardIsValid(vixl::aarch64::Register object, vixl::aarch64::Register value);
