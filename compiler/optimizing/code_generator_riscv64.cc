 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    if (entrypoint_.IsValid()) {
      ...
      DCHECK_EQ(entrypoint_.AsRegister<XRegister>(), T6);
      __ Jalr(entrypoint_.AsRegister<XRegister>());
      __ Nop();
    } else {
      int32_t entry_point_offset =
          Thread::ReadBarrierMarkEntryPointsOffset<kRiscv64PointerSize>(ref_reg - 4);
...
void InstructionCodeGeneratorRISCV64::GenerateGcRootFieldLoad(HInstruction* instruction,
  ...
  if (read_barrier_option == kWithReadBarrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      ScratchRegisterScope srs(GetAssembler());
      // FIXME: Is it safe to use T6 here?
      Location temp = Location::RegisterLocation(T6);
      srs.ExcludeXRegister(T6);
      SlowPathCodeRISCV64* slow_path =
          new (codegen_->GetScopedAllocator()) ReadBarrierMarkSlowPathRISCV64(instruction,
                                                                              root,
                                                                              temp);
      codegen_->AddSlowPath(slow_path);

      const int32_t entry_point_offset =
          Thread::ReadBarrierMarkEntryPointsOffset<kRiscv64PointerSize>(root.reg() - 4);
...
void InstructionCodeGeneratorRISCV64::HandleFieldSet(HInstruction* instruction,
  ...
  auto emit_store_with_type = [&](DataType::Type t, int ori, XRegister rs1, int64_t imm) {
    switch (t) {
      case DataType::Type::kBool:
      case DataType::Type::kUint8:
      case DataType::Type::kInt8:
        __ Storeb(XRegister(ori), rs1, imm);
        break;
      case DataType::Type::kUint16:
      case DataType::Type::kInt16:
      ...
      case DataType::Type::kInt64:
        ...
        break;
      case DataType::Type::kFloat32:
        __ FStorew(FRegister(ori), rs1, imm);
        break;
      case DataType::Type::kFloat64:
  ...
  if (value_location.IsConstant()) {
    ...
    emit_store_with_type(type, tmp, obj, offset);
  } else {
    if (!DataType::IsFloatingPointType(type)) {
      DCHECK(value_location.IsRegister());
      XRegister src = value_location.AsRegister<XRegister>();
      ScratchRegisterScope srs(GetAssembler());
      XRegister tmp = srs.AllocateXRegister();
      if (kPoisonHeapReferences && needs_write_barrier) {
        ...
        emit_store_with_type(type, src, obj, offset);
      }
    } else {
...
void InstructionCodeGeneratorRISCV64::HandleFieldGet(HInstruction* instruction,
  ...
  auto emit_load_with_type = [&](DataType::Type t, int rd,
                                   XRegister rs1,
                                   int64_t imm) {
    switch (t) {
      case DataType::Type::kBool:
      case DataType::Type::kUint8:
        __ Loadbu(XRegister(rd), rs1, imm);
        break;
      case DataType::Type::kInt8:
        __ Loadb(XRegister(rd), rs1, imm);
        break;
      case DataType::Type::kUint16:
        __ Loadhu(XRegister(rd), rs1, imm);
        break;
      case DataType::Type::kInt16:
        __ Loadh(XRegister(rd), rs1, imm);
        break;
      case DataType::Type::kInt32:
      ...
      case DataType::Type::kReference:
        ...
        break;
      case DataType::Type::kFloat32:
        __ FLoadw(FRegister(rd), rs1, imm);
        break;
      case DataType::Type::kFloat64:
  ...
  if (!DataType::IsFloatingPointType(type)) {
    ...
    XRegister dst = dst_loc.AsRegister<XRegister>();
    if (type == DataType::Type::kReference) {
      ...
      emit_load_with_type(type, dst, obj, offset);
    }
  } else {
...
void InstructionCodeGeneratorRISCV64::VisitArraySet(HArraySet* instruction) {
  ...
  switch (value_type) {
    ...
    case DataType::Type::kReference: {
      ...
      XRegister temp2 = srs.AllocateXRegister();  // Doesn't need to survive slow path.
...
TypeCheckKind type_check_kind = instruction->GetTypeCheckKind();
  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kInterfaceCheck: {
      ...
      __ Lw(temp2, temp, array_length_offset);
...
void InstructionCodeGeneratorRISCV64::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kArrayObjectCheck: {
      ...
      GenerateReferenceLoadOneRegister(
          instruction, out_loc, component_offset, maybe_temp_loc, read_barrier_option);
      // If `out` is null, we use it for the result, and jump to `done`.
      __ Beqz(out, &done);
...
void InstructionCodeGeneratorRISCV64::VisitLoadClass(HLoadClass* instruction) NO_THREAD_SAFETY_ANALYSIS {
  HLoadClass::LoadKind load_kind = instruction->GetLoadKind();
  if (load_kind == HLoadClass::LoadKind::kRuntimeCall) {
  ...
  Location out_loc = locations->Out();
  XRegister out = out_loc.AsRegister<XRegister>();
  XRegister current_method_reg = Zero;
  if (load_kind == HLoadClass::LoadKind::kReferrersClass ||
      load_kind == HLoadClass::LoadKind::kRuntimeCall) {
    current_method_reg = locations->InAt(0).AsRegister<XRegister>();
  }
  ...
  switch (load_kind) {
    ...
    case HLoadClass::LoadKind::kBootImageLinkTimePcRelative: {
      ...
      codegen_->EmitPcRelativeAuipcPlaceholder(info_high, temp);
      CodeGeneratorRISCV64::PcRelativePatchInfo* info_low =
          codegen_->NewBootImageTypePatch(instruction->GetDexFile(), instruction->GetTypeIndex(), info_high);
      codegen_->EmitPcRelativeAddiPlaceholder(info_low, temp, temp);
      break;
    }
    case HLoadClass::LoadKind::kBootImageRelRo: {
      ...
      codegen_->EmitPcRelativeLwuPlaceholder(info_low, temp, temp);
      break;
    }
    ...
    case HLoadClass::LoadKind::kJitTableAddress:
      __ Loadwu(out, codegen_->DeduplicateJitClassLiteral(
                         instruction->GetDexFile(), instruction->GetTypeIndex(), instruction->GetClass()));
      GenerateGcRootFieldLoad(instruction, out_loc, out, 0, read_barrier_option);
      break;
...
void InstructionCodeGeneratorRISCV64::VisitLoadString(HLoadString* instruction)
  ...
  switch (load_kind) {
    case HLoadString::LoadKind::kBootImageLinkTimePcRelative: {
      ...
      codegen_->EmitPcRelativeAuipcPlaceholder(info_high, temp);
      codegen_->EmitPcRelativeAddiPlaceholder(info_low, out, temp);
      return;
    }
    case HLoadString::LoadKind::kBootImageRelRo: {
      ...
      CodeGeneratorRISCV64::PcRelativePatchInfo* info_low =
          codegen_->NewBootImageRelRoPatch(boot_image_offset, info_high);
      codegen_->EmitPcRelativeAuipcPlaceholder(info_high, temp);
      codegen_->EmitPcRelativeLwuPlaceholder(info_low, out, temp);
      return;
    }
...
void InstructionCodeGeneratorRISCV64::VisitNullConstant([[maybe_unused]] HNullConstant* instruction) {
  // Will be generated at use site.
}
