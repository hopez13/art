...
static ArtMethod* ResolveMethod(uint16_t method_idx,
  ...
  if (*invoke_type == kDirect || *invoke_type == kStatic || *invoke_type == kSuper) {
    ...
    *target_method = MethodReference(/*dex_file=*/ nullptr, resolved_method->GetVtableIndex());
  } else {
    DCHECK_EQ(*invoke_type, kInterface);
    // For HInvokeInterface we need the IMT index.
    *target_method = MethodReference(/*dex_file=*/ nullptr, ImTable::GetImtIndex(resolved_method));
  }
...
bool HInstructionBuilder::BuildInvoke(const Instruction& instruction,
  ...
  ArtMethod* resolved_method = ResolveMethod(
      method_idx, graph_->GetArtMethod(), *dex_compilation_unit_, &invoke_type, &target_method);
