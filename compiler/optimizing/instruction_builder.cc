...
bool HInstructionBuilder::ProcessDexInstruction(const Instruction& instruction, uint32_t dex_pc) {
  switch (instruction.Opcode()) {
    ...
    case Instruction::MOVE_OBJECT_FROM16: {
      ...
      // this behavior and rely on our type propagation to eventually get the
      // types correctly, we just pretend we haven't seen a type change (int to
      // reference) for the 0 constant and phis.
