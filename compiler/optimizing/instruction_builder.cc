...
bool HInstructionBuilder::SetupInvokeArguments(HInvoke* invoke,
  ...
  for (size_t i = start_index;
       ...
       (i < operands->GetNumberOfOperands()) && (*argument_index < invoke->GetNumberOfArguments());
       i++, (*argument_index)++) {
    ...
    if (is_wide && ((i + 1 == operands->GetNumberOfOperands()) ||
...
HNewArray* HInstructionBuilder::BuildFilledNewArray(uint32_t dex_pc,
                                                    ...
                                                    const InstructionOperands* operands) {
  HInstruction* length = graph_->GetIntConstant(operands->GetNumberOfOperands(), dex_pc);
