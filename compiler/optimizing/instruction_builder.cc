...
bool HInstructionBuilder::ArrayTypeNeedsAccessCheck(dex::TypeIndex type_index) const {
  ScopedObjectAccess soa(Thread::Current());
  bool finalizable;
  return NeedsAccessCheck(type_index, &finalizable);
}
...
ObjPtr<mirror::Class> HInstructionBuilder::LookupResolvedType(
  ...
  ObjPtr<mirror::Class> type = dex_cache->GetResolvedType(type_index);
  if (type == nullptr) {
    type = compilation_unit->GetClassLinker()->LookupResolvedType(
        *compilation_unit->GetDexFile(),
        type_index,
        dex_cache,
        compilation_unit->GetClassLoader().Get());
  }
  return type;
}
