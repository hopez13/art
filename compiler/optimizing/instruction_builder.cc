...
bool HInstructionBuilder::BuildInstanceFieldAccess(const Instruction& instruction,
  ...
  if (instruction.IsQuickened()) {
    if (!CanDecodeQuickenedInfo()) {
      VLOG(compiler) << "Could not decode quickened instruction " << instruction.Opcode();
      return false;
    }
...
#define IF_XX(comparison, cond) \
    ...
    case Instruction::SGET_SHORT: {
      BuildStaticFieldAccess(instruction, dex_pc, false);
      break;
    }
    ...
    case Instruction::SPUT_SHORT: {
      BuildStaticFieldAccess(instruction, dex_pc, true);
      break;
    }
