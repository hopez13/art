...
static bool HasNonTrivialClinit(ObjPtr<mirror::Class> klass, PointerSize pointer_size)
    REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    for (; it.HasNext(); it.Next()) {
      switch (it.GetValueType()) {
        ...
        case EncodedArrayValueIterator::ValueType::kType:
          ...
          // which can execute arbitrary bytecode. We're not optimizing for know class loaders.
          return true;
        default:
...
static bool HasNonTrivialInitialization(ObjPtr<mirror::Class> cls,
  ...
  for (ObjPtr<mirror::Class> klass = cls; klass != nullptr; klass = klass->GetSuperClass()) {
    ...
    if (HasNonTrivialClinit(klass, pointer_size)) {
      return true;
    }
  }
  ...
  for (int32_t i = 0, count = iftable->Count(); i != count; ++i) {
    ...
    if (HasNonTrivialClinit(iface, pointer_size)) {
      return true;
    }
  }
  return false;
...
bool HInstructionBuilder::IsInitialized(Handle<mirror::Class> cls) const {
  ...
  //
  // If the initialization of `cls` does not need to execute any bytecode, it must complete,
  // either successfully or by throwing and marking `cls` as erroneous, without allocating any
  // instances of `cls` or subclasses (or any other class) and without calling any methods.
  ...
  if (is_subclass &&
      !HasNonTrivialInitialization(cls.Get(), compiler_driver_->GetCompilerOptions())) {
    return true;
  }
