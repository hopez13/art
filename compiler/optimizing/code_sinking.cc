...
static bool IsInterestingInstruction(HInstruction* instruction) {
  // Instructions from the entry graph (eg constants) are never interesting to move.
  if (instruction->GetBlock() == instruction->GetBlock()->GetGraph()->GetEntryBlock()) {
    return false;
  }
  // We want to move instructions that say they can move and cannot throw, as well as
  // heap stores and allocations.
  return instruction->IsNewInstance() ||
         instruction->IsNewArray() ||
         ((instruction->IsInstanceFieldSet() ||
           instruction->IsArraySet() ||
           instruction->CanBeMoved()) && !instruction->CanThrow());
}
...
// first in the sinking algorithm.
// This method is tailored to the sinking algorithm, unlike
// the generic HInstruction::FindIdealPosition.
...
void CodeSinking::SinkCodeToUncommonBranch(HBasicBlock* end_block) {
  ...
  // Step (1): Visit post order to guestimate blocks post dominated by `end_block`.
  // TODO(ngeoffray): We should try to compute the post dominator tree instead, but
  // that could be too time consuming. Also, we should start the analysis from blocks
  // dominated by an uncommon branch, but we don't profile branches yet.
  ...
  // Now that we've find a guestimate of dominated blocks, add to the worklist all inputs
