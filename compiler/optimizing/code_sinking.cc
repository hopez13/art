...
void CodeSinking::SinkCodeToUncommonBranch(HBasicBlock* end_block) {
  ...
  // We want to process the instructions in reverse dominated order. To guarantee this (including
  // the transitivity of incomparability) we have some extra bookkeeping.
  ...
  // Make sure we process instructions in reverse dominated order. This is required for heap stores.
  for (HBasicBlock* block : graph_->GetPostOrder()) {
    ...
    std::sort(instructions_to_move[block_id].begin(),
              ...
              [&block](HInstruction* a, HInstruction* b) { return block->GetInstructions().FoundBefore(b, a); });
  ...
  if (kIsDebugBuild) {
    ...
    if (!instructions_to_move_sorted.empty()) {
