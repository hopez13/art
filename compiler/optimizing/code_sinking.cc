...
void CodeSinking::ReturnSinking() {
  ...
  for (HBasicBlock* pred : exit->GetPredecessors()) {
    ...
    if (pred->GetLastInstruction()->IsReturn()) {
      saw_return = true;
      ++number_of_returns;
    } else if (pred->GetLastInstruction()->IsReturnVoid()) {
      saw_return_void = true;
      ++number_of_returns;
    }
  }
  ...
  if (kIsDebugBuild) {
    DCHECK_NE(saw_return, saw_return_void)
        << "Consistency check: We shouldn't have both Return and ReturnVoid in the same method.";
  }
  ...
  // TODO(solanes): Potentially we could just add the new block to the RPO right before the exit
  // block and avoid recomputing the whole graph. We currently don't allow that kind of RPO
  // manipulations.
