...
bool HDeadCodeElimination::CanPerformTryBoundaryRemoval(
...
bool HDeadCodeElimination::RemoveUnneededTries() {
  ...
  for (HBasicBlock* block : graph_->GetReversePostOrderSkipEntryBlock()) {
    if (block->IsTryBlock()) {
      tries[block->GetTryCatchInformation()->GetTryEntry().GetBlock()].blocks_in_try.insert(block);
    }
  }

  // Deduplicate the tries which have different try entries but they are really the same try.
  for (auto it = tries.begin(); it != tries.end(); /*it++ in the loop*/) {
    ...
    for (auto other_it = next(it); other_it != tries.end(); /*other_it++ in the loop*/) {
      ...
      HTryBoundary* ohter_try_boundary = other_it->first->GetLastInstruction()->AsTryBoundary();
      if (try_boundary->HasSameExceptionHandlersAs(*ohter_try_boundary)) {
  ...
  for (const auto& entry : tries) {
    if (CanPerformTryBoundaryRemoval(entry.second)) {
      ++removed_tries;
  ...
  if (removed_tries != 0) {
    ...
    // recompute it.
    // Also note that we are not guaranteed to remove a catch block if we have nested try blocks:
    //
    //   try {
    ...
    // removed as the TryBoundary B might still throw into that catch. They don't get coalesced
    // since TryBoundary A and B have different catch handlers.
...
bool HDeadCodeElimination::Run() {
  ...
  if (!graph_->HasIrreducibleLoops()) {
    ...
    // We call RemoveDeadBlocks to remove the dead blocks from the previous optimizations.
    // Otherwise, we might detect that a try has throwing instructions but they are actually dead
    // code. RemoveUnneededTryBoundary will call RemoveDeadBlocks again if needed.
