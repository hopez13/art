  ...
  for (HBasicBlock* block : graph_->GetReversePostOrder()) {
    ...
    if (block->GetLastInstruction()->IsGoto() &&
      ...
      if (succ != exit &&
        ...
        // We split the block at the throwing instruction, and those instructions will be
        // disconnected from the graph after `block` points to the exit. `RemoveDeadBlocks` will
        // take care of removing this new block and its instructions. The added instructions are:
        //   1) The Goto from the block split, and
        //   2) The instructions in the new block that are going to be removed.
