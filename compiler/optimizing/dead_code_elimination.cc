...
bool HDeadCodeElimination::SimplifyAlwaysThrows() {
  ...
  for (HBasicBlock* block : graph_->GetReversePostOrder()) {
    ...
    if (last->IsGoto() &&
      ...
      if (succ != exit &&
        ...
        // We split the block at the throwing instruction. Everything after that will be
        // disconnected from the graph after `block` points to the exit. `RemoveDeadBlocks` will
        // take care of removing this new block and its instructions.
        // Even though `SplitBefore` doesn't guarantee the graph to remain in SSA form, it is fine
        // for this optimization since later on we will guarantee it is in SSA form.
  ...
  DCHECK(graph_->IsInSsaForm()) << "Graph must remain in SSA form";
  return rerun_dominance_and_loop_analysis;
}
