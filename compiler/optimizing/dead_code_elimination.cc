...
void HDeadCodeElimination::MaybeAddPhi(HBasicBlock* block) {
  ...
  for (size_t index = 0; index < pred_size; index++) {
    ...
    if (dominated_by_true == dominated_by_false) {
      ...
      // The Phi would have `0`, `1`, and `cond`. If SimplifyIf is able to redirect 0s and 1s, we
      // will end up with Phi(cond,...,cond) which will be replaced by `cond`. Effectively, we will
      // able to redirect edges that we are able to redirect and the rest will remain the same.
      ...
      // `(same_input && dominated_by_true) || (!same_input && dominated_by_false)`
