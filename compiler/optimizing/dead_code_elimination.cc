...
bool HDeadCodeElimination::RemoveEmptyIfs() {
  ...
  for (HBasicBlock* block : graph_->GetPostOrder()) {
    ...
    if (merge_true != merge_false || !merge_true->GetPhis().IsEmpty()) {
      // TODO(solanes): Potentially, we could allow Phis iff both blocks have the same value for all Phis.
      continue;
    }
