...
bool HDeadCodeElimination::SimplifyAlwaysThrows() {
  ...
  for (HBasicBlock* block : graph_->GetReversePostOrder()) {
    if (block->GetTryCatchInformation() != nullptr) {
      // We want to block the simplify always throws optimizations for throws inside of tries since
      // those throws might not go to the exit block. We do that by checking the TryCatchInformation
      // of the blocks.
