...
// contain a suspend check. It cannot be applied to catch blocks
// either, because exception handler edges do not represent normal
// control flow. Though in theory this could still apply to normal
// code flow going directly to a catch block, we cannot support it
// at the moment because the catch Phi's inputs do not correspond
// to the catch block's predecessors, so we cannot identify which
