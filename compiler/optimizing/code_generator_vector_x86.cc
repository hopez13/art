...
static void CreateVecMemLocations(ArenaAllocator* arena,
  ...
  LocationSummary* locations = new (arena) LocationSummary(instruction);
#else
  LocationSummary* locations = new (arena) LocationSummary(
...
#if 1
  // TODO: request 7 xmm register for fun
  for (int i = 0; i < 7; i++) {
    instruction->GetLocations()->AddTemp(Location::RequiresFpuRegister());
  }
#endif
...
void InstructionCodeGeneratorX86::VisitVecLoad(HVecLoad* instruction) {
...
#if 0
  SlowPathCode* slow_path = new (GetGraph()->GetArena()) SpillingSlowPathX86(instruction);
  ...
  __ nop();
  for (int i = 0; i < 7; i++) {
    XmmRegister t = instruction->GetLocations()->GetTemp(i).AsFpuRegister<XmmRegister>();
    __ pxor(t, t);
  }
  __ nop();
  __ nop();
