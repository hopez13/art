...
class StringList {
 public:
  ...
  // Construct StringList from an IntrusiveForwardList<>. List element class T
  // must provide method `Dump`.
  template <typename T, typename HookTraits>
  explicit StringList(const IntrusiveForwardList<T, HookTraits>& list,
                      Format format = kArrayBrackets) : StringList(format) {
