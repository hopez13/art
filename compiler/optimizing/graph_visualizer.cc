...
class HGraphVisualizerPrinter : public HGraphDelegateVisitor {
 public:
  ...
  void StreamProperty(const char* name, Target t) {
    AddIndent();
    output_ << name << " \"" << t << "\"\n";
  }

  void PrintProperty(const char* name, HBasicBlock* blk) {
    StreamProperty(name, namer_.GetName(blk));
  }
...
std::ostream& HGraphVisualizer::OptionalDefaultNamer::PrintName(std::ostream &os, HBasicBlock *blk) const {
  if (namer_) {
...
void HGraphVisualizer::DumpGraphDebug() const {
  DumpGraph(kDebugDumpGraphName, false, true);
}
