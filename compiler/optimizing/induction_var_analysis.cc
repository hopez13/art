 ...
 */
static Primitive::Type ImplicitConv(Primitive::Type type) {
  switch (type) {
    case Primitive::kPrimShort:
    case Primitive::kPrimChar:
...
void HInductionVarAnalysis::ClassifyNonTrivial(HLoopInformation* loop) {
  ...
  for (size_t i = 1; i < size; i++) {
    ...
    if (instruction->IsPhi()) {
      ...
      update = SolveCnv(loop, phi, instruction->AsTypeConversion());
    }
    if (update == nullptr) {
      return;
    }
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::SolveCnv(HLoopInformation* loop,
                                                                      HInstruction* entry_phi,
...
void HInductionVarAnalysis::VisitControl(HLoopInformation* loop) {
  ...
  if (control->IsIf()) {
    ...
    if (if_expr->IsCondition()) {
      ...
      Primitive::Type type = ImplicitConv(condition->InputAt(0)->GetType());
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::CreateSimplifiedInvariant(
  ...
  return new (graph_->GetArena()) InductionInfo(
      kInvariant, op, a, b, nullptr, ImplicitConv(b->type));
}
