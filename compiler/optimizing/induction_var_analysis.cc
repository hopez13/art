...
static bool RewriteBreakLoopBody(HLoopInformation* loop,
  ...
  HInstruction* p = nullptr;
  for (HInstruction* m = cond->GetPrevious(); m && !m->IsSuspendCheck(); m = p) {
    p = m->GetPrevious();
    if (rewrite) {
...
size_t HInductionVarAnalysis::TryVisitNodes(
  ...
  while (!stack.empty()) {
    ...
    if (entry.low_depth < entry.node_info->depth) {
      DCHECK_LT(entry.user_link, current_entry) << entry.low_depth << " " << entry.node_info->depth;
      if (stack[entry.user_link].low_depth > entry.low_depth) {
        stack[entry.user_link].low_depth = entry.low_depth;
...
void HInductionVarAnalysis::ExtractScc(ArrayRef<const StackEntry> stack_tail,
  ...
  scc->reserve(size);
  for (size_t i = split_pos; i != 0;) {
    --i;
    scc->push_back(stack_tail[i].instruction);
  }
  for (size_t i = size; i != split_pos;) {
    --i;
    scc->push_back(stack_tail[i].instruction);
  }
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::SolveAddSub(
  ...
  if (op == kAdd) {
    ...
    if (is_first_call) {
      return SolveAddSub(
          loop, entry_phi, instruction, y, x, op, /*is_first_call=*/ false, cycle, type);
    }
  } else if (op == kSub) {
