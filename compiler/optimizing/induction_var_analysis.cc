...
void HInductionVarAnalysis::CalculateLoopHeaderPhisInARow(
    HPhi* initial_phi, ScopedArenaSafeMap<HPhi*, int>& cached_values) {
  DCHECK(initial_phi->IsLoopHeaderPhi());
  std::queue<HPhi*> worklist;
  ...
  while (!worklist.empty()) {
    ...
    if (phis_in_chain.find(current_phi) != phis_in_chain.end()) {
      // We already saw `current_phi` i.e. we are in a loop, bail. We will set `phi` in
      // `visited_phis` in the other instance.
      worklist.pop();
      continue;
    }
...
bool HInductionVarAnalysis::IsPathologicalCase() {
  ...
  for (HBasicBlock* block : graph_->GetReversePostOrder()) {
    ...
    for (HInstructionIterator it(block->GetPhis()); !it.Done(); it.Advance()) {
      ...
      CalculateLoopHeaderPhisInARow(phi, cached_values);
