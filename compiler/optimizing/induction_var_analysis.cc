...
void HInductionVarAnalysis::ClassifyTrivial(HLoopInformation* loop, HInstruction* instruction) {
  ...
  if (instruction->IsPhi()) {
    info = TransferPhi(loop, instruction, /*input_index*/ 0, /*adjust_size*/ 0);
    ...
    HInstruction* mulc = GetShiftConstant(loop, instruction, nullptr);
    if (mulc != nullptr) {
      info = TransferMul(LookupInfo(loop, instruction->InputAt(0)),
                         LookupInfo(loop, mulc));
    }
  } else if (instruction->IsSelect()) {
    info = TransferPhi(loop, instruction, /*input_index*/ 0, /*adjust_size*/ 1);
...
void HInductionVarAnalysis::ClassifyNonTrivial(HLoopInformation* loop) {
  ...
  if (size == 1) {
    InductionInfo* update = TransferPhi(loop, phi, /*input_index*/ 1, /*adjust_size*/ 0);
    if (update != nullptr) {
  ...
  for (size_t i = 1; i < size; i++) {
    ...
    if (instruction->IsPhi()) {
      ...
      update = SolvePhi(instruction, /*input_index*/ 0, /*adjust_size*/ 1);
    } else if (instruction->IsTypeConversion()) {
  ...
  InductionInfo* induction = SolvePhi(phi, /*input_index*/ 1, /*adjust_size*/ 0);
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::TransferPhi(HLoopInformation* loop,
  ...
  for (size_t i = input_index + 1, n = inputs.size() - adjust_size; i < n; i++) {
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::SolvePhiAllInputs(
  ...
  InductionInfo* match = SolvePhi(phi, /*input_index*/ 0, /*adjust_size*/ 0);
  if (match != nullptr) {
    return match;
  }
  ...
  if (phi->IsLoopHeaderPhi() && phi->InputCount() == 2) {
    ...
    if (a != nullptr && a->induction_class == kInvariant) {
      ...
      InductionInfo* b = SolvePhi(phi, /*input_index*/ 1, /*adjust_size*/ 0);
