...
std::ostream& operator<<(std::ostream& os, ShuffleOrder so) {
  switch (so) {
    ...
    case ShuffleOrder::kRandom:
      return os << "Random";
  }
}

template<typename Param>
class ParamReferenceTypePropagationTest : public ReferenceTypePropagationTestBase<CommonCompilerTestWithParam<Param>> {
 public:
...
std::ostream& operator<<(std::ostream& os, const LoopOptions& lo) {
  return os << "LoopOptions{ shuffle: " << lo.shuffle_ << ", null_phi: " << lo.null_insertion_
            << ", null_phi_arg: " << lo.null_phi_arg_
            << ", initial_null_state: " << lo.initial_null_state_ << "}";
}
