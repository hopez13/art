...
bool TryMergeWithAnd(HSub* instruction) {
  ...
  const bool left_is_equal = and_instr->GetLeft() == value;
  const bool right_is_equal = and_instr->GetLeft() == value;
  ...
  HBitwiseNegatedRight* bnr = new (instruction->GetBlock()->GetGraph()->GetAllocator())
      HBitwiseNegatedRight(instruction->GetType(),
                           ...
                           value,
                           left_is_equal ? and_instr->InputAt(1) : and_instr->InputAt(0),
