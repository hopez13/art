...
HBasicBlock* PeelUnrollHelper::DoPartialUnrolling(int iterations ATTRIBUTE_UNUSED, int unroll_factor) {
  ...
  // Perform checks for induction value and conditions for partial loop unrolling.
  ...
  // Initial value of copy loop's induction variable and replicate
  // orig loop body instructions as per unroll_factor.
...
HInstruction* PeelUnrollHelper::AddLoopUnrollEpilogue(HGraph* graph)  {
  ...
  // The "cond_val" in header block means that it's  value is somehow
...
void PeelUnrollHelper::ModifyLoopInductionPartialUnroll(HGraph* graph, HInstruction* phi_induc, int unroll_factor) {
  ...
  if (inst_induction_op->IsAdd() || inst_induction_op->IsSub() || inst_induction_op->IsShl()) {
    ...
    if (DataType::IsIntegralType(inst_induction_op_val_type)) {
      ...
      // Add unroll fator constant to loop preheader.
    ...
    if (inst_induction_op->IsAdd() || inst_induction_op->IsSub()) {
      ...
      // Add induction op value instruction to loop preheader.
    ...
    // Add check value for induction operation instruction to loop preheader.
  ...
  if (inst_induction_op_val->IsConstant()) {
    ...
    if (inst_induction_op->IsAdd() || inst_induction_op->IsSub()) {
      ...
      if (DataType::IsIntegralType(phi_induc_type)) {
        ...
        // Add multiplication value instruction to loop header.
      ...
      // Add unrolled loop advancing instruction to loop preheader (for Mul with constant).
      header->InsertInstructionBefore(inst_unrolled_advance_val, inst_induction_cond);
    } else if (inst_induction_op->IsShl()) {
      inst_unrolled_advance_val = new (allocator) HShl(phi_induc->GetType(), phi_induc,
                                      inst_check_val);

      DCHECK(inst_unrolled_advance_val);
      // Add unrolled loop advancing instruction to loop preheader (for Shl with constant).
      header->InsertInstructionBefore(inst_unrolled_advance_val, inst_induction_cond);
    }
    ...
    if (inst_induction_op->IsAdd() || inst_induction_op->IsSub()) {
      ...
      // Add updated comparison instruction to loop header.
      ...
      // Add "cond_val - induc_val" instruction to loop header.
      ...
      // Add "induc_val - cond_val" instruction to loop header.
  ...
  if (inst_induction_op->IsAdd() || inst_induction_op->IsSub()) {
    ...
    // Add final difference between cond_val & induc_val instruction to loop header.
    ...
    if (inst_check_val->GetType() != inst_final_sub->GetType()) {
      ...
      // Add final difference instruction (with modified type) to loop preheader.
...
// Replicate orig loop body instructions as per unroll factor
// Modify orig loop induction variable condition, initial value
// of copy loop's induction variable
bool PeelUnrollHelper::AdjustLoops(HGraph* graph, HInstruction* phi_induc,
                                   int unroll_factor)  {
  DCHECK(loop_info_ != nullptr);
