...
static const bool kPeelUnrollCopiesGoFirst = true;
...
static bool IsEdgeValid(HEdge edge, HGraph* graph) {
  if (!edge.IsValid()) {
    return false;
  }
...
void SuperblockCloner::RecalculateBackEdgesInfo(ArenaBitVector* outer_loop_bb_set) {
  ...
  if (outer_loop_ == nullptr) {
    ...
    outer_loop_bb_set->Copy(&outer_loop_->GetBlocks());
...
bool SuperblockCloner::IsFastCase() const {
  ...
  if (common_loop_info == nullptr || !orig_bb_set_->Equal(&common_loop_info->GetBlocks())) {
    return false;
  }
...
HBasicBlock* DoPeelUnrollImpl(HLoopInformation* loop_info,
  ...
  HBasicBlock* node_loop_header = nullptr;
  // Update current node.
  if (kPeelUnrollCopiesGoFirst) {
    node_loop_header = to_unroll ? bb_map->Get(loop_header) : loop_header;
  } else {
    node_loop_header = to_unroll ? loop_header : bb_map->Get(loop_header);
  }

  return node_loop_header;
}

HBasicBlock* PeelUnrollLoop(HLoopInformation* loop_info, bool do_unroll) {
  ArenaAllocator* arena = loop_info->GetHeader()->GetGraph()->GetArena();
  ...
  HBasicBlock* new_header = DoPeelUnrollImpl(loop_info, do_unroll, &bb_map, &hir_map);
  return new_header;
}
...
namespace std {
  size_t hash<art::HEdge>::operator()(art::HEdge const& x) const noexcept {
