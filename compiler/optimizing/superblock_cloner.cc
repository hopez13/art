...
/* Suppose loop looks like "for (i = a; i <comp_op> b; i <advance_op> c)"
   ...
   Enter partially unrolled loop only if "abs(b - a)" (i.e. loop iterations) > "(abs(c) * unroll_factor)"
   Compute new cond val for "i <comp_op> b" such that partial unrolled loop execution aligns with unroll_factor 
   Modify loop terminating condition against "phi_induc <comp_op> new_cond_val"
*/
void PeelUnrollHelper::ModifyLoopInductionPartialUnroll(HGraph* graph, HInstruction* phi_induc, int unroll_factor) {
  ...
  // add instruction to loop preheader
  preheader->InsertInstructionBefore(inst_divisor, preheader->GetLastInstruction());
  ...
  if (inst_divisor->GetType() != inst_final_sub->GetType()) {
    ...
    // add instruction to loop preheader
    preheader->InsertInstructionBefore(inst_divisor2, preheader->GetLastInstruction());
  } else {
    inst_divisor2 = inst_divisor;
  }

  HInstruction* inst_comp_length_unroll = new (allocator) HGreaterThan(inst_final_sub, inst_divisor2);
