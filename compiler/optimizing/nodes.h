 ...
 public:
  ...
  bool HasShouldDeoptimizeFlag() const {
    return number_of_cha_guards_ != 0 ||
           (Runtime::Current() && Runtime::Current()->IsJavaDebuggable());
  }
...
#define FOR_EACH_CONCRETE_INSTRUCTION_SCALAR_COMMON(M)                  \
  ...
  M(MethodExitHookVoid, Instruction)                                    \
...
class HMethodEntryHook : public HExpression<0> {
 public:
  HMethodEntryHook(uint32_t dex_pc) : HExpression(kMethodEntryHook, SideEffects::All(), dex_pc) {}
...
class HMethodExitHookVoid : public HExpression<0> {
 public:
  HMethodExitHookVoid(uint32_t dex_pc)
      : HExpression(kMethodExitHookVoid, SideEffects::All(), dex_pc) {}

  bool NeedsEnvironment() const override {
    return true;
  }

  DECLARE_INSTRUCTION(MethodExitHookVoid);

 protected:
  DEFAULT_COPY_CONSTRUCTOR(MethodExitHookVoid);
};
