...
class HGraph : public ArenaObject<kArenaAllocGraph> {
 public:
  HGraph(ArenaAllocator* arena,
      ...
      : arena_(arena),
        ...
        osr_(osr),
        cha_single_implementation_list_() {
  ...
  std::unordered_set<ArtMethod*>& GetCHASingleImplementationList() {
 ...
 private:
  ...
  std::unordered_set<ArtMethod*> cha_single_implementation_list_;
...
#define FOR_EACH_CONCRETE_INSTRUCTION_COMMON(M)                         \
  ...
  M(DeoptimizationFlag, Instruction)                                    \
...
// Represents a deoptimization flag. Currently used for CHA-based devirtualization.
class HDeoptimizationFlag FINAL : public HExpression<0> {
 public:
  HDeoptimizationFlag(uint32_t dex_pc)
      : HExpression(Primitive::kPrimInt, SideEffects::None(), dex_pc) {
  }

  DECLARE_INSTRUCTION(DeoptimizationFlag);

 private:
  DISALLOW_COPY_AND_ASSIGN(HDeoptimizationFlag);
};
