...
// constructor completition (i.e. escapes the current thread via a store), then any final field
// writes must be observable on other threads (once they observe this publish).
//
// Furthermore, anything reachable from the final fields must also be observable
// as at least up-to-date as the final fields after the freeze (so final object field could itself
...
class HConstructorFence FINAL : public HVariableInputSizeInstruction {
 ...
 public:
  ...
  HConstructorFence(HInstruction* fence_object,
                    uint32_t dex_pc,
                    ArenaAllocator* arena)
    // Initially starts out as an "all reads" side effect; this is to prevent final stores prior to
    // the fence from being reordered past this fence (or vice versa).
    //
    // Later phases can reduce the "all reads" down to just the types of final fields that were
    // written to (and for final object field writes, this should remain as "all reads" due to the
    // recursive reachability; see above).
