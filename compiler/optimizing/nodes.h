...
class HEnvironment : public ArenaObject<kArenaAllocEnvironment> {
 public:
  ...
  void SetHolder(HInstruction* holder) {
    holder_ = holder;
  }
...
#undef DECLARE_KIND
  ...
  HInstruction(InstructionKind kind, DataType::Type type, SideEffects side_effects, uint32_t dex_pc)
    ...
    SetPackedFlag<kFlagDoNullCheck>(false);
  }
  ...
  virtual bool NeedsEnvironment() const {
    DCHECK(!DoNullCheck()) << "Subclass should have overridden CanThrow";
    return false;
  }
...
#undef INSTRUCTION_TYPE_CAST
  ...
  bool DoNullCheck() const { return GetPackedFlag<kFlagDoNullCheck>(); }
  void SetDoNullCheck() { SetPackedFlag<kFlagDoNullCheck>(true); }

  bool SharesDexLocation(HInstruction* other) const {
    // Start with a quick dex pc check.
    if (GetDexPc() != other->GetDexPc()) {
      return false;
    }
    ...
    const HEnvironment* this_environment = GetEnvironment();
    const HEnvironment* other_environment = other->GetEnvironment();
    while (this_environment != nullptr || other_environment != nullptr) {
      if (this_environment == nullptr || other_environment == nullptr) {
      ...
      if (this_environment->GetDexPc() != other_environment->GetDexPc() ||
          this_environment->GetMethod() != other_environment->GetMethod()) {
        return false;
      }
      this_environment = this_environment->GetParent();
      other_environment = other_environment->GetParent();
    }
    return true;
  }
 ...
 protected:
  ...
  static constexpr size_t kFlagDoNullCheck = kFlagEmittedAtUseSite + 1;
  static constexpr size_t kFlagReferenceTypeIsExact = kFlagDoNullCheck + 1;
...
class HInvoke : public HVariableInputSizeInstruction {
 public:
  ...
  bool CanThrow() const OVERRIDE { return GetPackedFlag<kFlagCanThrow>() || DoNullCheck(); }
...
class HInstanceFieldGet FINAL : public HExpression<1> {
 public:
  ...
  bool CanThrow() const OVERRIDE { return DoNullCheck(); }
  bool NeedsEnvironment() const OVERRIDE { return DoNullCheck(); }
