 ...
 public:
  ...
  void SetType(DataType::Type new_type) {
    ...
    DCHECK(GetType() == new_type ||
           (GetType() == DataType::Type::kInt32 && new_type == DataType::Type::kFloat) ||
           (GetType() == DataType::Type::kInt32 && new_type == DataType::Type::kReference) ||
           (GetType() == DataType::Type::kInt64 && new_type == DataType::Type::kDouble));
...
class HFloatConstant FINAL : public HConstant {
 ...
 private:
  explicit HFloatConstant(float value, uint32_t dex_pc = kNoDexPc)
      : HConstant(DataType::Type::kFloat, dex_pc), value_(value) {}
  explicit HFloatConstant(int32_t value, uint32_t dex_pc = kNoDexPc)
      : HConstant(DataType::Type::kFloat, dex_pc), value_(bit_cast<float, int32_t>(value)) {}
...
class HDoubleConstant FINAL : public HConstant {
 ...
 private:
  explicit HDoubleConstant(double value, uint32_t dex_pc = kNoDexPc)
      : HConstant(DataType::Type::kDouble, dex_pc), value_(value) {}
  explicit HDoubleConstant(int64_t value, uint32_t dex_pc = kNoDexPc)
      : HConstant(DataType::Type::kDouble, dex_pc), value_(bit_cast<double, int64_t>(value)) {}
...
class HCondition : public HBinaryOperation {
 public:
  HCondition(HInstruction* first, HInstruction* second, uint32_t dex_pc = kNoDexPc)
      : HBinaryOperation(DataType::Type::kBoolean, first, second, SideEffects::None(), dex_pc) {
...
class HBooleanNot FINAL : public HUnaryOperation {
 public:
  explicit HBooleanNot(HInstruction* input, uint32_t dex_pc = kNoDexPc)
      : HUnaryOperation(DataType::Type::kBoolean, input, dex_pc) {}
...
class HTypeConversion FINAL : public HExpression<1> {
 public:
  ...
  HTypeConversion(DataType::Type result_type, HInstruction* input, uint32_t dex_pc)
    ...
    DCHECK_NE(DataType::Type::kBoolean, result_type);
  }
...
class HArraySet FINAL : public HTemplateInstruction<3> {
 public:
  ...
  DataType::Type GetComponentType() const {
    ...
    return ((value_type == DataType::Type::kFloat) || (value_type == DataType::Type::kDouble))
        ? value_type
...
class HInstanceOf FINAL : public HExpression<2> {
 public:
  HInstanceOf(HInstruction* object,
              ...
              uint32_t dex_pc)
      : HExpression(DataType::Type::kBoolean,
