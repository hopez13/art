...
class HConstructorFence FINAL : public HVariableInputSizeInstruction {
 ...
 public:
  ...
  HConstructorFence(HInstruction* fence_object,
    ...
    //
    // * If there are only stores to primitive final fields in the constructor:
    // ----> "Reads of type Xs" is the SideEffects, where Xs are all set of all types of the
    //       non-reference final field stores.
    ...
    // we can refine the side effect to the a smaller set of type reads (see above constraints).
