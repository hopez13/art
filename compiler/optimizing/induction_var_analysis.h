...
class HInductionVarAnalysis : public HOptimization {
 ...
 private:
  ...
  // Stating with initial_phi, it calculates how many loop header phis in a row we have. To do this,
  // we count the loop header phi which are used as an input of other loop header phis. If we have
  // more than 15 different loop header phis in a row, we don't perform the analysis.
  // It uses `cached_values` to avoid recomputing results.
