...
void GraphChecker::CheckWriteBarrier(
    ...
    HInstruction* value,
    std::function<bool(HInstruction*)> is_cast,
    std::function<WriteBarrierKind(HInstruction*)> get_write_barrier_kind) {
  ...
  // Reuse `found` for the B) check
  found = false;
  for (; !it.Done(); it.Advance()) {
    if (is_cast(it.Current()) &&
