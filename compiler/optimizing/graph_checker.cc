...
void GraphChecker::HandleTypeCheckInstruction(HTypeCheckInstruction* check) {
  ...
  if (check->GetTypeCheckKind() == TypeCheckKind::kBitstringCheck) {
    ...
    CheckTypeCheckBitstringInput(check, 2, check_values, expected_path_to_root, "path_to_root");
    CheckTypeCheckBitstringInput(check, 3, check_values, expected_mask, "mask");
  } else {
