  ...
  if (instruction->GetWriteBarrierKind() == WriteBarrierKind::kDontEmit) {
    ...
    for (; !it.Done(); it.Advance()) {
      ...
      if (arr == HuntForOriginalReference(it.Current()->AsArraySet()->InputAt(0)) &&
          it.Current()->AsArraySet()->GetWriteBarrierKind() == WriteBarrierKind::kEmitNoNullCheck) {
        found = true;
        break;
      }
  ...
  if (instruction->GetWriteBarrierKind() == WriteBarrierKind::kDontEmit) {
    ...
    for (; !it.Done(); it.Advance()) {
      ...
      if (obj == HuntForOriginalReference(it.Current()->AsInstanceFieldSet()->InputAt(0)) &&
          it.Current()->AsInstanceFieldSet()->GetWriteBarrierKind() == WriteBarrierKind::kEmitNoNullCheck) {
        found = true;
        break;
      }
    }
  ...
  if (instruction->GetWriteBarrierKind() == WriteBarrierKind::kDontEmit) {
    ...
    for (; !it.Done(); it.Advance()) {
      ...
      if (cls == HuntForOriginalReference(it.Current()->AsStaticFieldSet()->InputAt(0)) &&
          it.Current()->AsStaticFieldSet()->GetWriteBarrierKind() == WriteBarrierKind::kEmitNoNullCheck) {
        found = true;
        break;
      }
    }
