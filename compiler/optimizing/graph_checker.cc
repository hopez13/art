...
void GraphChecker::CheckWriteBarrier(HInstruction* instruction,
  ...
  auto is_cast = [instruction](HInstruction* it_instr) {
    if (instruction->IsStaticFieldSet()) {
      return it_instr->IsStaticFieldSet();
    } else if (instruction->IsInstanceFieldSet()) {
      return it_instr->IsInstanceFieldSet();
    } else {
      return it_instr->IsArraySet();
    }
  };

  auto get_write_barrier_kind = [instruction](HInstruction* it_instr) {
    if (instruction->IsStaticFieldSet()) {
