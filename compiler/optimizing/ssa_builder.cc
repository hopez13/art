...
void SsaBuilder::ReplaceUninitializedStringPhis() {
  ScopedArenaHashSet<HInstruction*> seen_instructions(local_allocator_->Adapter(kArenaAllocGraphBuilder));
  ...
  for (const auto& pair : uninitialized_string_phis_) {
    ...
    do {
      ...
      if (current->IsNewInstance()) {
        ...
        // The verifier prevents DEX uses of the uninitialized string.
