...
static bool TypePhiFromInputs(HPhi* phi) {
  ...
  for (HInstruction* input : phi->GetInputs()) {
    ...
    if (common_type == input_type) {
      ...
      DCHECK((common_type == DataType::Type::kFloat && input_type == DataType::Type::kReference) ||
             (common_type == DataType::Type::kReference && input_type == DataType::Type::kFloat));
      return false;
    }
  }
...
static HArrayGet* CreateFloatOrDoubleEquivalentOfArrayGet(HArrayGet* aget) {
  ...
  HArrayGet* equivalent = new (aget->GetBlock()->GetGraph()->GetArena()) HArrayGet(
      aget->GetArray(),
      aget->GetIndex(),
      type == DataType::Type::kInt32 ? DataType::Type::kFloat : DataType::Type::kDouble,
      aget->GetDexPc());
