...
class HDeadCodeElimination : public HOptimization {
 ...
 private:
  ...
  struct TryBelongingInformation {
    ...
    std::set<HBasicBlock*> blocks_in_try;
    // Which other try entries are referencing this same try.
    std::set<HBasicBlock*> coalesced_try_entries;
  };
  ...
  // Returns true iff the try doens't contain throwing instructions.
  bool CanPerformTryBoundaryRemoval(const TryBelongingInformation& try_belonging_info);
  // Removes the try by disconnecting all try entries and exits from their handlers. Also updates
  // the graph in the case that an `TryBoundary` instruction of kind `exit` has the Exit block as
  // its successor.
