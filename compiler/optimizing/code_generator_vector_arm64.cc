  ...
  switch (instruction->GetPackedType()) {
    case Primitive::kPrimChar:
      ...
      if (mirror::kUseStringCompression && instruction->IsStringCharAt()) {
        ...
        temps.Release(length);
        // Zero extend 8 compressed bytes into 8 chars.
        __ Ldr(DRegisterFrom(locations->Out()).V8B(),
               VecAddress(instruction, &temps, 1, /*is_string_char_at*/ true));
        __ Uxtl(reg.V8H(), reg.V8B());
        __ B(&done);
        // Load 8 direct uncompressed chars.
        __ Bind(&uncompressed_load);
        __ Ldr(reg, VecAddress(instruction, &temps, size, /*is_string_char_at*/ true));
        __ Bind(&done);
        return;
      }
...
void InstructionCodeGeneratorARM64::VisitVecStore(HVecStore* instruction) {
  ...
  switch (instruction->GetPackedType()) {
    ...
    case Primitive::kPrimDouble:
      DCHECK_LE(2u, instruction->GetVectorLength());
      DCHECK_LE(instruction->GetVectorLength(), 16u);
      __ Str(reg, VecAddress(instruction, &temps, size));
      break;
    default:
      LOG(FATAL) << "Unsupported SIMD type";
      UNREACHABLE();
