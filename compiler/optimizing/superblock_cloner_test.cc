...
TEST_F(SuperblockClonerTest, CloneBasicBlocks) {
  ...
  SuperblockCloner cloner(graph_,
                          ...
                          &bb_map,
                          &hir_map);
  EXPECT_TRUE(cloner.IsSubgraphCopyable());
...
TEST_F(SuperblockClonerTest, LoopPeeling) {
  ...
  EXPECT_TRUE(helper.IsLoopCopyable());
...
// Check that loop unrolling works fine for a loop with multiple back edges. Test that after
