...
class SsaLivenessAnalysisTest : public OptimizingUnitTest {
 public:
  SsaLivenessAnalysisTest()
    ...
    // Create entry block.
    entry_ = new (GetArena()) HBasicBlock(graph_);
 ...
 protected:
  HBasicBlock* CreateSuccessor(HBasicBlock* block) {
    ...
    HBasicBlock* successor = new (GetArena()) HBasicBlock(graph);
...
TEST_F(SsaLivenessAnalysisTest, TestReturnArg) {
  HInstruction* arg = new (GetArena()) HParameterValue(
  ...
  HInstruction* ret = new (GetArena()) HReturn(arg);
  block->AddInstruction(ret);
  block->AddInstruction(new (GetArena()) HExit());

  graph_->BuildDominatorTree();
  SsaLivenessAnalysis ssa_analysis(graph_, codegen_.get(), GetScopedArena());
...
TEST_F(SsaLivenessAnalysisTest, TestAput) {
  HInstruction* array = new (GetArena()) HParameterValue(
      graph_->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kReference);
  HInstruction* index = new (GetArena()) HParameterValue(
      graph_->GetDexFile(), dex::TypeIndex(1), 1, DataType::Type::kInt32);
  HInstruction* value = new (GetArena()) HParameterValue(
      graph_->GetDexFile(), dex::TypeIndex(2), 2, DataType::Type::kInt32);
  HInstruction* extra_arg1 = new (GetArena()) HParameterValue(
      graph_->GetDexFile(), dex::TypeIndex(3), 3, DataType::Type::kInt32);
  HInstruction* extra_arg2 = new (GetArena()) HParameterValue(
  ...
  HBasicBlock* block = CreateSuccessor(entry_);
  HInstruction* null_check = new (GetArena()) HNullCheck(array, 0);
  block->AddInstruction(null_check);
  HEnvironment* null_check_env = new (GetArena()) HEnvironment(GetArena(),
                                                               /* number_of_vregs */ 5,
                                                               /* method */ nullptr,
                                                               /* dex_pc */ 0u,
                                                               null_check);
  ...
  HInstruction* length = new (GetArena()) HArrayLength(array, 0);
  block->AddInstruction(length);
  HInstruction* bounds_check = new (GetArena()) HBoundsCheck(index, length, /* dex_pc */ 0u);
  block->AddInstruction(bounds_check);
  HEnvironment* bounds_check_env = new (GetArena()) HEnvironment(GetArena(),
                                                                 /* number_of_vregs */ 5,
                                                                 /* method */ nullptr,
                                                                 /* dex_pc */ 0u,
                                                                 bounds_check);
  ...
  HInstruction* array_set =
      new (GetArena()) HArraySet(array, index, value, DataType::Type::kInt32, /* dex_pc */ 0);
...
TEST_F(SsaLivenessAnalysisTest, TestDeoptimize) {
  ...
  HInstruction* ae = new (GetArena()) HAboveOrEqual(index, length);
  block->AddInstruction(ae);
  HInstruction* deoptimize =
      new(GetArena()) HDeoptimize(GetArena(), ae, DeoptimizationKind::kBlockBCE, /* dex_pc */ 0u);
  block->AddInstruction(deoptimize);
  HEnvironment* deoptimize_env = new (GetArena()) HEnvironment(GetArena(),
                                                               /* number_of_vregs */ 5,
                                                               /* method */ nullptr,
                                                               /* dex_pc */ 0u,
                                                               deoptimize);
