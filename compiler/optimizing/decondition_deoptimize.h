...
template<typename StorageType>
class RAIIBaseDeoptimizationRemover : public ValueObject {
 public:
  template<typename = std::enable_if<!std::is_same_v<StorageType, UnscopedStorageType>>>
  RAIIBaseDeoptimizationRemover(HGraph* graph, ArenaAllocKind kind) : remover_(graph, kind) {}

  template<typename = std::enable_if<std::is_same_v<StorageType, ScopedStorageType>>>
  RAIIBaseDeoptimizationRemover(HGraph* graph, ScopedArenaAllocator& alloc, ArenaAllocKind kind) : remover_(graph, alloc, kind) {}
  ~RAIIBaseDeoptimizationRemover() {
    remover_.Finalize();
  }

  void AddPredicatedDeoptimization(HDeoptimize* deopt, HInstruction* condition) {
    remover_.AddPredicatedDeoptimization(deopt, condition);
  }
 private:
  BaseDeoptimizationRemover<StorageType> remover_;
};

using RAIIScopedDeoptimizationRemover = RAIIBaseDeoptimizationRemover<ScopedStorageType>;
using RAIIUnscopedDeoptimizationRemover = RAIIBaseDeoptimizationRemover<UnscopedStorageType>;
