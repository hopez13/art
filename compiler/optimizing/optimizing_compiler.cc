...
CodeGenerator* OptimizingCompiler::TryCompile(ArenaAllocator* arena,
                                              ...
                                              bool osr,
                                              VariableSizedHandleScope* handles) const {
  OptimizingCompilerStats::MaybeRecordStat(compilation_stats_.get(),
    ...
    if (result != kAnalysisSuccess) {
      switch (result) {
        case kAnalysisSkipped: {
          OptimizingCompilerStats::MaybeRecordStat(
              compilation_stats_.get(),
...
CompiledMethod* OptimizingCompiler::Compile(const DexFile::CodeItem* code_item,
  ...
  if (compiler_driver->IsMethodVerifiedWithoutFailures(method_idx, class_def_idx, dex_file)
    ...
    OptimizingCompilerStats::MaybeRecordStat(compilation_stats_.get(), method_stat);
  }
