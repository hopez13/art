...
static std::string ToOptimizationName(const std::string& pass_name) {
  size_t pos = pass_name.find(kPassNameSeparator);    // Strip suffix to get optimization name.
...
static HOptimization* BuildOptimization(
  ...
  std::string opt_name = ToOptimizationName(pass_name);
