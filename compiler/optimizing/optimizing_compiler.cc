...
static ScopedArenaVector<uint8_t> CreateJniStackMap(ScopedArenaAllocator* allocator,
  ...
  return stack_map_stream->Encode(allocator);
}
...
CompiledMethod* OptimizingCompiler::JniCompile(uint32_t access_flags,
  ...
  ScopedArenaAllocator allocator2(&arena_stack);  // Will hold the stack map.
  ScopedArenaVector<uint8_t> stack_map = CreateJniStackMap(&allocator2, jni_compiled_method);
...
bool OptimizingCompiler::JitCompile(Thread* self,
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    ScopedArenaAllocator allocator2(&arena_stack);
    ScopedArenaVector<uint8_t> stack_map = CreateJniStackMap(&allocator2, jni_compiled_method);
