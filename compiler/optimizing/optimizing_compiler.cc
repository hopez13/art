  ...
  switch (GetCompilerDriver()->GetInstructionSet()) {
    ...
    case InstructionSet::kArm: {
      OptimizationDef arm_optimizations[] = {
        OptDef(kInstructionSimplifierArm),
        OptDef(kSideEffectsAnalysis),
        OptDef(kGlobalValueNumbering, "GVN$after_arch"),
        OptDef(kScheduling)
      };
      RunOptimizations(graph,
                       codegen,
    ...
    case InstructionSet::kArm64: {
      OptimizationDef arm64_optimizations[] = {
        OptDef(kInstructionSimplifierArm64),
        OptDef(kSideEffectsAnalysis),
        OptDef(kGlobalValueNumbering, "GVN$after_arch"),
        OptDef(kScheduling)
      };
    ...
    case InstructionSet::kMips: {
      OptimizationDef mips_optimizations[] = {
        OptDef(kInstructionSimplifierMips),
        OptDef(kSideEffectsAnalysis),
        OptDef(kGlobalValueNumbering, "GVN$after_arch"),
        OptDef(kPcRelativeFixupsMips)
      };
      RunOptimizations(graph,
                       codegen,
                       dex_compilation_unit,
    ...
    case InstructionSet::kMips64: {
      ...
      RunOptimizations(graph,
                       ...
                       arraysize(mips64_optimizations));
      break;
    }
#endif
    ...
    case InstructionSet::kX86: {
      OptimizationDef x86_optimizations[] = {
        OptDef(kSideEffectsAnalysis),
        OptDef(kGlobalValueNumbering, "GVN$after_arch"),
        OptDef(kPcRelativeFixupsX86),
        OptDef(kX86MemoryOperandGeneration)
      };
      RunOptimizations(graph,
                       codegen,
                       dex_compilation_unit,
    ...
    case InstructionSet::kX86_64: {
      ...
      RunOptimizations(graph,
                       ...
                       arraysize(x86_64_optimizations));
      break;
    }
#endif
    default:
...
void OptimizingCompiler::RunOptimizations(HGraph* graph,
  ...
  OptimizationDef optimizations1[] = {
    OptDef(kIntrinsicsRecognizer),
    OptDef(kSharpening),
    OptDef(kConstantFolding),
    OptDef(kInstructionSimplifier),
    OptDef(kDeadCodeElimination, "dead_code_elimination$initial")
  };
  RunOptimizations(graph,
                   codegen,
                   dex_compilation_unit,
  ...
  OptimizationDef optimizations2[] = {
    ...
    OptDef(kSelectGenerator),
    // TODO: if we don't inline we can also skip fold2.
    OptDef(kConstantFolding,       "constant_folding$after_inlining"),
    OptDef(kInstructionSimplifier, "instruction_simplifier$after_inlining"),
    OptDef(kDeadCodeElimination,   "dead_code_elimination$after_inlining"),
    OptDef(kSideEffectsAnalysis,   "side_effects$before_gvn"),
    OptDef(kGlobalValueNumbering),
    OptDef(kInvariantCodeMotion),
    OptDef(kInductionVarAnalysis),
    OptDef(kBoundsCheckElimination),
    OptDef(kLoopOptimization),
    // Evaluates code generated by dynamic bce.
    OptDef(kConstantFolding,       "constant_folding$after_bce"),
    OptDef(kInstructionSimplifier, "instruction_simplifier$after_bce"),
    OptDef(kSideEffectsAnalysis,   "side_effects$before_lse"),
    OptDef(kLoadStoreAnalysis),
    OptDef(kLoadStoreElimination),
    OptDef(kCHAGuardOptimization),
    OptDef(kDeadCodeElimination,   "dead_code_elimination$final"),
    OptDef(kCodeSinking),
    // The codegen has a few assumptions that only the instruction simplifier
    // can satisfy. For example, the code generator does not expect to see a
    // HTypeConversion from a type to the same type.
    OptDef(kInstructionSimplifier, "instruction_simplifier$before_codegen"),
    // Eliminate constructor fences after code sinking to avoid
    // complicated sinking logic to split a fence with many inputs.
    OptDef(kConstructorFenceRedundancyElimination)
  };
  RunOptimizations(graph,
                   codegen,
                   dex_compilation_unit,
...
CompiledMethod* OptimizingCompiler::Compile(const DexFile::CodeItem* code_item,
  ...
  if (compiler_driver->IsMethodVerifiedWithoutFailures(method_idx, class_def_idx, dex_file) ||
    ...
    if (codegen.get() != nullptr) {
      ...
      method = Emit(&allocator, &code_allocator, codegen.get(), code_item);
