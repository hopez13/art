...
bool HInliner::CanInlineBody(const HGraph* callee_graph,
  ...
  size_t number_of_instructions = 0;
  const bool can_encode_inlined_method_in_stack_map =
      CanEncodeInlinedMethodInStackMap(*caller_compilation_unit_.GetDexFile(), resolved_method);
  ...
  for (HBasicBlock* block : callee_graph->GetReversePostOrderSkipEntryBlock()) {
    ...
    for (HInstructionIterator instr_it(block->GetInstructions());
      ...
      if (current->NeedsEnvironment()) {
        ...
        if (!can_encode_inlined_method_in_stack_map) {
