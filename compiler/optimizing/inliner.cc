...
static dex::TypeIndex FindClassIndexIn(mirror::Class* cls,
  ...
  if (cls->GetDexCache() == nullptr) {
    ...
    if (index.IsValid()) {
      ObjPtr<mirror::DexCache> dex_cache = compilation_unit.GetDexCache().Get();
      ObjPtr<mirror::Class> resolved = dex_cache->GetResolvedType(index);
      if (resolved == nullptr) {
        resolved = Runtime::Current()->GetClassLinker()->LookupResolvedType(
            dex_file, index, dex_cache, compilation_unit.GetClassLoader().Get());
      }
      if (resolved != cls) {
