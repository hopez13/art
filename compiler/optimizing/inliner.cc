...
HInliner::InlineCacheType HInliner::GetInlineCacheAOT(
  ...
  for (const dex::TypeIndex& type_index : dex_pc_data.classes) {
    // TODO: Why are we doing a lookup when the comment above says "resolve"?
    uint32_t num_type_ids = caller_compilation_unit_.GetDexFile()->NumTypeIds();
    ...
    if (type_index.index_ < num_type_ids) {
      ObjPtr<mirror::DexCache> dex_cache = caller_compilation_unit_.GetDexCache().Get();
      clazz = class_linker->LookupResolvedType(type_index, dex_cache, class_loader);
      if (clazz == nullptr) {
        VLOG(compiler) << "Could not resolve class from inline cache in AOT mode "
            << invoke_instruction->GetMethodReference().PrettyMethod()
            << " : "
            << caller_compilation_unit_.GetDexFile()->StringByTypeIdx(type_index);
        return kInlineCacheMissingTypes;
      }
    } else {
      const std::string& descriptor = pci->GetExtraString(type_index.index_ - num_type_ids);
      clazz = class_linker->LookupResolvedType(descriptor.c_str(), class_loader);
      if (clazz == nullptr) {
        VLOG(compiler) << "Could not resolve class from inline cache in AOT mode "
            << invoke_instruction->GetMethodReference().PrettyMethod()
            << " : "
