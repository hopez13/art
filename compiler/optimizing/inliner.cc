...
art::ArtMethod* HInliner::FindActualCallTarget(HInvoke* invoke_instruction,
...
bool HInliner::TryInline(HInvoke* invoke_instruction) {
  ...
  art::ArtMethod* actual_method = FindActualCallTarget(invoke_instruction, &cha_devirtualize);
...
static inline bool MayInline(const CompilerOptions& compiler_options, const DexFile& inlined_from,
...
bool HInliner::IsInliningAllowed(art::ArtMethod* method, const CodeItemDataAccessor& accessor) const {
  if (!accessor.HasCodeItem()) {
    LOG_FAIL_NO_STAT()
...
bool HInliner::IsInliningSupported(const HInvoke* invoke_instruction, art::ArtMethod* method,
...
bool HInliner::IsInliningBudgetAvailable(art::ArtMethod* method,
                                         const CodeItemDataAccessor& accessor) const {
...
void HInliner::SubstituteArguments(HGraph* callee_graph, const HInvoke* invoke_instruction,
                                   ReferenceTypeInfo receiver_type,
...
bool HInliner::CanInlineBody(const HGraph* callee_graph, const HBasicBlock* target_block,
  ...
  const bool same_dex_file = IsSameDexFile(*outer_compilation_unit_.GetDexFile(), *resolved_method->GetDexFile());
...
bool HInliner::ArgumentTypesMoreSpecific(const HInvoke* invoke_instruction,
                                         ArtMethod* resolved_method) {
