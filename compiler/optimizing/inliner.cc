...
bool HInliner::TryInlinePolymorphicCall(
  ...
  for (size_t i = 0; i != number_of_types; ++i) {
    ...
    const bool too_many_polymorphic_recursive_calls = CountRecursiveCallsOf(method) > kMaximumNumberOfPolymorphicRecursiveCalls;
    if (too_many_polymorphic_recursive_calls) {
      LOG_FAIL(stats_, MethodCompilationStat::kNotInlinedPolymorphicRecursiveBudget)
          << "Method " << method->PrettyMethod()
          << " will not be inlined due to having too many polymorphic recursive calls";
    }
