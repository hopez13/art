  ...
  if (return_replacement != nullptr) {
    if (return_replacement->GetType() == DataType::Type::kReference) {
      if (!return_replacement->GetReferenceTypeInfo().IsValid()) {
        ...
        bool admissible = ReferenceTypePropagation::IsAdmissible(cls);
        ReferenceTypeInfo rti = ReferenceTypeInfo::Create(
            handles_->NewHandle(admissible ? cls : GetClassRoot<mirror::Object>()),
            /*is_exact=*/ admissible && cls->CannotBeAssignedFromOtherTypes());
