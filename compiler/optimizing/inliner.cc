...
bool HInliner::TryInline(HInvoke* invoke_instruction) {
  ...
  // We store the value to restore it since we will use the same HInliner for other inlinee
  // candidates.
...
bool HInliner::IsInliningSupported(const HInvoke* invoke_instruction,
  ...
  if (accessor.TriesSize() != 0) {
    ...
    if (inlined_into_try_catch) {
      LOG_FAIL(stats_, MethodCompilationStat::kNotInlinedTryCatchCallerCallee)
          << "Method " << method->PrettyMethod()
          << " is not inlined because it has a try catch and it would be inlined inside another"
          << " try catch.";
      return false;
    }
  }
