...
bool HInliner::UseOnlyPolymorphicInliningWithNoDeopt() {
  ...
  //
  // For AOT: Generating a deopt does not ensure that we will actually capture the new types;
  //          and the danger is that we could be stuck in a loop with "forever" deoptimizations.
  //          For example, if the method never becomes hot again, the inline cache will not be
  //          updated and the AOT code will keep deoptimizing.
  // TODO(calin): This is compromise because we will most likely never update the inline cache
  //              (unless there's another reason to deopt). So we might be stuck with
  ...
  //
  // For OSR: We may come from the interpreter and it may have seen different receiver types.
