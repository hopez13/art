...
bool HInliner::CanInlineBody(const HGraph* callee_graph,
  ...
  for (HBasicBlock* block : callee_graph->GetReversePostOrderSkipEntryBlock()) {
    ...
    for (HInstructionIterator instr_it(block->GetInstructions());
      ...
      // We can't inline across dex files on AoT when we need an environment if an instruction
      // requires to add bss info.
      if (at_least_one_needs_environment && at_least_one_needs_bss && needs_bss_check) {
        // TODO(solanes, 154012332): Add support for BCP dex file .bss entries.
