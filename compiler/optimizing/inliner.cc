...
HInliner::InlineCacheType HInliner::GetInlineCacheType(
  ...
  if (number_of_types == 0) return kInlineCacheUnitialized;
  if (number_of_types == 1) return kInlineCacheMonomorphic;
  if (number_of_types == InlineCache::kIndividualCacheSize) return kInlineCacheMegamorhpic;
  return kInlineCachePolymorhpic;
}
...
bool HInliner::TryInlineFromInlineCache(const DexFile& caller_dex_file,
  ...
  switch (inline_cache_type) {
    ...
    case kInlineCachePolymorhpic:
      MaybeRecordStat(kPolymorphicCall);
      return TryInlinePolymorphicCall(invoke_instruction, resolved_method, inline_cache);

    case kInlineCacheMegamorhpic:
      VLOG(compiler) << "Interface or virtual call to "
                     << caller_dex_file.PrettyMethod(invoke_instruction->GetDexMethodIndex())
...
HInliner::InlineCacheType HInliner::ExtractClassesFromOfflineProfile(
  ...
  if (it == offline_profile.inline_caches.end()) {
    return kInlineCacheUnitialized;
  }

  const ProfileCompilationInfo::DexPcData& dex_pc_data = it->second;

  if (dex_pc_data.is_missing_types) return kInlineCacheMissingTypes;
  if (dex_pc_data.is_megamorphic) return kInlineCacheMegamorhpic;
