...
bool HInliner::TryBuildAndInlineHelper(HInvoke* invoke_instruction,
  ...
  bool has_non_inlineable_throw_predecessor = false;
  ...
  for (HBasicBlock* predecessor : exit_block->GetPredecessors()) {
    if (predecessor->GetLastInstruction()->IsThrow()) {
      if (invoke_instruction->GetBlock()->IsTryBlock()) {
        ...
        has_non_inlineable_throw_predecessor = true;
        break;
      }
    } else {
      has_one_return = true;
  ...
  if (has_non_inlineable_throw_predecessor) {
    VLOG(compiler) << "Method " << callee_dex_file.PrettyMethod(method_index)
                   << " could not be inlined because one branch always throws and"
                   << " caller is in a loop or try/catch block";
    return false;
  }
