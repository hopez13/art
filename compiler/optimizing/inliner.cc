...
#define LOG_TRY() VLOG(compiler) << DepthString(__LINE__) << "Try inlinining call: "
#define LOG_NOTE() VLOG(compiler) << DepthString(__LINE__) << "Note: "
#define LOG_SUCCESS() VLOG(compiler) << DepthString(__LINE__) << "Success: "
#define LOG_FAIL(stat) MaybeRecordStat(stat); VLOG(compiler) << DepthString(__LINE__) << "Fail: "
#define LOG_FAIL_NO_STAT() VLOG(compiler) << DepthString(__LINE__) << "Fail: "

std::string HInliner::DepthString(int line) const {
  std::string value;
  // Indent according to the inlining depth.
  size_t count = depth_;
  // Lines get printed in the log, so add a space if the log's line is less
  // than 1000.
  if (!kIsTargetBuild) {
    if (line < 100) {
      value += " ";
...
bool HInliner::TryInline(HInvoke* invoke_instruction) {
  ...
  if (profiling_info == nullptr) {
    LOG_FAIL_NO_STAT() << "No profiling info for " << caller->PrettyMethod();
    return false;
  }
  ...
  if (inline_cache == nullptr) {
    ...
    LOG_FAIL_NO_STAT() << "Out of memory in the compiler when trying to inline";
    return false;
  }
