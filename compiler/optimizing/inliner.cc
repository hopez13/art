...
bool HInliner::TryInlineAndReplace(HInvoke* invoke_instruction,
                                   ...
                                   bool do_rtp,
                                   bool speculative_inline) {
  DCHECK(!invoke_instruction->IsIntrinsic());
  HInstruction* return_replacement = nullptr;

  if (!TryBuildAndInline(
          invoke_instruction, method, receiver_type, &return_replacement, speculative_inline)) {
    return false;
  }
...
bool HInliner::TryBuildAndInlineHelper(HInvoke* invoke_instruction,
  ...
  size_t number_of_instructions = 0;
  if (!CanInlineBody(
          callee_graph, invoke_instruction, &number_of_instructions, speculative_inline)) {
    return false;
  }
