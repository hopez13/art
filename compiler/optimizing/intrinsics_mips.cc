...
void IntrinsicCodeGeneratorMIPS::VisitIntegerValueOf(HInvoke* invoke) {
  ...
  if (invoke->InputAt(0)->IsConstant()) {
    ...
    if (value >= info.low && value <= info.high) {
      ...
      __ LoadConst32(out, static_cast<int32_t>(address));
    } else {
      // Allocate and initialize a new j.l.Integer.
      // TODO: If we JIT, we could allocate the j.l.Integer now, and store it in the
      // JIT object table.
      Register argument = Register(calling_convention.GetRegisterAt(0));
      uint32_t address =
          dchecked_integral_cast<uint32_t>(reinterpret_cast<uintptr_t>(info.integer));
      __ LoadConst32(argument, static_cast<int32_t>(address));
      codegen_->InvokeRuntime(kQuickAllocObjectInitialized, invoke, invoke->GetDexPc());
      CheckEntrypointTypes<kQuickAllocObjectWithChecks, void*, mirror::Class*>();
      __ LoadConst32(AT, value);
      __ Sw(AT, out, info.value_offset);
