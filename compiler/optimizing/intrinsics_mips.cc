...
static void CheckPosition(MipsAssembler* assembler,
  ...
  if (length.IsConstant()) {
    ...
    __ Sltu(AT, AT, length.AsRegister<Register>());
  }
  __ Bnez(AT, slow_path->GetEntryLabel());
}
...
void IntrinsicCodeGeneratorMIPS::VisitSystemArrayCopyChar(HInvoke* invoke) {
  ...
  if (srcPos.IsConstant()) {
    ...
    if (IsInt<16>((int32_t)(data_offset + char_size*srcPos_const))) {
      __ Addiu(src_base, src, data_offset + char_size*srcPos_const);
    } else {
      __ LoadConst32(AT, data_offset + char_size*srcPos_const);
      __ Addu(src_base, src, AT);
    }
  } else {
    if (IsInt<16>((int32_t)data_offset)) {
      __ Addiu(TMP, src, data_offset);
    } else {
      __ LoadConst32(AT, data_offset);
      __ Addu(TMP, src, AT);
    }
    if (IsR6()) {
      __ Lsa(src_base, srcPos.AsRegister<Register>(), TMP, char_shift);
    } else {
      __ Sll(AT, srcPos.AsRegister<Register>(), char_shift);
      __ Addu(src_base, TMP, AT);
    }
  }
  if (destPos.IsConstant()) {
    ...
    if (IsInt<16>((int32_t)(data_offset + char_size*destPos_const))) {
      __ Addiu(dest_base, dest, data_offset + char_size*destPos_const);
    } else {
      __ LoadConst32(AT, data_offset + char_size*destPos_const);
      __ Addu(dest_base, dest, AT);
    }
  } else {
    if (IsInt<16>((int32_t)data_offset)) {
      __ Addiu(dest_base, dest, data_offset);
    } else {
      __ LoadConst32(AT, data_offset);
      __ Addu(dest_base, dest, AT);
    }
    if (IsR6()) {
      __ Lsa(dest_base, destPos.AsRegister<Register>(), dest_base, char_shift);
    } else {
      __ Sll(AT, destPos.AsRegister<Register>(), char_shift);
