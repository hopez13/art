...
bool SchedulingGraph::HasSideEffectDependency(const HInstruction* node,
  ...
  if (node->GetSideEffects().MayHaveReorderingDependency(other->GetSideEffects())) {
    return true;
             ...
             (node->IsArrayGet() && other->IsBoundsCheck() &&
              node->InputAt(1) == other)) {
    return true;
  }
  return false;
}
...
void HScheduler::Schedule(HBasicBlock* block) {
  ArenaVector<SchedulingNode*> scheduling_nodes(arena_->Adapter(kArenaAllocScheduling));
  ...
  ArenaVector<SchedulingNode*> initial_candidates(arena_->Adapter(kArenaAllocScheduling));
...
void HScheduler::Schedule(HInstruction* instruction) {
  if (instruction == cursor_) {
    ...
    instruction->LocalMoveAfter(cursor_);
  }
}
