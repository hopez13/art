...
bool SchedulingGraph::HasImmediateDataDependency(const HInstruction* instruction,
  ...
  if (node == nullptr || other == nullptr) {
    // Both instructinos must be in current basic block, i.e. the SchedulingGraph can see their
    // corresponding SchedulingNode in the graph, and tell whether there is dependency.
...
bool HScheduler::IsSchedulable(const HBasicBlock* block) const {
  ...
  if (block->GetTryCatchInformation() != nullptr) {
    // Because the scheduler may aggressively reorder instructions that operate on local variables.
    // If the exception handling code for current method has strong requirements on the program
    // order of to local variables, the scheduler cannot guarantee that.
    // In following example, MulitiplyAccumulate may be scheduled before div-by-zero exception.
    //   try {
    //     a = a/b;    // DivZeroCheck; Div
