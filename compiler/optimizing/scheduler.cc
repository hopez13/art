...
bool SideEffectAnalysis::HasReorderingDependency(const HInstruction* instr1,
...
size_t SideEffectAnalysis::MemoryDependencyAnalysis::ArrayAccessHeapLocation(
...
bool SideEffectAnalysis::MemoryDependencyAnalysis::ArrayAccessMayAlias(HInstruction* instr1,
    HInstruction* instr2) const {
  DCHECK(heap_location_collector_ != nullptr);
...
size_t SideEffectAnalysis::MemoryDependencyAnalysis::FieldAccessHeapLocation(
...
bool SideEffectAnalysis::MemoryDependencyAnalysis::FieldAccessMayAlias(const HInstruction* instr1,
    const HInstruction* instr2) const {
  DCHECK(heap_location_collector_ != nullptr);
...
bool SideEffectAnalysis::MemoryDependencyAnalysis::HasMemoryDependency(HInstruction* instr1,
    HInstruction* instr2) const {
...
bool SideEffectAnalysis::HasExceptionDependency(const HInstruction* instr1,
                                                const HInstruction* instr2) {
...
void SchedulingGraph::AddDependencies(SchedulingNode* instruction_node,
  ...
  if (!instruction->GetSideEffects().DoesNothing() || instruction->CanThrow()) {
    ...
    for (HInstruction* other = instruction->GetNext(); other != nullptr; other = other->GetNext()) {
      ...
      if (side_effect_analysis_.HasSideEffectDependency(other, instruction)) {
        if (dep_chain_candidate != nullptr &&
            side_effect_analysis_.HasSideEffectDependency(other, dep_chain_candidate)) {
