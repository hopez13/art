...
void PrepareForRegisterAllocation::VisitConstructorFence(HConstructorFence* constructor_fence) {
  // Trivially remove redundant HConstructorFence when it immediately follow an HNewInstance
  ...
  // had introduced.
  HInstruction* previous = constructor_fence->GetPrevious();
  if (previous != nullptr && previous->IsNewInstance()) {
    ...
    if (previous->AsNewInstance()->GetEntrypoint() == kQuickAllocObjectResolved) {
      ...
      return;
      // TODO: actually remove the dmb from initialized variants only.
    }
    ...
    //
    // TODO: remove all the dmb from array resolved entrypoints,
    // and consider renaming them to array initializing because it's confusing.
  }
