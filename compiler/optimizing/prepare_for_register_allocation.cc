...
void PrepareForRegisterAllocation::VisitClinitCheck(HClinitCheck* check) {
  ...
  const bool is_initialized = load_class->IsInitialized();
  bool can_merge_with_load_class = is_initialized || CanMoveClinitCheck(load_class, check);
  ...
  if (implicit_clinit != nullptr) {
    ...
    // If already initialized, we don't need to generate a Clinit check.
    if (!is_initialized) {
