...
bool TryCombineAndNot(HNot* instruction) {
  ...
  if (instruction->HasOnlyOneNonEnvironmentUse()) {
    ...
    if (left->IsNot() ^ right->IsNot()) {
      ...
      HX86AndNot* and_not = new (arena) HX86AndNot(type, instruction->GetInput(), other_ins, instruction->GetDexPc());
  ...
  if (instruction->HasOnlyOneNonEnvironmentUse()) {
    ...
    if (use->IsAnd()) {
      ...
      if (left->IsNeg() ^ right->IsNeg()) {
        ...
        if (input == other_ins) {
          HX86AndNeg* and_neg = new (arena) HX86AndNeg(type, instruction->GetInput(), instruction->GetDexPc());
  ...
  if (cst_input->IsMinusOne() && instruction->HasOnlyOneNonEnvironmentUse()) {
    ...
    if (use->IsAnd()|| use->IsXor()) {
      ...
      if (left == other_input || right == other_input) {
        ...
        HX86BitwiseAddRight* add_right = new (arena) HX86BitwiseAddRight(type, kind, other_input, instruction->GetDexPc());
