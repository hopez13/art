...
#undef OPTIMIZING_INTRINSICS
  ...
  struct IntegerValueOfInfo {
    ...
    uint32_t integer_boot_image_offset;  // Set to 0u when compiling the boot image.
    ...
    union {
      ...
      // is out of range (use `low` and `length` to check), this value is bogus (set to 0)
      // and the code must allocate a new Integer.
