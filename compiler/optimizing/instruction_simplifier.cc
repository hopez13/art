...
static bool TryReplaceStringBuilderAppend(HInvoke* invoke) {
  ...
  // `NewInstance` and `StringBuilder.toString()`.
  std::vector<HInstruction*> instructions_in_the_middle;
  for (HBackwardInstructionIterator iter(block->GetInstructions()); !iter.Done(); iter.Advance()) {
    ...
    if (user->InputCount() == 0u || user->InputAt(0u) != sb) {
      // Only consider instructions between `NewInstance` and `StringBuilder.toString()`.
      if (seen_to_string && !seen_constructor_fence) {
        instructions_in_the_middle.push_back(user);
      }
      continue;
    }
  ...
  for (const HUseListNode<HEnvironment*>& use : sb->GetEnvUses()) {
    ...
    // If the environment use doesn't happen in between the string creation and finalization, we can
    // safely ignore it.
    if (std::find(instructions_in_the_middle.begin(), instructions_in_the_middle.end(), holder) ==
        instructions_in_the_middle.end()) {
      continue;
    }
    return false;
  }
