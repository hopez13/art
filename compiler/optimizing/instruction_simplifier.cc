...
void InstructionSimplifierVisitor::SimplifyVarHandleIntrinsic(HInvoke* invoke) {
  ...
  if (!compiler_options.IsAotCompiler() || expected_coordinates_count == 2u) {
    return;
  }
  ...
  if (var_handle_object == nullptr) {
    return;
  }
  if (var_handle_object->GetClass() != expected_var_handle_class) {
    return;
  }
  ...
  // All required checks passed. Update flags.
