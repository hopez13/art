...
// TODO(solanes): Do we have to take care of the fact that the invoke might be inlined? We had a
// similar situation in StringBuilderAppend in which a StringBuilder constructor
// (InvokeStaticOrDirect) turned into a NewArray + InstanceFieldSet Helper method for StringBuffer
// escape analysis.
...
static bool TryReplaceStringBuilderAppend(HInvoke* invoke) {
  ...
  for (HBackwardInstructionIterator iter(block->GetInstructions()); !iter.Done(); iter.Advance()) {
    ...
    if (user->IsInvokeVirtual()) {
      ...
      // "java.lang.AbstractStringBuilder.value" is the field name?
      // After arguments, we should see the constructor. It could be an invoke, or it could be
      // inlined in the method. We accept only the constructor with no extra arguments. This means
      // that if we inline it, we have to check it is setting it's field to a new array.
      DCHECK(!seen_constructor);
      DCHECK(!seen_constructor_fence);
