...
static bool TryReplaceStringBuilderAppend(HInvoke* invoke) {
  ...
  if (constructor_inlined) {
    ...
    HInstruction* new_arr = sb->GetEnvUses().front().GetUser()->GetHolder();
    DCHECK(new_arr->IsNewArray());
    DCHECK(new_arr->HasNonEnvironmentUses());
    HInstruction* fence = new_arr->GetUses().front().GetUser();
    DCHECK(fence->IsConstructorFence());
    block->RemoveInstruction(fence);
    // Also remove NewArray's inputs.
    HInstruction* input_0 = new_arr->InputAt(0);
    HInstruction* input_1 = new_arr->InputAt(1);
    block->RemoveInstruction(new_arr);
    block->RemoveInstruction(input_0);
    block->RemoveInstruction(input_1);
  } else {
    DCHECK(!sb->HasEnvironmentUses());
  }
