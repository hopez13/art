...
class InstructionSimplifierVisitor : public HGraphDelegateVisitor {
 ...
 private:
  ...
  void SimplifyNewIntrinsic(HInvoke* invoke);
...
void InstructionSimplifierVisitor::SimplifyNewIntrinsic(HInvoke* invoke) {
  if (!invoke->HasUses()) {
    ...
    HInstruction* receiver = invoke->InputAt(0);
    NoEscapeFunction no_escape = NoEscapeForStringBufferReference;
    if (!is_synchronized || IsNonEscapingSingleton(receiver, &no_escape)) {
...
void InstructionSimplifierVisitor::VisitInvoke(HInvoke* instruction) {
  switch (instruction->GetIntrinsic()) {
    ...
    case Intrinsics::kStringBuilderToString:
      SimplifyNewIntrinsic(instruction);
      break;
