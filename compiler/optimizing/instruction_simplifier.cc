...
bool InstructionSimplifierVisitor::TryCombineVecMultiplyAccumulate(HVecMul* mul) {
  ...
  HVecOperation::PredicationKind binop_predicate_kind;
  if (predicated_simd) {
    binop_predicate_kind = vec_binop->GetPredicationKind();
    binop_predicate = vec_binop->GetGoverningPredicate();
    if (mul->GetGoverningPredicate() != binop_predicate ||
        mul->GetPredicationKind() != binop_predicate_kind) {
  ...
  if (predicated_simd) {
    mulacc->SetGoverningPredicate(binop_predicate, binop_predicate_kind);
  }
