...
static bool CmpHasBoolType(HInstruction* input, HInstruction* cmp) {
  if (input->GetType() == DataType::Type::kBoolean) {
...
void InstructionSimplifierVisitor::VisitSelect(HSelect* select) {
  ...
  if (true_value == false_value) {
    ...
    if (t_type == f_type && (t_type == DataType::Type::kInt32 || t_type == DataType::Type::kInt64)) {
...
static bool IsTypeConversionImplicit(DataType::Type input_type, DataType::Type result_type) {
  ...
  return result_type == input_type ||
      (result_type == DataType::Type::kInt32 && (input_type == DataType::Type::kBoolean ||
                                              input_type == DataType::Type::kInt8 ||
                                              input_type == DataType::Type::kInt16 ||
                                              input_type == DataType::Type::kUint16)) ||
      (result_type == DataType::Type::kUint16 && input_type == DataType::Type::kBoolean) ||
      (result_type == DataType::Type::kInt16 && (input_type == DataType::Type::kBoolean ||
                                                input_type == DataType::Type::kInt8)) ||
      (result_type == DataType::Type::kInt8 && input_type == DataType::Type::kBoolean);
}
...
static bool IsTypeConversionLossless(DataType::Type input_type, DataType::Type result_type) {
  ...
  return DataType::Size(result_type) > DataType::Size(input_type) &&
      ...
      !(result_type == DataType::Type::kInt64 && input_type == DataType::Type::kFloat);
}
...
void InstructionSimplifierVisitor::VisitXor(HXor* instruction) {
  ...
  if ((input_cst != nullptr) && input_cst->IsOne()
      && input_other->GetType() == DataType::Type::kBoolean) {
    // Replace code looking like
    //    XOR dst, src, 1
    // with
    //    BOOLEAN_NOT dst, src
