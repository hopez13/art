...
void InstructionSimplifierVisitor::SimplifySystemArrayCopy(HInvoke* instruction) {
...

  {
    ...
    if ((source_component_type != Primitive::kPrimVoid) &&
      ...
      // the invoke, as we would need to look it up in the current dex file, if it
      // unlikely exists. The most usual situation for such typed arraycopy methods
      // is a direct pointer to the boot image.
