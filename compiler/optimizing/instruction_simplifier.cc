...
static HInstruction* NewIntegralMinMax(ArenaAllocator* allocator,
  ...
  if (is_min) {
    minmax = new (allocator) HMin(type, x, y, x->GetDexPc());
  } else {
    minmax = new (allocator) HMax(type, x, y, x->GetDexPc());
  }
...
void InstructionSimplifierVisitor::VisitSelect(HSelect* select) {
  ...
  if (true_value == false_value) {
    ...
    if (t_type == f_type &&
      ...
      // Try to replace typical integral MIN/MAX constructs.
      if (cmp == kCondLT || cmp == kCondLE) {
         // Found a < b ? a : b (MIN) or a < b ? b : a (MAX).
        if ((a == true_value && b == false_value) ||
            (b == true_value && a == false_value)) {
          replace_with = NewIntegralMinMax(GetGraph()->GetAllocator(), a, b, select, a == true_value);
        }
      } else if (cmp == kCondGT || cmp == kCondGE) {
         // Found a > b ? a : b (MAX) or a > b ? b : a (MIN).
        if ((a == true_value && b == false_value) ||
            (b == true_value && a == false_value)) {
          replace_with = NewIntegralMinMax(GetGraph()->GetAllocator(), a, b, select, b == true_value);
        }
      }
