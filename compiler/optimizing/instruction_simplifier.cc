...
//    N % kBitsPerByte = 0
// where
//    T = IntegralTypeFromSize(source_integral_size)
...
static bool TryReplaceShiftsByConstantWithTypeConversion(HBinaryOperation *instruction) {
  if ((!instruction->IsUShr() && !instruction->IsShr()) ||
      DataType::Is64BitType(instruction->GetResultType())) {
    return false;
  }

  HInstruction* shr_amount = instruction->GetRight();
  if (!shr_amount->IsConstant()) {
    return false;
  }

  int64_t shr_amount_cst = Int64FromConstant(shr_amount->AsConstant());
  if ((shr_amount_cst % kBitsPerByte) != 0) {
    return false;
  }

  HInstruction* shr_value = instruction->GetLeft();
  if (!shr_value->IsShl() || !shr_value->HasOnlyOneNonEnvironmentUse()) {
    return false;
  }

  HShl *shl = shr_value->AsShl();
  HInstruction *shl_amount = shl->GetRight();

  if (!shl_amount->IsConstant()) {
    return false;
  }

  int64_t shl_amount_cst = Int64FromConstant(shl_amount->AsConstant());
  if (shl_amount_cst != shr_amount_cst) {
    return false;
  }
  ...
  // supported.
  // Also it means that shift amount should be less than size of right shift type
  // that is actually Int32.
  DCHECK_LE(shl_amount_cst, kMaxIntShiftDistance);
  DCHECK_GT(DataType::Size(shr_value->GetType()), shl_amount_cst / kBitsPerByte);
  ...
  size_t source_significant_part_size =
    DataType::Size(shr_value->GetType()) - shl_amount_cst / kBitsPerByte;
  ...
  DataType::Type source_integral_type =
    DataType::IntegralTypeFromSize(source_significant_part_size);
  ...
  // Actually type of shift's value is always int so sign/zero extension only
  // depends on the type of the shift (shr/ushr)
  bool is_signed = instruction->IsShr();
  DataType::Type conv_type = is_signed ? source_integral_type :
    DataType::ToUnsigned(source_integral_type);
  HInstruction* shl_value = shl->GetLeft();
  HBasicBlock *block = instruction->GetBlock();

  HTypeConversion* new_conversion = new (block->GetGraph()->GetAllocator())
      HTypeConversion(conv_type, shl_value);
