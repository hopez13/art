...
TEST_F(ConstantFoldingTest, UnsignedComparisonsWithZero) {
  ...
  HInstruction* last;
  block->AddInstruction(last = new (GetArena()) HAbove(zero, parameter));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HAbove(parameter, zero));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HAboveOrEqual(zero, parameter));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HAboveOrEqual(parameter, zero));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HBelow(zero, parameter));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HBelow(parameter, zero));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HBelowOrEqual(zero, parameter));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(last = new (GetArena()) HBelowOrEqual(parameter, zero));
  block->AddInstruction(new (GetArena()) HSelect(last, parameter, parameter, 0));
  block->AddInstruction(new (GetArena()) HReturn(zero));
