...
/* static */ void HConstructorFence::RemoveConstructorFences(HInstruction* instruction) {
  DCHECK(instruction->GetBlock() != nullptr);
  ...
  for (auto it = uses.begin(), end = uses.end(); it != end; ) {
    ...
    if (use_instruction->IsConstructorFence()) {
      ...
      DCHECK_EQ(instruction, ctor_fence->GetInputRecords()[input_index].GetInstruction());
