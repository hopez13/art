...
bool HGraph::PathBetween(uint32_t source_idx, uint32_t dest_idx) const {
  ...
  return reachability_graph_.IsBitSet(source_idx * RoundUp(blocks_.size(), BitVector::kWordBits) +
                                      dest_idx);
}
...
class ReachabilityAnalysisHelper {
 public:
  ReachabilityAnalysisHelper(const HGraph* graph,
                             ArenaBitVector* reachability_graph,
                             ArenaStack* arenas)
      : graph_(graph),
        ...
        all_visited_nodes_(&temporaries_, block_size_, false, kArenaAllocReachabilityGraph),
    ...
    CHECK_GE(reachability_graph_->GetBitSizeOf(), graph_->GetBlocks().size() * block_size_);
 ...
 private:
  void AddEdge(uint32_t source, const HBasicBlock* dest) {
    reachability_graph_->SetBit(source * block_size_ + dest->GetBlockId());
  }

  // Union the reachability of 'idx' into 'source_idx'. This is done to
  // implement memoization. In order to improve performance we do this in 1-byte
  // blocks. Clang should be able to optimize this to larger blocks if possible.
  void UnionBlock(size_t source_idx, size_t idx) {
    size_t block_words = block_size_ / BitVector::kWordBits;
    uint32_t* dest =
        reachability_graph_->GetRawStorage() + ((source_idx * block_size_) / BitVector::kWordBits);
    uint32_t* source =
        reachability_graph_->GetRawStorage() + ((idx * block_size_) / BitVector::kWordBits);
    for (uint32_t i = 0; i < block_words; ++i, ++dest, ++source) {
      *dest = (*dest) | (*source);
    }
  }

  // Fill in the connectedness discovered between source_idx and blk's
  // successors.
  //
  // This is the actual DFS implementation using the 'visited' bitmap to keep
  // track of already visited nodes.
  void CalculateConnectednessDFS(uint32_t source_idx,
                                 const HBasicBlock* blk,
                                 ArenaBitVector* visited) {
    uint32_t idx = blk->GetBlockId();
    if (visited->IsBitSet(idx)) {
      return;
    }
    visited->SetBit(idx);
    if (all_visited_nodes_.IsBitSet(idx)) {
      // Use Memoization, we already calculated connecteness of this block. Just use that.
      // union source_idx -> <bits> with idx -> <bits>
      UnionBlock(source_idx, idx);
    } else {
      for (const HBasicBlock* succ : blk->GetSuccessors()) {
        AddEdge(source_idx, succ);
        CalculateConnectednessDFS(source_idx, succ, visited);
      }
    }
  }

  // Single DFS to get connectedness of a single block
  void CalculateConnectednessOn(const HBasicBlock* block) {
    ...
    CalculateConnectednessDFS(block->GetBlockId(), block, &visited);
  }
...
void HGraph::ComputeReachabilityInformation() {
  ...
  const size_t block_size = RoundUp(blocks_.size(), BitVector::kWordBits);
  ...
  // padded so each row starts on an 32-bit alignment for simplicity and
  // performance, allowing us to union blocks together without going bit-by-bit.
  reachability_graph_.SetBit(blocks_.size() * block_size - 1);
  reachability_graph_.ClearBit(blocks_.size() * block_size - 1);
