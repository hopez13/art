...
std::ostream& HInstanceFieldGet::DumpArguments(std::ostream& os, ArenaBitVector* visited) const {
  return HExpression<1>::DumpArguments(
...
std::ostream& HGraph::Dump(std::ostream& oss, const BlockNamer& namer) const {
  VariableIndentationOutputStream vios(&oss);
  struct DumpIns : public HGraphVisitor {
   public:
    explicit DumpIns(HGraph* graph, VariableIndentationOutputStream& oss, const BlockNamer& namer)
        : HGraphVisitor(graph), oss_(oss), namer_(namer) {}
   ...
   private:
    void PrintAdjLine(HBasicBlock* blk) {
      oss_.Stream() << namer_.GetName(blk) << "(" << blk->GetBlockId() << ") -> [";
      bool first = true;
      for (HBasicBlock* succ : blk->GetSuccessors()) {
        ...
        first = false;
        oss_.Stream() << namer_.GetName(succ) << "(" << std::dec << succ->GetBlockId() << ")";
      }
      oss_.Stream() << "] (preds: [";
      first = true;
      for (HBasicBlock* pred : blk->GetPredecessors()) {
        ...
        first = false;
        oss_.Stream() << namer_.GetName(pred) << "(" << std::dec << pred->GetBlockId() << ")";
      }
  ...
  vios.Stream() << "Blocks: " << std::endl;
  {
    ...
    DumpIns(const_cast<HGraph*>(this), vios, namer).VisitInsertionOrder();
  }
  return oss;
}
