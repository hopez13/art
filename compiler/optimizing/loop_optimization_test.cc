 ...
 public:
  LoopOptimizationTest()
      : graph_(CreateGraph()),
        ...
        loop_opt_(new (GetArena()) HLoopOptimization(graph_, nullptr, iva_, nullptr)) {
    BuildGraph();
  }
  ...
  void BuildGraph() {
    ...
    return_block_ = new (GetArena()) HBasicBlock(graph_);
    ...
    return_block_->AddInstruction(new (GetArena()) HReturnVoid());
  ...
  HBasicBlock* AddLoop(HBasicBlock* position, HBasicBlock* successor) {
    HBasicBlock* header = new (GetArena()) HBasicBlock(graph_);
    HBasicBlock* body = new (GetArena()) HBasicBlock(graph_);
    ...
    header->AddInstruction(new (GetArena()) HIf(parameter_));
    body->AddSuccessor(header);
    body->AddInstruction(new (GetArena()) HGoto());
    return header;
  }
...
TEST_F(LoopOptimizationTest, SimplifyLoop) {
  ...
  HInstruction* add = new (GetArena()) HAdd(DataType::Type::kInt32, phi, parameter_);
  header->AddPhi(phi);
