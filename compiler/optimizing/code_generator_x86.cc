...
void CodeGeneratorX86::GenerateInlineCacheCheck(HInstruction* instruction, Register klass) {
  DCHECK_EQ(EAX, klass);
  if (GetCompilerOptions().IsBaseline() && !Runtime::Current()->IsAotCompiler()) {
    ...
    uint32_t temp_index = instruction->IsInvokeInterface() ? 2 : 1;
    Register temp = instruction->GetLocations()->GetTemp(temp_index).AsRegister<Register>();
    DCHECK_EQ(EBP, temp);
    NearLabel done;
    __ movl(temp, Immediate(address));
    // Fast path for a monomorphic cache.
    __ cmpl(klass, Address(temp, 4));
    __ j(kEqual, &done);
