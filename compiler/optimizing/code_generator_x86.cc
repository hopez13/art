...
void LocationsBuilderX86::VisitArrayGet(HArrayGet* instruction) {
  ...
  locations->SetInAt(0, Location::RequiresRegister());
  locations->SetInAt(1, Location::RegisterOrConstant(instruction->InputAt(1)));
  // Need register to check the length of string
  // Compression flag is saved at the top-most bit of string's length
  // If it is compressed, the size of each character is 8-bit, otherwise it's 16-bit.
