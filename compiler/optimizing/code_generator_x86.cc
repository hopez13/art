    ...
    case Primitive::kPrimChar: {
      if (instruction->IsStringCharAt()) {
        ...
        if (index.IsConstant()) {
          ...
          __ movzxb(out, Address(obj,
            (index.GetConstant()->AsIntConstant()->GetValue() << ScaleFactor::TIMES_1) +
                                                                          data_offset));
          __ jmp(&done);
          __ Bind(&is_not_compressed_1);
          __ movzxw(out, Address(obj,
            (index.GetConstant()->AsIntConstant()->GetValue() << ScaleFactor::TIMES_2) +
                                                                          data_offset));
        } else {
          NearLabel is_not_compressed_2;
          __ cmpl(length, Immediate(0));
          __ j(kGreaterEqual, &is_not_compressed_2);
          __ movzxb(out, Address(obj, index.AsRegister<Register>(), ScaleFactor::TIMES_1,
                    data_offset));
          __ jmp(&done);
          __ Bind(&is_not_compressed_2);
          __ movzxw(out, Address(obj, index.AsRegister<Register>(), ScaleFactor::TIMES_2,
                    data_offset));
        }
        __ Bind(&done);
      } else {
