...
void LocationsBuilderX86::HandleFieldSet(HInstruction* instruction,
  ...
  if (is_byte_type) {
    ...
    // TODO(solanes): move under the right check with instruction->IsConstant(). Check the
    // `value_reg` cases, and the `kPoisonHeapReferences` case. Need an extra register in the case
    // of value being a constant.
    // locations->AddTemp(Location::RequiresRegister());
    // locations->AddTemp(Location::RegisterLocation(ECX));
    // locations->AddTemp(Location::RequiresRegister());
  }
  ...
  switch (field_type) {
    ...
    case DataType::Type::kReference: {
      if (kPoisonHeapReferences && needs_write_barrier) {
        ...
        if (value.IsConstant()) {
          // Don't poison null.
          // TODO(solanes): Investigate why we sometimes get non-null constant values here.
          // DCHECK(value.GetConstant()->IsNullConstant())
          //     << "value: " << CodeGenerator::GetInt32ValueOf(value.GetConstant()) << " "
          //     << GetGraph()->IsCompilingBaseline() << " "
          //     << CodeGenerator::StoreNeedsWriteBarrier(field_type, instruction->InputAt(1)) << "
          //     "
          //     << write_barrier_kind;
          __ movl(field_addr, Immediate(CodeGenerator::GetInt32ValueOf(value.GetConstant())));
        } else {
          Register temp = locations->GetTemp(0).AsRegister<Register>();
