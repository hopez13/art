...
class ReadBarrierMarkSlowPathX86 : public SlowPathCode {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    DCHECK(!locations->GetLiveRegisters()->ContainsCoreRegister(ref_reg));
 ...
 private:
  ...
  const Location ref_;
  // Should the reference in `ref` be unpoisoned prior to marking it?
  const bool unpoison_ref_before_marking_;

  DISALLOW_COPY_AND_ASSIGN(ReadBarrierMarkSlowPathX86);
};

// Slow path marking an object during a read barrier and updating the
// field in the holding object.
// Slow path marking an object during a read barrier.
class ReadBarrierMarkAndUpdateFieldSlowPathX86 : public SlowPathCode {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    // update the field in the holder (`obj_`).
    NearLabel done;
