...
// ReadBarrierMarkSlowPathX86, except that:
// - it handles two references instead of just one;
// - it has different assertions regarding its use cases (the only
//   instruction that may use it is MarkReferencesImplicitRBState; the
//   registers holding the references are expected to be live).
class ReadBarrierMarkTwoReferencesSlowPathX86 : public SlowPathCode {
 public:
  ReadBarrierMarkTwoReferencesSlowPathX86(HInstruction* instruction,
                                          Location ref1,
                                          Location ref2,
                                          bool unpoison_refs_before_marking)
      : SlowPathCode(instruction),
        ref1_(ref1),
        ref2_(ref2),
        unpoison_refs_before_marking_(unpoison_refs_before_marking) {
    DCHECK(kEmitCompilerReadBarrier);
  }

  const char* GetDescription() const OVERRIDE { return "ReadBarrierMarkTwoReferencesSlowPathX86"; }
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    // MarkReferencesImplicitRBState instruction.
    ...
    DCHECK(instruction_->IsMarkReferencesImplicitRBState())
        << "Unexpected instruction in read barrier marking slow path: "
        << instruction_->DebugName();

    __ Bind(GetEntryLabel());
    if (unpoison_refs_before_marking_) {
      // Object* ref1 = ref1_addr->AsMirrorPtr()
      __ MaybeUnpoisonHeapReference(ref1_reg);
      // Object* ref2 = ref2_addr->AsMirrorPtr()
      __ MaybeUnpoisonHeapReference(ref2_reg);
    }
    // No need to save live registers; it's taken care of by the
    // entrypoints. Also, there is no need to update the stack mask,
    // as these runtime calls will not trigger a garbage collection.
    CodeGeneratorX86* x86_codegen = down_cast<CodeGeneratorX86*>(codegen);
    DCHECK_NE(ref1_reg, ESP);
    DCHECK_NE(ref2_reg, ESP);
    DCHECK(0 <= ref1_reg && ref1_reg < kNumberOfCpuRegisters) << ref1_reg;
    DCHECK(0 <= ref2_reg && ref2_reg < kNumberOfCpuRegisters) << ref2_reg;
    DCHECK_NE(ref1_reg, ref2_reg);
    ...
    int32_t entry_point_offset1 =
        CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kX86PointerSize>(ref1_reg);
    int32_t entry_point_offset2 =
        CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kX86PointerSize>(ref2_reg);
    // These runtime calls do not require stack maps.
    x86_codegen->InvokeRuntimeWithoutRecordingPcInfo(entry_point_offset1, instruction_, this);
    x86_codegen->InvokeRuntimeWithoutRecordingPcInfo(entry_point_offset2, instruction_, this);
    __ jmp(GetExitLabel());
  }
 ...
 private:
  ...
  // Should the references in `ref1_` and `ref2_` be unpoisoned prior to marking them?
  const bool unpoison_refs_before_marking_;

  DISALLOW_COPY_AND_ASSIGN(ReadBarrierMarkTwoReferencesSlowPathX86);
};
...
void LocationsBuilderX86::HandleFieldGet(HInstruction* instruction, const FieldInfo& field_info) {
  ...
  if (Primitive::IsFloatingPointType(instruction->GetType())) {
    ...
    // an object field get generating its (own) read barrier, we do
    // not want the move to overwrite the object's location, as we
    // need it to emit the read barrier.
    locations->SetOut(
        Location::RequiresRegister(),
        (instruction->GetType() == Primitive::kPrimLong || call_on_read_barrier_slow_path)
    ...
    case Primitive::kPrimNot: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        if (generates_own_read_barrier) {
          ...
          // InstructionCodeGeneratorX86::VisitLoadReadBarrierState.
          // Likewise, potential heap unpoisoning is handled in
          // InstructionCodeGeneratorX86::VisitMarkReferencesImplicitRBState.
          __ movl(out.AsRegister<Register>(), Address(base, offset));  // Flags are unaffected.
        }
        if (is_volatile) {
...
void LocationsBuilderX86::VisitLoadReadBarrierState(HLoadReadBarrierState* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  ...
  LocationSummary* locations =
      new (GetGraph()->GetArena()) LocationSummary(instruction, LocationSummary::kNoCall);
  locations->SetInAt(0, Location::RequiresRegister());
  // The read barrier state is stored in the EFLAGS register and does
  // not need an (out) location on x86.
}

void InstructionCodeGeneratorX86::VisitLoadReadBarrierState(HLoadReadBarrierState* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
...
void LocationsBuilderX86::VisitMarkReferencesExplicitRBState(
    HMarkReferencesExplicitRBState* instruction) {
  // Instruction HMarkReferencesExplicitRBState is not used in the x86 back end.
  LOG(FATAL) << "Unexpected instruction " << instruction->DebugName();
}

void InstructionCodeGeneratorX86::VisitMarkReferencesExplicitRBState(
    HMarkReferencesExplicitRBState* instruction) {
  // Instruction HMarkReferencesExplicitRBState is not used in the x86 back end.
  LOG(FATAL) << "Unexpected instruction " << instruction->DebugName();
}

void LocationsBuilderX86::VisitMarkReferencesImplicitRBState(
    HMarkReferencesImplicitRBState* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
  ...
  // is stored in the EFLAGS register and is not an input of the
  // HMarkReferencesImplicitRBState instruction on x86.
  locations->SetInAt(0, Location::RequiresRegister());
  locations->SetInAt(1, Location::RequiresRegister());
}

void InstructionCodeGeneratorX86::VisitMarkReferencesImplicitRBState(
    HMarkReferencesImplicitRBState* instruction) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);

  LocationSummary* locations = instruction->GetLocations();
  Location ref1 = locations->InAt(0);
  Register ref1_reg = ref1.AsRegister<Register>();
  Location ref2 = locations->InAt(1);
  Register ref2_reg = ref2.AsRegister<Register>();
  DCHECK_NE(ref1_reg, ref2_reg);
  ...
  SlowPathCode* slow_path = new (GetGraph()->GetArena()) ReadBarrierMarkTwoReferencesSlowPathX86(
      instruction, ref1, ref2, /* unpoison_refs_before_marking */ true);
  codegen_->AddSlowPath(slow_path);

  // We have done the "if" of the gray bit check above in
  // InstructionCodeGeneratorX86::VisitLoadReadBarrierState, now
  // branch based on the flags.
