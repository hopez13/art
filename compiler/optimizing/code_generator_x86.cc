...
void InstructionCodeGeneratorX86::VisitX86FPNeg(HX86FPNeg* neg) {
  ...
  if (neg->GetType() == Primitive::kPrimFloat) {
    __ movss(mask, codegen_->LiteralInt32Address(INT32_C(0x80000000), neg->GetBaseMethodAddress(), constant_area));
    __ xorps(out.AsFpuRegister<XmmRegister>(), mask);
  } else {
     __ movsd(mask, codegen_->LiteralInt64Address(INT64_C(0x8000000000000000), neg->GetBaseMethodAddress(), constant_area));
     __ xorpd(out.AsFpuRegister<XmmRegister>(), mask);
  }
}
...
void CodeGeneratorX86::RecordBootStringPatch(HLoadString* load_string) {
  ...
  HX86ComputeBaseMethodAddress* address = (load_string->InputCount() == 1)
...
void CodeGeneratorX86::RecordBootTypePatch(HLoadClass* load_class) {
  HX86ComputeBaseMethodAddress* address = (load_class->InputCount() == 1)
...
Address CodeGeneratorX86::LiteralDoubleAddress(double v,
                                               ...
                                               Register reg) {
  AssemblerFixup* fixup = new (GetGraph()->GetArena()) RIPFixup(*this, method_base, __ AddDouble(v));
