...
class ReadBarrierMarkSlowPathBaseX86 : public SlowPathCode {
 protected:
  ReadBarrierMarkSlowPathBaseX86(HInstruction* instruction, Location ref)
      : SlowPathCode(instruction), ref_(ref) {
    DCHECK(kEmitCompilerReadBarrier);
  }

  const char* GetDescription() const OVERRIDE { return "ReadBarrierMarkSlowPathBaseX86"; }
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(ReadBarrierMarkSlowPathBaseX86);
};
...
class ReadBarrierMarkSlowPathX86 : public ReadBarrierMarkSlowPathBaseX86 {
 public:
  ReadBarrierMarkSlowPathX86(HInstruction* instruction,
                             Location ref,
                             bool unpoison_ref_before_marking)
      : ReadBarrierMarkSlowPathBaseX86(instruction, ref),
...
// LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathX86 below
// for that).
//
// This means that after the execution of this slow path, `ref` will
// always be up-to-date, but `obj.field` may not; i.e., after the
...
// another thread, or if another thread installed another object
// reference (different from `ref`) in `obj.field`).
class LoadReferenceWithBakerReadBarrierSlowPathX86 : public ReadBarrierMarkSlowPathBaseX86 {
 public:
  LoadReferenceWithBakerReadBarrierSlowPathX86(HInstruction* instruction,
                                               Location ref,
                                               Register obj,
                                               const Address& src,
  ...
  const char* GetDescription() const OVERRIDE {
    return "LoadReferenceWithBakerReadBarrierSlowPathX86";
  }
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    CodeGeneratorX86* x86_codegen = down_cast<CodeGeneratorX86*>(codegen);
    x86_codegen->GenerateMemoryBarrier(MemBarrierKind::kLoadAny);  // Flags are unaffected.

    // The actual reference load.
    // /* HeapReference<mirror::Object> */ ref = *src
    // A possible implicit null check has already been handled above.
    __ movl(ref_reg, src_);  // Flags are unaffected.
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(LoadReferenceWithBakerReadBarrierSlowPathX86);
};
...
// LoadReferenceWithBakerReadBarrierSlowPathX86 above, which never
...
class LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathX86
    : public ReadBarrierMarkSlowPathBaseX86 {
 public:
  LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathX86(HInstruction* instruction,
                                                             Location ref,
                                                             Register obj,
                                                             const Address& src,
                                                             bool needs_null_check,
  ...
  const char* GetDescription() const OVERRIDE {
    return "LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathX86";
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathX86);
};
...
void CodeGeneratorX86::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  if (always_update_field) {
    ...
    slow_path =
        new (GetGraph()->GetArena()) LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathX86(
            instruction, ref, obj, /* src */ field_addr, needs_null_check, *temp);
  } else {
    slow_path = new (GetGraph()->GetArena()) LoadReferenceWithBakerReadBarrierSlowPathX86(
  ...
  const int32_t entry_point_offset =
      CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kX86PointerSize>(ref.reg());
  __ fs()->cmpl(Address::Absolute(entry_point_offset), Immediate(0));
  __ j(kNotEqual, slow_path->GetEntryLabel());
