...
void InstructionCodeGeneratorX86::VisitX86FPNeg(HX86FPNeg* neg) {
  ...
  XmmRegister mask = locations->GetTemp(0).AsFpuRegister<XmmRegister>();
  if (neg->GetType() == DataType::Type::kFloat) {
...
void LocationsBuilderX86::VisitTypeConversion(HTypeConversion* conversion) {
  ...
  LocationSummary::CallKind call_kind =
      ((input_type == DataType::Type::kFloat || input_type == DataType::Type::kDouble)
...
void InstructionCodeGeneratorX86::GenerateRemFP(HRem *rem) {
  ...
  bool is_float = type == DataType::Type::kFloat;
...
void LocationsBuilderX86::HandleFieldSet(HInstruction* instruction, const FieldInfo& field_info) {
  ...
  bool is_volatile = field_info.IsVolatile();
  DataType::Type field_type = field_info.GetFieldType();
  bool is_byte_type = (field_type == DataType::Type::kBoolean)
  ...
  if (is_byte_type) {
    ...
    if (is_volatile && field_type == DataType::Type::kDouble) {
...
void LocationsBuilderX86::VisitArraySet(HArraySet* instruction) {
  ...
  bool is_byte_type = (value_type == DataType::Type::kBoolean)
