...
void CodeGeneratorX86::CheckGCCardIsValid(Register temp,
                                        Register card,
                                        Register object,
                                        Register value) {
  NearLabel done;
...
void InstructionCodeGeneratorX86::HandleFieldSet(HInstruction* instruction,
  ...
  if (needs_write_barrier) {
    ...
    if (value.IsConstant()) {
      ...
      Register temp = locations->GetTemp(0).AsRegister<Register>();
      Register card = locations->GetTemp(1).AsRegister<Register>();
      codegen_->CheckGCCardIsValid(temp, card, base, value.AsRegister<Register>());
    }
  }
  ...
  switch (value_type) {
    ...
    case DataType::Type::kReference: {
      ...
      if (needs_write_barrier) {
        ...
        Register temp = locations->GetTemp(0).AsRegister<Register>();
        Register card = locations->GetTemp(1).AsRegister<Register>();
        codegen_->CheckGCCardIsValid(temp, card, array, value.AsRegister<Register>());
      }
