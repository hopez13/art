...
void LocationsBuilderX86::HandleFieldGet(HInstruction* instruction, const FieldInfo& field_info) {
  ...
  bool object_field_get_with_read_barrier =
      gEmitCompilerReadBarrier && (instruction->GetType() == DataType::Type::kReference);
  bool is_predicated = instruction->IsPredicatedInstanceFieldGet();
  LocationSummary* locations =
      new (GetGraph()->GetAllocator()) LocationSummary(instruction,
                                                       gEmitCompilerReadBarrier
...
void LocationsBuilderX86::VisitLoadClass(HLoadClass* cls) {
  ...
  const bool requires_read_barrier = gEmitCompilerReadBarrier && !cls->IsInBootImage();
...
void InstructionCodeGeneratorX86::GenerateReferenceLoadOneRegister(
  ...
  if (read_barrier_option == kWithReadBarrier) {
    CHECK(gEmitCompilerReadBarrier);
