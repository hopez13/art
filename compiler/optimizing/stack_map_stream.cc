  ...
  if (EncodeArtMethodInInlineInfo(method)) {
    ...
    if (dex_pc != static_cast<uint32_t>(-1) && kIsDebugBuild) {
      ...
      if (!IsSameDexFile(*outer_dex_file, *dex_file)) {
        // TODO(solanes): encapsulate this. Maybe create a local variable of DexFile* to index? Is
        // the StackMapStream the same for every method? Try to find in the boot class path first
        ...
        // LOG(INFO) << "Success! Saving a stack map that crossed dex file boundary: from "
        //         << outer_dex_file->GetLocation() << "/"
        //         << static_cast<const void*>(outer_dex_file) << " to "
        //         << method->PrettyMethod() << " in "
