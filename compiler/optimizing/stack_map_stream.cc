...
void StackMapStream::BeginStackMapEntry(uint32_t dex_pc,
  ...
  if (stack_maps_.size() > 0) {
    ...
    DCHECK(std::make_tuple(stack_maps_.back().kind == StackMap::Kind::Catch,
                           stack_maps_.back().packed_native_pc) <=
           std::make_tuple(current_stack_map_.kind == StackMap::Kind::Catch,
                           current_stack_map_.packed_native_pc)) << "Stack map order";
  }
  if (register_mask != 0) {
