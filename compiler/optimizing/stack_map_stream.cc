...
void StackMapStream::BeginStackMapEntry(uint32_t dex_pc,
  ...
  if (stack_maps_.size() > 0) {
    // Check that stack maps are sorted by pc (with catch blocks at the end).
    // Catch blocks may be unordered - TODO: investigate why that may happen.
    if (stack_maps_.back().kind == StackMap::Kind::Catch) {
      DCHECK(current_stack_map_.kind == StackMap::Kind::Catch);
    } else if (current_stack_map_.kind != StackMap::Kind::Catch) {
      DCHECK_LE(stack_maps_.back().packed_native_pc, current_stack_map_.packed_native_pc);
    }
  }
  if (register_mask != 0) {
