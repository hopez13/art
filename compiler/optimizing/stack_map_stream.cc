...
void StackMapStream::SetStackMapNativePcOffset(size_t i, uint32_t native_pc_offset) {
  stack_maps_[i][StackMap::kPackedNativePc] = StackMap::PackNativePc(native_pc_offset, instruction_set_);
}
...
void StackMapStream::BeginStackMapEntry(uint32_t dex_pc,
  ...
  if (register_mask != 0) {
    ...
    BitTableBuilder<RegisterMask::kCount>::Entry entry { register_mask >> shift, shift };
...
void StackMapStream::AddInvoke(InvokeType invoke_type, uint32_t dex_method_index) {
  ...
  entry[InvokeInfo::kMethodInfoIndex] = method_infos_.emplace(
      dex_method_index, method_infos_.size()).first->second;
...
void StackMapStream::BeginInlineInfoEntry(ArtMethod* method,
  ...
  if (EncodeArtMethodInInlineInfo(method)) {
    ...
    entry[InlineInfo::kMethodInfoIndex] = method_infos_.emplace(
        dex_method_index, method_infos_.size()).first->second;
  }
...
void StackMapStream::CreateDexRegisterMap() {
  ...
  for (size_t i = 0; i < current_dex_registers_.size(); i++) {
    ...
    if (previous_dex_registers_[i] != reg || distance > kMaxDexRegisterMapSearchDistance) {
      uint16_t kind = static_cast<uint32_t>(reg.GetKind());
      uint32_t packed_value = DexRegisterInfo::PackValue(reg.GetKind(), reg.GetValue());
      BitTableBuilder<DexRegisterInfo::kCount>::Entry entry { kind, packed_value };
