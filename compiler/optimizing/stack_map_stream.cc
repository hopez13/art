...
void StackMapStream::BeginInlineInfoEntry(ArtMethod* method,
  ...
  if (EncodeArtMethodInInlineInfo(method)) {
    ...
    uint32_t is_in_bootclasspath = MethodInfo::kNotInBCP;
    ...
    if (dex_pc != static_cast<uint32_t>(-1)) {
      ...
      if (!IsSameDexFile(*outer_dex_file, *dex_file)) {
        if (method->GetDeclaringClass()->GetClassLoader() == nullptr) {
          ...
          is_in_bootclasspath = MethodInfo::KInBCP;
          dexfile_index = std::distance(boot_class_path.begin(), it);
        } else {
          const std::vector<const DexFile*>& dex_files =
              Runtime::Current()->GetCompilerCallbacks()->GetDexFiles();
  ...
  if (kVerifyStackMaps) {
    ...
    dchecks_.emplace_back([=](const CodeInfo& code_info) {
      ...
      if (encode_art_method) {
        ...
        CHECK(method_info.GetIsInBootClassPath() == MethodInfo::kNotInBCP ||
              method_info.GetIsInBootClassPath() == MethodInfo::KInBCP);
        ScopedObjectAccess soa(Thread::Current());
        if (inline_info.GetDexPc() != static_cast<uint32_t>(-1) &&
            !IsSameDexFile(*outer_dex_file, *method->GetDexFile())) {
          if (method->GetDeclaringClass()->GetClassLoader() == nullptr) {
            CHECK_EQ(method_info.GetIsInBootClassPath(), MethodInfo::KInBCP);
            ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
            const std::vector<const DexFile*>& boot_class_path = class_linker->GetBootClassPath();
            ...
            CHECK_EQ(method_info.GetIsInBootClassPath(), MethodInfo::kNotInBCP);
            const std::vector<const DexFile*>& dex_files =
                Runtime::Current()->GetCompilerCallbacks()->GetDexFiles();
