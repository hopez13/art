...
size_t StackMapStream::PrepareForFillIn() {
  ...
  if (stack_mask_bits > 0) {
    for (size_t i = 0; i < num_stack_masks; ++i) {
      ...
      MemoryRegion src(&stack_masks_[i * stack_mask_bytes], stack_mask_bytes);
      BitMemoryRegion dst = stack_mask_region.Subregion(i * stack_mask_bits, stack_mask_bits);
      for (size_t bit = 0; bit < stack_mask_bits; bit += sizeof(uint32_t) * kBitsPerByte) {
        size_t num_bits = std::min(stack_mask_bits - bit, sizeof(uint32_t) * kBitsPerByte);
