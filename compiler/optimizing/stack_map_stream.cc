...
size_t StackMapStream::DexRegisterMapEntry::ComputeSize(size_t catalog_size) const {
  ...
  if ((num_dex_registers == 0u) || (live_dex_registers_mask->NumSetBits() == 0)) {
...
size_t StackMapStream::PrepareForFillIn() {
  ...
  // Write stack maps.
  ScopedBitTableBuilder<StackMap::Field::kCount> stack_map_builder((allocator_adapter_));
  ScopedBitTableBuilder<InvokeInfo::Field::kCount> invoke_info_builder((allocator_adapter_));
  ScopedBitTableBuilder<InlineInfo::Field::kCount> inline_info_builder((allocator_adapter_));
  ...
  ScopedBitTableBuilder<1> register_mask_builder((allocator_adapter_));
  ...
  MemoryRegion stack_mask_region =
      EncodeMemoryRegion(&out_, &bit_offset, stack_mask_bits * num_stack_masks);
  if (stack_mask_bits != 0) {
    for (size_t i = 0; i < num_stack_masks; ++i) {
      ...
      BitMemoryRegion stack_mask(stack_mask_region, i * stack_mask_bits, stack_mask_bits);
  ...
  out_.resize(BitsToBytesRoundUp(bit_offset));
