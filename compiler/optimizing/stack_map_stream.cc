...
void StackMapStream::BeginStackMapEntry(uint32_t dex_pc,
  ...
  DCHECK(in_stack_map_.exchange(true) == false) << "Mismatched Begin/End calls";

  current_stack_map_ = StackMapEntry{
    .native_pc_offset = CodeOffset::FromOffset(native_pc_offset, instruction_set_),
    .dex_pc = dex_pc,
  };
  if (register_mask != 0) {
    current_stack_map_.register_mask_index =
      register_masks_.Dedup(&register_mask, MinimumBitsToStore(register_mask));
  }
  if (sp_mask != nullptr && sp_mask->GetNumberOfBits() != 0) {
    current_stack_map_.stack_mask_index =
      stack_masks_.Dedup(sp_mask->GetRawStorage(), sp_mask->GetNumberOfBits());
  }
  ...
  if (kVerifyStackMaps) {
    // Create copy of the stack mask. TODO: Remove?
    std::shared_ptr<BitVector> sp_mask_copy;
    if (sp_mask != nullptr) {
      sp_mask_copy = std::make_shared<BitVector>(*sp_mask, false, Allocator::GetMallocAllocator());
    }
    ...
    dchecks_.emplace_back([=](const CodeInfo& code_info) {
      ...
      CHECK_GE(stack_mask.size_in_bits(), sp_mask_copy ? sp_mask_copy->GetNumberOfBits() : 0);
      for (size_t b = 0; b < stack_mask.size_in_bits(); b++) {
        CHECK_EQ(stack_mask.LoadBit(b), sp_mask_copy != nullptr && sp_mask_copy->IsBitSet(b));
      }
      CHECK_EQ(stack_map.HasInlineInfo(), (inlining_depth != 0));
      if (inlining_depth != 0) {
        CHECK_EQ(code_info.GetInlineInfoOf(stack_map).GetDepth(), inlining_depth);
      }
      // TODO: CHECK_EQ(stack_map.HasDexRegisterMap(), (num_dex_registers != 0));
    });
  }
}

void StackMapStream::EndStackMapEntry() {
  DCHECK(in_stack_map_.exchange(false) == true) << "Mismatched Begin/End calls";
...
void StackMapStream::AddInvoke(InvokeType invoke_type, uint32_t dex_method_index) {
  CodeOffset native_pc_offset = current_stack_map_.native_pc_offset;
  size_t invoke_info_index = invoke_infos_.Add(InvokeInfoEntry{
    .native_pc_offset = native_pc_offset,
    .invoke_type = invoke_type,
    .method_index_idx = method_indices_.Dedup(&dex_method_index),
  });
  ...
  if (kVerifyStackMaps) {
    dchecks_.emplace_back([=](const CodeInfo& code_info) {
      ...
      CHECK_EQ(invoke_info.GetNativePcOffset(instruction_set_),
               native_pc_offset.Uint32Value(instruction_set_));
      CHECK_EQ(invoke_info.GetInvokeType(), invoke_type);
      CHECK_EQ(*method_indices_[invoke_info.GetMethodIndexIdx()], dex_method_index);
    });
  }
}
...
void StackMapStream::BeginInlineInfoEntry(ArtMethod* method,
  ...
  DCHECK(in_inline_info_.exchange(true) == false) << "Mismatched Begin/End calls";
  DCHECK_EQ(expected_num_dex_registers_, current_dex_registers_.size());
  ...
  if (EncodeArtMethodInInlineInfo(method)) {
    ...
    uint32_t method_index = method->GetDexMethodIndexUnchecked();
    entry.method_index_idx = method_indices_.Dedup(&method_index);
  }
  ...
  if (kVerifyStackMaps) {
    ...
    dchecks_.emplace_back([=](const CodeInfo& code_info) {
      ...
      if (encode_art_method) {
        ...
        CHECK_EQ(*method_indices_[inline_info.GetMethodIndexIdxAtDepth(depth)],
                 method->GetDexMethodIndexUnchecked());
      }
      // TODO: CHECK_EQ(inline_info.HasDexRegisterMapAtDepth(depth), (num_dex_registers != 0));
    });
  }
}

void StackMapStream::EndInlineInfoEntry() {
  DCHECK(in_inline_info_.exchange(false) == true) << "Mismatched Begin/End calls";
  DCHECK_EQ(expected_num_dex_registers_, current_dex_registers_.size());
}
...
void StackMapStream::CreateDexRegisterMap() {
  ...
  for (size_t i = 0; i < current_dex_registers_.size(); i++) {
    ...
    if (reg.IsLive()) {
      DexRegisterEntry entry = DexRegisterEntry{
        ...
        .packed_value = static_cast<uint32_t>(reg.GetValue()),
      };
      if (reg.GetKind() == DexRegisterLocation::Kind::kInStack) {
        DCHECK(IsAligned<kFrameSlotSize>(entry.packed_value));
        entry.packed_value /= kFrameSlotSize;
      }
  ...
  if (temp_dex_register_mask_.GetNumberOfBits() != 0) {
    uint32_t mask_index = dex_register_masks_.Dedup(temp_dex_register_mask_.GetRawStorage(),
  ...
  if (kVerifyStackMaps) {
    ...
    dchecks_.emplace_back([=](const CodeInfo& code_info) {
      ...
      if (map.IsValid()) {
        CHECK_EQ(map.GetDexRegisterCount(), num_dex_registers);
        for (size_t r = 0; r < num_dex_registers; r++) {
          CHECK_EQ(expected_dex_registers->at(r), map[r]);
        }
      } else {
        // If all the registers are None, then the map might be automatically removed.
        // TODO: Consider removing this behaviour.  Runtime checks depends on it now.
        for (size_t r = 0; r < num_dex_registers; r++) {
          CHECK_EQ(expected_dex_registers->at(r), DexRegisterLocation::None());
        }
      }
    });
  }
