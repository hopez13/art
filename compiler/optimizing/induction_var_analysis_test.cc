 ...
 public:
  ...
  HInstruction* InsertArrayStore(HInstruction* subscript, int d) {
    ...
    return InsertInstruction(new (&allocator_) HArraySet(
        parameter_, subscript, float_constant0_, DataType::Type::kFloat, 0), d);
  }
...
TEST_F(InductionVarAnalysisTest, FindBooleanEqPeriodicInduction) {
  ...
  EXPECT_STREQ("periodic((0), (1)):Boolean", GetInductionInfo(k_header, 0).c_str());
  EXPECT_STREQ("periodic((1), (0)):Boolean", GetInductionInfo(x, 0).c_str());
}
