...
class InductionVarAnalysisTest : public OptimizingUnitTest {
 public:
  ...
  void BuildForLoop(int d, int n) {
    ASSERT_LT(d, n);
    loop_preheader_[d] = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(loop_preheader_[d]);
    loop_header_[d] = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(loop_header_[d]);
    loop_preheader_[d]->AddSuccessor(loop_header_[d]);
    if (d < (n - 1)) {
      BuildForLoop(d + 1, n);
    }
    loop_body_[d] = new (GetArena()) HBasicBlock(graph_);
  ...
  void BuildLoopNest(int n) {
    ...
    BuildForLoop(0, n);
    return_ = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(return_);
    exit_ = new (GetArena()) HBasicBlock(graph_);
    ...
    parameter_ = new (GetArena()) HParameterValue(
    ...
    return_->AddInstruction(new (GetArena()) HReturnVoid());
    exit_->AddInstruction(new (GetArena()) HExit());

    // Provide loop instructions.
    for (int d = 0; d < n; d++) {
      basic_[d] = new (GetArena()) HPhi(GetArena(), d, 0, DataType::Type::kInt32);
      loop_preheader_[d]->AddInstruction(new (GetArena()) HGoto());
      loop_header_[d]->AddPhi(basic_[d]);
      HInstruction* compare = new (GetArena()) HLessThan(basic_[d], constant100_);
      loop_header_[d]->AddInstruction(compare);
      loop_header_[d]->AddInstruction(new (GetArena()) HIf(compare));
      increment_[d] = new (GetArena()) HAdd(DataType::Type::kInt32, basic_[d], constant1_);
      loop_body_[d]->AddInstruction(increment_[d]);
      loop_body_[d]->AddInstruction(new (GetArena()) HGoto());
  ...
  HPhi* BuildIf(int d, HBasicBlock** ifT, HBasicBlock** ifF) {
    HBasicBlock* cond = new (GetArena()) HBasicBlock(graph_);
    HBasicBlock* ifTrue = new (GetArena()) HBasicBlock(graph_);
    HBasicBlock* ifFalse = new (GetArena()) HBasicBlock(graph_);
    ...
    cond->AddInstruction(new (GetArena()) HIf(parameter_));
    *ifT = ifTrue;
    *ifF = ifFalse;

    HPhi* select_phi = new (GetArena()) HPhi(GetArena(), -1, 0, DataType::Type::kInt32);
  ...
  HPhi* InsertLoopPhi(int vreg, int d) {
    HPhi* phi = new (GetArena()) HPhi(GetArena(), vreg, 0, DataType::Type::kInt32);
  ...
  HInstruction* InsertArrayStore(HInstruction* subscript, int d) {
    ...
    return InsertInstruction(new (GetArena()) HArraySet(
...
TEST_F(InductionVarAnalysisTest, FindDerivedInduction) {
  ...
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, constant100_, basic_[0]), 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, constant100_, basic_[0]), 0);
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, constant100_, basic_[0]), 0);
  HInstruction* shl = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, basic_[0], constant1_), 0);
  HInstruction* neg = InsertInstruction(
      new (GetArena()) HNeg(DataType::Type::kInt32, basic_[0]), 0);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindChainInduction) {
  ...
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant100_), 0);
  HInstruction* store1 = InsertArrayStore(add, 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, add, constant1_), 0);
...
TEST_F(InductionVarAnalysisTest, FindTwoWayBasicInduction) {
  ...
  // True-branch.
  HInstruction* inc1 = new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant1_);
  ifTrue->AddInstruction(inc1);
  k_body->AddInput(inc1);
  // False-branch.
  HInstruction* inc2 = new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant1_);
...
TEST_F(InductionVarAnalysisTest, FindTwoWayDerivedInduction) {
  ...
  // True-branch.
  HInstruction* inc1 = new (GetArena()) HAdd(DataType::Type::kInt32, basic_[0], constant1_);
  ifTrue->AddInstruction(inc1);
  k->AddInput(inc1);
  // False-branch.
  HInstruction* inc2 = new (GetArena()) HAdd(DataType::Type::kInt32, basic_[0], constant1_);
  ifFalse->AddInstruction(inc2);
  k->AddInput(inc2);
  // Merge over a phi.
...
TEST_F(InductionVarAnalysisTest, AddLinear) {
  ...
  HInstruction* add1 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, basic_[0], basic_[0]), 0);
  HInstruction* add2 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, constant7_, basic_[0]), 0);
  HInstruction* add3 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, add1, add2), 0);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindPolynomialInduction) {
  ...
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, basic_[0], constant2_), 0);
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, constant100_, mul), 0);
  HInstruction* pol = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, add, k_header), 0);
...
TEST_F(InductionVarAnalysisTest, FindPolynomialInductionAndDerived) {
  ...
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant100_), 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, k_header, constant1_), 0);
  HInstruction* neg = InsertInstruction(
      new (GetArena()) HNeg(DataType::Type::kInt32, sub), 0);
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, k_header, constant2_), 0);
  HInstruction* shl = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, k_header, constant2_), 0);
  HInstruction* pol = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, basic_[0]), 0);
  k_header->AddInput(pol);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, AddPolynomial) {
  ...
  HInstruction* add1 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, k_header), 0);
  HInstruction* add2 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, add1, k_header), 0);
  HInstruction* add3 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, basic_[0]), 0);
...
TEST_F(InductionVarAnalysisTest, FindGeometricMulInduction) {
  ...
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, k_header, constant100_), 0);
...
TEST_F(InductionVarAnalysisTest, FindGeometricShlInductionAndDerived) {
  ...
  HInstruction* add1 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant1_), 0);
  HInstruction* shl1 = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, k_header, constant1_), 0);
  HInstruction* add2 = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, shl1, constant100_), 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, shl1, constant1_), 0);
  HInstruction* neg = InsertInstruction(
      new (GetArena()) HNeg(DataType::Type::kInt32, sub), 0);
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, shl1, constant2_), 0);
  HInstruction* shl2 = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, shl1, constant2_), 0);
...
TEST_F(InductionVarAnalysisTest, FindGeometricDivInductionAndDerived) {
  ...
  HInstruction* div = InsertInstruction(
      new (GetArena()) HDiv(DataType::Type::kInt32, k_header, constant100_, kNoDexPc), 0);
...
TEST_F(InductionVarAnalysisTest, FindGeometricShrInduction) {
  ...
  HInstruction* shr = InsertInstruction(
      new (GetArena()) HShr(DataType::Type::kInt32, k_header, constant1_), 0);
...
TEST_F(InductionVarAnalysisTest, FindRemWrapAroundInductionAndDerived) {
  ...
  HInstruction* rem = InsertInstruction(
      new (GetArena()) HRem(DataType::Type::kInt32, k_header, constant7_, kNoDexPc), 0);
...
TEST_F(InductionVarAnalysisTest, FindSecondOrderWrapAroundInduction) {
  ...
  k_header->AddInput(t);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, constant100_, basic_[0], 0), 0);
  t->AddInput(sub);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindWrapAroundDerivedInduction) {
  ...
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant100_), 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, k_header, constant100_), 0);
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, k_header, constant100_), 0);
  HInstruction* shl1 = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, k_header, constant1_), 0);
  HInstruction* neg1 = InsertInstruction(
      new (GetArena()) HNeg(DataType::Type::kInt32, k_header), 0);
  HInstruction* shl2 = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, basic_[0], constant1_), 0);
  HInstruction* neg2 = InsertInstruction(
      new (GetArena()) HNeg(DataType::Type::kInt32, shl2), 0);
...
TEST_F(InductionVarAnalysisTest, FindIdiomaticPeriodicInduction) {
  ...
  HInstruction* store = InsertArrayStore(k_header, 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, constant1_, k_header), 0);
  k_header->AddInput(sub);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindXorPeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(
      new (GetArena()) HXor(DataType::Type::kInt32, k_header, constant1_), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindXorConstantLeftPeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(
      new (GetArena()) HXor(DataType::Type::kInt32, constant1_, k_header), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindXor100PeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(
      new (GetArena()) HXor(DataType::Type::kInt32, k_header, constant100_), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindBooleanEqPeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(new (GetArena()) HEqual(k_header, constant0_), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindBooleanEqConstantLeftPeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(new (GetArena()) HEqual(constant0_, k_header), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindBooleanNePeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(new (GetArena()) HNotEqual(k_header, constant1_), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindBooleanNeConstantLeftPeriodicInduction) {
  ...
  HInstruction* x = InsertInstruction(new (GetArena()) HNotEqual(constant1_, k_header), 0);
  k_header->AddInput(x);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, FindDerivedPeriodicInduction) {
  ...
  HInstruction* idiom = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, constant1_, k_header), 0);
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, idiom, constant100_), 0);
  HInstruction* sub = InsertInstruction(
      new (GetArena()) HSub(DataType::Type::kInt32, idiom, constant100_), 0);
  HInstruction* mul = InsertInstruction(
      new (GetArena()) HMul(DataType::Type::kInt32, idiom, constant100_), 0);
  HInstruction* shl = InsertInstruction(
      new (GetArena()) HShl(DataType::Type::kInt32, idiom, constant1_), 0);
  HInstruction* neg2 = InsertInstruction(
      new (GetArena()) HNeg(DataType::Type::kInt32, idiom), 0);
...
TEST_F(InductionVarAnalysisTest, FindDeepLoopInduction) {
  ...
  HInstruction* inc = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, constant1_, k_header[9]), 9);
...
TEST_F(InductionVarAnalysisTest, ByteInductionIntLoopControl) {
  ...
  HInstruction* conv = InsertInstruction(
      new (GetArena()) HTypeConversion(DataType::Type::kInt8, basic_[0], kNoDexPc), 0);
...
TEST_F(InductionVarAnalysisTest, ByteInductionDerivedIntLoopControl) {
  ...
  HInstruction* store1 = InsertArrayStore(conv, 0);
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, conv, constant1_), 0);
...
TEST_F(InductionVarAnalysisTest, ByteInduction) {
  ...
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, k_header, constant1_), 0);
  HInstruction* conv = InsertInstruction(
      new (GetArena()) HTypeConversion(DataType::Type::kInt8, add, kNoDexPc), 0);
...
TEST_F(InductionVarAnalysisTest, NoByteInduction2) {
  ...
  HInstruction* conv = InsertInstruction(
      new (GetArena()) HTypeConversion(DataType::Type::kInt8, k_header, kNoDexPc), 0);
  HInstruction* add = InsertInstruction(
      new (GetArena()) HAdd(DataType::Type::kInt32, conv, constant1_), 0);
...
TEST_F(InductionVarAnalysisTest, ByteLoopControl1) {
  ...
  HInstruction* conv =
      new (GetArena()) HTypeConversion(DataType::Type::kInt8, increment_[0], kNoDexPc);
...
TEST_F(InductionVarAnalysisTest, ShortLoopControl1) {
  ...
  HInstruction* conv =
      new (GetArena()) HTypeConversion(DataType::Type::kInt16, increment_[0], kNoDexPc);
  loop_body_[0]->InsertInstructionBefore(conv, increment_[0]->GetNext());
  basic_[0]->ReplaceInput(conv, 1);
  PerformInductionVarAnalysis();
...
TEST_F(InductionVarAnalysisTest, CharLoopControl1) {
  ...
  HInstruction* conv =
      new (GetArena()) HTypeConversion(DataType::Type::kUint16, increment_[0], kNoDexPc);
  loop_body_[0]->InsertInstructionBefore(conv, increment_[0]->GetNext());
  basic_[0]->ReplaceInput(conv, 1);
  PerformInductionVarAnalysis();
