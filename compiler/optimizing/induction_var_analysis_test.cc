...
TEST_F(InductionVarAnalysisTest, FindGeometricDivInductionAndDerived) {
  ...
  EXPECT_STREQ("geo((2) * 100 ^ -i + (0)):PrimInt", GetInductionInfo(mul, 0).c_str());
...
TEST_F(InductionVarAnalysisTest, FindGeometricRemInductionAndDerived) {
  ...
  // Note, only the phi in the cycle is classified.
  EXPECT_STREQ("geo((1) * 100 mod + (0)):PrimInt", GetInductionInfo(k_header, 0).c_str());
  EXPECT_STREQ("geo((1) * 100 mod + (100)):PrimInt", GetInductionInfo(add, 0).c_str());
  // TODO: oops this one is not accurate, fix Monday!
  EXPECT_STREQ("geo((2) * 100 mod + (0)):PrimInt", GetInductionInfo(mul, 0).c_str());
