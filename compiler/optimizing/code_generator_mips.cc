...
void LocationsBuilderMIPS::VisitDiv(HDiv* div) {
  ...
  bool generic_long_div = false;
  if (type == DataType::Type::kInt64) {
    if (div->InputAt(1)->IsConstant()) {
      int64_t imm = CodeGenerator::GetInt64ValueOf(div->InputAt(1)->AsConstant());
      generic_long_div = (imm != 0) && !IsPowerOfTwo(static_cast<uint64_t>(AbsOrMin(imm)));
    } else {
      generic_long_div = true;
    }
  }
  LocationSummary::CallKind call_kind = generic_long_div
...
void LocationsBuilderMIPS::VisitRem(HRem* rem) {
  ...
  bool generic_long_rem;
  if ((type == DataType::Type::kInt64) && rem->InputAt(1)->IsConstant()) {
    int64_t imm = CodeGenerator::GetInt64ValueOf(rem->InputAt(1)->AsConstant());
    generic_long_rem = (imm != 0) && !IsPowerOfTwo(static_cast<uint64_t>(AbsOrMin(imm)));
  } else {
    generic_long_rem = (type != DataType::Type::kInt32);
  }
  LocationSummary::CallKind call_kind = generic_long_rem
      ? LocationSummary::kCallOnMainOnly
      : LocationSummary::kNoCall;
