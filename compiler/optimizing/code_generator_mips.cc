...
void ParallelMoveResolverMIPS::EmitSwap(size_t index) {
  ...
  if (loc1.IsRegister() && loc2.IsRegister()) {
    ...
    // Swap FPR and GPR.
    DCHECK_EQ(type, DataType::Type::kFloat);  // Can only swap a float.
      ...
      if (source.IsRegister()) {
        ...
        if (GetGraph()->HasSIMD()) {
          ...
          if (DataType::Is64BitType(dst_type)) {
            ...
            DCHECK_EQ(dst_type, DataType::Type::kFloat);
...
void LocationsBuilderMIPS::VisitRem(HRem* rem) {
  ...
  LocationSummary::CallKind call_kind =
      (type == DataType::Type::kInt32) ? LocationSummary::kNoCall : LocationSummary::kCallOnMainOnly;
  ...
  if (result_type == DataType::Type::kInt64 && DataType::IsIntegralType(input_type)) {
    ...
    if (input_type == DataType::Type::kInt64) {
      if (isR6) {
        ...
        QuickEntrypointEnum entrypoint = (result_type == DataType::Type::kFloat) ? kQuickL2f
                                                                                : kQuickL2d;
    ...
    if (result_type == DataType::Type::kInt64) {
      if (isR6) {
        ...
        QuickEntrypointEnum entrypoint = (input_type == DataType::Type::kFloat) ? kQuickF2l
                                                                               : kQuickD2l;
        codegen_->InvokeRuntime(entrypoint, conversion, conversion->GetDexPc());
        if (input_type == DataType::Type::kFloat) {
