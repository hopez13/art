...
void InstructionCodeGeneratorMIPS::DivRemOneOrMinusOne(HBinaryOperation* instruction) {
  ...
  if (instruction->GetResultType() == DataType::Type::kInt32) {
    ...
    if (instruction->IsRem()) {
      ...
      } else if ((out_low != in_low) || (out_high != in_high)) {
  ...
  if (instruction->GetResultType() == DataType::Type::kInt32) {
    ...
    if (instruction->IsDiv()) {
      ...
      if (ctz_imm == 1) {
        ...
        __ Sra(AT, in_high, 31);
        if (ctz_imm <= 16) {
          __ Andi(out_low, in_low, (1 << ctz_imm) - 1);
        } else if (codegen_->GetInstructionSetFeatures().IsMipsIsaRevGreaterThanEqual2()) {
        ...
        if (codegen_->GetInstructionSetFeatures().IsR6()) {
          __ Selnez(AT, AT, out_low);
        } else {
          __ Movz(AT, ZERO, out_low);
        }
        __ Move(out_high, AT);
        if (codegen_->GetInstructionSetFeatures().IsMipsIsaRevGreaterThanEqual2()) {
          __ Ins(out_low, AT, ctz_imm, 32 - ctz_imm);
        } else {
          __ Sll(AT, AT, ctz_imm);
          __ Or(out_low, out_low, AT);
        }
      } else if (ctz_imm == 32) {
        __ Sra(AT, in_high, 31);
  ...
  switch (type) {
    ...
    case DataType::Type::kInt64: {
      ...
      if (div->InputAt(1)->IsConstant()) {
        ...
        if (imm == 0) {
          // Do not generate anything. DivZeroCheck would prevent any code to be executed.
        } else if (IsPowerOfTwo(AbsOrMin(imm)) || imm == 1 || imm == -1) {
