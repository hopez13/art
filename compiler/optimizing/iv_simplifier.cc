...
/*Helper Functions*/
static HConstant* GetNewConstant(HInstruction::InstructionKind kind,
                                 HConstant* cst1, HConstant* cst2) {
  ...
  if (control->IsIf()) {
    ...
    if (if_expr->IsCondition()) {
      ...
      HInductionVarAnalysis::InductionInfo* a =
          induction_analysis_->LookupInfo(loop, condition->InputAt(0));
      HInductionVarAnalysis::InductionInfo* b =
          induction_analysis_->LookupInfo(loop, condition->InputAt(1));
      if (HInductionVarAnalysis::InductionEqual(a, phi_info) ||
          HInductionVarAnalysis::InductionEqual(b, phi_info)) {
...
void IvStrengthReduction::PerformReduction(HLoopInformation* loop,
  ...
  HInstruction* new_mul =
      new (allocator) HMul(derived_var->GetType(), input, std::get<1>(val), derived_var->GetDexPc());
  DCHECK(new_mul != nullptr);
  loop->GetPreHeader()->InsertInstructionBefore(new_mul, loop->GetPreHeader()->GetLastInstruction());
  HInstruction* new_add =
      new (allocator) HAdd(new_mul->GetType(), new_mul, std::get<2>(val), derived_var->GetDexPc());
  DCHECK(new_add != nullptr);
  loop->GetPreHeader()->InsertInstructionBefore(new_add,
                                                loop->GetPreHeader()->GetLastInstruction());
  ...
  HConstant* biv_cst =
      GetNewConstant(HInstruction::InstructionKind::kMul, biv_inc_cst, std::get<1>(val));
...
bool IvStrengthReduction::IsCandidateForReduction(
  ...
  if (to_check->IsMul()) {
    ...
    HInductionVarAnalysis::InductionInfo* left_ins_info =
        induction_analysis_->LookupInfo(loop, left_ins);
    HInductionVarAnalysis::InductionInfo* right_ins_info =
        induction_analysis_->LookupInfo(loop, right_ins);
    ...
    HInductionVarAnalysis::InductionInfo* left_ins_info =
        induction_analysis_->LookupInfo(loop, left);
    HInductionVarAnalysis::InductionInfo* right_ins_info =
        induction_analysis_->LookupInfo(loop, right);
    ...
    if (!HInductionVarAnalysis::InductionEqual(left_ins_info, biv_ind_info) &&
      ...
      for (auto it : *candidate) {
        HInductionVarAnalysis::InductionInfo* info =
           induction_analysis_->LookupInfo(loop, it.first);
    ...
    if (IsCandidatePhi(loop, phi)) {
      for (HBlocksInLoopIterator it1(*loop); !it1.Done(); it1.Advance()) {
        ...
        for (HInstructionIterator it2(block->GetInstructions()); !it2.Done(); it2.Advance()) {
          ...
          if (to_check->IsBinaryOperation() && DataType::IsIntegralType(to_check->GetType()))
      ...
      for (auto itr = candidate.begin(); itr != candidate.end(); itr++) {
        ...
        if (var->GetId() != phi->GetId()) {
          ...
          if (derived_var->IsAdd() || derived_var->IsSub()) {
            ...
            HConstant* new_cst =
                GetNewConstant(kind, std::get<2>(val), std::get<2>(candidate.Get(var_bin)));
            ...
            HConstant* new_cst1 =
                GetNewConstant(kind, std::get<2>(val), std::get<1>(candidate.Get(var_bin)));
            HConstant* new_cst2 =
                GetNewConstant(kind, std::get<2>(val), std::get<2>(candidate.Get(var_bin)));
