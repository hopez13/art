...
template <typename CodeGeneratorARMType, typename IntrinsicLocationsBuilderARMType>
static void RunFixup(HGraph* graph, CodeGenerator* codegen) {
...
void DexCacheArrayFixups::Run() {
  if (codegen_->GetCompilerOptions().IsUseVixl()) {
    RunFixup<CodeGeneratorARMVIXL, IntrinsicLocationsBuilderARMVIXL>(graph_, codegen_);
  } else {
    RunFixup<CodeGeneratorARM, IntrinsicLocationsBuilderARM>(graph_, codegen_);
  }
}
