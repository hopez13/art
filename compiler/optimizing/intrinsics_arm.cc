...
void IntrinsicCodeGeneratorARM::VisitStringCompareTo(HInvoke* invoke) {
  ...
  if (mirror::kUseStringCompression) {
    // For compressed strings need to clear 0x7 from temp1, for uncompressed we need to clear 0xf.
    // We also need to prepare the character extraction mask `uncompressed ? 0xffffu : 0xffu`.
  ...
  __ sub(out, temp1, ShifterOperand(temp2));
