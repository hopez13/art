...
static void GenSystemArrayCopyBaseSourceAddress(ArmAssembler* assembler,
                                                Primitive::Type type,
                                                const Register& src,
  ...
  if (src_pos.IsConstant()) {
    ...
    __ AddConstant(src_base, src, element_size * constant + data_offset);
  } else {
    __ add(src_base, src, ShifterOperand(src_pos.AsRegister<Register>(), LSL, element_size_shift));
    __ AddConstant(src_base, data_offset);
  }
}

// Compute base destination address for the System.arraycopy intrinsic in `dst_base`.
static void GenSystemArrayCopyBaseDestinationAddress(ArmAssembler* assembler,
                                                     Primitive::Type type,
                                                     const Register& dst,
  ...
  if (dst_pos.IsConstant()) {
    ...
    __ AddConstant(dst_base, dst, element_size * constant + data_offset);
  } else {
    __ add(dst_base, dst, ShifterOperand(dst_pos.AsRegister<Register>(), LSL, element_size_shift));
    __ AddConstant(dst_base, data_offset);
  }
}

// Compute end source address for the System.arraycopy intrinsic in `src_end`.
static void GenSystemArrayCopyEndSourceAddress(ArmAssembler* assembler,
                                               Primitive::Type type,
  ...
  if (copy_length.IsConstant()) {
    ...
    __ AddConstant(src_end, src_base, element_size * constant);
  } else {
    __ add(src_end,
           src_base,
           ShifterOperand(copy_length.AsRegister<Register>(), LSL, element_size_shift));
  }
}
