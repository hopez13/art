...
// Some comparators used by these tests to avoid having to deal with arenas.
bool operator==(const BlockSet& bs, const ScopedArenaSet<const HBasicBlock*>& sas) {
  return bs.size() == sas.size() && std::all_of(bs.begin(), bs.end(), [&](auto b) {
    return sas.find(b) != sas.end();
  });
}
bool operator==(const ScopedArenaSet<const HBasicBlock*>& sas, const BlockSet& bs) {
  return bs == sas;
}
bool operator!=(const BlockSet& bs, const ScopedArenaSet<const HBasicBlock*>& sas) {
  return !(bs == sas);
