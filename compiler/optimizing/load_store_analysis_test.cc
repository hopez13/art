...
TEST_F(LoadStoreAnalysisTest, ArrayHeapLocations) {
  ...
  HeapLocationCollector heap_location_collector(graph_, &allocator, true);
...
void LoadStoreAnalysisTest::CheckReachability(const AdjacencyListGraph &adj, const std::vector<AdjacencyListGraph::Edge> & reach) {
  uint32_t cnt = 0;
      ...
      for (HBasicBlock* other : graph_->GetBlocks()) {
        ...
        if (adj.HasBlock(other)) {
          if (graph_->PathBetween(blk, other)) {
            cnt++;
            EXPECT_TRUE(std::find(reach.begin(),
                                  reach.end(),
                                  AdjacencyListGraph::Edge { adj.GetName(blk),
                                                             adj.GetName(other) }) != reach.end())
                << "Unexpected edge found between " << adj.GetName(blk) << " and "
                << adj.GetName(other);
          } else {
            EXPECT_TRUE(std::find(reach.begin(),
                                  reach.end(),
                                  AdjacencyListGraph::Edge { adj.GetName(blk),
                                                             adj.GetName(other) }) == reach.end())
                << "Expected edge not found between " << adj.GetName(blk) << " and "
                << adj.GetName(other);
          }
...
TEST_F(LoadStoreAnalysisTest, ReachabilityTest2) {
  AdjacencyListGraph blks(SetupFromAdjacencyList(
      "entry",
      "exit",
      { { "entry", "pre-loop" }, { "pre-loop", "loop" }, { "loop", "pre-loop" } }));
  CheckReachability(blks,
                    {
                        { "entry", "pre-loop" },
                        { "entry", "loop" },
...
static bool AreExclusionsIndependent(const ExecutionSubgraph* esg) {
  ...
  for (auto first = excluded.begin(); first != (excluded.begin() + (excluded.size() - 1));
       ++first) {
    for (auto second = (first + 1); second != excluded.end(); ++second) {
      for (auto entry : first->EntryBlocks()) {
        for (auto exit : second->ExitBlocks()) {
          if (exit->GetGraph()->PathBetween(exit, entry)) {
            return false;
          }
        }
      }
      for (auto entry : second->EntryBlocks()) {
        for (auto exit : first->ExitBlocks()) {
          if (exit->GetGraph()->PathBetween(exit, entry)) {
            return false;
          }
        }
...
TEST_F(LoadStoreAnalysisTest, PartialEscape) {
  ...
  std::unique_ptr<ScopedArenaAllocator> allocator(new ScopedArenaAllocator(graph_->GetArenaStack()));
  LOG(INFO) << "FOOBAR " << reinterpret_cast<uintptr_t>(allocator.get()) << " " << allocator->ApproximatePeakBytes();
  {
