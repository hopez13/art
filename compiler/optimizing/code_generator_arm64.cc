...
Location ARM64ReturnLocation(DataType::Type return_type) {
  ...
  if (return_type == DataType::Type::kFloat) {
    return LocationFrom(s0);
  } else if (return_type == DataType::Type::kDouble) {
 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    codegen->EmitParallelMoves(
        locations->InAt(0), LocationFrom(calling_convention.GetRegisterAt(0)), DataType::Type::kInt32,
        locations->InAt(1), LocationFrom(calling_convention.GetRegisterAt(1)), DataType::Type::kInt32);
...
static bool CoherentConstantAndType(Location constant, DataType::Type type) {
  ...
  return (cst->IsIntConstant() && type == DataType::Type::kInt32) ||
         ...
         (cst->IsFloatConstant() && type == DataType::Type::kFloat) ||
         (cst->IsDoubleConstant() && type == DataType::Type::kDouble);
}
...
void InstructionCodeGeneratorARM64::GenerateDivRemWithAnyConstant(HBinaryOperation* instruction) {
  ...
  int64_t magic;
  int shift;
  CalculateMagicAndShiftForDivRem(imm, type == DataType::Type::kInt64 /* is_long */, &magic, &shift);
