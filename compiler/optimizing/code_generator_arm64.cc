...
void LocationsBuilderARM64::VisitMethodExitHookVoid(HMethodExitHookVoid* method_hook) {
  ...
  locations->SetTempAt(0, LocationFrom(x4));
}
...
void InstructionCodeGeneratorARM64::GenerateExitHook(HInstruction* instruction) {
  ...
  Register should_deopt = WRegisterFrom(instruction->GetLocations()->GetTemp(0));
  __ Ldr(should_deopt, MemOperand(sp, codegen_->GetStackOffsetOfShouldDeoptimizeFlag()));
