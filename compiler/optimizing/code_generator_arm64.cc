...
void LocationsBuilderARM64::HandleFieldGet(HInstruction* instruction,
  ...
  if (object_field_get_with_read_barrier && kUseBakerReadBarrier) {
    ...
    if (Runtime::Current()->UseJitCompilation() || field_info.IsVolatile()) {
      locations->AddTemp(Location::RequiresRegister());
    } else {
      // For AOT non-volatile loads we need a temporary only if the offset is too big.
...
void LocationsBuilderARM64::VisitArrayGet(HArrayGet* instruction) {
  ...
  if (object_array_get_with_read_barrier && kUseBakerReadBarrier) {
    ...
    if (Runtime::Current()->UseJitCompilation() || !instruction->GetIndex()->IsConstant()) {
      locations->AddTemp(Location::RequiresRegister());
    } else {
      // Array loads with constant index are treated as field loads.
      // For AOT constant index loads we need a temporary only if the offset is too big.
      uint32_t offset = CodeGenerator::GetArrayDataOffset(instruction);
      int64_t index = instruction->GetIndex()->AsIntConstant()->GetValue();
...
void CodeGeneratorARM64::GenerateFieldLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  if (!use_load_acquire && !Runtime::Current()->UseJitCompilation()) {
    ...
    UseScratchRegisterScope temps(GetVIXLAssembler());
    temps.Exclude(ip0, ip1);
    uint32_t custom_data = linker::Arm64RelativePatcher::EncodeBakerReadBarrierOffsetData(
        base.GetCode(),
        obj.GetCode());
