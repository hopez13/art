...
// If entrypoint is a valid location it is assumed to already be holding the entrypoint.
class ReadBarrierMarkSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ReadBarrierMarkSlowPathARM64(HInstruction* instruction, Location ref, Location entrypoint)
      : SlowPathCodeARM64(instruction),
        ref_(ref),
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (entrypoint_.IsValid()) {
      ...
      BlockPoolsScope block_pools(arm64_codegen->GetVIXLAssembler());
      __ Blr(XRegisterFrom(entrypoint_));
    } else {
      // Entrypoint is not already loade.d
...
void CodeGeneratorARM64::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  if (always_update_field) {
    ...
    slow_path = new (GetGraph()->GetArena()) ReadBarrierMarkSlowPathARM64(instruction,
                                                                          ref,
                                                                          Location::NoLocation());
  }
  AddSlowPath(slow_path);
