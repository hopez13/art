  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    if (index_.IsValid()) {
      ...
      if (instruction_->IsArrayGet()) {
        ...
        // In the case of the the following intrinsics `index_` is not shifted by a scale factor of
        // 2 (as in the case of ArrayGet), as it is actually an offset to an object field within an
        // object.
