...
void CodeGeneratorARM64::CheckGCCardIsSet(Register object, Register value) {
  UseScratchRegisterScope temps(GetVIXLAssembler());
  Register card = temps.AcquireX();
  Register temp = temps.AcquireW();  // Index within the CardTable - 32bit.
  vixl::aarch64::Label done;
  ...
  __ Cmp(temp, gc::accounting::CardTable::kCardDirty);
  __ B(eq, &done);
  __ Unreachable();  // Force a runtime crash.
  __ Bind(&done);
}
...
void InstructionCodeGeneratorARM64::HandleFieldSet(HInstruction* instruction,
  ...
  if (needs_write_barrier) {
    ...
    if (kEnableRuntimeGCCardChecks &&
      ...
      codegen_->CheckGCCardIsSet(obj, Register(value));
    }
  }
}
...
void InstructionCodeGeneratorARM64::VisitArraySet(HArraySet* instruction) {
  ...
  if (!needs_write_barrier) {
    if (kEnableRuntimeGCCardChecks &&
      ...
      // Check that the dirty bit is already set for elided write barriers.
      DCHECK(!GetGraph()->IsCompilingBaseline());
      codegen_->CheckGCCardIsSet(array, value.W());
    }
