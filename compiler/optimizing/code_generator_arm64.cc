...
void InstructionCodeGeneratorARM64::GenerateMethodEntryExitHook(HInstruction* instruction) {
  ...
  __ Cbz(value, slow_path->GetExitLabel());

  // Check if there is place in buffer, if no, take slow path.
  uint32_t trace_buffer_index_addr = Thread::TraceBufferIndexOffset<kArm64PointerSize>().Int32Value();
  ...
  __ ComputeAddress(addr, MemOperand(addr, index, LSL, 3));
  // Advance the index
