...
// Check whether val is 2^n - 2^m, 0 <= m < n.
// If it is, the found values of n and m are stored into output parameters.
static inline bool Is2PowerNMinus2PowerM(
    uint64_t val,
    /* out */ int* n,
    /* out */ int* m) {
  DCHECK_LT(val, std::numeric_limits<uint64_t>::max());

  uint64_t upper = RoundUpToPowerOfTwo(val);

  if (IsPowerOfTwo(val + 1)) {
    *n = WhichPowerOf2(val + 1);
    *m = 0;
    return true;
  } else if (IsPowerOfTwo(upper - val)) {
    *n = WhichPowerOf2(upper);
    *m = WhichPowerOf2(upper - val);
    return true;
  } else {
    return false;
  }
...
// Check whether val is 2^n + 2^m, 0 <= m < n.
// If it is, the found values of n and m are stored into output parameters.
static inline bool Is2PowerNPlus2PowerM(
    uint64_t val,
    /* out */ int* n,
    /* out */ int* m) {
  DCHECK_GT(val, 2ULL);

  int tzeros_count = CTZ(val);

  if (IsPowerOfTwo(val - 1)) {
    *n = WhichPowerOf2(val - 1);
    *m = 0;
    return true;
  } else if (tzeros_count > 0 && IsPowerOfTwo(val ^ (1ULL << tzeros_count))) {
    *n = WhichPowerOf2(val ^ (1ULL << tzeros_count));
    *m = tzeros_count;
    return true;
  } else {
    return false;
...
void InstructionCodeGeneratorARM64::GenerateResultRemWithAnyConstant(
  ...
  // Check if the divisor is 2^n - 2^m or 2^n + 2^m, 0 <= m < n.
  int n = 0;
  int m = 0;
  if (Is2PowerNPlus2PowerM(divisor, &n, &m)) {
    // rem = dividend - quotient * (2^n + 2^m)
    // = dividend - 2^m * (quotient + quotient * 2^(n - m))
    __ Add(quotient, quotient, Operand(quotient, LSL, n - m));
    if (m == 0) {
      __ Sub(out, dividend, quotient);
    } else {
      __ Sub(out, dividend, Operand(quotient, LSL, m));
    }
  } else if (Is2PowerNMinus2PowerM(divisor, &n, &m)) {
    // rem = dividend - quotient * (2^n - 2^m)
    // = dividend + 2^m * (quotient - quotient * 2^(n - m))
    __ Sub(quotient, quotient, Operand(quotient, LSL, n - m));
    if (m == 0) {
      __ Add(out, dividend, quotient);
    } else {
      __ Add(out, dividend, Operand(quotient, LSL, m));
    }
  } else {
...
void InstructionCodeGeneratorARM64::GenerateInt64DivRemWithAnyConstant(
  ...
  // Check the divisor is not power of 2. The more optimized code for it is
  // generated by GenerateIntDivForPower2Denom/GenerateIntRemForPower2Denom.
  DCHECK(!IsPowerOfTwo(static_cast<uint64_t>(AbsOrMin(imm))));
  if (instruction->IsRem()) {
    // The sign of the HRem result is the sign of the dividend. It does not depend on the sign
    // of the divisor. Positive divisors simplify analysis.
    imm = AbsOrMin(imm);
  }
...
void InstructionCodeGeneratorARM64::GenerateIntDivForConstDenom(HDiv *instruction) {
  ...
  if (IsPowerOfTwo(static_cast<uint64_t>(AbsOrMin(imm)))) {
