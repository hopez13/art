...
void CodeGeneratorARM64::RecordTraceEvent(bool method_entry ATTRIBUTE_UNUSED) {
  if (!kAlwaysOnProfile) {
    return;
  }
  ...
  __ Cmp(mr, 0x1100);
  __ B(ne, &done);
  ThreadOffset64 entrypoint_offset = GetThreadOffset<kArm64PointerSize>(kQuickMethodEntryHook);
