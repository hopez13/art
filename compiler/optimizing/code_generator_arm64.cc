...
class ReadBarrierMarkSlowPathBaseARM64 : public SlowPathCodeARM64 {
 protected:
  ReadBarrierMarkSlowPathBaseARM64(HInstruction* instruction,
                                   Location ref,
                                   Location entrypoint)
...
class RawReferenceLoad : public ArenaObject<kArenaAllocCodeGenerator> {
 public:
  RawReferenceLoad(HInstruction* instruction,
                   Location ref,
                   Register obj,
                   uint32_t offset,
                   Location index,
  ...
  void Emit(CodeGenerator* codegen, bool needs_null_check) {
    ...
    Register ref_reg = RegisterFrom(ref_, type);

    if (index_.IsValid()) {
      // Load types involving an "index": ArrayGet,
      // UnsafeGetObject/UnsafeGetObjectVolatile and UnsafeCASObject
      // intrinsics.
      if (use_load_acquire_) {
        ...
        DCHECK(instruction_->AsInvoke()->GetIntrinsic() == Intrinsics::kUnsafeGetObjectVolatile)
            << instruction_->AsInvoke()->GetIntrinsic();
        DCHECK_EQ(offset_, 0u);
        DCHECK_EQ(scale_factor_, 0u);
        DCHECK_EQ(needs_null_check, false);
        // /* HeapReference<Object> */ ref = *(obj + index)
        MemOperand field = HeapOperand(obj_, XRegisterFrom(index_));
        arm64_codegen->LoadAcquire(instruction_, ref_reg, field, /* needs_null_check */ false);
      } else {
        // ArrayGet and UnsafeGetObject intrinsics cases.
        // /* HeapReference<Object> */ ref = *(obj + offset + (index << scale_factor))
        if (index_.IsConstant()) {
          uint32_t computed_offset = offset_ + (Int64ConstantFrom(index_) << scale_factor_);
          arm64_codegen->Load(type, ref_reg, HeapOperand(obj_, computed_offset));
        } else {
          UseScratchRegisterScope temps(masm);
          Register temp = temps.AcquireW();
          __ Add(temp, obj_, offset_);
          arm64_codegen->Load(
              type, ref_reg, HeapOperand(temp, XRegisterFrom(index_), LSL, scale_factor_));
        }
      }
    } else {
      // /* HeapReference<Object> */ ref = *(obj + offset)
      MemOperand field = HeapOperand(obj_, offset_);
      if (use_load_acquire_) {
        // A possible implicit null check is handled below.
        arm64_codegen->LoadAcquire(instruction_, ref_reg, field, /* needs_null_check */ false);
      } else {
        arm64_codegen->Load(type, ref_reg, field);
      }
    }

    if (needs_null_check) {
      arm64_codegen->MaybeRecordImplicitNullCheck(instruction_);
    }

    // Object* ref = ref_addr->AsMirrorPtr()
    arm64_codegen->GetAssembler()->MaybeUnpoisonHeapReference(ref_reg);
  }
...
// object `obj` into `ref` using via `raw_ref_load`, and marking `ref`
// if `obj` is gray according to the lock word (Baker read
// barrier). The field `obj.field` in the object `obj` holding this
// reference does not get updated by this slow path after marking (see
// LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathARM64 below
// for that).
//
// This means that after the execution of this slow path, `ref` will
// always be up-to-date, but `obj.field` may not; i.e., after the
...
class LoadReferenceWithBakerReadBarrierSlowPathARM64 : public ReadBarrierMarkSlowPathBaseARM64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    __ Tbz(temp_, LockWord::kReadBarrierStateShift, &done);
    GenerateReadBarrierMarkRuntimeCall(codegen);
    __ Bind(&done);
 ...
 private:
  ...
  RawReferenceLoad* raw_ref_load_;
...
// object `obj` into `ref` using via `raw_ref_load`, and marking `ref`
// if `obj` is gray according to the lock word (Baker read
// barrier). If needed, this slow path also atomically updating the
// field `obj.field` in the object `obj` holding this reference after
// marking (contrary to LoadReferenceWithBakerReadBarrierSlowPathARM64
...
void CodeGeneratorARM64::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  RawReferenceLoad* raw_ref_load = new (GetGraph()->GetArena()) RawReferenceLoad(
      instruction, ref, obj, offset, index, scale_factor, use_load_acquire);
  ...
  if (always_update_field) {
    // LoadReferenceWithBakerReadBarrierAndUpdateFieldSlowPathARM64 only supports
    ...
    slow_path = new (GetGraph()->GetArena()) LoadReferenceWithBakerReadBarrierSlowPathARM64(
        instruction, raw_ref_load, ref, obj, needs_null_check, temp, LocationFrom(temp2));
  }
  AddSlowPath(slow_path);
