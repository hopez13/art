...
CodeGeneratorARM64::CodeGeneratorARM64(HGraph* graph,
    ...
    : CodeGenerator(graph,
      ...
      codegen_patches_(&assembler_, graph->GetAllocator()),
...
void CodeGeneratorARM64::MaybeIncrementHotness(bool is_frame_entry) {
  ...
  if (GetGraph()->IsCompilingBaseline() && !Runtime::Current()->IsAotCompiler()) {
    ...
    __ Ldr(temp, codegen_patches_.DeduplicateUint64Literal(address));
  ...
  switch (load_kind) {
    ...
    case MethodLoadKind::kJitDirectAddress: {
      ...
      __ Ldr(XRegisterFrom(temp),
             codegen_patches_.DeduplicateUint64Literal(
...
void CodeGeneratorARM64::EmitJitRootPatches(uint8_t* code, const uint8_t* roots_data) {
  codegen_patches_.EmitJitRootPatches(code, roots_data, *GetCodeGenerationData());
}
