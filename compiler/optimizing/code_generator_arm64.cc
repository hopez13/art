...
void InstructionCodeGeneratorARM64::GenerateInt32DivRemWithAnyConstant(
  ...
  if (NeedToCorrectMulResult(magic, imm)) {
    ...
    // As between 'lsr temp.X(), temp.X(), #32' and 'asr temp.X(), temp.X(), #shift' there are
    // no other instructions modifying 'temp', they can be combined into one
    // 'asr temp.X(), temp.X(), #32 + shift'.
    // This results in no need to do the final right shift.
    // When shift is 0, 'asr temp.X(), temp.X(), #32' produces the same low 32 bits as
    // 'lsr temp.X(), temp.X(), #32'. As only those low bits are used later by the 'add'
    // instruction it is safe to use 'asr' instead.
    DCHECK_LT(shift, 32);
