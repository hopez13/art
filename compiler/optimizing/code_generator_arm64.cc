...
class LoadClassSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    arm64_codegen->RefreshMarkingRegister();
    __ B(GetExitLabel());
  }
...
void InstructionCodeGeneratorARM64::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  codegen_->RefreshMarkingRegister();
}
...
static bool TryGenerateIntrinsicCode(HInvoke* invoke, CodeGeneratorARM64* codegen) {
  if (invoke->GetLocations()->Intrinsified()) {
    ...
    codegen->RefreshMarkingRegister();
    return true;
  }
  return false;
}
...
void CodeGeneratorARM64::RefreshMarkingRegister() {
  __ Ldr(mr.W(), MemOperand(tr, Thread::IsGcMarkingOffset<kArm64PointerSize>().Int32Value()));
}
  ...
  if (read_barrier_option == kWithReadBarrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      if (kBakerReadBarrierLinkTimeThunksEnableForGcRoots &&
        ...
        //     if (mr != nullptr) {  // Thread::Current()->GetIsGcMarking()
