...
void InstructionCodeGeneratorARM64::VisitPredicatedInstanceFieldGet(
  ...
  vixl::aarch64::Label use_normal_field_get, finish;
  __ Cbnz(InputRegisterAt(instruction, 0), &use_normal_field_get);
  if (DataType::IsFloatingPointType(instruction->GetType())) {
    __ Move(OutputFPRegister(instruction), InputFPRegisterAt(instruction, 1));
  } else {
    __ Move(OutputRegister(instruction), InputRegisterAt(instruction, 1));
  }
  __ B(&finish);
