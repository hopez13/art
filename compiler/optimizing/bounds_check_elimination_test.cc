...
TEST_F(BoundsCheckEliminationTest, OverflowArrayBoundsElimination) {
  ...
  HBasicBlock* block2 = new (GetAllocator()) HBasicBlock(graph_);
  graph_->AddBlock(block2);
  HInstruction* add = new (GetAllocator()) HAdd(DataType::Type::kInt32, parameter2, constant_max_int);
  HNullCheck* null_check = new (GetAllocator()) HNullCheck(parameter1, 0);
  HArrayLength* array_length = new (GetAllocator()) HArrayLength(null_check, 0);
...
TEST_F(BoundsCheckEliminationTest, UnderflowArrayBoundsElimination) {
  ...
  HBasicBlock* block2 = new (GetAllocator()) HBasicBlock(graph_);
  graph_->AddBlock(block2);
  HInstruction* sub1 = new (GetAllocator()) HSub(DataType::Type::kInt32, parameter2, constant_max_int);
...
TEST_F(BoundsCheckEliminationTest, ModArrayBoundsElimination) {
  ...
  HBoundsCheck* bounds_check_i_mod_200 = new (GetAllocator()) HBoundsCheck(i_mod_200, constant_10, 0);
  array_set = new (GetAllocator()) HArraySet(
  ...
  // array[i % -10] = 10;
  HRem* i_mod_minus_10 = new (GetAllocator()) HRem(DataType::Type::kInt32, phi, constant_minus_10, 0);
  ...
  HArrayLength* array_length = new (GetAllocator()) HArrayLength(null_check, 0);
  HRem* i_mod_array_length = new (GetAllocator()) HRem(DataType::Type::kInt32, phi, array_length, 0);
