...
TEST_F(NodeTest, RemoveInstruction) {
  ...
  HInstruction* null_check = new (GetArena()) HNullCheck(parameter, 0);
  first_block->AddInstruction(null_check);
  first_block->AddInstruction(new (GetArena()) HReturnVoid());
  ...
  HEnvironment* environment = new (GetArena()) HEnvironment(
      GetArena(), 1, graph->GetArtMethod(), 0, null_check);
...
TEST_F(NodeTest, InsertInstruction) {
  ...
  entry->AddInstruction(new (GetArena()) HExit());

  ASSERT_FALSE(parameter1->HasUses());

  HInstruction* to_insert = new (GetArena()) HNullCheck(parameter1, 0);
...
TEST_F(NodeTest, AddInstruction) {
  ...
  HInstruction* to_add = new (GetArena()) HNullCheck(parameter, 0);
...
TEST_F(NodeTest, ParentEnvironment) {
  ...
  HInstruction* parameter1 = new (GetArena()) HParameterValue(
      graph->GetDexFile(), dex::TypeIndex(0), 0, DataType::Type::kReference);
  HInstruction* with_environment = new (GetArena()) HNullCheck(parameter1, 0);
  entry->AddInstruction(parameter1);
  ...
  HEnvironment* environment = new (GetArena()) HEnvironment(
      GetArena(), 1, graph->GetArtMethod(), 0, with_environment);
  ArenaVector<HInstruction*> array(GetArena()->Adapter());
  ...
  HEnvironment* parent1 = new (GetArena()) HEnvironment(
      GetArena(), 1, graph->GetArtMethod(), 0, nullptr);
  parent1->CopyFrom(array);

  ASSERT_EQ(parameter1->GetEnvUses().SizeSlow(), 2u);

  HEnvironment* parent2 = new (GetArena()) HEnvironment(
      GetArena(), 1, graph->GetArtMethod(), 0, nullptr);
  parent2->CopyFrom(array);
  parent1->SetAndCopyParentChain(GetArena(), parent2);
  ...
  environment->SetAndCopyParentChain(GetArena(), parent1);
