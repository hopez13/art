...
class FastCompilerARM64 : public FastCompiler {
 public:
  ...
  ArrayRef<const uint8_t> GetStackMaps() const override {
    ScopedArenaVector<uint8_t> stack_map = code_generation_data_->GetStackMapStream()->Encode();
    return ArrayRef<const uint8_t>(stack_map.data(), stack_map.size());
  }
 ...
 private:
  ...
  // Generate code to check the inline cache and call the helper if not hit.
  void GenerateInlineCacheCheck(Register klass, uint32_t dex_pc);
  ...
  void BuildLoadString(
      uint32_t vreg, dex::StringIndex string_index, uint32_t dex_pc, const Instruction* next);
  ...
  // The profiling information object which the generated code will update.
  ...
  // The current mask to know which dex register is an object.
...
void FastCompilerARM64::GenerateInlineCacheCheck(Register klass, uint32_t dex_pc) {
  // The entrypoint expects the class to be in W0.
  DCHECK_EQ(klass.GetCode(), 0u);
  // We know the destination of an intrinsic, so no need to record inline
  // caches.
  if (Runtime::Current()->IsAotCompiler() || profiling_info_ == nullptr) {
    return;
  }
  UseScratchRegisterScope temps(GetVIXLAssembler());
  Register temp = temps.AcquireX();
  InlineCache* cache = profiling_info_->GetInlineCache(dex_pc);
  uint64_t address = reinterpret_cast64<uint64_t>(cache);
  vixl::aarch64::Label done;
  // x8 is expected to contain the profiling info address in the kQuickUpdateInlineCache
  // entrypoint.
  __ Mov(x8, address);
  __ Ldr(temp.W(), MemOperand(x8, InlineCache::ClassesOffset().Int32Value()));
  // Fast path for a monomorphic cache.
  __ Cmp(klass, temp);
  __ B(eq, &done);
  InvokeRuntime(kQuickUpdateInlineCache, dex_pc);
  __ Bind(&done);
}
...
void FastCompilerARM64::DropFrameAndReturn() {
  ...
  if (has_frame_) {
    ...
    DCHECK_EQ(frame_size, 0);
    __ Ret();
  }
  GetAssembler()->cfi().RestoreState();
  GetAssembler()->cfi().DefCFAOffset(frame_size);
}

void FastCompilerARM64::GenerateFrame() {
  if (has_frame_) {
  ...
  // Increment hotness.
  if (!Runtime::Current()->IsAotCompiler() && profiling_info_ != nullptr) {
    uint64_t address = reinterpret_cast64<uint64_t>(profiling_info_);
    UseScratchRegisterScope temps(masm);
    Register temp = temps.AcquireX();
  ...
  size_t offset = 0u;
  {
    ...
    resolved_method = method_->SkipAccessChecks()
        ? class_linker->ResolveMethod<ClassLinker::ResolveMode::kNoChecks>(
              self, method_index, method_, invoke_type)
  ...
  if (invoke_type != kStatic) {
    ...
    if (can_be_null || invoke_type == kVirtual || invoke_type == kInterface) {
      ...
      if (invoke_type == kVirtual || invoke_type == kInterface) {
        GenerateInlineCacheCheck(kArtMethodRegister, dex_pc);
      }
    }
  }
...
void FastCompilerARM64::BuildLoadString(uint32_t vreg,
  ...
  __ Ldr(dst.W(), MemOperand(dst.X(), 0));
  __ Cbz(mr, &exit);
  int32_t entry_point_offset =
      Thread::ReadBarrierMarkEntryPointsOffset<kArm64PointerSize>(dst.GetCode());
  __ Ldr(lr, MemOperand(tr, entry_point_offset));
  __ Blr(lr);
  __ Bind(&exit);
  UpdateLocal(vreg, /* is_object= */ true);
}
...
void FastCompilerARM64::BuildNewInstance(uint32_t vreg,
  ...
  if (klass == nullptr || !method_->GetDeclaringClass()->CanAccess(klass) || klass->IsStringClass()) {
  ...
  __ Ldr(cls_reg.W(), jit_patches_.DeduplicateJitClassLiteral(GetDexFile(),
                                                              ...
                                                              code_generation_data_.get()));
  vixl::aarch64::Label exit;
  __ Ldr(cls_reg.W(), MemOperand(cls_reg.X(), 0));
  __ Cbz(mr, &exit);
  int32_t entry_point_offset =
      Thread::ReadBarrierMarkEntryPointsOffset<kArm64PointerSize>(cls_reg.GetCode());
  __ Ldr(lr, MemOperand(tr, entry_point_offset));
  __ Blr(lr);
  __ Bind(&exit);
...
void FastCompilerARM64::BuildCheckCast(uint32_t vreg, dex::TypeIndex type_index, uint32_t dex_pc) {
  ...
  __ Ldr(cls.W(), jit_patches_.DeduplicateJitClassLiteral(GetDexFile(),
                                                          ...
                                                          h_klass ,
                                                          code_generation_data_.get()));
  __ Ldr(cls.W(), MemOperand(cls.X(), 0));
  __ Ldr(obj_cls.W(), MemOperand(obj.X(), 0));
  __ Cmp(cls.W(), obj_cls.W());
  __ B(eq, &exit);

  __ Cbz(mr, &read_barrier_exit);
  // Read barrier on the GC Root.
  int32_t entry_point_offset =
      Thread::ReadBarrierMarkEntryPointsOffset<kArm64PointerSize>(cls.GetCode());
  __ Ldr(lr, MemOperand(tr, entry_point_offset));
  __ Blr(lr);

  // Read barrier on the object's class.
  entry_point_offset =
      Thread::ReadBarrierMarkEntryPointsOffset<kArm64PointerSize>(obj_cls.GetCode());
  __ Ldr(lr, MemOperand(tr, entry_point_offset));
  __ Blr(lr);
...
#define IF_XX(comparison, cond) \
    ...
    case Instruction::IGET_OBJECT:
      // Generate a frame as we may have a read barrier slow path.
      GenerateFrame();
      FALLTHROUGH_INTENDED;
    ...
    case Instruction::IGET_SHORT: {
      ...
      ArtField* field = nullptr;
      {
        ...
        if (field == nullptr) {
          soa.Self()->ClearException();
          unimplemented_reason_ = "UnresolvedIGet";
          return false;
        }
        if (field->IsVolatile()) {
          unimplemented_reason_ = "VolatileIGet";
          return false;
        }
      }
      ...
      switch (instruction.Opcode()) {
        ...
        case Instruction::IGET_OBJECT: {
          ...
          __ blr(lr);
          __ Bind(&exit);
          // Early return as we have recorded the pc info for the null check already.
          return true;
        }
        default:
    ...
    case Instruction::IPUT_SHORT: {
      ...
      if (vreg_locations_[source_or_dest_reg].IsConstant()) {
        assigning_constant = true;
        src = Location::RegisterLocation(temps.AcquireW().GetCode());
        MoveLocation(src, vreg_locations_[source_or_dest_reg], DataType::Type::kInt32);
      } else {
        src = vreg_locations_[source_or_dest_reg];
        if (src.IsStackSlot() || src.IsDoubleStackSlot()) {
...
#define ARRAY_XX(kind, anticipated_type)                                          \
    ...
    case Instruction::CONST_STRING: {
      ...
      BuildLoadString(instruction.VRegA_21c(), string_index, dex_pc, next);
      return true;
    }

    case Instruction::CONST_STRING_JUMBO: {
      dex::StringIndex string_index(instruction.VRegB_31c());
      BuildLoadString(instruction.VRegA_31c(), string_index, dex_pc, next);
      return true;
    }
...
std::unique_ptr<FastCompiler> FastCompiler::CompileARM64(
  ...
  if (!compiler_options.GetImplicitNullChecks() ||
      ...
      !compiler_options.GetImplicitSuspendChecks() ||
  ...
  std::unique_ptr<arm64::FastCompilerARM64> compiler(new arm64::FastCompilerARM64(
      method, profiling_info, allocator, arena_stack, handles, compiler_options, dex_compilation_unit));
  if (compiler->Compile()) {
    return compiler;
  }
  LOG(ERROR) << "Did not compile because of " << compiler->GetUnimplementedReason();
  return nullptr;
}
