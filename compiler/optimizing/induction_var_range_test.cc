...
class InductionVarRangeTest : public OptimizingUnitTest {
 public:
  InductionVarRangeTest()
      : graph_(CreateGraph()),
        iva_(new (GetArena()) HInductionVarAnalysis(graph_)),
        range_(iva_) {
    BuildGraph();
  }
  ...
  void BuildGraph() {
    ...
    entry_block_ = new (GetArena()) HBasicBlock(graph_);
    exit_block_ = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(entry_block_);
    graph_->AddBlock(exit_block_);
    graph_->SetEntryBlock(entry_block_);
    graph_->SetExitBlock(exit_block_);
    // Two parameters.
    x_ = new (GetArena()) HParameterValue(graph_->GetDexFile(),
                                           dex::TypeIndex(0),
                                           0,
                                           DataType::Type::kInt32);
    entry_block_->AddInstruction(x_);
    y_ = new (GetArena()) HParameterValue(graph_->GetDexFile(),
                                           dex::TypeIndex(0),
                                           0,
                                           DataType::Type::kInt32);
  ...
  void BuildLoop(int32_t lower, HInstruction* upper, int32_t stride) {
    // Control flow.
    loop_preheader_ = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(loop_preheader_);
    loop_header_ = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(loop_header_);
    loop_body_ = new (GetArena()) HBasicBlock(graph_);
    graph_->AddBlock(loop_body_);
    HBasicBlock* return_block = new (GetArena()) HBasicBlock(graph_);
    ...
    // Instructions.
    loop_preheader_->AddInstruction(new (GetArena()) HGoto());
    HPhi* phi = new (GetArena()) HPhi(GetArena(), 0, 0, DataType::Type::kInt32);
    loop_header_->AddPhi(phi);
    phi->AddInput(graph_->GetIntConstant(lower));  // i = l
    if (stride > 0) {
      condition_ = new (GetArena()) HLessThan(phi, upper);  // i < u
    } else {
      condition_ = new (GetArena()) HGreaterThan(phi, upper);  // i > u
    }
    loop_header_->AddInstruction(condition_);
    loop_header_->AddInstruction(new (GetArena()) HIf(condition_));
    increment_ =
        new (GetArena()) HAdd(DataType::Type::kInt32, phi, graph_->GetIntConstant(stride));
    loop_body_->AddInstruction(increment_);  // i += s
    phi->AddInput(increment_);
    loop_body_->AddInstruction(new (GetArena()) HGoto());
    return_block->AddInstruction(new (GetArena()) HReturnVoid());
    exit_block_->AddInstruction(new (GetArena()) HExit());
  }
...
TEST_F(InductionVarRangeTest, ArrayLengthAndHints) {
  ...
  HInstruction* new_array = new (GetArena()) HNewArray(x_, x_, 0);
  entry_block_->AddInstruction(new_array);
  HInstruction* array_length = new (GetArena()) HArrayLength(new_array, 0);
