 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    DCHECK((V0 <= ref_reg && ref_reg <= T2) ||
           ...
           (ref_reg == S8)) << ref_reg;
    DCHECK(0 <= ref_reg && ref_reg < kNumberOfGpuRegisters) << ref_reg;
    ...
    if (entrypoint_.IsValid()) {
      ...
      int32_t entry_point_offset =
          CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kMips64PointerSize>(ref_reg);
  ...
  if (read_barrier_option == kWithReadBarrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      const int32_t entry_point_offset =
          CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kMips64PointerSize>(root.reg());
