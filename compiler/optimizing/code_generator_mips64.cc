...
Location Mips64ReturnLocation(DataType::Type return_type) {
  switch (return_type) {
    ...
    case DataType::Type::kFloat:
    case DataType::Type::kDouble:
  ...
  if (destination.IsRegister() || destination.IsFpuRegister()) {
    if (unspecified_type) {
      ...
      if (source.IsStackSlot() ||
        ...
        dst_type = destination.IsRegister() ? DataType::Type::kInt32 : DataType::Type::kFloat;
      } else {
        ...
        // move.
        dst_type = destination.IsRegister() ? DataType::Type::kInt64 : DataType::Type::kDouble;
      }
    }
    ...
    if (source.IsStackSlot() || source.IsDoubleStackSlot()) {
      ...
      if (dst_type == DataType::Type::kInt32 || dst_type == DataType::Type::kFloat) {
      ...
      if (dst_type == DataType::Type::kFloat) {
        __ Mtc1(gpr, destination.AsFpuRegister<FpuRegister>());
      } else if (dst_type == DataType::Type::kDouble) {
      ...
      if (destination.IsFpuRegister()) {
        if (GetGraph()->HasSIMD()) {
          ...
          if (dst_type == DataType::Type::kFloat) {
            ...
            DCHECK_EQ(dst_type, DataType::Type::kDouble);
    ...
    if (source.IsRegister() || source.IsFpuRegister()) {
      if (unspecified_type) {
        if (source.IsRegister()) {
          ...
          dst_type = destination.IsStackSlot() ? DataType::Type::kFloat : DataType::Type::kDouble;
        }
      }
...
void CodeGeneratorMIPS64::SwapLocations(Location loc1, Location loc2, DataType::Type type) {
  ...
  if (loc2.IsRegister() && loc1.IsRegister()) {
    ...
    if (type == DataType::Type::kFloat) {
      __ MovS(FTMP, r1);
      __ MovS(r1, r2);
      __ MovS(r2, FTMP);
    } else {
      DCHECK_EQ(type, DataType::Type::kDouble);
      __ MovD(FTMP, r1);
      __ MovD(r1, r2);
      __ MovD(r2, FTMP);
    }
...
static bool CanMoveConditionally(HSelect* select, LocationSummary* locations_to_set) {
  ...
  DataType::Type cond_type = materialized ? DataType::Type::kInt32 : condition->InputAt(0)->GetType();
...
void InstructionCodeGeneratorMIPS64::VisitRem(HRem* instruction) {
  ...
  switch (type) {
    ...
    case DataType::Type::kFloat:
    case DataType::Type::kDouble: {
      QuickEntrypointEnum entrypoint = (type == DataType::Type::kFloat) ? kQuickFmodf : kQuickFmod;
      codegen_->InvokeRuntime(entrypoint, instruction, instruction->GetDexPc());
      if (type == DataType::Type::kFloat) {
...
void InstructionCodeGeneratorMIPS64::VisitTypeConversion(HTypeConversion* conversion) {
  ...
  if (DataType::IsIntegralType(result_type) && DataType::IsIntegralType(input_type)) {
    ...
    if (input_type == DataType::Type::kInt64) {
      __ Dmtc1(src, FTMP);
      if (result_type == DataType::Type::kFloat) {
        __ Cvtsl(dst, FTMP);
      } else {
        __ Cvtdl(dst, FTMP);
      }
    ...
    if (result_type == DataType::Type::kInt64) {
      if (input_type == DataType::Type::kFloat) {
        __ TruncLS(FTMP, src);
      } else {
        __ TruncLD(FTMP, src);
      }
