...
class BCEVisitor : public HGraphVisitor {
 ...
 private:
  ...
  void AddCompareWithDeoptimization(HBasicBlock* block,
    ...
    InsertDeoptInBlock(bounds_check, new (GetGraph()->GetAllocator()) HAboveOrEqual(upper, array_length));
    ...
    if ((base != nullptr || min_c >= 0) &&  // reject certain OOB
      ...
      if (base == nullptr) {
        ...
        if (min_c != max_c) {
          DCHECK(min_lower == nullptr && min_upper != nullptr &&
                 max_lower == nullptr && max_upper != nullptr);
          InsertDeoptInLoop(loop, block, new (GetGraph()->GetAllocator()) HAbove(min_upper, max_upper));
        } else {
          DCHECK(min_lower == nullptr && min_upper == nullptr &&
                 max_lower == nullptr && max_upper != nullptr);
        }
        ...
        if (min_c != max_c) {
          DCHECK(min_lower != nullptr && min_upper != nullptr &&
                 max_lower != nullptr && max_upper != nullptr);
          InsertDeoptInLoop(loop, block, new (GetGraph()->GetAllocator()) HAbove(min_lower, max_lower));
        } else {
          DCHECK(min_lower == nullptr && min_upper == nullptr &&
                 max_lower != nullptr && max_upper != nullptr);
        }
        InsertDeoptInLoop(loop, block, new (GetGraph()->GetAllocator()) HAbove(max_lower, max_upper));
      }
      InsertDeoptInLoop(
  ...
  void InsertDeoptInBlock(HBoundsCheck* bounds_check, HInstruction* condition) {
    ...
    HDeoptimize* deoptimize = new (GetGraph()->GetAllocator()) HDeoptimize(
        GetGraph()->GetAllocator(), condition, DeoptimizationKind::kBlockBCE, bounds_check->GetDexPc());
