...
static bool TryToIntrinsify(HInvokePolymorphic* invoke) {
  ...
  // exaustive: `invoke-static` method call with one argument also matches this condition.
...
void IntrinsicLocationsBuilderX86_64::VisitMethodHandleInvokeExact(HInvoke* invoke) {
  if (TryToIntrinsify(invoke->AsInvokePolymorphic())) {
    ArenaAllocator* allocator = invoke->GetBlock()->GetGraph()->GetAllocator();
...
void IntrinsicCodeGeneratorX86_64::VisitMethodHandleInvokeExact(HInvoke* invoke) {
  DCHECK(TryToIntrinsify(invoke->AsInvokePolymorphic()))
      << "IntrinsicLocationsBuilderX86_64::VisitMethodHandleInvokeExact was updated";
