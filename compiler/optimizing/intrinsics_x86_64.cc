...
static void GenerateVarHandleArrayChecks(HInvoke* invoke,
  ...
  __ movl(temp, Address(varhandle, coordinate_type1_offset.Int32Value()));
  __ testl(temp, temp);
  __ j(kZero, slow_path->GetEntryLabel());
  __ movl(temp, Address(varhandle, coordinate_type0_offset.Int32Value()));
  ...
  __ movl(temp2, Address(object, class_offset.Int32Value()));
  ...
  __ movl(temp2, Address(temp, component_type_offset.Int32Value()));
  codegen->GetAssembler()->MaybeUnpoisonHeapReference(temp2);
  ...
  if (primitive_type == Primitive::kPrimNot) {
    ...
    __ cmpw(Address(temp2, primitive_type_offset), Immediate(static_cast<uint16_t>(0)));
    ...
    // back to slow path.
    __ cmpw(Address(temp2, primitive_type_offset), Immediate(static_cast<uint16_t>(primitive_type)));
    __ j(kNotZero, slow_path->GetEntryLabel());
  }
...
static LocationSummary* CreateVarHandleCommonLocations(HInvoke* invoke) {
  ...
  if (expected_coordinates_count == 0u) {
    ...
    locations->AddTemp(Location::RequiresRegister());
  } else if (expected_coordinates_count == 2u) {
    // Add a second temporary for `GenerateVarHandleArrayChecks`.
    locations->AddTemp(Location::RequiresRegister());
  }
