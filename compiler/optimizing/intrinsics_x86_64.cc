...
static void CreateUnsafeCASLocations(ArenaAllocator* allocator,
  ...
  locations->SetOut(Location::RequiresRegister());
  if (type == DataType::Type::kReference) {
    // Need temporary registers for card-marking, and possibly for (Baker) read barrier.
    locations->AddTemp(Location::RequiresRegister());  // Possibly used for reference poisoning too.
    locations->AddTemp(Location::RequiresRegister());
  }

  // RAX is clobbered in CMPXCHG, so it should be added as a temporary.
...
static void CreateVarHandleCompareAndSetOrExchangeLocations(HInvoke* invoke) {
  ...
  if (DataType::IsFloatingPointType(type)) {
    // A temporary is needed to load the new floating-point value into a register for CMPXCHG.
    locations->AddTemp(Location::RequiresRegister());
    // RAX is needed to load the expected floating-point value for CMPXCHG.
    locations->AddTemp(Location::RegisterLocation(RAX));
  } else {
    ...
    if (type == DataType::Type::kReference) {
      // References need two temps (for MarkGCCard and GenerateReferenceLoadWithBakerReadBarrier).
      locations->AddTemp(Location::RequiresRegister());
      locations->AddTemp(Location::RequiresRegister());
    }
    // Add RAX as a temporary, as it gets clobbered in CMPXCHG.
    locations->AddTemp(Location::RegisterLocation(RAX));
  }
}
...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke,
  ...
  // Make sure RAX is the last temporary, so that we don't pass it as a free temporary.
  // RAX is used by CMPXCHG and shouldn't be used for anything else.
  DCHECK_EQ(RAX, locations->GetTemp(temp_count - 1).AsRegister<Register>());
  ...
  GenCompareAndSetOrExchange(codegen,
                             ...
                             /*temp1_index=*/ temp_count - 2,
                             /*temp2_index=*/ temp_count - 3,
