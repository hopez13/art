...
static void GenerateVarHandleGetAndSet(HInvoke* invoke,
  ...
  if (DataType::IsFloatingPointType(type)) {
    ...
    __ movd(out.AsFpuRegister<XmmRegister>(), temp.AsRegister<CpuRegister>());
    ...
    if (kPoisonHeapReferences) {
      __ PoisonHeapReference(valreg);
      __ xchgl(valreg, field_addr);
      __ UnpoisonHeapReference(valreg);
    } else {
      __ xchgl(valreg, field_addr);
    }
...
void IntrinsicCodeGeneratorX86_64::VisitVarHandleGetAndSetAcquire(HInvoke* invoke) {
  // `getAndSetAquire` has `getAcquire` + `set` semantics, so it doesn't need any barriers.
  GenerateVarHandleGetAndSet(
