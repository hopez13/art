...
static void GenCompareAndSetOrExchangeInt(CodeGeneratorX86_64* codegen,
  ...
  if (is_cmpxchg) {
    ...
    switch (type) {
      case DataType::Type::kBool:
        __ movzxb(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      case DataType::Type::kInt8:
        __ movsxb(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      case DataType::Type::kInt16:
        __ movsxw(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      case DataType::Type::kUint16:
        __ movzxw(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      case DataType::Type::kInt32:
        __ movsxd(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      case DataType::Type::kUint32:
        __ movl(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      case DataType::Type::kInt64:
      case DataType::Type::kUint64:
        __ movq(out_loc.AsRegister<CpuRegister>(), CpuRegister(RAX));
        break;
      default:
        LOG(FATAL) << "Unexpected non-integral CAS type " << type;
    }
  } else {
    GenZFlagToResult(assembler, out_loc.AsRegister<CpuRegister>());
  }
}
...
static void GenCompareAndSetOrExchangeFloat(CodeGeneratorX86_64* codegen,
  ...
  if (is_cmpxchg) {
    __ movd(out_loc.AsFpuRegister<XmmRegister>(), CpuRegister(RAX), is64bit);
  } else {
    GenZFlagToResult(assembler, out_loc.AsRegister<CpuRegister>());
  }
}
...
static void GenCompareAndSetOrExchangeRef(CodeGeneratorX86_64* codegen,
  ...
  if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
    ...
    __ movl(out.AsRegister<CpuRegister>(), field_addr);
