...
static void GenerateVarHandleGetAndOp(CodeGeneratorX86_64* codegen,
                                      LocationSummary* locations,
                                      uint32_t value_index,
                                      DataType::Type type,
                                      Address field_addr,
  ...
  // Move the new value into a temporary register. Do this before loading the old value from memory
  // into RAX, because the location of the new value may be RAX (if this function is called from the
  // byte swapping path of GenerateVarHandleGetAndAdd).
  codegen->Move(temp_loc, value);
  DCHECK_NE(temp.AsRegister(), RAX);
...
static void CreateVarHandleGetAndAddLocations(HInvoke* invoke) {
  ...
  if (DataType::IsFloatingPointType(type)) {
    ...
    if (GetExpectedVarHandleCoordinatesCount(invoke) == 2) {
      // A temporary may be needed for byte swap with byte array views.
      locations->AddTemp(Location::RequiresRegister());
    }
  }
}
  ...
  if (byte_swap) {
    if (DataType::IsFloatingPointType(type)) {
      ...
      GenerateVarHandleGetAndOp(
          codegen, locations, value_index, type, field_addr, GetAndUpdateOp::kAdd, byte_swap);
    }
    ...
    CpuRegister temp = locations->GetTemp(temp_count - 1).AsRegister<CpuRegister>();
...
static void GenerateVarHandleGetAndUpdate(HInvoke* invoke,
  ...
  switch (get_and_update_op) {
    case GetAndUpdateOp::kSet:
      ...
      break;
    case GetAndUpdateOp::kAdd:
      GenerateVarHandleGetAndAdd(codegen, locations, value_index, type, field_addr, byte_swap);
      break;
    case GetAndUpdateOp::kBitwiseAnd:
    case GetAndUpdateOp::kBitwiseOr:
    case GetAndUpdateOp::kBitwiseXor:
      GenerateVarHandleGetAndOp(
          codegen, locations, value_index, type, field_addr, get_and_update_op, byte_swap);
      break;
  }
