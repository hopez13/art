...
static bool CanIntrinsifyInvokeExact(HInvokePolymorphic* invoke) {
...
void IntrinsicLocationsBuilderX86_64::VisitMethodHandleInvokeExact(HInvoke* invoke) {
  if (CanIntrinsifyInvokeExact(invoke->AsInvokePolymorphic())) {
    ArenaAllocator* allocator = invoke->GetBlock()->GetGraph()->GetAllocator();
    ...
    // TODO(b/297147201): probably 0th input register can be re-used instead.
    locations->AddTemp(Location::RequiresRegister());
...
void IntrinsicCodeGeneratorX86_64::VisitMethodHandleInvokeExact(HInvoke* invoke) {
  DCHECK(CanIntrinsifyInvokeExact(invoke->AsInvokePolymorphic()))
  ...
  HLoadMethodType* load_method_type =
      new (graph->GetAllocator()) HLoadMethodType(graph->GetCurrentMethod(),
                                                  ...
                                                  graph->GetDexFile(),
                                                  invoke->GetDexPc());
  // load_method_type won't be added to the graph. Block is need to access graph for its allocator
  // in CreateLoadMethodTypeRuntimeCallLocationSummary.
