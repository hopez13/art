...
static void CreateVarHandleGetAndBitwiseOpLocations(HInvoke* invoke) {
  ...
  locations->AddTemp(Location::RequiresRegister());
  // We need value in a register in order to perform the arithmetic operation on it.
  locations->SetInAt(new_value_index, Location::RequiresRegister());
...
static void GenerateVarHandleGetAndOp(HInvoke* invoke,
  ...
  CpuRegister value = value_loc.AsRegister<CpuRegister>();
  ...
  // as it has alredy been done by `LoadFromMemoryNoReference` above (and not altered by CMPXCHG).
  if (byte_swap) {
...
static void GenerateVarHandleGetAndAdd(HInvoke* invoke,
  ...
  if (DataType::IsFloatingPointType(type)) {
    ...
    // If necessary, byte swap RAX back before doing CMPXCHG as it has to match the value in memory.
    if (byte_swap) {
      codegen->GetInstructionCodegen()->Bswap(Location::RegisterLocation(RAX), bswap_type);
    }
    if (is64bit) {
    ...
    if (byte_swap) {
      ...
      __ movq(temp.AsRegister<CpuRegister>(), value.AsRegister<CpuRegister>());
