...
static void CreateVarHandleSetLocations(HInvoke* invoke) {
  ...
  if (GetExpectedVarHandleCoordinatesCount(invoke) > 1) {
    // For byte array views we may need to BSWAP value inplace in register (and then restore it).
    uint32_t value_index = invoke->GetNumberOfArguments() - 1;
    if (locations->InAt(value_index).IsConstant()) {
      // For constants BSWAP can be done at compile time.
    } else if (DataType::IsFloatingPointType(GetDataTypeFromShorty(invoke, value_index))) {
