...
static void CreateVarHandleGetAndBitwiseOpLocations(HInvoke* invoke) {
  ...
  // Require the input value in register, and use it as a temporary to compute the bitwise
  // operation. It doesn't have to be RBX, but we need to choose some to make sure it's the same.
  locations->SetInAt(new_value_index, Location::RegisterLocation(RBX));
  locations->AddTemp(Location::RegisterLocation(RBX));
...
static void GenerateVarHandleGetAndBitwiseOp(HInvoke* invoke,
  ...
  // Use the new value register as first operand and destination of the bitwise operations, as the
  ...
  bool is64Bit = (type == DataType::Type::kInt64 || type == DataType::Type::kUint64);
