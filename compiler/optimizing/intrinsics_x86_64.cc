...
static void CreateUnsafeCASLocations(ArenaAllocator* allocator,
  ...
  if (type == DataType::Type::kReference) {
    // Need two temporaries for MarkGCCard and three for GenerateReferenceLoadWithBakerReadBarrier.
    locations->AddTemp(Location::RequiresRegister());  // Possibly used for reference poisoning too.
    locations->AddTemp(Location::RequiresRegister());
    locations->AddTemp(Location::RequiresRegister());
  }
...
static void GenCompareAndSetOrExchangeRef(CodeGeneratorX86_64* codegen,
                                          ...
                                          CpuRegister temp3,
                                          bool is_cmpxchg) {
  // The only supported read barrier implementation supporting the is the Baker-style read barriers.
  DCHECK(!kEmitCompilerReadBarrier || kUseBakerReadBarrier);
...
static void GenCompareAndSetOrExchange(CodeGeneratorX86_64* codegen,
  ...
  if (DataType::IsFloatingPointType(type)) {
    ...
    if (type == DataType::Type::kReference) {
      ...
      CpuRegister temp3 = locations->GetTemp(temp3_index).AsRegister<CpuRegister>();
      DCHECK(RegsAreAllDifferent({base, offset, temp1, temp2, temp3, CpuRegister(RAX)}));
...
static void CreateVarHandleCompareAndSetOrExchangeLocations(HInvoke* invoke) {
  ...
  if (DataType::IsFloatingPointType(expected_type)) {
    ...
    if (expected_type == DataType::Type::kReference) {
      // MarkGCCard needs 2 temps, and GenerateReferenceLoadWithBakerReadBarrier needs 3 temps.
      locations->AddTemp(Location::RequiresRegister());
      locations->AddTemp(Location::RequiresRegister());
      locations->AddTemp(Location::RequiresRegister());
    }
