...
#if defined(__BIONIC__)
  sigset64_t mask;
  // The uc_sigmask field in bionic's struct ucontext is a sigset_t with the RT
  // signals being stored in manually added padding bytes.
  sigset64_t* uc_sigmask = reinterpret_cast<sigset64_t*>(&ucontext->uc_sigmask);
  sigorset(&mask, uc_sigmask, &chains[signo].action_.sa_mask);
#else
  sigset_t mask;
