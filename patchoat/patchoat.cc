...
static bool VerifySymlink(const std::string& intended_target,
...
static bool VerifyVdexAndOatSymlinks(const std::string& input_image_filename,
  ...
  return VerifySymlink(GetVdexLocationFromImageLocation(input_image_filename),
                       GetVdexLocationFromImageLocation(output_image_filename))
      && VerifySymlink(GetOatLocationFromImageLocation(input_image_filename),
                       GetOatLocationFromImageLocation(output_image_filename));
}

bool PatchOat::CreateVdexAndOatSymlinks(
    const std::string& input_image_filename,
    const std::string& output_image_filename) {
  std::string input_vdex_filename =
      GetVdexLocationFromImageLocation(input_image_filename);
  std::string input_oat_filename =
      GetOatLocationFromImageLocation(input_image_filename);
...
bool PatchOat::Patch(const std::string& image_location,
  ...
  for (size_t i = 0; i < spaces.size(); ++i) {
    ...
    if (output_image) {
      ...
      if (!CreateVdexAndOatSymlinks(input_image_filename,
                                    output_image_filename))
        return false;
...
bool PatchOat::Verify(const std::string& image_location,
  ...
  for (size_t i = 0; i < spaces.size(); ++i) {
    ...
    if (!VerifyVdexAndOatSymlinks(original_image_filename,
