...
void PatchOat::PatchDexFileArrays(mirror::ObjectArray<mirror::Object>* img_roots) {
  ...
  for (size_t i = 0, count = dex_caches->GetLength(); i < count; ++i) {
    ...
    mirror::CallSiteDexCacheType* orig_call_sites = orig_dex_cache->GetResolvedCallSites();
    mirror::CallSiteDexCacheType* relocated_call_sites =
        RelocatedAddressOfPointer(orig_call_sites);
    copy_dex_cache->SetField64<false>(
