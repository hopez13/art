...
class ProfileCompilationInfo {
 public:
  ...
  // Returns the aggregation counter the the given method.
  // CHECKs that the profile is configured to store aggregations counters.
  uint16_t GetMethodAggregationCounter(const MethodReference& method_ref) const;
  // Returns the aggregation counter the the given class.
  // CHECKs that the profile is configured to store aggregations counters.
  uint16_t GetClassAggregationCounter(const TypeReference& type_ref) const;
  // Returns the number of time the profile was merged.
  // CHECKs that the profile is configured to store aggregations counters.
 ...
 private:
  ...
  struct DexFileData : public DeletableArenaObject<kArenaAllocProfile> {
    ...
    uint16_t GetMethodAggregationCounter(uint16_t method_index) const;
    uint16_t GetClassAggregationCounter(uint16_t type_index) const;
