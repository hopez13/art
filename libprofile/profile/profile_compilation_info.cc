...
enum class ProfileCompilationInfo::FileSectionType : uint32_t {
  ...
  // Extra descriptors for referencing classes from a different dex file,
  // even outside of the dex files in the profile.
  kExtraDescriptors = 1,
 ...
 * where `profile_key` is a null-delimited string.
 *
 * ExtraDescriptors:
 *    number_of_extra_descriptors
 *    (extra_descriptor)[number_of_extra_descriptors]
 * where `extra_descriptor` is a null-delimited string.
 *
...
void ProfileCompilationInfo::DexFileData::WriteMethods(SafeBuffer& buffer) const {
  ...
  for (const auto& method_entry : method_map) {
    ...
    for (const auto& inline_cache_entry : inline_cache_map) {
      ...
      // TODO(calin): in case we miss types there is still value to add the rest of the
