...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  if (lseek(fd, sizeof(FileHeader), SEEK_SET) != 0) {
    return false;
  }
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::ReadSectionData(
  ...
  // FIXME: Seek to the right position.
...
void ProfileCompilationInfo::DexFileData::WriteClasses(SafeBuffer& buffer) const {
  ...
  WriteClassSet(class_set);
}

ProfileCompilationInfo::ProfileLoadStatus ReadClasses(
    SafeBuffer& buffer,
    const dchecked_vector<uint16_t>& extra_strings_remap,
    std::string* error) {
  ...
  uint16_t num_valid_indexes = dchecked_integral_cast<uint16_t>(
      std::min<size_t>(extra_strings_remap.size(), DexFile::kNoDexIndex16));
  uint16_t type_index = 0u;
  for (size_t i = 0; i != clases_size; ++i) {
    ...
    if (type_index_diff >= num_valid_indexes - type_index) {
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::SkipClasses(SafeBuffer& buffer,
                                                                              std::string* error) {
  uint16_t classes_size;
  if (!buffer.ReadUintAndAdvance(&classes_size)) {
  ...
  size_t following_data_size = static_cast<size_t>(classes_size) * sizeof(uint16);
...
uint32_t ProfileCompilationInfo::DexFileData::MethodsDataSize() const {
  auto is_zero = [](uint8_t b) { return b == 0u; };
  if (method_map.empty() && std::all_of(bitmap_storage.begin(), bitmap_storage.end(), is_zero)) {
    return 0u;
  }

  size_t num_hot_methods = dex_data.method_map.size();
  size_t dex_pc_entries = 0u;
  size_t class_entries = 0u;
  for (const auto& method_it : method_map) {
    const InlineCacheMap& inline_cache_map = entry.second;
    dex_pc_entries += inline_cache_map.size();
    for (const auto& inline_cache_entry : inline_cache_map) {
      const DexPcData& dex_pc_data = inline_cache_entry.second;
      class_entries += dex_pc_data.classes.size();
    }
  }

  constexpr size_t kPerMethodSize =
  ...
  return sizeof(ProfileIndexType) +                 // Which dex file.
         ...
         sizeof(uint32_t) +                         // Bitmap size.
         bitmap_storage.size() +                    // Bitmap data.
         num_hot_methods * kPerHotMethodSize +      // Data for hot methods.
         num_dex_pc_entries * kPerDexPcEntrySize +  // Data for dex pc entries.
         class_entries * kPerClassEntrySize;        // Data for inline cache class entries.
}
...
void ProfileCompilationInfo::DexFileData::WriteMethods(SafeBuffer& buffer) const {
  ...
  // Write the size of the bitmap storage.
  // FIXME: What about writing the bit size or the number of bits per method instead?
  buffer.WriteUintAndAdvance(dchecked_integral_cast<uint32_t>(bitmap_storage.size());
  // Write the bitmap storage.
  buffer.WriteAndAdvance(bitmap_storage.begin(), bitmap_storage.size());

  DCHECK(!method_map.empty());
  uint16_t last_method_index = 0;
  for (const auto& entry : dex_data.method_map) {
    uint16_t method_index = entry.first;
    const InlineCacheMap& inline_cache_map = entry.second;
...
ProfileCompilationInfo::ProfileLoadStatus ReadMethods(
    SafeBuffer& buffer,
    const dchecked_vector<uint16_t>& extra_strings_remap,
    std::string* error) {
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::SkipMethods(SafeBuffer& buffer,
                                                                              std::string* error) {
  uint32_t following_data_size;
  if (!buffer.ReadUintAndAdvance(following_data_size)) {
