 ...
 *   class_counters,method_counters
 *   The class and mehtod counters are only stored if the profile version is
 *   kProfileVersionWithCounters
...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  for (const DexFileData* dex_data_ptr : info_) {
    ...
    if (StoresAggregationCounters()) {
      required_capacity += sizeof(uint16_t) +  // class size
          sizeof(uint16_t) * dex_data.class_set.size() +
          sizeof(uint16_t) +  // method size
          sizeof(uint16_t) * dex_data.method_map.size();
    }
  }
  if (StoresAggregationCounters()) {
  ...
  for (const DexFileData* dex_data_ptr : info_) {
    ...
    if (StoresAggregationCounters()) {
      ...
      AddUintToBuffer(&buffer, static_cast<uint16_t>(dex_data.method_map.size()));
      for (const auto& method_it : dex_data.method_map) {
        uint16_t method_idx = method_it.first;
...
bool ProfileCompilationInfo::ReadAggregationCounters(
  ...
  uint16_t class_size;
  READ_UINT(uint16_t, buffer, class_size, error);
  if (class_size != dex_data.class_set.size()) {
  ...
  READ_UINT(uint16_t, buffer, method_size, error);
  if (method_size != dex_data.method_map.size()) {
    *error = "Invalid class size when reading counters";
    return false;
  }
  for (const auto& method_it : dex_data.method_map) {
    READ_UINT(uint16_t, buffer, dex_data.method_counters[method_it.first], error);
  }
...
bool ProfileCompilationInfo::MergeWith(const ProfileCompilationInfo& other,
  ...
  for (const DexFileData* other_dex_data : other.info_) {
    ...
    for (const auto& other_method_it : other_dex_data->method_map) {
      ...
      if (StoresAggregationCounters()) {
        dex_data->method_counters[other_method_index]++;
      }
...
uint16_t ProfileCompilationInfo::DexFileData::GetMethodAggregationCounter(uint16_t method_idx) const {
...
uint16_t ProfileCompilationInfo::DexFileData::GetClassAggregationCounter(uint16_t type_idx) const {
  CHECK_GT(class_counters.size(), type_idx) << "Profile not prepared for aggregation counters";
  if (!GetHotnessInfo(type_idx).IsInProfile()) {
    return -1;
  }
...
uint16_t ProfileCompilationInfo::GetMethodAggregationCounter(
      const MethodReference& method_ref) const {
...
uint16_t ProfileCompilationInfo::GetClassAggregationCounter(const TypeReference& type_ref) const {
  CHECK(StoresAggregationCounters()) << "Profile not prepared for aggregation counters";
  const DexFileData* dex_data = FindDexData(type_ref.dex_file);
  return dex_data == nullptr ? -1 : dex_data->GetMethodAggregationCounter(type_ref.index);
}
...
void ProfileCompilationInfo::DexFileData::PrepareForAggregationCounters() {
  ...
  // TODO(calim): we should store the maximum number of types in the profile.
