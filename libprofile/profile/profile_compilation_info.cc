...
enum class ProfileCompilationInfo::ProfileLoadStatus : uint32_t {
  ...
  kVersionMismatch,
  kBadData,
  kMergeError,  // Too many extra strings or classes without TypeId in the dex file.
};
...
enum class ProfileCompilationInfo::FileSectionType : uint32_t {
  ...
  // Extra strings for referencing classes (and possibly method names) from
  // a different dex file, even outside of the dex files in the profile.
  kExtraStrings = 1,
...
class ProfileCompilationInfo::FileSectionInfo {
 public:
  // Constructor for reading from a `ProfileSource`. Data shall be filled by the source.
  FileSectionInfo() {}
...
class ProfileCompilationInfo::FileHeader {
 public:
  ...
  // Constructor for writing to a file.
  explicit FileHeader(uint32_t file_section_count)
    ...
    memcpy(version_, kProfileVersion, sizeof(kProfileVersion));
  ...
  bool IsValid() const {
    return memcmp(magic_, kProfileMagic, sizeof(kProfileMagic)) == 0 &&
           memcmp(version_, kProfileVersion, sizeof(kProfileVersion)) == 0 &&
  ...
  uint32_t GetSizeOfHeaderAndSectionInformation() const {
    return sizeof(FileHeader) + GetFileSectionCount() * sizeof(FileSectionInfo);
  }
...
// Also used for writing where we only DCHECK() we do not write beyond the end.
...
ProfileCompilationInfo::ProfileCompilationInfo(ArenaPool* custom_arena_pool, bool for_boot_image)
    : default_arena_pool_(),
      ...
      extra_strings_(),
      extra_strings_indexes_(ExtraStringIndexHash(&extra_strings_),
                             ExtraStringIndexEquals(&extra_strings_)),
      is_for_boot_image_(for_boot_image) {
}
 ...
 * FIXME: Describe the format.
 *   FileHeader
 *   FileSectionInfo[]
 *   Section:DexFiles - mandatory
 *   Section:ExtraStrings - optional
...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  uint64_t extra_strings_section_size = 0u;
  if (!extra_strings_.empty()) {
    extra_strings_section_size += sizeof(uint16_t);  // Number of strings.
    for (const std::string& str : extra_strings_) {
      extra_strings_section_size += str.size() + 1u;  // Null-terminated string.
    }
  }
  ...
  const uint32_t file_section_count =
      /* dex files */ 1u +
      /* extra_strings */ (extra_strings_section_size != 0u ? 1u : 0u) +
  ...
  // Write the extra strings section.
  if (extra_strings_section_size != 0u) {
    SafeBuffer buffer(dex_files_section_size);
    buffer.WriteUintAndAdvance(dchecked_integral_cast<uint16_t>(extra_strings_.size()));
    for (const std::string& str : extra_strings_) {
      buffer.WriteAndAdvance(str.c_str(), str.size() + 1u);
    }
    if (!buffer.Deflate()) {
      return false;
    }
    ...
    add_section_info(FileSectionType::kExtraStrings, buffer.Size(), extra_strings_section_size);
  }
  ...
  FileHeader header(section_index);
...
ProfileCompilationInfo::ExtraStringIndex ProfileCompilationInfo::AddExtraString(
    std::string_view extra_string) {
  DCHECK(extra_strings_indexes_.find(extra_string) == extra_strings_indexes_.end());
  ExtraStringIndex new_extra_string_index = extra_strings_.size();
  DCHECK_LE(new_extra_string_index, kMaxExtraStrings);
  if (UNLIKELY(new_extra_string_index == kMaxExtraStrings)) {
    return kMaxExtraStrings;  // Cannot add another extra string.
  }
  // Add the extra string and record the new index.
  extra_strings_.emplace_back(extra_string);
  extra_strings_indexes_.insert(new_extra_string_index);
  return new_extra_string_index;
}

dex::TypeIndex ProfileCompilationInfo::FindOrCreateTypeIndex(const DexFile& referrer_dex_file,
                                                             TypeReference class_ref) {
  if (class_ref.dex_file == &referrer_dex_file) {
  ...
  const dex::TypeId* type_id = referrer_dex_file.FindTypeId(descriptor);
  if (type_id != nullptr) {
    return referrer_dex_file.GetIndexForTypeId(*type_id);
  }
  // Try to find an existing extra string.
  uint32_t num_type_ids = referrer_dex_file.NumTypeIds();
  uint32_t max_artificial_ids = DexFile::kDexNoIndex16 - num_type_ids;
  std::string_view descriptor_view(descriptor);
  auto it = extra_strings_indexes_.find(descriptor_view);
  if (it != extra_strings_indexes_.end()) {
    return (*it < max_artificial_ids) ? dex::TypeIndex(num_type_ids + *it) : dex::TypeIndex();
  }
  // Check if inserting the extra string yields a valid artificial type index.
  if (UNLIKELY(extra_strings_.size() >= max_artificial_ids)) {
    return dex::TypeIndex();  // Invalid.
  }
  // Add the descriptor to extra strings and return the artificial type index.
  ExtraStringIndex new_extra_string_index = AddExtraString(descriptor_view);
  DCHECK_NE(new_extra_string_index, kMaxExtraStrings);
  return dex::TypeIndex(num_type_ids + new_extra_string_index);
}
        ...
        for (const auto& inline_cache_it : inline_cache_map) {
          ...
          for (const dex::TypeIndex& type_index : dex_pc_data.classes) {
            if (type_index.index_ >= dex_data->num_type_ids) {
              CHECK_LT(type_index.index_ - dex_data->num_type_ids, extra_strings_.size());
            }
          }
        }
      }
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::ReadDexFilesSection(
  ...
  for (ProfileIndexType i = 0u; i != num_dex_files; ++i) {
    ...
    const void* null_char = memchr(buffer.GetCurrentPtr(), 0, buffer.GetAvailableBytes());
    if (null_char == nullptr) {
      *error += "Missing terminating null character for profile key";
      return ProfileLoadStatus::kBadData;
    }
    size_t profile_key_size = reinterpret_cast<const uint8_t*>(null_char) - buffer.GetCurrentPtr();
    if (profile_key_size == 0u || profile_key_size > kMaxDexFileKeyLength) {
      *error = "ProfileKey has an invalid size: " +
          std::to_string(static_cast<uint32_t>(profile_key_size));
      return ProfileLoadStatus::kBadData;
    }
    std::string profile_key(reinterpret_cast<const char*>(buffer.GetCurrentPtr()),
                            profile_key_size);
    buffer.Advance(profile_key_size + 1u);
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::ReadExtraStringsSection(
    ProfileSource& source,
    const FileSectionInfo& section_info,
    /*out*/ dchecked_vector<uint16_t>* extra_strings_remap,
    /*out*/ std::string* error) {
  DCHECK(section_info.GetType() == FileSectionType::kExtraStrings);
  SafeBuffer buffer;
  ProfileLoadStatus status = ReadSectionData(source, section_info, &buffer, error);
  if (status != ProfileLoadStatus::kSuccess) {
    return status;
  }
  uint16_t num_extra_strings;
  if (!buffer.ReadUintAndAdvance(&num_extra_strings)) {
    *error = "Error reading number of extra strings.";
    return ProfileLoadStatus::kBadData;
  }
  // Note: We allow multiple extra strings sections in a single profile file
  // but that can lead to `kMergeError` if there are too many extra strings.
  // Other sections can reference only extra strings from preceding sections.
  extra_strings_remap->reserve(std::min<size_t>(extra_strings_remap->size() + num_extra_strings,
                                                std::numeric_limits<uint16_t>::max()));
  for (uint16_t i = 0; i != num_extra_strings; ++i) {
    const void* null_char = memchr(buffer.GetCurrentPtr(), 0, buffer.GetAvailableBytes());
    if (null_char == nullptr) {
      *error += "Missing terminating null character for extra string.";
      return ProfileLoadStatus::kBadData;
    }
    size_t extra_string_size = reinterpret_cast<const uint8_t*>(null_char) - buffer.GetCurrentPtr();
    std::string_view extra_string(reinterpret_cast<const char*>(buffer.GetCurrentPtr()),
                                  extra_string_size);
    buffer.Advance(extra_string_size + 1u);
    // Try to match an existing extra string.
    auto it = extra_strings_indexes_.find(extra_string);
    if (it != extra_strings_indexes_.end()) {
      extra_strings_remap->push_back(*it);
      continue;
    }
    // Try to insert a new extra string.
    ExtraStringIndex extra_string_index = AddExtraString(extra_string);
    if (extra_string_index == kMaxExtraStrings) {
      *error = "Too many extra strings.";
      return ProfileLoadStatus::kMergeError;
    }
    extra_strings_remap->push_back(extra_string_index);
  }
  return ProfileLoadStatus::kSuccess;
}
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::ReadClassesSection(
  ...
  while (buffer.GetAvailableBytes() != 0u) {
    ...
    if (profile_index == MaxProfileIndex()) {
      ...
      status = info_[profile_index]->ReadClasses(buffer, extra_strings_remap, error);
    }
    if (status != ProfileLoadStatus::kSuccess) {
      return status;
    }
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::ReadMethodsSection(
  ...
  while (buffer.GetAvailableBytes() != 0u) {
    ...
    if (profile_index == MaxProfileIndex()) {
      ...
      DCHECK_EQ(is_for_boot_image_, info_[profile_index]->is_for_boot_image) << profile_index;
      status = info_[profile_index]->ReadMethods(buffer, extra_strings_remap, error);
      if (!is_for_boot_image_ && info_[profile_index]->is_for_boot_image) {
        // We loaded profile data for boot image. Uprade all other dex file
        // records for boot image data collection.
        for (const std::unique_ptr<DexFileData>& dex_data : info_) {
          if (!dex_data->is_for_boot_image) {
            dex_data->UpgradeForBootImageDataCollection();
          }
        }
        is_for_boot_image_ = true;
      }
    }
    if (status != ProfileLoadStatus::kSuccess) {
      return status;
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::LoadInternal(
  ...
  dchecked_vector<uint16_t> extra_strings_remap;
    ...
    switch (section_info.GetType()) {
      case FileSectionType::kDexFiles:
        *error = "Unsupported additional dex files section";
        status = ProfileLoadStatus::kBadData;
        break;
      case FileSectionType::kExtraStrings:
        status = ReadExtraStringsSection(*source, section_info, &extra_strings_remap, error);
        break;
      case FileSectionType::kClasses:
        if (merge_classes) {
          status = ReadClassesSection(
              *source, section_info, dex_profile_index_remap, extra_strings_remap, error);
        }
        break;
...
bool ProfileCompilationInfo::MergeWith(const ProfileCompilationInfo& other,
  ...
  // Then merge extra strings.
  dchecked_vector<uint16_t> extra_strings_remap;
  extra_strings_remap.reserve(other.extra_strings_.size());
  for (const std::string& other_extra_string : other.extra_strings_) {
    ExtraStringIndex extra_string_index = AddExtraString(other_extra_string);
    if (extra_string_index == kMaxExtraStrings) {
      // Too many strings.
      return false;
    }
  ...
  for (const std::unique_ptr<DexFileData>& other_dex_data : other.info_) {
    ...
    if (merge_classes) {
      ...
      // in the dex file first, followed by classes using extra strings.
      ...
      for (auto end = other_dex_data->class_set.end(); it != end; ++it) {
        uint32_t new_extra_string_index = extra_strings_remap[it->index_ - num_type_ids];
        if (new_extra_string_index >= DexFile::kDexNoIndex16 - num_type_ids) {
          // Cannot represent the type with new extra string index.
          return false;
        }
        dex_data->class_set.insert(dex::TypeIndex(num_type_ids + new_extra_string_index));
      }
    }
        ...
        if (other_ic_it.second.is_missing_types) {
          ...
          for (dex::TypeIndex type_index : other_class_set) {
            if (type_index.index_ >= num_type_ids) {
              uint32_t new_extra_string_index =
                  extra_strings_remap[type_index.index_ - num_type_ids];
              if (new_extra_string_index >= DexFile::kDexNoIndex16 - num_type_ids) {
                // Cannot represent the type with new extra string index.
                return false;
              }
              type_index = dex::TypeIndex(num_type_ids + new_extra_string_index);
            }
            dex_pc_data->AddClass(type_index);
          }
        }
...
std::string ProfileCompilationInfo::DumpInfo(const std::vector<const DexFile*>& dex_files,
  ...
  for (size_t k = 0; k <  kProfileVersionSize - 1; k++) {
    ...
    os << static_cast<char>(kProfileVersion[k]);
  }
  os << "]\n";
  ...
  for (const std::unique_ptr<DexFileData>& dex_data : info_) {
    ...
    for (dex::TypeIndex type_index : dex_data->class_set) {
      if (dex_file != nullptr) {
        if (type_index.index_ < dex_data->num_type_ids) {
          os << "\n\t\t" << dex_file->PrettyType(type_index);
        } else {
          os << "\n\t\t" << extra_strings_[type_index.index_ - dex_data->num_type_ids];
        }
      } else {
...
void ProfileCompilationInfo::DexFileData::UpgradeForBootImageDataCollection() {
  DCHECK(!is_for_boot_image);
  is_for_boot_image = true;
...
uint16_t ProfileCompilationInfo::DexFileData::GetUsedBitmapFlags() const {
  ...
  for (uint32_t flag = MethodHotness::kFlagFirst; flag <= lastFlag; flag = flag << 1) {
    ...
    if (method_bitmap.HasSomeBitSet(index * num_method_ids, num_method_ids)) {
      used_flags |= dchecked_integral_cast<uint16_t>(flag);
    }
  }
  return used_flags;
}
  ...
  for (const DexFile* dex_file : dex_files) {
    ...
    if (data != nullptr) {
      for (dex::TypeIndex type_idx : data->class_set) {
        DCHECK(type_idx.IsValid());
        if (type_idx.index_ < data->num_type_ids) {
          ret.insert(dex_file->StringByTypeIdx(type_idx));
        } else {
          ret.insert(extra_strings_[type_idx.index_ - data->num_type_ids]);
        }
      }
    } else {
...
void ProfileCompilationInfo::ClearData() {
  ...
  info_.clear();
  extra_strings_indexes_.clear();
  extra_strings_.clear();
}
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::DexFileData::ReadClasses(
  ...
  uint16_t num_valid_type_indexes = dchecked_integral_cast<uint16_t>(
      std::min<size_t>(num_type_ids + extra_strings_remap.size(), DexFile::kDexNoIndex16));
  ...
  for (size_t i = 0; i != classes_size; ++i) {
    ...
    if (type_index >= num_type_ids) {
      uint32_t new_extra_string_index = extra_strings_remap[type_index - num_type_ids];
      if (new_extra_string_index >= DexFile::kDexNoIndex16 - num_type_ids) {
        *error = "Remapped type index out of range.";
        return ProfileLoadStatus::kMergeError;
      }
      type_index = num_type_ids + new_extra_string_index;
    }
...
uint32_t ProfileCompilationInfo::DexFileData::MethodsDataSize() const {
  uint16_t method_flags = GetUsedBitmapFlags();
  size_t saved_bitmap_bit_size = POPCOUNT(method_flags) * num_method_ids;
  if (!method_map.empty()) {
    method_flags |= enum_cast<uint32_t>(MethodHotness::kFlagHot);
  }
  if (method_flags == 0u) {
    return 0u;  // No data to write.
  }
  ...
  size_t saved_bitmap_byte_size = RoundUp(saved_bitmap_bit_size, kBitsPerByte) / kBitsPerByte;
  return sizeof(ProfileIndexType) +                 // Which dex file.
...
void ProfileCompilationInfo::DexFileData::WriteMethods(SafeBuffer& buffer) const {
  ...
  size_t saved_bitmap_bit_size = POPCOUNT(method_flags) * num_method_ids;
  if (!method_map.empty()) {
    method_flags |= enum_cast<uint16_t>(MethodHotness::kFlagHot);
  }
  if (method_flags == 0u) {
    return;  // No data to write.
  }

  uint32_t methods_data_size = MethodsDataSize();
  DCHECK_NE(methods_data_size, 0u);
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::DexFileData::ReadMethods(
  ...
  if (!is_for_boot_image && method_flags >= (MethodHotness::kFlagLastRegular << 1)) {
    // The profile we're loading contains data for boot image. Upgrade the type.
    UpgradeForBootImageDataCollection();
  }
  ...
  for (uint32_t flag = MethodHotness::kFlagFirst; flag <= lastFlag; flag = flag << 1) {
    ...
    if ((method_flags & flag) != 0u) {
      size_t index = FlagBitmapIndex(static_cast<MethodHotness::Flag>(flag));
      BitMemoryRegion src = saved_bitmap.Subregion(index * num_method_ids, num_method_ids);
      method_bitmap.OrBits(saved_bitmap_index * num_method_ids, src, num_method_ids);
      ++saved_bitmap_index;
    }
  }
  buffer.Advance(saved_bitmap_byte_size);
  ...
  if ((method_flags & MethodHotness::kFlagHot) != 0u) {
    uint32_t num_valid_method_indexes = std::min<uint32_t>(0x10000u, num_method_ids);
    uint16_t num_valid_type_indexes = dchecked_integral_cast<uint16_t>(
        std::min<size_t>(num_type_ids + extra_strings_remap.size(), DexFile::kDexNoIndex16));
    ...
    while (buffer.GetAvailableBytes() > expected_available_bytes_at_end) {
      ...
      for (uint16_t ic_index = 0; ic_index != inline_cache_size; ++ic_index) {
        ...
        if (inline_cache_size == kIsMissingTypesEncoding) {
          dex_pc_data->SetIsMissingTypes();
        } else if (inline_cache_size == kIsMegamorphicEncoding) {
          dex_pc_data->SetIsMegamorphic();
        } else if (inline_cache_size >= kIndividualInlineCacheSize) {
          *error = "Inline cache size too large.";
          return ProfileLoadStatus::kBadData;
        } else {
          uint16_t type_index = 0u;
          for (size_t i = 0; i != inline_cache_size; ++i) {
            uint16_t type_index_diff;
            if (!buffer.ReadUintAndAdvance(&type_index_diff)) {
  ...
  if (buffer.GetAvailableBytes() != expected_available_bytes_at_end) {
    // We can get here if method flags do not specify hot methods,
    // or for one byte over-read for hot method data.
...
void ProfileCompilationInfo::DexFileData::WriteClassSet(
  ...
  // Store the difference between the type indices for better compression.
