...
static uint16_t IncrementAggregationCounter(uint16_t counter, uint16_t value) {
  if (counter < (std::numeric_limits<uint8_t>::max() - value)) {
...
bool ProfileCompilationInfo::MergeWith(const ProfileCompilationInfo& other,
  ...
  for (const DexFileData* other_dex_data : other.info_) {
    ...
    if (merge_classes) {
      ...
      if (StoresAggregationCounters()) {
        for (const dex::TypeIndex& type_idx : other_dex_data->class_set) {
          uint16_t ammount = 1 +  // this aggregation
              (other.StoresAggregationCounters()
                  ? other_dex_data->class_counters[type_idx.index_]
                  : 1);
          dex_data->class_counters[type_idx.index_] =
            IncrementAggregationCounter(dex_data->class_counters[type_idx.index_], ammount);
        }
      }
    }
    ...
    // Merge counters for methods. Must be done after we merge the bitmaps so that
    // we include the (post) startup methods as well.
    if (StoresAggregationCounters()) {
      for (uint16_t method_idx = 0; method_idx < other_dex_data->num_method_ids; method_idx++) {
        if (dex_data->GetHotnessInfo(method_idx).IsInProfile()) {
          ...
          dex_data->method_counters[method_idx] =
              IncrementAggregationCounter(dex_data->method_counters[method_idx], ammount);
        }
      }
    }
  }
  ...
  if (StoresAggregationCounters()) {
    uint16_t ammount = other.StoresAggregationCounters() ? other.aggregation_count_ : 1;
    aggregation_count_ = IncrementAggregationCounter(aggregation_count_, ammount);
  }
