...
std::unique_ptr<ProfileCompilationInfo::OfflineProfileMethodInfo> ProfileCompilationInfo::GetHotMethodInfo(
    const MethodReference& method_ref) const {
...
bool ProfileCompilationInfo::OfflineProfileMethodInfo::operator==(
  ...
  for (const auto& inline_cache_it : *inline_caches) {
    ...
    if (dex_pc_data.is_missing_types != runtime_cache->is_missing_types) {
      return false;
    }
    if (dex_pc_data.is_megamorphic !=
          (runtime_cache->classes.size() >= ProfileCompilationInfo::kIndividualInlineCacheSize)) {
      return false;
    }
