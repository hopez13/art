...
int32_t ProfileCompilationInfo::DexFileData::GetMethodAggregationCounter(uint16_t method_idx) const {
...
int32_t ProfileCompilationInfo::GetMethodAggregationCounter(
  ...
  const DexFileData* dex_data = FindDexData(method_ref.dex_file);
  return dex_data == nullptr ? 0 : dex_data->GetMethodAggregationCounter(method_ref.index);
}
...
int32_t ProfileCompilationInfo::GetClassAggregationCounter(const TypeReference& type_ref) const {
  ...
  return dex_data == nullptr ? 0 : dex_data->GetMethodAggregationCounter(type_ref.index);
}

int32_t ProfileCompilationInfo::GetAggregationCounter() const {
  CHECK(StoresAggregationCounters()) << "Profile not prepared for aggregation counters";
