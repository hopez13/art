...
ProfileCompilationInfo::ProfileSampleAnnotation::kNone = ProfileCompilationInfo::ProfileSampleAnnotation("");
...
std::string ProfileCompilationInfo::ReatachAnnotationInfo(
...
const ProfileCompilationInfo::DexFileData* ProfileCompilationInfo::FindDexDataUsingAnnotations(
  ...
  if (annotation == ProfileSampleAnnotation::kNone) {
    ...
    return FindDexData(profile_key, dex_file->GetLocationChecksum(), /* verify_checksum */ false);
  }
...
std::string ProfileCompilationInfo::DumpInfo(const std::vector<const DexFile*>& dex_files,
  ...
  for (const DexFileData* dex_data : info_) {
    ...
    if (print_full_dex_location) {
      ...
      std::string multidex_suffix = DexFileLoader::GetMultiDexSuffix(GetBaseKeyFromAugmentedKey(dex_data->profile_key));
    ...
    for (DexFileData* dex_data : info_) {
      if (dex_data->checksum == dex_file->GetLocationChecksum()
        ...
        if (dex_data_base_key != new_profile_key) {
          ...
          // form the old key.
          profile_key_map_.Put(ReatachAnnotationInfo(new_profile_key, dex_data->profile_key),
...
bool ProfileCompilationInfo::ProfileSampleAnnotation::operator==(
  ...
  return origin_package_name == other.origin_package_name;
}
}  // namespace art
