...
enum class ProfileCompilationInfo::ProfileLoadStatus : uint32_t {
  ...
  kVersionMismatch,
  kBadData,
  kMergeError,  // Too many extra descriptors or classes without TypeId in the dex file.
};
...
enum class ProfileCompilationInfo::FileSectionType : uint32_t {
  ...
  // Extra descriptors for referencing classes (and possibly method names) from
...
ProfileCompilationInfo::ProfileCompilationInfo(ArenaPool* custom_arena_pool, bool for_boot_image)
    : default_arena_pool_(),
      ...
      extra_descriptors_(),
      extra_descriptors_indexes_(ExtraDescriptorIndexHash(&extra_descriptors_),
                                 ExtraDescriptorIndexEquals(&extra_descriptors_)),
 ...
 *   Section:ExtraDescriptors - optional
...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  if (!extra_descriptors_.empty()) {
    ...
    for (const std::string& str : extra_descriptors_) {
      extra_descriptors_section_size += str.size() + 1u;  // Null-terminated string.
    }
  }
...
dex::TypeIndex ProfileCompilationInfo::FindOrCreateTypeIndex(const DexFile& referrer_dex_file,
  ...
  DCHECK_NE(new_extra_descriptor_index, kMaxExtraDescriptors);
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::ReadExtraDescriptorsSection(
  ...
  for (uint16_t i = 0; i != num_extra_descriptors; ++i) {
    ...
    size_t extra_descriptor_size =
        reinterpret_cast<const uint8_t*>(null_char) - buffer.GetCurrentPtr();
    const char* descriptor = reinterpret_cast<const char*>(buffer.GetCurrentPtr());
    if (!IsValidDescriptor(descriptor)) {
      *error += "Invalid extra descriptor.";
      return ProfileLoadStatus::kBadData;
    }
    std::string_view extra_descriptor(descriptor, extra_descriptor_size);
    buffer.Advance(extra_descriptor_size + 1u);
...
ProfileCompilationInfo::ProfileLoadStatus ProfileCompilationInfo::LoadInternal(
  ...
  dchecked_vector<uint16_t> extra_descriptors_remap;
  ...
  for (const std::unique_ptr<DexFileData>& other_dex_data : other.info_) {
    ...
    if (merge_classes) {
      ...
      for (auto end = other_dex_data->class_set.end(); it != end; ++it) {
        uint32_t new_extra_descriptor_index = extra_descriptors_remap[it->index_ - num_type_ids];
        ...
        if (other_ic_it.second.is_missing_types) {
          ...
          for (dex::TypeIndex type_index : other_class_set) {
            if (type_index.index_ >= num_type_ids) {
              uint32_t new_extra_descriptor_index =
    ...
    for (dex::TypeIndex type_index : dex_data->class_set) {
      if (dex_file != nullptr) {
        if (type_index.index_ < dex_data->num_type_ids) {
          ...
          os << "\n\t\t" << extra_descriptors_[type_index.index_ - dex_data->num_type_ids];
        }
      } else {
