...
TEST_F(ProfileCompilationInfoTest, AddAnnotationsToMethods) {
  ...
  auto run_test = [&dex1 = dex1, &psa1 = psa1, &psa2 = psa2](const ProfileCompilationInfo& info) {
    ...
    EXPECT_FALSE(info.GetMethodHotness(MethodReference(dex1, 0), ProfileSampleAnnotation("new_test")).IsInProfile());
    EXPECT_FALSE(info.GetHotMethodInfo(MethodReference(dex1, 0), ProfileSampleAnnotation("new_test")) != nullptr);
  };
...
TEST_F(ProfileCompilationInfoTest, ExtractInfoWithAnnations) {
  ...
  EXPECT_TRUE(info.GetClassesAndMethods(*dex1, &classes, &hot_methods, &startup_methods, &post_startup_methods, psa1));
  ...
  EXPECT_FALSE(info.GetClassesAndMethods(
      *dex1, &classes, &hot_methods, &startup_methods, &post_startup_methods, ProfileSampleAnnotation("new_test")));
}
