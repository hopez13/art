...
TEST_F(ProfileCompilationInfoTest, MergeWithAggregationCounters) {
  ...
  ASSERT_EQ(-1, info1.GetClassAggregationCounter(TypeReference(dex.get(), dex::TypeIndex(4))));
}
...
TEST_F(ProfileCompilationInfoTest, MergeTwoWithAggregationCounters) {
  ...
  AddClass(location, checksum, dex::TypeIndex(1), &info1);
  AddClass(location, checksum, dex::TypeIndex(1), &info2);
  ...
  ASSERT_EQ(1, info1.GetMethodAggregationCounter(MethodReference(dex.get(), 1)));
  ASSERT_EQ(2, info1.GetMethodAggregationCounter(MethodReference(dex.get(), 2)));

  ASSERT_EQ(2, info1.GetClassAggregationCounter(TypeReference(dex.get(), dex::TypeIndex(1))));
  ASSERT_EQ(1, info1.GetClassAggregationCounter(TypeReference(dex.get(), dex::TypeIndex(2))));
}
