...
class Dex2Oat final {
 public:
  ...
  dex2oat::ReturnCode Setup() {
    ...
    if (PaletteGetHooks(&hooks) == PaletteStatus::kOkay) {
      ...
      // We dup the image file descriptor, as the image writer will close it in ImageWriter::Write.
      int image_fd = IsAppImage() ? app_image_fd_ : image_fd_;
      if (image_fd != -1) {
 ...
 private:
  ...
  int image_dup_fd_;  // A dup of the image fd in case we report it to Palette.
