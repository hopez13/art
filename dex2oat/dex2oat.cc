...
class Dex2Oat final {
 ...
 private:
  ...
  friend class ScopedDex2oatReporting;
  ...
  explicit ScopedDex2oatReporting(const Dex2Oat& dex2oat) {
    ...
    if (PaletteGetHooks(&hooks) == PALETTE_STATUS_OK) {
      if (dex2oat.zip_fd_ != -1) {
        zip_dup_fd_ = DupCloexec(dex2oat.zip_fd_);
        if (zip_dup_fd_ < 0) {
      ...
      if (image_fd != -1) {
        image_dup_fd_ = DupCloexec(image_fd);
        if (image_dup_fd_ < 0) {
          LOG(ERROR) << "Error dup'ing a file descriptor " << strerror(errno);
          error_reporting_ = true;
          return;
      ...
      oat_dup_fd_ = DupCloexec(dex2oat.oat_fd_);
      if (oat_dup_fd_ < 0) {
        LOG(ERROR) << "Error dup'ing a file descriptor " << strerror(errno);
        error_reporting_ = true;
        return;
      }
      vdex_dup_fd_ = DupCloexec(dex2oat.output_vdex_fd_);
      if (vdex_dup_fd_ < 0) {
        LOG(ERROR) << "Error dup'ing a file descriptor " << strerror(errno);
        error_reporting_ = true;
        return;
...
static dex2oat::ReturnCode Dex2oat(int argc, char** argv) {
  ...
  ScopedDex2oatReporting sdr(*dex2oat.get());
