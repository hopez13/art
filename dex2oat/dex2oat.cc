...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("      runtime loading context for the compiled dex files. It describes how the");
  UsageError("      class loader chain should be build in order to ensure classes are resolved");
  UsageError("      during dex2aot as they would be resolved at runtime. This spec will be");
  UsageError("      encoded in the oat file. If at runtime the dex file will be loaded in a");
  UsageError("      different context, the oat file will be rejected.");
 ...
 public:
  ...
  void Compile() {
    ...
    if (!no_inline_filters.empty()) {
      ...
      if (class_loader_context_ != nullptr) {
         CHECK(class_loader_context_->FlattenOpenedDexFiles(&class_path_files));
      }
