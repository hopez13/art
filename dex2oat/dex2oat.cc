...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("");
  UsageError("  --dirty-image-classes=<directory-path>: list of image classes profiled to have");
  UsageError("      dirty static fields. The image writer will group them together.");
  UsageError("");
...
class Dex2Oat FINAL {
 public:
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
      ...
      } else if (option.starts_with("--dirty-image-classes=")) {
        dirty_image_classes_filename_ = option.substr(strlen("--dirty-image-classes=")).data();
 ...
 private:
  ...
  bool PrepareDirtyClasses() {
    if (dirty_image_classes_filename_ != nullptr) {
      dirty_image_classes_.reset(ReadCommentedInputFromFile<std::unordered_set<std::string>>(
          dirty_image_classes_filename_,
          nullptr));
      if (dirty_image_classes_ == nullptr) {
        LOG(ERROR) << "Failed to create list of compiled methods from '"
            << dirty_image_classes_filename_ << "'";
        return false;
      }
    } else {
      dirty_image_classes_.reset(nullptr);
    }
    return true;
  }
  ...
  const char* dirty_image_classes_filename_;
  ...
  std::unique_ptr<std::unordered_set<std::string>> dirty_image_classes_;
