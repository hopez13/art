...
class Dex2Oat FINAL {
 public:
  ...
  void ProcessOptions(ParserOptions* parser_options) {
    ...
    if (!vdex_filenames_.empty() && !image_filenames_.empty()) {
  ...
  bool OpenFile() {
    ...
    bool create_vdex_file = true;
    if (create_vdex_file) {
      for (const char* vdex_filename : vdex_filenames_) {
        if (OS::FileExists(vdex_filename)) {
          std::unique_ptr<File> vdex_file(OS::OpenFileForReading(vdex_filename));
          if (vdex_file.get() == nullptr) {
            PLOG(ERROR) << "Failed to open vdex file for reading: " << vdex_filename;
            return false;
          }
          DCHECK(vdex_file->ReadOnlyMode());
          vdex_files_.push_back(std::move(vdex_file));
        } else {
          std::unique_ptr<File> vdex_file(OS::CreateEmptyFile(vdex_filename));
          if (vdex_file.get() == nullptr) {
  ...
  bool Setup() {
    ...
    if (vdex_files_.size() == 1u) {
      ...
      CHECK(vdex_file != nullptr);

      for (size_t i = 0; i < dex_files_.size(); ++i) {
        const DexFile* dex_file = dex_files_[i];
        CHECK(dex_file != nullptr);

        verifier::VerifierMetadata* metadata = nullptr;
        if (vdex_file->ReadOnlyMode()) {
          bool success = false;
          metadata = new verifier::VerifierMetadata(*dex_file, vdex_file, &success);
          if (!success) {
            metadata = new verifier::VerifierMetadata(*dex_file);
          }
        } else {
          metadata = new verifier::VerifierMetadata(*dex_file);
        }
        vdex_metadata_.push_back(std::unique_ptr<verifier::VerifierMetadata>(metadata));
      }
    }
  ...
  void Compile() {
    ...
    std::vector<verifier::VerifierMetadata*> non_owning_vdex_metadata =
        MakeNonOwningPointerVector(vdex_metadata_);
    ...
    driver_->CompileAll(class_loader_, dex_files_, &non_owning_vdex_metadata, timings_);
  }

  bool WriteVdexFiles() {
    TimingLogger::ScopedTiming t("dex2oat Vdex", timings_);

    if (vdex_files_.empty()) {
      return true;
    }

    if (vdex_files_.size() == 1u) {
      File* vdex = vdex_files_.front().get();
      if (!vdex->ReadOnlyMode()) {
        for (auto&& entry : vdex_metadata_) {
          if (!entry->WriteToFile(vdex)) {
            return false;
          }
        }
      }
