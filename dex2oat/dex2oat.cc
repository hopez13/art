...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("");
  UsageError("  --class-loader-spec=<class-loader-spec>: a string specifying the intended");
  UsageError("      runtime loading context for the compiled dex files. ");
  UsageError("      ");
  UsageError("      It describes how the class loader chain should be build in order to ensure");
  UsageError("      classes are resolved during dex2aot as they would be resolved at runtime.");
  UsageError("      This spec will be encoded in the oat file. If at runtime the dex file will");
  UsageError("      be loaded in a different context, the oat file will be rejected.");
  UsageError("      ");
  UsageError("      The chain is interpreted in the natural 'parent order', meaning that class");
  UsageError("      loader 'i+1' will be the parend of class loader 'i'.");
  ...
  UsageError("      removed from the any class loader's classpath possibly resulting in empty");
    ...
    if (!IsBootImage()) {
      ...
      if (class_loader_context_ == nullptr) {
        ...
        if (runtime_->GetClassPathString().empty()) {
          class_loader_context_ = std::unique_ptr<ClassLoaderCompilationContext>(
              new ClassLoaderCompilationContext());
        } else {
          ...
          class_loader_context_ = ClassLoaderCompilationContext::Create(spec);
        }
      }
      CHECK(class_loader_context_ != nullptr);
      DCHECK_EQ(oat_writers_.size(), 1u);
      // Note: this is currently needed for our non-prebuild test and benchmark which expects on
      // the fly compilation.
      ...
      if (!class_loader_context_->OpenDexFiles(runtime_->GetInstructionSet(), classpath_dir_)) {
        LOG(ERROR) << "Failed to open classpath dex files";
        return dex2oat::ReturnCode::kOther;
      }
