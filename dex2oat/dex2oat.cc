...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("");
  UsageError("  --base-dir=<directory-path>: directory path used to resolve relative class paths.");
  UsageError("");
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
      ...
      if (option.starts_with("--dex-file=")) {
        ...
        force_determinism_ = true;
      } else if (option.starts_with("--base-dir=")) {
        base_dir_ = option.substr(strlen("--base-dir=")).data();
  ...
  dex2oat::ReturnCode Setup() {
    ...
    if (!IsBootImage()) {
      ...
      if (class_path_locations.size() == 1 &&
        ...
        encoded_class_path = OatFile::EncodeDexFileDependencies(class_path_files, base_dir_);
      }
