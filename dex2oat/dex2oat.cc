 ...
 public:
  ...
  bool OpenFile() {
    ...
    if (dm_file_ != nullptr) {
      ...
      static const char* kVdexName = "primary.vdex";
      static const char* kDexMetadata = "DexMetadata";
      std::unique_ptr<ZipEntry> zip_entry(dm_file_->Find(kVdexName, &error_msg));
      if (zip_entry == nullptr) {
        LOG(INFO) << "No " << kVdexName << " file in DexMetadata archive. "
                  << "Not doing fast verification.";
      } else {
        std::unique_ptr<MemMap> input_file;
        if (zip_entry->IsUncompressed()) {
          input_file.reset(zip_entry->MapDirectlyFromFile(kVdexName, &error_msg));
        } else {
          input_file.reset(zip_entry->ExtractToMemMap(kDexMetadata, kVdexName, &error_msg));
        }
