...
class Dex2Oat FINAL {
 public:
  ...
  dex2oat::ReturnCode Setup() {
    ...
    is_very_large_mode_ = IsVeryLarge(dex_files_);
    if (!IsBootImage() && is_very_large_mode_) {
  ...
  bool ShouldCompileDexFilesIndividually() const {
    // Compile individually if we are not building an image, using AOT compilation but are doing
    // multidex.
  ...
  jobject Compile() {
    ...
    if (ShouldCompileDexFilesIndividually()) {
      ...
      return nullptr;  // Null classloader since we already freed it.
    }
  ...
  bool IsVeryLargeMode() const {
    return is_very_large_mode_;
  }
 ...
 private:
  ...
  // Whether or not we are running in "very large" mode.
  bool is_very_large_mode_ = false;
