...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("      the default behavior). This option is only meaninfgul when used with");
  UsageError("      --dump-cfg.");
  UsageError("");
  UsageError("  --honor-noinline: do not try to inline methods whose name contain the substring");
  UsageError("      \"$noinline$\".");
  UsageError("");
