...
/* TODO: OTH
    if (image_fds_.size() != 0u && multi_image_) {
...
*/
  ...
  bool WriteOutputFiles(jobject class_loader) {
    ...
    if (IsImage()) {
      ...
      LOG(ERROR) << "OTH: oat_filenames_.size() = " << oat_filenames_.size()
                 << " oat_locations_.size() = " << oat_locations_.size();

      // TODO(oth): HACK HACK
      if (oat_locations_.size() > oat_filenames_.size()) {
        oat_filenames_ = oat_locations_;
      }
