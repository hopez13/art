 ...
 public:
  ...
  void ExpandOatAndImageFilenames() {
    ...
    if (!oat_unstripped_.empty()) {
      ...
      oat_unstripped_ = ImageSpace::ExpandMultiImageLocations(
           ArrayRef<const std::string>(dex_locations_), oat_unstripped_[0]);
    }
  }
  ...
  dex2oat::ReturnCode Setup() {
    ...
    callbacks_.reset(new QuickCompilerCallbacks(
        // For class verification puproses, boot image extension is the same as boot image.
    ...
    if (IsBootImage() || IsBootImageExtension()) {
      ...
      auto has_image = [](const DexFile* df) {
      ...
      if (!std::all_of(bcp_dex_files.begin(), bcp_dex_files.end() - dex_files.size(), has_image)) {
