...
#define CHECK_WATCH_DOG_PTHREAD_CALL(call, args, what) \
 ...
 public:
  explicit WatchDog(int64_t timeout_in_seconds)
 ...
 private:
  ...
  void Wait() {
    timespec timeout_ts;
    InitTimeSpec(true, CLOCK_REALTIME, timeout_in_seconds_ * 1000, 0, &timeout_ts);
  ...
  const int64_t timeout_in_seconds_;
  bool shutting_down_;
...
class Dex2Oat FINAL {
 public:
  ...
  struct ParserOptions {
    ...
    int64_t watch_dog_timeout_in_s = -1;
  ...
  void ProcessOptions(ParserOptions* parser_options) {
    ...
    if (parser_options->watch_dog_enabled) {
      ...
      //       large.
      constexpr int64_t kWatchdogVerifyMultiplier =
      ...
      constexpr int64_t kWatchdogSlowdownFactor = kIsDebugBuild ? 5U : 1U;
      ...
      constexpr int64_t kWatchDogTimeoutSeconds = kWatchdogSlowdownFactor * (9 * 60 + 30);

      constexpr int64_t kDefaultWatchdogTimeout =
          kWatchdogVerifyMultiplier * kWatchDogTimeoutSeconds;

      int64_t timeout = parser_options->watch_dog_timeout_in_s > 0
                            ? parser_options->watch_dog_timeout_in_s
