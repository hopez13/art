...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("      file. If at runtime the dex file will be loaded in a different context, the oat");
  UsageError("      file will be rejected.");
  UsageError("");
 ...
 public:
  ...
  dex2oat::ReturnCode Setup() {
    ...
    if (!IsBootImage()) {
      ...
      if (!class_loader_context_->ValidateUniquenessOfElements(oat_writers_[0]->GetSourceLocations())) {
