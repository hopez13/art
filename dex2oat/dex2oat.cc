...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("");
  UsageError("  --avoid-copying-dex: Avoid copying or extracing the dex file into the output");
  UsageError("      vdex.");
  UsageError("");
...
class Dex2Oat FINAL {
 public:
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    AssignTrueIfExists(args, M::AvoidCopyingDex, &avoid_copying_dex_);
    ...
    // Now that we have finalized key_value_store_, start writing the oat file.
    {
      ...
      for (size_t i = 0, size = oat_writers_.size(); i != size; ++i) {
        ...
        if (!oat_writers_[i]->WriteAndOpenDexFiles(
            ...
            verify,
            update_input_vdex_,
            /*copy_dex_files*/ !avoid_copying_dex_,
