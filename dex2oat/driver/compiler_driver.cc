...
CompilerDriver::~CompilerDriver() {
  compiled_methods_.Visit(
      [this](const DexFileReference& ref [[maybe_unused]], CompiledMethod* method) {
        if (method != nullptr) {
...
static void CompileMethodQuick(
    ...
    Handle<mirror::DexCache> dex_cache,
    ProfileCompilationInfo::ProfileIndexType profile_index) {
  auto quick_fn = [profile_index](Thread* self [[maybe_unused]],
                                  CompilerDriver* driver,
                                  const dex::CodeItem* code_item,
                                  uint32_t access_flags,
                                  InvokeType invoke_type,
...
void CompilerDriver::PrepareDexFilesForOatFile(TimingLogger* timings [[maybe_unused]]) {
