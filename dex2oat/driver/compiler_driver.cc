  ...
  for (const DexFile* dex_file : dex_files) {
    for (ClassAccessor accessor : dex_file->GetClasses()) {
      ...
      if (cls == nullptr) {
        ...
        DCHECK(cls->IsErroneous() || cls->IsVerified() || cls->ShouldVerifyAtRuntime() || cls->GetStatus() == ClassStatus::kVerifiedWithAccessChecks)
            << cls->PrettyClass()
            << " " << cls->GetStatus();
      }
    }
  ...
  void Visit(size_t class_def_index) REQUIRES(!Locks::mutator_lock_) override {
    ...
    if (klass == nullptr) {
      ...
      if (kIsDebugBuild) {
        ...
        } else if (klass->GetStatus() == ClassStatus::kVerifiedWithAccessChecks) {
...
void CompilerDriver::RecordClassStatus(const ClassReference& ref, ClassStatus status) {
  switch (status) {
    ...
    case ClassStatus::kVerifiedWithAccessChecks:
