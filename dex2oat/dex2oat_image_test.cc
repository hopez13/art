...
TEST_F(Dex2oatImageTest, TestExtension) {
  ...
  // with BCP in the scratch_dir so that the images can be found based BCP paths.
  ...
  // Load primary image with just the name now succeeds.
  load_ok = load(base_name);
  ASSERT_TRUE(load_ok) << error_msg;

  // Loading the load primary image with a search path still fails.
  load_ok = load("*");
  ASSERT_FALSE(load_ok);
  load_ok = load(scratch_dir + "*");
  ASSERT_FALSE(load_ok);
  ...
  load_ok = load(base_location + ':' + mid_name);
  ASSERT_TRUE(load_ok) << error_msg;  // Primary image loaded successfully.
  ASSERT_EQ(mid_bcp.size(), boot_image_spaces.size());  // But only the primary image.
  ...
  load_ok = load(base_location + ':' + mid_name + ':' + tail_location);
  ASSERT_TRUE(load_ok) << error_msg;  // Primary image loaded successfully.
  ASSERT_EQ(full_bcp.size(), boot_image_spaces.size());  // But only the primary image.
