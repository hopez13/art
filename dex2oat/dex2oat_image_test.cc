...
TEST_F(Dex2oatImageTest, TestExtension) {
  ...
  GenerateProfile(head_dex_files,
                  ...
                  /*method_frequency=*/ 1u,
                  /*type_frequency=*/ 1u);
  std::string head_profile_filename = head_profile_file.GetFilename();
  ScratchFile mid_profile_file;
  GenerateProfile(mid_dex_files,
                  mid_profile_file.GetFile(),
                  /*method_frequency=*/ 5u,
                  /*type_frequency=*/ 4u);
  std::string mid_profile_filename = mid_profile_file.GetFilename();
  ScratchFile tail_profile_file;
  GenerateProfile(tail_dex_files,
                  tail_profile_file.GetFile(),
                  /*method_frequency=*/ 5u,
                  /*type_frequency=*/ 4u);
  std::string tail_profile_filename = tail_profile_file.GetFilename();
  ...
  for (bool r : { false, true }) {
    ...
    // Load primary boot image and the single extension, specifying invalid extension component name
    // but a valid profile file. (Compile extension by running dex2oat.)
    load_ok = load(base_location + ":/non-existent/" + single_name + "!" + single_profile_filename);
    ASSERT_TRUE(load_ok) << error_msg;
    ASSERT_EQ(head_dex_files.size() + 1u, boot_image_spaces.size());
    ASSERT_EQ(single_dex_files.size(),
    ...
    // Load primary boot image and two extensions, specifying invalid extension component names
    // but valid profile files. (Compile extensions by running dex2oat.)
    ...
    // Load primary boot image and fail to load extensions, specifying invalid component names
    // but valid profile file only for the second one. As we fail to load the first extension,
    // the second extension has a missing dependency and cannot be compiled.
