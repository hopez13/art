...
TEST_F(Dex2oatImageTest, TestExtension) {
  ...
  // Recompile the single-image using file descriptors and compare contents.
  std::vector<std::string> expanded_single_ext_filename =
      gc::space::ImageSpace::ExpandMultiImageLocations(
          single_dex_files.SubArray(/*pos=*/ 0u, /*length=*/ 1u),
          single_filename_prefix,
          /*boot_image_extension=*/ true);
  CHECK_EQ(1u, expanded_single_ext_filename.size());
  std::string extension_filename_prefix = expanded_single_ext_filename[0];
  std::string single_filename_prefix2 = extension_filename_prefix + "2";
  error_msg.clear();
  single_ok = CompileBootImage(extra_args,
                               single_filename_prefix,
                               single_dex_files,
                               &error_msg,
                               /*use_fd_prefix=*/ single_filename_prefix2);
  ASSERT_TRUE(single_ok) << error_msg;
  EXPECT_TRUE(CompareFiles(extension_filename_prefix + ".art", single_filename_prefix2 + ".art"));
  EXPECT_TRUE(CompareFiles(extension_filename_prefix + ".vdex", single_filename_prefix2 + ".vdex"));
  EXPECT_TRUE(CompareFiles(extension_filename_prefix + ".oat", single_filename_prefix2 + ".oat"));
