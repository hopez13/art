...
TEST_F(Dex2oatVdexTest, VerifyCorruptVdexFile) {
  ...
  // Check that we ignored the file.
  // We already know this because otherwise dex2oat would have crashed, but having an explit check
  // here makes everything clearer.
  ASSERT_TRUE(output_.find("The dex metadata .vdex is not valid. Ignoring it.")
      != std::string::npos) << output_;
}
