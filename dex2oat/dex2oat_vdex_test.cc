...
class Dex2oatVdexTest : public Dex2oatEnvironmentTest {
 ...
 protected:
  bool RunDex2oat(const std::string& dex_location,
                  ...
                  const std::string* public_api_classpath,
                  const std::vector<std::string>& extra_args = {}) {
    std::vector<std::string> args;
    args.push_back("--dex-file=" + dex_location);
    args.push_back("--oat-file=" + odex_location);
    if (public_api_classpath != nullptr) {
      args.push_back("--public-api-classpath=" + *public_api_classpath);
    }
    args.push_back("--compiler-filter=" + CompilerFilter::NameOfFilter(CompilerFilter::Filter::kVerify));
...
// - create a vdex file contraints by a predefine list of public API (passed as separate dex)
// - compile with the above vdex file as input to validate the compilation flow
TEST_F(Dex2oatVdexTest, VerifyPublicApiStubs) {
  ...
  // Dex2oatVdexPublicApiDex serves as the public API-stubs, restricting what can be verified.
  const std::string api_dex_location = GetTestDexFileName("Dex2oatVdexPublicApiDex");
  ...
  // TOOD(calin):
  // Accessing unresolved static fields do not lead to class verification failures.
  // Is this expected?
  //
  // api_checker.cc:149] Deny for java.lang.Class java.lang.Integer.TYPE
  // method_verifier.cc:4660] Unable to resolve static field 0 (TYPE) in Ljava/lang/Integer;
  // class_verifier.cc:308] VerifyClass took 104us, class: AccessNonPublicStaticField
  ...
  // TODO(calin): see comment above
  ASSERT_TRUE(HasVerifiedClass(deps2, "LAccessNonPublicStaticField;", *dex_file));
}

// Verify that if there's a mismatch in assumptions between the vdex from the .dm file
// and the current boot class path the file is re-verified.
TEST_F(Dex2oatVdexTest, VerifyPublicApiChange) {
  // TODO(CALIN): change the type assignability in vdex to simulate this.
}
}  // namespace art
