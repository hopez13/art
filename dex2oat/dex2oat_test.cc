...
class Dex2oatLayoutTest : public Dex2oatTest {
 protected:
  ...
  void CompileProfileOdex(const std::string& dex_location,
                          ...
                          bool expect_success = true) {
    std::vector<std::string> copy(extra_args);
    std::transform(profile_locations.begin(),
                   profile_locations.end(),
                   std::back_inserter(copy),
                   [](const std::string& location) { return "--profile-file=" + location; });
...
TEST_F(Dex2oatLayoutTest, TestLayoutMultipleProfiles) {
  ...
  uint64_t image_file_size_mutiple_profile = GetImageObjectSectionSize(app_image_file);

  CheckCompilerFilter(dex_location, odex_location, CompilerFilter::Filter::kSpeedProfile);

  // The image file generated with multiple profiles should be larger than any image file generated
  // with each profile.
  ASSERT_GT(image_file_size_mutiple_profile, image_file_size_profile1);
  ASSERT_GT(image_file_size_mutiple_profile, image_file_size_profile2);
}
