...
TEST_F(Dex2oatDeterminism, UnloadCompile) {
  ...
  const std::string nounload_oat_name = out_dir + "/nounload.oat";
  const std::string nounload_vdex_name = out_dir + "/nounload.vdex";
  ...
  const int res2 = GenerateOdexForTestWithStatus(
      ...
      CompilerFilter::Filter::kQuicken,
      &error_msg,
      {"--avoid-storing-invocation", "--app-image-file=" + app_image_name});
  EXPECT_EQ(res2, 0);
  Copy(base_oat_name, nounload_oat_name);
  Copy(base_vdex_name, nounload_vdex_name);
  std::unique_ptr<File> nounload_oat(OS::OpenFileForReading(nounload_oat_name.c_str()));
  std::unique_ptr<File> nounload_vdex(OS::OpenFileForReading(nounload_vdex_name.c_str()));
  ASSERT_TRUE(nounload_oat != nullptr);
  ASSERT_TRUE(nounload_vdex != nullptr);
  EXPECT_GT(nounload_oat->GetLength(), 0u);
  EXPECT_GT(nounload_vdex->GetLength(), 0u);
  // Verify that both of the files are the same (odex and vdex).
  EXPECT_EQ(unload_oat->GetLength(), nounload_oat->GetLength());
  EXPECT_EQ(unload_vdex->GetLength(), nounload_vdex->GetLength());
  EXPECT_EQ(unload_oat->Compare(nounload_oat.get()), 0)
      << unload_oat_name << " " << nounload_oat_name;
  EXPECT_EQ(unload_vdex->Compare(nounload_vdex.get()), 0)
      << unload_vdex_name << " " << nounload_vdex_name;
  // App image file.
