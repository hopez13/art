...
static void AddGeneratedArtifactMappings(Builder& builder) {
  builder.
      ...
      .Define("--oat-location=_")
          .WithType<std::string>()
          .WithHelp("specifies a symbolic name for the file corresponding"
...
static void AddImageMappings(Builder& builder) {
  builder.
      ...
      .Define("--image-fd=_")
          .WithType<int>()
          .WithHelp("specifies an output image file descriptor. Cannot be used with --image. Eg:"
                    " --image-fd=7")
          .IntoKey(M::ImageFd)
      .Define("--base=_")
          .WithType<std::string>()
...
Parser CreateDex2oatArgumentParser() {
  ...
  parser_builder->
      ...
      .Define("-j_")
          .WithType<unsigned int>()
          .WithHelp("specifies the number of threads used for compilation. Default is the number of"
                    " detected hardware threads available on the host system.")
          .IntoKey(M::Threads)
      .Define("--cpu-set=_")
          .WithType<std::vector<int32_t>>()
          .WithHelp("sets the cpu affinitiy to the given <set>. The <set> is a comma separated list"
                    " of cpus. Eg: --cpu-set=0,1,2,3")
          .WithMetavar("<set>")
          .IntoKey(M::CpuSet)
