...
class ImageWriter final {
 ...
 private:
  ...
  // Parse dirty-image-objects into (offset->entry) map. Returns empty map on parse error.
  static HashMap<uint32_t, DirtyEntry> ParseDirtyObjectOffsets(
      const HashSet<std::string>& dirty_image_objects) REQUIRES_SHARED(Locks::mutator_lock_);
  // Get all objects that match dirty_entries by offset. Returns empty set if there is a mismatch.
  HashSet<mirror::Object*> MatchDirtyObjectOffsets(
