...
class ImageWriter final {
 ...
 private:
  ...
  struct ImageInfo {
     ...
     *
     * TODO (chriswailes): Update the above list when the relocations section
     * gets removed.
     *
   ...
   * pointer has been tagged (testable with HasDexCacheTag) it indicates that
   * the referenced object is a DexCache object that requires special handling
   * during loading and the second element has no meaningful value.  If the
   * pointer isn't tagged then the second element is an object-relative offset
   * to a field containing a string reference.
   *
   * Not that it is possible for an untagged DexCache pointer to occur in the
