...
void OatWriter::WriteQuickeningInfo(/*out*/std::vector<uint8_t>* buffer) {
  ...
  uint32_t padding_offset = start_offset + quicken_info_offset;
  size_t current_offset = RoundUp(padding_offset, CompactOffsetTable::kAlignment);
  const size_t extra_bytes = current_offset - padding_offset;
...
bool OatWriter::WriteDexFiles(File* file,
  ...
  if (extract_dex_files_into_vdex_) {
    ...
    if (!dex_files_map.Sync()) {
      LOG(ERROR) << "Failed to sync dex file data to oat file. File: " << file->GetPath();
      return false;
    }
...
bool OatWriter::OpenDexFiles(
  ...
  uint8_t* dex_files_map_begin = opened_dex_files_map->front().Begin();
  const ArtDexFileLoader dex_file_loader;
  std::vector<std::unique_ptr<const DexFile>> dex_files;
  for (OatDexFile& oat_dex_file : oat_dex_files_) {
    const uint8_t* raw_dex_file = dex_files_map_begin + oat_dex_file.dex_file_offset_;
