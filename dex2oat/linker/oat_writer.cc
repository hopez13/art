      ...
      for (size_t class_def_index = 0; class_def_index != class_def_count; ++class_def_index) {
        ...
        for (ClassDataItemIterator class_it(*dex_file, class_data);
          ...
          if (HasQuickeningInfo(compiled_method)) {
            ...
            uint32_t debug_info_off = dex_file->GetDebugInfoOffset(code_item);
            // Abort if the offset is > 2GB.
            CHECK_LT(quickening_info_bytes_ + written_bytes_,
                     DexFile::kMaxDebugInfoOffsetForQuickening);
            const_cast<DexFile::CodeItem*>(code_item)->debug_info_off_ =
                DexFile::kMaxDebugInfoOffsetForQuickening + quickening_info_bytes_ + written_bytes_;
            if (!out_->WriteFully(&debug_info_off, sizeof(debug_info_off)) ||
            ...
            written_bytes_ += sizeof(debug_info_off) + sizeof(offset);
          } else {
            const DexFile::CodeItem* code_item = class_it.GetMethodCodeItem();
            // Some applications incorrectly use 0xFFFFFFFF.
            if (code_item->debug_info_off_ != 0xFFFFFFFF) {
              // Abort if the offset is > 2GB.
              CHECK_LT(code_item->debug_info_off_, DexFile::kMaxDebugInfoOffsetForQuickening);
            }
          }
        }
      }
