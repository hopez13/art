...
using QuickenInfoOffsetIndices = std::vector<uint32_t>;
using QuickenInfoOffsetIndicesDexMap = SafeMap<const DexFile*, std::vector<uint32_t>>;
...
class OatWriter::WriteQuickeningInfoMethodVisitor {
 public:
  ...
  bool VisitDexMethods(const std::vector<const DexFile*>& dex_files, const CompilerDriver& driver) {
    ...
    for (const DexFile* dex_file : dex_files) {
      QuickenInfoOffsetIndices* const offsets =
          &quicken_info_offset_indices_.Put(dex_file, QuickenInfoOffsetIndices()).second;
      ...
      for (uint32_t method_idx = 0; method_idx < dex_file->NumMethodIds(); ++method_idx) {
        ...
        // Use the existing quicken info it it exists.
        ...
        // The current approach prevents deduplication since there is only one index per 16
        // dequickening maps. Dex indices are not commonly duplicated so this should be a net win.
  ...
  QuickenInfoOffsetIndicesDexMap& GetQuickenInfoOffsetIndicies() {
 ...
 private:
  ...
  QuickenInfoOffsetIndicesDexMap quicken_info_offset_indices_;
};
...
class OatWriter::WriteQuickeningInfoOffsetsMethodVisitor {
 public:
  WriteQuickeningInfoOffsetsMethodVisitor(
      ...
      const QuickenInfoOffsetIndicesDexMap* quicken_info_offset_indices,
  ...
  bool VisitDexMethods(const std::vector<const DexFile*>& dex_files, const CompilerDriver& driver) {
    for (const DexFile* dex_file : dex_files) {
      QuickenInfoOffsetIndices* const offsets =
          quicken_info_offset_indices_.Get(dex_file, QuickenInfoOffsetIndices());

      const uint32_t current_offset = start_offset_ + written_bytes_;
      // Write the table.
      CHECK_ALIGNED(current_offset, QuickenInfoOffsetTableAccessor::Alignment());
      std::vector<uint8_t> table_data;
      QuickenInfoOffsetTableAccessor::Builder builder(&table_data);
      builder.WriteIndices(&offsets[0], offsets->size());
 ...
 private:
  ...
  const QuickenInfoOffsetIndicesDexMap* const quicken_info_offset_indices_;
...
bool OatWriter::WriteQuickeningInfo(OutputStream* vdex_out) {
  ...
  if (compiler_driver_->GetCompilerOptions().IsQuickeningCompilationEnabled()) {
    ...
    if (!write_quicken_info_visitor.VisitDexMethods(*dex_files_, *compiler_driver_)) {
      PLOG(ERROR) << "Failed to write the vdex quickening info. File: " << vdex_out->GetLocation();
      return false;
    }
    const uint32_t quicken_info_offset = write_quicken_info_visitor.GetNumberOfWrittenBytes();
    start_offset += write_quicken_info_visitor.GetNumberOfWrittenBytes();
    ...
    WriteQuickeningInfoOffsetsMethodVisitor table_visitor(
        ...
        /*out*/ &table_offsets);
    if (!table_visitor.VisitDexMethods(*dex_files_, *compiler_driver_)) {
    ...
    // TODO: Store the offsets for each vdex's table for fast access later.
    ...
    size_quickening_info_ = write_quicken_info_visitor.GetNumberOfWrittenBytes() +
        table_visitor.GetNumberOfWrittenBytes();
  } else {
