    ...
    if (dex_container_ != nullptr) {
      ...
      if (update_input_vdex) {
        for (OatDexFile& oat_dex_file : oat_dex_files_) {
          ...
          if (seen_shared_data && header.data_size_ != shared_data_size) {
            PLOG(ERROR) << "Shared data section size " << header.data_size_
                        << " does not match expected value " << shared_data_size;
            return false;
          }
          shared_data_size = header.data_size_;
        }
      }
    }
