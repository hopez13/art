...
class OatWriter::WriteQuickeningInfoOffsetsMethodVisitor {
 public:
  WriteQuickeningInfoOffsetsMethodVisitor(
      ...
      QuickenInfoOffsetIndicesDexMap* quicken_info_offset_indices,
  ...
  bool VisitDexMethods(const std::vector<const DexFile*>& dex_files) {
    for (const DexFile* dex_file : dex_files) {
      QuickenInfoOffsetIndices* const offsets =
          &quicken_info_offset_indices_->FindOrAdd(dex_file)->second;
 ...
 private:
  ...
  QuickenInfoOffsetIndicesDexMap* const quicken_info_offset_indices_;
...
bool OatWriter::WriteQuickeningInfo(OutputStream* vdex_out) {
  ...
  if (compiler_driver_->GetCompilerOptions().IsQuickeningCompilationEnabled() && (false)) {
...
bool OatWriter::SeekToDexFile(OutputStream* out, File* file, OatDexFile* oat_dex_file) {
  ...
  // size_dex_file_alignment_ += sizeof(uint32_t);

  size_t file_offset = start_offset;
  LOG(ERROR) << "DEX START " << file_offset;
...
bool OatWriter::OpenDexFiles(
  ...
  for (OatDexFile& oat_dex_file : oat_dex_files_) {
    ...
    LOG(ERROR) << "OPENING DEX FILE " << oat_dex_file.GetLocation() << " " << oat_dex_file.dex_file_offset_;
    ...
    LOG(ERROR) << dex_files.back()->NumMethodIds();
