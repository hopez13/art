...
class OatWriter::WriteQuickeningInfoOffsetsMethodVisitor {
 public:
  ...
  bool VisitDexMethods(const std::vector<const DexFile*>& dex_files) {
    for (const DexFile* dex_file : dex_files) {
      auto it = quicken_info_offset_indices_->FindOrAdd(dex_file);
      CHECK(it != quicken_info_offset_indices_->end()) << "Failed to find dex file "
                                                       << dex_file->GetLocation();
      const QuickenInfoOffsetIndices* const offsets = &it->second;
...
bool OatWriter::WriteQuickeningInfo(OutputStream* vdex_out) {
  ...
  if (compiler_driver_->GetCompilerOptions().IsQuickeningCompilationEnabled()) {
    ...
    size_t index = 0;
    for (const OatDexFile& oat_dex_file : oat_dex_files_) {
      const off_t desired_offset = oat_dex_file.dex_file_offset_ - sizeof(uint32_t);
