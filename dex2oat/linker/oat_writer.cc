...
void OatWriter::WriteTypeLookupTables(/*out*/std::vector<uint8_t>* buffer) {
  for (size_t i = 0, size = type_lookup_table_oat_dex_files_.size(); i != size; ++i) {
    ...
    // Initialize type lookup table offset for the first iteration.
    if (vdex_lookup_tables_offset_ == 0u) {
      CHECK_EQ(i, 0u);
...
bool OatWriter::FinishVdexFile(File* vdex_file, verifier::VerifierDeps* verifier_deps) {
  ...
  if (extract_dex_files_into_vdex_) {
    ...
    if (vdex_size_ > mmapped_vdex_size) {
      size_t tail_size = vdex_size_ - mmapped_vdex_size;
      std::string error_msg;
    ...
    size_t bytes_available = mmapped_vdex_size - old_vdex_size;
    memcpy(vdex_begin + old_vdex_size, buffer.data(), std::min(buffer.size(), bytes_available));
    if (buffer.size() > bytes_available) {
      memcpy(extra_map.Begin(), buffer.data() + bytes_available, buffer.size() - bytes_available);
    }
  } else {
    DCHECK(vdex_begin == nullptr);
    std::string error_msg;
  ...
  new (ptr) VdexFile::VdexSectionHeader(VdexSection::kVerifierDepsSection,
                                        ...
                                        vdex_lookup_tables_offset_ - vdex_verifier_deps_offset_);
  ptr += sizeof(VdexFile::VdexSectionHeader);
