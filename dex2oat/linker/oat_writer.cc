...
const UnalignedDexFileHeader* GetDexFileHeader(File* file,
                                               uint8_t* raw_header,
                                               const char* location) {
...
bool OatWriter::AddDexFileSource(const char* filename,
  ...
  if (fd.Fd() == -1) {
    ...
    oat_dex_files_.emplace_back(/* OatDexFile */ location,
...
bool OatWriter::AddZippedDexFilesSource(File&& zip_fd,
  ...
  for (size_t i = 0; ; ++i) {
    ...
    oat_dex_files_.emplace_back(/* OatDexFile */ full_location,
