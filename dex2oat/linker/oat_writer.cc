    ...
    if (HasCompiledCode(compiled_method)) {
      for (const LinkerPatch& patch : compiled_method->GetPatches()) {
        if (patch.GetType() == LinkerPatch::Type::kDataBimgRelRo) {
          ...
          printf("adding string bss entry\n");
          ...
          printf("adding method type patch\n");
        ...
        for (const LinkerPatch& patch : compiled_method->GetPatches()) {
          ...
          switch (patch.GetType()) {
            ...
            case LinkerPatch::Type::kMethodTypeBssEntry: {
              ...
              uint32_t target_offset = writer_->bss_start_ + writer_->bss_method_type_entries_.Get(ref);
              writer_->relative_patcher_->PatchPcRelativeReference(&patched_code_, patch, offset_ + literal_offset, target_offset);
              break;
            }
