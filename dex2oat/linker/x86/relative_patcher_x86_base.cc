...
uint32_t X86BaseRelativePatcher::ReserveSpace(uint32_t offset,
                                              const CompiledMethod* compiled_method
                                              [[maybe_unused]],
                                              MethodReference method_ref [[maybe_unused]]) {
...
uint32_t X86BaseRelativePatcher::WriteThunks(OutputStream* out [[maybe_unused]], uint32_t offset) {
  return offset;  // No thunks added; no limit on relative call distance.
}

std::vector<debug::MethodDebugInfo> X86BaseRelativePatcher::GenerateThunkDebugInfo(
    uint32_t executable_offset [[maybe_unused]]) {
