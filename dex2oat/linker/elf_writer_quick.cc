...
class DebugInfoTask : public Task {
 public:
  DebugInfoTask(InstructionSet isa,
                ...
                uint64_t vdex_section_address,
                size_t vdex_section_size,
      ...
      : isa_(isa),
        ...
        vdex_section_address_(vdex_section_address),
        vdex_section_size_(vdex_section_size),
 ...
 private:
  ...
  uint64_t vdex_section_address_;
  size_t vdex_section_size_;
...
void ElfWriterQuick<ElfTypes>::PrepareDynamicSection(size_t rodata_size,
  ...
  bss_size_ = bss_size;
  DCHECK_EQ(vdex_size_, 0u);
  vdex_size_ = vdex_size;
  ...
  if (!debug_info.Empty() && compiler_options_->GetGenerateMiniDebugInfo()) {
    ...
    debug_info_task_ = std::unique_ptr<DebugInfoTask>(
        new DebugInfoTask(builder_->GetIsa(),
                          ...
                          text_size_,
                          builder_->GetVdex()->Exists() ? builder_->GetVdex()->GetAddress() : 0,
                          vdex_size_,
                          debug_info));
