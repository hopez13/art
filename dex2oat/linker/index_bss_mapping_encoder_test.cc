...
TEST(IndexBssMappingEncoder, TryMerge16BitIndex) {
  for (PointerSize pointer_size : {PointerSize::k32, PointerSize::k64}) {
    ...
    ASSERT_FALSE(encoder.TryMerge(18u, 3 * raw_pointer_size));      // Method index out of range.
  }
}
...
TEST(IndexBssMappingEncoder, TryMerge8BitIndex) {
  for (PointerSize pointer_size : {PointerSize::k32, PointerSize::k64}) {
    ...
    encoder.Reset(1u, 0u);
    ASSERT_FALSE(encoder.TryMerge(5u, raw_pointer_size + 1));       // Wrong bss_offset difference.
    ASSERT_FALSE(encoder.TryMerge(26u, raw_pointer_size));          // Method index out of range.
    ASSERT_TRUE(encoder.TryMerge(5u, raw_pointer_size));
    ASSERT_EQ(0u, encoder.GetEntry().GetBssOffset(encoder.GetIndexBits(), 1u, raw_pointer_size));
    ASSERT_EQ(raw_pointer_size,
              encoder.GetEntry().GetBssOffset(encoder.GetIndexBits(), 5u, raw_pointer_size));
    ...
    ASSERT_FALSE(encoder.TryMerge(26u, 2 * raw_pointer_size));      // Method index out of range.
    ...
    ASSERT_FALSE(encoder.TryMerge(26u, 3 * raw_pointer_size));      // Method index out of range.
  }
}
...
TEST(IndexBssMappingEncoder, TryMerge20BitIndex) {
  for (PointerSize pointer_size : {PointerSize::k32, PointerSize::k64}) {
    ...
    encoder.Reset(1u, 0u);
    ASSERT_FALSE(encoder.TryMerge(5u, raw_pointer_size + 1));       // Wrong bss_offset difference.
    ASSERT_FALSE(encoder.TryMerge(14u, raw_pointer_size));          // Method index out of range.
    ASSERT_TRUE(encoder.TryMerge(5u, raw_pointer_size));
    ASSERT_EQ(0u, encoder.GetEntry().GetBssOffset(encoder.GetIndexBits(), 1u, raw_pointer_size));
    ASSERT_EQ(raw_pointer_size,
              encoder.GetEntry().GetBssOffset(encoder.GetIndexBits(), 5u, raw_pointer_size));
    ...
    ASSERT_FALSE(encoder.TryMerge(14u, 2 * raw_pointer_size));      // Method index out of range.
    ...
    ASSERT_FALSE(encoder.TryMerge(14u, 3 * raw_pointer_size));      // Method index out of range.
  }
}
