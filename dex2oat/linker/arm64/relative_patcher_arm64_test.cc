...
TEST_F(Arm64RelativePatcherTestDefault, CallOtherJustTooFarAfter) {
  ...
  if (sizeof(OatQuickMethodHeader) < kArm64Alignment) {
    // The thunk needs to be on a kArm64Alignment-aligned address before the address where the
    // last method would have been if there was no thunk. If the size of the OatQuickMethodHeader
    // is at least kArm64Alignment, the thunk shall fit between the previous filler method and
    // that address. Otherwise, it shall be inserted before that filler method.
