...
void ImageWriter::RecordNativeRelocations(ObjPtr<mirror::Class> klass, size_t oat_index) {
  ...
  if (num_methods != 0) {
    ...
    // since some of their native methods are not compiled into machine code. And the write only
    // happens in non-debuggable since boot JNI stub exits without checking deoptimization, which
    // is needed in some debuggable situations.
    if (compiler_options_.IsBootImage() &&
