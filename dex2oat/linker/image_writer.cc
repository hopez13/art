...
bool ImageWriter::PrepareImageAddressSpace(TimingLogger* timings) {
  ...
  if (ClassLinker::kAppImageMayContainStrings && compile_app_image_) {
     ...
     * to or the beginning of the native reference array it is located in.  The
     * second element is the offset of the field relative to the base pointer.
     */
    for (const HeapReferencePointerInfo& ref_info : string_ref_info) {
      uint32_t base_offset;

      if (HasDexCacheStringArrayTag(ref_info.first)) {
        ++native_string_refs;

        void* obj_ptr = reinterpret_cast<void*>(ClearDexCacheStringArrayTag(ref_info.first));

        base_offset = SetDexCacheStringArrayTag(native_object_relocations_[obj_ptr].offset);
      } else {
        ++managed_string_refs;
        base_offset = GetImageOffset(reinterpret_cast<mirror::Object*>(ref_info.first));

        CHECK_EQ(ref_info.second & 1 , 0u);
      }
 ...
 public:
  ...
  void operator() (ObjPtr<mirror::Object> obj,
    ...
    if (image_writer_.IsValidAppImageStringReference(referred_obj)) {
      string_ref_info_.emplace_back(reinterpret_cast<uintptr_t>(obj.Ptr()), member_offset.Uint32Value());
    }
  }
...
/*
 * TODO (chriswailes): Investigate improved handling of DexCache objects by
 * walking their strings arrays at runtime.  This would require adding a single
 * info item for all DexCache objects with a non-empty strings array.
 */
    ...
    if (!IsInBootImage(object.Ptr())) {
      ...
      if (object->IsDexCache()) {
        ...
        for (size_t string_index = 0; string_index < dex_cache->NumStrings(); ++string_index) {
          mirror::StringDexCachePair dc_string_pair = dex_cache->GetStrings()[string_index].load();
          mirror::Object* referred_obj = dc_string_pair.object.AddressWithoutBarrier()->AsMirrorPtr();

          if (IsValidAppImageStringReference(referred_obj)) {
            ++new_native_ref_counter;

            visitor.string_ref_info_.emplace_back(
                SetDexCacheStringArrayTag(reinterpret_cast<uintptr_t>(dex_cache->GetStrings())),
                                          string_index);
          }
        }

        DCHECK_EQ(visitor.GetDexCacheStringRefCounter(), new_native_ref_counter);
      } else {
...
void ImageWriter::CopyMetadata() {
  ...
  AppImageReferenceOffsetInfo* sfo_section_base = reinterpret_cast<AppImageReferenceOffsetInfo*>(
