...
class RelocationEncoderBitmap {
 public:
  RelocationEncoderBitmap() {}

  dchecked_vector<uint8_t> Process(const std::vector<uint32_t>& relocations) {
    if (relocations.empty()) {
      return {};
    }
    CHECK(std::is_sorted(relocations.begin(), relocations.end()));
    size_t start_bits = relocations.back() / 4u + 1u;
    BitVector bv(start_bits, false, Allocator::GetMallocAllocator());
    bv.ClearAllBits();
    for (uint32_t v : relocations) {
      DCHECK_ALIGNED(v, 4u);
      bv.SetBit(v / 4u);
    }
    dchecked_vector<uint8_t> result(bv.GetSizeOf());
    bv.CopyTo(result.data(), bv.GetSizeOf());
    return result;
  }
};
...
dchecked_vector<uint8_t> ImageWriter::EncodeRelocations(const std::vector<uint32_t>& relocations) {
  ...
  RelocationEncoderBitmap encoderBitmap;
  dchecked_vector<uint8_t> encodingBitmap = encoderBitmap.Process(relocations);
  LOG(ERROR) << "VMARKO: relocations.size()= " << relocations.size()
      ...
      << " eB:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encodingBitmap));
