...
class RelocationEncoder0 {
 public:
  ...
  void Encode(uint32_t offset) {
    ...
    EncodeUnsignedLeb128(&data_, index - base_index_);
    base_index_ = index;
  }
...
class RelocationEncoder5 {
 public:
  void Encode(uint32_t offset) {
    DCHECK_ALIGNED(offset, kReferenceSize);
    uint32_t index = offset / kReferenceSize;
    data_.resize(BitsToBytesRoundUp(index + 1));
    BitMemoryRegion region(MemoryRegion(data_.data(), data_.size()));
    region.StoreBit(index, true);
  }
...
class RelocationEncoder6 {
 public:
  RelocationEncoder6() : data_(), writer_(&data_) {
  }

  void Encode(uint32_t offset) {
    DCHECK_ALIGNED(offset, kReferenceSize);
    uint32_t index = offset / kReferenceSize;
    EncodeVarintBits(writer_, index - base_index_);
    base_index_ = index;
  }
 ...
 private:
  ...
  dchecked_vector<uint8_t> data_;
  BitMemoryWriter<dchecked_vector<uint8_t>> writer_;
...
static size_t lz4_size(ArrayRef<const uint8_t> source) {
  ...
  std::vector<char> compressed_data(compressed_max_size);
...
static size_t lz4hc_size(ArrayRef<const uint8_t> source) {
  const size_t compressed_max_size = LZ4_compressBound(source.size());
  std::vector<char> compressed_data(compressed_max_size);
  size_t lz4_data_size = LZ4_compress_HC(
      reinterpret_cast<const char*>(source.data()),
      compressed_data.data(),
      source.size(),
      compressed_data.size(),
...
std::string SizeAndLZ4Sizes(ArrayRef<const uint8_t> source) {
  std::ostringstream oss;
  oss << source.size() << "/" << lz4_size(source) << "/" << lz4hc_size(source);
  return oss.str();
}
...
dchecked_vector<uint8_t> ImageWriter::EncodeRelocations(const std::vector<uint32_t>& relocations) {
  ...
  RelocationEncoder0 encoder0;
  for (uint32_t value : relocations) {
    encoder0.Encode(value);
  }
  const dchecked_vector<uint8_t>& encoding0 = encoder0.Finish();
  ...
  RelocationEncoder5 encoder5;
  for (uint32_t value : relocations) {
    encoder5.Encode(value);
  }
  const dchecked_vector<uint8_t>& encoding5 = encoder5.Finish();
  RelocationEncoder6 encoder6;
  for (uint32_t value : relocations) {
    encoder6.Encode(value);
  }
  const dchecked_vector<uint8_t>& encoding6 = encoder6.Finish();
  LOG(ERROR) << "VMARKO: relocations.size()= " << relocations.size()
      << " encoded_size=" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(result))
      << " e0:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding0))
      << " e1:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding1))
      << " e2:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding2))
      << " e3:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding3))
      << " e4:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding4))
      << " e5:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding5))
      << " e6:" << SizeAndLZ4Sizes(ArrayRef<const uint8_t>(encoding6));
