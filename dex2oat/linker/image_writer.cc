...
dchecked_vector<uint8_t> ImageWriter::EncodeRelocations(const std::vector<uint32_t>& relocations) {
  ...
  LOG(ERROR) << "VMARKO: relocations.size()= " << relocations.size()
      << " encoded_size=" << result.size() << "/" << lz4_size(ArrayRef<const uint8_t>(result))
      << " e0:" << encoding0.size() << "/" << lz4_size(ArrayRef<const uint8_t>(encoding0))
      << " e1:" << encoding1.size() << "/" << lz4_size(ArrayRef<const uint8_t>(encoding1))
      << " e2:" << encoding2.size() << "/" << lz4_size(ArrayRef<const uint8_t>(encoding2))
      << " e3:" << encoding3.size() << "/" << lz4_size(ArrayRef<const uint8_t>(encoding3))
      << " e4:" << encoding4.size() << "/" << lz4_size(ArrayRef<const uint8_t>(encoding4));
