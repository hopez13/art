...
class ImageWriter::LayoutHelper {
 ...
 private:
  ...
  void ProcessNonBootImageInterns(Thread* self) REQUIRES_SHARED(Locks::mutator_lock_);
...
void ImageWriter::LayoutHelper::ProcessDexFileObjects(Thread* self) {
  ...
  ProcessNonBootImageInterns(self);
...
void ImageWriter::LayoutHelper::ProcessNonBootImageInterns(Thread* self) {
...
void ImageWriter::LayoutHelper::FinalizeInternTables() {
  ...
  non_dex_file_interns_.resize(std::distance(non_dex_file_interns_.begin(), kept_it));
...
void ImageWriter::CopyAndFixupNativeData(size_t oat_index) {
  ...
  if (image_info.intern_table_bytes_ > 0) {
    ...
    if (kIsDebugBuild) {
      ...
      DCHECK_EQ(temp_intern_table.strong_interns_.tables_[0].Size(), intern_table.size());
    }
  }
