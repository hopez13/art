...
bool ImageWriter::IsImageObject(ObjPtr<mirror::Object> obj) const {
  ...
  // Exclude also dex_cache->GetLocation().
  // TODO: Do it some other way. We could try and restrict IsImageObject() to the LayoutHelper,
  // make explicit exclusion in VerifyImageBinSlotsAssigned() and use IsImageBinSlotAssigned()
  // for all checks after the layout.
  if (obj->IsString()) {
    ...
    for (const ClassLinker::DexCacheData& data : class_linker->GetDexCachesData()) {
      ...
      if (dex_cache->GetLocation() == obj) {
        return ContainsElement(compiler_options_.GetDexFilesForOatFile(), dex_cache->GetDexFile());
      }
    }
  }
  return true;
...
bool ImageWriter::IsImageDexCache(ObjPtr<mirror::DexCache> dex_cache) const {
  ...
  // Dex caches for the boot class path components that are not a part of the boot image
  // cannot be garbage collected in PrepareImageAddressSpace() but we do not want to
