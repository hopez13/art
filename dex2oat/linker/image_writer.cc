...
bool ImageWriter::PrepareImageAddressSpace(TimingLogger* timings) {
  ...
  if (ClassLinker::kAppImageMayContainStrings && compile_app_image_) {
     ...
     * the first case second element is the offset of the field relative to the
     * object's base address.  In the second case, it is the index of the
  ...
  heap->VisitObjects([this, &visitor](ObjPtr<mirror::Object> object)
    ...
    if (!IsInBootImage(object.Ptr())) {
      ...
      if (object->IsDexCache()) {
        ...
        // Not all strings in this array are AppImage strings.  Some live in
        // the boot image.  As such, walking the entire array at load time
        // would be slow, so  we need an offset info pair for each AppImage
        // string in the array.
