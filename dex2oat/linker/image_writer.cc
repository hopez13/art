...
std::optional<DirtyEntry> ParseDirtyEntry(std::string_view entry_str) {
  ...
  Split(std::string_view(entry_str), ' ', &tokens);
  if (tokens.empty()) {
    // entry_str is empty.
    return std::nullopt;
  }
...
HashMap<mirror::Object*, uint32_t> MatchDirtyObjectPaths(
  ...
  auto get_array_element = [](mirror::Object* cur_obj, const DirtyEntry::RefInfo& ref_info)
    ...
    ObjPtr<mirror::Object> next_obj = cur_obj->AsObjectArray<mirror::Object>()->Get(idx);
    std::string temp;
    if (!next_obj.IsValid() || next_obj->GetClass()->GetDescriptor(&temp) != ref_info.type) {
      return nullptr;
    }
    return next_obj.Ptr();
  };
