...
class ImageWriter::PruneClassesVisitor : public ClassVisitor {
 public:
  ...
  size_t Prune() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    // table also contains class tables from boot images we're compiling against but we are
    // not pruning these boot image classes, so all classes to remove are in the last set.
