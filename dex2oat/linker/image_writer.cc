...
std::optional<HashSet<mirror::Object*>> ImageWriter::MatchDirtyObjectOffsets(
  ...
  HashSet<mirror::Object*> matched_objects;
  ...
  if (matched_objects.size() != dirty_entries.size()) {
...
void ImageWriter::TryRecalculateOffsetsWithDirtyObjects() {
  ...
  const std::optional<HashSet<mirror::Object*>> matched_dirty_objects =
      MatchDirtyObjectOffsets(*dirty_entries);

  if (!matched_dirty_objects || matched_dirty_objects->empty()) {
    return;
  }

  // Calculate offsets again, now with dirty object offsets
  LOG(INFO) << "Recalculating object offsets using dirty-image-objects";
  dirty_objects_ = std::move(*matched_dirty_objects);
  ResetObjectOffsets();
  CalculateNewObjectOffsets();
}
