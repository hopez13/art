...
std::vector<ImageWriter::RefInfoPair> ImageWriter::CollectStringReferenceInfo()
    REQUIRES_SHARED(Locks::mutator_lock_) {
  gc::Heap* const heap = Runtime::Current()->GetHeap();
...
class ImageWriter::NativeGCRootInvariantVisitor {
 public:
  ...
  void VisitRoot(mirror::CompressedReference<mirror::Object>* root) const
    ...
    if (curr_obj_->IsClass()) {
      ...
      class_loader_violation = class_loader_violation ||
                                     image_writer_.IsValidAppImageStringReference(
                                         referred_obj);
  ...
  // Returns true iff the only reachable native string references are through DexCach objects.
...
void ImageWriter::VerifyNativeGCRootInvariants() REQUIRES_SHARED(Locks::mutator_lock_) {
  gc::Heap* const heap = Runtime::Current()->GetHeap();
  ...
  heap->VisitObjects([this, &visitor](ObjPtr<mirror::Object> object)
    ...
    if (!IsInBootImage(object.Ptr())) {
      object->VisitReferences<true, kVerifyNone, kWithoutReadBarrier>(visitor, visitor);
    }
  });
...
void ImageWriter::CopyMetadata() {
  ...
  ImageSection image_sections[ImageHeader::kSectionCount];
  image_info.CreateImageSections(image_sections);
...
bool ImageWriter::Write(int image_fd,
  ...
  for (size_t i = 0; i < image_filenames.size(); ++i) {
    ...
    if (kIsDebugBuild) {
      size_t tail_size = bitmap_section.Size() +
                         image_header->GetImageRelocationsSection().Size() +
                         sizeof(ImageHeader);

      size_t total_uncompressed_size = raw_image_data.size() + tail_size,
             total_compressed_size   = image_data.size() + tail_size;
...
size_t ImageWriter::ImageInfo::CreateImageSections(ImageSection* out_sections) const {
   ...
   * Objects section
   */
  new(&out_sections[ImageHeader::kSectionObjects])
    ImageSection(0u, image_end_);

  /*
   * Field section
   */
  new(&out_sections[ImageHeader::kSectionArtFields])
    ImageSection(GetBinSlotOffset(Bin::kArtField), GetBinSlotSize(Bin::kArtField));

  /*
   * Method section
   */
  new(&out_sections[ImageHeader::kSectionArtMethods])
  ...
  ImageSection& interned_strings_section =
      *new(&out_sections[ImageHeader::kSectionInternedStrings])
  ...
  ImageSection& class_table_section =
      *new(&out_sections[ImageHeader::kSectionClassTable])
  ...
  ImageSection& string_reference_offsets =
      *new(&out_sections[ImageHeader::kSectionStringReferenceOffsets])
        ImageSection(cur_pos, sizeof(uint32_t) * num_string_references_);

  // Return the number of bytes described by these sections.
  return string_reference_offsets.End();
}
