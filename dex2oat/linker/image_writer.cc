...
class RelocationEncoderLZ4 {
 public:
  RelocationEncoderLZ4()
      : data_(),
        base_index_(0u) {
    data_.reserve(64 * KB);
  }
  ...
  const dchecked_vector<uint8_t>& Finish() {
    if (base_index_ != 0u) {
      data_lz4_.resize(LZ4_compressBound(data_.size()));
      data_lz4_.resize(LZ4_compress_HC(
          reinterpret_cast<const char*>(data_.data()),
          reinterpret_cast<char*>(data_lz4_.data()),
          data_.size(),
          data_lz4_.size(),
          LZ4HC_CLEVEL_MAX));
    }
 ...
 private:
  ...
  dchecked_vector<uint8_t> data_;
  dchecked_vector<uint8_t> data_lz4_;
  uint32_t base_index_;
};
...
dchecked_vector<uint8_t> ImageWriter::EncodeRelocations(const std::vector<uint32_t>& relocations) {
  ...
  RelocationEncoderLZ4 encoderLZ4;
  for (uint32_t value : relocations) {
    encoderLZ4.Encode(value);
  }
  const dchecked_vector<uint8_t>& encodingLZ4 = encoderLZ4.Finish();
  LOG(ERROR) << "VMARKO: relocations.size()= " << relocations.size()
      ...
      << " encodingLZ4_size=" << encodingLZ4.size();
