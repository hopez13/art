  ...
  for (size_t i = 0; i < image_filenames.size(); ++i) {
    ...
    auto add_blocks = [&](uint32_t offset, uint32_t size) {
      while (size != 0u) {
        ...
        blocks.push_back({.image_offset_ = offset, .image_size_ = cur_size});
        offset += cur_size;
        size -= cur_size;
      }
    };

    if (block_mode_ == ImageBlockMode::kSolid) {
      add_blocks(sizeof(ImageHeader), image_header->GetImageSize() - sizeof(ImageHeader));
    } else {
      CHECK(block_mode_ == ImageBlockMode::kPerSection);
      for (size_t j = 0; j < ImageHeader::kSectionCount; ++j) {
        const ImageHeader::ImageSections section_type = static_cast<ImageHeader::ImageSections>(j);
        if (section_type == ImageHeader::kSectionImageBitmap) {
          // Skip the bitmap section since it currently has special handling.
          continue;
        }
        const ImageSection& section = image_header->GetImageSection(section_type);
        add_blocks(section.Offset(), section.Size());
      }
    }
    ...
    for (ImageHeader::Block& block : blocks) {
      ArrayRef<const uint8_t> raw_image_data(image_info.image_.Begin() + block.image_offset_,
                                             block.image_size_);
      ...
      if (!is_compressed) {
        ...
        out_offset = block.image_offset_;
      }

      // Fill in the compressed location of the block.
      block.data_offset_ = out_offset;
      block.data_size_ = image_data.size();
      block.storage_mode_ = image_storage_mode_;
...
ImageWriter::ImageWriter(
    ...
    : compiler_options_(compiler_options),
      ...
      block_mode_(ImageBlockMode::kSolid),
