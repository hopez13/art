...
bool ImageWriter::PruneImageClassInternal(
  ...
  if (klass == nullptr || IsInBootImage(klass.Ptr())) {
    if (compiler_options_.IsAppImage() && IsInBootImage(klass.Ptr()) && !klass->IsObjectClass() && !klass->IsStringClass() && !klass->IsClassClass()  && klass->PrettyDescriptor() != "java.io.Serializable") {
      return true;
    }
    return false;
  }
