...
bool ImageWriter::AllocMemory() {
  for (ImageInfo& image_info : image_infos_) {
    ...
    image_info.image_bitmap_ = gc::accounting::ContinuousSpaceBitmap::Create("image bitmap",
        image_info.image_.Begin(), RoundUp(image_info.image_end_, kElfSegmentAlignment),
        /* is_image_part= */ true);
...
void ImageWriter::CreateHeader(size_t oat_index, size_t component_count) {
  ...
  // Bitmap size alignment isn't strictly needed and is performed to ensure image file
  // contents are exactly same independently of the page size - to simplify verification.
  *bitmap_section = ImageSection(RoundUp(image_end, kElfSegmentAlignment),
      RoundUp(bitmap_bytes, kElfSegmentAlignment));
