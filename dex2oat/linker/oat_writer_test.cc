...
class OatTest : public CommonCompilerDriverTest {
 protected:
  ...
  void CheckOatWriteResult(ScratchFile& oat_file,
    ...
    int index = 0;
    for (const std::unique_ptr<const DexFile>& dex_file_data : input_dexfiles) {
      std::unique_ptr<const DexFile> opened_dex_file =
          opened_oat_file->GetOatDexFiles()[index]->OpenDexFile(&error_msg);

      ASSERT_EQ(opened_oat_file->GetOatDexFiles()[index]->GetDexFileLocationChecksum(),
