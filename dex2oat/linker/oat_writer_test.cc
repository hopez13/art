...
void OatTest::TestDexFileInput(bool verify, bool low_4gb, bool use_profile) {
  ...
  ASSERT_EQ(opened_oat_file->GetOatDexFiles()[0]->GetDexFileLocationChecksum(), dex_file1_data->GetHeader().checksum_);
  ASSERT_EQ(opened_oat_file->GetOatDexFiles()[1]->GetDexFileLocationChecksum(), dex_file2_data->GetHeader().checksum_);
