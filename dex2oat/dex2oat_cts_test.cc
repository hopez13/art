...
TEST_F(Dex2oatTest, CompilationHooks) {
  ...
  CHECK(oat_file != nullptr) << oat_location;
  args.push_back("--oat-fd=" + std::to_string(oat_file->Fd()));
  args.push_back("--oat-location=" + oat_location);

  std::unique_ptr<File> vdex_file(OS::CreateEmptyFile(vdex_location.c_str()));
  CHECK(vdex_file != nullptr) << vdex_location;
  args.push_back("--output-vdex-fd=" + std::to_string(vdex_file->Fd()));

  args.push_back("--enable-palette-compilation-hooks");
