...
constexpr const char* kBindMountTmpDir = "/mnt/pre_reboot_dexopt_tmp";
...
ScopedAStatus DexoptChrootSetup::setUp() {
...
Result<void> DexoptChrootSetup::SetUpChroot() const {
  ...
  for (const std::string& src : additional_system_partitions) {
    OR_RETURN(BindMount(src, CHROOT_DIR + src));
  }
  ...
  for (const std::string& dir : tmpfs_dirs) {
    if (mount(/*source=*/"tmpfs",
              (CHROOT_DIR + dir).c_str(),
              /*fs_type=*/"tmpfs",
              MS_NODEV | MS_NOEXEC | MS_NOSUID,
              ART_FORMAT("mode={:#o}", kMode).c_str()) != 0) {
      return ErrnoErrorf("Failed to mount tmpfs at '{}'", CHROOT_DIR + dir);
    }
  }

  // Set SELinux file context. We can't use restorecon here because we are not in chroot.
  OR_RETURN(SetFileCon(CHROOT_DIR + "/apex"s, "u:object_r:apex_mnt_dir:s0"));
  OR_RETURN(SetFileCon(CHROOT_DIR + "/linkerconfig"s, "u:object_r:linkerconfig_file:s0"));
  ...
  for (const std::string& dir : tmp_dirs) {
    OR_RETURN(CreateDirs(CHROOT_DIR + dir));
  }

  OR_RETURN(SetFileCon(CHROOT_DIR + "/mnt/expand"s, "u:object_r:mnt_expand_file:s0"));
...
Result<void> DexoptChrootSetup::TearDownChroot() const {
  ...
  std::error_code ec;
  std::filesystem::remove_all(CHROOT_DIR, ec);
  if (ec) {
