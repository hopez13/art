...
Result<void> MountTmpfs(const std::string& target, const char* se_context) {
...
Result<void> DexoptChrootSetup::SetUpChroot(const std::optional<std::string>& ota_slot) const {
  ...
  OR_RETURN(MountTmpfs(CHROOT_DIR + "/mnt"s, /*se_context=*/nullptr));
