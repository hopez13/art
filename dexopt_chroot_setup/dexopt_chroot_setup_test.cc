...
class DexoptChrootSetupTest : public CommonArtTest {
 protected:
  void SetUp() override {
    ...
    char* con;
    if (getfilecon(DexoptChrootSetup::CHROOT_DIR, &con) != 0) {
      ASSERT_EQ(errno, ENOENT) << ART_FORMAT(
          "Failed to getfilecon '{}': {}", DexoptChrootSetup::CHROOT_DIR, strerror(errno));
      GTEST_SKIP() << ART_FORMAT("This platform is too old and doesn't have directory '{}'",
                                 DexoptChrootSetup::CHROOT_DIR);
    }
