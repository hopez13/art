...
static constexpr const char* kPaletteSystemLibrary = "libpalette-system.so";
...
#define PALETTE_METHOD_TYPE_ALIAS(Name, ...) \
  using Name ## Method = enum PaletteStatus(*)(__VA_ARGS__);
...
#undef PALETTE_LOADER_METHOD_ACCESSOR
 ...
 private:
  PaletteLoader();
  PaletteLoader(const PaletteLoader&) = delete;
  PaletteLoader(PaletteLoader&&) = delete;
  PaletteLoader& operator=(const PaletteLoader&) = delete;
...
void* PaletteLoader::OpenLibrary() {
  ...
  if (handle == nullptr) {
    // TODO(oth): Take a dependency on liblog here.
    abort();
  }
  return handle;
}
...
enum PaletteStatus PaletteMetricsLogEvent(
  ...
  PaletteMetricsLogEventMethod m = PaletteLoader::Instance().GetPaletteMetricsLogEventMethod();
  return m(category, package_name, tagged_data, tagged_data_count);
}
...
enum PaletteStatus PaletteTraceInit() {
  PaletteTraceInitMethod m = PaletteLoader::Instance().GetPaletteTraceInitMethod();
  return m();
}
