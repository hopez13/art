...
TEST(JNIInvocation, Debuggable) {
    ...
    const char* result =
        JniInvocation::GetLibrary(nullptr, buffer, is_debuggable, get_library_system_property);
    EXPECT_NE(nullptr, result);
    if (result != nullptr) {
        EXPECT_EQ(0, strcmp(result, kTestNonNull2));
        EXPECT_NE(0, strcmp(result, kExpected));
    }
    ...
    EXPECT_NE(nullptr, result);
    if (result != nullptr) {
        EXPECT_EQ(0, strcmp(result, kTestNonNull));
        EXPECT_NE(0, strcmp(result, kTestNonNull2));
    }
#else
...
TEST(JNIInvocation, NonDebuggable) {
    ...
    char buffer[PROPERTY_VALUE_MAX];
    const char* result = JniInvocation::GetLibrary(nullptr, buffer, is_debuggable, nullptr);
    EXPECT_NE(nullptr, result);
    if (result != nullptr) {
        EXPECT_EQ(0, strcmp(result, kExpected));
        EXPECT_NE(0, strcmp(result, kTestNonNull));
        EXPECT_NE(0, strcmp(result, kTestNonNull2));
    }
