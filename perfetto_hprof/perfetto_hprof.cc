...
// These statics need to be local to the functions to have deterministic initialization order.
...
class JavaHprofDataSource : public perfetto::DataSource<JavaHprofDataSource> {
 public:
  ...
  void OnStart(const StartArgs&) override {
    LOG(INFO) << "trying to signal data source start";
    art::MutexLock lk(art_thread(), GetStateMutex());
    if (state == State::kWaitForStart) {
      state = State::kStart;
      LOG(INFO) << "signal data source start";
      GetStateCV().Broadcast(art_thread());
    } else {
      LOG(INFO) << "not wait for start";
    }
  }
  ...
  static art::Thread* art_thread() {
    if (self_)
      return self_;
    self_ = art::Thread::Current();
    if (!self_) {
      art::Runtime* runtime = art::Runtime::Current();
      if (!runtime->AttachCurrentThread("hprof_producer", /*as_daemon=*/ true,
...
extern "C" bool ArtPlugin_Initialize()
  ...
  std::thread th([] {
    art::Runtime* runtime = art::Runtime::Current();
    if (!runtime->AttachCurrentThread("hprof_listener", /*is_daemon=*/ true,
                                      runtime->GetSystemThreadGroup(), /*create_peer=*/ false)) {
      LOG(ERROR) << "failed to attach thread.";
      return;
    }
