...
void DumpPerfettoOutOfMemory() {
  ...
  ForkAndRun(
    ...
    [](pid_t child) {
      ...
      // in case of any trouble, the watchdog of the child will sigkill it and
    ...
    [self](pid_t dumped_pid, uint64_t timestamp) {
      ...
      // TODO: Use ActivateTriggers once b/259929612 is fixed
      // perfetto::Tracing::ActivateTriggers({"com.android.telemetry.art-outofmemory"});
      std::string error_msg;
      art::ExecAndReturnCode(
        {"/system/bin/trigger_perfetto", "com.android.telemetry.art-outofmemory"},
        &error_msg);
...
bool IsUserDebug() {
  return android::base::GetProperty("ro.build.type", "") == "userdebug";
}
