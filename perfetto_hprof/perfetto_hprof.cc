 ...
 public:
  ...
  void OnStart(const StartArgs&) override {
    ...
    if (g_state == State::kWaitForStart) {
      // State transition in the child process
      if (enabled()) {
...
// Waits for the data source OnStart with a timeout.
// Returns false on timeout or data source rejection.
  ...
  ForkAndRun(
    ...
    [self](pid_t dumped_pid, uint64_t timestamp) {
      ...
      if (!TimedWaitForDataSource(self, 500)) {
        if (g_state == State::kEnd) {
          LOG(INFO) << "OOME hprof rejected by config";
        } else {
