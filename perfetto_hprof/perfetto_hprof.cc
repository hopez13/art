...
class ReferredObjectsFinder {
 public:
  ...
  void operator()(art::ObjPtr<art::mirror::Object> obj, art::MemberOffset offset,
    ...
    if (min_nonnull_ptr_ && (!*min_nonnull_ptr_ || *min_nonnull_ptr_ > ref)) {
 ...
 private:
  ...
  art::mirror::Object** min_nonnull_ptr_ = 0;
};
            ...
            art::Runtime::Current()->GetHeap()->VisitObjectsPaused(
                [&writer, &interned_fields, &interned_locations, &reference_field_ids,
                 &reference_object_ids, &interned_classes, &ignored_types](
                  ...
                  if (base_obj_id) {
                    // We need to decrement the base for object ids so t hat we can tell apart
                    // null references.
                    base_obj_id--;
                  }
                  ...
                  for (auto& p : referred_objects) {
                    ...
                    if (!referred_obj) continue;
                    if (IsIgnored(ignored_types, referred_obj)) {
                      referred_obj = nullptr;
                    }
                    if (base_obj_id && referred_obj) {
                  ...
                  for (auto& p : referred_objects) {
                    ...
                    reference_object_ids->Append(GetObjectId(referred_obj) - base_obj_id);
                  }
                  if (emit_field_ids) {
