...
int g_listener_thread_ready_event_fd;
...
extern "C" bool ArtPlugin_Initialize()
  ...
  g_listener_thread_ready_event_fd = eventfd(0, EFD_CLOEXEC);
  if (g_listener_thread_ready_event_fd == -1) {
    close(g_signal_pipe_fds[0]);
    close(g_signal_pipe_fds[1]);
    PLOG(ERROR) << "Failed to eventfd";
    return false;
  }
  ...
  std::thread th([] {
    ...
    uint64_t one = 1;
    if (!write(g_listener_thread_ready_event_fd, &one, sizeof(one))) {
      PLOG(ERROR) << "write";
      return;
    }
  ...
  th.detach();
  // Wait for the listener thread to spawn to prevent spurious thread notifications.
  // We cannot use a Mutex + CV variable here, as we are holding MutatorLock.
  uint64_t recv;
  if (read(g_listener_thread_ready_event_fd, &recv, sizeof(recv)) == -1) {
    PLOG(ERROR) << "read";
  }
  close(g_listener_thread_ready_event_fd);
  return true;
}
