...
void DumpPerfettoOutOfMemory() {
  ...
  ForkAndRun(
    ...
    [self](pid_t dumped_pid, uint64_t timestamp) {
      ...
      // TODO: Use ActivateTriggers once available.
      // perfetto::Tracing::ActivateTriggers({"art-oome"});
      std::string error_msg;
      art::ExecAndReturnCode({"/system/bin/trigger_perfetto", "art-oome"}, &error_msg);
