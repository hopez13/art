 ...
 public:
  void OnSetup(const SetupArgs& args) override {
    ...
    if (cfg->has_process_cmdline()) {
      ...
      ssize_t sz = perfetto::profiling::NormalizeCmdLine(&cmdline_ptr, static_cast<size_t>(rd));
      if (sz == -1) {
        PLOG(ERROR) << "failed to normalize cmdline";
      }
      std::string cmdline_normalized(cmdline_ptr, static_cast<size_t>(sz)); 
      for (auto it = cfg->process_cmdline(); it; ++it) {
          ...
          std::string other_normalized(other_ptr, other_sz);
          if (other_normalized == cmdline_normalized) {
            enabled_ = true;
            return;
          }
      }
