...
public class NewArrayLengthChanger extends CodeMutator {
  ...
  protected void applyMutation(Mutation uncastMutation) {
    ...
    boolean foundInsnIdx = foundInstruction(newArrayInsn, newArrayInsnIdx, mutatableCode);
  ...
  protected boolean foundInstruction(MInsn newArrayInsn, int newArrayInsnIdx, MutatableCode mutatableCode){
    int regB = (int) newArrayInsn.insn.vregB;
    for (int i = newArrayInsnIdx - 1; i >= 0; i--) {
      MInsn candidateInsn = mutatableCode.getInstructionAt(i);
      if (isConst(candidateInsn)) {
        int tempReg = (int) candidateInsn.insn.vregA;
        if (tempReg == regB) {
          candidateInsn.insn.vregB = rng.nextInt(100);
          Log.info("Changed the length of the array to " + candidateInsn.insn.vregB);
          return true;
        }
      }
    }
