...
public class NewArrayLengthChanger extends CodeMutator {
  ...
  // Cast the Mutation to our AssociatedMutation, so we can access its fields.
    ...
    int newArrayIdx = mutation.newArrayToChangeIdx;
    MInsn newArrayInsn = mutatableCode.getInstructionAt(newArrayIdx);
    boolean foundInsnIdx = foundInstruction(newArrayInsn, mutatableCode);
    ...
    if (!foundInsnIdx) {
      ...
      mutatableCode.insertInstructionAt(newInsn, newArrayIdx);
  ...
  protected boolean foundInstruction(MInsn newArrayInsn, MutatableCode mutatableCode){

    int newArrayIdx= mutatableCode.getInstructionCount();
    for (MInsn mInsn : mutatableCode.getInstructions()) {
      if (newArrayInsn == mInsn) {
        newArrayIdx = mutatableCode.getInstructionIndex(mInsn);
      }
    }

    int regB = (int) newArrayInsn.insn.vregB;
    for (int i = newArrayIdx - 1; i >= 0; i--) {
      if (isConst(i)) {
        MInsn constInsn = mutatableCode.getInstructionAt(i);
        int tempReg = (int) constInsn.insn.vregA;
        if (tempReg == regB) {
          constInsn.insn.vregA = rng.nextInt(100);
          Log.info("Changed the length of the array to " + constInsn.insn.vregB);
          return true;
        }
      }
    }
  ...
  protected boolean isConst(int insnIdx) {
    MInsn arrayInstruction = newArrayLengthInsns.get(insnIdx);
    Opcode opcode = arrayInstruction.insn.info.opcode;
    return opcode == Opcode.CONST_4 || opcode == Opcode.CONST_16 || opcode == Opcode.CONST;
  }

  private boolean isNewArray(Opcode opcode) {
