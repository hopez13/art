...
public class CmpCondChanger extends CodeMutator {
  /**
  ...
  public CmpCondChanger() { }

  public CmpCondChanger(Random rng, MutationStats stats, List<Mutation> mutations) {
  ...
  private void generateCachedEqualityInsns(MutatableCode mutatableCode) {
    ...
    for (MInsn mInsn : mutatableCode.getInstructions()) {
      if (isComparisionOperation(mInsn)) {
  ...
  protected boolean canMutate(MutatableCode mutatableCode) {
    ...
    Log.debug("No comparision operation, skipping...");
    return false;
  }
  ...
  private boolean isComparisionOperation(MInsn mInsn) {
    Opcode opcode = mInsn.insn.info.opcode;
