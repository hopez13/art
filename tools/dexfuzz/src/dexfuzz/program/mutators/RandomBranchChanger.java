...
public class RandomBranchChanger extends IfBranchChanger {
  ...
  protected Opcode getModifiedOpcode(MInsn mInsn) {
    ...
    isNonZeroComparator(opcode);
    if (isNonZeroComparator(opcode)) {
      int index = findIndex(EQUALITY_CMP_OP_LIST, opcode);
      int length = EQUALITY_CMP_OP_LIST.length;
      if (index == -1) {
        Log.errorAndQuit("Not a comparison operator.");
      }
  ...
  private boolean isNonZeroComparator(Opcode opcode) {
    if(Opcode.isBetween(opcode, Opcode.IF_EQZ, Opcode.IF_LEZ)) {
      return false;
    }
    return true;
  }
