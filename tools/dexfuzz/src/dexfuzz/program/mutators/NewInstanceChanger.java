...
public class NewInstanceChanger extends CodeMutator {
  ...
  protected void applyMutation(Mutation uncastMutation) {
    ...
    changeInvokeDirect(mutation.foundInsnIdx,mutation);
   ...
   */
  protected void changeInvokeDirect
  (int foundInsnIdx,Mutation uncastMutation) {
    AssociatedMutation mutation = (AssociatedMutation) uncastMutation;
    MutatableCode mutatableCode = mutation.mutatableCode;
    if (foundInsnIdx == -1 ||
        foundInsnIdx == mutatableCode.getInstructionCount()) {
      return;
    }
    MInsn invokeInsn = mutatableCode.getInstructionAt(foundInsnIdx + 1);
    long oldMethodIdx =
        ((ContainsPoolIndex)invokeInsn.insn.info.format).getPoolIndex(invokeInsn.insn);
    if (invokeInsn.insn.info.opcode == Opcode.INVOKE_DIRECT || invokeInsn.insn.info.opcode ==
        Opcode.INVOKE_DIRECT_RANGE) {
      String className = mutatableCode.program.getMethodName(mutation.newInstanceTypeIdx);
      String methodName = mutatableCode.program.getMethodName((int) oldMethodIdx);
      String shorty = mutatableCode.program.getMethodName((int) oldMethodIdx);
