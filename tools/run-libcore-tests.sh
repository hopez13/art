...
if [ $execution_mode = "device" -o $execution_mode = "host" ]; then
  ...
  if $gcstress; then
    ...
    # Also set pause threshold as long pauses cause explicit gc logging to occur irrespective
    # of -XX:AlwayLogExplicitGcs:false. The pause threshold is in milliseconds (default: 5ms).
    vogar_args="$vogar_args --vm-arg -XX:LongPauseLogThreshold=15"
  else
