...
std::map<std::string, std::set<std::string>> ReadDeps() {
  ...
  while (getline(&line.str, &len, in) != -1) {
    ...
    slash_pos = included.rfind('/');
    if (slash_pos != std::string::npos) {
      included = included.substr(slash_pos + 1u);
    }
...
int main(int argc, const char* const* argv) {
  ...
  if (argc == 2) {
    ...
    size_t num_cc_files = 0;
    size_t total_seen_for_cc_files = 0u;
    for (auto&& entry : map) {
      if (entry.first.size() >= 3u && entry.first.substr(entry.first.size() - 3u) == ".cc") {
        Dumper dumper(dev_null, "", &map);
        dumper.Dump(entry.first, 0u);
        std::cout << entry.first << ": " << dumper.NumSeen() << " files seen." << std::endl;
        num_cc_files += 1u;
        total_seen_for_cc_files += dumper.NumSeen();
      }
    }
    std::cout << "Seen " << total_seen_for_cc_files << " for " << num_cc_files << " cc files."
        << std::endl;
  }
}
