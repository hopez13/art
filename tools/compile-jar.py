...
def parse_args():
  parser = argparse.ArgumentParser(description="compile dex or jar files", epilog="Unrecognized options are passed on to dex2oat unmodified.")
  parser.add_argument(
      "--dex2oat",
      action="store",
  ...
  parser.add_argument("--odex-file",
                      action="store",
                      help="odex file to write. File discarded if not set",
                      default=None)
  parser.add_argument("dex_files", help="dex/jar files", nargs="+", metavar="DEX")
  return parser.parse_known_args()

def get_bcp_runtime_args(image, arch):
  if arch != "host32" and arch != "host64":
    args = ['art/tools/host_bcp.sh', os.path.expandvars('${{OUT}}/system/framework/oat/{}/services.odex'.format(arch)), '--use-first-dir']
    ...
    res = open(os.path.expandvars("$ANDROID_HOST_OUT/apex/art_boot_images/javalib/{}/boot.oat".format("x86" if arch == "host32" else "x86_64")), 'rb').read()
    bcp_tag = b'bootclasspath\0'
    bcp_start = res.find(bcp_tag) + len(bcp_tag)
    bcp = res[bcp_start:bcp_start + res[bcp_start:].find(b'\0')]
    str_bcp = bcp.decode()
    return ['--runtime-arg', '-Xbootclasspath:{}'.format(str_bcp), '--runtime-arg', '-Xbootclasspath-locations:{}'.format(str_bcp)]
...
def get_profile_args(args, location_base):
  ...
  if args.profile_file:
    ...
    prof_in = os.fdopen(prof_in_fd, 'w')
  ...
  for f in args.dex_files:
    ...
    profman_args.append('--dex-location={}/{}'.format(location_base, os.path.basename(f)))
  ...
  if args.debug:
    ...
    os.lseek(prof_out_fd, 0, os.SEEK_SET)
    return ['--profile-file-fd={}'.format(prof_out_fd)]
...
def main():
  ...
  if args.arch == "host32" or args.arch == "host64":
    ...
    boot_image = os.path.expandvars("$ANDROID_HOST_OUT/apex/art_boot_images/javalib/boot.art")
    android_root="."
    for f in args.dex_files:
      extra.append("--dex-location={}".format("{}/{}".format(location_base, os.path.basename(f))))
      extra.append("--dex-file={}".format(f))
  else:
    location_base = "/system/framework"
    real_arch = args.arch
    boot_image = os.path.expandvars("${{OUT}}/apex/com.android.art.debug/javalib/boot.art:${{OUT}}/system/framework/boot-framework.art")
  ...
  if args.odex_file is not None:
    ...
    extra.append('--output-vdex-fd={}'.format(os.memfd_create("vdex_fd", flags=0)))
  pre_args = []
  if args.debug:
