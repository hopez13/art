...
def get_profile_args(args, location_base):
  ...
  for f in args.dex_files:
    ...
    profman_args.append("--dex-location={}/{}".format(location_base,
                                                      os.path.basename(f)))
...
def main():
  ...
  if args.arch == "host32" or args.arch == "host64":
    ...
    android_root = "."
    for f in args.dex_files:
      extra.append("--dex-location={}".format("{}/{}".format(
          location_base, os.path.basename(f))))
      extra.append("--dex-file={}".format(f))
  else:
    location_base = "/system/framework"
    real_arch = args.arch
    boot_image = os.path.expandvars(
        "${{OUT}}/apex/com.android.art.debug/javalib/boot.art:${{OUT}}/system/framework/boot-framework.art"
    )
