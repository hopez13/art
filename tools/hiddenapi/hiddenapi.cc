...
class HiddenApi final {
 ...
 private:
  ...
  const std::string_view api_to_string(ApiStubsKind api) {
    switch (api) {
  ...
  std::map<std::string, ApiList> OpenApiFile(const std::string& path) {
    ...
    for (std::string line; std::getline(api_file, line); line_number++) {
      ...
      values.erase(std::remove_if(values.begin() + 1, values.end(),
        [&](const std::string& str) {
          return kIgnoredApiFlags.find(str) != kIgnoredApiFlags.end();
        }), values.end());
  ...
  void ListApi() {
    ...
    for (const auto& cp_entry : stub_classpaths_) {
      ...
      const ApiStubsKind stub_api = cp_entry.second;
          ...
          [&](const DexMember& stub_member) {
            ...
            bool resolved = boot_hierarchy.ForEachResolvableMember(
                ...
                [&](const DexMember& boot_member) {
                  ...
                  it->second.insert(api_to_string(stub_api));
                });
            if (!resolved) {
  ...
  std::vector<std::pair<std::string, ApiStubsKind>> stub_classpaths_;
...
const std::set<std::string_view> HiddenApi::kIgnoredApiFlags{
    kPublicApiStr,
    kSystemApiStr,
    kTestApiStr
  };
