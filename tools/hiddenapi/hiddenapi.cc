...
class HierarchyClass FINAL {
 public:
  ...
  // Returns true if at least one matching member matching `other` was found in this class.
  ...
  // Recursively iterates over all subclasses of this class and invoked `fn`
...
class HiddenApi FINAL {
 ...
 private:
      ...
      if (command == "encode") {
        ...
        for (int i = 1; i < argc; ++i) {
          ...
          if (option.starts_with("--boot-dex=")) {
            ...
            stub_dex_paths_.push_back(android::base::Split(
  ...
  void ListApi() {
    ...
    // Mark all boot dex members private. Skip <clinit> methods as they cannot
    // be discovered invoked directly anyway.
    ...
    for (const std::vector<std::string>& stub_classpath_dex : stub_dex_paths_) {
      ClassPath stub_class_path(stub_classpath_dex, /* open_writable */ false);
      Hierarchy stub_hierarchy(stub_class_path);
  ...
  std::vector<std::vector<std::string>> stub_dex_paths_;
