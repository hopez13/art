...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("");
  UsageError("    --max-hiddenapi-level=<max-target-o|max-target-p|max-target-q|max-target-r>:");
      ...
      if (command == "encode") {
        for (int i = 1; i < argc; ++i) {
          ...
          if (StartsWith(option, "--input-dex=")) {
            ...
            max_hidden_api_target_sdk_ = std::string(option.substr(strlen("--max-hiddenapi-level=")));
          } else {
