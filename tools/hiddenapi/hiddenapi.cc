...
NO_RETURN static void Usage(const char* fmt, ...) {
  ...
  UsageError("Usage: hiddenapi [list|encode] [options]...");
  UsageError("");
...
class ReadWriteClassPath {
 public:
  ReadWriteClassPath(const std::vector<std::string>& dex_paths, bool write_to_disk) {
    OpenDexFiles(dex_paths, write_to_disk);
  }
 ...
 private:
  void OpenDexFiles(const std::vector<std::string>& dex_paths, bool write_to_disk) {
...
class HiddenApi FINAL {
 ...
 private:
  ...
  void EncodeAccessFlags() {
    ...
    // Open all dex files.
    ReadWriteClassPath boot_class_path(boot_dex_paths_, /* write_to_disk */ true);
