...
class DexClass {
 public:
  ...
  inline bool operator==(const DexClass& other) const {
...
class HierarchyClass FINAL {
 public:
  ...
  void AddDexClass(const DexClass& klass) {
    ...
    CHECK(dex_classes_.front() == klass);
  }

  void AddExtends(HierarchyClass& other) {
    CHECK(!Contains(extends_, &other));
    CHECK(!Contains(other.extended_by_, this));
    extends_.push_back(&other);
    other.extended_by_.push_back(this);
  }
 ...
 private:
  ...
  // All dex entries representing this class.
