...
TEST_F(HiddenApiTest, InstanceFieldCorePlatformApiMatch) {
  ...
  auto dex_file = RunHiddenapiEncode(flags_csv, {}, dex);
  ASSERT_NE(dex_file.get(), nullptr);
  ASSERT_EQ(hiddenapi::ApiList::CorePlatformApi() | hiddenapi::ApiList::Greylist(), GetIFieldHiddenFlags(*dex_file));
}
...
TEST_F(HiddenApiTest, InstanceFieldTestApiMatch) {
  ...
  auto dex_file = RunHiddenapiEncode(flags_csv, {}, dex);
  ASSERT_NE(dex_file.get(), nullptr);
  ASSERT_EQ(hiddenapi::ApiList::TestApi() | hiddenapi::ApiList::Greylist(), GetIFieldHiddenFlags(*dex_file));
}
