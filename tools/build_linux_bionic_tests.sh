...
tmp_soong_config=$(mktemp --tmpdir soong.config.bak.XXXXXX)

echo "Saving soong.config to " $tmp_soong_config
cat $out_dir/soong/soong.config > ${tmp_soong_config}
python3 <<END - ${tmp_soong_config} ${out_dir}/soong/soong.config
import json
import sys
...
mv $tmp_soong_config $out_dir/soong/soong.config

# Having built with Host_bionic confuses soong somewhat by making it think the
