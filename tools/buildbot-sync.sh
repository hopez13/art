...
if [[ -n "$ART_TEST_VM" ]]; then
  if [[ -z "$SSH_CMD" ]]; then
    ...
    msgerror "ART_TEST_SSH_USER not set"
    exit 1
  elif [[ -z "$ART_TEST_SSH_HOST" ]]; then
    msgerror "ART_TEST_SSH_HOST not set"
    exit 1
  elif [[ -z "$RSYNC_CMD" ]]; then
    msgerror "RSYNC_CMD not set"
    exit 1
...
(
  ...
  for dir in system/* linkerconfig data; do
    [ -d $dir ] || continue
    [ $dir = system/system_ext ] && continue
    ...
    if [[ -n "$ART_TEST_VM" ]]; then
      $RSYNC_CMD -R $dir $ART_TEST_SSH_USER@$ART_TEST_SSH_HOST:$ART_TEST_CHROOT
    else
...
if [[ -n "$ART_TEST_VM" ]]; then
  $RSYNC_CMD "$ANDROID_BUILD_TOP/art/tools/public.libraries.buildbot.txt" \
...
if [[ -n "$ART_TEST_VM" ]]; then
  $SSH_CMD "$CHROOT_CMD mkdir -p $ART_TEST_CHROO_VMT/system/framework"
else
...
activate_apex() {
  ...
  if [[ -n "$ART_TEST_VM" ]]; then
    $RSYNC_CMD $src_apex_path/* "$ART_TEST_SSH_USER@$ART_TEST_SSH_HOST:$ART_TEST_CHROOT/apex/${dst_apex}"
  else
...
for b in {32,64}; do
  ...
  if [ -f $bin_on_host ]; then
    ...
    if [[ -n "$ART_TEST_VM" ]]; then
      $RSYNC_CMD "$bin_on_host" "$ART_TEST_SSH_USER@$ART_TEST_SSH_HOST:$ART_TEST_CHROOT$bin_on_device"
      $SSH_CMD "mkdir -p $ART_TEST_CHROOT$output_dir"
    else
    ...
    # the boot image.
    if [[ -n "$ART_TEST_VM" ]]; then
      $SSH_CMD "$CHROOT_CMD $bin_on_device --output-dir=$output_dir --compiler-filter=speed-profile"
    else
