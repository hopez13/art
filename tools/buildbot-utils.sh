...
if [[ -n "$ART_TEST_ON_VM" ]]; then
  ...
  export ART_SSH_CMD="ssh -q -i ~/.ssh/ubuntu -p $ART_TEST_SSH_PORT $ART_TEST_SSH_USER@$ART_TEST_SSH_HOST"
  export ART_SCP_CMD="scp -i ~/.ssh/ubuntu -P $ART_TEST_SSH_PORT -p -r"
  export ART_RSYNC_CMD="rsync -az"
  export RSYNC_RSH="ssh -i ~/.ssh/ubuntu -p $ART_TEST_SSH_PORT" # don't prefix with "ART_", rsync expects this name
