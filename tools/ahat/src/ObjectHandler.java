...
class ObjectHandler implements AhatHandler {
  ...
  public void handle(Doc doc, Query query) throws IOException {
    ...
    if (inst.isClassInstance()) {
      printClassInstanceFields(doc, query, mSnapshot, inst.asClassInstance());
    } else if (inst.isArrayInstance()) {
      printArrayElements(doc, query, mSnapshot, inst.asArrayInstance());
    } else if (inst.isClassObj()) {
      printClassInfo(doc, query, mSnapshot, inst.asClassObj());
    }
  ...
  private static void printClassInstanceFields(
      Doc doc, Query query, AhatSnapshot snapshot, AhatClassInstance inst) {
    doc.section("Fields");
  ...
  private static void printArrayElements(
      Doc doc, Query query, AhatSnapshot snapshot, AhatArrayInstance array) {
    doc.section("Array Elements");
    doc.table(new Column("Index", Column.Align.RIGHT), new Column("Value"));
    List<Value> elements = Arrays.asList(array.getValues());
  ...
  private static void printClassInfo(
      Doc doc, Query query, AhatSnapshot snapshot, AhatClassObj clsobj) {
  ...
  private static void printReferences(
      Doc doc, Query query, AhatSnapshot snapshot, AhatInstance inst) {
