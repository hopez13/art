  ...
  private static void printFields(Doc doc, Query query, String id, boolean diff,
    ...
    for (DiffedFieldValue field : selector.selected()) {
      ...
      if (field.deleted) {
        ...
        value = Summarizer.summarize(field.current);
      }
      ...
      if (field.added) {
        ...
        if (!Objects.equals(field.current, previous)) {
          delta.append("was ");
          delta.append(Summarizer.summarize(previous));
        }
      }
      doc.row(DocString.text(field.type), DocString.text(field.name), value, delta);
    }
    doc.end();
    selector.render(doc);
  }
