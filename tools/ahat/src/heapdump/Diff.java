...
public class Diff {
  ...
  private static class Key {
    ...
    // Corresponding bjects must belong to heaps of the same name.
  ...
  private static void instances(List<AhatInstance> a, List<AhatInstance> b,
    ...
    while (!deque.isEmpty()) {
      ...
      for (InstanceListPair pair : byKey.values()) {
        ...
        // correspond to each other in that order.
        Collections.sort(pair.a, new Sort.InstanceByTotalRetainedSize());
        Collections.sort(pair.b, new Sort.InstanceByTotalRetainedSize());
        ...
        for (int i = common; i < pair.a.size(); i++) {
          b.add(createPlaceHolders(pair.a.get(i), placeholders));
        }

        for (int i = common; i < pair.b.size(); i++) {
          a.add(createPlaceHolders(pair.b.get(i), placeholders));
        }
      }
    }
  }
  ...
  private static void sites(Site a, Site b) {
    ...
    for (Site achild : a.getChildren()) {
      ...
      if (achild.getBaseline() == null) {
        // We did not fin any match for achild in site B.
  ...
  /**
   * Class representing a diff between FieldValues in two different snapshots,
   * with respect to one of those snapshots.
   * Cases:
   *  * current is null to indicate the field has been removed.
   *  * previous is null to indicate the field has been added.
   *  * current and previous are == to indicate the field is unchanged.
   *  * otherwise the field has been modified, and previous.getValue returns
   *    the value of the field already mapped to the current snapshot.
   */
  public static class DiffedFieldValue {
    public String name;
    public String type;
