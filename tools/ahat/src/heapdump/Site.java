...
public class Site implements Diffable<Site> {
  ...
  private static Site add(Site site, StackFrame[] frames, int depth, AhatInstance inst) {
    while (true) {
      ...
      if (inst.isReachable()) {
        ...
        site.mSizesByHeap[heap.getIndex()].add(inst.getSize());

        info.numInstances++;
        info.numBytes.add(inst.getSize());
      }
  ...
  public Size getSize(AhatHeap heap) {
    ...
    return index >= 0 && index < mSizesByHeap.length ? mSizesByHeap[index] : new  Size();
  }
