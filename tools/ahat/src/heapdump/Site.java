...
public class Site implements Diffable<Site> {
  ...
  private static Site add(Site site, StackFrame[] frames, int depth, AhatInstance inst) {
    while (true) {
      ...
      if (inst.isReachable()) {
        ...
        site.mSizesByHeap[heap.getIndex()] = site.mSizesByHeap[heap.getIndex()].add(inst.getSize());

        info.numInstances++;
        info.numBytes = info.numBytes.add(inst.getSize());
      }
  ...
  public Size getTotalSize() {
    ...
    for (int i = 0; i < mSizesByHeap.length; i++) {
      total = total.add(mSizesByHeap[i]);
    }
    return total;
  }
