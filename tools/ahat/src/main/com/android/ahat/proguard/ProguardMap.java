...
public class ProguardMap {
  ...
  private static class FrameData {
    ...
    public int getClearLine(int obfuscatedLine) {
      Map.Entry<Integer, LineNumberMapping> lineNumberEntry = lineNumbers.floorEntry(obfuscatedLine);
  ...
  private static class LineNumber {
    public LineNumber(int start, int end) {
      this.start = start;
      this.end = end;
    }
  ...
  private static class LineNumberMapping {
    public LineNumberMapping(LineNumber obfuscatedLine, LineNumber clearLine) {
      this.obfuscatedLine = obfuscatedLine;
      this.clearLine = clearLine;
    }

    public boolean hasObfuscatedLine(int lineNumber) {
        return obfuscatedLine.hasLine(lineNumber);
    }

    public int mapObfuscatedLine(int lineNumber) {
        int mappedLine = clearLine.start + lineNumber - obfuscatedLine.start;
        if (!clearLine.hasLine(mappedLine)) {
            return clearLine.end;
        }
        return mappedLine;
    }

    public final LineNumber obfuscatedLine;
    public final LineNumber clearLine;
  }
  ...
  private static class ClassData {
    ...
    public void addFrame(String obfuscatedMethodName, String clearMethodName,
            String clearSignature, LineNumber obfuscatedLine, LineNumber clearLine) {
        ...
        data.lineNumbers.put(
            obfuscatedLine.start, new LineNumberMapping(obfuscatedLine, clearLine));
    ...
    while (line != null) {
      ...
      while (line != null && (line.startsWith("    ") || isCommentLine(line))) {
        ...
        if (clearName.indexOf('(') == -1) {
          ...
          LineNumber obfuscatedLine = new LineNumber(obfuscatedLineStart, obfuscatedLineEnd);
          ...
          int clearLineStart = obfuscatedLine.start;
          int clearLineEnd = obfuscatedLine.end;
          ...
          if (colon != -1) {
            if (compilerVersion.compareTo(LINE_MAPPING_BEHAVIOR_CHANGE_VERSION) < 0) {
              ...
              clearLineEnd = clearLineStart + obfuscatedLine.end - obfuscatedLine.start;
            } else {
          ...
          LineNumber clearLine = new LineNumber(clearLineStart, clearLineEnd);
