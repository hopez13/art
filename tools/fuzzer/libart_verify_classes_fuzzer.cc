...
// Global variable to signal LSAN that we are not leaking by using sigaltstack.
...
std::string GetDexFileName(const std::string& jar_name, bool host) {
  std::string prefix_device = android::base::StringPrintf(
      "/data/fuzz/%s",
      GetInstructionSetString(art::kRuntimeISA));
  std::string prefix(host ? art::GetAndroidRoot() : prefix_device);
  std::string result = android::base::StringPrintf(
      "%s/libart_verify_classes_fuzzer/data/%s.jar",
      prefix.c_str(), jar_name.c_str());
  return result;
}
