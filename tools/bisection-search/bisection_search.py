...
def PrepareParser():
  ...
  command_opts.add_argument('classname', nargs='?', type=str,
  ...
  command_opts.add_argument('--64', dest='x64', action='store_true',
...
def main():
  ...
  if not args.raw_cmd and (not args.classpath or not args.classname):
    parser.error('Either --raw-cmd or both -cp and classname are required')
