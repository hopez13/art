...
def GetAndroidTop():
  """Return path to android top from environmental variable ANDROID_BUILD_TOP.

  Returns:
    string, value of ANDROID_BUILD_TOP environmental variable.

  Raises:
    FatalError: ANDROID_BUILD_TOP not set.
  """
  top = os.environ.get('ANDROID_BUILD_TOP')
  if top is None:
    raise FatalError('ANDROID_BUILD_TOP environmental variable not set.')
  return top
...
def _RunCommandForOutputAndLog(cmd, env, logfile, timeout=60):
  ...
  return (output.decode("utf-8"), err_output.decode("utf-8"), ret_code)
