...
void update_to_unprotected(volatile uint64_t *page, size_t idx) {
  size_t k = 0, i = idx;
  do {
    i = find_next_set_bit(i);
    if (i - idx >= from_wc) {
...
void segfault_hdl(int sig, siginfo_t *info, void *ctxt) {
  uint64_t* addr = (uint64_t*)((unsigned long) info->si_addr & ~(page_sz - 1));
  ptrdiff_t diff = addr - to;
  update_to_unprotected(to_unprotected + diff, diff / page_wc * from_wc);
...
void sigbus_hdl(int sig, siginfo_t *info, void *ctxt) {
  ...
  int cmd;
  if (choice == 1) {
    update_to_unprotected(to_unprotected, diff / page_wc * from_wc);
    uffdio_copy.src = (unsigned long) to_unprotected;
    cmd = UFFDIO_COPY;
  } else {
    update_to_unprotected(from + diff, diff / page_wc * from_wc);
...
#if 0
  if (mremap((void*)to_unprotected, page_sz, page_sz,
             MREMAP_MAYMOVE | MREMAP_FIXED, (void*)to) == MAP_FAILED) {
    errExit("mremap");
  }
#endif
}
...
int main(int argc, char** argv) {
  ...
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << " <#from-pages/to-page> <#to-pages> <#iters> [1 for UFFD_COPY/2 for UFFD_REMAP]\n";
    exit(EXIT_FAILURE);
  }
  ...
  if (choice == 0) {
    ...
    to = (volatile uint64_t*)mmap(NULL, to_pg_count * page_sz, PROT_READ | PROT_WRITE,
                                  MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    to_unprotected = (volatile uint64_t*)mmap(NULL, page_sz,
  ...
  for (size_t i = 0; i < iters; i++) {
    ...
    if (choice != 0) {
      ...
      if (from_pg_count > 1) {
        for (size_t j = 0; j < page_wc * to_pg_count; j += page_wc) {
          update_to_unprotected(from + j, j / page_wc * from_wc);
        }
      }
      struct uffdio_copy uffdio_copy;
    ...
    //cacheflush(reinterpret_cast<uint8_t*>(const_cast<uint64_t*>(to_unprotected)), page_sz);
    std::fill(bset.begin(), bset.end(), 0);
    ...
    if (choice == 0) {
      ...
      madvise((void*)to, to_pg_count * page_sz, MADV_DONTNEED);
#if 0
      to_unprotected = (volatile uint64_t*)mmap((void*)to_unprotected, page_sz,
                                                PROT_READ | PROT_WRITE,
