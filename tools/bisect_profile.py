...
def get_parser():
  ...
  apks.add_argument(
      ...
      nargs=1,
      default=[],
      help="an apk/dex/jar to get methods from. Uses same path as location. Use --apk-and-location if this isn't desired."
  )
  apks.add_argument(
      "--apk-and-location",
      action=ApkAction,
...
def main():
  ...
  print(
      "type 'yes' if the behavior you are looking for is present (i.e. the compiled code crashes or something)"
  )
  print("Performing single sanity check with all methods")
  ...
  while len(meth_list) > 1:
    ...
    if result[0].lower() == "y":
      ...
      needs_dump = True
  if needs_dump:
    with open(args.output_file, "wt") as output:
      dump_files(meth_list, args, open(output))
  print("Found result!")
