       ...
       public:
        bool operator()(art::ObjPtr<art::mirror::Class> klass)
          ...
          for (art::ArtMethod& method : klass->GetMethods(art::kRuntimePointerSize)) {
            const void* old_target = method.GetEntryPointFromQuickCompiledCode();
            const void* target = art::Runtime::Current()->GetTraceCompiler()->GetTrampolineTo(
                reinterpret_cast<uintptr_t>(old_target));
            method.SetEntryPointFromQuickCompiledCode(target);
            LOG(INFO) << "setting trampoline " << target << "->" << old_target << " on "
                      << method.PrettyMethod();
          }
          return true;
        }
