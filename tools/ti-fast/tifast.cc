...
static jint AgentStart(JavaVM* vm,
  ...
  std::vector<jvmtiEvent> events = impl::GetRequestedEventList(args);

  jvmtiCapabilities caps{};
  for (jvmtiEvent e : events) {
    impl::AddCapsForEvent(e, &caps);
  }
  ...
  if (is_log) {
    error = jvmti->SetEventCallbacks(&impl::kLogCallbacks,
                                     static_cast<jint>(sizeof(impl::kLogCallbacks)));
  } else {
    error = jvmti->SetEventCallbacks(&impl::kEmptyCallbacks,
                                     static_cast<jint>(sizeof(impl::kEmptyCallbacks)));
  }
  if (error != JVMTI_ERROR_NONE) {
