...
static std::vector<jvmtiEvent> GetAllAvailibleEvents(jvmtiEnv* jvmti) {
  union caps_bin {
  ...
  caps_bin avail{};
  jvmti->GetPotentialCapabilities(&avail.caps);
  for (jvmtiEvent e : kAllEvents) {
    caps_bin req{};
    AddCapsForEvent(e, &req.caps);
    bool good = true;
    for (uint32_t i = 0; i < sizeof(avail.bytes); i++) {
      if ((req.bytes[i] & avail.bytes[i]) != req.bytes[i]) {
        good = false;
        break;
      }
    }
...
static std::vector<jvmtiEvent> GetRequestedEventList(jvmtiEnv* jvmti, const std::string& args) {
  ...
  while (std::getline(args_stream, item, ',')) {
    if (item == "") {
      ...
      return GetAllAvailibleEvents(jvmti);
    }
