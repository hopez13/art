...
class TestRunnerRIOnHost(TestRunner):
  ...
  def CompileAndRunTest(self):
    if RunCommand('javac', 'Test.java', None, None, timeout=30) == EXIT_SUCCESS:
...
class TestRunnerArtOnHost(TestRunner):
  ...
  def  __init__(self, interpreter):
    ...
    if interpreter:
      ...
      self._id = 'HInt'
      self._art_args = '-Xint ' + self._arg_args
    else:
  ...
  def CompileAndRunTest(self):
    if RunCommand('jack', self._jack_args,
                  None, 'jackerr.txt', timeout=30) == EXIT_SUCCESS:
  ...
  def CompileAndRunTest(self):
    if RunCommand('jack', self._jack_args,
      ...
      if RunCommand('adb push', 'classes.dex /data/local/tmp/',
                    'adb.txt', None) != EXIT_SUCCESS:
        raise FatalError('Cannot push to target device')
      out = self.GetId() + '_run_out.txt'
      retc = RunCommand('adb shell dalvik', self._dalvik_args, out, None)
      if retc != EXIT_SUCCESS and retc != EXIT_TIMEOUT:
        retc = EXIT_NOTRUN
    else:
      retc = EXIT_NOTCOMPILED
    # Cleanup and return.
    RunCommand('rm', '-f classes.dex jackerr.txt adb.txt', None, None)
    RunCommand('adb shell', 'rm -f /data/local/tmp/classes.dex', None, None)
    return retc
