...
def RunCommand(cmd, args, out, err):
  ...
  timeout = Timer(5, proc.kill)  # with timeout
  timeout.start()
  proc.communicate()
  if timeout.is_alive():
    timeout.cancel()
...
class TestRunnerRIOnHost(TestRunner):
  ...
  def CompileAndRunTest(self):
    if RunCommand('javac', 'Test.java', None, None) == EXIT_SUCCESS:
...
class TestRunnerArtOnHost(TestRunner):
  ...
  def  __init__(self, interpreter):
    ...
    if interpreter:
      self._art_args = '-cp classes.dex -Xint Test'
    else:
    ...
    classpath = libdir + '/core-libart-hostdex_intermediates/classes.jack:' \
              + libdir + '/core-oj-hostdex_intermediates/classes.jack'
    self._jack_args = '-cp ' + classpath + ' --output-dex . Test.java'

  def GetDescription(self):
    if self._interpreter:
      return 'Art on host (interpreter)'
    return 'Art on host (optimizing)'
  ...
  def CompileAndRunTest(self):
    if RunCommand('jack', self._jack_args, None, 'jackerr.txt') == EXIT_SUCCESS:
      ...
      retc = EXIT_NOTCOMPILED
    RunCommand('rm', '-f classes.dex jackerr.txt arterr.txt', None, None)
    return retc
...
class JavaFuzzTester(object):
  ...
  def  __init__(self, ri):
    ...
    Args:
    ri: boolean, when set, compare again RI
    """
    self._save_dir = None
    self._tmp_dir = None
    # Statistics.
    ...
    # Execution modes.
    if ri:
      self._runner1 = TestRunnerRIOnHost()
    else:
      self._runner1 = TestRunnerArtOnHost(True)
    self._runner2 = TestRunnerArtOnHost(False)
  ...
  def Run(self):
    ...
    print
    self.ShowStats()
    for self._test in range(1, 1000000):
...
def main():
  ...
  parser.add_argument('--target', action='store_true',
                      default=False, help='run on target')
  parser.add_argument('--ri', action='store_true',
                      default=False, help='compare against RI')
  args = parser.parse_args()
  if args.target:
    raise FatalError('target execution mode not supported yet')
  # Run the JavaFuzz tester.
  with JavaFuzzTester(args.ri) as fuzzer:
    fuzzer.Run()
