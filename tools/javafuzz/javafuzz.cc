...
static inline uint32_t random0(uint32_t max) {
  return random() % max;
}

/* Returns random integer in range [1,max]. */
static inline uint32_t random1(uint32_t max) {
  return 1 + random0(max);
}
...
class JavaFuzz {
 ...
 private:
  ...
  void emitUnaryOp(Type tp) const {
    if (tp == kBoolean) {
      fputs("!", out_);
    } else if (isInteger(tp)) {
      switch (random1(3)) {
        case 1: fputs("+", out_); break;
        case 2: fputs("-", out_); break;
        case 3: fputs("~", out_); break;
      }
    } else {  // isFP(tp)
      switch (random1(2)) {
        case 1: fputs("+", out_); break;
  ...
  void emitBinaryOp(Type tp) const {
    if (tp == kBoolean) {
      switch (random1(5)) {
        case 1: fputs(" && ", out_); break;
        case 2: fputs(" || ", out_); break;
        // Less common, but also allowed.
        case 3: fputs(" & ", out_); break;
        case 4: fputs(" | ", out_); break;
        case 5: fputs(" ^ ", out_); break;
      }
    } else if (isInteger(tp)) {
      switch (random1(11)) {
        case 1:  fputs(" + ",   out_); break;
        ...
        case 4:  fputs(" / ",   out_); break;
        case 5:  fputs(" % ",   out_); break;
        case 6:  fputs(" >> ",  out_); break;
        case 7:  fputs(" >>> ", out_); break;
        case 8:  fputs(" << ",  out_); break;
        case 9:  fputs(" & ",   out_); break;
        case 10: fputs(" | ",   out_); break;
        case 11: fputs(" ^ ",   out_); break;
      }
    } else {  // isFP(tp)
      switch (random1(4)) {
        case 1: fputs(" + ",   out_); break;
  ...
  void emitAssignmentOp(Type tp) const {
    if (tp == kBoolean) {
      switch (random1(4)) {
        case 1: fputs(" = ", out_); break;
        // Less common, but also allowed.
  ...
  void emitRelationalOp(Type tp) const {
    if (tp == kBoolean) {
      switch (random1(2)) {
        case 1: fputs(" == ", out_); break;
        case 2: fputs(" != ", out_); break;
  ...
  Type emitTypeConv(Type tp) const {
    if (tp == kInt) {
      switch (random1(5)) {
  ...
  Type emitIntrinsic1(Type tp) const {
    if (tp == kBoolean) {
      switch (random1(4)) {
  ...
  Type emitIntrinsic2(Type tp) const {
    if (tp == kBoolean) {
      switch (random1(3)) {
      ...
      switch (random1(3)) {
        ...
        case 3: fputs("Math.copySign", out_); break;
      }
    }
    return tp;  // same type in-out
  }
  ...
  // Emit array index. Most of the time within proper range, but potential
  // out-of-bounds indexing is introduced with low probability.
  void emitIndex() {
    ...
    // Introduce potential off by one errors.
  }
  ...
  void emitExpression(Type tp) {
    ...
    switch (random1(12)) {  // favor binary operations
      ...
      case 5:
        ...
        break;
      case 6:
        // Intrinsic with one argument: foo(x)
        ...
        break;
      case 7:
        // Intrinsic with two arguments: bar(x, y)
