  ...
  def FromSymbol(cls, s):
    for log_severity in LogSeverity:
      if log_severity.symbol == s:
        return log_severity
    raise ValueError("{0} is not a valid symbol".format(s))
...
class DeviceTestEnv(ITestEnv):
  ...
  def RunCommand(self, cmd, log_severity=LogSeverity.ERROR):
    ...
    # There is a race condition here, we hope that all relevant logcat logs will
    # reach us before logcat process gets killed. If this optimistic assumption
    # causes issues we should rewrite this method.
    logcat_proc.kill()
