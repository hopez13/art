...
class DeviceTestEnv(ITestEnv):
  ...
  def RunCommand(self, cmd, log_severity=LogSeverity.ERROR):
    ...
    # TODO: Make sure to only kill logcat once all relevant logs arrive.
    # Depending on logcat guarantees could be done using a short sleep or
    # by reading logcat output in real time until we get a termination message
    # from runtime and/or every dex2oat instance.
    logcat_proc.kill()
