...
while true; do
  if [[ $1 == "--debug" ]]; then
    ...
    mode="host"
    shift
  elif [[ $1 == --verbose-file ]]; then
    # remove both the --verbose-file and the argument.
    shift
    verbose_file=$1
    unset args[arg_idx]
    arg_idx=$((arg_idx + 1))
...
if [[ "$has_verbose" = "yes" ]]; then
  args+=(--vm-arg)
  args+=(-Djpda.settings.debuggeeAgentExtraOptions=directlog=y,logfile=$verbose_file,logflags=$verbose_level)
fi
