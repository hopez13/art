...
def display_html(total, node):
    ...
    label = "%d %6.2f%% %6d %s %s %s" % (node.size, 100*node.size/float(total), node.number, lib, fd.function, fd.location)
    label = label.replace("&", "&amp;")
    label = label.replace("'", "&apos;")
    label = label.replace('"', "&quot;")
    label = label.replace("<", "&lt;")
...
if html_output:
    print html_header
    display_html(app.size, app)
    if zygote.size>0:
        display_html(zygote.size, zygote)
    print html_footer
else:
    print ""
