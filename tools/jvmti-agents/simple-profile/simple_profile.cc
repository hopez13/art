...
class SimpleProfileData {
 public:
  SimpleProfileData(jvmtiEnv* env, std::string out_fd_name, int fd, bool dump_on_shutdown, bool dump_on_main_stop)
      : dump_id_(0), out_fd_name_(out_fd_name), out_fd_(fd), shutdown_(false), dump_on_shutdown_(dump_on_shutdown || dump_on_main_stop), dump_on_main_stop_(dump_on_main_stop) {
...
void SimpleProfileData::DoDump(jvmtiEnv* jvmti, JNIEnv* jni, std::unordered_map<jmethodID, uint64_t> copy) {
  ...
  oss << "]";
  CHECK_GE(TEMP_FAILURE_RETRY(write(out_fd_, oss.str().c_str(), oss.str().size())), 0) << strerror(errno) << out_fd_ << " " << out_fd_name_;
  fsync(out_fd_);
}
...
static bool ParseArgs(const std::string& start_options, /*out*/ std::string* fd_name, /*out*/ int* fd, /*out*/ bool* dump_on_shutdown, /*out*/ bool* dump_on_main_stop) {
...
static jint AgentStart(StartType start,
  ...
  if (!ParseArgs(options, /*out*/ &fd_name, /*out*/ &fd, /*out*/&dump_on_shutdown, /*out*/&dump_on_main_stop)) {
  ...
  SimpleProfileData* data = new (data_mem) SimpleProfileData(jvmti, fd_name, fd, dump_on_shutdown, dump_on_main_stop);
  ...
  callbacks.ThreadEnd = [](jvmtiEnv* env, JNIEnv* jni, jthread thr ATTRIBUTE_UNUSED) { VMDeathCB(env, jni); };
