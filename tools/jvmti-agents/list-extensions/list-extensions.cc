...
static jint SetupJvmtiEnv(JavaVM* vm) {
  jint res = 0;
  ...
  for (jint i = 0; i < n_ext; i++) {
    ...
    Dealloc(env, info.short_description, info.id, info.errors);
  ...
  for (jint i = 0; i < n_ext; i++) {
    ...
    Dealloc(env, info.short_description, info.id);
    DeallocParams(env, info.params, info.param_count);
  }
  // Cleanup the array.
  Dealloc(env, events);
...
static jint
AgentStart(JavaVM* vm, char* options ATTRIBUTE_UNUSED, void* reserved ATTRIBUTE_UNUSED) {
