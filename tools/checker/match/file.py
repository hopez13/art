...
class IfStack:
  ...
  - BranchNotTakenYet: the branch's parent was taken, but this wasn't as its
  ...
  That means that we can check if the line currently pointed by the cursor should be
  processed just by looking at the top of the stack.
  CHECK-FI will pop the last element.

  BranchTaken and BranchNotTaken could be negative values. Their meaning doesn't change, but it
  indicates that we went past the ELSE branch. Knowing that, we can output a precise error message
  if the user creates a malformed branching structure.
  """
