...
def _extract_line(prefix, line, arch=None, debuggable=False):
  ...
  r_comment_symbols = [r'///', r"##"]
  arch_specifier = r'-{}'.format(arch) if arch is not None else r""
  dbg_specifier = r'-DEBUGGABLE' if debuggable else r""
  regex_prefix = (r_ignore_whitespace +
                  r'(' + r'|'.join(r_comment_symbols) + r')'
                  + r_ignore_whitespace + prefix + arch_specifier + dbg_specifier + r':')
...
def _processLine(line, line_no, prefix, filename, target_arch):
...
def parse_checker_statement(parent, line, variant, line_no):
  ...
  if statement.is_no_content_statement() and line:
    Logger.fail("Expected empty statement: '" + line + "'", statement.filename,
                statement.line_no)
...
def parse_checker_stream(file_name, prefix, stream, target_arch=None):
  ...
  def fn_process_line(line, line_no):
    return _processLine(line, line_no, prefix, file_name, target_arch)

  def fn_line_outside_chunk(line, line_no):
    Logger.fail('Checker line not inside a group', file_name, line_no)

  for case_name, case_lines, start_line_no, testData in split_stream(stream, fn_process_line,
                                                                     fn_line_outside_chunk):
    test_arch = testData[0]
    for_debuggable = testData[1]
