        ...
        switch (inst->Opcode()) {
          ...
          case Instruction::CONST_STRING: {
            ...
            if (strchr(name.c_str(), ' ') == nullptr) {
              if (hidden_api_.IsInRestrictionList(name)) {
                // If the string is already in a restricted list, it might be a class name.
                classes_.insert(name);
              }
...
std::ostream& PrintUses(std::ostream& os,
                        const std::vector<MethodReference>& uses,
                        const std::string& extra = "") {
  os << uses.size() << " " << extra << (uses.size() == 1 ? "use:" : "uses:");
  return os;
}
...
void HiddenApiFinder::Dump(std::ostream& os) {
  ...
  for (const std::pair<std::string, std::vector<MethodReference>>& pair : method_locations_) {
    ApiList api_list = hidden_api_.GetApiList(pair.first);
    api_counts[api_list]++;
    os << "#" << ++count << ": Linking " << api_list << " " << pair.first << " ";
    PrintUses(os, pair.second);
    os << std::endl;
  ...
  for (const std::string& cls : classes_) {
    for (const std::string& name : strings_) {
      ...
      ApiList api_list = hidden_api_.GetApiList(full_name);
      api_counts[api_list]++;
      if (api_list != ApiList::kWhitelist) {
        os << "#" << ++count << ": Reflection " << api_list << " " << full_name << " ";
        PrintUses(os, reflection_locations_[name], "potential ");
        os << std::endl;
  ...
  os << kPrefix << api_counts[ApiList::kBlacklist] << " in blacklist" << std::endl;
  os << kPrefix << api_counts[ApiList::kDarkGreylist] << " in dark greylist" << std::endl;
  os << kPrefix << api_counts[ApiList::kLightGreylist] << " in light greylist" << std::endl;
}
