...
if [[ $mode == "target" ]]; then
  ...
  linkerconfig_root=$ANDROID_PRODUCT_OUT/linkerconfig_root

  rm -rf $linkerconfig_root

  # Linkerconfig reads following two files from /system/etc
  mkdir -p $linkerconfig_root/system/etc
  cp $ANDROID_PRODUCT_OUT/system/etc/sanitizer.libraries.txt $linkerconfig_root/system/etc
  cp $ANDROID_PRODUCT_OUT/system/etc/vndkcorevariant.libraries.txt $linkerconfig_root/system/etc
