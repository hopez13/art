...
if [[ $mode == "target" ]]; then
  ...
  echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>" > $apex_xml_file
  echo "<apex-info-list>" >> $apex_xml_file
  for apex in ${apexes[@]}; do
    if [[ $apex == com.android.art.* ]]; then
      apex=com.android.art
    fi
    echo "    <apex-info moduleName=\"${apex}\" modulePath=\"/system/apex/${apex}.apex\" preinstalledModulePath=\"/system/apex/${apex}.apex\" versionCode=\"1\" versionName=\"\" isFactory=\"true\" isActive=\"true\">" >> $apex_xml_file
    echo "    </apex-info>" >> $apex_xml_file
  done
  echo "</apex-info-list>" >> $apex_xml_file
