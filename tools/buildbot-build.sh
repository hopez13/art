...
if [[ $mode == "host" ]]; then
  ...
  # Fail fast for public_libraries.txt target.
  if [ ! -z "$ART_TEST_ANDROID_ROOT" ]; then
...
if [ $make_status -ne 0 -a $mode == "target" -a ! -z "$ART_TEST_ANDROID_ROOT" -a ! -f "$public_libraries" ]; then
  # A typical target build will use the master-art manifest with the
  # armv8-eng lunch combo. The public libraries file has a valid make
  # target in master-art for lunch combo's that support running target
  # tests.
  echo "Building for a device target requires the master-art manifest with a supported lunch combo." >&2
fi
exit $make_status
