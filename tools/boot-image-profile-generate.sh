...
if [[ "$#" -lt 5 ]]; then
  echo "Usage $0 <output-dir> <boot.zip-location> <preloaded-denylist-location> <preloaded-classes-output> <profile-input1> <profile-input2> ... <profman args>"
  echo "Without any profman args the script will use defaults."
  echo "Example: $0 output-dir boot.zip frameworks/base/config/preloaded-classes-denylist device/google_car/bramble_car/preloaded-classes-wear android1.prof android2.prof"
  echo "         $0 output-dir boot.zip frameworks/base/config/preloaded-classes-denylist device/google_car/bramble_car/preloaded-classes-wear android.prof --profman-arg --upgrade-startup-to-hot=true"
