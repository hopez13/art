...
activate_system_package() {
  ...
  # Make sure that the new (absolute) path is less than 64 characters, which
  # is a hard limit set in the kernel for loop device filenames (otherwise, we
  # would get an error message from `losetup`, used by `mount` to manage the
  # loop device).
  [[ $(echo "$abs_image_filename" | wc -c) -gt 64 ]] \
    && { echo "Filename $abs_image_filename is too long to be used a loop device"; exit 1; }
