...
class JFuzz {
 ...
 private:
  ...
  // Emits a random operator selected from an array of operator strings.
  template <std::uint32_t N>
  inline void emitOp(const char* const (&ops)[N]) {
  ...
  void emitAssignmentOp(Type tp) {
    if (tp == kBoolean) {
      emitOp(kBoolAssignOps);
    } else if (isInteger(tp)) {
      emitOp(kIntAssignOps);
    } else {  // isFP(tp)
      emitOp(kFpAssignOps);
    }
  }
  ...
  bool emitTryCatchFinally() {
    // Continuing try-catch-finally nest becomes less likely as the depth grows.
