...
class CountDexIndices : public Experiment {
 public:
  ...
  void Dump(std::ostream& os) const {
    ...
    os << "Num strings from code: " << num_string_ids_from_code_ << "\n";
    ...
    os << "Total unique method ids: " << total_unique_method_idx_ << "\n";
    ...
    os << "Same class total: " << same_class_total << "\n";
    os << "Other class total: " << other_class_total << "\n";
    os << "Total method ids: " << (same_class_total + other_class_total) << "\n";
  }
  ...
  static int Run(int argc, char** argv) {
    ...
    for (const std::string& filename : options.filenames_) {
      ...
      if (!android::base::ReadFileToString(filename.c_str(), &content)) {
        std::cerr << "ReadFileToString failed for " + filename << std::endl;
        continue;
      }
      ...
      if (!dex_file_loader.OpenAll(reinterpret_cast<const uint8_t*>(content.data()),
                                   ...
                                   &error_msg,
                                   &dex_files)) {
        std::cerr << "OpenAll failed for " + filename << " with " << error_msg << std::endl;
        continue;
      }
      for (std::unique_ptr<const DexFile>& dex_file : dex_files) {
        if (options.dump_per_input_dex_) {
          ...
          if (!current.ProcessDexFile(*dex_file)) {
            std::cerr << "Failed to process " << filename << " with error " << error_msg;
            continue;
          }
          std::cout << "Analysis for " << dex_file->GetLocation() << std::endl;
          current.Dump(std::cout);
        }
        cumulative.ProcessDexFile(*dex_file);
      }
    }
    std::cout << "Cumulative analysis for " << cumulative.dex_count_ << " DEX files" << std::endl;
    cumulative.Dump(std::cout);
    return 0;
  }
};
