...
#include "dexanalyze_strings-inl.h"
...
uint32_t PrefixDictionary::AddOffset(uint32_t offset, uint32_t length) {
...
size_t PrefixStrings::AddString(uint16_t prefix, const std::string& str) {
...
std::string PrefixStrings::GetString(uint32_t string_idx) const {
...
size_t NormalStrings::AddString(const std::string& str) {
  const size_t string_offset = chars_.size();
  EncodeUnsignedLeb128(&chars_, str.length());
  const uint8_t* ptr = reinterpret_cast<const uint8_t*>(&str[0]);
  chars_.insert(chars_.end(), ptr, ptr + str.length());
...
std::string NormalStrings::GetString(uint32_t string_idx) const {
  const size_t offset = string_offsets_[string_idx];
