...
void AnalyzeStrings::ProcessStrings(const std::vector<std::string>& strings) {
  ...
  LOG(INFO) << "Prefix strings";
  Benchmark(prefix_strings, strings);
...

  {
    ...
    LOG(INFO) << "Normal strings";
    Benchmark(normal_strings, strings);
  }
}
...
void AnalyzeStrings::Benchmark(const Strings& strings,
  ...
  const uint64_t comparisons = reference.size() * kIterations;
  ...
  LOG(INFO) << "Compare equal " << static_cast<double>(NanoTime() - start) / comparisons
            << " ns per comparison";
  ...
  LOG(INFO) << "Compare reverse " << static_cast<double>(NanoTime() - start) / comparisons
            << " ns per comparison";
}

template void AnalyzeStrings::Benchmark(const PrefixStrings&, const std::vector<std::string>&);
template void AnalyzeStrings::Benchmark(const NormalStrings&, const std::vector<std::string>&);
