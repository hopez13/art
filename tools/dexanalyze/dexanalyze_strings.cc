...
void AnalyzeStrings::ProcessDexFiles(const std::vector<std::unique_ptr<const DexFile>>& dex_files) {
  ...
  static constexpr bool kUseGreedyTrie = false;
  if (kUseGreedyTrie) {
    std::vector<std::string> prefixes(prefix_construct->ExtractPrefixes(1 << 15));
  ...
  total_num_prefixes_ += used_prefixes.size();
