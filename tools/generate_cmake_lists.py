  ...
  for name in files:
    if name == 'CMakeLists.txt':
      if (os.path.samefile(root, out_art_cmakelists_dir)):
        continue
      if 'arm64' not in root:
        # arm64 is chosen randomly. We need to filter out only one architecture
        # for CMakeLists to avoid collisions.
        continue
      cmake_sub_dir = cmake_sub_dirs.add(root.replace(out_art_cmakelists_dir,
                                                      '.'))


# Now generating CMakeLists.txt
