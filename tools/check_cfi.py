...
Inst = collections.namedtuple("Inst", ["addr", "inst"])
...
def get_instructions(asm: Asm) -> List[Inst]:
  ...
  return [Inst(int(addr, 16), inst) for addr, inst in inst_re.findall(data)]
...
def get_infered_cfa_offsets(insts: List[Inst]) -> List[CfaOffset]:
  ...
  for addr, inst in insts:
    ...
    offset = future_offset.get(addr, offset)
...
def check_fde(fde: Fde, insts: List[Inst], srcs, verbose: bool = False) -> Tuple[str, Set[int]]:
  ...
  for inst in insts:
    ...
    if dwarf_cfa[i].offset is not None and dwarf_cfa[i].offset != infered_cfa[j].offset:
      if inst.addr in srcs:  # Only report if it maps to source code (not padding or litrals).
...
def check_lib(lib: pathlib.Path):
  ...
  for fde in fdes:
    ...
    while asms and asms[0].addr < fde.end:
      ...
      if asm.name in IGNORE or asm.name.startswith("$d."):
