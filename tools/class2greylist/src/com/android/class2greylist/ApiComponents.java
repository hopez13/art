...
public class ApiComponents {
    ...
    /**
     * Parse a fully qualified JNI class. e.g. Lfoo/bar/Baz;
     *
    ...
    private static String parseFullyQualifiedJNIClass(StringCursor sc) {
        ...
        if (sc.consume() != 'L') {
            sc.rollback();
            throw new IllegalArgumentException("Expected JNI class name definition to start with L, "
                    + "but instead got " + sc);
        }
        while (!sc.reachedEnd() && sc.peek() != ';') {
            char c = sc.consume();
            if (c != '/') {
                sb.append(c);
            } else {
    ...
    /**
     * Parse a JNI type; can be either a primitive or object type. Intrinsic arrays are handled
     * separately.
     ...
     *
     * This parameter can be an intrinsic array, in which case it will be preceded by a number of
     * open square braces (corresponding to its dimensionality)
     * @param sc cursor over the string assumed to contain a JNI type.
     * @return Same as {@link #parseJNIType(StringCursor)}, but also handle intrinsic
     *         arrays.
     */
    private static String parseSingleJNIMethodParameterType(StringCursor sc) {
    ...
    private static String convertJNIMethodParametersToJavadoc(StringCursor sc) {
        ...
        while(sc.peek() != ')') {
            methodParameterTypes.add(parseSingleJNIMethodParameterType(sc));
        }
    ...
    public static ApiComponents fromLinkTag(String linkTag, String contextSignature) {
        ...
        sc.consume();
        PackageAndClassName sp = splitClassName(fullyQualifiedClassName);

        packageName = sp.getPackageName();
