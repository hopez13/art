...
public class FileWritingGreylistConsumer implements GreylistConsumer {
    ...
    private final Map<Integer, PrintStream> mSdkToFilenameMap;
    ...
    public FileWritingGreylistConsumer(Status status, Map<Integer, String> sdkToFilenameMap)
        ...
        mStatus = status;
        mSdkToFilenameMap = openFiles(sdkToFilenameMap);
    }

    @Override
    public void greylistEntry(String signature, Integer maxTargetSdk) {
        PrintStream p = mSdkToFilenameMap.get(maxTargetSdk);
        if (p == null) {
    ...
    public void close() {
        for (PrintStream p : mSdkToFilenameMap.values()) {
            p.close();
        }
    }
}
