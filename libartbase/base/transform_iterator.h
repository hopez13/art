...
class TransformIterator {
 private:
  ...
  using ResultType = typename std::result_of_t<Function(InputType)>;

 public:
  using iterator_category = typename std::iterator_traits<BaseIterator>::iterator_category;
  using value_type =
      typename std::remove_const_t<typename std::remove_reference_t<ResultType>>;
  using difference_type = typename std::iterator_traits<BaseIterator>::difference_type;
  using pointer = typename std::conditional_t<
      std::is_reference_v<ResultType>,
      typename std::add_pointer_t<typename std::remove_reference_t<ResultType>>,
      TransformIterator>;
