...
class BaseBitVectorArray {
 public:
  ...
  // Union the vector of 'other' into 'dest_row'. In order to improve
  // performance we do this in 4-byte blocks. Clang should be able to optimize
  // this to larger blocks if possible.
  void UnionRows(size_t dest_row, size_t other) {
    ...
    uint32_t* source =
        GetRawData().GetRawStorage() + ((other * num_columns_) / BitVector::kWordBits);
