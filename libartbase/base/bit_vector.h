...
/*
 * Expanding bitmap, used for tracking resources. Bits are numbered starting from zero. All
 * operations on a BitVector are unsynchronized. New BitVectors are not necessarily zeroed out. If
 * the used allocator doesn't do clear the vector (e.g. ScopedArenaAllocator), the responsability of
 * clearing it relies on the caller (e.g. ArenaBitVector).
 */
class BitVector {
 public:
