...
TEST_F(HashSetTest, TestSwap) {
  ...
  hash_seta.insert(std::string("TEST"));
  hash_setb.insert(std::string("TEST2"));
  for (size_t i = 0; i < count; ++i) {
    strings.push_back(RandomString(10));
    hash_seta.insert(strings[i]);
  }
...
TEST_F(HashSetTest, StringSearchyStringPiece) {
  ...
  HashSet<std::string> hash_set;
  HashSet<std::string>::iterator insert_pos = hash_set.insert(std::string(test_string));
