...
struct MinimumTypeUnsignedHelper {
  using type =
    typename std::conditional_t<kBitSize == 0, void,       // NOLINT [whitespace/operators] [3]
    typename std::conditional_t<kBitSize <= 8, uint8_t,    // NOLINT [whitespace/operators] [3]
    typename std::conditional_t<kBitSize <= 16, uint16_t,  // NOLINT [whitespace/operators] [3]
    typename std::conditional_t<kBitSize <= 32, uint32_t,
    typename std::conditional_t<kBitSize <= 64, uint64_t,
    typename std::conditional_t<kBitSize <= BitSizeOf<uintmax_t>(), uintmax_t, void>>>>>>;
};
...
struct MinimumTypeHelper {
  ...
  using type =
    typename std::conditional_t</* if */   std::is_signed_v<T>,
                                /* then */ typename std::make_signed_t<type_unsigned>,
                                /* else */ type_unsigned>;
};
