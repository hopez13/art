...
TEST_F(FileUtilsTest, GetApexDataOdexFilename) {
  ...
  EXPECT_EQ(i18n_odex, filename);

  const std::string system_jar_apexdata_odex =
    std::string(kArtApexDataDefaultPath) + "/system/framework/oat/x86/rolo.odex";
  EXPECT_TRUE(
      GetApexDataOdexFilename("/system/framework/rolo.jar", InstructionSet::kX86, &filename));
  EXPECT_EQ(system_jar_apexdata_odex, filename);
}
