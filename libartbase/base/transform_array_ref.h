...
class TransformArrayRef {
 private:
  ...
  template <typename F, typename = typename std::result_of_t<F(const BaseType&)>>
 ...
 public:
  ...
  using const_iterator =
      typename std::conditional_t<std::is_same_v<ConstIter, FallbackConstIter>, void, ConstIter>;
  using reverse_iterator = std::reverse_iterator<Iter>;
  using const_reverse_iterator = typename std::conditional_t<
      std::is_same_v<ConstIter, FallbackConstIter>,
      void,
  ...
  template <typename OtherBT,
            typename = typename std::enable_if_t<std::is_same_v<BaseType, const OtherBT>>>
