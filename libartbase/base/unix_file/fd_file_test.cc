...
TEST_F(FdFileTest, CopySparse) {
  ...
  auto verify = [&](size_t empty_prefix, size_t empty_suffix, size_t input_offset) {
    ...
    ASSERT_TRUE(dest.ReadFully(check_data, empty_prefix));
    ASSERT_EQ(0, memcmp(check_data, zero_buffer.data(), empty_prefix));

    for (size_t i = 0; i < 2 * kChunksNumber - 1; i++) {
      ASSERT_TRUE(dest.ReadFully(check_data, sizeof(check_data)));

      if (i % 2 == 0) {
        ASSERT_EQ(0, memcmp(check_data, data_buffer.data(), kChunkSize));
      } else {
        ASSERT_EQ(0, memcmp(check_data, zero_buffer.data(), kChunkSize));
      }
    }

    ASSERT_TRUE(dest.ReadFully(check_data, empty_suffix));
    ASSERT_EQ(0, memcmp(check_data, zero_buffer.data(), empty_suffix));
