...
TEST_F(FdFileTest, CopySparseCreateSparseFile) {
  ...
  EXPECT_LE(kNumChunks * kChunkSize / kStatBlockSize, src1_stat.st_blocks);
...
TEST_F(FdFileTest, CopySparseFullCopy) {
  ...
  for (size_t empty_region_size = 128; empty_region_size <= kChunkSize/2; empty_region_size <<= 4) {
    // Empty prefix.
    ASSERT_NO_FATAL_FAILURE(verify_fullcopy(empty_region_size, /*empty_suffix=*/0u));
    // Empty suffix.
    ASSERT_NO_FATAL_FAILURE(verify_fullcopy(/*empty_prefix=*/0u, empty_region_size));
    // Both.
    ASSERT_NO_FATAL_FAILURE(verify_fullcopy(empty_region_size, empty_region_size));
  }
}
...
TEST_F(FdFileTest, CopySparsePartialCopy) {
  ...
  for (const auto& empty : outer_regions) {
    for (size_t discard = 0; discard <= 8 * art::KB; discard += 1 * art::KB) {
      ...
      ASSERT_NO_FATAL_FAILURE(verify_partialcopy(empty, empty, discard, /*copy_end_offset=*/0u));
      // End copy <discard> bytes before the file end.
      ASSERT_NO_FATAL_FAILURE(verify_partialcopy(empty, empty, /*copy_start_offset=*/0u, discard));
      // Both.
      ASSERT_NO_FATAL_FAILURE(verify_partialcopy(empty, empty, discard, discard));
    }
  }
}
