...
class FdFileTest : public RandomAccessFileTest {
 protected:
  ...
  static constexpr size_t kEstimateMaxFileMetadataSize = 128 * art::KB;
  ...
  void CreateSparseSourceFile(FdFile** file,
                              size_t empty_prefix,
                              size_t empty_suffix,
                              off_t input_offset);
...
#ifdef __linux__
// Create a sparse file and return a pointer to it via the 'file' argument, necessary because gtest
// assertions require the function to return void.
void FdFileTest::CreateSparseSourceFile(FdFile** file, /* out */
                                        size_t empty_prefix,
                                        size_t empty_suffix,
                                        off_t input_offset) {
/*
...
TEST_F(FdFileTest, CopySparse_CreateSparseFile) {
  ...
  EXPECT_LE(src1_stat.st_blocks,
            kEstimateMaxFileMetadataSize + kNumChunks * kChunkSize / kStatBlockSize);
  ...
  ASSERT_EQ(src1->Close(), 0);
  ASSERT_EQ(src2->Close(), 0);
  delete src1;
  delete src2;
}
...
void FdFileTest::TestSparseCopiedData(FdFile* dest,
  ...
  ASSERT_EQ(0, memcmp(check_buffer_.data(), zero_buffer_.data(), first_hole_size));
  ASSERT_TRUE(dest->ReadFully(check_buffer_.data(), first_data_size));
  ASSERT_EQ(0, memcmp(check_buffer_.data(), data_buffer_.data(), first_data_size));

  for (size_t i = 1; i < 2 * kNumChunks - 2; i++) {
    ASSERT_TRUE(dest->ReadFully(check_buffer_.data(), kChunkSize));
    if (i % 2 == 0) {
      ASSERT_EQ(0, memcmp(check_buffer_.data(), data_buffer_.data(), kChunkSize));
    } else {
      ASSERT_EQ(0, memcmp(check_buffer_.data(), zero_buffer_.data(), kChunkSize));
    }
  }

  ASSERT_TRUE(dest->ReadFully(check_buffer_.data(), last_data_size));
  ASSERT_EQ(0, memcmp(check_buffer_.data(), data_buffer_.data(), last_data_size));
  ASSERT_TRUE(dest->ReadFully(check_buffer_.data(), last_hole_size));
  ASSERT_EQ(0, memcmp(check_buffer_.data(), zero_buffer_.data(), last_hole_size));
}
...
TEST_F(FdFileTest, CopySparse_FullCopy) {
  auto verify_fullcopy = [&](size_t empty_prefix, size_t empty_suffix, size_t offset) {
    ...
    ASSERT_TRUE(dest.Copy(src, offset, copy_size));
    ASSERT_EQ(dest.Flush(), 0);
...
TEST_F(FdFileTest, CopySparse_PartialCopy) {
  auto verify_partialcopy = [&](size_t empty_prefix,
    ...
    ASSERT_TRUE(dest.Copy(src, offset + copy_start_offset, copy_size));
    ASSERT_EQ(dest.Flush(), 0);

    // Test destination length.
    ASSERT_EQ(dest.GetLength(), copy_size);

    // Test FD offsets.
    ASSERT_EQ(lseek(dest.Fd(), 0, SEEK_CUR), dest.GetLength());
    ASSERT_EQ(lseek(src->Fd(), 0, SEEK_CUR), src->GetLength() - copy_end_offset);
  ...
  // Test partial copies with outer skip regions of sizes [0, 128, 2048, 32768].
  for (size_t empty=0; empty<= kChunkSize/2; empty<<= 4) {
...
TEST_F(FdFileTest, CopySparse_TestAlignment) {
  ...
  auto verify_partialcopy = [&](size_t empty_prefix,
    ...
    // preserve the sparsity depending on data layout.
    ...
    if ((offset + copy_start_offset) % fs_blocksize == 0) {
      ...
      ASSERT_EQ(dest_stat.st_blocks, src_stat.st_blocks - discarded_blocks);
    } else {
      ...
      ASSERT_GT(dest_stat.st_blocks, src_stat.st_blocks - discarded_blocks);
    }
...
// Test the case where the destination file's FD offset is non-zero before a sparse copy.
TEST_F(FdFileTest, CopySparse_ToNonZeroOffset) {
  ...
  ASSERT_EQ(lseek(dest.Fd(), existing_holesize, SEEK_CUR), existing_length);
  ASSERT_EQ(0, dest.SetLength(existing_length));

  off_t copy_size = src->GetLength();
  ASSERT_TRUE(dest.Copy(src, 0, copy_size));
  ASSERT_EQ(dest.Flush(), 0);
