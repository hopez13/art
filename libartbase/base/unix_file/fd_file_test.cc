...
TEST_F(FdFileTest, Rename) {
  ...
  ASSERT_FALSE(art::OS::FileExists(old_filename.c_str()));
  FdFile dest(new_filename, O_RDONLY, false);
  ...
  // Confirm the data within the file at the new location matches the data that we wrote to the old
  // file location.
