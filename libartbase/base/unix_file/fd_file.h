...
class FdFile : public RandomAccessFile {
 public:
  ...
  // To preserve the same sparsity as the input file, both the input data offset ('offset') and the
  // output file descriptor's offset must be aligned with the filesystem blocksize.
 ...
 private:
  ...
  bool UserspaceSparseCopy(FdFile* input_file, off_t off, size_t size);
