...
std::unique_ptr<MetricsReporter> MetricsReporter::Create(ReportingConfig config, const ArtMetrics* metrics) {
...
MetricsReporter::~MetricsReporter() {
  ...
  if (config_.dump_to_logcat) {
    // LOG_STREAM(INFO) destroys the stream at the end of the statement, which makes it tricky pass it to
    // store as a field in StreamBackend. To get around this, we use an immediately-invoked lambda
    // expression to act as a let-binding, letting us access the stream for long enough to dump the
    // metrics.
    [this](std::ostream& os) {
      StreamBackend backend{os};
