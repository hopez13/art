...
std::string GetDefaultBootImageLocation(const std::string& android_root,
  ...
  if (kIsTargetBuild && !deny_art_apex_data_files) {
    ...
    // odrefresh generates a minimal boot image, which only includes code from BCP jars in the ART
    // module, when it fails to generate a single boot image for the entire bootclasspath (i.e.,
