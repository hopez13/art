...
std::string GetDefaultBootImageLocation(const std::string& android_root) {
  ...
  if (kIsTargetBuild) {
    ...
    const std::string boot_extension_image =
        StringPrintf("%s/system/framework/%s/boot-framework.art", GetArtApexData().c_str(), isa);
...
static bool GetApexDataAotFile(const char* location,
                               const char* extension,
                               const char* subdir,
                               InstructionSet isa,
                               std::string* filename) {
  // Check if `location` is on /system/framework then it could be a system server location or a boot
  // classpath extension. We also check if `location` is on the I18n module as this is not
  // updatable and is performance critical.
  if (!LocationIsOnSystemFramework(location) && !LocationIsOnI18nModule(location)) {
    return false;
  }

  // Transform:
  //  location: "/system/framework/example.jar"
  //  filename: "/data/misc/apexdata/com.android.art/system/framework/<subdir>/<arch>/example.oat"

  const std::string arch = GetInstructionSetString(isa);
  const std::string aot_file = ReplaceFileExtension(android::base::Basename(location), extension);
  *filename = StringPrintf("%s/system/framework/%s%s/%s", GetArtApexData().c_str(), subdir,
                           arch.c_str(), aot_file.c_str());
  return true;
}

bool GetApexDataOatFilename(const char* location, InstructionSet isa, std::string* filename) {
  return GetApexDataAotFile(location, "oat", /*subdir=*/"", isa, filename);
}

bool GetApexDataOdexFilename(const char* location, InstructionSet isa, std::string* filename) {
  return GetApexDataAotFile(location, "odex", "oat/", isa, filename);
}
