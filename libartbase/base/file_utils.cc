...
static std::string GetFirstBootClasspathExtensionJar(const std::string& android_root) {
  ...
  return android_root + "/framework/framework.jar";
}
...
std::string GetDalvikCacheDirectory(const char* subdir) {
  CHECK(subdir != nullptr);
  const std::string dalvik_cache = GetDalvikCacheDirectory(GetAndroidData(), subdir);
...
static std::string GetApexDataDalvikCacheFilename(std::string_view dex_location,
  ...
  if (encode_location) {
    // Input: "/system/framework/xyz.jar", "arm", true, "odex"
    // Output:
    ...
    // Args: "/system/framework/xyz.jar", "x86_64", false, "art"
    // Output: "/data/misc/apexdata/com.android.art/dalvik-cache/x86_64/boot-xyz.jar@classes.art"
