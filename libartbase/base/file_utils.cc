...
std::string GetAndroidRootSafe(std::string* error_msg) {
  ...
  *error_msg = "GetAndroidRootSafe unsupported for Windows.";
  ...
  if (!OS::DirectoryExists(kAndroidRootDefaultPath)) {
    *error_msg =
        StringPrintf("Failed to find %s directory %s", kAndroidRootEnvVar, kAndroidRootDefaultPath);
    return "";
  }
...
std::string GetAndroidRuntimeRootSafe(std::string* error_msg) {
#ifdef _WIN32
  UNUSED(kAndroidRootRuntimeEnvVar, kAndroidRuntimeApexDefaultPath, GetRootContainingLibartbase);
  *error_msg = "GetAndroidRuntimeRootSafe unsupported for Windows.";
  ...
  if (android_runtime_root_from_env != nullptr) {
    if (!OS::DirectoryExists(android_runtime_root_from_env)) {
  ...
  if (kIsTargetBuild) {
    // *However*, a copy of libartbase may still be installed out of the Android
    // Runtime Root in some occasions, as ART target gtests install their
    // binaries and their dependencies under the Android Root, i.e. "/system"
    // (see b/129534335). For that reason, we cannot reliably use
  ...
  // Try the default path.
  if (!OS::DirectoryExists(kAndroidRuntimeApexDefaultPath)) {
    *error_msg = StringPrintf("Failed to find %s directory %s",
                              kAndroidRuntimeRootEnvVar,
...
bool LocationIsOnRuntimeModule(const char* full_path) {
  std::string unused_error_msg;
  std::string module_path = GetAndroidRuntimeRootSafe(&unused_error_msg);
