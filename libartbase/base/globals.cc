  ...
  static struct CheckLoadedBuild {
    CheckLoadedBuild() {
      ...
      if (debug_build_loaded == release_build_loaded) {
        LOG(FATAL_WITHOUT_ABORT) << "Check build dependencies on libartbase. "
          << " debug_build_loaded=" << debug_build_loaded
          << " release_build_loaded=" << release_build_loaded;
        exit(1);  // We are in static initializer. CHECK might cause deadlock due to recursion.
      }
    }
