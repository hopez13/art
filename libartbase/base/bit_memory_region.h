...
class BitMemoryRegion final : public ValueObject {
 public:
  ...
  ALWAYS_INLINE void CopyBits(const BitMemoryRegion& src) {
    ...
    VisitChunks([this, &src](size_t offset, size_t num_bits, size_t value [[maybe_unused]])
                    ALWAYS_INLINE {
  ...
  ALWAYS_INLINE size_t PopCount() const {
    size_t result = 0u;
    VisitChunks([&](size_t offset [[maybe_unused]], size_t num_bits [[maybe_unused]], size_t value)
                    ALWAYS_INLINE {
  ...
  ALWAYS_INLINE bool HasAllBitsClear() const {
    return VisitChunks(
        [](size_t offset [[maybe_unused]], size_t num_bits [[maybe_unused]], size_t value)
