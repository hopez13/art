...
class BitMemoryRegion final : public ValueObject {
 public:
  ...
  ALWAYS_INLINE Result LoadBits(size_t bit_offset, size_t bit_length) const {
    ...
    // It just spells out the otherwise implicit behaviour and thus generates no extra code.
    // It is always no-op.  In particular, if `shift == 0` then `value == extra` because of
