...
class BitMemoryRegion final : public ValueObject {
 public:
  ...
  ALWAYS_INLINE void CopyBits(const BitMemoryRegion& src) {
    ...
    src.VisitChunks([this](size_t offset, size_t num_bits, size_t value) {
      StoreBits(offset, value, num_bits);
      return true;
    });
  }
  ...
  ALWAYS_INLINE void AndBits(const BitMemoryRegion& src) {
    DCHECK_EQ(size_in_bits(), src.size_in_bits());
    VisitChunks([this, &src](size_t offset, size_t num_bits, size_t value) {
      StoreBits(offset, value & src.LoadBits(offset, num_bits), num_bits);
      return true;
    });
  }
  ...
  ALWAYS_INLINE size_t PopCount() const {
    size_t result = 0u;
    VisitChunks(
        [&](size_t offset ATTRIBUTE_UNUSED, size_t num_bits ATTRIBUTE_UNUSED, size_t value) {
  ...
  static int Compare(const BitMemoryRegion& lhs, const BitMemoryRegion& rhs) {
    ...
    int result = 0;
    bool equals = lhs.VisitChunks([&](size_t offset, size_t num_bits, size_t lhs_value) {
  ...
  static bool Equals(const BitMemoryRegion& lhs, const BitMemoryRegion& rhs) {
    ...
    return lhs.VisitChunks([&rhs](size_t offset, size_t num_bits, size_t lhs_value) {
 ...
 private:
  ...
  bool VisitChunks(VisitorType&& visitor) const {
    ...
    if ((bit_start % kChunkSize) != 0u) {
      size_t num_bits = kChunkSize - (bit_start % kChunkSize);
      ...
      if (!visitor(offset, num_bits, value)) {
        return false;
      }
      offset += num_bits;
