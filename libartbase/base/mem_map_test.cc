...
TEST_F(MemMapTest, MapFile32Bit) {
  ...
  const size_t kMapSize = gPageSize;
...
TEST_F(MemMapTest, RemapFileViewAtEnd) {
  ...
  const size_t kMapSize = 3 * gPageSize;
  std::unique_ptr<uint8_t[]> data(new uint8_t[kMapSize]());
...
TEST_F(MemMapTest, Reservation) {
  ...
  std::string error_msg;
  ScratchFile scratch_file;
  const size_t kMapSize = 5 * gPageSize;
  std::unique_ptr<uint8_t[]> data(new uint8_t[kMapSize]());
  ASSERT_TRUE(scratch_file.GetFile()->WriteFully(&data[0], kMapSize));
  ...
  const size_t kChunk1Size = gPageSize - 1u;
  CHECK(kChunk1Size < kMapSize) << "We want to split the reservation.";
  ...
  const size_t kChunk2Size = 2 * gPageSize;
  ...
  const size_t kChunk2ASize = gPageSize - 1u;
