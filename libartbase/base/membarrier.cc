...
int membarrier(MembarrierCommand command) {
  ...
  static std::atomic<int> api_level = ATOMIC_VAR_INIT(0);
  if (UNLIKELY(api_level.load(std::memory_order_relaxed) == 0)) {
    api_level.store(android_get_device_api_level(), std::memory_order_seq_cst);
  }
  if (api_level.load(std::memory_order_relaxed) < _ANDROID_API_Q__) {
    errno = ENOSYS;
    return -1;
  }
#endif  // __BIONIC__
