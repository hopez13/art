...
TEST(BitTableTest, TestVarint) {
  for (size_t start_bit_offset = 0; start_bit_offset <= 32; start_bit_offset++) {
    ...
    for (uint32_t value : values) {
      ...
      MemoryRegion region(MemoryRegion(buffer.data(), buffer.size()));
...
TEST(BitTableTest, TestSingleColumnTable) {
  ...
  EXPECT_EQ(table.Get(0), 42u);
  EXPECT_EQ(table.Get(1), kNoValue);
  EXPECT_EQ(table.Get(2), 1000u);
  EXPECT_EQ(table.Get(3), kNoValue);
  EXPECT_EQ(table.NumColumnBits(0), 10u);
}
...
TEST(BitTableTest, TestBigTable) {
  ...
  builder.AddRow(42u, kNoValue, 00u, static_cast<uint32_t>(-2));
  ...
  EXPECT_EQ(table.Get(0, 0), 42u);
  EXPECT_EQ(table.Get(0, 1), kNoValue);
  EXPECT_EQ(table.Get(0, 2), 00u);
  EXPECT_EQ(table.Get(0, 3), static_cast<uint32_t>(-2));
  EXPECT_EQ(table.Get(1, 0), 62u);
  EXPECT_EQ(table.Get(1, 1), kNoValue);
  EXPECT_EQ(table.Get(1, 2), 63u);
  EXPECT_EQ(table.Get(1, 3), static_cast<uint32_t>(-3));
  EXPECT_EQ(table.NumColumnBits(0), 6u);
  EXPECT_EQ(table.NumColumnBits(1), 0u);
  EXPECT_EQ(table.NumColumnBits(2), 7u);
  EXPECT_EQ(table.NumColumnBits(3), 32u);
}
