...
static uintptr_t GenerateNextMemPos([[maybe_unused]] size_t page_size) {
#ifdef __BIONIC__
...
void MemMap::ZeroMemory(void* address, size_t length, bool release_eagerly) {
  ...
  uint8_t* const page_begin = AlignUp(mem_begin, GetPageSize());
  uint8_t* const page_end = AlignDown(mem_end, GetPageSize());
...
#if defined(__linux__)
  if (!release_eagerly) {
    size_t vec_len = (page_end - page_begin) / GetPageSize();
    ...
    if (mincore(page_begin, page_end - page_begin, vec.get()) == 0) {
      ...
      size_t current_size = GetPageSize();
      uint32_t old_state = vec[0] & 0x1;
      for (size_t i = 1; i < vec_len; ++i) {
        uint32_t new_state = vec[i] & 0x1;
        if (old_state == new_state) {
          current_size += GetPageSize();
        } else {
          ClearMemory(current_page, current_size, old_state, GetPageSize());
