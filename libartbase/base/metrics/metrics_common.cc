...
void XmlFormatter::FormatBeginReport(uint64_t timestamp_millis,
                                     const std::optional<SessionData>& session_data) {
  tinyxml2::XMLElement* art_internal_metrics = document_.NewElement("art_internal_metrics");
  document_.InsertEndChild(art_internal_metrics);

  tinyxml2::XMLElement* metadata = art_internal_metrics->InsertNewChildElement("metadata");
  ...
  if (session_data.has_value()) {
    ...
    metadata->InsertNewChildElement("compilation_reason")->SetText(CompilationReasonName(session_data->compilation_reason));
    metadata->InsertNewChildElement("compiler_filter")->SetText(CompilerFilterReportingName(session_data->compiler_filter));
  }

  art_internal_metrics->InsertNewChildElement("metrics");
}
...
void XmlFormatter::FormatReportHistogram(DatumId histogram_type,
  ...
  tinyxml2::XMLElement* histogram = metrics->InsertNewChildElement(DatumName(histogram_type).data());
