...
ALWAYS_INLINE static uint32_t DecodeVarintBits(MemoryRegion region, size_t* bit_offset) {
  uint32_t x = region.LoadBits(*bit_offset, kVarintHeaderBits);
  *bit_offset += kVarintHeaderBits;
  if (x > kVarintSmallValue) {
    size_t num_bits = (x - kVarintSmallValue) * kBitsPerByte;
...
ALWAYS_INLINE void EncodeVarintBits(Vector* out, size_t* bit_offset, uint32_t value) {
  if (value <= kVarintSmallValue) {
    ...
    region.StoreBits(*bit_offset, value, kVarintHeaderBits);
    *bit_offset += kVarintHeaderBits;
  } else {
    ...
    MemoryRegion region(out->data(), out->size());
    region.StoreBits(*bit_offset, kVarintSmallValue + num_bits / kBitsPerByte, kVarintHeaderBits);
    *bit_offset += kVarintHeaderBits;
...
class BitTable {
 public:
  ...
  ALWAYS_INLINE void Decode(MemoryRegion region, size_t* bit_offset) {
    ...
    *bit_offset += num_rows_ * column_offset_[kNumColumns];
  }

  ALWAYS_INLINE uint32_t Get(uint32_t row, uint32_t column = 0) const {
    DCHECK_LT(row, num_rows_);
    DCHECK_LT(column, kNumColumns);
    size_t offset = data_bit_offset_ + row * NumRowBits() + column_offset_[column];
