...
void* MemMap::TargetMMap(void* start, size_t len, int prot, int flags, int fd, off_t fd_off) {
  PLOG(INFO) << StringPrintf("TargetMMap start=%p, len=%zd, prot=%x, flags=%x, fd=%d, fd_off=%lld",
                             start, len, prot, flags, fd, fd_off);
...
int MemMap::TargetMUnmap(void* start, size_t len) {
  PLOG(INFO) << StringPrintf("TargetMUnmap start=%p, len=%zd\n", start, len);
  return 0;
}
