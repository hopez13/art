...
#ifdef ART_TARGET_ANDROID
  // android::base::LogdLogger breaks messages up into line delimited 4K chunks,
  // since that is the most that logd can handle per message.  To prevent other
  // threads from interleaving their messages, LogdLoggerLocked uses a mutex to
  // ensure that only one ART thread is logging at a time.
