...
// Return the basename of `path` and strip optional `extension` if present.
std::string_view Basename(std::string_view path, std::string_view extension = "");

// Return the directory name in `path` according to rules of dirname(3) without affecting the input.
