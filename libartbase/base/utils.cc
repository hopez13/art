...
static constexpr int kCachesForArm = 0;
...
bool TouchAndFlushCacheLinesWithinPage(uintptr_t start, uintptr_t limit, size_t attempts) {
  ...
  // Limit is where flushing stops.
  CHECK_EQ(RoundDown(start, kPageSize), RoundDown(limit - 1u, kPageSize)) << "spans page boundary";
  volatile uint8_t v = 0;
  for (size_t i = 0; i < attempts; ++i) {
    // Touch page to maximize chance page is resident.
    v = *reinterpret_cast<uint8_t*>(start);
    if (LIKELY(cacheflush(start, limit - start, kCachesForArm) == 0)) {
      return true;
    }
...
bool FlushCpuCaches(void* begin, void* end) {
  ...
  // where we can detect the error and return.
  //
