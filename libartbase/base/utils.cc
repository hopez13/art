...
static constexpr int kCachesForArm = 0x0;

// Number of retry attempts when flushing cache ranges.
static constexpr size_t kMaxFlushAttemptsForArm = 4;

int CacheFlush(uintptr_t start, uintptr_t limit) {
  // The Linux cacheflush(2) method is documented as having the following signature:
  //    int cacheflush(long __addr, long __nbytes, long __cache);
  // But on ARM with bionic it is just a system call wrapper (bionic/SYSCALLS.TXT) of the form:
  //    int cacheflush(long start, long end, long flags);
#ifdef __BIONIC__
  return cacheflush(start, limit, kCachesForArm);
#endif
}
...
bool FlushCpuCaches(void* begin, void* end) {
  ...
  while (start < limit) {
    ...
    if (!TouchAndFlushCacheLinesWithinPage(start, boundary, kMaxFlushAttemptsForArm)) {
      return false;
    }
    start = boundary;
    next_page += kPageSize;
