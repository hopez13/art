...
inline To down_cast(From* f) {                   // so we only accept pointers
  static_assert(std::is_base_of_v<From, typename std::remove_pointer_t<To>>,
...
inline To down_cast(From& f) {           // so we only accept references
  static_assert(std::is_base_of_v<From, typename std::remove_reference_t<To>>,
                "down_cast unsafe as To is not a subtype of From");
...
template <typename Dest, typename Source>
constexpr
typename std::enable_if_t<!std::is_enum_v<Source>, Dest>
enum_cast(Source value) {
  return static_cast<Dest>(
      dchecked_integral_cast<typename std::underlying_type_t<Dest>>(value));
}

template <typename Dest = void, typename Source>
constexpr
typename std::enable_if_t<std::is_enum_v<Source>,
                          typename std::conditional_t<std::is_same_v<Dest, void>,
...
enum_cast(Source value) {
  ...
  return dchecked_integral_cast<return_type>(
      static_cast<typename std::underlying_type_t<Source>>(value));
}
