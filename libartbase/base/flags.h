...
class Flag : public FlagBase {
 public:
  ...
  // If the flag is set via a server side setting, system property or cmdline arg,
  // then it returns the value of the flag.
  // Otherwise it return a nullopt (meaning this never returns the default value).
  //
  ...
  ALWAYS_INLINE std::optional<Value> GetValueOptional() const {
    std::pair<Value, FlagOrigin> result = GetValueLocation();
  ...
  ALWAYS_INLINE std::pair<Value, FlagOrigin> GetValueLocation() const {
    DCHECK(initialized_);
