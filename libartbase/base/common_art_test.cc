...
// Path beneath GetAndroidHostOut() where the art_common mk module (typically
// built through "m art-host-tests") installs support files for host tests.
...
std::string CommonArtTestImpl::GetLibCoreDexFilePrefix() const {
  if (kIsTargetBuild) {
    return "";
  } else {
    return GetAndroidHostOut() + "/" + kArtCommonHostInstallPath;
  }
}

std::vector<std::string> CommonArtTestImpl::GetLibCoreDexFileNames(
    const std::vector<std::string>& modules) const {
  return art::testing::GetLibCoreDexFileNames(GetLibCoreDexFilePrefix(), modules);
}
...
std::string CommonArtTestImpl::GetImageDirectory() {
  if (IsHost()) {
    ...
    return std::string(host_dir) + "/" + kArtCommonHostInstallPath +
