...
std::string CommonArtTestImpl::GetAndroidBuildTop() {
  ...
  if (android::base::ReadFileToString("/proc/self/cmdline", &argv)) {
    ...
    LOG(WARNING) << "android_build_top " << android_build_top << " from cmdline "
                 << argv.substr(0, argv.find('\0'));
  }
  ...
  if (android_build_top_from_env != nullptr) {
    if (std::filesystem::weakly_canonical(android_build_top).string() !=
      ...
      LOG(WARNING) << "Overriding android_build_top from env: " << android_build_top;
    }
  } else {
...
static std::string GetLibCoreDexFilePrefix(const std::string& abt) {
  if (kIsTargetBuild) {
    return "";
  } else {
    LOG(WARNING) << "GetLibCoreDexFilePrefix, root: " << GetAndroidRoot() << ", build top: " << abt;
    return GetAndroidRoot() + "/" + testing::kArtCommonHostInstallPath;
  }
}

std::vector<std::string> CommonArtTestImpl::GetLibCoreDexFileNames(
    const std::vector<std::string>& modules) const {
  return art::testing::GetLibCoreDexFileNames(GetLibCoreDexFilePrefix(GetAndroidBuildTop()),
                                              modules);
}
...
std::string CommonArtTestImpl::GetImageDirectory() {
  if (IsHost()) {
    ...
    return std::string(host_dir) + "/" + testing::kArtCommonHostInstallPath +
           "/apex/art_boot_images/javalib";
  }
