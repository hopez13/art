...
void CommonArtTestImpl::SetUpAndroidRootAndRuntime() {
  if (IsHost()) {
    ...
    if (android_host_out_from_env == nullptr) {
      ...
      android_host_out += android_build_top_from_env;
...
#error unsupported OS
    ...
    // ${ANDROID_RUNTIME_ROOT} is set on the device, but not necessarily on the host.
    // But it needs to be set so that icu4c can find its locale data.
    const char* android_runtime_from_env = getenv("ANDROID_RUNTIME_ROOT");
    if (android_runtime_from_env == nullptr) {
      std::string android_runtime;
      // Use ${ANDROID_HOST_OUT}/runtime for ANDROID_RUNTIME_ROOT.
      android_runtime += android_host_out_from_env;
      android_runtime += "/runtime";
      setenv("ANDROID_RUNTIME_ROOT", android_runtime.c_str(), 1);
    }
