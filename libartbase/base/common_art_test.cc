...
static std::string GetDexFileName(const std::string& jar_prefix, bool host) {
  ...
  return StringPrintf("%s/apex/%s/javalib/%s.jar", prefix.c_str(), apex, jar_prefix.c_str());
}
...
std::string CommonArtTestImpl::GetCoreFileLocation(const char* suffix) {
  ...
  std::string prefix(IsHost() ? GetAndroidRoot() : "");
  return StringPrintf("%s/apex/com.android.art/javalib/boot.%s", prefix.c_str(), suffix);
}
