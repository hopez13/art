...
std::string CommonArtTestImpl::GetAndroidBuildTop() {
  ...
  if (android_build_top == nullptr) {
    // Not set by build server, so set to default.
    SetUpAndroidRootEnvVars();
    android_build_top = getenv("ANDROID_BUILD_TOP");
    CHECK(android_build_top != nullptr);
  }
  std::string root = android_build_top;
  CHECK(!root.empty());
  if (root.back() != '/') {
    root += '/';
  }
