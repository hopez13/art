  ...
  MethodCacheEntry* GetMethodCacheEntryForOffset(int64_t dex_offset) {
    ...
    for (; class_def_index_ < dex_file_->NumClassDefs(); class_def_index_++) {
      ...
      for (const ClassAccessor::Method& method : accessor.GetMethods()) {
        ...
        auto res = method_cache_.emplace(offset + len, MethodCacheEntry{offset, len, index, ""});
...
const std::string empty_string = "";  // NOLINT [runtime/string]
...
const ExtDexFileString* ExtDexFileMakeString(const char* str) {
  if (str[0] == '\0') {
    return static_cast<const ExtDexFileString*>(&art::empty_string);
  }
  return art::MakeExtDexFileString(str);
}
...
void ExtDexFileFreeString(const ExtDexFileString* ext_string) {
  auto* str = static_cast<const std::string*>(ext_string);
  DCHECK(str != nullptr);
  if (str != &art::empty_string) {
    delete (str);
  }
}

bool ExtDexFileOpenFromMemory(const void* addr, /*inout*/ size_t* size, const char* location,
  ...
  std::unique_ptr<const art::DexFile> dex_file = loader.Open(
      static_cast<const uint8_t*>(addr), *size, loc_str, header->checksum_,
      /*oat_dex_file=*/nullptr, /*verify=*/false, /*verify_checksum=*/false, &error_msg);
  if (dex_file == nullptr) {
...
bool ExtDexFileOpenFromFd(int fd, off_t offset, const char* location,
                          /*out*/ const ExtDexFileString** ext_error_msg,
                          /*out*/ ExtDexFile** ext_dex_file) {
  size_t length;
  {
...
bool ExtDexFileGetMethodInfoForOffset(ExtDexFile* ext_dex_file, int64_t dex_offset,
  ...
  if (dex_file.dex_file_->IsCompactDexFile()) {
    ...
    const art::CompactDexFile::Header& cdex_header =
        dex_file.dex_file_->AsCompactDexFile()->GetHeader();
...
void ExtDexFileGetAllMethodInfos(ExtDexFile* ext_dex_file, bool with_signature,
                                 ExtDexFileMethodInfoCallback* method_info_cb, void* ctx) {
  auto& dex_file = *static_cast<art::DexFileWrapper*>(ext_dex_file);
  ...
  for (art::ClassAccessor accessor : dex_file.dex_file_->GetClasses()) {
    for (const art::ClassAccessor::Method& method : accessor.GetMethods()) {
      ...
      method_info.offset = static_cast<int32_t>(reinterpret_cast<const uint8_t*>(code.Insns()) -
                                                dex_file.dex_file_->Begin());
      method_info.len = code.InsnsSizeInBytes();
      method_info.name = art::MakeExtDexFileString(
          dex_file.dex_file_->PrettyMethod(method.GetIndex(), with_signature));
