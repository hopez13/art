 ...
 private:
  bool GetClassDefIndex(uint32_t dex_offset, uint32_t* class_def_index) {
    if (class_cache_.empty()) {
      ...
      for (uint32_t idx = 0; idx < dex_file_->NumClassDefs(); idx++) {
        art::ClassAccessor accessor(*dex_file_, idx);
        for (const art::ClassAccessor::Method& method : accessor.GetMethods()) {
          art::CodeItemInstructionAccessor code = method.GetInstructions();
          if (code.HasCodeItem()) {
            int32_t offset = reinterpret_cast<const uint8_t*>(code.Insns()) - dex_file_->Begin();
            cache.emplace(offset + code.InsnsSizeInBytes(), idx);
          }
        }
      }
  ...
  std::vector<std::pair<uint32_t, uint32_t>> class_cache_;  // end_dex_offset -> class_def_index.
