...
struct ExtDexFile {
 ...
 private:
  bool GetClassDefIndex(uint32_t dex_offset, uint32_t* class_def_index) {
    std::deque<std::pair<uint32_t, uint32_t>>& cache = class_cache_;
    if (cache.empty()) {
      ...
      for (const art::ClassAccessor& accessor : dex_file_->GetClasses()) {
        for (const art::ClassAccessor::Method& method : accessor.GetMethods()) {
          art::CodeItemInstructionAccessor code = method.GetInstructions();
          if (code.HasCodeItem()) {
            int32_t offset = reinterpret_cast<const uint8_t*>(code.Insns()) - dex_file_->Begin();
      ...
      if (cache.size() > 1) {
        ...
        auto pred = [](auto it) { return it.first == 0; };  // Entries to remove.
        cache.erase(std::remove_if(cache.begin(), cache.end(), pred), cache.end());
      }
    }
    ...
    auto it = std::upper_bound(cache.begin(), cache.end(), dex_offset, comp);
    if (it != cache.end()) {
  ...
  std::deque<std::pair<uint32_t, uint32_t>> class_cache_;
