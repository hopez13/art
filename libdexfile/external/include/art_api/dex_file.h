...
struct ExtDexFileMethodInfo {
  int32_t offset;
  int32_t len;
  const ExtDexFileString* name;  // Owned by caller except in ExtDexFileMethodInfoCb.
};
...
// the stable C ABI and handles instance ownerships.
class DexFile {
 public:
  ...
  std::vector<MethodInfo> GetAllMethodInfos() {
    MethodInfoVector res;
    ExtDexFileGetAllMethodInfos(ext_dex_file_, AddMethodInfo, static_cast<void*>(&res));
    return res;
  }
