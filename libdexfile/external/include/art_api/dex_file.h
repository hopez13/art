...
class DexString final {
 ...
 private:
  ...
  const ExtDexFileString* ext_string_;  // Owned instance.
 ...
 public:
  ...
  static std::unique_ptr<DexFile> OpenFromMemory(const void* addr, size_t* size,
    ...
    if (ext_error_msg == nullptr) {
      ...
      *error_msg = static_cast<std::string>(DexString(ext_error_msg));
    }
    return nullptr;
  }
