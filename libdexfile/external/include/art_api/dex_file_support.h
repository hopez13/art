...
class DexFile {
 public:
  ...
  void GetMethodInfoForOffset(int64_t dex_offset, T& callback, uint32_t flags = kWithSignature) {
  ...
  // Call the provided callback for all dex method.
  template<typename T /* lambda taking (ExtDexFileMethodInfo*) */>
  void GetAllMethodInfos(T& callback, uint32_t flags = kWithSignature) {
    auto cb = [](void* ctx, ExtDexFileMethodInfo* info) { (*reinterpret_cast<T*>(ctx))(info); };
