...
TEST_F(FuzzerCorpusTest, VerifyCorpusDexFiles) {
  ...
  const std::unordered_set<std::string> valid_dex_files = {
      // Dex files created for the fuzzer.
      "Main.dex",
      "hello_world.dex",
      // Other dex files which may appear in the folder.
      "art-gtest-jars-ArrayClassWithUnresolvedComponent.dex",
      "art-gtest-jars-Dex2oatVdexPublicSdkDex.dex",
      "art-gtest-jars-LinkageTest.dex",
      "art-gtest-jars-SuperWithAccessChecks.dex",
      "art-gtest-jars-VerifierDeps.dex",
      "art-gtest-jars-VerifierDepsMulti.dex",
      "art-gtest-jars-VerifySoftFailDuringClinit.dex"};
  ...
  // Run the verifier on the dex files present in the folder.
  for (const auto& file : std::filesystem::directory_iterator(folder)) {
    if (file.path().extension() != ".dex") {
      continue;
    }
