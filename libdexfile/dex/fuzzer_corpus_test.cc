...
class FuzzerCorpusTest : public testing::Test {
  ...
  static std::string GetAndroidBuildTop() {
    return CommonArtTestImpl::GetAndroidBuildTop();
  }
...
TEST_F(FuzzerCorpusTest, VerifyCorpusDexFiles) {
  ...
  // TODO(solanes): How to run for device?
  if (!IsHost()) {
    return;
  }

  // Consistency checks.
  const std::string corpus_location = "art/tools/fuzzer/corpus";
  std::string folder(IsHost() ? GetAndroidBuildTop() + corpus_location : corpus_location);
