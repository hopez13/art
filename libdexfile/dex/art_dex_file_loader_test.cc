...
TEST_F(ArtDexFileLoaderTest, GetChecksum) {
  ...
  ASSERT_TRUE(DexFileLoader::AccMultiDexChecksum(java_lang_dex_file_, &expected_checksum));
...
TEST_F(ArtDexFileLoaderTest, GetMultiDexChecksum) {
  ...
  EXPECT_TRUE(DexFileLoader::AccMultiDexChecksum(dexes[0].get(), &expected_checksum));
  EXPECT_EQ(dexes[1]->GetLocation(), DexFileLoader::GetMultiDexLocation(1, multidex_file.c_str()));
  EXPECT_TRUE(DexFileLoader::AccMultiDexChecksum(dexes[1].get(), &expected_checksum));
  EXPECT_EQ(expected_checksum, checksum.value());
}
