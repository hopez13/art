...
TEST_F(ArtDexFileLoaderTest, GetChecksum) {
  ...
  EXPECT_TRUE(dex_loader.GetMultiDexChecksum(&checksum, &error_msg))
      << error_msg;
  ASSERT_TRUE(checksum.has_value());
  EXPECT_EQ(java_lang_dex_file_->GetLocationChecksum(), checksum.value());
}
...
TEST_F(ArtDexFileLoaderTest, GetMultiDexChecksum) {
  ...
  uint32_t checksum0 = dexes[0]->GetHeader().checksum_;

  EXPECT_EQ(dexes[1]->GetLocation(), DexFileLoader::GetMultiDexLocation(1, multidex_file.c_str()));
  uint32_t checksum1 = dexes[0]->GetHeader().checksum_;

  ASSERT_EQ(checksum.value(), checksum0 ^ checksum1);
}
