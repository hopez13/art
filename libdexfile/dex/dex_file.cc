...
bool DexFile::EnableWrite() const {
  CHECK(container_.get() != nullptr);
  return container_->SetReadOnly(false);
}

bool DexFile::DisableWrite() const {
  CHECK(container_.get() != nullptr);
  return container_->SetReadOnly(true);
}
...
ArrayRef<const uint8_t> DexFile::GetDataRange(const uint8_t* data,
  ...
  if (size >= sizeof(StandardDexFile::Header) && StandardDexFile::IsMagicValid(data)) {
    ...
    if (size >= sizeof(*header) && header->header_size_ >= sizeof(*header)) {
      ...
      // TODO: Get multidex_size from header.
      size = std::numeric_limits<size_t>::max() - reinterpret_cast<size_t>(data);
    }
