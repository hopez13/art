...
uint32_t DexFile::CalculateChecksum() const {
  return CalculateChecksum(Begin(), header_->file_size_);
}
...
ArrayRef<const uint8_t> DexFile::GetDataRange(const uint8_t* data,
  ...
  if (size >= sizeof(StandardDexFile::Header) && StandardDexFile::IsMagicValid(data)) {
    ...
    CHECK_EQ(container->Data().size(), 0) << "Unsupported for standard dex";
    if (size >= sizeof(*header) && header->header_size_ >= sizeof(*header)) {
      ////////////////////////////////
      CHECK_GE(header->GetVersion(), 41);
      data -= header->multidex_offset;
      // TODO: Add multidex_size to header?
      size = container->End() - data;
    } else {
      /////////////////////////////////////
      CHECK_LT(header->GetVersion(), 41);
    }
    ...
    // The DexFileLoader should have checked the validity of the header already.
    LOG(FATAL) << "Invalid dex header";
  }
  DCHECK_GE(data, container->Begin());
  DCHECK_LE(data, container->End());
  DCHECK_GE(data + size, container->Begin());
  DCHECK_LE(data + size, container->End());
  return {data, size};
}
