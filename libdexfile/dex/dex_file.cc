...
void DexFile::Header::SetDexContainer(size_t header_offset, size_t continer_size) {
  if (HasDexContainer()) {
    DCHECK_LE(header_offset, continer_size);
    DCHECK_LE(file_size_, continer_size - header_offset);
    data_off_ = 0;
    data_size_ = 0;
    ...
    headerV41->container_size_ = continer_size;
  } else {
    DCHECK_EQ(header_offset, 0u);
    DCHECK_EQ(continer_size, file_size_);
  }
}
