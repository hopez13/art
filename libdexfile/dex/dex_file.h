...
class DexFile {
 public:
  ...
  const uint8_t* Begin() const {
    return dex_main_->Begin();
  }

  size_t Size() const {
    return dex_main_->Size();
  }

  const uint8_t* DataBegin() const {
    return dex_data_->Begin();
  }

  size_t DataSize() const {
    return dex_data_->Size();
  }
 ...
 protected:
  ...
  DexFile(std::unique_ptr<DexFileContainer> dex_main,
          const std::string& location,
  ...
  std::unique_ptr<DexFileContainer> dex_main_;

  // The container for the data section
  std::unique_ptr<DexFileContainer> dex_data_;
