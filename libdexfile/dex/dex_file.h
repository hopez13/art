...
class DexFile {
 public:
  ...
  const uint8_t* GetHiddenapiAccessFlagsStream(const ClassDef& class_def) const {
    if (hiddenapi_metadata_ == nullptr) {
      return nullptr;
    }
    DCHECK(GetClassData(class_def) != nullptr);
    return hiddenapi_metadata_->GetFlagsStream(GetIndexForClassDef(class_def), *this);
  }
