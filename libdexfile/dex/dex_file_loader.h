...
class DexFileLoader {
 ...
 protected:
  ...
  class DexScopedTrace {
   public:
    ~DexScopedTrace() {}
  };

  // No-op in DexFileLoader since it depends on libartpalette. Implemented in ArtDexFileLoader.
  virtual std::unique_ptr<DexScopedTrace> CreateScopedTrace(const std::string&) const { return {}; }
