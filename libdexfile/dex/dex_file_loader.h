...
class DexFileLoader {
 public:
  ...
  DexFileLoader(std::unique_ptr<DexFileContainer>&& container, const std::string& location)
      : container_(std::move(container)), location_(location) {}
 ...
 protected:
  ...
  // Implemented by ArtDexFileLoader, since users of DexFileLoader might not have libartpalette.
  virtual void ATraceBegin(const char*) const {}
  virtual void ATraceEnd() const {}

  // Call ATraceBegin and return an object that will call ATraceEnd during destruction.
  auto ScopedTrace(const std::string& name) const {
    ATraceBegin(name.c_str());
    struct Scope {
      ~Scope() { atrace_end_(); }
      std::function<void()> atrace_end_;
    };
    return Scope{.atrace_end_ = std::bind(&DexFileLoader::ATraceEnd, this)};
  }

  std::unique_ptr<DexFile> OpenCommon(const std::shared_ptr<DexFileContainer> container,
                                      const uint8_t* base,
                                      size_t size,
  ...
  const std::shared_ptr<DexFileContainer> container_;
