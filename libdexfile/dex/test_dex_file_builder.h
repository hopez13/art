...
class TestDexFileBuilder {
 public:
  ...
  std::unique_ptr<const DexFile> Build(const std::string& dex_location,
    ...
    std::string map_name = "memory for " + dex_location;
    std::string error_msg;
    MemMap map = MemMap::MapAnonymous(map_name.c_str(),
                                      total_size,
                                      PROT_READ | PROT_WRITE,
                                      /*low_4gb=*/ false,
                                      &error_msg);
    CHECK(map.IsValid());
    ArrayRef<uint8_t> dex_file_data(map.Begin(), map.Size());
    CHECK_EQ(total_size, dex_file_data.size());
 ...
 private:
  ...
  static void Write32(ArrayRef<uint8_t> dex_file_data, size_t offset, uint32_t value) {
  ...
  static void Write16(ArrayRef<uint8_t> dex_file_data, size_t offset, uint32_t value) {
