...
class DexFileVerifier {
 ...
 private:
  ...
  bool CheckArrayElement(size_t size, ToDoList* todo);
  bool CheckEncodedAnnotation(ToDoList* todo);
  bool CheckAnnotationElement(uint32_t last_idx, uint32_t i, uint32_t size, ToDoList* todo);
...
bool DexFileVerifier::CheckArrayElement(size_t size, ToDoList* todo) {
  if (size != 0u) {
...
bool DexFileVerifier::CheckEncodedAnnotation(ToDoList* todo) {
  ...
  DECODE_UNSIGNED_CHECKED_FROM(ptr_, size);
  todo->emplace([=]() { return CheckAnnotationElement(0, 0, size, todo); });
  return true;
}

bool DexFileVerifier::CheckAnnotationElement(uint32_t last_idx,
                                             uint32_t i,
                                             uint32_t size,
                                             ToDoList* todo) {
  if (i < size) {
    ...
    todo->emplace([=]() { return CheckAnnotationElement(idx, i + 1, size, todo); });
