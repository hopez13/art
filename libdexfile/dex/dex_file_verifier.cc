...
bool DexFileVerifier::VerifyStringIds() {
  ...
  for (; it < last; it++) {
    ...
    if (utf8_data >= data_end_as_char) {
      // The start of the string is past the end.
      return false;
    }

    size_t max_len = data_end_as_char - utf8_data;
    size_t str_len = strnlen(utf8_data, data_end_as_char - utf8_data);
    if (str_len == max_len) {
      // Didn't find a null-terminator.
      return false;
    }

    size_t utf16_size = CountModifiedUtf8Chars(utf8_data, str_len);
    if (length != utf16_size) {
      // Length mismatch.
      return false;
    }
  }
