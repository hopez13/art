...
bool DexFileVerifier::CheckEncodedArray() {
  ...
  todo_.emplace([=]() { return CheckArrayElement(size); });
  return true;
}
...
bool DexFileVerifier::CheckArrayElement(uint32_t size) {
  if (size != 0u) {
    ...
    todo_.emplace([=]() { return CheckArrayElement(size - 1); });
...
bool DexFileVerifier::CheckEncodedAnnotation() {
  ...
  DECODE_UNSIGNED_CHECKED_FROM(ptr_, size);
  todo_.emplace([=]() { return CheckAnnotationElement(kDexNoIndex, size); });
  return true;
}
...
bool DexFileVerifier::CheckAnnotationElement(uint32_t last_idx, uint32_t size) {
  if (size != 0u) {
    ...
    todo_.emplace([=]() { return CheckAnnotationElement(idx, size - 1); });
...
bool DexFileVerifier::FlushToDoList() {
  while (!todo_.empty()) {
    std::function<bool()> fn = std::move(todo_.top());
    todo_.pop();
    if (!fn()) {
      return false;
    }
