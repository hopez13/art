...
class DexFileVerifier {
 public:
  DexFileVerifier(const DexFile* dex_file, bool verify_checksum)
      : dex_file_(dex_file),
        ...
        location_(dex_file->GetLocation().c_str()),
...
bool DexFileVerifier::CheckHeader() {
  ...
  if (!StandardDexFile::IsVersionValid(header_->magic_)) {
...
bool Verify(const DexFile* dex_file, bool verify_checksum, std::string* error_msg) {
  DexFileVerifier verifier(dex_file, verify_checksum);
  if (!verifier.Verify()) {
