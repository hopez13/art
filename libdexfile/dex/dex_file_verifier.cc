...
bool DexFileVerifier::CheckHeader() {
  ...
  if (dex_version_ >= 41) {
    ...
    if (header_->data_off_ != 0) {
      ErrorStringPrintf("data_off must be 0 in dex version 041");
      return false;
    }
    ...
    if (headerV41->container_size <= headerV41->container_offset) {
      ErrorStringPrintf("Multi-dex container is too small: size=%ud header_offset=%ud",
                        headerV41->container_size,
                        headerV41->container_offset);
      return false;
    }
    uint32_t remainder = headerV41->container_size - headerV41->container_offset;
