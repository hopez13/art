...
inline ClassAccessor::ClassAccessor(const DexFile& dex_file,
  ...
  if (parse_hiddenapi_class_data &&
      class_data != nullptr &&
      class_def_index != DexFile::kDexNoIndex32) {
    const DexFile::HiddenapiClassData* hiddenapi_class_data = dex_file.GetHiddenapiClassData();
    if (hiddenapi_class_data != nullptr) {
      hiddenapi_ptr_pos_ = hiddenapi_class_data->GetFlags(class_def_index);
    }
  }
}
...
inline void ClassAccessor::Method::Read() {
  ...
  if (hiddenapi_ptr_pos_ != nullptr) {
    ...
    if (hiddenapi_flags_ >= 4) {
      LOG(ERROR) << "Invalid flags for method " << dex_file_.PrettyMethod(GetIndex())
                 << ", is_compact=" << dex_file_.IsCompactDexFile();
    }
  }
}
...
inline void ClassAccessor::Field::Read() {
  ...
  if (hiddenapi_ptr_pos_ != nullptr) {
    hiddenapi_flags_ = DecodeUnsignedLeb128(&hiddenapi_ptr_pos_);
    if (hiddenapi_flags_ >= 4) {
      LOG(ERROR) << "Invalid flags for field " << dex_file_.PrettyField(GetIndex())
                 << ", is_compact=" << dex_file_.IsCompactDexFile();
    }
  }
}
