...
std::unique_ptr<const DexFile> ArtDexFileLoader::Open(const uint8_t* base,
  ...
  return OpenCommon(std::make_unique<NonOwningDexFileContainer>(base, size),
                    std::make_unique<NonOwningDexFileContainer>(nullptr, 0u),
                    location,
                    location_checksum,
                    oat_dex_file,
                    verify,
...
std::unique_ptr<DexFile> ArtDexFileLoader::OpenCommon(std::unique_ptr<DexFileContainer> dex_main,
                                                      ...
                                                      std::string* error_msg,
                                                      VerifyResult* verify_result) {
  std::unique_ptr<DexFile> dex_file = DexFileLoader::OpenCommon(std::move(dex_main),
                                                                std::move(dex_data),
                                                                location,
                                                                location_checksum,
                                                                oat_dex_file,
