...
class MemMapContainer : public DexFileContainer {
 public:
  ...
  uint8_t* Begin() override { return mem_map_.Begin(); }

  uint8_t* End() override { return mem_map_.End(); }
...
std::unique_ptr<const DexFile> ArtDexFileLoader::Open(const uint8_t* base,
  ...
  auto container = std::make_shared<MemoryDexFileContainer>(base, base + size);
...
std::unique_ptr<const DexFile> ArtDexFileLoader::Open(std::shared_ptr<DexFileContainer> container,
                                                      const std::string& location,
                                                      uint32_t location_checksum,
                                                      const OatDexFile* oat_dex_file,
                                                      bool verify,
