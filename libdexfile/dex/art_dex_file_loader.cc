...
std::unique_ptr<ArtDexFileLoader::DexScopedTrace> ArtDexFileLoader::CreateScopedTrace(
    const std::string& name) const {
  class DexScopedTraceImpl : public DexFileLoader::DexScopedTrace, ScopedTrace {
   public:
    explicit DexScopedTraceImpl(const std::string& name) : ScopedTrace(name) {}
  };
  return std::make_unique<DexScopedTraceImpl>(name);
}
