...
TEST(ProtoReference, WithinOneDexFile) {
    ...
    EXPECT_EQ(6u, dex_file->NumProtoIds());

    ProtoReference I = ProtoReference(dex_file.get(), dex::ProtoIndex(0));
    ProtoReference IString = ProtoReference(dex_file.get(), dex::ProtoIndex(1));
    ProtoReference IIString = ProtoReference(dex_file.get(), dex::ProtoIndex(2));
    ProtoReference IJString = ProtoReference(dex_file.get(), dex::ProtoIndex(3));
    ProtoReference JJString = ProtoReference(dex_file.get(), dex::ProtoIndex(4));
    ProtoReference V = ProtoReference(dex_file.get(), dex::ProtoIndex(5));

    ProtoReferenceValueComparator cmp;

    EXPECT_FALSE(cmp(V, V));
    EXPECT_FALSE(cmp(JJString, JJString));

    EXPECT_TRUE(cmp(I, IString));
    EXPECT_TRUE(cmp(I, V));
    EXPECT_TRUE(cmp(IString, IIString));
    EXPECT_TRUE(cmp(IIString, IJString));
    EXPECT_TRUE(cmp(IJString, JJString));
    EXPECT_TRUE(cmp(JJString, V));
}

TEST(ProtoReference, AccrossDifferentDexFiles) {
    ...
    EXPECT_FALSE(cmp(V_dex1, V_dex1));
