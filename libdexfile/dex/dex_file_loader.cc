...
class DexZipEntry {
 public:
  ...
  bool IsUncompressed() { return zip_entry_->method == kCompressStored; }
  ...
  uint32_t Offset() { return zip_entry_->offset; }
...
DexFileLoader::DexFileLoader(const char* filename, int fd, const std::string& location)
    : DexFileLoader(std::make_shared<FileContainer>(filename, fd), location) {}
...
std::unique_ptr<const DexFile> DexFileLoader::Open(uint32_t location_checksum,
  ...
  auto trace = CreateScopedTrace(std::string("Open dex file ") + location_);
...
std::unique_ptr<DexFile> DexFileLoader::OpenCommon(
  ...
  if (verify) {
    auto trace = CreateScopedTrace(std::string("Verify dex file ") + location);
...
std::unique_ptr<const DexFile> DexFileLoader::OpenOneDexFileFromZip(
  ...
  if (zip_entry->IsUncompressed() && IsAligned<sizeof(uint32_t)>(zip_entry->Offset())) {
    ...
    auto trace = CreateScopedTrace(std::string("Extract dex file ") + location);
