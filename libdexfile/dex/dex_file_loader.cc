...
class MemMapContainer : public DexFileContainer {
 public:
  ...
  bool IsReadOnly() const override {
    CHECK(mem_map_.IsValid());
    return mem_map_.GetProtect() == PROT_READ;
  }

  bool SetReadOnly(bool read_only = true) override {
    CHECK(mem_map_.IsValid());
    return mem_map_.Protect(read_only ? PROT_READ : (PROT_READ | PROT_WRITE));
  }
...
std::unique_ptr<const DexFile> DexFileLoader::Open(uint32_t location_checksum,
  ...
  DCHECK_EQ(dex_file->GetHeader().file_size_, root_container_->Size());
  return dex_file;
}
