...
void DexFileLoader::AccMultiDexChecksum(const std::string& dex_location,
                                        uint32_t dex_location_checksum,
                                        /*out*/ std::optional<uint32_t>* combined_checksum) {
  if (!combined_checksum->has_value()) {
    CHECK(!IsMultiDexLocation(dex_location.c_str())) << dex_location;  // Expect primary dex.
  } else {
    CHECK(IsMultiDexLocation(dex_location.c_str())) << dex_location;  // Expect secondary dex.
  }
