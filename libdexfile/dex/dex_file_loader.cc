...
bool DexFileLoader::Open(bool verify,
  ...
  if (IsMagicValid(magic)) {
    ...
    DCHECK(root_container_ != nullptr);
    for (size_t header_offset = 0, i = 0; header_offset < root_container_->Size(); i++) {
      ...
      header_offset += dex_files->back()->GetHeader().file_size_;
    }
    return true;
  }
...
bool DexFileLoader::OpenFromZipEntry(const ZipArchive& zip_archive,
  ...
  for (size_t header_offset = 0, i = 0; header_offset < container->Size(); i++) {
