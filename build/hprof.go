...
func getTestDumpJar(ctx android.ModuleContext, testDump string) android.Path {
	...
	if outputFileProducer, ok := module.(android.OutputFileProducer); ok {
		jars, err := outputFileProducer.OutputFiles("")
		if err != nil {
			ctx.PropertyErrorf("test_dump", "failed to get jar from test_dump module %q: %s", testDump, err.Error())
			return nil
		}
		if len(jars) == 0 {
			ctx.PropertyErrorf("test_dump", "failed to get jar from test_dump module %q", testDump)
			return nil
		} else if len(jars) > 1 {
			ctx.PropertyErrorf("test_dump", "got multiple jars from test_dump module %q, expected exactly one", testDump)
			return nil
		}
		return jars[0]
	} else {
		ctx.PropertyErrorf("test_dump", "test_dump module %q is not an OutputFileProducer", testDump)
		return nil
	}
}
...
func getCoreImage(ctx android.ModuleContext, coreImage string) android.Path {
	...
	if module == nil {
		ctx.ModuleErrorf("failed to get cor image module")
		return nil
	}
