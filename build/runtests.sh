...
# This script requires `guestmount`, `guestunmount`, and `virt-filesystems` from
# libguestfs. On Debian-based systems, these tools can be installed with:
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    (-s|--skip-build) build_art_apex_p=false;;
...
# Build the ART APEX package (optional).
$build_art_apex_p && say "Building package" && make "$art_apex"
...
$list_image_files_p && say "Listing image files" && tree -a "$mount_point"
