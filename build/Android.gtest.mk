...
valgrind-$$(gtest_rule): VALGRIND_PRIVATE_GTEST_WITNESS := $$(valgrind_gtest_witness)

.PHONY: valgrind-$$(gtest_rule)
valgrind-$$(gtest_rule): $(ART_VALGRIND_TARGET_DEPENDENCIES) test-art-target-sync
	$(hide) adb shell touch $$(VALGRIND_PRIVATE_GTEST_WITNESS)
	$(hide) adb shell rm $$(VALGRIND_PRIVATE_GTEST_WITNESS)
	$(hide) adb shell $$(PRIVATE_MAYBE_CHROOT_COMMAND) chmod 755 $$(PRIVATE_TARGET_EXE)
	$(hide) $$(call ART_TEST_SKIP,$$@) && \
	  (adb shell "$$(PRIVATE_MAYBE_CHROOT_COMMAND) env $(GCOV_ENV) LD_LIBRARY_PATH=$(4) \
	     ...
	     && touch $$(VALGRIND_PRIVATE_GTEST_WITNESS)" \
	   && (adb pull $$(VALGRIND_PRIVATE_GTEST_WITNESS) /tmp/ && $$(call ART_TEST_PASSED,$$@)) \
	   || $$(call ART_TEST_FAILED,$$@))
	$(hide) rm -f /tmp/$$@-$$$$PPID
