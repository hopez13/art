...
class DebugTargetChecker:
  ...
  def run(self):
    ...
    # rather that we don't get behavioural differences between release and
    # userdebug/eng builds, which could happen if the debug package has
    # duplicate library instances where releases don't. Iow, it's uncontroversal
    # to add debug-only dependencies, as long as they don't make assumptions on
    # having a single global state (ideally they should have
