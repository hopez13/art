...
class ThirtyTwoChecker(Checker):
  def __init__(self, provider):
...
class SixtyFourChecker(Checker):
  def __init__(self, provider):
    super().__init__(provider)
...
class DebugChecker:
  ...
  def run(self):
    ...
    self._checker.check_binary_symlink('dex2oatd')
...
def artApexTestMain(args):
  ...
  if args.bitness == '32':
    base_checker = ThirtyTwoChecker(apex_provider)
  elif args.bitness == '64':
    base_checker = SixtyFourChecker(apex_provider)
  else:
