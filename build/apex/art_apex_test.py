  ...
  def print_list(self):
    ...
    def print_list_rec(path):
      ...
      for (_, val) in sorted(apex_map.items()):
        ...
        val_size = "[%11d]  " % val.size if self._print_size else ""
        print('%s%s' % (val_size, val_path))
        if val.is_dir:
  ...
  def print_tree(self):
    ...
    def print_tree_rec(path):
      ...
      for i, key in enumerate(key_list):
        ...
        val = apex_map[key]
        size = "[%11d]  " % val.size if self._print_size else ""
        print('%s%s%s%s' % (prev, last, size, val.name))
        if val.is_dir:
