...
class Checker:
  ...
  def check_file(self, path):
    chk = self.is_file(path)
    if not chk[0]:
      self.fail(chk[1], path)
    self._expected_files.add(path)
    return chk[0]
  ...
  def check_single_library(self, filename):
    ...
    res1 = self.is_file(lib_path)
    if res1[0]:
      self._expected_files.add(lib_path)
    res2 = self.is_file(lib64_path)
    if res2[0]:
      self._expected_files.add(lib64_path)
  ...
  def check_no_superfluous_files(self, dir_path):
    dir_path += '/'
    expected_filenames = set(p[len(dir_path):].partition('/')[0]
                             for p in self._expected_files
  ...
  def check_64only_library(self, filename):
    raise NotImplementedError
...
class Arch64Checker(Checker):
  ...
  def check_both_bitness_binary(self, filename):
    # TODO(mast): Not sure about this - haven't seen a pure 64-bit package.
    self.check_binary('%s64' % filename)
...
class DebugTargetChecker:
  ...
  def run(self):
    ...
    # Check internal native library dependencies.
    self._checker.check_64only_library('libjsoncpp.so')
    self._checker.check_64only_library('libmeminfo.so')
    self._checker.check_64only_library('libprocessgroup.so')
    self._checker.check_64only_library('libprocinfo.so')
    self._checker.check_64only_library('libutils.so')
