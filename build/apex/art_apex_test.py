...
class FSObject:
  ...
  def __str__(self):
    return '%s(dir=%r,exec=%r,symlnk=%r)' % (self.name, self.is_dir, self.is_exec, self.is_symlink)
...
class Checker:
  ...
  def check_multilib_binary(self, file):
    res1 = self.check_binary('%s32' % (file))
    res2 = self.check_binary('%s64' % (file))
  ...
  def check_library(self, file):
    ...
    res1 = self.check_file('lib/%s' % (file))
    res2 = self.check_file('lib64/%s' % (file))
    return res1 and res2
...
class ReleaseChecker(Checker):
  ...
  def run(self):
    ...
    self.check_library('libjavacrypto.so')
...
def artApexTestMain(args):
  if not args.host and not args.target and not args.debug:
    logging.error("None of --host, --target and --debug set")
    return 1
