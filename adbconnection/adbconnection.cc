...
void FixLogfile(JdwpArgs& parameters) {
  ...
  if (logfile.front() == '/') {
    // We only fix logfile is it is not using an absolute path
    return;
  }

  art::AppInfo* appInfo = art::Runtime::Current()->GetAppInfo();
  if (!appInfo->HasRegisteredAppInfo()) {
    VLOG(jdwp) << "Unable to fix relative path logfile='" + logfile + "' without package name.";
    return;
  }
  //  art::Runtime::Current()->GetProcessPackageName()

  std::string packageName = appInfo->PackageName();
