...
void AdbConnectionDebuggerController::StartDebugger() {
  // TODO We should check this somewhere else. Avoid loading everything all the time.
...
void AdbConnectionState::StartDebuggerThreads() {
  // First set up all the files we need.
  ...
  // Setup the socketpair we use to talk to the agent (when it actually gets loaded).
...
bool AdbConnectionState::SetupAdbConnection() {
  ...
  while (!shutting_down_) {
    ...
    if (ret == 0) {
      ...
      ret = TEMP_FAILURE_RETRY(send(sock, buff, 4, 0));
      if (ret == 4) {
        VLOG(jdwp) << "PID " << getpid() << " send to adb. Control socket is "
                   << sock.get();
  ...
  while (!shutting_down_) {
    ...
    while (!shutting_down_ && control_sock_ != -1) {
      struct pollfd pollfds[4] = {
        ...
        // have a real connection yet or the socket to adb.
      ...
      if (FlagsSet(agent_control_sock_poll.revents, POLLIN)) {
        ...
        if (!agent_loaded_) {
          ...
          if (self->IsExceptionPending()) {
            LOG(ERROR) << "Failed to load agent " << kJdwpAgentName;
            art::ScopedObjectAccess soa(self);
...
std::string AdbConnectionState::MakeAgentArg() {
  ...
  const std::string& agent_name = kJdwpAgentName;
  const std::string& opts = art::Runtime::Current()->GetJdwpOptions();
  return agent_name + "=" + opts + (opts.empty() ? "" : ",")
