  ...
  while (!shutting_down_) {
    ...
    while (!shutting_down_ && control_ctx_) {
      ...
      struct pollfd pollfds[4] = {
          ...
          // Check for the control_sock_ actually going away. If it does we need to re-establish the
          // communication with adbd. We also also monitor for POLLIN, even if we already have an
          // adb socket. This allows to reject incoming debugger connection if there is already have
          // one connected.
