...
AdbApexProcessName apex_adbconnection_client_on_process_named = nullptr;
using AdbApexPackageName = void (*)(const char*);
AdbApexPackageName apex_adbconnection_client_on_application_added = nullptr;
using AdbApexWaitingForDebugger = void (*)(bool);
AdbApexWaitingForDebugger apex_adbconnection_client_on_waiting_for_debugger = nullptr;
...
using AdbApexOnUserIdKnown = void (*)(int);
AdbApexOnUserIdKnown apex_adbconnection_client_on_user_id_known = nullptr;

void apex_adbconnection_client_on_process_named_noop(const char*) {}

void apex_adbconnection_client_on_application_added_noop(const char*) {}

void apex_adbconnection_client_on_waiting_for_debugger_noop(bool) {}
...
void apex_adbconnection_client_on_user_id_known_noop(int) {}

static void RetrieveApexPointers() {
  apex_adbconnection_client_on_process_named =
      (AdbApexProcessName)dlsym(RTLD_DEFAULT, "adbconnection_client_on_process_named");
  if (!apex_adbconnection_client_on_process_named) {
    VLOG(jdwp) << "Unable to dlsym adbconnection_client_on_process_named";
    apex_adbconnection_client_on_process_named = apex_adbconnection_client_on_process_named_noop;
  }

  apex_adbconnection_client_on_application_added =
      (AdbApexPackageName)dlsym(RTLD_DEFAULT, "adbconnection_client_on_application_added");
  if (!apex_adbconnection_client_on_application_added) {
    VLOG(jdwp) << "Unable to dlsym adbconnection_client_on_application_added";
    apex_adbconnection_client_on_application_added =
        apex_adbconnection_client_on_application_added_noop;
  }

  apex_adbconnection_client_on_waiting_for_debugger = (AdbApexWaitingForDebugger)dlsym(
      RTLD_DEFAULT, "adbconnection_client_on_waiting_for_debugger");
  if (!apex_adbconnection_client_on_waiting_for_debugger) {
    VLOG(jdwp) << "Unable to dlsym adbconnection_client_on_waiting_for_debuffer";
    apex_adbconnection_client_on_waiting_for_debugger =
        apex_adbconnection_client_on_waiting_for_debugger_noop;
  }
  ...
  apex_adbconnection_client_on_user_id_known =
      (AdbApexOnUserIdKnown)dlsym(RTLD_DEFAULT, "adbconnection_client_on_user_id_known");
  if (!apex_adbconnection_client_on_user_id_known) {
    VLOG(jdwp) << "Unable to dlsym adbconnection_client_on_user_id_known";
    apex_adbconnection_client_on_user_id_known = apex_adbconnection_client_on_user_id_known_noop;
  }
}
...
void AdbConnectionAppInfoCallback::OnProcessNamed(const std::string& process_name) {
  connection_->OnProcessNamed(process_name);
}

void AdbConnectionAppInfoCallback::OnApplicationAdded(const std::string& package_name) {
  connection_->OnApplicationAdded(package_name);
}

void AdbConnectionAppInfoCallback::OnWaitingForDebugger(bool waiting) {
  connection_->OnWaitingForDebugger(waiting);
}

void AdbConnectionAppInfoCallback::OnUserIdKnown(int user_id) {
  connection_->OnUserIdKnown(user_id);
}
...
void AdbConnectionState::OnProcessNamed(const std::string& process_name) {
  if (!IsDebuggableOrProfilable()) {
    return;
  }

  VLOG(jdwp) << "OnProcessNamed '" << process_name << "'";
  apex_adbconnection_client_on_process_named(process_name.c_str());

  if (!control_ctx_) {
    VLOG(jdwp) << "OnProcessNamed: No context";
    return;
  }
  WakeupPollLoop();
}

void AdbConnectionState::OnApplicationAdded(const std::string& package_name) {
  if (!IsDebuggableOrProfilable()) {
    return;
  }

  VLOG(jdwp) << "OnApplicationAdded '" << package_name << "'";
  apex_adbconnection_client_on_application_added(package_name.c_str());

  if (!control_ctx_) {
    VLOG(jdwp) << "OnApplicationAdded: No context";
    return;
  }
  WakeupPollLoop();
}

void AdbConnectionState::OnWaitingForDebugger(bool waiting) {
  if (!IsDebuggableOrProfilable()) {
    return;
  }

  VLOG(jdwp) << "OnWaitingForDebugger'" << waiting << "'";
  apex_adbconnection_client_on_waiting_for_debugger(waiting);

  if (!control_ctx_) {
    VLOG(jdwp) << "OnWaitingForDebugger: No context";
    return;
  }
  WakeupPollLoop();
}

void AdbConnectionState::OnUserIdKnown(int user_id) {
  if (!IsDebuggableOrProfilable()) {
    return;
  }

  VLOG(jdwp) << "OnUserIdKnown '" << user_id << "'";
  apex_adbconnection_client_on_user_id_known(user_id);

  if (!control_ctx_) {
    VLOG(jdwp) << "OnUserIdKnown: No context";
    return;
  }
  WakeupPollLoop();
}
  ...
  while (!shutting_down_) {
    ...
    while (!shutting_down_ && control_ctx_) {
      ...
      if (FlagsSet(agent_control_sock_poll.revents, POLLIN)) {
        ...
        // Poll was awakened via fdevent, we need to decrease the counter (ack)
        AcknowledgeWakeup();
      } else {
