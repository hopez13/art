...
using AdbApexSetState = void (*)(int);
AdbApexSetState apex_adbconnection_client_on_set_state = nullptr;
...
void apex_adbconnection_client_on_set_state_noop(int) {}
...
static void RetrieveApexPointers() {
  ...
  apex_adbconnection_client_on_set_state =
      (AdbApexSetState)dlsym(RTLD_DEFAULT, "adbconnection_client_on_set_state");
  if (!apex_adbconnection_client_on_set_state) {
    VLOG(jdwp) << "Unable to dlsym adbconnection_client_on_set_state";
    apex_adbconnection_client_on_set_state = apex_adbconnection_client_on_set_state_noop;
  }
...
void AdbConnectionAppInfoCallback::OnSetstate(int state) { connection_->OnSetstate(state); }
...
void AdbConnectionState::OnSetstate(int state) {
  if (!IsDebuggableOrProfilable()) {
    return;
  }

  VLOG(jdwp) << "OnSetstate '" << state << "'";
  apex_adbconnection_client_on_set_state(state);

  if (!control_ctx_) {
    VLOG(jdwp) << "OnSetstate: No context";
    return;
  }
  WakeupPollLoop();
}
...
bool AdbConnectionState::SetupAdbConnection() {
  ...
  while (!shutting_down_) {
    ...
    VLOG(jdwp) << "Creating adbconnection context";
