...
void AdbConnectionState::HandleDataWithoutAgent(art::Thread* self) {
  ...
  int res = TEMP_FAILURE_RETRY(recv(adb_connection_socket_.get(),
                                    packet_header,
  ...
  if (res < static_cast<int>(kPacketHeaderLen)) {
    PLOG(ERROR) << "Unable to peek into adb socket. Loading agent to handle this.";
...
void AdbConnectionState::PerformHandshake() {
  ...
  if (res < 0 || avail < sizeof(kJdwpHandshake)) {
    if (res < 0) {
      PLOG(ERROR) << "Failed to ioctl(, FIONREAD,)!";
    }
