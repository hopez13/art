  ...
  while (!shutting_down_) {
    ...
    while (!shutting_down_ && control_ctx_) {
      ...
      struct pollfd pollfds[4] = {
          ...
          // communication with adbd. We alwaways monitor for POLLIN, even if we already have an
          // adbd socket. This allows to reject incoming debugger connection if there is already
          // have one connected.
