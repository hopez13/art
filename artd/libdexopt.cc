...
bool AddBootClasspathFds(/*inout*/ std::vector<std::string>& cmdline,
...
bool AddDex2oatArgsFromBcpExtensionArgs(const DexoptBcpExtArgs& args,
  ...
  if (args.dirtyImageObjectsFd >= 0) {
    cmdline.emplace_back(android::base::StringPrintf("--dirty-image-objects-fd=%d", args.dirtyImageObjectsFd));
  }
  ...
  if (!AddBootClasspathFds(cmdline, args.bootClasspathFds)) {
    return false;
  }
...
bool AddDex2oatArgsFromSystemServerArgs(const DexoptSystemServerArgs& args,
                                        /*out*/ std::vector<std::string>& cmdline) {
  ...
  if (args.compilerFilter == CompilerFilter::SPEED_PROFILE && args.profileFd >= 0) {
    ...
    cmdline.emplace_back("--compiler-filter=" + std::string(CompilerFilterAidlToString(args.compilerFilter)));
  }
  ...
  if (args.updatableBcpPackagesTxtFd >= 0) {
    cmdline.emplace_back(android::base::StringPrintf("--updatable-bcp-packages-fd=%d", args.updatableBcpPackagesTxtFd));
  }
  ...
  if (args.classloaderFds.empty()) {
    ...
    cmdline.emplace_back("--class-loader-context-fds=" + android::base::Join(args.classloaderFds, ':'));
  }
  ...
  if (iter == jar_paths.end()) {
    LOG(ERROR) << "Missing BCP extension compatible JAR";
    return false;
  }
  ...
  if (extension_image.empty()) {
    LOG(ERROR) << "Can't identify the first boot extension compatible jar";
    return false;
  }
