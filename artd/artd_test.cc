...
class ArtdPreRebootTest : public CommonArtTest {
 protected:
  void SetUp() override {
    CommonArtTest::SetUp();
  ...
  MockExecUtils* mock_exec_utils_;
  MockFunction<int(const char*, const char*, const char*, unsigned long, const void*)> mock_mount_;
};
...
TEST_F(ArtdPreRebootTest, preRebootInit) {
  ...
  ASSERT_TRUE(WriteStringToFile(ART_FORMAT(kInitEnvironRcTmpl, art_root_), init_environ_rc_path_));
