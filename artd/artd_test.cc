 ...
 protected:
  ...
  RunCopyAndRewriteProfileResult<kExpectOk> RunCopyAndRewriteProfile() {
    ...
    ndk::ScopedAStatus status =
        artd_->copyAndRewriteProfile(*profile_path_, &dst, dex_file_, &result);
...
TEST_F(ArtdTest, copyAndRewriteProfileSuccess) {
  std::string src_file = OR_FATAL(BuildProfileOrDmPath(*profile_path_));
...
TEST_F(ArtdTest, mergeProfiles) {
  std::string reference_profile_file = OR_FATAL(BuildProfileOrDmPath(*profile_path_));
