 ...
 */
ADexoptContext* ADexopt_CreateAndValidateDexoptContext(const uint8_t* marshaled, size_t size)
        __INTRODUCED_IN(33);
 ...
 *
 * @param context An opaque object returned by ADexopt_CreateAndValidateDexoptContext.
 * @return an array of pointers terminated by null. Each pointer points to a C string (ASCII-only)
 *         living in context, so the client doesn't need to manage the lifetime.
 *
 * Available since API level 33.
 */
