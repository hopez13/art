...
Result<void> SetFileCon(const std::string& path, const char* con) {
  if (setfilecon(path.c_str(), con) != 0) {
    return ErrnoErrorf("Could not set label on '{}'", path);
  }
  return {};
}
...
ScopedAStatus Artd::commitPreRebootStagedFiles(const std::vector<ArtifactsPath>& in_artifacts,
  ...
  auto move = [](const std::string& src, const std::string& dst) -> Result<void> {
    ...
    if (ec && ec.value() != ENOENT) {
      return Errorf("Failed to move new file '{}' to path '{}': {}", src, dst, ec.message());
    }
    return {};
  };
...
Result<void> Artd::PreRebootMountArtApexData() {
  std::string art_apex_data_dir = "/mnt/artd_tmp/art_apex_data";
  ...
  // TODO(jiakaiz): Investigate why Restorecon doesn't work.
  OR_RETURN(SetFileCon(art_apex_data_dir, "u:object_r:apex_art_data_file:s0"));
  return {};
}

Result<void> Artd::PreRebootInitBootImage() {
  // TODO(jiakaiz): Avoid writing to /data/misc/odrefresh/compilation-log.txt.
  CmdlineBuilder args;
  args.Add(OR_RETURN(GetArtExec()))
      .Add("--drop-capabilities")
      .Add("--")
