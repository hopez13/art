...
ScopedAStatus Artd::checkPreRebootSystemRequirements(const std::string& in_chrootDir,
  ...
  if (new_release - old_release >= 2) {
    ...
    // can't run Pre-reboot Dexopt, but the chance that we get random crashes increases. We give up
    // early in order to avoid receiving too many random crash reports.
    LOG(WARNING) << ART_FORMAT(
