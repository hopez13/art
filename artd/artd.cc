...
ScopedAStatus Artd::preRebootInit() {
  ...
  bool preparation_done = OS::FileExists("/mnt/artd_tmp/preparation_done");

  OR_RETURN_NON_FATAL(PreRebootInitSetEnvFromFile("/init.environ.rc"));
  if (!preparation_done) {
    OR_RETURN_NON_FATAL(PreRebootInitDeriveClasspath());
  }
  OR_RETURN_NON_FATAL(PreRebootInitSetEnvFromFile("/mnt/artd_tmp/classpath"));
  OR_RETURN_NON_FATAL(PreRebootMountArtApexData());
  if (!preparation_done) {
    OR_RETURN_NON_FATAL(PreRebootMountOdrefresh());
    OR_RETURN_NON_FATAL(PreRebootInitBootImage());
  }

  if (!WriteStringToFile(/*content=*/"", "/mnt/artd_tmp/preparation_done")) {
    return NonFatal(
        ART_FORMAT("Failed to write '/mnt/artd_tmp/preparation_done': {}", strerror(errno)));
  }
...
Result<void> Artd::PreRebootInitDeriveClasspath() {
  std::unique_ptr<File> output(OS::CreateEmptyFile("/mnt/artd_tmp/classpath"));
  if (output == nullptr) {
    return ErrnoErrorf("Failed to create '/mnt/artd_tmp/classpath'");
  }
  ...
  if (output->FlushClose() != 0) {
    return ErrnoErrorf("Failed to flush and close '/mnt/artd_tmp/classpath'");
  }

  return {};
}

Result<void> Artd::PreRebootMountArtApexData() {
  std::string src_art_apex_data_dir = "/mnt/artd_tmp/art_apex_data";
  std::string dst_art_apex_data_dir = GetArtApexData();
  OR_RETURN(CreateDir(src_art_apex_data_dir));
  if (mount(src_art_apex_data_dir.c_str(),
            dst_art_apex_data_dir.c_str(),
            /*fs_type=*/nullptr,
            MS_BIND,
            /*data=*/nullptr) != 0) {
    return ErrnoErrorf(
        "Failed to bind-mount '{}' at '{}'", src_art_apex_data_dir, dst_art_apex_data_dir);
  }
  Restorecon(dst_art_apex_data_dir, /*se_context=*/std::nullopt);
  return {};
}

Result<void> Artd::PreRebootMountOdrefresh() {
  std::string src_odrefresh_dir = "/mnt/artd_tmp/odrefresh";
  std::string dst_odrefresh_dir = "/data/misc/odrefresh";
  OR_RETURN(CreateDir(src_odrefresh_dir));
  if (mount(src_odrefresh_dir.c_str(),
            dst_odrefresh_dir.c_str(),
            /*fs_type=*/nullptr,
            MS_BIND,
            /*data=*/nullptr) != 0) {
    return ErrnoErrorf("Failed to bind-mount '{}' at '{}'", src_odrefresh_dir, dst_odrefresh_dir);
  }
  Restorecon(dst_odrefresh_dir, /*se_context=*/std::nullopt, /*recurse=*/false);
  return {};
}
...
Result<void> Artd::PreRebootInitBootImage() {
  ...
  args.Add(OR_RETURN(GetArtExec()))
      .Add("--drop-capabilities")
      .Add("--")
      .Add("/apex/com.android.art/bin/odrefresh")
