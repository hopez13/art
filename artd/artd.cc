...
Result<void> Artd::BindMountNewDir(const std::string& source, const std::string& target) const {
  ...
  if (ec) {
    return Errorf("Failed to remove existing dir '{}': {}", source, ec.message());
  }

  OR_RETURN(CreateDir(source));
  OR_RETURN(BindMount(source, target));
  OR_RETURN(Restorecon(target, /*se_context=*/std::nullopt, /*recurse=*/false));
...
ScopedAStatus Artd::preRebootInit() {
  ...
  // Create a new mount namespace as we don't need to share the mounts with anyone else.
  if (unshare(CLONE_NEWNS) != 0) {
    return NonFatal("Failed to unshare mount namespace");
  }

  std::string tmp_dir =
      options_.test_only_override_pre_reboot_tmp_dir.value_or(kDefaultPreRebootTmpDir);
  ...
  if (preparation_done) {
    OR_RETURN_NON_FATAL(BindMount(art_apex_data_dir, GetArtApexData()));
    OR_RETURN_NON_FATAL(BindMount(odrefresh_dir, "/data/misc/odrefresh"));
  } else {
