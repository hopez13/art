...
#define RETURN_FATAL_IF_PRE_REBOOT(options)                                      \
  if (options.is_pre_reboot) {                                                   \
    return Fatal("This method is only not supported in Pre-reboot Dexopt mode"); \
  }
...
#define RETURN_FATAL_IF_PRE_REBOOT_MISMATCH(options, expr, log_name)                         \
  auto&& __return_fatal_tmp = (expr);                                                        \
  if (options.is_pre_reboot != __return_fatal_tmp) {                                         \
    return Fatal(ART_FORMAT("Expected flag 'isPreReboot' in argument '{}' to be {}, got {}", \
                            log_name,                                                        \
                            options.is_pre_reboot,                                           \
                            __return_fatal_tmp));                                            \
  }
...
ndk::ScopedAStatus Artd::CopyAndRewriteProfileImpl(File src,
  ...
  RETURN_FATAL_IF_PRE_REBOOT_MISMATCH(
      options_, PreRebootFlag(dst_aidl->profilePath.finalPath), "dst");
...
ndk::ScopedAStatus Artd::commitTmpProfile(const TmpProfilePath& in_profile) {
  RETURN_FATAL_IF_PRE_REBOOT_MISMATCH(options_, PreRebootFlag(in_profile.finalPath), "dst");
...
ndk::ScopedAStatus Artd::mergeProfiles(const std::vector<ProfilePath>& in_profiles,
  ...
  RETURN_FATAL_IF_PRE_REBOOT_MISMATCH(
      options_, PreRebootFlag(in_outputProfile->profilePath.finalPath), "outputProfile");
...
ndk::ScopedAStatus Artd::dexopt(
  ...
  RETURN_FATAL_IF_PRE_REBOOT_MISMATCH(
      options_, in_outputArtifacts.artifactsPath.isPreReboot, "outputArtifacts");
...
ScopedAStatus Artd::commitPreRebootStagedFiles(
  ...
  // Only used as the storage of the string_view's.
  std::vector<std::unique_ptr<RawArtifactsPath>> artifacts_paths;
  std::vector<std::unique_ptr<std::string>> profile_paths;

  std::vector<std::pair<std::string_view, std::string_view>> files_to_move;
  std::vector<std::string_view> files_to_remove;
  ...
  for (const ArtifactsPath& artifacts : in_artifacts) {
    ...
    if (OS::FileExists(src_artifacts->oat_path.c_str())) {
      ...
      artifacts_paths.push_back(std::move(src_artifacts));
      artifacts_paths.push_back(std::move(dst_artifacts));
    }
  }
  for (const WritableProfilePath& profile : in_profiles) {
    ...
    if (OS::FileExists(src_profile->c_str())) {
      ...
      profile_paths.push_back(std::move(src_profile));
      profile_paths.push_back(std::move(dst_profile));
    }
  }
...
Result<void> Artd::PreRebootInitClearEnvs() {
  ...
  for (const std::string& key : keys) {
    if (unsetenv(key.c_str()) != 0) {
      return ErrnoErrorf("Failed to clear environment variable '{}'", key);
    }
  }
  return {};
}
...
Result<void> Artd::PreRebootInitSetEnvFromFile(const std::string& path) {
  ...
  for (const std::string& line : Split(content, "\n")) {
    if (line.find('\\') != std::string::npos) {
      return Errorf("Backslashes in env var file are not supported for now, got '{}'", line);
    }
    std::smatch match;
