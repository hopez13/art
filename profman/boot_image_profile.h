...
struct BootImageOptions {
 public:
  // Threshold for classes that may be dirty or clean.
  uint32_t image_class_theshold = std::numeric_limits<uint32_t>::max();

  // Threshold for classes that are likely to remain clean.
  uint32_t image_class_clean_theshold = std::numeric_limits<uint32_t>::max();

  // Threshold for non-hot methods to be compiled.
