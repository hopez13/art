...
TEST_F(ProfileAssistantTest, TestProfileCreateWithInvalidData) {
  ...
  dex::TypeIndex invalid_class(std::numeric_limits<uint16_t>::max() - 1);
  ASSERT_EQ(1u, dex_pc_data.classes.size());
  ASSERT_EQ(invalid_class, dex_pc_data.classes.begin()->type_index);
  ...
  ASSERT_TRUE(classes.find(invalid_class) != classes.end());
  ASSERT_EQ(2u, methods.size());
  uint16_t invalid_method = std::numeric_limits<uint16_t>::max() - 1;
  ASSERT_TRUE(methods.find(invalid_method) != methods.end());
}
