...
TEST_F(ProfileAssistantTest, CopyAndUpdateProfileKey) {
  ...
  for (uint16_t i = 0; i < num_methods_to_add; i ++) {
    ...
    ASSERT_FALSE(result.GetMethodHotness(MethodReference(dex_to_be_updated1, i)).IsHot());
    ASSERT_FALSE(result.GetMethodHotness(MethodReference(dex_to_be_updated2, i)).IsHot());
  }
}
