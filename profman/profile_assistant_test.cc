...
TEST_F(ProfileAssistantTest, BootImageMerge) {
  ...
  int return_code_2 = ProcessProfiles({profile1.GetFd(), profile2.GetFd(), profile3.GetFd()},
                                      output_profile.GetFd(),
                                      {"--force-merge", "--boot-image-merge"});
  ASSERT_EQ(return_code_2, ProfmanResult::kSuccess);

  // Verify the result: it should be equal to info2 union info3 since info1 is a regular profile
  // and should be ignored.
  ProfileCompilationInfo result2(/*for_boot_image=*/true);
  ASSERT_TRUE(result2.Load(output_profile.GetFd()));
  ASSERT_TRUE(info2.MergeWith(info3));
  ASSERT_TRUE(result2.Equals(info2));
}
...
TEST_F(ProfileAssistantTest, ForceMergeIgnoreProfilesItCannotLoad) {
  ...
  ProfileCompilationInfo result2(/*for_boot_image=*/true);
  ASSERT_TRUE(result2.Load(reference_profile_fd));
  ASSERT_TRUE(info2.Equals(result2));
