...
class ProfileAssistantTest : public CommonRuntimeTest {
 ...
 protected:
  ...
  ProfileCompilationInfo::InlineCacheMap* CreateInlineCacheMap() {
    used_inline_caches.emplace_back(
        new ProfileCompilationInfo::InlineCacheMap(std::less<uint16_t>(), arena_->Adapter(kArenaAllocProfile)));
  ...
  // This make it easier to pass data between different utilities and ensure that the
