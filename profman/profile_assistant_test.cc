...
TEST_F(ProfileAssistantTest, MergeProfilesWithCounters) {
  ...
  for (uint16_t i = 0; i < kNumberOfMethodsToEnableCompilation; i++) {
    ASSERT_EQ(2, result.GetMethodAggregationCounter(MethodReference(&d1, i)));
    ASSERT_EQ(2, result.GetMethodAggregationCounter(MethodReference(&d2, i)));
  }
  for (uint16_t i = 0; i < kNumberOfClasses; i++) {
    ASSERT_EQ(2, result.GetClassAggregationCounter(TypeReference(&d1, dex::TypeIndex(i))));
    ASSERT_EQ(2, result.GetClassAggregationCounter(TypeReference(&d2, dex::TypeIndex(i))));
  }
}
