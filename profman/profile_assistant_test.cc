...
class ProfileAssistantTest : public CommonRuntimeTest {
 ...
 protected:
  ...
  bool RunProfman(const std::string& filename,
    ...
    argv_str.push_back(std::string("--apk=").append(target));
    argv_str.push_back(std::string("--dex-location=").append(target));
  ...
  bool DumpClassesAndMethods(const std::string& filename, std::string* file_contents, std::optional<const std::string_view> target = std::nullopt) {
    std::vector<std::string> extra_args;
    extra_args.push_back("--dump-classes-and-methods");
    return RunProfman(filename, extra_args, file_contents, target.value_or(GetLibCoreDexFileNames()[0]));
  }
  ...
  bool CreateAndDump(const std::string& input_file_contents,
                     std::string* output_file_contents, std::optional<const std::string> target = std::nullopt) {
  ...
  uint16_t GetDexPcOfCallTo(ArtMethod* meth, Handle<mirror::Class> klass, int32_t start_pc = -1)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    const DexFile* dex_file = meth->GetDexFile();
    for (const DexInstructionPcPair& inst :
         CodeItemInstructionAccessor(*dex_file, meth->GetCodeItem())) {
      if (static_cast<int32_t>(inst.DexPc()) <= start_pc) {
        continue;
      } else if (inst->IsInvoke()) {
        const dex::MethodId& meth_id = dex_file->GetMethodId(inst->VRegB());
        std::string_view desc(dex_file->GetTypeDescriptor(dex_file->GetTypeId(meth_id.class_idx_)));
        std::string scratch;
    ...
    EXPECT_TRUE(false) << "Unable to find dex-pc in " << meth->PrettyMethod() << " for call to "
                       << klass->PrettyClass() << " after dexpc: " << start_pc;
    return -1;
  }
...
TEST_F(ProfileAssistantTest, TestProfileRoundTrip) {
  ...
  std::string text_two;
  ASSERT_TRUE(DumpClassesAndMethods(profile_file.GetFilename(), &text_two, GetTestDexFileName("ProfileTestMultiDex")));
