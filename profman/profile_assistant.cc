...
ProfmanResult::ProcessingResult ProfileAssistant::ProcessProfilesInternal(
  ...
  if (!options.IsForceMerge()) {
    ...
    if (options.IsForceMergeAndAnalyze()) {
      // Return `kSkipCompilationSmallDelta` if the merge didn't change things (i.e., the classes
      // and methods in the reference profile was already a superset of those in all current
      // profiles before the merge.)
