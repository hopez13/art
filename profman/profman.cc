...
NO_RETURN static void Usage(const char *fmt, ...) {
  ...
  UsageError("      to include a class in the boot image profile");
  UsageError("  --boot-image-clean-class-threshold=<value>: specify minimum number of clean class");
  UsageError("      occurrences to include a class in the boot image profile");
  UsageError("");
...
class ProfMan FINAL {
 public:
  ...
  std::unique_ptr<const ProfileCompilationInfo> LoadProfile(const std::string& filename, int fd) {
    if (!filename.empty()) {
      ...
      if (fd < 0) {
        ...
        return std::unique_ptr<const ProfileCompilationInfo>();
      }
    }
  ...
  int CreateBootProfile() {
    ...
    GenerateBootImageProfile(&out_profile, dex_files, profiles, boot_image_options_);
