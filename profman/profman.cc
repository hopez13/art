...
class ProfMan FINAL {
 public:
  ...
  bool FindClass(const std::vector<std::unique_ptr<const DexFile>>& dex_files,
    ...
    uint16_t max_valid_type_id = std::numeric_limits<uint16_t>::max() - 1;
    for (const std::unique_ptr<const DexFile>& dex_file_ptr : dex_files) {
      ...
      if (klass_descriptor == kInvalidClassDescriptor) {
        if (max_valid_type_id > dex_file->NumTypeIds()) {
          // The dex file still has space to generate an invalid type id.
          // Assign the max valid type id to the class reference.
          class_ref->dex_file = dex_file;
          class_ref->type_index = dex::TypeIndex(max_valid_type_id);
          return true;
        } else {
          // The dex file contains all possible type ids. We can't generate an
          // invalid one for it.
          continue;
        }
      }
  ...
  uint32_t FindMethodIndex(const ProfileMethodInfo::ProfileClassReference& class_ref,
    ...
    if (method_spec == kInvalidMethod) {
      uint16_t last_valid_method_index = std::numeric_limits<uint16_t>::max() - 1;
      return last_valid_method_index > dex_file->NumMethodIds()
             ? last_valid_method_index
             : DexFile::kDexNoIndex;
    }
