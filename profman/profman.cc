...
NO_RETURN static void Usage(const char *fmt, ...) {
  ...
  UsageError("");
  UsageError("  --create-profile-from=<filename>: creates profile from a list of classes/methods.");
  UsageError("");
  UsageError("");
...
class ProfMan FINAL {
 public:
  ...
  void OpenApkFilesFromLocations(std::vector<const DexFile*>* dex_files) {
    bool use_apk_fd_list = !apks_fd_.empty();
    if (use_apk_fd_list) {
      assert(apk_files_.empty());
      assert(dex_locations_.size() == apks_fd_.size());
    } else {
      assert(dex_locations_.size() == apks_files_.size());
      assert(!apk_files_.empty());
    }
  ...
  int CreateProfile() {
    ...
    if (!FdIsValid(fd)) {
      std::cerr << "Opening: " << reference_profile_file_ << std::endl;
      assert(!reference_profile_file_.empty());
      fd = open(reference_profile_file_.c_str(), O_CREAT | O_TRUNC | O_WRONLY, 0644);
      if (fd < 0) {
        std::cerr << "Cannot open " << reference_profile_file_ << strerror(errno);
        return -1;
      }
    }

    // Read the list of classes.
    std::unique_ptr<std::unordered_set<std::string>> profiled_classes;
    profiled_classes.reset(ReadCommentedInputFromFile<std::unordered_set<std::string>>(
        create_profile_from_file_.c_str(),
        nullptr));          // No post-processing.

    // Open the dex files to lookup class names.
    std::vector<const DexFile*> dex_files;
    OpenApkFilesFromLocations(&dex_files);
    ...
    for (const DexFile* dex_file : dex_files) {
      // Insert the DexCacheResolved Classes into the set expected for AddMethodsAndClasses.
      ...
      for (const auto& klass : *profiled_classes) {
        const DexFile::TypeId* type_id = dex_file->FindTypeId(klass.c_str());
        if (type_id == nullptr) {
          continue;
        }
        dex_resolved_classes->AddClass(dex_file->GetIndexForTypeId(*type_id));
      }
    }

#ifdef DCS
    for (const auto& elt : resolved_class_set) {
      const std::unordered_set<dex::TypeIndex>& indexes = elt.GetClasses();
      std::cerr << "ELT =================" << indexes.size() << std::endl;
    }
#endif  // DCS
