...
class ProfMan final {
 public:
  ...
  std::string GetInlineCacheLine(const ProfileCompilationInfo& profile_info,
    ...
    auto meth_info = profile_info.GetHotMethodInfo(MethodReference(dex_file, dex_method_idx));
    if (meth_info != nullptr && !meth_info->inline_caches->empty()) {
      const ProfileCompilationInfo::InlineCacheMap* inline_caches = meth_info->inline_caches;
      struct IcLineInfo {
      ...
      for (const auto& [pc, dat] : *inline_caches) {
        ...
        if (dat.is_megamorphic) {
          val->second.is_megamorphic_ = true;
        }
        if (dat.is_missing_types) {
          val->second.is_missing_types_ = true;
        }
        for (auto cls : dat.classes) {
          auto it = std::find_if(dex_files->begin(), dex_files->end(), [&](const auto& d) {
            return meth_info->dex_references[cls.dex_profile_index].MatchesDex(&*d);
          });
          if (it == dex_files->end()) {
            val->second.is_missing_types_ = true;
            continue;
      ...
      if (profile_info.GetClassesAndMethods(*dex_file.get(),
        ...
        for (uint16_t dex_method_idx : combined_methods) {
          ...
          std::string inline_cache_string =
              GetInlineCacheLine(profile_info, dex_files, id, &*dex_file, dex_method_idx);
  ...
  int DumpClassesAndMethods() {
    ...
    if (!FdIsValid(dump_output_to_fd_)) {
      ...
      LOG(ERROR) << "Dumping " << dump << " to " << dump_output_to_fd_;
      unix_file::FdFile out_fd(dump_output_to_fd_, /*check_usage=*/ false);
      if (!out_fd.WriteFully(dump.c_str(), dump.length())) {
        return -1;
      }
