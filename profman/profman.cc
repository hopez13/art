...
NO_RETURN static void Usage(const char *fmt, ...) {
  ...
  UsageError("  --boot-jars: treat APKs as boot class path JARs");
  UsageError("");
 ...
 public:
  ...
  void ParseArgs(int argc, char **argv) {
    ...
    for (int i = 0; i < argc; ++i) {
      ...
      } else if (option == "--boot-jars") {
        open_apks_as_boot_jars_ = true;
  ...
  bool ShouldOpenApksAsBootJars() const {
    return open_apks_as_boot_jars_;
  }
    ...
    for (size_t i = 0; i < dex_locations_.size(); ++i) {
      ...
      if (use_apk_fd_list) {
        if (dex_file_loader.OpenZip(apks_fd_[i],
                                    dex_locations_[i],
                                    /* verify */ !ShouldOpenApksAsBootJars(),
                                    kVerifyChecksum,
                                    &error_msg,
