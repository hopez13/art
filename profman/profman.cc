 ...
 public:
  ...
  bool ProcessLine(const std::vector<std::unique_ptr<const DexFile>>& dex_files,
    ...
    if (method_str.empty() || method_str == kClassAllMethods) {
      ...
      profile->AddMethods(methods, static_cast<Hotness::Flag>(flags));
    ...
    if (is_hot) {
      profile->AddMethod(ProfileMethodInfo(ref, inline_caches), static_cast<Hotness::Flag>(flags));
    }
    if (flags != 0) {
