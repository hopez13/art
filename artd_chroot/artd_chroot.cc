...
Result<void> ArtdChroot::SetUpChroot() const {
  ...
  std::string env_path = "/init.environ.rc";
  std::error_code ec;
  std::filesystem::copy(env_path, kChrootDir + env_path, ec);
  if (ec) {
    return Errorf("Failed to copy '{}' to '{}': {}", env_path, kChrootDir + env_path, ec.message());
  }
  ...
  std::vector<std::string> bind_mount_srcs = {
      ...
      "/metadata",
      // For libprecessgroup to load task profiles.
      "/etc",
  };
