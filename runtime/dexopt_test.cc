...
// Generate an oat file for the given dex location in the oat location (under
// the dalvik cache) with the following configuration:
//   filter - controls the compilation filter
//   pic - whether or not the code will be PIC
//   relocate - if true dalvik cache will be temporarily moved so dex2oat
//      doesn't find the
//   with_alternate_image - if true dex2oat will use the image from GetImageLocation2
//      otherwise it will use GetImageLocation.
