...
void DexoptTest::GenerateOatForTest(const std::string& dex_location,
  ...
  std::string boot_image_checksums = gc::space::ImageSpace::GetBootImageChecksums(
  ...
  if (CompilerFilter::DependsOnImageChecksum(filter)) {
    const char* checksums = oat_header.GetStoreValueByKey(OatHeader::kBootImageChecksumsKey);
