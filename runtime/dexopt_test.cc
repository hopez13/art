...
void DexoptTest::GenerateOatForTest(const std::string& dex_location,
  ...
  std::string boot_image_checksums =
      gc::space::ImageSpace::GetBootImageChecksums(image_location, kRuntimeISA, &error_msg);
  ASSERT_FALSE(boot_image_checksums.empty()) << error_msg;

  const OatHeader& oat_header = odex_file->GetOatHeader();

  if (CompilerFilter::DependsOnImageChecksum(filter)) {
    const char* checksums = oat_header.GetStoreValueByKey(OatHeader::kBootImageChecksumKey);
