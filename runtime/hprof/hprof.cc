...
class Hprof : public SingleRootVisitor {
 ...
 private:
  ...
  void DumpFakeArray(mirror::Object* obj, const std::set<mirror::Object*>& roots)
      REQUIRES_SHARED(Locks::mutator_lock_);
...
class NativeRootVisitor {
 ...
 private:
  // Fake roots that are actually GcRoots held live from the object. These are deduplicated. Avoid
  // marking them as roots in hprof to make it easier to debug class unloading.
...
void Hprof::DumpFakeArray(mirror::Object* obj, const std::set<mirror::Object*>& roots) {
...
void Hprof::DumpHeapInstanceObject(mirror::Object* obj,
  ...
  if (string_value != nullptr) {
    ...
    DumpFakeArray(fake_object_array, fake_roots);
  }
}
