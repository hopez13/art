...
class DdmEndianOuputput FINAL : public EndianOutputBuffered {
 public:
  DdmEndianOuputput(std::vector<uint8_t>& data, size_t reserved_size)
...
class Hprof : public SingleRootVisitor {
 ...
 private:
  ...
  bool DumpToDdmsDirect(size_t overall_size, size_t max_length, uint32_t chunk_type)
    ...
    // TODO It would be really good to have some streaming thing again.
    DdmEndianOuputput output(out_data, max_length);
    output_ = &output;
