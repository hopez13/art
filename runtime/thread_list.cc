...
static constexpr unsigned kSuspendBarrierIters = 20;
...
static bool WaitOnceForSuspendBarrier(AtomicInteger* barrier,
  ...
  if (kShortSuspendTimeouts) {
    timeout_ns = std::min(MsToNs(kSuspendBarrierIters), timeout_ns / 2'000);
    ...
    if (attempt_of_4 == 3) {
      ...
      if (culprit == nullptr) {
        ...
        std::string thr_descr =
            StringPrintf("tid: %d, state&flags: 0x%x, priority: %d,  barrier value: %d, ",
                         tid,
...
bool ThreadList::SuspendThread(Thread* self,
  ...
  while (!is_suspended) {
    if (attempt_of_4 > 0 && attempt_of_4 < 4) {
      // Caller will try again. Give up and resume the thread for now.  We need to make make sure
      ...
      bool resumed = Resume(thread, reason);
      CHECK(resumed);
      return false;
    }
    std::string name;
