 ...
 public:
  ...
  void Run(Thread* thread) override {
    ...
    int print_order;
    {
      ...
      print_order = thread->Dump(local_os, unwinder_, dump_native_stack_);
    }
    {
      MutexLock mu(self, os_lock_);
      // Sort, so that the most interesting threads for ANR are printed first (ANRs can be trimmed).
      std::pair<int, uint32_t> sort_key(print_order, thread->GetThreadId());
      bool ok = os_.emplace(sort_key, std::move(local_os)).second;
      CHECK(ok);
    }
    barrier_.Pass(self);
  }
