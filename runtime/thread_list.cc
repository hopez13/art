 ...
 public:
  ...
  void WaitForThreadsToRunThroughCheckpoint(size_t threads_running_checkpoint) {
    ...
    if (timed_out) {
      ...
      LOG_S((kIsDebugBuild && (gAborting == 0)) ? ::android::base::FATAL : ::android::base::ERROR)
    ...
    if (LIKELY(cur_val > 0)) {
      ...
      if (futex(pending_threads.Address(), FUTEX_WAIT, cur_val, &wait_timeout, nullptr, 0) != 0) {
        ...
        if ((errno != EAGAIN) && (errno != EINTR)) {
          if (errno == ETIMEDOUT) {
            LOG_S(kIsDebugBuild ? ::android::base::FATAL : ::android::base::ERROR)
...
static void ThreadSuspendByPeerWarning(Thread* self,
  ...
  if (scoped_name_chars.c_str() == nullptr) {
      LOG_S(severity) << message << ": " << peer;
      env->ExceptionClear();
  } else {
      LOG_S(severity) << message << ": " << peer << ":" << scoped_name_chars.c_str();
  }
}
...
static void ThreadSuspendByThreadIdWarning(LogSeverity severity,
                                           const char* message,
                                           uint32_t thread_id) {
  LOG_S(severity) << StringPrintf("%s: %d", message, thread_id);
}
