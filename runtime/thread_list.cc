...
void ThreadList::FlipThreadRoots(Closure* thread_flip_visitor,
  ...
  // Try to run the closure on the other threads.
  {
    ...
    for (int i = 0; i < thread_count; ++i) {
      bool finished;
      flipping_threads[i]->EnsureFlipFunctionStarted(
          self, Thread::StateAndFlags(0), &exit_flags[i], &finished);
      if (finished) {
      ...
      if (LIKELY(TrackIfFalse(suspend_all_count_ == 0, &di.suspend_all_) &&
        ...
        for (const auto& thread : list_) {
          if (thread == self) {
            ...
            // before checking, it it checks kActiveSuspendBarrier after noticing kSuspendRequest,
            // it must notice kActiveSspendBarrier when it does. Thus it is guaranteed to
