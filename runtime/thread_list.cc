...
size_t ThreadList::FlipThreadRoots(Closure* thread_flip_visitor,
  ...
  std::vector<Thread*> flipping_threads;  // Includes us.
  {
    ...
    for (Thread* thread : list_) {
      ...
      flipping_threads.push_back(thread);
    }
  }
  if (pause_listener != nullptr) {
  ...
  // reentering a runnable state. We will also attempt to run the flip functions ourselves.
  // Exactly one of those attempts will succeed, and the target thread will not be able to reenter
  // a runnable state until one of them does.
...
void ThreadList::SuspendAllInternal(Thread* self, SuspendReason reason) {
  ...
  for (int iter_count = 1;; ++iter_count) {
    ...
    // to sleep and retry.
    CHECK_LT(iter_count, kMaxSuspendRetries) << di;
    {
      MutexLock mu2(self, *Locks::thread_suspend_count_lock_);
...
bool ThreadList::Resume(Thread* thread, SuspendReason reason) {
...

  {
    ...
    thread->DecrementSuspendCount(self, reason == SuspendReason::kForUserCode);
  }
...
Thread* ThreadList::SuspendThreadByPeer(jobject peer, SuspendReason reason) {
  ...
  for (int iter_count = 1;; ++iter_count) {
    ...
    CHECK_LT(iter_count, kMaxSuspendRetries);
    usleep(kThreadSuspendSleepUs);
  }
