...
void ThreadList::FlipThreadRoots(Closure* thread_flip_visitor,
  ...
  std::unique_ptr<ThreadExitFlag[]> exit_flags;
  {
    ...
    thread_count = flipping_threads.size();
    exit_flags.reset(new ThreadExitFlag[flipping_threads.size()]);
    for (int i = 0; i < thread_count; ++i) {
      flipping_threads[i]->NotifyOnThreadExit(&exit_flags[i]);
    }
  }
  ...
  // Any newly created threads here will only see to-space references.
  ResumeAll();
  ...
  // Try to run the closure on the other threads.
  {
    ...
    // Make sure all flips completed before we return.
    for (int i = 0; i < thread_count; ++i) {
