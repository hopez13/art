  ...
  while (true) {
    {
      ...
      if (LIKELY(suspend_all_count_ == 0 && (self == nullptr || self->GetSuspendCount() == 0)
                 && !in_flip)) {
    ...
    // We're in a bad state for initiating a suspend. The suspend barrier slot is already in use,
    // and/or proceeding risks deadlock. See b/31683379 for an explanation of the thread flip
    // condition. Note that in the event of a competing Suspend or SuspendAll, we are about
    // to be suspended anyway. We hold no locks, so it's safe to sleep and retry.
