...
Thread* ThreadList::SuspendThreadByPeer(jobject peer,
  ...
  *timed_out = false;
  ...
  while (true) {
    ...
    if (request_suspension && (thread_is_heap_task || kAlwaysForceGcFinishOnSuspend)) {
    ...
    Thread* thread;
    {
      ...
      }
      {
        ...
        ScopedMaybePreventGc smpc(self, thread_is_heap_task || kAlwaysForceGcFinishOnSuspend);
        MutexLock suspend_count_mu(self, *Locks::thread_suspend_count_lock_);
