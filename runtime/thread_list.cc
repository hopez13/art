...
void ThreadList::SuspendAll(const char* cause, bool long_suspend) {
  ...
  // Note that a flip function can be installed only while another thread is inside a SuspendAll,
  // and that's impossible here, because we only allow one at a time. No other thread can be
  // running the flip function because that requires Runnable state, and everything else is
  // suspended here.
  if (self->ReadFlag(ThreadFlag::kPendingFlipFunction)) {
    Thread::EnsureFlipFunctionStarted(self, self);
  }
