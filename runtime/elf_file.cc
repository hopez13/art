...
bool ElfFileImpl<ElfTypes>::Load(File* file,
  ...
  bool reserved = false;
  for (Elf_Word i = 0; i < GetProgramHeaderNum(); i++) {
    Elf_Phdr* program_header = GetProgramHeader(i);  // FIXME: Move the failure more out.
    ...
    if (!reserved) {
      ...
      DCHECK(reservation == nullptr || reservation->IsValid());
      if (reservation != nullptr && reservation->Size() < vaddr_size) {
        *error_msg = StringPrintf("Insufficient reservation for file '%s': %zu < %zu",
                                  file->GetPath().c_str(),
                                  reservation->Size(),
                                  vaddr_size);
        return false;
      ...
      base_address_ = reinterpret_cast<uint8_t*>(
          static_cast<uintptr_t>(reservation->Begin() - vaddr_begin));
...
bool ElfFile::GetLoadedAddressRange(/*out*/uint8_t** vaddr_begin,
                                    /*out*/size_t* vaddr_size,
                                    /*out*/std::string* error_msg) const {
  DELEGATE_TO_IMPL(GetLoadedAddressRange, vaddr_begin, vaddr_size, error_msg);
}
