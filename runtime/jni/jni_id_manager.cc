...
size_t JniIdManager::GetLinearSearchStartId<ArtMethod>(ReflectiveHandle<ArtMethod> m) {
  if (CanUseIdArrays(m)) {
    // If we are searching because we couldn't allocate because of defer
    // allocate scope, then we should only look
    // from deferred_allocation_method_id_start_. Once we exit the deferred
    // scope all these method ids will be updated to the id arrays in the
    // respective ClassExt objects.
    return deferred_allocation_method_id_start_;
  } else {
    // If we cannot use ID arrays, then the method can be anywhere in the list,
    // so look for it from the start
    return 1;
  }
}
