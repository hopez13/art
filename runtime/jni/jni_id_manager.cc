...
size_t GetIdOffset(ObjPtr<mirror::Class> k, ArtMethod* method, PointerSize pointer_size) {
  return k->GetMethodIdOffset(method, pointer_size);
}
...
template <typename ArtType> uintptr_t JniIdManager::EncodeGenericId(ArtType* t) {
  if (!Runtime::Current()->JniIdsAreIndicies()) {
...
void JniIdManager::EndDefer() {
  ...
  // This depends on the fact that the method_id_map_ and field_id_map_ are the ultimate source of
  // truth and no id is ever reused to be valid.
  // TODO This needs much better explainer of why there's no UB/races!
