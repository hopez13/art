...
bool EnsureIdsArray(Thread* self, ObjPtr<mirror::Class> k, ArtField* field) {
  ...
  if (Locks::mutator_lock_->IsExclusiveHeld(self)) {
    field->IsStatic() ? h_k->GetStaticFieldIds() : h_k->GetInstanceFieldIds();
  } else {
...
template <> std::string PrettyGeneric(ReflectiveHandle<ArtMethod> f) {
  return f->PrettyMethod();
}
template <> std::string PrettyGeneric(ArtField* f) {
  return f->PrettyField();
}
template <> std::string PrettyGeneric(ReflectiveHandle<ArtField> f) {
  return f->PrettyField();
}

// Checks if the field or method is obsolete.
template <typename ArtType> bool IsObsolete(ReflectiveHandle<ArtType> t) REQUIRES_SHARED(Locks::mutator_lock_);
template <> bool IsObsolete(ReflectiveHandle<ArtField> t ATTRIBUTE_UNUSED) {
  return false;
}
template <> bool IsObsolete(ReflectiveHandle<ArtMethod> t) {
  return t->IsObsolete();
}

// Get the canonical (non-copied) version of the field or method. Only relevant for methods.
template <typename ArtType> ArtType* Canonicalize(ReflectiveHandle<ArtType> t) REQUIRES_SHARED(Locks::mutator_lock_);
template <> ArtField* Canonicalize(ReflectiveHandle<ArtField> t) {
  return t.Get();
}
template <> ArtMethod* Canonicalize(ReflectiveHandle<ArtMethod> t) {
...
template <> uintptr_t JniIdManager::GetNextId<ArtField>(JniIdType type, ReflectiveHandle<ArtField> f) {
...
template <> uintptr_t JniIdManager::GetNextId<ArtMethod>(JniIdType type, ReflectiveHandle<ArtMethod> m) {
  if (LIKELY(type == JniIdType::kIndices)) {
...
template <> size_t JniIdManager::GetLinearSearchStartId<ArtField>(ReflectiveHandle<ArtField> t ATTRIBUTE_UNUSED) {
  return deferred_allocation_field_id_start_;
}

template <> size_t JniIdManager::GetLinearSearchStartId<ArtMethod>(ReflectiveHandle<ArtMethod> m) {
  if (m->IsObsolete()) {
    return 1;
  } else {
...
uintptr_t JniIdManager::EncodeGenericId(ReflectiveHandle<ArtType> t) {
  ...
  if (!ids.IsNull()) {
    ...
    if (found) {
      ...
      DCHECK_EQ(vec[index], t.Get()) << "Expected: " << PrettyGeneric(vec[index]) << " got "
...
void JniIdManager::VisitReflectiveTargets(ReflectiveValueVisitor *rvv) {
  ...
  for (auto it = field_id_map_.begin(); it != field_id_map_.end(); ++it) {
    ...
    ArtField* new_field = rvv->VisitField(old_field, JniIdReflectiveSourceInfo(reinterpret_cast<jfieldID>(id)));
  ...
  for (auto it = method_id_map_.begin(); it != method_id_map_.end(); ++it) {
    ...
    ArtMethod* new_method = rvv->VisitMethod(old_method, JniIdReflectiveSourceInfo(reinterpret_cast<jmethodID>(id)));
