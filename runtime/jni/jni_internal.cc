...
template <bool kCheckBad, typename GoodFunc, typename BadFunc>
size_t VisitUtf8Chars(const char* utf8, size_t byte_count, GoodFunc good, BadFunc bad) {
  ...
  while (utf8 != end) {
    ...
    if ((ic & 0x20) == 0) {
      // Two-byte encoding.
      if (kCheckBad && static_cast<size_t>(end - utf8) < 2u) {
        bad();
        return len + 1u;  // Reached end of sequence.
      }
      DCHECK_GE(static_cast<size_t>(end - utf8), 2u);
      good(utf8, 2u);
      utf8 += 2u;
      len += 1u;
      continue;
    ...
    if ((ic & 0x10) == 0) {
      // Three-byte encoding.
      if (kCheckBad && static_cast<size_t>(end - utf8) < 3u) {
        bad();
        return len + 1u;  // Reached end of sequence
      }
      DCHECK_GE(static_cast<size_t>(end - utf8), 3u);
      good(utf8, 3u);
      utf8 += 3u;
      len += 1u;
      continue;
    ...
    if (kCheckBad && static_cast<size_t>(end - utf8) < 4u) {
      bad();
      return len + 1u;  // Reached end of sequence.
    }
    DCHECK_GE(static_cast<size_t>(end - utf8), 4u);
    good(utf8, 4u);
    utf8 += 4u;
    len += 2u;
  }
 ...
 public:
  ...
  void operator()(ObjPtr<mirror::Object> obj, size_t usable_size ATTRIBUTE_UNUSED) const
    ...
    if (string->IsCompressed()) {
      ...
      if (UNLIKELY(has_bad_char_)) {
        VisitUtf8Chars</*kCheckBad=*/ true>(utf_, utf8_length_, good, bad);
      } else {
        VisitUtf8Chars</*kCheckBad=*/ false>(utf_, utf8_length_, good, bad);
      }
    } else {
      // Uncompressed.
...
#define SET_STATIC_PRIMITIVE_FIELD(fn, value) \
  ...
  // considered an extension of the the JNI specification.
  static jstring NewStringUTF(JNIEnv* env, const char* utf) {
    ...
    size_t utf16_length = VisitUtf8Chars</*kCheckBad=*/ true>(
        utf,
        utf8_length,
