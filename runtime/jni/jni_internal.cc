...
ALWAYS_INLINE static bool ShouldDenyAccessToMember(T* member, Thread* self)
  ...
  return hiddenapi::ShouldDenyAccessToMember(
      member,
      [&]() REQUIRES_SHARED(Locks::mutator_lock_) {
        return hiddenapi::IsCallingClassTrusted(
            GetCallingClass(self, /* num_frames */ 1), /* trust_if_null= */ true);
      },
...
static jmethodID FindMethodID(ScopedObjectAccess& soa, jclass jni_class,
  ...
  if (ShouldDenyAccessToMember(method, soa.Self())) {
    method = nullptr;
  }
...
static jfieldID FindFieldID(const ScopedObjectAccess& soa, jclass jni_class, const char* name,
  ...
  if (ShouldDenyAccessToMember(field, soa.Self())) {
    field = nullptr;
  }
  if (field == nullptr) {
    soa.Self()->ThrowNewExceptionF("Ljava/lang/NoSuchFieldError;",
