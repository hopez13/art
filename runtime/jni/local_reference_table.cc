...
void SmallLrtAllocator::Deallocate(LrtEntry* unneeded, size_t size) {
  ...
  while (index < kNumSlots) {
    ...
    if (index + 1u == kNumSlots && *before == other && *other == nullptr) {
      // Do not free the page if we do not have other allocated blocks with index `kNumSlots - 1`.
      break;
    }
...
void LocalReferenceTable::SetCheckJniEnabled(bool enabled) {
  free_entries_list_ = FirstFreeField::Update(kFreeListEnd, enabled ? 1u << kFlagCheckJni : 0u);
  if (enabled) {
    // Add non-CheckJNI holes up to the next serial number entry if needed.
    while (!IsAligned<kCheckJniEntriesPerReference>(segment_state_.top_index)) {
      GetEntry(segment_state_.top_index)->SetNextFree(GetFirstFreeIndex());
      free_entries_list_ = FirstFreeField::Update(segment_state_.top_index, 1u << kFlagCheckJni);
...
IndirectRef LocalReferenceTable::Add(LRTSegmentState previous_state,
  ...
  if (LIKELY(small_table != nullptr)) {
    ...
    if (LIKELY(free_entries_list_ == kEmptyFreeListAndCheckJniDisabled)) {
      ...
      if (first_free_index != kFreeListEnd && first_free_index >= previous_state.top_index) {
        ...
        // Use the `free_entry` only if it was not created with CheckJNI enabled.
  ...
  if (IsCheckJniEnabled()) {
    ...
    return store_obj(free_entry, "check-jni");
  }
...
bool LocalReferenceTable::Remove(LRTSegmentState previous_state, IndirectRef iref) {
  ...
  if (UNLIKELY(IsCheckJniEnabled())) {
    ...
    prune_chunk_size = kCheckJniEntriesPerReference;
  }
  if (entry_index >= top_index - prune_chunk_size) {
    // Top-most entry. Scan up and consume holes created with the same CheckJNI setting.
    constexpr uint32_t kDeadLocalValue = 0xdead10c0;
    entry->SetReference(reinterpret_cast32<mirror::Object*>(kDeadLocalValue));

    uint32_t prune_start;
    size_t prune_count;
    auto find_prune_range = [&](size_t chunk_size, auto is_prev_entry_free) {
      DCHECK_EQ(chunk_size, prune_chunk_size);
      size_t prune_end = top_index - chunk_size;
      prune_start = prune_end;
      while (prune_start > bottom_index && is_prev_entry_free(prune_start)) {
        prune_start -= prune_chunk_size;
      }
    ...
    if (UNLIKELY(IsCheckJniEnabled())) {
      auto is_prev_entry_free = [&](size_t index) {
        ...
        if (!serial_number_entry->GetSerialNumber()) {
          return false;
        }
...
void LocalReferenceTable::VisitRoots(RootVisitor* visitor, const RootInfo& root_info) {
  ...
  auto visit_table = [&](LrtEntry* table, size_t count, size_t chunk_size, auto get_entry)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    for (size_t i = 0; i != count; i += chunk_size) {
      LrtEntry* entry = get_entry(table, i);
      if (!entry->IsFree()) {
        GcRoot<mirror::Object>* ref = entry->GetRootAddress();
        DCHECK(!ref->IsNull());
        root_visitor.VisitRoot(*ref);
      }
    }
  };
  auto visit_tables = [&](size_t chunk_size, auto get_entry)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    size_t top_index = segment_state_.top_index;
    if (small_table_ != nullptr) {
      visit_table(small_table_, top_index, chunk_size, get_entry);
    } else {
      uint32_t remaining_capacity = top_index;
      size_t table_index = 0u;
      while (remaining_capacity != 0u) {
        size_t count = std::min<size_t>(remaining_capacity, GetTableSize(table_index));
        visit_table(tables_[table_index], count, chunk_size, get_entry);
        ++table_index;
  ...
  if (IsCheckJniEnabled()) {
    auto get_entry = [&](LrtEntry* table, size_t index) {
      LrtEntry* serial_number_entry = &table[index];
      DCHECK_ALIGNED(serial_number_entry, kCheckJniEntriesPerReference * sizeof(LrtEntry));
      uint32_t serial_number = serial_number_entry->GetSerialNumber();
      DCHECK(IsValidSerialNumber(serial_number));
      return serial_number_entry + serial_number;
    };
    visit_tables(kCheckJniEntriesPerReference, get_entry);
  } else {
    visit_tables(1u, [](LrtEntry* table, size_t index) { return &table[index]; });
  }
}
...
void LocalReferenceTable::Dump(std::ostream& os) const {
  ...
  auto fill = [&](size_t chunk_size, auto get_entry) REQUIRES_SHARED(Locks::mutator_lock_) {
    for (size_t i = 0, size = segment_state_.top_index; i != size; i += chunk_size) {
      LrtEntry* entry = get_entry(i);
      if (!entry->IsFree()) {
  ...
  if (IsCheckJniEnabled()) {
    auto get_entry = [&](size_t index) {
      LrtEntry* serial_number_entry = GetEntry(index);
      DCHECK_ALIGNED(serial_number_entry, kCheckJniEntriesPerReference * sizeof(LrtEntry));
      uint32_t serial_number = serial_number_entry->GetSerialNumber();
      DCHECK(IsValidSerialNumber(serial_number)) << index << " " << serial_number;
      LrtEntry* entry = serial_number_entry + serial_number;
      DCHECK_EQ(entry, GetEntry(index + serial_number));
      return entry;
    };
    fill(kCheckJniEntriesPerReference, get_entry);
  } else {
    fill(1u, [&](size_t index) { return GetEntry(index); });
  }
