...
void LocalReferenceTable::SetCheckJniEnabled(bool enabled) {
  // AssertEmpty();
  CHECK_EQ(segment_state_.top_index, 0u);
...
inline uint32_t LocalReferenceTable::IncrementSerialNumber(LrtEntry* serial_number_entry) {
  ...
  // The old serial number can be 0 if it was not used before. It can also be the numeric
  ...
  uint32_t old_serial_number =
      serial_number_entry->GetSerialNumber() % kCheckJniEntriesPerReference;
...
bool LocalReferenceTable::Remove(LRTSegmentState previous_state, IndirectRef iref) {
  ...
  if (entry_index >= top_index - prune_chunk_size) {
    ...
    if (UNLIKELY(IsCheckJniEnabled())) {
      ...
      find_prune_range(1u, [&](size_t index) { return GetEntry(index - 1u)->IsFree(); });
    }
