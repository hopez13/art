...
//
// With CheckJNI, we rotate the slots that we use based on a "serial number", incremented when we
// we add a reference to an empty segment. This increases the memory use but it allows for decent
// error detection.
...
// it can be marked as free and hold the index of the next free entry.
// If CheckJNI is enabled, some entries contain serial numbers instead.
class LrtEntry {
 public:
  ...
  void SetNextFree(uint32_t next_free);

  uint32_t GetNextFree() {
    DCHECK(IsFree());
    return NextFreeField::Decode(GetVRegValue());
  }

  bool IsFree() {
    return (GetVRegValue() & (1u << kFlagFree)) != 0u;
  }

  void SetSerialNumber(uint32_t serial_number) {
    SetVRegValue(serial_number);
  }
 ...
 private:
  ...
  static constexpr size_t kFieldNextFree = 1u;
  ...
  void SetVRegValue(uint32_t value) {
    root_ = GcRoot<mirror::Object>(
        mirror::CompressedReference<mirror::Object>::FromVRegValue(value));
  }

  uint32_t GetVRegValue() {
    return root_.AddressWithoutBarrier()->AsVRegValue();
  }

  // We record the contents as a `GcRoot<>` but it is an actual `GcRoot<>` only if it's below the
  // current segment's top index, it's not marked as "free" in the bottom bit and it's not null.
...
class LocalReferenceTable {
 ...
 private:
  ...
  static constexpr size_t kFieldFirstFree = 1u;
  static constexpr size_t kFieldFirstFreeBits = BitSizeOf<uint32_t>() - kFieldFirstFree;

  using FirstFreeField = BitField<uint32_t, kFieldFirstFree, kFieldFirstFreeBits>;
  ...
  static_assert(kFreeListEnd == MaxInt<uint32_t>(kFieldFirstFreeBits));
  ...
  LrtEntry* GetCheckJniSerialNumberEntry(LrtEntry* entry) const {
    DCHECK(IsCheckJniEnabled());
    return AlignDown(entry, kCheckJniEntriesPerReference * sizeof(LrtEntry));
  }

  uint32_t IncrementSerialNumber(LrtEntry* serial_number_entry);
  ...
  // We use the lowest bit to record whether CheckJNI is enabled.
  // This helps check that the list is empty and the CheckJNI is disabled in a single comparison.
