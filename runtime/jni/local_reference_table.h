...
// The entry in the `LocalReferenceTable` can contain a null (initially or after a `Trim()`)
...
class LocalReferenceTable {
 public:
  ...
  // and may be wrong if there are mixed CheckJNI and non-CheckJNI entries.
 ...
 private:
  ...
  // Indirect reference kind encoding. This must be the same as in `IndirectReferenceTable`.
  ...
  void VisitRootsInternal(Visitor&& visitor) const
      REQUIRES_SHARED(Locks::mutator_lock_);
