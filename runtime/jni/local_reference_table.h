...
// If CheckJNI is (or was) enabled, some entries contain serial numbers instead.
class LrtEntry {
 public:
  ...
  uint32_t GetSerialNumberUnchecked() {
    return SerialNumberField::Decode(GetVRegValue());
  }

  bool IsSerialNumber() {
    return (GetVRegValue() & (1u << kFlagSerialNumber)) != 0u;
  }
