...
TEST_F(ClassTableTest, ClassTable) {
  ...
  // Collect all fo the roots and make sure there is nothing missing.
  ...
  static const size_t kBufferSize = 32 * KB;
  std::unique_ptr<uint8_t[]> buffer(new uint8_t[kBufferSize]());
  table.Insert(h_X.Get());
  size_t count = table.WriteToMemory(&buffer[0]);
  ASSERT_LE(count, kBufferSize);
  ClassTable table2;
