...
ClassLoaderContext::EncodeClassPathContexts(const std::string& base_dir) const {
  ...
  for (const std::string& path : class_loader_chain_->classpath) {
    ...
    const std::string flattened_path(DexFileLoader::GetBaseLocation(path));
    if (!seen_locations.insert(flattened_path).second) {
      continue;
    }
    ...
    results.emplace(flattened_path, out.str());

    dex_locations.push_back(flattened_path);
  }
...
ClassLoaderContext::EncodeClassPathContextsForClassLoader(jobject class_loader) {
  ...
  if (clc != nullptr) {
    // Note: we don't need to call OpenDexFiles because initializing a context from a ClassLoader
    // automatically "opens" the dex files.
