...
// Returns true for success for false if an unexpected state is discovered (e.g. a null dex cookie,
...
static bool CollectDexFilesFromSupportedClassLoader(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if (dex_elements_obj == nullptr) {
    ...
    LOG(ERROR) << "Unexpected null for the dex_elements_obj";
    return false;
  } else {
...
static bool GetDexFilesFromDexElementsArray(
  ...
  for (int32_t i = 0; i < dex_elements->GetLength(); ++i) {
    ...
    // list of dex files opened before.
    if (element == nullptr) {
      continue;
    }
...
// the classpath.
bool ClassLoaderContext::AddInfoToContextFromClassLoader(ScopedObjectAccessAlreadyRunnable& soa,
                                                         Handle<mirror::ClassLoader> class_loader,
                                                         jobjectArray dex_elements)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  if (ClassLinker::IsBootClassLoader(soa, class_loader.Get())) {
    // Nothing to do for the boot class loader as we don't add it's dex files to the context.
    return true;
  }
  ...
  if (IsPathOrDexClassLoader(soa, class_loader)) {
    ...
    LOG(ERROR) << "Unsupported class loader";
    return false;
  }
  ...
  //      appending them to the current class loader. The logic in BaseDexClassLoader will pass
  //      the new paths as a separate array.
  ...
  // Note that t dex_elements array is part of the current class loader and not passed to the
  // parents.
  return AddInfoToContextFromClassLoader(soa, parent, /*dex_elements*/ nullptr);
}
...
std::unique_ptr<ClassLoaderContext> ClassLoaderContext::CreateContextForClassLoader(
  ...
  if (result->AddInfoToContextFromClassLoader(soa, h_class_loader, dex_elements)) {
    return result;
  } else {
    return nullptr;
  }
...
bool ClassLoaderContext::VerifyClassLoaderContextMatch(const std::string& context_spec) {
  ...
  if (expected_context.class_loader_chain_.size() != class_loader_chain_.size()) {
    LOG(ERROR) << "ClassLoaderContext size mismatch. expected="
  ...
  for (size_t i = 0; i < class_loader_chain_.size(); i++) {
    ...
    if (info.type != expected_info.type) {
      LOG(ERROR) << "ClassLoaderContext type mismatch for position " << i
    ...
    if (info.classpath.size() != expected_info.classpath.size()) {
      LOG(ERROR) << "ClassLoaderContext classpath size mismatch for position " << i
    ...
    for (size_t k = 0; k < info.classpath.size(); k++) {
      if (info.classpath[k] != expected_info.classpath[k]) {
        LOG(ERROR) << "ClassLoaderContext classpath element mismatch for position " << i
            << ". expected=" << expected_info.classpath[k]
            << ", found=" << info.classpath[k];
        return false;
      }
      if (info.checksums[k] != expected_info.checksums[k]) {
        LOG(ERROR) << "ClassLoaderContext classpath element checksum mismatch for position " << i
