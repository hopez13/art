...
ClassLoaderContext::ClassLoaderContext()
    : special_shared_library_(false),
      dex_files_state_(ContextDexFilesState::kDexFilesUnopened),
      owns_the_dex_files_(true) {}

ClassLoaderContext::ClassLoaderContext(bool owns_the_dex_files)
    : special_shared_library_(false),
      dex_files_state_(ContextDexFilesState::kOpenDexOk),
...
bool ClassLoaderContext::OpenDexFiles(const std::string& classpath_dir,
  ...
  switch (dex_files_state_) {
    case kDexFilesUnopened: break;  // files not opened, continue.
    case kOpenDexFailed: return false;  // previous attempt failed.
    case kOpenDexOk: return true;  // previous attempt succeed.
    case kReadDexChecksumsOk:
  ...
  dex_files_state_ = only_read_checksums ? kReadDexChecksumsOk : kOpenDexOk;
...
ClassLoaderContext::VerificationResult ClassLoaderContext::VerifyClassLoaderContextMatch(
  ...
  if (verify_names || verify_checksums) {
    DCHECK(dex_files_state_ == kReadDexChecksumsOk || dex_files_state_ == kOpenDexOk)
        << "dex_files_state_=" << dex_files_state_;
  }
...
std::set<const DexFile*> ClassLoaderContext::CheckForDuplicateDexFiles(
  ...
  DCHECK(dex_files_state_ == kOpenDexOk);
