...
static bool is_absolute(const std::string& location) {
  return location[0] == '/';
}
...
bool ClassLoaderContext::VerifyClassLoaderContextMatch(const std::string& context_spec) const {
  ...
  for (size_t i = 0; i < class_loader_chain_.size(); i++) {
    ...
    for (size_t k = 0; k < info.classpath.size(); k++) {
      ...
      // We shouln't do a naive comparison `info.classpath[k] == expected_info.classpath[k]`
      // because even if they refer to the same file, one could encoded as a relative location
      // and the other as an abosulte one.
      bool is_dex_name_absolute = is_absolute(info.classpath[k]);
      bool is_expected_dex_name_absolute = is_absolute(expected_info.classpath[k]);
      std::string dex_name;
