...
void ClassLoaderContext::EncodeClassPath(const std::string& base_dir,
  ...
  CHECK(checksums.empty() || locs.size() == checksums.size());
  out << GetClassLoaderTypeName(type);
  out << kClassLoaderOpeningMark;
  const size_t len = locs.size();
  for (size_t k = 0; k < len; k++) {
    std::string location = locs[k];
    if (k > 0) {
...
void ClassLoaderContext::EncodeContextInternal(const ClassLoaderInfo& info,
  ...
  EncodeSharedLibParent(info, base_dir, for_dex2oat, stored_info, out);
}

void ClassLoaderContext::EncodeSharedLibParent(const ClassLoaderInfo& info,
                                               const std::string& base_dir,
                                               bool for_dex2oat,
                                               ClassLoaderInfo* stored_info,
                                               std::ostringstream& out) const {
