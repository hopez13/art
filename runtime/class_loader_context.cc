      ...
      if (only_read_checksums) {
        ...
        if (!dex_file_loader.Open(/*verify=*/false,
          ...
          for (size_t k = opened_dex_files_index; k < info->opened_dex_files.size(); k++) {
            ...
            CHECK(DexFileLoader::AccMultiDexChecksum(dex.get(), &dex_checksum));
          }
        }
      }
...
void ClassLoaderContext::EncodeContextInternal(const ClassLoaderInfo& info,
  ...
  for (size_t k = 0; k < info.opened_dex_files.size(); k++) {
    ...
    CHECK(DexFileLoader::AccMultiDexChecksum(dex_file.get(), &checksum));
    for (; k + 1 < info.opened_dex_files.size(); k++) {
      if (!DexFileLoader::AccMultiDexChecksum(info.opened_dex_files[k + 1].get(), &checksum)) {
        break;
      }
    }
...
bool ClassLoaderContext::CreateInfoFromClassLoader(
  ...
  for (size_t i = 0; i < dex_files_loaded.size(); i++) {
    ...
    for (; i + 1 < dex_files_loaded.size(); i++) {
      const DexFile* secondary_dex_file = dex_files_loaded[i + 1];
      if (!DexFileLoader::AccMultiDexChecksum(secondary_dex_file, &checksum)) {
        break;  // Not a secondary dex file.
      }
      info->opened_dex_files.emplace_back(secondary_dex_file);
    }
...
std::set<const DexFile*> ClassLoaderContext::CheckForDuplicateDexFiles(
  ...
  ClassLoaderInfo* info = class_loader_chain_.get();
  for (size_t k = 0; k < info->classpath.size(); k++) {
    for (size_t i = 0; i < dex_files_to_check.size(); i++) {
      const DexFile* dex_file = dex_files_to_check[i];
      std::optional<uint32_t> checksum;
      CHECK(DexFileLoader::AccMultiDexChecksum(dex_file, &checksum));
      for (; i + 1 < dex_files_to_check.size(); i++) {
        if (!DexFileLoader::AccMultiDexChecksum(dex_file, &checksum)) {
          break;
        }
      }
