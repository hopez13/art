...
jobject ClassLoaderContext::CreateClassLoader(
  ...
  // Now, that we set up the chain, create the first class loader.
  // Its classpath comes first, followed by compilation sources. This way, whenever we need to
  // resolve classes from it we ensure the classpath elements come first.
...
bool ClassLoaderContext::AddInfoToContextFromClassLoader(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  // Note that dex_elements array is null here. The elemets are considered to be part of the
