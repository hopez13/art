...
template <typename Visitor>
inline void ImageHeader::VisitPackedPointerFixups(const Visitor& visitor, uint8_t* base) const {
  const ImageSection& section = GetImageSection(ImageHeader::kSectionPointerFixups);
  for (size_t pos = 0; pos < section.Size(); ) {
    const gc::space::PointerSizedFixup* fixup =
        reinterpret_cast<const gc::space::PointerSizedFixup*>(base + section.Offset() + pos);
    visitor(*fixup);
    pos += fixup->SizeOf();
  }
}
