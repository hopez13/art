...
class RuntimeImageHelper {
 ...
 private:
  ...
  void CreateImageSections() {
    ...
    sections_[ImageHeader::kSectionArtMethods] =
        ImageSection(sections_[ImageHeader::kSectionArtFields].End(), art_methods_.size());
    sections_[ImageHeader::kSectionImTables] =
        ImageSection(sections_[ImageHeader::kSectionArtMethods].End(), im_tables_.size());

    sections_[ImageHeader::kSectionIMTConflictTables] =
        ImageSection(sections_[ImageHeader::kSectionImTables].End(), 0u);
    ...
    size_t cur_pos = RoundUp(sections_[ImageHeader::kSectionRuntimeMethods].End(), sizeof(uint64_t));
  ...
  class ClassDescriptorHash {
   public:
    ...
    uint32_t operator()(const ClassTable::TableSlot& slot) const NO_THREAD_SAFETY_ANALYSIS {
      ...
      if (helper_->IsInBootImage(reinterpret_cast<const void*>(ptr))) {
        return reinterpret_cast<mirror::Class*>(ptr)->DescriptorHash();
      }
  ...
  class ClassTableVisitor {
   public:
    ...
    bool operator()(ObjPtr<mirror::Class> klass) REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (klass->GetClassLoader() == loader_.Get() || klass->GetClassLoader() == nullptr) {
  ...
  class PruneVisitor {
   public:
    PruneVisitor(Thread* self,
                 ...
                 const std::set<const DexFile*> dex_files,
    ...
    bool CanEmitHelper(Handle<mirror::Class> cls) REQUIRES_SHARED(Locks::mutator_lock_) {
      visited_[cls->GetClassDef()] = VisitState::kVisiting;
      ...
      for (size_t i = 0, num_interfaces = cls->GetIfTableCount(); i < num_interfaces; ++i) {
        other_class.Assign(cls->GetIfTable()->GetInterface(i));
        if (!CanEmit(other_class)) {
          return false;
        }
      }
    ...
    bool CanEmit(Handle<mirror::Class> cls) REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (cls->GetClassDef() == nullptr) {
        // Covers array classes and proxy classes.
        // TODO: Handle these differently.
        return false;
      }
      auto existing = visited_.find(cls->GetClassDef());
      ...
      if (CanEmitHelper(cls)) {
        visited_[cls->GetClassDef()] = VisitState::kCanEmit;
        return true;
      } else {
        visited_[cls->GetClassDef()] = VisitState::kCannotEmit;
        return false;
      }
    }
   ...
   private:
    ...
    const std::set<const DexFile*> dex_files_;
    std::map<const dex::ClassDef*, VisitState> visited_;
    std::vector<Handle<mirror::Class>>& classes_to_write_;
  };

  void VisitDexCaches(Thread* self, Handle<mirror::ObjectArray<mirror::Object>> dex_cache_array)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    std::set<const DexFile*> dex_files;
    ...
    std::vector<Handle<mirror::Class>> classes_to_write;
    {
      PruneVisitor prune_visitor(self, this, dex_files, classes_to_write);
  ...
  // Helper visitor returning the location in the image of a native pointer.
  ...
  void RelocateMethodPointerArrays(mirror::Class* klass, const Visitor& visitor)
    ...
    // and do pointer comparison between either these classes (which do not
    // move), or boot image objects (which do not move).
  ...
  void CopyFieldArrays(ObjPtr<mirror::Class> cls, uint32_t class_image_offset)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    for (LengthPrefixedArray<ArtField>* cur_fields : fields) {
      if (cur_fields != nullptr) {
        ...
        art_fields_.resize(art_fields_.size() + size);
        auto* dest_array =
        ...
        for (size_t i = 0; i != number_of_fields; ++i) {
          dest_array->At(i).GetDeclaringClassAddressWithoutBarrier()->Assign(
              reinterpret_cast<mirror::Class*>(class_image_offset));
        }
      }
    }
  }

  void CopyMethodArrays(ObjPtr<mirror::Class> cls, uint32_t class_image_offset)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    art_methods_.resize(art_methods_.size() + size);
    auto* dest_array =
  ...
  void CopyImTable(ObjPtr<mirror::Class> cls) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    im_tables_.resize(im_tables_.size() + size);
  ...
  bool WriteObjects(std::string* error_msg) {
    ...
    VisitDexCaches(soa.Self(), dex_cache_array);
  ...
  uint32_t CopyClass(ObjPtr<mirror::Class> cls) REQUIRES_SHARED(Locks::mutator_lock_) {
    auto it = classes_.find(cls->GetClassDef());
    if (it != classes_.end()) {
      return it->second;
    }
    uint32_t offset = CopyObject(cls);
    classes_[cls->GetClassDef()] = offset;

    uint32_t hash = cls->DescriptorHash();
    // Save the hash, in debug mode `HashSet` requires to find it.
    class_hashes_[offset] = hash;
    uint32_t image_offset = image_begin_ + sizeof(ImageHeader) + offset;
    bool inserted =
        class_table_.InsertWithHash(ClassTable::TableSlot(image_offset, hash), hash).second;
    ...
    CopyFieldArrays(cls, image_offset);
    CopyMethodArrays(cls, image_offset);
    CopyImTable(cls);
  ...
  uint32_t CopyObject(ObjPtr<mirror::Object> obj) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    objects_.resize(RoundUp(objects_.size() + object_size, kObjectAlignment));
    ...
    if (obj->IsString()) {
      ...
      reinterpret_cast<mirror::String*>(objects_.data() + offset)->GetHashCode();
    }
    return offset;
  }
