 ...
 public:
  ...
  bool Generate(std::string* error_msg) {
    ...
    *bitmap_section = ImageSection(RoundUp(sections_end, kPageSize),
    ...
    header_ = ImageHeader(
        /* total_reservation_size= */ RoundUp(sections_end, kPageSize),
...
bool Runtime::WriteImageToDisk(std::string* error_msg) const {
  ...
  if (access(android::base::Dirname(path).c_str(), W_OK) != 0) {
    LOG(ERROR) << "WHAT?? " << std::string(strerror(errno));
    *error_msg = "Runtime does not have access to " + path + ": " + std::string(strerror(errno));
    return false;
  }
