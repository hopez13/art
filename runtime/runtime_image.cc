...
class RuntimeImageHelper {
 ...
 private:
  ...
  void RelocateDexCacheArrays(mirror::DexCache* cache,
    ...
    // No need to relocate, we have already done this when creating the
    // DexCache copy.
  }
    ...
    if (strings != nullptr) {
      for (uint32_t i = 0; i < cache->GetDexFile()->NumStringIds(); ++i) {
        ...
        if (str == nullptr || IsInBootImage(str.Ptr())) {
          ...
          string_reference_offsets_.emplace_back(offset + sizeof(ImageHeader), sizeof(mirror::DexCache) + i);
        }
      }
    }
