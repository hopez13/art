...
class RuntimeImageHelper {
 ...
 private:
  ...
  void EmitClasses(Thread* self, Handle<mirror::ObjectArray<mirror::Object>> dex_cache_array)
    ...
    // Relocate the type array entries.
    for (auto it : dex_caches_) {
      ...
      if (HasNativeRelocation(old_types_array)) {
        auto it2 = native_relocations_[old_types_array];
        std::vector<uint8_t>& data = (it2.first == NativeRelocationKind::kFullNativeDexCacheArray)
            ? dex_cache_arrays_ : metadata_;

        mirror::GcRootArray<mirror::Class>* content_array =
            reinterpret_cast<mirror::GcRootArray<mirror::Class>*>(data.data() + it2.second);
        for (uint32_t i = 0; i < dex_file.NumTypeIds(); ++i) {
          ...
          if (cls == nullptr) {
            ...
            auto it3 = classes_.find(class_def);
            if (it3 == classes_.end()) {
              content_array->Set(i,  nullptr);
            } else {
              content_array->Set(
                  i,
                  reinterpret_cast<mirror::Class*>(image_begin_ + sizeof(ImageHeader) + it3->second));
            }
          }
        }
      }
