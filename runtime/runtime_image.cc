 ...
 private:
  ...
  template <typename T> T* NativeLocationInImage(T* ptr, bool must_have_relocation) const {
    ...
    switch (it->second.first) {
      ...
      case NativeRelocationKind::kNativeDexCacheArray: {
  ...
  void RelocateNativeDexCacheArray(mirror::NativeArray<T>* old_method_array,
    ...
    mirror::NativeArray<T>* content_array =
        reinterpret_cast<mirror::NativeArray<T>*>(metadata_.data() + it.second);
  ...
  template <typename T>
  void CopyNativeDexCacheArray(uint32_t num_entries, mirror::NativeArray<T>* array) {
    if (array == nullptr) {
      return;
    }
    size_t size = num_entries * sizeof(ArtMethod*);
    size_t metadata_offset = metadata_.size() + sizeof(uint32_t);
    metadata_.resize(metadata_offset + size);
    // We need to store `num_entries` because ImageSpace doesn't have
    // access to the dex files when relocating dex caches.
    reinterpret_cast<uint32_t*>(metadata_.data() + metadata_offset)[-1] = num_entries;
    memcpy(metadata_.data() + metadata_offset, array, size);
    native_relocations_[array] =
        std::make_pair(NativeRelocationKind::kNativeDexCacheArray, metadata_offset);
  }
  ...
  uint32_t CopyDexCache(ObjPtr<mirror::DexCache> cache) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    CopyNativeDexCacheArray(cache->GetDexFile()->NumMethodIds(), resolved_methods);
    ...
    CopyNativeDexCacheArray(cache->GetDexFile()->NumFieldIds(), resolved_fields);
    // Store the array pointer in the dex cache, which will be relocated at the end.
