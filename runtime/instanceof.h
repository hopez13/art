...
#ifndef ART_RUNTIME_INSTANCEOF_H_
#define ART_RUNTIME_INSTANCEOF_H_

#include "instanceof_bitstring.h"
#include "instanceof_and_status.h"

// Forward-declare for testing purposes.
struct InstanceOfTest;

namespace art {

/**
 * InstanceOf is a label for the tree data, which is necessary to perform
 ...
 *
 * For convenience, we also store the class depth within an InstanceOf, since nearly all
 * calculations are dependent on knowing the depth of the class.
 *
 * An InstanceOf logically has:
 *          * Depth - How many levels up to the root (Object)?
 ...
 * Invariants:
 *
 *   Uninitialized    => Parent == Uninitialized
 *
 ...
 *
 * Definitions:
 *   (see also instanceof_bitstring.h definitions).
 *
 ...
 *         s.t. unrelated.State == (Initialized || Assigned):
...
struct InstanceOf {
  ...
  // The result of a "src IsInstanceOf target" check:
  enum Result {
    kUnknownInstanceOf,  // Not enough data. Operand states weren't high enough.
    kNotInstanceOf,      // Enough data. src is not a subchild of the target.
    kInstanceOf          // Enough data. src is a subchild of the target.
  };

  using SlicedType = InstanceOfData;
  ...
  SlicedType Slice() const {
    return bitstring_and_of_;
  }

  // Infuse the depth and the bitstring+of state.
  ...
  static InstanceOf Infuse(SlicedType compressed_value, size_t depth) {
    InstanceOf io;
    io.depth_ = depth;
  ...
  // Is this an instance of the target?
  //
  ...
  //
  // Normally, return kInstanceOf or kNotInstanceOf.
  Result IsInstanceOf(const InstanceOf& target) {
    if (target.GetState() != InstanceOf::kAssigned) {
      return Result::kUnknownInstanceOf;
    } else if (GetState() == InstanceOf::kUninitialized) {
     return Result::kUnknownInstanceOf;
    }
    ...
    return result ? Result::kInstanceOf : Result::kNotInstanceOf;
  }

  // Create a new root InstanceOf with a blank PathToRoot.
  // Returns: A new root with an Assigned state.
  static InstanceOf NewForRoot() {
    InstanceOf io{};
    io.depth_ = 0u;
    io.SetNext(io.GetNext() + 1u);

    // The root is always considered assigned once it is no longer Initialized.
    DCHECK_EQ(InstanceOf::kAssigned, io.GetState());
    return io;
  }

  // Copies the current PathToRoot into the child.
  //
  // If assign_next is true, then also assign a new InstanceOf for a child by
  ...
  InstanceOf NewForChild(bool assign_next) {
    InstanceOf child = *this;  // Copy everything (path, next, of).
    ...
    auto OverwriteNextValueFromParent = [&](InstanceOf& child, BitChar value) {
      if (HasNext()) {
        ...
        DCHECK(child.HasBitCharStorage(depth_));
    ...
    OverwriteNextValueFromParent(child, BitChar{});
    ...
    if (HasNext() && !bitstring_and_of_.overflow_) {
      ...
      BitChar next = GetNext();
      if (next != next.MaximumValue()) {
        // The parent's "next" value is now the child's latest path element.
        OverwriteNextValueFromParent(child, next);
        // Update self next value, so that future NewForChild calls
  ...
  bool MaybeGetNext(/*out*/BitChar* next) const {
    DCHECK(next != nullptr);
  ...
  // See the "InstanceOf" documentation above which explains how a state is determined.
  State GetState() const {
    if (GetBitString() == BitString{}) {
    ...
    DCHECK(!HasNext() || GetNext() != BitChar{})
  ...
  // Create an InstanceOf with the same depth, but with everything else reset.
  // Returns: InstanceOf in the Uninitialized state.
  InstanceOf CopyCleared() const {
    InstanceOf cleared_copy{};
    cleared_copy.depth_ = depth_;
    DCHECK_EQ(InstanceOf::kUninitialized, cleared_copy.GetState());
    return cleared_copy;
  }
 ...
 private:
  ...
  InstanceOf(BitString path_to_root, BitChar next, bool overflow, size_t depth) {
    InstanceOfData iod;
  ...
  static InstanceOf MakeUnchecked(BitString bitstring, bool overflow, size_t depth) {
    InstanceOfData iod;
  ...
  void SetNextUnchecked(BitChar next) {
    BitString bs = GetBitString();
    bs.SetAt(depth_, next);
  ...
  static constexpr bool HasBitCharStorage(size_t idx) {
  ...
  InstanceOf() = default;
  size_t depth_;
  InstanceOfData bitstring_and_of_;

  friend struct ::InstanceOfTest;
  friend std::ostream& operator<<(std::ostream& os, const InstanceOf& io);
};

// Prints the InstanceOf::State, e.g. "kUnitialized".
inline std::ostream& operator<<(std::ostream& os, const InstanceOf::State& state) {
  switch (state) {
    case InstanceOf::kUninitialized:
      os << "kUninitialized";
      break;
    case InstanceOf::kInitialized:
      os << "kInitialized";
      break;
    case InstanceOf::kAssigned:
      os << "kAssigned";
      break;
    case InstanceOf::kOverflowed:
      os << "kOverflowed";
      break;
    default:
      os << "(Invalid InstanceOf::State " << static_cast<int>(state) << ")";
  }
  return os;
}

// Prints e.g. "InstanceOf{BitString[1,2,3], depth: 3, of:1}"
inline std::ostream& operator<<(std::ostream& os, const InstanceOf& io) {
  os << "InstanceOf{" << io.GetBitString() << ", "
...
#endif  // ART_RUNTIME_INSTANCEOF_H_
