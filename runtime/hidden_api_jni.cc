...
// that stores these values. This is necessary because we can't change the JNI interfaces and some paths
// call into each other, ie checked JNI typically calls plain JNI.
struct CorePlatformApiCookie {
  explicit CorePlatformApiCookie(uint32_t value) : u32(value) {}

  union {
    uint32_t u32;  // opaque 32-bit cookie value
    struct {
      uint32_t approved:1;  // indicator whether outermost ScopedCorePlatformApiCheck met with approval.
      uint32_t depth:31;  // 31-bit count of nested ScopedCorePlatformApiCheck instances.
    };
  };
};
static_assert(sizeof(CorePlatformApiCookie) == 4);

ScopedCorePlatformApiCheck::ScopedCorePlatformApiCheck() {
  Thread* self = Thread::Current();

  CorePlatformApiCookie cookie(self->CorePlatformApiCookie());

  auto policy = Runtime::Current()->GetCorePlatformApiEnforcementPolicy();
  bool disabled_by_policy = (policy == hiddenapi::EnforcementPolicy::kDisabled);

  // Assume Core Platform API usage is pre-approved if the build is not for a target device or if
  // checks are disabled.
  bool is_core_platform_api_approved = !kIsTargetBuild | disabled_by_policy;

  // Perform caller check for the outermost ScopedCorePlatformApiCheck instance on target devices.
  if (!is_core_platform_api_approved) {
    if (cookie.depth == 0u && kIsTargetBuild) {
      DCHECK_EQ(cookie.approved, 0u);
  ...
  cookie.depth += 1;
  self->SetCorePlatformApiCookie(cookie.u32);
}
