...
static constexpr uint32_t kTypeIdLimit = 65536;

static bool IsSignExtendedTypeId(uint16_t low, uint16_t high) {
  return (((low & 0x8000U) == 0x8000U) && (high == 0xffffU)) ||
         (((low & 0x8000U) == 0) && (high == 0));
}
...
bool DexFileVerifier::CheckInterProtoIdItem() {
  ...
  if (UNLIKELY(!IsSignExtendedTypeId(item->return_type_idx_, item->pad_))) {
    ErrorStringPrintf("proto with return type idx outside uint16_t range '%x:%x'",
                      item->pad_, item->return_type_idx_);
    return false;
  }
...
bool DexFileVerifier::CheckInterClassDefItem() {
  ...
  if (UNLIKELY(!IsSignExtendedTypeId(item->class_idx_, item->pad1_))) {
  ...
  if (UNLIKELY(!IsSignExtendedTypeId(item->superclass_idx_, item->pad2_))) {
