...
class ArtMethod FINAL {
 public:
  ...
  void SetIntrinsic(uint32_t intrinsic) {
    ...
    uint32_t new_value = (GetAccessFlags() & kAccIntrinsicMask) |
        kAccIntrinsic |
        (intrinsic << POPCOUNT(kAccIntrinsicMask));
    if (kIsDebugBuild) {
  ...
  uint32_t GetIntrinsic() {
    DCHECK(IsIntrinsic());
    return (GetAccessFlags() >> POPCOUNT(kAccIntrinsicMask)) & kAccMaxIntrinsic;
  }

  bool IsCopied() {
    static_assert((kAccCopied & kAccIntrinsicMask) == kAccCopied,
  ...
  bool IsMiranda() {
    static_assert((kAccMiranda & kAccIntrinsicMask) == kAccMiranda,
  ...
  bool IsDefault() {
    static_assert((kAccDefault & kAccIntrinsicMask) == kAccDefault,
