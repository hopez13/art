...
class ArtMethod final {
 public:
  ...
  ALWAYS_INLINE uint32_t GetCodeItemOffset();

  ALWAYS_INLINE void SetCodeItemOffsetOrImtIndex(uint32_t new_code_off)
      REQUIRES_SHARED(Locks::mutator_lock_);

  ALWAYS_INLINE uint32_t GetImtIndex()
      REQUIRES_SHARED(Locks::mutator_lock_);
 ...
 protected:
  ...
  union {
    ...
    // Abstract methods: Cached IMT index of the interface method.
    uint32_t imt_index_;
  };
