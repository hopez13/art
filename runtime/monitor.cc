...
#endif
      DCHECK_EQ(monitor_lock_.GetExclusiveOwnerTid(), owner->GetTid()) << " my tid = " << SafeGetTid(self);
...
void Monitor::Lock(Thread* self) {
  ...
  // Not renetrant and contended. We hold no locks, so tread carefully.
  ...
  // We've successfully acquired monitor_lock_.

  // We avoid touching monitor fields while suspended, so set owner_ here.
  owner_.store(self, std::memory_order_relaxed);
