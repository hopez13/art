...
bool Monitor::capture_method_eagerly_ = false;
...
void Monitor::Init(uint32_t lock_profiling_threshold,
  ...
  capture_method_eagerly_ = ATraceEnabled();
}
...
void Monitor::SetLockingMethodNoProxy() {
  Thread* owner = owner_.load(std::memory_order_relaxed);
  uint32_t lock_owner_dex_pc;
...
void Monitor::Lock(Thread* self) {
  ...
  uint32_t owners_dex_pc;
  if (capture_method_eagerly_) {
    // Capture owner info while fresh, if it's available.
  ...
  if (log_contention && !capture_method_eagerly_) {
    ...
    if (orig_owner_thread_id != 0u) {
      ...
      if (log_contention) {
        ...
        if (sample_percent != 0 && (static_cast<uint32_t>(rand() % 100) < sample_percent)) {
          if (!capture_method_eagerly_) {
            GetLockOwnerInfo(&owners_method, &owners_dex_pc, orig_owner_thread_id);
          } //  Otherwise we already retreived them earlier.
