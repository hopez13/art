...
static constexpr uint64_t kMonitorTimout = 500 * 1000 * 1000;
#define PROP_LOCK_MONITOR_PACKAGE   "debug.art.monitor.package"
#define PROP_LOCK_MONITOR_TIMEOUT   "debug.art.monitor.timeout"
#define PROP_MONITOR_LOCK_ENABLE    "dalvik.monitor.lock.enable"
...
Monitor::Monitor(Thread* self, Thread* owner, ObjPtr<mirror::Object> obj, int32_t hash_code)
  ...
  bool enable_monitor_lock = android::base::GetBoolProperty(PROP_MONITOR_LOCK_ENABLE, false);
  if (enable_monitor_lock) {
    CheckMonitorType();
  }
}
...
void Monitor::CheckMonitorType() {
  ...
  if (current_package == "android" || track_package == current_package) {
    uint64_t timeout = android::base::GetUintProperty<uint64_t>(PROP_LOCK_MONITOR_TIMEOUT,
                                                                kMonitorTimout);
    monitor_lock_.setCheckTimeOut(true, timeout);
