...
void Monitor::SetLockingMethod() {
  ...
  SetLockOwnerInfo(lock_owner_method, lock_owner_dex_pc);
...
bool Monitor::Install(Thread* self) NO_THREAD_SAFETY_ANALYSIS {
  ...
  switch (lw.GetState()) {
    case LockWord::kThinLocked: {
      ...
      monitor_lock_.ExclusiveLockUncontendedFor(owner, lockCount + 1, self);
...
bool Monitor::TryLock(Thread* self) {
  bool success = monitor_lock_.ExclusiveTryLock(self);  // Handles recursive case.
  if (success) {
    Thread * owner = owner_.load(std::memory_order_relaxed);
    if (owner == nullptr) {  // Unowned.
      ...
      if (lock_profiling_threshold_ != 0) {
        uint32_t lock_owner_dex_pc;
        ArtMethod* lock_owner_method = self->GetCurrentMethod(&lock_owner_dex_pc);
...
void Monitor::Lock(Thread* self) {
  ...
  ArtMethod* owners_method;
  uint32_t owners_dex_pc;
  // TODO: This isn't always needed, and may cause cache contention. Consider doing
  // this conditionally. Log_contention is often on, so this is currently minor.
  GetLockOwnerInfo(&owners_method, &owners_dex_pc);
  ...
  if (ATraceEnabled()) {
    Thread* owner = owner_.load(std::memory_order_relaxed);
    if (owner != nullptr) {  // Did the owner_ give the lock up?
      std::ostringstream oss;
      std::string name;
  ...
  self->SetMonitorEnterObject(GetObject().Ptr());
  {
    ...
    const Thread* owner = owner_.load(std::memory_order_relaxed);
    const uint32_t original_owner_thread_id = owner == nullptr? 0u : owner->GetThreadId();
...
void Monitor::Wait(Thread* self, int64_t ms, int32_t ns,
  ...
  ArtMethod* saved_method;
  uint32_t saved_dex_pc;
  GetLockOwnerInfo(&saved_method, &saved_dex_pc);
  ClearLockOwnerInfo();
...
MonitorInfo::MonitorInfo(ObjPtr<mirror::Object> obj) : owner_(nullptr), entry_count_(0) {
  ...
  switch (lock_word.GetState()) {
    ...
    case LockWord::kFatLocked: {
      ...
      if (owner_ != nullptr) {
        entry_count_ = 1 + mon->monitor_lock_.GetDepth();
      } else {
