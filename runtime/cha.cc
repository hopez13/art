...
void ClassHierarchyAnalysis::ResetSIInHierarchyByDeletedAllocation(ObjPtr<mirror::Class> klass,
  ...
  // The class is going to be deleted. Iterate over the virtual methods of its superclasss to see
  // if they have SingleImplementations methods defined by 'klass'.
  // Skip all virtual methods that have not been overridden by the class since they cannot be
    ...
    while (super_it != nullptr &&
      ...
      if (super_method->IsAbstract<kWithoutReadBarrier>() &&
        ...
        DCHECK(!super_method->HasSingleImplementation<kWithoutReadBarrier>() ||
               !super_method->IsAbstract<kWithoutReadBarrier>() && super_method == method);
        break;
      }
