...
void ClassHierarchyAnalysis::ResetSingleImplementationInHierarchy(ObjPtr<mirror::Class> klass,
  ...
  // This method is supposed to be called from some sort of a class table visitor. That means,
  // some 'klass'es can belong to other classloaders. Argument 'alloc' allows to explicitly
  // indicate a classloader, which is going to be deleted.
  ...
  mirror::ClassLoader* loader = klass->GetClassLoader<kDefaultVerifyFlags, kWithoutReadBarrier>();
  for (int vtbl_index = 0; vtbl_index < vtbl_size; ++vtbl_index) {
    ...
    for (mirror::Class* super_it = super;
         super_it != nullptr &&
      ...
      mirror::ClassLoader* super_loader = super_it->
