...
static const std::vector<ClassHierarchyAnalysis::MethodAndMethodHeaderPair> empty_vector;

const std::vector<ClassHierarchyAnalysis::MethodAndMethodHeaderPair>&
...
void ClassHierarchyAnalysis::RemoveDependentsWithMethodHeaders(
  ...
  for (auto map_it = cha_dependency_map_.begin(); map_it != cha_dependency_map_.end(); ) {
    ...
    dependents.erase(std::remove_if(dependents.begin(),
                                    dependents.end(),
                                    [method_headers](auto dependent) {
                                      return std::find(method_headers.begin(),
                                                       method_headers.end(),
                                                       dependent.second)
                                          != method_headers.end();
                                    }),
