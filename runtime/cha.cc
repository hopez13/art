...
void ClassHierarchyAnalysis::CheckSingleImplementationInfo(
  ...
  // To make updating single-implementation flags simple, we always maintain this invariant:
  // Say all virtual methods in the same vtable slot, starting from the bottom child class
  // to super classes, is a sequence of uniq methods m1, m2, m3, ... (after removing duplicate
  // neighboring methods for inherited methods).
  // The single-implementation status for that sequence of methods begin with one or two true's,
  // then all falses. The only case where two true's are possible is for one abstract method
  // A and one non-abstract method B that overrides method A.
  ...
  if (method_in_super->IsAbstract()) {
    if (virtual_method->IsAbstract()) {
      if (method_in_super == virtual_method) {
        ...
        // Invoking method_in_super may throw AbstractMethodError. This is an uncommon
        // case, so we simply treat method_in_super as not having single-implementation.
...
void ClassHierarchyAnalysis::InitSingleImplementationFlag(Handle<mirror::Class> klass,
  ...
  if (method->IsNative()) {
    ...
    if (method->GetDeclaringClass()->IsInstantiable()) {
      // Rare case, but we do accept it. Do not attempt to devirtualize it.
