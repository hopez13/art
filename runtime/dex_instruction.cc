...
uint32_t RangeInstructionOperands::GetOperand(size_t index) const {
  DCHECK_LT(index, GetNumberOfOperands());
  return first_operand + index;
}

uint32_t VarArgsInstructionOperands::GetOperand(size_t index) const {
  DCHECK_LT(index, GetNumberOfOperands());
