...
class TrackingHeader final {
 public:
  ...
  TrackingHeader(uint32_t size, LinearAllocKind kind, bool is_16_aligned = false)
  ...
  uint32_t GetSize() const { return size_ & ~kIs16Aligned; }
...
class LinearAlloc {
 public:
  ...
  void* Realloc(Thread* self, void* ptr, size_t old_size, size_t new_size)
      REQUIRES(!lock_);
