...
template <typename FromAccessor, typename ToAccessor, typename G, typename S>
bool PerformConversions(const ThrowWrongMethodTypeFunction& throw_wmt,
                        FromAccessor from_accessor,
                        ToAccessor to_accessor,
                        G* getter,
                        S* setter) {
  DCHECK_EQ(from_accessor.GetPTypesLength(), to_accessor.GetPTypesLength());
  for (int32_t i = 0, length = to_accessor.GetPTypesLength(); i != length; ++i) {
    ObjPtr<mirror::Class> from = from_accessor.GetPType(i);
    ObjPtr<mirror::Class> to = to_accessor.GetPType(i);
