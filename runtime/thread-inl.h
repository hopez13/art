...
inline bool Thread::IsThreadSuspensionAllowable() const {
  ...
  if (GetHeldMutex(kUserCodeSuspensionLock) != nullptr && tls32_.user_code_suspend_count != 0) {
    return false;
  }
  return true;
}
...
inline void Thread::AssertThreadSuspensionIsAllowable(bool check_locks) const {
  if (kIsDebugBuild) {
    ...
    if (check_locks) {
      ...
      // user_code_suspend_count which would prevent the thread from ever waking up.
      if (GetHeldMutex(kUserCodeSuspensionLock) != nullptr && tls32_.user_code_suspend_count != 0) {
