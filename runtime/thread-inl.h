...
inline bool Thread::GetWeakRefAccessEnabled() const {
  ...
  if (s == WeakRefAccessState::VISIBLY_ENABLED) {
    return true;
  } else if (s == WeakRefAccessState::DISABLED) {
    return false;
  }
  DCHECK(s == WeakRefAccessState::ENABLED);
  ...
  tls32_.weak_ref_access_enabled.store(WeakRefAccessState::VISIBLY_ENABLED,
