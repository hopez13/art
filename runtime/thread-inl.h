...
template <>
inline uint8_t* Thread::GetStackEnd<StackType::kNative>() const {
  return tlsPtr_.stack_end;
}
template <>
inline void Thread::SetStackEnd<StackType::kNative>(uint8_t* new_stack_end) {
  tlsPtr_.stack_end = new_stack_end;
}
template <>
inline uint8_t* Thread::GetStackBegin<StackType::kNative>() const {
  return tlsPtr_.stack_begin;
}
template <>
inline void Thread::SetStackBegin<StackType::kNative>(uint8_t* new_stack_begin) {
  tlsPtr_.stack_begin = new_stack_begin;
}
template <>
inline size_t Thread::GetStackSize<StackType::kNative>() const {
  return tlsPtr_.stack_size;
}
template <>
inline void Thread::SetStackSize<StackType::kNative>(size_t new_stack_size) {
  tlsPtr_.stack_size = new_stack_size;
}

inline uint8_t* Thread::GetStackEndForInterpreter(bool implicit_overflow_check) const {
  uint8_t* end = GetStackEnd<StackType::kNative>() + (implicit_overflow_check
