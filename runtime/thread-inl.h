...
inline void Thread::CheckSuspend(bool implicit) {
  ...
  while (true) {
    ...
    if (state_and_flags.IsFlagSet(ThreadFlag::kSuspendRequest)
    ...
    if (state_and_flags.IsFlagSet(ThreadFlag::kEmptyCheckpointRequest)) {
      RunEmptyCheckpoint();
    } else {
...
Locks::thread_list_lock_->AssertNotHeld(this);
