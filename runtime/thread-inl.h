...
inline void Thread::ModifySuspendCount(Thread* self,
                                int delta,
  ...
  // Avoid deadlocks during a thread flip. b/31683379
  DCHECK(!kUseReadBarrier || delta <= 0 || this == self || GetFlipFunction() == nullptr);

  uint32_t flags = enum_cast<uint32_t>(ThreadFlag::kSuspendRequest);
  if (suspendall_barrier != nullptr) {
    DCHECK(delta > 0 && suspend1_barrier == nullptr);
...
inline void Thread::ModifySuspendCount(Thread* self, int delta) {
  ModifySuspendCount(self, delta, nullptr, nullptr, SuspendReason::kInternal);
}
