...
REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  DCHECK_EQ(operands->GetNumberOfOperands(), static_cast<uint32_t>(callsite_type->GetNumberOfPTypes()));
...
bool DoVarHandleInvokeTranslation(Thread* self,
  ...
  if (!mh_ptypes->Get(0)->IsAssignableFrom(callsite_ptypes->Get(0)) ||
  ...
  //
  // When invoking a VarHandle there are potentially two layers of argument conversion.
  //
  //  1. Call-site type to MethodHandle type. The success of this step depends on
  //     whether MethodHandle.invoke() or MethodHandle.invokeExact() was used as well
  //     as the compatability.
  //
  //  2. Conversion from the MethodHandle type to the VarHandle type. The success of this
  //     step depends on whether the MethodHandle represents an invoker created with
  //     MethodHandles.varHandleInvoker() or MethodHandles.varHandleExactInvoker().
  //
  // We should only have to perform one conversion once we've
  // established the path is valid.
  //
  if (invokeExact) {
    const int32_t n = mh_ptypes->GetLength();
    for (int32_t i = 1; i < n; ++i) {
      if (mh_ptypes->Get(i) != callsite_ptypes->Get(i)) {
        ThrowWrongMethodTypeException(method_handle->GetMethodType(), callsite_type.Get());
        return false;
      }
    }
    if (method_handle->GetMethodType()->GetRType() != callsite_type->GetRType()) {
        ThrowWrongMethodTypeException(method_handle->GetMethodType(), callsite_type.Get());
        return false;
    }
  }
  ...
  // TODO: CloneWithoutLeadingParameter allocates and it called twice
  // in this function, a helper class (or a cache) to avoid this would
