...
bool MethodHandleInvoke(Thread* self,
  ...
  StackHandleScope<1> hs(self);
  Handle<mirror::MethodType> method_handle_type(hs.NewHandle(method_handle->GetMethodType()));
  if (callsite_type->IsParameterMatch(method_handle->GetMethodType())) {
    // The parameters from the call site match those of the target MethodHandle.
    //
    // invokeExact() can be used if the call site return value type matches the method handle type
    // OR if the call site return type is void. In the latter case, we'll do the exact invoke but
    // discard the return value. We handle this special case to avoid the expense of an asType()
    // adapter for the simple case.
    //
    // TODO: consider if can include trivial argument conversions, e.g. reference parameters are
    // assignable, 32-bit in-place value widening for primitives (B->I, S->I, C->I, etc).
    if ((callsite_type->GetRType() == method_handle_type->GetRType()) ||
        callsite_type->GetRType()->IsPrimitiveVoid()) {
