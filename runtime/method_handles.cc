...
#define CASE_PRIMITIVE(primitive, _, java_name, __)                        \
    case (primitive): {                                                    \
      jmethodID m = WellKnownClasses::java_lang_ ## java_name ## _valueOf; \
...
bool IsParameterTypeConvertible(ObjPtr<mirror::Class> from, ObjPtr<mirror::Class> to)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  // See rules in MethodType.asType() documentation for following the logic here. This function
  // returns true if there's a conceivable conversion from |from| to |to|.
  if (from == to) {
    return true;
  }
