...
inline bool IsInvokeVarHandle(const mirror::MethodHandle::Kind handle_kind) {
  return handle_kind == mirror::MethodHandle::Kind::kInvokeVarHandleExact;
}
...
static inline bool MethodHandleInvokeMethod(ArtMethod* called_method,
  ...
  ShadowFrameAllocaUniquePtr caller_frame_unique_ptr =
      CREATE_SHADOW_FRAME(num_regs, &caller_frame, called_method, /* dex pc */ 0);
  ShadowFrame* callee_frame = caller_frame_unique_ptr.get();

  // Thread might be suspended during PerformArgumentConversions due to the
  // allocations performed during boxing.
  {
    ScopedStackedShadowFramePusher pusher(
        self, callee_frame, StackedShadowFrameType::kShadowFrameUnderConstruction);
    ...
    CopyArgumentsFromCallerFrame(caller_frame, callee_frame, operands, first_dst_reg);
  }
...
bool DoVarHandleInvokeTranslation(Thread* self,
  ...
  return vh->Access(access_mode, &shadow_frame, &varhandle_operands, result);
}
