...
class TraceWriterTask final : public Task {
 public:
  ...
  void Run(Thread* self ATTRIBUTE_UNUSED) override {
    ...
    if (index_ == -1) {
      // This was a temporary buffer we allocated since there are no more free
      // buffers and we couldn't find one by flushing the pending tasks either.
      // This should only happen when we have fewer buffers than the number of
      // threads. So just free the buffer here.
      free(buffer_);
    } else {
      trace_writer_->ReleaseTraceBufferForThread(index_, reserve_buf_for_tid_);
    }
  }
  ...
  int ReserveBufferForTid(size_t tid) {
...
TraceWriter::TraceWriter(File* trace_file,
    ...
    : trace_file_(trace_file),
      ...
      num_trace_buffers_(num_trace_buffers),
...
int TraceWriterThreadPool::FinishTaskAndClaimBuffer(size_t tid) {
  Thread* self = Thread::Current();
  int index = -1;
  while (index != -1) {
    ...
    if (task == nullptr) {
      ...
      // of threads for efficiency. We make sure that number of buffers are more than the number of
      // threads when tracing has started. It's a bit unlikely to hit this case, so we don't handle
      // this yet.
      LOG(ERROR) << "There are no pending tasks / free buffers.";
      return -1;
    }

    index = task->ReserveBufferForTid(tid);
    task->Run(self);
    task->Finalize();
  }
  return index;
...
void TraceWriter::InitializeTraceBuffers() {
  for (int i = 0; i < num_trace_buffers_; i++) {
    owner_tids_[i].store(0);
  }

  trace_buffer_ = reinterpret_cast<uintptr_t*>(
      malloc(sizeof(uintptr_t) * kPerThreadBufSize * num_trace_buffers_));
  CHECK(trace_buffer_ != nullptr);
}

uintptr_t* TraceWriter::AcquireTraceBuffer(size_t tid) {
  int new_index = -1;
  for (int index = 0; index < num_trace_buffers_; index++) {
    size_t owner = 0;
  ...
  // No free buffers, flush the buffer synchronously and then use that buffer.
  if (new_index == -1) {
    new_index = thread_pool_->FinishTaskAndClaimBuffer(tid);
  }

  if (new_index == -1) {
    // We couldn't find a free buffer even after flushing all the tasks. So
    // allocate a new buffer here. This should only happen if we have more
    // threads than the number of pool buffers
    // TODO(mythria): Add a check for the above case here.
    return reinterpret_cast<uintptr_t*>(malloc(sizeof(uintptr_t) * kPerThreadBufSize));
  }
  return trace_buffer_ + new_index * kPerThreadBufSize;
}

void TraceWriter::ReleaseTraceBufferForThread(int index, size_t tid) {
...
int TraceWriter::GetMethodTraceIndex(uintptr_t* current_buffer) {
  if (current_buffer < trace_buffer_ ||
      current_buffer > trace_buffer_ + (num_trace_buffers_ - 1) * kPerThreadBufSize) {
    // This was the temporary buffer we allocated.
    return -1;
  }
  return (current_buffer - trace_buffer_) / kPerThreadBufSize;
}
