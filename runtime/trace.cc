...
Trace::Trace(File* trace_file,
  ...
  // Update current offset, ensuring visible to all threads.
  cur_offset_.store(kTraceHeaderLength, std::memory_order_seq_cst);
...
void Trace::WriteToBuf(const uint8_t* src, size_t src_size) {
  DCHECK_EQ(TracingMode::kStreaming, trace_output_mode_);
...
void Trace::LogMethodTraceEvent(Thread* thread, ArtMethod* method,
  ...
  // This method is can be called in both tracing modes (method and
