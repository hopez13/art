...
void TraceWriter::FlushEntriesFormatV2(
  ...
  uint8_t* curr_buffer_ptr = init_buffer_ptr;
  ...
  for (size_t i = 0; i < num_records; i++) {
    ...
    // On 64-bit this means method ids would use 8 bytes but that is okay since we use the diff and
    // that is usually expected to be small.
    auto [method_id, is_new_method] = GetMethodEncoding(record.method);
    if (is_new_method && trace_output_mode_ == TraceOutputMode::kStreaming) {
      RecordMethodInfo(method_infos.find(record.method)->second, method_id);
    }
    ...
    if (is_first_entry) {
      ...
      EncodeEventBlockHeader(init_buffer_ptr,
                             ...
                             prev_wall_timestamp,
                             num_records);
      curr_buffer_ptr += header_size;
    } else {
      curr_buffer_ptr = EncodeSignedLeb128(curr_buffer_ptr,
                                           (method_action_encoding - init_method_action_encoding));

      if (has_wall_clock) {
        curr_buffer_ptr =
            EncodeUnsignedLeb128(curr_buffer_ptr, (record.wall_clock_time - prev_wall_timestamp));
        prev_wall_timestamp = record.wall_clock_time;
      }

      if (has_thread_cpu_clock) {
        curr_buffer_ptr =
            EncodeUnsignedLeb128(curr_buffer_ptr, (record.thread_cpu_time - prev_thread_timestamp));
        prev_thread_timestamp = record.thread_cpu_time;
      }
    }
  }
  ...
  Append2LE(total_size_loc, curr_buffer_ptr - (init_buffer_ptr + header_size));
  *current_index += curr_buffer_ptr - init_buffer_ptr;
}
