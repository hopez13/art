...
static void ClearThreadStackTraceAndClockBase(Thread* thread, void* arg [[maybe_unused]]) {
...
void Trace::Start(std::unique_ptr<File>&& trace_file_in,
  ...
  auto deleter = [](File* file) {
    if (file != nullptr) {
      ...
      int result [[maybe_unused]] = file->Close();
      delete file;
    }
  };
...
void Trace::DexPcMoved(Thread* thread [[maybe_unused]],
...
void Trace::FieldRead(Thread* thread [[maybe_unused]],
                      Handle<mirror::Object> this_object [[maybe_unused]],
                      ArtMethod* method,
                      uint32_t dex_pc,
                      ArtField* field [[maybe_unused]]) REQUIRES_SHARED(Locks::mutator_lock_) {
  // We're not recorded to listen to this kind of event, so complain.
  LOG(ERROR) << "Unexpected field read event in tracing " << ArtMethod::PrettyMethod(method)
             << " " << dex_pc;
}

void Trace::FieldWritten(Thread* thread [[maybe_unused]],
                         Handle<mirror::Object> this_object [[maybe_unused]],
                         ArtMethod* method,
                         uint32_t dex_pc,
                         ArtField* field [[maybe_unused]],
                         const JValue& field_value [[maybe_unused]])
...
void Trace::MethodExited(Thread* thread,
                         ...
                         JValue& return_value [[maybe_unused]]) {
  uint32_t thread_clock_diff = 0;
  uint64_t timestamp_counter = 0;
  ReadClocks(thread, &thread_clock_diff, &timestamp_counter);
...
void Trace::MethodUnwind(Thread* thread, ArtMethod* method, uint32_t dex_pc [[maybe_unused]]) {
  uint32_t thread_clock_diff = 0;
  uint64_t timestamp_counter = 0;
  ReadClocks(thread, &thread_clock_diff, &timestamp_counter);
...
void Trace::ExceptionThrown(Thread* thread [[maybe_unused]],
                            Handle<mirror::Throwable> exception_object [[maybe_unused]])
    REQUIRES_SHARED(Locks::mutator_lock_) {
  LOG(ERROR) << "Unexpected exception thrown event in tracing";
}

void Trace::ExceptionHandled(Thread* thread [[maybe_unused]],
                             Handle<mirror::Throwable> exception_object [[maybe_unused]])
    REQUIRES_SHARED(Locks::mutator_lock_) {
  LOG(ERROR) << "Unexpected exception thrown event in tracing";
}
...
void Trace::WatchedFramePop(Thread* self [[maybe_unused]],
                            const ShadowFrame& frame [[maybe_unused]]) {
