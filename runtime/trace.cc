...
// Here we compute the scaling factor by sleeping for a millisecond. Instead, we could generate
// just dump raw timestamp counters and report the time using clock_gettime at the start and the
// end of the trace and compute the frequency in the post processing step. That would require a
// change in Android Studio which is the main consumer of these profiles. For now, just compute
// the frequency of tsc updates here.
...
#if defined(__arm__)
  uint64_t seconds_to_microseconds = 1000 * 1000;
  uint64_t freq = 0;
  // See Architecture Reference Manual ARMv7-A and ARMv7-R edition section B4.1.21
  asm volatile("mrc p15, 0, %0, c14, c0, 0" : "=r"(freq));
  tsc_to_microsec_scaling_factor = seconds_to_microseconds / freq;
#elif defined(__aarch64__)
  uint64_t seconds_to_microseconds = 1000 * 1000;
  uint64_t freq = 0;
  // See Arm Architecture Registers  Armv8 section System Registers
...
ALWAYS_INLINE uint64_t GetMicroTime(uint64_t counter) {
  DCHECK(tsc_to_microsec_scaling_factor > 0.0);
