...
void Trace::PreZygoteFork() {
  MutexLock mu(Thread::Current(), *Locks::trace_lock_);
  if (the_trace_ == nullptr) {
    return;
  }
  // Delete all thread pool threads in the trace writer.
  the_trace_->trace_writer_->DeleteThreads();
}

void Trace::PostZygoteFork() {
  MutexLock mu(Thread::Current(), *Locks::trace_lock_);
  if (the_trace_ == nullptr) {
    return;
  }
  // Create threads if not already created. For the child process these should
  // be already setup but for the parent process we need to create threads.
  // We delete threads in pre-zygote fork.
  the_trace_->trace_writer_->CreateThreads();
}
...
void TraceWriter::DeleteThreads() {
  thread_pool_->WaitForWorkersToBeCreated();
  thread_pool_->DeleteThreads();
}

void TraceWriter::CreateThreads() {
  if (thread_pool_->GetThreadCount() == 0) {
...
void TraceWriter::FinishTracing(int flags, bool finish_tracing, bool flush_file) {
  Thread* self = Thread::Current();
  if (finish_tracing) {
    // Wait for any workers to be created. If we are stopping tracing as a part of runtime shutdown
    // any unstarted workers can create problems if they try attaching while shutting down.
    thread_pool_->WaitForWorkersToBeCreated();
    ...
    DCHECK_EQ(thread_pool_->GetThreadCount(), 0u);
    thread_pool_->WaitForWorkersToBeCreated();
  }
