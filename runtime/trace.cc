...
void Trace::StopTracing(bool finish_tracing, bool flush_file) {
  ...
  // are now visible.
  if (finish_tracing) {
    the_trace->trace_writer_->FinishTracing(the_trace->flags_, flush_file);
  }
  delete the_trace;
...
void TraceWriter::FinishTracing(int flags, bool flush_file) {
...
void Trace::LogMethodTraceEvent(Thread* thread,
  ...
  if (*current_offset < required_entries) {
    // This means there was a overflow and we cannot record any more entries in the buffer.
    // This only happens in non-streaming mode where we have a fixed size buffer.
