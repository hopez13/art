...
void Trace::FinishTracing() {
  // The caller has acquired one or more locks which ensures all
  // writes in flight for the trace buffer (or other fields) are
  // visible.
...
bool Trace::NonStreamingReserveFixedSizeRecord(size_t record_bytes, size_t* offset) {
  // Find a region in buffer to write record, advancing cur_offset_ atomically.
  *offset = cur_offset_.load(std::memory_order_relaxed);  // Speculative read
  do {
    if (offset + bytes > buffer_size_) {
      return false;
    }
  } while (!cur_offset_.compare_exchange_weak(*offset, *offset + bytes, std::memory_order_relaxed));
  return true;
}
...
void Trace::WriteMethodTraceEvent(size_t reserved_offset,
  ...
  DCHECK_LE(ptr - buf.get(), reserved_bytes);
}
