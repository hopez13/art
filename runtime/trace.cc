...
TraceWriter::TraceWriter(File* trace_file,
  ...
  // Don't create threadpool for a zygote. This causes slowdown when forking because we need to
  // stop and start this thread pool. Method tracing on zygote isn't a frequently used case and
...
void TraceWriter::RecordMethodInfo(ArtMethod* method,
  ...
  tracing_lock_.ExclusiveUnlock(Thread::Current());
  // This method is called both by thread pool worker thread (in streaming mode) or by the mutator
  // thread (when tracing is stopped or non-streaming). We may need ScopedObjectAccess depending on
  // whether we are thread pool worker or not.
  if (!Locks::mutator_lock_->IsSharedHeld(Thread::Current())) {
    ScopedObjectAccess soa(Thread::Current());
    method_line = GetMethodLine(method, method_index);
  } else {
    Locks::mutator_lock_->AssertSharedHeld(Thread::Current());
    method_line = GetMethodLine(method, method_index);
  }
  tracing_lock_.ExclusiveLock(Thread::Current());
