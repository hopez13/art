...
TraceDebugLevel GetTraceDebugLevelFromFlags(int flags, Runtime::RuntimeDebugState runtime_state) {
  // TODO(mythria): This is a bit of a workaround to make it work more intuitively till flags are
  // correctly supported for specifying debug level.
  if (runtime_state == Runtime::RuntimeDebugState::kNonJavaDebuggable) {
...
bool ShouldUpdateDebugState(TraceDebugLevel debug_level, Runtime::RuntimeDebugState runtime_state) {
  if (runtime_state == Runtime::RuntimeDebugState::kJavaDebuggable || runtime_state == Runtime::RuntimeDebugState::kJavaDebuggableAtInit) {
    return false;
...
void Trace::Start(std::unique_ptr<File>&& trace_file_in,
  ...
  // Create Trace object.
  {
    ...
    if (the_trace_ != nullptr) {
      ...
      TraceDebugLevel debug_level = GetTraceDebugLevelFromFlags(flags, runtime->GetRuntimeDebugState());
      ...
      if (trace_mode == TraceMode::kSampling) {
        ...
        if (ShouldUpdateDebugState(debug_level, runtime->GetRuntimeDebugState())) {
          if (runtime->GetRuntimeDebugState() != Runtime::RuntimeDebugState::kNonJavaDebuggable) {
            LOG(FATAL) << "This is unsupported -- switching trace level while using AOT code when"
                       << "possible isn't fully supported. It requires support to identify AOT code"
                       << " via code Info to properly support it.";
          }
