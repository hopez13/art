...
void FlushAlwaysOnThreadBuffer(uintptr_t* buf, uintptr_t* curr_index)
  REQUIRES_SHARED(Locks::mutator_lock_) {
  uintptr_t kMethodEntryMask = 0x1;
  std::unordered_map<ArtMethod*, uint32_t> art_method_id_map;
  uint32_t current_method_index = 0;

  int size_entry = 0;
  int size_method_info = 0;
  int num_methods = 0;
  int size_exit = 0;
  int index = curr_index - buf;
  int start_index = index;
  while (true) {
    // Update index;
    index = index - 1;
    if (index <= 0) {
      index = kPerThreadBufSize - 1;
    }
    if (index == start_index) {
      break;
    }

    uintptr_t curr_entry = buf[index];
    if (curr_entry == 0) {
      // Empty entry. Just skip
    } else if ((curr_entry & kMethodEntryMask) == 1) {
      size_exit += 1;
    } else {
      ArtMethod* method = reinterpret_cast<ArtMethod*>(curr_entry & (~kMethodEntryMask));
      method = method->GetInterfaceMethodIfProxy(kRuntimePointerSize);

      auto it = art_method_id_map.find(method);
      uint32_t method_index = 0;
      // If we haven't seen this method before record information about the method.
      if (it == art_method_id_map.end()) {
        art_method_id_map.emplace(method, current_method_index);
        method_index = current_method_index;
        current_method_index++;

        // Write a special block with the name.
        std::string method_line(StringPrintf("%#x\t%s\t%s\t%s\t%s\n",
        ...
        size_method_info += method_line.length();
        size_method_info += 5;
        num_methods += 1;
      } else {
  ...
  LOG(ERROR) << "method_info:entry:exit " << size_method_info << " " << size_entry << " " << size_exit << " " << num_methods;
}

void Trace::FlushThreadBuffer(Thread* self) {
  if (the_trace_ == nullptr) {
    // This is for always-on-profiling.
    // FlushAlwaysOnThreadBuffer(self->GetMethodTraceBuffer(), *(self->GetMethodTraceIndexPtr()));
    return;
  }
  MutexLock mu(self, *Locks::trace_lock_);
