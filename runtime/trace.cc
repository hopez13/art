...
uintptr_t* TraceWriter::AcquireTraceBuffer(size_t tid) {
  ...
  // Waiting to suspend all therads.
  ...
  // in the same order. That ensures we don't suspend in tracing logic after Trace::StopTracing has
  // returned. This is required so that we can safely delete tracing data.
