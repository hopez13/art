...
ArtMethod* Trace::DecodeTraceMethod(uint32_t tmid) {
  ...
  // This is used only for logging which is usually needed only for debugging. So it's not
...
Trace::Trace(File* trace_file,
    ...
    : trace_file_(trace_file),
      ...
      overflow_(false),
      interval_us_(0),
      tracing_lock_(new Mutex("tracing lock", LockLevel::kTracingStreamingLock)) {
...
void Trace::FlushStreamingBuffer(Thread* thread) {
  ...
  for (size_t entry_index = 0; entry_index < num_entries;) {
    ...
    EncodeEventEntry(buffer.get() + current_index, thread, method_index, action, thread_time, wall_time);
...
void Trace::RecordMethodEvent(Thread* thread,
  ...
  MutexLock mu(Thread::Current(), *tracing_lock_);
  EncodeEventEntry(ptr, thread, EncodeTraceMethod(method), action, thread_clock_diff, wall_clock_diff);
}
