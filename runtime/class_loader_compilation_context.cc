...
static const std::string kPathClassLoaderString = "PCL";
static const std::string kDelegateLastClassLoaderString = "DLC";
...
bool ClassLoaderCompilationContext::ParseClassLoaderSpec(
  ...
  CHECK_EQ(0, class_loader_spec.compare(0, class_loader_type_str.length(), class_loader_type_str));
  ...
  // Note that we allowed class loaders with an empty class path in order to support a non default
  // class loader for the source dex files.
  // i.e. by default all apks are loaded with PathClassLoader, but apps can specify a different
  // class loader for their primary apk (e.g. DelegateLastClassLoader). Even if in this case
  // it won't make a difference for the resolution order we need to be sure we are consistent
  // and reflect the correct class loaders in the oat headers.
...
// Return ClassLoaderCompilationContext::kInvalidClassLoader if the class loader type is not
// recognized.
ClassLoaderCompilationContext::ClassLoaderType
ClassLoaderCompilationContext::ExtractClassLoaderType(const std::string& class_loader_spec) const {
  const ClassLoaderType kValidTypes[] = {kPathClassLoader, kDelegateLastClassLoader};
  for (const ClassLoaderType& type : kValidTypes) {
    std::string type_str = GetClassLoaderTypeName(type);
    if (class_loader_spec.compare(0, type_str.length(), type_str) == 0) {
      return type;
    }
  }
...
bool ClassLoaderCompilationContext::OpenDexFiles(InstructionSet isa,
  ...
  for (ClassLoaderInfo& info : class_loader_chain_) {
    for (const std::string& cp_elem : info.classpath) {
      ...
      // When opening the dex files from the context we expect their checksum to match. So pass
      // true to verify_checksum.
      if (!DexFile::Open(location.c_str(),
        ...
        // This could happen when we need to recompile a pre-build whose dex code has bee stripped.
