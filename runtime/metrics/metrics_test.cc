  ...
  class TestBackend : public TestBackendBase {
   public:
    ...
    void ReportHistogram(DatumId,
      ...
      const int64_t percentile = HistogramPercentile(0.50, minimum_value, maximum_value, cumulative_buckets);
      EXPECT_LT(percentile, high_value);
      EXPECT_GT(percentile, low_value);

      // Some basic reasonableness checks for confidence intervals.
      const auto interval = HistogramConfidenceInterval(0.95, minimum_value, maximum_value, cumulative_buckets);
