...
// The functionality is identically with the exception of the metrics source.
// Instead of taking its metrics from the current Runtime, this class will
// keep its own copy so that it does not get interference from other runtime
// setup logic.
...
class MetricsReporterTest : public CommonRuntimeTest {
 protected:
  ...
  void VerifyReports(uint32_t size, bool with_metrics) {
    ...
    // We should ads one
    ASSERT_EQ(backend_->reports_.size(), size);
    for (auto report : backend_->reports_) {
