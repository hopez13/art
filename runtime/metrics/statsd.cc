...
// EncodeDatumId a std::optional that optionally provides a enum value from atoms.proto.
// We use an optionalbecause the list of datums that ART collects is a superset of what we
  ...
  void ReportCounter(DatumId counter_type, uint64_t value) override {
    ...
    if (datum_id.has_value()) {
      statsd::stats_write(
          ...
          current_timestamp_,
          statsd::ART_DATUM_REPORTED__THREAD_TYPE__ART_THREAD_UNKNOWN,  // TODO: collect and
                                                                        // report thread type
          datum_id.value(),
