...
void MetricsReporter::ReloadConfig(ReportingConfig config) {
...
void MetricsReporter::BackgroundThreadRun() {
  LOG_STREAM(ERROR) << "Metrics reporting thread started";
  ...
  while (running) {
    messages_.SwitchReceive(
        [&](BeginSessionMessage message) {
          ...
          LOG_STREAM(ERROR) << "Received session metadata: " << session_data_.session_id;
        },
        [&]([[maybe_unused]] ShutdownRequestedMessage message) {
          LOG_STREAM(ERROR) << "Shutdown request received " << session_data_.session_id;
          running = false;

          ReportMetrics();
        },
        [&](RequestMetricsReportMessage message) {
          LOG_STREAM(ERROR) << "Explicit report request received " << session_data_.session_id;
          ReportMetrics();
        ...
        [&]([[maybe_unused]] TimeoutExpiredMessage message) {
          LOG_STREAM(ERROR) << "Timer expired, reporting metrics" << session_data_.session_id;

          ReportMetrics();
          MaybeResetTimeout();
        },
        [&]([[maybe_unused]] StartupCompletedMessage message) {
          LOG_STREAM(ERROR) << "App startup completed, reporting metrics" << session_data_.session_id;
          ReportMetrics();
          startup_reported_ = true;
          MaybeResetTimeout();
        },
        [&](CompilationInfoMessage message) {
          LOG_STREAM(ERROR) << "Compilation info received" << session_data_.session_id;
  ...
  LOG_STREAM(ERROR) << "Metrics reporting thread terminating";
}

void MetricsReporter::MaybeResetTimeout() {
  if (ShouldReportAtPeriod()) {
...
bool MetricsReporter::ShouldReportAtStartup() const {
  return config_.period_spec != nullptr && config_.period_spec->report_startup_first;
}

bool MetricsReporter::ShouldReportAtPeriod() const {
  bool result =
      // Only if we have period spec
      config_.period_spec != nullptr &&
      ...
      // and we still have unreported intervals or we are asked to report continuosly.
      (config_.period_spec->continuous_reporting ||
              (report_interval_index_ < config_.period_spec->periods_seconds.size()));

  LOG(ERROR) << "CALIN result="  << result
      << " sr="
      << startup_reported_
      << " ri="
      ...
      << " ps="
      << (config_.period_spec == nullptr ? 999 : config_.period_spec->periods_seconds.size());

  return result;
}

uint32_t MetricsReporter::GetNextPeriodSeconds() {
  CHECK(ShouldReportAtPeriod());

  // The index is either the current report_interval_index or the last index
  // if we are in continuos mode and reached the end.
...
ReportingConfig ReportingConfig::FromFlags(bool is_system_server) {
  ...
  std::unique_ptr<ReportingPeriodSpec> period_spec = nullptr;
  ...
  return {
      ...
      .period_spec = std::move(period_spec),
  };
}

std::unique_ptr<ReportingPeriodSpec> ReportingPeriodSpec::Parse(
  ...
  *error_msg = "";
  ...
  std::unique_ptr<ReportingPeriodSpec> spec(new ReportingPeriodSpec());
  spec->spec = spec_str;
