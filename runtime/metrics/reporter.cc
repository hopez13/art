  ...
  while (running) {
    messages_.SwitchReceive(
        [&](BeginSessionMessage message) {
          LOG_STREAM(ERROR) << "Received session metadata";
          session_data_ = message.session_data;
        },
        [&]([[maybe_unused]] ShutdownRequestedMessage message) {
          LOG_STREAM(ERROR) << "Shutdown request received";
          running = false;

          ReportMetrics();
        },
        [&](RequestMetricsReportMessage message) {
          LOG_STREAM(ERROR) << "Explicit report request received";
          ReportMetrics();
        ...
        [&]([[maybe_unused]] TimeoutExpiredMessage message) {
          LOG_STREAM(ERROR) << "Timer expired, reporting metrics";

          ReportMetrics();
          MaybeResetTimeout();
        },
        [&]([[maybe_unused]] StartupCompletedMessage message) {
          LOG_STREAM(ERROR) << "App startup completed, reporting metrics";
          ReportMetrics();
          startup_reported_ = true;
          MaybeResetTimeout();
        },
        [&](CompilationInfoMessage message) {
          LOG_STREAM(ERROR) << "Compilation info received";
...
bool MetricsReporter::ShouldReportAtPeriod() const {
  LOG(ERROR) << "CALIN " << startup_reported_ << " " << report_interval_index_;
  return
      // Only if we have period spec.
      config_.period_spec != nullptr &&
      // And if we already reported startup or not required to report startup
      (startup_reported_ || !config_.period_spec->report_startup_first) &&
      // And if we still have reported interval or we are asked to report continuosly.
...
uint32_t MetricsReporter::GetNextPeriodSeconds() {
  ...
  uint32_t result = config_.period_spec->periods_seconds[report_interval_index_];
  // Advance the index if we didn't get to the end.
  if (report_interval_index_ < config_.period_spec->periods_seconds.size() - 1) {
