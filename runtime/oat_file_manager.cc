...
bool OatFileManager::HasCollisions(const OatFile* oat_file,
  ...
  if (!class_loader_ok) {
      VLOG(class_linker) << "Skipping duplicate class check due to unrecognized classloader";
      return false;
  }
    ...
    if (!accept_oat_file) {
      ...
      if (Runtime::Current()->IsDexFileFallbackEnabled()) {
        LOG(WARNING) << "Found duplicate classes, falling back to extracting from APK : "
                     << dex_location;
        // We won't waste RAM or hurt startup performance if we have no original
        // dex files, but we are executing potentially incorrect code in this
        // case. (We warn about that later).
