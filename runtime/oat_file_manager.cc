...
static bool CreateDirectories(const std::string& child_path, /* out */ std::string* out) {
...
class AnonymousDexFileVerifierTask final : public Task {
 public:
  AnonymousDexFileVerifierTask(const std::string& vdex_path,
  ...
  void Run(Thread* self) override {
    ...
    verifier::VerifierDeps verifier_deps(dex_files_, /* ignore_external_deps= */ true);
    {
      ...
      for (const auto& dex_file : dex_files_) {
        ...
        for (uint32_t cdef_idx = 0; cdef_idx < dex_file->NumClassDefs(); cdef_idx++) {
          ...
          h_class.Assign(class_linker->FindClass(self, descriptor, h_loader));
          if (h_class == nullptr) {
    ...
    if (!CreateDirectories(vdex_path_, &error_msg) ||
        !UnlinkLeastRecentlyUsedVdexIfNeeded(vdex_path_, &error_msg) ||
 ...
 private:
  ...
  DISALLOW_IMPLICIT_CONSTRUCTORS(AnonymousDexFileVerifierTask);
};
...
void OatFileManager::VerifyAnonymousDexFiles(const std::vector<const DexFile*>& dex_files,
                                             jobject class_loader,
                                             const char* class_loader_context) {
  ...
  if (OatFileAssistant::AnonymousDexVdexLocation(GetDexFileHeaders(dex_files),
    ...
    verification_thread_pool_->AddTask(self, new AnonymousDexFileVerifierTask(
        vdex_path,
        dex_files,
        class_loader,
...
void OatFileManager::DeleteVerifierThreadPool() {
  Thread* self = Thread::Current();
...
void OatFileManager::WaitForVerifierTasks() {
  Thread* self = Thread::Current();
  if (verification_thread_pool_ != nullptr) {
