...
class VerifyTask final : public Task {
 public:
  VerifyTask(const std::string& vdex_path,
    ...
    class_loader_ = soa.Vm()->AddGlobalRef(soa.Self(),
  ...
  void Run(Thread* self) override {
    if (!CreateDirectories(vdex_path_)) {
      LOG(ERROR) << "Could not create directories for " << vdex_path_;
      return;
    }
    ...
    std::string error_msg;
    bool verification_skipped;  // result unused

    verifier::VerifierDeps verifier_deps(dex_files_, /* ignore_external_deps= */ true);
    {
      ScopedObjectAccess soa(self);
      ...
      for (const auto& dex_file : dex_files_) {
        ...
        for (uint32_t cdef_idx = 0; cdef_idx < dex_file->NumClassDefs(); cdef_idx++) {
          ...
          h_class.Assign(class_linker->FindClass(soa.Self(), descriptor, h_loader));
    ...
    if (!UnlinkLeastRecentlyUsedVdexIfNeeded(vdex_path_, &error_msg) ||
        !VdexFile::WriteToDisk(vdex_path_,
                               dex_files_,
                               verifier_deps,
                               class_loader_context_,
                               &error_msg)) {
      LOG(ERROR) << "Could not write " << vdex_path_ << ": " << error_msg;
      return;
    }
  }
 ...
 private:
  ...
  DISALLOW_IMPLICIT_CONSTRUCTORS(VerifyTask);
};
...
uint32_t OatFileManager::ComputeAnonymousDexLocationChecksum(
  ...
  for (const DexFile::Header* dex_header : dex_headers) {
    checksum = adler32_combine(checksum,
                               ...
                               dex_header->file_size_ - DexFile::NumNonChecksumBytes());
  }
  return checksum;
}
...
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  std::vector<const DexFile::Header*> dex_headers = GetDexFileHeaders(dex_mem_maps);
  const uint32_t location_checksum = ComputeAnonymousDexLocationChecksum(dex_headers);

  // Determine to dex/vdex locations.
  std::string dex_location;
  std::string vdex_path;
  bool has_vdex =
      OatFileAssistant::DexFilesToAnonymousDexLocation(location_checksum, &dex_location) &&
  ...
  // TODO: Explain that we don't need a class loader collision check because
  ...
  if (oat_file != nullptr) {
    ...
    *out_oat_file = RegisterOatFile(std::move(oat_file));
    ...
    LOG(ERROR) << "Could not initialize oat file from " << vdex_path << ": " << error_msg;
  }

  return DisableWrite(dex_files, error_msgs);
}

bool OatFileManager::PreverifyDexFiles(const std::vector<const DexFile*>& dex_files,
                                       jobject class_loader,
                                       const char* class_loader_context,
                                       /* out */ std::string* error_msg) {
  DCHECK(class_loader_context != nullptr);
  Thread* self = Thread::Current();

  std::string dex_location;
  std::string vdex_path;
  if (!OatFileAssistant::DexFilesToAnonymousDexLocation(
          ComputeAnonymousDexLocationChecksum(GetDexFileHeaders(dex_files)),
          &dex_location) ||
      !OatFileAssistant::DexLocationToVdexFilename(
          dex_location,
          kRuntimeISA,
          &vdex_path,
          error_msg)) {
    // We don't have a path for vdex. That means the optimization is disabled. Report success.
    return true;
  }
  ...
  verification_thread_pool_->AddTask(self, new VerifyTask(vdex_path,
                                                          dex_files,
                                                          class_loader,
...
void OatFileManager::WaitForVerifyTasks() {
  Thread* self = Thread::Current();
  if (verification_thread_pool_ != nullptr) {
