...
const OatFile* OatFileManager::RegisterOatFile(std::unique_ptr<const OatFile> oat_file) {
  ...
  CHECK(!only_use_system_oat_files_ ||
        LocationIsTrusted(oat_file->GetLocation(), !Runtime::Current()->GetArtApexDataIsUntrusted()) ||
...
void OatFileManager::RunBackgroundVerification(const std::vector<const DexFile*>& dex_files,
  ...
  if (LocationIsOnArtApexData(odex_filename) && Runtime::Current()->GetArtApexDataIsUntrusted()) {
...
void OatFileManager::SetOnlyUseTrustedOatFiles() {
  ...
  for (const std::unique_ptr<const OatFile>& oat_file : oat_files_) {
    if (boot_set.find(oat_file.get()) == boot_set.end()) {
      if (!LocationIsTrusted(oat_file->GetLocation(), /* trust_art_apex_data_files=*/ true)) {
