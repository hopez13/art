...
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  if ((class_loader != nullptr || dex_elements != nullptr) && oat_file != nullptr) {
    ...
    bool accept_oat_file = check_collision_result != CheckCollisionResult::kPerformedHasCollisions;
    if (!accept_oat_file) {
    ...
    if (source_oat_file->IsExecutable()) {
      ...
      if (kEnableAppImage && (!runtime->IsJavaDebuggable() || source_oat_file->IsDebuggable())) {
        ...
        if (!load_image &&
          ...
          load_image = !CheckClassCollision(source_oat_file, context.get(), &error_msg);
        }
        if (load_image) {
