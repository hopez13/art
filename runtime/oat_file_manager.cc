...
class BackgroundVerificationTask final : public Task {
 public:
  ...
  void Run(Thread* self) override {
    ...
    // Check how many vdex files exist in the same directory as the vdex file we
    // are about to write. If more than or equal to kAnonymousVdexCacheSize, unlink
    // the least recently used one(s) (according to stat-reported atime).
...
void OatFileManager::RunBackgroundVerification(const std::vector<const DexFile*>& dex_files,
  ...
  if (OatFileAssistant::AnonymousDexVdexLocation(GetDexFileHeaders(dex_files),
                                                 ...
                                                 &dex_location,
                                                 &vdex_path)) {
    // Threads created by ThreadPool are considered runtime threads and not allowed
    // to load classes when debuggable. Load them prior to creating the background thread.
    if (Runtime::Current()->IsJavaDebuggable()) {
      ...
      for (const DexFile* dex_file : dex_files) {
        for (uint32_t cdef_idx = 0; cdef_idx < dex_file->NumClassDefs(); cdef_idx++) {
          if (class_linker->FindClass(self,
