...
class BackgroundVerificationTask final : public Task {
 public:
  ...
  void Run(Thread* self) override {
    ...
    // Make sure all dex files are registered. This is done lazily, typically during
    ...
    for (const DexFile* dex_file : dex_files_) {
      ...
      if (class_linker->RegisterDexFile(*dex_file, h_loader.Get()) == nullptr) {
        ...
        LOG(ERROR) << "Failed to register dex file " << dex_file->GetLocation()
            << "before background verification: " << self->GetException()->Dump();
        self->ClearException();
        return;
      }
    }
