    ...
    if (oat_file != nullptr) {
      ...
      if (ShouldLoadAppImage(oat_file.get())) {
        ...
        if (image_space == nullptr && !compilation_enabled) {
          std::string art_file = runtime->GetRuntimeImagePath(dex_location);
          std::string error_msg;
          ScopedObjectAccess soa(self);
          image_space = gc::space::ImageSpace::CreateFromAppImage(art_file.c_str(), oat_file.get(), &error_msg);
          if (image_space == nullptr) {
            LOG(DEBUG) << "Could not load runtime generated app image: " << error_msg;
          }
        }
      }
      if (image_space != nullptr) {
