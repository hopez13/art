        ...
        if (UNLIKELY(current_fragment->GetTopQuickFrameGenericJniTag())) {
          ...
          if (!class_linker->IsQuickGenericJniStub(existing_entry_point) &&
            ...
            if (code != nullptr) {
              ...
              // running JITed code or boot stub. For debuggable runtimes we might have JIT code
              // even when AOT code or boot stub is present but we tag SP in JITed JNI stubs
