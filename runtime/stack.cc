...
void StackVisitor::WalkStack(bool include_transitions) {
  ...
  uint64_t current_instrumentation_id;
  ...
  do {
    ...
    {
      MutexLock mu(self, *GetThread()->GetInstrumentationInstallIdMutex());
      current_instrumentation_id = GetThread()->GetInstrumentationInstallId();
    }
          ...
          if (UNLIKELY(exit_stubs_installed ||
            ...
            if (reinterpret_cast<uintptr_t>(GetQuickInstrumentationExitPc()) == return_pc) {
              ...
              if (!IsStackInstrumentWalk() &&
                  GetThread()->GetInstrumentationInstallId() != current_instrumentation_id) {
                if (kDebugStackWalk) {
