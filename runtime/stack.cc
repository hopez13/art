...
bool StackVisitor::GetVRegFromOptimizedCode(ArtMethod* m, uint16_t vreg, VRegKind kind,
  ...
  const DexFile::CodeItem* code_item = m->GetCodeItem();
  DCHECK(code_item != nullptr) << m->PrettyMethod();  // Can't be null or how would we
                                                      // compile its instructions?
...
std::string StackVisitor::DescribeLocation() const {
  ...
  result += ArtMethod::PrettyMethod(m);
  ...
  if (kIsDebugBuild) {
    ...
    if (!linear_alloc->Contains(method)) {
      ...
      if (!class_linear_alloc->Contains(method)) {
        ...
        CHECK(in_image) << ArtMethod::PrettyMethod(method) << " not in linear alloc or image";
      }
    }
    if (cur_quick_frame_ != nullptr) {
      ...
      CHECK_LE(frame_size, kMaxExpectedFrameSize) << ArtMethod::PrettyMethod(method);
...
QuickMethodFrameInfo StackVisitor::GetCurrentQuickFrameInfo() const {
  ...
  const void* entry_point = runtime->GetInstrumentation()->GetQuickCodeFor(method,
                                                                           kRuntimePointerSize);
  DCHECK(class_linker->IsQuickGenericJniStub(entry_point)) << ArtMethod::PrettyMethod(method);
  // Generic JNI frame.
  ...
  if (!found_object) {
    ...
    self->ThrowNewExceptionF("Ljava/lang/IllegalMonitorStateException;",
                             ...
                             mirror::Object::PrettyTypeOf(
                                 const_cast<mirror::Object*>(obj)).c_str());
  }
}
