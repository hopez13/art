...
uint8_t* StackVisitor::GetReturnPcAddr() const {
  ...
  for (const ManagedStack* current_fragment = thread_->GetManagedStack();
    ...
    if (cur_quick_frame_ != nullptr) {  // Handle quick stack frames.
      ...
      while (method != nullptr) {
        ...
        uint8_t* return_pc_addr = GetReturnPcAddr();
        uintptr_t return_pc = *reinterpret_cast<uintptr_t*>(return_pc_addr);

        if (reinterpret_cast<uintptr_t>(GetQuickInstrumentationExitPc()) == return_pc) {
          // While profiling, the return pc is restored from the side stack, except when walking
          // the stack for an exception where the side stack will be unwound in VisitFrame.
          const std::map<uint8_t*, instrumentation::InstrumentationStackFrame>& instrumentation_stack =
              *thread_->GetInstrumentationStack();
          const auto& pair = instrumentation_stack.find(return_pc_addr);
          CHECK(pair != instrumentation_stack.end());
          const instrumentation::InstrumentationStackFrame& instrumentation_frame = pair->second;
          if (GetMethod() ==
