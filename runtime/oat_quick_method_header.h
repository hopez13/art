...
#if defined(__aarch64__)  // Let's not make assumptions about other architectures.
    // Verify that the code pointer is not tagged. Memory for code gets allocated with
    // mspace_memalign which does not get tagged by either MTE or HWASan.
    DCHECK_EQ(code_start, reinterpret_cast<uintptr_t>(p) & ((1ULL << 56) - 1));
#endif
