...
class ClassTable {
 public:
  ...
  EXPORT size_t NumReferencedZygoteClasses() const REQUIRES(!lock_)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  EXPORT size_t ReadFromMemory(uint8_t* ptr) REQUIRES(!lock_) REQUIRES_SHARED(Locks::mutator_lock_);
