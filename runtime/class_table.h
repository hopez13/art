  ...
  class PACKED(4) TableSlot {
   public:
    ...
    TableSlot(ObjPtr<mirror::Class> klass, uint32_t descriptor_hash)
        : data_(Encode(klass, descriptor_hash & kHashMask)) {}
    ...
    uint32_t Hash() const {
      return data_.LoadRelaxed() & kHashMask;
    }

    bool MaskedHashEquals(uint32_t other) const {
      return (other & kHashMask) == Hash();
    }
   ...
   private:
    ...
    // Data contains the class pointer as well as the low bits of the descriptor hash.
