...
class ClassLoaderContext {
 public:
  ...
  static constexpr const char* EncodedUnsupportedClassLoaderContext =
  ...
  //
  // If a foreign ClassLoader is found in the class loader hierarchy, then
  // an vector of ClassLoaderContext::EncodedUnsupportedClassLoaderContext will be returned (one for
  // each classpath entry in `class_loader`). If `class_loader` is not a BaseDexClassLoader then the
  // result is empty.
  static std::vector<std::string> EncodeClassPathContextsForClassLoader(jobject class_loader);
