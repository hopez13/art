...
static uint32_t GetDebugInfoOffsetInternal(uint32_t offset_in_code_item,
                                           const ArrayRef<const uint8_t>& quickening_info) {
  if (quickening_info.size() == 0) {
    // No quickening info: offset is the right one, return it.
    return offset_in_code_item;
  }
  uint32_t quickening_offset = offset_in_code_item - DexFile::kMaxDebugInfoOffsetForQuickening;
  return *reinterpret_cast<const unaligned_uint32_t*>(quickening_info.data() + quickening_offset);
}

static uint32_t GetQuickeningInfoOffsetFrom(uint32_t offset_in_code_item,
                                            const ArrayRef<const uint8_t>& quickening_info) {
  if (offset_in_code_item < DexFile::kMaxDebugInfoOffsetForQuickening) {
    return kNoQuickeningInfoOffset;
  }
  if (quickening_info.size() == 0) {
    // No quickening info.
    return kNoQuickeningInfoOffset;
  ...
  CHECK_LT(quickening_offset + 2 * sizeof(uint32_t), quickening_info.size());
...
static ArrayRef<const uint8_t> GetQuickeningInfoAt(const ArrayRef<const uint8_t>& quickening_info,
  ...
  return (quickening_offset == kNoQuickeningInfoOffset)
    ...
    if (class_data != nullptr) {
      for (ClassDataItemIterator class_it(target_dex_file, class_data);
        ...
        if (class_it.IsAtMethod() && class_it.GetMethodCodeItem() != nullptr) {
          ...
          uint32_t quickening_offset =
              GetQuickeningInfoOffsetFrom(code_item->debug_info_off_, quickening_info);
          if (quickening_offset != kNoQuickeningInfoOffset) {
            // If we have quickening data, put back the original debug_info_off.
            const_cast<DexFile::CodeItem*>(code_item)->debug_info_off_ =
                GetDebugInfoOffsetInternal(code_item->debug_info_off_, quickening_info);
          }
...
uint32_t VdexFile::GetDebugInfoOffset(uint32_t offset_in_code_item) const {
  return GetDebugInfoOffsetInternal(offset_in_code_item, GetQuickeningInfo());
}
...
const uint8_t* VdexFile::GetQuickenedInfoOf(const DexFile& dex_file,
  ...
  uint32_t quickening_offset = GetQuickeningInfoOffsetFrom(
      dex_file.GetCodeItem(code_item_offset)->debug_info_off_, quickening_info);
