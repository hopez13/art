...
std::unique_ptr<VdexFile> VdexFile::Open(int file_fd,
  ...
  if (unquicken) {
    ...
    // We need to use a non-uniqe_ptr vector for Unquicken.
    std::vector<const DexFile*> dex_files;
    for (const std::unique_ptr<const DexFile>& dex_file : unique_ptr_dex_files) {
      dex_files.push_back(dex_file.get());
    }
    Unquicken(dex_files, vdex->GetQuickeningInfo());
