...
void VdexFile::Unquicken(const std::vector<const DexFile*>& dex_files,
  ...
  for (const DexFile* dex_file : dex_files) {
    UnquickenDexFile(*dex_file, *dex_file, quickening_info, decompile_return_instruction);
  }
}

static uint32_t GetQuickeningInfoTableOffset(const DexFile& dex_file) {
  return reinterpret_cast<const VdexFile::QuickeningTableOffsetType*>(dex_file.Begin())[-1];
}

QuickenInfoOffsetTableAccessor GetQuickenInfoOffsetTable(
    const DexFile& source_dex_file,
    const ArrayRef<const uint8_t>& quickening_info) {
  // The offset a preheader right before the dex file.
  const uint32_t offset = GetQuickeningInfoTableOffset(source_dex_file);
  const uint8_t* data_ptr = quickening_info.data() + offset;
  return QuickenInfoOffsetTableAccessor(data_ptr, source_dex_file.NumMethodIds());
}
...
// QuickenInfoOffsetTableAccessor table(GetQuickenInfoOffsetTable(dex_file, quickening_info));
      ...
      for (ClassDataItemIterator class_it(target_dex_file, class_data);
        ...
        if (class_it.IsAtMethod() && class_it.GetMethodCodeItem() != nullptr) {
          ...
          const uint32_t quickening_offset = GetQuickeningInfoOffset(
              GetQuickenInfoOffsetTable(source_dex_file, quickening_info),
              class_it.GetMemberIndex(),
              quickening_info);
          ArrayRef<const uint8_t> data(GetQuickeningInfoAt(quickening_info, quickening_offset));
