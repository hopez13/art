...
static std::string ComputeClassLoaderContextString(const ClassLoaderContext& context) {
  return context.EncodeContextForOatFile(/* classpath_dir= */ "");
}
...
bool VdexFile::WriteToDisk(const std::string& path,
  ...
  std::string str_boot_checksum = ComputeBootClassPathChecksumString();
  std::string str_context = ComputeClassLoaderContextString(context);
  DCHECK_NE(str_boot_checksum, "");
  DCHECK_NE(str_context, "");

  VdexFile::VerifierDepsHeader deps_header(dex_files.size(),
                                           verifier_deps_data.size(),
                                           /* has_dex_section= */ false,
                                           str_boot_checksum.size(),
                                           str_context.size());
  ...
  if (!out->WriteFully(str_boot_checksum.c_str(), str_boot_checksum.size())) {
    *error_msg = "Could not write boot classpath checksum to " + path;
    out->Unlink();
    return false;
  }

  if (!out->WriteFully(str_context.c_str(), str_context.size())) {
...
bool VdexFile::MatchesDexFileChecksums(const std::vector<const DexFile*>& dex_files) const {
  const VerifierDepsHeader& header = GetVerifierDepsHeader();
  if (dex_files.size() != header.GetNumberOfDexFiles()) {
    LOG(WARNING) << "Mismatch of number of dex files in vdex (expected="
        << header.GetNumberOfDexFiles() << ", actual=" << dex_files.size() << ")";
    return false;
  }
  const VdexChecksum* checksums = header.GetDexChecksumsArray();
  for (size_t i = 0; i < dex_files.size(); ++i) {
    if (checksums[i] != dex_files[i]->GetHeader().checksum_) {
