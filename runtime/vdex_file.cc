...
class QuickeningInfoIterator {
 public:
  QuickeningInfoIterator(uint32_t dex_file_index,
    ...
    const uint32_t* dex_file_indices = reinterpret_cast<const uint32_t*>(
  ...
  bool HasNext() const {
    return current_code_item_ptr_ + 2 <= current_code_item_end_;
  }
  ...
  const ArrayRef<const uint8_t> GetCurrentQuickeningInfo() const {
    return ArrayRef<const uint8_t>(
        ...
        *reinterpret_cast<const uint32_t*>(quickening_info_.data() + current_code_item_ptr_[1]));
  }
    ...
    if (class_data != nullptr) {
      for (ClassDataItemIterator class_it(target_dex_file, class_data);
        ...
        if (class_it.IsAtMethod() && class_it.GetMethodCodeItem() != nullptr) {
          ...
          if (it.HasNext() && offset == it.GetCurrentCodeItemOffset()) {
            optimizer::ArtDecompileDEX(
