...
ClassStatus VdexFile::ComputeClassStatus(Thread* self, Handle<mirror::Class> cls) const {
  ...
  uint32_t class_def_offset =
      reinterpret_cast<const uint32_t*>(verifier_deps + dex_file_offset)[class_def_index];
  ...
  for (uint32_t i = class_def_index + 1; i < dex_file.NumClassDefs() + 1; ++i) {
    end_offset = reinterpret_cast<const uint32_t*>(verifier_deps + dex_file_offset)[i];
  ...
  // Fetch extra strings information.
  uint32_t end_of_assignability_types =
      reinterpret_cast<const uint32_t*>(verifier_deps + dex_file_offset)[dex_file.NumClassDefs()];
  const uint8_t* strings_data_start =
      AlignUp(verifier_deps + end_of_assignability_types, sizeof(uint32_t));
  uint32_t number_of_extra_strings = *reinterpret_cast<const uint32_t*>(strings_data_start);
  const uint32_t* extra_strings_offsets =
      reinterpret_cast<const uint32_t*>(strings_data_start + sizeof(uint32_t));
  ...
  while (cursor < end) {
    ...
    if (!destination->IsAssignableFrom(source.Get())) {
      // An implicit assignability check is failing in the code, force slow
      // interpreter.
      return ClassStatus::kResolved;
    }
  }
