...
constexpr uint8_t VdexFile::VerifierDepsHeader::kNoDexSectionVersion[4];
...
bool VdexFile::VerifierDepsHeader::IsDexSectionVersionValid() const {
  return (memcmp(dex_section_version_, kDexSectionVersion, sizeof(kDexSectionVersion)) == 0) ||
      (memcmp(dex_section_version_, kNoDexSectionVersion, sizeof(kNoDexSectionVersion)) == 0);
}
...
VdexFile::VerifierDepsHeader::VerifierDepsHeader(uint32_t verifier_deps_size,
  ...
  if (has_dex_section) {
    ...
    memcpy(dex_section_version_, kNoDexSectionVersion, sizeof(kNoDexSectionVersion));
  }
  DCHECK(IsMagicValid());
...
VdexFile::DexSectionHeader::DexSectionHeader(uint32_t number_of_dex_files,
                                             uint32_t dex_size,
...
std::unique_ptr<VdexFile> VdexFile::OpenAtAddress(uint8_t* mmap_addr,
  ...
  if (unquicken && vdex->HasDexSection()) {
    ...
    reinterpret_cast<DexSectionHeader*>(vdex->mmap_->Begin() + sizeof(VerifierDepsHeader))
