...
std::unique_ptr<VdexFile> VdexFile::OpenAtAddress(uint8_t* mmap_addr,
  ...
  return OpenAtAddress(mmap_addr, mmap_size, vdex_file->Fd(), vdex_length, vdex_filename,
...
std::unique_ptr<VdexFile> VdexFile::OpenAtAddress(uint8_t* mmap_addr,
                                                  ...
                                                  std::string* error_msg) {
  if (low_4gb) {
    LOG(ERROR) << "Can not mmap vdex in low 4GB";  // TODO: Implement in MemMap.
    mmap_addr = nullptr;
  }
  if (mmap_addr != nullptr && mmap_size < (int64_t)vdex_length) {
    LOG(ERROR) << "Insufficient pre-allocated space to mmap vdex.";
    mmap_addr = nullptr;
  }
