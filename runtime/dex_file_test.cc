...
static void DecodeAndWriteDexFile(const char* base64, const char* location) {
  ...
  CHECK(dex_bytes.size() != 0);

  // write to provided file
  std::unique_ptr<File> file(OS::CreateEmptyFile(location));
  CHECK(file.get() != nullptr);
  if (!file->WriteFully(&dex_bytes[0], dex_bytes.size())) {
...
static std::unique_ptr<const DexFile> OpenDexFileInMemoryBase64(const char* base64,
  ...
  MemMap* region = MemMap::MapAnonymous("", nullptr, dex_bytes.size(), PROT_READ|PROT_WRITE,
                                        false, false, &error_message);
  memcpy(region->Begin(), &dex_bytes[0], dex_bytes.size());
  std::unique_ptr<const DexFile> dex_file(DexFile::Open(location, location_checksum, region,
                                                        &error_message));
  CHECK(dex_file.get() != nullptr) << error_message;
  return dex_file;
}
...
TEST_F(DexFileTest, HeaderInMemory) {
  ScratchFile tmp;
  std::unique_ptr<const DexFile> raw(OpenDexFileInMemoryBase64(kRawDex, tmp.GetFilename().c_str(), 0x00d87910U));
  ASSERT_TRUE(raw.get() != nullptr);
