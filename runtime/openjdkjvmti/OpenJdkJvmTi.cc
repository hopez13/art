...
struct ArtJvmTiEnv : public jvmtiEnv {
  ...
  explicit ArtJvmTiEnv(art::JavaVMExt* runtime) : vm(runtime), art_env(runtime) {
 ...
 public:
  ...
  static jvmtiError GetAllThreads(jvmtiEnv* env, jint* threads_count_ptr, jthread** threads_ptr) {
    if (threads_count_ptr == nullptr || threads_ptr == nullptr) {
      // Null pointer on return values.
      return ERR(NULL_POINTER);
    }
    art::ScopedSuspendAll temp_suspension("jvmti GetAllThreads function");
    {
      art::MutexLock mu(art::Thread::Current(), *art::Locks::thread_list_lock_);
      std::list<art::Thread*> threads = art::Runtime::Current()->GetThreadList()->GetList();
      *threads_count_ptr = threads.size();
      jvmtiError err;
      if ((err = env->Allocate(*threads_count_ptr * sizeof(jthread),
                              reinterpret_cast<unsigned char**>(threads_ptr))) != OK) {
        // Something happened while allocating.
        return err;
      }
      jthread* cur_thread = *threads_ptr;
      art::JNIEnvExt* jnienv = art::Thread::Current()->GetJniEnv();
      for (art::Thread* thr : threads) {
        *cur_thread = jnienv->AddLocalReference<jobject>(thr->GetPeer());
        cur_thread++;
      }
    }
    return OK;
  ...
  static jvmtiError SuspendThreadList(jvmtiEnv* env,
    ...
    if (GetCapabilities(env, &caps) != OK || !caps.can_suspend) {
      ...
      return ERR(NULL_POINTER);
    } else if (env->GetCurrentThread(&this_thread) != OK) {
      // Must be able to get current thread.
      return ERR(INTERNAL);
    } else {
      bool has_this_thread = false;
      jvmtiError* this_threads_result = nullptr;
      jvmtiError* final_result = results + request_count;
      for (; results != final_result; results++, request_list++) {
        if (*request_list == this_thread) {
          has_this_thread = true;
          this_threads_result = results;
        } else {
          *results = SuspendThread(env, *request_list);
        }
      }
      // Suspend ourselves last.
      if (has_this_thread) {
        *this_threads_result = SuspendThread(env, this_thread);
      }
      return OK;
    }
  }
  ...
  static jvmtiError ResumeThread(jvmtiEnv* env, jthread thread) {
    ...
    art::Thread* thr = art::Thread::FromManagedThread(soa, thread);
    if (thr == nullptr) {
      // TODO ERROR
    }
    art::Runtime::Current()->GetThreadList()->Resume(thr);
    return OK;
  }
  ...
  static jvmtiError ResumeThreadList(jvmtiEnv* env,
    ...
    if (GetCapabilities(env, &caps) != OK || !caps.can_suspend) {
      ...
      if (has_this_thread) {
        *this_threads_result = ResumeThread(env, this_thread);
      }
      return OK;
    }
  }
...
bool IsJvmtiVersion(jint version) {
...
void CreateArtJvmTiEnv(art::JavaVMExt* vm, /*out*/void** new_jvmtiEnv) {
  struct ArtJvmTiEnv* env = new ArtJvmTiEnv(vm);
  env->functions = &gJvmtiInterface;
  *new_jvmtiEnv = env;
}

// A hook that the runtime uses to allow plugins to handle GetEnv calls. It returns true and places
// the return value in 'env' if this library can handle the GetEnv request. Otherwise returns false
// and does not modify the 'env' pointer.
extern "C" jint GetEnvHandler(art::JavaVMExt* vm, /*out*/void** env, jint version) {
