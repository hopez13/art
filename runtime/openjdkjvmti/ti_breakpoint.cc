...
void BreakpointUtil::RemoveBreakpointsFromClass(ArtJvmTiEnv* env, art::mirror::Class* klass) {
...
jvmtiError BreakpointUtil::SetBreakpoint(jvmtiEnv* jenv, jmethodID method, jlocation location) {
  ...
  auto res_pair = env->breakpoints.insert({art_method, location});
  if (!res_pair.second) {
...
jvmtiError BreakpointUtil::ClearBreakpoint(jvmtiEnv* jenv, jmethodID method, jlocation location) {
  ...
  auto pos = env->breakpoints.find(
      {art::jni::DecodeArtMethod(method)->GetCanonicalMethod(), location});
