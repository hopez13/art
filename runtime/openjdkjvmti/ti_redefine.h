...
class Redefiner {
 ...
 private:
  class ClassRedefinition {
   public:
    // NO_THREAD_SAFETY_ANALYSIS so we can get a lock on the object in the initializer which is
    // called from vector code.
    ClassRedefinition(Redefiner* driver,
                      jclass klass,
                      const art::DexFile* redefined_dex_file,
                      const char* class_sig) NO_THREAD_SAFETY_ANALYSIS;
  ...
  Redefiner(art::Runtime* runtime,
            art::Thread* self,
            std::vector<ClassRedefinition>* redefs,
            std::string* error_msg)
      : result_(ERR(INTERNAL)),
        runtime_(runtime),
        self_(self),
        redefinitions_(redefs),
  ...
  bool FinishAllRemainingAllocations(
