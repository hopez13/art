...
// Class that can redefine a single classes methods.
class Redefiner {
 public:
  // Redefine the given class with the given dex data. Note this function does not take ownership of
  // the dex_data pointer. It is not required after this call however and may be freed if desired.
 ...
 private:
  ...
  art::Thread* self_;
  // Kept as a jclass since we have weird run-state changes that make keeping it around difficult.
  jclass klass_;
  ...
  bool FinishRemainingAllocations(/*out*/art::MutableHandle<art::mirror::ClassLoader>& source_class_loader,
