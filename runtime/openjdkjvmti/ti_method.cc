...
class GetLocalVariablesClosure : public art::Closure {
 public:
  GetLocalVariablesClosure(art::Thread* caller,
                           jint depth,
                           jint slot,
                           art::Primitive::Type type,
                           jvalue* val)
  ...
  void Run(art::Thread* self) OVERRIDE REQUIRES(art::Locks::mutator_lock_) {
    ...
    if (pc == art::DexFile::kDexNoIndex) {
      ...
      jvmtiError err = GetTypeErrorForGet(type_, slot_, method, pc);
      if (err != OK) {
        result_ = err;
        return;
      }
...
class SetLocalVariablesClosure : public art::Closure {
 public:
  SetLocalVariablesClosure(art::Thread* caller,
                           jint depth,
                           jint slot,
                           art::Primitive::Type type,
                           jvalue val)
  ...
  void Run(art::Thread* self) OVERRIDE REQUIRES(art::Locks::mutator_lock_) {
    ...
    if (pc == art::DexFile::kDexNoIndex) {
      ...
      jvmtiError err = GetTypeErrorForSet(caller_, type_, val_, slot_, method, pc);
      if (err != OK) {
        result_ = err;
        return;
      }
