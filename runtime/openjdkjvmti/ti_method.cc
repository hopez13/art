...
static jvmtiError GetTypeMatchesRequestError(art::Primitive::Type expected_type,
                                             jint jslot,
  ...
  struct GetLocalVariableInfoContext {
    explicit GetLocalVariableInfoContext(jint slot, uint32_t pc)
        : found_(false), type_(art::Primitive::kPrimVoid), slot_(slot), pc_(pc) { }
  ...
  GetLocalVariableInfoContext context(jslot, dex_pc);
  if (!dex_file->DecodeDebugLocalInfo(code_item,
                                      method->IsStatic(),
    ...
    switch (context.type_) {
      ...
      default:
        return expected_type == context.type_ ? OK : ERR(TYPE_MISMATCH);
    }
  } else {
...
class GetLocalVariablesClosure : public art::Closure {
 public:
  GetLocalVariablesClosure(jint depth, jint slot, char type, jvalue* val)
      : result_(ERR(INTERNAL)), depth_(depth), slot_(slot), type_(type), val_(val) {}
  ...
  void Run(art::Thread* self) OVERRIDE REQUIRES(art::Locks::mutator_lock_) {
    ...
    if (pc == art::DexFile::kDexNoIndex) {
      ...
      jvmtiError err = GetTypeMatchesRequestError(ptype, slot_, method, pc);
      if (err != OK) {
        result_ = err;
        return;
      }
    ...
    switch (ptype) {
      case art::Primitive::kPrimNot: {
        ...
        val_->l = obj.IsNull() ? nullptr : self->GetJniEnv()->AddLocalReference<jobject>(obj);
        break;
      }
      case art::Primitive::kPrimInt:
...
jvmtiError MethodUtil::GetLocalVariableGeneric(jvmtiEnv* env ATTRIBUTE_UNUSED,
  ...
  GetLocalVariablesClosure c(depth, slot, type, val);
...
class SetLocalVariablesClosure : public art::Closure {
 public:
  SetLocalVariablesClosure(jint depth, jint slot, char type, jvalue val)
      : result_(ERR(INTERNAL)), depth_(depth), slot_(slot), type_(type), val_(val) {}
  ...
  void Run(art::Thread* self) OVERRIDE REQUIRES(art::Locks::mutator_lock_) {
    ...
    switch (ptype) {
      case art::Primitive::kPrimNot: {
        uint32_t ptr_val;
        art::ObjPtr<art::mirror::Object> obj(self->DecodeJObject(val_.l));
...
jvmtiError MethodUtil::SetLocalVariableGeneric(jvmtiEnv* env ATTRIBUTE_UNUSED,
  ...
  SetLocalVariablesClosure c(depth, slot, type, val);
  art::MutexLock mu(self, *art::Locks::thread_list_lock_);
  if (!target->RequestSynchronousCheckpoint(&c)) {
    return ERR(THREAD_NOT_ALIVE);
  } else {
