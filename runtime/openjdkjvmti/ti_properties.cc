...
static jvmtiError Copy(jvmtiEnv* env, const char* in, char** out) {
  unsigned char* data;
  jvmtiError result = env->Allocate(strlen(in), &data);
  if (result != ERR(NONE)) {
    return result;
  }
...
jvmtiError PropertiesUtil::GetSystemProperties(jvmtiEnv* env,
...

  {
    ...
    property_copies.push_back(MakeJvmtiUniquePtr(env,
                                                 reinterpret_cast<unsigned char*>(libpath_data)));
  }

  {
    ...
    property_copies.push_back(MakeJvmtiUniquePtr(env,
                                                 reinterpret_cast<unsigned char*>(classpath_data)));
  }
  ...
  for (size_t i = 0; i != kPropertiesSize; ++i) {
    ...
    array[i + 2] = data;
    property_copies.push_back(MakeJvmtiUniquePtr(env, reinterpret_cast<unsigned char*>(data)));
  }

  return ERR(NONE);
  unsigned char* libpath_data;
  jvmtiError libpath_result = env->Allocate(strlen(kPropertyLibraryPath), &libpath_data);
}

jvmtiError PropertiesUtil::GetSystemProperty(jvmtiEnv* env ATTRIBUTE_UNUSED,
                                             const char* property,
                                             char** value_ptr) {
  ...
  if (strcmp(property, kPropertyLibraryPath) == 0) {
    ...
    for (const std::string& prop_assignment : runtime_props) {
      ...
      if (assign_pos != std::string::npos && assign_pos > 0) {
        if (prop_assignment.substr(0, assign_pos - 1) == kPropertyLibraryPath) {
