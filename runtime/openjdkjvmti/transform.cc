...
jvmtiError Transformer::FillInTransformationData(ArtJvmTiEnv* env,
  ...
  if (def->dex_data.get() == nullptr) {
    ...
    const art::DexFile& dex = hs_klass->GetDexFileForRetransformation();
    def->dex_len = static_cast<jint>(dex.Size());
    unsigned char* new_dex_data = nullptr;
    jvmtiError alloc_error = env->Allocate(def->dex_len, &new_dex_data);
    if (alloc_error != OK) {
      return alloc_error;
    }
    ...
    memcpy(reinterpret_cast<void*>(new_dex_data),
           ...
           def->dex_len);
    def->dex_data = MakeJvmtiUniquePtr(env, new_dex_data);
  }
  return OK;
}
