...
jvmtiError FieldUtil::SetGlobalFieldAccessWatch(jvmtiEnv* jenv) {
  ArtJvmTiEnv* env = ArtJvmTiEnv::AsArtJvmTiEnv(jenv);
  if (env->access_watch_all_fields) {
    return ERR(DUPLICATE);
  }
  env->access_watch_all_fields = true;
  return OK;
}

jvmtiError FieldUtil::SetGlobalFieldModificationWatch(jvmtiEnv* jenv) {
  ArtJvmTiEnv* env = ArtJvmTiEnv::AsArtJvmTiEnv(jenv);
  if (env->modify_watch_all_fields) {
    return ERR(DUPLICATE);
  }
  env->modify_watch_all_fields = true;
  return OK;
}

jvmtiError FieldUtil::ClearGlobalFieldAccessWatch(jvmtiEnv* jenv) {
  ArtJvmTiEnv* env = ArtJvmTiEnv::AsArtJvmTiEnv(jenv);
  if (!env->access_watch_all_fields) {
    return ERR(NOT_FOUND);
  }
  env->access_watch_all_fields = false;
  return OK;
}

jvmtiError FieldUtil::ClearGlobalFieldModificationWatch(jvmtiEnv* jenv) {
  ArtJvmTiEnv* env = ArtJvmTiEnv::AsArtJvmTiEnv(jenv);
  if (!env->modify_watch_all_fields) {
    return ERR(NOT_FOUND);
  }
  env->modify_watch_all_fields = false;
  return OK;
}
