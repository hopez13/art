...
bool Redefiner::CheckSingleClass() {
  ...
  if (interfaces == nullptr) {
    ...
    for (uint32_t i = 0; i < interfaces->Size(); i++) {
      // TODO Investigate on RI if the order matters. Assume it does for now.
      if (strcmp(
...
// TODO Move this into the definition of IsRedefinable when that is made.
bool Redefiner::CheckRedefinable() {
  // TODO Investigate the behavior of the RI in this case. Might be worth it to try to match it. The
  // way we implement default methods may make it difficult however.
  art::ObjPtr<art::mirror::Class> klass(GetMirrorClass());
  if (klass->IsPrimitive()) {
    // TODO Investigate the exact error returned by the RI in this situation.
  ...
  // TODO We should check if the class has non-obsoletable methods on t he stack
