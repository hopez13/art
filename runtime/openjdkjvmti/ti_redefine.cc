...
jvmtiError Redefiner::AddRedefinition(ArtJvmTiEnv* env, const jvmtiClassDefinition& def) {
  ...
  JvmtiUniquePtr sig_unique_ptr(reinterpret_cast<unsigned char*>(signature_ptr));
  JvmtiUniquePtr generic_unique_ptr(reinterpret_cast<unsigned char*>(generic_ptr_unused));
  if (map.get() == nullptr) {
