...
class ObsoleteMethodStackVisitor : public art::StackVisitor {
 public:
  ObsoleteMethodStackVisitor(art::Thread* thread,
                             art::Context* ctx,
                             const std::unordered_set<art::ArtMethod*>& obsoleted_methods,
                             std::unordered_map<art::ArtMethod*, art::ArtMethod*> obsolete_maps)
      : StackVisitor(thread, ctx, StackVisitor::StackWalkKind::kIncludeInlinedFrames),
  ...
  bool VisitFrame() override NO_THREAD_SAFETY_ANALYSIS {
    ...
    // this down.
    UNUSED(obsolete_maps_);
    if (obsoleted_methods_.find(GetMethod()) != obsoleted_methods_.end()) {
      // TODO RECORD OBSOLETE METHOD FOUND. Allocate and stash the translation. Update stack frame.
      LOG(FATAL) << "FOUND OBSOLETE METHOD!";
    }
    return true;
  }
 ...
 private:
  ...
  // the redefined classes ClassExt.
...
jvmtiError Redefiner::RedefineClass(ArtJvmTiEnv* env,
  ...
  jvmtiError result;
  {
    ...
    r.GetMirrorClass()->MonitorEnter(self);
    // TODO write this.
    result = r.Run();
    r.GetMirrorClass()->MonitorExit(self);
  }
  return result;
}
...
// TODO Really wishing I had that mirror now.
art::mirror::LongArray* Redefiner::AllocateDexFileCookie(
  ...
  // TODO This is a really crappy thing here.
...
jvmtiError Redefiner::Run() {
  ...
  //
  // First we just allocate the ClassExt and it's fields that we need. These can be updated
  ...
  // TODO We might want to give this it's own suspended state! This will let us do the next 2
  // comments in a non-terrible way.
  // Don't poison the ObjPointers. THIS IS TERRIBLE HACK!
  // TODO This isn't right. We need to change state without any chance of suspend!
  self_->TransitionFromRunnableToSuspended<false>(art::ThreadState::kNative);
  ...
  if (!UpdateJavaDexFile(java_dex_file.Get(), new_dex_file_cookie.Get(), &original_dex_file_cookie)) {
    // Release suspendAll
  ...
  art::ObjPtr<art::mirror::DexCache> original_dex_cache(nullptr);
  if (!UpdateClass(art_class.Get(), new_dex_cache.Get(), original_dex_cache)) {
...
// TODO Write a test that running this twice with the 2nd & 3rd arguments swapped causes no net
// change to class.
bool Redefiner::UpdateClass(art::ObjPtr<art::mirror::Class> mclass,
                            art::ObjPtr<art::mirror::DexCache> new_dex_cache,
                            art::ObjPtr<art::mirror::DexCache> orig_dex_cache) {
  UNUSED(orig_dex_cache);
  // TODO
