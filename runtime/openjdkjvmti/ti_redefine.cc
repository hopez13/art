...
jvmtiError Redefiner::RedefineClasses(ArtJvmTiEnv* env,
  ...
  Redefiner r(runtime, self, &redefinitions, error_msg);
  for (jint i = 0; i < class_count; i++) {
    const jvmtiClassDefinition* def = &definitions[i];

    std::string original_dex_location;
    jvmtiError ret = OK;
    if ((ret = GetClassLocation(env, def->klass, &original_dex_location))) {
      *error_msg = "Unable to get original dex file location!";
      return ret;
    }
    ...
    if (env->GetClassSignature(def->klass, &signature_ptr, &generic_ptr_unused) != OK) {
      *error_msg = "A jclass passed in does not seem to be valid";
      return ERR(INVALID_CLASS);
    }
    if (map.get() == nullptr) {
      ...
      *error_msg = os.str();
      env->Deallocate(reinterpret_cast<unsigned char*>(signature_ptr));
      env->Deallocate(reinterpret_cast<unsigned char*>(generic_ptr_unused));
    ...
    redefinitions.push_back(
        Redefiner::ClassRedefinition(&r, def->klass, dex_file.release(), signature_ptr));
    env->Deallocate(reinterpret_cast<unsigned char*>(signature_ptr));
    env->Deallocate(reinterpret_cast<unsigned char*>(generic_ptr_unused));
  }
  return r.Run();
...
struct CallbackCtx {
  ...
  explicit CallbackCtx(art::LinearAlloc* alloc)
      : allocator(alloc) {}
};
...
class MANAGED RedefinitionDataHolder : public art::mirror::ObjectArray<art::mirror::Object> {
 public:
  ...
  static RedefinitionDataHolder* FromArray(
      art::Handle<art::mirror::ObjectArray<art::mirror::Object>> arr)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    DCHECK_EQ(arr->GetLength() % kNumSlots, 0);
    return reinterpret_cast<RedefinitionDataHolder*>(arr.Get());
  }

  static art::mirror::ObjectArray<art::mirror::Object>* Alloc(
      art::Thread* self, int32_t num_redefinitions) REQUIRES_SHARED(art::Locks::mutator_lock_) {
    CHECK_GT(num_redefinitions, 0);
    art::Runtime* runtime = art::Runtime::Current();
    art::ObjPtr<art::mirror::Class> object_array_class =
        runtime->GetClassLinker()->GetClassRoot(art::ClassLinker::kObjectArrayClass);
    return art::mirror::ObjectArray<art::mirror::Object>::Alloc(self,
                                                                object_array_class,
                                                                num_redefinitions * kNumSlots);
  }

  // TODO Maybe make a view type. The handle stuff could get confusing though. Just do this for the
  // moment.
 ...
 private:
  art::mirror::Object* GetSlot(jint klass_index, DataSlot slot)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    return Get((kNumSlots * klass_index) + slot);
  }

  void SetSlot(jint klass_index, DataSlot slot, art::ObjPtr<art::mirror::Object> obj)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
...
bool Redefiner::FinishAllRemainingAllocations(
    art::Handle<art::mirror::ObjectArray<art::mirror::Object>> holder) {
  int32_t cnt = 0;
  ...
  for (Redefiner::ClassRedefinition& redef : *redefinitions_) {
    ...
    RedefinitionDataHolder::FromArray(holder)->SetSourceClassLoader(cnt, source_class_loader.Get());
    RedefinitionDataHolder::FromArray(holder)->SetJavaDexFile(cnt, java_dex_file.Get());
    RedefinitionDataHolder::FromArray(holder)->SetNewDexFileCookie(cnt, new_dex_file_cookie.Get());
    RedefinitionDataHolder::FromArray(holder)->SetNewDexCache(cnt, new_dex_cache.Get());
    RedefinitionDataHolder::FromArray(holder)->SetMirrorClass(cnt, redef.GetMirrorClass());
    cnt++;
  }
  return true;
}
...
jvmtiError Redefiner::Run() {
  ...
  art::Handle<art::mirror::ObjectArray<art::mirror::Object>> holder(
      hs.NewHandle(RedefinitionDataHolder::Alloc(self_, redefinitions_->size())));
  if (holder.IsNull()) {
  ...
  int32_t cnt = 0;
  for (Redefiner::ClassRedefinition& redef : *redefinitions_) {
    art::mirror::Class* klass = RedefinitionDataHolder::FromArray(holder)->GetMirrorClass(cnt);
    redef.UpdateJavaDexFile(RedefinitionDataHolder::FromArray(holder)->GetJavaDexFile(cnt),
                            RedefinitionDataHolder::FromArray(holder)->GetNewDexFileCookie(cnt));
    // TODO Rewrite so we don't do a stack walk for each and every class.
    redef.FindAndAllocateObsoleteMethods(klass);
    redef.UpdateClass(klass, RedefinitionDataHolder::FromArray(holder)->GetNewDexCache(cnt));
    cnt++;
  }
...
bool Redefiner::ClassRedefinition::EnsureClassAllocationsFinished() {
  art::StackHandleScope<2> hs(driver_->self_);
  art::Handle<art::mirror::Class> klass(hs.NewHandle(driver_->self_->DecodeJObject(klass_)->AsClass()));
