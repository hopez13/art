 ...
 private:
  ...
  static art::mirror::Object* FindDalvikSystemDexFileForClass(
    ...
    art::MutableHandle<art::mirror::Object> first_dex_file(
        hs.NewHandle<art::mirror::Object>(nullptr));
    art::MutableHandle<art::mirror::LongArray> dex_file_cookie(
    ...
    for (size_t i = 0; i < num_elements; i++) {
      ...
      // dex_file_cookie.Assign(
      //     art::down_cast<art::mirror::LongArray*>(
      //         dex_file_cookie_field->GetObject(first_dex_file.Get())));
      // int32_t len = dex_file_cookie->GetLength();
      // for (int32_t i = 0; i < len; i++) {
      //   if (dex_file_cookie->GetWithoutChecks(i) ==
      //       static_cast<uint64_t>(reinterpret_cast<uintptr_t>(&klass->GetDexFile()))) {
      //     return first_dex_file.Get();
      //   }
      // }
      // TODO For each element in cookie
      // TODO   Find if we have the dex file in this DexFile
      // TODO     Return that DexFile
