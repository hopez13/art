...
class JvmtiMethodTraceListener FINAL : public art::instrumentation::InstrumentationListener {
 public:
  ...
  void DexPcMoved(art::Thread* self,
    ...
    // DCHECK(event_handler_->IsEventEnabledAnywhere(ArtJvmtiEvent::kSingleStep) ||
    //        event_handler_->IsEventEnabledAnywhere(ArtJvmtiEvent::kBreakpoint))
    //     << "DexPcMoved event recieved without either of the initiating events set!";
    art::JNIEnvExt* jnienv = self->GetJniEnv();
