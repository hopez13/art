...
// Makes sure that all methods are AsyncDeopt so we can deoptimize when we try to get a local
// variable.
    ...
    bool operator()(art::ObjPtr<art::mirror::Class> klass)
      ...
      for (auto& m : klass->GetMethods(art::kRuntimePointerSize)) {
        ...
        if (m.IsNative() || m.IsProxyMethod()) {
          ...
          runtime_->GetInstrumentation()->UpdateMethodsCodeForInterpreter(&m);
        }
      }
      return true;
    }
