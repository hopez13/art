...
class FollowReferencesHelper FINAL {
 public:
  FollowReferencesHelper(HeapUtil* h,
                         ...
                         const jvmtiHeapCallbacks* callbacks,
                         const void* user_data)
      ACQUIRE(tag_table_->GetAllowDisallowLock())
  ...
  ~FollowReferencesHelper() RELEASE(tag_table_->GetAllowDisallowLock()) {
    tag_table_->Unlock();
  }
 ...
 private:
  ...
  jint ReportReference(jvmtiHeapReferenceKind kind,
    ...
    tag_table_->AssertLocked();

    const jlong class_tag = tag_table_->GetTagOrZeroLocked(referree->GetClass());
    const jlong referrer_class_tag =
        referrer == nullptr ? 0 : tag_table_->GetTagOrZeroLocked(referrer->GetClass());
    const jlong size = static_cast<jlong>(referree->SizeOf());
    jlong tag = tag_table_->GetTagOrZeroLocked(referree);
    jlong saved_tag = tag;
    ...
    if (referrer == nullptr) {
      ...
      if (referrer == referree) {
        ...
        referrer_tag = saved_referrer_tag = tag_table_->GetTagOrZeroLocked(referrer);
    ...
    if (tag != saved_tag) {
      tag_table_->SetLocked(referree, tag);
    }
    if (referrer_tag != saved_referrer_tag) {
      tag_table_->SetLocked(referrer, referrer_tag);
    }
