...
class Breakpoint {
 public:
  Breakpoint(art::ArtMethod* m, jlocation loc) : method_(m), location_(loc) {
    DCHECK(!m->IsDefault() || !m->IsCopied() || !m->IsInvokable())
        << "Flags are: 0x" << std::hex << m->GetAccessFlags();
  }

  Breakpoint(jmethodID m, jlocation loc)
      : method_(art::jni::DecodeArtMethod(m)), location_(loc) {}
...
class BreakpointUtil {
 public:
  ...
  static void RemoveBreakpointsFromClass(ArtJvmTiEnv* env, art::mirror::Class* klass)
