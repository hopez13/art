...
inline ArtMethod* FindMethodFast(uint32_t method_idx,
  ...
  if (type == kInterface) {  // Most common form of slow path dispatch.
    ...
    ObjPtr<mirror::Class> method_reference_class = dex_cache->GetResolvedType(method_type_idx);
    if (method_reference_class == nullptr) {
      method_reference_class = Runtime::Current()->GetClassLinker()->LookupResolvedType(
          *dex_cache->GetDexFile(), method_type_idx, dex_cache, referrer->GetClassLoader());
    }
    if (method_reference_class == nullptr) {
