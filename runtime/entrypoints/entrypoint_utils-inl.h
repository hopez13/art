...
inline ArtMethod* GetResolvedMethod(ArtMethod* outer_method,
  ...
  if (klass == nullptr) {
    // Not found in the caller's class loader, look it up in the boot class loader.
    if (class_loader != nullptr) {
      klass = class_linker->LookupClass(self, descriptor, nullptr);
    }
    if (klass == nullptr) {
      LOG(FATAL) << "Could not find an inlined method from an .oat file: "
                 << "the class " << descriptor << " was not found in the class loader of "
                 << caller->PrettyMethod() << " nor in the boot class loader. "
