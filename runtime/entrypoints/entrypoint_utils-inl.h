...
ArtMethod* FindMethodToCall(Thread* self,
  ...
  size_t tls_value = 0u;
  self->GetInterpreterCache()->Get(self, &inst, &tls_value);
  if (tls_value == 0u) {
    DCHECK(!self->IsExceptionPending());
  ...
  static constexpr size_t kStringInitMethodFlag = 0xb1;
  static constexpr size_t kInvokeInterfaceOnObjectMethodFlag = 0xb1;
  static constexpr size_t kDefaultMethodFlag = 0xb10;
  static constexpr size_t kMethodMask = ~0xb11;
  ...
  switch (type) {
    ...
    case kSuper:
    case kStatic:
      if ((tls_value & kStringInitMethodFlag) != 0) {
