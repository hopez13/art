...
ALWAYS_INLINE inline mirror::Class* CheckObjectAlloc(mirror::Class* klass,
  ...
  if (kInstrumented) {
    if (Runtime::Current()->IsActiveStrictTransactionMode()) {
      if (!Runtime::Current()->GetTransaction()->AddMemoryUsed(klass->GetObjectSize())) {
        *slow_path = true;
        return nullptr;
      }
    }
...
inline mirror::Object* AllocObjectFromCode(mirror::Class* klass,
  ...
  klass = CheckObjectAlloc<kInstrumented>(klass, self, &slow_path);
...
inline mirror::Class* CheckArrayAlloc(dex::TypeIndex type_idx,
  ...
  if (kInstrumented) {
    if (runtime->IsActiveStrictTransactionMode()) {
      if (!runtime->GetTransaction()->AddMemoryUsed(klass->GetComponentSize() * component_count)) {
        *slow_path = true;
        return nullptr;
      }
    }
...
inline mirror::Array* AllocArrayFromCode(dex::TypeIndex type_idx,
                                         ...
                                         gc::AllocatorType allocator_type) {
  bool slow_path = false;
  mirror::Class* klass = CheckArrayAlloc<kAccessCheck, kInstrumented>(type_idx,
                                                                      component_count,
                                                                      method,
                                                                      &slow_path);
  if (UNLIKELY(slow_path)) {
