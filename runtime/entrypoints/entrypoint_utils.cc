...
static inline ArtMethod* DoGetCalleeSaveMethodCaller(ArtMethod** sp,
                                                     CalleeSaveType type,
...
ArtMethod* GetCalleeSaveMethodCaller(ArtMethod** sp,
  ...
  ArtMethod* caller =
      DoGetCalleeSaveMethodCaller(sp, type, outer_method, caller_pc, dex_pc, do_caller_check);
  return caller;
}
...
CallerAndOuterMethod GetCalleeSaveMethodCallerAndOuterMethod(Thread* self, CalleeSaveType type) {
  ...
  uintptr_t caller_pc = outer_caller_and_pc.second;
  uint32_t dex_pc;
  result.caller = DoGetCalleeSaveMethodCaller(sp,
                                              type,
                                              result.outer_method,
                                              caller_pc,
                                              &dex_pc,
