...
void MaybeUpdateBssMethodEntryHelper(ArtMethod* callee,
  ...
  if (bss_offset != IndexBssMappingLookup::npos) {
    ...
    // TODO(solanes): Is this DCHECK okay for the BCP?
...
void MaybeUpdateBssMethodEntry(ArtMethod* callee,
  ...
  if (dex_file->GetOatDexFile() != nullptr) {
    MaybeUpdateBssMethodEntryHelper(callee,
                                    callee_reference,
                                    dex_file->GetOatDexFile()->GetMethodBssMapping(),
                                    dex_file->GetOatDexFile()->GetOatFile());
  } else if (callee->GetDeclaringClass()->GetClassLoader() == nullptr) {
    if (caller->GetDexFile()->GetOatDexFile() == nullptr) {
      return;
    }
    const OatFile* oat_file = caller->GetDexFile()->GetOatDexFile()->GetOatFile();
    ...
    MaybeUpdateBssMethodEntryHelper(
        callee, callee_reference, mapping_info_vector[dexfile_index].method_bss_mapping, oat_file);
  }
}
