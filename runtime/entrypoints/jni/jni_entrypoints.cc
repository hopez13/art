...
const void* artFindNativeMethodFastJni_(Thread* self,
                                        ScopedObjectAccess& soa) NO_THREAD_SAFETY_ANALYSIS {  // We continue as Native.
  bool was_slow = false;
  bool is_fast = false;
...
extern "C" const void* artFindNativeMethod(Thread* self) {
  ...
  if (Runtime::Current()->IsAutoFastDetect()) {
     return artFindNativeMethodFastJni_(self, soa);
  } else {
