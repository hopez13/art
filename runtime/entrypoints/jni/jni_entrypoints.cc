...
extern "C" const void* artFindNativeMethodRunnable(Thread* self)
  ...
  if (!method->IsNative()) {
    ...
    *sp = target_method;
    ...
    void*  native_code = target_method->GetEntryPointFromJni();
    if (!class_linker->IsJniDlsymLookupCriticalStub(native_code)) {
      // We already have a registered native method.
      return native_code;
    }
...
extern "C" size_t artCriticalNativeFrameSize(ArtMethod* method, uintptr_t caller_pc)
  ...
  if (method->IsNative()) {
    ...
    // Get the caller (may be inlined) and dex pc.
