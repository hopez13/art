...
extern "C" int artDeoptimizeIfNeeded(Thread* self, uintptr_t result, bool is_ref)
...
extern "C" int artImplicitSuspendFromCode(Thread* self) REQUIRES_SHARED(Locks::mutator_lock_) {
  // Called when there is a pending checkpoint or suspend request.
  ScopedQuickEntrypointChecks sqec(self);
