...
extern "C" const void* artQuickResolutionTrampoline(
  ...
  if (LIKELY(!self->IsExceptionPending())) {
    ...
    if (success) {
      instrumentation::Instrumentation* instrumentation = Runtime::Current()->GetInstrumentation();
      // Check if we need instrumented code here. Since resoultion stubs could suspend, it is
      // possible that we instrumented the entry points after we started executing resolution stub.
