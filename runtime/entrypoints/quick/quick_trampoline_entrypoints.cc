...
extern "C" int artMethodExitHook(Thread* self,
  ...
  if (self->IsExceptionPending() || self->ObserveAsyncException()) {
    ...
    // callbacks for this case. Indicate this by setting a fake exception. Exception handler will
    // retrieve the original exception from the exception context.
    CHECK(self->GetException() != Thread::GetDeoptimizationException() &&
          self->GetException() != Thread::GetMethodExitException());
    self->PushMethodExitExceptionContext(self->GetException());
    self->SetException(Thread::GetMethodExitException());
    return 1;
  }

  if (deoptimize) {
    CHECK_EQ(self->GetException(), nullptr);
