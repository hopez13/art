...
extern "C" uint64_t artInstrumentationToInterpreterBridge(ArtMethod* method,
    ...
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ScopedQuickEntrypointChecks sqec(self);
  // We were called to make sure that stack walking would succeed when we throw an exception. We can
...
extern "C" const void* artInstrumentationMethodEntryFromCode(ArtMethod* method,
  ...
  // TODO It might be good to give the pending exception state its own path but that would require
  ...
  if (result == GetQuickToInterpreterBridge() || self->IsExceptionPending()) {
    ...
    // return due to a pending exception. We need to do all this work to ensure that stack walking
