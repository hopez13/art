...
static TwoWordReturn artInvokeCommon(uint32_t method_idx,
  ...
  ArtMethod* method = FindMethodToCall<type>(
      self, caller_method, &this_object, instr, /* can_suspend= */ false, &string_init);
  ...
  if (UNLIKELY(method == nullptr)) {
    ...
    const char* shorty = dex_file->GetMethodShorty(dex_file->GetMethodId(method_idx), &shorty_len);
    {
      ...
      method = FindMethodToCall<type>(
          self, caller_method, &this_object, instr, /* can_suspend= */ true, &string_init);
