...
extern "C" uint64_t artQuickToInterpreterBridge(ArtMethod* method, Thread* self, ArtMethod** sp)
  ...
  DCHECK(!method->IsNative()) << ArtMethod::PrettyMethod(method);
  ...
  DCHECK(code_item != nullptr) << ArtMethod::PrettyMethod(method);
  ...
  if (deopt_frame != nullptr) {
    ...
    if (kIsDebugBuild) {
      ...
      CHECK_EQ(method, linked->GetMethod()) << ArtMethod::PrettyMethod(method) << " "
    ...
    if (needs_initialization) {
      ...
      if (!Runtime::Current()->GetClassLinker()->EnsureInitialized(self, h_class, true, true)) {
        DCHECK(Thread::Current()->IsExceptionPending())
            << ArtMethod::PrettyMethod(shadow_frame->GetMethod());
...
extern "C" uint64_t artQuickProxyInvokeHandler(
    ...
    REQUIRES_SHARED(Locks::mutator_lock_) {
  DCHECK(proxy_method->IsProxyMethod()) << ArtMethod::PrettyMethod(proxy_method);
  DCHECK(receiver->GetClass()->IsProxyClass()) << ArtMethod::PrettyMethod(proxy_method);
  // Ensure we don't get thread suspension until the object arguments are safely in jobjects.
  const char* old_cause =
      self->StartAssertNoThreadSuspension("Adding to IRT proxy object arguments");
  // Register the top of the managed stack, making stack crawlable.
  DCHECK_EQ((*sp), proxy_method) << ArtMethod::PrettyMethod(proxy_method);
  self->VerifyStack();
  ...
  CHECK(!non_proxy_method->IsStatic()) << ArtMethod::PrettyMethod(proxy_method) << " "
                                       << ArtMethod::PrettyMethod(non_proxy_method);
  ...
  DCHECK_GT(args.size(), 0U) << ArtMethod::PrettyMethod(proxy_method);
  args.erase(args.begin());

  // Convert proxy method into expected interface method.
  ArtMethod* interface_method = proxy_method->FindOverriddenMethod(kRuntimePointerSize);
  DCHECK(interface_method != nullptr) << ArtMethod::PrettyMethod(proxy_method);
  DCHECK(!interface_method->IsProxyMethod()) << ArtMethod::PrettyMethod(interface_method);
  self->EndAssertNoThreadSuspension(old_cause);
  DCHECK_EQ(Runtime::Current()->GetClassLinker()->GetImagePointerSize(), kRuntimePointerSize);
...
extern "C" const void* artQuickResolutionTrampoline(
  ...
  if (LIKELY(!self->IsExceptionPending())) {
    ...
    CHECK(!called->CheckIncompatibleClassChange(invoke_type))
        << ArtMethod::PrettyMethod(called) << " " << invoke_type;
    if (virtual_or_interface || invoke_type == kSuper) {
      ...
      CHECK(called != nullptr) << ArtMethod::PrettyMethod(orig_called) << " "
...
extern "C" TwoWordReturn artQuickGenericJniTrampoline(Thread* self, ArtMethod** sp)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ArtMethod* called = *sp;
  DCHECK(called->IsNative()) << ArtMethod::PrettyMethod(called, true);
  uint32_t shorty_len = 0;
...
static TwoWordReturn artInvokeCommon(uint32_t method_idx, mirror::Object* this_object, Thread* self,
  ...
  DCHECK(code != nullptr) << "Code was null in method: " << ArtMethod::PrettyMethod(method)
                          << " location: "
...
extern "C" TwoWordReturn artInvokeInterfaceTrampoline(uint32_t deadbeef ATTRIBUTE_UNUSED,
  ...
  DCHECK(interface_method != nullptr) << dex_method_idx << " "
                                      << ArtMethod::PrettyMethod(caller_method);
