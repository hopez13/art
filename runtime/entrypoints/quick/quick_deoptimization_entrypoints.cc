...
NO_RETURN static void artDeoptimizeImpl(Thread* self,
  ...
  if (single_frame) {
    ...
    exception_handler.DeoptimizeStack(after_method_exit_cb);
  }
...
extern "C" NO_RETURN void artDeoptimize(Thread* self, bool after_method_exit_cb)
  ...
  artDeoptimizeImpl(self, DeoptimizationKind::kFullFrame, false, after_method_exit_cb);
}
...
extern "C" NO_RETURN void artDeoptimizeFromCompiledCode(DeoptimizationKind kind, Thread* self)
  ...
  artDeoptimizeImpl(self, kind, true, /* after_method_exit_cb= */ false);
}
