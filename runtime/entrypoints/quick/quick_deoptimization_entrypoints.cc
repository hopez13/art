...
NO_RETURN static void artDeoptimizeImpl(Thread* self,
  ...
  if (single_frame) {
    ...
    exception_handler.DeoptimizeStack(skip_method_exit_cb);
  }
...
extern "C" NO_RETURN void artDeoptimize(Thread* self, bool skip_method_exit_cb)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ScopedQuickEntrypointChecks sqec(self);
  artDeoptimizeImpl(self, DeoptimizationKind::kFullFrame, false, skip_method_exit_cb);
}
...
extern "C" NO_RETURN void artDeoptimizeFromCompiledCode(DeoptimizationKind kind, Thread* self)
  ...
  // if required.
  artDeoptimizeImpl(self, kind, true, /* skip_method_exit_cb= */ false);
}
