...
// Update .bss method entrypoint if:
//   A) the `callee_reference` has an associated oat file, or
//   B) the `callee` comes from a BCP DexFile that was present during `caller`'s OatFile compilation
...
void MaybeUpdateBssMethodEntry(ArtMethod* callee,
                               ...
                               ArtMethod* caller) REQUIRES_SHARED(Locks::mutator_lock_);
