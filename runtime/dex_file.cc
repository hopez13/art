...
bool DexFile::IsDexOrCDexMagic(uint32_t magic) {
  return IsDexOrCDexMagic(reinterpret_cast<uint8_t*>(&magic));
}

bool DexFile::IsDexOrCDexMagic(const uint8_t* magic) {
  return NativeDexFile::IsMagicValid(magic) || CDexFile::IsMagicValid(magic);
}
...
std::unique_ptr<DexFile> DexFile::OpenCommon(const uint8_t* base,
  ...
  } else if (CDexFile::IsMagicValid(base)) {
    dex_file.reset(new CDexFile(base, size, location, location_checksum, oat_dex_file));
  }
  if (dex_file == nullptr) {
