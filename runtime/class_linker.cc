...
ArtField* ClassLinker::LookupResolvedField(uint32_t field_idx,
  ...
  ArtField* resolved_field = nullptr;
  if (lookup_type == FieldLookup::kInstance) {
    resolved_field = klass->FindInstanceField(dex_cache, field_idx);
  } else if (lookup_type == FieldLookup::kStatic) {
    resolved_field = mirror::Class::FindStaticField(self, klass, dex_cache, field_idx);
  }
  ...
  if (resolved_field == nullptr) {
    ...
    if (lookup_type == FieldLookup::kInstance) {
      resolved_field = klass->FindInstanceField(name, type);
    } else if (lookup_type == FieldLookup::kStatic) {
      resolved_field = mirror::Class::FindStaticField(self, klass, name, type);
    } else {
      DCHECK(lookup_type == FieldLookup::kStaticOrInstance);
      resolved_field = mirror::Class::FindField(self, klass, name, type);
    }
  }
