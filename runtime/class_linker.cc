...
void ClassLinker::LoadClass(Thread* self,
  ...
  Runtime* const runtime = Runtime::Current();
  {
    ...
    const uint8_t* hiddenapi_flags = nullptr;
    if (klass->IsBootStrapClassLoaded()) {
      const DexFile::HiddenapiMetadata* metadata = dex_file.GetHiddenapiMetadata();
      if (metadata != nullptr) {
        hiddenapi_flags = metadata->GetFlagsStream(
            dex_file.GetIndexForClassDef(dex_class_def), dex_file);
      }
    }
