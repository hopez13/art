...
static ObjPtr<mirror::IfTable> SetupInterfaceLookupTable(
  ...
  if (num_interfaces == 0) {
    // If the class just inherits marker interfaces from parent, recycle parent's iftable.
    auto is_marker_iface = [super_iftable](size_t index) REQUIRES_SHARED(Locks::mutator_lock_) {
