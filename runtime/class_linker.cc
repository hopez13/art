...
bool ClassLinker::LinkMethodsHelper<kPointerSize>::LinkMethods(
  ...
  if (klass->IsInterface()) {
    ...
    return LinkInterfaceMethods(self, klass);
...
bool ClassLinker::LinkMethodsHelper<kPointerSize>::LinkInterfaceMethods(
  ...
  for (size_t i = ifcount; i != 0u; ) {
    ...
    size_t num_methods = iftable->GetInterface(i)->NumDeclaredVirtualMethods();
    if (num_methods > 0) {
      // We don't actually create or fill these tables for interfaces,
      // we just copy some methods for conflict methods.

      ArraySlice<ArtMethod> input_virtual_methods =
          klass->GetDeclaredVirtualMethodsSlice(kPointerSize);
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        {
          for (int32_t k = input_virtual_methods.size() - 1; k >= 0; --k) {
            ArtMethod* vtable_method = &input_virtual_methods[k];
        ...
        UNUSED(current_method);
      }  // For each method in interface end.
    }  // if (num_methods > 0)
  }  // For each interface.

  if ((false)) {  // Keeping for interface method linking rewrite.
