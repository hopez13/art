...
mirror::MethodHandle* ClassLinker::ResolveMethodHandleForMethod(
  ...
  switch (handle_type) {
    ...
    case DexFile::MethodHandleType::kInstanceGet:
      UNREACHABLE();
    case DexFile::MethodHandleType::kInvokeStatic: {
      invoke_type = InvokeType::kStatic;
      kind = mirror::MethodHandle::Kind::kInvokeStatic;
      receiver_count = 0;
      break;
    }
    case DexFile::MethodHandleType::kInvokeInstance: {
      invoke_type = InvokeType::kVirtual;
    ...
    case DexFile::MethodHandleType::kInvokeInterface: {
      invoke_type = InvokeType::kInterface;
  ...
  ArtMethod* target_method =
      ResolveMethod<kNoICCECheckForCache>(self,
                                          method_handle.field_or_method_idx_,
                                          referrer,
                                          invoke_type);
  if (LIKELY(target_method != nullptr)) {
  ...
  uint32_t num_params = shorty_length + receiver_count - 1;
  ...
  Handle<mirror::Class> return_type;
  if (handle_type == DexFile::MethodHandleType::kInvokeConstructor) {
    // TODO(oth): is this necessary?
    //    return_type = hs.NewHandle(FindPrimitiveClass('V'));
...
mirror::MethodHandle* ClassLinker::ResolveMethodHandle(uint32_t method_handle_idx,
  ...
  LOG(FATAL) << "Unknown method handle kind " << method_handle.method_handle_type_;
  UNREACHABLE();
}
