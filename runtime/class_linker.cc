...
void ClassLinker::LoadField(const ClassAccessor::Field& field,
  ...
  if (klass->IsBootStrapClassLoaded()) {
    access_flags |= hiddenapi::CreateRuntimeFlags(field.GetHiddenapiFlags());
  }
...
void ClassLinker::LoadMethod(const DexFile& dex_file,
  ...
  if (klass->IsBootStrapClassLoaded()) {
    access_flags |= hiddenapi::CreateRuntimeFlags(method.GetHiddenapiFlags());
  }
...
ArtMethod* ClassLinker::FindResolvedMethod(ObjPtr<mirror::Class> klass,
  ...
  if (hiddenapi::ShouldDenyAccessToMember(
...
static bool CheckNoSuchMethod(ArtMethod* method,
  ...
  return method == nullptr || hiddenapi::ShouldDenyAccessToMember(
      method, class_loader, dex_cache, /* no warnings */ hiddenapi::AccessMethod::kNone);
}
