...
// Whether we allow classes of objects that can be allocated in the Large-Object
// Space (LOS) to move (in the case where the garbage collector can move class
// objects). Objects allocated in the LOS are (large) java.lang.String instances
// and primitive arrays (see art::gc::Heap::ShouldAllocLargeObject).
//
// We need to make these classes non-movable when using Generational Concurrent
// Copying (CC) collection, for the unlikely case where such a class were to be
// moved by a sticky-bit (minor) collection, potentially creating a stale
// reference in the `klass_` field of an object allocated in the LOS (see the
// comment about the dirty card scanning logic in
// art::gc::collector::ConcurrentCopying::MarkingPhase).
static constexpr bool kAllowMovableClassesForLOSObjects =
...
bool ClassLinker::InitWithoutImage(std::vector<std::unique_ptr<const DexFile>> boot_class_path,
  ...
  // Setup java.lang.String. Honors `art::kAllowMovableClassesForLOSObjects` (see above).
  Handle<mirror::Class> java_lang_String(hs.NewHandle(
      AllocClass</* kMovable */ kAllowMovableClassesForLOSObjects>(
          self,
          java_lang_Class.Get(),
  ...
  // Honors `art::kAllowMovableClassesForLOSObjects` (see above).
...
ObjPtr<mirror::Class> ClassLinker::AllocPrimitiveArrayClass(Thread* self,
                                                            ObjPtr<mirror::Class> java_lang_Class) {
  return AllocClass</* kMovable */ kAllowMovableClassesForLOSObjects>(
