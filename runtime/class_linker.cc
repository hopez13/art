...
void ClassLinker::LinkMethodsHelper<kPointerSize>::FinalizeIfTable(
  ...
  if (!is_klass_abstract) {
    ...
    if (is_super_abstract) {
      ...
      DCHECK_EQ(new_methods_begin, 0u);
      ...
      if (methods != nullptr) {
        DCHECK_NE(methods->size(), 0u);
        new_methods_begin = reinterpret_cast<uintptr_t>(&methods->At(0));
        new_methods_size = methods->size() * kMethodSize;
      }
    }
  }
