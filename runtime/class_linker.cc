...
void ClassLinker::LinkMethodsHelper<kPointerSize>::FinalizeIfTable(
  ...
  uintptr_t new_methods_begin = 0u;
  size_t new_methods_size = 0u;
  ...
  if (!is_klass_abstract) {
    ...
    if (is_super_abstract) {
      DCHECK(klass->ShouldHaveImt());
      DCHECK(!klass->GetSuperClass()->ShouldHaveImt());
      // There was no IMT in superclass to copy to `out_imt[]`, so we need
      // to fill it with all implementation methods from superclass.
      new_methods_begin = 0u;
      new_methods_size = std::numeric_limits<size_t>::max();  // No method at the last byte.
    } else {
      DCHECK(klass->ShouldHaveImt());
      DCHECK(klass->GetSuperClass()->ShouldHaveImt());
      // If the superclass has IMT, we have already copied it to `out_imt[]`
      // and we do not need to call `SetIMTRef()` for interfaces from superclass
      // when the implementation method is already in the superclass.
      // For simplicity, use the entire method array, including direct methods, in the range.
      LengthPrefixedArray<ArtMethod>* const methods = klass_->GetMethodsPtr();
      new_methods_begin = reinterpret_cast<uintptr_t>(methods);
      new_methods_size = (methods != nullptr)
          ? LengthPrefixedArray<ArtMethod>::ComputeSize(methods->size(),
                                                        kMethodSize,
                                                        kMethodAlignment)
          : 0u;
    }
  ...
  for (size_t i = 0; i != super_ifcount; ++i) {
    ...
    for (size_t j = 0; j != num_methods; ++j) {
      ...
      if (reinterpret_cast<uintptr_t>(implementation) - new_methods_begin < new_methods_size) {
        // Place method in imt if entry is empty, place conflict otherwise.
