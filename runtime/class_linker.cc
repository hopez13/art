...
bool ClassLinker::AddImageSpace(
  ...
  for (int32_t i = 0; i < dex_caches->GetLength(); i++) {
    ...
    // If dex cache location non qualified, qualify it.
    std::string dex_file_name;
    const std::string& dex_file_location = OatFile::ResolveRelativeEncodedDexLocation(
        dex_location,
        dex_cache->GetLocation()->ToModifiedUtf8(),
        &dex_file_name);
    std::unique_ptr<const DexFile> dex_file = OpenOatDexFile(oat_file,
                                                             dex_file_location.c_str(),
                                                             error_msg);
