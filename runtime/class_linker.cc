...
class NonProxyInterfacesAccessor {
 public:
  NonProxyInterfacesAccessor(Thread* self, ClassLinker* class_linker, Handle<mirror::Class> klass)
      REQUIRES_SHARED(Locks::mutator_lock_)
      : interfaces_(klass->GetInterfaceTypeList()),
        lookup_data_(std::nullopt) {
    DCHECK(!klass->IsProxyClass());
    // Assume that non-null `interfaces_` points to non-empty type list.
    if (interfaces_ != nullptr) {
      lookup_data_.emplace(self, class_linker, klass);
    }
  }
  ...
  ObjPtr<mirror::Class> GetInterface(size_t index) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    DCHECK(lookup_data_.has_value());
    auto dex_cache = ObjPtr<mirror::DexCache>::DownCast(lookup_data_->hs_.GetReference(0u));
    DCHECK(dex_cache->IsDexCache());
    auto class_loader = ObjPtr<mirror::ClassLoader>::DownCast(lookup_data_->hs_.GetReference(1u));
    DCHECK(class_loader == nullptr || class_loader->IsClassLoader());
    return lookup_data_->class_linker_->LookupResolvedType(type_index, dex_cache, class_loader);
  }

 private:
  class LookupData {
   public:
    LookupData(Thread* self, ClassLinker* class_linker, Handle<mirror::Class> klass)
        REQUIRES_SHARED(Locks::mutator_lock_)
        : class_linker_(class_linker),
          hs_(self) {
      hs_.NewHandle(klass->GetDexCache());
      hs_.NewHandle(klass->GetClassLoader());
    }

    ClassLinker* class_linker_;
    StackHandleScope<2u> hs_;
  };

  const dex::TypeList* interfaces_;
  std::optional<LookupData> lookup_data_;
};
...
bool ClassLinker::LinkMethodsHelper<kPointerSize>::LinkMethods(
  ...
  if (klass->IsInterface()) {
    ...
    ObjPtr<mirror::IfTable> iftable = SetupInterfaceLookupTable(
        self, klass, &allocator_, NonProxyInterfacesAccessor(self, class_linker_, klass));
    if (UNLIKELY(iftable == nullptr)) {
      self->AssertPendingException();
      return false;
    }
