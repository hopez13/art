...
bool ClassLinker::AddImageSpace(
  ...
  // Make the string intern table and class table immutable (after the fixup for apps).
  uint8_t* const_section_begin = space->Begin() + header.GetConstantTablesMappedOffset();
  mprotect(const_section_begin, header.GetConstantTablesMappedSize(), PROT_READ);
