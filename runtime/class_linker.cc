...
class ClassLinker::LinkMethodsHelper {
 ...
 private:
  ...
  void AddCopiedMethods() REQUIRES_SHARED(Locks::mutator_lock_);
...
template <PointerSize kPointerSize>
void ClassLinker::LinkMethodsHelper<kPointerSize>::AddCopiedMethods() {
  ScopedAssertNoThreadSuspension sants(__FUNCTION__);
  ...
  if (methods != old_methods) {
    ...
    // read barriers since `LinarAlloc::Realloc()` won't do read barriers when it copies.
  ...
  const CopiedMethodRecord* sorted_records_buffer[kSortedRecordsBufferSize];
  const CopiedMethodRecord** sorted_records = (num_new_copied_methods <= kSortedRecordsBufferSize)
      ? sorted_records_buffer
      : allocator_.AllocArray<const CopiedMethodRecord*>(num_new_copied_methods);
  size_t filled_sorted_records = 0u;
  for (CopiedMethodRecord& record : copied_method_records_) {
    if (record.GetState() != CopiedMethodState::kUseSuperMethod) {
  ...
  std::sort (sorted_records,
             ...
             [](const CopiedMethodRecord* lhs, const CopiedMethodRecord* rhs) {
               return lhs->GetVTableIndex() < rhs->GetVTableIndex();
             });
  ...
  for (size_t i = 0; i != num_new_copied_methods; ++i) {
    ...
    new_method.SetMethodIndex(record->GetVTableIndex());
...
void ClassLinker::LinkMethodsHelper<kPointerSize>::FinalizeIfTable(
  ...
  // For new interfaces method arrays contain vtable indexes. Translate them to methods.
...
void ClassLinker::LinkMethodsHelper<kPointerSize>::ReallocMethods() {
  ...
  for (ScopedArenaVector<ArtMethod*>* methods_vec : {&default_conflict_methods_,
    ...
    for (size_t i = 0; i < methods_vec->size(); ++i) {
      ...
      // (possibly default) interface method. We need to set its entry point to be the bridge so
