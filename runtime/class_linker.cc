...
void ClassLinker::FinishArrayClassSetup(ObjPtr<mirror::Class> array_class) {
  // Note: We allow creating array classes during transaction but we do not remove them
  // during rollback. Therefore we need to emit an unchecked non-transactional stores.
...
ObjPtr<mirror::Class> ClassLinker::CreateProxyClass(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  // the methods.
  temp_klass->SetAccessFlagsDuringLinking(kAccClassIsProxy | kAccPublic | kAccFinal | kAccVerificationAttempted);
