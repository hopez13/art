...
#if 0
      DCHECK(is_copied || in_image_space || Runtime::Current()->GetHeap()->ObjectIsInBootImageSpace())
...
bool ClassLinker::UpdateAppImageClassLoadersAndDexCaches(
  ...
  std::unordered_map<void*, void*> pointers_to_fixup;
  header.VisitPackedFixups([&inserted_in_boot_class_path, &pointers_to_fixup, this, space](
    ...
    LOG(ERROR) << existing << " " << existing->PrettyClass();
    LOG(ERROR) << klass << " " << klass->PrettyClass();
    fixup.Dump(LOG_STREAM(ERROR));
    if (existing != nullptr) {
      ...
      auto new_it = existing->GetMethods(image_pointer_size_).begin();
