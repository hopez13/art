  ...
  bool operator()(ObjPtr<mirror::Class> klass) OVERRIDE REQUIRES_SHARED(Locks::mutator_lock_) {
    if (!klass->IsProxyClass() &&
      ...
      if (!klass->IsArrayClass()) {
        CHECK_LT(klass->GetDexClassDefIndex(), dex_file.NumClassDefs());
      }
      last_resolved_classes_->AddClass(klass->GetDexTypeIndex());
      if (UNLIKELY(vlog_is_on_)) {
        ...
        if (last_extra_stats_ == extra_stats_.end() ||
          ...
          if (klass->IsArrayClass()) {
            last_extra_stats_->second.number_of_resolved_arrays_ += 1u;
          }
        }
      }
    }
  ...
  void PrintStatistics() const {
    if (vlog_is_on_) {
      for (const DexCacheResolvedClasses& resolved_classes : *result_) {
        ...
        uint32_t number_of_resolved_classes =
            resolved_classes.GetClasses().size() - extra_stats.number_of_resolved_arrays_;
        LOG(INFO) << "Dex location " << resolved_classes.GetDexLocation()
                  << " has " << number_of_resolved_classes << " / "
                  << extra_stats.number_of_class_defs_ << " resolved classes and "
                  << extra_stats.number_of_resolved_arrays_ << " resolved arrays.";
      }
    }
  }
  ...
  struct ExtraStats {
    explicit ExtraStats(uint32_t number_of_class_defs)
        : number_of_class_defs_(number_of_class_defs),
          number_of_resolved_arrays_(0u) {}
    uint32_t number_of_class_defs_;
    uint32_t number_of_resolved_arrays_;
  };
