...
ArtMethod* ClassLinker::ResolveMethod(const DexFile& dex_file,
  ...
  if (resolved != nullptr && !resolved->IsRuntimeMethod()) {
    ...
    if (kResolveMode == ClassLinker::kForceICCECheck) {
      // TODO: We need to check the type of the referenced class as well (interface/non-interface).
  ...
  if (klass->IsInterface()) {
    ...
    resolved = klass->FindInstanceMethod(dex_cache.Get(), method_idx, pointer_size);
  }
...
ArtMethod* ClassLinker::ResolveMethodWithoutInvokeType(const DexFile& dex_file,
  ...
  resolved = klass->FindInstanceMethod(dex_cache.Get(), method_idx, image_pointer_size_);
