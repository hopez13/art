...
static ArrayRef<const NewInterfaceReference> FindNewIfTableInterfaces(
    ...
    REQUIRES_SHARED(Locks::mutator_lock_) {
  DCHECK_EQ(kProxyClass, klass->IsProxyClass());
  DCHECK_EQ(kProxyClass, interfaces != nullptr);
  ScopedAssertNoThreadSuspension nts(__FUNCTION__);
  ...
  for (size_t i = 0; i != num_interfaces; ++i) {
    ObjPtr<mirror::Class> interface =
        kProxyClass ? interfaces->Get(i) : klass->GetDirectInterface(i);
...
static ObjPtr<mirror::IfTable> SetupInterfaceLookupTable(
  ...
  const size_t num_interfaces =
      kProxyClass ? interfaces->GetLength() : klass->NumDirectInterfaces();

  // If there are no new interfaces, we can recycle parent's interface table if the class
  // inherits no interfaces (this is always the case for interfaces as their superclass
  // `java.lang.Object` does not implement any interface) or only marker interfaces
  // from the superclass, or for a concrete class with no new virtuals.
  ...
  auto can_reuse_super_iftable = [=]() REQUIRES_SHARED(Locks::mutator_lock_) ALWAYS_INLINE{
    if (std::all_of(CountIter(0), CountIter(super_ifcount), is_marker_iface)) {
      return true;
    }
    DCHECK(!klass->IsInterface());
  ...
  for (size_t i = 0; i < num_interfaces; i++) {
    ObjPtr<mirror::Class> interface =
        kProxyClass ? interfaces->GetWithoutChecks(i) : klass->GetDirectInterface(i);
  ...
  ArrayRef<const NewInterfaceReference> new_interface_references =
      FindNewIfTableInterfaces<kProxyClass>(
          klass.Get(),
          interfaces.Get(),
          super_iftable,
          super_ifcount,
  ...
  for (NewInterfaceReference ref : new_interface_references) {
    ObjPtr<mirror::Class> direct_interface = kProxyClass
        ? interfaces->GetWithoutChecks(ref.direct_interface_index)
        : klass->GetDirectInterface(ref.direct_interface_index);
...
bool ClassLinker::LinkMethodsHelper<kPointerSize>::LinkMethods(
  ...
  if (klass->IsInterface()) {
    ...
    ObjPtr<mirror::IfTable> iftable =
        SetupInterfaceLookupTable</*kProxyClass=*/ false>(self, klass, interfaces, &allocator_);
    if (UNLIKELY(iftable == nullptr)) {
      self->AssertPendingException();
      return false;
    }
    ...
    Handle<mirror::IfTable> iftable = hs.NewHandle(UNLIKELY(klass->IsProxyClass())
        ? SetupInterfaceLookupTable</*kProxyClass=*/ true>(self, klass, interfaces, &allocator_)
        : SetupInterfaceLookupTable</*kProxyClass=*/ false>(self, klass, interfaces, &allocator_));
    if (UNLIKELY(iftable == nullptr)) {
      self->AssertPendingException();
      return false;
