...
mirror::Class* ClassLinker::FindClass(Thread* self,
  ...
  if (descriptor[0] == '[') {
    ...
    if (!self->HasPeer()) {
      ...
      //
      // Note that there a time window when the main thread has no peer. That is between
      // Runtime::Init(), when the thread is created, and Runtime::Start(), when the peer is
      // attached. It is still OK to throw here NoClassDefFoundError. During that time window
