    ...
    *out_forward_dex_cache_array = num_dex_caches_with_bss_arrays != 0;
    ...
    for (size_t i = 0; i < num_dex_caches; i++) {
      ...
      // If the oat file expects the dex cache arrays to be in the BSS, then allocate there and
        ...
        const size_t num_strings = mirror::DexCache::kDexCacheStringCacheSize;
        ...
        if (num_strings != 0u) {
          ...
          for (size_t j = 0; kIsDebugBuild && j < num_strings; ++j) {
            ...
            DCHECK_EQ(strings[j].load(std::memory_order_relaxed).string_pointer, 0u);
...
mirror::DexCache* ClassLinker::AllocDexCache(Thread* self,
  ...
  if (kIsDebugBuild) {
    ...
    for (size_t i = 0; i < mirror::DexCache::kDexCacheStringCacheSize; ++i) {
      CHECK_EQ(strings[i].load(std::memory_order_relaxed).string_index, 0u);
      CHECK_EQ(strings[i].load(std::memory_order_relaxed).string_pointer, 0u);
    }
