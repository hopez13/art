...
bool ClassLinker::AttemptSupertypeVerification(Thread* self,
  ...
  if (supertype->IsVerified()
      ...
      || (supertype->GetStatus() == ClassStatus::kVerifiedWithAccessChecks)) {
...
verifier::FailureKind ClassLinker::VerifyClass(
    Thread* self, Handle<mirror::Class> klass, verifier::HardFailLogMode log_level) {
  {
    ...
    if (klass->GetStatus() == ClassStatus::kVerifiedWithAccessChecks) {
      if (!Runtime::Current()->IsAotCompiler()) {
        klass->SetVerificationAttempted();
    ...
    if (verifier_failure == verifier::FailureKind::kNoFailure) {
      ...
      if (Runtime::Current()->IsAotCompiler()) {
        if (verifier_failure == verifier::FailureKind::kSoftFailure) {
          ...
          mirror::Class::SetStatus(klass, ClassStatus::kVerifiedWithAccessChecks, self);
        }
      } else {
        mirror::Class::SetStatus(klass, ClassStatus::kVerified, self);
  ...
  if (preverified || verifier_failure == verifier::FailureKind::kNoFailure) {
    ...
    if (oat_file_class_status == ClassStatus::kVerifiedWithAccessChecks ||
...
bool ClassLinker::VerifyClassUsingOatFile(const DexFile& dex_file,
  ...
  CHECK_NE(oat_file_class_status, ClassStatus::kRetryVerificationAtRuntime) << klass->PrettyClass() << " " << dex_file.GetLocation();
    ...
    if (!klass->IsVerified()) {
      ...
      if (!klass->IsVerified()) {
        ...
        if (klass->IsErroneous()) {
          ...
          CHECK(Runtime::Current()->IsAotCompiler()) << klass->PrettyClass() << klass->GetStatus();
          CHECK(klass->GetStatus() == ClassStatus::kRetryVerificationAtRuntime ||
                klass->GetStatus() == ClassStatus::kVerifiedWithAccessChecks);
          self->AssertNoPendingException();
