...
void ClassLinker::SetRecordClassFlagIfNeeded(Handle<mirror::Class> klass) {
  ...
  ObjPtr<mirror::Class> super = klass->GetSuperClass();
  if (super == nullptr || !super->DescriptorEquals("Ljava/lang/Record;")) {
    return;
  }

  // Record class should have record attributes specifying various things,
  // including the order of record components. As this was written, a equivlent
  // instance field is added for every record component. We will verify that every
  // record component in the attributes can be mapped into a field.

  // Before the dexer emitting @dalvik.annotation.Record annotation correctly,
  // we work around by taking @MethodParameters information as a temporary replacement.
  uint32_t numOfFields = klass->NumInstanceFields();
  for (ArtMethod& method : klass->GetDirectMethods(kRuntimePointerSize)) {
    ...
    if (method.IsConstructor() && method.GetNumberOfParameters() >= numOfFields) {
      if (annotations::IsMethodAnnotationPresent(
        ...
        // class constructor to confirm that it's the canoncial constructor.
