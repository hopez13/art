...
ObjPtr<mirror::ClassLoader> ClassLinker::FindClassLoader(Thread* self,
                                                         ObjPtr<mirror::DexCache> dex_cache) {
  ClassTable* class_table = FindClassTable(self, dex_cache);
  CHECK(class_table != nullptr);

  WriterMutexLock mu(self, *Locks::classlinker_classes_lock_);

  if (class_table == boot_class_table_.get()) {
    return nullptr;
  }

  for (const ClassLoaderData& data : class_loaders_) {
    if (data.class_table == class_table) {
      return ObjPtr<mirror::ClassLoader>::DownCast(self->DecodeJObject(data.weak_root));
    }
  }

  // Do not return 'nullptr' here as it may be misinterpreted as the boot class loader.
  LOG(FATAL) << "Could not find class loader";
  UNREACHABLE();
}
