...
bool ClassLinker::LinkFields(Thread* self,
  ...
  if (num_reference_fields != 0u) {
    field_offset =
        field_gaps.AlignFieldOffset<sizeof(mirror::HeapReference<mirror::Object>)>(field_offset);
    for (; index != num_reference_fields; ++index) {
      ArtField* field = sorted_fields[index].first;
      field_offset =
          AssignFieldOffset<sizeof(mirror::HeapReference<mirror::Object>)>(field, field_offset);
    }
  }
