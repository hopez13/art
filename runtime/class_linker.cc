...
void ClassLinker::LinkMethodsHelper<kPointerSize>::FinalizeIfTable(
  ...
  if (!is_klass_abstract) {
    ...
    if (is_super_abstract) {
      ...
      LengthPrefixedArray<ArtMethod>* const new_methods = klass_->GetMethodsPtr();
      if (new_methods != nullptr) {
        DCHECK_NE(new_methods->size(), 0u);
        new_methods_begin = reinterpret_cast<uintptr_t>(&new_methods->At(0));
        new_methods_size = new_methods->size() * kMethodSize;
      }
    }
  }
