...
static bool GetDexFileNames(ScopedObjectAccessUnchecked& soa,
                            ObjPtr<mirror::ClassLoader> class_loader,
                            std::list<ObjPtr<mirror::String>>& dex_files,
                            std::string* error_msg)
    REQUIRES_SHARED(Locks::mutator_lock_) {
...
static bool CompareKinds(ScopedObjectAccessUnchecked& soa,
...
static bool CompareDexFiles(const std::list<ObjPtr<mirror::String>>& image_dex_files,
  ...
  if (equal) {
    auto it1 = image_dex_files.begin();
    auto it2 = loader_dex_files.begin();
    for (size_t i = 0; equal && i < image_dex_files.size(); ++i, ++it1, ++it2) {
...
static bool CompareClassLoaders(ScopedObjectAccessUnchecked& soa,
                                ...
                                std::string* error_msg)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  if (!CompareKinds(soa, image_class_loader, class_loader, error_msg)) {
    return false;
  }
...
bool ClassLinker::AddImageSpace(
  ...
  if (app_image) {
    ...
    ScopedAssertNoThreadSuspension sants("Creating app image", soa.Self());
    ...
    // Special case the comparison of dex files of this class loader, as these
    // are not setup yet.
