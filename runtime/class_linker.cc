    ...
    for (size_t i = 0; i < num_dex_caches; i++) {
      ...
      if (oat_dex_file != nullptr && oat_dex_file->GetDexCacheArrays() != nullptr) {
        ...
        if (num_call_sites != 0u) {
          mirror::CallSiteDexCacheType* const image_resolved_call_sites =
              dex_cache->GetResolvedCallSites();
          mirror::CallSiteDexCacheType* const call_sites =
              reinterpret_cast<mirror::CallSiteDexCacheType*>(
                  raw_arrays + layout.CallSitesOffset());
          for (size_t j = 0; j < num_call_sites; ++j) {
            DCHECK_EQ(call_sites[j].load(std::memory_order_relaxed).index, 0u);
            DCHECK(call_sites[j].load(std::memory_order_relaxed).object.IsNull());
            call_sites[j].store(
                image_resolved_call_sites[j].load(std::memory_order_relaxed),
                std::memory_order_relaxed);
          }

          mirror::CallSiteDexCachePair::Initialize(call_sites);
...
mirror::MethodHandleImpl* ClassLinker::ResolveMethodHandle(uint32_t method_handle_idx,
                                                           ArtMethod* referrer)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  Thread* const self = Thread::Current();
