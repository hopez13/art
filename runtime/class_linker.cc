...
size_t ClassLinker::LinkMethodsHelper<kPointerSize>::AssignVtableIndexes(
  ...
  if (super_vtable_length > mirror::Object::kVTableLength) {
    auto insert_super_method = [&](size_t i, ArtMethod* m) ALWAYS_INLINE
        REQUIRES_SHARED(Locks::mutator_lock_) {
      size_t hash = ComputeMethodHash(m);
    ...
    if (super_class->ShouldHaveEmbeddedVTable()) {
      ...
      for (size_t i = mirror::Object::kVTableLength; i < super_vtable_length; i++) {
        insert_super_method(i, super_vtable->GetElementPtrSize<ArtMethod*, kPointerSize>(i));
      }
    }
  }
  ...
  for (size_t i = 0; i < num_virtual_methods; ++i) {
    ...
    auto it = super_vtable_signatures.FindWithHash(virtual_method, hash);
...
bool ClassLinker::LinkMethodsHelper<kPointerSize>::LinkVirtualMethods(
  ...
  if (klass->IsInterface()) {
    ...
    // For non-overriden vtable slots, copy a method from `super_class`.
