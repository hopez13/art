    ...
    header.VisitPackedArtMethods([&](ArtMethod& method) REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (method.HasCodeItem()) {
        const dex::CodeItem* code_item = method.GetDexFile()->GetCodeItem(
            reinterpret_cast<uintptr_t>(method.GetDataPtrSize(image_pointer_size_)));
...
void ClassLinker::LoadMethod(const DexFile& dex_file,
  ...
  if (dst->HasCodeItem()) {
    ...
    if (Runtime::Current()->IsAotCompiler()) {
      dst->SetDataPtrSize(reinterpret_cast<void*>(method.GetCodeItemOffset()), image_pointer_size_);
    } else {
      dst->SetDataPtrSize(dst->GetDexFile()->GetCodeItem(method.GetCodeItemOffset()), image_pointer_size_);
    }
  } else {
