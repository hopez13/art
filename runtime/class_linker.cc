...
bool ClassLinker::AddImageSpace(
  ...
  // Make the string intern table and class table immutable.
  const ImageSection& intern_table_section = header.GetInternedStringsSection();
  const ImageSection& class_table_section = header.GetClassTableSection();
  DCHECK_LE(intern_table_section.Offset(), class_table_section.Offset());
  uint8_t* const_section_begin = space->Begin() + intern_table_section.Offset();
  size_t const_section_size =
      class_table_section.Offset() + class_table_section.Size() - intern_table_section.Offset();
  mprotect(const_section_begin, RoundUp(const_section_size, kPageSize), PROT_READ);
