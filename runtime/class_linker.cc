        ...
        for (int32_t j = 0; j < static_cast<int32_t>(num_types); j++) {
          ...
          if (space->HasAddress(klass)) {
            ...
            if (num_dex_caches > 1) {
              ...
              if (existing != nullptr) {
                DCHECK_EQ(existing, klass) << klass->PrettyClass();
              } else {
                table->Insert(klass);
              }
            } else {
            ...
            DCHECK(klass == nullptr || heap->ObjectIsInBootImageSpace(klass))
                << klass << " " << mirror::Class::PrettyClass(klass);
          }
        }
      }
      if (kIsDebugBuild) {
    ...
    for (int32_t i = 0; i < dex_caches->GetLength(); i++) {
      ...
      for (int32_t j = 0, num_types = dex_cache->NumResolvedTypes(); j < num_types; j++) {
        ...
        if (klass != nullptr) {
          ...
          mirror::Class* existing = class_table->Lookup(descriptor, hash);
          if (existing != nullptr) {
            CHECK_EQ(existing, klass) << existing->PrettyClassAndClassLoader() << " != "
