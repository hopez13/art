...
class ClassLinker::LinkMethodsHelper {
 ...
 private:
  ...
  void FindCopiedMethodsForInterface(ObjPtr<mirror::Class> klass,
                                     size_t num_virtual_methods,
                                     ObjPtr<mirror::IfTable> iftable)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  class CopiedMethodRecord {
   ...
   private:
    ...
    //
    // Returns
    // - true:  An overriding method was found in any subinterface of `iface`.
    // - false: An overriding method was not found in any subinterface of `iface`.
...
void ClassLinker::LinkMethodsHelper<kPointerSize>::AddCopiedMethods() {
  ...
  for (size_t i = 0; i != num_new_copied_methods; ++i) {
    ...
    new_method.SetMethodIndex(record->GetMethodIndex());
...
template <PointerSize kPointerSize>
void ClassLinker::LinkMethodsHelper<kPointerSize>::FindCopiedMethodsForInterface(
    ObjPtr<mirror::Class> klass,
    size_t num_virtual_methods,
    ObjPtr<mirror::IfTable> iftable) {
  DCHECK(klass->IsInterface());
  ...
  for (size_t i = ifcount; i != 0u; ) {
    ...
    for (size_t j = 0; j != num_methods; ++j) {
      ...
      size_t hash = ComputeMethodHash(interface_method);
      if (declared_virtual_signatures.FindWithHash(interface_method, hash) !=
          declared_virtual_signatures.end()) {
  ...
  // Prune records without conflict.
...
bool ClassLinker::LinkMethodsHelper<kPointerSize>::LinkMethods(
  ...
  if (klass->IsInterface()) {
    ...
    if (have_super_with_defaults) {
      FindCopiedMethodsForInterface(klass.Get(), num_virtual_methods, iftable);
    }
...

  {
    ...
    for (const CopiedMethodRecord& record : copied_method_records_) {
      ...
      switch (record.GetState()) {
        case CopiedMethodRecord::State::kAbstractSingle:
        case CopiedMethodRecord::State::kAbstract:
          CHECK(false) << "No miranda methods in interfaces!";
          break;
        case CopiedMethodRecord::State::kDefaultSingle:
        case CopiedMethodRecord::State::kDefault:
          CHECK(false) << "No default methods in interfaces!";
          break;
        case CopiedMethodRecord::State::kDefaultConflict:
          container = &default_conflict_methods_;
          ++num_default_conflict;
          break;
        case CopiedMethodRecord::State::kUseSuperMethod:
          // Ignore.
          break;
      }
      if (container != nullptr) {
        CHECK(std::any_of(container->begin(),
                          ...
                          })) << enum_cast<uint32_t>(record.GetState());
      }
    }
