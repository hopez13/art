...
class CHACheckpoint FINAL : public Closure {
 public:
  CHACheckpoint(const std::unordered_set<OatQuickMethodHeader*>& method_headers)
      : barrier_(0),
        method_headers_(method_headers) {}
  ...
  void Run(Thread* thread) OVERRIDE {
    ...
    CHAStackVisitor visitor(thread, context.get(), method_headers_);
    visitor.WalkStack();
    barrier_.Pass(self);
  }
...
void ClassLinker::InitSingleImplementationFlag(Handle<mirror::Class> klass,
  ...
  if (method->IsNative()) {
    // Skip native method since we may use the native entry point to
...
void ClassLinker::UpdateCHAInfo(Handle<mirror::Class> klass) {
  ...
  // Keeps track of all methods whose single-impelmentation assumption
  ...
  if (!invalidated_single_impl_methods.empty()) {
...

    {
      ...
      for (ArtMethod* invalidated : invalidated_single_impl_methods) {
        ...
        if (runtime->IsAotCompiler()) {
          // No need to invalidate any compiled code since currently CHA only
          // works for jitted code.
          continue;
        }
        ...
        for (auto it = dependents->begin(); it != dependents->end(); ++it) {
          ...
          OatQuickMethodHeader* method_header = it->second;
...
void ClassLinker::AddCHADependency(ArtMethod* method,
  ...
  if ((it == cha_dependency_map_.end()) || (it->second == nullptr)) {
