...
enum class RecordElementType : uint8_t {
  ...
  kTypeTypes = 1,
  kTypeSignatures = 2,
  kTypeAnnotationVisibilities = 3,
  kTypeAnnotations = 4
};

static const char* RECORD_ELEMENT_NAMES[] = {"componentNames",
                                             "componentTypes",
                                             "componentSignatures",
                                             "componentAnnotationVisibilities",
                                             "componentAnnotations"};

class RecordAnnotationVisitor : public annotations::AnnotationVisitor {
 public:
  ...
  const std::string& getErrorMsg() {
    ...
    if (count_ >= 2) {
      ...
      SetErrorMsg(StringPrintf("componentSignatures size is %i, but is expected to be %i",
                               signatures_count_,
                               names_count_));
    } else if (visibilities_count_ != UINT32_MAX && visibilities_count_ != names_count_) {
      SetErrorMsg(
          StringPrintf("componentAnnotationVisibilities size is %i, but is expected to be %i",
                       visibilities_count_,
                       names_count_));
    } else if (annotations_count_ != UINT32_MAX && annotations_count_ != names_count_) {
  ...
  annotations::VisitorStatus VisitAnnotation(const char* descriptor, uint8_t visibility) {
    if (is_error_) {
  ...
  annotations::VisitorStatus VisitAnnotationElement(const char* element_name,
                                                    uint8_t type,
                                                    [[maybe_unused]] const JValue& value) {
    if (is_error_) {
      return annotations::VisitorStatus::kVisitBreak;
    }
    ...
    if (strcmp(element_name, "componentNames") == 0) {
      visiting_type = RecordElementType::kTypeNames;
      if (names_count_ != UINT32_MAX) {
        duplicated = true;
      }
      names_count_ = 0;
    } else if (strcmp(element_name, "componentTypes") == 0) {
      visiting_type = RecordElementType::kTypeTypes;
      if (types_count_ != UINT32_MAX) {
        duplicated = true;
      }
      types_count_ = 0;
    } else if (strcmp(element_name, "componentSignatures") == 0) {
      visiting_type = RecordElementType::kTypeSignatures;
      if (signatures_count_ != UINT32_MAX) {
        duplicated = true;
      }
      signatures_count_ = 0;
    } else if (strcmp(element_name, "componentAnnotationVisibilities") == 0) {
      visiting_type = RecordElementType::kTypeAnnotationVisibilities;
      if (visibilities_count_ != UINT32_MAX) {
        duplicated = true;
      }
      visibilities_count_ = 0;
    } else if (strcmp(element_name, "componentAnnotations") == 0) {
      visiting_type = RecordElementType::kTypeAnnotations;
      if (annotations_count_ != UINT32_MAX) {
        duplicated = true;
      }
      annotations_count_ = 0;
    } else {
    ...
    if (duplicated) {
      SetErrorMsg(StringPrintf("Two %s annotation elements are found but only one is expected",
                               RECORD_ELEMENT_NAMES[static_cast<uint8_t>(visiting_type)]));
      return annotations::VisitorStatus::kVisitBreak;
    }
  ...
  annotations::VisitorStatus VisitArrayElement(uint8_t depth,
    ...
    switch (visiting_type_) {
      case RecordElementType::kTypeNames: {
        if (depth == 0) {
      ...
      case RecordElementType::kTypeTypes: {
        if (depth == 0) {
          if (!ExpectedTypeOrError(
      ...
      case RecordElementType::kTypeSignatures: {
        if (depth == 0) {
      ...
      case RecordElementType::kTypeAnnotationVisibilities: {
        if (depth == 0) {
          if (!ExpectedTypeOrError(
      ...
      case RecordElementType::kTypeAnnotations: {
        if (depth == 0) {
          if (!ExpectedTypeOrError(
                  type, DexFile::kDexAnnotationArray, visiting_type_, index, depth)) {
            return annotations::VisitorStatus::kVisitBreak;
    ...
    DCHECK(false) << StringPrintf("Unexpected depth %i for element %s",
                                  depth,
                                  RECORD_ELEMENT_NAMES[static_cast<uint8_t>(visiting_type_)]);
    return annotations::VisitorStatus::kVisitBreak;
  }
...
bool ClassLinker::VerifyRecordClass(Handle<mirror::Class> klass, ObjPtr<mirror::Class> super) {
  ...
  const std::string msg = visitor.getErrorMsg();
  if (!msg.empty()) {
    ThrowClassFormatError(klass.Get(), "%s", msg.c_str());
    return false;
  }
