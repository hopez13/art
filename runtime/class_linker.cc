...
class NonProxyInterfacesAccessor {
 public:
  ...
  ObjPtr<mirror::Class> GetInterface(size_t index) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    return lookup_data_->class_linker_->LookupResolvedType(
        type_index, lookup_data_->dex_cache_.Get(), lookup_data_->class_loader_.Get());
  }
  ...
  class LookupData {
   public:
    LookupData(Thread* self, ClassLinker* class_linker, Handle<mirror::Class> klass)
        ...
        : class_linker_(class_linker),
          hs_(self),
          dex_cache_(hs_.NewHandle(klass->GetDexCache())),
          class_loader_(hs_.NewHandle(klass->GetClassLoader())) {}
