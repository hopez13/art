...
bool ClassLinker::InitFromBootImage(std::string* error_msg) {
  ...
  if (kIsDebugBuild) {
    ...
    for (size_t i = 1; i < oat_files.size(); ++i) {
      ...
      CHECK_EQ(ith_oat_header.GetJniDlsymLookupTrampoline(), nullptr);
      CHECK_EQ(ith_oat_header.GetJniDlsymLookupCriticalTrampoline(), nullptr);
      CHECK_EQ(ith_oat_header.GetQuickResolutionTrampoline(), nullptr);
      CHECK_EQ(ith_oat_header.GetQuickImtConflictTrampoline(), nullptr);
      CHECK_EQ(ith_oat_header.GetQuickGenericJniTrampoline(), nullptr);
      CHECK_EQ(ith_oat_header.GetQuickToInterpreterBridge(), nullptr);
      CHECK_EQ(ith_oat_header.GetNterpTrampoline(), nullptr);
    }
  }
...
bool ClassLinker::LinkVirtualMethods(
  ...
  if (klass->IsInterface()) {
    ...
    for (size_t j = 0; j < super_vtable_length; ++j) {
      ...
      // smaller as we go on.
      uint32_t hash = (j < kObjectVTableLength)
    ...
    static_assert(kObjectVTableLength == mirror::Object::kVTableLength);
