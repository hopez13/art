...
ArtMethod* ClassLinker::ResolveMethod(const DexFile& dex_file,
  ...
  switch (type) {
    ...
    case kStatic:
    case kVirtual:
      resolved = klass->FindDirectOrVirtualMethod(dex_cache.Get(), method_idx, pointer_size);
  ...
  if (LIKELY(resolved != nullptr && !resolved->CheckIncompatibleClassChange(type))) {
    ...
    if (resolved == nullptr) {
      ...
      DCHECK(resolved == nullptr || resolved->CheckIncompatibleClassChange(type));
    }
    if (resolved != nullptr) {
      ThrowIncompatibleClassChangeError(type, resolved->GetInvokeType(), resolved, referrer);
    } else {
...
ArtMethod* ClassLinker::ResolveMethodWithoutInvokeType(const DexFile& dex_file,
  ...
  resolved = klass->FindDirectOrVirtualMethod(dex_cache.Get(), method_idx, image_pointer_size_);
