...
// This function decodes hidden API access flags from the ClassDataItemIterator
// and decides which runtime access flags should be assigned to the runtime
// representation of the class member.
// APIs are split into four categories in DexFile: whitelist, light greylist,
// dark greylist and blacklist. At runtime, we either grant access (no flags),
// print a warning when the member is accessed (kAccHiddenGreylist), or
// completely hide it from the client code (kAccHiddenBlacklist). This logic
// maps the four dex-level categories onto three run-time behaviours.
...
void ClassLinker::LoadField(const ClassDataItemIterator& it,
  ...
  // also decode hidden API access flags and set corresponding runtime modifiers
  // (see GetHiddenAccessFlags for details).
