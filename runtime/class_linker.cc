...
static void CopyPages(const void* src,
  ...
  while (src_ptr < src_end) {
    ...
    size_t num_bytes = std::min(static_cast<size_t>(kPageSize), static_cast<size_t>(src_end - src_ptr));
    if (IsBitSet(bitmask[page_num / 8], page_num % 8)) {
...
bool ClassLinker::UpdateAppImageClassLoadersAndDexCaches(
  ...
  const ImageHeader& header = space->GetImageHeader();
  {
    ...
    *out_forward_dex_cache_array = num_dex_caches_with_bss_arrays != 0;
    ...
    const ImageSection& bitmap_section = header.GetImageSection(ImageHeader::kSectionDexCacheBitmap);
    const uint8_t* dex_cache_bitmap = space->Begin() + bitmap_section.Offset();
    const uint8_t* dex_cache_array_data = space->Begin() + header.GetImageSection(ImageHeader::kSectionDexCacheArrays).Offset();
    ...
    for (size_t i = 0; i < num_dex_caches; i++) {
      ...
      // If the oat file expects the dex cache arrays to be in the BSS, then allocate there and
        ...
        if (num_types != 0u) {
          ...
          CopyPages(image_resolved_types,
                    types, num_types * layout.TypeElementSize(),
                    dex_cache_bitmap,
      ...
      if (new_class_set == nullptr) {
        DexCacheArraysLayout layout(image_pointer_size_, dex_file);
        uint32_t type_idx = 0;
        while (type_idx < num_types) {
          size_t offset = image_resolved_types_begin +
                          layout.TypeOffset(type_idx) -
                          dex_cache_array_data;
          size_t page_num = offset / kPageSize;
          mirror::Class* klass = nullptr;
          if (IsBitSet(dex_cache_bitmap[page_num / 8], page_num % 8)) {
            // The image space is not yet added to the heap, avoid read barriers.
            klass = types[type_idx].Read();
            ++type_idx;
          } else {
            // Skip this page
            type_idx += kPageSize / layout.TypeElementSize();
          }
