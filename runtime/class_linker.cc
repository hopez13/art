...
// Since direct and virtual methods are separated, we need to reset the iteration when we
// switch from loading direct methods to loading virtual methods.
class ClassLinker::MethodAnnotationsIterator {
 public:
  MethodAnnotationsIterator(const DexFile& dex_file,
      ...
      : begin_((annotations_dir != nullptr) ? dex_file.GetMethodAnnotations(annotations_dir)
                                            : nullptr),
        end_((annotations_dir != nullptr) ? begin_ + annotations_dir->methods_size_ : nullptr),
        current_(begin_) {}
 ...
 private:
  const dex::MethodAnnotationsItem* const begin_;
...
void ClassLinker::LoadClass(Thread* self,
  ...
  Runtime* const runtime = Runtime::Current();
  {
    ...
    MethodAnnotationsIterator mai(dex_file, dex_file.GetAnnotationsDirectory(dex_class_def));
    ...
    accessor.VisitFieldsAndMethods([&](
        const ClassAccessor::Field& field) REQUIRES_SHARED(Locks::mutator_lock_) {
          ...
          ArtMethod* art_method = klass->GetDirectMethodUnchecked(class_def_method_index,
              image_pointer_size_);
          LoadMethod(dex_file, method, klass.Get(), &mai, art_method);
