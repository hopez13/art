...
class QuickExceptionHandler {
 public:
  ...
  // skip_method_exit_cb specifies if we should skip method exit callbacks for the top frame. It is
  // set if a deopt is needed after calling method exit callback for ex: if the callback throws or
  // performs other actions that require a deopt.
  void DeoptimizeStack(bool skip_method_exit_cb) REQUIRES_SHARED(Locks::mutator_lock_);
