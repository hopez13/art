...
bool VarHandleInvokeAccessorWithConversions(Thread* self,
  ...
  VariableSizedHandleScope accessor_type(self);
  ...
  auto callsite_type_accessor = mirror::MethodType::NewHandleAccessor(callsite_type, &hs);
  auto accessor_type_accessor = mirror::MethodType::NewHandleAccessor(&accessor_type, &hs);
  ...
  if (!PerformConversions(
          throw_wmt, callsite_type_accessor, accessor_type_accessor, &getter, &setter)) {
  ...
  if (!ConvertReturnValue(throw_wmt,
                          accessor_type_accessor.GetRType(),
                          callsite_type_accessor.GetRType(),
                          result)) {
    DCHECK(self->IsExceptionPending());
    return false;
  }
