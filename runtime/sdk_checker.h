...
class SdkChecker {
 public:
  static SdkChecker* Create(const std::string& api_classpath, std::string* error_msg);
  ...
  // Simillar to ShouldDenyAccess(ArtMethod* art_method).
 ...
 private:
  ...
  std::vector<std::unique_ptr<const DexFile>> api_classpath_;
};
