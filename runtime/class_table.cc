...
ObjPtr<mirror::Class> ClassTable::Lookup(const char* descriptor, size_t hash) {
  DescriptorHashPair pair(descriptor, hash);
  ReaderMutexLock mu(Thread::Current(), lock_);
  // Search from the last table, assuming apps shall search for their own classes
  // more often than for boot image classes. For prebuilt boot images, this also
  // helps by searching the large table from the framework boot image extension
  // compiled as single-image before the individual small tables from the primary
  // boot image compiled as multi-image.
