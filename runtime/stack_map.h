...
class DexRegisterMap {
 public:
  // Create map of given size and initialize all locations to None.
  DexRegisterMap(size_t size) : size_(size), regs_small_{} {
    if (size_ <= kSmallSize) {
      std::fill_n(regs_small_.begin(), size, DexRegisterLocation::None());
    } else {
      regs_large_.resize(size, DexRegisterLocation::None());
    }
  }

  bool IsValid() const { return size_ != 0; }

  size_t GetDexRegisterCount() const { return size_; }

  DexRegisterLocation& operator[](size_t index) {
    DCHECK_LT(index, size_);
    return size_ <= kSmallSize ? regs_small_[index] : regs_large_[index];
  }

  DexRegisterLocation operator[](size_t index) const {
    DCHECK_LT(index, size_);
    return size_ <= kSmallSize ? regs_small_[index] : regs_large_[index];
  }

  DexRegisterLocation::Kind GetLocationKind(uint16_t dex_register_number) const {
    return (*this)[dex_register_number].GetKind();
  }

  DexRegisterLocation GetDexRegisterLocation(uint16_t dex_register_number) const {
    return (*this)[dex_register_number];
  }

  int32_t GetStackOffsetInBytes(uint16_t dex_register_number) const {
    DexRegisterLocation location = (*this)[dex_register_number];
  ...
  ALWAYS_INLINE bool IsDexRegisterLive(uint16_t dex_register_number) const {
    return (*this)[dex_register_number].IsLive();
  }
 ...
 private:
  static constexpr size_t kSmallSize = 16;
  size_t size_;
  std::array<DexRegisterLocation, kSmallSize> regs_small_;
...
class StackMap : public BitTable<7>::Accessor {
 public:
  ...
  static const char** GetFieldNames() {
    static const char* names[kCount] = {
      "NativePcOffset",
      "DexPc",
      "RegisterMaskIndex",
      "StackMaskIndex",
      "InlineInfoIndex",
      "DexRegisterMaskIndex",
      "DexRegisterMapIndex",
    };
    return names;
  }
...
class DexRegisterInfo : public BitTable<2>::Accessor {
 public:
  ...
  ALWAYS_INLINE DexRegisterLocation GetLocation() const {
    ...
    uint32_t value = Get<kPackedValue>();
...
class CodeInfo {
 public:
  ...
  uint32_t GetRegisterMaskOf(const StackMap& stack_map) const {
    ...
    // The register mask is a bitmask, not an integer.  So we need to load it with 0 bias.
    return (index == StackMap::kNoValue) ? 0 : register_masks_.Get(index, 0, 0 /* bias */);
  }
 ...
 private:
  ALWAYS_INLINE DexRegisterMap DecodeDexRegisterMap(uint32_t mask_index,
    ...
    if (mask_index != StackMap::kNoValue) {
      ...
      for (uint32_t r = 0; r < num_dex_registers; r++) {
        if (mask.LoadBit(r) /* is_live */) {
          map[r] = GetDexRegisterCatalogEntry(dex_register_maps_.Get(map_index++));
        }
      }
    }
    return map;
