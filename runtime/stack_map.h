...
class DexRegisterMap {
 public:
  ...
  DexRegisterLocation* data() {
    return size_ <= kSmallSize ? regs_small_.data() : regs_large_.data();
  }
 ...
 private:
  // To avoid memory allocations, store the data inline if the 'size_' is small enough.
  // If size_ <= kSmallSize, we use the regs_small_ array, otherwise the regs_large_ vector.
