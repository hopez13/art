 ...
 */
class StackMap : public BitTable<6>::Iterator {
 public:
  enum Field {
    kNativePcOffset,
    kDexPc,
  ...
  StackMap() : BitTable<kCount>::Iterator(nullptr, -1) {}
  StackMap(BitTable<kCount>* table, uint32_t row) : BitTable<kCount>::Iterator(table, row) {}
 ...
 */
class InlineInfo : public BitTable<5>::Iterator {
 public:
  enum Field {
  ...
  InlineInfo(BitTable<kCount>* table, uint32_t row) : BitTable<kCount>::Iterator(table, row) {}
...
class InvokeInfo : public BitTable<3>::Iterator {
 public:
  enum Field {
    kNativePcOffset,
    kInvokeType,
  ...
  InvokeInfo(BitTable<kCount>* table, uint32_t row) : BitTable<kCount>::Iterator(table, row) {}
...
class CodeInfo {
 public:
  ...
  bool HasInlineInfo() const {
    return stack_map_.NumColumnBits(StackMap::kInlineInfoIndex) != 0;
  }
  ...
  ALWAYS_INLINE StackMap GetStackMapAt(size_t index) const {
    return stack_map_.ConstIterator<StackMap>(index);
  }

  BitMemoryRegion GetStackMask(size_t index) const {
    return BitMemoryRegion(stack_mask_, index * stack_mask_bits_, stack_mask_bits_);
  }
  ...
  uint32_t GetRegisterMaskOf(const StackMap& stack_map) const {
    return register_mask_.Get(stack_map.GetRegisterMaskIndex());
  }
  ...
  uint32_t GetNumberOfStackMaps() const {
    return stack_map_.NumRows();
  }

  InvokeInfo GetInvokeInfo(size_t index) const {
    return invoke_info_.ConstIterator<InvokeInfo>(index);
  }
  ...
  DexRegisterMap GetDexRegisterMapOf(StackMap stack_map,
    ...
    return DexRegisterMap(dex_register_map_.Subregion(offset, size));
  }
  ...
  InlineInfo GetInlineInfo(size_t index) const {
    return inline_info_.ConstIterator<InlineInfo>(index);
  }
 ...
 private:
  ...
  size_t ComputeDexRegisterMapSizeOf(uint32_t dex_register_map_offset,
    ...
    DexRegisterMap dex_register_map_without_locations(
        MemoryRegion(dex_register_map_.Subregion(dex_register_map_offset,
  ...
  MemoryRegion DecodeMemoryRegion(MemoryRegion& region, size_t* bit_offset) {
    size_t length = DecodeVarintBits(region, bit_offset);
    size_t offset = RoundUp(*bit_offset, kBitsPerByte) / kBitsPerByte;
  ...
  void Decode(const uint8_t* data) {
    ...
    stack_map_.Decode(region, &bit_offset);
    invoke_info_.Decode(region, &bit_offset);
    inline_info_.Decode(region, &bit_offset);
    register_mask_.Decode(region, &bit_offset);
    stack_mask_bits_ = DecodeVarintBits(region, &bit_offset);
    stack_mask_ = DecodeMemoryRegion(region, &bit_offset);
    dex_register_map_ = DecodeMemoryRegion(region, &bit_offset);
    location_catalog_entries_ = DecodeVarintBits(region, &bit_offset);
    location_catalog_ = DecodeMemoryRegion(region, &bit_offset);
    DCHECK_EQ(BitsToBytesRoundUp(bit_offset), non_header_size);
  }

  size_t size_;
  BitTable<StackMap::Field::kCount> stack_map_;
  BitTable<InvokeInfo::Field::kCount> invoke_info_;
  BitTable<InlineInfo::Field::kCount> inline_info_;
  BitTable<1> register_mask_;
  uint32_t stack_mask_bits_ = 0;
  MemoryRegion stack_mask_;
  MemoryRegion dex_register_map_;
