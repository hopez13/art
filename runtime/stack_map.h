...
struct EncodingTable {
  ...
  // How the coding is laid out (serialized).
  Encoding encoding;
  ...
  void Encode(Vector* dest) {
    static_assert(alignof(EncodingTable<Encoding>) == 1,
                  "TableEncoding should not require alignment");
    EncodeUnsignedLeb128(dest, num_entries);
    const uint8_t* ptr = reinterpret_cast<const uint8_t*>(&encoding);
    dest->insert(dest->end(), ptr, ptr + sizeof(encoding));
  }

  void Decode(const uint8_t** ptr) {
    num_entries = DecodeUnsignedLeb128(ptr);
    encoding = *reinterpret_cast<const Encoding*>(ptr);
    ptr += sizeof(StackMapEncoding);
  }

  // Set the bit offset of the table. Returns the total number of bits to store the table.
  size_t UpdateBitOffset(size_t offset) {
    bit_offset = offset;
    return encoding.BitSize() * num_entries;
  }

  // Return the bit region for the map at index i.
  BitMemoryRegion BitRegion(MemoryRegion region, size_t index) const {
    DCHECK_NE(bit_offset, bit_offset) << "Invalid table offset";
...
struct ByteSizedTable {
  ...
  // Set the bit offset of the table. Returns the total number of bits to store the table.
  size_t UpdateBitOffset(size_t offset) {
    size_t alignment_bits = RoundUp(offset, kBitsPerByte) - offset;
    byte_offset = (offset + alignment_bits) / kBitsPerByte;
    return alignment_bits + num_bytes * kBitsPerByte;
  }
};
...
struct CodeInfoEncoding {
  ...
  EncodingTable<StackMapEncoding> stack_map;
  EncodingTable<BitRegionEncoding> register_mask;
  EncodingTable<BitRegionEncoding> stack_mask;
  EncodingTable<InlineInfoEncoding> inline_info;
  // Non table entries:
  size_t dex_register_map_size;
  ...
  explicit CodeInfoEncoding(const void* data) {
    ...
    if (stack_map.encoding.GetInlineInfoEncoding().BitSize() > 0) {
      ...
      inline_info = EncodingTable<InlineInfoEncoding>();
    }
  ...
  void ComputeTableOffsets() {
    // Skip the header.
    size_t bit_offset = header_size * kBitsPerByte;
    bit_offset += dex_register_map.UpdateBitOffset(bit_offset);
    bit_offset += location_catalog.UpdateBitOffset(bit_offset);
    bit_offset += stack_map.UpdateBitOffset(bit_offset);
    bit_offset += register_mask.UpdateBitOffset(bit_offset);
    bit_offset += stack_mask.UpdateBitOffset(bit_offset);
    bit_offset += inline_info.UpdateBitOffset(bit_offset);
    non_header_size = RoundUp(bit_offset, kBitsPerByte) / kBitsPerByte - header_size;
  }

  size_t HeaderSize() const {
    DCHECK_NE(header_size, kInvalidSize);
    return header_size;
  }

  size_t NonHeaderSize() const {
    DCHECK_NE(non_header_size, kInvalidSize);
    return non_header_size;
  }

 private:
  // Computed fields (not serialized).
  // Header size in bytes.
  uint32_t header_size = kInvalidSize;
  // Non header size in bytes.
  uint32_t non_header_size = kInvalidSize;
};
