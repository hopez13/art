...
class InlineInfo : public BitTableAccessor<8> {
 public:
  ...
  static constexpr uint32_t kNotInBootClassPath = -1;
  static constexpr uint32_t kInBootClassPath = 0;
...
class CodeInfo {
 public:
  ...
  uint32_t GetDexFileIndexOf(InlineInfo inline_info) const {
    DCHECK(method_infos_.GetRow(inline_info.GetMethodInfoIndex()).GetIsInBootClassPath());
    return method_infos_.GetRow(inline_info.GetMethodInfoIndex()).GetBootClassPathIndex();
  }
