...
class DexRegisterMap {
 public:
  ...
  explicit DexRegisterMap(size_t size) : size_(size), regs_small_{} {
    if (size_ <= kSmallSize) {
...
class CodeInfo {
 public:
  ...
  uint32_t GetRegisterMaskOf(const StackMap& stack_map) const {
    ...
    BitMemoryRegion mask = register_masks_.GetBitMemoryRegion(index);
    return mask.LoadBits(0, mask.size_in_bits());
  }
