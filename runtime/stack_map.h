...
struct EncodingTable {
  ...
  void Encode(Vector* dest) const {
    static_assert(alignof(Encoding) == 1, "Encoding should not require alignment");
...
struct CodeInfoEncoding {
  ...
  void ComputeTableOffsets() {
    ...
    non_header_size = RoundUp(bit_offset, kBitsPerByte) / kBitsPerByte - HeaderSize();
  }
