...
class CatchBlockStackVisitor FINAL : public StackVisitor {
 public:
  ...
  bool VisitFrame() OVERRIDE REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    LOG(INFO) << "Currently at depth " << GetFrameDepth() << " want to skip " << skip_frames_;
    if (method == nullptr) {
      DCHECK_EQ(skip_frames_, 0u) << "We tried to skip an upcall!";
...
void QuickExceptionHandler::FindCatch(ObjPtr<mirror::Throwable> exception) {
  ...
  instrumentation::ScopedInstrumentationStackPopper popper(self_);
