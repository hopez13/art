...
void QuickExceptionHandler::FindCatch(mirror::Throwable* exception) {
  ...
  if (kDebugExceptionDelivery) {
    ...
    if (handler_method_ != nullptr) {
      ...
      LOG(INFO) << "Handler: " << ArtMethod::PrettyMethod(handler_method_) << " (line: "
                << line_number << ")";
    }
  }
  if (clear_exception_) {
 ...
 public:
  ...
  void FinishStackWalk() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (GetMethod() == nullptr) {
      ...
      CHECK(callee_method_ != nullptr) << ArtMethod::PrettyMethod(GetMethod(), false);
 ...
 public:
  ...
  bool VisitFrame() OVERRIDE REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (method == nullptr) {
      ...
      if (show_details_) {
        LOG(INFO) << "R  " << ArtMethod::PrettyMethod(method, true);
      }
      return true;
    } else {
