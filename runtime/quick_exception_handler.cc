  ...
  bool HandleTryItems(ArtMethod* method)
    ...
    if (dex_pc != dex::kDexNoIndex) {
      ...
      if (found_dex_pc != dex::kDexNoIndex) {
        std::vector<uint32_t> dex_pc_list = GetDexPcList(found_dex_pc);
        exception_handler_->SetHandlerDexPcList(dex_pc_list);
        exception_handler_->SetHandlerQuickFramePc(
            GetCurrentOatQuickMethodHeader()->ToNativeQuickPc(
                method, ArrayRef<const uint32_t>(dex_pc_list), /* is_for_catch_handler= */ true));
        exception_handler_->SetHandlerQuickFrame(GetCurrentQuickFrame());
...
void QuickExceptionHandler::SetCatchEnvironmentForOptimizedHandler(StackVisitor* stack_visitor) {
  ...
  StackMap catch_stack_map = code_info.GetCatchStackMapForDexPc(GetHandlerDexPcList());
  DCHECK(catch_stack_map.IsValid());
  DexRegisterMap catch_vreg_map =
      stack_visitor->IsInInlinedFrame() ?
          code_info.GetInlineDexRegisterMapOf(catch_stack_map,
                                              stack_visitor->GetCurrentInlinedFrame()) :
          code_info.GetDexRegisterMapOf(catch_stack_map);
  ...
  DexRegisterMap throw_vreg_map =
      stack_visitor->IsInInlinedFrame() ?
          code_info.GetInlineDexRegisterMapOf(throw_stack_map,
                                              stack_visitor->GetCurrentInlinedFrame()) :
          code_info.GetDexRegisterMapOf(throw_stack_map);

  DCHECK_LE(catch_vreg_map.size(), throw_vreg_map.size());
  // Copy values between them.
  for (uint16_t vreg = 0; vreg < catch_vreg_map.size(); ++vreg) {
