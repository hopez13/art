...
void QuickExceptionHandler::SetCatchEnvironmentForOptimizedHandler(StackVisitor* stack_visitor) {
  ...
  size_t catch_vreg_start;
  if (catch_depth == 0) {
    // No inline frames, so all vregs are part of the catch's environment.
    catch_vreg_start = 0;
  } else if (catch_depth == 1) {
    // Only one inline frame. We get the number directly from the code info.
    catch_vreg_start = code_info.GetNumberOfDexRegisters();
  } else {
    // Several frames. We get the number by looking at the previous inline info.
    catch_vreg_start =
        code_info.GetInlineInfosOf(catch_stack_map)[catch_depth - 1].GetNumberOfDexRegisters();
  }
