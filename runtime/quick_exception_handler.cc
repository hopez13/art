...
void QuickExceptionHandler::FindCatch(ObjPtr<mirror::Throwable> exception) {
  ...
  do {
    ...
    popped_to_top = popper.PopFramesTo(reinterpret_cast<uint8_t*>(handler_quick_frame_), exception_ref);
  } while (!popped_to_top);
  if (!clear_exception_) {
...
uintptr_t QuickExceptionHandler::UpdateInstrumentationStack() {
  ...
  if (method_tracing_active_) {
    ...
    return_pc = instrumentation->PopFramesForDeoptimization(self_, reinterpret_cast<uint8_t*>(handler_quick_frame_));
  }
  return return_pc;
}
