...
class DeoptimizeStackVisitor final : public StackVisitor {
 public:
  DeoptimizeStackVisitor(Thread* self,
                         ...
                         QuickExceptionHandler* exception_handler,
                         bool single_frame,
                         bool skip_method_exit_cb) REQUIRES_SHARED(Locks::mutator_lock_)
      : StackVisitor(self, context, StackVisitor::StackWalkKind::kIncludeInlinedFrames),
        exception_handler_(exception_handler),
        ...
        skip_method_exit_cb_(skip_method_exit_cb) {}
  ...
  bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (method == nullptr || single_frame_done_) {
      ...
      if (GetFrameDepth() == 0U && skip_method_exit_cb_) {
...
void QuickExceptionHandler::DeoptimizeStack(bool skip_method_exit_cb) {
  DCHECK(is_deoptimization_);
  if (kDebugExceptionDelivery) {
    self_->DumpStack(LOG_STREAM(INFO) << "Deoptimizing: ");
  }

  DeoptimizeStackVisitor visitor(self_, context_, this, false, skip_method_exit_cb);
  visitor.WalkStack(true);
...
void QuickExceptionHandler::DeoptimizeSingleFrame(DeoptimizationKind kind) {
  ...
  DeoptimizeStackVisitor visitor(self_, context_, this, true, /* skip_method_exit_cb= */ false);
  visitor.WalkStack(true);
