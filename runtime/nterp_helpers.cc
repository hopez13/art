 ...
 *    - Before doing the longjmp, the register containing the dex PC pointer
 *    must be updated.
 *
 *    Stack walking:
...
static constexpr size_t pointer_size = static_cast<size_t>(kRuntimePointerSize);

static size_t NterpGetFrameEntrySize() {
  ...
  return (POPCOUNT(core_spills) + POPCOUNT(fp_spills)) * pointer_size;
}
...
static size_t NterpGetFrameSize(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  size_t frame_size =
      ...
      (num_regs * 4) * 2 +  // register array and reference array
      pointer_size +  // previous frame
      pointer_size +  // saved dex pc
      (out_regs * 4) +  // out arguments
      pointer_size;  // method
...
uintptr_t NterpGetReferenceArray(ArtMethod** frame) {
  ...
  return NterpGetRegistersArray(frame) - (num_regs * 4);
}

uint32_t NterpGetDexPC(ArtMethod** frame) {
  uintptr_t dex_pc_ptr = NterpGetReferenceArray(frame) -
      pointer_size -  // saved previous frame
      pointer_size;   // saved dex pc
  CodeItemInstructionAccessor accessor2((*frame)->DexInstructions());
  return *reinterpret_cast<const uint16_t**>(dex_pc_ptr) - accessor2.Insns();
}
...
OatQuickMethodHeader* OatQuickMethodHeader::NterpMethodHeader =
  reinterpret_cast<OatQuickMethodHeader*>(
      reinterpret_cast<uintptr_t>(interpreter::ExecuteNterpImpl) - sizeof(OatQuickMethodHeader));
