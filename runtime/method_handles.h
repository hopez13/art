...
class ShadowFrameSetter {
 public:
  ...
  REQUIRES_SHARED(Locks::mutator_lock_)
  inline void Set(uint32_t value) ALWAYS_INLINE {
    shadow_frame_->SetVReg(arg_index_++, value);
  }

  REQUIRES_SHARED(Locks::mutator_lock_)
  inline void SetReference(ObjPtr<mirror::Object> value) ALWAYS_INLINE {
    shadow_frame_->SetVRegReference(arg_index_++, value.Ptr());
  }

  REQUIRES_SHARED(Locks::mutator_lock_)
  inline void SetLong(int64_t value) ALWAYS_INLINE {
