...
static JdwpSocketState* SocketStartup(JdwpState* state, uint16_t port, bool probe) {
  ...
  if (netState->listenSock < 0) {
    PLOG_S(probe ? ::android::base::ERROR : ::android::base::FATAL) << "Socket create failed";
    goto fail;
  }
  ...
  /* allow immediate re-use */
  {
    ...
    if (setsockopt(netState->listenSock, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) < 0) {
      PLOG_S(probe ? ::android::base::ERROR : ::android::base::FATAL)
  ...
  if (listen(netState->listenSock, 5) != 0) {
    PLOG_S(probe ? ::android::base::ERROR : ::android::base::FATAL) << "Listen failed";
    goto fail;
  }
