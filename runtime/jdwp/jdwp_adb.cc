 ...
 * socket. But in case adb is buggy and dosesn't reponse to any connection, the
 * connect will block. For send, actually it won't never block since we only send
 * several bytes and the kernel buffer is big enough to accept it. 10 seconds
 * should be far enough.
 */
#define kControlSockSendTimeout 10
