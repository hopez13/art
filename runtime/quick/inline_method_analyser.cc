...
bool RecordConstructorIPut(ArtMethod* method,
  ...
  ArtField* field = class_linker->GetResolvedField(field_index, dex_cache, /* is_static */ false);
  ...
  for (size_t old_pos = 0; old_pos != arraysize(iputs); ++old_pos) {
    ...
    ArtField* f = class_linker->GetResolvedField(iputs[old_pos].field_index,
                                                 dex_cache,
                                                 /* is_static */ false);
    DCHECK(f != nullptr);
    if (f == field) {
...
bool InlineMethodAnalyser::ComputeSpecialAccessorInfo(ArtMethod* method,
  ...
  ObjPtr<mirror::DexCache> dex_cache = method->GetDexCache();
  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
  ArtField* field = class_linker->GetResolvedField(field_idx, dex_cache, /* is_static */ false);
