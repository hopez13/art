...
struct SubtypeCheckInfo {
  ...
  using SlicedType = SubtypeCheckBits;
  ...
  static SubtypeCheckInfo Infuse(SlicedType compressed_value, size_t depth) {
    SubtypeCheckInfo io;
    io.depth_ = depth;
  ...
  // Create a new root SubtypeCheckInfo with a blank PathToRoot.
  // Returns: A new root with an Assigned state.
  static SubtypeCheckInfo NewForRoot() {
    SubtypeCheckInfo io{};
    io.depth_ = 0u;
  ...
  SubtypeCheckInfo NewForChild(bool assign_next) {
    ...
    auto OverwriteNextValueFromParent = [&](SubtypeCheckInfo& child, BitStringChar value) {
      if (HasNext()) {
    ...
    OverwriteNextValueFromParent(child, BitStringChar{});
  ...
  State GetState() const {
    ...
    DCHECK(!HasNext() || GetNext() != BitStringChar{})
  ...
  SubtypeCheckInfo CopyCleared() const {
 ...
 private:
  ...
  SubtypeCheckInfo(BitString path_to_root, BitStringChar next, bool overflow, size_t depth) {
    ...
    if (HasNext()) {
      if (kIsDebugBuild) {
        did_overlap = (GetNext() != BitStringChar{});
      }
