...
TEST(X86InstructionSetFeaturesTest, X86FeaturesFromDefaultVariant) {
  const bool is_host = kRuntimeISA == InstructionSet::kX86;
  ...
  EXPECT_EQ(x86_features->GetFeatureString(),
            is_host ? X86InstructionSetFeatures::FromCppDefines()->GetFeatureString()
                    : "-ssse3,-sse4.1,-sse4.2,-avx,-avx2,-popcnt");
  EXPECT_EQ(x86_features->AsBitmap(),
            is_host ? X86InstructionSetFeatures::FromCppDefines()->AsBitmap() : 0);
}
