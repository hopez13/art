...
#define SKIP_BYTE() FETCH_OR_SKIP_BYTE(do {} while(0))
...
bool NullPointerHandler::Action(int, siginfo_t* sig, void* context) {
  ...
  // exception generator.  The return address must be the address of the next
  // instruction.  The return address is on the stack at the top address of
  // the current frame.
