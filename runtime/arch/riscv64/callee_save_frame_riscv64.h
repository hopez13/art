...
// it cannot contain object reference, and S1(X9/TR) is excluded as the ART thread register).
...
// All callee-saved registers include S1(X9/TR), but not stack pointer SP(X2): although it is
// callee-saved by calling convention, it is restored by the code logic and not from a stack frame.
...
static constexpr uint32_t kRiscv64CalleeSaveArgSpills =
    ...
    (1 << art::riscv64::A7);
// All registers except immutable Zero and unallocatable TP (thread pointer), GP (global pointer).
static constexpr uint32_t kRiscv64CalleeSaveEverythingSpills =
    (1 << art::riscv64::SP) | (1 << art::riscv64::TR) | (1 << art::riscv64::T0) |
    ...
    (1 << art::riscv64::A6) | (1 << art::riscv64::A7);
