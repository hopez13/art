...
void Riscv64Context::FillCalleeSaves(uint8_t* frame, const QuickMethodFrameInfo& frame_info) {
  ...
  // Core registers come first, from the highest down to the lowest, with the exception if RA/X1.
  int spill_pos = 1;
  for (uint32_t core_reg : HighToLowBits(frame_info.CoreSpillMask() & ~(1lu << RA))) {
...
void Riscv64Context::SmashCallerSaves() {
  ...
  gprs_[Zero] = const_cast<uint64_t*>(&gZero);
  gprs_[T0] = nullptr;
  gprs_[T1] = nullptr;
  gprs_[T2] = nullptr;
  gprs_[A0] = nullptr;
  ...
  // f0 - f7, f11 - f17, f28 - f31 are caller-saved
  fprs_[F0] = nullptr;
  fprs_[F1] = nullptr;
  fprs_[F2] = nullptr;
  fprs_[F3] = nullptr;
