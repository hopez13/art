...
TEST(X86_64InstructionSetFeaturesTest, X86Features) {
  const bool is_host = kRuntimeISA == InstructionSet::kX86_64;
  ...
  EXPECT_EQ(x86_64_features->GetFeatureString(),
            is_host ? X86_64InstructionSetFeatures::FromCppDefines()->GetFeatureString()
                    : "-ssse3,-sse4.1,-sse4.2,-avx,-avx2,-popcnt");
  EXPECT_EQ(x86_64_features->AsBitmap(),
            is_host ? X86_64InstructionSetFeatures::FromCppDefines()->AsBitmap() : 0);
}
