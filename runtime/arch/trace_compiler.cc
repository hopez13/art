...
extern "C" const void* art_quick_ttrace_entrypoint_template();
extern "C" const void* art_quick_ttrace_entrypoint_template_end();
#ifdef __x86_64__
extern "C" const void* art_quick_ttrace_x86_64_start_movabs();
#endif
...
const void* TraceCompiler::GetTrampolineTarget(const void* trampoline) {
  MutexLock mu(art::Thread::Current(), lock_);
  auto it = trampoline_to_target_map_.find(trampoline);
  if (it != trampoline_to_target_map_.end()) {
...
const void* TraceCompiler::GetTrampolineTo(uintptr_t target) {
  MutexLock mu(art::Thread::Current(), lock_);
  ...
  memcpy(data.data(),
         reinterpret_cast<const void*>(&art_quick_ttrace_entrypoint_template),
         entrypoint_size);
  ...
  trampoline_to_target_map_.Put(&data.front(), target);
...
void TraceCompiler::WriteTraceTarget(ArrayRef<uint8_t> trampoline, uintptr_t target) const {
  ...
  uintptr_t runtime_cur = htole64(reinterpret_cast<uintptr_t>(Runtime::Current()));
  memcpy(trampoline.SubArray(2).data(), &runtime_cur, sizeof(runtime_cur));
  ...
  memcpy(
      trampoline.SubArray(
          2 + (reinterpret_cast<uintptr_t>(art_quick_ttrace_x86_64_start_movabs) -
               reinterpret_cast<uintptr_t>(art_quick_ttrace_entrypoint_template))).data(),
      &target,
      sizeof(target));
#else
