...
static void VisitMembers(const DexFile& dex_file,
                         ...
                         std::function<void(const ClassAccessor::Field&)> fn_visit) {
...
static void VisitMembers(const DexFile& dex_file,
                         const DexFile::ClassDef& class_def,
                         std::function<void(const ClassAccessor::Method&)> fn_visit) {
  ClassAccessor accessor(dex_file, class_def, /* parse_hiddenapi_class_data= */ true);
...
uint32_t GetDexFlags(T* member) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  typedef typename std::conditional<std::is_same<T, ArtField>::value,
      ClassAccessor::Field, ClassAccessor::Method>::type AccessorType;
  ...
  if (LIKELY(original_dex == nullptr)) {
    ...
    // indices are lost. Iterate over all members of the class def and find and find
    // the one corresponding to this `member` by name & type string comparison.
