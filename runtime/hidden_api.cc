...
void MemberSignature::LogAccessToEventLog(AccessMethod access_method, bool access_denied) {
  PaletteHiddenApiAccessMethod palette_access_method;
  switch (access_method) {
    case AccessMethod::kJNI:
      palette_access_method = PaletteHiddenApiAccessMethod::kJNI;
      break;
    case AccessMethod::kLinking:
    ...
    case AccessMethod::kNone:
      ...
      return;
    case AccessMethod::kReflection:
      palette_access_method = PaletteHiddenApiAccessMethod::kReflection;
      break;
  }
  ...
  Dump(signature_oss);
  const std::string signature = signature_oss.str();

  PaletteLogHiddenApiAccess(package_name.c_str(),
                            signature.c_str(),
                            palette_access_method,
                            access_denied);
}
