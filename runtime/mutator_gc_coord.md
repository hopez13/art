...
**Suspending the GC**
Under unusual conditions, the GC can run on any thread. Suspending a thread, and
actually causing it to block, while the suspended thread is running the GC, is
very deadlock prone. For example, if we allocate with the other thread
suspended, and happen to need a GC to complete the allocation, we deadlock.
This we ensure that the GC, together with anything else that can block GCs
cannot be blocked for thread suspension while it's doing this critical work.
This is accomplished by ensuring that it is always in a suspended state, and
thus does not notice suspend requuests until it completes.
