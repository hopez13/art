...
The suspend calls invoke `ModifySuspendCount()` to increment the thread suspend
count for each thread. That adds a "suspend barrier" (atomic counter) to the
per-thread list of such counters to decrement. It normally sets the
...
then calls `PassActiveSuspendBarriers()` to check for the
...
barrier data structures is done under the suspend\_count\_lock_, we always
...
active suspend barriers for each target thread, relyin on the fact that each
...
For `SuspendAll()`, we enforce a requirement that at most one
`SuspendAll()` request is running at one time. In addition, in all cases,
we refuse to initiate a suspend request from a registered thread that is also
being asked to suspend, instead waiting for that condition to change.
These checks are serialized, since we always acquire suspend\_count\_lock\_.
