...
CallStackTracker* CallStackTracker::instance_ = nullptr;
...
void CallStackTracker::Record(std::string&& name, size_t skip_frames) {
  ...
  if (it == call_stacks_.end()) {
    ...
    DumpNativeStack(oss, self->GetTid(), nullptr, "", nullptr, nullptr, skip_frames);
...
void CallStackTracker::Reset(CallStackTracker* instance) {
  delete instance_;
  instance_ = instance;
}

bool CallStackTracker::TrackDexLocation(const std::string& dex_location) const {
  for (const std::string& location : tracked_dex_files_) {
    if (android::base::StartsWith(dex_location, location.c_str())) {
      return true;
    }
  }
  return false;
