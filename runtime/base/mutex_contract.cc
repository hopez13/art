...
void MutexContract::CheckSafeToWait(const Thread* self,
  ...
  if (!RuntimeIsAborting()) {  // Avoid recursive aborts.
...
void MutexContract::SetCurrentMutexChecked(Thread* self,
                                           LockLevel level,
                                           BaseMutex* mutex) {
...
void MutexContract::SetCurrentMutexToNullChecked(Thread* self,
                                                 LockLevel level,
                                                 const BaseMutex* mutex) {
  if (UNLIKELY(self == nullptr)) {
    CheckUnattachedThread(level);
  ...
  if (level != kMonitorLock) {
    if (kDebugLocking && !RuntimeIsAborting()) {  // Avoid recursive aborts.
...
void MutexContract::FatalUnlockFailed(int32_t cur_state,
                                      BaseMutex* mutex,
                                      const char* file,
                                      uint32_t line) {
  ...
  if (mutex != Locks::logging_lock_) {
    ...
    LogHelper::LogLineLowStack(file,
                               line,
...
bool MutexContract::RuntimeIsAborting() {
  return gAborting != 0;
}
