...
struct BitStringChar {
  ...
  BitStringChar() { data_ = 0; bitlength_ = 0; }
  ...
  // Add a BitStringChar with an integer. The resulting BitStringChar's data must still fit within this
  // BitStringChar's bit length.
...
inline std::ostream& operator<<(std::ostream& os, const BitStringChar& bc) {
  os << "BitStringChar<" << bc.GetBitLength() << ">(" << static_cast<BitStringChar::StorageType>(bc) << ")";
  return os;
}
