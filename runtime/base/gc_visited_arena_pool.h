...
class TrackedArena final : public Arena {
 public:
  ...
  void SetupForDefferedDeletion(TrackedArena* next_arena) {
...
class GcVisitedArenaPool final : public ArenaPool {
 public:
  ...
  template <typename PageVisitor>
  void VisitRoots(PageVisitor& visitor) REQUIRES_SHARED(Locks::mutator_lock_) REQUIRES(!lock_) {
    ReaderMutexLock rmu(Thread::Current(), lock_);
  ...
  void ForEachAllocatedArena(Callback cb) REQUIRES_SHARED(Locks::mutator_lock_) REQUIRES(!lock_) {
    ReaderMutexLock rmu(Thread::Current(), lock_);
    ForEachAllocatedArenaLocked(cb);
  }

  template <typename Callback>
  void ForEachAllocatedArenaLocked(Callback cb) REQUIRES_SHARED(Locks::mutator_lock_, lock_) {
