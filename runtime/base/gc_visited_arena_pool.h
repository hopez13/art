...
class GcVisitedArenaPool final : public ArenaPool {
 ...
 private:
  ...
  class SortByArenaAddr {
   public:
    bool operator()(const std::unique_ptr<TrackedArena>& a,
                    const std::unique_ptr<TrackedArena>& b) const {
  ...
  std::set<Chunk*, SortByChunkSize> best_fit_allocs_ GUARDED_BY(lock_);
  std::set<Chunk*, SortByChunkAddr> free_chunks_ GUARDED_BY(lock_);
  ...
  std::set<std::unique_ptr<TrackedArena>, SortByArenaAddr> allocated_arenas_ GUARDED_BY(lock_);
