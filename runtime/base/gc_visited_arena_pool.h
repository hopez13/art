...
class GcVisitedArenaPool final : public ArenaPool {
 public:
  ...
  void ForEach(Callback cb) REQUIRES_SHARED(Locks::mutator_lock_) {
    std::lock_guard<std::mutex> lock(lock_);
    DCHECK_EQ(maps_.size(), shadow_maps_.size());
    size_t shadow_iter = 0;
    for (auto& map : maps_) {
      cb(map, shadow_maps_[shadow_iter++]);
    }
  }
 ...
 private:
  ...
  // MemMaps corresponding to map_ for userfaultfd GC.
  std::vector<MemMap> shadow_maps_ GUARDED_BY(lock_);
