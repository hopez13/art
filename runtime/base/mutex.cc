  ...
  if (kDebugLocking) {
    ...
    for (int i = kLockLevelCount - 1; i >= 0; --i) {
      if (i != level_) {
        ...
        // are a threads resume_cond_ (level_ == kThreadSuspendCountLock). This just means that gc
        // or some other internal reason we are suspending this thread while this thread is trying
        // to suspend some other thread. So long as this thread is not being held suspended by other
        // SuspendReason::kForUserCode this is fine.
        if (held_mutex == Locks::user_code_suspension_lock_ && level_ == kThreadSuspendCountLock) {
          ...
          // from the line above and the ThreadSuspendCountLock since it is our level_.
