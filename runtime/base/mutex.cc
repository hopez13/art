...
class ScopedAllMutexesLock FINAL {
 public:
  explicit ScopedAllMutexesLock(const BaseMutex* mutex) : mutex_(mutex) {
    const BaseMutex* expected_value = nullptr;
    for (uint32_t i = 0;
         !gAllMutexData->all_mutexes_guard.compare_exchange_weak(expected_value,
                                                                 mutex,
  ...
  ~ScopedAllMutexesLock() {
    ...
    gAllMutexData->all_mutexes_guard.store(0, std::memory_order_release);
  }
...
class Locks::ScopedExpectedMutexesOnWeakRefAccessLock FINAL {
 public:
  explicit ScopedExpectedMutexesOnWeakRefAccessLock(const BaseMutex* mutex) : mutex_(mutex) {
    const BaseMutex* expected_value = nullptr;
    for (uint32_t i = 0;
         !Locks::expected_mutexes_on_weak_ref_access_guard_.compare_exchange_weak(
             expected_value,
             mutex,
             std::memory_order_acquire);
         ++i) {
  ...
  ~ScopedExpectedMutexesOnWeakRefAccessLock() {
    DCHECK_EQ(Locks::expected_mutexes_on_weak_ref_access_guard_.load(std::memory_order_relaxed),
              mutex_);
    Locks::expected_mutexes_on_weak_ref_access_guard_.store(0, std::memory_order_release);
  }
...
void BaseMutex::RecordContention(uint64_t blocked_tid,
  ...
  if (kLogLockContentions) {
    ...
    if (log[slot].blocked_tid == blocked_tid &&
      ...
      } while (!data->cur_content_log_entry.compare_exchange_weak(slot,
                                                                  new_slot,
  ...
  if (!recursive_ || !IsExclusiveHeld(self)) {
    ...
    do {
      ...
      if (LIKELY(cur_state == 0)) {
        ...
        done = state_.compare_exchange_weak(cur_state, 1, std::memory_order_acquire);
      } else {
  ...
  if (!recursive_ || recursion_count_ == 0) {
    ...
    do {
      ...
      if (LIKELY(cur_state == 1)) {
        ...
        done = state_.compare_exchange_weak(cur_state, 0, std::memory_order_seq_cst);
...
void ConditionVariable::WaitHoldingLocks(Thread* self) {
  ...
  guard_.exclusive_owner_.store(0, std::memory_order_relaxed);
...
bool Locks::IsSafeToCallAbortRacy() {
  Locks::ClientCallback* safe_to_call_abort_cb = safe_to_call_abort_callback.load(std::memory_order_acquire);
