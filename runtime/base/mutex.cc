...
Mutex* Locks::bitstring_lock_ = nullptr;
...
void Locks::Init() {
  if (logging_lock_ != nullptr) {
    ...
    DCHECK(bitstring_lock_ != nullptr);
    ...
    UPDATE_CURRENT_LOCK_LEVEL(kBitstringLock);
    DCHECK(bitstring_lock_ == nullptr);
    bitstring_lock_ = new Mutex("Bitstring lock", current_lock_level);
