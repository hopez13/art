...
static Atomic<Locks::ClientCallback*> gSafeToCallAbortCallback(nullptr);
...
Mutex::~Mutex() {
  ...
  if (state_.LoadRelaxed() != 0) {
    LOG(safe_to_call_abort
            ? ::android::base::FATAL
            : ::android::base::WARNING) << "destroying mutex with owner: " << exclusive_owner_;
  } else {
    if (exclusive_owner_ != 0) {
      LOG(safe_to_call_abort
              ? ::android::base::FATAL
              : ::android::base::WARNING) << "unexpectedly found an owner on unlocked mutex "
                                          << name_;
    }
    if (num_contenders_.LoadSequentiallyConsistent() != 0) {
      LOG(safe_to_call_abort
              ? ::android::base::FATAL
              : ::android::base::WARNING) << "unexpectedly found a contender on mutex " << name_;
    }
  }
#else
  ...
  if (rc != 0) {
    errno = rc;
    PLOG(safe_to_call_abort
             ? ::android::base::FATAL
             : ::android::base::WARNING) << "pthread_mutex_destroy failed for " << name_;
  }
#endif
}
...
ConditionVariable::~ConditionVariable() {
  ...
  if (rc != 0) {
    errno = rc;
    bool is_safe_to_call_abort = IsSafeToCallAbortSafe();
    PLOG(is_safe_to_call_abort
             ? ::android::base::FATAL
             : ::android::base::WARNING) << "pthread_cond_destroy failed for " << name_;
  }
#endif
}
...
void Locks::SetClientCallback(ClientCallback* safe_to_call_abort_cb) {
  gSafeToCallAbortCallback.StoreRelease(safe_to_call_abort_cb);
}

// Helper to allow checking shutdown while ignoring locking requirements.
bool Locks::IsSafeToCallAbortRacy() {
  Locks::ClientCallback* safe_to_call_abort_cb = gSafeToCallAbortCallback.LoadAcquire();
