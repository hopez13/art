...
// TODO: get rid of sleep loops and synchronize precisely
static void BackOff(uint32_t i) {
  static constexpr uint32_t kSpinMax = 100;
  static constexpr uint32_t kYieldMax = kSpinMax + 10;
  static constexpr uint32_t kMaxWaitNs = MsToNs(10);
  if (i <= kSpinMax) {
    CpuRelax();
  } else if (i <= kYieldMax) {
    sched_yield();
  } else {
    NanoSleep(std::min(1000ull * (i - kYieldMax), kMaxWaitNs));
  }
}
