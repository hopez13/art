...
static inline bool WaitBrieflyFor(AtomicInteger* testLoc, Pred pred) {
  ...
  for (uint32_t i = 1; i <= kMaxIters; ++i) {
    ...
    if (Runtime::Current()->MaybeShuttingDown()) {
      // Once we're shutting down, reach a quiescent state as soon as possible to
      // avoid touching data that may be deallocted by the shutdown process. It
      // currently relies on a timeout.
      return false;
    }
  }
  return false;
