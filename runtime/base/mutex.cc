...
void BaseMutex::BackOff(uint32_t usec_increment) {
  ...
  if (usec_increment <= kSpinMax) {
    ...
    const uint32_t spin_count = 10 * usec_increment;
  ...
  } else if (usec_increment <= kYieldMax) {
    sched_yield();
  } else {
    NanoSleep(1000ull * (usec_increment - kYieldMax));
  }
}
  ...
  if (!recursive_ || recursion_count_ == 0) {
    ...
    do {
      ...
      if (LIKELY(cur_state == 1)) {
        ...
        MutexContract::FatalUnlockFailed(cur_state, this, __FILE__, __LINE__);
      }
    } while (!done);
#else
    exclusive_owner_.StoreRelaxed(0);
