...
static constexpr uint64_t kIntervalTime = 50;
    ...
    do {
      ...
      if (LIKELY((cur_state & kHeldMask) == 0) /* lock not held */) {
        ...
        if (!WaitBrieflyFor(&state_and_contenders_, self,
          ...
          uint64_t wait_start_ms = is_check_timeout_ ? MilliTime() : 0;
          uint64_t try_times = 0;
          do {
            timespec timeout_ts;
            timeout_ts.tv_sec = 0;
            timeout_ts.tv_nsec = monitor_timeout_;
            if (futex(state_and_contenders_.Address(), FUTEX_WAIT_PRIVATE, cur_state,
                      is_check_timeout_ ? &timeout_ts : nullptr , nullptr, 0) != 0) {
...
void Mutex::DumpStack(Thread* self, uint64_t wait_start_ms, uint64_t try_times) {
  ...
  if (owner != nullptr && Runtime::Current()->GetThreadList()->Contains(owner)) {
    ...
    struct CollectStackTrace : public Closure {
      void Run(art::Thread* thread) override
        ...
        thread->SetLastDumpStackTime(MilliTime());
    ...
    LOG(WARNING) << "ContentStack with tid " << owner_tid << ", wait time: " << wait_ms
...
bool Mutex::IsDumpFrequent(Thread* thread, uint64_t try_times) {
  uint64_t interval = MilliTime() - thread->GetLastDumpStackTime();
  if (interval < kIntervalTime * try_times) {
    return true;
  } else {
    return false;
  }
