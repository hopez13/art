...
uint8_t* GcVisitedArenaPool::AddPreZygoteForkMap(size_t size) {
  ...
  std::string name = "Pre-zygote";
  name += name_;
  std::string err_msg;
  maps_.emplace_back(
      MemMap::MapAnonymous(name.c_str(), size, PROT_READ | PROT_WRITE, low_4gb_, &err_msg));
  MemMap& map = maps_.back();
  if (!map.IsValid()) {
    LOG(FATAL) << "Failed to allocate " << name << ": " << err_msg;
    UNREACHABLE();
  }
  return map.Begin();
}
...
Arena* GcVisitedArenaPool::AllocArena(size_t size) {
  ...
  if (pre_zygote_fork_) {
    ...
    auto emplace_result = allocated_arenas_.emplace(addr, size, /*pre_zygote_mode=*/true);
