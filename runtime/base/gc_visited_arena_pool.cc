...
void GcVisitedArenaPool::FreeArenaChain(Arena* first) {
...

  {
    ...
    while (first != nullptr) {
      ...
      allocated_arenas_.erase(iter);
      if (defer_arena_freeing_) {
        temp->SetupForDefferedDeletion(unused_arenas_);
        unused_arenas_ = temp;
      } else {
        delete temp;
      }
