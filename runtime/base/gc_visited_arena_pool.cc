...
uint8_t* GcVisitedArenaPool::AddMap(size_t min_size) {
...
#endif
  size_t alignment = size < kPUDSize ? kPMDSize : kPUDSize;
  ...
  if (gUseUserfaultfd) {
    ...
    if (!runtime->IsZygote() && mark_compact->IsUffdMinorFaultSupported()) {
      void* mmap_ret = mmap(map.Begin(),
                            map.Size(),
                            PROT_READ | PROT_WRITE,
                            MAP_ANONYMOUS | MAP_SHARED | MAP_FIXED,
                            /*fd=*/-1,
                            /*offset=*/0);
      CHECK_EQ(mmap_ret, map.Begin()) << "mmap failed: " << strerror(errno);
      mapped_shared = true;
    }
