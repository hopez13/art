...
TEST_F(MessageQueueTest, SwitchReceiveTest) {
  ...
  while (!(empty_received && int_received && other_int_received && two_int_received &&
    ...
    queue.SwitchReceive(
        [&]([[maybe_unused]] EmptyMessage message) {
...
TEST_F(MessageQueueTest, SwitchReceiveReturn) {
  ...
  ASSERT_TRUE(queue.SwitchReceive<bool>([&]([[maybe_unused]] EmptyMessage message) { return true; },
                                        [&]([[maybe_unused]] auto message) { return false; }));

  queue.SendMessage(IntMessage{42});

  ASSERT_FALSE(
      queue.SwitchReceive<bool>([&]([[maybe_unused]] EmptyMessage message) { return true; },
                                [&]([[maybe_unused]] auto message) { return false; }));
}
