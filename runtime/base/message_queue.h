...
// MessageQueue is a multiple producer, multiple consumer (MPMC) queue that can be specialized to
// send messages between threads. The queue is parameterized by a set of types that serve as the
// message types.
//
// Example:
//
...
//     }
//
// The message queue also supports a special timeout message. This is schedule to be sent by the
