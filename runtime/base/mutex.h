...
class Locks {
 public:
  ...
  // Guard the update of bitstring stored in status of each class.
  // This lock is used in Class::EnsuredInitializedInstanceOf() which is the entry point of
  // all other bitstring related methods.
  // The initialization of bitstring may involve the update in the super class chain, thus
  // we use a lock to protect the update.
  // In Class::IsSubClass, the lock is not required since it does not update the bitstring.
  // TODO: Use atomic instead of lock.
