...
class MutexContract {
 public:
  ...
  static void RegisterAsLocked(Thread* self,
                               LockLevel level,
                               BaseMutex* mutex);

  // Marks that 'self's current mutex at 'level' is 'mutex' and unlocked and checks consistency.
  static void RegisterAsUnlocked(Thread* self,
                                 LockLevel level,
                                 const BaseMutex* mutex);

  // Report that attempting to lock 'mutex' failed because the state was 'cur_state'.
  static void LogUnlockFailed(int32_t cur_state, BaseMutex* mutex);

  // If the current art::Runtime is already shut down, park 'self'.
