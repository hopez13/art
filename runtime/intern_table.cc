...
ObjPtr<mirror::String> InternTable::InternStrong(uint32_t utf16_length, const char* utf8_data) {
  ...
  DCHECK_EQ(utf16_length, CountModifiedUtf8Chars(utf8_data));
  // Note: Initialize `is_ascii` before taking the lock to allow C++ compiler to use common
  // expression elimination for the same expression in inlined `Utf8String::Hash()`.
  ...
  Thread* self = Thread::Current();
  {
    ...
    MutexLock mu(self, *Locks::intern_table_lock_);
    ObjPtr<mirror::String> s = strong_interns_.Find(Utf8String(utf16_length, utf8_data, hash));
    if (s != nullptr) {
      return s;
    }
  }
  ...
  ObjPtr<mirror::String> s =
      mirror::String::AllocFromModifiedUtf8(self, utf16_length, utf8_data, utf8_length);
