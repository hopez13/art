...
class OatFileAssistant {
 ...
 private:
  ...
  // The AOT file of an app when the APK of the app is in /data.
  OatFileInfo odex_;
  // The AOT file of an app when the APK of the app is on a read-only partition
  // (for example /system).
  OatFileInfo oat_;

  // The vdex-only file next to `odex_` when `odex_' cannot be used.
  OatFileInfo vdex_for_odex_;
  // The vdex-only file next to 'oat_` when `oat_' cannot be used.
