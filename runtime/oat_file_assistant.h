...
class OatFileAssistant {
 public:
  enum DexOptNeeded {
    ...
    // use of existing optimized dex code.
    // Matches Java: dalvik.system.DexFile.DEX2OAT_FROM_SCRATCH = 1
    kDex2OatFromScratch = 1,

    // dex2oat should be run to update the apk/jar using the existing
    // optimized dex code as input. The existing optimized dex is up to date
    // with respect to the apk/jar, but is out of date with respect to the
    // boot image.
    ...
    // dex2oat should be run to update the apk/jar using the existing
    // optimized dex code as input. The existing optimized dex is up to date
    // with respect to the apk/jar and boot image, but the compiled code is
    // out of date with respect to the compiler filter.
    // Matches Java: dalvik.system.DexFile.DEX2OAT_FOR_FILTER
    kDex2OatForFilter = 3,

    // dex2oat should be run to update the apk/jar using the existing
    // optimized dex code as input. The existing optimized dex code is up to
    // date with respect to the apk/jar and boot image. patchoat cannot be
    // called because the existing oat file does not have the necessary patch
    // information.
