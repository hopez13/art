...
class OatFileAssistant {
 public:
  ...
  // If a strict comparision between the current compiler filter and the
  // target compiler filter, then strict should be true.
 ...
 private:
  ...
  class OatFileInfo {
   public:
    ...
    // profile_changed should be true to indicate the profile has recently
    // changed for this dex location.
    // strict should be true to indicate that strict comparison is required between
    // target compiler filter and current compiler filter.
   ...
   private:
    ...
    // location.
    // strict should be true for strict comparison between current compiler filter
    // and the target compiler filter.
    bool CompilerFilterIsOkay(CompilerFilter::Filter target, bool profile_changed, bool strict);
