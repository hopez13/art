...
class TraceWriter {
 public:
  ...
  // pending tasks are greater than the the number of waiting threads. If that isn't the case we
  ...
  // Ensures there are no threads suspended waiting for a free buffer. It signals threads waiting
  // for the free buffer and waits all the threads to respond to the signal.
  void StopTracing();
