...
class TraceWriter {
 public:
  ...
  // Flushes all per-thread buffer and also a summary entry. Depending on the output mode format
  // will differ a bit.
 ...
 private:
  // Get a 32-bit id for the method and specify if the method hasn't been seen before. We need this
  // information in streaming mode to record the information about the method.
  ...
  // Helper functions to record method information when processing the events. These are used by
...
class Trace final : public instrumentation::InstrumentationListener {
 public:
  ...
  static void PreZygoteFork();
  static void PostZygoteFork();
