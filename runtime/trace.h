...
class Trace FINAL : public instrumentation::InstrumentationListener {
 ...
 private:
  ...
  // thread.
  //
  // Reads to the buffer occur in FinishTracing() called from
  // StopTracing(). These reads are associated with I/O, such
  // as writing buf_ to a file or publishing via DDM.
