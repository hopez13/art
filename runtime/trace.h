...
class TraceWriter {
 ...
 private:
  ...
  // Buffer used when generating trace data from the raw entries. In streaming mode, the trace data
  // is flushed to file when the per-thread buffer gets full. In non-streaming mode, this data is
  // flushed at the end of tracing. The size of this buffer is specified by the user in
  // non-streaming mode.
