...
bool AotClassLinker::InitializeClass(Thread* self,
  ...
  if (!strict_mode &&
      ...
      !klass->DescriptorEquals("Ldalvik/system/TransactionAbortError;")) {
    DCHECK(runtime->IsActiveTransaction()) << klass->PrettyDescriptor();
    runtime->AbortTransactionAndThrowAbortError(self, "Can't initialize " + klass->PrettyTypeOf()
         + " because it defined in a boot image dex file.");
    return false;
  }
