...
class Heap {
 public:
  ...
  static constexpr ssize_t kDefaultNativeAllocationGcWatermark = 2 * MB;
  static constexpr ssize_t kDefaultNativeAllocationBlockingGcWatermark = 64 * MB;
  ...
  Heap(size_t initial_size,
       ...
       ssize_t native_allocation_gc_watermark,
       ssize_t native_allocation_blocking_gc_watermark);
  ...
  void RegisterNativeAllocation(JNIEnv* env, ssize_t bytes)
      REQUIRES(!*gc_complete_lock_, !*pending_task_lock_);
  void RegisterNativeFree(JNIEnv* env, ssize_t bytes)
      REQUIRES(!*gc_complete_lock_, !*pending_task_lock_);
 ...
 private:
  ...
  // allocations. Conceptually this number should never be negative,
  // because if there have been more frees than allocs since the last GC, the
  // extra frees are taken out of old_native_bytes_allocated_. In practice
  // this number may be (a little bit) negative as a result of how concurrent
  // accesses are interleaved.
  // See the REDESIGN section of go/understanding-register-native-allocation.
  Atomic<ssize_t> new_native_bytes_allocated_;
  ...
  // Conceptually this number should never be negative. In practice it may be
  // negative as a result of how concurrent accesses are interleaved, or if
  // the user has erroneously registered more native frees than allocs.
  Atomic<ssize_t> old_native_bytes_allocated_;

  // How large new_native_bytes_allocated_ can grow before we trigger a new
  // GC.
  ssize_t native_allocation_gc_watermark_;
  ...
  ssize_t native_allocation_blocking_gc_watermark_;
