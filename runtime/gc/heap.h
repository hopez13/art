...
class Heap {
 public:
  ...
  // Notify the garbage collector of a malloc allocations that might be reclaimable
  ...
  size_t GetFreeMemoryUntilGC() const {
    return UnsignedDifference(target_footprint_, GetBytesAllocated());
  }
 ...
 private:
  ...
  ALWAYS_INLINE bool ShouldConcurrentGCForNative();
  ...
  ALWAYS_INLINE size_t NativeAllocationGcWatermark() const {
    // Reuse target_footprint_ for the native allocation gc watermark.
  ...
  // The value of GetNativeBytes() at last GC.
