...
class Heap {
 ...
 private:
  ...
  // GC.
  size_t NativeAllocationGcWatermark() const ALWAYS_INLINE {
  ...
  size_t NativeAllocationBlockingGcWatermark() const ALWAYS_INLINE {
    // Historically the native allocations were bounded by growth_limit_. This
    // uses that same value, dividing growth_limit_ by 2 because worst case we
    // can end up with outstanding native allocations a little over double
    // what the blocking gc watermark is set to.
