...
class AtomicStack {
 public:
  ...
  bool AtomicBumpBack(size_t num_slots, StackReference<T>** start_address,
    ...
    } while (!back_index_.compare_exchange_weak(index, new_index, std::memory_order_relaxed));
  ...
  T* PopBack() REQUIRES_SHARED(Locks::mutator_lock_) {
    DCHECK_GT(back_index_.load(std::memory_order_relaxed), front_index_.load(std::memory_order_relaxed));
  ...
  size_t Size() const {
    DCHECK_LE(front_index_.load(std::memory_order_relaxed), back_index_.load(std::memory_order_relaxed));
    return back_index_.load(std::memory_order_relaxed) - front_index_.load(std::memory_order_relaxed);
  }
 ...
 private:
  ...
  bool AtomicPushBackInternal(T* value, size_t limit) ALWAYS_INLINE
    ...
    } while (!back_index_.compare_exchange_weak(index, index + 1, std::memory_order_relaxed));
    begin_[index].Assign(value);
    return true;
  }
