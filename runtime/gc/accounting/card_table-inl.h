...
static inline bool byte_cas(uint8_t old_value, uint8_t new_value, uint8_t* address) {
  ...
  return byte_atomic->compare_exchange_weak(old_value, new_value, std::memory_order_relaxed);
#else
  ...
  uintptr_t old_word = cur_word | (static_cast<uintptr_t>(old_value) << shift_in_bits);
  const uintptr_t new_word = cur_word | (static_cast<uintptr_t>(new_value) << shift_in_bits);
  return word_atomic->compare_exchange_weak(old_word, new_word, std::memory_order_relaxed);
#endif
}
...
inline void CardTable::ModifyCardsAtomic(uint8_t* scan_begin,
  ...
  while (word_cur < word_end) {
    while (true) {
      ...
      if (LIKELY(atomic_word->compare_exchange_weak(expected_word,
                                                    new_word,
