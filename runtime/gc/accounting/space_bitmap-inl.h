...
inline mirror::Object* FindPrecedingObject(uintptr_t visit_begin, uintptr_t visit_end) const {
  ...
  DCHECK_LE(visit_begin, HeapLimit());
  ...
  while (index_start > index_end) {
    ...
    if (word != 0) {
      ...
      const size_t shift = CLZ(word);
      return reinterpret_cast<mirror::Object*>(ptr_base + shift * kAlignment);
    }
  ...
  if (word != 0) {
    ...
    // Reached the beginning of the heap and didn't find an object.
    return nullptr;
  }
}
