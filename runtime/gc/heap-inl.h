  ...
  size_t new_num_bytes_allocated = 0;
  {
    ...
    if (IsTLABAllocator(allocator) && byte_count <= self->TlabSize()) {
      ...
      if (bytes_tl_bulk_allocated > 0) {
        ...
        // Attempt to push notification as long as size to report is larger
        // than last reported size and is bigger than partial tlab size, which
        // should be very close to average value of bytes_tl_bulk_allocated.
        while (last_reported_size < size_to_report
               && size_to_report - last_reported_size >= kPartialTlabSize) {
