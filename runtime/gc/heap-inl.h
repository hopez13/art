    ...
    if (IsTLABAllocator(allocator) && byte_count <= self->TlabSize()) {
      ...
      if (bytes_tl_bulk_allocated > 0) {
        ...
        if (TraceEnabled()) {
           ...
           * when this thread resumes, an inflated (and wrong) heap size will get reported.
           * By loading num_byes_allocated after last_reported_heap_size, we ensure that
           * in the described event our size_to_report will not be wrongly inflated.
           */
          const size_t last_reported_size =
                  last_reported_heap_size_.load(std::memory_order_acquire);
          size_t size_to_report = num_bytes_allocated_.load(std::memory_order_relaxed);
          /*
          ...
          while (curr_reported_size < size_to_report
            ...
            if (last_reported_heap_size_.compare_exchange_strong(curr_reported_size,
                                                                 size_to_report,
