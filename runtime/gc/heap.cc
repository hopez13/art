...
void Heap::DumpGcPerformanceInfo(std::ostream& os) {
  ...
  os << "Registered native bytes allocated: "
     << old_native_bytes_allocated_.load(std::memory_order_relaxed) + new_native_bytes_allocated_.load(std::memory_order_relaxed)
     << "\n";
...
static inline AllocationListener* GetAndOverwriteAllocationListener(
  ...
  do {
    old = storage->load(std::memory_order_seq_cst);
  } while (!storage->compare_exchange_strong(old, new_value, std::memory_order_seq_cst));
  return old;
}
...
Heap::~Heap() {
  ...
  if (unique_backtrace_count_.load(std::memory_order_relaxed) != 0 || seen_backtrace_count_.load(std::memory_order_relaxed) != 0) {
    LOG(INFO) << "gc stress unique=" << unique_backtrace_count_.load(std::memory_order_relaxed)
        << " total=" << seen_backtrace_count_.load(std::memory_order_relaxed) +
            unique_backtrace_count_.load(std::memory_order_relaxed);
  }
...
void Heap::VerifyObjectBody(ObjPtr<mirror::Object> obj) {
  ...
  if (UNLIKELY(static_cast<size_t>(num_bytes_allocated_.load(std::memory_order_relaxed)) < 10 * KB)) {
    return;
  }
...
void Heap::RecordFree(uint64_t freed_objects, int64_t freed_bytes) {
  ...
  DCHECK_LE(freed_bytes, static_cast<int64_t>(num_bytes_allocated_.load(std::memory_order_relaxed)));
  // Note: This relies on 2s complement for handling negative freed_bytes.
  num_bytes_allocated_.fetch_sub(static_cast<ssize_t>(freed_bytes), std::memory_order_seq_cst);
...
void Heap::RecordFreeRevoke() {
  ...
  size_t bytes_freed = num_bytes_freed_revoke_.load(std::memory_order_seq_cst);
  CHECK_GE(num_bytes_freed_revoke_.fetch_sub(bytes_freed, std::memory_order_seq_cst),
           bytes_freed) << "num_bytes_freed_revoke_ underflow";
  CHECK_GE(num_bytes_allocated_.fetch_sub(bytes_freed, std::memory_order_seq_cst),
      ...
      case kAllocatorTypeDlMalloc: {
        if (use_homogeneous_space_compaction_for_oom_ &&
          ...
          VLOG(heap) << "Ran heap homogeneous space compaction, "
                    ...
                    << count_requested_homogeneous_space_compaction_.load(std::memory_order_seq_cst)
                    << " performed defragmentation "
                    << count_performed_homogeneous_space_compaction_.load(std::memory_order_seq_cst)
                    << " ignored homogeneous space compaction "
                    << count_ignored_homogeneous_space_compaction_.load(std::memory_order_seq_cst)
                    << " delayed count = "
                    << count_delayed_oom_.load(std::memory_order_seq_cst);
        }
        break;
      }
...
void Heap::TransitionCollector(CollectorType collector_type) {
  ...
  uint64_t start_time = NanoTime();
  uint32_t before_allocated = num_bytes_allocated_.load(std::memory_order_seq_cst);
  Runtime* const runtime = Runtime::Current();
  Thread* const self = Thread::Current();
  ScopedThreadStateChange tsc(self, kWaitingPerformingGc);
  // TODO: Clang prebuilt for r316199 produces bogus thread safety analysis warning for holding both
...
void Heap::RequestConcurrentGC(Thread* self, GcCause cause, bool force_full) {
  ...
  if (CanAddHeapTask(self) &&
      concurrent_gc_pending_.compare_exchange_strong(expected_value,
                                                     true,
...
void Heap::RevokeThreadLocalBuffers(Thread* thread) {
  if (rosalloc_space_ != nullptr) {
    ...
    if (freed_bytes_revoke > 0U) {
      ...
      CHECK_GE(num_bytes_allocated_.load(std::memory_order_relaxed), num_bytes_freed_revoke_.load(std::memory_order_relaxed));
    }
  }
  if (bump_pointer_space_ != nullptr) {
...
void Heap::RegisterNativeFree(JNIEnv*, size_t bytes) {
  ...
  } while (!new_native_bytes_allocated_.compare_exchange_weak(allocated,
