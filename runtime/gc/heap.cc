...
class Heap::TriggerPostForkCCGcTask : public HeapTask {
 public:
  ...
  virtual void Run(Thread* self) OVERRIDE {
    gc::Heap* heap = Runtime::Current()->GetHeap();
    // Trigger a GC, if not already done.
...
void Heap::PostForkChildAction(Thread* self) {
  // Temporarily increase max_allowed_footprint and concurrent_start_bytes to
  // max values to avoid GC during app launch. This speeds up app launch. b/2308740
  ...
  concurrent_start_bytes_ =
       std::max(max_allowed_footprint_ / 2, bytes_allocated);

  GetTaskProcessor()->AddTask(self,
                              new TriggerPostForkCCGcTask(NanoTime() +
                                                          MsToNs(kPostForkMaxHeapDurationMS)));
}
