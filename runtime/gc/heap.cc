...
mirror::Object* Heap::AllocateInternalWithGc(Thread* self,
  ...
  // much space. Otherwise throw OOME.
  constexpr double kMinFreeHeapAfterGcForAlloc = 0.05;
...
#define PERFORM_SUSPENDING_OPERATION(op)                                          \
  ...
  // We perform one GC as per the next_gc_type_ (chosen in GrowForUtiliztion),
  // if it's not already tried. If that doesn't succeed then go for the most
  // exhaustive opton. Perform a full-heap collection including clearing
  ...
  if (last_gc < tried_type) {
    ...
    if (gc_ran) {
      ...
      double curr_free_heap =
          static_cast<double>(target_footprint - curr_bytes_allocated) / target_footprint;
      if (curr_free_heap >= kMinFreeHeapAfterGcForAlloc) {
  ...
  size_t curr_bytes_allocated = GetBytesAllocated();
  size_t target_footprint = target_footprint_.load(std::memory_order_relaxed);
  double curr_free_heap = static_cast<double>(target_footprint - curr_bytes_allocated) / target_footprint;
