...
void Heap::ThrowOutOfMemoryError(Thread* self, size_t byte_count, AllocatorType allocator_type) {
  ...
  // native stack is overflowed. If the simulated stack overflows, it is still possible that the
  // native stack has room to create a new exception.
  if (self->IsHandlingStackOverflow<StackType::kNative>()) {
    self->SetException(
...
collector::GcType Heap::CollectGarbageInternal(collector::GcType gc_type,
  ...
  SelfDeletingTask* clear;  // Unconditionally set below.
  {
    ...
    if (self->IsHandlingStackOverflow<StackType::kNative>()) {
      ...
      // space to run the GC. Note: we only care if the native stack is overflowed. If the
...
static bool CanAddHeapTask(Thread* self) REQUIRES(!Locks::runtime_shutdown_lock_) {
  Runtime* runtime = Runtime::Current();
  // We only care if the native stack is overflowed. If the simulated stack overflows, it is still
  ...
  return runtime != nullptr && runtime->IsFinishedStarting() && !runtime->IsShuttingDown(self) &&
      !self->IsHandlingStackOverflow<StackType::kNative>();
}
