...
static constexpr size_t kHugeNativeAllocs = 100*1024*1024;

// Return the ratio of the weighted native + java allocated bytes to its target value.
// A return value > 1.0 means we should collect.
  ...
  if (UNLIKELY(gc_urgency >= 1.0)) {
    if (IsGcConcurrent()) {
      ...
      if (gc_urgency > kStopForNativeFactor
          && current_native_bytes - old_native_bytes_allocated_.load(std::memory_order_relaxed)
             > kHugeNativeAllocs) {
