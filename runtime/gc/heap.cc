...
Heap::Heap(size_t initial_size,
  ...
  */
  ...
  if (foreground_collector_type_ == kCollectorTypeCC) {
    ...
    region_space_ = space::RegionSpace::Create(kRegionSpaceName,
        std::move(region_space_mem_map), use_generational_cc_);
  ...
  if (kMovingCollector) {
    ...
    if (MayUseCollector(kCollectorTypeCC)) {
      collector::ConcurrentCopying::use_generational_cc_ = use_generational_cc_;
...
void Heap::GrowForUtilization(collector::GarbageCollector* collector_ran,
  ...
  if (gc_type != collector::kGcTypeSticky) {
    ...
    float sticky_gc_throughput_adjustment = use_generational_cc_ ? 0.5 : 1.0;
