...
void EnableHeapSamplerCallback(void* enable_ptr,
  ...
  // will silently fail. Heap is not fully constructed yet in some cases.
...
void DisableHeapSamplerCallback(void* disable_ptr,
                                const AHeapProfileDisableCallbackInfo* disable_info_ptr ATTRIBUTE_UNUSED) {
...
void Heap::InitPerfettoJavaHeapProf() {
  ...
  // Use a Perfetto heap name = "JavaHeapProfiler".
  AHeapInfo* info = AHeapInfo_create("JavaHeapProfiler");
  // Set the Enable Callback, there is no callback data ("nullptr").
  AHeapInfo_setEnabledCallback(info, &EnableHeapSamplerCallback, this);
  // Set the Disable Callback.
  AHeapInfo_setDisabledCallback(info, &DisableHeapSamplerCallback, this);
  prof_heap_sampler.SetSessionInfo(info);
  ...
  prof_heap_sampler.EnableHeapSampler();
  #endif
  SetPerfettoJavaHeapProfHeapID(heap_id);
  prof_heap_sampler.SetHeapID(heap_id);
