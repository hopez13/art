...
void Heap::RegisterNativeAllocation(JNIEnv* env, size_t bytes) {
  ...
  // native_allocation_gc_watermark_. If GC and reference processing can't keep
  // up and we find that the number of recent native bytes allocated is
  // as high as native_allocation_blocking_gc_watermark_, we block on GC
  ...
  if (new_value > native_allocation_blocking_gc_watermark_) {
    // Wait for GC to finish, because we seem to be falling way behind our target.
    WaitForGcToComplete(kGcCauseForNativeAlloc, ThreadForEnv(env));
    RunFinalization(env, kNativeAllocationFinalizeTimeout);
    CHECK(!env->ExceptionCheck());
    new_value = recent_native_bytes_allocated_.LoadRelaxed();
  }
