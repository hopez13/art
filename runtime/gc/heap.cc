...
space::ContinuousSpace* Heap::FindContinuousSpaceFromAddress(const void* addr) const {
  for (const auto& space : continuous_spaces_) {
...
bool Heap::IsNonDiscontinuousSpaceHeapAddress(const void* addr) const {
  return FindContinuousSpaceFromAddress(addr) != nullptr;
}
...
class InstanceCollector {
 public:
  InstanceCollector(ObjPtr<mirror::Class> c,
                    int32_t max_count,
 ...
 private:
  ObjPtr<mirror::Class> const class_;
  const uint32_t max_count_;
  std::vector<ObjPtr<mirror::Object>>& instances_;
  DISALLOW_COPY_AND_ASSIGN(InstanceCollector);
};

void Heap::GetInstances(ObjPtr<mirror::Class> c,
                        int32_t max_count,
                        std::vector<ObjPtr<mirror::Object>>& instances) {
