  ...
  if (desired_collector_type == kCollectorTypeHomogeneousSpaceCompact) {
    ...
    if (!CareAboutPauseTimes()) {
      ...
      CollectGarbageInternal(collector::kGcTypeFull,
                             ...
                             /*clear_soft_references=*/false, GC_NO_ANY);
    } else {
...
#define PERFORM_SUSPENDING_OPERATION(op)                                          \
  ...
  if (last_gc < tried_type) {
    const bool gc_ran = PERFORM_SUSPENDING_OPERATION(
        CollectGarbageInternal(tried_type, kGcCauseForAlloc, false, GC_NO_ANY) != collector::kGcTypeNone);
  ...
  PERFORM_SUSPENDING_OPERATION(
      CollectGarbageInternal(gc_plan_.back(), kGcCauseForAlloc, true, GC_NO_ANY));
  if ((was_default_allocator && allocator != GetCurrentAllocator()) ||
      (!instrumented && EntrypointsInstrumented())) {
    return nullptr;
  }
...
void Heap::CollectGarbage(bool clear_soft_references, GcCause cause) {
  ...
  CollectGarbageInternal(gc_plan_.back(), cause, clear_soft_references, GC_NO_ANY);
}
...
void Heap::PreZygoteFork() {
  if (!HasZygoteSpace()) {
    ...
    CollectGarbageInternal(collector::kGcTypeFull, kGcCauseBackground, false, GC_NO_ANY);
...
static inline bool GCNumberLt(uint32_t gcs_completed, uint32_t gcs_requested) {
  unsigned int difference = gcs_requested - gcs_completed;
...
collector::GcType Heap::CollectGarbageInternal(collector::GcType gc_type,
  ...
  bool compacting_gc;
  {
    ...
    if (requested_gc_no != GC_NO_ANY && !GCNumberLt(GetCurrentGcNo(), requested_gc_no)) {
...
void Heap::FinishGC(Thread* self, collector::GcType gc_type) {
  ...
  thread_running_gc_ = nullptr;
  if (gc_type != collector::kGcTypeNone) {
    gcs_completed_.fetch_add(1, std::memory_order_relaxed);
  }
...
void Heap::PushOnAllocationStackWithInternalGC(Thread* self, ObjPtr<mirror::Object>* obj) {
  ...
  do {
    ...
    CollectGarbageInternal(collector::kGcTypeSticky, kGcCauseForAlloc, GC_NO_ANY, false);
...
void Heap::RequestConcurrentGCAndSaveObject(Thread* self,
  ...
  StackHandleScope<1> hs(self);
  HandleWrapperObjPtr<mirror::Object> wrapper(hs.NewHandleWrapper(obj));
  RequestConcurrentGC(self, kGcCauseBackground, force_full, observed_gc_no);
}

class Heap::ConcurrentGCTask : public HeapTask {
 public:
  ConcurrentGCTask(uint64_t target_time, GcCause cause, bool force_full, uint32_t gc_no)
      : HeapTask(target_time), cause_(cause), force_full_(force_full), my_gc_no_(gc_no) {}
  void Run(Thread* self) override {
    gc::Heap* heap = Runtime::Current()->GetHeap();
    heap->ConcurrentGC(self, cause_, force_full_, my_gc_no_);
  }

 private:
  const GcCause cause_;
  const bool force_full_;  // If true, force full (or partial) collection.
  const uint32_t my_gc_no_;  // Sequence number of requested GC.
};
...
void Heap::RequestConcurrentGC(Thread* self, GcCause cause, bool force_full, uint32_t observed_gc_no) {
  uint32_t gcs_requested = gcs_requested_.load(std::memory_order_relaxed);
  if (!GCNumberLt(observed_gc_no, gcs_requested)) {
    // Nobody beat us to requesting the next gc after observed_gc_no.
    if (CanAddHeapTask(self)
        && gcs_requested_.CompareAndSetStrongRelaxed(gcs_requested, observed_gc_no + 1)) {
...
void Heap::ConcurrentGC(Thread* self, GcCause cause, bool force_full, uint32_t requested_gc_no) {
  if (!Runtime::Current()->IsShuttingDown(self)) {
    ...
    if (WaitForGcToComplete(cause, self) == collector::kGcTypeNone) {
      ...
      if (CollectGarbageInternal(next_gc_type, cause, false, requested_gc_no) == collector::kGcTypeNone) {
        for (collector::GcType gc_type : gc_plan_) {
          // Attempt to run the collector, if we succeed, we are done.
          uint32_t gcs_completed = GetCurrentGcNo();
          if (!GCNumberLt(gcs_completed, requested_gc_no)) {
            // Somebody did it for us.
            break;
          }
          if (gc_type > next_gc_type &&
              CollectGarbageInternal(gc_type, cause, false, requested_gc_no) != collector::kGcTypeNone) {
            break;
          }
        }
      }
...
inline void Heap::CheckGCForNative(Thread* self) {
  ...
  uint32_t starting_gc_no = GetCurrentGcNo();
  ...
  if (UNLIKELY(gc_urgency >= 1.0)) {
    if (is_gc_concurrent) {
      RequestConcurrentGC(self, kGcCauseForNativeAlloc, /*force_full=*/true, starting_gc_no);
      if (gc_urgency > kStopForNativeFactor
        ...
        if (!(GCNumberLt(starting_gc_no, GetCurrentGcNo()))) {
          usleep(2000);  // Encourage our requested GC to start. It's unlikely to finish in 2 msec.
        }
        if (!(GCNumberLt(starting_gc_no, GetCurrentGcNo()))) {
      ...
      CollectGarbageInternal(NonStickyGcType(), kGcCauseForNativeAlloc, false, starting_gc_no + 1);
    }
  }
}
 ...
 public:
  ...
  void Run(Thread* self) override {
    ...
    if (heap->target_footprint_.load(std::memory_order_relaxed) == heap->growth_limit_) {
      heap->RequestConcurrentGC(self, kGcCauseBackground, false, heap->GetCurrentGcNo());
    }
  }
};
