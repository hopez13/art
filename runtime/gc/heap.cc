...
void Heap::RegisterNativeAllocation(JNIEnv* env, size_t bytes) {
  ...
  // max_free_. If GC and reference processing can't keep up and we find that
  // the number of recent native bytes allocated is significantly higher than
  // max_free_, we trigger a blocking GC to slow down the allocating thread.
  size_t new_value = bytes + recent_native_bytes_allocated_.FetchAndAddRelaxed(bytes);
  if (new_value > max_free_ && !IsGCRequestPending()) {
    // Time for another GC.
