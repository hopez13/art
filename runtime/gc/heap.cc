...
void Heap::PostForkChildAction(Thread* self) {
  ...
  uint64_t last_adj_time = current_time;
  ...
  uint64_t postForkGcTime = last_adj_time
      + MsToNs(4 * kPostForkMaxHeapDurationMS + GetPseudoRandomFromUid());
  GetTaskProcessor()->AddTask(self, new TriggerPostForkCCGcTask(postForkGcTime, starting_gc_num));
}
