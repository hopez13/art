...
collector::GcType Heap::CollectGarbageInternal(collector::GcType gc_type,
  ...
  // Turn any new native bytes allocated into old native bytes allocated now
  // that GC has been triggered.
...
ssize_t Heap::NativeAllocationGcWatermark() {
  // Reuse max_free_ for the native allocation gc watermark, so that the
  // native heap is treated in the same way as the Java heap in the case where
  // the gc watermark update would exceed max_free_. Using max_free_ instead
  // of the target utilization means the watermark doesn't depend on the
  // current number of registered native allocations.
  return max_free_;
}

ssize_t Heap::NativeAllocationBlockingGcWatermark() {
  // Heuristic for how large the new registered native allocations can grow
  // before we start to block RegisterNativeAllocation on GC. Historically the
  // native allocations were bounded by growth_limit_. This uses that same
  // value, dividing growth_limit_ by 2 because worst case we can end up with
  // outstanding native allocations about double what the blocking gc
  // watermark is set to.
  return growth_limit_ / 2;
}

void Heap::RegisterNativeAllocation(JNIEnv* env, ssize_t bytes) {
  CHECK_GE(bytes, 0) << "RegisterNativeAllocation negative bytes: " << bytes;
  ssize_t new_value = bytes + new_native_bytes_allocated_.FetchAndAddRelaxed(bytes);

  if (new_value > NativeAllocationBlockingGcWatermark()) {
    // Wait for GC to finish and finalizers to run, because the allocation
    // rate is too high.
    WaitForGcToComplete(kGcCauseForNativeAlloc, ThreadForEnv(env));
    RunFinalization(env, kNativeAllocationFinalizeTimeout);
    CHECK(!env->ExceptionCheck());
    new_value = new_native_bytes_allocated_.LoadRelaxed();
  }

  if (new_value > NativeAllocationGcWatermark() && !IsGCRequestPending()) {
...
void Heap::RegisterNativeFree(JNIEnv*, ssize_t bytes) {
  CHECK_GE(bytes, 0) << "RegisterNativeFree negative bytes: " << bytes;
  ssize_t allocated = new_native_bytes_allocated_.LoadRelaxed();
  ssize_t new_freed_bytes = std::max(ssize_t{0}, std::min(allocated, bytes));
  ssize_t old_freed_bytes = bytes - new_freed_bytes;
  new_native_bytes_allocated_.FetchAndSubRelaxed(new_freed_bytes);
  old_native_bytes_allocated_.FetchAndSubRelaxed(old_freed_bytes);
}
