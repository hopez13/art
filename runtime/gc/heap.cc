...
size_t Heap::GetNativeBytes() {
  ...
  // Jemalloc and glibc dlmalloc seem to interpret hblkhd differently, namely as
  // mapped blocks backing the entire heap vs. large objects directly allocated via mmap.
  // Thus we no longer do this for Bionic.
  // TODO: Re-test whether this is still needed for glibc. We did observe problems early on.
#if defined(GLIBC)
