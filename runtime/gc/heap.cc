...
static constexpr size_t kPreferredAllocSpaceBeginAsUint = 320 * MB - Heap::kDefaultNonMovingSpaceCapacity;
static constexpr int kMaxPMDSize = (kMaxPageSize / sizeof(uint64_t)) * kMaxPageSize;
static_assert(IsAligned<kMaxPMDSize>(kPreferredAllocSpaceBeginAsUint),
              "kPreferredAllocSpaceBegin should be aligned to the maximum "
              "supported PMD size.");

uint8_t* const Heap::kPreferredAllocSpaceBegin =
    reinterpret_cast<uint8_t*>(kPreferredAllocSpaceBeginAsUint);
#else
#ifdef __ANDROID__
