...
#if defined(__BIONIC__) && defined(ART_TARGET)
void Heap::PerformUffdIoctls(GcCause cause, uint32_t requested_gc_num) const {
  if (uffd_ >= 0
...
#else
void Heap::PerformUffdIoctls(GcCause cause ATTRIBUTE_UNUSED,
      ...
      if (CollectGarbageInternal(next_gc_type, cause, false, requested_gc_num)
        ...
        for (collector::GcType gc_type : gc_plan_) {
          ...
          if (gc_type > next_gc_type &&
              ...
              != collector::kGcTypeNone) {
            PerformUffdIoctls(cause, requested_gc_num);
            break;
          }
        }
      } else {
