...
Heap::Heap(size_t initial_size,
    ...
    : non_moving_space_(nullptr),
      ...
      native_allocation_gc_watermark_(native_allocation_gc_watermark),
      native_allocation_blocking_gc_watermark_(native_allocation_blocking_gc_watermark),
...
void Heap::RegisterNativeAllocation(JNIEnv* env, ssize_t bytes) {
  CHECK(bytes >= 0) << "RegisterNativeAllocation negative bytes: " << bytes;
  ssize_t new_value = bytes + new_native_bytes_allocated_.FetchAndAddRelaxed(bytes);

  if (new_value > native_allocation_blocking_gc_watermark_) {
  ...
  if (new_value > native_allocation_gc_watermark_ && !IsGCRequestPending()) {
...
void Heap::RegisterNativeFree(JNIEnv*, ssize_t bytes) {
  CHECK(bytes >= 0) << "RegisterNativeFree negative bytes: " << bytes;
