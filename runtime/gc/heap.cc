...
size_t Heap::GetNativeBytes() {
  // TODO: Check whether we can instead get RSS from /proc/self/statm. This looks too
  // expensive, since it's a full-fledged kernel call, and we have to do it every few
  // memory allocations. But we should also measure mallinfo()
  // cost to compare. If we could get RSS cheaply, we could probably avoid treating non-malloc
  // memory separately.
#if defined(__BIONIC__) || defined(__GLIBC__)
  int malloc_bytes = mallinfo().uordblks;
  // In spite of the documentation, the jemalloc version of this call seems to do what we want,
  // and it is thread-safe.
  // Note that the use of int here is unfortunate, but beyond our control. The cast below will do
  // the right thing up to 4GB. Beyond that, this accounting is unavoidably wrong, and we may
  // garbage collect too infrequently as a result. But we're currently likely to have worse
  // problems in that case.
#else
  // Shouldn't happen where GC triggering details matter. This effectively disables GC
  // triggering based on malloc().
  constexpr int malloc_bytes = 1000;
#endif
  return (size_t)malloc_bytes + native_bytes_registered_.load(std::memory_order_relaxed);
}
...
collector::GcType Heap::CollectGarbageInternal(collector::GcType gc_type,
  ...
  // TODO: Possibly distinguish gc_type == NonStickyGcType()?
...
inline bool Heap::ShouldConcurrentGCForNative() {
  ...
  if (old_native_bytes > current_native_bytes) {
    ...
    return GetBytesAllocated() + weighted_native_bytes >= adj_start_bytes;
  }
}

inline void Heap::CheckConcurrentGCForNative(Thread* self) {
  if (UNLIKELY(ShouldConcurrentGCForNative())) {
    if (IsGcConcurrent()) {
...
// otherwise see.
// TODO: Check whether we can avoid this completely by reading RSS instead of calling mallinfo().
