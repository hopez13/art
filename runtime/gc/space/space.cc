...
void ContinuousMemMapAllocSpace::BindLiveToMarkBitmap() {
  ...
  if (!HasBoundBitmaps()) {
    Runtime::Current()->GetHeap()->GetMarkBitmap()->ReplaceBitmap(&mark_bitmap_, &live_bitmap_);
...
bool ContinuousMemMapAllocSpace::HasBoundBitmaps() const {
  return live_bitmap_.HeapBegin() != mark_bitmap_.HeapBegin();
}
...
void ContinuousMemMapAllocSpace::UnBindBitmaps() {
  ...
  Runtime::Current()->GetHeap()->GetMarkBitmap()->ReplaceBitmap(&live_bitmap_, &mark_bitmap_);
}
