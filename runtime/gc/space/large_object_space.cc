...
class AllocationInfo {
 public:
  ...
  // Return the number of gPageSize-sized blocks that the allocation info covers.
  size_t AlignSize() const {
    return alloc_size_ & kFlagsMask;
  }
  // Returns the allocation size in bytes.
  size_t ByteSize() const {
    return AlignSize() * gPageSize;
  }
  ...
  void SetByteSize(size_t size, bool free) {
    ...
    alloc_size_ = (size / gPageSize) | (free ? kFlagFree : 0u);
  }
 ...
 private:
  ...
  // Contains the size of the previous free block with gPageSize as the unit. If 0 then
  // the allocation before us is not free.
  // These variables are undefined in the middle of allocations / free blocks.
  uint32_t prev_free_;
  // Allocation size of this object in gPageSize as the unit.
...
FreeListSpace::FreeListSpace(const std::string& name,
  ...
  CHECK_ALIGNED_PARAM(space_capacity, gPageSize);
  const size_t alloc_info_size = sizeof(AllocationInfo) * (space_capacity / gPageSize);
  std::string error_msg;
...
void FreeListSpace::ClampGrowthLimit(size_t new_capacity) {
  MutexLock mu(Thread::Current(), lock_);
  new_capacity = RoundUp(new_capacity, gPageSize);
  ...
  size_t alloc_info_size = sizeof(AllocationInfo) * (new_capacity / gPageSize);
...
size_t FreeListSpace::Free(Thread* self, mirror::Object* obj) {
  ...
  DCHECK_ALIGNED_PARAM(obj, gPageSize);
  ...
  DCHECK_ALIGNED_PARAM(allocation_size, gPageSize);
  ...
  if (next_addr >= free_end_start) {
    ...
    if (next_info->IsFree()) {
      ...
      DCHECK_ALIGNED_PARAM(next_next_info->ByteSize(), gPageSize);
...
mirror::Object* FreeListSpace::Alloc(Thread* self, size_t num_bytes, size_t* bytes_allocated,
                                     size_t* usable_size, size_t* bytes_tl_bulk_allocated) {
  MutexLock mu(self, lock_);
  const size_t allocation_size = RoundUp(num_bytes, gPageSize);
