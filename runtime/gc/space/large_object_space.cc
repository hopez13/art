...
//  LOG(INFO) << "---   FreeListSpace::Alloc ------"
//      			<< " \n\t\tnum_bytes: " << num_bytes
//  				<< " \n\t\tkLargeObjectAlignment: " << kLargeObjectAlignment
//  				<< " \n\t\tallocation_size: " << allocation_size << "\n";
  ...
  *bytes_tl_bulk_allocated = allocation_size;
  ...
  num_bytes_allocated_ += allocation_size;
  total_bytes_allocated_ += allocation_size;
  uintptr_t addressX = GetAddressForAllocationInfo(new_info);
  mirror::Object* obj = reinterpret_cast<mirror::Object*>(addressX);
  // We always put our object at the start of the free block, there cannot be another free block
  // before it.
  if (kIsDebugBuild) {
	LOG(INFO) << "--- REQUEST:   FreeListSpace::Alloc mprotect ------"
	                    <<  std::dec
	                    << " \n\t\t num_bytes: " << num_bytes
				        << " \n\t\t kLargeObjectAlignment: " << kLargeObjectAlignment
				        << " \n\t\t allocation_size: " << allocation_size
				        << " \n\t\t address : " << reinterpret_cast<const void*>(addressX)
				        << " \n\t\t range : [" << reinterpret_cast<const void*>(addressX) << ", "
						<< reinterpret_cast<const void*>(addressX + allocation_size) << "]"
						<< "\n";
    CheckedCall(mprotect, __FUNCTION__, obj, allocation_size, PROT_READ | PROT_WRITE);
	LOG(INFO) << "--- RESPONSE:   FreeListSpace::Alloc mprotect ------"
			            <<  std::dec
	                    << " \n\t\t num_bytes: " << num_bytes
				        << " \n\t\t kLargeObjectAlignment: " << kLargeObjectAlignment
				        << " \n\t\t allocation_size: " << allocation_size
