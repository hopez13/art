...
class RegionSpace final : public ContinuousMemMapAllocSpace {
 public:
  ...
  void RevokePartialThreadLocalBuffers() REQUIRES(!region_lock_);
  ...
  bool AllocNewTlab(Thread* self, const size_t min_bytes, size_t* bytes_tl_bulk_allocated)
      REQUIRES(!region_lock_);
 ...
 private:
  ...
  void RevokeThreadLocalBuffersLocked(Thread* thread, bool keep_aside) REQUIRES(region_lock_);
  ...
  std::multimap<uint8_t*, Region*, std::greater<uint8_t*>> partial_tlabs_ GUARDED_BY(region_lock_);
