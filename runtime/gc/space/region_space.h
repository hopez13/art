...
class RegionSpace final : public ContinuousMemMapAllocSpace {
 public:
  ...
  template <RegionType kRegionType, RegionAgeFlags kRegionAgeFlag, typename Visitor>
  ...
  static constexpr uint8_t kRegionAgeTenured = -1;
  // Age region map size
  static constexpr uint8_t kAgeRegionMapSize = kRegionAgeTenureThreshold + 2;
  ...
  bool IsTenuring(mirror::Object* ref) {
    if (HasAddress(ref)) {
      Region* r = RefToRegionUnlocked(ref);
      return ((r->IsInToSpace() && r->IsTenured()) || (r->IsInUnevacFromSpace() && r->Age() >= (kRegionAgeTenureThreshold - 2)));
    }
    return false;
  }
  ...
  template <RegionType kRegionType, RegionAgeFlags kRegionAgeFlag>
  void ClearBitmap();
 ...
 private:
  ...
  class Region {
   public:
    ...
    RegionAgeFlags AgeCategory() {
      if (IsTenured()) {
        return RegionAgeFlags::kRegionAgeFlagTenured;
      } else if (age_ > kRegionAgeNewlyAllocated && age_ <= kRegionAgeTenureThreshold) {
        return RegionAgeFlags::kRegionAgeFlagAged;
      } else {
        return RegionAgeFlags::kRegionAgeFlagNewlyAllocated;
      }
    }
    ...
    void SetUnevacFromSpaceAsToSpace() {
      ...
      if (!IsTenured()) {
        ...
        bool high_live_percent = live_bytes_ * 100U > kEvacuateLivePercentThreshold * bytes_allocated;
        if (high_live_percent) {
          if (age_ >= (kRegionAgeTenureThreshold - 2)) {
   ...
   private:
    ...
    // age ranges from 0 to kRegionAgeTenureThreshold,
    // Tenured objects have a special value of -1
