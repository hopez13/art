  ...
  void SetUpRuntimeOptions(RuntimeOptions* options) override {
    ...
    if (kProfile) {
      ...
      GenerateBootProfile(ArrayRef<const std::string>(dex_files),
                          ...
                          /*method_frequency=*/6,
                          /*type_frequency=*/6);
      // GenerateProfile(profile1_->GetFilename(), dex_file, /*num_classes=*/1);
      profile2_ = std::make_unique<ScratchFile>();
      GenerateBootProfile(ArrayRef<const std::string>(dex_files),
                          profile2_->GetFile(),
                          /*method_frequency=*/8,
                          /*type_frequency=*/8);
      // GenerateProfile(profile2_->GetFilename(), dex_file, /*num_classes=*/1, /*class_offset=*/1);
...
class NoAccessAndroidDataTest
    : public ImageSpaceLoadingTest</*kImage=*/false, /*kRelocate=*/true, /*kProfile=*/false> {
 protected:
  ...
  void SetUpRuntimeOptions(RuntimeOptions* options) override {
    ...
    result = close(fd);
    CHECK_EQ(result, 0) << strerror(errno);
    ImageSpaceLoadingTest::SetUpRuntimeOptions(options);
  }

  void TearDown() override {
    ImageSpaceLoadingTest::TearDown();
