...
TEST_F(ImageSpaceTest, StringDeduplication) {
  ...
  std::string full_image_locations = base_image_location;
  for (const char* base_name : kBaseNames) {
    ...
    full_image_locations += ':' + scratch_dir + prefix + '-' + GetFilenameBase(jar_name) + ".art";
  }
  ...
  std::string app_image_name = scratch_dir + app_base_name + ".art";
  {
    ...
    std::string error_msg;
    bool success = StartDex2OatCommandLine(&argv, &error_msg, /*use_runtime_image_and_bcp=*/ false);
    ASSERT_TRUE(success) << error_msg;
  ...
  auto contains_test_string = [](ImageSpace* space) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  bcp.pop_back();
  bcp_locations.pop_back();
  size_t last_extension_separator = full_image_locations.rfind(ImageSpace::kComponentSeparator);
  ASSERT_NE(std::string::npos, last_extension_separator);
  full_image_locations.resize(last_extension_separator);
  success = load_boot_image();
  ASSERT_TRUE(success);
  ASSERT_EQ(bcp.size(), boot_image_spaces.size());
  ASSERT_TRUE(contains_test_string(boot_image_spaces[boot_image_spaces.size() - 1u].get()));

  // Load odex file and app image.
  std::string error_msg;
