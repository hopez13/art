...
std::string ImageSpace::GetMultiImageName(const std::string& primary_image_name,
                                          const std::string& dex_location) {
  std::string dex_name = dex_location;
  size_t last_slash_index = dex_name.rfind('/');
  if (last_slash_index != std::string::npos) {
    dex_name = dex_location.substr(last_slash_index + 1);
  }

  std::string base = RemoveFileExtension(primary_image_name);
  std::string extension = GetFileExtension(primary_image_name);
  return ReplaceFileExtension(base + "-" + dex_name, extension);
}
...
void ImageSpace::ExtractMultiImageLocations(const std::string& input_image_file_name,
  ...
  for (size_t i = 1; i < dexes.size(); ++i) {
    image_file_names->push_back(GetMultiImageName(input_image_file_name, dexes[i]));
  }
}
