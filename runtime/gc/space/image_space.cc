...
class ImageSpace::PatchObjectVisitor final {
 public:
  ...
  void VisitDexCacheArrays(ObjPtr<mirror::DexCache> dex_cache)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    mirror::NativeArray<ArtMethod>* resolved_methods =
        native_visitor_(dex_cache->GetResolvedMethodsArray());
    dex_cache->SetResolvedMethodsArray(resolved_methods);
    VisitNativeDexCacheArray(resolved_methods);

    mirror::NativeArray<ArtField>* resolved_fields =
        native_visitor_(dex_cache->GetResolvedFieldsArray());
