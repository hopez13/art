...
class PatchObjectVisitor final {
 public:
  ...
  void VisitPointerArray(mirror::PointerArray* pointer_array)
    ...
    for (int32_t i = 0; i != length; ++i) {
      ...
      PatchNativePointer<kPointerSize, /*kMayBeNull=*/ false>(method_entry);
    }
  }
 ...
 private:
  ...
  ALWAYS_INLINE void PatchGcRoot(/*inout*/GcRoot<T>* root) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  template <bool kMayBeNull = true, typename T>
  ALWAYS_INLINE void PatchNativePointer(/*inout*/T** entry) {
...
class PatchArtFieldVisitor final : public ArtFieldVisitor {
 public:
  explicit PatchArtFieldVisitor(const PatchObjectVisitor& visitor) : visitor_(visitor) {}

  void Visit(ArtField* field) override REQUIRES_SHARED(Locks::mutator_lock_) {
    visitor_.PatchGcRoot</*kMayBeNull=*/ false>(&field->DeclaringClassRoot());
  }

 private:
  const PatchObjectVisitor& visitor_;
};

template <typename PatchObjectVisitor, typename PatchCodeVisitor>
class PatchArtMethodVisitor final : public ArtMethodVisitor {
 public:
  explicit PatchArtMethodVisitor(const PatchObjectVisitor& object_visitor,
                                 const PatchCodeVisitor& code_visitor)
  ...
  void Visit(ArtMethod* method) override REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    code_visitor_.PatchNativePointer<kPointerSize>(entrypoint_address);
  }
 ...
 private:
  ...
  const PatchObjectVisitor object_visitor_;
  const PatchCodeVisitor code_visitor_;
};
...
class ImageSpace::BootImageLoader {
 ...
 private:
  class RelocateVisitor {
   public:
    const RelocateVisitor(uint32_t diff) : diff_(diff) {}
