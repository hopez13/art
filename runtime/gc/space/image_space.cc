...
class ImageSpace::Loader {
 ...
 private:
  static bool ValidateBootImageChecksum(const char* image_filename,
                                        ...
                                        const OatFile* oat_file,
                                        /*out*/std::string* error_msg) {
    // Oat checksums have already been validated, so we know that they match
    // the loaded image spaces. Therefore, instead of parsing the checksums
    // formatted as "i;%u/%08x", see AppendImageChecksum(), we just need to
    // check how many image components are present in the oat checksums and
    // use the corresponding loaded image spaces to count components and
    // collect the boot image checksum. Thus the oat checksums are used only
    // to look for the kImageChecksumPrefix at the start of each component.
    ...
    DCHECK(oat_boot_class_path_checksums != nullptr);
    uint32_t component_count = 0u;
    uint32_t checksum = 0u;
    while (*oat_boot_class_path_checksums == kImageChecksumPrefix) {
      DCHECK_LT(component_count, image_spaces.size());
      const ImageHeader& current_header = image_spaces[component_count]->GetImageHeader();
      DCHECK_NE(0u, current_header.GetComponentCount());
      DCHECK_LE(current_header.GetComponentCount(), image_spaces.size() - component_count);
    ...
    if (image_header.GetBootImageComponentCount() != component_count) {
      *error_msg = StringPrintf("Boot image component count mismatch (%u != %u) in image %s",
                                image_header.GetBootImageComponentCount(),
                                component_count,
                                image_filename);
      return false;
    }
