...
class ImageSpace::Loader {
 ...
 private:
  static bool ValidateBootImageChecksum(const char* image_filename,
                                        ...
                                        const OatFile* oat_file,
                                        /*out*/std::string* error_msg) {
    // Oat checksums have already been validated, so use the loaded image spaces
    // and just look for the 'i' prefix in the oat file checksums.
    ...
    while (*oat_boot_class_path_checksums == kImageChecksumPrefix) {
      ...
      // Find the end of oat BCP checksums or the start of the next component.
      do {
        ++oat_boot_class_path_checksums;
      } while (*oat_boot_class_path_checksums != 0 && *oat_boot_class_path_checksums != ':');
    }
