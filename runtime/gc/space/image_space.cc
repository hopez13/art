...
void ImageSpace::ReleaseMetadata() {
  ...
  if (!GetImageHeader().HasCompressedBlock()) {
    // This call is only safe for compressed images since uncompressed will restore to unrelocated
    // pointers instead of nulls that are safely handled.
    return;
  }
