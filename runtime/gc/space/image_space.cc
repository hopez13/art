...
class ImageSpace::BootImageLoader {
 public:
  ...
  const std::string& GetDalvikCache() const { return dalvik_cache_; }
...
bool ImageSpace::LoadBootImage(
  ...
  if (loader.HasCache()) {
    std::string local_error_msg;
    // If we have system image, check system image checksums, otherwise validate the oat file.
    if (loader.LoadFromDalvikCache(/* check_system_checksums */ loader.HasSystem(),
  ...
  if (loader.HasSystem() && relocate) {
    ...
    if (!dex2oat_enabled) {
      ...
      if (patch_success) {
        if (loader.LoadFromDalvikCache(/* check_system_checksums */ false,
                                       /* validate_oat_file */ false,
                                       boot_image_spaces,
                                       oat_file_end,
                                       &local_error_msg)) {
