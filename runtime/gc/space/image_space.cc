...
bool ImageSpace::ValidateOatFile(const OatFile& oat_file,
  ...
  for (size_t i = 0; i < oat_dex_files.size(); i++) {
    ...
    CHECK(DexFileLoader::AccMultiDexChecksum(oat_dex_files[i]->GetDexFileLocation(),
                                             oat_dex_files[i]->GetDexFileLocationChecksum(),
                                             &oat_checksum));

    // Secondary dex files.
    for (; i + 1 < oat_dex_files.size(); i++) {
      if (!DexFileLoader::AccMultiDexChecksum(oat_dex_files[i + 1]->GetDexFileLocation(),
                                              oat_dex_files[i + 1]->GetDexFileLocationChecksum(),
                                              &oat_checksum)) {
        break;
      }
    }
...
std::string ImageSpace::GetBootClassPathChecksums(
  ...
  for (size_t i = 0; i < boot_class_path_tail.size(); i++) {
    ...
    CHECK(DexFileLoader::AccMultiDexChecksum(dex_file, &checksum));
    for (; i + 1 < boot_class_path_tail.size(); i++) {
      if (!DexFileLoader::AccMultiDexChecksum(boot_class_path_tail[i + 1], &checksum)) {
        break;
      }
    }
