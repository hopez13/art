...
class ImageSpace::BootImageLayout {
 public:
  ...
  struct ImageChunk {
    ...
    // immutable (const), so make these fields explicitly mutable.
...
class ImageSpace::BootImageLoader {
 ...
 private:
  ...
  std::unique_ptr<ImageSpace> Load(const std::string& image_location,
      ...
      REQUIRES_SHARED(Locks::mutator_lock_) {
    if (art_fd.get() != -1) {
      // No need to lock memfd (as we do below for normal files)
      // for which we hold the only file descriptor.
