...
// FIXME: Properly describe the image location specification.
//
// path-to-boot-image/boot.art        // direct location; exact or dalvik-cache.
// boot.art                           // searched based on primary BCP component
// path-to-extension/boot-<ext>.art   // direct location; exact or dalvik-cache based on boot image.
// boot-<ext>.art                     // searched based on BCP component
// path-to-extension/*                // search path for extra extensions
// *                                  // search for extra extensions based on BCP component
class ImageSpace::BootImageLayout {
 public:
  ...
  bool LoadFromSystem(InstructionSet image_isa, /*out*/std::string* error_msg) {
    return LoadOrValidateFromSystem(image_isa, /*checksums=*/ nullptr, error_msg);
  }
  ...
  bool LoadFromDalvikCache(const std::string& dalvik_cache, /*out*/std::string* error_msg) {
    return LoadOrValidateFromDalvikCache(dalvik_cache, /*checksums=*/ nullptr, error_msg);
  }
  ...
  void SetBaseAddress(uint32_t base_address) {
    ...
    if (bcp_index == 0u) {
      if (kIsDebugBuild) {
        std::vector<std::string> expanded = ExpandMultiImageLocations(
            ...
            location,
            /*boot_image_extension=*/ false);
        CHECK_EQ(expanded.size(), 1u);
        CHECK_EQ(expanded[0], location);
      }
      return location;
    } else {
      std::vector<std::string> expanded = ExpandMultiImageLocations(
...
std::string ImageSpace::BootImageLayout::GetPrimaryImageLocation() {
  ...
  if (image_location_.find('/') == std::string::npos) {
    ...
    location.insert(0u, boot_class_path_[0].substr(slash_pos + 1u));
  }
  return location;
}
...
bool ImageSpace::BootImageLayout::VerifyImageLocation(
  ...
  for (size_t i = 0; i != components_size; ++i) {
    ...
    if (wildcard_pos == std::string::npos) {
      ...
      // And it must be wither plain wildcard or preceded by a path separator.
...
class ImageSpace::BootImageLoader {
 ...
 private:
  bool LoadImage(
    ...
    uint32_t base_address = layout.GetBaseAddress();
    DCHECK(!chunks.empty());

    // Calculate total component count and reservation size.
    size_t bcp_component_count = boot_class_path_.size();
    size_t image_component_count = 0u;
    size_t image_reservation_size = 0u;
    for (const BootImageLayout::ImageChunk& chunk : chunks) {
      DCHECK_LE(chunk.component_count, bcp_component_count - image_component_count);
      image_component_count += chunk.component_count;
      DCHECK_LE(chunk.reservation_size, kMaxTotalImageReservationSize - image_reservation_size);
      image_reservation_size += chunk.reservation_size;
    }
  ...
  bool LoadComponents(const BootImageLayout::ImageChunk& chunk,
    ...
    ArrayRef<const std::unique_ptr<ImageSpace>> available_dependencies =
        ArrayRef<const std::unique_ptr<ImageSpace>>(*spaces).SubArray(/*pos=*/ 0u,
                                                                      max_image_space_dependencies);
    for (std::size_t i = 0u, size = locations.size(); i != size; ++i) {
      ImageSpace* space = (*spaces)[chunk.start_index + i].get();
