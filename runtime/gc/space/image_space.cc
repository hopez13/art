 ...
 public:
  ...
  void FixupDexCacheArrayEntry(std::atomic<mirror::NativeDexCachePair<T>>* array, uint32_t index)
    ...
    if (pair.object != nullptr) {
      pair.object = heap_visitor_(pair.object);
...
class ImageSpace::Loader {
 ...
 private:
  ...
  class ForwardAddress {
   public:
    ...
    ALWAYS_INLINE T* operator()(T* src) const {
      ...
      DCHECK(range0_.InSource(uint_src)) << reinterpret_cast<const void*>(src);
  ...
  template <typename ForwardObject, typename ForwardCode>
   ...
   public:
    ...
    void Visit(ArtMethod* method) override NO_THREAD_SAFETY_ANALYSIS {
      ...
      if (UNLIKELY(method->IsRuntimeMethod())) {
        ...
        if (table != nullptr) {
          ImtConflictTable* new_table = forward_object_(table);
          if (table != new_table) {
  ...
  static bool RelocateInPlace(ImageHeader& image_header,
    ...
    }
    {
      ...
      FixupArtMethodVisitor method_visitor(kPointerSize, forward_object, forward_code);
    ...
    if (fixup_image) {
      ...
      }
      {
        ...
        image_header.VisitPackedImTables(forward_object, target_base, kPointerSize);
      }
      {
        TimingLogger::ScopedTiming timing("Fixup conflict tables", &logger);
        image_header.VisitPackedImtConflictTables(forward_object, target_base, kPointerSize);
      }
