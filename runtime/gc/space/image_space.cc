...
class ImageSpace::Loader {
 ...
 private:
  ...
  static MemMap LoadImageFile(const char* image_filename,
    ...
    // Simple utility for madvise'ing the image file into memory, accelerating either decompresion
    // (for the compressed case), or fixup (for the uncompressed case).
    auto madvise_image_memmap = [](const MemMap& map) {
