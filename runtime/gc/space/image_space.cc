...
class ImageSpace::Loader {
 public:
  static std::unique_ptr<ImageSpace> Init(const char* image_filename,
    ...
    // FIXME GetImageBegin is the preferred address to map the image. If we manage to map the
...
class ImageSpace::BootImageLoader {
 public:
  ...
  bool LoadFromSystem(/*out*/ std::vector<std::unique_ptr<space::ImageSpace>>* boot_image_spaces,
    ...
    if (!ReserveBootImageMemory(locations, &image_reservation, &oat_reservation, error_msg)) {
      return false;
    }
 ...
 private:
  ...
  bool ReserveBootImageMemory(const std::vector<std::string>& locations,
                              /*inout*/MemMap* image_reservation,
                              /*inout*/MemMap* oat_reservation,
                              /*out*/std::string* error_msg) {
    DCHECK(!locations.empty());
    std::string first_filename = GetSystemImageFilename(locations.front().c_str(), image_isa_);
    uint32_t image_start;
    uint32_t image_end;
    uint32_t oat_end;
    if (!GetBootImageAddressRange(first_filename, &image_start, &image_end, &oat_end, error_msg)) {
      return false;
    }
    if (locations.size() > 1u) {
      std::string last_filename = GetSystemImageFilename(locations.back().c_str(), image_isa_);
      uint32_t dummy1, dummy2;
      if (!GetBootImageAddressRange(last_filename, &dummy1, &dummy2, &oat_end, error_msg)) {
        return false;
      }
    }
    ...
    *oat_reservation = image_reservation->RemapAtEnd(reinterpret_cast32<uint8_t*>(image_end),
    ...
    LOG(ERROR) << "Reserved boot image memory "
        << reinterpret_cast32<void*>(image_start) << "-" << reinterpret_cast32<void*>(image_end)
        << "-" << reinterpret_cast32<void*>(oat_end);
    return true;
  }
...
bool ImageSpace::LoadBootImage(
  ...
  // Step 1: Check if we have a system image.
  if (loader.HasSystem()) {
    std::string local_error_msg;
