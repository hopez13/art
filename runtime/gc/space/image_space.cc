...
bool ImageSpace::LoadBootImage(const std::string& image_file_name,
                               ...
                               const std::string* boot_class_path_comparison,
  ...
  if (!error && boot_class_path_comparison != nullptr && !boot_class_path_comparison->empty()) {
    std::vector<std::string> boot_class_path_elements;
    Split(*boot_class_path_comparison, ':', &boot_class_path_elements);

    // Support both single and multi image.
    std::vector<const std::string*> image_path_elements;
    for (const ImageSpace* space : *boot_image_spaces) {
      ...
      for (auto oat_dex_file : oat_dex_files) {
        image_path_elements.push_back(&oat_dex_file->GetCanonicalDexFileLocation());
      }
    }
    ...
    auto image_path_element_it = image_path_elements.begin();
    auto image_path_element_it_end = image_path_elements.end();

    for (;
        boot_class_path_it != boot_class_path_it_end &&
            image_path_element_it != image_path_element_it_end;
        ++boot_class_path_it, ++image_path_element_it) {
      const std::string* spaces_str = *image_path_element_it;

      // Skip multidex components.
      if (DexFile::IsMultiDexLocation(spaces_str->c_str())) {
        ++image_path_element_it;
        continue;
      }
    ...
    for (; image_path_element_it != image_path_element_it_end; ++image_path_element_it) {
      const std::string* spaces_str = *image_path_element_it;
    ...
    const bool image_done = image_path_element_it == image_path_element_it_end;
    error = error || !boot_class_path_done || !image_done;

    if (error) {
      std::ostringstream oss;
      for (auto str : image_path_elements) {
        oss << *str << ':';
      }

      LOG(WARNING) << "Boot classpath mismatch: requested " << *boot_class_path_comparison
