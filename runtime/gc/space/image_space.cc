...
class ImageSpace::BootImageLayout {
 public:
  ...
  BootImageLayout(ArrayRef<const std::string> image_locations,
                  ...
                  ArrayRef<const int> boot_class_path_art_fds,
     ...
     : image_locations_(image_locations),
       ...
       boot_class_path_art_fds_(boot_class_path_art_fds),
 ...
 private:
  ...
  ArrayRef<const int> boot_class_path_art_fds_;
...
bool ImageSpace::BootImageLayout::ReadHeader(const std::string& base_location,
  ...
  int bcp_art_fd = bcp_index < boot_class_path_art_fds_.size()
      ? boot_class_path_art_fds_[bcp_index]
      : -1;
  ImageHeader header;
  auto image_file = bcp_art_fd >= 0
      ? std::make_unique<File>(bcp_art_fd, actual_filename, /*check_usage=*/ false)
  ...
  if (bcp_index < boot_class_path_art_fds_.size()) {
    // The FD of .art needs to be duplicated because it'll be owned/used later.
    int fd = boot_class_path_art_fds_[bcp_index];
    if (fd >= 0) {
...
class ImageSpace::BootImageLoader {
 public:
  BootImageLoader(const std::vector<std::string>& boot_class_path,
                  ...
                  const std::vector<int>& boot_class_path_art_fds,
 ...
 private:
  ...
  const ArrayRef<const int> boot_class_path_art_fds_;
...
bool ImageSpace::IsBootClassPathOnDisk(InstructionSet image_isa) {
  ...
  BootImageLayout layout(ArrayRef<const std::string>(runtime->GetImageLocations()),
                         ...
                         ArrayRef<const int>(runtime->GetBootClassPathArtFds()),
...
bool ImageSpace::VerifyBootClassPathChecksums(std::string_view oat_checksums,
  ...
  if (StartsWith(oat_checksums, "i")) {
    ...
    BootImageLayout layout(image_locations,
                           ...
                           bcp_fds,
                           /*boot_class_path_art_fds=*/ ArrayRef<const int>(),
