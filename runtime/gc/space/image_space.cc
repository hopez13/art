...
class ImageSpace::BootImageLoader {
 ...
 private:
  template <typename T>
  static T* RelocatedAddress(T* src, uint32_t diff) {
    DCHECK(src != nullptr);
    return reinterpret_cast32<T*>(reinterpret_cast32<uint32_t>(src) + diff);
  }

  template <typename T>
  static void PatchGcRoot(uint32_t diff, /*inout*/GcRoot<T>* root)
      REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  template <PointerSize kPointerSize, typename T>
  static void PatchNativePointer(uint32_t diff, /*inout*/T** entry) {
    if (kPointerSize == PointerSize::k64) {
      DCHECK(IsUint<32>(*reinterpret_cast<uint64_t*>(entry)));
    }
    if (kPointerSize == PointerSize::k64) {
  ...
  class PatchedObjectsMap {
   public:
    ...
    bool IsVisited(mirror::Object* object) const {
      return visited_objects_.LoadBit(GetIndex(object));
    }

    void MarkVisited(mirror::Object* object) {
      DCHECK(!IsVisited(object));
      visited_objects_.StoreBit(GetIndex(object), true);
    }
  ...
  class PatchArtFieldVisitor : public ArtFieldVisitor {
   public:
    ...
    void Visit(ArtField* field) override REQUIRES_SHARED(Locks::mutator_lock_) {
      PatchGcRoot(diff_, &field->DeclaringClassRoot());
    }
  ...
  class PatchObjectVisitor {
   public:
    ...
    void VisitClass(mirror::Class* klass) REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      PatchReferenceField(klass, mirror::Object::ClassOffset());
    ...
    void VisitPointerArray(mirror::PointerArray* pointer_array)
      ...
      PatchReferenceField(pointer_array, mirror::Object::ClassOffset());
      ...
      for (int32_t i = 0; i != length; ++i) {
        ...
        PatchNativePointer<kPointerSize>(diff_, method_entry);
      }
    }
   ...
   private:
    void PatchReferenceField(mirror::Object* object, MemberOffset offset) const
        REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    void FixupDexCacheArrayEntry(std::atomic<mirror::DexCachePair<T>>* array, uint32_t index)
      ...
      GcRoot<T>* root = &(reinterpret_cast<mirror::DexCachePair<T>*>(array)[index].object);
      PatchGcRoot(diff_, root);
    }
    ...
    void FixupDexCacheArrayEntry(std::atomic<mirror::NativeDexCachePair<T>>* array, uint32_t index)
      ...
      if (pair.object != nullptr) {
        pair.object = reinterpret_cast32<T*>(reinterpret_cast32<uint32_t>(pair.object) + diff_);
  ...
  static void DoRelocateSpaces(const std::vector<std::unique_ptr<ImageSpace>>& spaces,
    ...
    for (size_t s = 0, size = spaces.size(); s != size; ++s) {
      ...
      if (image_header.GetInternedStringsSection().Size() != 0u) {
        ...
        // TODO: ObjPtr<> has been updated to deal with Thread::Current() == nullptr.
      ...
      if (image_header.GetClassTableSection().Size() != 0u) {
        ...
        for (uint32_t& slot : temp_set) {
          ...
          DCHECK(klass->GetClass() == class_class);
            ...
            for (int32_t i = 0; i != ifcount; ++i) {
              ...
              if (unpatched_ifarray != nullptr) {
                ...
                mirror::PointerArray* ifarray = reinterpret_cast32<mirror::PointerArray*>(
                    reinterpret_cast32<uint32_t>(unpatched_ifarray) + diff);
  ...
  static void MaybeRelocateSpaces(const std::vector<std::unique_ptr<ImageSpace>>& spaces)
    ...
    if ((false))
    LOG(ERROR) << "VMARKO: #spaces: " << spaces.size()
