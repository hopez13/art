...
void RegionSpace::SetFromSpace(accounting::ReadBarrierTable* rb_table,
  ...
  std::vector<std::pair<Region*, uint8_t*>>* alloc_count_vec =
      new std::vector<std::pair<Region*, uint8_t*>>();
  ...
  // Some processes (like dex2oat) don't get heap's thread-pool created.
  if (UNLIKELY(thread_pool == nullptr)) {
    // Run function deletes the vector.
    CountAllocatedObjectsTask(alloc_count_vec).Run(self);
  } else {
