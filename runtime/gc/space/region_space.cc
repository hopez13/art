...
void RegionSpace::ClearFromSpace(uint64_t* adjusted_from_bytes,
                                 uint64_t* adjusted_from_objects) {
  DCHECK(adjusted_from_bytes != nullptr);
  DCHECK(adjusted_from_objects != nullptr);
  *adjusted_from_bytes = 0;
  *adjusted_from_objects = 0;
  MutexLock mu(Thread::Current(), region_lock_);
  for (size_t i = 0; i < num_regions_; ++i) {
    Region* r = &regions_[i];
    if (r->IsInFromSpace()) {
      *adjusted_from_bytes += r->BytesAllocated();
      *adjusted_from_objects += r->ObjectsAllocated();
      --num_non_free_regions_;
      r->Clear();
    } else if (r->IsInUnevacFromSpace()) {
      if (r->LiveBytes() == 0) {
        // Special case for 0 live bytes, this all of the objects in the region are dead and we can
        // clear it. This is important for large objects since we must not visit dead ones in
