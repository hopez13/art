...
void RegionSpace::Region::MakeUnevacFromSpaceWalkable(
  ...
  if (LiveBytes() == static_cast<size_t>(Top() - Begin())) {
    // Fully occupied without no holes. Nothing to do.
    return;
  }
  ...
  while (cur < end) {
    DCHECK_ALIGNED(cur, kObjectAlignment);
    ...
    if (!region_space_bitmap->Test(obj)) {
      ...
      uintptr_t find_begin = cur + kObjectAlignment;
      ...
      cur = cur + static_cast<uintptr_t>(RoundUp(obj->SizeOf(), kObjectAlignment));
    }
  }
}
