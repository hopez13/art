...
inline void ObjectFixup::FixupAllReferences(uint8_t* image_base,
  ...
  for (size_t i = 0; i < NumHeapReferenceFixups(); ++i) {
    DCHECK_EQ(GetHeapReference(image_base, i)->AsMirrorPtr(), expected_obj);
    GetHeapReference(image_base, i)->Assign(new_obj);
  }
  for (size_t i = 0; i < NumCompressedReferenceFixups(); ++i) {
    DCHECK_EQ(GetCompressedReference(image_base, i)->AsMirrorPtr(), expected_obj);
...
inline void PointerSizedFixup::FixupAllPointers(uint8_t* image_base,
                                                void* new_ptr,
                                                const void* expected_ptr) const {
  for (size_t i = 0; i < NumOffsets(); ++i) {
    DCHECK_EQ(*PointerAddr(image_base, i), expected_ptr);
    *PointerAddr(image_base, i) = new_ptr;
  }
}
