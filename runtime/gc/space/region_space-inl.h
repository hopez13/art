...
inline mirror::Object* RegionSpace::AllocNonvirtual(size_t num_bytes,
  ...
  uint8_t age_idx = age + 1;
  CHECK(age_idx >= 0 && age_idx < kAgeRegionMapSize);
  DCHECK(kForEvac == (age != kRegionAgeNewlyAllocated));
  mirror::Object* obj;
  if (LIKELY(num_bytes <= kRegionSize)) {
    // Non-large object.
    obj = (kForEvac ? evac_region_[age_idx] : current_region_)->Alloc(num_bytes,
                                                             bytes_allocated,
                                                             usable_size,
                                                             bytes_tl_bulk_allocated);
...
inline uint64_t RegionSpace::GetBytesAllocatedInternal() {
  ...
  VisitRegions<kRegionType,
               RegionAgeFlags::kRegionAgeFlagAll> ([&bytes](Region* r) {
...
template <RegionSpace::RegionType kRegionType, RegionSpace::RegionAgeFlags kRegionAgeFlag>
void RegionSpace::ClearBitmap() {
  if (kRegionType == RegionType::kRegionTypeAll && kRegionAgeFlag == RegionAgeFlags::kRegionAgeFlagAll) {
    mark_bitmap_.Clear();
    return;
  }
  VisitRegionRanges<kRegionType, kRegionAgeFlag> ([this](uint8_t* begin, uint8_t* end) {
                                                    mark_bitmap_.ClearRange(reinterpret_cast<mirror::Object*>(begin),
                                                                             reinterpret_cast<mirror::Object*>(end));
                                                 });
}

template <RegionSpace::RegionType kRegionType, RegionSpace::RegionAgeFlags kRegionAgeFlag, typename Visitor>
void RegionSpace::VisitRegionRanges(const Visitor& visitor) {
  uint8_t* prev_begin = nullptr;
  uint8_t* prev_end = nullptr;
  VisitRegions<kRegionType, kRegionAgeFlag>([&visitor, &prev_begin, &prev_end](Region *r) {
                                        if (r->Begin() == prev_end) {
                                          prev_end = r->End();
                                        } else {
