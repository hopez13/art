...
GarbageCollector::GarbageCollector(Heap* heap, const std::string& name)
    : heap_(heap),
      ...
      gc_work_throughput_hist_(nullptr),
...
void GarbageCollector::Run(GcCause gc_cause, bool clear_soft_references) {
  ...
  if (gc_work_throughput_hist_ != nullptr) {
    // Throughput in bytes/s. Add 1ms to prevent possible division by 0.
    uint64_t throughput = (current_iteration->GetScannedBytes() * 1000)
            / (NsToMs(duration_ns) + 1);
    gc_work_throughput_hist_->Add(throughput / MB);
  }
...
void GarbageCollector::DumpPerformanceInfo(std::ostream& os) {
  ...
  os << GetName() << " total time: " << PrettyDuration(total_ns)
     ...
     << GetName() << " work throughput: " << PrettySize(scanned_bytes / seconds) << "/s"
     << " per cpu-time: "
