...
void MarkSweep::FinishPhase() {
  ...
  if (kCountTasks) {
    VLOG(gc) << "Total number of work chunks allocated: " << work_chunks_created_.load(std::memory_order_relaxed);
  }
  if (kMeasureOverhead) {
    VLOG(gc) << "Overhead time " << PrettyDuration(overhead_time_.load(std::memory_order_relaxed));
  }
  if (kProfileLargeObjects) {
    VLOG(gc) << "Large objects tested " << large_object_test_.load(std::memory_order_relaxed)
        << " marked " << large_object_mark_.load(std::memory_order_relaxed);
  }
  if (kCountMarkedObjects) {
    VLOG(gc) << "Marked: null=" << mark_null_count_.load(std::memory_order_relaxed)
