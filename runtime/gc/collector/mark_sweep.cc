 ...
 public:
  ...
  void operator()(const mirror::Object* obj) const NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (UNLIKELY(obj == nullptr || !IsAligned<kPageSize>(obj) ||
      ...
      // and we want it to have log priority.
      std::ostringstream oss;
      oss << "Tried to mark " << obj << " not contained by any spaces";
      if (holder_ != nullptr) {
      ...
      oss << "Attempting see if it's a bad thread root";
  ...
  void VisitRoot(mirror::Object* root, const RootInfo& info) OVERRIDE
    ...
    if (heap->GetLiveBitmap()->GetContinuousSpaceBitmap(root) == nullptr) {
      ...
      if (large_object_space != nullptr && !large_object_space->Contains(root)) {
        os_ << "Found invalid root: " << root << " " << info;
      }
    }
  }
