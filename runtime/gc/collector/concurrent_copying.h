...
class ConcurrentCopying : public GarbageCollector {
 ...
 private:
  ...
  // ReclaimPhase, and are read by DumpPerformanceInfo (potentially from
  // another thread). However, at present, DumpPerformanceInfo is only called
  // when the runtime shuts down, so no concurrent access. The same reasoning
  // goes for gc_count_ and reclaimed_bytes_ratio_sum_
