...
class ConcurrentCopying : public GarbageCollector {
 ...
 private:
  ...
  // copied_live_bytes_ratio_sum_ and gc_count_ are updated by CC per GC, and
  // are read by DumpPerformanceInfo (potentially from another thread).
  // Currently, DumpPerformanceInfo is only called when the runtime shuts down,
  // no concurrent access.
  // The sum of of all copied live bytes ratio (to_bytes/from_bytes)
  Atomic<float> copied_live_bytes_ratio_sum_;
