...
void MarkCompact::FreeFromSpacePages(size_t cur_page_idx) {
  ...
  bool dont_reclaim = false;
  // Check if the 'class_after_obj_map_' map allows to free pages.
  for (; class_after_obj_iter_ != class_after_obj_map_.rend(); class_after_obj_iter_++) {
    if (reinterpret_cast<uint8_t*>(class_after_obj_iter_->first.AsMirrorPtr()) >= reclaim_begin) {
      // Found a class which is in the reclaim range.
      if (reinterpret_cast<uint8_t*>(class_after_obj_iter_->second.AsMirrorPtr()) < idx_addr) {
        // Its lowest-address object is not compacted yet.
        dont_reclaim = true;
        break;
      }
    } else {
      // All the remaining pairs are lower than the reclaim range.
      break;
    }
  }

  ssize_t size = last_reclaimed_page_ - reclaim_begin;
  if (!dont_reclaim && size >= kMinFromSpaceMadviseSize) {
