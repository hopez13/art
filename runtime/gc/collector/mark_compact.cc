...
static Histogram<uint32_t> mmap_trylock_hist("mmap_trylock histogram", 16);
...
void MarkCompact::ZeropageIoctl(void* addr,
  ...
  uint32_t max_backoff = self == thread_running_gc_ ? 128 : 16;
  ...
  while (length > 0) {
    ...
    if (LIKELY(ret == 0)) {
      ...
      if (uffd_zeropage.zeropage > 0) {
        ...
        CHECK_EQ(uffd_zeropage.zeropage, -EAGAIN);
      }
  ...
  while (length > 0) {
    ...
    if (ret == 0) {
      ...
      if (uffd_copy.copy > 0) {
        ...
        CHECK_EQ(uffd_copy.copy, -EAGAIN);
      }
      BackOff(backoff_count++);
      if (backoff_count == max_backoff) {
        uffd_copy.mode = 0;
  ...
  while (true) {
    ...
    if (state == PageState::kProcessing || state == PageState::kMutatorProcessing ||
      ...
      if (state == PageState::kProcessed) {
        size_t arr_len =
            mmap_lock_is_contended_ ? page_idx + 1 : moving_first_objs_count_ + black_page_count_;
  ...
  if (arena_iter == linear_alloc_arenas_.end() ||
    ...
    while (true) {
      switch (state) {
        ...
        {
          size_t arr_len = mmap_lock_is_contended_ ? page_idx + 1 : space_data->end_ - fault_page;
