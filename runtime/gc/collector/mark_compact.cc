...
MarkCompact::MarkCompact(Heap* heap)
  ...
  live_words_bitmap_.reset(LiveWordsBitmap<kAlignment>::Create(bump_pointer_space_->Begin(),
                                                               bump_pointer_space_->Limit()));
}
...
void MarkCompact::InitializePhase() {
  ...
  // TODO: replace this with a mem-map so that we can madvise-dontneed it at the
  // end.
  offset_vector_.reset(new uint32_t[vector_length_]);
}
...
void MarkCompact::PrepareForCompaction() {
  ...
  // that will be slid with TLAB holes and the TLAB info in TLS will be
