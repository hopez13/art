...
class MarkCompact::ClassLoaderRootsUpdater : public BufferedRootVisitor<kBufferSize>,
                                             public ClassLoaderVisitor {
 public:
  explicit ClassLoaderRootsUpdater(MarkCompact* collector)
      : BufferedRootVisitor<kBufferSize>(collector, RootInfo(RootType::kRootVMInternal)) {}
...
class MarkCompact::LinearAllocPageUpdater {
 public:
  explicit NativeRootsUpdateVisitor(MarkCompact* collector) : collector_(collector) {}
...
void MarkCompact::PreCompactionPhase() {
  ...
  }
  {
    ...
    {
      ClassLoaderRootsUpdater</*kBufferSize=*/16> updater(this);
...
void MarkCompact::ConcurrentlyProcessMovingPage(uint8_t* fault_page,
  ...
  auto zeropage_ioctl = [this] (void* addr, bool tolerate_eexist) {
                          ...
                          CHECK(ret == 0
                                || ret == ENOENT
                                || (tolerate_eexist && ret == -1 && errno == EEXIST))
  ...
  if (first_obj == nullptr) {
    ...
    PageState expected_state = PageState::kUnprocessed;
    if (moving_page_status_[page_idx].compare_exchange_strong(expected_state,
                                                              PageState::kProcessingAndMapping,
                                                              std::memory_order_relaxed)) {
...
void MarkCompact::CompactionPhase() {
  ...
  // TODO: Add logic here to process linear-alloc.
  ...
  // TODO: Add logic here to unregister linear-alloc.
