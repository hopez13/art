...
MarkCompact::MarkCompact(Heap* heap)
  ...
  CHECK_LT(kOffsetChunkSize, gPageSize);
  if (kIsDebugBuild) {
...
void MarkCompact::MarkingPause() {
  ...
  Locks::mutator_lock_->AssertExclusiveHeld(thread_running_gc_);
  {
    ...
    // ClearCardRange().
    black_allocations_begin_ = bump_pointer_space_->AlignEnd(thread_running_gc_, gPageSize);
    DCHECK(IsAligned<kAlignment>(black_allocations_begin_));
    black_allocations_begin_ = AlignUp(black_allocations_begin_, gPageSize);
