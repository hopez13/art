  ...
  if (gHaveMremapDontunmap) {
    ...
    if (fd >= 0) {
      ...
      gUffdFeatures = api.features;
      {
        ...
        void* mem;
        const size_t page_size = GetPageSizeSlow();
        CHECK(posix_memalign(&mem, page_size, page_size) == 0);
        CHECK(IsAlignedParam(mem, page_size));
        ...
        if (errno != EINVAL) {
          ...
          LOG(INFO) << "MMAP_TRYLOCK is not supported in uffd addr:"
                    << mem << " page-size:" << page_size;
        }
        free(mem);
      }
      close(fd);
