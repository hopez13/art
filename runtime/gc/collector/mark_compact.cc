...
void MarkCompact::CompactMovingSpace(uint8_t* page) {
  ...
  // compacted.
  for (auto iter = class_after_obj_map_.begin(); iter != class_after_obj_map_.end(); iter++) {
    if (iter->first.AsMirrorPtr() != iter->second.first.AsMirrorPtr()) {
      auto cur_iter = iter;
      iter--;
      ...
      // otherwise as well.
      class_after_obj_map_.insert(std::move(nh));
    }
  }
