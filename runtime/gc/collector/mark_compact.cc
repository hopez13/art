...
MarkCompact::MarkCompact(Heap* heap)
  ...
  std::string err_msg;
  info_map_ = MemMap::MapAnonymous("Concurrent mark-compact offset-vector",
  ...
  if (UNLIKELY(!info_map_.IsValid())) {
    ...
    offset_vector_ = reinterpret_cast<uint32_t*>(begin);
    vector_length_ = offset_vector_size;

    begin += offset_vector_size * sizeof(uint32_t);
    first_objs_non_moving_space_ = reinterpret_cast<ObjReference*>(begin);

    begin += nr_non_moving_pages * sizeof(ObjReference);
    first_objs_moving_space_ = reinterpret_cast<ObjReference*>(begin);

    begin += nr_moving_pages * sizeof(ObjReference);
    pre_compact_offset_moving_space_ = reinterpret_cast<uint32_t*>(begin);
  }
...
void MarkCompact::InitMovingSpaceFirstObjects(const size_t vec_len) {
  ...
  DCHECK(live_words_bitmap_->Test(offset)) << "offset=" << offset
                                           << " vec_idx=" << offset_vec_idx
                                           << " N=0"
                                           << " offset_in_word=" << offset_in_vec_word
                                           << " word=" << std::hex << live_words_bitmap_->GetWord(offset_vec_idx);
  ...
  while (true) {
    ...
    DCHECK_LE(page_live_bytes, offset_vector_[offset_vec_idx])
        ...
        << " vec_len=" << vec_len;
    DCHECK(IsAligned<kAlignment>(offset_vector_[offset_vec_idx] - page_live_bytes));
    offset_in_vec_word =
    ...
    DCHECK(live_words_bitmap_->Test(offset)) << "offset=" << offset
                                             << " vec_idx=" << offset_vec_idx
                                             << " N=" << ((offset_vector_[offset_vec_idx] - page_live_bytes) / kAlignment)
                                             << " offset_in_word=" << offset_in_vec_word
                                             << " word=" << std::hex << live_words_bitmap_->GetWord(offset_vec_idx);
...
void MarkCompact::InitNonMovingSpaceFirstObjects() {
  ...
  while (begin < end) {
    ...
    if (prev_obj != nullptr && prev_obj_end > begin) {
      ...
      first_objs_non_moving_space_[page_idx].Assign(prev_obj);
      LOG(WARNING) << "found inter-page object in non-moving space";
    } else {
      prev_obj_end = 0;
...
void MarkCompact::PrepareForCompaction() {
  ...
  if (kIsDebugBuild) {
    for (size_t i = 0; i < vector_len; i++) {
      CHECK_LE(offset_vector_[i], kOffsetChunkSize);
      CHECK_EQ(offset_vector_[i], live_words_bitmap_->LiveBytesInBitmapWord(i));
    }
  }
  ...
  if (kIsDebugBuild) {
    for (size_t i = vector_len; i < vector_length_; i++) {
      CHECK_EQ(offset_vector_[i], 0u);
    }
  }
  // We need this to accommodate for black allocations, which will be
  // incorporated later in the compaction-pause.
  if (vector_len < vector_length_) {
    offset_vector_[vector_len] = total;
  }
...
void MarkCompact::MarkingPause() {
  ...
  // TODO: Find correct strategy to visit/walk objects in bump-pointer space
  // when we have mark-bitmap to indicate live objects. At the same time we also
  // need to be able to visit black allocations, even though they are not marked
  // in the bitmap. Without both of these we fail pre-sweeping verification. As
  // well as we leave windows open wherein a VisitObjects/Walk on the space
  // would either miss some objects or visit unreachable ones. These windows are
  // when we are switching from shared mutator-lock to exclusive and vice-versa
  // starting from here till compaction pause.
...
void MarkCompact::ReclaimPhase() {
  ...
  // references during compaction pause.
...
void MarkCompact::CompactPage(mirror::Object* obj, uint32_t offset, uint8_t* addr) {
  ...
  auto verify_class = [&] (mirror::Object* ref) REQUIRES_SHARED(Locks::mutator_lock_) {
                        if (kIsDebugBuild) {
                          ...
                          mirror::Class* pre_compact_klass =
                                ref->GetClass<kVerifyNone, kWithoutReadBarrier>();
                          mirror::Class* klass_klass =
                                klass->GetClass<kVerifyNone, kWithFromSpaceBarrier>();
                          mirror::Class* klass_klass_klass =
                                klass_klass->GetClass<kVerifyNone, kWithFromSpaceBarrier>();
                          ...
                          CHECK(IsValidObject(ref))
                               ...
                               << heap_->GetVerification()
                                  ->DumpRAMAroundAddress(
                                        reinterpret_cast<uintptr_t>(klass), 128)
                               << "spaces:\n" << oss.str();
                        }
                      };
  ...
  if (offset > 0) {
    ...
    if (stride_count > 1) {
      ...
      obj_size = obj->VisitRefsForCompaction</*kFetchObjSize*/true, /*kVisitNativeRoots*/false>(
              visitor, MemberOffset(offset), MemberOffset(-1));
    } else {
...
void MarkCompact::SlideBlackPage(mirror::Object* first_obj,
  ...
  if (bytes_copied < kPageSize) {
    ...
    current_space_bitmap_->VisitMarkedRange(
            ...
            page_end,
            [&found_obj, pre_compact_addr, dest, this, verify_class] (mirror::Object* obj)
            REQUIRES_SHARED(Locks::mutator_lock_) {
...
void MarkCompact::CompactMovingSpace() {
  ...
  while (idx < count) {
    ...
    if (first_obj != nullptr) {
      DCHECK_GT(pre_compact_offset_moving_space_[idx], 0u);
      SlideBlackPage(first_obj,
                     pre_compact_offset_moving_space_[idx],
                     first_objs_moving_space_[idx + 1].AsMirrorPtr(),
                     pre_compact_page,
                     begin);
    }
...
void MarkCompact::UpdateNonMovingPage(mirror::Object* holder, uint8_t* page) {
  ...
  non_moving_space_bitmap_->VisitMarkedRange(
          reinterpret_cast<uintptr_t>(holder) + mirror::kObjectHeaderSize,
          ...
          [&](mirror::Object* next_obj) {
            ...
            if (reinterpret_cast<uint8_t*>(holder) < page) {
              ...
              holder->VisitRefsForCompaction</*kFetchObjSize*/false, /*kVisitNativeRoots*/false>(
                      visitor, begin_offset, MemberOffset(-1));
            } else {
              RefsUpdateVisitor</*kCheckBegin*/false, /*kCheckEnd*/false>
                      visitor(this, holder, page, page + kPageSize);
              holder->VisitRefsForCompaction</*kFetchObjSize*/false>(visitor,
                                                                     MemberOffset(0),
                                                                     MemberOffset(-1));
            }
            holder = next_obj;
  ...
  if (reinterpret_cast<uint8_t*>(holder) < page) {
    RefsUpdateVisitor</*kCheckBegin*/true, /*kCheckEnd*/true>
            visitor(this, holder, page, page + kPageSize);
    holder->VisitRefsForCompaction</*kFetchObjSize*/false, /*kVisitNativeRoots*/false>(
            visitor, MemberOffset(page - reinterpret_cast<uint8_t*>(holder)), end_offset);
  } else {
    RefsUpdateVisitor</*kCheckBegin*/false, /*kCheckEnd*/true>
            visitor(this, holder, page, page + kPageSize);
    holder->VisitRefsForCompaction</*kFetchObjSize*/false>(visitor, MemberOffset(0), end_offset);
  }
}
...
void MarkCompact::UpdateMovingSpaceFirstObjects() {
  // For sliding black pages, we need the first-object and live objects info of
  // a given page. Fortunately, we don't need live objects info for every page
  // as all objects are considered black. So we can store of live-bytes
  // chunk size in the pre_compact_offset_moving_space_ array. This should cover
  // majority of pages. The only few pages where a new TLAB starts in the middle
  // of the page we need mark-bit info to find them.
  // So if the first-chunk size is smaller than kPageSize, then we will use the
  // bitmap for the remaining of the pages. Please see SlideBlackPage() for that
  // implementation.
    ...
    for (size_t block_size : *block_sizes) {
      ...
      while (black_allocs < block_end
        ...
        if (page_size + obj_size >= kPageSize) {
          ...
          first_obj = page_size > 0 ? obj : nullptr;
 ...
 public:
  ...
  void Visit(ObjPtr<mirror::DexCache> dex_cache) override
    ...
    if (!dex_cache.IsNull()) {
      ...
      if (dex_caches_visited_.find(cache) == dex_caches_visited_.end()) {
        dex_caches_visited_.insert(cache);
 ...
 private:
  void VisitClassRoots(ObjPtr<mirror::Class> klass)
    ...
    if ((collector_->bump_pointer_space_->HasAddress(klass_ptr)
        ...
        && classes_visited_.find(k) == classes_visited_.end()) {
      classes_visited_.insert(k);
...
template <size_t kAlignment>
size_t MarkCompact::LiveWordsBitmap<kAlignment>::LiveBytesInBitmapWord(size_t vec_idx) const {
  const size_t index = vec_idx * kBitmapWordsPerVectorWord;
  size_t words = 0;
...
void MarkCompact::UpdateLivenessInfo(mirror::Object* obj) {
  ...
  for (size -= first_chunk_portion; size > kOffsetChunkSize; size -= kOffsetChunkSize) {
    DCHECK_EQ(offset_vector_[vec_idx], 0u);
