...
void MarkCompact::PrepareForCompaction() {
  ...
  if (uffd_ != kFallbackMode) {
     ...
     *    use mremap (without donunmap) to expand the size. If failed, then use
     *    copy-mode.
    ...
    if (minor_fault_initialized_) {
      ...
      if (UNLIKELY(!shadow_to_space_map_.IsValid())) {
        ...
        size_t reqd_size = std::min(moving_first_objs_count_ * kPageSize + 4 * MB,
                                    bump_pointer_space_->Capacity());
        // We cannot support memory-tool with shadow-map in this case as the
        // mapping may have to be increased in size using mremap (in
        // KernelPreparation()). MapAnonymous doesn't add a redzone in the end,
        // so use it to reserve the mapping.
        std::string err_msg;
      ...
      if (shadow_to_space_map_.IsValid() &&
        ...
        {
          // memfds are considered as files fomr resource limits point of view.
          // And the moving space could be several hundred MBs. So increase the
          // limit, if it's lower than moving-space size.
          rlimit rlim;
          CHECK_EQ(getrlimit(RLIMIT_FSIZE, &rlim), 0) << "getrlimit failed: " << strerror(errno);
...
void MarkCompact::MapProcessedPages(uint8_t* to_space_start,
  ...
  if (length > 0) {
    ...
    // Therefore, the ioctl must always be called with 'to_space_start' as the
    // 'start' address because it can bail out in the middle (not attempting
    // to map the subsequent pages) if it finds any page either already mapped
    // in between, or missing on the shadow-map.
...
void MarkCompact::DoPageCompactionWithStateChange(size_t page_idx,
  ...
  // to moving_spaces_status_[page_idx] is released before the contents of the page.
  //
...
void MarkCompact::FreeFromSpacePages(size_t cur_page_idx,
  ...
  // freed.
  while (idx > cur_page_idx) {
    idx--;
    PageState state = moving_pages_status_[idx].load(std::memory_order_acquire);
...
void MarkCompact::PreCompactionPhase() {
  ...
  if (kIsDebugBuild) {
    DCHECK_EQ(thread_running_gc_, Thread::Current());
    stack_end_ = thread_running_gc_->GetStackEnd();
    stack_top_ = reinterpret_cast<char*>(stack_end_) + thread_running_gc_->GetStackSize();
  }
...
void MarkCompact::UnregisterUffd(uint8_t* start, size_t len) {
  ...
  // Due to an overlook in the kernel implementation of 'unregister', the
