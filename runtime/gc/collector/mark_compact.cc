...
// using mark bitmap. All this info is prepared in UpdateMovingSpaceFirstObjects().
// It's unlikely to have more than 2 TLABs on a page. Either ways, if we find a
// set bit in the bitmap, then we copy the remaining page and then use the bitmap
// to visit each object for updating references.
void MarkCompact::SlideBlackPage(mirror::Object* first_obj,
  ...
  size_t bytes_copied;
  const uint32_t first_chunk_size = pre_compact_offset_moving_space_[page_idx];
...
void MarkCompact::UpdateMovingSpaceFirstObjects() {
  ...
  // size in pre_compact_offset_moving_space_ array.
  ...
  if (block_sizes != nullptr) {
    ...
    for (size_t block_size : *block_sizes) {
      ...
      bool set_mark_bit = chunk_size > 0;
      ...
      while (black_allocs < block_end
        ...
        // than page size.
        if (chunk_size + obj_size >= kPageSize) {
          set_mark_bit = false;
          first_chunk_size += kPageSize - chunk_size;
          chunk_size += obj_size;
      ...
      DCHECK_LT(chunk_size, kPageSize);
      ...
      if (black_allocs < block_end) {
        ...
        first_obj = nullptr;
        size_t page_remaining = kPageSize - chunk_size;
        ...
        if (page_remaining <= block_remaining) {
          ...
          chunk_size = block_remaining % kPageSize;
        } else {
          chunk_size += block_remaining;
        }
