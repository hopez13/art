  ...
  if (post_fork || uffd_ == kFdUnused) {
    ...
    if (gHaveMremapDontunmap) {
      ...
      if (UNLIKELY(uffd_ == -1)) {
        ...
        if (use_uffd_sigbus_) {
          static_assert((kUffdFeaturesRequired & kUffdFeaturesForSigbus) == kUffdFeaturesForSigbus);
          DCHECK_EQ(gUffdFeatures & kUffdFeaturesRequired, kUffdFeaturesRequired);
          // Request all the required features, which may include minor-fault
          // feature as well.
          api.features = kUffdFeaturesRequired;
