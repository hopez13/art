...
bool MarkCompact::FreeFromSpacePages(size_t cur_page_idx, int mode, size_t end_idx_for_mapping) {
  ...
  if (size > kMinFromSpaceMadviseSize) {
    ...
    if (cur_page_idx < end_idx_for_mapping) {
      ...
      // The wait is not expected to be short as the read state indicates
      // another thread actively working on mapping the page.
    ...
    int behavior = minor_fault_initialized_ ? MADV_REMOVE : MADV_FREE;
...
bool MarkCompact::MapUpdatedLinearAllocPages(uint8_t* start_page,
  ...
  while (start_page < end_page) {
    ...
    if (map_len == 0) {
      ...
      if (free_pages) {
        CHECK_EQ(madvise(start_shadow_page, map_len, MADV_FREE), 0)
            << "madvise of from-space failed: " << strerror(errno);
      }
      if (single_ioctl) {
        break;
