...
size_t MarkCompact::ZeropageIoctl(void* addr, size_t length, bool tolerate_enoent) {
  Thread *self = Thread::Current();
  uint32_t backoff_count = 0;
  uint32_t max_backoff = 10;  // max native priority.
  bool atrace_started = false;
  struct uffdio_zeropage uffd_zeropage;
  ...
  while (length > 0) {
    ...
    if (ret == 0) {
      ...
      if (uffd_zeropage.zeropage > 0) {
        ...
        // is already done, which is was what we care about.
        ...
        if (backoff_count == 0) {
          DCHECK_LE(self->GetNativePriority(), 10);
          max_backoff -= self->GetNativePriority();
        }
        if (backoff_count < max_backoff) {
          BackOff<3>(backoff_count++);
        } else {
...
void MarkCompact::CopyIoctl(void* dst, void* buffer, size_t length) {
  Thread* self = Thread::Current();
  uint32_t backoff_count = 0;
  uint32_t max_backoff = 10;
  bool atrace_started = false;
  auto update_histogram = [&]() {
  ...
  while (length > 0) {
    ...
    if (ret == 0) {
      ...
      if (backoff_count == 0) {
        DCHECK_LE(self->GetNativePriority(), 10);
        max_backoff -= self->GetNativePriority();
        if (!atrace_started && ATraceEnabled()) {
          ATraceBegin(__FUNCTION__);
          atrace_started = true;
        }
      }
      if (uffd_copy.copy > 0) {
  ...
  mmap_lock_is_contended_ = backoff_count > 0;
  update_histogram();
  if (atrace_started) {
    ATraceEnd();
  }
...
bool MarkCompact::FreeFromSpacePages(size_t cur_page_idx, int mode, size_t end_idx_for_mapping) {
  ...
  if (size > kMinFromSpaceMadviseSize) {
    ...
    if (mode == kCopyMode && cur_page_idx < end_idx_for_mapping) {
      if (MapMovingSpacePages(cur_page_idx, end_idx_for_mapping) ==
...
void MarkCompact::CompactMovingSpace(uint8_t* page) {
  ...
  while (idx > 0) {
    ...
    if (kMode == kCopyMode && (!success || page == reserve_page) && end_idx_for_mapping - idx > 1) {
      // map the pages in the following pages as they can't be mapped with the
...
size_t MarkCompact::MapMovingSpacePages(size_t start_idx, size_t arr_len, bool from_fault, bool return_on_contention) {
  DCHECK_LT(arr_idx, arr_len);
  ...
  while (arr_idx < arr_len) {
    ...
    for (size_t i = arr_idx; i < arr_len; i++, map_len++) {
    ...
    if (map_len == 0) {
      ...
      for (; arr_idx < arr_len; arr_idx++) {
        PageState s = static_cast<PageState>(static_cast<uint8_t>(
            moving_pages_status_[arr_idx].load(std::memory_order_acquire)));
      ...
      size_t ioctl_len = CopyIoctl(to_space_start, from_space_start, map_len * gPageSize);
      for (size_t l = 0; l < ioctl_len; l += gPageSize, arr_idx++) {
      ...
      if (from_fault) {
        return DivideByPageSize(ioctl_len);
      }
  ...
  if (wait_for_unmapped) {
    for (size_t i = start_idx; i < arr_len; i++) {
      PageState s = static_cast<PageState>(static_cast<uint8_t>(
          moving_pages_status_[i].load(std::memory_order_acquire)));
      DCHECK_GT(s, PageState::kProcessed);
      uint32_t backoff_count = 0;
      while (s != PageState::kProcessedAndMapped) {
        BackOff(backoff_count++);
        s = static_cast<PageState>(static_cast<uint8_t>(
            moving_pages_status_[i].load(std::memory_order_acquire)));
      }
    }
  }
  ...
  if (first_obj == nullptr) {
    ...
    for (size_t idx = page_idx; idx < end_idx; idx++, length += gPageSize) {
      ...
      if (cur_state != static_cast<uint8_t>(PageState::kUnprocessed)) {
        DCHECK_EQ(expected_state, static_cast<uint8_t>(PageState::kProcessedAndMapped));
        break;
      }
    }
    if (length > 0) {
      length = ZeropageIoctl(fault_page, length, /*tolerate_enoent=*/true);
  ...
  while (true) {
    ...
    if (state == PageState::kProcessing || state == PageState::kMutatorProcessing ||
      ...
      if (state == PageState::kProcessed) {
        ...
        if (MapMovingSpacePages(page_idx, arr_len) > 0) {
          break;
        }
        raw_state = moving_pages_status_[page_idx].load(std::memory_order_acquire);
      }
...
bool MarkCompact::MapUpdatedLinearAllocPages(uint8_t* start_page,
  ...
  while (start_page < end_page) {
    ...
    if (map_len == 0) {
      ...
      map_len = CopyIoctl(start_page, start_shadow_page, map_len);
      if (use_uffd_sigbus_) {
    ...
    while (true) {
      switch (state) {
        ...
        case PageState::kProcessed: {
          size_t arr_len = space_data->end_ - fault_page;
