...
void MarkCompact::CompactPage(mirror::Object* obj, const uint32_t offset, uint8_t* addr) {
  ...
  // TODO: assert that bit corresponding to first offset is set.
  ...
  if (stride_count > 1) {
    ...
    // We can't call VisitRefsForCompaction on the last object if it is
    // smaller than Array/string size as that could trigger page fault on next
    // page for computing object size. But, in that case we only need to
    // update the klass. So explicitly do that.
    ...
    if (addr < end_addr) {
      ...
      RefsUpdateVisitor</*kCheckBegin*/false, /*kCheckEnd*/false>
              visitor(this, ref, nullptr, start_addr + kPageSize);
      visitor(ref, mirror::Object::ClassOffset(), /*is_static*/ false);
    }
  }
}
...
void MarkCompact::CompactMovingSpace() {
  ...
  for (size_t i = 0; i < moving_first_objs_count_; i++) {
    CompactPage(first_objs_moving_space_[i], pre_compact_offset_moving_space_[i], begin);
    begin += kPageSize;
  }
}
