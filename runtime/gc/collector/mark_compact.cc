...
bool ShouldUseUserfaultfd() {
  ...
  // together.
  CheckKernelVersion([&lower_kernel](int major, int minor) {
                       static constexpr int kRequiredMajor = 5;
                       static constexpr int kRequiredMinor = 13;
...
static constexpr size_t kPageFaultThreadCount = 2;

bool MarkCompact::CreateUserfaultfd() {
  if (uffd_ == -1) {
...
MarkCompact::MarkCompact(Heap* heap)
  ...
  const size_t num_pages = 1 + std::max(kPageFaultThreadCount,
                                        std::max(heap_->GetParallelGCThreadCount(),
...
void MarkCompact::PrepareForCompaction() {
  ...
  if (!uffd_initialized_) {
    ...
    // Set to true even if we are in fallback mode so that we don't retry this.
    uffd_initialized_ = true;
  }
  ...
  if (UNLIKELY(pool == nullptr)) {
    heap_->CreateThreadPool(kPageFaultThreadCount);
...
void MarkCompact::CompactMovingSpace(uint8_t* page) {
  ...
  // Relaxed memory-order is using as the subsequent ioctl syscall will
  // act as a fence.
  size_t idx = 0;
...
void MarkCompact::ConcurrentCompaction(uint8_t* page) {
  ...
  while (true) {
    ...
    if (state == PageState::kUncompacted) {
      ...
      // it to be flushed before this thread attempts to copy/zeropage the
      // fault_page.
...
void MarkCompact::CompactionPhase() {
  ...
  // untouched during compaction and hence is already unmapped (in FinishPhase).
