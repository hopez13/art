...
class MarkCompact::LinearAllocPageUpdater {
 public:
  ...
  bool WasPageTouched() const { return page_touched_; }
 ...
 private:
  ...
  MarkCompact* const collector_;
  // Whether the last updated page was touched or not.
  bool page_touched_;
};
            ...
            if (kMode == kCopyMode || is_minor_fault) {
              ...
              if (kMode == kCopyMode) {
                if (updater.WasPageTouched()) {
                  ...
                  // in the kernel.
                  ZeropageIoctl(fault_page, /*tolerate_eexist=*/false, /*tolerate_enoent=*/false);
                }
                if (use_uffd_sigbus_) {
      ...
      if (state_arr[page_idx].compare_exchange_strong(
        ...
        if (!minor_fault_initialized_) {
          if (updater.WasPageTouched()) {
            ...
            // correct but also efficient as it avoids unnecessary memcpy
            // in the kernel.
            ZeropageIoctl(page_begin, /*tolerate_eexist=*/false, /*tolerate_enoent=*/false);
          }
          if (use_uffd_sigbus_) {
...
void MarkCompact::CompactionPhase() {
  ...
  // uffd worker thread) starts processing a moving-space page after this will
