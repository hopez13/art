...
static constexpr ssize_t kMinFromSpaceMadviseSize = 4 * MB;
...
void MarkCompact::CompactMovingSpace(uint8_t* page) {
  ...
  MapMovingSpacePages(idx, page_status_arr_len);
...
size_t MarkCompact::MapMovingSpacePages(size_t arr_idx, size_t arr_len) {
  ...
  for (size_t i = arr_idx; i < arr_len; length += page_size, i++) {
    ...
    CHECK_EQ(from_space_idx++, cur_state >> kPageStateShift);
  }
  if (length > 0) {
...
void MarkCompact::MapUpdatedLinearAllocPage(uint8_t* page,
  ...
  DCHECK(!minor_fault_initialized_);
  if (page_touched) {
    CopyIoctl(page, shadow_page, gPageSize);
  } else {
