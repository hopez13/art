...
void MarkCompact::CompactionPhase() {
  ...
  if (minor_fault_initialized_) {
    if (IsValidFd(moving_from_space_fd_)) {
      ...
      // page isn't missing. A possible reason for this could be h/w prefetch. The
      ...
      // We don't have a valid fd, so only option to reclaim page is to use madvise(MADV_REMOVE).
...
void MarkCompact::FinishPhase() {
  ...
  if (IsValidFd(moving_to_space_fd_)) {
    ...
    struct stat buf;
    DCHECK_EQ(fstat(moving_to_space_fd_, &buf), 0);
    DCHECK_EQ(buf.st_blocks, 0);
  }
}
