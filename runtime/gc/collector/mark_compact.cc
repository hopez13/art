...
void MarkCompact::CompactionPhase() {
  ...
  SigbusCounterType sigbus_count = sigbus_in_progress_count_[0].fetch_or(
      kSigbusCounterCompactionDoneMask, std::memory_order_acq_rel);
  // Wait for SIGBUS handlers already in play.
  for (uint32_t i = 0; sigbus_count > 0; i++) {
    BackOff(i);
    sigbus_count = sigbus_in_progress_count_[0].load(std::memory_order_acquire);
    sigbus_count &= ~kSigbusCounterCompactionDoneMask;
  }
  ...
  sigbus_count = sigbus_in_progress_count_[1].fetch_or(kSigbusCounterCompactionDoneMask,
                                                       std::memory_order_acq_rel);
  // Wait for SIGBUS handlers already in play.
  for (uint32_t i = 0; sigbus_count > 0; i++) {
    BackOff(i);
    sigbus_count = sigbus_in_progress_count_[1].load(std::memory_order_acquire);
    sigbus_count &= ~kSigbusCounterCompactionDoneMask;
  }
