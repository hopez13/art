...
// latter was introduced earlier than the former.
...
// phase.
static constexpr size_t kMutatorCompactionBufferCount = 1024;
...
void MarkCompact::MapProcessedPages(uint8_t* to_space_start,
  ...
  if (length > 0) {
    ...
    if (use_uffd_sigbus_) {
      ...
      // store is sufficient. Relaxed order is sufficient as this happens
  ...
  class ScopedInProgressCount {
   public:
    explicit ScopedInProgressCount(MarkCompact* collector) : collector_(collector) {
      // Sequentially consistent order is used wrt. compacting_ which is read
      // after this constructor below.
