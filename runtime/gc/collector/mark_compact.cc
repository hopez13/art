...
static gc::CollectorType FetchCmdlineProcessAndGcType() {
  std::string argv;
...
#ifdef __linux__
  gc::CollectorType gc_type = FetchCmdlineProcessAndGcType();
  // Use CMC/CC if that is being explicitly asked for on cmdline. Otherwise,
  // always use CC on host.
  if (gc_type == gc::CollectorType::kCollectorTypeCMC) {
    return true;
  } else if (gc_type == gc::CollectorType::kCollectorTypeCC) {
    return false;
  } else if (kIsTargetAndroid && SysPropSaysUffdGc() && KernelSupportsUffd()) {
    return true;
  }
#endif
  return false;
