...
class ConcurrentCopying::ComputeLiveBytesAndMarkRefFieldsVisitor {
 public:
  explicit ComputeLiveBytesAndMarkRefFieldsVisitor(ConcurrentCopying* collector,
      ...
      : collector_(collector),
      verification_(collector_->GetHeap()->GetVerification()),
 ...
 private:
  void SanityCheck(mirror::Object* obj, MemberOffset offset, mirror::Object* ref) const
      ALWAYS_INLINE
      REQUIRES_SHARED(Locks::mutator_lock_) {
    bool valid_ref = (offset == mirror::Object::ClassOffset()
        ? verification_->IsValidClass(ref) : verification_->IsValidHeapObjectAddress(ref));
    if (valid_ref) {
      verification_->LogHeapCorruption(obj, offset, ref, /*fatal*/ true);
    }
  }
  ...
  ConcurrentCopying* const collector_;
  Verification const * const verification_;
