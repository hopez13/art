...
void ConcurrentCopying::ReclaimPhase() {
...

  {
    ...
    if (from_bytes > 0) {
      ...
      float old_v = copied_live_bytes_ratio_sum_.load(std::memory_order_relaxed);
      float new_v = old_v + static_cast<float>(to_bytes) / from_bytes;
      auto success = copied_live_bytes_ratio_sum_.compare_exchange_strong(
...
void ConcurrentCopying::DumpPerformanceInfo(std::ostream& os) {
  ...
  float average_ratio = copied_live_bytes_ratio_sum_.load(std::memory_order_relaxed)
