...
void ConcurrentCopying::InitializePhase() {
  ...
  done_scanning_.StoreRelaxed(false);
  BindBitmaps();
  if (kVerboseMode) {
...
class ConcurrentCopying::FlipCallback : public Closure {
 public:
  ...
  virtual void Run(Thread* thread) OVERRIDE REQUIRES(Locks::mutator_lock_) {
    ...
    cc->region_space_->SetFromSpace(cc->rb_table_, evac_mode, /*clear_live_bytes*/ !cc->young_gen_);
    cc->SwapStacks();
    if (ConcurrentCopying::kEnableFromSpaceAccountingCheck) {
...
void ConcurrentCopying::MarkingPhase() {
  ...
  if (young_gen_) {
    ...
    CHECK_EQ(done_scanning_.LoadRelaxed(), false);
    if (kIsDebugBuild) {
          ...
          [this, space](mirror::Object* obj)
            ...
            if (kIsDebugBuild && space == region_space_) {
              if (!region_space_->IsInUnevacFromSpace(obj)) {
                region_space_->DumpRegionForObject(LOG(INTERNAL_FATAL), obj);
                LOG(FATAL) << "Scanning " << obj << " in unevac region "
                           << region_space_bitmap_->Test(obj);
              }
            }
            Scan<true>(obj);
          },
    ...
    done_scanning_.StoreSequentiallyConsistent(true);
  }
  {
    // This phase needs to be after the card scanning since the mutator may read an unevac space
    // object out of an image object. If the image object is no longer gray it will trigger a read
    // barrier for the unevac space object.
...
class ConcurrentCopying::VerifyNoFromSpaceRefsVisitor : public SingleRootVisitor {
 public:
  ...
  void operator()(mirror::Object* obj, MemberOffset offset, mirror::Object* ref) const
      SHARED_REQUIRES(Locks::mutator_lock_) ALWAYS_INLINE {
    if (ref == nullptr) {
      // OK.
      return;
    }
    collector_->AssertToSpaceInvariant(obj, offset, ref);
    if (kUseBakerReadBarrier) {
  ...
  void operator()(mirror::Object* ref) const
      SHARED_REQUIRES(Locks::mutator_lock_) ALWAYS_INLINE {
    operator()(nullptr, MemberOffset(0), ref);
  }
  ...
  if (is_asserting_to_space_invariant_) {
    if (region_space_->IsInToSpace(ref)) {
      ...
      if (!IsMarkedInUnevacFromSpace(ref)) {
        if (obj != nullptr) {
          LOG(ERROR) << obj->GetMarkBit() << " " << obj->GetReadBarrierPointer();
        }
        if (ref != nullptr) {
          LOG(ERROR) << ref->GetMarkBit() << " " << ref->GetReadBarrierPointer();
        }
        Thread::Current()->DumpJavaStack(LOG(ERROR));
        LOG(FATAL) << obj << " " << ref;
      }
      ...
      if (obj != nullptr) {
        ...
        LOG(ERROR) << "UNEVAC " << region_space_->IsInUnevacFromSpace(obj) << " " << obj << " " << obj->GetMarkBit();
        if (region_space_->HasAddress(obj)) {
          region_space_->DumpRegionForObject(LOG(ERROR), obj);
        }
      ...
      LOG(ERROR) << "REF= " << ref << " UNEVAC " << region_space_->IsInUnevacFromSpace(ref) << " " << ref->GetMarkBit();
      ref->GetLockWord(false).Dump(LOG(INTERNAL_FATAL));
      region_space_->DumpRegionForObject(LOG(ERROR), ref);
...
inline void ConcurrentCopying::Process(mirror::Object* obj, MemberOffset offset) {
  ...
  mirror::Object* to_ref = Mark</*kGrayImmuneObject*/false, kNoUnEvac, /*kFromGCThread*/true>(ref);
  if (to_ref == ref) {
    return;
  }
...
void ConcurrentCopying::DelayReferenceReferent(mirror::Class* klass, mirror::Reference* reference) {
  heap_->GetReferenceProcessor()->DelayReferenceReferent(
      klass,
      Mark(reference)->AsReference<kVerifyNone, kWithoutReadBarrier>(),
      this);
}
...
mirror::Object* ConcurrentCopying::MarkFromReadBarrierWithMeasurements(mirror::Object* from_ref) {
  ...
  mirror::Object* ret =
      Mark</*kGrayImmuneObject*/true, /*kNoUnEvac*/false, /*kFromGCThread*/false>(from_ref);
  if (measure_read_barrier_slow_path_) {
