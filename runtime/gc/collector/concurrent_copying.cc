...
void ConcurrentCopying::ReclaimPhase() {
...

  {
    ...
    max_peak_num_non_free_regions_ = std::max(max_peak_num_non_free_regions_, region_space_->GetNumNonFreeRegions());
    if (kEnableFromSpaceAccountingCheck) {
...
void ConcurrentCopying::DumpPerformanceInfo(std::ostream& os) {
  ...
  os << "Peak heap usage " << PrettySize(max_peak_num_non_free_regions_ * space::RegionSpace::kRegionSize) << "\n";
}
