...
void ConcurrentCopying::AssertThreadMarkStackMappingEmpty() {
  if (!thread_mark_stack_map_.empty()) {
    LOG(FATAL_WITHOUT_ABORT) << "thread_mark_stack_map not empty. size:" << thread_mark_stack_map_.size()
                             << " Mappings:";
...
void ConcurrentCopying::ProcessMarkStackForMarkingAndComputeLiveBytes() {
                               ...
                               });
  {
    ...
    AssertThreadMarkStackMappingEmpty();
