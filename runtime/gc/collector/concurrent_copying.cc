...
class ConcurrentCopying::ComputeLiveBytesAndMarkRefFieldsVisitor {
 public:
  ...
  void operator()(mirror::Object* obj, MemberOffset offset, bool /* is_static */) const
    ...
    mirror::Object* ref = obj->GetFieldObject<mirror::Object, kVerfiyNone, kWithoutReadBarrier>(offset);
 ...
 private:
  void SanityCheck(mirror::Object* obj, MemberOffset offset, mirror::Object* ref) {
    bool valid_ref = (offset == obj->ClassOffset())
        ? collector_->GetHeap()->GetVerification()->IsValidClass(ref)
        : collector_->GetHeap()->GetVerification()->IsValidHeapObjectAddress(ref);
    if (valid_ref) {
      collector_->GetHeap()->LogHeapCoruption(obj, offset, ref, /*fatal*/ true);
    }
  }
...
void ConcurrentCopying::AddLiveBytesAndScanRef(mirror::Object* ref) {
  ...
  if (LIKELY(region_space_->HasAddress(ref))) {
    ...
    if (!region_space_->IsRegionNewlyAllocated(obj_region_idx)) {
      ...
      CHECK(klass != nullptr && heap_->GetVerification()->IsValidClass(klass))
            << heap_->GetVerification()->DumpObjectInfo(ref, "Invalid class for obj");
