...
class ConcurrentCopying::VerifyNoMissingCardMarkVisitor {
 public:
  ...
  void operator()(ObjPtr<mirror::Object> obj,
      ...
      REQUIRES_SHARED(Locks::mutator_lock_) ALWAYS_INLINE {
    // The class does not have the same invariant since we do not mark the card of the object when
    // we store the class during allocation.
...
void ConcurrentCopying::VerifyNoMissingCardMarkCallback(mirror::Object* obj, void* arg) {
  ...
  if (!collector->heap_->GetCardTable()->IsDirty(obj)) {
    ...
    // Visit native roots since most of them are only marked from the holder. We need
    // to have dirty cards when these are modified so that GCs that use the card table
    // know to gray/scan the holder.
