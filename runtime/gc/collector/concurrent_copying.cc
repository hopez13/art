...
class ConcurrentCopying::DisableMarkingCallback : public Closure {
 public:
  ...
  void Run(Thread* self ATTRIBUTE_UNUSED) OVERRIDE {
    // This needs to run under a thread list lock critical section to avoid a race with
    // ThreadList::Register().
...
class ConcurrentCopying::DisableWeakRefAccessCallback : public Closure {
 public:
  ...
  void Run(Thread* self ATTRIBUTE_UNUSED) OVERRIDE {
    // This needs to run under a thread list lock critical section to avoid a deadlock b/31500969.
    CHECK(concurrent_copying_->weak_ref_access_enabled_.LoadRelaxed());
    concurrent_copying_->weak_ref_access_enabled_.StoreRelaxed(false);
  }
