...
class ConcurrentCopying::FlipCallback : public Closure {
 public:
  ...
  virtual void Run(Thread* thread) OVERRIDE REQUIRES(Locks::mutator_lock_) {
    ...
    cc->is_marking_ = true;
    cc->mark_stack_mode_.store(ConcurrentCopying::kMarkStackModeThreadLocal, std::memory_order_relaxed);
    if (kIsDebugBuild) {
...
inline void ConcurrentCopying::VisitRoots(
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    } while (!addr->compare_exchange_weak(expected_ref, new_ref, std::memory_order_relaxed));
  }
}
    ...
    if (to_ref != nullptr) {
      ...
      if (kVerboseMode) {
        LOG(INFO) << "Out of memory in the to-space. Fall back to non-moving. skipped_bytes="
                  ...
                  << " skipped_objects=" << to_space_objects_skipped_.load(std::memory_order_seq_cst);
      }
      fall_back_to_non_moving = true;
...
void ConcurrentCopying::FinishPhase() {
  ...
  if (measure_read_barrier_slow_path_) {
    ...
    rb_slow_path_time_histogram_.AdjustAndAddValue(rb_slow_path_ns_.load(std::memory_order_relaxed));
