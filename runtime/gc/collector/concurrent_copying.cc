...
ConcurrentCopying::ConcurrentCopying(Heap* heap,
  ...
  if (young_gen_) {
    ...
    gc_work_throughput_hist_ = metrics->YoungGcWorkThroughput();
  } else {
    ...
    gc_work_throughput_hist_ = metrics->FullGcWorkThroughput();
  }
}
...
inline void ConcurrentCopying::ProcessMarkStackRef(mirror::Object* to_ref) {
  ...
  if (add_to_live_bytes) {
    ...
    obj_size = RoundUp(obj_size, space::RegionSpace::kAlignment);
    region_space_->AddLiveBytes(to_ref, obj_size);
  }
