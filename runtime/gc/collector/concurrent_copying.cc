...
void ConcurrentCopying::ReclaimPhase() {
...

  {
    ...
    // adjusted_from_bytes and adjusted_from_objects may be greater than the from space equivalents
    // since ClearFromSpace may clear empty unevac regions.
    uint64_t adjusted_from_bytes;
    uint64_t adjusted_from_objects;
    {
      TimingLogger::ScopedTiming split4("ClearFromSpace", GetTimings());
      region_space_->ClearFromSpace(&adjusted_from_bytes, &adjusted_from_objects);
    }
    int64_t freed_bytes = adjusted_from_bytes - to_bytes;
    int64_t freed_objects = adjusted_from_objects - to_objects;
    if (kVerboseMode) {
      LOG(INFO) << "RecordFree:"
