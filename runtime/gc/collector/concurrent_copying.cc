      ...
      if (use_generational_cc_) {
        if (space == region_space_) {
          if (young_gen_) {
            ...
            // Newly allocated regions anyway doesnt need to be cleared, with an exception of Large objects in some cases
            region_space_->ClearBitmap<space::RegionSpace::RegionType::kRegionTypeAll,
                                        space::RegionSpace::RegionAgeFlags::kRegionAgeFlagAgedOrNewlyAllocated> ();
          } else {
            region_space_->ClearBitmap<space::RegionSpace::RegionType::kRegionTypeAll,
                                        space::RegionSpace::RegionAgeFlags::kRegionAgeFlagAll> ();
          }
...
inline void ConcurrentCopying::ScanImmuneObject(mirror::Object* obj) {
  ...
  if (use_generational_cc_ && young_gen_) {
    // Young GC does not care about references to unevac space that is tenured. It is safe to not gray these as
    // long as scan immune objects happens after scanning the dirty cards.
    Scan<RegionTypeFlags::kIgnoreUnevacFromSpaceTenured, false>(obj);
  } else {
    Scan<RegionTypeFlags::kIgnoreNone, false>(obj);
  }
}
...
template <ConcurrentCopying::RegionTypeFlags kRegionTypeFlag>
void ConcurrentCopying::ScanDirtyObject(mirror::Object* obj) {
  Scan<kRegionTypeFlag, false>(obj);
...
class ConcurrentCopying::RegionSpaceRemSetObjVisitor : public accounting::RememberedSetObjectVisitor {
 public:
  explicit RegionSpaceRemSetObjVisitor(ConcurrentCopying* cc) : cc_(cc) { }

  void operator()(ObjPtr<mirror::Object> obj) const REQUIRES(Locks::heap_bitmap_lock_)
    REQUIRES_SHARED(Locks::mutator_lock_) {
      bool contains_ref_to_young_regions = false;
      cc_->Scan<ConcurrentCopying::RegionTypeFlags::kIgnoreUnevacFromSpaceTenured, true>(obj.Ptr(), &contains_ref_to_young_regions);
...
void ConcurrentCopying::CopyingPhase() {
  ...
  if (use_generational_cc_) {
    ...
    for (space::ContinuousSpace* space : GetHeap()->GetContinuousSpaces()) {
      ...
      if (young_gen_) {
        RegionSpaceRemSetObjVisitor region_space_rem_set_obj_visitor(this);
        rem_set->UpdateAndMarkReferences(region_space_/*target_space*/,
                                          this/*collector*/,
                                          &region_space_rem_set_obj_visitor);
      } else {
        ...
        card_table->Scan<false>(
            ...
            [this, space](mirror::Object* obj)
              ...
              if (space != region_space_) {
                ...
                ScanDirtyObject</*kRegionTypeFlag*/ RegionTypeFlags::kIgnoreNone>(obj);
        ...
        auto visitor = [this](mirror::Object* obj) REQUIRES_SHARED(Locks::mutator_lock_) {
                         ...
                         ScanDirtyObject</*kRegionTypeFlag*/ RegionTypeFlags::kIgnoreUnevacFromSpace>(obj);
                       };
        if (space == region_space_) {
...
inline void ConcurrentCopying::ProcessMarkStackRef(mirror::Object* to_ref) {
  ...
  switch (rtype) {
    case space::RegionSpace::RegionType::kRegionTypeUnevacFromSpace:
      ...
      if (!kUseBakerReadBarrier || !region_space_bitmap_->Set(to_ref)) {
        ...
        if (use_generational_cc_ && young_gen_ && region_space_->IsLargeObject(to_ref)) {
  ...
  if (perform_scan) {
    if (use_generational_cc_ && young_gen_) {
      Scan<RegionTypeFlags::kIgnoreUnevacFromSpaceTenured, false>(to_ref);
    } else {
      Scan<RegionTypeFlags::kIgnoreNone, false>(to_ref);
    }
  }
  if (kUseBakerReadBarrier) {
    DCHECK(to_ref->GetReadBarrierState() == ReadBarrier::GrayState())
...
// Used to scan ref fields of an object.
template <ConcurrentCopying::RegionTypeFlags kRegionTypeFlag , bool kRememberIfAnyRegsToYoungRegions>
class ConcurrentCopying::RefFieldsVisitor {
 public:
  explicit RefFieldsVisitor(ConcurrentCopying* collector, Thread* const thread,
                            bool* const contains_reference_to_young_regions = nullptr)
      : collector_(collector), thread_(thread),
        contains_reference_to_young_regions_(contains_reference_to_young_regions) {
    // Cannot have any other flags except 'kIgnoreNone' when Generational CC collection is disabled.
    DCHECK((kRegionTypeFlag == RegionTypeFlags::kIgnoreNone) || collector_->use_generational_cc_);
    CHECK(!kRememberIfAnyRegsToYoungRegions || (collector_->use_generational_cc_ && contains_reference_to_young_regions != nullptr));
  }
  ...
  void operator()(mirror::Object* obj, MemberOffset offset, bool /* is_static */)
    ...
    if (kRememberIfAnyRegsToYoungRegions) {
 ...
 private:
  inline void RememberRefsToYoungRegions(mirror::Object* obj) const {
    ...
    if (collector_->RegionSpace()->HasAddress(obj) &&
        !collector_->RegionSpace()->IsTenured(obj)) {
      *contains_reference_to_young_regions_ = true;
    }
  }

  ConcurrentCopying* const collector_;
  Thread* const thread_;
  bool* const contains_reference_to_young_regions_;
};

template <ConcurrentCopying::RegionTypeFlags kRegionTypeFlag, bool kRememberIfAnyRegsToYoungRegions>
inline void ConcurrentCopying::Scan(mirror::Object* to_ref,
                                    bool* const contains_reference_to_young_regions) {
  bool has_refs_to_aged_regions = false;
  // Cannot have any other flags except 'kIgnoreNone' when Generational CC collection is disabled.
  DCHECK((kRegionTypeFlag == RegionTypeFlags::kIgnoreNone) || use_generational_cc_);
  DCHECK(!kRememberIfAnyRegsToYoungRegions || (use_generational_cc_ && contains_reference_to_young_regions != nullptr));
  ...
  if (region_space_->IsTenuring(to_ref)) {
    DCHECK(use_generational_cc_);
    ...
    RefFieldsVisitor<kRegionTypeFlag, kRememberIfAnyRegsToYoungRegions> visitor(this, thread_running_gc_,
                                                                            &has_refs_to_aged_regions);
    // Disable the read barrier for a performance reason.
    to_ref->VisitReferences</*kVisitNativeRoots*/true, kDefaultVerifyFlags, kWithoutReadBarrier>(
        visitor, visitor);
  ...
  if (kRememberIfAnyRegsToYoungRegions) {
    *contains_reference_to_young_regions = has_refs_to_aged_regions;
  }
...
inline void ConcurrentCopying::Process(mirror::Object* obj, MemberOffset offset) {
  ...
  mirror::Object* to_ref = Mark</*kGrayImmuneObject=*/false, /*kRegionTypeFlag=*/kRegionTypeFlag, /*kFromGCThread=*/true>(
      thread_running_gc_,
      ref,
      /*holder=*/ obj,
      offset);
...
mirror::Object* ConcurrentCopying::AllocateInSkippedBlock(Thread* const self, size_t alloc_size, uint8_t age) {
  ...
  CHECK(age_idx < kAgeSkipBlockMapSize);
  std::multimap<size_t, uint8_t*>* skipped_blocks_map = &age_skipped_blocks_map_[age_idx];
    ...
    if (old_lock_word.GetState() == LockWord::kForwardingAddress) {
      ...
      if (!fall_back_to_non_moving) {
        ...
        if (bytes_allocated > space::RegionSpace::kRegionSize) {
          ...
          MutexLock mu(self, skipped_blocks_lock_);
          uint8_t age_idx = age + 1;
          age_skipped_blocks_map_[age_idx].insert(std::make_pair(bytes_allocated,
...
mirror::Object* ConcurrentCopying::MarkFromReadBarrierWithMeasurements(Thread* const self,
  ...
  mirror::Object* ret =
      Mark</*kGrayImmuneObject=*/true, /*kRegionTypeFlag=*/RegionTypeFlags::kIgnoreNone, /*kFromGCThread=*/false>(self,
                                                                                     from_ref);
  if (measure_read_barrier_slow_path_) {
...
void ConcurrentCopying::AddDirtyCard(mirror::Object* ref) {
  ...
  DCHECK(region_space_->IsTenuring(ref)) << ref;
...
void ConcurrentCopying::ClearDirtyCards(accounting::RememberedSet* rem_set) {
  ...
  if (rem_set->GetSpace() == region_space_) {
    ...
    card_table->ModifyCardsAtomic(region_space_->Begin(),
                                  region_space_->End(),
                                  AgeCardVisitor(),
                                  [this, card_table, rem_set](uint8_t* card, uint8_t expected_value, uint8_t new_value ATTRIBUTE_UNUSED) {
