...
ConcurrentCopying::ConcurrentCopying(Heap* heap,
    ...
    : GarbageCollector(heap,
      ...
      is_marking_(false),
      use_read_barrier_entrypoints_(false),
      is_active_(false),
...
void ConcurrentCopying::RunPhases() {
  ...
  if (kUseBakerReadBarrier && kGrayDirtyImmuneObjects) {
    // Switch to read barrier ark entrypoints before we gray the objects. This is required in case
    // a mutator sees a gray bit and dispatches on the entrpoint. (b/37876887).
    SwitchToReadBarrierEntrypoints();
...
class ConcurrentCopying::SwitchToReadBarrierEntrypointsCheckpoint : public Closure {
 public:
  explicit SwitchToReadBarrierEntrypointsCheckpoint(ConcurrentCopying* concurrent_copying)
  ...
  void Run(Thread* thread) OVERRIDE NO_THREAD_SAFETY_ANALYSIS {
    ...
    // Switch to the read
    thread->SwitchToReadBarrierEntrypoints();
...
class ConcurrentCopying::SwitchToReadBarrierEntrypointsCallback : public Closure {
 public:
  explicit SwitchToReadBarrierEntrypointsCallback(ConcurrentCopying* concurrent_copying)
      : concurrent_copying_(concurrent_copying) {}
  ...
  void Run(Thread* self ATTRIBUTE_UNUSED) OVERRIDE REQUIRES(Locks::thread_list_lock_) {
    ...
    CHECK(concurrent_copying_->use_read_barrier_entrypoints_);
    concurrent_copying_->use_read_barrier_entrypoints_ = false;
  }
...
void ConcurrentCopying::SwitchToReadBarrierEntrypoints() {
  Thread* const self = Thread::Current();
  SwitchToReadBarrierEntrypointsCheckpoint checkpoint(this);
  ThreadList* thread_list = Runtime::Current()->GetThreadList();
  gc_barrier_->Init(self, 0);
  SwitchToReadBarrierEntrypointsCallback callback(this);
