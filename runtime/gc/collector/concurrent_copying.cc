...
void ConcurrentCopying::ReclaimPhase() {
...

  {
    ...
    if (from_bytes > 0) {
      auto live_ratio = static_cast<float>(to_bytes)/from_bytes;
      auto sum = copy_live_ratio_average_ * gc_count_;
      gc_count_++;
      copy_live_ratio_average_ = (sum + live_ratio) / gc_count_;
    }
...
void ConcurrentCopying::DumpPerformanceInfo(std::ostream& os) {
  ...
  if (young_gen_) {
    os << "Average minor GC live ratio " << copy_live_ratio_average_
       << " over " << gc_count_ << " minor GC\n";
  } else {
    os << "Average major GC live ratio " << copy_live_ratio_average_
