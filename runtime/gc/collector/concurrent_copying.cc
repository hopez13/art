    ...
    if (kUseBakerReadBarrier) {
      ...
      if (is_los) {
        if (!IsAligned<kPageSize>(to_ref)) {
          ...
          heap_->GetVerification()->LogHeapCorruption(nullptr,
                                                      MemberOffset(0),
                                                      to_ref,
                                                      /* fatal= */ true);
        }
...
bool ConcurrentCopying::IsMarkedInNonMovingSpace(mirror::Object* from_ref) {
  ...
  if (kUseBakerReadBarrier) {
    is_marked = from_ref->GetReadBarrierState() == ReadBarrier::GrayState();
  }
  if ((!kEnableGenerationalConcurrentCopyingCollection
       || !young_gen_
    ...
    is_marked = is_los ? los_bitmap->Test(from_ref) : mark_bitmap->Test(from_ref);
  }
  return is_marked || IsOnAllocStack(from_ref);
}
