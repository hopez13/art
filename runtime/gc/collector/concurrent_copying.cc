...
void ConcurrentCopying::ReclaimPhase() {
...

  {
    ...
    uint64_t to_bytes = bytes_moved_.load(std::memory_order_seq_cst) + bytes_moved_gc_;
    cumulative_bytes_moved_.fetch_add(to_bytes, std::memory_order_relaxed);
    uint64_t to_objects = objects_moved_.load(std::memory_order_seq_cst) + objects_moved_gc_;
