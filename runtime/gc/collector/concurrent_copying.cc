...
class ConcurrentCopying::ActivateReadBarrierEntrypointsCallback : public Closure {
 public:
  ...
  void Run(Thread* self [[maybe_unused]]) override REQUIRES(Locks::thread_list_lock_) {
...
class ConcurrentCopying::AssertToSpaceInvariantFieldVisitor {
 public:
  ...
  void operator()(ObjPtr<mirror::Class> klass, ObjPtr<mirror::Reference> ref [[maybe_unused]]) const
      REQUIRES_SHARED(Locks::mutator_lock_) ALWAYS_INLINE {
    CHECK(klass->IsTypeOfReferenceClass());
  }
