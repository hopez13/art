...
inline bool MarkCompact::VerifyRootSingleUpdate(void* root,
                                                ...
                                                const RootInfo& info) {
  // With memory-tool enabled the stack addresses are not set as expected and
  // therefore ends up reporting false-negative double-root update bugs.
  if (kIsDebugBuild && !kMemoryToolIsAvailable) {
    ...
    if (root < stack_addr || root > stack_end) {
      ...
      DCHECK(ret.second) << "root=" << root << " old_ref=" << old_ref
                         << " stack_addr=" << stack_addr << " stack_end=" << stack_end;
    }
