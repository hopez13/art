...
inline bool MarkCompact::VerifyRootSingleUpdate(void* root,
  ...
  if (kIsDebugBuild && !kMemoryToolIsAvailable) {
    ...
    void* stack_top = stack_top_;
    if (!live_words_bitmap_->HasAddress(old_ref)) {
      return false;
    }
    if (UNLIKELY(stack_end == nullptr)) {
      Thread* self = Thread::Current();
      stack_end = self->GetStackEnd();
      stack_top = reinterpret_cast<char*>(stack_end) + self->GetStackSize();
    }
    if (root < stack_end || root > stack_top) {
      auto ret = updated_roots_.insert(root);
      DCHECK(ret.second) << "root=" << root << " old_ref=" << old_ref << " stack_end=" << stack_end
                         << " stack_top=" << stack_top;
    }
