...
inline uintptr_t MarkCompact::LiveWordsBitmap<kAlignment>::SetLiveWords(uintptr_t begin,
  ...
  uintptr_t* address = Bitmap::Begin() + Bitmap::BitIndexToWordIndex(begin_bit_idx);
  uintptr_t* const end_address = Bitmap::Begin() + Bitmap::BitIndexToWordIndex(end_bit_idx);
...
inline void MarkCompact::LiveWordsBitmap<kAlignment>::VisitLiveStrides(uintptr_t begin_bit_idx,
  ...
  // Setup the firt word.
  size_t idx_in_word = begin_bit_idx % Bitmap::kBitsPerBitmapWord;
  begin_bit_idx -= idx_in_word;
  word >>= idx_in_word;
  if (UNLIKELY(begin_word_idx == Bitmap::BitIndexToWordIndex(end_bit_idx))) {
    word &= Bitmap::BitIndexToMask(end_bit_idx) - 1;
  }
  DCHECK_EQ(begin_bit_idx % Bitmap::kBitsPerBitmapWord, 0u);
  ...
  do {
    ...
    DCHECK_EQ(begin_bit_idx % Bitmap::kBitsPerBitmapWord, 0u);
    if (UNLIKELY(begin_bit_idx >= end_bit_idx)) {
...
inline size_t MarkCompact::LiveWordsBitmap<kAlignment>::CountLiveWordsUpto(size_t bit_idx) const {
  ...
  if (kBitmapWordsPerVectorWord > 1) {
    ...
    while (remainder > 0) {
      word = Bitmap::Begin()[word_offset - remainder];
      ret += POPCOUNT(word);
      remainder--;
    }
  }
...
inline mirror::Object* MarkCompact::PostCompactAddressUnchecked(mirror::Object* old_ref) const {
  ...
  DCHECK(current_space_bitmap_->Test(old_ref))
       << "ref=" << old_ref;
