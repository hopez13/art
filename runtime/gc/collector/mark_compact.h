...
class MarkCompact : public GarbageCollector {
 ...
 private:
  ...
  // into the kPageSize sized memory pointed by 'addr'. Then update the references
  // within the copied objects. The boundary objects are partially updated such that
  // only the references that lie in the page are updated. This is necessary to avoid
  ...
  // Use FindPrecedingObject() to find the object that overlaps with the given
  // page's start address. If none exists, then use VisitMarkedRange() to find
  // the first live object within the page. Pages which don't have any live words
  // in them will have null first-object.
  ...
  // copied to the page.
  // Find the chunk that has the 4KB boundary. Then scan through the
  // corresponding live-words bitmap to find the offset of the first word for that page.
  // Then use FindPrecedingObject() to find the object that overlaps with the page-begin
  // boundary.
  ...
  // conc_compaction_termination_page_ and used for termination of concurrent
  // compaction.
  ...
  // Number of pages containing black objects, indicating number of pages to be
  // slid.
  ...
  // be false for the first GC for non-zygote processes, and always for zygote
  // processes.
