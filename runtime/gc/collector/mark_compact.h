...
class MarkCompact final : public GarbageCollector {
 public:
  ...
  bool IsCompacting(Thread* self) const {
    return compacting_.load(std::memory_order_relaxed) && self == thread_running_gc_;
  }

  bool IsUsingSigbusFeature() const { return use_uffd_sigbus_; }

  bool SigbusHandler(siginfo_t* info) REQUIRES(!lock_) REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  mirror::Object* GetFromSpaceAddrFromBarrier(mirror::Object* old_ref) {
    CHECK(compacting_.load(std::memory_order_relaxed));
 ...
 private:
  ...
  union {
    ...
    // When using uffd's sigbus feature, any number of mutators may need page
    // buffer for compaction. Maintain a vector so that they can be freed in
    // FinishPhases(). When kObjPtrPoisining == true, we maintain MemMaps as they
    // have to be <4GB, otherwise we use malloc. Both the vectors are guarded by
    // lock_. Anotation with GUARDED_BY fails due to being in union.
    std::unique_ptr<std::vector<uint8_t*>> malloced_page_buffers_;
    std::unique_ptr<std::vector<MemMap>> mapped_page_buffers_;
  };
  ...
  std::atomic<unsigned int> compaction_in_progress_count_;
  // Same as above but for linear-alloc space.
  std::atomic<unsigned int> native_roots_processing_count_;
  // Used to exit from compaction loop at the end of concurrent compaction
  uint8_t thread_pool_counter_;
  // True while compacting.
  Atomic<bool> compacting_;
