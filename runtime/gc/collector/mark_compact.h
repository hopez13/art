...
class MarkCompact : public GarbageCollector {
 public:
  ...
  static constexpr int kCopyMode = kFallbackMode + 1;
  static constexpr int kMinorFaultMode = kCopyMode + 1;
  ...
  void AddLinearAllocSpaceData(uint8_t* begin, size_t len, int fd);
 ...
 private:
  ...
  void KernelPrepareRange(uint8_t* mremap_src, uint8_t* mremp_dst, size_t size, int fd);
  ...
  using TrackedArenaLastBytePair = std::pair<TrackedArena*, uint8_t*>;
  class ClassTrackedArenaHash {
   public:
    // Arenas are page aligned. So work in terms of page numbers.
    size_t operator()(const TrackedArenaLastBytePair& pair) const {
      return std::hash<size_t>{}(reinterpret_cast<uintptr_t>(pair.first->Begin()) / kPageSize);
    }

    size_t operator()(uint8_t* page) const {
      return std::hash<size_t>{}(reinterpret_cast<uintptr_t>(page) / kPageSize);
    }
  };

  class ClassTrackedArenaEquals {
   public:
    bool operator()(const TrackedArenaLastBytePair& a, const TrackedArenaLastBytePair& b) const {
      return std::equal_to<uint8_t*>{}(a.first->Begin(), b.first->Begin())
             && a.first->Size() == b.first->Size();
    }

    // This case is to find arena for a given fault-page.
    bool operator()(const TrackedArenaLastBytePair& a, uint8_t* addr) const {
      return !(std::less<uint8_t*>{}(addr, a.first->Begin())
               || std::greater<uint8_t*>{}(addr, a.first->End() - 1));
    }
  };

  class ClassTrackedArenaEmptyFn {
   public:
    void MakeEmpty(TrackedArenaLastBytePair& item) const {
      item = TrackedArenaLastBytePair();
    }
    bool IsEmpty(const TrackedArenaLastBytePair& item) const {
  ...
  HashSet<TrackedArenaLastBytePair,
          ClassTrackedArenaEmptyFn,
          ClassTrackedArenaHash,
          ClassTrackedArenaEquals> linear_alloc_arenas_;
  ...
  class LinearAllocSpaceData {
   public:
    ...
    uint8_t* begin_;
    uint8_t* end_;
    // memfd of the linear-alloc map. -2 indicates that the descriptor has
    // already been closed. -1 indicates that the space needs to relocated to a
    // shmem file and then set to -2. >= 0 indicates that the given descriptor
    // must be closed in FinishPhase.
    int fd_;
  };
