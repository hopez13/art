...
class MarkCompact final : public GarbageCollector {
 ...
 private:
  ...
  // Map 'buffer' to 'dst', both being 'length' bytes. 'single_ioctl' indicates
  // that if mmap_lock contention is detected after one ioctl call, then return.
  // 'return_on_contention' indicates that the function should return as soon as
  // mmap_lock contention is detected. Like ZeropageIoctl(), this function also
  // uses thread's priority to decide how long we delay before forcing the ioctl
  // operation. If ioctl returns EEXIST, then also function returns. Returns
  // number of bytes (multiple of page-size) mapped.
  size_t CopyIoctl(
