...
class MarkCompact : public GarbageCollector {
 ...
 private:
  ...
  void KernelPrepareRange(uint8_t* mremap_src,
                          uint8_t* mremp_dst,
                          size_t map_size,
                          size_t uffd_size,
                          int fd);
  ...
  class LinearAllocSpaceData {
   public:
    ...
    // already been closed. -1 indicates that the space needs to relocate to a
    // MAP_SHARED file and then set to -2. >= 0 indicates that the given descriptor
