...
class MarkCompact final : public GarbageCollector {
 ...
 private:
  ...
  // compaction. Until compaction is finished, 0th counter is incremented by
  // mutators before they start working on the uffd spaces. After gc-thread is
  // done comaction, gc-thread switches to 1th counter before unregistering.
  // Once 1th counter also reaches 0, then gc-thread madvises spaces and data
  // structures like page-status array.
