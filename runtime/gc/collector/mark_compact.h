...
class MarkCompact final : public GarbageCollector {
 ...
 private:
  ...
  // compaction. 0th counter represents the number of mutators still working on
  // moving space pages which started before gc-thread finished compacting pages,
  // whereas the other counter represents those which started afterwards but
  // before unregistering the space from uffd. Once 1th counter reaches 0, the
