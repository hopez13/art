...
ObjPtr<mirror::Object> ReferenceProcessor::GetReferent(Thread* self,
  ...
  if (referent.IsNull()) {
    return referent;  // Optimization to avoid mutex acquisition.
  }
  ...
  while (slow_path_required()) {
    ...
    if (UNLIKELY(reference->IsFinalizerReferenceInstance()
                 || rp_state_ == RpState::STARTING /* too early to determine mark state */)) {
    ...
    if (rp_state_ == RpState::INIT_CLEARING_DONE) {
    ...
    // we hold reference_processor_lock_ which is required to advance to INIT_CLEARING_DONE.
    ...
    DCHECK(rp_state_ == RpState::INIT_MARKING_DONE);
    ObjPtr<mirror::Object> forwarded_ref = collector_->IsMarked(referent.Ptr());
    if (forwarded_ref == nullptr) {
      // Mutator-reachable objects have been marked. Since this one hasn't been marked,
      // we know the reference will be cleared.
      if (started_trace) {
        finish_trace(start_millis);
      }
      return nullptr;
    }
    // Referent was marked before finalization marking; hence Reference only refers to marked
    // objects, and won't be cleared.
    if (started_trace) {
      finish_trace(start_millis);
    }
...
uint32_t ReferenceProcessor::ForwardSoftReferences(TimingLogger* timings) {
  TimingLogger::ScopedTiming split(concurrent_ ? "ForwardSoftReferences" :
...
void ReferenceProcessor::Setup(Thread* self,
  ...
  rp_state_ = RpState::STARTING;
  concurrent_ = concurrent;
  clear_soft_references_ = clear_soft_references;
}

// Process reference class instances and schedule finalizations.
// We advance rp_state_ to signal partial completeion for the benefit of GetReferent.
void ReferenceProcessor::ProcessReferences(Thread* self,
  ...
  }
  {
    ...
    DCHECK(rp_state_ == RpState::STARTING);
    rp_state_ = RpState::INIT_MARKING_DONE;
    condition_.Broadcast(self);
  }
  ...
  // Defer PhantomReference processing until we've finished marking through finalizers.
  {
    ...
    MutexLock mu(self, *Locks::reference_processor_lock_);
    rp_state_ = RpState::INIT_CLEARING_DONE;
  ...
  // Process all finalizer referent reachable soft and weak references with white referents. It
  // is unclear that there is any way to do this without slightly violating some language spec. We
  // choose to apply normal Reference processing rules for these. This exposes the following
  // issues:
  ...
  //    Or we could clear the reference without enquieing it, which also seems strange and
  //    unhelpful.
  // 2) In the case of a marked referent, we will preserve a reference to objects that may have
  //    been enqueued for finalization. Again fixing this would seem to involve at least undoing previous
  //    finalization / reference clearing decisions. (This would also mean than an object containing
  //    both a strong and a WeakReference to the same referent could see the WeakkReference cleared.)
