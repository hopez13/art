...
class EXPORT Thread {
 ...
 public:
  ...
  // Can weak references safely be dereferenced? Must be called from current thread.
  // Valid as long as this thread remains runnable; only a hint if it is not.
 ...
 private:
  ...
  struct alignas(4) tls_32bit_sized_values {
    ...
    //
    // Can be concurrently accessed by GetReferent() and set (by iterating over threads). Set only
    // in a pause, with mutator lock exclusively held. Can be changed from kEnabled to
    // kVisiblyEnabled by readers. No other concurrent access is possible when that happens.
