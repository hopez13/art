...
class Thread {
 ...
 private:
  ...
  // 32 bits that correspond to a locally cached copy of a threads
  // state_and_flags field. This allows for local modifications to be
  // made before writing back to thread local state. It is not
  // shareable between threads since it lacks the atomic fields of
  // TlsStateAndFlags.
  struct PACKED(4) CachedStateAndFlags {
    ...
    union {
      struct PACKED(4) {
        // Bitfield of flag values. See ThreadFlags for bit field means.
        uint16_t flags;
        // Holds the thread state. See ThreadState for possible states.
        uint16_t state;
      } as_struct;

      // Convenience field used for transferring the fields
      // from a TlsStateAndFlags instance in a single atomic move.
      int32_t as_int;
    };
  };
