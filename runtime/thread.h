...
class Thread {
 ...
 public:
  ...
  void DisallowPreMonitorMutexes() {
    if (kIsDebugBuild) {
      ...
      static Mutex placeholder_mutex("checkpoint dummy mutex", kMonitorLock);
 ...
 private:
  ...
  // cleared but another thread may still be running the flip function as indicated by the
