...
// per-thread flags back to ENABLED with release memory ordering semantics. Each mutator thread
// should check its flag with acquire semantics before assuming that it is enabled. However,
// that is often too expensive, so the reading thread sets it to VISIBLY_ENABLED after seeing it
// ENABLED.  The Reference.get() intrinsic can thus read it in relaxed mode, and reread (by
// resorting to the slow path) with ACQUIRE semantics if it sees a value of ENABLED rather than
// VISIBLY_ENABLED.
enum class WeakRefAccessState : int32_t {
  VISIBLY_ENABLED = 0,  // Enabled, and previously read with acquire load by this thread.
  ENABLED,
  DISABLED
};
 ...
 public:
  ...
  void SetWeakRefAccessEnabled(bool enabled) {
    ...
    WeakRefAccessState new_state = enabled ?
        WeakRefAccessState::ENABLED : WeakRefAccessState::DISABLED;
  ...
  struct PACKED(4) tls_32bit_sized_values {
    ...
    explicit tls_32bit_sized_values(bool is_daemon)
        : state_and_flags(0u),
          ...
          is_gc_marking(false),
          weak_ref_access_enabled(WeakRefAccessState::ENABLED),
    ...
    // Can be changed from ENABLED to VISIBLY_ENABLED by readers. No other concurrent access is
