...
class Thread {
 public:
  ...
  ALWAYS_INLINE void DecrementSuspendCount(Thread* self, bool forUserCode = false)
      REQUIRES(Locks::thread_suspend_count_lock_);
 ...
 public:
  ...
  // thread remains live.
  void WaitForFlipFunction(Thread* self) REQUIRES(!Locks::thread_suspend_count_lock_);
  ...
  // ThreadExitFlag, and pass it in. The caller is responsible for either either waiting until the
 ...
 private:
  ...
  struct PACKED(sizeof(void*)) tls_ptr_sized_values {
    ...
    // Pointer to the first node an intrusively doubly-linked list of ThreadExitFlags.
