...
class Thread {
 public:
  ...
  // Need NO_THREAD_SAFETY_ANALYSIS so we can read tls32_.user_code_suspend_count.
  void AssertThreadSuspensionIsAllowable(bool check_locks = true) const NO_THREAD_SAFETY_ANALYSIS;

  // Return true if thread suspension is allowable. NO_THREAD_SAFETY_ANALYSIS so we can read
  // tls32_.user_code_suspend_count.
  bool IsThreadSuspensionAllowable() const NO_THREAD_SAFETY_ANALYSIS;
