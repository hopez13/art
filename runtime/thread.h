...
class Thread {
 ...
 private:
  ...
  // Used only by `Thread` destructor and stack trace colection in semi-space GC (currently
  ...
  // Hepler class for manipulating the 32 bits of atomically changed state and flags.
  class StateAndFlags {
   public:
  ...
  struct PACKED(4) tls_32bit_sized_values {
    ...
    // See `ThreadStateAndFlags` for bit assignments of `ThreadFlag` and `ThreadState` values.
