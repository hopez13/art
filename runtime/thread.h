...
enum class ThreadFlag : uint16_t {
  kNoFlags = 0,                 // No flags set.
  kSuspendRequest = 1,          // If set implies that suspend_count_ > 0 and the Thread should
                                // enter the safepoint handler.
  kCheckpointRequest = 2,       // Request that the thread do some checkpoint work and then
                                // continue.
...
inline std::ostream& operator<<(std::ostream& os, ThreadFlag flags) {
  return os << static_cast<uint16_t>(flags);
}
...
class Thread {
 public:
  ...
  ThreadState GetState() const {
    ThreadState state = GetCurrentStateAndFlags(std::memory_order_relaxed).State();
  ...
  bool IsSuspended() const {
    StateAndFlags state_and_flags = GetCurrentStateAndFlags(std::memory_order_relaxed);
    return (state_and_flags.State() != kRunnable &&
            state_and_flags.TestFlags(ThreadFlag::kSuspendRequest));
  }
 ...
 public:
  ...
  bool ReadFlag(ThreadFlag flag) const {
    return GetCurrentStateAndFlags().TestFlags(flag);
  }

  bool TestAllFlags() const {
    return GetCurrentStateAndFlags().TestFlags(ThreadFlag::kAllFlags);
  }

  void AtomicSetFlag(ThreadFlag flag) {
    StateAndFlags state_and_flags(static_cast<ThreadState>(0), flag);
    tls32_.state_and_flags.fetch_or(state_and_flags.AsInt(), std::memory_order_seq_cst);
  }

  void AtomicClearFlag(ThreadFlag flag) {
    StateAndFlags state_and_flags(static_cast<ThreadState>(~0), ThreadFlag::kAllFlags ^ flag);
    tls32_.state_and_flags.fetch_and(state_and_flags.AsInt(), std::memory_order_seq_cst);
  }
 ...
 private:
  ...
  ThreadState SetStateUnsafe(ThreadState new_state) REQUIRES(Locks::mutator_lock_) {
    // This method is called from:
    //   (a) Dbg::ManageDeoptimization() with all threads suspended. The current thread is
    //       transitioned to runnable whilst the deopt request is handled then transitioned back
    //       to the original state.
    //   (b) SemiSpace::MarkPhase() when the mutator lock is exclusively held. Transitions to
    //       Runnable and back to original state.
    CHECK_EQ(Current(), this);
    StateAndFlags old_state_and_flags = GetCurrentStateAndFlags();
    if (old_state_and_flags.State() == kRunnable && new_state != kRunnable) {
      ...
      tls32_.state_and_flags.store(new_state_and_flags.AsInt(), std::memory_order_relaxed);
    }
  ...
  // Class representing a thread's execution state and request flags. This is used to
  // create, query, and update the state_and_flags field of a thread.
  class StateAndFlags {
   public:
    explicit StateAndFlags(int32_t int_value = 0) : as_int(int_value) {}
    ...
    StateAndFlags WithState(ThreadState new_state) const {
      return StateAndFlags(new_state, Flags());
    }

    StateAndFlags WithFlags(ThreadFlag flags) const {
      return StateAndFlags(State(), Flags() | flags);
    }

    ThreadState State() const {
      return as_struct.state;
    }

    ThreadFlag Flags() const {
      return as_struct.flags;
    }

    bool TestFlags(ThreadFlag flags) const {
      return (Flags() & flags) != ThreadFlag::kNoFlags;
    }
   ...
   private:
    union {
      struct {
        // Bitfield of flag values. This must e changed atomically so that flag values aren't lost.
        ThreadFlag flags;
  ...
  StateAndFlags GetCurrentStateAndFlags(std::memory_order order = std::memory_order_seq_cst) const {
  ...
  struct PACKED(4) tls_32bit_sized_values {
    ...
    // The execution state and request flags for a thread. The 32-bit value is composed of
    // 16 bits for flags (TheadFlags) followed by 16-bits for execution state (ThreadState).
    // For convenience, values are formed using StateAndFlags instances.
    AtomicInteger state_and_flags;
    static_assert(sizeof(StateAndFlags) == sizeof(int32_t),
