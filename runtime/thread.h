...
;// ???? FIXME      REQUIRES(!Locks::thread_list_lock_);
  ...
  // fresh ThreadExitFlag, and pass it in. The caller is responsible for either waiting until the
 ...
 public:
  ...
  // Check that the thread state is valid, and the thread has not been terminated.
  // Note that once the thread has been terminated, it can also be deallocated.
  // But if this has been overwritten, the value is unlikely to be in the correct range.
  void VerifyState() {
    if (kIsDebugBuild) {
