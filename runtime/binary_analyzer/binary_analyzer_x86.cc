...
ptrdiff_t AnalyzeInstruction(const uint8_t* instr,
  ...
  *is_bb_end = kNone;
  ...
  const capstone::cs_insn* insn = disassembler->Next();
  if (insn == nullptr) {
    return -1;
  }
  *target = instr + insn->size;
  const capstone::cs_x86& insn_x86 = insn->detail->x86;

  switch (insn_x86.prefix[0]) {
  case capstone::X86_PREFIX_REP:
  case capstone::X86_PREFIX_REPNE:
    *is_bb_end = kCycle;
    break;
  case capstone::X86_PREFIX_LOCK:
    *is_bb_end = kLock;
    break;
  }

  switch (insn->id) {
  case capstone::X86_INS_LOOP:
  case capstone::X86_INS_LOOPE:
  case capstone::X86_INS_LOOPNE:
    *is_bb_end = kCycle;
    break;
  case capstone::X86_INS_INVALID:
    *is_bb_end = kUnknown;
    break;
  case capstone::X86_INS_INT:
  case capstone::X86_INS_INT1:
  case capstone::X86_INS_INT3:
    *is_bb_end = kInterrupt;
    break;
  case capstone::X86_INS_JMP:
    DCHECK_GE(insn_x86.op_count, 1);
    if (insn_x86.operands[0].type != capstone::X86_OP_IMM) {
    ...
    break;
  case capstone::X86_INS_JA:
  case capstone::X86_INS_JAE:
  case capstone::X86_INS_JB:
  case capstone::X86_INS_JBE:
  case capstone::X86_INS_JCXZ:
  case capstone::X86_INS_JE:
  case capstone::X86_INS_JECXZ:
  case capstone::X86_INS_JG:
  case capstone::X86_INS_JGE:
  case capstone::X86_INS_JL:
  case capstone::X86_INS_JLE:
  case capstone::X86_INS_JNE:
  case capstone::X86_INS_JNO:
  case capstone::X86_INS_JNP:
  case capstone::X86_INS_JNS:
  case capstone::X86_INS_JO:
  case capstone::X86_INS_JP:
  case capstone::X86_INS_JRCXZ:
  case capstone::X86_INS_JS:
    DCHECK_GE(insn_x86.op_count, 1);
    if (insn_x86.operands[0].type != capstone::X86_OP_IMM) {
      *is_bb_end = kIndirectJump;
    } else {
    ...
    break;
  case capstone::X86_INS_RET:
    *is_bb_end = kReturn;
    break;
  case capstone::X86_INS_CALL:
    DCHECK_GE(insn_x86.op_count, 1);
    if (insn_x86.operands[0].type != capstone::X86_OP_IMM) {
