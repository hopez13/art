...
class SafeDexFileTest : public CommonRuntimeTest {
 ...
 protected:
  bool OpenTestDexFile(std::string* error_msg) {
    return OpenTestDexFile("Main", error_msg);
  }

  bool OpenTestDexFile(const char* name, std::string* error_msg) {
    MemMap::Init();
    std::vector<std::unique_ptr<const DexFile>> dex_files;
    std::string location = GetTestDexFileName(name);
    ArtDexFileLoader loader;
    if (!loader.Open(location.c_str(),
                     location,
                     /* verify */ true,
    ...
    dex_files.clear();
    std::unique_ptr<const DexFile> dex_file = loader.Open(data->Begin(),
                                                          src_dex_file_size,
                                                          "TestDexFile",
  ...
  const DexFile::TypeId* FindTypeId(const char* descriptor) const {
    const DexFile::StringId* sid = dex_file_->FindStringId(descriptor);
    if (sid == nullptr) {
      return nullptr;
    }
    return dex_file_->FindTypeId(dex_file_->GetIndexForStringId(*sid));
  }
...
TEST_F(SafeDexFileTest, String) {
  ...
  EXPECT_EQ("", SafeDexFile::String(unreadable_dex_file, dex::StringIndex::Invalid()));
  EXPECT_EQ("", SafeDexFile::String(dex_file_.get(), dex::StringIndex::Invalid()));
...
TEST_F(SafeDexFileTest, Field) {
  std::string error_msg;
  ASSERT_TRUE(OpenTestDexFile("AllFields", &error_msg)) << error_msg;
  const DexFile::TypeId* all_fields_tid = FindTypeId("LAllFields;");
  ASSERT_TRUE(all_fields_tid != nullptr);
  const DexFile::TypeId* int_tid = FindTypeId("I");
  ASSERT_TRUE(int_tid != nullptr);
  const DexFile::StringId* sI_sid = dex_file_->FindStringId("sI");
  ASSERT_TRUE(sI_sid != nullptr);
  const DexFile::FieldId* sI_fid = dex_file_->FindFieldId(*all_fields_tid, *sI_sid, *int_tid);
  ASSERT_TRUE(sI_fid != nullptr);
  uint32_t sI_field_index = dex_file_->GetIndexForFieldId(*sI_fid);
  constexpr bool kWithType = true;
  constexpr bool kWithoutType = false;
  ASSERT_EQ("int AllFields.sI",
            SafeDexFile::PrettyField(dex_file_.get(), sI_field_index, kWithType));
  ASSERT_EQ("AllFields.sI",
            SafeDexFile::PrettyField(dex_file_.get(), sI_field_index, kWithoutType));


  // Test unreadable DexFile or dex file header.
  mprotect(const_cast<uint8_t*>(dex_file_->Begin()), kPageSize, PROT_NONE);
  const DexFile* unreadable_dex_file = reinterpret_cast<const DexFile*>(unreadable_page_);
  EXPECT_EQ("FieldInUnreadableDexFile",
            SafeDexFile::PrettyField(unreadable_dex_file, sI_field_index, kWithType));
  EXPECT_EQ("FieldInUnreadableDexFile",
            SafeDexFile::PrettyField(unreadable_dex_file, sI_field_index, kWithoutType));
  EXPECT_EQ("FieldInUnreadableDexFile",
            SafeDexFile::PrettyField(dex_file_.get(), sI_field_index, kWithType));
  EXPECT_EQ("FieldInUnreadableDexFile",
            SafeDexFile::PrettyField(dex_file_.get(), sI_field_index, kWithoutType));
  ...
  // Test invalid large index.
  constexpr uint32_t kLargeIndex = 60000u;  // The dex file should not have that many fields.
  ASSERT_GE(kLargeIndex, dex_file_->NumFieldIds());
  EXPECT_EQ("FieldOOB#" + std::to_string(kLargeIndex) +
                "/" + std::to_string(dex_file_->NumFieldIds()),
            SafeDexFile::PrettyField(dex_file_.get(), kLargeIndex, kWithType));
  EXPECT_EQ("FieldOOB#" + std::to_string(kLargeIndex) +
                "/" + std::to_string(dex_file_->NumFieldIds()),
            SafeDexFile::PrettyField(dex_file_.get(), kLargeIndex, kWithoutType));

  // Test unreadable FieldId.
  const DexFile::Header* header = &dex_file_->GetHeader();
  uint32_t original_field_ids_offset = header->field_ids_off_;
  const_cast<DexFile::Header*>(header)->field_ids_off_ =
      unreadable_page_ - dex_file_->Begin() - sI_field_index * sizeof(DexFile::FieldId);
  EXPECT_EQ("FieldWithUnreadableId#" + std::to_string(sI_field_index),
            SafeDexFile::PrettyField(dex_file_.get(), sI_field_index, kWithType));
  EXPECT_EQ("FieldWithUnreadableId#" + std::to_string(sI_field_index),
            SafeDexFile::PrettyField(dex_file_.get(), sI_field_index, kWithoutType));
  const_cast<DexFile::Header*>(header)->field_ids_off_ = original_field_ids_offset;

  // Do not test broken sub-components of the PrettyField().
}

TEST_F(SafeDexFileTest, Method) {
  std::string error_msg;
  ASSERT_TRUE(OpenTestDexFile(&error_msg)) << error_msg;
  const DexFile::TypeId* main_tid = FindTypeId("LMain;");
  ASSERT_TRUE(main_tid != nullptr);
  const DexFile::TypeId* void_tid = FindTypeId("V");
  ASSERT_TRUE(void_tid != nullptr);
  const DexFile::TypeId* args_tid = FindTypeId("[Ljava/lang/String;");
  ASSERT_TRUE(args_tid != nullptr);
  dex::TypeIndex args_type_index = dex_file_->GetIndexForTypeId(*args_tid);
  const DexFile::ProtoId* proto_id =
      dex_file_->FindProtoId(dex_file_->GetIndexForTypeId(*void_tid), &args_type_index, 1u);
  ASSERT_TRUE(proto_id != nullptr);
  const DexFile::StringId* main_sid = dex_file_->FindStringId("main");
  ASSERT_TRUE(main_sid != nullptr);
  const DexFile::MethodId* main_mid = dex_file_->FindMethodId(*main_tid, *main_sid, *proto_id);
  ASSERT_TRUE(main_mid != nullptr);
  uint32_t main_method_index = dex_file_->GetIndexForMethodId(*main_mid);
  constexpr bool kWithSignature = true;
  constexpr bool kWithoutSignature = false;
  ASSERT_EQ("void Main.main(java.lang.String[])",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  ASSERT_EQ("Main.main",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));

  // Test unreadable DexFile or dex file header.
  mprotect(const_cast<uint8_t*>(dex_file_->Begin()), kPageSize, PROT_NONE);
  const DexFile* unreadable_dex_file = reinterpret_cast<const DexFile*>(unreadable_page_);
  EXPECT_EQ("MethodInUnreadableDexFile",
            SafeDexFile::PrettyMethod(unreadable_dex_file, main_method_index, kWithSignature));
  EXPECT_EQ("MethodInUnreadableDexFile",
            SafeDexFile::PrettyMethod(unreadable_dex_file, main_method_index, kWithoutSignature));
  EXPECT_EQ("MethodInUnreadableDexFile",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  EXPECT_EQ("MethodInUnreadableDexFile",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));
  ...
  // Test invalid large method index.
  constexpr uint32_t kLargeIndex = 60000u;  // The dex file should not have that many types.
  ASSERT_GE(kLargeIndex, dex_file_->NumMethodIds());
  EXPECT_EQ("MethodOOB#" + std::to_string(kLargeIndex) +
                "/" + std::to_string(dex_file_->NumMethodIds()),
            SafeDexFile::PrettyMethod(dex_file_.get(), kLargeIndex, kWithSignature));
  EXPECT_EQ("MethodOOB#" + std::to_string(kLargeIndex) +
                "/" + std::to_string(dex_file_->NumMethodIds()),
            SafeDexFile::PrettyMethod(dex_file_.get(), kLargeIndex, kWithoutSignature));

  // Test unreadable MethodId.
  const DexFile::Header* header = &dex_file_->GetHeader();
  uint32_t original_method_ids_offset = header->method_ids_off_;
  const_cast<DexFile::Header*>(header)->method_ids_off_ =
      unreadable_page_ - dex_file_->Begin() - main_method_index * sizeof(DexFile::MethodId);
  EXPECT_EQ("MethodWithUnreadableId#" + std::to_string(main_method_index),
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  EXPECT_EQ("MethodWithUnreadableId#" + std::to_string(main_method_index),
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));
  const_cast<DexFile::Header*>(header)->method_ids_off_ = original_method_ids_offset;

  // Test invalid large proto index.
  ASSERT_GE(kLargeIndex, dex_file_->NumProtoIds());
  dex::ProtoIndex original_proto_index = main_mid->proto_idx_;
  const_cast<DexFile::MethodId*>(main_mid)->proto_idx_.index_ = kLargeIndex;
  EXPECT_EQ("Main.main/ProtoOOB#" + std::to_string(kLargeIndex) +
                "/" + std::to_string(dex_file_->NumProtoIds()),
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  EXPECT_EQ("Main.main",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));
  const_cast<DexFile::MethodId*>(main_mid)->proto_idx_ = original_proto_index;

  // Test unreadable ProtoId.
  uint32_t original_proto_ids_offset = header->proto_ids_off_;
  const_cast<DexFile::Header*>(header)->proto_ids_off_ =
      unreadable_page_ - dex_file_->Begin() - main_mid->proto_idx_.index_ * sizeof(DexFile::ProtoId);
  EXPECT_EQ("Main.main/UnreadableProtoId#" + std::to_string(main_mid->proto_idx_.index_),
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  EXPECT_EQ("Main.main",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));
  const_cast<DexFile::Header*>(header)->proto_ids_off_ = original_proto_ids_offset;

  // Test unreadable parameters size.
  const DexFile::ProtoId& main_param_pid = dex_file_->GetProtoId(main_mid->proto_idx_);
  uint32_t original_parameters_off = main_param_pid.parameters_off_;
  const_cast<DexFile::ProtoId&>(main_param_pid).parameters_off_ =
      unreadable_page_ - dex_file_->Begin();
  EXPECT_EQ("void Main.main/UnreadableParametersSize",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  EXPECT_EQ("Main.main",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));
  const_cast<DexFile::ProtoId&>(main_param_pid).parameters_off_ = original_parameters_off;

  // Test unreadable parameter TypeItem.
  const_cast<DexFile::ProtoId&>(main_param_pid).parameters_off_ =
      unreadable_page_ - dex_file_->Begin() - sizeof(uint32_t);
  reinterpret_cast<uint32_t*>(unreadable_page_)[-1] = 1u;
  EXPECT_EQ("void Main.main([UNREADABLE])",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithSignature));
  EXPECT_EQ("Main.main",
            SafeDexFile::PrettyMethod(dex_file_.get(), main_method_index, kWithoutSignature));
  const_cast<DexFile::ProtoId&>(main_param_pid).parameters_off_ = original_parameters_off;

  // Do not test broken sub-components of the PrettyMethod().
}
