...
class Runtime {
 public:
  ...
  EXPORT void VisitReflectiveTargets(ReflectiveValueVisitor* visitor) REQUIRES(Locks::mutator_lock_);
 ...
 private:
  ...
  // referes to this symbol and it can't link with R_386_PC32 relocation.
  // Use protected visibility instead. Protected symbols must be declared with
  // "protected" visibility attribute when building the library and "default"
  // visibility when referred to from external libraries/binaries. Otherwise,
  // the external code will expect the symbol to be defined locally and fail to
  // link.
#ifdef BUILDING_LIBART
#define RUNTIME_INSTANCE_VISIBILITY PROTECTED
#else
#define RUNTIME_INSTANCE_VISIBILITY EXPORT
#endif
  // A pointer to the active runtime or null.
  RUNTIME_INSTANCE_VISIBILITY static Runtime* instance_;
