...
class Runtime {
 public:
  ...
  void SetHiddenApiNewEventLogSampleRate(uint32_t rate) {
    hidden_api_access_new_event_log_rate_ = rate;
  }

  uint32_t GetHiddenApiNewEventLogSampleRate() const {
    return hidden_api_access_new_event_log_rate_;
  }
 ...
 private:
  ...
  // How often to log hidden API access to the new event log. An integer between 0
  // (never) and 0x10000 (always).
  uint32_t hidden_api_access_new_event_log_rate_;
