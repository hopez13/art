...
void ReadBarrierForVarHandleAccess(ObjPtr<Object> obj, MemberOffset field_offset)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  if (kUseReadBarrier) {
...
bool ArrayElementVarHandle::Access(AccessMode access_mode,
  ...
  ObjPtr<Array> target_array(shadow_frame->GetVRegReference(operands->GetOperand(0))->AsArray());
  ...
  const int target_element = shadow_frame->GetVReg(operands->GetOperand(1));
