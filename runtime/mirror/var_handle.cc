...
class AtomicGetAndAddWithByteSwapAccessor : public Object::Accessor<T> {
 public:
  explicit AtomicGetAndAddWithByteSwapAccessor(T value, JValue* result)
      : value_(value), result_(result) {}
...
template <typename T>
using GetAndAddAcquireWithByteSwapAccessor =
    AtomicGetAndAddWithByteSwapAccessor<T, std::memory_order_acquire>;

template <typename T>
using GetAndAddReleaseWithByteSwapAccessor =
    AtomicGetAndAddWithByteSwapAccessor<T, std::memory_order_release>;
  ...
  static bool Dispatch(const VarHandle::AccessMode access_mode,
    ...
    switch (access_mode) {
      ...
      case VarHandle::AccessMode::kGetAndAddAcquire: {
        T value = ValueGetter<T>::Get(getter);
        if (byte_swap) {
          GetAndAddAcquireWithByteSwapAccessor<T> accessor(value, result);
          accessor.Access(element_address);
        } else {
          GetAndAddAcquireAccessor<T> accessor(value, result);
          accessor.Access(element_address);
      ...
      case VarHandle::AccessMode::kGetAndAddRelease: {
        T value = ValueGetter<T>::Get(getter);
        if (byte_swap) {
          GetAndAddReleaseWithByteSwapAccessor<T> accessor(value, result);
          accessor.Access(element_address);
        } else {
          GetAndAddReleaseAccessor<T> accessor(value, result);
          accessor.Access(element_address);
