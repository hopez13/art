...
MethodType* VarHandle::GetMethodTypeForAccessMode(Thread* self, AccessMode access_mode) {
  ...
  const int32_t ptypes_count =
      GetParameterCount(access_mode_template, GetCoordinateType0(), GetCoordinateType1());

  // Setup a handle scope to hold the current instance as we are about
  // to do an allocation which could result in 'this' changing its address.
...
bool ByteArrayViewVarHandle::GetChangeByteOrder() {
  return GetFieldBoolean(ChangeByteOrderOffset());
}
...
bool ByteBufferViewVarHandle::GetChangeByteOrder() {
  return GetFieldBoolean(ChangeByteOrderOffset());
}
