...
NO_RETURN void UnsupportedAccessMode(const char* access_mode, const char* type_name) {
  LOG(FATAL) << "Unsupported access mode :" << access_mode << " for type " << type_name;
  UNREACHABLE();
}

#define UNSUPPORTED_ACCESS_MODE(ACCESS_MODE, TYPE)             \
template<> void ACCESS_MODE ## Accessor<TYPE>::Access(TYPE*) { \
  UnsupportedAccessMode(#ACCESS_MODE, #TYPE);                  \
}
...
bool ObjectAccessor<ObjPtr<Object>>::Dispatch(VarHandle::AccessMode access_mode,
  ...
  switch (access_mode) {
    ...
    case VarHandle::AccessMode::kSet: {
      ...
      if (Runtime::Current()->IsActiveTransaction()) {
        obj->SetFieldObject<true>(field_offset, new_value);
      } else {
        obj->SetFieldObject<false>(field_offset, new_value);
      }
      break;
    }
    case VarHandle::AccessMode::kGetAcquire:
    ...
    case VarHandle::AccessMode::kSetVolatile: {
      ...
      ObjPtr<Object> new_value = GetValue(values, 0);
      if (Runtime::Current()->IsActiveTransaction()) {
        obj->SetFieldObjectVolatile<true>(field_offset, new_value);
      } else {
        obj->SetFieldObjectVolatile<false>(field_offset, new_value);
      }
      break;
    }
    case VarHandle::AccessMode::kCompareAndSet: {
      ...
      bool cas_result;
      if (Runtime::Current()->IsActiveTransaction()) {
        cas_result = obj->CasFieldStrongSequentiallyConsistentObject<true, true>(field_offset,
                                                                                 expected_value,
                                                                                 desired_value);
      } else {
        cas_result = obj->CasFieldStrongSequentiallyConsistentObject<false, true>(field_offset,
                                                                                  expected_value,
                                                                                  desired_value);
      }
    ...
    case VarHandle::AccessMode::kWeakCompareAndSetRelease: {
      ...
      bool cas_result;
      if (Runtime::Current()->IsActiveTransaction()) {
        cas_result = obj->CasFieldWeakSequentiallyConsistentObject<true, true>(field_offset,
                                                                               expected_value,
                                                                               desired_value);
      } else {
        cas_result = obj->CasFieldWeakSequentiallyConsistentObject<false, true>(field_offset,
                                                                                expected_value,
                                                                                desired_value);
      }
      StoreResult(cas_result, result);
    ...
    case VarHandle::AccessMode::kGetAndSetAcquire:
    case VarHandle::AccessMode::kGetAndSetRelease:
      // TODO(oth): these require new mirror::Object methods.
    ...
    case VarHandle::AccessMode::kGetAndBitwiseXorRelease: {
      ...
      UnsupportedAccessMode(access_mode_name, "Object");
    }
  }
  return true;
}
