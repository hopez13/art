...
bool ArrayElementVarHandle::Access(AccessMode access_mode,
  ...
  if (raw_array == nullptr) {
    ThrowNullPointerException("array is null");
    return false;
  }
...
bool ByteArrayViewVarHandle::Access(AccessMode access_mode,
  ...
  int64_t required_length = data_offset + element_width;
  if (data_offset < 0 || required_length > byte_array->GetLength()) {
    ThrowIndexOutOfBoundsException(data_offset, byte_array->GetLength());
    return false;
  }
