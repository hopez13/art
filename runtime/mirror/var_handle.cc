...
VarHandle::MatchKind VarHandle::GetMethodTypeMatchForAccessModeImpl(
  ...
  auto mt_accessor = MethodType::GetAccessor(method_type);
  ObjPtr<Class> mt_rtype = mt_accessor.GetRType();
  ...
  if (vh_ptypes_count != mt_accessor.GetPTypesLength()) {
    return MatchKind::kNone;
  }

  // Check the parameter types are compatible.
  for (int32_t i = 0; i < vh_ptypes_count; ++i) {
    if (vh_ptypes[i]->IsAssignableFrom(mt_accessor.GetPType(i))) {
      continue;
    }
    if (mt_accessor.GetPType(i) == void_type && !vh_ptypes[i]->IsPrimitive()) {
    ...
    if (!IsParameterTypeConvertible(mt_accessor.GetPType(i), vh_ptypes[i])) {
      return MatchKind::kNone;
    }
    match = MatchKind::kWithConversions;
  }
...
VarHandle::MatchKind VarHandle::GetMethodTypeMatchForAccessMode(
    AccessMode access_mode, VariableSizedHandleScope* method_type) {
  return GetMethodTypeMatchForAccessModeImpl(access_mode, this, method_type);
}
...
ObjPtr<MethodType> VarHandle::GetMethodTypeForAccessMode(Thread* self, AccessMode access_mode) {
  ...
  GetMethodTypeForAccessMode(access_mode, &method_type_hs);
  return MethodType::Create(self, &method_type_hs);
}

void VarHandle::GetMethodTypeForAccessMode(AccessMode access_mode,
                                           VariableSizedHandleScope* method_type) {
  DCHECK_EQ(method_type->Size(), 0u);
  AccessModeTemplate access_mode_template = GetAccessModeTemplate(access_mode);

  // Store return type in `method_type`.
  method_type->NewHandle(GetReturnType(access_mode_template, GetVarType()));
  ...
  for (int32_t i = 0; i < ptypes_count; ++i) {
    method_type->NewHandle(ptypes_array[i]);
  }
}
