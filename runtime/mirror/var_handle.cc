...
template <typename T, bool is_float, std::memory_order MO>
...
template <typename T, std::memory_order MO>
class AtomicGetAndAddOperator<T, true, MO> {
 public:
  static T Apply(T* addr, T addend) {
...
class AtomicGetAndAddAccessor<float, MO> : public Object::Accessor<float> {
 public:
  AtomicGetAndAddAccessor(float value, JValue* result) : value_(value), result_(result) {}

  void Access(float* addr) OVERRIDE {
  std::atomic<float>* atom = reinterpret_cast<std::atomic<float>*>(addr);
    float previous_value = atom->load(std::memory_order_relaxed);
    float sum;
    do {
      sum = previous_value + value_;
    } while (!atom->compare_exchange_weak(previous_value, sum, MO, std::memory_order_relaxed));
    StoreResult(previous_value, result_);
  }
...
bool ByteBufferViewVarHandle::Access(AccessMode access_mode,
  ...
  if (byte_buffer == nullptr) {
    ThrowNullPointerException("buffer is null");
    return false;
  }
  ...
  const Primitive::Type primitive_type = GetVarType()->GetPrimitiveType();
  const int32_t element_width = Primitive::ComponentSize(primitive_type);

  // Check it is safe to add byte_buffer_offset to byte_index and the result is within the bounds
  // of the heap buffer.
  const int64_t checked_offset64 = byte_buffer_offset + byte_index;
  const int32_t checked_offset32 = static_cast<int32_t>(checked_offset64);
  if (checked_offset32 != checked_offset64 ||
      checked_offset64 < byte_buffer_offset ||
      checked_offset64 + element_width > byte_buffer_limit) {
    ThrowIndexOutOfBoundsException(byte_index, byte_buffer_limit - byte_buffer_offset);
    return false;
  }
