...
inline uint32_t Class::GetObjectSize() {
  ...
  DCHECK((!IsVariableSize<kVerifyFlags, kReadBarrierOption>())) << "class="
                                                                << this->PrettyTypeOf();
...
inline ArtMethod* Class::GetVirtualMethod(size_t i, PointerSize pointer_size) {
  ...
  DCHECK(IsResolved<kVerifyFlags>() || IsErroneous<kVerifyFlags>())
      << mirror::Class::PrettyClass() << " status=" << GetStatus();
...
inline bool Class::IsAssignableFromArray(ObjPtr<Class> src) {
  ...
  if (!IsArrayClass()) {
    ...
    DCHECK(java_lang_Object != nullptr) << Class::PrettyClass(src);
...
inline ArtMethod* Class::FindVirtualMethodForInterface(ArtMethod* method,
  ...
  DCHECK(declaring_class->IsInterface()) << ArtMethod::PrettyMethod(method);
