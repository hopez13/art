...
inline void Class::VerifyOverflowReferenceBitmap() {
  ...
  if (super_class != nullptr) {
    ...
    for (auto iter = klasses.rbegin(); iter != klasses.rend(); iter++) {
      ...
      CHECK_LE(idx, num_bits) << PrettyDescriptor();
    }
  }

  uint32_t ref_offsets = GetReferenceInstanceOffsets<kVerifyFlags>();
  CHECK_NE(ref_offsets, 0u) << PrettyDescriptor();
  CHECK((ref_offsets & kVisitReferencesSlowpathMask) != 0) << PrettyDescriptor();
  ...
  for (uint32_t i = 0, field_offset = 0; i < bitmap_num_words; i++, field_offset += 32) {
    ...
    // Confirm that all the bits that ought to be set, are set.
    while (ref_offsets != 0) {
      if ((ref_offsets & 1) != 0) {
        CHECK(check_bitmap[check_bitmap_idx]) << PrettyDescriptor();
  ...
  for_each(check_bitmap.begin(),
           check_bitmap.end(),
           [this](bool bit)
               REQUIRES_SHARED(Locks::mutator_lock_) { CHECK(!bit) << PrettyDescriptor(); });
}
