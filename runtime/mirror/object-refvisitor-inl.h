...
// Called with from-space address as we need to access klass and length (in case
// of arrays) and we don't want to cause cascading faults.
template <bool kVisitNativeRoots,
          VerifyObjectFlags kVerifyFlags,
          ReadBarrierOption kReadBarrierOption,
          typename Visitor>
inline size_t Object::VisitReferencesForCompaction(const Visitor& visitor,
                                                   MemberOffset begin,
                                                   MemberOffset end) {
  ...
  if (LIKELY(class_flags == kClassFlagNormal)) {
    ...
    if ((class_flags & kClassFlagNoReferenceFields) == 0) {
      ...
      if (class_flags == kClassFlagClass) {
        ...
        DCHECK((klass->IsObjectArrayClass<kVerifyFlags>()));
        ObjPtr<ObjectArray<mirrot::Object>> obj_arr = AsObjectArray<mirror::Object, kVerifyNone>();
      ...
      if (class_flags == kClassFlagString) {
        ...
        DCHECK_NE(class_flags & kClassFlagNormal, 0);
