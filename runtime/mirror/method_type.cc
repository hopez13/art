...
ObjPtr<MethodType> MethodType::Create(Thread* self, VariableSizedHandleScope* method_type_hs) {
  RawMethodTypeAccessor accessor(method_type_hs);
  Handle<mirror::Class> return_type = method_type_hs->GetHandle<mirror::Class>(0u);
  DCHECK_EQ(return_type.Get(), accessor.GetRType());
  int32_t num_method_args = accessor.GetPTypesLength();
  ...
  for (int32_t i = 0; i != num_method_args; ++i) {
    method_params->Set(i, accessor.GetPType(i));
  }
...
size_t NumberOfVRegsImpl(MethodTypeType method_type) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  auto accessor = MethodType::GetAccessor(method_type);
  const int32_t p_types_length = accessor.GetPTypesLength();
  ...
  for (int32_t i = 0; i < p_types_length; ++i) {
    ObjPtr<Class> klass = accessor.GetPType(i);
...
size_t MethodType::NumberOfVRegs(VariableSizedHandleScope* method_type) {
  return NumberOfVRegsImpl(method_type);
}
...
std::string PrettyDescriptorImpl(MethodTypeType method_type)
  ...
  const int32_t params_length = accessor.GetPTypesLength();
  for (int32_t i = 0; i < params_length; ++i) {
    ss << accessor.GetPType(i)->PrettyDescriptor();
  ...
  ss << ")";
  ss << accessor.GetRType()->PrettyDescriptor();
...
std::string MethodType::PrettyDescriptor(VariableSizedHandleScope* method_type) {
  return PrettyDescriptorImpl(method_type);
}
