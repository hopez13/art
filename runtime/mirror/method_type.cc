...
ObjPtr<MethodType> MethodType::Create(Thread* self, VariableSizedHandleScope* method_type_hs) {
  ...
  DCHECK_EQ(return_type.Get(), GetRType(method_type_hs));
  auto raw_p_types = GetPTypes(method_type_hs);
  int32_t num_method_args = raw_p_types.GetLength();
  ...
  for (int32_t i = 0; i != num_method_args; ++i) {
    method_params->Set(i, raw_p_types.Get(i));
  }
...
std::string PrettyDescriptorImpl(const Accessor& p_types, ObjPtr<mirror::Class> r_type)
    REQUIRES_SHARED(Locks::mutator_lock_) {
...
std::string MethodType::PrettyDescriptor(ObjPtr<mirror::MethodType> method_type) {
  ...
  return PrettyDescriptorImpl(GetPTypes(method_type), GetRType(method_type));
}
