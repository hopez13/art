...
static bool IsInPlaceConvertibleX(ObjPtr<Class> from, ObjPtr<Class> to)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  if (from == to) {
    return true;
  }
...
bool MethodType::IsInPlaceConvertible(ObjPtr<MethodType> target) {
  ...
  for (int32_t i = 0; i < ptypes_length; ++i) {
    if (!IsInPlaceConvertibleX(ptypes->GetWithoutChecks(i), target_ptypes->GetWithoutChecks(i))) {
      return false;
    }
  }

  return GetRType()->IsPrimitiveVoid() || IsInPlaceConvertibleX(target->GetRType(), GetRType());
}
