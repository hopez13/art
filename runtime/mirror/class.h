...
class MANAGED Class final : public Object {
 public:
  ...
  static bool IsInSamePackage(const std::string_view& descriptor1,
                              const std::string_view& descriptor2);
  ...
  ArtMethod* FindInterfaceMethod(const std::string_view& name,
  ...
  ArtMethod* FindClassMethod(const std::string_view& name,
                             const std::string_view& signature,
                             PointerSize pointer_size)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  ArtMethod* FindConstructor(const std::string_view& signature, PointerSize pointer_size)
      REQUIRES_SHARED(Locks::mutator_lock_);

  ArtMethod* FindDeclaredVirtualMethodByName(const std::string_view& name, PointerSize pointer_size)
      REQUIRES_SHARED(Locks::mutator_lock_);

  ArtMethod* FindDeclaredDirectMethodByName(const std::string_view& name, PointerSize pointer_size)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  ArtField* FindInstanceField(const std::string_view& name, const std::string_view& type)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  ArtField* FindDeclaredInstanceField(const std::string_view& name, const std::string_view& type)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  ArtField* FindDeclaredStaticField(const std::string_view& name, const std::string_view& type)
      REQUIRES_SHARED(Locks::mutator_lock_);
