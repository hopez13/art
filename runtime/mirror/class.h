...
class MANAGED Class FINAL : public Object {
 public:
  ...
  // Find a method with the given name and signature in an interface class (3 overloads).
  //
  // Search for the method declared in the class, then search the superclass java.lang.Object,
  // then search for a method declared in any superinterface.
  ...
  // Find a method with the given name and signature in a non-interface class (3 overloads).
  //
  // Search for the method in the class, following the JLS rules which conflict with JVMS
  ...
  //      performing the lookup. If it's inherited, return it. Otherwise, just remember the
  //      method and its declaring class and proceed to step 3.
  ...
  //
  // If there is an inherited method with the given name and signature, this procedure shall
  // find it. However, we may return a method that's not inherited (found in step 2, returned
  // in step 4) and its the caller's responsibility to check for IncompatibleClassChangeError
