...
class MANAGED DexCache final : public Object {
 public:
  ...
  void* AllocArray(size_t alignment, size_t size) REQUIRES_SHARED(Locks::mutator_lock_);

  void* AllocArrayLocked(size_t alignment, size_t size)
      REQUIRES(Locks::classlinker_classes_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_);

  void FreeArray(void* ptr) REQUIRES_SHARED(Locks::mutator_lock_);
 ...
 private:
  ...
  uint32_t num_resolved_call_sites_;  // Number of entries in the resolved_call_sites_.
