...
template <typename T> struct PACKED(8) DexCachePair {
  ...
  // lookup index matches the stored index (for a >0 string index)
  ...
  // that a valid string is stored at that index for a dex section id of 0.
  //
  // As an optimization, we want to avoid branching on the object pointer since
  // it's always non-null if the id branch succeeds (except for the 0th string id).
