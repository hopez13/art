...
class PACKED(2 * sizeof(T)) NativeDexCacheLine {
 public:
  ...
  ALWAYS_INLINE NativeDexCachePair<T> Load() const {
    while(true) {
      uint32_t s0 = seqlock_.load(std::memory_order_relaxed);
      std::atomic_thread_fence(std::memory_order_acquire);
      NativeDexCachePair<T> pair(object_, index_);
      std::atomic_thread_fence(std::memory_order_acquire);
      uint32_t s1 = seqlock_.load(std::memory_order_relaxed);
      if (UNLIKELY((s0 & 1) == 1 || s0 != s1)) {
  ...
  ALWAYS_INLINE void Store(NativeDexCachePair<T> pair) {
    while (true) {
      uint16_t s = seqlock_.load(std::memory_order_relaxed);
      ...
      this->object_ = pair.object;
      this->index_ = dchecked_integral_cast<uint16_t>(pair.index);
      std::atomic_thread_fence(std::memory_order_release);
      seqlock_.store(s + 2, std::memory_order_relaxed);  // Unlock.
      return;
    }
  }
