...
class PACKED(2 * sizeof(T)) NativeDexCacheLine {
 public:
  ...
  ALWAYS_INLINE NativeDexCachePair<T> Load() const {
    while(true) {
      uint16_t s = seqlock_.load(std::memory_order_acquire);
      T object = object_.load(std::memory_order_acquire);
      uint16_t index = index_.load(std::memory_order_acquire);
      if (UNLIKELY((s & 1) == 1 || s != seqlock_.load(std::memory_order_relaxed))) {
        continue;  // Retry - the data is locked or was modified during the copy.
      }
  ...
  ALWAYS_INLINE void Store(NativeDexCachePair<T> pair) {
    while (true) {
      uint16_t s = seqlock_.load();
      if (UNLIKELY((s & 1) == 1 || !seqlock_.compare_exchange_weak(s, s + 1))) { // Lock.
        continue;  // Retry - other thread locked the data for modification.
      }
      object_.store(pair.object);
      index_.store(dchecked_integral_cast<uint16_t>(pair.index));
      seqlock_.store(s + 2);  // Unlock.
      return;
    }
  }
 ...
 private:
  ...
  std::atomic_uint16_t index_;
  std::atomic_uint16_t seqlock_;  // Odd number if the data is locked for modification.
