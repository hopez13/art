...
template <typename T> struct PACKED(2 * __SIZEOF_POINTER__) NativeDexCachePair {
  ...
  static GcRoot<T> Lookup(std::atomic<NativeDexCachePair<T>>* dex_cache,
                          uint32_t idx,
                          uint32_t cache_size) {
    DCHECK_NE(cache_size, 0u);
    NativeDexCachePair<T> element = dex_cache[idx % cache_size].load(std::memory_order_relaxed);
    if (idx != element.index) {
      return GcRoot<T>(nullptr);
    }
  ...
  static void Assign(std::atomic<NativeDexCachePair<T>>* dex_cache,
                     uint32_t idx,
                     T* object,
                     uint32_t cache_size) {
    DCHECK_LT(idx % cache_size, cache_size);
    dex_cache[idx % cache_size].store(
        NativeDexCachePair<T>(object, idx), std::memory_order_relaxed);
  }
 ...
 private:
  ...
  ALWAYS_INLINE static ConversionPair64 AtomicRelaxedLoad16B(
    ...
    __asm__ __volatile__(
        // We need a successful LDXP+STXP pair for atomic load, see B2.6.2 in the ARMv8 manual.
        "1:\n\t"
        "ldxp %0, %1, [%3]\n\t"
        "stxp %w2, %0, %1, [%3]\n\t"
        "cbnz %w2, 1b"
        : "=&r"(first), "=&r"(second), "=&r"(status)
        : "r"(target));
  ...
  ALWAYS_INLINE static void AtomicRelaxedStore16B(
    ...
    __asm__ __volatile__(
        ...
        "cbnz %w2, 1b"
        : "=&r"(first), "=&r"(second), "=&r"(status)
        : "r"(target), "r"(value.first), "r"(value.second)
        : );
  }
