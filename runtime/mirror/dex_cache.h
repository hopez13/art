...
template <typename T> struct PACKED(8) DexCachePair {
  ...
  T* GetObjectForIndexUnchecked(uint32_t idx) REQUIRES_SHARED(Locks::mutator_lock_) {
    // TODO(oth): Remove this.
    if (idx != index) {
      return nullptr;
    }
    return object.Read();
...
using CallSiteDexCachePair =  DexCachePair<mirror::CallSite>;
...
class MANAGED DexCache FINAL : public Object {
 public:
  ...
  template <ReadBarrierOption kReadBarrierOption = kWithReadBarrier, typename Visitor>
  void FixupResolvedCallSites(CallSiteDexCacheType* dest, const Visitor& visitor)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  CallSiteDexCacheType* GetResolvedCallSites()
      ALWAYS_INLINE REQUIRES_SHARED(Locks::mutator_lock_) {
    return GetFieldPtr64<CallSiteDexCacheType*>(ResolvedCallSitesOffset());
  }

  void SetResolvedCallSites(CallSiteDexCacheType* resolved_call_sites)
      ALWAYS_INLINE
      REQUIRES_SHARED(Locks::mutator_lock_) {
 ...
 private:
  ...
  uint64_t resolved_call_sites_;    // std::atomic<CallSiteDexCachePair>* array with
                                    // num_resolved_call_sites_ elements.
