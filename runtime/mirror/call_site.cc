...
mirror::CallSite* CallSite::Create(Thread* const self, Handle<MethodHandleImpl> target) {
  StackHandleScope<1> hs(self);
  Handle<mirror::CallSite> cs(
      hs.NewHandle(ObjPtr<CallSite>::DownCast(StaticClass()->AllocObject(self))));
  // TODO: Do we ever create a CallSite during a transaction ? There doesn't
  // seem like a good reason to do an invoke-custom that results in the
  // resolution of a call site in an unstarted runtime.
