...
ALWAYS_INLINE
static bool IsResultPreferredOver(ArtMethod* orig_method,
  ...
  // so we can never be in situation of `orig_method` being direct and
  // `new_method` being virtual.
...
ObjPtr<Method> Class::GetDeclaredMethodInternal(
  ...
  for (auto& m : h_klass->GetDeclaredVirtualMethods(kPointerSize)) {
    ...
    } else if (IsResultPreferredOver(result, result_hidden, &m, m_hidden)) {
  ...
  if ((result != nullptr) && !result_hidden) {
    ...
    return Method::CreateFromArtMethod<kPointerSize, kTransactionActive>(self, result);
  }
