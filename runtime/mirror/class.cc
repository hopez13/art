...
static std::tuple<bool, ArtMethod*> FindDeclaredClassMethod(ObjPtr<mirror::Class> klass,
  ...
  DCHECK_LE(num_direct_methods, declared_methods.size());
  const size_t ranges[2][2] = {
     {0u, num_direct_methods},                      // Declared direct methods.
     {num_direct_methods, declared_methods.size()}  // Declared virtual methods.
  };
  for (const size_t (&range)[2] : ranges) {
    uint32_t begin = dchecked_integral_cast<uint32_t>(range[0]);
    uint32_t end = dchecked_integral_cast<uint32_t>(range[1]);
    auto [success, mid] =
        ClassMemberBinarySearch(begin, end, name_cmp, signature_cmp, get_name_idx);
    if (success) {
