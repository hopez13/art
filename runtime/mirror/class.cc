...
void Class::SetExtData(ObjPtr<ClassExt> ext) {
  ...
  // TODO It might be wise to just create an internal (global?) mutex that we syncronize on instead
  // to prevent any possibility of deadlocks with java code.
  DCHECK_EQ(GetLockOwnerThreadId(), Thread::Current()->GetThreadId())
      << "The " << PrettyClass() << " object should be locked when writing to the extData field.";
  DCHECK(GetExtData() == nullptr) << "The extData for " << PrettyClass() << " has already been set!";
