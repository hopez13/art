...
ClassExt* Class::EnsureExtDataPresent(Thread* self) {
  ...
  if (new_ext.Get() == nullptr) {
    ...
    if (Runtime::Current()->IsActiveTransaction()) {
      set = h_this->CasFieldStrongSequentiallyConsistentObject<true>(
          MemberOffset(OFFSET_OF_OBJECT_MEMBER(Class, ext_data_)),
                       ObjPtr<ClassExt>(nullptr),
                       new_ext.Get());
    } else {
      set = h_this->CasFieldStrongSequentiallyConsistentObject<false>(
          MemberOffset(OFFSET_OF_OBJECT_MEMBER(Class, ext_data_)),
                       ObjPtr<ClassExt>(nullptr),
                       new_ext.Get());
    }
