...
class MANAGED ByteArrayViewVarHandle : public VarHandle {
 public:
  bool GetChangeByteOrder() REQUIRES_SHARED(Locks::mutator_lock_);
 ...
 private:
  static MemberOffset ChangeByteOrderOffset() {
    return MemberOffset(OFFSETOF_MEMBER(ByteArrayViewVarHandle, change_byte_order_));
  }

  // Flag for whether and byte order should swapped.
  uint8_t change_byte_order_;
...
class MANAGED ByteBufferViewVarHandle : public VarHandle {
 public:
  ...
  static ByteBufferViewVarHandle* Create(Thread* const self, bool change_byte_order)
      REQUIRES_SHARED(Locks::mutator_lock_) REQUIRES(!Roles::uninterruptible_);
 ...
 private:
  static MemberOffset ChangeByteOrderOffset() {
    return MemberOffset(OFFSETOF_MEMBER(ByteBufferViewVarHandle, change_byte_order_));
  }
