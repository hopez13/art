...
class VarHandleTest : public CommonRuntimeTest {
 public:
  ...
  static ByteArrayViewVarHandle* CreateByteArrayViewVarHandle(Thread* const self,
    ...
    bvh->SetFieldBoolean<false>(ByteArrayViewVarHandle::ChangeByteOrderOffset(), change_byte_order);
    return bvh.Get();
  }
  ...
  static ByteBufferViewVarHandle* CreateByteBufferViewVarHandle(Thread* const self,
    ...
    bvh->SetFieldBoolean<false>(ByteBufferViewVarHandle::ChangeByteOrderOffset(),
                                change_byte_order);
    return bvh.Get();
  }
...
TEST_F(VarHandleTest, ByteArrayViewVarHandle) {
  ...
  const bool change_byte_order = true;
  Handle<mirror::ByteArrayViewVarHandle> vh(hs.NewHandle(CreateByteArrayViewVarHandle(self, char_array_class, change_byte_order, mask)));
  EXPECT_FALSE(vh.IsNull());
  EXPECT_EQ(change_byte_order, vh->GetChangeByteOrder());
...
TEST_F(VarHandleTest, ByteBufferViewVarHandle) {
  ...
  const bool change_byte_order = false;
  Handle<mirror::ByteBufferViewVarHandle> vh(hs.NewHandle(CreateByteBufferViewVarHandle(self, double_array_class, change_byte_order, mask)));
  EXPECT_FALSE(vh.IsNull());
  EXPECT_EQ(change_byte_order, vh->GetChangeByteOrder());
