...
class MANAGED MethodType : public Object {
 public:
  ...
  bool IsNonExactMatch(mirror::MethodType* target) REQUIRES_SHARED(Locks::mutator_lock_);

  // Returns true iff. |this| is a match for |target|. The comparison
  // is made with IsExactMatch() if |exact| is true, targetwise with IsNonExactMatch().
  bool IsMatch(bool exact, mirror::MethodType* target) REQUIRES_SHARED(Locks::mutator_lock_) {
    return exact ? IsExactMatch(target) : IsNonExactMatch(target);
  }
