...
void ClassExt::SetObsoleteArrays(ObjPtr<PointerArray> methods,
  ...
  // TODO This might eventually need some synchronization with the Getters. ATM though I think
  // everything is fine since this will only be called when all other threads are paused.
  // TODO Is this needed?
  DCHECK_EQ(GetLockOwnerThreadId(), Thread::Current()->GetThreadId())
      << "Obsolete arrays are set without synchronization!";
  // CHECK(methods == nullptr);
  // CHECK(dex_caches == nullptr);
  ...
  if (Runtime::Current()->IsActiveTransaction()) {
    SetFieldObject<true>(obsolete_dex_cache_off, dex_caches.Ptr());
    SetFieldObject<true>(obsolete_methods_off, methods.Ptr());
  } else {
...
// TODO Should maybe just move to an out-ptr
...
bool ClassExt::ExtendObsoleteArrays(Thread* self, uint32_t increase) {
  ...
  Handle<ObjectArray<DexCache>> new_dex_caches(hs.NewHandle<ObjectArray<DexCache>>(
      ObjectArray<DexCache>::Alloc(
          self, cl->FindClass(self, "[Ljava/lang/DexCache;", ScopedNullHandle<ClassLoader>()), new_len)));
