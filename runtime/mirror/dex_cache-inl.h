...
class DexCacheArray {
 public:
  ...
  // The size of the array is encoded in the low bits of the data pointer.
  // This avoids need for any synchronization when they arrays are resized.
...
class DexCacheMap {
 public:
  ...
  void Set(uint32_t key, V value) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (resize) {
      SetPackedField(DexCacheArray<K, V>::Allocate(cache_, map.size() * 2));
    }
  }
...
inline void DexCache::ClearMethodType(dex::ProtoIndex proto_idx) {
  DCHECK(Runtime::Current()->IsAotCompiler());
  ResolvedMethodTypes().Set(proto_idx.index_, GcRoot<MethodType>());
}
...
inline CallSite* DexCache::GetResolvedCallSite(uint32_t call_site_idx) {
  ...
  std::atomic<GcRoot<CallSite>>* call_sites = GetResolvedCallSites();
  if (UNLIKELY(call_sites == nullptr)) {
    return nullptr;
  }
  return call_sites[call_site_idx].load(std::memory_order_seq_cst).Read();
}
...
inline ObjPtr<CallSite> DexCache::SetResolvedCallSite(uint32_t call_site_idx,
  ...
  std::atomic<GcRoot<CallSite>>* call_sites = GetResolvedCallSites();
  if (UNLIKELY(call_sites == nullptr)) {
    // The lock ensures we allocate on just one thread.
    WriterMutexLock mu(Thread::Current(), *Locks::classlinker_classes_lock_);
    call_sites = GetResolvedCallSites();
    if (call_sites == nullptr) {
      size_t size = sizeof(std::atomic<GcRoot<CallSite>>) * GetDexFile()->NumCallSiteIds();
      void* array = AllocArrayLocked(sizeof(std::atomic<GcRoot<CallSite>>), size);
      memset(array, 0, size);
      std::atomic_thread_fence(std::memory_order_release);
      call_sites = new (array) std::atomic<GcRoot<CallSite>>[GetDexFile()->NumCallSiteIds()];
      SetFieldPtr<false>(OFFSET_OF_OBJECT_MEMBER(DexCache, resolved_call_sites_), call_sites);
    }
  }

  // The first assignment for a given call site wins.
  if (call_sites[call_site_idx].compare_exchange_strong(seen_call_site, candidate)) {
