...
// Separate declaration required only to specify the lock attributes.
template <typename T, ReadBarrierOption kReadBarrierOption, typename Visitor>
...
inline void VisitDexCachePairs(std::atomic<DexCachePair<T>>* pairs,
  ...
  for (size_t i = 0; i < num_pairs; ++i) {
    ...
    // failure due to the ambiguity caused by the "<" sign given that
    // kReadBarrierOption is of type ReadBarrierOption, which is an
    // enum value.
