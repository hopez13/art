...
class DexCacheMap {
  ...
  class Buckets {
   public:
    ...
    ALWAYS_INLINE std::optional<std::atomic<V>*> Get(uint32_t key) {
      ...
      while (true) {
        ...
        if (UNLIKELY(next == kNoNextEntry)) {
          return std::optional<std::atomic<V>*>();
        }
        index = next;
      }
    }
 ...
 public:
  ...
  ALWAYS_INLINE V Get(uint32_t key) REQUIRES_SHARED(Locks::mutator_lock_) {
    for (Buckets* buckets = GetBuckets(); buckets != nullptr; buckets = buckets->Next()) {
      std::optional<std::atomic<V>*> v = buckets->Get(key);
      if (v.has_value()) {
        if (kIsDebugBuild) {
          for (buckets = buckets->Next(); buckets != nullptr; buckets = buckets->Next()) {
            DCHECK(!buckets->Get(key).has_value()) << "Key found in multiple buckets";
          }
        }
  ...
  std::optional<std::atomic<V>*> GetLValueBottomUp(Buckets* buckets, uint32_t key)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    if (buckets == nullptr) {
      return std::optional<std::atomic<V>*>();  // Not found.
    }
    std::optional<std::atomic<V>*> v = GetLValueBottomUp(buckets->Next(), key);
    if (v.has_value()) {
      DCHECK(!buckets->Get(key).has_value()) << "Key found in multiple buckets";
      return v;  // Found in older bucket.
    }
    return buckets->GetLValue(key);
  }
  ...
  std::atomic<V>* GetLValue(uint32_t key) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    std::optional<std::atomic<V>*> v = GetLValueBottomUp(buckets, key);
    if (v.has_value()) {
      return v.value();
    }
    ...
    buckets = GetBuckets();
    v = GetLValueBottomUp(buckets, key);
    if (v.has_value()) {
      return v.value();
    }
    ...
    buckets = new (alloc_data) Buckets(new_size, buckets);
  ...
  void ForEachEntry(EntryCallback callback)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    for (Buckets* buckets = GetBuckets(); buckets != nullptr; buckets = buckets->Next()) {
      for (auto& entry : *buckets) {
