...
inline CallSite* DexCache::GetResolvedCallSite(uint32_t call_site_idx) {
  ...
  return GetResolvedCallSites()[call_site_idx].Read();
}
...
inline CallSite* DexCache::SetResolvedCallSite(uint32_t call_site_idx, CallSite* resolved) {
  ...
  reinterpret_cast<Atomic<GcRoot<mirror::CallSite>>&>(GetResolvedCallSites()[call_site_idx]).
      CompareExchangeStrongSequentiallyConsistent(GcRoot<CallSite>(nullptr),
                                                  GcRoot<CallSite>(resolved));
  return GetResolvedCallSites()[call_site_idx].Read();
}
