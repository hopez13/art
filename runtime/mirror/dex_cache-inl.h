...
class DexCacheMap {
 ...
 public:
  ...
  ALWAYS_INLINE size_t Capacity() REQUIRES_SHARED(Locks::mutator_lock_) {
    size_t capacity = 0;
      ...
      if (v.has_value()) {
        if (kIsDebugBuild) {
          for (buckets = buckets->Next(); buckets != nullptr; buckets = buckets->Next()) {
            DCHECK(!buckets->Lookup(key).has_value()) << "Key found in multiple buckets";
          }
        }
  ...
  std::atomic<V>* Insert(uint32_t key) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    std::optional<std::atomic<V>*> v = Insert(buckets, key);
    if (v.has_value()) {
      return v.value();
    }
    ...
    v = buckets->Lookup(key, /*insert=*/ true);
    DCHECK(v.has_value());
    SetBuckets(buckets);
    return v.value();
  }
 ...
 private:
  std::optional<std::atomic<V>*> Insert(Buckets* buckets, uint32_t key)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    if (buckets == nullptr) {
      return std::optional<std::atomic<V>*>();  // Not found.
    }
    // Try to insert in the oldest bucket first, to ensure that we never insert the key
    // in multiple buckets (if different inserting thread still sees some older bucket).
    std::optional<std::atomic<V>*> v = Insert(buckets->Next(), key);
    if (v.has_value()) {
      DCHECK(!buckets->Lookup(key).has_value()) << "Key found in multiple buckets";
      return v;  // Found in older bucket.
    }
    return buckets->Lookup(key, /*insert=*/ true);
  }
