...
void DexCache::VisitReflectiveTargets(ReflectiveValueVisitor* visitor) {
  ...
  for (size_t i = 0; fields != nullptr && i < num_fields; i++) {
    ...
    if (UNLIKELY(new_val != pair.object)) {
      if (new_val == nullptr) {
        pair = NativeDexCachePair<ArtField>(nullptr, NativeDexCachePair<ArtField>::InvalidIndexForSlot(i));
      } else {
        pair.object = new_val;
      }
  ...
  for (size_t i = 0; methods != nullptr && i < num_methods; i++) {
    ...
    if (UNLIKELY(new_val != pair.object)) {
      if (new_val == nullptr) {
        pair = NativeDexCachePair<ArtMethod>(nullptr, NativeDexCachePair<ArtMethod>::InvalidIndexForSlot(i));
      } else {
        pair.object = new_val;
      }
