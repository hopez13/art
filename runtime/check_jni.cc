 ...
 public:
  ...
  bool CheckMethodAndSig(ScopedObjectAccess& soa, jobject jobj, jclass jc,
    ...
    if (type != Primitive::GetType(m->GetShorty()[0])) {
      AbortF("the return type of %s does not match %s", function_name_,
             ArtMethod::PrettyMethod(m).c_str());
      return false;
    }
    ...
    if (is_static != m->IsStatic()) {
      if (is_static) {
        AbortF("calling non-static method %s with %s",
               ArtMethod::PrettyMethod(m).c_str(), function_name_);
      } else {
    ...
    if (invoke != kVirtual) {
      ...
      if (!m->GetDeclaringClass()->IsAssignableFrom(c)) {
        AbortF("can't call %s %s with class %s", invoke == kStatic ? "static" : "nonvirtual",
            ArtMethod::PrettyMethod(m).c_str(), mirror::Class::PrettyClass(c).c_str());
        return false;
      }
    }
    if (invoke != kStatic) {
      ...
      if (o == nullptr) {
        AbortF("can't call %s on null object", ArtMethod::PrettyMethod(m).c_str());
        return false;
      } else if (!o->InstanceOf(m->GetDeclaringClass())) {
        AbortF("can't call %s on instance of %s", ArtMethod::PrettyMethod(m).c_str(),
  ...
  bool CheckStaticMethod(ScopedObjectAccess& soa, jclass java_class, jmethodID mid)
    ...
    ObjPtr<mirror::Class> c = soa.Decode<mirror::Class>(java_class);
    if (!m->GetDeclaringClass()->IsAssignableFrom(c)) {
      AbortF("can't call static %s on class %s", ArtMethod::PrettyMethod(m).c_str(),
             mirror::Class::PrettyClass(c).c_str());
      return false;
    }
    return true;
  ...
  bool CheckConstructor(ScopedObjectAccess& soa, jmethodID mid)
    ...
    if (!method->IsConstructor() || method->IsStatic()) {
      AbortF("expected a constructor but %s: %p", ArtMethod::PrettyMethod(method).c_str(), mid);
      return false;
    }
    return true;
  }
  ...
  bool CheckThrowableClass(ScopedObjectAccess& soa, jclass jc)
    ...
    if (!c->IsThrowableClass()) {
      AbortF("expected java.lang.Throwable class but got object of "
             "type %s: %p", mirror::Class::PrettyDescriptor(c).c_str(), c.Ptr());
      return false;
    }
    return true;
  }
  ...
  bool CheckInstantiableNonArray(ScopedObjectAccess& soa, jclass jc)
    ...
    if (!c->IsInstantiableNonArray()) {
      AbortF("can't make objects of type %s: %p", mirror::Class::PrettyDescriptor(c).c_str(),
             c.Ptr());
      return false;
    }
    return true;
  ...
  bool CheckPrimitiveArrayType(ScopedObjectAccess& soa, jarray array, Primitive::Type type)
    ...
    if (a->GetClass()->GetComponentType()->GetPrimitiveType() != type) {
      AbortF("incompatible array type %s expected %s[]: %p",
             mirror::Class::PrettyDescriptor(a->GetClass()).c_str(),
             PrettyDescriptor(type).c_str(), array);
      return false;
    }
    return true;
  }
    ...
    if (is_static) {
      ...
      if (o == nullptr || !o->IsClass()) {
        AbortF("attempt to access static field %s with a class argument of type %s: %p",
               field->PrettyField().c_str(), mirror::Object::PrettyTypeOf(o).c_str(), fid);
        return false;
      }
    ...
    switch (fmt) {
      ...
      case 'c': {  // jclass
        ...
        if (c == nullptr) {
          ...
          *msg += mirror::Class::PrettyClass(c);
          if (!entry) {
