  ...
  bool CheckThread(JNIEnv* env) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    case kFlag_CritRelease:  // this is a "release" call
      if (threadEnv->critical_ == 0) {
        ...
        uint64_t critical_duration = threadEnv->critical_start_us_ - self->GetCpuMicroTime();
        if (critical_duration > kCriticalWarnTimeUs) {
          LOG(WARNING) << "JNI critical lock held for " << critical_duration << "us on " << *self;
        }
      }
      threadEnv->critical_--;
      break;
