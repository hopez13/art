...
class Jit {
 public:
  ...
  void Resume(bool invalidate_in_progress);
 ...
 private:
  ...
  class PauseHandler {
   public:
    PauseHandler(Thread* self, Jit* jit);
    ~PauseHandler();

    bool IsPaused();
    bool IsInvalidated();
    void WaitForPauseToFinish();
    void DcheckCommitWasValid();

   private:
    Thread* self_;
    Jit* jit_;
    bool is_jit_lock_held_;
  };
