...
class Jit {
 ...
 private:
  ...
  // The number of generations of jit-code that have been invalidated.
  uint64_t jit_generation_ GUARDED_BY(lock_);
  // The generation this thread was on when it started performing compilation. If this is different
  // from 'jit_generation_' the compilation should be aborted and any results ignored. This requires
  // holding the Jit::lock_ but the analysis doesn't support this.
  static thread_local uint64_t start_generation_;  // GUARDED_BY(lock_);
