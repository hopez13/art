...
class TestZygoteMemory : public testing::Test {
 public:
  void BasicTest() {
    ...
    int32_t* addr =
        (int32_t*)mmap(nullptr, kPageSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd.get(), 0);
    CHECK(addr != nullptr);
    ...
    uint32_t* addr2 =
        (uint32_t*)mmap(nullptr, kPageSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd.get(), 0);
    CHECK_EQ(addr2, MAP_FAILED);

    // With the existing mapping, we can toggle read/write.
    CHECK_EQ(mprotect(addr, size, PROT_READ), 0);
    CHECK_EQ(mprotect(addr, size, PROT_READ | PROT_WRITE), 0);
#endif
  }
};
