...
class TestZygoteMemory : public testing::Test {
 public:
  ...
  void TestFromSharedToPrivate() {
    ...
    // Create another mapping to communicate between processes.
    android::base::unique_fd fd2(JitMemoryRegion::CreateZygoteMemory(size, &error_msg));
    CHECK_NE(fd2.get(), -1);
    volatile int32_t* shared = reinterpret_cast<volatile int32_t*>(
    ...
    int32_t parent_value = 66;
    int32_t child_value = 33;
    int32_t starting_value = 22;
    ...
    pid = fork();
    if (pid == 0) {
      // Map it private with write access.
      CHECK_EQ(mmap(addr, kPageSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_FIXED, fd.get(), 0),
               addr);
      addr[0] = child_value;
    ...
    if (pid == 0) {
      ...
      CHECK_EQ(mmap(addr, kPageSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_FIXED, fd.get(), 0), addr);
      CHECK_EQ(addr[0], parent_value);
      exit(0);
    } else {
