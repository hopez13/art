...
class TestZygoteMemory : public testing::Test {
 public:
  void BasicTest() {
    ...
    android::base::unique_fd fd(JitMemoryRegion::CreateZygoteMemory(kPageSize, &error_msg));
    CHECK_NE(fd.get(), -1);

    // Create a writable mapping.
    int32_t* addr =
        (int32_t*)mmap(NULL, kPageSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd.get(), 0);
    CHECK(addr != nullptr);
    ...
    uint32_t* addr2 =
        (uint32_t*)mmap(NULL, kPageSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd.get(), 0);
