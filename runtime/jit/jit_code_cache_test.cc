...
JitFunction WriteJitFunction(uint32_t* instruction_address, size_t instruction_count) {
  // TODO:
  // Might there be better sequences for trying to get a repro?
  // Conditionally (but always) branching over an undefined instruction?
  // Other sequences?
...
void* WorkerMain(void* cookie) {
  ...
  barrier->Initialize();
  while (g_iteration.load() < kMaxIterations) {
    for (size_t i = 0; i < kJitFunctionCount; ++i) {
      g_jit_function[i]();
...
void DriverMain(Barrier* barrier) {
  ...
  while (++g_iteration < kMaxIterations) {
    ...
    barrier->Unblock();
    Spin(1000);
    if ((g_iteration.load() % kReportIterations) == 0) {
      fputc('.', stdout);
      fflush(stdout);
    }
  }
