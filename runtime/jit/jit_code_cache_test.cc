...
void UpdateJitFunction(size_t index) {
  ...
  int mb_result = membarrier(MEMBARRIER_CMD_PRIVATE_EXPEDITED_SYNC_CORE, 0);
  if (mb_result != 0) {
    perror("membarrier(MEMBARRIER_CMD_PRIVATE_EXPEDITED_SYNC_CORE)");
  }
...
int main() {
  ...
  int membarrier_cmds = membarrier(MEMBARRIER_CMD_QUERY, 0);
  if (membarrier_cmds == -1) {
    perror("membarrier(MEMBARRIER_CMD_QUERY)");
  } else {
    fprintf(stderr, "MEMBARRIER_CMD_QUERY %08x\n", membarrier_cmds);
  }

  int register_result = membarrier(MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED_SYNC_CORE, 0);
  if (register_result != 0) {
    perror("membarrier(MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED_SYNC_CORE)");
    exit(EXIT_FAILURE);
  }
