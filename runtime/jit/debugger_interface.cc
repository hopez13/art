...
static void RepackEntries(const PackElfFileForJITFunction& pack,
  ...
  for (auto group_it = entries.begin(); group_it != entries.end();) {
    ...
    JITCodeEntry* new_entry = CreateJITCodeEntryInternal(
        __jit_debug_descriptor,
        __jit_debug_register_code_ptr,
        ArrayRef<const uint8_t>(packed),
        /*copy_symfile=*/ true);
    new_entry->addr_ = group_ptr;
    new_entry->allow_packing_ = true;
    new_entry->is_compressed_ = compress;
    for (auto it : elfs) {
...
void AddNativeDebugInfoForJit(const void* code_ptr,
  ...
  JITCodeEntry* entry = CreateJITCodeEntryInternal(
      ...
      ArrayRef<const uint8_t>(symfile),
      /*copy_symfile=*/ true);
  entry->addr_ = code_ptr;
  entry->allow_packing_ = allow_packing;
  entry->is_compressed_ = false;
...
void RemoveNativeDebugInfoForJit(ArrayRef<const void*> removed, PackElfFileForJITFunction pack) {
  MutexLock mu(Thread::Current(), g_jit_debug_lock);
