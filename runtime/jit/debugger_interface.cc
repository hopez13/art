...
static Mutex g_jit_removed_entries_lock("JIT remove debug entry", kNativeDebugInterfaceShortLock);
...
// We postpone removal, so that it is done in bulk and outside the GC hot-path.
static std::deque<const void*> g_jit_removed_entries GUARDED_BY(g_jit_removed_entries_lock);
...
static void MaybePackJitMiniDebugInfo(Thread* self,
  ...
  constexpr uint32_t kGroupSize = 16 * KB;
  // Even if there are no removed entries, we want to pack new entries on regular basis.
  constexpr uint32_t kPackFrequency = 64;

  std::deque<const void*> removed_entries;
  {
    // Read and clear the list of entries that are pending removal.
    // We keep the lock for very short time to avoid blocking the GC.
    MutexLock mu(self, g_jit_removed_entries_lock);
    removed_entries.swap(g_jit_removed_entries);
  }
  std::sort(removed_entries.begin(), removed_entries.end());
  if (removed_entries.empty() && g_jit_debug_entries.size() % kPackFrequency != 0) {
...
void AddNativeDebugInfoForJit(Thread* self,
  ...
  MaybePackJitMiniDebugInfo(self, pack, isa, features);
...
void RemoveNativeDebugInfoForJit(Thread* self, const void* code_ptr) {
  ...
  if (g_jit_debug_lock.ExclusiveTryLock(self)) {
    bool empty = g_jit_debug_entries.empty();
    g_jit_debug_lock.ExclusiveUnlock(self);
    if (empty) {
      return;
    }
  }
