...
extern "C" {
  ...
  struct JITDescriptor : public JITDescriptorPublic {
    const JITCodeEntry* tail_;                    // Tail of link list of all live entries.
...
// Insert 'entry' in the linked list before 'next' and mark it as valid.
// This method must called under global lock (g_jit_debug_lock or g_dex_debug_lock).
...
static void InsertNewEntry(const JITCodeEntry* entry, const JITCodeEntry* next) {
  ...
  // Backward pointers should not be used be used by readers, so they are non-atomic.
  if (next != nullptr) {
    NativeInfo::Writable(next)->prev_ = entry;
  } else {
    descriptor.tail_ = entry;
  }
  // Forward pointers must be atomic and they must point to valid entry at all times.
  if (prev != nullptr) {
