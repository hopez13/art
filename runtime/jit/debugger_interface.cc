...
static uint32_t LockActionCounter(JITDescriptor& descriptor) {
  return descriptor.action_counter_.exchange(~0u);
}
...
static void UnlockActionCounter(JITDescriptor& descriptor, uint32_t old_value) {
  descriptor.action_counter_.store(old_value + 1);
}
...
static JITCodeEntry* CreateJITCodeEntryInternal(
  ...
  uint32_t old_action_counter = LockActionCounter(descriptor);
...
void AddNativeDebugInfoForJit(const void* handle, const std::vector<uint8_t>& symfile) {
  ...
  CHECK_EQ(__jit_debug_entries.count(handle), 0u);
...
void RemoveNativeDebugInfoForJit(const void* handle) {
  ...
  DeleteJITCodeEntryInternal(__jit_debug_descriptor,
                             __jit_debug_register_code_ptr,
                             entry);
  __jit_debug_entries.erase(handle);
