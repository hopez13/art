...
static void PackJitMiniDebugInfo(JITCodeEntries* entries,
...
void AddNativeDebugInfoForJit(Thread* self,
  ...
  if (!g_jit_removed_entries.empty()) {
    ...
    PackJitMiniDebugInfo(&g_compressed_jit_debug_entries, pack, isa, features, true);
  ...
  if (g_jit_num_unpacked_entries >= kJitMaxUnpackedEntries) {
    PackJitMiniDebugInfo(&g_uncompressed_jit_debug_entries, pack, isa, features, false);
    g_jit_num_unpacked_entries = 0;
  }
}
