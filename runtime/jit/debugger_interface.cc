...
// Some writes are synchronized so that libunwindstack can read the memory safely from other process.
constexpr std::memory_order non_racing_relaxed = std::memory_order_relaxed;
...
static const JITCodeEntry* CreateJITCodeEntryInternal(
  ...
  const JITCodeEntry* next = head->load(non_racing_relaxed);

  // Pop entry from the free list.
  const JITCodeEntry* entry = descriptor.free_entries_;
  descriptor.free_entries_ = descriptor.free_entries_->next_.load(non_racing_relaxed);
  CHECK_EQ(entry->seqlock_.load(non_racing_relaxed) & 1, 1u) << "Expected invalid entry";

  // Create the entry and set all its fields.
  JITCodeEntry* writable_entry = NativeInfo::Writable(entry);
  writable_entry->next_.store(next, non_racing_relaxed);
...
static void DeleteJITCodeEntryInternal(const JITCodeEntry* entry) {
  ...
  const JITCodeEntry* next = entry->next_.load(non_racing_relaxed);
  if (entry->prev_ != nullptr) {
    NativeInfo::Writable(entry->prev_)->next_.store(next, non_racing_relaxed);
  } else {
    descriptor.head_.store(next, non_racing_relaxed);
  }
  if (next != nullptr) {
  ...
  // Delete the entry.
  JITCodeEntry* writable_entry = NativeInfo::Writable(entry);
  CHECK_EQ(writable_entry->seqlock_.load(non_racing_relaxed) & 1, 0u) << "Expected valid entry";
  ...
  writable_entry->next_.store(descriptor.free_entries_, non_racing_relaxed);
...
void NativeDebugInfoPreFork() {
  ...
  app_tail->seqlock_.store(2, non_racing_relaxed);  // Mark as valid.
