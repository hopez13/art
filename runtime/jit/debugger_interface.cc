...
struct JitNativeInfo {
  ...
  static void Free(void* ptr) { Memory()->FreeData(reinterpret_cast<uint8_t*>(ptr)); }
...
static const JITCodeEntry* CreateJITCodeEntryInternal(
  ...
  if (memory == nullptr) {
    LOG(ERROR) << "Failed to allocate memory for native debug info";
    if (copy != nullptr) {
      NativeInfo::Free(NativeInfo::Writable(copy));
    }
    return nullptr;
  }
...
static void DeleteJITCodeEntryInternal(const JITCodeEntry* entry) {
  ...
  NativeInfo::Free(NativeInfo::Writable(entry));
  if (NativeInfo::kCopySymfileData) {
    NativeInfo::Free(NativeInfo::Writable(symfile));
  }
}
