...
//
// There are three ways for native tools to access the debug data safely:
//
...
struct JitNativeInfo {
  ...
  template<class T> static T* Writable(const T* v) {
    // Special case: This entry is static and not allocated in JIT memory.
...
static const JITCodeEntry* CreateJITCodeEntryInternal(
  ...
  descriptor.free_entries_ = descriptor.free_entries_->next_.load();
...
static void DeleteJITCodeEntryInternal(const JITCodeEntry* entry) {
  ...
  writable_entry->next_.store(descriptor.free_entries_);
...
void NativeDebugInfoPreFork() {
  ...
  app_tail->seqlock_.store(2, std::memory_order_release);  // Mark as valid.
