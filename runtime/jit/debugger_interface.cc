...
void AddNativeDebugInfoForJit(const void* handle, const std::vector<uint8_t>& symfile) {
  ...
  DCHECK(entry == nullptr) << "Native debug entry already exists for " << std::hex << handle;
  if (entry == nullptr) {
    ...
    entry = CreateJITCodeEntryInternal(__jit_debug_descriptor,
...
void RemoveNativeDebugInfoForJit(const void* handle) {
  JITCodeEntry* entry = __jit_debug_entries[handle];
  DCHECK(entry != nullptr) << "Could not find native debug entry for " << std::hex << handle;
  if (entry != nullptr) {
