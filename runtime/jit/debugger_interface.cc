...
static void RepackJitMiniDebugInfo(PackElfFileForJITFunction pack,
                                   InstructionSet isa,
  ...
  while (!entries->empty()) {
    ...
    // Create new sinlge JITCodeEntry that covers this memory range.
...
void AddNativeDebugInfoForJit(Thread* self,
  ...
  if (!g_jit_removed_entries.empty()) {
    ...
    RepackJitMiniDebugInfo(pack, isa, features, true, &g_compressed_jit_debug_entries);
  ...
  if (g_jit_num_unpacked_entries >= kJitMaxUnpackedEntries) {
    RepackJitMiniDebugInfo(pack, isa, features, false, &g_uncompressed_jit_debug_entries);
    g_jit_num_unpacked_entries = 0;
  }
}
