...
void RegisterDexFileForNative(const void* dexfile) {
  MutexLock mu(Thread::Current(), *Locks::native_debug_interface_lock_);
  if (g_dexfile_entries.count(dexfile) == 0) {
    ...
    g_dexfile_entries.emplace(dexfile, entry);
  }
}

void DeregisterDexFileForNative(const void* dexfile) {
  MutexLock mu(Thread::Current(), *Locks::native_debug_interface_lock_);
  auto it = g_dexfile_entries.find(dexfile);
