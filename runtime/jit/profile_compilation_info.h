 ...
 private:
  ...
  class DexFileData : public DeletableArenaObject<kArenaAllocProfile> {
   public:
    ...
    DexFileData(DexFileData&& other) = default;
    DexFileData& operator=(DexFileData&& other) = default;
    ...
    const ArenaSet<dex::TypeIndex>& GetClassSet() const {
      return class_set_;
    }
    ...
    // Merge with another dex file data.
    void MergeWith(DexFileData&& other,
                   const SafeMap<uint8_t, uint8_t>& dex_profile_index_remap);

    MethodMap& MethodInlineCaches() {
      return method_inline_caches_;
    }

    const MethodMap& MethodInlineCaches() const {
      return method_inline_caches_;
    }
    ...
    bool operator==(const DexFileData& other) const {
      return location_checksum_ == other.location_checksum_ &&
          method_inline_caches_ == other.method_inline_caches_;
    }
  ...
  static DexPcData* FindOrAddDexPc(InlineCacheMap* inline_cache,
                                   uint32_t dex_pc,
