...
class ProfileCompilationInfo {
 ...
 private:
  ...
  class DexFileData : public ArenaObject<kArenaAllocProfile> {
   public:
    DexFileData(ArenaAllocator* arena,
                const std::string& profile_key,
                uint32_t location_checksum,
                uint16_t profile_index,
                uint32_t num_method_idx)
        : arena_(arena),
          profile_key_(profile_key),
          profile_index_(profile_index),
          checksum_(location_checksum),
          method_bitmap_storage_(RoundUp(num_method_idx * ProfileMethodInfo::kHotnessBits,
                                         kBitsPerByte),
                                 0,
                                 arena->Adapter(kArenaAllocProfile)),
          method_bitmap_(MemoryRegion(method_bitmap_storage_.data(),
                                      method_bitmap_storage_.size()),
                         0,
                         num_method_idx * ProfileMethodInfo::kHotnessBits),
