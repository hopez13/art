...
class ProfileCompilationInfo {
 public:
  ...
  // Add the given initialized classes to the current profile object.
  bool AddInitializedClasses(const std::set<DexCacheResolvedClasses>& resolved_classes);
 ...
 private:
  ...
  struct DexFileData : public DeletableArenaObject<kArenaAllocProfile> {
    ...
    // The classes which have been profiled and have been initialized.
    // Note that these don't necessarily include all the classes that can be
    // found in the inline caches reference.
  ...
  // Add an initialized class index to the profile.
  bool AddInitializedClassIndex(const std::string& dex_location,
                                uint32_t checksum,
                                dex::TypeIndex type_idx,
                                uint32_t num_method_ids);
  ...
  // Add all initialized classes from the given dex cache to the the profile.
  bool AddInitializedResolvedClasses(const DexCacheResolvedClasses& classes);
