...
class ProfileCompilationInfo {
 public:
  ...
  // This will clear the existing data from the profile.
  bool Load(int fd);
 ...
 private:
  ...
  // Note that we could do without profile_key and profile_index fields in this
  // struct because we can infer them from profile_key_map_ and info_. However,
  // it makes the profiles logic much simpler if we have references here as well.
  struct DexFileData {
