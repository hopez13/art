...
class ProfileCompilationInfo {
 public:
  ...
  struct DexPcData : public ArenaObject<kArenaAllocProfile> {
    explicit DexPcData(ArenaAllocator* arena) : is_missing_types(false), is_megamorphic(false), classes(std::less<ClassReference>(), arena->Adapter(kArenaAllocProfile)) {}
  ...
  explicit ProfileCompilationInfo(const ProfileCompilationInfo* pci);
  ~ProfileCompilationInfo();
 ...
 private:
  ...
  struct DexFileData : public ArenaObject<kArenaAllocProfile> {
    DexFileData(ArenaAllocator* arena, const std::string& key, uint32_t location_checksum, uint16_t index)
        : arena_(arena),
          profile_key(key),
          profile_index(index),
