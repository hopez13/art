...
class JitCodeCache {
 public:
  ...
  void PostForkChildAction(bool is_system_server);

 private:
  JitCodeCache();

  void InitState(size_t initial_capacity, size_t max_capacity) REQUIRES(lock_);

  bool InitMappings(bool rwx_memory_allowed, bool is_zygote, std::string* error_msg)
      REQUIRES(lock_);

  void InitSpaces() REQUIRES(lock_);
