...
class ZygoteMap {
 public:
  explicit ZygoteMap(JitMemoryRegion* region) : map_(nullptr), capacity_(0), region_(region) {}
 ...
 private:
  ...
  // Pointer to the map, allocated with `region_`. Implementation note: for
  // simplicity, we use linear probing.
  Entry* map_;

  // Capacity of the map. Must be a power of two.
  uint32_t capacity_;
