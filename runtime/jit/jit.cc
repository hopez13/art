...
void Jit::MaybeCompileMethod(Thread* self,
                             ArtMethod* method,
                             uint32_t old_count,
                             uint32_t new_count,
                             bool with_backedges) {
  ...
  if (IgnoreSamplesForMethod(method)) {
    // Keep the counter of such methods at 0 (as required by the tests).
    method->SetCounter(0);
    return;
  }
  ...
  if (UseJitCompilation()) {
    ...
    if (old_count < OSRMethodThreshold() && new_count >= OSRMethodThreshold()) {
      if (!with_backedges) {
        // If the samples don't contain any back edge, we reset the counter and retry later.
        method->SetCounter(old_count);
        return;
      }
