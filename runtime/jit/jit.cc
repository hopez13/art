...
JitOptions* JitOptions::CreateFromRuntimeArguments(const RuntimeArgumentMap& options) {
  ...
  const uint32_t kThresholdStep = Jit::kSlowMode ? 1u : kJitSamplesBatchSize;
  ...
  DCHECK_EQ(RoundUp(jit_options->warmup_threshold_, kThresholdStep),
            jit_options->warmup_threshold_);
  ...
  jit_options->compile_threshold_ = RoundUp(jit_options->compile_threshold_, kThresholdStep);
  ...
  jit_options->warmup_threshold_ = RoundUp(jit_options->warmup_threshold_, kThresholdStep);
  ...
  jit_options->osr_threshold_ = RoundUp(jit_options->osr_threshold_, kThresholdStep);
  ...
  if (jit_options->compile_threshold_ != 0) {
    ...
    jit_options->osr_threshold_ = std::clamp(jit_options->osr_threshold_,
                                             2u * kThresholdStep,
                                             RoundDown(kJitMaxThreshold, kThresholdStep));
    jit_options->compile_threshold_ = std::clamp(jit_options->compile_threshold_,
                                                 kThresholdStep,
                                                 jit_options->osr_threshold_ - kThresholdStep);
    jit_options->warmup_threshold_ = std::clamp(jit_options->warmup_threshold_,
                                                0u,
                                                jit_options->compile_threshold_ - kThresholdStep);
  }
