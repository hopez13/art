...
uint32_t Jit::CompileMethodsFromProfile(
  ...
  // We currently only JIT compile methods from a profile on profiles stored on
  // /system, which cannot be changed and therefore we don't need to lock the
  // file.
  DCHECK(LocationIsOnSystem(profile_file.c_str()));
  unix_file::FdFile profile(profile_file.c_str(), O_RDONLY, true);
