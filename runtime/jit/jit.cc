...
void Jit::CreateThreadPool() {
  ...
  if (runtime->IsZygote() && !runtime->IsUsingDefaultBootImageLocation()) {
    thread_pool_->AddTask(
        Thread::Current(), new JitCompileTask(nullptr, JitCompileTask::TaskKind::kZygote));
  }
}
...
void Jit::AddNonAotBootMethodsToQueue(Thread* self) {
  ...
  for (const DexFile* dex_file : boot_class_path) {
    ...
    if (dex_cache == nullptr) {
      LOG(ERROR) << "Could not find dex cache for " << dex_file->GetLocation();
      continue;
    }
