    ...
    if (page_end > page_start) {
      ...
      header.VisitPackedArtMethods([&](ArtMethod& method) NO_THREAD_SAFETY_ANALYSIS {
        ...
        if (!method.GetDeclaringClassUnchecked()->IsVisiblyInitialized() &&
          ...
          // into the mapping.
          if (pointer < page_start && (pointer + sizeof(ArtMethod)) > page_start) {
            ...
            memcpy(GetChildMappingMethods().Begin() + offset + capacity - bytes_to_copy,
                   ...
                   bytes_to_copy);
          } else if (pointer > page_start && pointer < page_end) {
...
void Jit::CreateThreadPool() {
  ...
  if (runtime->IsZygote() && runtime->IsUsingApexBootImageLocation() && UseJitCompilation()) {
    ...
    // And create mappings to share boot image methods memory between the
    // child processes and the zygote.
    ...
    if (total_capacity > 0) {
      unique_fd mem_fd = unique_fd(art::memfd_create("boot-image-methods", /* flags= */ 0));
      if (mem_fd.get() == -1) {
