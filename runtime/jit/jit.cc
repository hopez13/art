...
// What the start_generation_ is by default.
thread_local uint64_t Jit::start_generation_ = -1;
  ...
  void Run(Thread* self) OVERRIDE {
    {
      ...
      // We are doing a compilation now.
      DCHECK(Jit::start_generation_ <= jit_->jit_generation_ ||
             Jit::start_generation_ == reinterpret_cast<uint64_t>(-1));
          << "jit generation should not go backwards: jit_generation = " << jit_->jit_generation_
          << "start_generation_ = " << Jit::start_generation_;
      Jit::start_generation_ = jit_->jit_generation_;
    }
...
void Jit::Resume(bool invalidate_in_progress) {
  ...
  if (invalidate_in_progress) {
    // Increase the jit_generation to mark all threads as being invalidated.
    jit_generation_++;
  }
  if (paused_count_ == 0) {
...
bool Jit::RunCommitCodeStep(Thread* self, Closure* closure) {
  MutexLock mu(self, lock_);
  while (paused_count_ > 0 && jit_generation_ == start_generation_) {
    lock_.Unlock(self);
    {
      // make sure we only have lock_ (release mutator_lock_) when we start waiting.
  ...
  if (jit_generation_ != start_generation_) {
    // We were killed.
    return false;
  }
