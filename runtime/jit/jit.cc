...
void Jit::AddCompileTask(Thread* self,
                         ...
                         bool precompile) {
  if (precompile) {
    // For simplicity, push a generic task for precompilation.
    thread_pool_->AddTask(
        self, new JitCompileTask(method, JitCompileTask::TaskKind::kPreCompile, compilation_kind));
  } else {
...
size_t JitThreadPool::GetTaskCount(Thread* self) {
  ...
  return tasks_.size() + baseline_queue_.size() + optimized_queue_.size() + osr_queue_.size();
}
...
Task* JitThreadPool::TryGetTaskLocked() {
  ...
  // OSR requests first, then baseline and finally optimized.
