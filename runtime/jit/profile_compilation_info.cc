...
bool ProfileCompilationInfo::AddSampledMethod(bool startup,
  ...
  DexFileData* data = GetOrAddDexFileData(dex_location, checksum, num_method_ids);
  if (data == nullptr) {
    return false;
  }
...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::ReadProfileLine(
      ...
      const ProfileLineHeader& line_header,
      /*out*/std::string* error) {
  if (GetOrAddDexFileData(line_header.dex_location,
                          line_header.checksum,
                          line_header.num_method_ids) == nullptr) {
