...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::LoadInternal(
  ...
  size_t compress_data_size = end - cur;
  lseek(fd, compress_data_size, SEEK_END);

  std::unique_ptr<uint8_t[]> compressed_data(new uint8_t[compress_data_size]);
  int bytes_read =
      TEMP_FAILURE_RETRY(read(fd, compressed_data.get(), compress_data_size));

  if (bytes_read < static_cast<int>(compress_data_size)) {
