...
void ProfileCompilationInfo::DexPcData::AddClass(uint16_t dex_profile_idx,
  ...
  // Check if the type index exists to avoid arena leaks.
  // Note that emplace() allocates the node before doing the lookup and if it then finds
  // an identical element, it shall deallocate the node. For Arena allocations,
  // that's essentially a leak.
  ...
  // The type does not exists and the inline cache will not be megamorphic.
  classes.insert(ref);
}
