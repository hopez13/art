...
uint32_t ProfileCompilationInfo::GetMethodsRegionSize(const DexFileData& dex_data) {
  ...
  // std::cout << "GetMethodsRegionSize " << dex_data.method_map.size();
  uint32_t size = sizeof(uint16_t) * 2 * dex_data.method_map.size();
  for (const auto& method_it : dex_data.method_map) {
    const InternalInlineCache& inline_cache = method_it.second;
...
void ProfileCompilationInfo::AddInlineCacheToBuffer(std::vector<uint8_t>* buffer,
                                                    const InternalInlineCache& inline_cache) {
  ...
  for (auto inline_cache_it : inline_cache) {
    ...
    const DexPcData dexPcData = inline_cache_it.second;
    const ClassSet& classes = dexPcData.classes;
...
bool ProfileCompilationInfo::AddMethodIndex(const std::string& dex_location,
  ...
  return AddMethod(OfflineProfileMethodInfo(OfflineDexReference(dex_location, checksum),
                                            method_index));
}

bool ProfileCompilationInfo::AddMethod(const OfflineProfileMethodInfo& pmi) {
  ...
  auto ic_it = data->method_map.FindOrAdd(pmi.dex_method_index);
  // std::cout << "add method\n";
  for (const OfflineProfileInlineCache& inline_cache : pmi.inline_caches) {
    auto dex_pc_data_it = ic_it->second.FindOrAdd(inline_cache.dex_pc);
    if (inline_cache.classes.size() >= InlineCache::kIndividualCacheSize) {
      dex_pc_data_it->second.is_megamorphic = true;
      continue;
    }
    for (const OfflineProfileClassReference& class_ref : inline_cache.classes) {
      const auto dex_ref = class_ref.class_dex_ref;
      DexFileData* class_dex_data = GetOrAddDexFileData(GetProfileDexFileKey(dex_ref.dex_location),
                                                        dex_ref.dex_checksum);
      if (class_dex_data == nullptr) {
        return false;
      }
      dex_pc_data_it->second.classes.emplace(class_dex_data->profile_index, class_ref.type_index);
    }
  }
  return true;
}
...
bool ProfileCompilationInfo::ReadInlineCache(SafeBuffer& line_buffer,
                                             ...
                                             /*out*/ InternalInlineCache* inline_cache,
  ...
  for (; inline_cache_size > 0; inline_cache_size--) {
    ...
    for (; dex_to_classes_map_size > 0; dex_to_classes_map_size--) {
      ...
      if (!line_buffer.ReadUintAndAdvance<uint8_t>(&dex_classes_size)) {
        *error = "Could not read dex_classes_size";
        return false;
      }
      ...
      for (; dex_classes_size > 0; dex_classes_size--) {
        ...
        dex_pc_data_it->second.classes.emplace(dex_profile_index, dex::TypeIndex(type_index));
      }
    }
  }
  return true;
    ...
    for (const auto& other_method_it : other_dex_data.method_map) {
      ...
      for (const auto& other_ic_it : other_inline_cache) {
        ...
        for (const auto& class_it : other_class_set) {
          class_set->second.classes.emplace(dex_profile_index_remap.Get(
...
const ProfileCompilationInfo::InternalInlineCache*
...
bool ProfileCompilationInfo::GetMethod(const std::string& dex_location,
                                       ...
                                       /*out*/XXXProfileMethodInfo* pmi) const {
  const InternalInlineCache* inline_caches = FindMethod(dex_location,
                                                        dex_checksum,
                                                        dex_method_index);
  ...
  for (const auto& info_it : info_) {
    OfflineDexReference& dex_ref = pmi->dex_references[info_it.second.profile_index];
    dex_ref.dex_location = info_it.first;
    dex_ref.dex_checksum = info_it.second.checksum;
  }
  // TODO: maybe expose a direct pointer to avoid copying
