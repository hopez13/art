 ...
 *    dex_map_size is the number of dex_indecies that follows.
 *       Classes are group per their dex files and the line
...
bool ProfileCompilationInfo::MergeWith(const ProfileCompilationInfo& other) {
  ...
  // actual indices.
  // The reason we can't rely on the actual multidex index is that a single profile may store
