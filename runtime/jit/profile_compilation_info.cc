...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::OpenSource(
    ...
    /*out*/ std::string* error) {
  if (IsProfileFile(fd)) {
    source->reset(new ProfileSource(fd, nullptr));
    return kProfileLoadSuccess;
  } else {
    ...
    if (zip_entry == nullptr) {
      *error = std::string("Could not find entry ") + kDexMetadataProfileEntry +
            " in the zip archive";
      return kProfileLoadBadData;
    }
    ...
    if (map != nullptr) {
      source->reset(new ProfileSource(fd, std::move(map)));
      return kProfileLoadSuccess;
    } else {
      return kProfileLoadBadData;
    }
