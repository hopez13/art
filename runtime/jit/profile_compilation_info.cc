...
const uint8_t ProfileCompilationInfo::kProfileVersion[] = { '0', '0', '3', '\0' };
...
void ProfileCompilationInfo::AddInlineCacheToBuffer(std::vector<uint8_t>* buffer,
  ...
  for (auto inline_cache_it : inline_cache_map) {
    ...
    if (dexPcData.is_megamorphic) {
      ...
      // If megamorphic, we ndon't add the rest of the classes.
 ...
 *    M stands for megamorphic and it's encoded as the byte 7. When present, there will
 *    be no class ids following.
 **/
...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::ReadProfileLineHeader(
  ...
  if (!header_buffer.ReadUintAndAdvance<uint32_t>(&line_header->method_region_size_bytes)) {
    return kProfileLoadBadData;
  }
...
bool ProfileCompilationInfo::OfflineProfileMethodInfo::operator==(
  ...
  // Size match, but the dex references might not be the same.
  // This might be due to a different profile_index assignment for dex references.
