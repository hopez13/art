...
// We use MemMap for the profile arena in order to be able to madvise the memory
// right after profile destruction(jemalloc may delay the madvise).
ProfileCompilationInfo::ProfileCompilationInfo(const ProfileCompilationInfo* pci)
    : arena_pool_(new ArenaPool(/*use_malloc*/false, /*low_4gb*/false, "ProfileCompilationInfo" )),
      arena_(new ArenaAllocator(arena_pool_)),
...
ProfileCompilationInfo::ProfileCompilationInfo(const ProfileCompilationInfo& pci)
    : ProfileCompilationInfo(&pci) {
}

ProfileCompilationInfo::ProfileCompilationInfo() : ProfileCompilationInfo(nullptr) {}

ProfileCompilationInfo::~ProfileCompilationInfo() {
  MemStats mem_stats(arena_->GetMemStats());
  VLOG(profiler) << Dumpable<MemStats>(mem_stats);
}
...
bool ProfileCompilationInfo::AddMethodIndex(const std::string& dex_location,
  ...
  return AddMethod(dex_location, dex_checksum, method_index, OfflineProfileMethodInfo(arena_));
}
...
std::unique_ptr<ProfileCompilationInfo::OfflineProfileMethodInfo> ProfileCompilationInfo::GetMethod(
  ...
  std::unique_ptr<OfflineProfileMethodInfo> pmi(new OfflineProfileMethodInfo(arena_));
