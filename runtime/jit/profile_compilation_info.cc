...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  if (required_capacity > kProfileSizeErrorThreshholdInBytes) {
    LOG(ERROR) << "Profile data size exceeds "
               << std::to_string(kProfileSizeErrorThreshholdInBytes)
               << " bytes. Profile will not be written to disk.";
    return false;
  }
  if (required_capacity > kProfileSizeWarningThreshholdInBytes) {
    LOG(WARNING) << "Profile data size exceeds "
                 << std::to_string(kProfileSizeWarningThreshholdInBytes);
  }
...
bool ProfileCompilationInfo::ReadClasses(SafeBuffer& buffer,
  ...
  if (total_bytes_read != line_header.class_set_size) {
...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::LoadInternal(
  ...
  if (uncompressed_data_size > kProfileSizeErrorThreshholdInBytes) {
    LOG(ERROR) << "Profile data size exceeds "
               << std::to_string(kProfileSizeErrorThreshholdInBytes)
               << " bytes";
  }
  if (uncompressed_data_size > kProfileSizeWarningThreshholdInBytes) {
    LOG(WARNING) << "Profile data size exceeds "
                 << std::to_string(kProfileSizeWarningThreshholdInBytes)
                 << " bytes";
  }
  ...
  if (static_cast<off_t >(compressed_data_size) != end - cur) {
    *error += "compressed_data_size doesn't match the size of the compressed data";
    return kProfileLoadBadData;
  }
