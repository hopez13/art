...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  for (const DexFileData* dex_data_ptr : info_) {
    ...
    for (const auto& method_it : dex_data.method_map) {
      ...
      DCHECK_GT(method_it.first, last_method_index);
      uint16_t diff_with_last_method_id = method_it.first - last_method_index;
    ...
    for (const auto& class_id : dex_data.class_set) {
      ...
      DCHECK_GT(class_id.index_, last_class_index);
      uint16_t diff_with_last_class_id = class_id.index_ - last_class_index;
