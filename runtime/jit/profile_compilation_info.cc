...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  size_t data_size = LZ4_compress_HC(reinterpret_cast<const char*>(buffer.data()),
                                     ...
                                     compressed_max_size,
                                     0);
  VLOG(verifier) << "Compressed from " << required_capacity << " to " << data_size;
...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::ReadProfileLineHeader(
  ...
  line_header->dex_location.assign(
      reinterpret_cast<char*>(buffer.Get()), dex_location_size);
...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::LoadInternal(
  ...
  std::unique_ptr<uint8_t[]> compressed_data(new uint8_t[end-cur]);
  int bytes_read = TEMP_FAILURE_RETRY(read(fd, compressed_data.get(), end-cur));
