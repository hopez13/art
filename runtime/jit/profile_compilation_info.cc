...
bool ProfileCompilationInfo::Save(int fd) {
  ...
  bool success = WriteBuffer(fd, kProfileMagic, sizeof(kProfileMagic));
  if (!success) {
    return false;
  }
  success = WriteBuffer(fd, kProfileVersion, sizeof(kProfileVersion));
  if (!success) {
    return false;
  }
  ...
  if (required_capacity > kProfileSizeErrorThreshhold) {
    LOG(ERROR) << "Profile data size exceeds "
               << std::to_string(kProfileSizeErrorThreshhold)
               << " bytes";
  }
  if (required_capacity > kProfileSizeWarningThreshhold) {
    LOG(WARNING) << "Profile data size exceeds "
                 << std::to_string(kProfileSizeWarningThreshhold)
                 << " bytes";
  }
  AddUintToBuffer(&buffer, required_capacity);
  success = WriteBuffer(fd, buffer.data(), buffer.size());
  if (!success) {
    return false;
  }
  ...
  for (const DexFileData* dex_data_ptr : info_) {
    ...
    for (const auto& method_it : dex_data.method_map) {
      // Store the difference between the method ids. The SafeMap is ordered by
      // method_id, so the difference will always be positive.
    ...
    for (const auto& class_id : dex_data.class_set) {
      // Store the difference between the class ids. The set is ordered by
      // class_id, so the difference will always be positive.
      uint16_t diff_with_last_class_id = class_id.index_ - last_class_id;
      last_class_id = class_id.index_;
      AddUintToBuffer(&buffer, diff_with_last_class_id);
    }
  }
  ...
  success = WriteBuffer(fd, compressed_buffer.get(), output_size);
  if (!success) {
    return false;
  }
...
bool ProfileCompilationInfo::ReadClasses(SafeBuffer& buffer,
  ...
  for (uint16_t i = 0; i < classes_to_read; i++) {
    uint16_t diff_with_last_class_id;
    READ_UINT(uint16_t, buffer, diff_with_last_class_id, error);
    uint16_t type_index = last_class_id + diff_with_last_class_id;
    last_class_id = type_index;
...
ProfileCompilationInfo::ProfileLoadSatus ProfileCompilationInfo::LoadInternal(
  ...
  if (uncompressed_data_size > kProfileSizeErrorThreshhold) {
    LOG(ERROR) << "Profile data size exceeds "
                 << std::to_string(kProfileSizeErrorThreshhold)
                 << " bytes";
  }
  if (uncompressed_data_size > kProfileSizeWarningThreshhold) {
    LOG(WARNING) << "Profile data size exceeds "
                 << std::to_string(kProfileSizeWarningThreshhold)
                 << " bytes";
  }
  ...
  if (static_cast<int32_t >(compressed_data_size) != end - cur) {
    *error += "Bad profile data";
    return kProfileLoadBadData;
  }
  off_t lseek_pos = lseek(fd, -(static_cast<int32_t>(compressed_data_size)), SEEK_END);
  if (lseek_pos != cur) {
    *error += "Error reading compressed profile data";
  }
