...
class ProfileCompilationInfoTest : public CommonRuntimeTest {
 ...
 protected:
  ...
  bool AddClass(const std::string& dex_location,
    ...
    return info->AddClasses(std::set<DexCacheResolvedClasses> {classes});
  }
...
TEST_F(ProfileCompilationInfoTest, SampledMethodsTest) {
  ...
  auto run_test = [](const ProfileCompilationInfo& info) {
    ...
    EXPECT_TRUE(info.GetMethodHotness(kDex1, kChecksum1, 5).IsStartup());
  ...
  // Test bulk adding.
  {
    ...
    EXPECT_FALSE(info.GetMethodHotness(MethodReference(dex.get(), 0)).HasAnyFlags());
    EXPECT_FALSE(info.GetMethodHotness(MethodReference(dex.get(), 2)).HasAnyFlags());
