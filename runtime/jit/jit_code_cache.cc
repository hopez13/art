...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  //          +----------------+ --------------------
  //
  // The code_sync_map_ is used to drain the instruction queues of all
  // cores following the addition of new JIT code. It's a single page.
  //
  ...
  // of this function.
  //
  size_t total_padding_size = kMaxMapSpacingPages * kPageSize;
  size_t data_size = RoundUp((max_capacity - total_padding_size) / 2, kPageSize);
  size_t pre_code_size =
      GetRandomNumber(kMinMapSpacingPages, kMaxMapSpacingPages) * kPageSize;
  size_t code_size = max_capacity - total_padding_size - data_size;
  size_t post_code_size = total_padding_size - pre_code_size - kPageSize;
  size_t code_sync_size = kPageSize;
  DCHECK_EQ(code_size + data_size + total_padding_size + code_sync_size, max_capacity);
...
static void FlushInstructionPiplines(uint8_t* sync_page) {
  ...
  // flush their instruction fetch queues. In the absence of system
  // call to do this, we can achieve this by toggling permissions on a
  // writable page. This should send an IPI to each core to update the
  // TLB entry, and the IPI will discard the current instruction fetch
  // queue.
  CHECKED_MPROTECT(sync_page, kPageSize, kProtAll);
  // Ensure page is present otherwise a TLB update may not be necessary.
  sync_page[0] = 0;
