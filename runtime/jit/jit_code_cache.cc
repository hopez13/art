  ...
  for (const auto& entry : method_code_map_) {
    ...
    for (uint32_t i = 0; i < number_of_roots; ++i) {
      ...
      if (object == nullptr || object == Runtime::GetWeakClassSentinel()) {
        ...
        // ArtMethod is alive at this point, so MethodType it is referencing can't be null.
  ...
  OatQuickMethodHeader* method_header = nullptr;
  {
    ...
    if (UNLIKELY(method->IsNative())) {
      ...
      if (method->IsPreCompiled() && IsSharedRegion(*region)) {
        ...
        // Searching for MethodType-s in roots. They need to be treated as strongly reachable from
        // `method`.
        ObjPtr<mirror::Class> method_type_class =
            WellKnownClasses::java_lang_invoke_MethodType.Get<kWithoutReadBarrier>();
        ...
        for (auto root : roots) {
          ...
          if (klass == method_type_class ||
            ...
            method_types_.insert({method, GcRoot(mt)});
          }
        }
      }
...
void JitCodeCache::MoveObsoleteMethod(ArtMethod* old_method, ArtMethod* new_method) {
  ...
  // method_types_ is a multimap and extract removes only single key-value pair, hence a while-loop.
  auto node = method_types_.extract(old_method);
  while (!node.empty()) {
    node.key() = new_method;
    method_types_.insert(std::move(node));
    node = method_types_.extract(old_method);
  }
...
void JitCodeCache::RemoveUnmarkedCode(Thread* self) {
  ...
  for (auto it = processed_zombie_code_.begin(); it != processed_zombie_code_.end();) {
    ...
    if (GetLiveBitmap()->Test(allocation)) {
      ...
      // To clean method_types_ here we need to know whether an ArtMethod corresponding to
      // `code_ptr` still exists in `method_code_map_`. We can't do linear search here and instead
      // of making `method_types_` more complicated rely on classloader removal and `method_types_`
      // being cleared in `RemoveMethodsIn`.
