...
bool JitCodeCache::ContainsMethod(ArtMethod* method) {
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    if (method_code_map_reverted_.find(method) != method_code_map_reverted_.end()) {
      return true;
    }
...
void JitCodeCache::RemoveMethodsIn(Thread* self, const LinearAlloc& alloc) {
  ...
  std::unordered_set<OatQuickMethodHeader*> method_headers;
  {
    ...
    // lead to a deadlock.
    {
      ...
      for (auto it = method_code_map_reverted_.begin(); it != method_code_map_reverted_.end();) {
        ...
        if (alloc.ContainsUnsafe(method)) {
          ...
          it = method_code_map_reverted_.erase(it);
        } else {
          ++it;
        }
      }
  ...
  OatQuickMethodHeader* method_header = nullptr;
  {
    ...
    if (UNLIKELY(method->IsNative())) {
      ...
      if (method->IsPreCompiled() && IsSharedRegion(*region)) {
        ...
        method_code_map_reverted_.emplace(method, code_ptr);
      }
...
bool JitCodeCache::RemoveMethodLocked(ArtMethod* method, bool release_memory) {
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    auto it = method_code_map_reverted_.find(method);
    while (it != method_code_map_reverted_.end()) {
      in_cache = true;
      if (release_memory) {
        FreeCodeAndData(it->second);
      }
      VLOG(jit) << "JIT removed " << it->first->PrettyMethod() << ": " << it->second;
      DCHECK(method_code_map_.Get(it->second) == it->first);
...
void JitCodeCache::MoveObsoleteMethod(ArtMethod* old_method, ArtMethod* new_method) {
  ...
  auto it = method_code_map_reverted_.find(old_method);
  while (it != method_code_map_reverted_.end()) {
    const void* code_ptr = it->second;
    it = method_code_map_reverted_.erase(it);
    method_code_map_reverted_.emplace(new_method, code_ptr);

    DCHECK(method_code_map_.count(code_ptr) == 1);
