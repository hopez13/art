...
void JitCodeCache::RemoveMethodsIn(Thread* self, const LinearAlloc& alloc) {
  ScopedTrace trace(__PRETTY_FUNCTION__);
  std::vector<const void*> code_pointers;
    ...
    // lead to a deadlock.
    {
      ...
      for (auto it = method_code_map_.begin(); it != method_code_map_.end();) {
        if (alloc.ContainsUnsafe(it->second)) {
          code_pointers.push_back(it->first);
  ...
  {
    // Remove CHA dependency tracking for those in method_headers.
    MutexLock mu(self, *Locks::cha_lock_);
    Runtime::Current()->GetClassHierarchyAnalysis()
        ->RemoveDependentsWithMethodHeaders(method_headers);
  }
  // Do this after removing CHA dependency tracking before the memory can be reused.
  FreeCode(code_pointers);
}

void JitCodeCache::FreeCode(const std::vector<const void*>& code_pointers) {
  MutexLock mu(Thread::Current(), lock_);
  ScopedCodeCacheWrite scc(code_map_.get());
  for (const void* code_ptr : code_pointers) {
    FreeCode(code_ptr);
  }
}
