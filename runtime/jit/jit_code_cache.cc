...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  if (mem_fd.get() < 0) {
    // Unable to use dual view of JIT code cache.
    VLOG(jit) << "Failed to create memory file descriptor: " << strerror(errno);
    LOG(FATAL) << "OTH: REMOVE ME";
  }
  ...
  if (exec_capacity > 0) {
    ...
    if (mem_fd.get() >= 0) {
      // Create second view of code memory for updating JIT code cache.
...
class ScopedCodeCacheWrite : ScopedTrace {
 public:
  ...
  ~ScopedCodeCacheWrite() {
    ...
    if (code_cache_->non_exec_pages_.IsValid()) {
      SetCacheProtection(code_cache_->non_exec_pages_, PROT_READ | PROT_WRITE, "Cache R");
    } else if (code_cache_->exec_pages_.IsValid()) {
