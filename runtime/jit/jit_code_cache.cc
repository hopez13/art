...
uint8_t* JitCodeCache::CommitCodeInternal(Thread* self,
  ...
  WaitForPotentialCollectionToCompleteRunnable(self);
  {
    ...
    // shootdown (incidentally invalidating the CPU pipelines by sending an IPI to all cores).
    // Some architectures, notably ARM and ARM64, have hardware support for broadcasting TLB
    // invalidation and so do not require a software based TLB shootdown. Instead, we call
    // FlushInstructionPipeline() which is a wrapper around a Linux system call added
    // for this purpose (membarrier(MEMBARRIER_CMD_PRIVATE_EXPEDITED)).
