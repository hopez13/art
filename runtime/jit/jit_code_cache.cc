...
JitCodeCache::JitCodeCache()
    : is_weak_access_enabled_(true),
      ...
      needs_instrumentation_support_(false),
...
bool JitCodeCache::Commit(Thread* self,
  ...
  // Check if a non-native was compiled with the expected instrumentation support for
  // non-debuggable. Debuggable mode always have instrumentation enabled so we can omit
  // this check.
  if (!method->IsNative() && !Runtime::Current()->IsJavaDebuggable()) {
    if (needs_instrumentation_support_ != has_instrumentation_support) {
      return false;
    }
  }
...
void JitCodeCache::InvalidateAllCompiledCodeForInstrumentation(bool needs_instrumentation_support) {
  ...
  for (auto it : zygote_map_) {
    ArtMethod* method = it.method;
    // We were compiled, so we must be warm.
    ClearMethodCounter(method, /*was_warm=*/true);
    if (method->IsObsolete()) {
      DCHECK(method->IsNative() ||
             method->GetEntryPointFromQuickCompiledCode() == GetInvokeObsoleteMethodStub());
    } else {
