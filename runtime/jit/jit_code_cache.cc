...
void JitCodeCache::GetProfiledMethods(const std::set<std::string>& dex_base_locations,
                                      std::vector<OnlineProfileMethodInfo>& methods) {
  ScopedTrace trace(__FUNCTION__);
  MutexLock mu(Thread::Current(), lock_);
  for (const ProfilingInfo* info : profiling_infos_) {
    ...
    std::vector<OnlineProfileMethodInfo::OfflineInlineCache> inline_caches;
    for (size_t i = 0; i < info->number_of_inline_caches_; ++i) {
      std::vector<OnlineProfileMethodInfo::ProfileClassReference> profile_classes;
      ...
      for (size_t k = 0; k < InlineCache::kIndividualCacheSize; k++) {
        ...
        if (ContainsElement(dex_base_locations, class_dex_file.GetBaseLocation())) {
          // Only consider classes from the same apk (considering multidex).
          profile_classes.emplace_back(&class_dex_file, type_index);
        }
      }
      if (!profile_classes.empty()) {
        inline_caches.emplace_back(cache.dex_pc_, profile_classes);
      }
    }
    methods.emplace_back(dex_file, method->GetDexMethodIndex(), inline_caches);
  }
}
