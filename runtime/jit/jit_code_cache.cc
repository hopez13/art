...
void JitCodeCache::FreeCodeAndData(const void* code_ptr, bool free_debug_info) {
  ...
  if (OatQuickMethodHeader::FromCodePointer(code_ptr)->IsOptimized()) {
    private_region_.FreeData(private_region_.GetWritableDataAddress(GetRootTable(code_ptr)));
...
void JitCodeCache::ClearData(Thread* self,
                             ...
                             const uint8_t* roots_data) {
  MutexLock mu(self, *Locks::jit_lock_);
  region->FreeData(region->GetWritableDataAddress(roots_data));
}
...
void ZygoteMap::Initialize(uint32_t number_of_methods) {
  ...
  if (data != nullptr) {
    Entry* writable_data = region_->GetWritableDataAddress(data);
    region_->FillData(writable_data, capacity, Entry { nullptr, nullptr });
    map_ = ArrayRef(writable_data, capacity);
  }
}
