...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  if (!used_only_for_profile_data) {
    if (art::membarrier(art::MembarrierCommand::kRegisterPrivateExpeditedSyncCore) != 0) {
      ...
      VLOG(jit) << "Kernel does not support for membarrier sync-core";
    }
  }
...
JitCodeCache::JitCodeCache(MemMap&& data_pages,
  ...
  if (non_exec_pages_.IsValid()) {
    ...
    // writes heap metadata in the region so the region is toggled to writable. On platforms that
    // use a software based TLB-shootdown (via IPI) toggling permission RW->R after updating the
    // code cache will cause the CPU instruction pipelines to be invalidated (via CPU exception
    // entry). On platforms with hardware TLB updates, we hope that membarrier(sync_core) is
    // available on the platform and rely on faith otherwise.
    //
    // There are weaker arguments for this toggling on the basis of catching stray writes into
    // the code cache.
    ...
    // writes heap metadata in the region so the region is toggled to writable. The permissions
    // are toggled during updates for reasons outlined above.
...
uint8_t* JitCodeCache::CommitCodeInternal(Thread* self,
  ...
  WaitForPotentialCollectionToCompleteRunnable(self);
  {
    ...
    // based TLB shootdown. This command will fail on older kernels.
