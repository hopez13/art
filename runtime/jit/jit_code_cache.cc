...
void JitCodeCache::RemoveUnmarkedCode(Thread* self,
                                      std::set<const void*>& zombies,
                                      std::set<ArtMethod*>& zombie_jnis) {
  ...
  ScopedDebugDisallowReadBarriers sddrb(self);
  {
    ...
    MutexLock mu(self, *Locks::jit_lock_);
    for (auto it = zombie_jnis.begin(); it != zombie_jnis.end();) {
      ArtMethod* method = *it;
      auto stub = jni_stubs_map_.find(JniStubKey(method));
      CHECK(stub != jni_stubs_map_.end());
      JniStubData& data = stub->second;
      CHECK(data.IsCompiled());
      CHECK(ContainsElement(data.GetMethods(), method));
      uintptr_t allocation = FromCodeToAllocation(data.GetCode());
      if (method->GetEntryPointFromQuickCompiledCode() ==
        ...
        // list.
        it = zombie_jnis.erase(it);
      } else if (!GetLiveBitmap()->Test(allocation)) {
        ...
        } else if (ContainsElement(data.GetMethods(), method)) {
          if (data.GetMethods().size() == 1) {
            OatQuickMethodHeader* header = OatQuickMethodHeader::FromCodePointer(data.GetCode());
            method_headers.insert(header);
            CHECK(ContainsPc(header));
            VLOG(jit) << "JIT removed native code of" << method->PrettyMethod();
            ...
            stub->second.RemoveMethod(method);
...

  {
    MutexLock mu(self, *Locks::jit_lock_);
    for (const OatQuickMethodHeader* method_header : method_headers) {
      method_code_map_.erase(method_header->GetCode());
    }
  }
  FreeAllMethodHeaders(method_headers);
}
...
void JitCodeCache::DoCollection(Thread* self) {
  ...
  // Temporary sets for the collection. This makes sure that at the point of
  // walking thread stacks, we don't dynamically add new zombie code which could
  // be missed by the collection logic.
  std::set<const void*> zombies;
  std::set<ArtMethod*> zombie_jnis;
  {
    ScopedDebugDisallowReadBarriers sddrb(self);
    ...
    zombies = std::move(zombie_code_);
    zombie_jnis = std::move(zombie_jni_code_);
  ...
  TimingLogger logger("JIT code cache timing logger", true, VLOG_IS_ON(jit));
  {
...

    {
      ...
      RemoveUnmarkedCode(self, zombies, zombie_jnis);
    }

    MutexLock mu(self, *Locks::jit_lock_);
    // Put back zombie code that we weren't able to remove into the shared sets.
    zombie_code_.insert(zombies.begin(), zombies.end());
    zombie_jni_code_.insert(zombie_jnis.begin(), zombie_jnis.end());
