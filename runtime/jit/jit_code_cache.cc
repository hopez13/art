...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  //          +----------------+ --------------------
  //
  // The code_sync_map_ contains a page that is used to in the
  // synchronization of CPU state following the addition of new JIT
  // code. It's a single page.
  //
...
static void FlushInstructionPiplines(uint8_t* sync_page) {
  ...
  // permissions on a writable page. This should send an IPI to each
  // core to update the TLB entry, and the IPI will discard the
  // current instruction fetch queue.
