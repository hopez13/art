...
void JitCodeCache::FreeCode(const void* code_ptr,
                            ArenaSet<OatQuickMethodHeader*>* method_headers_of_freed_code,
                            ArenaVector<uintptr_t>* allocations_to_be_freed) {
  ...
  method_headers_of_freed_code->insert(method_header);
  allocations_to_be_freed->push_back(allocation);
}
...
void JitCodeCache::RemoveMethodsIn(Thread* self, const LinearAlloc& alloc) {
  ...
  ArenaSet<OatQuickMethodHeader*> method_headers(allocator.Adapter(kArenaAllocCHA));
  ArenaVector<uintptr_t> allocations(allocator.Adapter(kArenaAllocCHA));
  {
