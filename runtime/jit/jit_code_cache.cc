...
JitCodeCache::JitCodeCache(MemMap* code_map,
    ...
    : lock_("Jit code cache", kJitCodeCacheLock),
      ...
      inline_cache_cond_("Jit inline cache variable", lock_) {
  ...
  for (ProfilingInfo* info : profiling_infos_) {
    for (size_t i = 0; i < info->number_of_inline_caches_; ++i) {
      ...
      for (size_t j = 0; j < InlineCache::kIndividualCacheSize; ++j) {
        // This does not need a read barrier because this is called by GC.
        mirror::Class* object = cache->classes_[j].Read<kWithoutReadBarrier>();
        if (object != nullptr) {
          // This does not need a read barrier because this is called by GC.
          mirror::Object* class_loader =
              object->GetClassLoader<kDefaultVerifyFlags, kWithoutReadBarrier>();
          if (visitor->IsMarked(class_loader) != nullptr) {
            mirror::Class* new_object = reinterpret_cast<mirror::Class*>(visitor->IsMarked(object));
            // Note that new_object can be null for CMS and newly allocated objects.
            if (new_object != nullptr && new_object != object) {
              cache->classes_[j] = GcRoot<mirror::Class>(new_object);
            }
          } else {
