...
void JitCodeCache::MoveObsoleteMethod(ArtMethod* old_method, ArtMethod* new_method) {
  ...
  for (auto it = range.first; it != range.second; ++it) {
    ...
    remapped_code_ptrs.insert({new_method, code_ptr});

    DCHECK_EQ(method_code_map_.count(code_ptr), 1u);
    method_code_map_.find(code_ptr)->second = new_method;
  }

  DCHECK_EQ(remapped_code_ptrs.size(), method_code_map_reversed_.count(old_method));
  method_code_map_reversed_.erase(old_method);
