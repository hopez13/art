...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  char shm_name[32];
  snprintf(shm_name, sizeof(shm_name), "/jit-cache-%d", reinterpret_cast<int32_t>(getpid()));
  mem_fd = unique_fd(shm_open(shm_name, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR));
  if (mem_fd.get() < 0) {
    VLOG(jit) << "Failed to create memory file descriptor: " << strerror(errno);
  }
  CheckedCall(shm_unlink, "unlink jit cache", shm_name);
#endif
