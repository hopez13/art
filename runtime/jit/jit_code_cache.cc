    ...
    for (uint32_t i = 0; i < number_of_roots; ++i) {
      ...
      if (object == nullptr || object == Runtime::GetWeakClassSentinel()) {
        ...
        if (new_method_type != nullptr) {
          ...
          DCHECK(
              (new_method_type->InstanceOf<kVerifyNone, kWithoutReadBarrier>(method_type_class)));
...
bool JitCodeCache::RemoveMethodLocked(ArtMethod* method, bool release_memory) {
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    for (auto it = method_code_map_reversed_.find(method); it != method_code_map_reversed_.end();) {
      in_cache = true;
      if (release_memory) {
...
void JitCodeCache::MoveObsoleteMethod(ArtMethod* old_method, ArtMethod* new_method) {
  ...
  auto it = method_code_map_reversed_.find(old_method);
  while (it != method_code_map_reversed_.end()) {
    const void* code_ptr = it->second;
    it = method_code_map_reversed_.erase(it);
    method_code_map_reversed_.emplace(new_method, code_ptr);
