...
bool JitCodeCache::NotifyCompilationOf(ArtMethod* method,
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    MutexLock mu(self, *Locks::jit_lock_);
    if (IsMethodBeingCompiled(method, baseline, osr)) {
      return false;
    }
    AddMethodBeingCompiled(method, baseline, osr);
    return true;
  }
}
...
void JitCodeCache::DoneCompiling(ArtMethod* method, Thread* self, bool osr, bool baseline) {
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    DCHECK(IsMethodBeingCompiled(method, baseline, osr));
    RemoveMethodBeingCompiled(method, baseline, osr);
  }
}
