...
void JitCodeCache::FreeCodeAndData(const void* code_ptr, bool free_debug_info) {
  ...
  if (free_debug_info) {
    ...
    ArrayRef<const void*> removed_symbols(&code_ptr, 1);
    JitCompilerInterface* jit_compiler = Runtime::Current()->GetJit()->GetJitCompiler();
    jit_compiler->RemoveNativeDebugInfoForJit(removed_symbols);
  }
...
void JitCodeCache::FreeAllMethodHeaders(
  ...
  JitCompilerInterface* jit_compiler = Runtime::Current()->GetJit()->GetJitCompiler();
  jit_compiler->RemoveNativeDebugInfoForJit(ArrayRef<const void*>(removed_symbols));
