...
// Translate an address belonging to one memory map into an address in a second. This is useful when
// there are two memory mappings for the same memory.
template <typename T>
...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  MemMap data_pages;
  if (mem_fd.get() >= 0) {
    // Dual view of JIT code cache case. Create an Initial mapping of data pages is large enough
    ...
    // Single view of JIT code cache case. Create an Initial mapping of data pages is large enough
  ...
  if (exec_capacity > 0) {
    ...
    // (especially for SystemServer). It doesn't otherwise need to be executable  as there is no
    // code in the cache yet.
