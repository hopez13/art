...
void JitCodeCache::DoCollection(Thread* self, bool collect_profiling_info) {
  ...
  if (collect_profiling_info) {
    ...
    auto profiling_kept_end = std::remove_if(profiling_infos_.begin(), profiling_infos_.end(),
      [this] (const ProfilingInfo* profiling_info) NO_THREAD_SAFETY_ANALYSIS {
        ProfilingInfo* info = private_region_.GetWritableDataAddress(profiling_info);
        const void* ptr = info->GetMethod()->GetEntryPointFromQuickCompiledCode();
        ...
        if (ContainsPc(ptr) &&
          ...
          private_region_.FreeData(reinterpret_cast<const uint8_t*>(profiling_info));
          return true;
        }
        return false;
      });
...
ProfilingInfo* JitCodeCache::AddProfilingInfoInternal(Thread* self ATTRIBUTE_UNUSED,
  ...
  profiling_infos_.push_back(reinterpret_cast<const ProfilingInfo*>(data));
