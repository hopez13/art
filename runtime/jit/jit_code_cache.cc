 ...
 public:
  explicit JniStubKey(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_)
      : shorty_(method->GetShorty()),
        ...
        is_fast_native_(method->IsAnnotatedWithFastNative()),
        is_critical_native_(method->IsAnnotatedWithCriticalNative()),
    ...
    // lead to a deadlock.
    {
      ...
      for (auto it = jni_stubs_map_.begin(); it != jni_stubs_map_.end();) {
        ...
        if (it->second.GetMethods().empty()) {
          ...
          it->first.UpdateShorty(it->second.GetMethods()[0]);
          ++it;
        }
      }
...
bool JitCodeCache::NotifyCompilationOf(ArtMethod* method, Thread* self, bool osr) {
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    if (data->IsCompiled()) {
      ...
      Runtime::Current()->GetInstrumentation()->UpdateMethodsCode(method, entrypoint);
      // TODO: Should we also update entrypoints of other methods held by the JniStubData?
    }
...
void JitCodeCache::DoneCompiling(ArtMethod* method, Thread* self, bool osr) {
  ...
  if (UNLIKELY(method->IsNative())) {
    ...
    if (UNLIKELY(!data->IsCompiled())) {
      ...
      jni_stubs_map_.erase(it);  // Remove the entry.
