...
bool JitCodeCache::InitMappings(bool rwx_memory_allowed,
                                bool is_zygote,
...
JitCodeCache* JitCodeCache::Create(bool used_only_for_profile_data,
  ...
  jit_code_cache->InitState(initial_capacity, max_capacity);
  ...
  if (!jit_code_cache->InitMappings(rwx_memory_allowed, is_zygote, error_msg)) {
    return nullptr;
  }

  jit_code_cache->InitSpaces();
...
void JitCodeCache::InitState(size_t initial_capacity, size_t max_capacity) {
...
void JitCodeCache::PostForkChildAction(bool is_system_server) {
  MutexLock mu(Thread::Current(), lock_);
  ...
  InitState(initial_capacity, max_capacity);

  std::string error_msg;
  if (!InitMappings(/* rwx_memory_allowed= */ !is_system_server,
                    /* is_zygote= */ false,
                    &error_msg)) {
