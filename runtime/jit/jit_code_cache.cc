...
JitCodeCache::JitCodeCache(MemMap&& data_pages,
  ...
  if (code_heap != nullptr) {
    // Make all pages reserved for the code heap writable.
    ...
    // Protect pages containing heap metadata.
    CheckedCall(mprotect, "protect code heap", code_heap->Begin(), exec_end_, kProtR);
  } else {
    SetFootprintLimit(current_capacity_);
