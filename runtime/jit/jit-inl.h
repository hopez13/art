...
inline void Jit::AddSamples(Thread* self,
  ...
  static constexpr uint32_t kBatchSize = 32;
  uint32_t old_batch = RoundDown(old_count, kBatchSize);  // Clear lower bits.
  uint32_t new_batch = RoundDown(new_count, kBatchSize);  // Clear lower bits.
  if (UNLIKELY(old_batch == 0)) {
    // For low sample counts, we check every time (which is important for tests).
    MaybeCompileMethod(self, method, old_count, new_count, with_backedges);
  } else if (UNLIKELY(old_batch != new_batch)) {
    // For high sample counts, we check only when we move past the batch boundary.
    MaybeCompileMethod(self, method, old_batch, new_batch, with_backedges);
  }
}
