...
class ProfileSaver {
 ...
 private:
  ...
  // Resolved the locations stored in tracked_dex_base_locations_to_be_resolved_ and puts the
  // result in tracked_dex_base_locations_.
  void ResolvedTrackedLocations() REQUIRES(!Locks::profiler_lock_);
  ...
  // Collection of code paths that the profiler tracks but have not been resolved to their
  // realpath. The resolution is done async to minimize the time it takes for someone
  // to register a path.
