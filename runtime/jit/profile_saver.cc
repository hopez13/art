...
class ProfileSaver::GetClassesAndMethodsHelper {
 public:
  GetClassesAndMethodsHelper(bool startup,
      ...
      : startup_(startup),
        ...
        base_flags_((startup ? Hotness::kFlagStartup : Hotness::kFlagPostStartup) |
                    GetExtraMethodHotnessFlags(options)),
  ...
  void CollectClasses(pthread_t profiler_pthread, Thread* self);
  ...
  // For vising collected class loaders.
  void VisitRootIfNonNull(StackReference<mirror::Object>* ref)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    if (ref->IsNull()) {
      return;
    }
    CollectInternal(ref->AsMirrorPtr()->AsClassLoader());
  }
 ...
 private:
  ...
  // Collect classes from one class loader.
  void CollectInternal(ObjPtr<mirror::ClassLoader> class_loader)
      REQUIRES_SHARED(Locks::mutator_lock_);

  const bool startup_;
  const bool profile_boot_class_path_;
  const uint32_t hot_method_sample_threshold_;
  const uint32_t base_flags_;
...
void ProfileSaver::GetClassesAndMethodsHelper::CollectInternal(
  ...
  ClassTable* const table = class_linker->ClassTableForClassLoader(class_loader);
  if (table == nullptr) {
    return;
  }
  ...
  table->Visit([&](ObjPtr<mirror::Class> klass) REQUIRES_SHARED(Locks::mutator_lock_) {
    if (klass->GetClassLoader() != class_loader) {
      // To avoid processing a class more then once, we process each class only
...
void ProfileSaver::GetClassesAndMethodsHelper::CollectClasses(pthread_t profiler_pthread,
                                                              Thread* self) {
  ScopedTrace trace(__PRETTY_FUNCTION__);
  ...
  // so that method array pointers in collected `ClassRecords` remain valid.
  ...
  if (profile_boot_class_path_) {
    ...
    CollectInternal(/*class_loader=*/ nullptr);
  }
  class_loaders_->VisitRoots(*this);
  ...
  for (const auto& entry : dex_file_records_map_) {
    ...
    for (const ClassRecord& class_record : dex_file_records->class_records) {
      ...
      for (size_t index = class_record.copied_methods_start; index != methods_size; ++index) {
        // Note: Implicitly using `ArtMethod` array with `kRuntimePointerSize`.
        ...
        if (method.IsInvokable()) {
          ...
          if (method_dex_file == dex_file) {
            dex_file_records->copied_methods.push_back(&method);
          } else {
            ...
            if (it == dex_file_records_map_.end()) {
              ...
              // interface's defining class loader before it has any resolved classes, then the
              // interface is resolved and an implementing class is defined in a child class
              // loader before we visit that child class loader's classes.
              continue;
            }
            DexFileRecords* method_dex_file_records = it->second;
...
void ProfileSaver::GetClassesAndMethodsHelper::UpdateProfile(const std::set<std::string>& locations,
  ...
  const uint32_t base_flags = base_flags_;
  ...
  for (const auto& entry : dex_file_records_map_) {
    ...
    if (profile_index == ProfileCompilationInfo::MaxProfileIndex()) {
      // Either not a profiled location, or error adding dex file to the `profile_info`.
      continue;
    }
...
void ProfileSaver::FetchAndCacheResolvedClassesAndMethods(bool startup) {
  ...
  helper.CollectClasses(profiler_pthread, self);
  ...
  for (const auto& it : tracked_dex_base_locations_) {
    ...
    // Log the stats individually for each saved profile.
