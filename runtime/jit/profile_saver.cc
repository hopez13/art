...
void ProfileSaver::AddTrackedLocations(const std::string& output_filename,
  ...
  // the profiler will skip the dex due to location miss match.
  // To avoid this, we add the code paths to the temporary cache of 'to_be_reolved'
  // locations. When the profiler thread executes we will resolve the paths their
  // real path.
