...
using TypeStatusReferenceCollection = DexReferenceCollection<std::pair<dex::TypeIndex, mirror::Class::Status>,
  ...
  for (Handle<mirror::ClassLoader> class_loader : class_loaders) {
    ...
    for (ObjPtr<mirror::Class> klass : classes) {
      if (startup) {
        ...
        resolved_classes->AddReference(&klass->GetDexFile(), std::pair<dex::TypeIndex, mirror::Class::Status>(klass->GetDexTypeIndex(), klass->GetStatus()));
      }
