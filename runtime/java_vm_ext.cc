 ...
 public:
  ...
  void* FindNativeMethod(ArtMethod* m, std::string& detail)
    ...
    for (const auto& lib : libraries_) {
      ...
      if (fn != nullptr) {
        VLOG(jni) << "[Found native code for " << ArtMethod::PrettyMethod(m)
                  << " in \"" << library->GetPath() << "\"]";
        return fn;
      }
    }
    detail += "No implementation found for ";
    detail += ArtMethod::PrettyMethod(m);
...
void JavaVMExt::JniAbort(const char* jni_function_name, const char* msg) {
  ...
  if (current_method != nullptr) {
    os << "\n    from " << ArtMethod::PrettyMethod(current_method);
  }
  os << "\n";
  self->Dump(os);
...
void* JavaVMExt::FindCodeForNativeMethod(ArtMethod* m) {
  ...
  CHECK(c->IsInitializing()) << c->GetStatus() << " " << ArtMethod::PrettyMethod(m);
  std::string detail;
  void* native_method;
  Thread* self = Thread::Current();
  {
