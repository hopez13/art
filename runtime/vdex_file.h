...
class VdexFile {
 public:
  ...
  const DexSectionHeader& GetDexSectionHeader() const {
    ...
    return *reinterpret_cast<const DexSectionHeader*>(
        Begin() + sizeof(VerifierDepsHeader) + GetVerifierDepsHeader().GetSizeOfChecksumsSection());
  }
 ...
 private:
  ...
  const uint8_t* DexBegin() const {
    ...
    return Begin() +
        ...
        GetVerifierDepsHeader().GetSizeOfChecksumsSection() +
