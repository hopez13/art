...
//
//   Optionally:
//      VdexFile::OtherDataHeader       fixed-length header
//      Dex file checksums
//
...
class VdexFile {
 public:
  struct VerifierDepsHeader {
   public:
    VerifierDepsHeader(uint32_t verifier_deps_size, bool has_dex_section);
   ...
   private:
    ...
    static constexpr uint8_t kVerifierDepsVersion[] = { '0', '1', '8', '\0' };
    static constexpr uint8_t kDexSectionVersion[] = { '0', '0', '1', '\0' };
    static constexpr uint8_t kNoDexSectionVersion[] = { '0', '0', '0', '\0' };
  ...
  struct DexSectionHeader {
   public:
    DexSectionHeader(uint32_t number_of_dex_files_,
                     uint32_t dex_size,
  ...
  const DexSectionHeader& GetDexSectionHeader() const {
    ...
    return *reinterpret_cast<const DexSectionHeader*>(Begin() + sizeof(VerifierDepsHeader));
  }
  ...
  const uint8_t* GetVerifierDepsStart() const {
    if (GetVerifierDepsHeader().HasDexSection()) {
      ...
      return Begin() + sizeof(VerifierDepsHeader) + GetDexSectionHeader().GetDexSectionSize();
    } else {
      // When there is no dex section, the verifier deps are just after the header.
      return Begin() + sizeof(VerifierDepsHeader);
    }
  }
  ...
  uint32_t GetLocationChecksum(uint32_t dex_file_index) const {
    DCHECK(GetVerifierDepsHeader().HasDexSection());
    DCHECK_LT(dex_file_index, GetDexSectionHeader().GetNumberOfDexFiles());
    return reinterpret_cast<const uint32_t*>(
        Begin() + sizeof(VerifierDepsHeader) + sizeof(DexSectionHeader))[dex_file_index];
  }
 ...
 private:
  ...
  const uint8_t* DexBegin() const {
    ...
    return Begin() +
        ...
        sizeof(DexSectionHeader) +
        GetDexSectionHeader().GetSizeOfChecksumsSection();
  }
