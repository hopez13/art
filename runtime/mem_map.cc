...
bool MemMap::ReplaceMapping(MemMap** dest_ptr, MemMap** source_ptr, /*out*/std::string* error) {
  ...
  *source_ptr = nullptr;
  ...
  // Delete the old dest, don't unmap it though (set reuse).
  std::string dest_name = dest->GetName();
  uint8_t* dest_begin = dest->Begin();
  void* dest_base_begin = dest->BaseBegin();
  int dest_prot = dest->GetProtect();

  dest->already_unmapped_ = true;
  delete dest;
  dest = nullptr;

  // Create the new memmap for return.
  *dest_ptr = new MemMap(dest_name,
                         dest_begin,
                         source_size,
                         dest_base_begin,
                         new_base_size,
                         dest_prot,
                         /*reuse*/false);  // we checked this above

  // Unmap extra space
  (*dest_ptr)->SetSize(source_size);
...
void MemMap::SetSize(size_t new_size) {
  ...
  size_t new_base_size = RoundUp(new_size, kPageSize);
