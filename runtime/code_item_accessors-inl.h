...
inline void CodeItemInstructionsAccessor::Init(const CompactDexFile::CodeItem& code_item) {
  insns_size_in_code_units_ = code_item.insns_size_in_code_units_;
  insns_ = code_item.insns_;
}

inline void CodeItemInstructionsAccessor::Init(const StandardDexFile::CodeItem& code_item) {
  insns_size_in_code_units_ = code_item.insns_size_in_code_units_;
  insns_ = code_item.insns_;
}

inline void CodeItemInstructionsAccessor::Init(const DexFile* dex_file,
...
inline CodeItemInstructionsAccessor::CodeItemInstructionsAccessor(
    const DexFile* dex_file,
    const DexFile::CodeItem* code_item) {
  Init(dex_file, code_item);
}

inline CodeItemInstructionsAccessor::CodeItemInstructionsAccessor(ArtMethod* method)
    : CodeItemInstructionsAccessor(method->GetDexFile(), method->GetCodeItem()) {}

inline IterationRange<DexInstructionIterator> CodeItemInstructionsAccessor::Instructions() const {
  return { DexInstructionIterator(insns_, 0u),
           DexInstructionIterator(insns_, insns_size_in_code_units_) };
}

inline CodeItemInstructionsAccessor CodeItemInstructionsAccessor::CreateNullable(
    ArtMethod* method) {
  DCHECK(method != nullptr);
  CodeItemInstructionsAccessor ret;
...
inline void CodeItemDataAccessor::Init(const CompactDexFile::CodeItem& code_item) {
  CodeItemInstructionsAccessor::Init(code_item);
