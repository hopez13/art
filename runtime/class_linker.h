...
class ClassLinker {
 public:
  ...
  // and ClassLoader, storing the result in the DexCache. The referrer is used to identify the
  ...
  // `referrer`, storing the result in the DexCache. The ClassLoader is used to search for
  // the type, since it may be referenced from but not contained within the given DexFile.
  ...
  // be referenced from but not contained within the given DexFile.
  ObjPtr<mirror::Class> LookupResolvedType(dex::TypeIndex type_idx,
                                           ObjPtr<mirror::DexCache> dex_cache,
                                           ObjPtr<mirror::ClassLoader> class_loader)
      REQUIRES_SHARED(Locks::mutator_lock_);
