...
void OatFile::ResolveRelativeEncodedDexLocation(const char* abs_dex_location,
  ...
  if (abs_dex_location == nullptr) {
    // `abs_dex_location` not provided. We cannot resolve `rel_dex_location`.
  } else if (!IsAbsoluteLocation(rel_dex_location) &&
             IsLocationSuffix(abs_dex_location, rel_dex_location)) {
    // The base location (no multidex suffix) of the relative `rel_dex_location` is a suffix
    ...
    //   On target always use it for both dex file name and dex location. On host assume
    //   we're cross-compiling and use `abs_dex_location` as a file name (for loading files)
    //   and `rel_dex_location` as the dex location. If we're not cross-compiling, the two
    //   paths should be equal.
    // Case 2: `rel_dex_location` is relative and not suffix of `abs_location`
    //   This should only happen for tests. On target always use `rel_dex_location`. On host
    //   use `abs_dex_location` with the appropriate multidex suffix because `rel_dex_location`
    //   might be the target path.
    resolve_location = false;
    resolve_filename = !kIsTargetBuild;
  }

  // Construct dex file location and dex file name if pointers provided by the caller.
