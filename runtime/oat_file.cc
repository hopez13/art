...
class OatFileBase : public OatFile {
 ...
 protected:
  ...
  // Setup function returns true on success, false on failure.
...
class OatFileBackedByVdex final : public OatFileBase {
 public:
  ...
  static OatFileBackedByVdex* Open(int zip_fd,
    ...
    if (vdex_file->HasDexSection()) {
      ...
      for (const uint8_t* dex_file_start = vdex_file->GetNextDexFileData(nullptr);
        ...
        OatDexFile* oat_dex_file =
            new OatDexFile(oat_file.get(), dex_file_start, vdex_file->GetLocationChecksum(i), location, canonical_location);
  ...
  void SetupHeader(size_t number_of_dex_files) {
    ...
    store.Put(OatHeader::kConcurrentCopying, kUseReadBarrier ? OatHeader::kTrueValue : OatHeader::kFalseValue);
...
OatFile* OatFile::Open(int zip_fd,
  ...
  // Check that the vdex file even exists, fast-fail.
