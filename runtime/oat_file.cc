...
bool OatFileBase::Setup(const char* abs_dex_location, std::string* error_msg) {
  ...
  for (size_t i = 0; i < dex_file_count; i++) {
    ...
    const uint8_t* dex_file_pointer = vdex_->Begin() + dex_file_offset;
    ...
    if (vdex_->Size() - dex_file_offset < header->file_size_) {
      *error_msg = StringPrintf("In oat file '%s' found OatDexFile #%zu for '%s' with dex file "
...
static std::string VdexFilename(const std::string& filename) {
  ...
  return filename.substr(0, last_ext) + ".vdex";
}
...
OatFile* OatFile::Open(const std::string& oat_filename,
  ...
  std::string vdex_filename = VdexFilename(oat_filename);

  // Check that the files even exists, fast-fail.
