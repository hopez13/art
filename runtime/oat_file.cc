...
class OatFileBase : public OatFile {
 public:
  ...
  template <typename kOatFileBaseSubType>
  static OatFileBase* OpenOatFile(const int vdex_fd,
                                  const int oat_fd,
 ...
 protected:
  ...
  virtual bool Load(const int oat_fd,
                    uint8_t* oat_file_begin,
                    bool writable,
                    bool executable,
                    bool low_4gb,
...
template <typename kOatFileBaseSubType>
OatFileBase* OatFileBase::OpenOatFile(const int vdex_fd,
                                      const int oat_fd,
...
bool OatFileBase::LoadVdex(const int vdex_fd,
                           const std::string& vdex_filename,
                           bool writable,
                           bool low_4gb,
                           std::string* error_msg) {
...
class DlOpenOatFile FINAL : public OatFileBase {
 ...
 protected:
  ...
  bool Load(const int, uint8_t*, bool, bool, bool, std::string*) {
    return false;
  }
...
bool ElfOatFile::Load(const int oat_fd,
                      uint8_t* oat_file_begin,  // Override where the file is loaded to if not null
                      bool writable,
                      bool executable,
                      bool low_4gb,
...
OatFile* OatFile::Open(const int vdex_fd,
                       const int oat_fd,
                       const std::string& oat_location,
                       uint8_t* requested_base,
                       uint8_t* oat_file_begin,
  ...
  // Check that the files even exist, fast-fail.
  if (kIsVdexEnabled && vdex_fd < 0) {
    *error_msg = StringPrintf("Invalid vdex_fd - %d.", vdex_fd);
    return nullptr;
  } else if (!OS::FileExists(oat_location.c_str())) {
    *error_msg = StringPrintf("Invalid oat_fd - %d.", oat_fd);
    return nullptr;
  }
