...
static inline bool MapConstantTables(const gc::space::ImageSpace* space,
  ...
  File file(space->GetImageFilename(), O_RDONLY, /* */ false);
  if (!file.IsOpened()) {
    LOG(ERROR) << "Failed to open boot image file " << space->GetImageFilename();
    return false;
  }

  size_t offset = space->GetImageHeader().GetInternedStringsSection().Offset();
  size_t size = space->GetImageHeader().GetConstantTablesMappedSize();
  std::string error_msg;
...
bool OatFileBase::Setup(const char* abs_dex_location, std::string* error_msg) {
  ...
  if (boot_image_tables != nullptr) {
    ...
    for (gc::space::ImageSpace* space : Runtime::Current()->GetHeap()->GetBootImageSpaces()) {
      tables_size += space->GetImageHeader().GetConstantTablesMappedSize();
    ...
    for (gc::space::ImageSpace* space : Runtime::Current()->GetHeap()->GetBootImageSpaces()) {
      if (!MapConstantTables(space, boot_image_tables)) {
        return false;
      }
      boot_image_tables += space->GetImageHeader().GetConstantTablesMappedSize();
    }
