...
static void CreateDexLocation(const char* abs_dex_location,
                              const std::string& rel_dex_location,
                              bool use_relative,
                              /* out */ std::string* out_location) {
  DCHECK(use_relative || abs_dex_location != nullptr);
  if (out_location != nullptr) {
    *out_location = use_relative
        ? rel_dex_location
...
void OatFile::ResolveRelativeEncodedDexLocation(const char* abs_dex_location,
  ...
  bool location_relative = false;
  bool filename_relative = false;
  ...
  if (abs_dex_location == nullptr) {
    // `abs_dex_location` not provided. All we can do is use `rel_dex_location`.
    location_relative = true;
    filename_relative = true;
  } else if (IsAbsoluteLocation(rel_dex_location)) {
    if (kIsTargetBuild) {
      ...
      // be equal.
      location_relative = true;
      filename_relative = false;
    }
  } else if (IsLocationSuffix(abs_dex_location, rel_dex_location)) {
    // The relative `rel_dex_location` is a suffix of `abs_dex_location`.
  ...
  // Construct the dex file location and dex file name if requested by the caller.
  CreateDexLocation(abs_dex_location, rel_dex_location, location_relative, dex_file_location);
  CreateDexLocation(abs_dex_location, rel_dex_location, filename_relative, dex_file_name);
}
