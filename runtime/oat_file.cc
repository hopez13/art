...
bool OatFileBase::Setup(const char* abs_dex_location, std::string* error_msg) {
  ...
  for (size_t i = 0; i < dex_file_count; i++) {
    ...
    if (method_bss_mapping != nullptr) {
      DCHECK(std::is_sorted(method_bss_mapping->begin(),
                            method_bss_mapping->end(),
                            MethodBssMappingEntryComparator()));
      DCHECK(std::all_of(
          method_bss_mapping->begin(),
          method_bss_mapping->end(),
          [this](const MethodBssMappingEntry& entry) { return entry.bss_offset < BssSize(); }));
      DCHECK(std::all_of(
          method_bss_mapping->begin(),
          method_bss_mapping->end(),
          [pointer_size](const MethodBssMappingEntry& entry) {
            return IsAlignedParam(entry.bss_offset, static_cast<size_t>(pointer_size));
          }));
    }
