...
class DlOpenOatFile final : public OatFileBase {
 ...
 protected:
  ...
  bool Load(int, bool, bool, bool, MemMap*, std::string*) override {
    return false;
  }
 ...
 private:
  ...
  // Note: Must be destroyed after dlclose() if it holds the owning reservation.
  ...
  *error_msg = "Dlopen unsupported on Mac.";
...
#else
  {
    ...
    if (reservation != nullptr && dlopen_handle_ != nullptr) {
      ...
      dl_iterate_context context = { shared_objects_before_, reservation };
