...
bool OatFileBase::Setup(int zip_fd,
  ...
  if (bcp_info_offset != 0) {
    ...
    if (linker != nullptr) {
      ...
      if (UNLIKELY(number_of_bcp_dexfiles > linker->GetBootClassPath().size())) {
        // If we compiled with more DexFiles that what we have at runtime, we expect to discard this
        // OatFile after verifying its checksum in OatFileAssistant. Then, we set
