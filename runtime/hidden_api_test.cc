...
static bool Copy(const char* src_filename, File* dst, /*out*/ std::string* error_msg) {
  ...
  int flush_result = dst->Flush();
  if (flush_result != 0) {
    *error_msg = StringPrintf("Failed to flush: %s", strerror(flush_result));
    return false;
  }
  return true;
}
...
class HiddenApiTest : public CommonRuntimeTest {
 protected:
  ...
  void TestLocation(const std::string& location, hiddenapi::Domain expected_domain) {
    ...
    memcpy(unique_location.get(), location.data(), location.length() - suffix.length());
    memcpy(unique_location.get() + location.length() - suffix.length(),
           placeholder.data(),
           placeholder.length());
    memcpy(unique_location.get() + location.length() - suffix.length() + placeholder.length(),
           location.data() + location.length() - suffix.length(),
           suffix.length());
    ...
    std::string error_msg;
    {
      ...
      if (!copied && flush_close_result == 0) {
        // Silently remove the file before reporting the `Copy()` failure.
        std::string ignored_error_msg;
        Remove(unique_location.get(), &ignored_error_msg);
      }
      ASSERT_TRUE(copied) << error_msg;
      ASSERT_EQ(flush_close_result, 0) << strerror(flush_close_result);
    }
