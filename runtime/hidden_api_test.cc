...
TEST_F(HiddenApiTest, CheckNothingMatchesEmptyVector) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_i_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class12_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class12_method1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method12_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class2_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class2_method1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class2_method1_i_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class3_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class3_method1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class3_method1_i_, prefixes));
}

TEST_F(HiddenApiTest, CheckEverythingMatchesL) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"L"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_i_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class12_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class12_method1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method12_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class2_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class2_method1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class2_method1_i_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class3_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class3_method1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class3_method1_i_, prefixes));
}

TEST_F(HiddenApiTest, CheckPackageMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
...
TEST_F(HiddenApiTest, CheckClassMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
...
TEST_F(HiddenApiTest, CheckMethodMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->method1"});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
...
TEST_F(HiddenApiTest, CheckMethodSignatureMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->method1(I)"});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_i_, prefixes));
...
TEST_F(HiddenApiTest, CheckMethodSignatureAndReturnMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->method1()V"});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_i_, prefixes));
...
TEST_F(HiddenApiTest, CheckFieldMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->field1"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_i_, prefixes));
...
TEST_F(HiddenApiTest, CheckFieldTypeMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->field1:I"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field12_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
}
...
TEST_F(HiddenApiTest, CheckConstructorMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;-><init>"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
}
...
TEST_F(HiddenApiTest, CheckConstructorExactMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;-><init>()V"});
  ASSERT_TRUE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
}
...
TEST_F(HiddenApiTest, CheckMethodSignatureTrailingCharsNoMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->method1()Vfoo"});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_method1_, prefixes));
}

TEST_F(HiddenApiTest, CheckConstructorTrailingCharsNoMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;-><init>()Vfoo"});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_init_, prefixes));
}

TEST_F(HiddenApiTest, CheckFieldTrailingCharsNoMatch) {
  ScopedObjectAccess soa(self_);
  std::vector<std::string> prefixes({"Lmypackage/packagea/Class1;->field1:Ifoo"});
  ASSERT_FALSE(hiddenapi::IsMemberExempted(class1_field1_, prefixes));
}
