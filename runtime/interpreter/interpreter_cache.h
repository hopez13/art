...
// All operations except Clear must be done from the owning thread.
class InterpreterCache {
 ...
 public:
  ...
  // Clear the cache (redirects the data to the shared read-only table).
  // This is the only method that is safe to call from different thread.
 ...
 private:
  ...
  // The data_ pointer can point either to local or empty shared table.
  // This allows us to invalidate all thread caches quickly and safely.
  // In particular, we can clear the cache even when it is being used.
  std::atomic<Entry*> data_;
  std::unique_ptr<Entry[]> local_data_;    // Writable thread-local table.
  static Entry shared_empty_data_[kSize];  // Shared read-only empty table.
};
