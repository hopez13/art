...
class ALIGNED(16) InterpreterCache {
 ...
 public:
  ...
  InterpreterCache(Thread* owning_tread) : owning_tread_(owning_tread) {
    std::fill_n(data_.data(), data_.size(), Entry{});
  }

  ALWAYS_INLINE void Clear() {
    DCHECK(IsCalledFromOwningThreadOrSuspended());
    data_.fill(Entry{});
  }
 ...
 private:
  bool IsCalledFromOwningThreadOrSuspended();
