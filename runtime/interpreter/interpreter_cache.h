...
class ALIGNED(16) InterpreterCache {
 public:
  ...
  static constexpr size_t kSharedSize = 4 * 1024;  // Value of 16k has around 90% cache hit rate.
  ...
  static void ForEachSharedEntry(Callback&& callback) {
    for (Locked& atomic_entry : shared_array_) {
      Entry old_entry = LoadAcquire(&atomic_entry);
      ...
      if (old_entry.second != new_entry.second) {
        StoreRelease(&atomic_entry, new_entry);
      }
    }
  }

  static constexpr uint64_t kSeqMask = (0xFFull << 56);
  static constexpr uint64_t kSeqLock = (0x01ull << 56);

  static ALWAYS_INLINE Entry LoadAcquire(Locked* pair) {
    while (true) {
      uint64_t key0 = pair->first.load(std::memory_order_acquire);
      uint64_t val = pair->second.load(std::memory_order_acquire);
      uint64_t key1 = pair->first.load(std::memory_order_acquire);
  ...
  static ALWAYS_INLINE void StoreRelease(Locked* pair, Entry value) {
    DCHECK_EQ(value.first & kSeqMask, 0ull);
    uint64_t key = pair->first.load(std::memory_order_relaxed);
    while ((key & kSeqLock) != 0 || !pair->first.compare_exchange_weak(key, key | kSeqLock)) {}
    key = (value.first & ~kSeqMask) + (key & kSeqMask) + 2*kSeqLock;
    pair->second.store(value.second, std::memory_order_release);
    pair->first.store(key, std::memory_order_release);
  }
 ...
 private:
  ...
  static std::array<Locked, kSharedSize> shared_array_;
};
