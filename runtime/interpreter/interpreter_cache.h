...
class ALIGNED(16) InterpreterCache {
 public:
  ...
  static constexpr size_t kLargeSize = 4 * 1024;
  ...
  template<bool kCheckLocal = true>
  ALWAYS_INLINE bool Get(Thread*, const void* dex_pc_ptr, /* out */ size_t* value) {
    DCHECK(IsCalledFromOwningThread());
    Entry& small_entry = small_array_[IndexOf<kSmallSize>(dex_pc_ptr)];
    if (kCheckLocal && LIKELY(small_entry.first == dex_pc_ptr)) {
