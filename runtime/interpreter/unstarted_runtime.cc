...
static constexpr double kMinLoadFactor = 0.5;  // Irrelevant, HashMap<>s below are never resized.
static constexpr double kMaxLoadFactor = 0.7;

constexpr size_t BufferSize(size_t size) {
  // Nore: ceil() is not suitable for constexpr, so cast to size_t and adjust by 1 if needed.
