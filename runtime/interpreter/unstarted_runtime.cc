...
void UnstartedRuntime::UnstartedJNIClassGetPublicFieldRecursive(
  ...
  ObjPtr<mirror::Class> klass = reinterpret_cast<mirror::Class*>(receiver)->AsClass();

  StackHandleScope<1> hs(self);
  LOG(ERROR) << "Slots: " << hs.RemainingSlots();
  Handle<mirror::Field> field = hs.NewHandle(GetPublicFieldRecursive(
      self, klass, name->AsString()));
  LOG(ERROR) << "Slots: " << hs.RemainingSlots();
  if (field.Get() == nullptr || ShouldDenyAccessToMember(field->GetArtField(), method)) {
    ThrowNoSuchFieldException(klass, name->AsString()->ToModifiedUtf8().c_str());
    return;
  }
