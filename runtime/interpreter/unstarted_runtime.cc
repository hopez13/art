...
#define ONE_PLUS(ShortName, Descriptor, Name, Signature) 1 +
...
// Buffer sizes are `ceil(size / kMaxLoadFactor)` but `ceil()` cannot be used for `constexpr`.
static constexpr size_t kInvokeHandlersBufferSize = ((kInvokeHandlersSize * 10u + 6u) / 7u) * 7u;
static constexpr size_t kJniHandlersBufferSize = ((kJniHandlersSize * 10u + 6u) / 7u) * 7u;
