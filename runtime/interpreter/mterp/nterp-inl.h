...
#ifndef ART_RUNTIME_INTERPRETER_MTERP_NTERP_INL_H_
#define ART_RUNTIME_INTERPRETER_MTERP_NTERP_INL_H_
...
ALWAYS_INLINE inline uint32_t GetNterpFastPathFlags(std::string_view shorty,
  ...
  const bool kNoFloatReturn = shorty[0] != 'F' && shorty[0] != 'D';
  if (isa != InstructionSet::kRiscv64 && all_parameters_are_reference_or_int && kNoFloatReturn) {
    nterp_flags |= kAccNterpInvokeFastPathFlag;
  } else if (isa == InstructionSet::kRiscv64 && all_parameters_are_reference && kNoFloatReturn) {
    nterp_flags |= kAccNterpInvokeFastPathFlag;
  }
  return nterp_flags;
}
...
#endif  // ART_RUNTIME_INTERPRETER_MTERP_NTERP_INL_H_
