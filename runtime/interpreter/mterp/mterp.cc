...
ALWAYS_INLINE void MterpFieldAccess(Instruction* inst,
                                    ...
                                    bool is_volatile)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  constexpr bool kIsStatic = kAccessType & FindFieldFlags::StaticBit;
  uint16_t vRegA = kIsStatic ? inst->VRegA_21c(inst_data) : inst->VRegA_22c(inst_data);
  if (kAccessType & FindFieldFlags::PrimitiveBit) {
    if (kAccessType & FindFieldFlags::ReadBit) {
      ...
      } else if (std::is_signed<PrimType>::value) {
        shadow_frame->SetVReg(vRegA, static_cast<int32_t>(value));  // Sign extend.
      } else {
        shadow_frame->SetVReg(vRegA, static_cast<uint32_t>(value));  // Zero extend.
      }
    } else {  // Write.
    ...
    if (kAccessType & FindFieldFlags::ReadBit) {
      ...
      mirror::Object* value = shadow_frame->GetVRegReference(vRegA);
      if (UNLIKELY(is_volatile)) {
...
NO_INLINE bool MterpFieldAccessSlow(Instruction* inst,
  ...
  ShadowFrame* shadow_frame = ShadowFrame::Current(vregs);
  ...
  mirror::Object* obj = (kAccessType & FindFieldFlags::InstanceBit)
      ? shadow_frame->GetVRegReference(inst->VRegB_22c(inst_data))
      : field->GetDeclaringClass().Ptr();
  if (obj == nullptr){
    constexpr bool is_read = kAccessType & FindFieldFlags::ReadBit;
    ThrowNullPointerExceptionForFieldAccess(field, is_read);
    return false;
  }
...
ALWAYS_INLINE bool MterpFieldAccessFast(Instruction* inst,
  ...
  return MterpFieldAccessSlow<PrimType, kAccessType>(inst, inst_data, vregs, self);
}

#define MTERP_FIELD_ACCESSOR(Name, PrimType, AccessType)                                          \
extern "C" bool Name(Instruction* inst, uint16_t inst_data, uint32_t* vregs, Thread* self)        \
    REQUIRES_SHARED(Locks::mutator_lock_) {                                                       \
  return MterpFieldAccessFast<PrimType, AccessType>(inst, inst_data, vregs, self);                \
}                                                                                                 \

#define MTERP_FIELD_ACCESSORS_FOR_TYPE(Sufix, PrimType, Kind)                                     \
  MTERP_FIELD_ACCESSOR(Mterp##IGet##Sufix, PrimType, Instance##Kind##Read)                        \
  MTERP_FIELD_ACCESSOR(Mterp##IPut##Sufix, PrimType, Instance##Kind##Write)                       \
  MTERP_FIELD_ACCESSOR(Mterp##SGet##Sufix, PrimType, Static##Kind##Read)                          \
  MTERP_FIELD_ACCESSOR(Mterp##SPut##Sufix, PrimType, Static##Kind##Write)                         \
