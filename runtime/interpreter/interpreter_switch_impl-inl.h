...
#define HANDLE_PENDING_EXCEPTION_WITH_INSTRUMENTATION(instr)                                    \
  do {                                                                                          \
    ...
    if (!MoveToExceptionHandler(self, shadow_frame, instr)) {                                   \
      ...
      return;  /* continue interpreting in this method */                                       \
    }                                                                                           \
  } while (false)
...
#pragma clang diagnostic pop
  ...
  ALWAYS_INLINE InstructionHandler(SwitchImplContext* _ctx,
                                   const instrumentation::Instrumentation* _instrumentation,
                                   const Instruction*& _inst,
                                   uint16_t _inst_data,
                                   bool& _exit_interpreter_loop)
    : ctx(_ctx),
      instrumentation(_instrumentation),
      self(ctx->self),
      accessor(ctx->accessor),
      shadow_frame(ctx->shadow_frame),
      result_register(ctx->result_register),
      interpret_one_instruction(ctx->interpret_one_instruction),
      dex_pc(shadow_frame.GetDexPC()),
      insns(accessor.Insns()),
      jit(Runtime::Current()->GetJit()),
      inst(_inst),
      inst_data(_inst_data),
      exit_interpreter_loop(_exit_interpreter_loop) {
  }
 ...
 private:
  ...
  bool const interpret_one_instruction;
...
ATTRIBUTE_NO_SANITIZE_ADDRESS void ExecuteSwitchImplCpp(SwitchImplContext* ctx) {
  ...
  while (true) {
    bool exit_interpreter_loop = false;
...
#define OPCODE_CASE(OPCODE, OPCODE_NAME, pname, f, i, a, e, v)                           \
      case OPCODE: {                                                                     \
        InstructionHandler<do_access_check, transaction_active> handler(                 \
            ctx, instrumentation, inst, inst_data, exit_interpreter_loop);               \
        handler.OPCODE_NAME();                                                           \
        if (UNLIKELY(exit_interpreter_loop)) {                                           \
          return;                                                                        \
        }                                                                                \
        break;                                                                           \
      }
