...
#define HANDLER_ATTRIBUTES ALWAYS_INLINE FLATTEN WARN_UNUSED REQUIRES_SHARED(Locks::mutator_lock_)
  ...
  HANDLER_ATTRIBUTES bool HandlePendingException() {
    ...
    if (!MoveToExceptionHandler(Self(), shadow_frame_, skip_event ? nullptr : Instrumentation())) {
      ...
      ctx_->result_ = JValue(); /* Handled in caller. */
      ExitInterpreterLooop();
  ...
  HANDLER_ATTRIBUTES bool Preamble() {
    ...
    if (UNLIKELY(Instrumentation()->HasDexPcListeners())) {
      ...
      JValue* save_ref = is_move_result_object ? &ctx_->result_register_ : nullptr;
...
#pragma clang diagnostic pop
 ...
 private:
  ...
  ALWAYS_INLINE const CodeItemDataAccessor& Accessor() { return ctx_->accessor_; }
  ALWAYS_INLINE const uint16_t* Insns() { return ctx_->accessor_.Insns(); }
  ALWAYS_INLINE JValue* ResultRegister() { return &ctx_->result_register_; }
  ...
  ALWAYS_INLINE void ExitInterpreterLooop() {
...
void ExecuteSwitchImplCpp(SwitchImplContext* ctx) {
  Thread* self = ctx->self_;
  const CodeItemDataAccessor& accessor = ctx->accessor_;
  ShadowFrame& shadow_frame = ctx->shadow_frame_;
  self->VerifyStack();
  ...
  bool const interpret_one_instruction = ctx->interpret_one_instruction_;
  while (true) {
...
#undef OPCODE_CASE
    ...
    if (interpret_one_instruction) {
      ...
      ctx->result_ = ctx->result_register_;
      return;
    }
  }
