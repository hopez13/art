...
ASAN_NO_INLINE static const Instruction* OP_##OPCODE_NAME(                                        \
  ...
  const Instruction* next = inst->RelativeAt(Instruction::SizeInCodeUnits(Instruction::FORMAT));  \
  ...
  return LIKELY(handler.OPCODE_NAME()) ? next : nullptr;                                          \
}
...
#define OPCODE_CASE(OPCODE, OPCODE_NAME, NAME, FORMAT, i, a, e, v)                                \
        case OPCODE: {                                                                            \
          DCHECK_EQ(self->IsExceptionPending(), (OPCODE == Instruction::MOVE_EXCEPTION));         \
          next = OP_##OPCODE_NAME<do_access_check, transaction_active>(                           \
              ctx, instrumentation, self, shadow_frame, dex_pc, inst, inst_data, exit);           \
          if (next != nullptr && LIKELY(!interpret_one_instruction)) {                            \
            DCHECK(!exit) << NAME;                                                                \
            continue;                                                                             \
          }                                                                                       \
          if (exit) {                                                                             \
