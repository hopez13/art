...
TEST_F(UnstartedRuntimeTest, ThreadCurrentThread) {
  ...
  StackHandleScope<1> hs(self);
  Handle<mirror::Class> thread_class = hs.NewHandle(class_linker_->FindClass(
      self, "Ljava/lang/Thread;", ScopedNullHandle<mirror::ClassLoader>()));
  ASSERT_TRUE(thread_class.Get() != nullptr);
  ASSERT_TRUE(class_linker_->EnsureInitialized(self, thread_class, true, true));
}
