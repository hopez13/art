...
static ALWAYS_INLINE bool DoInvoke(Thread* self,
  ...
  if (is_mterp && !is_range && UseInterpreterToInterpreterFastPath<type>(called_method)) {
    ...
    for (size_t i = 0, dst = first_dest_reg + i; i < number_of_inputs; ++i, ++dst) {
    ...
    size_t frame_size = ShadowFrame::ComputeSize(num_regs);
