...
static inline bool DoInvokePolymorphic(Thread* self, ShadowFrame& shadow_frame,
  ...
  mirror::MethodType* method_type = class_linker->ResolveMethodType(
  ...
  // TODO: Implement non-exact invokes.
  CHECK(method_type->IsExactMatch(method_handle->GetMethodType()))
      << "Non exact MethodHandle invokes are not implemented yet.";
  ...
  if (UNLIKELY(called_method == nullptr)) {
    ...
    return DoCallCommon<is_range, do_access_check, kVarArgsMax>(
