...
void EnterInterpreterFromDeoptimize(Thread* self,
  ...
  while (shadow_frame != nullptr) {
    ...
    if (UNLIKELY(self->IsExceptionPending())) {
      ...
      if (method_type == DeoptimizationMethodType::kKeepDexPc) {
        DCHECK(first);
        ...
        // not be skipped. A suspend check should also execute the dex
        // instruction at the corresponding dex pc.
        new_dex_pc = dex_pc;
        ...
        if ((value.GetL()->IsString())) {
          ...
          DCHECK(method_type == DeoptimizationMethodType::kDefault);
          DCHECK(first);
