...
#define POSSIBLY_HANDLE_PENDING_EXCEPTION_ON_INVOKE_IMPL(_is_exception_pending, _next_function) \
  do {                                                                                          \
    ...
    } else if (UNLIKELY(shadow_frame.GetForceRetryInstruction())) {                             \
      /* Don't need to do anything except clear the flag. We leave the */                       \
      /* instruction the same so it will be re-executed.               */                       \
