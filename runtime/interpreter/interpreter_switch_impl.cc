...
// This handles moving the interpreter to the right exception handler in the current method if there
// is one. It will also send all the appropriate instrumentation events as it does so. It returns
// true if execution should continue in the current method (the exception was handled) and false if
// the exception could not be handled in this stack frame.
...
NO_INLINE static bool MoveToExceptionHandler(
  ...
  Handle<mirror::Throwable> exception(hs.NewHandle<mirror::Throwable>(self->GetException()));
  DCHECK(!exception.IsNull());
  Handle<mirror::Class> exception_klass(hs.NewHandle<mirror::Class>(exception->GetClass()));
  ...
  if (found_dex_pc == DexFile::kDexNoIndex) {
    if (instrumentation->HasWatchedFramePopListeners() && shadow_frame.NeedsNotifyPop()) {
      instrumentation->WatchedFramePopped(self, shadow_frame);
    }
    if (send_event == SendUnwindEvent::kSendEvent && instrumentation->HasMethodUnwindListeners()) {
      // Exception is not caught by the current method. We will unwind to the caller. Notify any
    ...
    // Exception is caught in the current method. We will jump to the found_dex_pc.  Move to the
    // handler.
    *inst_out = (*inst_out)->RelativeAt(
        static_cast<int32_t>(found_dex_pc) - static_cast<int32_t>(dex_pc));
    // Also update the shadow-frame so that stack-walks know where we actually are.
    shadow_frame.SetDexPC((*inst_out)->GetDexPc(raw_instructions));
    // Send the ExceptionHandled event if needed
    if (instrumentation->HasExceptionHandledListeners()) {
      ...
      if (self->GetException() != nullptr) {
        return MoveToExceptionHandler<send_event>(self,
                                                  shadow_frame,
                                                  raw_instructions,
                                                  instrumentation,
                                                  inst_out);
