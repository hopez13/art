...
#define PREAMBLE() PREAMBLE(JValue())
...
// to detect exceptions thrown by the event. We had to move this into its own function because it
...
static bool DoDexPcMoveEvent(Thread* self,
  ...
  ScopedNullHandle<mirror::Object> null_handle;
  Handle<mirror::Object> save_obj(UNLIKELY(save_ref.GetL() != nullptr)
                                  ? hs.NewHandle(save_ref.GetL())
                                  : null_handle);
  self->ClearException();
