  ...
  do {
    ...
    switch (inst->Opcode(inst_data)) {
      ...
      case Instruction::CHECK_CAST: {
        PREAMBLE();
        ObjPtr<mirror::Class> c = ResolveVerifyAndClinit(inst->VRegB_21c(), shadow_frame.GetMethod(),
      ...
      case Instruction::INSTANCE_OF: {
        PREAMBLE();
        ObjPtr<mirror::Class> c = ResolveVerifyAndClinit(inst->VRegC_22c(), shadow_frame.GetMethod(),
                                          self, false, do_access_check);
        if (UNLIKELY(c == nullptr)) {
          HANDLE_PENDING_EXCEPTION();
        } else {
      ...
      case Instruction::THROW: {
        PREAMBLE();
        ObjPtr<mirror::Object> exception = shadow_frame.GetVRegReference(inst->VRegA_11x(inst_data));
