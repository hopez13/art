...
#define CREATE_SHADOW_FRAME(num_vregs, link, method, dex_pc) ({                              \
  ...
  int64_t GetVRegLong(size_t i) const {
    ...
    const uint32_t* vreg = &vregs_[i];
    using unaligned_int64 = const int64_t __attribute__ ((aligned (4)));
  ...
  double GetVRegDouble(size_t i) const {
    DCHECK_LT(i + 1, NumberOfVRegs());
    const uint32_t* vreg = &vregs_[i];
    using unaligned_double = const double __attribute__ ((aligned (4)));
  ...
  void SetVRegLong(size_t i, int64_t val) {
    DCHECK_LT(i + 1, NumberOfVRegs());
    uint32_t* vreg = &vregs_[i];
    using unaligned_int64 = int64_t __attribute__ ((aligned (4)));
  ...
  void SetVRegDouble(size_t i, double val) {
    DCHECK_LT(i + 1, NumberOfVRegs());
    uint32_t* vreg = &vregs_[i];
    using unaligned_double = double __attribute__ ((aligned (4)));
