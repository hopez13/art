...
class CallerRegisterRange final : public CallerRegisters {
 public:
  CallerRegisterRange(uint32_t start, size_t count) : CallerRegisters(count), start_(start) {}
  uint32_t GetRegister(size_t index) const override {
    DCHECK(index < count_);
    return start_ + index;
  }
 private:
...
class CallerVarArgsRegisters final : public CallerRegisters {
 public:
  CallerVarArgsRegisters(const uint32_t* varargs, size_t count)
      : CallerRegisters(count), varargs_(varargs) {}
  uint32_t GetRegister(size_t index) const override {
    DCHECK(index < count_);
    return varargs_[index];
  }
 private:
  const uint32_t* varargs_;
};
...
bool DoInvokePolymorphic(Thread* self,
  ...
  bool is_invoke_exact = IsInvokeExact(shadow_frame.GetMethod()->GetDeclaringClass()->GetDexFile(),
                                       invoke_method_idx);
  if (is_range) {
    CallerRegisterRange registers(inst->VRegC_4rcc() + 1, inst->VRegA() - 1);
    if (is_invoke_exact) {
      return DoInvokePolymorphicExact(self,
                                      shadow_frame,
                                      method_handle,
                                      callsite_type,
                                      registers,
    ...
    CallerVarArgsRegisters registers(arg + 1, inst->VRegA() - 1);
    if (is_invoke_exact) {
      return DoInvokePolymorphicExact(self,
                                      shadow_frame,
                                      method_handle,
