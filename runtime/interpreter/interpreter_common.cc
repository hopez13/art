...
static inline bool DoCallCommon(ArtMethod* called_method,
  ...
  if (LIKELY(accessor.HasCodeItem())) {
    ...
    if (!use_interpreter_entrypoint) {
      ...
      DCHECK_EQ(string_init ? number_of_inputs - 1 : number_of_inputs, accessor.InsSize()) << string_init;
    }
  } else {
    DCHECK(called_method->IsNative() || called_method->IsProxyMethod()) << called_method->PrettyMethod();
    num_regs = number_of_inputs;
  }
