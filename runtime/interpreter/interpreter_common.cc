...
bool DoFieldPut(Thread* self, const ShadowFrame& shadow_frame, const Instruction* inst,
  ...
  // TODO(oth): ScopedAssertNoThreadSuspension ants(__FUNCTION__);
  uint32_t vregA = is_static ? inst->VRegA_21c(inst_data) : inst->VRegA_22c(inst_data);
...
inline bool DoInvokePolymorphic(Thread* self,
  ...
  if (IsInvoke(handle_kind)) {
    ...
    switch (handle_kind) {
      ...
      case kInstancePut: {
        // TODO(oth): ScopedAssertNoThreadSuspension ants("MH instance put");
      ...
      case kStaticPut: {
        // TODO(oth): ScopedAssertNoThreadSuspension ants("MH static put");
