...
inline bool DoInvokePolymorphic(Thread* self, ShadowFrame& shadow_frame,
  ...
  ArtMethod* invoke_method =
      class_linker->ResolveMethod<ClassLinker::ResolveMode::kForceICCECheck>(
          ...
          shadow_frame.GetMethod(),
          InvokeType::kVirtual);
  if (UNLIKELY(invoke_method == nullptr)) {
    CHECK(self->IsExceptionPending());
    result->SetJ(0);
    return false;
  }
  DCHECK(invoke_method->IsMethodHandleInvokeExact() || invoke_method->IsMethodHandleInvoke())
      << ArtMethod::PrettyMethod(invoke_method);
  bool is_invoke_exact = invoke_method->IsMethodHandleInvokeExact();
  ...
  if (IsInvoke(handle_kind)) {
    ...
    if (!is_invoke_exact) {
      if (handle_type->GetRType()->GetPrimitiveType() == Primitive::kPrimVoid &&
          callsite_type->GetRType()->GetPrimitiveType() != Primitive::kPrimVoid) {
        // Setter but caller is attempting to use return value.
        ThrowWrongMethodTypeException(callsite_type.Get(), handle_type.Get());
        return false;
      } else if (handle_type->GetPTypes()->GetLength() != callsite_type->GetPTypes()->GetLength()) {
    ...
    switch (handle_kind) {
      case kInstanceGet: {
        ObjPtr<Object> obj = shadow_frame.GetVRegReference(receiver_vregC);
      ...
      case kStaticGet: {
        ObjPtr<Object> obj = field->GetDeclaringClass();
        DoFieldGetForInvokePolymorphic(self, shadow_frame, obj, field, field_type, result);
        return true;
      }
      case kStaticPut: {
