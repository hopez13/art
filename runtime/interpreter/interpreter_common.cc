...
static bool DoMethodHandleInvokeCommon(Thread* self,
                                       ShadowFrame& shadow_frame,
                                       ArtMethod* method_handle_method,
                                       const Instruction* inst,
                                       uint16_t inst_data,
                                       JValue* result)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  bool isExact =
      (jni::EncodeArtMethod(method_handle_method) ==
  ...
  if (UNLIKELY(is_range)) {
    ...
    if (isExact) {
      ...
      DCHECK_EQ(jni::EncodeArtMethod(method_handle_method),
...
bool DoMethodHandleInvokeExact(Thread* self,
  ...
  ArtMethod* const method_handle_method =
  ...
  if (inst->Opcode() == Instruction::INVOKE_POLYMORPHIC) {
    ...
    return DoMethodHandleInvokeCommon<kIsRange>(
        self, shadow_frame, method_handle_method, inst, inst_data, result);
  } else {
...
bool DoInvokePolymorphic(Thread* self,
  ...
  if (!invoke_method->IsIntrinsic()) {
    InitializeSignaturePolymorphicIntrinsics();
  }
  ...
  switch (static_cast<art::Intrinsics>(invoke_method->GetIntrinsic())) {
    case Intrinsics::kMethodHandleInvokeExact:
      return DoMethodHandleInvokeExact(self, shadow_frame, inst, inst_data, result);
    case Intrinsics::kMethodHandleInvoke:
