...
static bool GetArgumentForBootstrapMethod(Thread* self,
  ...
  switch (type) {
    ...
    case EncodedArrayValueIterator::ValueType::kInt:
    case EncodedArrayValueIterator::ValueType::kFloat:
      decoded_value->SetI(encoded_value.i);
      return true;
    case EncodedArrayValueIterator::ValueType::kLong:
    case EncodedArrayValueIterator::ValueType::kDouble:
      decoded_value->SetJ(encoded_value.j);
      return true;
    case EncodedArrayValueIterator::ValueType::kMethodType: {
      StackHandleScope<2> hs(self);
      Handle<mirror::ClassLoader> class_loader(hs.NewHandle(referrer->GetClassLoader()));
      Handle<mirror::DexCache> dex_cache(hs.NewHandle(referrer->GetDexCache()));
      uint32_t index = static_cast<uint32_t>(encoded_value.i);
    ...
    case EncodedArrayValueIterator::ValueType::kString: {
      StackHandleScope<1> hs(self);
      Handle<mirror::DexCache> dex_cache(hs.NewHandle(referrer->GetDexCache()));
      dex::StringIndex index(static_cast<uint32_t>(encoded_value.i));
      ClassLinker* cl = Runtime::Current()->GetClassLinker();
    ...
    case EncodedArrayValueIterator::ValueType::kType: {
      ...
      Handle<mirror::ClassLoader> class_loader(hs.NewHandle(referrer->GetClassLoader()));
      Handle<mirror::DexCache> dex_cache(hs.NewHandle(referrer->GetDexCache()));
      dex::TypeIndex index(static_cast<uint32_t>(encoded_value.i));
      ClassLinker* cl = Runtime::Current()->GetClassLinker();
...
static ObjPtr<mirror::Array> BuildCollectorArrayForBootstrapMethod(Thread* self,
                                                                   ArtMethod* referrer,
...
#define COLLECT_PRIMITIVE_ARRAY(Descriptor, Type)                       \
  ...
  for (int32_t i = 0; it->HasNext(); it->Next(), ++i) {                 \
    const EncodedArrayValueIterator::ValueType type = it->GetValueType(); \
    jvalue encoded_value = it->GetJavaValue();                          \
    DCHECK_EQ(type, EncodedArrayValueIterator::ValueType::k ## Type);   \
    GetArgumentForBootstrapMethod(self, referrer, type, encoded_value, &decoded_value); \
    array->Set(i, decoded_value.Get ## Descriptor());                   \
  }                                                                     \
  return array.Get()

#define COLLECT_REFERENCE_ARRAY(T, EncodedType)                         \
  Handle<mirror::ObjectArray<T>> array =                                \
      hs.NewHandle(mirror::ObjectArray<T>::Alloc(self, array_type, array_length)); \
  if (array.IsNull()) {                                                 \
    return nullptr;                                                     \
  }                                                                     \
  for (int32_t i = 0; it->HasNext(); it->Next(), ++i) {                 \
    const EncodedArrayValueIterator::ValueType type = it->GetValueType(); \
    jvalue encoded_value = it->GetJavaValue();                          \
    DCHECK_EQ(type, EncodedArrayValueIterator::ValueType::k ## EncodedType); \
    if (!GetArgumentForBootstrapMethod(self, referrer, type, encoded_value, &decoded_value)) { \
      return nullptr;                                                   \
    }                                                                   \
  ...
  if (array_type->GetComponentType() == class_linker->FindPrimitiveClass('Z')) {
    COLLECT_PRIMITIVE_ARRAY(Z, Boolean);
  } else if (array_type->GetComponentType() == class_linker->FindPrimitiveClass('B')) {
    COLLECT_PRIMITIVE_ARRAY(B, Byte);
  } else if (array_type->GetComponentType() == class_linker->FindPrimitiveClass('C')) {
    COLLECT_PRIMITIVE_ARRAY(C, Char);
  } else if (array_type->GetComponentType() == class_linker->FindPrimitiveClass('S')) {
    COLLECT_PRIMITIVE_ARRAY(S, Short);
  } else if (array_type->GetComponentType() == class_linker->FindPrimitiveClass('I')) {
...
static ObjPtr<mirror::MethodType> BuildCallSiteForBootstrapMethod(Thread* self,
  ...
  int index = 1;
  while (it.HasNext()) {
    ObjPtr<mirror::Class> param_type = GetClassForBootstrapArgument(it.GetValueType());
    if (param_type.IsNull()) {
      ThrowBootstrapMethodError("Unsupported argument type");
      return nullptr;
    }
    ptypes->Set(index, param_type);
    index++;
    it.Next();
  }
...
static ObjPtr<mirror::CallSite> InvokeBootstrapMethod(Thread* self,
  ...
  // Check if target is variable arity. We'll need to collect arguments into an array if so.
  ...
  if (bsm->GetTargetMethod()->IsVarargs()) {
    ...
    for (int i = number_of_bsm_parameters; i < call_site_type->GetNumberOfPTypes(); ++i) {
      if (call_site_type->GetPTypes()->Get(i) != collector_array_class->GetComponentType()) {
        // This may be too restrictive, e.g. collected arguments are boxed or instances-of.
        ThrowWrongMethodTypeException(bsm->GetMethodType(), call_site_type.Get());
        return nullptr;
      }
    }
  ...
  // "opportunities".
  if (!call_site_type->IsConvertible(bsm->GetMethodType())) {
    ThrowClassCastException("TBD: IMPLEMENT ME");
    return nullptr;
  }
  ...
  for (argument_index = 1; argument_index < number_of_arguments; ++argument_index) {
    if (argument_index == number_of_arguments - 1 &&
      ...
      ObjPtr<mirror::Array> collector_array =
          BuildCollectorArrayForBootstrapMethod(self,
                                                referrer,
                                                array_type,
                                                collector_arguments_length,
                                                &it);
      if (collector_array.IsNull()) {
        DCHECK(self->IsExceptionPending());
        return nullptr;
      }
      setter.SetReference(collector_array);
    } else if (!PackArgumentForBootstrapMethod(self,
                                               referrer,
                                               it.GetValueType(),
                                               it.GetJavaValue(),
                                               &setter)) {
