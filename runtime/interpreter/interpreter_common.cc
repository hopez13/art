...
inline bool DoInvokePolymorphic(Thread* self, ShadowFrame& shadow_frame,
  ...
  // that we disallow calls via invoke-polymorphic to non sig-poly methods. This
  // would also have the side effect of verifying that vRegC really is a
  // reference type.
  ...
  if (UNLIKELY(method_handle == nullptr)) {
    ...
    ThrowNullPointerExceptionForMethodAccess(
        (is_range) ? inst->VRegB_4rcc() : inst->VRegB_45cc(),
        InvokeType::kVirtual);
  ...
  // signature polymorphic callsite. This callsite information is recorded by
  // the frontend compiler (JACK or JavaC).
  ...
  if (IsInvoke(handle_kind)) {
    if (handle_kind == kInvokeVirtual || handle_kind == kInvokeInterface) {
      ...
      LOG(FATAL) << "Direct invokes are not implemented yet.";
      return false;
    }
    ...
    LOG(FATAL) << "Non exact invokes are not implemented yet.";
    return false;
  } else {
    // TODO(narayan): Implement field getters and setters.
    LOG(FATAL) << "Field references in method handles are not implemented yet.";
    return false;
  }
}
