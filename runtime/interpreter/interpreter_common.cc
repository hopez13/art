...
static bool DoMethodHandleInvokeCommon(Thread* self,
  ...
  if (UNLIKELY(is_range)) {
    ...
    RangeOperandIterator operands(inst->VRegC_4rcc() + 1, inst->VRegA_4rcc() - 1);
    if (invoke_exact) {
      return art::MethodHandleInvokeExact(self,
                                          shadow_frame,
                                          method_handle,
                                          callsite_type,
                                          &operands,
      ...
      return art::MethodHandleInvoke(self,
                                     shadow_frame,
                                     method_handle,
                                     callsite_type,
                                     &operands,
    ...
    VarArgsOperandIterator operands(args, inst->VRegA_45cc() - 1);
    if (invoke_exact) {
      return art::MethodHandleInvokeExact(self,
                                          shadow_frame,
                                          method_handle,
...
static ObjPtr<mirror::CallSite> InvokeBootstrapMethod(Thread* self,
  ...
  JValue result;
  RangeOperandIterator operands(0, vreg);
  bool invoke_success = art::MethodHandleInvokeExact(self,
                                                     *bootstrap_frame,
                                                     bootstrap,
...
bool DoInvokeCustom(Thread* self,
  ...
  if (is_range) {
    RangeOperandIterator operands(inst->VRegC_3rc(), inst->VRegA_3rc());
    return art::MethodHandleInvokeExact(self,
                                        shadow_frame,
                                        target,
                                        target_method_type,
    ...
    uint32_t args[Instruction::kMaxVarArgRegs];
    inst->GetVarArgs(args, inst_data);
    VarArgsOperandIterator operands(args, inst->VRegA_35c());
    return art::MethodHandleInvokeExact(self,
                                        shadow_frame,
                                        target,
                                        target_method_type,
