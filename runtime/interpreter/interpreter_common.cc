...
inline bool DoInvokePolymorphic(Thread* self,
  ...
  if (IsInvoke(handle_kind)) {
    if (handle_kind == kInvokeVirtual || handle_kind == kInvokeInterface) {
      // TODO: This is a wart. We have to postpone dynamic receiver based checks
      // as much as possible, because the receiver might be cast or might
      // come from an emulated stack frame, which means that it is unknown
      // at this point. We perform these checks inside InvokePolyMorphic right
      // before we do the actualy invoke.
