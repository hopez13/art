...
class ActiveTransactionChecker {
 public:
  ...
  static void RecordAllocatedObject([[maybe_unused]] ObjPtr<mirror::Object> new_object)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    GetClassLinker()->GetTransaction()->SetLastAllocatedObject(new_object);
  }
...
void ActiveTransactionChecker::RecordArrayElementsInTransaction(ObjPtr<mirror::Object> array,
  ...
  if (array == transaction->GetLastAllocatedObject()) {
    // We do not need to record changes to an array that becomes unreachable on rollback.
    return;
  }
