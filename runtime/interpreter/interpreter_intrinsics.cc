...
static inline T peek(uint64_t addr) {
  ...
  const T* aligned_p = reinterpret_cast<const T*>(addr);
  if ((sizeof(T) == 1) || (IsAligned<sizeof(T)>(aligned_p))) {
    return *aligned_p;
  } else {
    // Force unaligned access pattern.
    const unaligned_T* unaligned_p = reinterpret_cast<const unaligned_T*>(addr);
    return *unaligned_p;
  }
}
...
static inline void poke(uint64_t addr, T val) {
  typedef __attribute__((aligned(1))) T unaligned_T;
  T* aligned_p = reinterpret_cast<T*>(addr);
  if ((sizeof(T) == 1) || (IsAligned<sizeof(T)>(aligned_p))) {
    *aligned_p = val;
  } else {
    // Force unaligned access pattern.
    unaligned_T* unaligned_p = reinterpret_cast<unaligned_T*>(addr);
    *unaligned_p = val;
  }
}
