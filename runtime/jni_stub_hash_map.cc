...
size_t JniStubKeyOptimizedHash(const JniStubKey& key) {
  ...
  constexpr size_t max_float_or_double_register_args = GetMaxFloatOrDoubleRegisterArgs(kIsa);
  constexpr size_t max_int_like_register_args = GetMaxIntLikeRegisterArgs(kIsa);
  ...
  for (char c : shorty.substr(1u)) {
    ...
    if (IsFloatOrDoubleArg(c)) {
      ...
      if (int_like_args > max_int_like_register_args || IsReferenceArg(c)) {
        ...
        // spilled as raw data even it's on register.
        is_stack_offset_matters = true;
      } else if (!is_static) {
        // For non-static method, 2 managed arguments 'ArtMethod*' and 'Object* this' correspond
        // to 'JNIEnv*' and 'jobject'. So trailing integral arguments will remain in the same
        // registers, which do not need any generated codes.
        continue;
      }
    }
...
template size_t JniStubKeyOptimizedHash<InstructionSet::kArm64>(const JniStubKey& key);
template size_t JniStubKeyOptimizedHash<InstructionSet::kX86_64>(const JniStubKey& key);
...
template size_t JniStubKeyGenericHash<InstructionSet::kRiscv64>(const JniStubKey& key);
template size_t JniStubKeyGenericHash<InstructionSet::kArm>(const JniStubKey& key);
template size_t JniStubKeyGenericHash<InstructionSet::kX86>(const JniStubKey& key);
...
template bool JniStubKeyOptimizedEquals<InstructionSet::kArm64>(const JniStubKey& lhs,
                                                                const JniStubKey& rhs);
template bool JniStubKeyOptimizedEquals<InstructionSet::kX86_64>(const JniStubKey& lhs,
                                                                 const JniStubKey& rhs);
...
template bool JniStubKeyGenericEquals<InstructionSet::kRiscv64>(const JniStubKey& lhs,
                                                                const JniStubKey& rhs);
template bool JniStubKeyGenericEquals<InstructionSet::kArm>(const JniStubKey& lhs,
                                                            const JniStubKey& rhs);
template bool JniStubKeyGenericEquals<InstructionSet::kX86>(const JniStubKey& lhs,
                                                            const JniStubKey& rhs);
