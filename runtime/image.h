...
class PACKED(8) ImageHeader {
 ...
 private:
  ...
  // Boot image begin and end (boot image extension and app image headers).
  uint32_t boot_image_begin_ = 0u;
  uint32_t boot_image_size_ = 0u;  // Includes heap (*.art) and code (.oat).

  // Number of boot image components that this image depend on and their composite checksum
  // (boot image extension and app image headers).
