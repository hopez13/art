...
class ClassData {
 public:
  // Requires Scope to be able to create at least 3 handles.
  template <typename Scope>
  static ClassData&& FromMethod(Scope& hs, ArtMethod* method)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    return std::move(ClassData(hs.NewHandle(method->GetDeclaringClass()),
  ...
  // Requires Scope to be able to create at least 3 handles.
  template <typename Scope>
  static ClassData&& FromField(Scope& hs, ArtField* field)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    return FromClass(hs, hs.NewHandle(field->GetDeclaringClass()));
  }

  // Requires Scope to be able to create at least 2 handles.
  template <typename Scope>
  static ClassData&& FromClass(Scope& hs, Handle<mirror::Class> klass)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    return std::move(ClassData(klass,
  ...
  Handle<mirror::DexCache> GetDexCache() const REQUIRES_SHARED(Locks::mutator_lock_) {
    return dex_cache_;
  }
  Handle<mirror::ClassLoader> GetClassLoader() const REQUIRES_SHARED(Locks::mutator_lock_) {
    return loader_;
  }

  Handle<mirror::Class> GetRealClass() const REQUIRES_SHARED(Locks::mutator_lock_) {
    return real_klass_;
  }
    ...
    case DexFile::kDexAnnotationString: {
      ...
      if (result_style == DexFile::kAllRaw) {
        ...
        element_object = Runtime::Current()->GetClassLinker()->ResolveString(
            klass.GetDexFile(), dex::StringIndex(index), klass.GetDexCache());
        set_object = true;
    ...
    case DexFile::kDexAnnotationType: {
      ...
      if (result_style == DexFile::kAllRaw) {
        ...
        element_object = Runtime::Current()->GetClassLinker()->ResolveType(
            klass.GetDexFile(), type_index, klass.GetDexCache(), klass.GetClassLoader());
        set_object = true;
        if (element_object == nullptr) {
    ...
    case DexFile::kDexAnnotationMethod: {
      ...
      if (result_style == DexFile::kAllRaw) {
        ...
        ArtMethod* method = class_linker->ResolveMethodWithoutInvokeType(
            klass.GetDexFile(), index, klass.GetDexCache(), klass.GetClassLoader());
        if (method == nullptr) {
          return false;
        }
    ...
    case DexFile::kDexAnnotationEnum: {
      ...
      if (result_style == DexFile::kAllRaw) {
        ...
        ArtField* enum_field = Runtime::Current()->GetClassLinker()->ResolveField(
            klass.GetDexFile(), index, klass.GetDexCache(), klass.GetClassLoader(), true);
...
mirror::Object* GetAnnotationForField(ArtField* field, Handle<mirror::Class> annotation_class) {
  ...
  const ClassData klass(ClassData::FromField(hs, field));
  return GetAnnotationObjectFromAnnotationSet(klass, annotation_set,
...
mirror::ObjectArray<mirror::Object>* GetAnnotationsForField(ArtField* field) {
  const DexFile::AnnotationSetItem* annotation_set = FindAnnotationSetForField(field);
  StackHandleScope<3> hs(Thread::Current());
  const ClassData field_class(ClassData::FromField(hs, field));
...
mirror::Object* GetAnnotationDefaultValue(ArtMethod* method) {
  ...
  const ClassData klass(ClassData::FromMethod(hs, method));
  const DexFile* dex_file = &klass.GetDexFile();
  const DexFile::AnnotationsDirectoryItem* annotations_dir =
      dex_file->GetAnnotationsDirectory(*klass.GetClassDef());
  if (annotations_dir == nullptr) {
...
mirror::Object* GetAnnotationForMethod(ArtMethod* method, Handle<mirror::Class> annotation_class) {
  ...
  StackHandleScope<3> hs(Thread::Current());
  return GetAnnotationObjectFromAnnotationSet(ClassData::FromMethod(hs, method), annotation_set,
                                              DexFile::kDexVisibilityRuntime, annotation_class);
}
...
mirror::ObjectArray<mirror::Object>* GetAnnotationsForMethod(ArtMethod* method) {
  const DexFile::AnnotationSetItem* annotation_set = FindAnnotationSetForMethod(method);
  StackHandleScope<3> hs(Thread::Current());
  return ProcessAnnotationSet(ClassData::FromMethod(hs, method),
                              annotation_set,
                              DexFile::kDexVisibilityRuntime);
}
...
mirror::ObjectArray<mirror::Class>* GetExceptionTypesForMethod(ArtMethod* method) {
  ...
  StackHandleScope<3> hs(Thread::Current());
  return GetThrowsValue(ClassData::FromMethod(hs, method), annotation_set);
}
...
mirror::ObjectArray<mirror::Object>* GetParameterAnnotations(ArtMethod* method) {
  ...
  StackHandleScope<3> hs(Thread::Current());
  return ProcessAnnotationSetRefList(ClassData::FromMethod(hs, method), set_ref_list, size);
}
...
mirror::Object* GetAnnotationForMethodParameter(ArtMethod* method,
  ...
  StackHandleScope<3> hs(Thread::Current());
  return GetAnnotationObjectFromAnnotationSet(ClassData::FromMethod(hs, method),
                                              annotation_set,
                                              DexFile::kDexVisibilityRuntime,
                                              annotation_class);
}
...
bool GetParametersMetadataForMethod(ArtMethod* method,
  ...
  StackHandleScope<3> class_data_hs(Thread::Current());
  ClassData data(ClassData::FromMethod(class_data_hs, method));
...
mirror::ObjectArray<mirror::String>* GetSignatureAnnotationForMethod(ArtMethod* method) {
  ...
  StackHandleScope<3> hs(Thread::Current());
  return GetSignatureValue(ClassData::FromMethod(hs, method), annotation_set);
}
...
bool IsMethodAnnotationPresent(ArtMethod* method, Handle<mirror::Class> annotation_class,
  ...
  StackHandleScope<3> hs(Thread::Current());
  const DexFile::AnnotationItem* annotation_item =
      GetAnnotationItemFromAnnotationSet(ClassData::FromMethod(hs, method),
...
mirror::Object* GetAnnotationForClass(Handle<mirror::Class> klass,
  ...
  ClassData data(ClassData::FromClass(hs, klass));
...
mirror::Class* GetEnclosingClass(Handle<mirror::Class> klass) {
  ...
  ArtMethod* method = Runtime::Current()->GetClassLinker()->ResolveMethodWithoutInvokeType(
      data.GetDexFile(), annotation_value.value_.GetI(), data.GetDexCache(), data.GetClassLoader());
  if (method == nullptr) {
    return nullptr;
  }
