...
const uint8_t* SearchEncodedAnnotation(const DexFile& dex_file, const uint8_t* annotation,
                                       const char* name) {
...
bool GetInnerClass(Handle<mirror::Class> klass, mirror::String** name) {
  ...
  DexFile::AnnotationValue annotation_value;
  if (!ProcessAnnotationValue(klass, &annotation, &annotation_value,
                              ScopedNullHandle<mirror::Class>(), DexFile::kAllObjects)) {
    return false;
  }
...
const DexFile::DexFile::AnnotationItem* GetAnnotationItemFromAnnotationSet(
...
mirror::Object* GetAnnotationObjectFromAnnotationSet(Handle<mirror::Class> klass,
    const DexFile::AnnotationSetItem* annotation_set, uint32_t visibility,
    Handle<mirror::Class> annotation_class) {
  const DexFile::AnnotationItem* annotation_item =
...
mirror::Object* GetAnnotationValue(Handle<mirror::Class> klass,
    const DexFile::AnnotationItem* annotation_item, const char* annotation_name,
    Handle<mirror::Class> array_class, uint32_t expected_type) {
  const DexFile& dex_file = klass->GetDexFile();
  const uint8_t* annotation =
...
mirror::ObjectArray<mirror::String>* GetSignatureValue(
