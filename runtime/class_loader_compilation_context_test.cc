...
TEST_F(ClassLoaderCompilationContextTest, ParseValidEmptyContextDLC) {
  std::unique_ptr<ClassLoaderCompilationContext> context =
      ClassLoaderCompilationContext::Create("DLC[]");
  verifyContextSize(context.get(), 1);
...
TEST_F(ClassLoaderCompilationContextTest, OpenInvalidDexFilesMix) {
  std::string dex_name = GetTestDexFileName("Main");
  std::unique_ptr<ClassLoaderCompilationContext> context =
      ClassLoaderCompilationContext::Create("PCL[does_not_exist.dex];DLC[" + dex_name + "]");
  ASSERT_FALSE(context->OpenDexFiles(InstructionSet::kArm, ""));
}

TEST_F(ClassLoaderCompilationContextTest, CreateClassLoader) {
  std::string dex_name = GetTestDexFileName("Main");
  std::unique_ptr<ClassLoaderCompilationContext> context =
      ClassLoaderCompilationContext::Create("PCL[" + dex_name + "]");
...
TEST_F(ClassLoaderCompilationContextTest, RemoveSourceLocations) {
  std::unique_ptr<ClassLoaderCompilationContext> context =
      ClassLoaderCompilationContext::Create("PCL[a.dex]");
...
TEST_F(ClassLoaderCompilationContextTest, EncodeInOatFile) {
  std::string dex1_name = GetTestDexFileName("Main");
  std::string dex2_name = GetTestDexFileName("MyClass");
  std::unique_ptr<ClassLoaderCompilationContext> context =
      ClassLoaderCompilationContext::Create("PCL[" + dex1_name + ":" + dex2_name + "]");
  ASSERT_TRUE(context->OpenDexFiles(InstructionSet::kArm, ""));
