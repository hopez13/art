...
void InstrumentationInstallStack(Thread* thread, void* arg)
  ...
  struct InstallStackVisitor final : public StackVisitor {
    InstallStackVisitor(Thread* thread_in, Context* context, uintptr_t instrumentation_exit_pc, uint64_t force_deopt_id)
...
void Instrumentation::UpdateStubs() {
  ...
  if (requested_level > InstrumentationLevel::kInstrumentNothing) {
    ...
    if (empty) {
      ...
      // Check that there are no other deoptimizing forces. Do it here so we only need to lock thread_list_lock once.
