...
bool InstrumentationStackPopper::PopFramesTo(uintptr_t stack_pointer,
  ...
  if (pop_until_ != 0u) {
    while (i != stack->end() && i->first >= pop_until_) {
      i++;
    }
  }
  ...
  struct InstallStackVisitor final : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (return_pc == instrumentation_exit_pc_) {
        const auto& pair =
            instrumentation_stack_->find(reinterpret_cast<uintptr_t>(GetReturnPcAddr()));
        ...
        instrumentation_stack_->insert(
            {reinterpret_cast<uintptr_t>(GetReturnPcAddr()), instrumentation_frame});
...
static void InstrumentationRestoreStack(Thread* thread, void* arg)
  ...
  struct RestoreStackVisitor final : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      const auto& pair = instrumentation_stack_->find(
          reinterpret_cast<uintptr_t>(GetReturnPcAddr()));
