...
struct RuntimeMethodShortyVisitor : public StackVisitor {
  RuntimeMethodShortyVisitor(Thread* thread)
  ...
  bool VisitFrame() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (m != nullptr && !m->IsRuntimeMethod()) {
      // The first java method. If it's an invoke, use its shorty.
      // Otherwise, just stop stack walking.
      const DexFile::CodeItem* code_item = m->GetCodeItem();
      const Instruction* instr = Instruction::At(&code_item->insns_[GetDexPc()]);
      if (instr->IsInvoke()) {
        uint32_t method_idx;
        if (instr->Opcode() == Instruction::INVOKE_POLYMORPHIC_RANGE) {
          method_idx = instr->VRegB_4rcc();
        } else {
          method_idx = instr->IsRangeInvoke() ?
                       instr->VRegB_3rc() : instr->VRegB_35c();
        }
        shorty = m->GetDexFile()->GetMethodShorty(method_idx)[0];
      }
      return false;
    }
    return true;
