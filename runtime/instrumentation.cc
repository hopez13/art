...
void Instrumentation::MethodEnterEventImpl(Thread* thread,
  ...
  DCHECK(method == nullptr || !method->IsRuntimeMethod());
...
TwoWordReturn Instrumentation::PopInstrumentationStackFrame(Thread* self,
  ...
  *return_pc = instrumentation_frame.return_pc_;
  ...
  if (method->IsRuntimeMethod()) {
    // We don't keep track of whether a runtime method returns a reference or
    // not. But since there is no suspension point below, we don't need to reassign
    // return value.
    return_shorty = 'V';
  } else {
  ...
  if (deoptimize && Runtime::Current()->IsAsyncDeoptimizeable(*return_pc)) {
    ...
    if (method->IsRuntimeMethod()) {
      ...
      if (method == Runtime::Current()->GetCalleeSaveMethod(
        ...
        method_type = DeoptimizationMethodType::kKeepDexPc;
