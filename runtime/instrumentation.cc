...
void Instrumentation::InstallStubsForMethod(ArtMethod* method) {
  ...
  if (uninstall) {
    if ((forced_interpret_only_ || IsDeoptimized(method)) && !method->IsNative()) {
      ...
      new_quick_code = GetCodeFor(method, kRuntimePointerSize);
    } else {
...
bool Instrumentation::IsDeoptimizedMethod(ArtMethod* method) const {
...
void Instrumentation::Undeoptimize(ArtMethod* method) {
  ...
  if (!interpreter_stubs_installed_) {
    ...
    if (method->IsStatic() && !method->IsConstructor() &&
      ...
      const void* quick_code = GetCodeFor(method, kRuntimePointerSize);
...
bool Instrumentation::IsDeoptimized(ArtMethod* method) const {
...
const void* Instrumentation::GetCodeFor(ArtMethod* method, PointerSize pointer_size) const {
  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
  if (LIKELY(!instrumentation_stubs_installed_)) {
  ...
  } else if (IsDeoptimized(method)) {
    return GetQuickToInterpreterBridge();
  }
  jit::Jit* jit = Runtime::Current()->GetJit();
  if (!method->IsNative() && jit != nullptr) {
    const void* ep = jit->GetCodeCache()->GetCompiledEntrypoint(method);
    if (ep != nullptr) {
      return ep;
    }
  }
