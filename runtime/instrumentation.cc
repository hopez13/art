...
void Instrumentation::ConfigureStubs(const char* key,
  ...
  UpdateStubs(try_switch_runtime);
}
...
void Instrumentation::EnableEntryExitHooks(const char* key) {
  ...
  ConfigureStubs(
      key, InstrumentationLevel::kInstrumentWithEntryExitHooks, /*try_switch_runtime=*/false);
}
...
void Instrumentation::UpdateStubs(bool try_switch_runtime) {
...
void Instrumentation::DisableDeoptimization(const char* key, bool try_switch_runtime) {
  // Remove any instrumentation support added for deoptimization.
  ConfigureStubs(key, InstrumentationLevel::kInstrumentNothing, try_switch_runtime);
  Locks::mutator_lock_->AssertExclusiveHeld(Thread::Current());
...
void Instrumentation::DeoptimizeEverything(const char* key) {
  ConfigureStubs(
      key, InstrumentationLevel::kInstrumentWithInterpreter, /*try_switch_runtime=*/false);
}

void Instrumentation::UndeoptimizeEverything(const char* key) {
  CHECK(InterpreterStubsInstalled());
  ConfigureStubs(key, InstrumentationLevel::kInstrumentNothing, /*try_switch_runtime=*/false);
}
...
void Instrumentation::EnableMethodTracing(const char* key,
  ...
  ConfigureStubs(key, level, /*try_switch_runtime=*/false);
...
void Instrumentation::DisableMethodTracing(const char* key) {
  requested_instrumentation_levels_.Overwrite(key, InstrumentationLevel::kInstrumentNothing);
  ConfigureStubs(key, InstrumentationLevel::kInstrumentNothing, /*try_switch_runtime=*/true);
}
