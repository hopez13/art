...
void InstrumentationInstallStack(Thread* thread, void* arg, bool should_deopt)
  ...
  struct InstallStackVisitor final : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (return_pc == instrumentation_exit_pc_) {
        ...
        if (should_deopt_ && method_header != nullptr && method_header->HasShouldDeoptimizeFlag()) {
...
void Instrumentation::DeoptimizeAllThreadFrames() {
  ...
  tl->ForEach([&](Thread* t) {
    ...
    InstrumentThreadStack(t, /* should_deopt= */ true);
  });
...
void Instrumentation::UpdateStubs() {
  ...
  if (requested_level > InstrumentationLevel::kInstrumentNothing) {
    ...
    for (Thread* thread : Runtime::Current()->GetThreadList()->GetList()) {
      InstrumentThreadStack(thread, /* should_deopt= */ false);
    }
  } else {
    interpreter_stubs_installed_ = false;
    entry_exit_stubs_installed_ = false;
