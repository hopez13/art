...
static bool CodeNeedsEntryExitStub(const void* entry_point, ArtMethod* method)
  ...
  // GenericJni trampoline and JITed JNI stubs handle entry / exit hooks. AOT
  // code isn't used in debuggable runtimes.
...
TwoWordReturn Instrumentation::PopInstrumentationStackFrame(Thread* self,
  ...
  *return_pc_addr = instrumentation_frame.return_pc_;
  ...
  if (deoptimize && Runtime::Current()->IsAsyncDeoptimizeable(visitor.caller, *return_pc_addr)) {
    ...
    if (deoptimize) {
      DCHECK(!Runtime::Current()->IsAsyncDeoptimizeable(visitor.caller, *return_pc_addr));
