...
const void* Instrumentation::GetCodeForInvoke(ArtMethod* method) const {
  ...
  if (LIKELY(!instrumentation_stubs_installed_ && !interpreter_stubs_installed_)) {
    ...
    const void* result =
        ...
        ? method->GetProfilingInfo(kRuntimePointerSize)->GetSavedEntryPoint()
        : nullptr;

    CHECK(result == nullptr ||
          code == result ||
          method->IsNative() ||
          runtime->GetClassLinker()->IsQuickToInterpreterBridge(code) ||
          runtime->GetClassLinker()->IsQuickResolutionStub(code) ||
          code == GetQuickInstrumentationEntryPoint())
        << "For normal get code " << method->PrettyMethod() << " expected " << result
        << " or interpreter but got " << code << " (in jit? "
        << Runtime::Current()->GetJit()->GetCodeCache()->ContainsPc(code) << ")";
    return code;
