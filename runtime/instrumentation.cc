...
bool Instrumentation::CodeNeedsEntryExitStub(const void* code) {
  if (!Runtime::Current()->IsJavaDebuggable() || code == nullptr) {
    return true;
  }
...
// used to check if the frame needs to be deoptimized.
// Since we may already have done this previously, we need to push new instrumentation frame before
// existing instrumentation frames.
void InstrumentationInstallStack(Thread* thread, void* arg, bool force_deopt)
  ...
  struct InstallStackVisitor final : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (return_pc == instrumentation_exit_pc_) {
        ...
        if (force_deopt_ && method_header != nullptr && method_header->HasShouldDeoptimizeFlag()) {
...
void Instrumentation::DeoptimizeAllThreadFrames() {
  ...
  tl->ForEach([&](Thread* t) {
    ...
    InstrumentThreadStack(t, /* force_deopt= */ true);
  });
...
void Instrumentation::UpdateStubs() {
  ...
  if (requested_level > InstrumentationLevel::kInstrumentNothing) {
    ...
    for (Thread* thread : Runtime::Current()->GetThreadList()->GetList()) {
      InstrumentThreadStack(thread, /* force_deopt= */ false);
    }
  } else {
    interpreter_stubs_installed_ = false;
    entry_exit_stubs_installed_ = false;
...
void Instrumentation::Deoptimize(ArtMethod* method) {
  ...
  if (!interpreter_stubs_installed_) {
    ...
    MutexLock mu(self, *Locks::thread_list_lock_);
    for (Thread* thread : Runtime::Current()->GetThreadList()->GetList()) {
      // This isn't a strong deopt. We deopt this method if it is still the
...
bool Instrumentation::ShouldDeoptimizeMethod(Thread* self, const NthCallerVisitor& visitor) {
  ...
  if (header != nullptr && header->HasShouldDeoptimizeFlag()) {
    should_deoptimize_frame = visitor.GetShouldDeoptimizeFlag();
  }
  return (visitor.caller != nullptr) &&
         (interpreter_stubs_installed_ || IsDeoptimized(visitor.caller) ||
          ...
          Dbg::IsForcedInterpreterNeededForUpcall(self, visitor.caller) || should_deoptimize_frame);
}
