...
InstrumentationStackPopper::~InstrumentationStackPopper() {
  std::map<uint8_t*, instrumentation::InstrumentationStackFrame>* stack =
...
bool InstrumentationStackPopper::PopFramesTo(uint8_t* stack_pointer,
  ...
  auto i = stack->begin();
  // Ignore the entries already popped.
  if (pop_until_ != nullptr) {
    while (i != stack->end() && i->first >= pop_until_) i++;
  }
  ...
  struct InstallStackVisitor final : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (return_pc == instrumentation_exit_pc_) {
        const auto& pair = instrumentation_stack_->find(GetReturnPcAddr());
        CHECK(pair != instrumentation_stack_->end());
        const InstrumentationStackFrame& frame = pair->second;
    ...
    std::map<uint8_t*, InstrumentationStackFrame>* const instrumentation_stack_;
  ...
  if (instrumentation->ShouldNotifyMethodEnterExitEvents()) {
    ...
    for (auto isi = thread->GetInstrumentationStack()->rbegin(),
      ...
      if (!isi->second.interpreter_entry_ && !isi->second.method_->IsRuntimeMethod()) {
        instrumentation->MethodEnterEvent(thread, isi->second.this_object_, isi->second.method_, dex_pc);
      }
    }
  }
  thread->VerifyStack();
...
static void InstrumentationRestoreStack(Thread* thread, void* arg)
  ...
  struct RestoreStackVisitor final : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      const auto& pair = instrumentation_stack_->find(GetReturnPcAddr());
      if (pair != instrumentation_stack_->end()) {
        const InstrumentationStackFrame& instrumentation_frame = pair->second;
        if (kVerboseInstrumentation) {
    ...
    const uintptr_t instrumentation_exit_pc_;
    Instrumentation* const instrumentation_;
    std::map<uint8_t*, instrumentation::InstrumentationStackFrame>* const instrumentation_stack_;
  ...
  std::map<uint8_t*, instrumentation::InstrumentationStackFrame>* stack = thread->GetInstrumentationStack();
  if (stack->size() > 0) {
    Instrumentation* instrumentation = reinterpret_cast<Instrumentation*>(arg);
    uintptr_t instrumentation_exit_pc =
    ...
    if (empty) {
      ...
      runtime->GetThreadList()->ForEach([&](Thread* t) {
        no_remaining_deopts =
            ...
            std::all_of(t->GetInstrumentationStack()->begin(),
                        t->GetInstrumentationStack()->end(),
...
TwoWordReturn Instrumentation::PopInstrumentationStackFrame(Thread* self,
  ...
  auto it = stack->find(reinterpret_cast<uint8_t*>(return_pc_addr));
  CHECK(it != stack->end());
...
uintptr_t Instrumentation::PopFramesForDeoptimization(Thread* self, uint8_t* pop_until) const {
  std::map<uint8_t*, instrumentation::InstrumentationStackFrame>* stack =
      self->GetInstrumentationStack();
