...
TEST_F(ParsedOptionsTest, ParsedOptionsDefaultEvacuateLivePercentThreshold) {
  RuntimeOptions options;
  ...
  XGcOption xgc = map.GetOrDefault(Opt::GcOption);
  ASSERT_EQ(kDefaultEvacuateLivePercentThreshold, xgc.evacuate_live_percent_threshold_);
}
