...
TEST_F(MemMapTest, ReplaceMapping_SameSize) {
  ...
  ASSERT_TRUE(MemMap::ReplaceMapping(&dest, &source, &error_msg)) << error_msg;

  ASSERT_FALSE(IsAddressMapped(source_addr));
  ASSERT_TRUE(IsAddressMapped(dest_addr));
  ASSERT_TRUE(source == nullptr);
  ASSERT_FALSE(dest == nullptr);
...
TEST_F(MemMapTest, ReplaceMapping_MakeLarger) {
  ...
  MemMap* dest = MemMap::MapAnonymous("MapAnonymousEmpty-atomic-replace-dest",
                                      nullptr,
                                      5 * kPageSize,  // Need to make it larger initially so we know
                                                      // there won't be mappings in the way we we
                                                      // move source.
                                      PROT_READ,
                                      false,
                                      false,
...
TEST_F(MemMapTest, ReplaceMapping_FailureOverlap) {
  ...
  MemMap* dest = MemMap::MapAnonymous("MapAnonymousEmpty-atomic-replace-dest",
                                      nullptr,
                                      3 * kPageSize,  // Need to make it larger initially so we know
                                                      // there won't be mappings in the way we we
                                                      // move source.
                                      PROT_READ | PROT_WRITE,
                                      false,
  ...
  ASSERT_FALSE(MemMap::ReplaceMapping(&dest, &source, &error_msg)) << error_msg;

  ASSERT_TRUE(source == orig_source);
  ASSERT_TRUE(dest == orig_dest);
  ASSERT_TRUE(IsAddressMapped(source_addr));
  ASSERT_TRUE(IsAddressMapped(dest_addr));

  ASSERT_EQ(memcmp(source->Begin(), data.data(), dest->Size()), 0);
