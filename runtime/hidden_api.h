...
class MemberSignature {
 ...
 public:
  inline explicit MemberSignature(ArtField* field) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  inline explicit MemberSignature(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  inline void WarnAboutAccess(AccessMethod access_method, HiddenApiAccessFlags::ApiList list) {
    LOG(WARNING) << "Accessing hidden " << GetTypeStr() << " " << Dumpable<MemberSignature>(*this)
                 << " (" << list << ", " << access_method << ")";
  }

  inline void LogAccessToEventLog(AccessMethod access_method, Action action_taken) {
