...
// framework or not. Because different users of this function determine this
// in a different way, `fn_caller_in_framework(self)` is called and should
// return true if the caller is located in the framework.
// This function might print warnings into the log if the member is hidden.
template<typename T>
inline bool ShouldBlockAccessToMember(T* member,
                                      Thread* self,
                                      std::function<bool(Thread*)> fn_caller_in_framework,
  ...
  // Member is hidden. Invoke `fn_caller_in_framework` and find the origin of the access.
  // This can be *very* expensive. Save it for last.
  if (fn_caller_in_framework(self)) {
    // Caller in the framework. Exit.
    return false;
  }

  // Member is hidden and caller is not in the framework.
...
inline bool IsCallerInFramework(ObjPtr<mirror::ClassLoader> caller_class_loader,
  ...
  if (caller_class_loader.IsNull()) {
    ...
    return caller_dex_file != nullptr && caller_dex_file->IsFrameworkDexFile();
  }
}

inline bool IsCallerInFramework(ObjPtr<mirror::Class> caller)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  return !caller.IsNull() && IsCallerInFramework(caller->GetClassLoader(), caller->GetDexCache());
}
...
inline bool ShouldBlockAccessToMember(T* member,
                                      ...
                                      AccessMethod access_method)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  bool caller_in_framework = IsCallerInFramework(caller_class_loader, caller_dex_cache);
  return ShouldBlockAccessToMember(member,
                                   /* thread */ nullptr,
                                   [caller_in_framework] (Thread*) { return caller_in_framework; },
                                   access_method);
}
