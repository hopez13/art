...
inline bool ShouldBlockAccessToMember(T* member,
                                      ...
                                      AccessMethod access_method = kLinking)
    REQUIRES_SHARED(Locks::mutator_lock_) {
...
// Returns true if access to member with `member_flags` should be denied to
// a caller loaded with `caller_class_loader`.
// Note that this function will not print any warnings.
inline bool WouldBlockAccessToMember(uint32_t member_flags,
                                     ObjPtr<mirror::ClassLoader> caller_class_loader) {
  bool caller_in_boot = (caller_class_loader.IsNull());
  return !caller_in_boot &&
         Runtime::Current()->AreHiddenApiChecksEnabled() &&
         GetMemberAction(member_flags) == kDeny;
}
