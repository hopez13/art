...
struct AccessContext {
 public:
  AccessContext(ObjPtr<mirror::Class> klass, bool trust_if_null)
      : is_trusted_(GetIsTrusted(klass, trust_if_null)) {}
 ...
 private:
  ...
  static bool GetIsTrusted(ObjPtr<mirror::Class> klass, bool trust_if_class_null)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    if (klass.IsNull()) {
      // Class is unknown. Return the default value.
      return trust_if_class_null;
    }
...
ALWAYS_INLINE inline uint32_t CreateRuntimeFlags(uint32_t dex_hiddenapi_flags) {
  ...
  ApiList api_list = static_cast<hiddenapi::ApiList>(hiddenapi_flags);
