...
struct SubtypeCheck {
  ...
  static SubtypeCheckInfo::State ForceUninitialize(KlassT& klass)
    ...
    SubtypeCheckBits io_uninitialized = SubtypeCheckBits{};  // NOLINT
 ...
 private:
  ...
  static SubtypeCheckInfo InitializeOrAssign(KlassT& klass, bool assign)
    ...
    if (UNLIKELY(!klass->HasSuperClass())) {
      ...
      const SubtypeCheckInfo io = GetSubtypeCheckInfo(klass);
      if (io.GetState() != SubtypeCheckInfo::kUninitialized) {
        return io;  // No change needed.
      }

      const SubtypeCheckInfo new_io = io.CreateRoot();
      SetSubtypeCheckInfo(klass, new_io);
    ...
    // Read.
    SubtypeCheckInfo io = GetSubtypeCheckInfo(klass);
    SubtypeCheckInfo parent_io = GetSubtypeCheckInfo(parent_klass);

    // Modify.
    const SubtypeCheckInfo::State io_state = io.GetState();
    ...
    if (io_state == SubtypeCheckInfo::kUninitialized ||
      ...
      io = parent_io.CreateChild(assign);  // Note: Parent could be mutated.
    } else {
    ...
    // Write.
    SetSubtypeCheckInfo(klass, io);                     // self
    SetSubtypeCheckInfo(parent_klass, parent_io);       // parent
  ...
  static SubtypeCheckInfo GetSubtypeCheckInfo(const KlassT& klass)
    ...
    const SubtypeCheckInfo current =
        SubtypeCheckInfo::Create(current_ios.instance_of_, depth);
    return current;
  }
  ...
  static void SetSubtypeCheckInfo(KlassT& klass,
        ...
        REQUIRES(Locks::subtype_check_lock_)
        REQUIRES_SHARED(Locks::mutator_lock_) {
    SubtypeCheckBits new_bits = new_instanceof.GetSubtypeCheckBits();
