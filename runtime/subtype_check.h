...
// This is the base class using the curiously-recurring template pattern (where in a base class has
// the subclass as a template parameter) to enable testability without losing performance. The real
// class to be used in ART is "SubtypeCheck" (see below).
...
struct SubtypeCheck {
  ...
  static SubtypeCheckInfo::State EnsureAssigned(KlassT& klass)
      REQUIRES(Locks::subtype_check_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    SubtypeCheckInfo io = InitializeOrAssign(klass, /*assign*/true);
    return io.GetState();
  }
  ...
  static SubtypeCheckInfo::State ForceUninitialize(KlassT& klass)
    ...
    SubtypeCheckBits io_uninitialized = SubtypeCheckBits{};
    WriteSubtypeCheckBits(klass, io_uninitialized);
  ...
  static SubtypeCheckInfo::Result IsSubtypeOf(const KlassT& source, const KlassT& target)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    SubtypeCheckInfo io = GetSubtypeCheckInfo(source);
    SubtypeCheckInfo target_io = GetSubtypeCheckInfo(target);
 ...
 private:
  ...
  static SubtypeCheckInfo InitializeOrAssign(KlassT& klass, bool assign)
    ...
    if (UNLIKELY(!klass->HasSuperClass())) {
      ...
      SubtypeCheckInfo io = Update(klass, func);
    ...
    const std::pair<SubtypeCheckInfo, SubtypeCheckInfo> io_and_parent =
        UpdateSelfAndParent(klass, func);
  ...
  template <typename T>
  static SubtypeCheckInfo Update(KlassT& klass, T func)
      REQUIRES(Locks::subtype_check_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    // Read.
    const SubtypeCheckInfo current = GetSubtypeCheckInfo(klass);
    // Modify.
    const SubtypeCheckInfo updated = func(current);
    // Write.
    SetSubtypeCheckInfo(klass, updated);
    // Return written copy.
    return updated;
  }
  ...
  // Returns the return value of calling func.
  template <typename T>
  static std::pair<SubtypeCheckInfo, SubtypeCheckInfo> UpdateSelfAndParent(KlassT& klass, T func)
        REQUIRES(Locks::subtype_check_lock_)
        REQUIRES_SHARED(Locks::mutator_lock_) {
    DCHECK(klass->HasSuperClass());
    ...
    const SubtypeCheckInfo current = GetSubtypeCheckInfo(klass);
    KlassT parent_klass = GetParentClass(klass);
    const SubtypeCheckInfo current_parent = GetSubtypeCheckInfo(parent_klass);

    // Modify.
    const std::pair<SubtypeCheckInfo, SubtypeCheckInfo> updated_pair = func(current, current_parent);

    // Write.
    SetSubtypeCheckInfo(klass, updated_pair.first);               // self
    SetSubtypeCheckInfo(parent_klass, updated_pair.second);       // parent
  ...
  template <typename T>
  static uint32_t AsUint32Copy(T value) {
  ...
  static void WriteSubtypeCheckBits(const KlassT& klass, const SubtypeCheckBits& new_bits)
    ...
    while (true) {
      ...
      {
        SubtypeCheckBits old_bits = old_value.instance_of_;
      ...
      new_value = old_value;
      new_value.instance_of_ = new_bits;
  ...
  static SubtypeCheckInfo GetSubtypeCheckInfo(const KlassT& klass)
        REQUIRES_SHARED(Locks::mutator_lock_) {
    SubtypeCheckBitsAndStatus current_ios = ReadField(klass);
  ...
  static void SetSubtypeCheckInfo(KlassT& klass,
                                  const SubtypeCheckInfo& new_instanceof)
        REQUIRES(Locks::subtype_check_lock_)
        REQUIRES_SHARED(Locks::mutator_lock_) {
    SubtypeCheckBits new_bits = new_instanceof.Slice();
    WriteSubtypeCheckBits(klass, new_bits);
  }

  friend std::ostream& operator<<(std::ostream& os, const SubtypeCheck& tree)
      REQUIRES_SHARED(Locks::mutator_lock_) {
