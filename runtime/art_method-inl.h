...
template <> struct ShortyTraits<'L'> {
  ...
  static Type Get(const JValue& value) REQUIRES_SHARED(Locks::mutator_lock_) { return value.GetL(); }
  static constexpr size_t kVRegCount = 1u;
...
template <char... ArgType>
auto MaterializeVRegs(typename ShortyTraits<ArgType>::Type... args)
    REQUIRES_SHARED(Locks::mutator_lock_) {
