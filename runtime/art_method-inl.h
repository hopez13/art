...
void ArtMethod::VisitArrayRoots(RootVisitorType& visitor,
  ...
  const size_t method_alignment = ArtMethod::Alignment(kRuntimePointerSize);
  const size_t method_size = ArtMethod::Size(kRuntimePointerSize);
  ...
  while (idx < end_idx) {
    ...
    visitor.VisitRoot(method.declaring_class_.AddressWithoutBarrier());
  }
}
