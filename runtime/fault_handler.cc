...
void FaultManager::AddGeneratedCodeRange(const void* start, size_t size) {
  ...
  // memory visibility for an arbitrary chain of reference stores and loads that led
...
void FaultManager::RemoveGeneratedCodeRange(const void* start, size_t size) {
  ...
  GeneratedCodeRange* range = nullptr;
  {
    ...
    if (range != nullptr) {
      // We can use a relaxed store as we're not publishing any newly written
      // memory to potential reader threads. Whether they see the removed node
      // or not is unimportant as we should not execute that code anymore.
      // We're keeping the `next` link of the removed node, so that concurrent
      // walk can use it to reach remaining retained nodes, if any.
  ...
  if (runtime->IsStarted() && runtime->GetThreadList() != nullptr) {
    ...
    // checkpoint also ensures the correct memory visibility, so the thread shall
    // not see the pointer during future walks.
...
bool NullPointerHandler::IsValidReturnPc(ArtMethod** sp, uintptr_t return_pc) {
  ...
  // below the mutator lock, the PC should be in somewhere in ART code and should
