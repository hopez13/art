...
void FaultManager::Init() {
  ...
  int result = art::membarrier(MembarrierCommand::kRegisterPrivateExpeditedSyncCore);
  if (result != 0) {
    LOG(WARNING) << "FaultHandler: MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED_SYNC_CORE failed: "
...
void FaultManager::AddGeneratedCodeRange(const void* start, size_t size) {
  ...
  art::membarrier(MembarrierCommand::kPrivateExpeditedSyncCore);
}
