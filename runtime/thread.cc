...
Thread::Thread(bool daemon)
  ...
  // Start out with a permit, in case the java Thread object was unparked before
  // the native peer was created. Could happen in the case of subclasses
  // invoking unpark() in the constructor, calling unpark() before the thread is started, or
  // unsafe publication of the Thread object. This will cause the first call to park() to
  // spuriously return, which is permitted by the contract.
  tls32_.park_state_.store(0, std::memory_order_relaxed);
