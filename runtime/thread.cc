...
void Thread::SetNativePriority(int newPriority) {
  CHECK_EQ(PaletteSchedSetPriority(GetTid(), newPriority), PaletteStatus::kOkay);
}

int Thread::GetNativePriority() {
  int priority = 0;
  CHECK_EQ(PaletteSchedGetPriority(Thread::Current()->GetTid(), &priority), PaletteStatus::kOkay);
  return priority;
}
