...
static ObjPtr<mirror::StackFrameInfo> InitStackFrameInfo(
  ...
  CHECK(method_name != nullptr);
  auto method_name_object(hs.NewHandle(mirror::String::AllocFromModifiedUtf8(
    soa.Self(), method_name)));

  dex::ProtoIndex proto_idx = method->GetDexFile()->GetIndexForProtoId(
    interface_method->GetPrototype());
