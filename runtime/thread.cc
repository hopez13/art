 ...
 public:
  BuildDeepLimitedInternalStackTraceVisitor(Thread* self,
    ...
    : StackVisitor(self, nullptr, StackVisitor::StackWalkKind::kIncludeInlinedFrames),
      self_(self),
      desired_depth_((int32_t)desired_depth),
  ...
  bool VisitFrame() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    return count_ < (int32_t)desired_depth_;
  }
  ...
  ObjPtr<mirror::ObjectArray<mirror::Object>> CreateInternalStackTrace()
    ...
    Handle<mirror::ObjectArray<mirror::Object>> trace(
        hs.NewHandle(
            mirror::ObjectArray<mirror::Object>::Alloc(hs.Self(), array_class, (int32_t)desired_depth_ + 1)));
    if (trace.Get() == nullptr) {
      self_->AssertPendingOOMException();
      return nullptr;
    }
    ObjPtr<mirror::PointerArray> methods_and_pcs = class_linker->AllocPointerArray(self_, (size_t)(desired_depth_ * 2));
    if (methods_and_pcs == nullptr) {
      self_->AssertPendingOOMException();
      return nullptr;
    }
  ...
  void FillInInternalStackTrace(ObjPtr<mirror::ObjectArray<mirror::Object>> dest_traces)
    REQUIRES_SHARED(Locks::mutator_lock_) {
    uint32_t length = (uint32_t)(dest_traces->GetLength() - 1);
    ...
    for (uint32_t i = 0; i < count; i++) {
      ...
      dest_traces->SetWithoutChecks<false, false, kVerifyNone>((int32_t)(i + 1), methods_buffer_[i]->GetDeclaringClass());
    }
  }
...
jobject Thread::CreateInternalStackTrace(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  BuildDeepLimitedInternalStackTraceVisitor build_trace_visitor(
    soa.Self(), (int32_t)desired_depth, methods, dex_pcs);
...
jobject Thread::CreateEmptyInternalStackTrace(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  ObjPtr<mirror::PointerArray> methods_and_pcs = class_linker->AllocPointerArray(soa.Self(), (size_t)depth * 2);
  if (methods_and_pcs == nullptr) {
    soa.Self()->AssertPendingOOMException();
    return nullptr;
  }
  ...
  for (int i = 0; i < depth; i++) {
    methods_and_pcs->SetElementPtrSize<false, true>((uint32_t)i, (ArtMethod *)nullptr, pointer_size);
    methods_and_pcs->SetElementPtrSize<false, true>((uint32_t)(depth + i), dex::kDexNoIndex, pointer_size);
...
void Thread::ResetInternalStackTrace(const ScopedObjectAccessAlreadyRunnable& soa, jobject dest) {
  ...
  for (int i = 0; i < length; i++) {
    dest_method_traces->SetElementPtrSize<false, true>((uint32_t)i, (ArtMethod *)nullptr, pointer_size);
    dest_method_traces->SetElementPtrSize<false, true>((uint32_t)(length + i), dex::kDexNoIndex, pointer_size);
...
jobject Thread::CloneInternalStackTrace(const ScopedObjectAccessAlreadyRunnable& soa, jobject src) {
  ...
  PointerSize pointer_size = Runtime::Current()->GetClassLinker()->GetImagePointerSize();
  for (int i = 0; i < length; i++) {
    dest_method_traces->SetElementPtrSize<false, true>((uint32_t)i,
      src_method_traces->GetElementPtrSize<ArtMethod *>((uint32_t)i, pointer_size), pointer_size);
    dest_method_traces->SetElementPtrSize<false, true>((uint32_t)(length + i),
      src_method_traces->GetElementPtrSize<uint64_t>((uint32_t)(length + i), pointer_size), pointer_size);
...
static int32_t GetInternalStackTraceDepth(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  for (int32_t i = 0; i < length; i++) {
    ArtMethod*  method = method_traces->GetElementPtrSize<ArtMethod*>((uint32_t)i, pointer_size);
    if (method == nullptr) {
      break;
    }
    depth++;
