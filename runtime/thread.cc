...
mirror::Object* Thread::GetPeerFromOtherThread() {
  ...
  if (kIsDebugBuild && !Locks::mutator_lock_->IsExclusiveHeld(self)) {
    Locks::thread_list_lock_->AssertNotHeld(self);
  }
  // Some JVMTI code may or may not hold thread_list_lock_, but if it does, it should hold the
  ...
  if (kIsDebugBuild && Locks::thread_list_lock_->IsExclusiveHeld(self)) {
    // We shouldn't need to run the flip function, and can't safely do so.
