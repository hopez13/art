 ...
 private:
  ...
  inline void VisitQuickFrameWithVregCallback() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (!m->IsNative() && !m->IsRuntimeMethod() && (!m->IsProxyMethod() || m->IsConstructor())) {
      ...
      T vreg_info(m, code_info, encoding, map);
      ...
      for (size_t i = 0; i < number_of_bits; ++i) {
        if (map.GetStackMaskBit(encoding.stack_map_encoding, i)) {
          ...
          if (ref != nullptr) {
            mirror::Object* new_ref = ref;
            visitor_(&new_ref, vreg_info.StackVReg(i), this);
            if (ref != new_ref) {
      ...
      for (size_t i = 0; i < BitSizeOf<uint32_t>(); ++i) {
        if (register_mask & (1 << i)) {
          ...
          if (*ref_addr != nullptr) {
            visitor_(ref_addr, vreg_info.RegisterVReg(i), this);
          }
        }
      }
    }
  ...
  void VisitQuickFrameNonPrecise() REQUIRES_SHARED(Locks::mutator_lock_) {
    struct UndefinedVRegInfo {
      ...
      size_t StackVReg(size_t stack_slot ATTRIBUTE_UNUSED) {
        return -1;
      }

      size_t RegisterVReg(size_t physical_reg ATTRIBUTE_UNUSED) {
        return -1;
      }
    };
  ...
  void VisitQuickFramePrecise() REQUIRES_SHARED(Locks::mutator_lock_) {
    struct StackMapVRegInfo {
      ...
      size_t FindWithType(const size_t index, const DexRegisterLocation::Kind kind) {
      ...
      size_t StackVReg(size_t stack_slot) {
        return FindWithType(stack_slot, DexRegisterLocation::Kind::kInStack);
      }

      size_t RegisterVReg(size_t physical_reg) {
        return FindWithType(physical_reg, DexRegisterLocation::Kind::kInRegister);
      }
