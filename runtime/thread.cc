...
static const size_t kUsePthreadGuardRegion = []() constexpr {
...
// Given a user-supplied stack size request in stack_size, return the stack sizes we want to
// pass into the pthreads library.  Input of zero means "use your best judgment, ART", and a
// return value of zero means to have pthreads just use its own default stack size. Return
// needed stack and guard sizes.
static std::pair<size_t, size_t> FixStackSize(size_t stack_size) {
  ...
  if (stack_size && stack_size < PTHREAD_STACK_MIN) {
...
#if defined(__BIONIC__)
  pthread_prevent_stack_reuse_np();
#endif
