...
bool Thread::RequestSynchronousCheckpoint(Closure* function, ThreadState wait_state) {
  ...
  //       certain situations.
  {
    ...
    if (!is_suspended) {
      CHECK(Runtime::Current()->GetThreadList()->WaitForSuspendBarrier(&wrapped_barrier.barrier_));
    }
...
void Thread::RunFlipFunction(Thread* self) {
  ...
  constexpr uint32_t kFlagToClear = enum_cast<uint32_t>(ThreadFlag::kRunningFlipFunction);
  StateAndFlags old_state_and_flags(
      tls32_.state_and_flags.fetch_and(~kFlagToClear, std::memory_order_release));
