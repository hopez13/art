...
template <>
NO_INLINE uint8_t* Thread::FindStackTop<StackType::kNative>() {
...
bool Thread::Init(ThreadList* thread_list, JavaVMExt* java_vm, JNIEnvExt* jni_env_ext) {
  ...
  if (!InitStack<StackType::kNative>(reinterpret_cast<uint8_t*>(read_stack_base),
                                     read_stack_size,
                                     read_guard_size)) {
    return false;
  }
...
bool Thread::InitStack(uint8_t* read_stack_base, size_t read_stack_size, size_t read_guard_size) {
  ...
  if constexpr (stack_type == StackType::kNative) {
    stack_type_str = "Native";
  } else if constexpr (stack_type == StackType::kSimulated) {
    stack_type_str = "Simulated";
  }
...
void Thread::DumpState(std::ostream& os, const Thread* thread, pid_t tid) {
  ...
  if (thread != nullptr) {
    ...
    os << "  | stack=" << reinterpret_cast<void*>(thread->GetStackBegin<StackType::kNative>())
        << "-" << reinterpret_cast<void*>(thread->GetStackEnd<StackType::kNative>())
        << " stackSize=" << PrettySize(thread->GetStackSize<StackType::kNative>()) << "\n";
...
bool Thread::IsRawObjOnStack(uint8_t* raw_obj) const {
  return (static_cast<size_t>(raw_obj - GetStackBegin<stack_type>()) < GetStackSize<stack_type>());
}
...
bool Thread::IsJniTransitionReference(jobject obj) const {
  ...
  if (IsRawObjOnStack<kQuickStackType>(raw_obj)) {
