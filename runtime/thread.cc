...
jobjectArray Thread::CreateAnnotatedStackTrace(const ScopedObjectAccessAlreadyRunnable& soa) const {
  ...
  ObjArray* array = ObjArray::Alloc(soa.Self(), aste_array_class, length);
  if (array == nullptr) {
    return nullptr;
  }
  ...
  MutableHandle<ObjArray> handle2(hs.NewHandle<ObjArray>(nullptr));
  for (size_t i = 0; i != length; ++i) {
    ...
    stack_trace_element_field->SetObject<false>(
        handle.Get(), soa.Decode<mirror::Object>(dumper.stack_trace_elements[i].get()));

    // Create locked-on array.
    if (!dumper.lock_objects[i].empty()) {
      handle2.Assign(ObjArray::Alloc(soa.Self(),
                                     h_o_array_class.Get(),
    ...
    if (i == 0) {
      if (dumper.block_jobject != nullptr) {
        blocked_on_field->SetObject<false>(
            handle.Get(), soa.Decode<mirror::Object>(dumper.block_jobject.get()));
      }
    }
