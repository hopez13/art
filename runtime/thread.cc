...
bool Thread::InitStackHwm() {
  ...
  // The minimum stack size we can cope with is the protected region size +
  // stack overflow check region + some memory for normal stack usage.
  //
  // The protected region size is located at the very beginning of the stack
  // region. It should start at a page-aligned address and its size should be
  // a multiple of page sizes. Typically, it is one page in size, however this
  // varies in some configurations.
  //
  // The overflow reserved bytes is size of the stack overflow check region,
  // located right after the protected region, so also starts at a page-aligned
  // address. The size is discretionary. Typically it is 8K, but this varies
  // in some configurations.
  //
  // The rest of the stack memory is available for normal stack usage. It is
  // located right after the stack overflow check region, so its starting
  // address isn't necessarily page-aligned. The size of the region is
  // discretionary, however should be chosen in a way that the overall stack
  // size is a multiple of page sizes. Historically, it is chosen to be at
  // least 4 KB.
  //
