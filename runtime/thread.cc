...
Thread::Thread(bool daemon)
  ...
  tlsPtr_.instrumentation_stack = new std::map<uint8_t*, instrumentation::InstrumentationStackFrame>;
...
void Thread::VisitRoots(RootVisitor* visitor) {
  ...
  for (auto& it : *GetInstrumentationStack()) {
    visitor->VisitRootIfNonNull(&it.second.this_object_, RootInfo(kRootVMInternal, thread_id));
  }
}
