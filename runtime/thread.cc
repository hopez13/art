...
// Parking for 0ns of relative time means an untimed park
  ...
  if (tls32_.park_state_.fetch_add(1, std::memory_order_relaxed) == 1) {
    ...
    if (result == -1) {
      switch(errno) {
        ...
        case ETIMEDOUT:
        case EINTR: break;
        default: LOG(FATAL) << "Failed to park: " << std::strerror(errno);
      }
    }
...
void Thread::Unpark() {
  ...
  if (tls32_.park_state_.exchange(0, std::memory_order_relaxed) == 2) {
    int result = futex(tls32_.park_state_.Address(),
                       ...
                       0);
    if (result == -1) {
      LOG(FATAL) << "Failed to unpark: " << std::strerror(errno);
    }
  }
#else
  UNIMPLEMENTED(WARNING);
