...
bool Thread::InitStackHwm() {
  ...
  if (read_stack_size <= min_stack) {
    ...
    LogMessage::LogLineLowStack(__PRETTY_FUNCTION__,
                                __LINE__,
                                ::android::base::ERROR,
...
void Thread::ThrowOutOfMemoryError(const char* msg) {
  ...
  if (!tls32_.throwing_OutOfMemoryError) {
    ...
    Dump(ART_LOG_STREAM(WARNING));  // The pre-allocated OOME has no stack, so help out and log one.
    SetException(Runtime::Current()->GetPreAllocatedOutOfMemoryError());
  }
}
    ...
    if (klass != nullptr) {
      if (kVerifyImageObjectsMarked) {
        ...
        if (space != nullptr && space->IsImageSpace()) {
          ...
          if (failed) {
            GetThread()->Dump(ART_LOG_STREAM(FATAL_WITHOUT_ABORT));
            space->AsImageSpace()->DumpSections(ART_LOG_STREAM(FATAL_WITHOUT_ABORT));
...
void Thread::SetStackEndForStackOverflow() {
  ...
  if (tlsPtr_.stack_end == tlsPtr_.stack_begin) {
    ...
    LOG(ERROR) << "Need to increase kStackOverflowReservedBytes (currently "
               << GetStackOverflowReservedBytes(kRuntimeISA) << ")?";
    DumpStack(ART_LOG_STREAM(ERROR));
