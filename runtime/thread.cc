...
bool Thread::Init(ThreadList* thread_list, JavaVMExt* java_vm, JNIEnvExt* jni_env_ext) {
  ...
  // if (kAlwaysOnProfile && (tls32_.thin_lock_thread_id == ThreadList::kMainThreadId )) {
  if (kAlwaysOnProfile) {
    /*uintptr_t* method_trace_buffer = new uintptr_t[std::max(kMinBufSize, kPerThreadBufSize)]();
    tlsPtr_.method_trace_buffer = method_trace_buffer;
...
Thread::~Thread() {
  ...
  if (tlsPtr_.method_trace_buffer != nullptr) {
    uintptr_t* ptr = tlsPtr_.method_trace_buffer;
    delete[] ptr;
  }
