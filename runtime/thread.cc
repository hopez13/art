...
void Thread::RunCheckpointFunction() {
  // In suspended state we may have a pending flip function that we need to run
  // for the sake of those checkpoints that need to walk the stack.
...
void Thread::TryToRunFlipFunction(Thread* self) {
  while (true) {
    StateAndFlags old_state_and_flags = GetStateAndFlags(std::memory_order_relaxed);
...
void Thread::RunFlipFunction(Thread* self, bool notify) {
  ...
  if (notify) {
    // Clear the `ThreadFlag::kRunningFlipFunction`.
    AtomicClearFlag(ThreadFlag::kRunningFlipFunction, std::memory_order_release);

    // Notify any threads that may wait for completion.
...
void Thread::WaitForFlipFunction(Thread* self) {
  ...
  // Check the flag while holing the mutex so that we do not miss the broadcast.
  MutexLock mu(self, *Locks::thread_suspend_count_lock_);
  ...
  while (old_state_and_flags.IsFlagSet(ThreadFlag::kRunningFlipFunction)) {
    resume_cond_->Wait(self);
    old_state_and_flags = GetStateAndFlags(std::memory_order_acquire);
  }
