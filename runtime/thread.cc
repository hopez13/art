...
void Thread::SweepInterpreterCaches(IsMarkedVisitor* visitor) {
  ...
  Runtime::Current()->GetThreadList()->ForEach([self, visitor](Thread* thread) {
    // We can assert exclusive held as SweepInterpreterCaches() must be
    // always called with that. Having the assertion once before ForEach() complains
    // about shared-held guarantee that SweepInterpreterCache() needs.
