...
int Thread::DumpJavaStack(std::ostream& os, bool check_suspended, bool dump_locks) const {
  ...
  dumper.WalkStack();
  // Returns priority/importance of the thread. Higher numbers get printed first in ANRs.
  return IsJitSensitiveThread() ? 3 : dumper.num_blocked ? 2 : dumper.num_locked ? 1 : 0;
}
