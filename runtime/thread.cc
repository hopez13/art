...
int Thread::Dump(std::ostream& os, bool dump_native_stack, bool force_dump_stack) const {
  DumpState(os);
  return DumpStack(os, dump_native_stack, force_dump_stack);
}

int Thread::Dump(std::ostream& os, unwindstack::AndroidLocalUnwinder& unwinder,
...
int Thread::DumpJavaStack(std::ostream& os, bool check_suspended, bool dump_locks) const {
  ...
  dumper.WalkStack();
  // Return print order of the thread for ANRs (ANRs can be trimmed so print important ones first).
  enum PrintOrder {
    kMain    = -3,  // Always print the main thread first (there might not be one).
    kBlocked = -2,  // Then print all threads that are blocked due to waiting on lock.
    kLocked  = -1,  // Then print all threads that are holding some lock already.
    kDefault = 0,   // Print all other threads which might not be interesting for ANR.
  };
  bool main = IsJitSensitiveThread();  // The main/UI thread is marked with this flag.
  return main ? kMain : dumper.num_blocked ? kBlocked : dumper.num_locked ? kLocked : kDefault;
}

int Thread::DumpStack(std::ostream& os,
