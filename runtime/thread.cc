...
class JniTransitionReferenceVisitor : public StackVisitor {
 public:
  JniTransitionReferenceVisitor(Thread* thread, jobject obj) REQUIRES_SHARED(Locks::mutator_lock_)
...
bool Thread::IsJniTransitionReference(jobject obj) const {
  ...
  if (static_cast<size_t>(raw_obj - tlsPtr_.stack_begin) < tlsPtr_.stack_size) {
    JniTransitionReferenceVisitor</*kPointsToStack=*/ true> visitor(thread, obj);
    visitor.WalkStack();
    return visitor.Found();
  } else {
    JniTransitionReferenceVisitor</*kPointsToStack=*/ false> visitor(thread, obj);
    visitor.WalkStack();
    return visitor.Found();
  }
}
