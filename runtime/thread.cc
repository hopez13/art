...
bool Thread::InitStackHwm() {
  ...
  // The minimum stack size we can cope with is the overflow reserved bytes + the protected
  // region size (one page) + another page size.
  //
  // The overflow reserved bytes is typically 8K if page size is 4K and one page size otherwise.
  //
  // On systems with 4K page size, typically the minimum stack size will be 8+4+4 = 16K.
  // The thread won't be able to do much with this stack: even the GC takes between 8K and 12K.
  size_t min_stack = GetStackOverflowReservedBytes(kRuntimeISA) + gStackOverflowProtectedSize
    + gPageSize;
