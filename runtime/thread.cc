...
class BuildInternalStackTraceVisitor : public StackVisitor {
 public:
  ...
  void AddFrame(ArtMethod* method, uint32_t dex_pc) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    // method (or the class of `this` but that could keep even more class loaders alive).
    // We could search `ClassLinker::class_loaders_` for the `LinearAlloc` that holds the method,
