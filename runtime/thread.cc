...
bool Thread::PassActiveSuspendBarriers(Thread* self) {
  ...
  // flag. The ModifySuspendCount function requires the lock so we prevent a race between setting
  ...
  std::vector<AtomicInteger*> pass_barriers{};
  {
    ...
    for (WrappedSuspend1Barrier* w = tlsPtr_.active_suspend1_barriers;
         w != nullptr; w = w->next_) {
...
bool Thread::RequestSynchronousCheckpoint(Closure* function, ThreadState suspend_state) {
  ...
  // Note: ModifySuspendCount also expects the thread_list_lock to be held in
  //       certain situations.
  while (true) {
    MutexLock mu2(self, *Locks::thread_suspend_count_lock_);

    if (!kUseReadBarrier || GetFlipFunction() == nullptr) {
      ModifySuspendCount(self, +1);
      break;
    }
    sched_yield();
  }
