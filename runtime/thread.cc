...
class BarrierClosure : public Closure {
 public:
  explicit BarrierClosure(Closure* wrapped) : wrapped_(wrapped), barrier_(2) {}
...
void Thread::RequestSynchronousCheckpoint(Closure* function) {
  ...
  for (;;) {
    ...
    //       certain situations.
    {
      ...
      if (GetState() == ThreadState::kRunnable) {
        // We became runnable again. Try another round of the loop.
        continue;
      }

      if (!ModifySuspendCount(self, +1, nullptr, false)) {
        // TODO: Uh-oh. Flip functions something something. Better just retry.
        continue;
      }
    }
