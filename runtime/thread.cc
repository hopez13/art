...
static ObjPtr<mirror::StackFrameInfo> InitStackFrameInfo(
  ...
  CHECK(method_name != nullptr);
  auto method_name_object(hs.NewHandle(mirror::String::AllocFromModifiedUtf8(soa.Self(), method_name)));

  dex::ProtoIndex proto_idx = method->GetDexFile()->GetIndexForProtoId(interface_method->GetPrototype());
  auto method_type_object(hs.NewHandle<mirror::MethodType>(
    class_linker->ResolveMethodType(soa.Self(), proto_idx, interface_method)));
  // If any of the below 3 pointers are null, we may get an OOM exception to
  // allocate the objects or we have a bug to lookup such objects.
  // For now, we return null here to indicate an invalid StackFrameInfo object.
  DCHECK(declaring_class_object != nullptr);
  DCHECK(method_name_object != nullptr);
  DCHECK(method_type_object != nullptr);
  if (declaring_class_object == nullptr ||
      method_name_object == nullptr ||
      method_type_object == nullptr) {
    return nullptr;
  }
...
jint Thread::InternalStackTraceToStackFrameInfoArray(
  ...
  Handle<mirror::Class> sfe_class = hs.NewHandle(class_linker->FindSystemClass(
      soa.Self(), "Ljava/lang/StackFrameInfo;"));
  DCHECK(sfe_class != nullptr);

  #ifdef CHECK_OR_BREAK
    #error CHECK_OR_BREAK has been defined.
  #endif
  #define CHECK_OR_BREAK(x) \
    CHECK(x); \
    if (UNLIKELY(!(x))) break;

  for (uint32_t i = static_cast<uint32_t>(startLevel); i < static_cast<uint32_t>(depth); ++i) {
    if (endBufferIndex >= startBufferIndex + batchSize ||
        endBufferIndex >= bufferSize) {
      break;
    }
    ...
    CHECK_OR_BREAK(frameObject != nullptr);
    CHECK_OR_BREAK(frameObject->InstanceOf(sfe_class.Get()));
    ...
    CHECK_OR_BREAK(frame != nullptr);

    ++endBufferIndex;
  }
  #undef CHECK_OR_BREAK
