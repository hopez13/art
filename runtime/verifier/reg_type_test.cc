...
TEST_F(RegTypeTest, MergeSemiLatticeRef) {
  ...
  auto check = [&](const RegType& in1, const RegType& in2, const RegType& out)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    const RegType& merge_result = in1.SafeMerge(in2, &cache, nullptr);
    EXPECT_EQ(&out, &merge_result)
...
#undef ADD_EDGE
  ...
  for (auto r1 : all) {
    for (auto r2 : all) {
      ...
      std::set<const RegType*> no_in_edge(black);
      for (auto r : black) {
        for (Edge& e : edges) {
          if (&e.from == r) {
            no_in_edge.erase(&e.to);
          }
        }
      }
