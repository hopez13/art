...
bool MethodVerifier::CodeFlowVerifyInstruction(uint32_t* start_guess) {
  ...
  switch (inst->Opcode()) {
        ...
        if (!return_type.IsReferenceTypes()) {
          ...
          if (reg_type.IsUndefined()) {
            ...
            Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "return-object returns a non-reference type "
                                              << reg_type;
          } else if (!return_type.IsAssignableFrom(reg_type, deps_)) {
            if (reg_type.IsUnresolvedTypes() || return_type.IsUnresolvedTypes()) {
              ...
              if (reg_type.IsArrayTypes() && return_type.IsArrayTypes()) {
                return_type.CanAssignArray(reg_type, reg_types_, class_loader_, deps_, &soft_error);
                if (soft_error) {
    ...
    case Instruction::THROW: {
      ...
      if (!reg_types_.JavaLangThrowable(false).IsAssignableFrom(res_type, deps_)) {
        ...
        if (!orig_type.Equals(cast_type) &&
            ...
            (orig_type.IsZero() ||
             orig_type.IsStrictlyAssignableFrom(
                cast_type.Merge(orig_type, &reg_types_, deps_), deps_))) {
...
const RegType& MethodVerifier::ResolveClassAndCheckAccess(uint32_t class_idx) {
  ...
  if (deps_ != nullptr) {
    deps_->AddClassResolution(*dex_file_, class_idx, klass);
  }
      ...
      for (; iterator.HasNext(); iterator.Next()) {
        if (iterator.GetHandlerAddress() == (uint32_t) work_insn_idx_) {
          if (iterator.GetHandlerTypeIndex() == DexFile::kDexNoIndex16) {
            ...
            if (!reg_types_.JavaLangThrowable(false).IsAssignableFrom(exception, deps_)) {
              ...
              common_super = &common_super->Merge(exception, &reg_types_, deps_);
              if (FailOrAbort(this,
...
ArtMethod* MethodVerifier::ResolveMethodAndCheckAccess(
  ...
  if (res_method == nullptr) {
    ...
    } else if (res_type == kVirtualMethodResolution) {
      res_method = klass->FindVirtualMethod(name, signature, pointer_size);
    } else {
      DCHECK_EQ(res_type, kInterfaceMethodResolution);
    ...
    if (res_method != nullptr) {
      ...
      if (res_type != kDirectMethodResolution) {
        if (deps_ != nullptr) {
          // Record that the initial resolution attempt failed.
          deps_->AddMethodResolution(*dex_file_, dex_method_idx, res_type, /* method */ nullptr);
        }
        // Change resolution type to 'direct' and try to resolve again.
        res_type = kDirectMethodResolution;
        res_method = klass->FindDirectMethod(name, signature, pointer_size);
      }
    }
  }
  ...
  if (deps_ != nullptr) {
    deps_->AddMethodResolution(*dex_file_, dex_method_idx, res_type, res_method);
  }
...
ArtMethod* MethodVerifier::VerifyInvocationArgsFromIterator(
  ...
  if (method_type != METHOD_STATIC) {
    ...
    if (method_type != METHOD_INTERFACE && !adjusted_type.IsZero()) {
      ...
      if (!res_method_class->IsAssignableFrom(adjusted_type, deps_)) {
  ...
  if (method_type == METHOD_SUPER) {
    ...
    if (reference_type.GetClass()->IsInterface()) {
      ...
      if (!GetDeclaringClass().HasClass()) {
        ...
        return nullptr;
      } else if (!reference_type.IsStrictlyAssignableFrom(GetDeclaringClass(), deps_)) {
        Fail(VERIFY_ERROR_CLASS_CHANGE)
      ...
      if (!reference_type.IsStrictlyAssignableFrom(GetDeclaringClass(), deps_) ||
...
ArtMethod* MethodVerifier::VerifyInvokeVirtualQuickArgs(const Instruction* inst, bool is_range) {
  ...
  if (!actual_arg_type.IsZero()) {
    ...
    if (!res_method_class.IsAssignableFrom(actual_arg_type, deps_)) {
...
ArtField* MethodVerifier::GetStaticField(int field_idx) {
  ...
  if (deps_ != nullptr) {
    deps_->AddFieldResolution(*dex_file_, field_idx, field);
  }
  if (field == nullptr) {
...
ArtField* MethodVerifier::GetInstanceField(const RegType& obj_type, int field_idx) {
  ...
  if (field == nullptr) {
    ...
    } else if (!field_klass.IsAssignableFrom(obj_type, deps_)) {
...
void MethodVerifier::VerifyISFieldAccess(const Instruction* inst, const RegType& insn_type,
  ...
  if (kAccType == FieldAccessType::kAccPut) {
    ...
    if (is_primitive) {
      ...
      if (!insn_type.IsAssignableFrom(*field_type, deps_)) {
