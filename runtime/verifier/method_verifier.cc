...
void
PcToRegisterLineTable::Init(RegisterTrackingMode mode, InstructionFlags *flags,
  ...
  for (uint32_t i = 0; i < insns_size; i++) {
    ...
    switch (mode) {
      ...
      case kTrackCompilerInterestPoints:
        interesting =
            flags[i].IsCompileTimeInfoPoint() || flags[i].IsBranchTarget();
        break;
...
FailOrAbort(MethodVerifier *verifier, bool condition,
  ...
  if (kIsDebugBuild) {
    ...
    if (LIKELY(gAborting == 0)) {
      ...
      if (!condition) {
        ...
        verifier->Fail(VERIFY_ERROR_BAD_CLASS_HARD) << error_msg
                                                    << work_insn_idx;
        return true;
      }
    }
...
MethodVerifier::FailureKind MethodVerifier::VerifyClass(Thread *self,
  ...
  std::string temp;
  if (super == nullptr
      && strcmp("Ljava/lang/Object;", klass->GetDescriptor(&temp)) != 0) {
    early_failure = true;
    failure_message = " that has no super class";
  } else if (super != nullptr && super->IsFinal()) {
    early_failure = true;
    failure_message =
        " that attempts to sub-class final class " + super->PrettyDescriptor();
  } else if (class_def == nullptr) {
    early_failure = true;
    failure_message =
        " that isn't present in dex file " + dex_file.GetLocation();
  }
  if (early_failure) {
    *error = "Verifier rejected class " + klass->PrettyDescriptor()
        + failure_message;
  ...
  Handle <mirror::ClassLoader>
      class_loader(hs.NewHandle(klass->GetClassLoader()));
...
static MethodVerifier::FailureKind
FailureKindMax(MethodVerifier::FailureKind fk1,
...
MethodVerifier::FailureData MethodVerifier::VerifyMethod(Thread *self,
  ...
  if (verifier.Verify()) {
    ...
    if (verifier.failures_.size() != 0) {
      if (VLOG_IS_ON(verifier)) {
        verifier.DumpFailures(
            VLOG_STREAM(verifier) << "Soft verification failures in "
    ...
    if (method != nullptr) {
      ...
      if ((verifier.encountered_failure_types_
          & VerifyError::VERIFY_ERROR_LOCKING) != 0) {
    ...
    if (UNLIKELY(verifier.have_pending_experimental_failure_)) {
      ...
      if (hard_failure_msg != nullptr) {
        ...
        *hard_failure_msg =
            verifier.failure_messages_[verifier.failure_messages_.size()
                - 1]->str();
      }
...
static bool
HasMonitorEnterInstructions(const DexFile::CodeItem *const code_item) {
...
ArtField *
MethodVerifier::FindAccessedFieldAtDexPc(ArtMethod *m, uint32_t dex_pc) {
  StackHandleScope<2> hs(Thread::Current());
  Handle <mirror::DexCache> dex_cache(hs.NewHandle(m->GetDexCache()));
  Handle <mirror::ClassLoader> class_loader(hs.NewHandle(m->GetClassLoader()));
  MethodVerifier verifier(hs.Self(),
...
ArtField *MethodVerifier::FindAccessedFieldAtDexPc(uint32_t
dex_pc) {
CHECK(code_item_
!= nullptr);  // This only makes sense for methods with code.
...
return
GetQuickFieldAccess(inst, register_line
);
}

ArtMethod *
MethodVerifier::FindInvokedMethodAtDexPc(ArtMethod *m, uint32_t dex_pc) {
  StackHandleScope<2> hs(Thread::Current());
  Handle <mirror::DexCache> dex_cache(hs.NewHandle(m->GetDexCache()));
  Handle <mirror::ClassLoader> class_loader(hs.NewHandle(m->GetClassLoader()));
  MethodVerifier verifier(hs.Self(),
...
ArtMethod *MethodVerifier::FindInvokedMethodAtDexPc(uint32_t
dex_pc) {
CHECK(code_item_
!= nullptr);  // This only makes sense for methods with code.
...
const Instruction *inst = Instruction::At(code_item_->insns_ + dex_pc);
const bool
    is_range = (inst->Opcode() == Instruction::INVOKE_VIRTUAL_RANGE_QUICK);
return
GetQuickInvokedMethod(inst, register_line, is_range,
false);
}
...
bool MethodVerifier::Verify() {
  ...
  bool constructor_by_name =
      instance_constructor_by_name || static_constructor_by_name;
  ...
  if (kIsDebugBuild) {
    ...
    if (access_mod_count > 1) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD)
          << "method has more than one of public/protected/private";
      return false;
    }
  }
  ...
  if (code_item_ == nullptr) {
    ...
    if (kIsDebugBuild) {
      ...
      if (IsConstructor()) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD)
            << "constructors can't be abstract or native";
        return false;
      }
    }
    return true;
  ...
  if (kIsDebugBuild) {
    ...
    if ((method_access_flags_ & (kAccNative | kAccAbstract)) != 0) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD)
          << "non-zero-length code in abstract or native method";
      return false;
    }
    ...
    if ((class_def_.GetJavaAccessFlags() & kAccInterface) != 0) {
      ...
      if (!(IsConstructor() && IsStatic())) {
        if (IsInstanceConstructor()) {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "interfaces may not have non-static constructor";
          return false;
        } else if (method_access_flags_ & kAccFinal) {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "interfaces may not have final methods";
          return false;
        } else {
...
std::ostream &MethodVerifier::Fail(VerifyError error) {
  ...
  std::string location(StringPrintf("%s: [0x%X] ",
                                    dex_file_->PrettyMethod(dex_method_idx_).c_str(),
                                    work_insn_idx_));
  std::ostringstream *failure_message =
      new std::ostringstream(location, std::ostringstream::ate);
...
void MethodVerifier::PrependToLastFailMessage(std::string prepend) {
  ...
  failure_messages_[failure_num - 1] =
      new std::ostringstream(prepend, std::ostringstream::ate);
...
bool MethodVerifier::ScanTryCatchBlocks() {
  ...
  for (uint32_t idx = 0; idx < tries_size; idx++) {
    ...
    if ((start >= end) || (start >= insns_size) || (end > insns_size)) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "bad exception entry: startAddr="
                                        << start
                                        << " endAddr=" << end << " (size="
                                        << insns_size << ")";
      return false;
...
bool MethodVerifier::VerifyInstruction(const Instruction *inst,
  ...
  switch (inst->GetVerifyExtraFlags()) {
    ...
    case Instruction::kVerifyVarArg: {
      ...
      if ((inst->GetVerifyExtraFlags() == Instruction::kVerifyVarArgNonZero
          && v_a == 0) ||
          v_a > Instruction::kMaxVarArgRegs) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "invalid arg count (" << v_a
                                          << ") in "
                                              "non-range invoke";
        return false;
      }
    ...
    case Instruction::kVerifyVarArgRange:
      if (inst->GetVerifyExtraFlags() == Instruction::kVerifyVarArgRangeNonZero
          &&
              inst->VRegA() <= 0) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "invalid arg count ("
                                          << inst->VRegA() << ") in "
                                              "range invoke";
        return false;
      }
  ...
  if (inst->GetVerifyIsRuntimeOnly() && Runtime::Current()->IsAotCompiler()
      && !verify_to_dump_) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "opcode only expected at runtime "
                                      << inst->Name();
    result = false;
  }
  return result;
...
inline bool MethodVerifier::CheckRegisterIndex(uint32_t
idx) {
...
inline bool MethodVerifier::CheckWideRegisterIndex(uint32_t
idx) {
if (idx + 1 >= code_item_->registers_size_) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "wide register index out of range (" << idx
...
inline bool MethodVerifier::CheckFieldIndex(uint32_t
idx) {
if (idx >= dex_file_->
GetHeader()
.field_ids_size_) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "bad field index " << idx << " (max "
<< dex_file_->
GetHeader()
.field_ids_size_ << ")";
return false;
...
inline bool MethodVerifier::CheckMethodIndex(uint32_t
idx) {
if (idx >= dex_file_->
GetHeader()
.method_ids_size_) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "bad method index " << idx << " (max "
<< dex_file_->
GetHeader()
.method_ids_size_ << ")";
return false;
...
inline bool MethodVerifier::CheckNewInstance(dex::TypeIndex idx) {
  if (idx.index_ >= dex_file_->GetHeader().type_ids_size_) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "bad type index " << idx.index_
                                      << " (max "
  ...
  if (descriptor[0] != 'L') {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "can't call new-instance on type '"
                                      << descriptor << "'";
    return false;
...
inline bool MethodVerifier::CheckPrototypeIndex(uint32_t
idx) {
if (idx >= dex_file_->
GetHeader()
.proto_ids_size_) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "bad prototype index " << idx << " (max "
<< dex_file_->
GetHeader()
.proto_ids_size_ << ")";
return false;
...
inline bool MethodVerifier::CheckStringIndex(uint32_t
idx) {
if (idx >= dex_file_->
GetHeader()
.string_ids_size_) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "bad string index " << idx << " (max "
<< dex_file_->
GetHeader()
.string_ids_size_ << ")";
return false;
...
bool MethodVerifier::CheckArrayData(uint32_t
cur_offset) {
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "invalid array data start: at " << cur_offset
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "unaligned array data table: at " << cur_offset
<< ", data offset " <<
array_data_offset;
return false;
}
...
if (!
GetInstructionFlags(cur_offset
+ array_data_offset).
IsOpcode()
) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "array data table at " << cur_offset
<< ", data offset " << array_data_offset
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "invalid array data end: at " << cur_offset
...
bool MethodVerifier::CheckBranchTarget(uint32_t
cur_offset) {
int32_t offset;
bool isConditional, selfOkay;
if (!
GetBranchOffset(cur_offset, &offset, &isConditional, &selfOkay
)) {
return false;
}
if (!
...
== 0) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "branch offset of zero not allowed at"
<< reinterpret_cast
<void *>(cur_offset);
return false;
}
...
!
GetInstructionFlags(abs_offset)
.
IsOpcode()
) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
...
bool MethodVerifier::GetBranchOffset(uint32_t
cur_offset,
int32_t *pOffset,
bool *pConditional,
bool *selfOkay
...
bool MethodVerifier::CheckSwitchTargets(uint32_t
cur_offset) {
const uint32_t insn_count = code_item_->insns_size_in_code_units_;
DCHECK_LT(cur_offset, insn_count
);
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "invalid switch start: at " << cur_offset
...
// Make sure the table is at an even dex pc, that is, 32-bit aligned.
if (!
IsAligned<4>(switch_insns)
) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "unaligned switch table: at " << cur_offset
<< ", switch offset " <<
switch_offset;
return false;
}
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "invalid switch end: at " << cur_offset
<< ", switch offset " << switch_offset
...
int32_t max_first_key =
    std::numeric_limits<int32_t>::max()
        - (static_cast<int32_t>(switch_count) - 1);
if (first_key > max_first_key) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "invalid packed switch: first_key=" << first_key
<< ", switch_count=" <<
switch_count;
return false;
}
...
if (key <= last_key) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "invalid sparse switch: last key=" << last_key
<< ", this=" <<
key;
return false;
}
...
bool MethodVerifier::CheckVarArgRegs(uint32_t
vA,
uint32_t arg[]
) {
...
bool MethodVerifier::CheckVarArgRangeRegs(uint32_t
vA,
uint32_t vC
) {
uint16_t registers_size = code_item_->registers_size_;
...
void MethodVerifier::Dump(VariableIndentationOutputStream *vios) {
  ...
  for (size_t dex_pc = 0; dex_pc < code_item_->insns_size_in_code_units_;
    ...
    vios->Stream()
        << StringPrintf("0x%04zx", dex_pc) << ": "
        << GetInstructionFlags(dex_pc).ToString() << " ";
...
bool MethodVerifier::SetTypesFromSignature() {
  ...
  if (!IsStatic()) {
    if (expected_args == 0) {
      ...
      Fail(VERIFY_ERROR_BAD_CLASS_HARD)
          << "expected 0 args, but method is not static";
      return false;
    }
    ...
    if (IsConstructor()) {
      if (declaring_class.IsJavaLangObject()) {
        ...
        reg_line->SetRegisterType<LockOp::kClear>(this,
                                                  arg_start + cur_arg,
                                                  declaring_class);
      } else {
  ...
  for (; iterator.HasNext(); iterator.Next()) {
    ...
    switch (descriptor[0]) {
      ...
      {
        const RegType
            &reg_type = ResolveClassAndCheckAccess(iterator.GetTypeIdx());
      ...
      case 'D': {
        ...
        reg_line->SetRegisterTypeWide(this,
                                      arg_start + cur_arg,
                                      *lo_half,
                                      *hi_half);
        cur_arg++;
  ...
  if (IsPrimitiveDescriptor(descriptor[0]) || descriptor[0] == 'V') {
    result = descriptor[1] == '\0';
      ...
      == '[') {  // single/multi-dimensional array of object/primitive
    size_t i = 0;
    do {
      i++;
    ...
    if (descriptor[i] == 'L') {  // object array
      ...
      result =
          IsPrimitiveDescriptor(descriptor[i]) && descriptor[i + 1] == '\0';
    }
  ...
  if (!result) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD)
        << "unexpected char in return type descriptor '"
        << descriptor << "'";
  }
  return result;
}
...
bool MethodVerifier::CodeFlowVerifyMethod() {
    ...
    if (GetInstructionFlags(insn_idx).IsBranchTarget()) {
      ...
      if (register_line != nullptr) {
        if (work_line_->CompareLine(register_line) != 0) {
          ...
          LOG(FATAL) << "work_line diverged in "
  ...
  if (kDebugVerify) {
    ...
    for (; insn_idx < insns_size;
           insn_idx += Instruction::At(
               code_item_->insns_ + insn_idx)->SizeInCodeUnits()) {
      /*
...
// is no such field.
static uint32_t GetFirstFinalInstanceFieldIndex(const DexFile &dex_file,
...
static void AdjustReturnLine(MethodVerifier *verifier,
  ...
  switch (opcode) {
    ...
    case Instruction::RETURN_WIDE:
      line->MarkAllRegistersAsConflictsExceptWide(verifier,
                                                  ret_inst->VRegA_11x());
      break;
...
bool MethodVerifier::CodeFlowVerifyInstruction(uint32_t * start_guess) {
  ...
  if (monitor_enter_dex_pcs_ != nullptr
      && work_insn_idx_ == interesting_dex_pc_) {
  ...
  switch (inst->Opcode()) {
    case Instruction::NOP:
      ...
      if (inst->VRegA_10x() != 0) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD)
            << "encountered data table in instruction stream";
      }
      break;
    ...
    case Instruction::RETURN:
      if (!IsInstanceConstructor()
        ...
        if (!return_type.IsCategory1Types()) {
          ...
          bool success =
              work_line_->VerifyRegisterType(this,
                                             vregA,
                                             use_src ? src_type : return_type);
          if (!success) {
            AppendToLastFailMessage(StringPrintf(
                " return-1nr on invalid register v%d",
                vregA));
          }
        }
      }
    ...
    case Instruction::RETURN_WIDE:
      if (!IsInstanceConstructor()
        ...
        if (!return_type.IsCategory2Types()) {
          ...
          const uint32_t vregA = inst->VRegA_11x();
          bool success =
              work_line_->VerifyRegisterType(this, vregA, return_type);
          if (!success) {
            AppendToLastFailMessage(StringPrintf(
                " return-wide on invalid register v%d",
                vregA));
          }
        }
      }
      ...
      if (!IsInstanceConstructor()
        ...
        if (!return_type.IsReferenceTypes()) {
          ...
          if (reg_type.IsUndefined()) {
            ...
            if (reg_type.IsUnresolvedTypes()
                || return_type.IsUnresolvedTypes()) {
              Fail(VERIFY_ERROR_NO_CLASS) << " can't resolve returned type '"
                                          << return_type
                                          << "' or '" << reg_type << "'";
            } else {
              ...
              if (reg_type.IsArrayTypes() && return_type.IsArrayTypes()) {
                return_type.CanAssignArray(reg_type,
                                           reg_types_,
                                           class_loader_,
                                           this,
                                           &soft_error);
                if (soft_error) {
                  Fail(VERIFY_ERROR_BAD_CLASS_SOFT)
    ...
    case Instruction::CONST_WIDE_16: {
      ...
      const RegType
          &lo = reg_types_.FromCat2ConstLo(static_cast<int32_t>(val), true);
      const RegType &hi =
          reg_types_.FromCat2ConstHi(static_cast<int32_t>(val >> 32), true);
    ...
    case Instruction::CONST_CLASS: {
      ...
      const RegType &res_type =
          ResolveClassAndCheckAccess(dex::TypeIndex(inst->VRegB_21c()));
    ...
    case Instruction::MONITOR_ENTER:
      ...
      if (0 != work_insn_idx_
          && !GetInstructionFlags(work_insn_idx_).IsBranchTarget()) {
        uint32_t prev_idx = work_insn_idx_ - 1;
        while (0 != prev_idx && !GetInstructionFlags(prev_idx).IsOpcode()) {
          prev_idx--;
        }
        const Instruction
            *prev_inst = Instruction::At(code_item_->insns_ + prev_idx);
    ...
    case Instruction::INSTANCE_OF: {
      ...
      uint32_t orig_type_reg =
          (is_checkcast) ? inst->VRegA_21c() : inst->VRegB_22c();
      const RegType
          &orig_type = work_line_->GetRegisterType(this, orig_type_reg);
      if (!res_type.IsNonZeroReferenceTypes()) {
        if (is_checkcast) {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "check-cast on unexpected class "
                                            << res_type;
        } else {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "instance-of on unexpected class " << res_type;
        }
      } else if (!orig_type.IsReferenceTypes()) {
        if (is_checkcast) {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "check-cast on non-reference in v" << orig_type_reg;
        } else {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "instance-of on non-reference in v" << orig_type_reg;
        }
      } else if (orig_type.IsUninitializedTypes()) {
        if (is_checkcast) {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "check-cast on uninitialized reference in v"
              << orig_type_reg;
        } else {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "instance-of on uninitialized reference in v"
              << orig_type_reg;
        }
      } else {
        if (is_checkcast) {
          work_line_->SetRegisterType<LockOp::kKeep>(this,
                                                     inst->VRegA_21c(),
                                                     res_type);
        } else {
          work_line_->SetRegisterType<LockOp::kClear>(this,
    ...
    case Instruction::ARRAY_LENGTH: {
      const RegType
          &res_type = work_line_->GetRegisterType(this, inst->VRegB_12x());
      if (res_type.IsReferenceTypes()) {
        if (!res_type.IsArrayTypes()
            && !res_type.IsZero()) {  // ie not an array or null
          Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "array-length on non-array "
                                            << res_type;
        } else {
          work_line_->SetRegisterType<LockOp::kClear>(this,
                                                      inst->VRegA_12x(),
    ...
    case Instruction::NEW_INSTANCE: {
      ...
      const RegType
          &uninit_type = reg_types_.Uninitialized(res_type, work_insn_idx_);
    ...
    case Instruction::CMPG_FLOAT:
      if (!work_line_->VerifyRegisterType(this,
                                          inst->VRegB_23x(),
                                          reg_types_.Float())) {
        break;
      }
    ...
    case Instruction::CMPG_DOUBLE:
      if (!work_line_->VerifyRegisterTypeWide(this,
                                              inst->VRegB_23x(),
                                              reg_types_.DoubleLo(),
                                              reg_types_.DoubleHi())) {
        break;
    ...
    case Instruction::THROW: {
      const RegType
          &res_type = work_line_->GetRegisterType(this, inst->VRegA_11x());
      if (!reg_types_.JavaLangThrowable(false).IsAssignableFrom(res_type,
        ...
        if (res_type.IsUninitializedTypes()) {
          ...
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "thrown value of non-reference type " << res_type;
        } else {
          Fail(res_type.IsUnresolvedTypes() ? VERIFY_ERROR_NO_CLASS
    ...
    case Instruction::FILL_ARRAY_DATA: {
      ...
      const RegType
          &array_type = work_line_->GetRegisterType(this, inst->VRegA_31t());
      ...
      if (!array_type.IsZero()) {
        if (!array_type.IsArrayTypes()) {
          ...
          const RegType &component_type =
              reg_types_.GetComponentType(array_type, GetClassLoader());
          ...
          if (component_type.IsNonZeroReferenceTypes()) {
            ...
            if (array_data[0] != Instruction::kArrayDataSignature) {
              Fail(VERIFY_ERROR_BAD_CLASS_HARD)
                  << "invalid magic for array-data";
            } else {
              size_t elem_width =
                  Primitive::ComponentSize(component_type.GetPrimitiveType());
              ...
              if (array_data[1] != elem_width) {
                Fail(VERIFY_ERROR_BAD_CLASS_HARD)
                    << "array-data size mismatch (" << array_data[1]
                    << " vs " << elem_width << ")";
              }
            }
          }
    ...
    case Instruction::IF_NE: {
      const RegType
          &reg_type1 = work_line_->GetRegisterType(this, inst->VRegA_22t());
      const RegType
          &reg_type2 = work_line_->GetRegisterType(this, inst->VRegB_22t());
      bool mismatch = false;
      if (reg_type1.IsZero()) {  // zero then integral or reference expected
        mismatch =
            !reg_type2.IsReferenceTypes() && !reg_type2.IsIntegralTypes();
      ...
      if (mismatch) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "args to if-eq/if-ne ("
                                          << reg_type1 << ","
                                          << reg_type2
    ...
    case Instruction::IF_NEZ: {
      const RegType
          &reg_type = work_line_->GetRegisterType(this, inst->VRegA_21t());
      ...
      if (0 != work_insn_idx_) {
        ...
        while (0 != instance_of_idx
            && !GetInstructionFlags(instance_of_idx).IsOpcode()) {
          instance_of_idx--;
        }
        if (FailOrAbort(this,
                        GetInstructionFlags(instance_of_idx).IsOpcode(),
      ...
      const Instruction *instance_of_inst =
          Instruction::At(code_item_->insns_ + instance_of_idx);
      ...
      if (!CurrentInsnFlags()->IsBranchTarget() &&
        ...
        const RegType &orig_type =
            work_line_->GetRegisterType(this, instance_of_inst->VRegB_22c());
        ...
        if (!orig_type.Equals(cast_type) &&
            ...
            cast_type.HasClass()
            &&             // Could be conflict type, make sure it has a class.
                !cast_type.GetClass()->IsInterface() &&
            (orig_type.IsZero() ||
                orig_type.IsStrictlyAssignableFrom(
                    cast_type.Merge(orig_type, &reg_types_, this), this))) {
          RegisterLine *update_line =
              RegisterLine::Create(code_item_->registers_size_, this);
          ...
          update_line->SetRegisterType<LockOp::kKeep>(this,
                                                      ...
                                                      cast_type);
          if (!GetInstructionFlags(instance_of_idx).IsBranchTarget()
              && 0 != instance_of_idx) {
            ...
            if (FailOrAbort(this,
                            GetInstructionFlags(move_idx).IsOpcode(),
                            "Unable to get previous instruction of if-eqz/if-nez for work index ",
                            work_insn_idx_)) {
              break;
            }
            const Instruction
                *move_inst = Instruction::At(code_item_->insns_ + move_idx);
    ...
    case Instruction::IGET_BOOLEAN:
      VerifyISFieldAccess<FieldAccessType::kAccGet>(inst,
                                                    reg_types_.Boolean(),
                                                    true,
                                                    false);
      break;
    ...
    case Instruction::IPUT_BOOLEAN:
      VerifyISFieldAccess<FieldAccessType::kAccPut>(inst,
                                                    reg_types_.Boolean(),
                                                    true,
                                                    false);
      break;
    ...
    case Instruction::INVOKE_SUPER_RANGE: {
      ...
      if (called_method != nullptr) {
        mirror::Class *return_type_class =
            called_method->GetReturnType(can_load_classes_);
      ...
      if (return_type == nullptr) {
        uint32_t
            method_idx = (is_range) ? inst->VRegB_3rc() : inst->VRegB_35c();
      ...
      if (!return_type->IsLowHalf()) {
        ...
        work_line_->SetResultRegisterTypeWide(*return_type,
                                              return_type->HighHalf(&reg_types_));
      }
    ...
    case Instruction::INVOKE_DIRECT_RANGE: {
      ...
      ArtMethod
          *called_method = VerifyInvocationArgs(inst, METHOD_DIRECT, is_range);
      ...
      if (called_method == nullptr) {
        ...
        const DexFile::MethodId &method_id = dex_file_->GetMethodId(method_idx);
        is_constructor =
            strcmp("<init>", dex_file_->StringDataByIdx(method_id.name_idx_))
                == 0;
        dex::TypeIndex return_type_idx =
            dex_file_->GetProtoId(method_id.proto_idx_).return_type_idx_;
      ...
      if (return_type == nullptr) {
        return_type = &reg_types_.FromDescriptor(GetClassLoader(),
    ...
    case Instruction::INVOKE_STATIC_RANGE: {
      ...
      ArtMethod
          *called_method = VerifyInvocationArgs(inst, METHOD_STATIC, is_range);
    ...
    case Instruction::INVOKE_POLYMORPHIC_RANGE: {
      ...
      ArtMethod *called_method =
          VerifyInvocationArgs(inst, METHOD_POLYMORPHIC, is_range);
      if (called_method == nullptr) {
        ...
        if (failure_messages_.size() > 0) {
          ...
          Fail(VERIFY_ERROR_BAD_CLASS_HARD)
              << "invoke-polymorphic verification failure.";
        }
        break;
      }
      ...
      const uint32_t
          proto_idx = (is_range) ? inst->VRegH_4rcc() : inst->VRegH_45cc();
    ...
    case Instruction::INVOKE_CUSTOM_RANGE: {
      ...
      const uint32_t
          call_site_idx = is_range ? inst->VRegB_3rc() : inst->VRegB_35c();
      ...
      VerifyInvocationArgsFromIterator(&param_it,
                                       inst,
                                       METHOD_STATIC,
                                       is_range,
                                       nullptr);
      const char
          *return_descriptor = dex_file_->GetReturnTypeDescriptor(proto_id);
    ...
    case Instruction::NOT_LONG:
      work_line_->CheckUnaryOpWide(this,
                                   inst,
                                   reg_types_.LongLo(),
                                   reg_types_.LongHi(),
                                   reg_types_.LongLo(),
    ...
    case Instruction::INT_TO_LONG:
      work_line_->CheckUnaryOpToWide(this,
                                     inst,
                                     reg_types_.LongLo(),
                                     reg_types_.LongHi(),
                                     reg_types_.Integer());
    ...
    case Instruction::LONG_TO_INT:
      work_line_->CheckUnaryOpFromWide(this,
                                       inst,
                                       reg_types_.Integer(),
                                       reg_types_.LongLo(),
                                       reg_types_.LongHi());
    ...
    case Instruction::XOR_LONG:
      work_line_->CheckBinaryOpWide(this,
                                    inst,
                                    reg_types_.LongLo(),
                                    reg_types_.LongHi(),
                                    reg_types_.LongLo(),
    ...
    case Instruction::USHR_LONG:
      ...
      work_line_->CheckBinaryOpWideShift(this,
                                         inst,
                                         reg_types_.LongLo(),
                                         reg_types_.LongHi(),
                                         reg_types_.Integer());
    ...
    case Instruction::USHR_INT_2ADDR:
      work_line_->CheckBinaryOp2addr(this,
                                     inst,
                                     reg_types_.Integer(),
                                     reg_types_.Integer(),
                                     reg_types_.Integer(),
    ...
    case Instruction::XOR_LONG_2ADDR:
      work_line_->CheckBinaryOp2addrWide(this,
                                         inst,
                                         reg_types_.LongLo(),
                                         reg_types_.LongHi(),
                                         reg_types_.LongLo(),
    ...
    case Instruction::USHR_LONG_2ADDR:
      work_line_->CheckBinaryOp2addrWideShift(this,
                                              inst,
                                              reg_types_.LongLo(),
                                              reg_types_.LongHi(),
                                              reg_types_.Integer());
    ...
    case Instruction::REM_INT_LIT16:
      work_line_->CheckLiteralOp(this,
                                 inst,
                                 reg_types_.Integer(),
                                 reg_types_.Integer(),
                                 false,
      ...
      if (IsConstructor() && !IsStatic()) {
        ...
        if (declaring_class.IsUnresolvedReference()) {
          ...
          uint32_t first_index = GetFirstFinalInstanceFieldIndex(*dex_file_,
                                                                 dex_file_->GetMethodId(
                                                                     dex_method_idx_).class_idx_);
          if (first_index != DexFile::kDexNoIndex) {
            Fail(VERIFY_ERROR_BAD_CLASS_HARD)
                << "return-void-no-barrier not expected for field "
                << first_index;
          }
          break;
        }
        auto *klass = declaring_class.GetClass();
        for (uint32_t i = 0, num_fields = klass->NumInstanceFields();
             i < num_fields; ++i) {
          if (klass->GetInstanceField(i)->IsFinal()) {
            Fail(VERIFY_ERROR_BAD_CLASS_HARD)
                << "return-void-no-barrier not expected for "
    ...
    case Instruction::IGET_QUICK:
      VerifyQuickFieldAccess<FieldAccessType::kAccGet>(inst,
                                                       reg_types_.Integer(),
                                                       true);
      break;
    ...
    case Instruction::IPUT_QUICK:
      VerifyQuickFieldAccess<FieldAccessType::kAccPut>(inst,
                                                       reg_types_.Integer(),
                                                       true);
      break;
  ...
  if ((opcode_flags & Instruction::kBranch) != 0) {
    bool isConditional, selfOkay;
    if (!GetBranchOffset(work_insn_idx_,
                         &branch_target,
                         &isConditional,
                         &selfOkay)) {
    ...
    if (!CheckNotMoveExceptionOrMoveResult(code_item_->insns_,
                                           work_insn_idx_ + branch_target)) {
      return false;
    }
    /* update branch target, set "changed" if appropriate */
    if (nullptr != branch_line) {
      if (!UpdateRegisters(work_insn_idx_ + branch_target,
                           branch_line.get(),
                           false)) {
        return false;
      }
  ...
  if ((opcode_flags & Instruction::kSwitch) != 0) {
    ...
    for (targ = 0; targ < switch_count; targ++) {
      ...
      offset = switch_insns[offset_to_targets + targ * 2] |
          (static_cast<int32_t>(switch_insns[offset_to_targets + targ * 2 + 1])
              << 16);
  ...
  if ((opcode_flags & Instruction::kThrow) != 0
    ...
    for (; iterator.HasNext(); iterator.Next()) {
      ...
      if (!handler_type_idx.IsValid()) {
        ...
        mirror::Class *klass =
            linker->ResolveType(*dex_file_, handler_type_idx, dex_cache_,
                                class_loader_);
        if (klass != nullptr) {
       ...
       */
      if (!UpdateRegisters(iterator.GetHandlerAddress(),
                           saved_line_.get(),
                           false)) {
        return false;
      }
    ...
    if (work_line_->MonitorStackDepth() > 0 && !has_catch_all_handler) {
       ...
       */
      if (inst->Opcode() != Instruction::MONITOR_ENTER
          || work_line_->MonitorStackDepth() != 1) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD)
  ...
  if ((opcode_flags & Instruction::kContinue) != 0) {
    ...
    if (next_insn_idx >= code_item_->insns_size_in_code_units_) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD)
          << "Execution can walk off end of code area";
      return false;
    }
    ...
    if (GetInstructionFlags(next_insn_idx).IsReturn()) {
      ...
      const Instruction
          *ret_inst = Instruction::At(code_item_->insns_ + next_insn_idx);
...
void MethodVerifier::UninstantiableError(const char *descriptor) {
  Fail(VerifyError::VERIFY_ERROR_NO_CLASS)
      << "Could not create precise reference for "
...
const RegType &
MethodVerifier::ResolveClassAndCheckAccess(dex::TypeIndex class_idx) {
  mirror::Class *klass = can_load_classes_
          ...
          *dex_file_, class_idx, dex_cache_, class_loader_)
                         : ClassLinker::LookupResolvedType(class_idx,
                                                           dex_cache_.Get(),
                                                           class_loader_.Get()).Ptr();
  ...
  if (result->IsConflict()) {
    const char *descriptor = dex_file_->StringByTypeIdx(class_idx);
    Fail(VERIFY_ERROR_BAD_CLASS_SOFT) << "accessing broken descriptor '"
                                      << descriptor
        ...
        if (iterator.GetHandlerAddress() == (uint32_t) work_insn_idx_) {
          if (!iterator.GetHandlerTypeIndex().IsValid()) {
            ...
            const RegType &exception =
                ResolveClassAndCheckAccess(iterator.GetHandlerTypeIndex());
            if (!reg_types_.JavaLangThrowable(false).IsAssignableFrom(exception,
              ...
              if (exception.IsUnresolvedTypes()) {
                ...
                Fail(VERIFY_ERROR_NO_CLASS) << "unresolved exception class "
                                            << exception;
                return exception;
              } else {
                Fail(VERIFY_ERROR_BAD_CLASS_SOFT)
                    << "unexpected non-exception class " << exception;
...
MethodResolutionKind
    res_kind = GetMethodResolutionKind(method_type, klass->IsInterface());

ArtMethod
    *res_method = dex_cache_->GetResolvedMethod(dex_method_idx, pointer_size);
...
VerifierDeps::MaybeRecordMethodResolution(*dex_file_, dex_method_idx, res_kind,
nullptr);
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "rejecting non-direct call to constructor "
<< res_method->
PrettyMethod();
return nullptr;
}
...
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "rejecting call to class initializer "
<< res_method->
PrettyMethod();
return nullptr;
}
...
->
GetVersion()
< DexFile::kDefaultMethodsVersion) ||
method_type != METHOD_DIRECT) &&
method_type != METHOD_SUPER) {
Fail(VERIFY_ERROR_CLASS_CHANGE)
<< "non-interface method " << dex_file_->
PrettyMethod(dex_method_idx)
<< " is in an interface class " << klass->
PrettyClass();
return nullptr;
}
} else {
...
Fail(VERIFY_ERROR_CLASS_CHANGE)
<< "interface method " << dex_file_->
PrettyMethod(dex_method_idx)
<< " is in a non-interface class " << klass->
PrettyClass();
return nullptr;
}
}
...
if (stash_method) {
dex_cache_->
SetResolvedMethod(dex_method_idx, res_method, pointer_size
);
}

// Check if access is allowed.
if (!referrer.
CanAccessMember(res_method
->
GetDeclaringClass(), res_method
->
GetAccessFlags()
)) {
Fail(VERIFY_ERROR_ACCESS_METHOD)
<< "illegal method access (call "
<< res_method->
PrettyMethod()
<< " from " << referrer << ")";
return
...
// arguments.
if ((method_type == METHOD_DIRECT && (!res_method->
IsDirect()
|| res_method->
IsStatic()
)) ||
(method_type == METHOD_STATIC && !res_method->
IsStatic()
) ||
((method_type == METHOD_SUPER ||
method_type == METHOD_VIRTUAL ||
method_type == METHOD_INTERFACE) && res_method->
IsDirect()
) ||
((method_type == METHOD_POLYMORPHIC) &&
(!res_method->
...
Fail(VERIFY_ERROR_CLASS_CHANGE)
<< "invoke type (" << method_type << ") does not match method "
"type of " << res_method->
PrettyMethod();
return nullptr;
}
...
ArtMethod *MethodVerifier::VerifyInvocationArgsFromIterator(
  ...
  if (method_type != METHOD_POLYMORPHIC) {
    if (expected_args > code_item_->outs_size_) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "invalid argument count ("
                                        << expected_args
                                        << ") exceeds outsSize ("
                                        << code_item_->outs_size_ << ")";
      return nullptr;
    }
  }
  ...
  if (method_type != METHOD_STATIC) {
    ...
    if (actual_arg_type.IsUninitializedTypes()) {
      if (res_method) {
        ...
        const uint32_t
            method_idx = (is_range) ? inst->VRegB_3rc() : inst->VRegB_35c();
        if (strcmp(dex_file_->GetMethodName(dex_file_->GetMethodId(method_idx)),
                   "<init>") != 0) {
          Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "'this' arg must be initialized";
          return nullptr;
        }
    ...
    const RegType &adjusted_type = is_init
                                   ? GetRegTypeCache()->FromUninitialized(
            actual_arg_type)
                                   : actual_arg_type;
    if (method_type != METHOD_INTERFACE && !adjusted_type.IsZero()) {
      ...
      if (res_method != nullptr && !res_method->IsMiranda()) {
        ...
        const dex::TypeIndex
            class_idx = dex_file_->GetMethodId(method_idx).class_idx_;
  ...
  for (; it->HasNext(); it->Next()) {
    if (sig_registers >= expected_args) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Rejecting invocation, expected "
                                        << inst->VRegA() <<
                                        " argument registers, method signature has "
                                        << sig_registers + 1 << " or more";
      return nullptr;
    }
    ...
    if (param_descriptor == nullptr) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD)
          << "Rejecting invocation because of missing signature "
              "component";
      return nullptr;
    }

    const RegType &reg_type =
        reg_types_.FromDescriptor(GetClassLoader(), param_descriptor, false);
    uint32_t get_reg =
        is_range ? inst->VRegC() + static_cast<uint32_t>(sig_registers) :
        arg[sig_registers];
    if (reg_type.IsIntegralTypes()) {
      ...
      if (!work_line_->VerifyRegisterType(this, get_reg, reg_type)) {
        ...
        if (!is_range && sig_registers + 1 < expected_args) {
          ...
          if (second_reg != get_reg + 1) {
            Fail(VERIFY_ERROR_BAD_CLASS_HARD)
                << "Rejecting invocation, long or double parameter "
                    "at index " << sig_registers << " is not a pair: "
                << get_reg << " + "
                << second_reg << ".";
            return nullptr;
          }
...
void
MethodVerifier::VerifyInvocationArgsUnresolvedMethod(const Instruction *inst,
                                                     MethodType method_type,
                                                     bool is_range) {
  ...
  DexFileParameterIterator it(*dex_file_,
                              dex_file_->GetProtoId(dex_file_->GetMethodId(
                                  method_idx).proto_idx_));
  VerifyInvocationArgsFromIterator(&it, inst, method_type, is_range, nullptr);
}

bool MethodVerifier::CheckCallSite(uint32_t
call_site_idx) {
CallSiteArrayValueIterator
    it(*dex_file_, dex_file_->GetCallSiteId(call_site_idx));
...
it.
Next();
const DexFile::MethodHandleItem
    &mh = dex_file_->GetMethodHandle(method_handle_idx);
if (mh.method_handle_type_ != static_cast
<uint16_t>(DexFile::MethodHandleType::kInvokeStatic)
) {
Fail(VERIFY_ERROR_BAD_CLASS_HARD)
<< "Call site #" << call_site_idx
...
const DexFile::MethodId
    &method_id = dex_file_->GetMethodId(mh.field_or_method_idx_);
uint32_t length;
...
case
EncodedArrayValueIterator::ValueType::kBoolean:
case
EncodedArrayValueIterator::ValueType::kByte:
case
EncodedArrayValueIterator::ValueType::kShort:
case
EncodedArrayValueIterator::ValueType::kChar:
case
EncodedArrayValueIterator::ValueType::kInt:
...
break;
case
EncodedArrayValueIterator::ValueType::kFloat:
    match = ('F' == shorty[i]);
break;
case
EncodedArrayValueIterator::ValueType::kDouble:
    match = ('D' == shorty[i]);
break;
case
EncodedArrayValueIterator::ValueType::kMethodType:
case
EncodedArrayValueIterator::ValueType::kMethodHandle:
case
EncodedArrayValueIterator::ValueType::kString:
case
EncodedArrayValueIterator::ValueType::kType:
case
EncodedArrayValueIterator::ValueType::kNull:
    match = ('L' == shorty[i]);
break;
case
EncodedArrayValueIterator::ValueType::kField:
case
EncodedArrayValueIterator::ValueType::kMethod:
case
EncodedArrayValueIterator::ValueType::kEnum:
case
EncodedArrayValueIterator::ValueType::kArray:
case
EncodedArrayValueIterator::ValueType::kAnnotation:
...
class MethodParamListDescriptorIterator {
 public:
  explicit MethodParamListDescriptorIterator(ArtMethod *res_method) :
      res_method_(res_method), pos_(0),
      params_(res_method->GetParameterTypeList()),
...
ArtMethod *MethodVerifier::VerifyInvocationArgs(
  ...
  if (method_type == METHOD_SUPER) {
    ...
    if (reference_type.GetClass()->IsInterface()) {
      ...
      if (class_idx == class_def_.class_idx_) {
        Fail(VERIFY_ERROR_CLASS_CHANGE)
            << "Cannot invoke-super on self as interface";
        return nullptr;
      }
      ...
      if (!GetDeclaringClass().HasClass()) {
        Fail(VERIFY_ERROR_NO_CLASS)
            << "Unable to resolve the full class of 'this' used in an"
            << "interface invoke-super";
        return nullptr;
      } else if (!reference_type.IsStrictlyAssignableFrom(GetDeclaringClass(),
                                                          this)) {
        Fail(VERIFY_ERROR_CLASS_CHANGE)
            << "invoke-super in "
            << mirror::Class::PrettyClass(GetDeclaringClass().GetClass())
            << " in method "
            << dex_file_->PrettyMethod(dex_method_idx_) << " to method "
            << dex_file_->PrettyMethod(method_idx) << " references "
            << "non-super-interface type "
            << mirror::Class::PrettyClass(reference_type.GetClass());
        return nullptr;
      }
    } else {
  ...
  if (method_type == METHOD_POLYMORPHIC) {
    ...
    DexFileParameterIterator
        it(*dex_file_, dex_file_->GetProtoId(inst->VRegH()));
    return VerifyInvocationArgsFromIterator(&it,
                                            inst,
                                            method_type,
                                            is_range,
                                            res_method);
...
bool MethodVerifier::CheckSignaturePolymorphicMethod(ArtMethod *method) {
  ...
  if (strcmp(method_name, "invoke") != 0
      && strcmp(method_name, "invokeExact") != 0) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD)
  ...
  if (types->Size() != 1) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD)
        << "Signature polymorphic method has too many arguments "
        << types->Size() << " != 1";
    return false;
  }

  const dex::TypeIndex argument_type_index = types->GetTypeItem(0).type_idx_;
  const char *argument_descriptor =
      method->GetTypeDescriptorFromTypeIdx(argument_type_index);
  ...
  if (strcmp(return_descriptor, "Ljava/lang/Object;") != 0) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD)
        << "Signature polymorphic method has unexpected return type: "
        << return_descriptor;
    return false;
  }
...
bool
MethodVerifier::CheckSignaturePolymorphicReceiver(const Instruction *inst) {
  const RegType &this_type = work_line_->GetInvocationThis(this, inst);
  if (this_type.IsZero()) {
...
ArtMethod *MethodVerifier::GetQuickInvokedMethod(const Instruction *inst,
                                                 RegisterLine *reg_line,
                                                 bool is_range,
                                                 bool allow_failure) {
  if (is_range) {
  ...
  const RegType
      &actual_arg_type = reg_line->GetInvocationThis(this, inst, allow_failure);
  ...
  ArtMethod
      *res_method = dispatch_class->GetVTableEntry(vtable_index, pointer_size);
...
ArtMethod *MethodVerifier::VerifyInvokeVirtualQuickArgs(const Instruction *inst,
                                                        bool is_range) {
  DCHECK(Runtime::Current()->IsStarted() || verify_to_dump_)
      << dex_file_->PrettyMethod(dex_method_idx_, true) << "@"
      << work_insn_idx_;

  ArtMethod *res_method =
      GetQuickInvokedMethod(inst, work_line_.get(), is_range, false);
  if (res_method == nullptr) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Cannot infer method from "
                                      << inst->Name();
    return nullptr;
  }
  if (FailOrAbort(this,
  ...
  const size_t
      expected_args = (is_range) ? inst->VRegA_3rc() : inst->VRegA_35c();
  /* caught by static verifier */
  DCHECK(is_range || expected_args <= 5);
  if (expected_args > code_item_->outs_size_) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "invalid argument count ("
    ...
    if (!res_method_class.IsAssignableFrom(actual_arg_type, this)) {
      Fail(actual_arg_type.IsUninitializedTypes()    // Just overcautious - should have never
           ...
           : actual_arg_type.IsUnresolvedTypes()
             ? VERIFY_ERROR_NO_CLASS
             : VERIFY_ERROR_BAD_CLASS_SOFT) << "'this' argument '"
                                            << actual_arg_type
                                            << "' not instance of '"
                                            << res_method_class << "'";
      return nullptr;
  ...
  for (size_t param_index = 0; param_index < params_size; param_index++) {
    ...
    const RegType &reg_type =
        reg_types_.FromDescriptor(GetClassLoader(), descriptor, false);
    uint32_t
        get_reg = is_range ? inst->VRegC_3rc() + actual_args : arg[actual_args];
    if (!work_line_->VerifyRegisterType(this, get_reg, reg_type)) {
      return res_method;
    }
    actual_args =
        reg_type.IsLongOrDoubleTypes() ? actual_args + 2 : actual_args + 1;
  }
  if (actual_args != expected_args) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Rejecting invocation of "
                                      << res_method->PrettyMethod()
                                      << " expected "
                                      << expected_args << " arguments, found "
                                      << actual_args;
    return nullptr;
  } else {
    return res_method;
...
void MethodVerifier::VerifyNewArray(const Instruction *inst,
                                    bool is_filled,
                                    bool is_range) {
  dex::TypeIndex type_idx;
  if (!is_filled) {
  ...
  if (res_type.IsConflict()) {  // bad class
    ...
    // TODO: check Compiler::CanAccessTypeWithoutChecks returns false when res_type is unresolved
    if (!res_type.IsArrayTypes()) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "new-array on non-array class "
                                        << res_type;
    } else if (!is_filled) {
      ...
      const RegType &expected_type =
          reg_types_.GetComponentType(res_type, GetClassLoader());
...
void MethodVerifier::VerifyAGet(const Instruction *inst,
  ...
  const RegType
      &index_type = work_line_->GetRegisterType(this, inst->VRegC_23x());
  if (!index_type.IsArrayIndexTypes()) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Invalid reg type for array index ("
                                      << index_type << ")";
  } else {
    const RegType
        &array_type = work_line_->GetRegisterType(this, inst->VRegB_23x());
    if (array_type.IsZero()) {
      have_pending_runtime_throw_failure_ = true;
      ...
      Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "not array type " << array_type
                                        << " with aget";
    } else if (array_type.IsUnresolvedMergedReference()) {
      // Unresolved array types must be reference array types.
      if (is_primitive) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "reference array type "
                                          << array_type
      ...
      if (!component_type.IsReferenceTypes() && !is_primitive) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "primitive array type "
                                          << array_type
        ...
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "array type " << array_type
                                          << " incompatible with aget of type "
                                          << insn_type;
      } else {
        ...
        if (!component_type.IsLowHalf()) {
          ...
          work_line_->SetRegisterTypeWide(this,
                                          inst->VRegA_23x(),
                                          component_type,
...
void MethodVerifier::VerifyPrimitivePut(const RegType &target_type,
  ...
  if (target_type.IsIntegralTypes()) {
    ...
    instruction_compatible =
        insn_type.IsInteger();  // no put-float, so expect put-int
    ...
    if (instruction_compatible && work_line_->NumRegs() > vregA + 1) {
      const RegType
          &value_type_hi = work_line_->GetRegisterType(this, vregA + 1);
      value_compatible =
          value_type.IsLongTypes() && value_type.CheckWidePair(value_type_hi);
    } else {
      value_compatible = false;
    }
  } else if (target_type.IsDouble()) {
    instruction_compatible =
        insn_type.IsLong();  // no put-double, so expect put-long
    // Additional register check: this is not checked statically (as part of VerifyInstructions),
    // as target_type depends on the resolved type of the field.
    if (instruction_compatible && work_line_->NumRegs() > vregA + 1) {
      const RegType
          &value_type_hi = work_line_->GetRegisterType(this, vregA + 1);
      value_compatible =
          value_type.IsDoubleTypes() && value_type.CheckWidePair(value_type_hi);
    } else {
      value_compatible = false;
    }
  } else {
  ...
  if (!index_type.IsArrayIndexTypes()) {
    ...
    if (array_type.IsZero()) {
      ...
      if ((modified_reg_type == &reg_types_.Integer()) ||
        ...
        const RegType
            &value_type = work_line_->GetRegisterType(this, inst->VRegA_23x());
        if (modified_reg_type == &reg_types_.Integer()) {
...
ArtField *MethodVerifier::GetStaticField(int field_idx) {
  ...
  if (klass_type.IsConflict()) {  // bad class
    AppendToLastFailMessage(StringPrintf(
        " in attempt to access static field %d (%s) in %s",
        field_idx,
  ...
  ArtField *field = class_linker->ResolveFieldJLS(*dex_file_,
                                                  field_idx,
                                                  dex_cache_,
                                                  class_loader_);
...
ArtField *
MethodVerifier::GetInstanceField(const RegType &obj_type, int field_idx) {
  const DexFile::FieldId &field_id = dex_file_->GetFieldId(field_idx);
  // Check access to class.
  const RegType &klass_type = ResolveClassAndCheckAccess(field_id.class_idx_);
  if (klass_type.IsConflict()) {
    AppendToLastFailMessage(StringPrintf(
        " in attempt to access instance field %d (%s) in %s",
        field_idx,
        dex_file_->GetFieldName(field_id),
        dex_file_->GetFieldDeclaringClassDescriptor(field_id)));
    return nullptr;
  ...
  if (field == nullptr) {
    ...
    Fail(VERIFY_ERROR_BAD_CLASS_HARD)
        << "instance field access on object that has "
    ...
    if (obj_type.IsUninitializedTypes()) {
      ...
      if (is_aot && (field_klass.IsUnresolvedTypes()
          || obj_type.IsUnresolvedTypes())) {
  ...
  if (!GetDeclaringClass().CanAccessMember(field->GetDeclaringClass(),
                                           field->GetAccessFlags())) {
    Fail(VERIFY_ERROR_ACCESS_FIELD) << "cannot access instance field "
                                    << field->PrettyField()
                                    << " from " << GetDeclaringClass();
    return nullptr;
  } else if (field->IsStatic()) {
...
void MethodVerifier::VerifyISFieldAccess(const Instruction *inst,
                                         const RegType &insn_type,
                                         bool is_primitive,
                                         bool is_static) {
  ...
  if (is_static) {
    ...
    const RegType
        &object_type = work_line_->GetRegisterType(this, inst->VRegB_22c());
  ...
  if (field != nullptr) {
    if (kAccType == FieldAccessType::kAccPut) {
      if (field->IsFinal()
        ...
        Fail(VERIFY_ERROR_ACCESS_FIELD) << "cannot modify final field "
                                        << field->PrettyField()
                                        << " from other class "
                                        << GetDeclaringClass();
  ...
  static_assert(kAccType == FieldAccessType::kAccPut
                    || kAccType == FieldAccessType::kAccGet,
  ...
  if (kAccType == FieldAccessType::kAccPut) {
    ...
    if (is_primitive) {
      ...
      if (!insn_type.IsAssignableFrom(*field_type, this)) {
        ...
        VerifyError
            error = field_type->IsReferenceTypes() ? VERIFY_ERROR_BAD_CLASS_SOFT
    ...
    if (is_primitive) {
      if (field_type->Equals(insn_type) ||
        ...
        // the descriptors for the type should have been consistent within the same file at
        // compile time
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "expected field "
    ...
    if (!field_type->IsLowHalf()) {
      ...
      work_line_->SetRegisterTypeWide(this,
                                      vregA,
                                      *field_type,
                                      field_type->HighHalf(&reg_types_));
    }
  } else {
...
ArtField *MethodVerifier::GetQuickFieldAccess(const Instruction *inst,
  ...
  const RegType
      &object_type = reg_line->GetRegisterType(this, inst->VRegB_22c());
  ...
  ArtField *const f =
      ArtField::FindInstanceFieldWithOffset(object_type.GetClass(),
                                            field_offset);
  DCHECK_EQ(f->GetOffset().Uint32Value(), field_offset);
  if (f == nullptr) {
    VLOG(verifier) << "Failed to find instance field at offset '"
                   << field_offset
                   << "' from '"
                   << mirror::Class::PrettyDescriptor(object_type.GetClass())
                   << "'";
  }
  return f;
}
...
template<MethodVerifier::FieldAccessType kAccType>
void MethodVerifier::VerifyQuickFieldAccess(const Instruction *inst,
                                            const RegType &insn_type,
                                            bool is_primitive) {
  DCHECK(Runtime::Current()->IsStarted() || verify_to_dump_);
  ...
  if (field == nullptr) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Cannot infer field from "
                                      << inst->Name();
    return;
  }
  ...
  {
    ObjPtr <mirror::Class>
        field_type_class = can_load_classes_ ? field->GetType<true>() :
                           field->GetType<false>();
    ...
    if (field_type == nullptr) {
      Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Cannot infer field type from "
                                        << inst->Name();
      return;
    }
  }
  ...
  if (kAccType == FieldAccessType::kAccPut) {
    if (is_primitive) {
      ...
      if (field_type->IsIntegralTypes()) {
        ...
        instruction_compatible =
            insn_type.IsInteger();  // no [is]put-float, so expect [is]put-int
        value_compatible = value_type.IsFloatTypes();
        ...
        instruction_compatible =
            insn_type.IsLong();  // no [is]put-double, so expect [is]put-long
      ...
      if (!insn_type.IsAssignableFrom(*field_type, this)) {
        Fail(VERIFY_ERROR_BAD_CLASS_SOFT) << "expected field "
                                          << ArtField::PrettyField(field)
...
bool
MethodVerifier::CheckNotMoveException(const uint16_t *insns, int insn_idx) {
...
bool MethodVerifier::CheckNotMoveExceptionOrMoveResult(const uint16_t *insns,
                                                       int insn_idx) {
  return (CheckNotMoveException(insns, insn_idx)
      && CheckNotMoveResult(insns, insn_idx));
}

bool MethodVerifier::UpdateRegisters(uint32_t
next_insn,
...
if (kDebugVerify) {
copy.
reset(RegisterLine::Create(target_line->NumRegs(), this));
copy->
CopyFromLine(target_line);
}
...
const RegType &MethodVerifier::GetMethodReturnType() {
  if (return_type_ == nullptr) {
    if (mirror_method_ != nullptr) {
      mirror::Class
          *return_type_class = mirror_method_->GetReturnType(can_load_classes_);
      if (return_type_class != nullptr) {
    ...
    if (return_type_ == nullptr) {
      const DexFile::MethodId
          &method_id = dex_file_->GetMethodId(dex_method_idx_);
      const DexFile::ProtoId
          &proto_id = dex_file_->GetMethodPrototype(method_id);
      dex::TypeIndex return_type_idx = proto_id.return_type_idx_;
      const char *descriptor =
          dex_file_->GetTypeDescriptor(dex_file_->GetTypeId(return_type_idx));
      return_type_ =
          &reg_types_.FromDescriptor(GetClassLoader(), descriptor, false);
    }
  }
...
const RegType &MethodVerifier::GetDeclaringClass() {
  if (declaring_class_ == nullptr) {
    ...
    if (mirror_method_ != nullptr) {
      ...
      declaring_class_ = &FromClass(descriptor,
                                    klass,
                                    klass->CannotBeAssignedFromOtherTypes());
    } else {
      declaring_class_ =
...
std::vector <int32_t> MethodVerifier::DescribeVRegs(uint32_t
dex_pc) {
RegisterLine *line = reg_table_.GetLine(dex_pc);
DCHECK(line
!= nullptr) << "No register line at DEX pc " << StringPrintf("0x%x", dex_pc);
...
result.
push_back(kFloatVReg);
result.push_back(0);
} else if (type.
IsLong()
) {
result.
push_back(kLongLoVReg);
result.push_back(0);
result.
push_back(kLongHiVReg);
result.push_back(0);
++
i;
} else if (type.
...
result.
push_back(kDoubleLoVReg);
result.push_back(0);
result.
push_back(kDoubleHiVReg);
result.push_back(0);
++
i;
} else if (type.
...
const RegType &
MethodVerifier::DetermineCat1Constant(int32_t value, bool precise) {
  if (precise) {
...
void
MethodVerifier::VisitRoots(RootVisitor *visitor, const RootInfo &root_info) {
