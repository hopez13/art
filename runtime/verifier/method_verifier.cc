...
void MethodVerifier::VerifyAGet(const Instruction* inst,
  ...
  if (!index_type.IsArrayIndexTypes()) {
    ...
    if (array_type.IsZero()) {
      // At this point of the analysis, we have only seen null for the array type.
      // Infer a merge-able type from the instruction type.
      if (!is_primitive) {
        // Reference
        // Because we don't have the notion of a bottom type (see b/22059710),
        // we can only let this typed Zero. This means we won't complain about primitive uses of
        // this reference array access.
        // We make this a runtime throw to prevent compiling it in the presence of potential
        // type errors.
        have_pending_runtime_throw_failure_ = true;
