...
ArtMethod* MethodVerifier<kVerifierDebug>::ResolveMethodAndCheckAccess(
  ...
  // Check that invoke-virtual is not used on private methods of the same class, nor is invoke-super
  // used on a private method of a class outside the nest group.
  if (res_method->IsPrivate() &&
      (method_type == METHOD_VIRTUAL ||
       (method_type == METHOD_SUPER && !referrer.HasSameNestHost(klass_type)) )) {
