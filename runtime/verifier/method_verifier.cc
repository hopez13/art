...
bool MethodVerifier::CodeFlowVerifyInstruction(uint32_t* start_guess) {
  ...
  switch (inst->Opcode()) {
    ...
    case Instruction::INVOKE_POLYMORPHIC_RANGE: {
      ...
      ArtMethod* called_method = VerifyInvocationArgs(inst, METHOD_VIRTUAL, is_range);
      if (called_method != nullptr) {
...
ArtMethod* MethodVerifier::VerifyInvocationArgs(
  ...
  if (inst->IsInvokePolymorphic()) {
    // Process the parameters that the method handle invoke is called with.
