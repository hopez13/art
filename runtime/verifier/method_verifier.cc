...
bool MethodVerifier::CodeFlowVerifyInstruction(uint32_t* start_guess) {
  ...
  switch (inst->Opcode()) {
    ...
    case Instruction::INVOKE_CUSTOM_RANGE: {
      ...
      // method handle produced by step 1
...
bool MethodVerifier::CheckCallSite(uint32_t call_site_idx) {
  CallSiteArrayValueIterator it(*dex_file_, dex_file_->GetCallSiteId(call_site_idx));
  // Check essential arguments are provided
  if (it.Size() < 3) {
  ...
  if (method_handle_idx >= dex_file_->NumMethodHandles()) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Call site #" << call_site_idx
                                      << " argument 0 is an invalid method handle index: "
                                      << method_handle_idx;
    return false;
  }
  ...
  if (mh.field_or_method_idx_ >= dex_file_->NumMethodIds()) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Call site #" << call_site_idx
                                      << " argument 0 method handle has bad method index: "
                                      << mh.field_or_method_idx_ << " >= "
                                      << dex_file_->NumMethodIds();
    return false;
  }

  // Check the second argument: the name to resolve
  it.Next();
  if (it.GetValueType() != EncodedArrayValueIterator::ValueType::kString) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Call site has wrong argument type 1: "
                                      << it.GetValueType() << " != "
                                      << EncodedArrayValueIterator::ValueType::kString;
    return false;
  }
  uint32_t name_idx = static_cast<uint32_t>(it.GetJavaValue().i);
  if (name_idx > dex_file_->NumStringIds()) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Call site #" << call_site_idx
                                      << " argument 1 is an invalid method name index: "
                                      << name_idx;
    return false;
  }

  // Check the third argument: the method type expected to be associated with the method name
  it.Next();
  if (it.GetValueType() != EncodedArrayValueIterator::ValueType::kMethodType) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Call site has wrong argument type 2: "
                                      << it.GetValueType() << " != "
                                      << EncodedArrayValueIterator::ValueType::kMethodType;
    return false;
  }
  uint32_t method_type_idx = static_cast<uint32_t>(it.GetJavaValue().i);
  if (method_type_idx > dex_file_->NumProtoIds()) {
    Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "Call site #" << call_site_idx
                                      << " argument 2 is an invalid proto index: "
                                      << method_type_idx;
    return false;
  }
  it.Next();
  ...
  // (CallSite, Lookup, String, MethodType). If they not of the
