...
const RegType& MethodVerifier::ResolveClass(dex::TypeIndex class_idx) {
  ...
  if (C == CheckAccess::kYes &&
      ...
      (api_level_ >= kSdkVersionP || !result->IsUnresolvedTypes())) {
    const RegType& referrer = GetDeclaringClass();
    if ((api_level_ >= kSdkVersionP || !referrer.IsUnresolvedTypes()) &&
...
void MethodVerifier::VerifyISFieldAccess(const Instruction* inst, const RegType& insn_type,
  ...
  } else if (api_level_ >= kSdkVersionP) {
