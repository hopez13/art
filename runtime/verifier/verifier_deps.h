...
class VerifierDeps {
 ...
 private:
  ...
  struct DexFileDeps {
    ...
    // Bit vector indexed by class def inidices indicating whether the corresponding
  ...
  // Iterates over dex files in `classpath` and tries to find a class def matching
  // `descriptor`. Returns true if such class def is found.
  bool IsInClassPath(const char* descriptor,
                     size_t hash,
                     const std::vector<const DexFile*>& classpath,
