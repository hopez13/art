...
class VerifierDeps {
 ...
 private:
  ...
  VerifierDeps(const std::vector<const DexFile*>& dex_files, const std::vector<uint8_t>& data)
      REQUIRES(!Locks::verifier_deps_lock_);
  ...
  struct DexFileDeps {
    ...
    bool operator==(const DexFileDeps& rhs) const;
    bool operator!=(const DexFileDeps& rhs) const { return !(*this == rhs); }
  };
  ...
  bool operator==(const VerifierDeps& rhs) const
      REQUIRES(!Locks::verifier_deps_lock_);
