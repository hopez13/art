...
class VerifierDeps {
 ...
 private:
  ...
  // are not shadowed by classes in parent class loaders, e.g. when vdex was
  // created against SDK stubs and the app redefines a non-public class on
  // boot classpath. In that case, dependencies do not include the dependencies
  // on the presumed-internal class and verification must fail.
  ...
  bool VerifyClassRedefinition(Handle<mirror::ClassLoader> class_loader,
                               const DexFile& dex_file,
