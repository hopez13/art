...
class VerifierDeps {
 public:
  EXPORT explicit VerifierDeps(const std::vector<const DexFile*>& dex_files, bool output_only = true);
  ...
  EXPORT bool ParseStoredData(const std::vector<const DexFile*>& dex_files, ArrayRef<const uint8_t> data);

  // Merge `other` into this `VerifierDeps`'. `other` and `this` must be for the
  // same set of dex files.
  EXPORT void MergeWith(std::unique_ptr<VerifierDeps> other, const std::vector<const DexFile*>& dex_files);
  ...
  // should be emitted.
  EXPORT void Encode(const std::vector<const DexFile*>& dex_files, std::vector<uint8_t>* buffer) const;
