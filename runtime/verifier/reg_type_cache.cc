...
struct TypeHelper {
  ...
  explicit TypeHelper(ClassRoot c, const char* d, uint32_t id) : cls(c), descriptor(d), cache_id(id) {}
};
}  // namespace
...
void RegTypeCache::FillPrimitiveAndSmallConstantTypes() {
  ...
  auto create_primitive_type_instance = [&](auto type) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    const Type* entry = new (&allocator_) Type(handles_.NewHandle(GetClassRoot(type.cls, class_linker_)),
                                               type.descriptor,
                                               type.cache_id);
    entries_[type.cache_id] = entry;
    return entry;
  };
  create_primitive_type_instance(TypeHelper<BooleanType>(ClassRoot::kPrimitiveBoolean, "Z", kBooleanCacheId));
  create_primitive_type_instance(TypeHelper<ByteType>(ClassRoot::kPrimitiveByte, "B", kByteCacheId));
  create_primitive_type_instance(TypeHelper<ShortType>(ClassRoot::kPrimitiveShort, "S", kShortCacheId));
  create_primitive_type_instance(TypeHelper<CharType>(ClassRoot::kPrimitiveChar, "C", kCharCacheId));
  create_primitive_type_instance(TypeHelper<IntegerType>(ClassRoot::kPrimitiveInt, "I", kIntCacheId));
  create_primitive_type_instance(TypeHelper<LongLoType>(ClassRoot::kPrimitiveLong, "J", kLongLoCacheId));
  create_primitive_type_instance(TypeHelper<LongHiType>(ClassRoot::kPrimitiveLong, "J", kLongHiCacheId));
  create_primitive_type_instance(TypeHelper<FloatType>(ClassRoot::kPrimitiveFloat, "F", kFloatCacheId));
  create_primitive_type_instance(TypeHelper<DoubleLoType>(ClassRoot::kPrimitiveDouble, "D", kDoubleLoCacheId));
  create_primitive_type_instance(TypeHelper<DoubleHiType>(ClassRoot::kPrimitiveDouble, "D", kDoubleHiCacheId));

  entries_[kUndefinedCacheId] = new (&allocator_) UndefinedType(null_handle_, "", kUndefinedCacheId);
  entries_[kConflictCacheId]  = new (&allocator_) ConflictType(null_handle_, "", kConflictCacheId);
  entries_[kNullCacheId] = new (&allocator_) NullType(null_handle_, "", kNullCacheId);
}
...
const RegType& RegTypeCache::From(ObjPtr<mirror::ClassLoader> loader,
  ...
  if (klass != nullptr) {
    ...
    if (klass->CannotBeAssignedFromOtherTypes() || precise) {
      ...
      entry =
          new (&allocator_) PreciseReferenceType(handles_.NewHandle(klass), AddString(sv_descriptor), entries_.size());
    } else {
      entry = new (&allocator_) ReferenceType(handles_.NewHandle(klass), AddString(sv_descriptor), entries_.size());
    }
...
const RegType* RegTypeCache::InsertClass(const std::string_view& descriptor,
  ...
  RegType* const reg_type = precise
      ? static_cast<RegType*>(
          new (&allocator_) PreciseReferenceType(handles_.NewHandle(klass), descriptor, entries_.size()))
...
const UninitializedType& RegTypeCache::UninitializedThisArgument(const RegType& type) {
  ...
  if (type.IsUnresolvedTypes()) {
    ...
    entry = new (&allocator_) UninitializedThisReferenceType(handles_.NewHandle(klass), descriptor, entries_.size());
  }
  return AddEntry(entry);
}
