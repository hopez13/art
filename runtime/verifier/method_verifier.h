...
class MethodVerifier {
 public:
  ...
  // Calculates the type of the given `slot` dex register at the given `dex_pc`.
  // No classes will be loaded.
  // If verification fails hard the undefined type will be returned.
  static const RegType& CalculateTypeInfo(Thread* self,
                                          ArtMethod* method,
                                          uint32_t dex_pc,
                                          uint32_t slot)
      REQUIRES_SHARED(Locks::mutator_lock_);
