...
class DexVerifierMetadataTest : public CommonRuntimeTest {
 public:
  ...
  void LoadDexFile(ScopedObjectAccess* soa) SHARED_REQUIRES(Locks::mutator_lock_) {
    class_loader_ = LoadDex("VerifierMetadata");
    ...
    metadata_.reset(new VerifierMetadata(dex_files));
    dex_metadata_ = metadata_->GetDexMetadataFor(*dex_file_);
  }
  ...
  bool TestAssignabilityRecording(const std::string& dst,
    ...
    ScopedObjectAccess soa(Thread::Current());
    LoadDexFile(&soa);
    dex_metadata_->RecordAssignabilityTest(FindClassByName(dst, &soa),
  ...
  DexVerifierMetadata* dex_metadata_;
...
TEST_F(DexVerifierMetadataTest, StringToId) {
  ...
  MutexLock mu(Thread::Current(), dex_metadata_->lock_);

  uint32_t id_Main1 = dex_metadata_->GetIdFromString("LMain;");
  ASSERT_TRUE(dex_metadata_->IsStringIdInDexFile(id_Main1));
  ASSERT_EQ("LMain;", dex_metadata_->GetStringFromId(id_Main1));

  uint32_t id_Main2 = dex_metadata_->GetIdFromString("LMain;");
  ASSERT_TRUE(dex_metadata_->IsStringIdInDexFile(id_Main2));
  ASSERT_EQ("LMain;", dex_metadata_->GetStringFromId(id_Main2));

  uint32_t id_Lorem1 = dex_metadata_->GetIdFromString("Lorem ipsum");
  ASSERT_FALSE(dex_metadata_->IsStringIdInDexFile(id_Lorem1));
  ASSERT_EQ("Lorem ipsum", dex_metadata_->GetStringFromId(id_Lorem1));

  uint32_t id_Lorem2 = dex_metadata_->GetIdFromString("Lorem ipsum");
  ASSERT_FALSE(dex_metadata_->IsStringIdInDexFile(id_Lorem2));
  ASSERT_EQ("Lorem ipsum", dex_metadata_->GetStringFromId(id_Lorem2));
