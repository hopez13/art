...
bool VerifierDeps::IsInClassPath(const char* descriptor,
                                 size_t hash,
                                 const std::vector<const DexFile*>& classpath,
                                 /* out */ const DexFile** out_dex_file) const {
  for (const DexFile* cp_dex_file : classpath) {
    if (OatDexFile::FindClassDef(*cp_dex_file, descriptor, hash) != nullptr) {
      *out_dex_file = cp_dex_file;
      return true;
    }
  }
  return false;
...
bool VerifierDeps::VerifyInternalClasses(const DexFile& dex_file,
  ...
  for (ClassAccessor accessor : dex_file.GetClasses()) {
    ...
    if (IsInClassPath(descriptor, hash, boot_classpath, &cp_dex_file) ||
        IsInClassPath(descriptor, hash, classpath, &cp_dex_file)) {
      *error_msg = std::string("Class ") + descriptor
          + " redefines a class in a the classpath "
