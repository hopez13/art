...
void VerifierDeps::AddAssignability(const DexFile& dex_file,
  ...
  if (destination->IsPrimitive() || source->IsPrimitive()) {
    // Primitive types are trivially assignable to java.lang.Object, and
    // trivially non assignable to any other class. We do not need to
    // record trivial assignability, as it will not change across releases.
    return;
  }
