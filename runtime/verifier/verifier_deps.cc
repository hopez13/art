...
static void SetUint32InUint8Array(std::vector<uint8_t>* out,
                                  ...
                                  uint32_t value) {
  DCHECK(IsAligned<sizeof(uint32_t)>>(out->data() + uint_offset));
  ...
  *cursor += (num_class_defs + 1) * sizeof(uint32_t);
  for (uint32_t i = 0; i < num_class_defs; ++i) {
    ...
    *cursor = start + offset;
    ...
    // reading the checks. Note that we must find an offset as the last entry in
    // the `offsets` array point to the end of the assignability types data.
    while (next_valid_offset_index <= i || offsets[next_valid_offset_index] == kNotVerifiedMarker) {
      next_valid_offset_index++;
    }
    const uint32_t* end = start + offsets[next_valid_offset_index];
    // Decode each check.
    while (*cursor < end) {
      T tuple;
