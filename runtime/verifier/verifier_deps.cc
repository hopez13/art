...
template<bool kSkip, typename T>
...
bool VerifierDeps::DecodeDexFileDeps(DexFileDeps& deps,
                                     ...
                                     size_t num_class_defs) {
  return
      DecodeStringVector</*kSkip=*/ kOnlyVerifiedClasses>(data_start, data_end, &deps.strings_) &&
      DecodeSet</*kSkip=*/ kOnlyVerifiedClasses>(data_start, data_end, &deps.assignable_types_) &&
      DecodeSet</*kSkip=*/ kOnlyVerifiedClasses>(data_start, data_end, &deps.unassignable_types_) &&
      DecodeSet</*kSkip=*/ kOnlyVerifiedClasses>(data_start, data_end, &deps.classes_) &&
      DecodeSet</*kSkip=*/ kOnlyVerifiedClasses>(data_start, data_end, &deps.fields_) &&
      DecodeSet</*kSkip=*/ kOnlyVerifiedClasses>(data_start, data_end, &deps.methods_) &&
      DecodeUint16SparseBitVector</*kSkip=*/ false>(data_start,
                                                    data_end,
                                                    num_class_defs,
                                                    /* sparse_value= */ false,
                                                    &deps.verified_classes_) &&
      DecodeUint16SparseBitVector</*kSkip=*/ kOnlyVerifiedClasses>(data_start,
                                                                   data_end,
                                                                    num_class_defs,
                                                                    /* sparse_value= */ true,
...
bool VerifierDeps::ParseVerifiedClasses(
  ...
  const uint8_t* data_end = data_start + data.size();
  for (const DexFile* dex_file : dex_files) {
    DexFileDeps deps(/*num_class_defs=*/ 0u);  // Do not initalize sparse bool vectors.
    size_t num_class_defs = dex_file->NumClassDefs();
