...
static inline uint32_t DecodeUint32WithCheck(const uint8_t** in, const uint8_t* end) {
  CHECK_LT(*in, end);
...
static inline void DecodeTuple(const uint8_t** in, const uint8_t* end, std::tuple<T1, T2>* t) {
  T1 v1 = static_cast<T1>(DecodeUint32WithCheck(in, end));
  T2 v2 = static_cast<T2>(DecodeUint32WithCheck(in, end));
...
static inline void DecodeTuple(const uint8_t** in, const uint8_t* end, std::tuple<T1, T2, T3>* t) {
  T1 v1 = static_cast<T1>(DecodeUint32WithCheck(in, end));
  T2 v2 = static_cast<T2>(DecodeUint32WithCheck(in, end));
  T3 v3 = static_cast<T2>(DecodeUint32WithCheck(in, end));
...
static inline void DecodeSet(const uint8_t** in, const uint8_t* end, std::set<T>* set) {
  DCHECK(set->empty());
  size_t num_entries = DecodeUint32WithCheck(in, end);
...
static inline void DecodeStringVector(const uint8_t** in,
  ...
  size_t num_strings = DecodeUint32WithCheck(in, end);
...
VerifierDeps::VerifierDeps(const std::vector<const DexFile*>& dex_files,
  ...
  const uint8_t* data_end = &(*data.end());
  for (auto& entry : dex_deps_) {
...
bool VerifierDeps::operator==(const VerifierDeps& rhs) const {
  MutexLock mu(Thread::Current(), *Locks::verifier_deps_lock_);
...
bool VerifierDeps::DexFileDeps::operator==(const VerifierDeps::DexFileDeps& rhs) const {
