...
static inline void EncodeSetVector(std::vector<uint8_t>* out,
  ...
  for (const std::set<T>& set : vector) {
    if (verified_classes[class_def_index]) {
      ...
      (reinterpret_cast<uint32_t*>(out->data() + offsets_index))[class_def_index] = out->size();
      for (const T& entry : set) {
        EncodeTuple(out, entry);
      }
    } else {
      (reinterpret_cast<uint32_t*>(out->data() + offsets_index))[class_def_index] =
          kNotVerifiedMarker;
    }
    class_def_index++;
  }
...
static inline bool DecodeSetVector(const uint8_t** cursor,
                                   const uint8_t* start,
                                   const uint8_t* end,
  ...
  *cursor += (num_class_defs + 1) * sizeof(uint32_t);
  for (uint32_t i = 0; i < num_class_defs; ++i) {
    ...
    *cursor = start + offset;
    ...
    // Decode each check.
    while (*cursor < start + offsets[next_valid_offset_index]) {
      T tuple;
