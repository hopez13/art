...
bool VerifierDeps::IsInClassPath(mirror::Class* klass) {
  ...
  if (dex_cache == nullptr) {
    // This is a synthesized class, i.e. an array, which we always consider not
    // defined in the compiled DEX files and therefore part of the classpath.
    // There are three kinds of arrays from the perspective of dependecy collection:
    //   (a) primitive type arrays, e.g. [I
    //   (b) classpath reference type arrays, e.g. [Ljava/lang/String;
    //   (c) compiled DEX reference type arrays, e.g. [LMyClass;
    //
    if (kIsDebugBuild) {
      std::string temp;
      DCHECK(klass->IsArrayClass()) << klass->GetDescriptor(&temp);
    }
    return true;
  }
...
void VerifierDeps::MaybeRecordMethodResolution(const DexFile& dex_file,
  ...
  VerifierDeps* singleton = GetVerifierDepsSingleton();
  if (singleton != nullptr) {
    singleton->AddMethodResolution(dex_file, method_idx, resolution_type, method);
  }
}
