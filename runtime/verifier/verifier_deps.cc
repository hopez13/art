...
bool VerifierDeps::VerifyClassRedefinition(Handle<mirror::ClassLoader> class_loader,
                                           const DexFile& dex_file,
                                           Thread* self,
                                           /* out */ std::string* error_msg) const {
  ...
  for (ClassAccessor accessor : dex_file.GetClasses()) {
    ...
    if (UNLIKELY(cls == nullptr)) {
      // Could not resolve class from the currently verified dex file. This should never happen.
      *error_msg = "Could not resolve class " + descriptor;
      return false;
    }
...
bool VerifierDeps::VerifyDexFile(Handle<mirror::ClassLoader> class_loader,
                                 ...
                                 Thread* self,
                                 /* out */ std::string* error_msg) const {
  return VerifyClassRedefinition(class_loader, dex_file, self, error_msg) &&
