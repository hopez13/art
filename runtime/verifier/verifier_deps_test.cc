...
class VerifierDepsTest : public CommonRuntimeTest {
 public:
  ...
  void LoadDexFile(ScopedObjectAccess* soa) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    deps_.reset(new VerifierDeps(dex_files));
  }

  void ProcessDependencies() REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  bool TestAssignabilityRecording(const std::string& dst,
    ...
    ScopedObjectAccess soa(Thread::Current());
    LoadDexFile(&soa);
    deps_->AddAssignability(FindClassByName(dst, &soa),
  ...
  bool HasDependency(const std::string& str) {
    return ContainsElement(deps_dump_, str) || ContainsElement(deps_dump_, str + " ");
  }

  bool HasAssignable(const std::string& destination, const std::string& source, bool assignable) {
    MutexLock mu(Thread::Current(), deps_->lock_);
    for (auto& dex_dep : deps_->dex_deps_) {
      auto& storage = assignable ? dex_dep.second.assignable_types_
                                 : dex_dep.second.unassignable_types_;
      for (auto& entry : storage) {
        std::string entry_dest = deps_->GetStringFromId(
            *dex_dep.first, &dex_dep.second, entry.GetDestination());
        std::string entry_src = deps_->GetStringFromId(
            *dex_dep.first, &dex_dep.second, entry.GetSource());
  ...
  bool HasField(const std::string& klass,
    ...
    for (auto& dex_dep : deps_->dex_deps_) {
      for (auto& entry : dex_dep.second.fields_) {
        ...
        std::string entry_decl_klass = deps_->GetStringFromId(
            *dex_dep.first, &dex_dep.second, entry.GetDeclaringClass());
  ...
  std::unique_ptr<VerifierDeps> deps_;
  std::set<std::string> deps_dump_;
};
...
TEST_F(VerifierDepsTest, StringToId) {
  ...
  uint32_t id_Main1 = deps_->GetIdFromString(*dex_file_, nullptr, "LMain;");
  ASSERT_LT(id_Main1, dex_file_->NumStringIds());
  ASSERT_EQ("LMain;", deps_->GetStringFromId(*dex_file_, nullptr, id_Main1));

  uint32_t id_Main2 = deps_->GetIdFromString(*dex_file_, nullptr, "LMain;");
  ASSERT_LT(id_Main2, dex_file_->NumStringIds());
  ASSERT_EQ("LMain;", deps_->GetStringFromId(*dex_file_, nullptr, id_Main2));

  uint32_t id_Lorem1 = deps_->GetIdFromString(*dex_file_, nullptr, "Lorem ipsum");
  ASSERT_GE(id_Lorem1, dex_file_->NumStringIds());
  ASSERT_EQ("Lorem ipsum", deps_->GetStringFromId(*dex_file_, nullptr, id_Lorem1));

  uint32_t id_Lorem2 = deps_->GetIdFromString(*dex_file_, nullptr, "Lorem ipsum");
  ASSERT_GE(id_Lorem2, dex_file_->NumStringIds());
  ASSERT_EQ("Lorem ipsum", deps_->GetStringFromId(*dex_file_, nullptr, id_Lorem2));
