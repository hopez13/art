  ...
  if (lhs.Equals(rhs)) {
    ...
    if (lhs.IsBoolean()) {
      ...
      if (rhs.IsZero()) {
        ...
        if (deps != nullptr) {
          deps->AddAssignability(lhs.GetClass(), rhs.GetClass(), strict, result);
        }
        return result;
      } else {
...
inline bool RegType::IsAssignableFrom(const RegType& src, VerifierDeps* deps) const {
  return AssignableFrom(*this, src, false, deps);
}

inline bool RegType::IsStrictlyAssignableFrom(const RegType& src, VerifierDeps* deps) const {
  return AssignableFrom(*this, src, true, deps);
}
