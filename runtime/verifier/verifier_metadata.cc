...
void DexVerifierMetadata::RecordVerificationResult(const DexFile::ClassDef& class_def,
                                                   bool success) {
  MutexLock mu(Thread::Current(), lock_);
  if (!success) {
    uint16_t class_def_idx = dex_file_.GetIndexForClassDef(class_def);
    if (!ContainsElement(failed_classes_, class_def_idx)) {
      failed_classes_.push_back(class_def_idx);
    }
  }
}

bool DexVerifierMetadata::IsClassVerified(const DexFile::ClassDef& class_def) {
  DCHECK(parent_->IsSuccessfullyLoadedFromFile());
  MutexLock mu(Thread::Current(), lock_);
  uint16_t class_def_idx = dex_file_.GetIndexForClassDef(class_def);
  return !ContainsElement(failed_classes_, class_def_idx);
}
...
void DexVerifierMetadata::Dump(std::ostream& os) {
  ...
  for (uint16_t entry : failed_classes_) {
    const char* descriptor = dex_file_.GetClassDescriptor(dex_file_.GetClassDef(entry));
    os << "failed verification of " << descriptor << std::endl;
  }
}
...
bool DexVerifierMetadata::WriteToFile(File* file) {
  ...
  CHECK_LE(failed_classes_.size(), std::numeric_limits<uint16_t>::max());
  WriteInt16(static_cast<uint16_t>(failed_classes_.size()), &buffer);
  for (uint16_t entry : failed_classes_) {
    WriteInt16(entry, &buffer);
  }
...
bool DexVerifierMetadata::ReadFromFile(File* file) {
  ...
  if (!file->ReadFully(&buffer_length, sizeof(uint32_t))) {
    LOG(ERROR) << "Could not read metadata length from VDEX. File: " << file->GetPath();
    return false;
  }

  uint32_t dex_file_checksum = 0u;
  if (!file->ReadFully(&dex_file_checksum, sizeof(uint32_t))) {
    LOG(ERROR) << "Could not read dex checksum from VDEX. File: " << file->GetPath();
    return false;
  }

  if (dex_file_.GetHeader().checksum_ != dex_file_checksum) {
    LOG(ERROR) << "Checksum does not match DEX file."
               << " Actual: " << dex_file_.GetHeader().checksum_
               << " Expected: " << dex_file_checksum
               << " File: " << file->GetPath();
    return false;
  }
  ...
  if (!file->ReadFully(buffer.data(), buffer_length)) {
    LOG(ERROR) << "Could not read metadata from VDEX. File: " << file->GetPath();
    return false;
  }
...

  {
    ...
    uint16_t num_failed_classes = ReadInt16(&cursor, end);
    failed_classes_.reserve(num_failed_classes);
    for (uint16_t i = 0; i < num_failed_classes; ++i) {
      failed_classes_.push_back(ReadInt16(&cursor, end));
    }
  }
