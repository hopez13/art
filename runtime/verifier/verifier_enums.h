 ...
 * VERIFY_ERROR_BAD_CLASS_HARD denotes a failures that causes
 * the entire class to be rejected. Other errors denote non-fatal
 * verification errors that may be thrown when executing the method.
 */
enum VerifyError : uint32_t {
  VERIFY_ERROR_BAD_CLASS_HARD        = 1 << 0,  // VerifyError.
  ...
  VERIFY_ERROR_LOCKING               = 1 << 9,  // Could not guarantee balanced locking. This should
