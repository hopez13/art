...
const RegType& RegType::Merge(const RegType& incoming_type, MethodVerifier* verifier) const {
  ...
  if (this == &undefined || &incoming_type == &undefined) {
    ...
    if (IsUninitializedTypes() || incoming_type.IsUninitializedTypes()) {
      ...
      // Record verification dependency that both `c1` and `c2` are assignable to `join_class`.
      VerifierDeps::MaybeRecordAssignability(
          verifier, join_class, c1, true /* strict */, true /* is_assignable */);
      VerifierDeps::MaybeRecordAssignability(
          verifier, join_class, c2, true /* strict */, true /* is_assignable */);
