...
const RegType& RegType::Merge(const RegType& incoming_type,
  ...
  if (this == &undefined || &incoming_type == &undefined) {
    ...
    if (IsUninitializedTypes() || incoming_type.IsUninitializedTypes()) {
      ...
      ObjPtr<mirror::Class> c1 = GetClass();
      ObjPtr<mirror::Class> c2 = incoming_type.GetClass();
      DCHECK(c1 != nullptr && !c1->IsPrimitive());
      DCHECK(c2 != nullptr && !c2->IsPrimitive());
      ObjPtr<mirror::Class> join_class = ClassJoin(c1, c2);
      ...
      // Refresh c1 and c2 as they can be invalidated by ClassJoin()'s array type resolution.
      c1 = GetClass();
      c2 = incoming_type.GetClass();
