...
class RegType {
 public:
  ...
  // comment on
  // ClassJoin.
  bool IsAssignableFrom(const RegType& src, VerifierDeps* deps) const
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  bool IsStrictlyAssignableFrom(const RegType& src, VerifierDeps* deps) const
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  const RegType& SafeMerge(const RegType& incoming_type,
    ...
    return Merge(incoming_type, reg_types, deps);
  }
