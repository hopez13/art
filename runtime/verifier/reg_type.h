...
class RegType {
 public:
  ...
  // comment on
  // ClassJoin.
  bool IsAssignableFrom(const RegType& src, DexVerifierMetadata* metadata) const
      SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  bool IsStrictlyAssignableFrom(const RegType& src, DexVerifierMetadata* metadata) const
      SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  const RegType& Merge(const RegType& incoming_type,
                       RegTypeCache* reg_types,
                       DexVerifierMetadata* metadata) const
      SHARED_REQUIRES(Locks::mutator_lock_);
