...
StackReflectiveHandleScope<kNumFields, kNumMethods>::StackReflectiveHandleScope(Thread* self) {
...
void StackReflectiveHandleScope<kNumFields, kNumMethods>::VisitTargets(
  ...
  DCHECK(GetThread() == Thread::Current() ||
         Locks::mutator_lock_->IsExclusiveHeld(Thread::Current()))
      << *GetThread() << " on thread " << *Thread::Current();
  auto visit_one = [&](auto& rv) {
    if (!rv.IsNull()) {
