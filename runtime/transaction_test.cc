...
TEST_F(TransactionTest, InstanceFieldsTest) {
  ...
  // Fail to modify fields inside transaction with strong CAS, then rollback changes.
  EnterTransactionMode();
  bool cas_success = h_instance->CasField32</*kTransactionActive=*/ true>(intField->GetOffset(),
                                                                          /*old_value=*/ 1,
                                                                          /*new_value=*/ 2,
                                                                          CASMode::kStrong,
  ...
  // Fail to modify fields inside transaction with weak CAS, then rollback changes.
  EnterTransactionMode();
  cas_success = h_instance->CasField32</*kTransactionActive=*/ true>(intField->GetOffset(),
                                                                     /*old_value=*/ 3,
                                                                     /*new_value=*/ 4,
                                                                     CASMode::kWeak,
                                                                     std::memory_order_seq_cst);
