...
// Utility class which holds the class loader context used during compilation.
class ClassLoaderCompilationContext {
 public:
  ...
  // The 'classpath_dir' argument specifies the directory to use for the relative class paths.
  // Returns true if all dex files where successfully opened.
  // It can only be called once. The second call will abort.
  //
  // Note that a "false" return could mean that either an apk/jar contained no dex files or
  // that we hit a genuine error.
  ...
  // The compilation sources are added at the end of the last class loader classpath.
 ...
 private:
  ...
  // recognized.
  ClassLoaderType ExtractClassLoaderType(const std::string& class_loader_spec) const;
  ...
  DISALLOW_COPY_AND_ASSIGN(ClassLoaderCompilationContext);
};
