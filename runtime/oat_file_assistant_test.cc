...
class OatFileAssistantTest : public DexoptTest {
 public:
  void VerifyOptimizationStatus(const std::string& file,
      ...
      std::vector<std::string> status = OatFileAssistant::GetOptimizationStatus(file, kRuntimeISA);
      ASSERT_EQ(2u, status.size());
      ASSERT_EQ(expected_filter, status[0]);
      ASSERT_EQ(expected_reason, status[1]);
  }
