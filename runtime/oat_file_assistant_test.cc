...
TEST_F(OatFileAssistantTest, SystemFrameworkDir) {
  ...
  EXPECT_EQ(dex_files[0]->GetOatDexFile(), nullptr);

  // Register the dex file to get a domain.
  {
    ScopedObjectAccess soa(Thread::Current());
    Runtime::Current()->GetClassLinker()->RegisterDexFile(
        *dex_files[0], soa.Decode<mirror::ClassLoader>(Runtime::Current()->GetSystemClassLoader()));
  }
  std::string stored_dex_location = dex_files[0]->GetLocation();
  EXPECT_EQ(dex_files[0]->GetHiddenapiDomain(), hiddenapi::Domain::kPlatform);
  {
    // Create the oat file.
    std::vector<std::string> args;
    args.push_back("--dex-file=" + dex_location);
  ...
  EXPECT_EQ(dex_files.size(), 1u);
  EXPECT_NE(oat_file, nullptr);
  EXPECT_NE(dex_files[0]->GetOatDexFile(), nullptr);
  EXPECT_NE(dex_files[0]->GetOatDexFile()->GetOatFile(), nullptr);
