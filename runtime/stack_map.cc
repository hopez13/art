...
ALWAYS_INLINE static bool DecodeTable(BitTable<Accessor>& table,
...
size_t CodeInfo::Deduper::Dedupe(const uint8_t* code_info) {
  ...
  ForEachHeaderField([this, &reader](auto) {
    writer_.WriteVarint(reader.ReadVarint());
  });
  ForEachBitTableField([this, &reader](auto member_pointer) {
    bool is_deduped = reader.ReadBit();
    DCHECK(!is_deduped);
    size_t bit_table_start = reader.NumberOfReadBits();
    typedef decltype(((static_cast<CodeInfo*>(nullptr))->*member_pointer)) BitTableReference;
    typename std::remove_reference<BitTableReference>::type bit_table(reader);
...
void CodeInfo::CollectSizeStats(const uint8_t* code_info, /*out*/ Stats* parent) {
  BitMemoryReader reader(code_info);
  ...
  ForEachBitTableField([codeinfo_stats, &reader, code_info](auto member_pointer) {
    typedef decltype(((static_cast<CodeInfo*>(nullptr))->*member_pointer)) BitTableReference;
    typename std::remove_reference<BitTableReference>::type table;
    size_t bit_offset = reader.NumberOfReadBits();
    bool deduped = DecodeTable(table, reader, code_info);
    if (deduped) {
    ...
    if (table.NumRows() != 0) {
      ...
      if (verbose) {
        ...
        for (size_t r = 0; r < table.NumRows(); r++) {
          ...
          for (size_t c = 0; c < table.NumColumns(); c++) {
            vios->Stream() << (c != 0 ? " " : "");
            if (std::is_same<decltype(table), StackMask>::value ||
                std::is_same<decltype(table), DexRegisterMask>::value) {
