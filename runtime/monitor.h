...
class Monitor {
 ...
 private:
  ...
  // Set locking_method_ and locking_dex_pc_ corresponding to monitor owner state.
  void SetLockingMethod() REQUIRES(monitor_lock_) REQUIRES_SHARED(Locks::mutator_lock_);

  // The same, but without checking for a proxy method.
  void SetLockingMethodNoProxy() REQUIRES(monitor_lock_) REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  // - If tracing is enabled and thus capture_method_eagerly_ is set, they are needed immediately
  //   when we first notice contention, so we set them unconditionally when a monitor is acquired.
  // - If tracing is disabled, but contention reporting is enabled, we use the
  //   lock_owner_request_ field to have the contending thread request them. The current
  //   owner then sets them when releasing the monitor, making them available when
  //   the contending thread acquires the monitor.
  // We make these unconditionally atomic; for contention logging all accesses are in fact
  // protected by the monitor, but for tracing, reads are not. Writes are always
  // protected by the monitor.

  // They are always accessed without ordering. We store a checksum, and reread if the checksum
  // doesn't correspond to the values.  This results in values that are correct with very high
  // probability, but not certainty.
  // If we need lock_owner information for a certain thread, we store its tid in
  // lock_owner_request_. To satisfy the request, we store lock_owner_tid_, lock_owner_method_,
  // and lock_owner_dex_pc_ and the corresponding checksum while holding the monitor.
