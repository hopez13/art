...
void Runtime::SetStatsEnabled(bool new_state) {
  // TODO: we probably need a separate function for this.
  Thread* self = Thread::Current();
...
void RuntimeStats::Clear(int flags, Thread* thread) {
  ...
  if ((flags & KIND_ALLOCATED_OBJECTS) != 0) {
    allocated_objects = 0;
    if (thread != nullptr) {
      allocated_objects -= thread->GetThreadLocalObjectsAllocated();
    }
  }
  ...
  void Run(Thread* thread) NO_THREAD_SAFETY_ANALYSIS {
    ...
    switch (kind_) {
      case KIND_ALLOCATED_OBJECTS:
        total = stats->allocated_objects + thread->GetThreadLocalObjectsAllocated();
        DCHECK_GE(total, 0U);
        break;
...
// TODO: Add GetStats() to capture all stats in one go.

uint64_t Runtime::GetStat(int kind) {
  // TODO: dcheck that only one kind is fetched.
  if (kind < (1<<16)) {
