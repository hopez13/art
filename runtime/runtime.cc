...
void Runtime::InitializeApexVersions() {
  std::vector<std::string> bcp_apexes;
  ...
  // When runnin on host or chroot, we just encode empty markers.
  if (!kIsTargetBuild || !OS::FileExists(kApexFileName)) {
    ...
    std::map<std::string, apex::ApexInfo> apex_infos;
    for (const apex::ApexInfo& info : info_list->getApexInfo()) {
      if (info.getIsActive()) {
        apex_infos.emplace(std::make_pair(info.getModuleName(), info));
      }
    }
    for (const std::string& str : bcp_apexes) {
