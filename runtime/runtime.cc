...
static void WaitUntilSingleThreaded() {
  ...
  // Read num_threads field from /proc/self/stats, avoiding higher-level IO libraries that may
  // break atomicity of the read.
  static constexpr int NUM_TRIES = 100;
  ...
  for (int tries = 0; tries < NUM_TRIES; ++tries) {
    ...
    int ret = TEMP_FAILURE_RETRY(close(stat_fd));
    CHECK(ret == 0) << strerror(errno);
    ssize_t pos = 0;
    while (pos < bytes_read && buf[pos++] != ')') {}
    if (pos >= bytes_read - 2) {
      LOG(WARNING) << "/proc/self/stat read returned short; pos = " << pos;
    ...
    if (pos >= bytes_read - 2) {
      LOG(WARNING) << "/proc/self/stat read returned short";
      continue;  // Short read. Retry.
    }
  ...
  CHECK(false) << "Failed to reach single-threaded state";
