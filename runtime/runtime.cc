...
static constexpr const char* kJvmtiPluginLibrary = "libopenjdkjvmti.so";
...
static bool EnsureJvmtiPlugin(Runtime* runtime,
  ...
  for (Plugin p : *plugins) {
    if (p.GetLibrary() == kJvmtiPluginLibrary) {
      return true;
    }
  }
  ...
  Plugin new_plugin = Plugin::Create(kJvmtiPluginLibrary);
...
void Runtime::AttachAgent(const std::string& agent_arg) {
  ...
  if (!EnsureJvmtiPlugin(this, &plugins_, &error_msg)) {
    LOG(ERROR) << "Could not load plugin: " << error_msg;
    ScopedObjectAccess soa(Thread::Current());
  ...
  if (result == ti::Agent::kNoError) {
    ...
    LOG(ERROR) << "Agent attach failed (result=" << result << ") : " << error_msg;
    ScopedObjectAccess soa(Thread::Current());
    ThrowIOException("%s", error_msg.c_str());
  }
}
