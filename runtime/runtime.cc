...
//
void Runtime::AttachAgent(const std::string& agent_arg, bool allow_non_debuggable_tooling) {
  std::string error_msg;
 ...
 public:
  ...
  bool operator()(ObjPtr<mirror::Class> klass) OVERRIDE REQUIRES(Locks::mutator_lock_) {
    ...
    for (auto& m : klass->GetMethods(pointer_size)) {
      ...
      if (Runtime::Current()->GetHeap()->IsInBootImageOatFile(code) && !m.IsNative()) {
        const void* new_code;
        if (m.IsProxyMethod()) {
          new_code = GetQuickProxyInvokeHandler();
        } else {
          new_code = GetQuickToInterpreterBridge();
        }
        instrumentation_->UpdateMethodsCodeForJavaDebuggable(&m, new_code);
      }
    }
    return true;
  }
