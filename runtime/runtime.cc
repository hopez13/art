...
bool Runtime::Start() {
  ...
  if (jit_options_->UseJitCompilation() || jit_options_->GetSaveProfilingInfo()) {
    ...
    // pthread_create and dlopen that could cause a deadlock. ptherad_create interceptor in ASAN
    // uses dl_pthread_iterator with a callback that could request a dl_load_lock via call to
    // __tls_addr. dl_pthread_iterate would already hold dl_load_lock so this could cause a
    // deadlock. __tls_addr needs a dl_load_lock only when there is a dlopen happening in parallel.
    // As a workaround we wait for the pthread_create (i.e JIT thread pool creation) to finish
    // before going to the next phase. Creating a system class loader could need a dlopen so we wait
    // here till threads are initialized.
