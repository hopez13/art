...
static jobject CreateSystemClassLoader(Runtime* runtime) {
  ...
  ObjPtr<mirror::Object> peer = soa.Self()->GetPeer();
  DCHECK(peer != nullptr);
  ObjPtr<mirror::Class> thread_class = peer->GetClass();
  DCHECK(thread_class->DescriptorEquals("Ljava/lang/Thread;"));
  ArtField* contextClassLoader =
      thread_class->FindDeclaredInstanceField("contextClassLoader", "Ljava/lang/ClassLoader;");
  CHECK(contextClassLoader != nullptr);

  // We can't run in a transaction yet.
  contextClassLoader->SetObject<false>(peer, system_class_loader);
