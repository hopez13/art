...
class Runtime::NotifyStartupCompletedTask : public gc::HeapTask {
 public:
  NotifyStartupCompletedTask() : gc::HeapTask(/*target_time=*/ NanoTime()) {}
  ...
  void Run(Thread* self) override {
    ...
    Runtime* const runtime = Runtime::Current();
    {
      ...
      // it. Use GC exclusion to prevent deadlocks that may happen if multiple threads are
      // attempting to run checkpoints at the same time.
      {
...
void Runtime::NotifyStartupCompleted() {
  ...
  // Use the heap task processor since we want to be exclusive with the GC and we don't want
