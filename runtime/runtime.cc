...
static std::string CreateEmptyMarkers(const std::vector<std::string_view>& bcp_apexes) {
  std::string result;
...
void Runtime::InitializeApexVersions() {
  ...
  // When running on host, chroot, or not having access to the file, we just encode empty markers.
  if (!kIsTargetBuild || !OS::FileExists(kApexFileName) || (access(kApexFileName, R_OK) != 0)) {
    result = CreateEmptyMarkers(bcp_apexes);
  } else {
#ifdef ART_TARGET_ANDROID
    auto info_list = apex::readApexInfoList(kApexFileName);
    if (!info_list.has_value()) {
      LOG(WARNING) << "Failed to read " << kApexFileName;
      result = CreateEmptyMarkers();
    } else {
