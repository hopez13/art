  ...
  if (!kIsTargetBuild || !OS::FileExists(kApexFileName)) {
    ...
    for (const std::string_view& str : bcp_apexes) {
      ...
      if (info == apex_infos.end() || info->second->getIsFactory()) {
        ...
        // prefer to use it as version scheme. If it is field we fallback to the
        // version code of the APEX.
        auto version = info->second->hasLastUpdateMillis() ? info->second->getLastUpdateMillis() : info->second->getVersionCode();
