...
void Runtime::InitNonZygoteOrPostFork(
  ...
  if (is_system_server) {
    ...
    // (simillar to the apps). Once that's does this manual registration can be removed.
...
void Runtime::NotifyDexFileLoaded() const {
  if (metrics_reporter_ != nullptr) {
    metrics_reporter_->NotifyDexFileLoaded(app_info_);
  }
}
