...
void OatFileAssistant::GetOptimizationStatus(
  ...
  if (oat_file == nullptr) {
    ...
    *out_compilation_reason = "unknown";
    // This mostly happen when we cannot open the oat file.
    // Note that it's different than kOatCannotOpen.
    // TODO: this error codes should be cleaned up out.
    *out_odex_status = "out-of-date-io-error-no-oat";
    return;
  }
  ...
  switch (status) {
    ...
    case kOatCannotOpen:  // This should never happen, but be robust.
      ...
      *out_compilation_reason = "error";
      // This mostly happen when we cannot open the vdex file,
      // or the file is corrupt.
      *out_odex_status = "out-of-date-io-error-corrupt";
      return;

    case kOatBootImageOutOfDate:
      *out_compilation_filter = "run-from-apk-fallback";
      *out_odex_status = "out-of-date-with-boot-image";
      return;

    case kOatDexOutOfDate:
      *out_compilation_filter = "run-from-apk-fallback";
      *out_odex_status = "out-of-date-with-apk";
      return;
  }
