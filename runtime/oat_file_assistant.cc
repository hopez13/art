...
bool OatFileAssistant::ClassLoaderContextIsOkay(const OatFile& oat_file) const {
  ...
  // Open dex files in the passed class loader context, in case it wasn't done
  // earlier. If the opening requires file descriptors, the caller of
  // OatFileAssistant should have made sure the dex files are opened prior to
  // using the OatFileAssistant.
  size_t dir_index = dex_location_.rfind('/');
  std::string classpath_dir = (dir_index != std::string::npos)
      ? dex_location_.substr(0, dir_index)
      : "";

  std::vector<int> context_fds;
  if (!context_->OpenDexFiles(classpath_dir, context_fds, /*only_read_checksums*/ true)) {
  ...
  if (matches == ClassLoaderContext::VerificationResult::kMismatch) {
    VLOG(oat) << "ClassLoaderContext check failed. Context was "
              << oat_file.GetClassLoaderContext()
              << ". The expected context is " << context_->EncodeContextForOatFile(classpath_dir);
    return false;
  }
  return true;
}
