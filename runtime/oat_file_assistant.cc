...
OatFileAssistant::OatStatus OatFileAssistant::GivenOatFileStatus(const OatFile& file) {
  ...
  DCHECK(required_dex_checksums_succeeded_);
...
const std::vector<uint32_t>* OatFileAssistant::GetRequiredDexChecksums(std::string* error_msg) {
  if (!required_dex_checksums_succeeded_) {
    ...
    required_dex_checksums_succeeded_ = true;
    if (cached_required_dex_checksums_.empty()) {
