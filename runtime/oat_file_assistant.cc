...
OatFileAssistant::MakeUpToDate(bool profile_changed,
  ...
  // That is actually not trivial in the current logic as it could interact with the collision
  // check:
  ...
  //     loaded in other processes). So it would boil down to how far do we want to complicate
  //     the logic in order to enable the use of oat files. Maybe its time to try simplify the
  //     logic?
