...
bool OatFileAssistant::DexChecksumUpToDate(const OatFile& file, std::string* error_msg) {
  ...
  const OatDexFile* oat_dex_file = file.GetOatDexFile(dex_location_.c_str(), nullptr);
  if (oat_dex_file == nullptr) {
    *error_msg = StringPrintf("failed to find %s in %s", dex_location_.c_str(), file.GetLocation().c_str());
    return false;
  }
  uint32_t actual_checksum = oat_dex_file->GetDexFileLocationChecksum();
  if (checksum.value() != actual_checksum) {
    VLOG(oat) << "Dex checksum does not match for dex: " << dex_location_
              << ". Expected: " << checksum.value() << ", Actual: " << actual_checksum;
    return false;
  }
...
bool OatFileAssistant::GetRequiredDexChecksum(std::optional<uint32_t>* checksum,
  ...
  if (!required_dex_checksums_attempted_) {
    ...
    std::string error2;
    if (dex_loader.GetMultiDexChecksum(&checksum2,
                                       &error2,
