...
int OatFileAssistant::GetDexOptNeeded(CompilerFilter::Filter target,
  ...
  DexOptNeeded dexopt_needed = info.GetDexOptNeeded(target, profile_changed, strict);
...
OatFileAssistant::MakeUpToDate(bool profile_changed, std::string* error_msg) {
  ...
  OatFileInfo& info = GetBestInfo();
  switch (info.GetDexOptNeeded(target, profile_changed, /*strict*/ false)) {
...
OatFileAssistant::DexOptNeeded OatFileAssistant::OatFileInfo::GetDexOptNeeded(
    CompilerFilter::Filter target, bool profile_changed, bool strict) {
  bool compilation_desired = CompilerFilter::IsAotCompilationEnabled(target);
  bool filter_okay = CompilerFilterIsOkay(target, profile_changed, strict);
...
bool OatFileAssistant::OatFileInfo::CompilerFilterIsOkay(
  ...
  if (strict) {
    return CompilerFilter::AreSame(current, target);
  }
