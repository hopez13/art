...
static jobjectArray Executable_getParameters0(JNIEnv* env, jobject javaMethod) {
  ...
  std::unique_ptr<annotations::ParametersMetadata> parameters_metadata =
      annotations::GetParametersMetadataForMethod(method);
  if (parameters_metadata.get() == nullptr) {
    return nullptr;
  }

  // Validate the MethodParameters system annotation.
  if (parameters_metadata.get()->names == nullptr
      || parameters_metadata.get()->access_flags == nullptr) {
  ...
  // Extract the name and access_flags arrays from MethodParameters.
  Handle<mirror::ObjectArray<mirror::String>> names =
      hs.NewHandle(parameters_metadata.get()->names);
  Handle<mirror::IntArray> access_flags = hs.NewHandle(parameters_metadata.get()->access_flags);
