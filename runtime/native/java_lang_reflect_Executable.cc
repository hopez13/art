...
static jobjectArray Executable_getParameters0(JNIEnv* env, jobject javaMethod) {
  ...
  annotations::ParametersMetadata parameters_metadata;
  if (!annotations::GetParametersMetadataForMethod(method, &parameters_metadata)) {
    return nullptr;
  }

  // Validate the MethodParameters system annotation.
  if (parameters_metadata.names == nullptr
      || parameters_metadata.access_flags == nullptr) {
  ...
  Handle<mirror::ObjectArray<mirror::String>> names =
      hs.NewHandle(parameters_metadata.names);
  Handle<mirror::IntArray> access_flags = hs.NewHandle(parameters_metadata.access_flags);
