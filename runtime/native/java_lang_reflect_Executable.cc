...
static jobjectArray Executable_getParameters0(JNIEnv* env, jobject javaMethod) {
  ...
  for (int32_t parameter_index = 0; parameter_index < names_count; parameter_index++) {
    ...
    uint32_t args[5] = { static_cast<uint32_t>(reinterpret_cast<uintptr_t>(parameter.Get())),
                         static_cast<uint32_t>(reinterpret_cast<uintptr_t>(name.Get())),
                         static_cast<uint32_t>(modifiers),
                         static_cast<uint32_t>(reinterpret_cast<uintptr_t>(executable.Get())),
                         static_cast<uint32_t>(parameter_index)
    };
    JValue result;
    parameter_init->Invoke(self, args, sizeof(args), &result, "VLILI");
