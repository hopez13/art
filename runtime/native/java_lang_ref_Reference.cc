...
static jboolean Reference_refersTo0(JNIEnv* env, jobject javaThis, jobject o) {
  ...
  // remaining case in which we should return true is when referent still points to from_space.
  // ConcurrentCopying::Copy ensures that whenever a pointer to a to_space object is published,
  // the forwarding pointer is also visible. Thus if other and javaThis refer to the same object,
  // and we can ensure that the read of the forwarding pointer is ordered after the read of other,
  // which ensured the forwarding pointer was set, then we're guaranteed to see the correct
  // forwarding pointer, which should then match other. This fence ensures that the forwarding
  // pointer read is ordered with respect to the access to other:
