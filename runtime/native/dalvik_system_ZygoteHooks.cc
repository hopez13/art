...
static void ZygoteHooks_nativePostForkSystemServer(JNIEnv* env ATTRIBUTE_UNUSED,
  ...
  if (Runtime::Current()->GetJit() != nullptr) {
    Runtime::Current()->GetJit()->GetCodeCache()->PostForkChildAction(/* is_system_server= */ true);
  }
}
...
static void ZygoteHooks_nativePostForkChild(JNIEnv* env,
  ...
  if (Runtime::Current()->GetJit() != nullptr) {
    if (!is_system_server) {
      // System server already called the JIT cache post fork action in `nativePstForkSystemServer`.
