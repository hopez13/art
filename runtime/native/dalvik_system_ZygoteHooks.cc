...
enum {
  ...
  DEBUG_ENABLE_IGNORE_APP_SIGNAL_HANDLER         = 1 << 17,
...
static uint32_t EnableDebugFeatures(uint32_t runtime_flags) {
  ...
  if ((runtime_flags & DEBUG_ENABLE_IGNORE_APP_SIGNAL_HANDLER) != 0) {
    runtime->SetSignalHookDebuggable(true);
    runtime_flags &= ~DEBUG_ENABLE_IGNORE_APP_SIGNAL_HANDLER;
  }
