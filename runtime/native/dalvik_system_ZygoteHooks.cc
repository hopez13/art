...
enum {
  ...
  DEBUG_GENERATE_HEAPDUMP_ON_OOME    = 1 << 18,
...
static uint32_t EnableDebugFeatures(uint32_t runtime_flags) {
  ...
  if ((runtime_flags & DEBUG_GENERATE_HEAPDUMP_ON_OOME) != 0) {
    runtime->SetHeapDumpOnSigusr2(true);
    runtime_flags &= ~DEBUG_GENERATE_HEAPDUMP_ON_OOME;
  }
