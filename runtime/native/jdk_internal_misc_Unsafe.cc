...
namespace {
  // Checks a JNI argument `size` fits inside a size_t, throws IllegalArgumentException if not.
  bool ValidJniSizeArgument(jlong size) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (size >= 0 && size == maybe_truncated_size) {
      return true;
    }
    ThrowIllegalArgumentException("Bad size");
    return false;
  }

  // Checks a JNI argument `offset` fits inside a size_t and is less than the size, throws
  ...
  bool ValidJniOffsetArgument(jlong offset, jlong size) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (size >= 0 && offset < size && offset == maybe_truncated_offset) {
      return true;
    }
    ThrowIllegalArgumentException("Bad offset");
    return false;
  }
}  // namespace
