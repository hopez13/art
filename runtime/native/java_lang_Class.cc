...
// Should be the same as dalvik.system.VMRuntime.HIDDEN_API_META_REFLECTION_WORKAROUND.
// Corresponds to a bug id.
static constexpr uint64_t kHiddenApiMetaReflectionWorkaround = 142365358;
  ...
  struct FirstExternalCallerVisitor : public StackVisitor {
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (declaring_class->IsBootStrapClassLoaded()) {
        ...
        // Check for classes in the java.lang.reflect package, except for java.lang.Proxy.
        // java.lang.Proxy does its own hidden api checks, and walking over this frame would
        // cause a null pointer dereference.
        ObjPtr<mirror::Class> proxy_class = GetClassRoot<mirror::Proxy>();
        if (declaring_class->IsInSamePackage(proxy_class) && declaring_class != proxy_class) {
          if (Runtime::Current()->isChangeEnabled(kHiddenApiMetaReflectionWorkaround)) {
            return true;
          }
        }
      }
