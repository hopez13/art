...
static jobject Constructor_newInstance0(JNIEnv* env, jobject javaMethod, jobjectArray javaArgs) {
  ...
  if (UNLIKELY(c->IsAbstract())) {
    soa.Self()->ThrowNewExceptionF("Ljava/lang/InstantiationException;", "Can't instantiate %s %s",
                                   ...
                                   mirror::Class::PrettyDescriptor(c.Get()).c_str());
    return nullptr;
  }
  ...
  if (!m->IsAccessible() && !c->IsPublic()) {
    ...
    if (caller != nullptr && !caller->CanAccess(c.Get())) {
      if (mirror::Class::PrettyDescriptor(c.Get()) == "dalvik.system.DexPathList$Element") {
        // b/20699073.
        ...
        soa.Self()->ThrowNewExceptionF(
            ...
            mirror::Class::PrettyClass(c.Get()).c_str(),
            mirror::Class::PrettyClass(caller).c_str());
        return nullptr;
      }
    }
  }
