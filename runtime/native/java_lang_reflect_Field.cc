...
ALWAYS_INLINE inline static void SetFieldValue(ObjPtr<mirror::Object> o,
  ...
  transaction::BootChangeMonitor(f->GetArtField());
  ...
  case Primitive::kPrimChar:
    ...
    break;
  case Primitive::kPrimInt:
    // The case that using reflection to remove Modifier.FINAL from accessFlag. The
    // BootChangeMonitor cannot track the modification.
