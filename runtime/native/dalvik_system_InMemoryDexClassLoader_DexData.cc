...
static std::unique_ptr<MemMap> AllocateAnonymousMemory(JNIEnv* env, jint start, jint end) {
  if (end <= start) {
  ...
  return std::unique_ptr<MemMap>(allocated_map);
}
...
static jlong InMemoryDexClassLoader_DexData_initializeWithDirectBuffer(
  ...
  std::unique_ptr<MemMap> allocated_map = AllocateAnonymousMemory(env, start, end);
  if (allocated_map == nullptr) {
    return 0;
  }
  ...
  return DexFileToCookie(CreateDexFile(env, allocated_map.release()));
}
...
static jlong InMemoryDexClassLoader_DexData_initializeWithArray(
  ...
  auto destination = reinterpret_cast<jbyte*>(allocated_map.get()->Begin());
  env->GetByteArrayRegion(buffer, start, end - start, destination);
  return DexFileToCookie(CreateDexFile(env, allocated_map.release()));
}

static jlong InMemoryDexClassLoader_DexData_uninitialize(JNIEnv* env, jclass, jlong cookie) {
  CHECK_NE(cookie, 0);
  ScopedObjectAccess soa(env);
  ...
  if (class_linker->FindDexCache(soa.Self(), *dex_file, true) == nullptr) {
    ...
    // runtime crashes if there are calls to InMemoryDexClassLoader_DexData.close()
...
static JNINativeMethod gMethods[] = {
  NATIVE_METHOD(InMemoryDexClassLoader_DexData, initializeWithDirectBuffer,
                "(Ljava/nio/ByteBuffer;II)J"),
  NATIVE_METHOD(InMemoryDexClassLoader_DexData, initializeWithArray, "([BII)J"),
  NATIVE_METHOD(InMemoryDexClassLoader_DexData, uninitialize, "(J)J"),
  NATIVE_METHOD(InMemoryDexClassLoader_DexData, findClass,
