...
static jobjectArray DexFile_getDexFileOptimizationStatus(JNIEnv* env,
  ...
  std::vector<std::string> status = OatFileAssistant::GetOptimizationStatus(
      filename.c_str(), target_instruction_set);

  DCHECK_EQ(2u, status.size());

  ScopedLocalRef<jstring> j_compilation_filter(env, env->NewStringUTF(status[0].c_str()));
  if (j_compilation_filter.get() == nullptr) {
    return nullptr;
  }
  ScopedLocalRef<jstring> j_compilation_reason(env, env->NewStringUTF(status[1].c_str()));
