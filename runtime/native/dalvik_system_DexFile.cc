...
static void ReleaseDexFiles(JNIEnv* env,
                            std::vector<std::unique_ptr<const DexFile>>& dex_files,
                            bool check_registered) {
  ScopedObjectAccess soa(env);
  ClassLinker* linker = Runtime::Current()->GetClassLinker();
  for (auto& dex_file : dex_files) {
    if (!check_registered || linker->IsDexFileRegistered(soa.Self(), *dex_file)) {
      dex_file.release();  // NOLINT
    }
  }
}

static void ThrowLastErrorMessageAsException(JNIEnv* env,
...
static jobject CreateSingleDexFileCookie(JNIEnv* env,
                                         MemMap&& data,
  ...
  if (!dex_files.empty()) {
    ...
    if (array == nullptr) {
      ReleaseDexFiles(env, dex_files, /* check_registered= */ true);
    }
    return array;
  } else {
    ThrowLastErrorMessageAsException(env, error_msgs);
    return nullptr;
  }
}
...
static jobject DexFile_createCookieWithDirectBuffer(JNIEnv* env,
  ...
  return CreateSingleDexFileCookie(env, std::move(dex_mem_map), class_loader, dex_elements);
}
...
static JNINativeMethod gMethods[] = {
  ...
  NATIVE_METHOD(DexFile, createCookieWithDirectBuffer,
                ...
                "[Ldalvik/system/DexPathList$Element;"
                ")Ljava/lang/Object;"),
  NATIVE_METHOD(DexFile, createCookieWithArray,
                "([B"
                "I"
                "I"
                "Ljava/lang/ClassLoader;"
