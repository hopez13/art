...
bool ArtMethod::IsAnnotatedWithFastNative() {
  return IsAnnotatedWithOptimization(WellKnownClasses::dalvik_annotation_optimization_FastNative);
}

bool ArtMethod::IsAnnotatedWithCriticalNative() {
  return IsAnnotatedWithOptimization(
      WellKnownClasses::dalvik_annotation_optimization_CriticalNative);
}

bool ArtMethod::IsAnnotatedWith(jclass klass, uint32_t visibility, bool allow_class_resolution) {
  Thread* self = Thread::Current();
  ...
  // Note: If `allow_class_resolution` is true, resolve any of the
  // method's annotations' classes as a side effect.
  ...
  return annotations::IsMethodAnnotationPresent(
      this, annotation_handle, visibility, allow_class_resolution);
}

bool ArtMethod::IsAnnotatedWithOptimization(jclass klass) {
  return IsAnnotatedWith(klass, DexFile::kDexVisibilityBuild, /* allow_class_resolution */ false);
}
