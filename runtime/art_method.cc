...
mirror::DexCache* ArtMethod::GetObsoleteDexCache() {
  DCHECK(!Runtime::Current()->IsAotCompiler()) << PrettyMethod()
      << ": We should never hit obsolete methods in the AOT compiler";
  DCHECK(IsObsolete());
  // TODO Uninterruptable assertion?
  // TODO Maybe want to do some sort of locking assertion?
  ScopedAssertNoThreadSuspension ns("Getting obsolete dex cache");
