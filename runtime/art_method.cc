...
ObjPtr<mirror::DexCache> ArtMethod::GetObsoleteDexCache() {
  ...
  DCHECK(len == 0 || len == ext->GetObsoleteDexCaches()->GetLength()) << "len=" << len << " ext->GetObsoleteDexCaches()=" << ext->GetObsoleteDexCaches();
  ...
  CHECK(GetDeclaringClass()->IsObsoleteObject())
      ...
      << GetDeclaringClass()->PrettyClass();;
  CHECK_EQ(this,
           std::clamp(this,
                      &(*GetDeclaringClass()->GetMethods(pointer_size).begin()),
                      &(*GetDeclaringClass()->GetMethods(pointer_size).end())))
      << "class is marked as structurally obsolete but not found in normal obsolete-map despite "
      << "not being the original method pointer for " << GetDeclaringClass()->PrettyClass();
