...
class IndirectReferenceTable {
 ...
 private:
  ...
  static uint32_t ExtractIndex(IndirectRef iref) {
    ...
    return (uref >> kKindBits >> kSerialBits);
  }
  ...
  IndirectRef ToIndirectRef(uint32_t table_index) const {
    ...
    DCHECK_LE(MinimumBitsToStore(table_index), 32 - kSerialBits - kKindBits);

    uint32_t serial_chunk = table_[table_index].GetSerial();
    DCHECK_LT(serial_chunk, 1u << kSerialBits);
    DCHECK_LT(static_cast<uint32_t>(kind_), 1u << kKindBits);

    uintptr_t uref =
        (table_index << kKindBits << kSerialBits) | (serial_chunk << kKindBits) | kind_;
