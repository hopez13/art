...
// time. Conversions to and from indirect references are performed on upcalls and downcalls,
// so they need to be very fast.
//
...
// (There are additional requirements for local references, see `LocalReferenceTable`.)
...
// is used to locate the actual reference storage. For global and weak global references, we
// need a (potentially) large table index and we also reserve some bits to be used to detect stale
// indirect references: we put a serial number in the extra bits, and keep a copy of the serial
// number in the table. This requires more memory and additional memory accesses on add/get, but
// is moving-GC safe. It will catch additional problems, e.g.: create iref1 for obj, delete iref1,
// create iref2 for same obj, lookup iref1. A pattern based on object bits will miss this.
//
// Local references use the same bits for the reference kind but the rest of their `IndirectRef`
// encoding is different, see `LocalReferenceTable`.
...
class IndirectReferenceTable {
 public:
  // Constructs an unitialized table. Use `Initialize()` to initialize it.
  explicit IndirectReferenceTable(IndirectRefKind kind);

  // Initialize the indirect reference table.
  //
  // Max_count total capacity (not resizable).
 ...
 private:
  static constexpr uint32_t kSerialMask = (1u << kIRTSerialBits) - 1;
  ...
  static constexpr uint32_t DecodeSerial(uintptr_t uref) {
    return static_cast<uint32_t>(uref >> kKindBits) & kSerialMask;
  }
  ...
  // Bottom of the table. Do not directly access the object references
