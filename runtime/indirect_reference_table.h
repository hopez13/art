...
//
// We need a (large) table index and a 2-bit reference type (global, local,
...
//
// We reserve some bits to be used to detect stale indirect references.
// We put a serial number in the extra bits, and keep a copy of the serial
// number in the table. This requires 2x the memory and additional
// memory accesses on add/get, but is moving-GC safe. It will catch additional
// problems, e.g.: create iref1 for obj, delete iref1, create iref2 for same obj,
...
// also removed.  Thus, deletion of an entry may reduce "top_index" by more
// than one.
//
...
//
// Holes are being locally cached for the segment. Otherwise we'd have to
// pass bottom index and number of holes, which restricts us to 16 bits.
// The value is cached with the table. To avoid code in generated JNI
// transitions, which implicitly form segments, the code for adding and
// removing references needs to detect the change of a segment. Helper
// fields are used for this detection.
//
...
class IndirectReferenceTable {
 ...
 private:
  // For simplicity, use three bits for serial, even if less were enough.
  static constexpr size_t kSerialBits = 3;
  static_assert(kIRTPrevCount < (2 << kSerialBits), "Not enough bits reserved");

  static constexpr size_t kKindBits = 2;
  static_assert(IndirectRefKind::kWeakGlobal < (2 << kKindBits), "Not enough bits reserved");

  // Extract the table index from an indirect reference.
  static uint32_t ExtractIndex(IndirectRef iref) {
    uintptr_t uref = reinterpret_cast<uintptr_t>(iref);
    return (uref >> kKindBits) & ((~0u) >> kKindBits >> kSerialBits);
  }
  ...
  IndirectRef ToIndirectRef(uint32_t table_index) const {
    ...
    DCHECK_LT(table_index, (2u << (32 - kSerialBits - kKindBits)));

    uint32_t serial_chunk = table_[table_index].GetSerial();
    uintptr_t uref = (serial_chunk << (32 - kSerialBits)) | (table_index << kKindBits) | kind_;
  ...
  // max #of entries allowed (module resizing).
  size_t max_entries_;
