...
// references.
//
// The table contains object references that may be part of the GC root set
// (weak global references are not roots). When an object is added we return
// an IndirectRef that is not a valid pointer but can be used to find the
// original value in O(1) time. Conversions to and from indirect references
// are performed on upcalls and downcalls, so they need to be very fast.
//
...
//
// We need a (potentially) large table index and a 2-bit reference type (global,
// local, weak global).  We also reserve some bits to be used to detect stale
// indirect references: we put a serial number in the extra bits, and keep a copy
// of the serial number in the table. This requires more memory and additional
