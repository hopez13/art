...
static inline bool IsPathOrDexClassLoader(ScopedObjectAccessAlreadyRunnable& soa,
...
static inline bool IsDelegateLastClassLoader(ScopedObjectAccessAlreadyRunnable& soa,
                                             Handle<mirror::ClassLoader> class_loader)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  mirror::Class* class_loader_class = class_loader->GetClass();
...
static inline RetType VisitClassLoaderDexElements(ScopedObjectAccessAlreadyRunnable& soa,
                                                  Handle<mirror::ClassLoader> class_loader,
                                                  Visitor fn,
...
// This function assumes that the given classloader is a subclass of BaseDexClassLoader!
template <typename Visitor, typename RetType>
static inline RetType VisitClassLoaderDexFiles(ScopedObjectAccessAlreadyRunnable& soa,
                                               Handle<mirror::ClassLoader> class_loader,
                                               Visitor fn,
                                               RetType defaultReturn)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  auto visit_dex_files = [&](Thread* self, ObjPtr<mirror::Object> element, RetType* ret)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (dex_file != nullptr) {
      ...
      for (int32_t j = kDexFileIndexStart; j < long_array_size; ++j) {
        ...
        RetType ret_value;
        if (fn(self, cp_dex_file, &ret_value)) {
          *ret = ret_value;
          return true;
        }
      }
...
static inline void VisitClassLoaderDexFiles(ScopedObjectAccessAlreadyRunnable& soa,
                                            Handle<mirror::ClassLoader> class_loader,
                                            Visitor fn)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  auto helper = [&](art::Thread* self, const art::DexFile* dex_file, void** ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    return fn(self, dex_file);
  };
  VisitClassLoaderDexFiles<decltype(helper), void*>(soa, class_loader, helper, nullptr);
}
