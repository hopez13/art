...
enum DeoptimizationKind : uint32_t {
  kAotInlineCache = 0,
  kJitInlineCache,
  kJitSameTarget,
  kLoopBCE,
...
inline const char* GetDeoptimizationKindName(size_t i) {
  switch (i) {
    ...
    case kLoopBCE: return "loop bounds check elimination";
    case kLoopBCENull: return "loop bounds check elimination on null";
    case kBlockBCE: return "block bounds check elimination";
    case kCHA: return "class hierarchy analysis";
    case kFullFrame: return "full frame";
    case kLast: break;
  }
  LOG(FATAL) << "Unexpected kind " << i;
  UNREACHABLE();
}
