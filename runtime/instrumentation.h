...
class Instrumentation {
 public:
  ...
  static constexpr MemberOffset HaveMethodEntryListenersOffset() {
    // Assert that have_method_entry_slow_listeners_ is 8bits wide. If the size changes
    // update the compare instructions in the code generator when generating checks for
    // MethodEntryExitHooks.
    static_assert(sizeof(have_method_entry_slow_listeners_) == 1,
                  "have_method_entry_slow_listeners_ isn't expected size");
    static_assert(sizeof(have_method_entry_fast_trace_listeners_) == 1,
                  "have_method_entry_fast_trace_listeners_ isn't expected size");
  ...
  static constexpr MemberOffset HaveMethodExitListenersOffset() {
    ...
    static_assert(sizeof(have_method_exit_slow_listeners_) == 1,
                  "have_method_exit_slow_listeners_ isn't expected size");
    static_assert(sizeof(have_method_exit_fast_trace_listeners_) == 1,
                  "have_method_exit_fast_trace_listeners_ isn't expected size");
  ...
  bool HasMethodEntryListeners() const REQUIRES_SHARED(Locks::mutator_lock_) {
    return have_method_entry_slow_listeners_ || have_method_entry_fast_trace_listeners_;
  }

  bool HasMethodExitListeners() const REQUIRES_SHARED(Locks::mutator_lock_) {
    return have_method_exit_slow_listeners_ || have_method_exit_fast_trace_listeners_;
  }
