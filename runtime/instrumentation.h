...
class Instrumentation {
 public:
  ...
  EXPORT void Deoptimize(ArtMethod* method) REQUIRES(Locks::mutator_lock_, !Locks::thread_list_lock_);
  ...
  EXPORT void Undeoptimize(ArtMethod* method) REQUIRES(Locks::mutator_lock_, !Locks::thread_list_lock_);
  ...
  EXPORT void EnableMethodTracing(const char* key,
  ...
  EXPORT void InstrumentThreadStack(Thread* thread, bool force_deopt) REQUIRES(Locks::mutator_lock_);
