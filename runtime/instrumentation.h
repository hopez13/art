...
class Instrumentation {
 public:
  ...
  // try_switch_runtime specifies if we can switch the runtime back to non-debuggable. When a
  // debugger is attached to a non-debuggable app, we switch the runtime to debuggable and when we
  // are detaching the debugger we move back to non-debuggable. If we are disabling deoptimization
  // for other reasons (ex: removing the last breakpoint) while the debugger is still connected, we
  // pass false to stay in debuggable. Switching runtimes is expensive so we only want to switch
  // when we know debug features aren't needed anymore.
  void DisableDeoptimization(const char* key, bool try_switch_runtime)
 ...
 private:
  ...
  void ConfigureStubs(const char* key,
      ...
      REQUIRES(Locks::mutator_lock_, Roles::uninterruptible_)
      REQUIRES(!Locks::thread_list_lock_, !Locks::classlinker_classes_lock_);
  void UpdateStubs(bool try_switch_runtime) REQUIRES(Locks::mutator_lock_, Roles::uninterruptible_)
      REQUIRES(!Locks::thread_list_lock_, !Locks::classlinker_classes_lock_);
