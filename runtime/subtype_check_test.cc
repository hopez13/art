...
struct MockClass {
  ...
  ///////////////////////////////////////////////////////////////
  // Implementation of the SubtypeCheckBase static interface.
  ///////////////////////////////////////////////////////////////
  ...
  MockClass* parent_;
  SubtypeCheckBitsAndStatus instance_of_and_status_;
  size_t x_;
  size_t y_;
...
std::ostream& operator<<(std::ostream& os, const MockClass& kls) {
  SubtypeCheckBits iod = kls.instance_of_and_status_.instance_of_;
...
struct MockSubtypeCheck : public SubtypeCheckBase<MockClass*, MockSubtypeCheck> {
  using BaseT = SubtypeCheckBase<MockClass*, MockSubtypeCheck>;
  MockSubtypeCheck(MockClass* klass) : BaseT(klass) {}

  static SubtypeCheckBitsAndStatus ReadField(MockClass* klass)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    return klass->instance_of_and_status_;
  }

  static void WriteField(MockClass* klass, const SubtypeCheckBitsAndStatus& new_ios)
  ...
  SubtypeCheckInfo::State GetState() const
      REQUIRES(Locks::subtype_check_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    return GetSubtypeCheckInfo().GetState();
  }
...
std::ostream& operator<<(std::ostream& os, const MockSubtypeCheck& tree) {
...
TEST_F(SubtypeCheckTest, LookupAllChildren) {
  ...
  using IOTree = MockSubtypeCheck;

  root_->Visit([&](MockClass* kls) {
    MockScopedLockSubtypeCheck lock_a;
    MockScopedLockMutator lock_b;

    EXPECT_EQ(SubtypeCheckInfo::kUninitialized, IOTree::Lookup(kls).GetState());
...
TEST_F(SubtypeCheckTest, EnsureInitializedSecondLevelWithPreassign) {
  ...
  for (size_t i = 0; i < root_->GetNumberOfChildren(); ++i) {
    ...
    EXPECT_EQ(SubtypeCheckInfo::kAssigned, child_tree.EnsureAssigned()) << *child << ", root:" << *root_;
    for (size_t j = 0; j < child->GetNumberOfChildren(); ++j) {
      ...
      EXPECT_EQ(SubtypeCheckInfo::kUnknownSubtypeOf, child_tree.IsSubtypeOf(child2_tree)) << child2_tree;
      EXPECT_EQ(SubtypeCheckInfo::kUnknownSubtypeOf, child2_tree.IsSubtypeOf(root)) << child2_tree;
      EXPECT_EQ(SubtypeCheckInfo::kUnknownSubtypeOf, child2_tree.IsSubtypeOf(child_tree)) << child2_tree;
...
void ApplyTransition(MockSubtypeCheck io_tree, SubtypeCheckInfo::State transition, SubtypeCheckInfo::State expected) {
  MockScopedLockSubtypeCheck lock_a;
  MockScopedLockMutator lock_b;

  EXPECT_EQ(SubtypeCheckInfo::kUninitialized, io_tree.GetState()) << io_tree.GetClass();
...
SubtypeCheckInfo::State ApplyTransition(MockSubtypeCheck io_tree, MockSubtypeOfTransition transition) {
  MockScopedLockSubtypeCheck lock_a;
  MockScopedLockMutator lock_b;
...
void EnsureStateChangedTestRecursiveGeneric(MockClass* klass, size_t cur_depth, size_t total_depth, T2 transition_func, T expect_checks) {
  ...
  SubtypeCheckInfo::State state = ApplyTransition(io_tree, requested_transition);
  UNUSED(state);
...
void EnsureStateChangedTestRecursive(MockClass* klass, size_t cur_depth, size_t total_depth, std::vector<std::pair<SubtypeCheckInfo::State, SubtypeCheckInfo::State>> transitions) {
  MockScopedLockSubtypeCheck lock_a;
  MockScopedLockMutator lock_b;
  using IOTree = MockSubtypeCheck;
...
void EnsureStateChangedTest(MockClass* root, size_t depth, std::vector<std::pair<SubtypeCheckInfo::State, SubtypeCheckInfo::State>> transitions) {
...
TEST_F(SubtypeCheckTest, EnsureInitialized_NoOverflow) {
  ...
  auto expected = [=](MockClass* kls, int expect_when, SubtypeCheckInfo::State actual_state, MockSubtypeOfTransition transition) {
  ...
  auto expected_uninitialize = [](MockClass* kls, int expect_when, SubtypeCheckInfo::State actual_state, MockSubtypeOfTransition transition) {
    UNUSED(kls);
    UNUSED(transition);
    if (expect_when >= kAfterTransition) {
      EXPECT_EQ(SubtypeCheckInfo::kUninitialized, actual_state);
