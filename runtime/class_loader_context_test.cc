...
class ClassLoaderContextTest : public Dex2oatEnvironmentTest {
 public:
...
TEST_F(ClassLoaderContextTest, OpenValidDexFilesSymLink) {
  ...
  std::string link = scratch_dir + "/link.jar";
  ASSERT_EQ(0, symlink(myclass_dex_name.c_str(), link.c_str()));

  std::unique_ptr<ClassLoaderContext> context =
      ClassLoaderContext::Create("PCL[" + link + "]");
