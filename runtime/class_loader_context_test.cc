  ...
  void VerifyOpenDexFiles(
    ...
    for (auto& it : *all_dex_files) {
      if (!DexFileLoader::IsMultiDexLocation(it->GetLocation().c_str())) {
        ...
        primary_checksums.push_back({});
      }
      CHECK(!primary_checksums.empty());
      DexFileLoader::AccMultiDexChecksum(it.get(), &primary_checksums.back());
    }
...
TEST_F(ClassLoaderContextTest, EncodeInOatFileIMC) {
  ...
  DexFileLoader::AccMultiDexChecksum(dex2[0].get(), &expected_checksum);
  ASSERT_TRUE(expected_checksum.has_value());
