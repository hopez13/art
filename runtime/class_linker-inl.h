...
inline mirror::String* ClassLinker::ResolveString(uint32_t string_idx, ArtMethod* referrer) {
  ...
  mirror::StringDexCachePair index_string = declaring_class->GetDexCacheStrings()
      [string_idx % mirror::DexCache::kDexCacheStringCacheSize].load(std::memory_order_relaxed);
  uint32_t index = index_string.string_index;
  mirror::String* string = index_string.getStringPointer();
