...
inline ObjPtr<mirror::Class> ClassLinker::ResolveType(dex::TypeIndex type_idx,
  ...
  // lookup as both from-space and to-space copies point to the same resolved types array.
...
inline ArtMethod* ClassLinker::GetResolvedMethod(uint32_t method_idx, ArtMethod* referrer) {
  ...
  // lookup as both from-space and to-space copies point to the same resolved methods array.
...
inline ArtField* ClassLinker::LookupResolvedField(uint32_t field_idx,
  ...
  // lookup as both from-space and to-space copies point to the same resolved fields array.
