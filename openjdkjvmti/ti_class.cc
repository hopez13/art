...
struct ClassCallback : public art::ClassLoadCallback {
  void ClassPreDefine(const char* descriptor,
                      ...
                      const art::dex::ClassDef& initial_class_def [[maybe_unused]],
  ...
  class RootUpdater : public art::RootVisitor {
   public:
    ...
    void VisitRoots(art::mirror::Object*** roots,
                    size_t count,
                    const art::RootInfo& info [[maybe_unused]]) override {
...
jvmtiError ClassUtil::GetClassMethods(jvmtiEnv* env,
  ...
  if (art::kIsDebugBuild) {
    size_t count = 0;
    for (auto& m [[maybe_unused]] : klass->GetDeclaredMethods(art::kRuntimePointerSize)) {
      count++;
    }
...
jvmtiError ClassUtil::GetClassStatus(jvmtiEnv* env [[maybe_unused]],
                                     jclass jklass,
                                     jint* status_ptr) {
  art::ScopedObjectAccess soa(art::Thread::Current());
  art::ObjPtr<art::mirror::Class> klass = soa.Decode<art::mirror::Class>(jklass);
...
jvmtiError ClassUtil::IsInterface(jvmtiEnv* env [[maybe_unused]],
                                  jclass jklass,
...
jvmtiError ClassUtil::IsArrayClass(jvmtiEnv* env [[maybe_unused]],
                                   jclass jklass,
...
jvmtiError ClassUtil::GetClassModifiers(jvmtiEnv* env [[maybe_unused]],
                                        jclass jklass,
                                        jint* modifiers_ptr) {
  art::ScopedObjectAccess soa(art::Thread::Current());
  art::ObjPtr<art::mirror::Class> klass = soa.Decode<art::mirror::Class>(jklass);
...
jvmtiError ClassUtil::GetClassLoader(jvmtiEnv* env [[maybe_unused]],
                                     jclass jklass,
...
jvmtiError ClassUtil::GetClassVersionNumbers(jvmtiEnv* env [[maybe_unused]],
                                             jclass jklass,
