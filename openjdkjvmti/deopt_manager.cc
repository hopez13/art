...
class ScopedDeoptimizationContext : public art::ValueObject {
 public:
  ScopedDeoptimizationContext(art::Thread* self, DeoptManager* deopt)
    ...
    // Stop the jit.
    if (jit_ != nullptr && jit_->GetThreadPool() != nullptr) {
      jit_->Stop();
    }
...
static void DisableSingleIntrinsic(const char* class_name,
  ...
  // suspend.
  // std::string no_suspend_reason;
  // if (art::kIsDebugBuild) {
  //   // Just give us a more useful error message if we do end up suspending somehow.
  //   std::stringstream ss;
  //   ss << "Disabling the " << class_name << "->" << method_name << signature << " intrinsic.";
  //   no_suspend_reason = ss.str();
  // }
  // art::ScopedAssertNoThreadSuspension sants(no_suspend_reason.c_str());
  art::Thread* self = art::Thread::Current();
