...
class ScopedDeoptimizationContext : public art::ValueObject {
 public:
  ScopedDeoptimizationContext(art::Thread* self, DeoptManager* deopt)
      ...
      : self_(self),
        ...
        invalidate_in_progress_jit_code_(false) {
  ...
  ~ScopedDeoptimizationContext()
    ...
    if (jit_ != nullptr) {
      jit_->Resume(invalidate_in_progress_jit_code_);
    }
  ...
  void SetIntrinsicsStateChanged(bool changed) {
    invalidate_in_progress_jit_code_ = changed;
  }
 ...
 private:
  ...
  art::jit::Jit* jit_;
  bool invalidate_in_progress_jit_code_;
};
...
void DeoptManager::PerformGlobalDeoptimization(art::Thread* self,
  ...
  bool changed = MaybeDisableIntrinsics(disable_intrinsics);
  sdc.SetIntrinsicsStateChanged(changed);
}
...
bool DeoptManager::MaybeDisableIntrinsics(bool do_disable) {
