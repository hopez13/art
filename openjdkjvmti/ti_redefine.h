 ...
 private:
  class ClassRedefinition {
   public:
    ...
    art::LengthPrefixedArray<art::ArtField>*
    AllocateNewStaticFields(art::Handle<art::mirror::Class> new_class,
    ...
    bool CheckInstances() REQUIRES_SHARED(art::Locks::mutator_lock_);
    ...
    void AddNewField(uint32_t idx);
    void AddFieldMapping(const art::DexFile* old_dex_file, uint32_t old_idx,  uint32_t new_idx);
   ...
   private:
    ...
    std::vector<const art::dex::FieldId*> added_fields_;
    std::unordered_map<const art::dex::FieldId*, const art::dex::FieldId*> updated_field_map_;
  };
