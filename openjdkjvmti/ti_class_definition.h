...
class ArtClassDefinition {
 public:
  ...
  void InitializeMemory() const REQUIRES_SHARED(art::Locks::mutator_lock_);
 ...
 private:
  ...
  // A ref to the current dex data. This is either dex_data_memory_, or current_dex_file_mmap_. This
  // is what the dex file will be turned into.
  ...
  // This is a dequickened version of what is loaded right now. It is either dex_data_mmap_ (if no
  // other redefinition has ever happened to this) or the current dex_file_ directly (if this class
  // has been redefined, thus it cannot have any quickened stuff).
