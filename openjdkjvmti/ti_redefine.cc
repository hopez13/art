  ...
  for (const art::ClassAccessor::Method& new_method : accessor.GetMethods()) {
    ...
    const auto old_iter =
        std::find_if(old_methods.cbegin(), old_methods.cend(), [&](const auto& current_old_method) {
          MethodNameAndSignature old_field_id(&old_dex_file, current_old_method.GetIndex());
...
Redefiner::ClassRedefinition::AllocateNewClassObject(art::Handle<art::mirror::DexCache> cache) {
  ...
  art::Handle<art::mirror::Class> new_class(hs.NewHandle(linker->AllocClass(
      driver_->self_, GetNewClassSize(/*with_embedded_vtables=*/false, old_class))));
  if (new_class.IsNull()) {
    driver_->self_->AssertPendingOOMException();
...
void VisitReflectiveObjects(art::Runtime* runtime,
                            FieldVis&& fv,
                            MethodVis&& mv) REQUIRES(art::Locks::mutator_lock_) {
  // Horray for captures!
  art::FunctionReflectiveValueVisitor frvv([&fv, &mv](auto* v, const auto& info) NO_THREAD_SAFETY_ANALYSIS {
  ...
  runtime->VisitReflectiveTargets(&frvv);
}
