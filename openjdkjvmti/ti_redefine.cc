...
jvmtiError Redefiner::GetClassRedefinitionError(art::Handle<art::mirror::Class> klass,
  ...
  if (kType == RedefinitionType::kStructural) {
    ...
    // TODO This might not be strict enough.
...
void Redefiner::ClassRedefinition::UpdateClassStructurally(const RedefinitionDataIter& holder) {
  ...
  // TODO We should try to do this in one pass with the ReplaceReferences maybe.
  ...
  driver_->runtime_->GetHeap()->VisitObjectsPaused(
    [&](art::mirror::Object* ref) NO_THREAD_SAFETY_ANALYSIS {
      ...
      // Constructor, Method and FIeld are all final so we don't need to check for anything other
      // than equality.
      if (art::GetClassRoot<art::mirror::Constructor>() == klass ||
        ...
        art::ObjPtr<art::mirror::MethodHandle> h(art::down_cast<art::mirror::MethodHandle*>(ref));
        // The handle might hold either an ArtField or an ArtMethod. Since these cannot have the
        // same value we might as well just check as one then the other.
        auto fit = field_map.find(h->GetTargetField());
        if (fit != field_map.end()) {
          LOG(INFO) << "Updating (field) j.l.i.MethodHandle object for "
                    << fit->second->PrettyField();
          h->SetTarget(reinterpret_cast<uintptr_t>(fit->second));
        }
        auto mit = method_map.find(h->GetTargetMethod());
        if (mit != method_map.end()) {
          LOG(INFO) << "Updating (method) j.l.i.MethodHandle object for "
                    << mit->second->PrettyMethod();
          h->SetTarget(reinterpret_cast<uintptr_t>(mit->second));
        }
      } else if (art::GetClassRoot<art::mirror::FieldVarHandle>()->IsAssignableFrom(klass)) {
        art::ObjPtr<art::mirror::FieldVarHandle> h(art::down_cast<art::mirror::FieldVarHandle*>(ref));
        auto it = field_map.find(h->GetField());
        if (it == field_map.end()) {
          return;
        }
        LOG(INFO) << "Updating j.l.i.FieldVarHandle object for " << it->second->PrettyField();
        h->SetField(it->second);
      }
    });
  // Instrument all threads to deoptimize them.
  ...
  // jit code.
  holder.GetNewClassObject()->GetExtData()->SetObsoleteClass(orig);
  orig->VisitMethods([&](art::ArtMethod* method) REQUIRES_SHARED(art::Locks::mutator_lock_) {
    ...
    // set them back.
    DCHECK(method->GetDeclaringClass() == orig ||
           method->GetDeclaringClass() == holder.GetNewClassObject());
  ...
  for (art::ArtField& f : orig->GetSFields()) {
    if (art::Primitive::Is64BitType(f.GetTypeAsPrimitiveType())) {
      f.Set64<false>(orig, 0);
    } else {
      f.Set32<false>(orig, 0);
    }
  }
  ...
  FuncVisitor fv([&](art::ClassVisitor& thiz, art::ObjPtr<art::mirror::Class> klass)
                      REQUIRES(art::Locks::mutator_lock_) {
    if (klass.IsNull()) {
      return true;
    }
    constexpr uint32_t kMainDexCacheId = -1;
    ...
    auto update_dex_cache = [&](art::ObjPtr<art::mirror::DexCache> dc,
                                uint32_t dex_cache_id) REQUIRES(art::Locks::mutator_lock_) {
      // Clear dex-cache. We don't need to do anything with resolved-types since those are already
      // handled by ReplaceReferences.
      if (dc.IsNull()) {
        // Since we get these from class-exts obsolete-dex-caches we will often have a bunch of
        // nulls for methods without any obsolete versions.
        return;
      }
      ...
      for (size_t i = 0; i < dc->NumResolvedFields(); i++) {
        ...
        if (new_val != field_map.end()) {
          LOG(DEBUG) << "Updating field dex-cache entry " << i << " of class "
                     << klass->PrettyClass() << " dex cache " << dex_cache_id;
    ...
    update_dex_cache(klass->GetDexCache(), kMainDexCacheId);
    ...
    if (!ext.IsNull()) {
      ...
      for (int32_t i = 0; !obsolete_caches.IsNull() && i < obsolete_caches->GetLength(); i++) {
        ...
        if (!cur.IsNull() && cur != prev) {
          prev = cur;
          LOG(DEBUG) << "Clearing obsolete dex cache " << i << " of " << klass->PrettyClass();
          update_dex_cache(cur, i);
        }
      }
      if (!ext->GetObsoleteClass().IsNull()) {
        LOG(DEBUG) << "Recuring on obsolete class " << ext->GetObsoleteClass()->PrettyClass();
