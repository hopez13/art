...
void Redefiner::ClassRedefinition::UpdateClassStructurally(const RedefinitionDataIter& holder) {
  ...
  // jitted might encode field/method offsets.
  FuncVisitor fv([&](art::ClassVisitor& thiz,
    ...
    // Code to actually clear a dex-cache. Since non-structural obsolete methods can lead to a
    // single class having several dex-caches associated with it we factor this out a bit.
    auto clear_dex_cache = [&](art::ObjPtr<art::mirror::DexCache> dc, uint32_t dex_cache_id)
          REQUIRES(art::Locks::mutator_lock_) {
    ...
    clear_dex_cache(klass->GetDexCache(), kMainDexCacheId);
