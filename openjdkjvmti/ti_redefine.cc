...
#undef UPDATE_FIELD
  ...
  for (art::ArtField& f : orig->GetIFields()) {
    ...
    if (ftype == art::Primitive::kPrimNot) {
      for (int32_t i = 0; i < old_instances->GetLength(); i++) {
        new_field->SetObject<false>(new_instances->Get(i), f.GetObject(old_instances->Get(i)));
      }
    } else {
      switch (ftype) {
#define UPDATE_FIELD(TYPE)                                                                    \
  case art::Primitive::kPrim##TYPE:                                                           \
    for (int32_t i = 0; i < old_instances->GetLength(); i++) {                                \
      new_field->Set##TYPE<false>(new_instances->Get(i), f.Get##TYPE(old_instances->Get(i))); \
    }                                                                                         \
    break
...
#undef UPDATE_FIELD
  ...
  for (int32_t i = 0; i < old_instances->GetLength(); i++) {
    ...
    HeapExtensions::ReplaceReference(driver_->self_, old_instances->Get(i), new_instances->Get(i));
  }
