...
void Redefiner::ClassRedefinition::ReverifyClass(const RedefinitionDataIter &cur_data) {
  ...
  VLOG(plugin) << "Reverifying " << class_sig_ << " due to previous soft failures";
  std::string error;
  // TODO Make verification log level lower
  art::verifier::FailureKind failure =
  ...
  for (art::ArtMethod& m : cur_data.GetMirrorClass()->GetMethods(art::kRuntimePointerSize)) {
    LOG(INFO) << m.PrettyMethod() << " force-interp " << m.IsCompilable() << " count: " << m.MustCountLocks();
  }
}
