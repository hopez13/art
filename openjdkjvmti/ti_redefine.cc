...
void UpdateReflectiveObjects(art::Thread* self,
                             art::gc::Heap* heap,
                             FieldVis&& fv,
  ...
  heap->VisitObjectsPaused([&](art::mirror::Object* ref) NO_THREAD_SAFETY_ANALYSIS {
...
void Redefiner::ClassRedefinition::UpdateClassStructurally(const RedefinitionDataIter& holder) {
  ...
  UpdateReflectiveObjects(
      ...
      [&](art::ArtField* f, const auto& info) REQUIRES(art::Locks::mutator_lock_) {
        ...
        LOG(INFO) << "Updating " << info << " object for (field) " << it->second->PrettyField();
        return it->second;
      },
      [&](art::ArtMethod* m, const auto& info) REQUIRES(art::Locks::mutator_lock_) {
        ...
        LOG(INFO) << "Updating " << info << " object for (method) " << it->second->PrettyMethod();
        return it->second;
      });
