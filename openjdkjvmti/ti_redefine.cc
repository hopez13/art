...
class EQHandle {
 public:
  template <class MirrorType>
  bool operator()(const art::Handle<MirrorType>& ptr1,
                  const art::Handle<MirrorType>& ptr2) const NO_THREAD_SAFETY_ANALYSIS {
      ...
      paused_ = true;
      {
        ...
        uint32_t in_progress_defines = 0;
        {
          ...
          runtime_->GetThreadList()->ForEach([&](art::Thread* t) {
            if (t != self_) {
              in_progress_defines += t->GetDefineClassCount();
            }
          });
