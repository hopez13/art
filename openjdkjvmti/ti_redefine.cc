...
bool Redefiner::ClassRedefinition::FinishNewClassAllocations(RedefinitionDataHolder &holder,
  ...
  auto setup_single_redefinition =
      [this](RedefinitionDataIter* data, art::Handle<art::mirror::Class> super_class)
          REQUIRES_SHARED(art::Locks::mutator_lock_) -> bool {
  ...
  if (!setup_single_redefinition(cur_data, hs.NewHandle(old_klass->GetSuperClass()))) {
    return false;
  }
    ...
    if (old_class.Get() == cur_data->GetMirrorClass()) {
      ...
      auto new_redef = std::find_if(
          *cur_data + 1, holder.end(), [&](auto it) REQUIRES_SHARED(art::Locks::mutator_lock_) {
            return it.GetMirrorClass() == old_class.Get();
          });
      if (new_redef == holder.end()) {
        // We aren't also redefining this subtclass. Just allocate a new class and continue.
        ...
        // Allocate the new Class using the new dex file.
        if (!setup_single_redefinition(&new_redef, superclass)) {
          return false;
        }
        new_classes_arr->Set(i, new_redef.GetNewClassObject());
      }
    }
  }
