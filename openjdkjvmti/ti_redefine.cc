...
jvmtiError Redefiner::RedefineClassesGeneric(jvmtiEnv* jenv,
  ...
  for (jint i = 0; i < class_count; i++) {
    jvmtiError res = Redefiner::GetClassRedefinitionError<RedefinitionType::kNormal>(definitions[i].klass, &error_msg);
    if (res != OK) {
...
jvmtiError Redefiner::RedefineClassesDirect(ArtJvmTiEnv* env,
  ...
  // this is not expected to be a common occurance we will just wrap the whole thing in a TOP-level
  // lock.
