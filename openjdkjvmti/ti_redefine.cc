...
void Redefiner::ClassRedefinition::UpdateClass(const RedefinitionDataIter& holder) {
  if (driver_->IsStructuralRedefinition() && (added_fields_ || added_methods_)) {
    ...
    // TODO Currently we just have the new method start from scratch WRT jitting. We might want to
    // copy hotness data from the old method.
    ...
    // by the linker. At the same time reset all methods to have interpreter entrypoints, anything
    // jitted might encode field/method offsets. Also nuke the jit.
