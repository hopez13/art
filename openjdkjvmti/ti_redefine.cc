...
class RedefinitionDataHolder {
 public:
  ...
  void SetOldClasses(jint klass_index, art::ObjPtr<art::mirror::ObjectArray<art::mirror::Class>> klasses)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    SetSlot(klass_index, kSlotOldClasses, klasses);
  }
  void SetNewClasses(jint klass_index, art::ObjPtr<art::mirror::ObjectArray<art::mirror::Class>> klasses)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
...
class IDHashObject {
 public:
  ...
  size_t operator()(const art::Handle<MirrorType>& ptr) const NO_THREAD_SAFETY_ANALYSIS {
    return static_cast<size_t>(ptr->IdentityHashCode());
  }
};
...
class EQHandle {
 public:
  template<class MirrorType>
  bool operator()(const art::Handle<MirrorType>& ptr1, const art::Handle<MirrorType>& ptr2) const NO_THREAD_SAFETY_ANALYSIS {
...
bool Redefiner::ClassRedefinition::CollectAndCreateNewInstances(
  ...
  std::vector<art::Handle<art::mirror::Class>> old_subtypes;
  std::unordered_map<art::Handle<art::mirror::Class>,
                     art::Handle<art::mirror::Class>,
                     IDHashObject,
                     EQHandle> type_map;
  type_map[old_klass] = hs.NewHandle(cur_data->GetNewClassObject());
  art::gc::Heap* heap = driver_->runtime_->GetHeap();
  auto is_subtype = [&](art::mirror::Object* obj) REQUIRES_SHARED(art::Locks::mutator_lock_) {
    return obj->IsClass() && old_klass->IsAssignableFrom(obj->AsClass());
  };
  ...
  heap->VisitObjects([&](art::mirror::Object* obj) REQUIRES_SHARED(art::Locks::mutator_lock_) {
    ...
    if (is_subtype(obj)) {
      old_subtypes.push_back(hs.NewHandle(obj->AsClass()));
    }
  });
  VLOG(plugin) << "Collected " << old_instances.size() << " instances to recreate!";
  VLOG(plugin) << "Found " << old_subtypes.size() << " types that are/are subtypes of "
  ...
  art::Handle<art::mirror::ObjectArray<art::mirror::Class>> old_classes_arr(
      hs.NewHandle(art::mirror::ObjectArray<art::mirror::Class>::Alloc(
          driver_->self_, cls_array_class.Get(), old_subtypes.size())));
  ...
  // Sort the old_subtypes topologically.
  {
    art::ScopedAssertNoThreadSuspension sants("Sort classes");
    std::unordered_map<art::ObjPtr<art::mirror::Class>, uint32_t, art::HashObjPtr> initial_index;
    for (uint32_t i = 0; i < old_subtypes.size(); ++i) {
      initial_index[old_subtypes[i].Get()] = i;
    }
    auto compare_handles = [&](auto l, auto r) REQUIRES_SHARED(art::Locks::mutator_lock_) {
      if (l->IsAssignableFrom(r.Get())) {
        return true;
      } else if (r->IsAssignableFrom(l.Get())) {
        return false;
      } else {
        return initial_index[l.Get()] < initial_index[r.Get()];
      }
    };
    std::sort(old_subtypes.begin(), old_subtypes.end(), compare_handles);
  }
  for (uint32_t i = 0; i < old_subtypes.size(); ++i) {
    old_classes_arr->Set(i, old_subtypes[i].Get());
  }
  ...
  // TODO Fill this in.
  for (size_t i = 0; i < old_subtypes.size(); i++) {
    art::Handle<art::mirror::Class>& old_class = old_subtypes[i];
    if (old_class.Get() == cur_data->GetMirrorClass()) {
      ...
      auto old_super = std::find_if(old_subtypes.begin(),
                                    old_subtypes.begin() + i,
                                    [&](art::Handle<art::mirror::Class> v)
                                        REQUIRES_SHARED(art::Locks::mutator_lock_) {
                                          return v.Get() == old_class->GetSuperClass();
                                        });
      // Only the GetMirrorClass should not be in this list.
      CHECK(old_super != old_subtypes.begin() + i)
          << "from first " << i << " could not find super of " << old_class->PrettyClass()
          << " expected to find " << old_class->GetSuperClass()->PrettyClass();
      superclass.Assign(new_classes_arr->Get(std::distance(old_subtypes.begin(), old_super)));
      art::StackHandleScope<1> dchs(driver_->self_);
      new_classes_arr->Set(i,
                           AllocateNewClassObject(old_class,
                                                  superclass,
                                                  dchs.NewHandle(old_class->GetDexCache()),
                                                  old_class->GetDexClassDefIndex()));
    }
  }
...
uint32_t Redefiner::ClassRedefinition::GetNewClassSize(art::ClassAccessor& accessor) {
  ...
  return art::mirror::Class::ComputeClassSize(/*with_embedded_tables=*/ false,
...
class ScopedSuspendClassLoading {
 public:
  ScopedSuspendClassLoading(art::Runtime* runtime ATTRIBUTE_UNUSED,
                            RedefinitionDataHolder& h ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    LOG(WARNING) << "👩‍🏫  Class is in session and are still being loaded. I'll deal with this "
                    "later.";
  }
...
jvmtiError Redefiner::Run() {
  ...
  ScopedSuspendClassLoading suspend_class_load(runtime_, holder);
...
void Redefiner::ClassRedefinition::CollectNewFieldAndMethodMappings(
  ...
  for (int32_t i = 0; i < data.GetNewClasses()->GetLength(); i++) {
    art::ObjPtr<art::mirror::Class> old_cls(data.GetOldClasses()->Get(i));
    art::ObjPtr<art::mirror::Class> new_cls(data.GetNewClasses()->Get(i));
...
void Redefiner::ClassRedefinition::UpdateClassStructurally(const RedefinitionDataIter& holder) {
  ...
  for (int32_t i = 0; i < old_classes->GetLength(); i++) {
    art::ObjPtr<art::mirror::Class> new_class(new_classes->Get(i));
    art::ObjPtr<art::mirror::Class> old_class(old_classes->Get(i));
  ...
  for (int32_t i = 0; i < old_classes->GetLength(); i++) {
    ...
    for (art::ArtMethod& m : old_class->GetMethods(art::kRuntimePointerSize)) {
      m.SetIsObsolete();
      m.SetDontCompile();
      DCHECK_EQ(old_class, m.GetDeclaringClass());
    }
  }
  ...
  auto could_change_resolution_of = [&](auto* field_or_method,
    ...
    // if (orig->IsAssignableFrom(field_or_method->GetDeclaringClass())) {
    //   return true;
    // }
    ...
    if constexpr (is_method) {
      for (int32_t i = 0; i < old_classes->GetLength(); i++) {
        ...
        if (std::find_if(direct_methods.begin(),
                         direct_methods.end(),
      ...
      for (int32_t i = 0; i < old_classes->GetLength(); i++) {
        if (field_or_method->IsStatic()) {
          ...
          if (std::find_if(sfields.begin(), sfields.end(), pred) != sfields.end()) {
            return true;
          }
        } else {
          auto ifields = replacement->GetIFields();
          if (std::find_if(ifields.begin(), ifields.end(), pred) != ifields.end()) {
            return true;
          }
        }
      }
  ...
  std::unordered_map<art::ObjPtr<art::mirror::Object>,
                     ...
                     art::HashObjPtr> map;
  for (int32_t i = 0; i < old_classes->GetLength(); i++) {
    map.emplace(old_classes->Get(i), new_classes->Get(i));
  }
  for (int32_t i = 0; i < old_instances->GetLength(); i++) {
    map.emplace(old_instances->Get(i), new_instances->Get(i));
    CHECK(new_instances->Get(i)->GetClass() == map[old_instances->Get(i)->GetClass()]->AsClass())
        << new_instances->Get(i)->GetClass()->PrettyClass() << " vs "
        << map[old_instances->Get(i)->GetClass()]->AsClass()->PrettyClass();
  }
  ...
  for (int32_t i = 0; i < old_classes->GetLength(); i++) {
    new_classes->Get(i)->GetExtData()->SetObsoleteClass(old_classes->Get(i));
    art::ObjPtr<art::mirror::Class> klass(new_classes->Get(i));
    LOG(INFO) << "VTable of " << klass->PrettyClass();
    for (int32_t j = 0; j < klass->GetVTableLength(); j++) {
      LOG(INFO) << "\t" << j << " -> " << klass->GetVTableEntry(j, art::kRuntimePointerSize)->PrettyMethod();
    }
  }
