 ...
 public:
  ScopedSuspendAllocations(art::Runtime* runtime, RedefinitionDataHolder& h)
    ...
    if (std::any_of(h.begin(),
      ...
      LOG(WARNING) << "🤣 YOLO, pausing allocations.";
      paused_ = true;
...
void Redefiner::ClassRedefinition::UpdateClassStructurally(const RedefinitionDataIter& holder) {
  ...
  // Copy every non-java.lang.Object field from every old-instance to the new
  for (int32_t i = 0; i < old_instances->GetLength(); i++) {
    ...
    for (art::ObjPtr<art::mirror::Class> klass(old_instance->GetClass()),
                                         new_klass(new_instance->GetClass());
         !klass->IsObjectClass();
         klass = klass->GetSuperClass(), new_klass = new_klass->GetSuperClass()) {
      for (art::ArtField& f : klass->GetIFields()) {
        art::ArtField* new_field =
            new_klass->FindDeclaredInstanceField(f.GetName(), f.GetTypeDescriptor());
        CHECK(new_field != nullptr) << "could not find new version of " << f.PrettyField();
        CopyField(new_instance, new_field, old_instance, f);
      }
    }
  }
    ...
    if constexpr (is_method) {
      ...
      return std::find_if(direct_methods.begin(),
                          ...
                          [&](art::ArtMethod& m) REQUIRES(art::Locks::mutator_lock_) {
                            return UNLIKELY(m.GetNameView() == field_or_method->GetNameView() &&
                                            m.GetSignature() == field_or_method->GetSignature());
                          }) != direct_methods.end();
    } else {
  ...
  for (auto o : old_instances_saved) {
    for (art::ObjPtr<art::mirror::Class> klass(o->GetClass()); !klass->IsObjectClass(); klass = klass->GetSuperClass()) {
      for (art::ArtField& f : klass->GetIFields()) {
        ClearField(o, f);
      }
    }
