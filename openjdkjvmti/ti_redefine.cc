...
void Redefiner::ClassRedefinition::UpdateClass(
  ...
  if (driver_->IsStructuralRedefinition() && !added_fields_.empty()) {
    ...
    HeapExtensions::ReplaceReference(driver_->self_, holder.GetMirrorClass(), holder.GetNewClassObject());
    // Update dex-caches to point to new fields. We wait until here so that the new-class is known by the linker
    FuncVisitor fv([&](art::ObjPtr<art::mirror::Class> klass) REQUIRES(art::Locks::mutator_lock_) {
          ...
          for (size_t i = 0; i < dc->NumResolvedFields(); i++) {
            ...
            if (new_val != field_map.end()) {
              LOG(INFO) << "Updating field dex-cache entry " << i << " of class " << klass->PrettyClass();
          ...
          for (size_t i = 0; i < dc->NumResolvedMethods(); i++) {
            ...
            if (new_val != method_map.end()) {
              LOG(INFO) << "Updating meth dex-cache entry " << i << " of class " << klass->PrettyClass();
              pair.object = new_val->second;
