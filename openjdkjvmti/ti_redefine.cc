...
void Redefiner::ClassRedefinition::FindAndAllocateObsoleteMethods(
  ...
  if (driver_->IsStructuralRedefinition() && (added_fields_ || added_methods_)) {
    // We don't need to do anything since the old methods retain their pointers.
    return;
  }
...
bool Redefiner::ClassRedefinition::CheckMethods() {
  ...
  bool is_structural = driver_->IsStructuralRedefinition();
  ...
  std::vector<bool> seen_old_methods(
      (art::kIsDebugBuild || is_structural) ? old_accessor.NumMethods() : 0, false);
  ...
  for (const art::ClassAccessor::Method& new_method : accessor.GetMethods()) {
    ...
    const auto old_iter =
        std::find_if(old_methods.cbegin(), old_methods.cend(), [&](const auto& old_iter) {
          MethodNameAndSignature old_field_id(&old_dex_file, old_iter.GetIndex());
          return old_field_id == new_method_id;
        });

    if (old_iter == old_methods.cend()) {
      // TODO Support adding non-static methods.
      if (is_structural && new_method.IsStaticOrDirect()) {
        LOG(INFO) << "New method found " << new_method_id;
        AddNewMethod();
      } else {
        RecordFailure(
            ERR(UNSUPPORTED_REDEFINITION_METHOD_ADDED),
            StringPrintf("Unknown method %s was added!", new_method_id.ToString().c_str()));
        return false;
      }
      ...
      return false;
    } else if (art::kIsDebugBuild || is_structural) {
  ...
  if ((art::kIsDebugBuild || is_structural) &&
...
Redefiner::ClassRedefinition::AllocateNewClassObject(art::Handle<art::mirror::DexCache> cache) {
  ...
  // TODO Maybe set-interpreter to all the methods here?
...
void Redefiner::ClassRedefinition::UpdateClass(
  ...
  if (driver_->IsStructuralRedefinition() && (added_fields_ || added_methods_)) {
    ...
    for (art::ArtMethod& m : orig->GetMethods(art::kRuntimePointerSize)) {
      // TODO maybe pass something down to ReplaceReference to tell it not to update ArtMethod
      // pointers.
      // If any of the methods were obsolete they would have been visited by the stack-walk in
      // ReplaceReferences and had their declaring classes updated. We don't want that so we need to
      // set them back.
      m.SetDeclaringClass(orig);
    }
    ...
    FuncVisitor fv([&](art::ObjPtr<art::mirror::Class> klass) REQUIRES(art::Locks::mutator_lock_) {
      ...
      for (size_t i = 0; i < dc->NumResolvedMethods(); i++) {
        ...
        if (new_val != method_map.end()) {
          LOG(DEBUG) << "Updating method dex-cache entry " << i << " of class "
                     << klass->PrettyClass();
          pair.object = new_val->second;
