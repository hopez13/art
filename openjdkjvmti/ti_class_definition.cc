...
void ArtClassDefinition::InitializeMemory() const {
  ...
  art::MemMap* dest = dex_data_mmap_.release();
  std::string error;
  CHECK(art::MemMap::ReplaceMapping(&dest, &source, &error)) << "Failed to replace mmap for "
                                                             << name_ << " because " << error;
  dex_data_mmap_.reset(dest);
...
void ArtClassDefinition::InitWithDex(GetOriginalDexFile get_original,
  ...
  temp_mmap_.reset(art::MemMap::MapAnonymous(mmap_name.c_str(),
                                             ...
                                             /*reuse*/ false,
                                             &error));
  if (UNLIKELY(!art::MemMap::kCanReplaceMapping ||
