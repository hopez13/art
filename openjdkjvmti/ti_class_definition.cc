...
static jvmtiError GetDexDataForRetransformation(art::Handle<art::mirror::Class> klass,
  ...
  if (!ext.IsNull()) {
    ...
    if (!orig_dex.IsNull()) {
      if (orig_dex->IsArrayInstance()) {
        ...
        dex_data_memory->resize(orig_dex_bytes->GetLength());
        memcpy(dex_data_memory->data(), orig_dex_bytes->GetData(), dex_data->size());
        *dex_data = art::ArrayRef<const unsigned char>(dex_data_memory->data(), dex_data_memory->size());
        return OK;
  ...
  std::string storage;
  return DequickenDexFile(dex_file, klass->GetDescriptor(&storage), dex_data_memory, dex_data);
}
...
jvmtiError ArtClassDefinition::InitWithDex(GetOriginalDexFile get_original ATTRIBUTE_UNUSED,
  ...
  // manually.
  if (klass_ == nullptr) {
    DequickenDexFile(quick_dex, nullptr, &dex_data_memory_, &dex_data_);
  } else {
    art::StackHandleScope<1> hs(self);
    art::Handle<art::mirror::Class> m_klass(hs.NewHandle(self->DecodeJObject(klass_)->AsClass()));
    GetDexDataForRetransformation(m_klass, &dex_data_memory_, &dex_data_);
  }
  if (from_class_ext_) {
    // We got initial from class_ext so the current one must have undergone redefinition so no
    // cdex or quickening stuff.
...
jvmtiError ArtClassDefinition::Init(art::Thread* self, jclass klass) {
  ...
  auto get_original = [&](/*out*/std::vector<unsigned char>* dex_data ATTRIBUTE_UNUSED)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    return OK;
  };
