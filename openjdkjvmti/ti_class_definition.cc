...
void ArtClassDefinition::InitializeMemory() const {
  LOG(INFO) << "Initializing de-quickened memory for dex file of " << name_;
  CHECK(dex_data_mmap_ != nullptr);
  CHECK_EQ(dex_data_mmap_->GetProtect(), PROT_NONE);

  CHECK(dex_data_mmap_->Protect(PROT_READ | PROT_WRITE));
  ...
  CHECK_LE(fixed_dex_file->Size(), dex_data_mmap_->Size());
  memcpy(dex_data_mmap_->Begin(), fixed_dex_file->Begin(), fixed_dex_file->Size());
...
bool ArtClassDefinition::IsModified() const {
  ...
  if (dex_data_mmap_ != nullptr && dex_data_mmap_->GetProtect() == PROT_NONE) {
    if (current_dex_file_.data() == dex_data_mmap_->Begin()) {
      ...
      LOG(INFO) << "Lazy dex file for " << name_ << " was never touched but the dex_data_ is "
                << "changed!";
      InitializeMemory();
    }
  }
...
static bool DexNeedsDequickening(art::Handle<art::mirror::Class> klass,
  ...
  if (orig_dex.IsNull()) {
    ...
    *from_class_ext = true;
    ...
    // We got this from a RedefineClasses so it isn't quickened.
    DCHECK(orig_dex->GetClass()->GetComponentType()->IsPrimitiveByte());
    *from_class_ext = true;
    return false;
  }
...
void ArtClassDefinition::InitWithDex(GetOriginalDexFile get_original,
  ...
  dex_data_mmap_.reset(art::MemMap::MapAnonymous(mmap_name.c_str(),
                                                 ...
                                                 &error,
                                                 /*use_ashmem*/ false));
  if (UNLIKELY(dex_data_mmap_ == nullptr)) {
    // Failed to mmap a large enough area. This is unfortunate. Since currently the size is just a
    // guess though we might as well try to do it manually.
    ...
    dex_data_ = art::ArrayRef<const unsigned char>(dex_data_mmap_->Begin(), dex_data_mmap_->Size());
    if (from_class_ext_) {
      // We got initial from class_ext so the current one must have undergone redefinition so no
      // cdex or quickening stuff.
      // We can only do this if it's not a first load.
      ...
      current_dex_file_ = art::ArrayRef<const unsigned char>(dex_data_mmap_->Begin(), dequick_size);
    }
  }
}
