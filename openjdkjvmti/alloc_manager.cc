...
void JvmtiAllocationListener::PreOjectAllocated(art::Thread* self,
                                                art::MutableHandle<art::mirror::Class> type,
                                                size_t* byte_count) {
  manager_->PauseForAllocation(self);
...
AllocationManager::AllocationManager()
  ...
  // alloc_listener_mutex_.SetShouldRespondToEmptyCheckpointRequest(true);
}
...
void AllocationManager::PauseForAllocation(art::Thread* self) {
  ...
  while (true) {
    ...
    if (pausing_thread != nullptr) {
      VLOG(plugin) << "suspending " << *self << " due to allocation pause initiated by "
                   << *pausing_thread;
