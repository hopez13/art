...
class EventHandler {
 public:
  ...
  jvmtiError SetEvent(ArtJvmTiEnv* env,
                      art::Thread* thread,
                      ArtJvmtiEvent event,
                      jvmtiEventMode mode) REQUIRES(!envs_lock_);
  ...
  inline void HandleChangedCapabilities(ArtJvmTiEnv* env,
                                        ...
                                        bool added) REQUIRES(!envs_lock_);
 ...
 private:
  ...
  mutable art::Mutex envs_lock_ ACQUIRED_AFTER(art::Locks::instrument_entrypoints_lock_);
