...
class JvmtiMonitorListener : public art::MonitorCallback {
 public:
  ...
  // TODO This does not fully match the RI semantics. Specifically we will not send the
  // kMonitorWaited event in one situation where the RI would, there was an exception in the
  // kMonitorWait event but otherwise the call was fine. In that case the RI would immediately send
  // this event and wakeup. Unfortunately the organization of our monitor code makes matching this
  // behavior very complicated so we don't try. See b/65558434
