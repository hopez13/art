...
class JvmtiMonitorListener : public art::MonitorCallback {
 public:
  ...
  void ObjectWaitEnd(art::Handle<art::mirror::Object> obj, bool timeout, bool waited)
      OVERRIDE REQUIRES_SHARED(art::Locks::mutator_lock_) {
    // JVMTI doesn't report MonitorWaited in cases where the thread never went to sleep due to error
    // in inputs.
    if (waited && handler_->IsEventEnabledAnywhere(ArtJvmtiEvent::kMonitorWaited)) {
      art::Thread* self = art::Thread::Current();
      art::JNIEnvExt* jnienv = self->GetJniEnv();
      ScopedLocalRef<jobject> mon(jnienv, AddLocalRef<jobject>(jnienv, obj.Get()));
