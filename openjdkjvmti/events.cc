...
// Finds the location where this exception will most likely be caught. We ignore intervening native
// frames (which could catch the exception) and return the closest java frame with a compatible
// catch statement.
class CatchLocationFinder FINAL : public art::StackVisitor {
 public:
  CatchLocationFinder(art::Thread* self,
                      art::Handle<art::mirror::Class> exception_class,
                      art::Context* context,
                      /*out*/ art::ArtMethod** out_catch_method,
                      /*out*/ uint32_t* out_catch_pc)
      REQUIRES_SHARED(art::Locks::mutator_lock_)
    : StackVisitor(self, context, art::StackVisitor::StackWalkKind::kIncludeInlinedFrames),
  ...
  bool VisitFrame() OVERRIDE REQUIRES_SHARED(art::Locks::mutator_lock_) {
    ...
    if (!method->IsNative()) {
      ...
      uint32_t found_dex_pc = method->FindCatchBlock(
          exception_class_, dex_pc, &has_no_move_exception);
      DCHECK(!has_no_move_exception) << "Method " << method->PrettyMethod() << " should not be "
                                     << "executing since it has a catch without MOVE_EXCEPTION!";
...
class JvmtiMethodTraceListener FINAL : public art::instrumentation::InstrumentationListener {
 public:
  ...
  void FindCatchMethodsFromThrow(art::Thread* self,
                                 art::Handle<art::mirror::Throwable> exception,
                                 /*out*/ art::ArtMethod** out_method,
                                 /*out*/ uint32_t* dex_pc)
      REQUIRES_SHARED(art::Locks::mutator_lock_) {
    art::StackHandleScope<1> hs(self);
    art::Handle<art::mirror::Class> exception_class(hs.NewHandle(exception->GetClass()));
    *out_method = nullptr;
    *dex_pc = 0;
  ...
  void ExceptionThrown(art::Thread* self, art::Handle<art::mirror::Throwable> exception_object)
    ...
    // TODO We might need to change this behavior depending on what happens with throwing in this
    // event.
    DCHECK(!self->IsExceptionPending());
