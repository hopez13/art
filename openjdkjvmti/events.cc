  ...
  void MethodExited(art::Thread* self,
    ...
    if (frame.has_value() && UNLIKELY(event_handler_->IsEventEnabledAnywhere(
      ...
      if (has_return) {
        ...
        LOG(INFO) << "Setting object return value yeah";
...
int32_t& EventHandler::InternalEventRefcount(ArtJvmtiEvent event) {
  ...
  size_t idx = static_cast<size_t>(event) - static_cast<size_t>(ArtJvmtiEvent::kMinInternalEventTypeVal);
...
EventHandler::EventHandler()
  : envs_lock_("JVMTI Environment List Lock", art::LockLevel::kPostMutatorTopLockLevel),
    ...
    frame_pop_enabled(false), internal_event_refcount_({0}) {
