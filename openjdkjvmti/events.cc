...
class JvmtiMethodTraceListener final : public art::instrumentation::InstrumentationListener {
 public:
  ...
  void MethodExited(art::Thread* self,
    ...
    if (frame.has_value() && UNLIKELY(event_handler_->IsEventEnabledAnywhere(ArtJvmtiEvent::kForceEarlyReturnUpdateReturnValue))) {
      DCHECK(!frame->get().GetSkipMethodExitEvents());
      bool has_return = false;
      {
        ...
        if (it != non_standard_exits_.end()) {
          // self->Dump(LOG_STREAM(WARNING) << "Setting return value! ");
      ...
      if (has_return) {
        // TODO Yuck.
        art::ScopedThreadStateChange sts(self, art::ThreadState::kNative);
        ScopedLocalRef<jthread> thr(self->GetJniEnv(),
                                    self->GetJniEnv()->NewLocalRef(self->GetPeer()));
        event_handler_->SetInternalEvent(thr.get(), ArtJvmtiEvent::kForceEarlyReturnUpdateReturnValue, JVMTI_DISABLE);
      }
    }
    if (event_handler_->IsEventEnabledAnywhere(ArtJvmtiEvent::kMethodExit)) {
      DCHECK_NE(
...
static bool IsInternalEvent(ArtJvmtiEvent event) {
  return static_cast<uint32_t>(event) >=
         static_cast<uint32_t>(ArtJvmtiEvent::kMaxNormalEventTypeVal);
}

// TODO Both this and SetEvent below should be rewritten to remove possible races.
jvmtiError EventHandler::SetInternalEvent(jthread thread, ArtJvmtiEvent event, jvmtiEventMode mode) {
  ...
  bool new_state;
  {
    ...
    {
      int32_t adjustment = mode == JVMTI_ENABLE ? 1 : -1;
      CHECK_GE(InternalEventRefcount(event) + adjustment, 0);
      old_state = InternalEventRefcount(event) > 0;
      InternalEventRefcount(event) += adjustment;
      new_state = InternalEventRefcount(event) > 0;
...
static bool IsDirectlySettableEvent(ArtJvmtiEvent event) {
  ...
  case ArtJvmtiEvent::kClassFileLoadHookRetransformable:
    return false;
  default:
    return !IsInternalEvent(event) && !IsExtensionEvent(event);
  }
}
...
int32_t& EventHandler::InternalEventRefcount(ArtJvmtiEvent event) {
  CHECK(IsInternalEvent(event));
  size_t idx = static_cast<size_t>(event) - static_cast<size_t>(ArtJvmtiEvent::kMinEventTypeVal);
  return internal_event_refcount_[idx];
}
...
EventHandler::EventHandler()
  : envs_lock_("JVMTI Environment List Lock", art::LockLevel::kPostMutatorTopLockLevel),
    // internal_event_refcount_lock_("JVMTI Internal Event Refcount lock",
