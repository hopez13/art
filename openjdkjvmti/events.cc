...
class JvmtiMethodTraceListener final : public art::instrumentation::InstrumentationListener {
 public:
  ...
  void MethodUnwind(art::Thread* self, art::ArtMethod* method, uint32_t dex_pc [[maybe_unused]])
      REQUIRES_SHARED(art::Locks::mutator_lock_) override {
    if (!method->IsRuntimeMethod() &&
  ...
  void Branch(art::Thread* self [[maybe_unused]],
              art::ArtMethod* method [[maybe_unused]],
              uint32_t dex_pc [[maybe_unused]],
              int32_t dex_pc_offset [[maybe_unused]])
      REQUIRES_SHARED(art::Locks::mutator_lock_) override {
    return;
  }
