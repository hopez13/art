...
class SetupMethodExitEvents {
 public:
  SetupMethodExitEvents(art::Thread* self,
      ...
      : self_(self), env_(env), event_handler_(event_handler), target_(target) {
    ...
    // TODO Use a version that doesn't need env. Getting the right one later would be very hard.
    event_handler_->SetEvent(
        env_, target_, ArtJvmtiEvent::kForceEarlyReturnUpdateReturnValue, JVMTI_ENABLE);
  }
...
StackUtil::ForceEarlyReturn(jvmtiEnv* env, EventHandler* event_handler, jthread thread, T value) {
  ...
  // TODO We want this lock
  // art::MutexLock mu(self, *force_early_return_lock_);
  ...
  impl::SetupMethodExitEvents smee(self, ArtJvmTiEnv::AsArtJvmTiEnv(env), event_handler, thread);
  impl::NonStandardExitFrames frames(self, env, thread);
  if (frames.result_ != OK) {
    smee.NotifyFailure();
    return frames.result_;
...
art::Mutex* StackUtil::force_early_return_lock_;
art::ConditionVariable* StackUtil::force_early_return_cond_var_;

void StackUtil::Setup() {
  StackUtil::force_early_return_lock_ =
      new art::Mutex("JVMTI ForceEarlyReturnLock", art::LockLevel::kTopLockLevel);
  StackUtil::force_early_return_cond_var_ = new art::ConditionVariable("JVMTI_ForceEarlyReturnCondVar", *StackUtil::force_early_return_lock_);
}

void StackUtil::Shutdown() {
  delete StackUtil::force_early_return_cond_var_;
  StackUtil::force_early_return_cond_var_ = nullptr;
  delete StackUtil::force_early_return_lock_;
  StackUtil::force_early_return_lock_ = nullptr;
}
