...
class NonStandardExitFrames {
 public:
  NonStandardExitFrames(art::Thread* self, jvmtiEnv* env, jthread thread)
    ...
    penultimate_frame_ = penultimate_frame.GetOrCreateShadowFrame(&created_penultimate_frame);
...
void SetReturnJValue(art::ShadowFrame* frame, const art::JValue& value)
    REQUIRES_SHARED(art::Locks::mutator_lock_) REQUIRES(art::Roles::uninterruptible_) {
  ...
  // // NB We set this flag even if there is no move-result-* instruction to provide sanity checks
  // in
  // // the interpreter.
  const art::Instruction* invoke_instr = art::Instruction::At(frame->GetDexPCPtr());
...
template <typename T>
void SetReturnValue(art::ShadowFrame* frame, T value) REQUIRES_SHARED(art::Locks::mutator_lock_)
    REQUIRES(art::Locks::user_code_suspension_lock_, art::Locks::thread_list_lock_);

template <typename T> void SetReturnValue(art::ShadowFrame* frame, T value) {
  art::ScopedAssertNoThreadSuspension sants("Setting object return value!");
  art::JValue val = art::JValue::FromPrimitive(value);
  SetReturnJValue<false>(frame, val);
}

template <>
void SetReturnValue<std::nullptr_t>(art::ShadowFrame* frame,
...
template <> void SetReturnValue<jobject>(art::ShadowFrame* frame, jobject obj) {
  art::JValue value;
  art::ScopedAssertNoThreadSuspension sants("Setting object return value!");
  value.SetL(art::Thread::Current()->DecodeJObject(obj));
  SetReturnJValue<true>(frame, value);
}
...
jvmtiError StackUtil::PopFrame(jvmtiEnv* env, jthread thread) {
  ...
  if (frames.created_final_frame_) {
...
StackUtil::ForceEarlyReturn(jvmtiEnv* env, EventHandler* event_handler, jthread thread, T value) {
  ...
  // Tell the shadow-frame to return immediately and skip all exit events.
  frames.final_frame_->SetForcePopFrame(true);
  impl::SetReturnValue<T>(frames.penultimate_frame_, value);
  #if 0
