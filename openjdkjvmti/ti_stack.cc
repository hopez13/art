...
class NonStandardExitFrames {
 public:
  NonStandardExitFrames(art::Thread* self, jvmtiEnv* env, jthread thread)
    ...
    do {
      ThreadUtil::SuspendCheck(self);
      art::Locks::user_code_suspension_lock_->ExclusiveLock(self);
      ...
      if (LIKELY(!ThreadUtil::WouldSuspendForUserCodeLocked(self))) {
        break;
      }
      art::Locks::user_code_suspension_lock_->ExclusiveUnlock(self);
    } while (true);
    ...
    penultimate_frame_ = penultimate_frame.GetOrCreateShadowFrame(&created_penultimate_frame_);
  ...
  ~NonStandardExitFrames() RELEASE_SHARED(art::Locks::mutator_lock_)
      RELEASE(art::Locks::thread_list_lock_, art::Locks::user_code_suspension_lock_) {
    art::Thread* self = art::Thread::Current();
    art::Locks::thread_list_lock_->ExclusiveUnlock(self);
...
// TODO invoke-<...> {v1}, whatever \n move-result v1 breaks us.
template <bool kIsObject>
void SetReturnJValue(NonStandardExitFrames* frames, const art::JValue& value)
    REQUIRES_SHARED(art::Locks::mutator_lock_)
        REQUIRES(art::Locks::user_code_suspension_lock_, art::Locks::thread_list_lock_, art::Roles::uninterruptible_) {
  // We just fill in the register that the result would be placed in after being popped. This is
  // valid thanks to dalik constraint B19.
  art::ShadowFrame* frame = frames->penultimate_frame_;
  art::CodeItemDataAccessor accessor(frame->GetMethod()->DexInstructionData());
  const art::Instruction* invoke_instr = art::Instruction::At(accessor.Insns() + frame->GetDexPC());
  DCHECK(invoke_instr->IsInvoke());
  bool skip_move_result = true;
  const art::Instruction* maybe_move_result = invoke_instr->Next();
  switch (maybe_move_result->Opcode()) {
  case art::Instruction::MOVE_RESULT_OBJECT: {
    CHECK(kIsObject);
    uint8_t reg = maybe_move_result->VRegA_11x();
    frame->SetVRegReference(reg, value.GetL());
    if (frames->created_penultimate_frame_) {
      frames->target_->GetUpdatedVRegFlags(frames->penultimate_frame_id_)[reg] = true;
    }
    break;
  }
  case art::Instruction::MOVE_RESULT: {
    CHECK(!kIsObject);
    uint8_t reg = maybe_move_result->VRegA_11x();
    LOG(WARNING) << "Overwriting register " << static_cast<uint32_t>(reg) << " with " << std::hex
                 << value.GetI() << " method is " << frame->GetMethod()->PrettyMethod() << " pc: "
                 << "0x" << std::hex << frame->GetDexPC();
    frame->SetVReg(reg, value.GetI());
    if (frames->created_penultimate_frame_) {
      frames->target_->GetUpdatedVRegFlags(frames->penultimate_frame_id_)[reg] = true;
    }
    break;
  }
  case art::Instruction::MOVE_RESULT_WIDE: {
    CHECK(!kIsObject);
    uint8_t reg = maybe_move_result->VRegA_11x();
    LOG(WARNING) << "Overwriting register " << static_cast<uint32_t>(reg) << " with " << std::hex
                 << value.GetJ();
    frame->SetVRegLong(reg, value.GetJ());
    if (frames->created_penultimate_frame_) {
      frames->target_->GetUpdatedVRegFlags(frames->penultimate_frame_id_)[reg] = true;
      frames->target_->GetUpdatedVRegFlags(frames->penultimate_frame_id_)[reg + 1] = true;
    }
    break;
  }
  default: {
    // The result is being thrown away so we don't need to do anything.
    skip_move_result = false;
    break;
  }
  }
  if (LIKELY(skip_move_result)) {
    frame->SetForceSkipMoveResult(true);
  }
}

template <typename T>
void SetReturnValue(NonStandardExitFrames* frames, T value)
    REQUIRES_SHARED(art::Locks::mutator_lock_)
        REQUIRES(art::Locks::user_code_suspension_lock_, art::Locks::thread_list_lock_);

template <typename T> void SetReturnValue(NonStandardExitFrames* frames, T value) {
  art::ScopedAssertNoThreadSuspension sants("Setting object return value!");
  art::JValue val = art::JValue::FromPrimitive(value);
  SetReturnJValue<false>(frames, val);
}

template <>
void SetReturnValue<std::nullptr_t>(NonStandardExitFrames* frames,
...
template <> void SetReturnValue<jobject>(NonStandardExitFrames* frames, jobject obj) {
  art::JValue value;
  art::ScopedAssertNoThreadSuspension sants("Setting object return value!");
  value.SetL(art::Thread::Current()->DecodeJObject(obj));
  SetReturnJValue<true>(frames, value);
}
...
jvmtiError StackUtil::PopFrame(jvmtiEnv* env, jthread thread) {
  art::Thread* self = art::Thread::Current();
  impl::NonStandardExitFrames frames(self, env, thread);
  ...
  if (frames.created_final_frame_ || frames.created_penultimate_frame_) {
    DeoptManager::Get()->DeoptimizeThread(frames.target_);
  }
  return OK;
}
...
StackUtil::ForceEarlyReturn(jvmtiEnv* env, EventHandler* event_handler, jthread thread, T value) {
  ...
  if (frames.result_ != OK) {
    return frames.result_;
  } else if (!impl::ValidReturnType<T>(
  ...
  // Tell the shadow-frame to return immediately and skip all exit events.
  frames.final_frame_->SetForcePopFrame(true);
  impl::SetReturnValue<T>(&frames, value);
  impl::AddDelayedMethodExitEvent<T>(event_handler, frames.final_frame_, value);
  if (frames.created_final_frame_ || frames.created_penultimate_frame_) {
    DeoptManager::Get()->DeoptimizeThread(frames.target_);
  }
  return OK;
