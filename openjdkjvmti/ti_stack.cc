...
static uint32_t GetStackDepth(art::Thread* thread,
                              art::Context* ctx = nullptr)
    REQUIRES_SHARED(art::Locks::mutator_lock_) {
  struct CountDepthVisitor : public art::StackVisitor {
    explicit CountDepthVisitor(art::Thread* thread_in, art::Context* context)
        : art::StackVisitor(thread_in,
                            context,
  ...
  CountDepthVisitor visitor(thread, ctx);
  visitor.WalkStack();
  return visitor.depth_;
}
...
jvmtiError StackUtil::PopFrame(jvmtiEnv* env ATTRIBUTE_UNUSED, jthread thread) {
  ...
  do {
    ...
    // We cannot just get the last frame since we need to check that both the last and next-to last
    // frames are non-native.
    std::unique_ptr<art::Context> context(art::Context::Create());
    uint32_t depth = GetStackDepth(target, context.get());
    if (depth < 2) {
    ...
    // suspension stuff. The spec does specifically say to return THREAD_NOT_SUSPENDED though.
    // Find the requested stack frame.
    FindFrameAtDepthVisitor final_frame(target, context.get(), depth - 1);
    FindFrameAtDepthVisitor penultimate_frame(target, context.get(), depth - 2);
    final_frame.WalkStack();
    penultimate_frame.WalkStack();
    CHECK(penultimate_frame.FoundFrame()) << "Suspended thread changed depth!";
    CHECK(final_frame.FoundFrame()) << "Suspended thread changed depth!";
