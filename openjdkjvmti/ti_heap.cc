  ...
  class CollectAndReportRootsVisitor final : public art::RootVisitor {
   ...
   private:
    ...
    jvmtiHeapReferenceKind GetReferenceKind(const art::RootInfo& info,
        ...
        REQUIRES_SHARED(art::Locks::mutator_lock_) {
      // We do not necessarily hold thread_list_lock_ here, but we may if we are
      // called from VisitThreadRoots, which can happen from JVMTI FollowReferences.
      // If it was acquired ThreadList::VisitRoots, it's unsafe to temporarily release it.
      // Thus we assume we do not hold the thread_list_lock_ here, and relax CHECKs
      // appropriately. If it does happen, we are in a SuspendAll situation with concurrent
      // GC disabled, and should not need to run flip functions. TODO: Fix this.
