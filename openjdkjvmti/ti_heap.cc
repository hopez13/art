      ...
      [&](art::mirror::Object* ref) REQUIRES_SHARED(art::Locks::mutator_lock_) {
        ...
        class ResizeReferenceVisitor {
         public:
          ...
          // Ignore class roots.
          void VisitRootIfNonNull(CompressedObj* root) const REQUIRES_SHARED(art::Locks::mutator_lock_) {
            if (root != nullptr) {
              VisitRoot(root);
            }
          }
        ...
        if (ref->IsClass() && old_obj_ptr->IsClass()) {
          // Class object native roots include ArtField and ArtMethod 'declaring_class_' fields
          // which we don't want to be messing with.
...
static void ReplaceStrongRoots(art::Thread* self, ObjectPtr old_obj_ptr, ObjectPtr new_obj_ptr)
  ...
  if (!rrv.GetThreadsWithJavaFrameRoots().empty()) {
    ...
    for (uint32_t id : rrv.GetThreadsWithJavaFrameRoots()) {
      ...
      LOG(INFO) << "Instrumenting thread stack of thread " << *t;
