...
static void ReplaceStrongRoots(art::Thread* self, ArrayPtr old_arr_ptr, ArrayPtr new_arr_ptr)
  ...
  if (!rrv.GetThreadsWithJavaFrameRoots().empty()) {
    ...
    for (uint32_t id : rrv.GetThreadsWithJavaFrameRoots()) {
      ...
      // Realistically the cost for doing an extra stack walk is unlikely to be worth it though.
      instr->InstrumentThreadStack(t);
    }
  }
}

static void ReplaceWeakRoots(art::Thread* self, EventHandler* event_handler, ArrayPtr old_arr_ptr, ArrayPtr new_arr_ptr)
    REQUIRES(art::Locks::mutator_lock_,
             art::Locks::user_code_suspension_lock_,
             art::Roles::uninterruptible_) {
  ...
  event_handler->ForEachEnv(self, [&](ArtJvmTiEnv* env) NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (had_obsolete_tag || had_new_tag) {
      event_handler->DispatchEventOnEnv<ArtJvmtiEvent::kObsoleteObjectCreated>(env,
                                                                               ...
                                                                               &obsolete_tag,
                                                                               &new_tag);
      LOG(INFO) << "In " << env << " setting obsolete to " << obsolete_tag << " and new to " << new_tag;
