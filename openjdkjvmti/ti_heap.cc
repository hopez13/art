...
jvmtiError HeapExtensions::ChangeArraySize(jvmtiEnv* env, jobject arr, jsize new_size) {
         ...
         public:
          ...
          void operator()(art::ObjPtr<art::mirror::Object> obj,
            ...
            if (obj->GetFieldObject<art::mirror::Object>(off) == old_arr_) {
              LOG(INFO) << "Updating field at offset " << off.Uint32Value() << " of type " << obj->GetClass()->PrettyClass();
  ...
  struct ResizeRootVisitor : public art::RootVisitor {
   public:
    ResizeRootVisitor(art::mirror::Array* new_val, art::mirror::Array* old_val)
          ...
          if (threads_with_roots.find(t->GetThreadId()) != threads_with_roots.end()) {
            class UpdateFramesVisitor : public art::StackVisitor {
             public:
              ...
              ~UpdateFramesVisitor() {
                DCHECK(found_roots_) << "Expected to find frames containing array as a root on "
                                     << *GetThread();
              }
              ...
              bool VisitFrame() override REQUIRES_SHARED(art::Locks::mutator_lock_) {
                ...
                for (uint32_t i = 0; i < num_slots; i++) {
                  ...
                  if (GetVReg(method, i, art::VRegKind::kReferenceVReg, &val) &&
                    ...
                    found_roots_ = true;
                    LOG(INFO) << "Found ptr " << old_arr_ << " in slot " << i << " (of "
                              << num_slots << ")"
                              << " of method " << method->PrettyMethod();
                  }
                }
                return true;
              };
  ...
  gEventHandler->ForEachEnv(self, [&](ArtJvmTiEnv* env) NO_THREAD_SAFETY_ANALYSIS {
    ...
    env->object_tag_table->RemoveLocked(new_arr_ptr, &new_tag);
    env->object_tag_table->RemoveLocked(old_arr_ptr, &obsolete_tag);
    // Dispatch event.
  ...
  struct ReplaceWeaksVisitor : public art::IsMarkedVisitor {
   public:
    ReplaceWeaksVisitor(art::mirror::Array* old_arr, art::mirror::Array* new_arr)
        : old_arr_(old_arr), new_arr_(new_arr) {}
