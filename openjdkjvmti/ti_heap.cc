        ...
        class ResizeReferenceVisitor {
         public:
          ...
          void operator()(art::ObjPtr<art::mirror::Object> obj,
            ...
            if (it != map_.end()) {
              UNUSED(is_static);
              if (UNLIKELY(!is_static && off.SizeValue() == art::mirror::Object::ClassOffset().SizeValue())) {
                VLOG(plugin) << "Not updating declaring_class_ field of object of type "
...
static void ReplaceWeakRoots(art::Thread* self,
  ...
  event_handler->ForEachEnv(self, [&](ArtJvmTiEnv* env) {
    ...
    for (auto it : map) {
      ...
      bool had_new_tag = env->object_tag_table->RemoveLocked(it.first, &new_tag);
      bool had_obsolete_tag = env->object_tag_table->RemoveLocked(it.second, &obsolete_tag);
      // Dispatch event.
