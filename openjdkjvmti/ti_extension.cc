...
jvmtiError ExtensionUtil::GetExtensionFunctions(jvmtiEnv* env,
  ...
  // These require index-ids to function
  if (art::Runtime::Current()->GetJniIdType() == art::JniIdType::kIndices) {
    ...
    JVMTI_LOG(INFO, env) << "jni-type indicies are required to implement structural class "
                         << "redefinition extensions.";
  }
