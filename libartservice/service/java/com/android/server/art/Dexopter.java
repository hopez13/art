    ...
    public final List<DexContainerFileDexoptResult> dexopt() throws RemoteException {
        ...
        for (DexInfoType dexInfo : getDexInfoList()) {
            ...
            try {
                ...
                DexMetadataPath dmPath = buildDmPath(dexInfo);
                DexMetadataHelper dmHelper = mInjector.getDexMetadataHelper();
                DexMetadataInfo dmInfo = dmPath != null
                        ? dmHelper.getDexMetadataInfo(dexInfo.dexPath())
                        : dmHelper.getDefaultDexMetadataInfo();
                if (!dmInfo.exists()) {
                    dmPath = null;
                }
