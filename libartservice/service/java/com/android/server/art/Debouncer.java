...
public class Debouncer {
    ...
    private void runTask(@NonNull Runnable command, @NonNull ScheduledExecutorService executor) {
        synchronized (this) {
            ...
            // executor, but that's okay. Either that task won't pass the check below or it will be
            // cancelled, so the same executor won't be shut down twice.
    ...
    synchronized public void maybeRunAsync(@NonNull Runnable command) {
        ...
        mCurrentTask = mExecutor.schedule(
                () -> runTask(command, mExecutor), mIntervalMs, TimeUnit.MILLISECONDS);
    }
}
