...
public class PreRebootDexoptJob implements ArtServiceJobInterface {
    ...
    public boolean onStartJob(
        ...
        start().thenRunAsync(() -> {
            ...
            // it will like fail again. If it's cancelled, the job will be rescheduled because the
    ...
    public synchronized CompletableFuture<Void> start() {
        ...
        mRunningJob = new CompletableFuture().runAsync(() -> {
            try {
                ...
                mInjector.getPreRebootDriver().run(mOtaSlot, mCancellationSignal);
    ...
    public synchronized void updateOtaSlot(@NonNull String value) {
        ...
        // An OTA update has an higher priority compared to a Mainline update. When there are both
        // a pending OTA update and a pending Mainline update, the system discards the Mainline
        // update on the reboot.
