...
public class PreRebootDexoptJob implements ArtServiceJobInterface {
    ...
    /** The job that is running from the job scheduler's perspective. */
    @GuardedBy("this") @Nullable private CompletableFuture<Void> mJobSchedulerRunningJob = null;

    /** A ticket held by the job scheduler to identify the job that it requests to start. */
    @GuardedBy("this") @Nullable private String mJobSchedulerTicket = null;
    ...
    public synchronized boolean onStartJob(
        ...
        if (!params.getExtras().getString("ticket").equals(mJobSchedulerTicket)) {
            // Job expired. We can only get here due to a race, and this should be very rare.
            return false;
        }

        CompletableFuture<Void> job = mJobSchedulerRunningJob = startLocked();
        // No need to handle exceptions thrown by the future because exceptions are handled inside
        // the future itself.
        var unused = job.thenRunAsync(() -> {
            try {
                synchronized (this) {
                    if (job == mJobSchedulerRunningJob) {
                        // If it failed, it means something went wrong, so we don't reschedule the
                        // job because it will likely fail again. If it's cancelled, the job will be
                        // rescheduled because the return value of `onStopJob` will be respected,
                        // and this call will be skipped.
                        jobService.jobFinished(params, false /* wantsReschedule */);
                        mJobSchedulerRunningJob = null;
                    }
                }
    ...
    public synchronized boolean onStopJob(@NonNull JobParameters params) {
        if (mJobSchedulerRunningJob != null) {
            cancelOneLocked(mJobSchedulerRunningJob);
            mJobSchedulerRunningJob = null;
        }
    ...
    @GuardedBy("this")
    @VisibleForTesting
    public @ScheduleStatus int scheduleLocked() {
    ...
    @VisibleForTesting
    @NonNull
    public CompletableFuture<Void> startLocked() {
     ...
     *
     * When this method exits, it's guaranteed that the given job is not running, but other jobs
     * might be running.
     */
    @GuardedBy("this")
    @VisibleForTesting
    public void cancelOneLocked(@NonNull CompletableFuture<Void> job) {
    ...
    /**
     * Cancels all jobs, prevents jobs from being started by the job scheduler, and waits for the
     * jobs to exit. Temporarily releases the lock when waiting for the jobs to exit.
     *
     * When this method exits, it's guaranteed that the no job is running.
     */
    @GuardedBy("this")
    @VisibleForTesting
    public void cancelAllLocked() {
        unscheduleLocked();
    ...
    @GuardedBy("this")
    @VisibleForTesting
    public void updateOtaSlotLocked(@Nullable String value) {
    ...
    public boolean isAsynchronousForOta() {
        return SystemProperties.getBoolean(
                "dalvik.vm.pr_dexopt_asynchronous_for_ota", false /* def */);
    }
