...
Java_com_android_server_art_ArtJni_ensureNoProcessInDirNative(JNIEnv* env, jobject, jstring j_dir) {
  std::string dir(GET_UTF_OR_RETURN_VOID(env, j_dir));
  if (Result<void> result =
          tools::EnsureNoProcessInDir(dir, /*timeout_ms=*/5000, /*try_kill=*/true);
      !result.ok()) {
