...
std::once_flag g_checked_event_tag_map;

bool EventTagMapExists() {
  // Check whether the event tag map file exists (only present on Android).
  static bool tag_map_exists;
  std::call_once(g_checked_event_tag_map,
                 [&]() { tag_map_exists = (access(EVENT_TAG_MAP_FILE, R_OK) == 0); });
  return tag_map_exists;
}
...
enum PaletteStatus PaletteLogHiddenApiAccess(const char* package_name,
  ...
  //
  // This is fine, except the event logging library aborts if the Android specific event tag map
  // file does not exist. So if the file does not just return success.
  if (!EventTagMapExists()) {
    return PaletteStatus::kOkay;
  }
