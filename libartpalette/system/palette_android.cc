...
#ifndef ART_TARGET_LINUX
#include <metricslogger/metrics_logger.h>  // Workaround for b/79671158, see comments below.
#endif  // ART_TARGET_LINUX
...
// Although this source file is intended for Android, when we benchmark on ARM Linux we build
// an Android binary and use it on ARM Linux.
//
// This is problematic because the metric logging fails in static initializers on Linux, so need
// not to include the metrics_logger.h header nor link against it. There is some soong hackery in
// art.go to help with this (b/79671158).
#ifdef ART_TARGET_LINUX

enum PaletteStatus PaletteLogHiddenApiAccess(
    const char* package_name ATTRIBUTE_UNUSED,
    const char* signature ATTRIBUTE_UNUSED,
    enum PaletteHiddenApiAccessMethod access_method ATTRIBUTE_UNUSED,
    int32_t access_denied ATTRIBUTE_UNUSED) {
  return PaletteStatus::kOkay;
}

#else  // ART_TARGET_LINUX

enum PaletteStatus PaletteLogHiddenApiAccess(const char* package_name,
                                             const char* signature,
                                             enum PaletteHiddenApiAccessMethod access_method,
                                             int32_t access_denied) {
  android::metricslogger::ComplexEventLogger
      log_maker(android::metricslogger::ACTION_HIDDEN_API_ACCESSED);
  switch (access_method) {
    case PaletteHiddenApiAccessMethod::kJNI:
      log_maker.AddTaggedData(android::metricslogger::FIELD_HIDDEN_API_ACCESS_METHOD,
                              android::metricslogger::ACCESS_METHOD_JNI);
      break;
    case PaletteHiddenApiAccessMethod::kReflection:
      log_maker.AddTaggedData(android::metricslogger::FIELD_HIDDEN_API_ACCESS_METHOD,
  ...
  if (access_denied != 0) {
    log_maker.AddTaggedData(android::metricslogger::FIELD_HIDDEN_API_ACCESS_DENIED, 1);
  }
  ...
  log_maker.AddTaggedData(android::metricslogger::FIELD_HIDDEN_API_SIGNATURE, signature);
  log_maker.Record();
  return PaletteStatus::kOkay;
}
