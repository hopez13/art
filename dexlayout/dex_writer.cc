...
void DexWriter::WriteCodeItems() {
  ...
  for (std::unique_ptr<dex_ir::CodeItem>& code_item : header_.GetCollections().CodeItems()) {
    ...
    uint32_buffer[0] = code_item->DebugInfo()->GetOffset();
    ...
    if (code_item->TriesSize() != 0 && code_item->InsnsSize() % 2 != 0) {
...
void DexWriter::WriteMapItem() {
  ...
  std::priority_queue<std::unique_ptr<MapItemContainer>> queue;

  // Header and index section.
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
      DexFile::kDexTypeHeaderItem, 1, 0)));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
      DexFile::kDexTypeStringIdItem, collection.StringIdsSize(), collection.StringIdsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeTypeIdItem, collection.TypeIdsSize(), collection.TypeIdsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeProtoIdItem, collection.ProtoIdsSize(), collection.ProtoIdsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeFieldIdItem, collection.FieldIdsSize(), collection.FieldIdsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeMethodIdItem, collection.MethodIdsSize(), collection.MethodIdsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeClassDefItem, collection.ClassDefsSize(), collection.ClassDefsOffset())));

  // Data section.
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeMapList, 1, collection.MapItemOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeTypeList, collection.TypeListsSize(), collection.TypeListsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeAnnotationSetRefList, collection.AnnotationSetRefListsSize(),
        collection.AnnotationSetRefListsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeAnnotationSetItem, collection.AnnotationSetItemsSize(),
        collection.AnnotationSetItemsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeClassDataItem, collection.ClassDatasSize(),
        collection.ClassDatasOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeCodeItem, collection.CodeItemsSize(), collection.CodeItemsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeStringDataItem, collection.StringDatasSize(),
        collection.StringDatasOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeDebugInfoItem, collection.DebugInfoItemsSize(),
        collection.DebugInfoItemsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeAnnotationItem, collection.AnnotationItemsSize(),
        collection.AnnotationItemsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeEncodedArrayItem, collection.EncodedArrayItemsSize(),
        collection.EncodedArrayItemsOffset())));
  queue.push(std::unique_ptr<MapItemContainer>(new MapItemContainer(
        DexFile::kDexTypeAnnotationsDirectoryItem, collection.AnnotationsDirectoryItemsSize(),
  ...
  uint16_buffer[1] = 0;
  while (!queue.empty()) {
    const std::unique_ptr<MapItemContainer>& map_item = queue.top();
    if (map_item->size_ != 0) {
