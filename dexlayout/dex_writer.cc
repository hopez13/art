...
size_t DexWriter::Write(const void* buffer, size_t length, size_t offset) {
  DCHECK(offset + length < mem_map_->Size());
...
void DexWriter::WriteHeader() {
  ...
  offset += sizeof(uint32_t);  // Skip checksum to calculate and write later.
  ...
  // Compute and write new checksum.
  uint32_t adler_checksum = adler32(0L, Z_NULL, 0);
  uint32_t non_sum = sizeof(DexFile::Header::magic_) + sizeof(DexFile::Header::checksum_);
  buffer[0] = adler32(adler_checksum, mem_map_->Begin() + non_sum, file_size - non_sum);
  Write(buffer, sizeof(DexFile::Header::checksum_), sizeof(DexFile::Header::magic_));
}
...
void DexWriter::Output(dex_ir::Header* header, MemMap* mem_map) {
  DexWriter dex_writer(DexWriter(header, mem_map));
