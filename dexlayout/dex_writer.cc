...
uint32_t DexWriter::WriteCodeItems(uint32_t offset, bool reserve_only) {
  ...
  const DexLayoutHotnessInfo* layout_hotness_info = nullptr;
  if (!reserve_only && dex_layout_ != nullptr) {
    layout_hotness_info = dex_layout_->LayoutHotnessInfo();
  ...
  for (auto& code_item : header_->GetCollections().CodeItems()) {
    ...
    if (!reserve_only) {
      ...
      if (layout_hotness_info != nullptr) {
        auto it = layout_hotness_info->code_item_layout_.find(code_item.get());
        if (it != layout_hotness_info->code_item_layout_.end()) {
