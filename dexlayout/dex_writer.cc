...
void DexWriter::WriteHiddenapiClassData(Stream* stream) {
  ...
  // Check that hiddenapi class data and class defs share the same indices.
  if (kIsDebugBuild) {
    DCHECK_EQ(header_->HiddenapiClassDatas().Size(), header_->ClassDefs().Size());
    for (uint32_t i = 0; i < header_->ClassDefs().Size(); ++i) {
      const dex_ir::ClassDef* class_def = header_->ClassDefs()[i];
      const dex_ir::HiddenapiClassData* item = header_->HiddenapiClassDatas()[i];
  ...
  for (uint32_t i = 0; i < header_->ClassDefs().Size(); ++i) {
    ...
    if (header_->ClassDefs()[i]->GetClassData() == nullptr) {
      DCHECK_EQ(item_size, 0u);
      data_header[i + 1] = 0;
    } else {
      data_header[i + 1] = data_header[0];
    }
  ...
  for (uint32_t i = 0; i < header_->ClassDefs().Size(); ++i) {
    ...
    dex_ir::ClassData* class_data = header_->ClassDefs()[i]->GetClassData();
    if (class_data == nullptr) {
      DCHECK_EQ(data_header[i + 1], 0u);
    } else {
