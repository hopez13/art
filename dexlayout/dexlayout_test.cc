 ...
 protected:
  ...
  void CreateProfile(const std::string& input_dex,
    ...
    for (const std::unique_ptr<const DexFile>& dex_file : dex_files) {
      ...
      for (uint32_t i = 0; i < dex_file->NumClassDefs(); i += 2) {
  ...
  bool DexFileLayoutFixedPointExec(std::string* error_msg) {
    ...
    // Recreate the profile with the new dex location (required so that dexlayout matches samples).
