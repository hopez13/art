...
TEST_F(DexLayoutTest, LinkData) {
  ...
  MutateDexFile(temp_dex.GetFile(), GetTestDexFileName("ManyMethods"), [&] (DexFile* dex) {
    ...
    if (header.HasDexContainer()) {
      auto& headerV41 = const_cast<DexFile::HeaderV41&>(dex->GetHeaderV41());
      DCHECK_EQ(headerV41.container_offset_, 0u);
      headerV41.container_size_ = header.file_size_;
    }
