...
class Printer {
 public:
  Printer() {
    ...
    section_header_width_ = 0;
    for (const auto& section_info : kDexSectionInfoMap) {
      section_header_width_ = std::max(section_header_width_,
  ...
  void SkipLine() { std::cout << std::endl; }

 private:
  int section_header_width_;
  DISALLOW_COPY_AND_ASSIGN(Printer);
};

static void PrintLetterKey() {
  static const char kSpacingString[] = "      ";
  std::cout << "letter section_type" << std::endl;
  for (const auto& p : kDexSectionInfoMap) {
    const DexSectionInfo& section_info = p.second;
    std::cout << section_info.letter << kSpacingString << section_info.name.c_str() << std::endl;
  }
  std::cout << "*" << kSpacingString << "(Executable page resident)" << std::endl;
  std::cout << "." << kSpacingString << "(Mapped page not resident)" << std::endl;
}
