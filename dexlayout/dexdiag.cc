...
static void PrintLetterKey() {
  ...
  for (const auto& p : kDexSectionInfoMap) {
    ...
    printf("%c      %s\n", section_info.letter, section_info.name.c_str());
  }
}
...
static uint16_t FindSectionTypeForPage(size_t page,
  ...
  for (const auto& section : sections) {
    size_t first_page_of_section = section.offset / art::kPageSize;
...
static void ProcessOneDexMapping(uint64_t* pagemap,
  ...
  uint64_t end = art::RoundUp(start + dex_file_size, art::kPageSize) / art::kPageSize;
...
static void DisplayMappingIfFromVdexFile(pm_map_t* map) {
  ...
  std::unique_ptr<art::VdexFile> vdex(art::VdexFile::Open(vdex_name,
                                                          ...
                                                          &error_msg /*out*/));
  if (vdex == nullptr) {
    fprintf(stderr, "Could not open vdex file %s: error %s\n", vdex_name.c_str(),
            error_msg.c_str());
    return;
  }
  ...
  if (!vdex->OpenAllDexFiles(&dex_files, &error_msg)) {
    fprintf(stderr,
            "Dex files could not be opened for %s: error %s\n",
            vdex_name.c_str(),
            error_msg.c_str());
  }
