...
int32_t DexLayout::LayoutStrings() {
  // Identify all the strings referenced by code items.
  std::set<const dex_ir::StringId*> code_strings;
  ...
  uint32_t code_string_count = 0;
  uint32_t code_string_size = 0;
  for (const std::unique_ptr<dex_ir::StringId>& string_id : header_->GetCollections().StringIds()) {
    ++string_count;
    const char* data = string_id->Data();
    string_size += (strlen(data) + 1);
    if (code_strings.find(string_id.get()) != code_strings.end()) {
      ++code_string_count;
      code_string_size += (strlen(data) + 1);
    }
  }
  std::cerr << "========= string count: " << string_count << std::endl;
  std::cerr << "========= code string count: " << code_string_count << std::endl;
  std::cerr << "========= string size: " << string_size << std::endl;
  std::cerr << "========= code string size: " << code_string_size << std::endl;
