...
// NOTE: This will leave the last code item in place to preserve alignment.
// Layout needs an overhaul to handle movement of other sections properly.
int32_t DexLayout::LayoutCodeItems(std::vector<dex_ir::ClassData*> new_class_data_order) {
  // Find the last code item so we can leave it in place.
  ...
  // Mark the last code item visited so its offset is preserved.
