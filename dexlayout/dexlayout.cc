...
int32_t DexLayout::LayoutCodeItems(const DexFile* dex_file,
  ...
  for (InvokeType invoke_type : invoke_types) {
    for (std::unique_ptr<dex_ir::ClassDef>& class_def : header_->GetCollections().ClassDefs()) {
      ...
      for (auto& method : *(invoke_type == InvokeType::kDirect
        ...
        if (hotness.IsHot()) {
          // Hot code is compiled, maybe one day it wont be accessed. So lay it out together for
          // now.
  ...
  for (size_t i = 0; i < kCodeItemStateSize; ++i) {
    LOG(ERROR) << "Code items in bucket " << i << " : " << code_items[i].size();
  }
