...
static void DumpFileHeader(dex_ir::Header* header) {
  ...
  fprintf(out_file_, "string_ids_size     : %d\n", header->GetCollections().StringIdsSize());
  fprintf(out_file_, "string_ids_off      : %d (0x%06x)\n",
          header->GetCollections().StringIdsOffset(), header->GetCollections().StringIdsOffset());
  fprintf(out_file_, "type_ids_size       : %d\n", header->GetCollections().TypeIdsSize());
  fprintf(out_file_, "type_ids_off        : %d (0x%06x)\n",
          header->GetCollections().TypeIdsOffset(), header->GetCollections().TypeIdsOffset());
  fprintf(out_file_, "proto_ids_size      : %d\n", header->GetCollections().ProtoIdsSize());
  fprintf(out_file_, "proto_ids_off       : %d (0x%06x)\n",
          header->GetCollections().ProtoIdsOffset(), header->GetCollections().ProtoIdsOffset());
  fprintf(out_file_, "field_ids_size      : %d\n", header->GetCollections().FieldIdsSize());
  fprintf(out_file_, "field_ids_off       : %d (0x%06x)\n",
          header->GetCollections().FieldIdsOffset(), header->GetCollections().FieldIdsOffset());
  fprintf(out_file_, "method_ids_size     : %d\n", header->GetCollections().MethodIdsSize());
  fprintf(out_file_, "method_ids_off      : %d (0x%06x)\n",
          header->GetCollections().MethodIdsOffset(), header->GetCollections().MethodIdsOffset());
  fprintf(out_file_, "class_defs_size     : %d\n", header->GetCollections().ClassDefsSize());
  fprintf(out_file_, "class_defs_off      : %d (0x%06x)\n",
          header->GetCollections().ClassDefsOffset(), header->GetCollections().ClassDefsOffset());
...
static void DumpClassAnnotations(dex_ir::Header* header, int idx) {
  ...
  if (parameters != nullptr) {
    for (auto& parameter : *parameters) {
      ...
      uint32_t j = 0;
      for (dex_ir::AnnotationSetItem* annotation : *parameter->GetAnnotations()) {
        fprintf(out_file_, "#%u\n", j);
