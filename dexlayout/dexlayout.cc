...
void DexLayout::LayoutClassDefsAndClassData(const DexFile* dex_file) {
  ...
  for (dex_ir::ClassDef* class_def : new_class_def_order) {
    ...
    if (class_data != nullptr && visited_class_data.find(class_data) == visited_class_data.end()) {
      ...
      // Copy the class datas over the existing map, this is kind of gross because of the unique
      // pointers.
  ...
  if (kChangeClassDefOrder) {
    ...
    for (size_t i = 0; i < class_defs.size(); ++i) {
      // Copy the class defs over the existing map, this is kind of gross because of the unique
      // pointers.
...
void DexLayout::LayoutCodeItems(const DexFile* dex_file) {
  ...
  std::unordered_map<dex_ir::CodeItem*, LayoutType> layout_flags;
  ...
  for (InvokeType invoke_type : invoke_types) {
    for (std::unique_ptr<dex_ir::ClassDef>& class_def : header_->GetCollections().ClassDefs()) {
      ...
      for (auto& method : *(invoke_type == InvokeType::kDirect
        ...
        auto it = layout_flags.emplace(code_item, state);
        if (!it.second) {
          // Already exists, merge the hotness.
          it.first->second = MergeLayoutType(it.first->second, state);
        }
      }
    }
  }

  // TODO: Store the layout info so we can calculate the offsets later.
  /*
  ...
  */
  ...
  std::sort(code_items.begin(),
            code_items.end(),
            [&layout_flags](const std::unique_ptr<dex_ir::CodeItem>& a,
                            const std::unique_ptr<dex_ir::CodeItem>& b) {
    auto it_a = layout_flags.find(a.get());
    auto it_b = layout_flags.find(b.get());
    DCHECK(it_a != layout_flags.end());
    DCHECK(it_b != layout_flags.end());
    return it_a->second < it_b->second;
  });
}

void DexLayout::LayoutOutputFile(const DexFile* dex_file) {
  // All the offsets must be unassigned when this is called.
...
void DexLayout::OutputDexFile(const DexFile* dex_file) {
  ...
  // The main cause of expansion is currently from encoded method offsets getting larger.
  const size_t max_size = header_->FileSize() * 10 / 9;
...
void DexLayout::ProcessDexFile(const char* file_name,
  ...
  // Do not eagerly_assign_offsets for output mode since we do it all during writing.
  std::unique_ptr<dex_ir::Header> header(
      dex_ir::DexIrBuilder(*dex_file, /*eagerly_assign_offsets*/ !output));
