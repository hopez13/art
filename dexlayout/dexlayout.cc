...
// Increase offsets of every item in the specified section by diff bytes.
...
// Increase offsets of all sections with an address after the specified offset by diff bytes.
...
void DexLayout::LayoutOutputFile(const DexFile* dex_file) {
  uint32_t class_def_idx = 0;
  std::map<uint32_t, dex_ir::ClassDef*> index_mapping;
  for (std::unique_ptr<dex_ir::ClassDef>& class_def : header_->GetCollections().ClassDefs()) {
    if (info_->ContainsClass(*dex_file, class_def->GetIndex())) {
      index_mapping.insert(std::make_pair(class_def_idx++, class_def.get()));
    }
  }
  for (std::unique_ptr<dex_ir::ClassDef>& class_def : header_->GetCollections().ClassDefs()) {
    if (!info_->ContainsClass(*dex_file, class_def->GetIndex())) {
  ...
  for (std::pair<uint32_t, dex_ir::ClassDef*> it : index_mapping) {
    dex_ir::ClassDef* class_def = it.second;
    class_def->SetIndex(it.first);
    ...
    if (class_def->GetClassData() != nullptr) {
      ...
      for (auto& method : *class_def->GetClassData()->DirectMethods()) {
        ...
        if (code_item != nullptr) {
          ...
          code_item_offset += RoundUp(code_item->GetSize(), 4);
        }
      }
      for (auto& method : *class_def->GetClassData()->VirtualMethods()) {
        dex_ir::CodeItem* code_item = method->GetCodeItem();
        if (code_item != nullptr) {
          diff += UnsignedLeb128Size(code_item_offset) - UnsignedLeb128Size(code_item->GetOffset());
          code_item->SetOffset(code_item_offset);
      ...
      class_data_offset += class_def->GetClassData()->GetSize() + diff;
      total_diff += diff;
    }
  }
  // Adjust diff to be 4-byte aligned.
  total_diff = RoundUp(total_diff, 4);
  // Move sections after ClassData (currently only MapList).
  FixupSections(header_->GetCollections().ClassDatasOffset(), total_diff);
  // Update file size.
