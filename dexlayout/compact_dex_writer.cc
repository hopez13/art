...
uint32_t CompactDexWriter::WriteCodeItem(Stream* stream,
  ...
  if (compute_offsets_) {
    ...
    const uint32_t deduped_offset = DedupeData(data_start, stream->Tell(), code_item->GetOffset());
...
CompactDexWriter::Deduper::Deduper(bool enabled) : enabled_(enabled) {}
...
void CompactDexWriter::Write()  {
  // For now write both the
  Stream main_stream(dex_layout_->GetOutput)
}

void CompactDexWriter::WriteToStreams(Stream* main_stream, Stream* data_stream)  {
  ...
  if (dex_layout_->GetOptions().update_checksum_) {
    // header_->SetChecksum(DexFile::CalculateChecksum(mem_map_->Begin(), stream->Tell()));
