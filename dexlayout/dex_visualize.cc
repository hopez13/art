...
class Dumper {
 public:
  ...
  explicit Dumper(dex_ir::Header* header)
      : out_file_(nullptr),
        sorted_sections_(dex_ir::GetSortedDexFileSections(header, dex_ir::kSortDescending)) { }
  ...
  bool OpenAndPrintHeader(size_t dex_index) {
    ...
    for (const dex_ir::DexFileSection& s : sorted_sections_) {
      if (s.size_ > 0) {
        ...
        fprintf(out_file_, "\"%s\" %d", s.name_.c_str(), s.offset_ / kPageSize);
        printed_one = true;
      }
    }
    fprintf(out_file_, ")\n");
  ...
  int GetColor(uint32_t offset) const {
    ...
    for (const dex_ir::DexFileSection& file_section : sorted_sections_) {
      if (file_section.offset_ < offset) {
...
void ShowDexSectionStatistics(dex_ir::Header* header, size_t dex_file_index) {
  ...
  std::vector<dex_ir::DexFileSection> sorted_sections(
      GetSortedDexFileSections(header, dex_ir::kSortAscending));
  for (size_t i = 0; i < sorted_sections.size(); ++i) {
    const dex_ir::DexFileSection& file_section = sorted_sections[i];
    uint32_t bytes = 0;
    if (file_section.size_ > 0) {
      bytes = FindNextByteAfterSection(header, sorted_sections, i) - file_section.offset_;
    }
    fprintf(stdout, "%-10s %8d %8d %8d %8d %%%02d\n", file_section.name_.c_str(),
            file_section.offset_, file_section.size_, bytes,
