...
static std::vector<const FileSection*>* GetSortedSections(
  ...
  std::vector<const FileSection*>* sorted_sections = new std::vector<const FileSection*>;
...
class Dumper {
 public:
  ...
  Dumper(const dex_ir::Collections& collections)
 ...
 private:
  ...
  FILE* out_file_;
  std::unique_ptr<std::vector<const FileSection*>> sorted_sections_;
...
void ShowDexSectionStatistics(dex_ir::Header* header, size_t dex_file_index) {
  ...
  std::unique_ptr<std::vector<const FileSection*>> sorted_sections(
      GetSortedSections(collections, kSortAscending));
