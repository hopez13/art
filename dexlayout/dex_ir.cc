...
Header::Header(const DexFile& dex_file) : dex_file_(dex_file) {
  ...
  string_ids_.set_offset(disk_header.string_ids_off_);
  for (uint32_t i = 0; i < dex_file_.NumStringIds(); ++i) {
    string_ids_.add_at_position(i, new StringId(dex_file_.GetStringId(i), *this));
  }
  type_ids_.set_offset(disk_header.type_ids_off_);
  for (uint32_t i = 0; i < dex_file_.NumTypeIds(); ++i) {
    type_ids_.add_at_position(i, new TypeId(dex_file_.GetTypeId(i), *this));
  }
  proto_ids_.set_offset(disk_header.proto_ids_off_);
  for (uint32_t i = 0; i < dex_file_.NumProtoIds(); ++i) {
    proto_ids_.add_at_position(i, new ProtoId(dex_file_.GetProtoId(i), *this));
  }
  field_ids_.set_offset(disk_header.field_ids_off_);
  for (uint32_t i = 0; i < dex_file_.NumFieldIds(); ++i) {
    field_ids_.add_at_position(i, new FieldId(dex_file_.GetFieldId(i), *this));
  }
  method_ids_.set_offset(disk_header.method_ids_off_);
  for (uint32_t i = 0; i < dex_file_.NumMethodIds(); ++i) {
    method_ids_.add_at_position(i, new MethodId(dex_file_.GetMethodId(i), *this));
  }
  class_defs_.set_offset(disk_header.class_defs_off_);
  for (uint32_t i = 0; i < dex_file_.NumClassDefs(); ++i) {
    class_defs_.add_at_position(i, new ClassDef(dex_file_.GetClassDef(i), *this));
  }
}
...
void ArrayItem::Read(Header& header, const uint8_t** data, uint8_t type, uint8_t length) {
  ...
  switch (type_) {
    ...
    case DexFile::kDexAnnotationArray: {
      item_.annotation_array_val_ = new std::vector<ArrayItem*>();
      // Decode all elements.
    ...
    case DexFile::kDexAnnotationAnnotation: {
      ...
      item_.annotation_annotation_val_.string_ = (*header.type_ids())[type_idx]->string_id();
      item_.annotation_annotation_val_.array_ = new std::vector<NameValuePair*>();
...
ClassDef::ClassDef(const DexFile::ClassDef& disk_class_def, Header& header) {
  ...
  if (disk_annotations_directory_item == nullptr) {
    ...
    annotations_->set_offset(disk_class_def.annotations_off_);
  }
  ...
  if (static_data != nullptr) {
    ...
    if (static_value_count > 0) {
      static_values_ = new std::vector<ArrayItem*>();
  ...
  class_data_.set_offset(disk_class_def.class_data_off_);
  if (encoded_data != nullptr) {
    ...
    for (uint32_t i = 0; cdii.HasNextStaticField(); i++, cdii.Next()) {
      ...
      class_data_.static_fields()->push_back(new FieldItem(access_flags, field_item));
    }
    // Instance fields.
    for (uint32_t i = 0; cdii.HasNextInstanceField(); i++, cdii.Next()) {
      ...
      const_cast<FieldId*>(field_item)->setClass(this);
      uint32_t access_flags = cdii.GetRawMemberAccessFlags();
      class_data_.instance_fields()->push_back(new FieldItem(access_flags, field_item));
    }
    // Direct methods.
    for (uint32_t i = 0; cdii.HasNextDirectMethod(); i++, cdii.Next()) {
      ...
      class_data_.direct_methods()->push_back(new MethodItem(access_flags, method_item, nullptr));
    }
    // Virtual methods.
    for (uint32_t i = 0; cdii.HasNextVirtualMethod(); i++, cdii.Next()) {
      ...
      uint32_t access_flags = cdii.GetRawMemberAccessFlags();
      // const DexFile::CodeItem* disk_code_item = cdii.GetMethodCodeItem();
      class_data_.virtual_methods()->push_back(new MethodItem(access_flags, method_item, nullptr));
    }
  }
}
...
AnnotationsDirectoryItem::AnnotationsDirectoryItem(
  ...
  if (fields != nullptr) {
    for (uint32_t i = 0; i < disk_annotations_item->fields_size_; ++i) {
      FieldId* field_id = (*header.field_ids())[fields[i].field_idx_];
      const DexFile::AnnotationSetItem* field_set_item =
          header.dex_file().GetFieldAnnotationSetItem(fields[i]);
      dex_ir::AnnotationSetItem* annotation_set_item =
          new AnnotationSetItem(*field_set_item, header);
      field_annotations_.push_back(new FieldAnnotation(field_id, annotation_set_item));
    }
  }
  ...
  if (methods != nullptr) {
    for (uint32_t i = 0; i < disk_annotations_item->methods_size_; ++i) {
      MethodId* method_id = (*header.method_ids())[methods[i].method_idx_];
      const DexFile::AnnotationSetItem* method_set_item =
          header.dex_file().GetMethodAnnotationSetItem(methods[i]);
      dex_ir::AnnotationSetItem* annotation_set_item =
          new AnnotationSetItem(*method_set_item, header);
      method_annotations_.push_back(new MethodAnnotation(method_id, annotation_set_item));
    }
  }
  ...
  if (parameters != nullptr) {
    for (uint32_t i = 0; i < disk_annotations_item->parameters_size_; ++i) {
      MethodId* method_id = (*header.method_ids())[parameters[i].method_idx_];
