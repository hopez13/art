...
Header::Header(const DexFile::Header& disk_header, Builder& builder) {
  ...
  for (uint32_t i = 0; i < builder.dex_file().NumStringIds(); ++i) {
    string_ids_.push_back(new StringId(builder.dex_file().GetStringId(i), builder));
  }
...
AnnotationsDirectoryItem::AnnotationsDirectoryItem(
  ...
  if (parameters != nullptr) {
    for (uint32_t i = 0; i < disk_annotations_item->parameters_size_; ++i) {
      const MethodId* method_id = builder.methods()->Lookup(parameters[i].method_idx_);
      const DexFile::AnnotationSetRefList* list =
          builder.dex_file().GetParameterAnnotationSetRefList(&parameters[i]);
      parameter_annotations_.push_back(new ParameterAnnotation(method_id, list, builder));
    }
  }
}
