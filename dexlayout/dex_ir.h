...
template<class T> class CollectionVector : public CollectionBase<T> {
 public:
  ...
  CollectionVector() : check_bounds_(false) { }
  explicit CollectionVector(size_t size ATTRIBUTE_UNUSED) : check_bounds_(true) { }
...
template<class T> class IndexedCollectionVector : public CollectionVector<T> {
 ...
 private:
  ...
  T* CreateAndAddIndexedItem(uint32_t index, Args&&... args) {
    if (CollectionVector<T>::check_bounds_) {
      CHECK(index <= CollectionVector<T>::Size());
    }
  ...
  T* GetElement(uint32_t index) {
    CHECK_LT(index, CollectionVector<T>::Size());
