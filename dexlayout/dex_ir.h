...
class Collections {
 ...
 private:
  ...
  ParameterAnnotation* CreateParameterAnnotation(const DexFile& dex_file,
...
class ParameterAnnotation {
 public:
  ParameterAnnotation(MethodId* method_id, std::vector<AnnotationSetItem*>* annotations)
      : method_id_(method_id), annotations_(annotations) { }

  MethodId* GetMethodId() const { return method_id_; }
  std::vector<AnnotationSetItem*>* GetAnnotations() { return annotations_.get(); }

 private:
  MethodId* method_id_;
  std::unique_ptr<std::vector<AnnotationSetItem*>> annotations_;
