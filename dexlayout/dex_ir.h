...
class EncodedArrayItem : public Item {
 public:
  class NameValuePair {
   public:
    NameValuePair(const StringDataItem* name, const EncodedArrayItem* value) :
      name_(name), value_(value) { }

    const StringDataItem* name() const { return name_; }
    const EncodedArrayItem* value() const { return value_; }

   private:
    DISALLOW_COPY_AND_ASSIGN(NameValuePair);
    const StringDataItem* name_;
    const EncodedArrayItem* value_;
  };
  ...
  std::vector<const EncodedArrayItem*>* annotation_array_val() const {
  ...
  EncodedArrayItem(Builder& builder, const uint8_t** data);
  ~EncodedArrayItem() OVERRIDE { }

 private:
  DISALLOW_COPY_AND_ASSIGN(EncodedArrayItem);

  EncodedArrayItem(uint8_t type, const uint8_t** payload);
  ...
  union {
    ...
    std::vector<const EncodedArrayItem*>* annotation_array_val_;
    struct {
      const StringDataItem* string_;
...
class ClassDefItem : public Item {
 public:
  ...
  const std::vector<const EncodedArrayItem*>* static_values() const { return static_values_; }
  const std::vector<EncodedFieldItem*>* static_fields() const {
    return &class_data_.static_fields_;
  }
  const std::vector<EncodedFieldItem*>* instance_fields() const {
    return &class_data_.instance_fields_;
  }
  const std::vector<EncodedMethodItem*>* direct_methods() const {
    return &class_data_.direct_methods_;
  }
  const std::vector<EncodedMethodItem*>* virtual_methods() const {
