...
class Header : public Item {
 ...
 private:
  ...
  // layout just sort the vector. Note that you may only change the order ostd::unique_ptr<f the non indexed>vectors
...
using HiddenapiFlagsMap = std::map<const Item*, uint32_t>;
...
class HiddenapiClassData : public IndexedItem {
 public:
  HiddenapiClassData(const ClassDef* class_def, std::unique_ptr<HiddenapiFlagsMap>& flags)
      : class_def_(class_def), flags_(flags.release()) { }
  ...
  uint32_t GetFlags(const Item* field_or_method_item) const {
    if (flags_ == nullptr) {
      ...
      auto it = flags_->find(field_or_method_item);
      DCHECK(it != flags_->end());
      return it->second;
    }
  }
