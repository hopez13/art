...
// Can't run these tests on the host, as they will fail when trying to open
// /proc/pid/pagemap.
// On target, invoke 'dexdiag' against the current process.
// This should succeed because we have a runtime and so it should
// be able to map in the boot.art and do a diff for it.
#if defined (ART_TARGET) && !defined(__mips__)
TEST_F(DexDiagTest, DexDiagLetterKeyTest) {
#else
TEST_F(DexDiagTest, DISABLED_DexDiagLetterKeyTest) {
#endif
