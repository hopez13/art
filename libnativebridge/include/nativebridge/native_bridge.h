...
struct NativeBridgeCallbacks {
  ...
  // Deprecated in v7
  //   Starting with version 7 native bridge uses getTrampoline2 call below
  ...
  // has same sigature as the native method.
  //
  // Parameters:
  //   handle [IN] the handle returned from loadLibrary
  //   shorty [IN] short descriptor of native method
  //   len [IN] length of shorty
  //   jni_call_type = the type of JNI call
  // Returns:
  //   address of trampoline if successful, otherwise NULL
  // Deprecated in v7
  //   Starting with version 7 native bridge uses getTrampoline2 call below
  void* (*getTrampoline2)(void* handle,
                          const char* name,
                          const char* shorty,
                          uint32_t len,
                          enum JNICallType jni_call_type);
