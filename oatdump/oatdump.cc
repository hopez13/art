...
class IMTDumper {
 public:
  ...
  static bool DumpImtStats(Runtime* runtime) {
    const std::vector<const DexFile*>& boot_cp = runtime->GetClassLinker()->GetBootClassPath();
    ...
    for (const DexFile* dex_file : boot_cp) {
...
struct OatdumpArgs : public CmdlineArgs {
 protected:
  ...
  virtual ParseStatus ParseCustom(const StringPiece& option,
    ...
    if (option.starts_with("--oat-file=")) {
      ...
      imt_collision_dump_ = option.substr(strlen("--dump-imt=")).data();
  ...
  virtual std::string GetUsage() const {
    ...
    usage +=  // Optional.
        ...
        "                          types and interface methods in the given file\n"
        "      Example: --dump-imt=imt.txt\n"
        "\n"
        "  --dump-imt-stats: output IMT statistics for the given image\n"
 ...
 public:
  ...
  std::string imt_collision_dump_;
...
struct OatdumpMain : public CmdlineMain<OatdumpArgs> {
  virtual bool NeedsRuntime() OVERRIDE {
    ...
    return (args_->boot_image_location_ != nullptr ||
            ...
            !args_->imt_collision_dump_.empty()) &&
          !args_->symbolize_;
  }
  ...
  virtual bool ExecuteWithRuntime(Runtime* runtime) {
    ...
    if (!args_->imt_collision_dump_.empty()) {
      return IMTDumper::DumpImt(runtime, args_->imt_collision_dump_);
    }

    if (args_->imt_stat_dump_) {
      return IMTDumper::DumpImtStats(runtime);
    }
