 ...
 private:
  ...
  void DumpCode(VariableIndentationOutputStream* vios,
    ...
    if (code_size == 0 || quick_code == nullptr) {
      ...
      MethodInfo method_info(oat_method.GetOatQuickMethodHeader()->GetOptimizedMethodInfo());
      {
        auto code_info = helper.GetCodeInfo();
        auto stack_maps = code_info.stack_map_;
        ...
        if (stats_.AddBitsIfUnique(Stats::kByteKindCodeInfo,
          ...
          stats_.AddBits(
              ...
              code_info.stack_mask_.size() * kBitsPerByte);

          // Register masks
          stats_.AddBits(
              Stats::kByteKindCodeInfoRegisterMasks,
              code_info.register_mask_.DataBitSize());

          // Invoke infos
          stats_.AddBits(
              Stats::kByteKindCodeInfoInvokeInfo,
              code_info.invoke_info_.DataBitSize());
          ...
          // Inline infos.
          auto inline_info = code_info.inline_info_;
          const size_t num_inline_infos = inline_info.NumRows();
          if (num_inline_infos > 0u) {
            stats_.AddBits(
                Stats::kByteKindInlineInfoMethodIndexIdx,
                inline_info.NumColumnBits(InlineInfo::kMethodIndexIdx) * num_inline_infos);
            stats_.AddBits(
                Stats::kByteKindInlineInfoDexPc,
                inline_info.NumColumnBits(InlineInfo::kDexPc) * num_inline_infos);
            stats_.AddBits(
                Stats::kByteKindInlineInfoExtraData,
                inline_info.NumColumnBits(InlineInfo::kExtraData) * num_inline_infos);
            stats_.AddBits(
                Stats::kByteKindInlineInfoDexRegisterMap,
                inline_info.NumColumnBits(InlineInfo::kDexRegisterMapOffset) * num_inline_infos);
