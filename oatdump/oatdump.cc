...
static int DumpImages(Runtime* runtime, OatDumperOptions* options, std::ostream* os) {
  ...
  if (options->app_image_ != nullptr) {
    ...
    std::string error_msg;
    std::unique_ptr<OatFile> oat_file = OpenOat(
        *options->oat_filename_, options->dex_filename_, /*allow_no_dex_code=*/false, &error_msg);
    if (oat_file == nullptr) {
...
static int DumpOat(Runtime* runtime, OatDumperOptions* options, std::ostream* os) {
  std::string error_msg;
  std::unique_ptr<OatFile> oat_file = OpenOat(
      *options->oat_filename_, options->dex_filename_, /*allow_no_dex_code=*/true, &error_msg);
  if (oat_file == nullptr) {
...
struct OatdumpMain : public CmdlineMain<OatdumpArgs> {
  ...
  bool CanDumpWithRuntime(std::string* error_msg) {
    ...
    std::unique_ptr<OatFile> oat_file = OpenOat(*oat_dumper_options_->oat_filename_,
                                                oat_dumper_options_->dex_filename_,
                                                /*allow_no_dex_code=*/true,
                                                error_msg);
    if (oat_file == nullptr) {
