...
#undef DUMP_OAT_HEADER_OFFSET
    ...
    if (options_.export_dex_location_) {
      ...
      for (const auto& vdex_dex_file : vdex_dex_files) {
        ...
        // Remove hiddenapis
        for (art::ClassAccessor accessor : vdex_dex_file->GetClasses()) {
        ...
        // the original input Dex file, ExportDexFile() should know if DexLayout was used to adjust
        // the checksum logic before writing the file on disk.
