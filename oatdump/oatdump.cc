...
#undef DUMP_OAT_HEADER_OFFSET
    ...
    if (!options_.dump_header_only_ && !oat_file_.bcp_bss_info_.empty()) {
      Runtime* const runtime = Runtime::Current();
      ClassLinker* const linker = runtime != nullptr ? runtime->GetClassLinker() : nullptr;
      bool can_dump_for_bcp = false;
      if (linker != nullptr) {
        // Non-empty `bcp_bss_info_` implies that we have the dex file.
        const char* dex_filename = options_.dex_filename_->c_str();

        OatFileAssistant oat_file_assistant(dex_filename,
                                            runtime->GetInstructionSet(),
                                            /*context=*/nullptr,
                                            /*load_executable=*/false);
        if (oat_file_assistant.ValidateBootClassPathChecksums(oat_file_)) {
          LOG(INFO) << "BCP checksum check passed. Dumping entries for BCP DexFile";
          can_dump_for_bcp = true;
        } else {
          LOG(WARNING) << "BCP checksum check failed. Only dumping offsets for BCP DexFile";
        }
      } else {
        LOG(WARNING) << "Running without runtime. Only dumping offsets for BCP DexFile";
      }
...
static std::unique_ptr<OatFile> OpenOat(OatDumperOptions* options, std::string* error_msg) {
  if (!options->dex_filename_.has_value()) {
    LOG(WARNING) << "No dex filename provided, "
                 << "oatdump might fail if the oat file does not contain the dex code.";
  }
  ArrayRef<const std::string> dex_filenames =
      options->dex_filename_.has_value() ?
          ArrayRef<const std::string>(&options->dex_filename_.value(), /*size=*/1) :
          ArrayRef<const std::string>();
  return std::unique_ptr<OatFile>(OatFile::Open(/*zip_fd=*/-1,
                                                *options->oat_filename_,
                                                *options->oat_filename_,
                                                /*executable=*/false,
                                                /*low_4gb=*/false,
                                                dex_filenames,
...
static int DumpImages(Runtime* runtime, OatDumperOptions* options, std::ostream* os) {
  ...
  if (options->app_image_ != nullptr) {
    ...
    std::string error_msg;
    std::unique_ptr<OatFile> oat_file = OpenOat(options, &error_msg);
    if (oat_file == nullptr) {
  ...
  ParseStatus ParseChecks(std::string* error_msg) override {
    if (image_location_ != nullptr) {
      if (boot_image_location_ != nullptr) {
        std::cerr
            << "Warning: Invalid combination of --boot-image and --image. Use --image only to "
               "dump a boot image\n";
    ...
    // Infer boot image location if possible.
    if (boot_image_location_ == nullptr) {
      inferred_boot_image_ = InferBootImage();
      if (!inferred_boot_image_.empty()) {
        boot_image_location_ = inferred_boot_image_.c_str();
      }
    }
    ...
    if (image_location_ != nullptr && oat_filename_ != nullptr) {
      *error_msg =
          "--image and --oat-file must not be specified together. Use --image only to dump both "
          "the boot image(s) and their oat file(s)";
      return kParseError;
    }
...
Options:
 ...
 public:
  OatDumpMode GetMode() {
    ...
    if (boot_image_location_ != nullptr) {
      return OatDumpMode::kDumpOatWithRuntime;
    }
    return OatDumpMode::kDumpOatWithoutRuntime;
  }
 ...
 private:
  std::string InferBootImage() {
    ...
    if (env_value == nullptr || strlen(env_value) == 0) {
      LOG(WARNING) << "Failed to get environment variable 'BOOTCLASSPATH'";
      return "";
    }
    ...
    std::vector<std::string> boot_class_path = android::base::Split(env_value, ":");
    std::vector<std::string> image_locations = android::base::Split(boot_image, ":");
    auto ofa_context = std::make_unique<OatFileAssistantContext>(
        std::make_unique<OatFileAssistantContext::RuntimeOptions>(
            OatFileAssistantContext::RuntimeOptions{
                ...
                .boot_class_path_locations = boot_class_path,
            }));
    if (ofa_context->GetBootImageInfoList(instruction_set_).size() != image_locations.size()) {
  ...
  std::string inferred_boot_image_ = "";
};
...
struct OatdumpMain : public CmdlineMain<OatdumpArgs> {
  bool NeedsRuntime() override {
    ...
    return mode == OatDumpMode::kDumpImt || mode == OatDumpMode::kDumpImage ||
           mode == OatDumpMode::kDumpOatWithRuntime;
  }

  bool ExecuteWithoutRuntime() override {
    LOG(INFO) << "Running oatdump without runtime";

    CHECK(args_ != nullptr);

    OatDumpMode mode = args_->GetMode();
    CHECK(mode == OatDumpMode::kSymbolize || mode == OatDumpMode::kDumpOatWithoutRuntime);
  ...
  bool ExecuteWithRuntime(Runtime* runtime) override {
    LOG(INFO) << "Running oatdump with runtime";
    ...
    if (mode == OatDumpMode::kDumpOatWithRuntime) {
