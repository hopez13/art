    ...
    if (options_.export_dex_location_) {
      if (kIsVdexEnabled) {
        ...
        if (vdex_file == nullptr) {
          os << "Could not open file " << vdex_filename << " for reading\n";
          return false;
        }
        std::unique_ptr<VdexFile> vdex = VdexFile::Open(vdex_file->Fd(),
                                                        vdex_file->GetLength(),
                                                        vdex_filename,
                                                        /* writable */ true,
                                                        /* low_4gb */ false,
        ...
        if (vdex_file->FlushClose()) {
          os << "Could not flush and close vdex file\n";
        }
      } else {
        for (size_t i = 0; i < oat_dex_files_.size(); i++) {
          const OatFile::OatDexFile* oat_dex_file = oat_dex_files_[i];
