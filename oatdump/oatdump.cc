        ...
        if (num_methods != 0u) {
          ...
          for (size_t i = 0, length = dex_cache->NumResolvedMethods(); i < length; ++i) {
            ...
            if (elem == nullptr) {
              ...
              msg = ArtMethod::PrettyMethod(reinterpret_cast<ArtMethod*>(elem));
            } else {
 ...
 private:
  ...
  static void PrintTable(ImtConflictTable* table, PointerSize pointer_size)
    ...
    for (;;) {
      ...
      table_index++;
      std::cerr << "    " << ArtMethod::PrettyMethod(ptr, true) << std::endl;
    }
  }
    ...
    if (if_table != nullptr) {
      for (size_t i = 0, num_interfaces = klass->GetIfTableCount(); i < num_interfaces; ++i) {
        ...
        for (ArtMethod& iface_method : iface->GetVirtualMethods(pointer_size)) {
          ...
          std::cerr << "    " << ArtMethod::PrettyMethod(&iface_method, true)
              << " slot=" << imt_slot
              << std::hex
    ...
    for (size_t index = 0; index < ImTable::kSize; ++index) {
      ...
      if (ptr->IsRuntimeMethod()) {
        ...
        for (;;) {
          ...
          std::string p_name = ArtMethod::PrettyMethod(ptr2, true);
        ...
        std::string p_name = ArtMethod::PrettyMethod(ptr, true);
        if (StartsWith(p_name, method.c_str())) {
              ...
              if (num_methods > 0) {
                for (ArtMethod& iface_method : iface->GetMethods(pointer_size)) {
                  if (ImTable::GetImtIndex(&iface_method) == index) {
                    std::string i_name = ArtMethod::PrettyMethod(&iface_method, true);
