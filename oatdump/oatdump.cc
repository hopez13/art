 ...
 private:
  ...
  void DumpCode(VariableIndentationOutputStream* vios,
    ...
    if (code_size == 0 || quick_code == nullptr) {
      ...
      std::unordered_map<uint32_t, StackMap> stack_maps;
      for (const StackMap& it : code_info.GetStackMaps()) {
        stack_maps.emplace(it.GetNativePcOffset(instruction_set_), it);
      }
      ...
      while (offset < code_size) {
        ...
        if (it != stack_maps.end()) {
          ScopedIndentation indent1(vios);
          it->second.Dump(vios, code_info, oat_method.GetCodeOffset(), instruction_set_);
          stack_maps.erase(it);
        }
      }
...
class ImageDumper {
 public:
  ...
  bool Dump() REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    os << "STATS:\n" << std::flush;
    size_t pointer_size = image_header_.GetPointerSize() == PointerSize::k64 ? 8 : 4;
    ...
    stats_.art_file_stats.AddBytes(image_header_.GetImageSize());
 ...
 public:
  struct Stats {
    ...
    void Dump(std::ostream& os, std::ostream& indent_os ATTRIBUTE_UNUSED)
        REQUIRES_SHARED(Locks::mutator_lock_) {
      VariableIndentationOutputStream vios(&os);
