...
NO_RETURN static void Usage(const char *fmt, ...) {
  ...
  UsageError("          kNoCompilerInformation = 0");
  UsageError("       Compiler filter");
  UsageError("          kAssumeVerified = 1");
  UsageError("          kExtract = 2");
  UsageError("          kVerify = 3");
  UsageError("          kQuicken = 4");
  UsageError("          kSpaceProfile = 5");
  UsageError("          kSpace = 6");
  UsageError("          kSpeedProfile = 7");
  UsageError("          kSpeed = 8");
  UsageError("          kEverythingProfile = 9");
  UsageError("          kEverything = 10");
...
class DexoptAnalyzer FINAL {
 public:
  ...
  // Format of status code: right most 3 bits will hold dexopt_status, and
  // following 4 bits will hold compiler filter.
  static int PrepareDexOptStatus(CompilerFilter::Filter filter, int dexopt_needed) {
  ...
  int GetDexOptStatus() {
    ...
    // Always treat elements of the bootclasspath as up-to-date.
    if (oat_file_assistant->IsInBootClassPath()) {
      dexopt_needed = kNoDexOptNeeded;
    } else {
      dexopt_needed = oat_file_assistant->GetDexOptNeeded(current_compiler_filter,
                                                          assume_profile_changed_,
                                                          false /* true */);
    }
    return PrepareDexOptStatus(current_compiler_filter, dexopt_needed);
  }
