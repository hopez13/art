...
static int ERROR_CANNOT_CREATE_RUNTIME = 100;
 ...
 public:
  ...
  void ParseArgs(int argc, char **argv) {
    ...
    if (image_.empty()) {
      ...
      if (image_.empty()) {
        LOG(ERROR) << error_msg;
        Usage("--image unspecified and ANDROID_ROOT not set or image file does don exists.");
      }
    }
  }
  ...
  bool CreateRuntime() {
    ...
    // Make sure we don't attempt to relocate. The tool should get the analyze the dex files and
    // not attempt to relocate the image.
  ...
  int GetDexOptNeeded() {
    // If the file does not exists there;s nothing to do.
    // This is a fast path to avoid creating the runtime.
