...
TEST_F(OdrCompilationLogTest, LogSizeAndPeek) {
  OdrCompilationLog ocl(/*compilation_log_path=*/nullptr);
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    if (i < OdrCompilationLog::kMaxLoggedEntries) {
      ASSERT_EQ(i + 1, ocl.Size());
    } else {
      ASSERT_EQ(OdrCompilationLog::kMaxLoggedEntries, ocl.Size());
    }

    for (size_t j = 0; j < ocl.Size(); ++j) {
      const OdrCompilationLogEntry* logged = ocl.Peek(j);
      ASSERT_TRUE(logged != nullptr);
      const OdrCompilationLogEntry& expected = entries[i + 1 - ocl.Size() + j];
