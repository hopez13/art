...
int ExecAndReturnCode(std::vector<std::string>& cmdline,
                      time_t timeout_secs,
                      /*out*/ bool* timed_out,
                      /*out*/ std::string* error_msg) {
  LOG(DEBUG) << "odr_dexopt cmdline: " << android::base::Join(cmdline, ' ') << " [timeout " << timeout_secs << "s]";
  ExecUtils exec_utils;
...
class OdrDexoptLocal final : public OdrDexopt::Delegate {
 public:
  static OdrDexoptLocal* Create(const std::string& dex2oat_path) {
    return new OdrDexoptLocal(dex2oat_path);
  }
  ...
  int DexoptBcpExtension(const DexoptBcpExtArgs& args,
    ...
    return ExecAndReturnCode(cmdline, timeout_secs, timed_out, error_msg);
  }
 ...
 private:
  explicit OdrDexoptLocal(const std::string& dex2oat_path) : dex2oat_path_(dex2oat_path) {}
...
class OdrDexoptCompilationOS final : public OdrDexopt::Delegate {
 public:
  static OdrDexoptCompilationOS* Create(int cid) {
    return new OdrDexoptCompilationOS(cid);
  }
  ...
  int DexoptSystemServer(const DexoptSystemServerArgs& args,
    ...
    LOG(DEBUG) << "DexoptSystemServer cmdline: " << android::base::Join(cmdline, ' ') << " [timeout " << timeout_secs << "s]";
    ExecUtils exec_utils;
    return exec_utils.ExecAndReturnCode(cmdline, timeout_secs, timed_out, error_msg);
  }

 private:
  explicit OdrDexoptCompilationOS(int cid) : cid_(cid) {}
...
std::unique_ptr<OdrDexopt> OdrDexopt::Create(const OdrConfig& config) {
  ...
  if (config.UseCompilationOs()) {
    ...
    self->delegate_.reset(OdrDexoptCompilationOS::Create(cid));
  } else {
    self->delegate_.reset(OdrDexoptLocal::Create(config.GetDex2Oat()));
  }
  return self;
}
