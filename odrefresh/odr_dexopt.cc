...
// static
Result<std::unique_ptr<OdrDexopt>> OdrDexopt::Create(const OdrConfig& config,
                                                     ExecUtils* exec_utils) {
  if (config.UseCompilationOs()) {
    int cid;
    if (!android::base::ParseInt(config.GetCompilationOsAddress(), &cid)) {
      return Errorf("Failed to parse CID: %s", config.GetCompilationOsAddress().c_str());
    }
    return std::unique_ptr<OdrDexopt>(OdrDexoptCompilationOS::Create(cid, exec_utils));
  } else {
    return std::unique_ptr<OdrDexopt>(OdrDexoptLocal::Create(config.GetDex2Oat(), exec_utils));
  }
}
