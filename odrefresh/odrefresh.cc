...
class OnDeviceRefresh final {
 ...
 public:
  explicit OnDeviceRefresh(const OdrConfig& config) : config_(config), start_time_(time(nullptr)) {
    ...
    // odrefresh.h defines kOdrefreshArtifactDirectory for external callers of odrefresh. Check
    // this value aligns with the paths that odrefresh will use.
    CHECK(StartsWith(GetApexDataBootImage("/system/framework/canary.jar"),
                     kOdrefreshArtifactDirectory));
    CHECK(StartsWith(GetApexDataImage("/system/framework/canary.jar"),
                     kOdrefreshArtifactDirectory));
