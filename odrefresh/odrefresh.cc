...
class OnDeviceRefresh final {
 ...
 public:
  explicit OnDeviceRefresh(const OdrConfig& config)
    ...
    if (OS::FileExists(metrics_filename)) {
      if (unlink(metrics_filename) != 0) {
        PLOG(ERROR) << "Failed to unlink " << QuotePath(metrics_filename);
      }
    }
  }
  ...
  void WriteCacheInfo() const {
    if (OS::FileExists(cache_info_filename_.c_str())) {
      if (unlink(cache_info_filename_.c_str()) != 0) {
        PLOG(ERROR) << "Failed to unlink " << QuotePath(cache_info_filename_);
      }
    }
  ...
  ExitCode Compile(bool force_compile) {
    ...
    const metrics::SessionData session_data{
        /*session_id=*/GetRandomNumber<int64_t>(0, std::numeric_limits<int64_t>::max()),
