 ...
 public:
  explicit OnDeviceRefresh(const OdrConfig& config)
    ...
    for (const std::string& jar : android::base::Split(config_.GetBootClasspath(), ":")) {
      boot_classpath_jars_.emplace_back(jar);
    }
  }
  ...
  // Gets the `ApexInfo` for all APEXes.
  ...
  // Checks whether all boot extension artifacts are present. Returns true if all are present, false otherwise.
  ...
  // Checks whether all system_server artifacts are present. The artifacts are checked in their order of compilation. Returns true if all are present, false otherwise.
  ...
  WARN_UNUSED bool CheckBootExtensionArtifactsAreUpToDate(
    ...
    *cleanup_required = false;
    ...
    if (art_apex_info.getIsFactory()) {
      ...
      LOG(INFO) << "Incomplete boot extension artifacts on /system. Checking cache." << error_msg;
      trigger = OdrMetrics::Trigger::kMissingArtifacts;
    } else {
      LOG(INFO) << "ART APEX changed. Checking cache.";
      trigger = OdrMetrics::Trigger::kApexVersionMismatch;
    }
  ...
  WARN_UNUSED ExitCode CheckArtifactsAreUpToDate(OdrMetrics& metrics,
                                                 /*out*/ bool* compile_boot_extensions,
                                                 /*out*/ bool* compile_system_server) {
    metrics.SetStage(OdrMetrics::Stage::kCheck);
    *compile_boot_extensions = false;
    *compile_system_server = false;
    ...
    for (const InstructionSet isa : config_.GetBootExtensionIsas()) {
      *compile_boot_extensions |= !CheckBootExtensionArtifactsAreUpToDate(
    ...
    *compile_system_server = !CheckSystemServerArtifactsAreUpToDate(
    ...
    return *compile_boot_extensions || *compile_system_server ? ExitCode::kCompilationRequired
                                                              : ExitCode::kOkay;
  }
  ...
  WARN_UNUSED ExitCode Compile(OdrMetrics& metrics,
    ...
    for (const InstructionSet isa : bcp_instruction_sets) {
      ...
      if (force_compile || (compile_boot_extensions && !BootExtensionArtifactsExist(
                                                           /*on_system=*/false, isa, &error_msg))) {
    ...
    if (force_compile ||
        (compile_system_server && !SystemServerArtifactsExist(/*on_system*/ false, &error_msg))) {
  ...
  static int main(int argc, const char** argv) {
    ...
    for (int i = 0; i < argc; ++i) {
      ...
      if (action == "--check") {
        ...
        ExitCode compile_result = odr.Compile(
            metrics, /*force_compile=*/false, compile_boot_extensions, compile_system_server);
