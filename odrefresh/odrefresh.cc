 ...
 public:
  ...
  bool CheckSystemServerArtifacts(bool on_system) const {
    ...
    for (const std::string& jar_path : systemserver_compilable_jars_) {
      ...
      const std::string& root = config_.GetAndroidRoot();
  ...
  std::string GetBootImageExtensionImage(bool system = false) const {
    ...
    if (system) {
      ...
      const std::string& root = config_.GetAndroidRoot();
      return Concatenate({root, "/framework/boot-", basename, ".art"});
    } else {
  ...
  std::string GetSystemServerImageLocation(bool on_system, const std::string& jar_path) const {
    ...
    if (on_system) {
      // Typically /system/framework/oat/<arch>/<file>.art.
      const std::string& root = config_.GetAndroidRoot();
      return Concatenate({root, "/framework/oat/", isa_str, "/", basename, ".art"});
    } else {
      // Typically /data/misc/apexdata/system/framework/oat/<arch>/<file>.art.
  ...
  bool CompileBootExtensionArtifacts(const InstructionSet isa,
    ...
    const std::string& root = config_.GetAndroidRoot();
    const std::string boot_profile_file(root + "/etc/boot-image.prof");
    ...
    const std::string dirty_image_objects_file(root + "/etc/dirty-image-objects");
    ...
    for (const auto& location_kind_pair : location_kind_pairs) {
      ...
      const std::string staging_location = StagingLocation(staging_dir, location);
  ...
  bool CompileSystemServerArtifacts(const std::string_view staging_dir,
    ...
    for (const std::string& jar : systemserver_compilable_jars_) {
      ...
      const std::string profile_file =
        Concatenate({config_.GetAndroidRoot(), "framework/", jar_name, ".prof"});
      if (OS::FileExists(profile_file.c_str())) {
