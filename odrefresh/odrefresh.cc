...
bool AddDex2OatConcurrencyArguments(/*inout*/ std::vector<std::string>& args,
  ...
  std::string prop_prefix = "dalvik.vm." + (is_compilation_os ? "background" : "boot");

  std::string threads = android::base::GetProperty(prop_prefix + "-threads", "");
  if (!threads.empty()) {
    args.push_back("-j" + threads);
  }

  std::string cpu_set = android::base::GetProperty(prop_prefix + "-cpu-set", "");
  if (cpu_set.empty()) {
    return true;
  }
