...
class OnDeviceRefresh final {
 ...
 public:
  ...
  std::string GetBootImageExtensionImagePath(const std::string& isa) const {
    ...
    oss << bcp_output_dir_ << '/' << isa << "/boot-" << basename << ".art";
    return oss.str();
  }
  ...
  bool CompileBootExtensions(const std::string& isa, std::string* error_msg) const {
    ...
    DCHECK(EndsWith(bcp_compilable_jars_[0], "/framework.jar"));
    ...
    std::vector<std::pair<const char*, const char*>> path_kind_pairs = {
  ...
  static int main(int argc, const char** argv) {
    char* context;
    if (getcon(&context) != 0 || strcmp(context, "u:r:odrefresh:s0") != 0) {
