...
static void MakeReadOnly(std::vector<std::unique_ptr<File>>& files) {
  for (auto& file : files) {
    if (fchmod(file->Fd(), S_IRUSR | S_IRGRP | S_IROTH) != 0) {
      PLOG(ERROR) << "Failed to make file read-only: " << file->GetPath();
    }
  }
}
...
class OnDeviceRefresh final {
 private:
  ...
  static constexpr const char* kFinishedContext = "u:object_r:apex_art_image_data_file:s0";
  ...
  std::string dex2oat_;
  const std::string dexoptanalyzer_;
 ...
 public:
  OnDeviceRefresh() : dexoptanalyzer_(GetDexOptAnalyzer()) {
  ...
  bool CheckSystemServerJars() const {
    ...
    for (const std::string& jar_path : systemserver_compilable_jars_) {
      ...
      args.emplace_back(dexoptanalyzer_);
  ...
  std::string GetBootImageExtensionImage(bool system = false) const {
    ...
    if (system) {
      ...
      return Concatenate({bcp_output_dir_, "/boot-", basename, ".art"});
    }
  }
  ...
  bool CompileBootExtensions(const std::string& isa, std::string* error_msg) const {
    ...
    MakeReadOnly(output_files);
    FlushCloseOrEraseFiles(output_files);

    for (const auto& path_kind_pair : path_kind_pairs) {
      auto [path, kind] = path_kind_pair;
      SetFileContext(path, kFinishedContext);
    }
