...
class OdrArtifacts {
 public:
  ...
  static OdrArtifacts ForSystemServer(const std::string& dex_location, InstructionSet isa) {
    return OdrArtifacts(dex_location, isa);
  }
 ...
 private:
  ...
  OdrArtifacts(const std::string& dex_location, InstructionSet isa)
      : image_location_(GetApexDataDalvikCacheFilename(dex_location, isa, "art")),
        oat_location_(GetApexDataDalvikCacheFilename(dex_location, isa, "odex")),
        vdex_location_(GetApexDataDalvikCacheFilename(dex_location, isa, "vdex")) {}
 ...
 public:
  ...
  bool CheckSystemServerArtifactsAreUpToDate(bool on_system) const {
    ...
    for (const std::string& jar_path : systemserver_compilable_jars_) {
      ...
      OdrArtifacts artifacts = OdrArtifacts::ForSystemServer(jar_path, config_.GetSystemServerIsa());
      if (!on_system) {
        CHECK_EQ(artifacts.OatLocation(),
                 GetApexDataOdexFilename(jar_path.c_str(), config_.GetSystemServerIsa()));
      }
  ...
  void RemoveSystemServerArtifactsFromData() const {
    if (config_.GetDryRun()) {
      LOG(INFO) << "Removal of system_server artifacts from apexdata skipped (dry-run).";
      return;
    }
    for (const std::string& jar_path : systemserver_compilable_jars_) {
      const OdrArtifacts artifacts =
          OdrArtifacts::ForSystemServer(jar_path, config_.GetSystemServerIsa());
      LOG(INFO) << "Removing system_server artifacts for " << QuotePath(jar_path);
  ...
  void RemoveBootExtensionArtifactsFromData(InstructionSet isa) const {
    if (config_.GetDryRun()) {
      LOG(INFO) << "Removal of bcp extension artifacts from apexdata skipped (dry-run).";
      return;
    }
  ...
  static bool IsCompilableBootExtension(const std::string& boot_extension_jar) {
    // GetApexDataOatFilename() checks the `boot_extension_jar` path and returns an oat file path
    // on ART APEX data directory if that jar should be compiled, e.g. checks if
    // `boot_extension_jar` is under system/framework.
    return !LocationIsOnArtModule(boot_extension_jar.c_str());
  }

  static bool IsCompilableSystemServerJar(const std::string& system_server_jar) {
    // GetApexDataOdexFile() checks the `system_server_jar` path and returns an odex file path on
    // ART APEX data directory if that jar should be compiled, ie is under system/framework.
    return !LocationIsOnApex(system_server_jar.c_str());
  }
  ...
  std::string GetBootImageExtensionImage(bool on_system) const {
    ...
    if (on_system) {
      ...
      return GetApexDataBootImage(leading_jar.c_str());
    }
  }

  std::string GetBootImageExtensionImageLocation(const InstructionSet isa) const {
    // Typically "/data/misc/apexdata/system/framework/<isa>/boot-framework.art".
  ...
  std::string GetSystemServerImageLocation(bool on_system, const std::string& jar_path) const {
    if (on_system) {
      ...
      // Typically "/data/misc/apexdata/system/framework/oat/<isa>/services.art".
  ...
  bool CompileSystemServerArtifacts(const std::string& staging_dir, std::string* error_msg) const {
    ...
    for (const std::string& jar : systemserver_compilable_jars_) {
      ...
      OdrArtifacts artifacts = OdrArtifacts::ForSystemServer(jar, isa);
      CHECK_EQ(artifacts.OatLocation(), GetApexDataOdexFilename(jar.c_str(), isa));
  ...
  static bool InitializeCommonConfig(const char* arg, OdrConfig* config) {
    if (ArgumentEquals(arg, "--dry-run")) {
      config->SetDryRun();
      return true;
    }
    return false;
