...
WARN_UNUSED bool OnDeviceRefresh::CompileBootClasspathArtifacts(
  ...
  const std::string dirty_image_objects_file(GetAndroidRoot() + "/etc/preloaded-classes");
  if (OS::FileExists(dirty_image_objects_file.c_str())) {
    std::unique_ptr<File> file(OS::OpenFileForReading(dirty_image_objects_file.c_str()));
    args.emplace_back(android::base::StringPrintf("--preloaded-classes-fd=%d", file->Fd()));
    readonly_files_raii.push_back(std::move(file));
  } else {
    LOG(WARNING) << "Missing preloaded classes file : " << QuotePath(dirty_image_objects_file);
  }
