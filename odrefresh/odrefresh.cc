...
static std::optional<apex::ApexInfo> GetArtApexInfo(const std::vector<apex::ApexInfo>& info_list) {
...
std::string OnDeviceRefresh::GetBootImagePathWithoutIsa() {
  // Typically "/apex/com.android.art/javalib/boot.art".
  return GetArtRoot() + "/javalib/boot.art";
}

std::string OnDeviceRefresh::GetBootExtensionImagePathWithoutIsa(bool on_system) const {
...
std::string OnDeviceRefresh::GetBootExtensionImagePath(bool on_system,
                                                       const InstructionSet isa) const {
  // Typically "/data/misc/apexdata/com.android.art/dalvik-cache/<isa>/boot-framework.art".
  return GetSystemImageFilename(GetBootExtensionImagePathWithoutIsa(on_system).c_str(), isa);
}
...
WARN_UNUSED bool OnDeviceRefresh::RemoveBootExtensionArtifactsFromData(InstructionSet isa) const {
  ...
  const std::string apexdata_image_location = GetBootExtensionImagePath(/*on_system=*/false, isa);
...
WARN_UNUSED bool OnDeviceRefresh::BootExtensionArtifactsExist(
  ...
  const std::string apexdata_image_location = GetBootExtensionImagePath(on_system, isa);
...
WARN_UNUSED bool OnDeviceRefresh::VerifyBootExtensionArtifactsAreUpToDate(const InstructionSet isa,
  ...
  const std::string image_location = GetBootExtensionImagePathWithoutIsa(on_system);

  std::vector<std::string> args;
  args.emplace_back(config_.GetDexOptAnalyzer());
  args.emplace_back("--validate-bcp");
  args.emplace_back(Concatenate({"--image=", GetBootImagePathWithoutIsa(), ":", image_location}));
...
WARN_UNUSED bool OnDeviceRefresh::VerifySystemServerArtifactsAreUpToDate(bool on_system) const {
  ...
  for (const std::string& jar_path : systemserver_compilable_jars_) {
    ...
    args.emplace_back(Concatenate({"--image=",
                                   GetBootImagePathWithoutIsa(),
                                   ":",
                                   GetBootExtensionImagePathWithoutIsa(on_system)}));
    args.emplace_back(
...
WARN_UNUSED bool OnDeviceRefresh::CompileBootExtensionArtifacts(const InstructionSet isa,
  ...
  args.emplace_back("--boot-image=" + GetBootImagePathWithoutIsa());
  ...
  const std::string image_location = GetBootExtensionImagePath(/*on_system=*/false, isa);
...
WARN_UNUSED bool OnDeviceRefresh::CompileSystemServerArtifacts(const std::string& staging_dir,
  ...
  for (const std::string& jar : systemserver_compilable_jars_) {
    ...
    const std::string extension_image = GetBootExtensionImagePathWithoutIsa(/*on_system=*/false);
    args.emplace_back(
        Concatenate({"--boot-image=", GetBootImagePathWithoutIsa(), ":", extension_image}));
...
int OnDeviceRefresh::main(int argc, const char** argv) {
