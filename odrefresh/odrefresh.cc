...
class OnDeviceRefresh final {
 ...
 public:
  ...
  bool CheckSystemServerJars() const {
    const std::string output_isa_dir = systemserver_output_dir_ + '/' + systemserver_arch_;

    int missing_files = 0;
    for (const std::string& jar_path : systemserver_compilable_jars_) {
      static constexpr const char* extensions[] = {".art", ".odex", ".vdex"};
      for (auto extension : extensions) {
        std::string basename(Basename(jar_path, ".jar"));
        std::string path = Concatenate({ output_isa_dir, "/", basename, extension });
    ...
    for (const std::string& jar_path : systemserver_compilable_jars_) {
      ...
      const std::string paths[3] = {
        Concatenate({output_isa_dir, "/", basename, ".odex"}),
        Concatenate({output_isa_dir, "/", basename, ".vdex"}),
        jar_path
      };
      ...
      std::string profile_file = Concatenate({"/system/framework/", basename, ".jar.prof"});
  ...
  bool CheckBootExtensions(const std::string& output_dir, const std::string& isa) const {
    const std::string output_isa_dir = output_dir + '/' + isa;

    int missing_files = 0;
    for (const std::string& jar_path : bcp_compilable_jars_) {
      static constexpr const char* extensions[] = {".art", ".oat", ".vdex"};
      std::string basename(Basename(jar_path, ".jar"));
      for (auto extension : extensions) {
        std::string path = Concatenate({output_isa_dir, "/boot-", basename, extension});
        if (!OS::FileExists(path.c_str(), true)) {
          LOG(INFO) << "Missing " << path;
          missing_files++;
        }
  ...
  int CheckArtifacts() {
    ...
    for (const std::string& bcp_arch : bcp_archs_) {
      if (!CheckBootExtensions(bcp_output_dir, bcp_arch)) {
        return -1;  // TODO(oth)
      }
    }
  ...
  bool CompileBootExtensions(const std::string& isa, std::string* error_msg) const {
    const std::string output_isa_dir = bcp_output_dir_ + "/" + isa;
    ...
    for (const std::string& component : bcp_compilable_jars_) {
      // Boot class path extension artifacts are prefixed with "boot-".
      std::string jar_name(Basename(component));
      std::string image_path = output_isa_dir + "/boot-" + ReplaceFileExtension(jar_name, "art");
      std::string oat_path = ReplaceFileExtension(image_path, "oat");
      std::string vdex_path = ReplaceFileExtension(image_path, "vdex");

      auto add_location_arguments =
      [&args, &output_files](const std::string& location, const char* arg_prefix) -> bool {
        const char* const output_path = location.c_str();
        File* output_file = OS::CreateEmptyFile(output_path);
        if (output_file == nullptr) {
          PLOG(ERROR) << "Failed to create " << location;
          return false;
        }
        output_files.emplace_back(output_file);
        args.emplace_back(android::base::StringPrintf("--%s-location=%s", arg_prefix, output_path));
        args.emplace_back(android::base::StringPrintf("--%s-fd=%d", arg_prefix, output_file->Fd()));
        return true;
      };

      if (!add_location_arguments(image_path, "image") ||
          !add_location_arguments(oat_path, "oat") ||
          !add_location_arguments(vdex_path, "output-vdex")) {
  ...
  bool CompileSystemServerJars(std::string* error_msg) const {
    const std::string output_isa_dir =
      Concatenate({systemserver_output_dir_, "/", systemserver_arch_});
    EnsureDirectoryExists(output_isa_dir);
    ...
    for (const std::string& jar : systemserver_compilable_jars_) {
      ...
      const std::string stem(Basename(jar, ".jar"));
      const std::string app_image_path = Concatenate({output_isa_dir, "/", stem, ".art"});
      const std::string oat_file_path = ReplaceFileExtension(app_image_path, "odex");
      const std::string vdex_file_path = ReplaceFileExtension(app_image_path, "vdex");

      std::vector<std::unique_ptr<File>> output_files;

      const std::string output_paths[3] = { app_image_path, oat_file_path, vdex_file_path };
      static const char* kFdArgument[3] = { "--app-image-fd", "--oat-fd", "--output-vdex-fd" };

      for (size_t i = 0; i < std::size(output_paths); ++i) {
        File* output_file = OS::CreateEmptyFile(output_paths[i].c_str());
        if (output_file == nullptr) {
          PLOG(ERROR) << "Failed to open \"" << output_paths[i] << "\"";
          EraseFiles(output_files);
          return false;
        }
        output_files.emplace_back(output_file);
        args.emplace_back(
          android::base::StringPrintf("%s=%d", kFdArgument[i], output_files[i]->Fd()));
      }
      args.emplace_back("--oat-location=" + oat_file_path);

      const std::string profile_file = "/system/framework/" + stem + ".jar.prof";
      if (OS::FileExists(profile_file.c_str())) {
