...
class OdrConfig final {
 public:
  ...
  std::string GetDexOptAnalyzer() const { return art_bin_dir_ + "/dexoptanalyzer"; }
...
class OnDeviceRefresh final {
 ...
 public:
  explicit OnDeviceRefresh(const OdrConfig& config) : config_(config), start_time_(time(nullptr)) {
    ...
    VisitTokens(config_.GetDex2oatBootClasspath(), ':', [this](std::string_view component) {
    ...
    VisitTokens(config_.GetSystemServerClasspath(), ':', [this](std::string_view component) {
  ...
  static void AddDex2OatCommonOptions(/*out*/ std::vector<std::string>* args) {
  ...
  static void AddDex2OatConcurrencyArguments(/*out*/ std::vector<std::string>* args) {
  ...
  static void AddDex2OatDebugInfo(/*out*/ std::vector<std::string>* args) {
    args->emplace_back("--generate-debug-info");
    args->emplace_back("--generate-mini-debug-info");
    args->emplace_back("--strip");
  }

  static void AddDex2OatInstructionSet(InstructionSet isa, /*out*/ std::vector<std::string>* args) {
    const char* isa_str = GetInstructionSetString(isa);
    args->emplace_back(Concatenate({"--instruction-set=", isa_str}));
  }

  static void AddDex2OatProfileAndCompilerFilter(const std::string& profile_file,
  ...
  // The artifacts on /data are removed if the system server artifacts on /system are valid or
  // the artifacts on /data are determined to be not valid.
  ...
  bool CheckBootExtensionArtifactsAreUpToDate(const InstructionSet isa, bool on_system) const {
    ...
    if (rc == dexoptanalyzer::ReturnCode::kNoDexOptNeeded) {
      // Boot extension artifacts look good, clean the equivalents on /data.
  ...
  void RemoveBootExtensionArtifactsFromData() const {
    const std::string apexdata_image_location = GetBootImageExtensionImage(/*on_system=*/false);
  ...
  bool CheckBootExtensionArtifactsAreUpToDate(const InstructionSet isa) const {
    if (CheckBootExtensionArtifactsAreUpToDate(isa, /*on_system=*/true)) {
      ...
      RemoveBootExtensionArtifactsFromData();
      return true;
    }
  ...
  static bool IsCompilableBootExtension(std::string_view jar_path) {
  ...
  static bool IsCompilableSystemServerJar(std::string_view jar_path) {
  ...
  std::string GetStagingLocation(std::string_view staging_dir, std::string_view path) const {
  ...
  bool CompileBootExtensionArtifacts(const InstructionSet isa,
    ...
    AddDex2OatInstructionSet(isa, &args);
    const std::string boot_profile_file(GetAndroidRoot() + "/etc/boot-image.prof");
    AddDex2OatProfileAndCompilerFilter(boot_profile_file, &args);
    ...
    const std::string install_location{Dirname(image_location)};
  ...
  bool CompileSystemServerArtifacts(std::string_view staging_dir, std::string* error_msg) const {
    std::vector<std::string> classloader_context;
    ...
    for (const std::string& jar : systemserver_compilable_jars_) {
      ...
      AddDex2OatCommonOptions(&args);
      AddDex2OatDebugInfo(&args);
      AddDex2OatInstructionSet(config_.GetSystemServerIsa(), &args);
      const std::string jar_name(Basename(jar));
      const std::string profile = Concatenate({GetAndroidRoot(), "/framework/", jar_name, ".prof"});
      AddDex2OatProfileAndCompilerFilter(profile, &args);
