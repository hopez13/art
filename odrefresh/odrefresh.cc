...
class OnDeviceRefresh final {
 ...
 public:
  ...
  bool CheckSystemServerJarsOnData() const {
    int missing_files = 0;
  ...
  bool CheckBootExtensions(const InstructionSet isa, bool is_system_apex) const {
    const std::string dex_file = boot_extension_compilable_jars_.front();
    const std::string image_location = GetBootImageExtensionImage(is_system_apex);
  ...
  int CheckArtifacts(bool is_system_apex) {
    for (const InstructionSet isa : config_.GetBootExtensionIsas()) {
      bool okay = false;
      if (is_system_apex) {
        okay = CheckBootExtensions(isa, /*is_system_apex=*/ true);
        // TODO: if okay, ensure no artifacts on /data.
      }
      if (!okay && !CheckBootExtensions(isa, /*is_system_apex=*/ false)) {
    ...
    if (is_system_apex && CheckSystemServerOnSystem()) {
      LOG(INFO) << "System server on /system status: OK";
      // TODO: if okay, ensure no system server artifacts on /data.
      return ExitCode::kOkay;
    }
    if (CheckSystemServerJarsOnData()) {
  ...
  static int main(int argc, const char** argv) {
    ...
    for (int i = 0; i < argc; ++i) {
      ...
      if (action == "--check") {
        ...
        ReportSpace();
        if (is_system_apex) {
          // TODO: check artifacts and compile if necessary.
          return ExitCode::kOkay;
        }
        return odr.Compile();
