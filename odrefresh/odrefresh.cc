...
bool OnDeviceRefresh::CheckSystemServerArtifactsAreUpToDate(
  ...
  // Note: If an APEX used to be compilable, but now isn't, then ths will be detected by CheckComponents.
  std::unordered_set<std::string_view> relevant_apexes;
  relevant_apexes.reserve(apex_info_list.size());
  for (auto jar_list : { &boot_extension_compilable_jars_, &all_systemserver_jars_, &boot_classpath_jars_}) {
