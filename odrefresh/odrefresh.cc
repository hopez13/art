 ...
 public:
  explicit OnDeviceRefresh(const OdrConfig& config) : config_(config), start_time_(time(nullptr)) {
    ...
    for (const std::string& jar : android::base::Split(config_.GetDex2oatBootClasspath(), ":")) {
      if (IsCompilableBootExtension(jar, config.GetBootExtensionIsas().front())) {
    ...
    for (const std::string& jar : android::base::Split(config_.GetSystemServerClasspath(), ":")) {
      if (IsCompilableSystemServerJar(jar, config.GetSystemServerIsa())) {
  ...
  static bool IsCompilableBootExtension(const std::string& boot_extension_jar, InstructionSet isa) {
    ...
    return !GetApexDataOatFilename(boot_extension_jar.c_str(), isa).empty();
  }
  ...
  static bool IsCompilableSystemServerJar(const std::string& system_server_jar,
    ...
    return !GetApexDataOdexFilename(system_server_jar.c_str(), isa).empty();
  }
  ...
  bool CompileSystemServerArtifacts(const std::string& staging_dir, std::string* error_msg) const {
    ...
    for (const std::string& jar : systemserver_compilable_jars_) {
      ...
      OdrArtifacts artifacts = OdrArtifacts::ForSystemServer(image_location);
      CHECK_EQ(artifacts.OatLocation(),
               GetApexDataOdexFilename(jar.c_str(), config_.GetSystemServerIsa()));
