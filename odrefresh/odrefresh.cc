 ...
 public:
    ...
    for (const std::string& arch : boot_extension_archs_) {
      for (const std::string& component : boot_extension_compilable_jars_) {
        ...
        for (const auto& extension : {".art", ".oat", ".vdex"}) {
          std::string path = Concatenate({"/system/framework/", arch, "/boot-", name, extension});
          if (!OS::FileExists(path.c_str(), true)) {
            LOG(INFO) << "Missing " << path;
            return false;
          }
  ...
  bool CheckSystemServerOnSystem() const {
    for (const std::string& component : systemserver_compilable_jars_) {
      ...
      for (const auto& extension : {".odex", ".vdex"}) {
        std::string path =
          Concatenate({"/system/framework/oat/", systemserver_arch_, "/", name, extension});
        if (!OS::FileExists(path.c_str(), true)) {
          LOG(INFO) << "Missing " << path;
          return false;
        }
  ...
  void RemoveStagingFilesOrDie(const char* staging_dir) const {
    if (OS::DirectoryExists(staging_dir)) {
      nftw(staging_dir, RemoveArtifact, /*__max_fd_count=*/ 4, FTW_DEPTH | FTW_MOUNT);
    }
  }
  ...
  bool CompileBootExtensions(const std::string& isa,
  ...
  int Compile() const {
    ...
    for (const std::string& arch : boot_extension_archs_) {
      if (!CompileBootExtensions(arch, staging_dir, &error_msg)) {
