...
OnDeviceRefresh::RunDex2oatForBootClasspath(const std::string& staging_dir,
  ...
  if (input_boot_images.empty()) {
    ...
    std::string framework_dirty_image_objects_file(GetAndroidRoot() + "/etc/dirty-image-objects");
    std::string art_dirty_image_objects_file(GetArtRoot() + "/etc/dirty-image-objects");
    for (const std::string& dirty_image_objects_file :
         {framework_dirty_image_objects_file, art_dirty_image_objects_file}) {
