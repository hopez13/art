...
WARN_UNUSED bool OnDeviceRefresh::CheckSystemPropertiesAreDefault() const {
  // Currently, we don't check properties that match `kCheckedSystemPropertyPrefixes` for simplicity
  // because none of them is persistent.
...
WARN_UNUSED bool OnDeviceRefresh::CheckSystemPropertiesHaveNotChanged(
  ...
  for (const SystemPropertyConfig& system_property_config : *kSystemProperties.get()) {
    checked_properties.insert(system_property_config.name);
  }
  for (const auto& [key, value] : cached_system_properties) {
