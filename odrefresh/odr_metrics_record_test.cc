...
class OdrMetricsRecordTest : public CommonArtTest {
 protected:
  void WriteFile() {
    ...
    for (const std::string& line : file_contents_) {
      ofs << line;
    }
    ofs.close();
  }

  ScratchDir dir_{/*keep_files=*/false};
  std::string file_path_ = dir_.GetPath() + "/metrics-record.xml";

  std::vector<std::string> file_contents_ = {
      "<odrefresh_metrics>",
      android::base::StringPrintf("<odrefresh_metrics_version>%d</odrefresh_metrics_version>",
  ...
  static constexpr int kMetricsVersionIndex_ = 1;
  static constexpr int kStatusIndex_ = 5;
  static constexpr int kPrimaryBcpDex2oatResultIndex_ = 11;
  static constexpr int kSecondaryBcpDex2oatResultIndex_ = 12;
  static constexpr int kSystemServerDex2oatResultIndex_ = 13;
};
...
TEST_F(OdrMetricsRecordTest, ExpectedElementNotFound) {
  file_contents_[kMetricsVersionIndex_] = "<not_valid_metric>25</not_valid_metric>";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, ExpectedAttributeNotFound) {
  // Missing "status".
  file_contents_[kPrimaryBcpDex2oatResultIndex_] =
      R"(<primary_bcp_dex2oat_result exit-code="17" signal="18" />)";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, UnexpectedOdrefreshMetricsVersion) {
  file_contents_[kMetricsVersionIndex_] =
      "<odrefresh_metrics_version>0</odrefresh_metrics_version>";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, UnexpectedType) {
  file_contents_[kStatusIndex_] = "<status>abcd</status>";  // It should be an int32.
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, ResultExitCodeOutsideOfRange) {
  ...
  file_contents_[kSecondaryBcpDex2oatResultIndex_] =
...
TEST_F(OdrMetricsRecordTest, ResultSignalOutsideOfRange) {
  ...
  file_contents_[kSystemServerDex2oatResultIndex_] =
