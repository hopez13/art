...
class OdrMetricsRecordTest : public CommonArtTest {
 ...
 protected:
  ScratchDir dir = ScratchDir(/*keep_files=*/false);
  std::string file_path = dir.GetPath() + "/metrics-record.xml";;
  std::vector<std::string> file_contents = {
      "<odrefresh_metrics>",
      android::base::StringPrintf(
          "<odrefresh_metrics_version>%d</odrefresh_metrics_version>",
          kOdrefreshMetricsVersion),
      "<art_apex_version>81966764218039518</art_apex_version>",
      "<trigger>16909060</trigger>",
      "<stage_reached>286397204</stage_reached>",
      "<status>30</status>",
      "<cache_space_free_start_mib>1633837924</cache_space_free_start_mib>",
      "<cache_space_free_end_mib>1903326068</cache_space_free_end_mib>",
      "<primary_bcp_compilation_millis>825373492</primary_bcp_compilation_millis>",
      "<secondary_bcp_compilation_millis>1094861636</secondary_bcp_compilation_millis>",
      "<system_server_compilation_millis>1364349780</system_server_compilation_millis>",
...
TEST_F(OdrMetricsRecordTest, ExpectedElementNotFound) {
  file_contents[1] = "<not_valid_metric>25</not_valid_metric>";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, ExpectedAttributeNotFound) {
  // Missing "status".
  file_contents[11] = R"(<primary_bcp_dex2oat_result exit-code="17" signal="18" />)";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, UnexpectedOdrefreshMetricsVersion) {
  file_contents[1] = "<odrefresh_metrics_version>0</odrefresh_metrics_version>";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, UnexpectedType) {
  file_contents[5] = "<status>abcd</status>";  // It should be an int32.
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, ResultStatusOutsideOfRange) {
  ...
  file_contents[11] = R"(<primary_bcp_dex2oat_result status="-1" exit-code="-1" signal="0" />)";
  WriteFile();
  ...
  ASSERT_THAT(
      record.ReadFromFile(file_path),
      HasError(WithMessage(
          "Odrefresh metric primary_bcp_dex2oat_result.status has a value (-1) "
          "outside of the expected range ([0, 5])")));

  file_contents[11] = R"(<primary_bcp_dex2oat_result status="9" exit-code="-1" signal="0" />)";
  WriteFile();

  ASSERT_THAT(
      record.ReadFromFile(file_path),
      HasError(WithMessage(
          "Odrefresh metric primary_bcp_dex2oat_result.status has a value (9) "
          "outside of the expected range ([0, 5])")));
}

TEST_F(OdrMetricsRecordTest, ResultExitCodeOutsideOfRange) {
  // Exit Code is valid between -1 and 255
  file_contents[12] =  R"(<secondary_bcp_dex2oat_result status="2" exit-code="-2" signal="0" />)";
  WriteFile();
  ...
  file_contents[12] = R"(<secondary_bcp_dex2oat_result status="2" exit-code="258" signal="0" />)";
  WriteFile();
...
TEST_F(OdrMetricsRecordTest, ResultSignalOutsideOfRange) {
  ...
  file_contents[13] = R"(<system_server_dex2oat_result status="3" exit-code="0" signal="-6" />)";
  WriteFile();
  ...
  ASSERT_THAT(
      ...
      HasError(WithMessage(android::base::StringPrintf(
          ...
          "outside of the expected range ([0, %d])", SIGRTMAX))));

  file_contents[13] = R"(<system_server_dex2oat_result status="3" exit-code="0" signal="65" />)";
  WriteFile();
