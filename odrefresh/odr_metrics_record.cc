...
android::base::Result<int32_t> ReadInt32Attribute(
  ...
  if (result != tinyxml2::XML_SUCCESS) {
    ...
    return Errorf("Odrefresh metric {}.{} has an invalid value",
                  element_name, attribute_name);
  } else {
    return value;
  }
...
android::base::Result<OdrMetricsRecord::Dex2OatExecResult> ReadExecResult(
  ...
  return OdrMetricsRecord::Dex2OatExecResult(
      OR_RETURN(ReadInt32Attribute(element, nodeName, "status", 0, 5)),
      OR_RETURN(ReadInt32Attribute(element, nodeName, "exit-code", 0, 255)),
      OR_RETURN(ReadInt32Attribute(element, nodeName, "signal", -1, 31)));
}
