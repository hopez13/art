...
template<typename T>
void AddMetric(tinyxml2::XMLElement* parent,
               const char* name,
               const T& value) {
...
android::base::Result<void> OdrMetricsRecord::ReadFromFile(const std::string& filename) {
  ...
  odrefresh_metrics_version = OR_RETURN(
      ReadInt32(metrics, "odrefresh_metrics_version"));
  ...
  cache_space_free_start_mib = OR_RETURN(
      ReadInt32(metrics, "cache_space_free_start_mib"));
  cache_space_free_end_mib = OR_RETURN(
      ReadInt32(metrics, "cache_space_free_end_mib"));
  return {};
}
