...
TEST_F(OdrMetricsTest, PrimaryBcpResultWithValue) {
  ...
  OdrMetricsRecord record = metrics.ToRecord();
  EXPECT_EQ(record.primary_bcp_dex2oat_result_status, ExecResult::Status::kExited);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_exit_code, 0);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_signal, 0);
}

TEST_F(OdrMetricsTest, PrimaryBcpResultWithoutValue) {
  OdrMetrics metrics(GetCacheDirectory(), GetMetricsFilePath());
  // primary_bcp_dex2oat_result_ is std::nullopt
  OdrMetricsRecord record = metrics.ToRecord();
  EXPECT_EQ(record.primary_bcp_dex2oat_result_status, -1);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_exit_code, -1);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_signal, 0);
}
...
TEST_F(OdrMetricsTest, SecondaryBcpResultWithValue) {
  ...
  EXPECT_EQ(record.primary_bcp_dex2oat_result_exit_code, 0);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_signal, 0);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_status, ExecResult::Status::kTimedOut);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_exit_code, 3);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_signal, 0);
}
...
TEST_F(OdrMetricsTest, SecondaryBcpResultWithoutValue) {
  ...
  // secondary_bcp_dex2oat_result_ is std::nullopt
  OdrMetricsRecord record = metrics.ToRecord();
  EXPECT_EQ(record.primary_bcp_dex2oat_result_status, ExecResult::Status::kExited);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_exit_code, 0);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_signal, 0);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_status, -1);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_exit_code, -1);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_signal, 0);
}
...
TEST_F(OdrMetricsTest, SystemServerResultWithValue) {
  ...
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_exit_code, 3);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_signal, 0);
  EXPECT_EQ(record.system_server_dex2oat_result_status, ExecResult::Status::kSignaled);
  EXPECT_EQ(record.system_server_dex2oat_result_exit_code, 2);
  EXPECT_EQ(record.system_server_dex2oat_result_signal, 9);
}
...
TEST_F(OdrMetricsTest, SystemServerResultWithoutValue) {
  ...
  // system_server_dex2oat_result_ is std::nullopt
  OdrMetricsRecord record = metrics.ToRecord();
  EXPECT_EQ(record.primary_bcp_dex2oat_result_status, ExecResult::Status::kExited);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_exit_code, 0);
  EXPECT_EQ(record.primary_bcp_dex2oat_result_signal, 0);
  ...
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_exit_code, 3);
  EXPECT_EQ(record.secondary_bcp_dex2oat_result_signal, 0);
  EXPECT_EQ(record.system_server_dex2oat_result_status, -1);
  EXPECT_EQ(record.system_server_dex2oat_result_exit_code, -1);
  EXPECT_EQ(record.system_server_dex2oat_result_signal, 0);
}
