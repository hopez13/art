...
class OnDeviceRefresh final {
 ...
 private:
  ...
  // Returns the symbolic boot image location (without ISA). If `minimal` is true, return the
  // symbolic location of the minimal boot image.
  std::string GetBootImage(bool on_system, bool minimal) const;

  // Returns the real boot image location (with ISA).  If `minimal` is true, return the
  // symbolic location of the minimal boot image.
