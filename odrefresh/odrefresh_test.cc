...
TEST_F(OdRefreshTest, BootClasspathJars) {
  EXPECT_CALL(*mock_exec_utils_,
              DoExecAndReturnCode(AllOf(
                  ...
                  AllOf(Contains(FlagContains("--dex-fd=", FdOf(core_oj_jar_))),
                        ...
                        Contains(Concatenate({"--oat-location=", dalvik_cache_dir_,
                            "/x86_64/boot.oat"}))))))
      .WillOnce(Return(0));
...
TEST_F(OdRefreshTest, CompileSetsCompilerFilter_Speed) {
  // Test setup: use "speed" compiler filter.
  config_.SetSystemServerCompilerFilter("speed");

  // FIXME Uninteresting calls (compile BCP extension). The code checks --class-loader-context just as
  // one way to distintuish system server compilation.
  EXPECT_CALL(
      *mock_exec_utils_, DoExecAndReturnCode(Contains(HasSubstr("--app-image-fd="))))
...
TEST_F(OdRefreshTest, CompileSetsCompilerFilter_SpeedProfile) {
...
TEST_F(OdRefreshTest, CompileSetsCompilerFilter_Verify) {
  // Test setup: use "speed" compiler filter.
