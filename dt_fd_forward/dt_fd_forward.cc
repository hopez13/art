...
// Helper that put's line-number in error message.
#define DT_IO_ERROR(f) \
    SetLastError(::android::base::StringPrintf("%s:%d %s", __FILE__, __LINE__, f))
...
IOResult FdForwardTransport::ReadFully(void* data, size_t ndata) {
  ...
  while (nbytes < ndata) {
    ...
    if (written == 0) {
      ...
      // Don't bother to poll if the read_fd_ is invalid.
      if (read_fd_ < 0) {
        continue;
      }
      poll(pollfds, 2, -1);
...
jdwpTransportError FdForwardTransport::Close() {
  ...
  if (res == OK) {
    ...
    // Send a second wakeup in case we raced in the poll for ReadFully and were super unlucky with
    // an open creating a new file in the same fd as read_fd_.
    data = 1;
    TEMP_FAILURE_RETRY(write(wakeup_fd_, &data, sizeof(data)));
  }
  return res;
...
class JdwpTransportFunctions {
 public:
  static jdwpTransportError GetCapabilities(jdwpTransportEnv* env ATTRIBUTE_UNUSED,
    ...
    // TODO We should maybe support these options.
...
JNIEXPORT jint JNICALL jdwpTransport_OnLoad(JavaVM* vm ATTRIBUTE_UNUSED,
  ...
  FdForwardTransport* transport =
      new (cb->alloc(sizeof(FdForwardTransport))) FdForwardTransport(cb);
